...
> eCL Core가 작업을 조직하는 방식의 핵심 멘탈 모델.

---

## 루프란 무엇인가

eCL Core는 모든 개발 작업을 반복되는 사이클로 구조화한다.
...
시각적 컴포넌트가 있는 단계의 경우, 논의와 계획 사이에 선택적인 `/ecl-ui-phase` 단계가 있다. 코드 작성 전에 레이아웃, 인터랙션, 시각적 동작을 설명하는 디자인 계약인 `UI-SPEC.md`를 생성한다. UI가 복잡하여 디자인의 모호함이 다양한 구현 선택을 만들어낼 만큼 복잡할 때 이 단계를 실행할 가치가 있다. 명확한 디자인 계약은 재구현보다 훨씬 비용이 저렴하다.
...
구체적으로: "HMAC-SHA256 서명 검증 미들웨어 추가"는 좋은 단계 범위이다. "인증 시스템 구축"은 일반적으로 아니다 — 거의 항상 별도의 단계로 더 잘 처리될 여러 독립적인 관심사들을 포함한다. "README의 오타 수정"은 루프가 가치를 추가하는 임계값 아래이다; 대신 `/ecl-quick`을 사용하라.
...
루프가 정당화되는 임계값 아래의 작업을 위해 eCL Core는 더 가벼운 기본 도구들을 제공한다. 단계 루프는 하나의 도구이지, 유일한 도구가 아니다.
