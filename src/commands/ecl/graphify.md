...
**CJS-only (graphify):** `graphify` subcommands are not registered on `ecl-tools query`. Use the `ecl_run` launcher shim (defined in each bash block below) or invoke the binary directly: `node <runtime-home>/evolv-coder-lite/bin/ecl-tools.cjs graphify …` where `<runtime-home>` is your runtime's config directory (e.g. `~/.claude`, `~/.hermes`, `~/.cursor`). See `docs/CLI-TOOLS.md` for details. Other tooling may still use `ecl-tools query` where a handler exists.
...
Knowledge graph is disabled. To activate:
  ...
  node <runtime-home>/evolv-coder-lite/bin/ecl-tools.cjs config-set graphify.enabled true
...
graphify update . \
  ...
  && ecl_run graphify build snapshot \
  && ecl_run graphify status
```
