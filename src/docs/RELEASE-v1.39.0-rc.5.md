...
- Strips legacy `[agents]` (single-bracket) and `[[agents]]` (sequence) blocks
  ...
  whether a eCL marker is present.
- Emits the eCL-managed hook in the shape the user's config already uses:
...
```bash
npm install -g @evolvconsulting/evolv-coder-lite@1.39.0-rc.5
```
