// allow-test-rule: source-text-is-the-product — agents/ecl-intel-updater.md IS
 ...
 *
 * The "Runtime layout detection" block in ecl-intel-updater.md ran
 * unconditionally on every project analysed, emitting:
 *
 *   Layout detection returned "unknown" — this project is not a eCL-system
 *   installation (no `.claude/evolv-coder-lite/` or `.kilo/` runtime root).
 *
 * for every ordinary (non-eCL-framework) user project. The verdict was already
 * ignored by Steps 2-6 on non-eCL projects. The block was dead-but-noisy.
 *
 * Fix: gate the runtime bash detection on a positive "is-this-the-framework-
 * repo" check (package.json name === "@evolvconsulting/evolv-coder-lite") so it runs ONLY when
 * analysing the eCL framework's own repo, OR remove the block entirely if no
...
const AGENT_PATH = path.join(ROOT, 'agents', 'ecl-intel-updater.md');
...
describe('bug #3290 — Group A: layout-detection block must be gated or absent', () => {
  ...
  test('agent file exists', () => {
    assert.ok(fs.existsSync(AGENT_PATH), 'agents/ecl-intel-updater.md must exist');
  ...
  test(
    ...
    () => {
      ...
      // that the current project IS the eCL framework's own repo.
      const hasFrameworkGate =
        content.includes('@evolvconsulting/evolv-coder-lite') ||
        content.includes('is-this-the-framework') ||
        content.includes('framework repo') ||
        content.includes('Only run') ||
        /if.*package\.json.*evolv-coder-lite/i.test(content) ||
        /Only.*layout detection.*eCL framework/i.test(content) ||
        /Only.*layout detection.*framework/i.test(content);

      assert.ok(
        hasFrameworkGate,
        'agents/ecl-intel-updater.md contains a bare unconditional layout-detection ' +
...
describe('bug #3290 — Group B: layout-detection verdict has no downstream consumers', () => {
   ...
   * of the ecl-intel-updater layout detection (not general runtime references).
   *
  ...
  test('no file contains "Layout detection returned" (the noisy verdict phrase)', () => {
    ...
    assert.strictEqual(
      ...
      'phrase from the ecl-intel-updater layout-detection block). Found:\n' +
  ...
  test('no agent or workflow instructs reading the layout-detection verdict output', () => {
    ...
    // the producing file (ecl-intel-updater.md).
    ...
    for (const dir of SOURCE_DIRS) {
      ...
      for (const file of files) {
        ...
        if (path.basename(file) === 'ecl-intel-updater.md') continue;
        const src = fs.readFileSync(file, 'utf-8');
