...
```bash
ecl-sdk query commit "docs: initialize [project-name] ([N] phases)" --files .planning/
```
...
```bash
ecl-sdk query commit "docs({phase}-{plan}): complete [plan-name] plan" --files .planning/phases/XX-name/{phase}-{plan}-PLAN.md .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/STATE.md .planning/ROADMAP.md
```
...
```bash
ecl-sdk query commit "wip: [phase-name] paused at task [X]/[Y]" --files .planning/
```
...
```bash
ecl-sdk query commit-to-subrepo "feat(02-01): add user API" \
