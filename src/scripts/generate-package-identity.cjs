...
/**
 * Single source for eCL's published-package coordinates (issue #498).
 *
 ...
 * `evolv-coder-lite/bin/lib/package-identity.cjs` bakes those values at build
...
function deriveIdentity(pkg = {}) {
  ...
  const updateCacheFileName = cacheSlug ? `ecl-update-check-${cacheSlug}.json` : 'ecl-update-check.json';
...
function main() {
  ...
  const out = path.join(__dirname, '..', 'evolv-coder-lite', 'bin', 'lib', 'package-identity.cjs');
