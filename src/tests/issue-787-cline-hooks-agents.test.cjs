...
describe('#787 Cline pure helpers', () => {
  test('buildClineRulesBody returns eCL directory-form rules markdown', () => {
    const body = buildClineRulesBody();
    assert.equal(typeof body, 'string');
    assert.match(body, /eCL workflows live in `evolv-coder-lite\/workflows\/`/);
  ...
  test('buildClinePreToolUseHook returns a syntactically valid Node script', () => {
    ...
    const tmp = createTempDir('ecl-787-hookcheck-');
    try {
  ...
  test('PreToolUse hook allows a normal tool call (cancel:false)', () => {
    const tmp = createTempDir('ecl-787-hookrun-');
    try {
      const p = path.join(tmp, 'PreToolUse');
  ...
  test('PreToolUse hook cancels a write into .planning/ with an errorMessage', () => {
    const tmp = createTempDir('ecl-787-hookguard-');
    try {
      const p = path.join(tmp, 'PreToolUse');
      fs.writeFileSync(p, buildClinePreToolUseHook());
      const res = spawnSync(process.execPath, [p], {
  ...
  test('PreToolUse hook does NOT cancel a write to a non-planning path whose CONTENT mentions .planning/', () => {
    const tmp = createTempDir('ecl-787-hookfp-');
    try {
      const p = path.join(tmp, 'PreToolUse');
      fs.writeFileSync(p, buildClinePreToolUseHook());
      const res = spawnSync(process.execPath, [p], {
        input: JSON.stringify({
          ...
          toolInput: { path: 'docs/guide.md', content: 'Edit your .planning/ROADMAP.md via /ecl commands.' },
        }),
        encoding: 'utf8',
      });
      assert.equal(res.status, 0);
  ...
  test('PreToolUse hook fails open on malformed stdin', () => {
    const tmp = createTempDir('ecl-787-hookbad-');
    try {
      const p = path.join(tmp, 'PreToolUse');
      fs.writeFileSync(p, buildClinePreToolUseHook());
  ...
  test('mergeGsdAgentsMd creates a marker-delimited block when no file exists', () => {
    const tmp = createTempDir('ecl-787-agents-new-');
    try {
      const p = path.join(tmp, 'AGENTS.md');
      mergeGsdAgentsMd(p, buildClineAgentsMdBody());
      const content = fs.readFileSync(p, 'utf8');
      assert.ok(content.includes(ECL_AGENTS_MD_MARKER));
      assert.ok(content.includes(ECL_AGENTS_MD_CLOSE_MARKER));
  ...
  test('mergeGsdAgentsMd preserves pre-existing user content', () => {
    const tmp = createTempDir('ecl-787-agents-merge-');
    try {
      const p = path.join(tmp, 'AGENTS.md');
      ...
      const occurrences = twice.split(ECL_AGENTS_MD_MARKER).length - 1;
      assert.equal(occurrences, 1, 'eCL block must not duplicate on re-merge');
  ...
  test('stripGsdFromAgentsMd returns null when file was eCL-only, else cleaned content', () => {
    const onlyGsd = `${ECL_AGENTS_MD_MARKER}\nhi\n${ECL_AGENTS_MD_CLOSE_MARKER}\n`;
    assert.equal(stripGsdFromAgentsMd(onlyGsd), null);
    const mixed = `# Keep\n\n${ECL_AGENTS_MD_MARKER}\nhi\n${ECL_AGENTS_MD_CLOSE_MARKER}\n`;
    const cleaned = stripGsdFromAgentsMd(mixed);
    assert.match(cleaned, /# Keep/);
    assert.ok(!cleaned.includes(ECL_AGENTS_MD_MARKER));
  });
});
...
describe('#787 Cline local install — directory form + PreToolUse hook', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('ecl-787-cline-local-');
  ...
  test('writes .clinerules/ as a directory containing ecl.md', () => {
    install(false, 'cline');
    const dir = path.join(tmpDir, '.clinerules');
    assert.ok(fs.statSync(dir).isDirectory(), '.clinerules must be a directory');
    const ruleFile = path.join(dir, 'ecl.md');
    assert.ok(fs.existsSync(ruleFile), '.clinerules/ecl.md must exist');
    assert.match(fs.readFileSync(ruleFile, 'utf8'), /evolv-coder-lite\/workflows\//);
  });
  ...
  test('migrates a legacy single-file .clinerules into the directory form', () => {
    ...
    assert.ok(fs.existsSync(path.join(dir, 'ecl.md')));
  });
  ...
  test('does not follow a symlinked .clinerules (writes the real directory in place)', () => {
    ...
    // write eCL files through the link.
    ...
    assert.ok(!fs.existsSync(path.join(external, 'ecl.md')), 'must not write through the symlink target');
    assert.ok(fs.existsSync(path.join(dir, 'ecl.md')));
  });
  ...
  test('manifest tracks the new directory-form artifacts', () => {
    ...
    assert.ok(manifest.files['.clinerules/ecl.md'], 'manifest should track .clinerules/ecl.md');
...
describe('#787 Cline global install — ~/.agents/AGENTS.md', () => {
  function runGlobalClineInstall() {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-787-cline-global-'));
  ...
  test('writes ~/.agents/AGENTS.md with a eCL marker block', () => {
...
describe('#787 Cline uninstall removes managed artifacts', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('ecl-787-cline-uninstall-');
    previousCwd = process.cwd();
    process.chdir(tmpDir);
  });
  ...
  test('local uninstall removes .clinerules/ecl.md and the hook', () => {
    install(false, 'cline');
    assert.ok(fs.existsSync(path.join(tmpDir, '.clinerules', 'ecl.md')));
    uninstall(false, 'cline');
    assert.ok(!fs.existsSync(path.join(tmpDir, '.clinerules', 'ecl.md')), 'ecl.md should be removed');
