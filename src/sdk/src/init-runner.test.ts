...
// Mock fs/promises for template reading (InitRunner reads eCL templates)
...
describe('InitRunner', () => {
  ...
  describe('sdkPromptsDir preference and sanitizer integration', () => {
    ...
    beforeEach(async () => {
      ...
      await writeFile(
        join(sdkPromptsDir, 'agents', 'ecl-project-researcher.md'),
        '# Project Researcher Agent\nSDK_HEADLESS_MARKER_RESEARCHER\n',
      );
      await writeFile(
        join(sdkPromptsDir, 'agents', 'ecl-research-synthesizer.md'),
        '# Research Synthesizer Agent\nSDK_HEADLESS_MARKER_SYNTHESIZER\n',
      );
      await writeFile(
        join(sdkPromptsDir, 'agents', 'ecl-roadmapper.md'),
    ...
    it('readGSDFile prefers installed eCL over sdk/prompts/ template', async () => {
      const { runner } = createRunnerWithSdkPrompts();

      await runner.run('build a todo app');

      // The first session call is buildProjectPrompt → reads templates/project.md
      // Installed eCL templates (if present) are preferred over SDK bundled copies
    ...
    it('readAgentFile prefers installed agents over sdk/prompts/agents/', async () => {
      ...
      // Research calls (indices 1-4) use ecl-project-researcher.md agent def
    ...
    it('readGSDFile falls back to eCL-1 when sdk/prompts/ file does not exist', async () => {
      ...
      // falls through to eCL-1 path. If eCL-1 also missing, gets placeholder.
      const projectPrompt = mockRunSession.mock.calls[0]![0] as string;
    ...
    it('readAgentFile falls back to eCL-1 when sdk/prompts/agents/ file does not exist', async () => {
      ...
      // Research prompt uses agent def — not in empty agents dir, falls to eCL-1
      const researchPrompt = mockRunSession.mock.calls[1]![0] as string;
    ...
    it('buildProjectPrompt output passes through sanitizePrompt (no /ecl: patterns)', async () => {
      const { runner } = createRunnerWithSdkPrompts();
      await runner.run('build a todo app');

      const projectPrompt = mockRunSession.mock.calls[0]![0] as string;
      // sanitizePrompt should strip any /ecl: patterns from the assembled prompt
      expect(projectPrompt).not.toMatch(/\/ecl:\S+/);
      expect(projectPrompt).toContain('PROJECT.md');
    });

    it('buildResearchPrompt output passes through sanitizePrompt (no /ecl: patterns)', async () => {
      const { runner } = createRunnerWithSdkPrompts();
      await runner.run('build a todo app');

      const researchPrompt = mockRunSession.mock.calls[1]![0] as string;
      // sanitizePrompt should strip any /ecl: patterns from the assembled prompt
      expect(researchPrompt).not.toMatch(/\/ecl:\S+/);
      expect(researchPrompt).toContain('You are researching the');
    });

    it('buildRoadmapPrompt output passes through sanitizePrompt (no /ecl: patterns)', async () => {
      const { runner } = createRunnerWithSdkPrompts();
      await runner.run('build a todo app');
      ...
      // sanitizePrompt should strip any /ecl: patterns from the assembled prompt
      expect(roadmapPrompt).not.toMatch(/\/ecl:\S+/);
    });
  });
});
