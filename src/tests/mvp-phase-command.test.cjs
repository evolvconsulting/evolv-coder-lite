/**
 * /ecl mvp-phase command — frontmatter contract test
...
const CMD = path.join(__dirname, '..', 'commands', 'ecl', 'mvp-phase.md');
...
describe('/ecl mvp-phase command frontmatter', () => {
  ...
  test('frontmatter declares correct command name', () => {
    ...
    assert.equal(contract.name, 'ecl:mvp-phase');
  });
