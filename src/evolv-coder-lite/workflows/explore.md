...
offers mid-conversation research when useful, then routes crystallized outputs to eCL artifacts.
</purpose>

<required_reading>
Read all files referenced by the invoking prompt's execution_context before starting.

@~/.claude/evolv-coder-lite/references/questioning.md
...
- ecl-phase-researcher — Researches specific questions and returns concise findings
</available_agent_types>
...
| Spike | `/ecl:spike` (invoke) | Feasibility uncertainty surfaced — "will this API work?", "can we do X?" |
| Sketch | `/ecl:sketch` (invoke) | Design direction unclear — "what should this look like?", "how should this feel?" |
...
- **Phases:** Use existing `/ecl-add-phase` command via SlashCommand
...
$ECL_SDK query commit "docs: capture exploration — {topic_slug}" --files {file_list}
```
...
Continue exploring with `/ecl:explore` or start working with `/ecl:progress --next`.
```
