...
eCL uses a multi-agent architecture where thin orchestrators (workflow files) spawn specialized agents with fresh context windows. Each agent has a focused role, limited tool access, and produces specific artifacts.
...
### ecl-project-researcher

**Role:** Researches domain ecosystem before roadmap creation.

| Property | Value |
|----------|-------|
| **Spawned by** | `/ecl-new-project`, `/ecl-new-milestone` |
...
### ecl-research-synthesizer

**Role:** Combines outputs from parallel researchers into a unified summary.

| Property | Value |
|----------|-------|
| **Spawned by** | `/ecl-new-project` (after 4 researchers complete) |
...
| Property | Value |
|----------|-------|
| **Spawned by** | `/ecl-plan-phase`, `/ecl-quick` |
| **Parallelism** | Single instance |
...
**Role:** Executes eCL plans with atomic commits, deviation handling, and checkpoint protocols.

| Property | Value |
|----------|-------|
| **Spawned by** | `/ecl-execute-phase`, `/ecl-quick` |
...
| Property | Value |
|----------|-------|
| **Spawned by** | `/ecl-plan-phase` (verification loop, max 3 iterations) |
...
| Property | Value |
|----------|-------|
| **Spawned by** | `/ecl-audit-milestone` |
| **Parallelism** | Single instance |
| **Tools** | Read, Bash, Grep, Glob |
| **Disallowed Tools** | Write, Edit, MultiEdit |
| **Model (balanced)** | Sonnet |
...
| Property | Value |
|----------|-------|
| **Spawned by** | `/ecl-ui-phase` (validation loop, max 2 iterations) |
| **Parallelism** | Single instance |
| **Tools** | Read, Bash, Glob, Grep |
| **Disallowed Tools** | Write, Edit, MultiEdit |
| **Model (balanced)** | Sonnet |
...
| Property | Value |
|----------|-------|
| **Spawned by** | `/ecl-execute-phase` (after all executors complete) |
| **Parallelism** | Single instance |
...
- Logs issues for `/ecl-verify-work` to address
...
| Property | Value |
|----------|-------|
| **Spawned by** | `/ecl-map-codebase`, post-execute drift gate in `/ecl-execute-phase` |
...
| Property | Value |
|----------|-------|
| **Spawned by** | `/ecl-debug`, `/ecl-verify-work` (for failures) |
...
- Fix mode accepts failure objects from ecl-doc-verifier for targeted corrections
...
| Property | Value |
|----------|-------|
| **Spawned by** | `/ecl-docs-update` (after doc-writer completes) |
...
Twelve additional agents ship under `agents/ecl-*.md` and are used by specialty workflows (`/ecl-ai-integration-phase`, `/ecl-eval-review`, `/ecl-code-review`, `/ecl-code-review --fix`, `/ecl-debug`, `/ecl-map-codebase --query`, `/ecl-ingest-docs`) and by the planner pipeline. Each carries full frontmatter in its agent file; the stubs below are concise by design. The authoritative roster (with spawner and primary-doc status per agent) lives in [`docs/INVENTORY.md`](INVENTORY.md).
...
| Property | Value |
|----------|-------|
| **Spawned by** | `/ecl-plan-phase` (between research and planning) |
| **Parallelism** | Single instance |
...
**Role:** Runs the full `/ecl-debug` checkpoint-and-continuation loop in an isolated context so the orchestrator's main context stays lean; spawns `ecl-debugger` agents, dispatches specialist skills, and handles user checkpoints via AskUserQuestion.

| Property | Value |
|----------|-------|
| **Spawned by** | `/ecl-debug` |
...
| Property | Value |
|----------|-------|
| **Spawned by** | `/ecl-code-review --fix` |
| **Parallelism** | Single instance |
...
| Property | Value |
|----------|-------|
| **Spawned by** | `/ecl-ai-integration-phase` |
...
**Required reading:** `evolv-coder-lite/references/ai-evals.md` (evaluation framework).
...
**Required reading:** `evolv-coder-lite/references/ai-evals.md`.
...
**Required reading:** `evolv-coder-lite/references/ai-frameworks.md` (decision matrix).
...
| Property | Value |
|----------|-------|
| **Spawned by** | `/ecl-map-codebase --query` (refresh / update flows) |
| **Parallelism** | Single instance |
| **Tools** | Read, Write, Bash, Glob, Grep |
| **Model (balanced)** | Sonnet |
| **Color** | Cyan |
...
**Role:** Classifies a single planning document as ADR, PRD, SPEC, DOC, or UNKNOWN. Extracts title, scope summary, and cross-references. Writes a JSON classification file used by `ecl-doc-synthesizer` to build a consolidated context.

| Property | Value |
|----------|-------|
| **Spawned by** | `/ecl-ingest-docs` (parallel fan-out over the doc corpus) |
...
| Property | Value |
|----------|-------|
| **Spawned by** | `/ecl-ingest-docs` (after classifier fan-in) |
| **Parallelism** | Single instance |
...
- Follows the `references/doc-conflict-engine.md` contract so `/ecl-import` and `/ecl-ingest-docs` produce consistent conflict reports

---

### ecl-mempalace-curator
...
> **Scope:** this table covers the 21 primary agents only. The 13 advanced/specialized agents listed above carry their own tool surfaces in their `agents/ecl-*.md` frontmatter (summarized in the per-agent stubs above and in [`docs/INVENTORY.md`](INVENTORY.md)).
