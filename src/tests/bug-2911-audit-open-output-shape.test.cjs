 ...
 *
 * Two bugs in the `audit-open` dispatch case in bin/ecl-tools.cjs:
 *
...
describe('audit-open — output shape (#2911)', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempProject('ecl-bug-2911-');
  });
  ...
  test('--json mode emits parseable JSON matching auditOpenArtifacts shape', () => {
    ...
    // Shape contract from auditOpenArtifacts() in evolv-coder-lite/bin/lib/audit.cjs.
