...
const SH_HOOKS = [
  { name: 'ecl-validate-commit.sh', commandVar: 'validateCommitCommand' },
  { name: 'ecl-session-state.sh',   commandVar: 'sessionStateCommand' },
  { name: 'ecl-phase-boundary.sh',  commandVar: 'phaseBoundaryCommand' },
];
...
describe('bugs #2045 #2046: .sh hook paths must be absolute and quoted', () => {
  ...
  test('global .sh hook commands use buildHookCommand, not unquoted string concat', () => {
    ...
    //   'bash ' + targetDir.replace(/\\/g, '/') + '/hooks/ecl-*.sh'
    ...
    // wraps the path in double quotes: bash "/absolute/path/hooks/ecl-*.sh"
    ...
    assert.strictEqual(
      ...
      `('bash ' + targetDir). Use buildHookCommand(targetDir, 'ecl-*.sh') instead.`
    );
  });
  ...
  test('global .sh hook commands in source use bash with double-quoted path', () => {
    // After the fix, buildHookCommand produces: bash "/abs/path/hooks/ecl-*.sh"
