...
/**
 * Tests for canonical artifact registry and ecl-health W019 lint (#2448).
 */
...
const { isCanonicalPlanningFile, CANONICAL_EXACT } = require('../evolv-coder-lite/bin/lib/artifacts.cjs');
const { cmdValidateHealth } = require('../evolv-coder-lite/bin/lib/verify.cjs');

function makeTempProject(files = {}) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-2448-'));
...
describe('artifacts.cjs — isCanonicalPlanningFile', () => {
  ...
  test('returns true for RETROSPECTIVE.md (produced by /ecl-complete-milestone)', () => {
...
describe('ecl-health W019 — unrecognized .planning/ root files', () => {
  ...
  test('templates/README.md exists and documents W019', () => {
    const readme = fs.readFileSync(
      path.join(__dirname, '../evolv-coder-lite/templates/README.md'), 'utf-8'
    );
