...
Silent coercion hides invalid inputs from callers — a buggy upstream component may send garbage and never learn it was rejected. Mitigate with an opt-in debug log (`process.env.ECL_DEBUG`) that surfaces the coercion without affecting production behavior.
...
Rejected for the current CJS phase. Adding a runtime dependency to the core library violates the project's no-external-dependencies policy for `ecl-tools.cjs`. Project-internal patterns prefer surgical regex/predicate functions. The `src/` TypeScript phase may revisit if Zod is adopted broadly — that is a separate decision.
