 ...
 *
 * Ported from evolv-coder-lite/bin/lib/profile-pipeline.cjs, profile-output.cjs,
 * and learnings.cjs.
 *
 * @example
 * ```typescript
 ...
 * // { data: { mode: 'interactive', questions: [...] } } — same shape as ecl-tools.cjs
 * ```
 */
...
// ─── Learnings — ~/.ecl/knowledge/ knowledge store ───────────────────────

const STORE_DIR = join(homedir(), '.ecl', 'knowledge');
...
/**
 * List all entries in the global learnings store (`~/.ecl/knowledge/`).
 *
 ...
 * Called by ecl-planner agent to inject prior learnings into plan generation.
 *
 ...
 *
 * Port of `cmdExtractMessages` from profile-pipeline.cjs — JSON matches `ecl-tools extract-messages`
...
export const extractMessages: QueryHandler = async (args) => {
  ...
  if (!projectArg || projectArg.startsWith('--')) {
    throw new GSDError(
      'Usage: ecl-tools extract-messages <project> [--session <id>] [--limit N] [--path <dir>]\nRun scan-sessions first to see available projects.',
 ...
 * JSON matches `ecl-tools profile-sample` (`output_file` JSONL + metadata).
 */
