...
describe('verifyKeyLinks', () => {
  ...
  beforeEach(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), 'ecl-validate-'));
  });
...
`;
  ...
  it('reports invalid regex like ecl-tools.cjs (try/catch on new RegExp)', async () => {
    await writeFile(join(tmpDir, 'source.ts'), 'const x = 1;');
    await writeFile(join(tmpDir, 'target.ts'), 'const y = 2;');
...
describe('validateConsistency', () => {
  ...
  beforeEach(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), 'ecl-consistency-'));
  });
...
describe('validateHealth', () => {
  ...
  beforeEach(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), 'ecl-health-'));
  });
  ...
  // Regression: #3652 — after /ecl:complete-milestone, STATE.md body retains
