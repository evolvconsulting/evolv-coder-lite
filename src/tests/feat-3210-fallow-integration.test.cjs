...
describe('feat-3210: fallow integration module', () => {
  test('normalizes structural findings from a fallow report', () => {
    const { normalizeFallowReport } = require('../evolv-coder-lite/bin/lib/fallow-runner.cjs');
  ...
  test('falls back to node_modules/.bin/fallow when PATH does not contain fallow', () => {
    ...
    const tmp = fs.mkdtempSync(path.join(baseTmp, 'ecl-fallow-bin-'));
  ...
  test('ignores non-executable PATH candidate on non-Windows; prefers .cmd over bare extensionless on Windows', () => {
    ...
    if (process.platform === 'win32') {
      ...
      const tmp = fs.mkdtempSync(path.join(baseTmp, 'ecl-fallow-win-'));
      try {
      ...
      const tmp = fs.mkdtempSync(path.join(baseTmp, 'ecl-fallow-nonexec-'));
      try {
        const pathDir = path.join(tmp, 'bin');
        fs.mkdirSync(pathDir, { recursive: true });
  ...
  test('throws actionable error when fallow is enabled but binary is unavailable', () => {
    const { requireFallowBinary } = require('../evolv-coder-lite/bin/lib/fallow-runner.cjs');
    // N2: use shared helper
    const baseTmp = getWritableTmp();
    const tmp = fs.mkdtempSync(path.join(baseTmp, 'ecl-fallow-missing-'));
    assert.throws(
  ...
  test('runFallowAudit surfaces error state when binary exits non-zero', async () => {
    const { runFallowAudit } = require('../evolv-coder-lite/bin/lib/fallow-runner.cjs');
    // N2: use shared helper
    const baseTmp = getWritableTmp();
    const tmp = fs.mkdtempSync(path.join(baseTmp, 'ecl-fallow-fail-'));
...
describe('feat-3210: M2 - node_modules/.bin resolution order', () => {
  test('resolveFallowBinary prefers node_modules/.bin over PATH when both exist', () => {
    ...
    // N2: use shared helper
    const baseTmp = getWritableTmp();
    const tmp = fs.mkdtempSync(path.join(baseTmp, 'ecl-fallow-order-'));
    try {
...
describe('feat-3210: workflow and config contracts', () => {
  test('config schema allows code_quality.fallow.* keys in CJS and runtime manifest', () => {
    ...
    const manifestPath = path.join(ROOT, 'evolv-coder-lite', 'bin', 'shared', 'config-schema.manifest.json');
  ...
  test('config-set accepts code_quality.fallow keys', () => {
    ...
    const tmpDir = createTempProject('ecl-fallow-config-');
    try {
      const cases = [
  ...
  test('code-review workflow structural_pre_pass step is parseable and references FALLOW.json output', () => {
    const workflow = fs.readFileSync(
      path.join(ROOT, 'evolv-coder-lite', 'workflows', 'code-review.md'),
      'utf8',
    );
  ...
  // Live agent output is covered by /ecl-code-review e2e runs downstream.
  test('reviewer prompt defines ## Structural Findings (fallow) heading and review context echoes it', () => {
    const reviewer = fs.readFileSync(path.join(ROOT, 'agents', 'ecl-code-reviewer.md'), 'utf8');
    const reviewContext = fs.readFileSync(path.join(ROOT, 'evolv-coder-lite', 'contexts', 'review.md'), 'utf8');
    ...
    assert.ok(
      ...
      'ecl-code-reviewer.md must define ## Structural Findings (fallow) section heading',
    );
