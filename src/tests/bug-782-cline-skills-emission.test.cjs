 ...
 *
 * ecl now emits skills to ~/.cline/skills/<name>/SKILL.md for Cline >= v3.48.
 ...
 * (c) .clinerules/ecl.md still written by the install path (#787 dir form)
 * (d) Idempotency: running install twice leaves skills + .clinerules/ intact
 * (e) Full install() global: both skills AND .clinerules/ecl.md are written
 */
...
Audit Nyquist validation coverage. See ~/.claude/skills/ecl-help/SKILL.md for reference.
...
describe('convertClaudeCommandToClineSkill — unit', () => {
  test('emits frontmatter with name: ecl-<stem>', () => {
    const result = convertClaudeCommandToClineSkill(SAMPLE_COMMAND, 'ecl-execute-phase');
    const nameMatch = result.match(/^name:\s*(.+)$/m);
    assert.ok(nameMatch, 'frontmatter must contain name field');
    assert.ok(nameMatch[1].includes('ecl-execute-phase'), 'name must start with ecl-execute-phase');
  });
  ...
  test('frontmatter emits ONLY name and description — no allowed-tools/argument-hint/agent (RICH_COMMAND)', () => {
    const result = convertClaudeCommandToClineSkill(RICH_COMMAND, 'ecl-validate-phase');
    const fm = parseFrontmatter(result);
    assert.ok(fm !== null, 'result must have YAML frontmatter');
    assert.ok(!fm.includes('allowed-tools'), 'frontmatter must NOT contain allowed-tools');
    assert.ok(!fm.includes('argument-hint'), 'frontmatter must NOT contain argument-hint');
  ...
  test('name == ecl-validate-phase for RICH_COMMAND', () => {
    const result = convertClaudeCommandToClineSkill(RICH_COMMAND, 'ecl-validate-phase');
    const nameMatch = result.match(/^name:\s*(.+)$/m);
    ...
    assert.strictEqual(nameVal, 'ecl-validate-phase', `name must be ecl-validate-phase, got: ${nameVal}`);
  });
  ...
  test('description truncated to <=1024 chars when source description is very long', () => {
    ...
    const result = convertClaudeCommandToClineSkill(longDescCommand, 'ecl-test');
    const descMatch = result.match(/^description:\s*'?(.*?)'?$/m);
    assert.ok(descMatch, 'must have description field');
  ...
  test('returns content unchanged when source has no frontmatter', () => {
    ...
    const result = convertClaudeCommandToClineSkill(noFm, 'ecl-test');
...
describe('installRuntimeArtifacts — cline skills emission', () => {
  test('cline global: writes ecl-prefixed skill dirs under skills/', (t) => {
    const configDir = createTempDir('ecl-cline-skills-');
    ...
    assert.ok(
      ...
      `ecl-help/SKILL.md must exist under ${skillsKind.destSubpath}/`
    );
  });

  test('cline global: SKILL.md has valid cline frontmatter (name + description)', (t) => {
    const configDir = createTempDir('ecl-cline-fm-');
    t.after(() => cleanup(configDir));

    installRuntimeArtifacts('cline', configDir, 'global', RESOLVED_CORE);

    const skillsDir = path.join(configDir, 'skills');
    const helpSkill = path.join(skillsDir, 'ecl-help', 'SKILL.md');
    assert.ok(fs.existsSync(helpSkill), 'ecl-help/SKILL.md must exist');
    ...
    // name must be ecl-help
    const nameMatch = content.match(/^name:\s*(.+)$/m);
    assert.ok(nameMatch, 'must have name field');
    assert.ok(nameMatch[1].includes('ecl-help'), `name must include ecl-help, got: ${nameMatch[1]}`);
  });

  test('cline global: SKILL.md uses .cline/ paths not .claude/', (t) => {
    const configDir = createTempDir('ecl-cline-paths-');
    t.after(() => cleanup(configDir));
    ...
    const skills = fs.readdirSync(skillsDir).filter(n => n.startsWith('ecl-'));
    assert.ok(skills.length > 0, 'at least one ecl- skill must be installed');
  ...
  test('cline global: skill count matches resolved profile', (t) => {
    const configDir = createTempDir('ecl-cline-count-');
    t.after(() => cleanup(configDir));
    ...
    const count = fs.readdirSync(skillsDir)
      .filter(n => n.startsWith('ecl-') && fs.statSync(path.join(skillsDir, n)).isDirectory())
      .length;
...
describe('installRuntimeArtifacts — cline idempotency', () => {
  test('cline: running install twice leaves skills intact (idempotency)', (t) => {
    const configDir = createTempDir('ecl-cline-idempotent-');
    t.after(() => cleanup(configDir));
...
//   - skills/<ecl-*>/SKILL.md     (skills for Cline >= v3.48)
//   - .clinerules/ecl.md          (rules dir form introduced by #787)
//
...
describe('install() global cline — coexistence: skills AND .clinerules', () => {
  ...
  beforeEach(() => {
    ...
    tmpGlobalDir = createTempDir('ecl-cline-global-');
  ...
  test('global cline install writes at least one ecl-* SKILL.md under skills/', () => {
    ...
    // ecl-help is present in every profile (core, standard, full)
    const helpSkillFile = path.join(skillsDir, 'ecl-help', 'SKILL.md');
    assert.ok(
      fs.existsSync(helpSkillFile),
      `skills/ecl-help/SKILL.md must exist under ${tmpGlobalDir} — skills emission broken for global cline`
    );
  });

  test('global cline install writes .clinerules/ecl.md to the global config dir', () => {
    captureConsole(() => install(true, 'cline'));
    ...
    //   .clinerules/ecl.md  (rule file)
    //   .clinerules/hooks/PreToolUse  (lifecycle hook)
    const clinerulesMd = path.join(tmpGlobalDir, '.clinerules', 'ecl.md');
    assert.ok(
      fs.existsSync(clinerulesMd),
      `.clinerules/ecl.md must exist at ${clinerulesMd} — coexistence with skills broken for global cline (#782+#787)`
    );
  });

  test('global cline .clinerules/ecl.md contains eCL instructions', () => {
    captureConsole(() => install(true, 'cline'));

    // #787 dir form: rule content lives in .clinerules/ecl.md, not a flat .clinerules file
    const clinerulesMd = path.join(tmpGlobalDir, '.clinerules', 'ecl.md');
    assert.ok(fs.existsSync(clinerulesMd), '.clinerules/ecl.md must exist');
    const content = fs.readFileSync(clinerulesMd, 'utf8');
    assert.ok(
      content.includes('eCL') || content.includes('ecl'),
      '.clinerules/ecl.md must reference eCL'
    );
  });
});
...
describe('convertClaudeToCliineMarkdown — bare ~/.claude and CLAUDE_CONFIG_DIR (Fix 3)', () => {
  const surfacePath = path.join(__dirname, '..', 'commands', 'ecl', 'surface.md');
  ...
  test('installRuntimeArtifacts cline global: ecl-surface SKILL.md has no bare ~/.claude or CLAUDE_CONFIG_DIR', (t) => {
    const configDir = createTempDir('ecl-cline-surface-fix3-');
    t.after(() => cleanup(configDir));

    const MANIFEST_FULL = require('../evolv-coder-lite/bin/lib/install-profiles.cjs').loadSkillsManifest(
      path.join(__dirname, '..', 'commands', 'ecl')
    );
    const RESOLVED_FULL = require('../evolv-coder-lite/bin/lib/install-profiles.cjs').resolveProfile({
      modes: ['full'], manifest: MANIFEST_FULL,
    });

    installRuntimeArtifacts('cline', configDir, 'global', RESOLVED_FULL);

    const surfaceSkill = path.join(configDir, 'skills', 'ecl-surface', 'SKILL.md');
    assert.ok(fs.existsSync(surfaceSkill), 'ecl-surface/SKILL.md must exist for full profile');

    const content = fs.readFileSync(surfaceSkill, 'utf8');
    assert.ok(
      !/~\/\.claude\b/.test(content),
      'ecl-surface SKILL.md must not contain bare ~/.claude (Fix 3)'
    );
    assert.ok(
      !content.includes('CLAUDE_CONFIG_DIR'),
      'ecl-surface SKILL.md must not contain CLAUDE_CONFIG_DIR (Fix 3)'
    );
  });
});
...
describe('_applyRuntimeRewrites — cline custom-dir embedded path (Fix 1)', () => {
  test('default pathPrefix ($HOME/.cline/) leaves ~/.cline refs as $HOME/.cline', () => {
    const content = 'See ~/.cline/skills/ecl-help/SKILL.md for reference.\nBare: ~/.cline\n';
  ...
  test('installRuntimeArtifacts cline with CLINE_CONFIG_DIR custom: SKILL.md embeds custom path', (t) => {
    const configDir = createTempDir('ecl-cline-custom-dir-');
    t.after(() => cleanup(configDir));
    ...
    // ecl-surface SKILL.md references config paths; with a custom configDir
    // (not under $HOME), pathPrefix will be the absolute custom path.
    const surfaceSkill = path.join(configDir, 'skills', 'ecl-surface', 'SKILL.md');
    assert.ok(fs.existsSync(surfaceSkill), 'ecl-surface/SKILL.md must exist');
    ...
    assert.ok(
      ...
      `ecl-surface SKILL.md must not contain ~/.cline/ when configDir=${configDir} (Fix 1)`
    );
    ...
    assert.ok(
      ...
      `ecl-surface SKILL.md must embed custom configDir path ${expectedPath} (Fix 1)`
    );
  });
});
...
describe('convertClaudeCommandToClineSkill — code-point-aware truncation (Fix 4)', () => {
  ...
  test('emitted description is <= 1024 code points when source overflows', () => {
    ...
    const result = convertClaudeCommandToClineSkill(cmd, 'ecl-emoji-test');
...
describe('resolveRuntimeArtifactLayout — cline scope-aware (Fix 2)', () => {
  ...
  test('installRuntimeArtifacts cline local: no skills/ dir created', (t) => {
    const configDir = createTempDir('ecl-cline-local-noskills-');
    t.after(() => cleanup(configDir));
