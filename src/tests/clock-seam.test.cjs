...
const { acquireStateLock, releaseStateLock, readModifyWriteStateMd } = require('../evolv-coder-lite/bin/lib/state.cjs');
const { withPlanningLock } = require('../evolv-coder-lite/bin/lib/planning-workspace.cjs');
...
describe('acquireStateLock clock seam', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-clock-'));
...
describe('acquireStateLock recoverable errno budget + backoff (#1217)', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-clock-1217-'));
    fs.mkdirSync(path.join(tmpDir, '.planning'), { recursive: true });
    statePath = path.join(tmpDir, '.planning', 'STATE.md');
    fs.writeFileSync(statePath, '# State\n');
...
describe('acquireStateLock boundary coverage — recoverable-errno budget (#1217)', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-clock-boundary-'));
    fs.mkdirSync(path.join(tmpDir, '.planning'), { recursive: true });
    statePath = path.join(tmpDir, '.planning', 'STATE.md');
    fs.writeFileSync(statePath, '# State\n');
    origOpenSync = fs.openSync;
...
describe('acquireStateLock statSync/unlinkSync spin paths bounded (#1217)', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-clock-spin-'));
    fs.mkdirSync(path.join(tmpDir, '.planning'), { recursive: true });
    statePath = path.join(tmpDir, '.planning', 'STATE.md');
    fs.writeFileSync(statePath, '# State\n');
...
describe('withPlanningLock clock seam', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-clock-planning-'));
    fs.mkdirSync(path.join(tmpDir, '.planning'), { recursive: true });
  });
...
//    Uses CLI-level integration: each cmd* is exercised through ecl-tools and
