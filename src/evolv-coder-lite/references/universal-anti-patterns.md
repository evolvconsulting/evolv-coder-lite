...
15. **No direct Write/Edit to STATE.md or ROADMAP.md for mutations.** Always use `ecl-sdk query` for registered state/roadmap handlers (e.g. `state.update`, `state.advance-plan`, `roadmap.update-plan-progress`), or legacy `node …/ecl-tools.cjs` for CLI-only commands. Direct Write tool usage bypasses safe update logic and is unsafe in multi-session environments. Exception: first-time creation of STATE.md from template is allowed.
...
25. **Prefer `ecl-sdk query`** for orchestration when a handler exists; when shelling out to the legacy CLI, use **`ecl-tools.cjs`** (not `ecl-tools.js` or any other filename) — eCL ships the programmatic API as CommonJS for Node.js CLI compatibility.
