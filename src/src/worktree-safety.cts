...
function cmdWorktreeReapOrphans(cwd: string): void {
  ...
  try {
    ...
    process.stderr.write(`[ecl] worktree.reap-orphans failed: ${err && (err as Error).message ? (err as Error).message : String(err)}\n`);
    result = [];
  }
  ...
  if (skippedCount > 0) {
    ...
    process.stderr.write(`[ecl] worktree.reap-orphans: ${skippedCount} orphan(s) skipped (run with DEBUG=1 for details)\n`);
  }
