...
function makeTempProject() {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-fed-cfg-test-'));
...
describe('FIX 2: overlay applied on the no-config path', () => {
  test('project with NO config.json → federated default is surfaced (non-central key)', () => {
    ...
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-fed-noconfig-'));
    tmpDirs.push(tmpDir);
    fs.mkdirSync(path.join(tmpDir, '.planning', 'phases'), { recursive: true });
  ...
  test('no-config path with REAL registry → capability defaults are surfaced', () => {
    _resetFederatedRegistryForTests();
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-fed-noconfig-real-'));
    tmpDirs.push(tmpDir);
    fs.mkdirSync(path.join(tmpDir, '.planning', 'phases'), { recursive: true });
    // No config.json
