 ...
 * (e.g., ecl-intel-*.js) should NOT be flagged as stale by ecl-check-update.js.
 *
 * The stale hooks scanner should only check hooks that are part of the current
 * distribution, not every ecl-*.js file in the hooks directory.
 *
...
const CHECK_UPDATE_PATH = path.join(__dirname, '..', 'hooks', 'ecl-check-update.js');
const WORKER_PATH = path.join(__dirname, '..', 'hooks', 'ecl-check-update-worker.js');
...
describe('orphaned hooks stale detection (#1750)', () => {
  test('MANAGED_HOOKS is an array and does not use a broad ecl-* wildcard', () => {
    // The scanner must reference a known set of managed hook filenames,
    // not a broad startsWith('ecl-') filter that catches orphaned files.
    ...
    for (const entry of MANAGED_HOOKS) {
      ...
      assert.ok(entry.startsWith('ecl-'), `MANAGED_HOOKS entry '${entry}' must start with ecl-`);
    }
  });

  test('ecl-check-update-worker.js imports managed-hooks-registry.cjs (not inline array)', () => {
    const content = fs.readFileSync(WORKER_PATH, 'utf8');
    assert.ok(
      content.includes('managed-hooks-registry.cjs'),
      'ecl-check-update-worker.js must require managed-hooks-registry.cjs'
    );
    // The inline MANAGED_HOOKS array must no longer be in the worker
    assert.ok(
      !content.includes('const MANAGED_HOOKS = ['),
      'ecl-check-update-worker.js must not define MANAGED_HOOKS inline — it should import from managed-hooks-registry.cjs'
    );
  });

  test('ecl-check-update.js spawns the worker by file path (not inline -e code)', () => {
    const content = fs.readFileSync(CHECK_UPDATE_PATH, 'utf8');
    assert.ok(
      content.includes('ecl-check-update-worker.js'),
      'ecl-check-update.js must reference ecl-check-update-worker.js as the spawn target'
    );
    assert.ok(
      !content.includes("'-e'"),
      'ecl-check-update.js must not use node -e inline code (logic moved to worker file)'
    );
  });
  ...
  test('MANAGED_HOOKS is a superset of all ecl-* hooks in HOOKS_TO_COPY (all extensions)', () => {
    // Every hook-named file in HOOKS_TO_COPY (matching the ecl-* naming pattern
    ...
    assert.ok(Array.isArray(HOOKS_TO_COPY), 'HOOKS_TO_COPY must be an array');
    const gsdHooks = HOOKS_TO_COPY.filter(h => h.startsWith('ecl-'));
    assert.ok(gsdHooks.length >= 5, `expected at least 5 ecl-* hooks in HOOKS_TO_COPY, got ${gsdHooks.length}`);
  ...
  test('every same-dir require() target of a shipped hook is itself shipped (#606)', () => {
    // Regression guard for #606: ecl-check-update-worker.js does
    ...
    // dir into sibling package dirs (e.g. evolv-coder-lite/) whose shipping is governed
