...
This document defines the migration layer for eCL installs and upgrades.
It is for contributors who need to retire files, move install surfaces,
rewrite runtime config, or preserve user data while changing how eCL is
installed.
...
- replacing eCL-managed command, skill, agent, hook, and engine files
...
inside a eCL-managed directory.
...
2. Remove stale eCL-managed files when a feature is retired.
...
A file that eCL installed and recorded in the install manifest. Managed files
...
files must never be removed just because they sit under a eCL directory.
...
eCL version, install mode, and hashes for distribution-owned files.
...
Remove a path only when it is known to be eCL-managed and unchanged from the
previous manifest, or when the migration provides a purpose-built detector for
an old eCL-owned shape.
...
purpose-built detector that proves eCL ownership.
...
Use this for legacy JSON config cleanup such as Codex `hooks.json`, where eCL
...
retired eCL-generated artifacts that need an explicit user choice.
...
contains a specific detector proving the file is a stale eCL artifact.
...
Runtime config is mixed ownership. eCL may own marker blocks, generated agent
sections, or hook entries, but it does not own the whole file unless the file
was created as a eCL-only file. Config migrations should remove or rewrite
...
- **What:** the eCL invocation, agent, skill, rule, hook, or config surface
...
- **Why:** the upstream loader contract or current eCL compatibility shim
...
| Runtime | What eCL installs | Where eCL installs it | Config ownership boundary | Upstream contract snapshot |
| --- | --- | --- | --- | --- |
| Claude Code | Global skills in `skills/ecl-*/SKILL.md`; local slash commands in `commands/ecl/*.md`; agents in `agents/ecl-*.md`; hooks in `hooks/`; `settings.json` registrations | Global `CLAUDE_CONFIG_DIR` or `~/.claude`; local `./.claude` | eCL owns only generated skills, local commands, `ecl-*` agents, hook files, and eCL hook/statusLine entries in `settings.json` | [Slash commands](https://docs.anthropic.com/en/docs/claude-code/slash-commands), [settings](https://docs.anthropic.com/en/docs/claude-code/settings), [hooks](https://docs.anthropic.com/en/docs/claude-code/hooks), [subagents](https://docs.anthropic.com/en/docs/claude-code/sub-agents); docs not versioned, checked 2026-05-11 |
| OpenCode | Flat markdown commands in `command/ecl-*.md`; agents in `agents/ecl-*.md`; config updates in `opencode.json` or `opencode.jsonc` | Global `OPENCODE_CONFIG_DIR`, `dirname(OPENCODE_CONFIG)`, `XDG_CONFIG_HOME/opencode`, or `~/.config/opencode`; local `./.opencode` | eCL owns generated command/agent files and eCL entries in structured config only | [Config](https://opencode.ai/docs/config/); docs published 2026-05, checked 2026-05-11 |
| Kilo | OpenCode-style flat markdown commands in `command/ecl-*.md`; agents in `agents/ecl-*.md`; config updates in `kilo.json` or `kilo.jsonc` | Global `KILO_CONFIG_DIR`, `dirname(KILO_CONFIG)`, `XDG_CONFIG_HOME/kilo`, or `~/.config/kilo`; local `./.kilo` | eCL owns generated command/agent files and eCL entries in structured config only | [Custom subagents](https://docs.kilo.ai/docs/customize/custom-subagents); docs not versioned, checked 2026-05-11 |
| Gemini CLI | TOML slash commands in `commands/ecl/*.toml`; agents in `agents/ecl-*.md`; `settings.json` feature flag, hooks, and statusline | Global `GEMINI_CONFIG_DIR` or `~/.gemini`; local `./.gemini` | eCL owns generated commands/agents/hooks and only eCL settings entries; local command copy may be skipped when global eCL commands already exist | [Custom commands](https://google-gemini.github.io/gemini-cli/docs/cli/custom-commands.html), [configuration](https://google-gemini.github.io/gemini-cli/docs/cli/configuration.html); docs checked 2026-05-11 |
| Kimi CLI | Agent Skills in `skills/ecl-*/SKILL.md`; explicit custom agent YAML/prompt artifacts in `agents/ecl.yaml`, `agents/ecl.md`, and `agents/subagents/ecl-*`; `evolv-coder-lite/` payload files referenced by generated skills; manifest, pristine, local-patch, and migration journal files from the normal installer safety pipeline | Global `KIMI_CONFIG_DIR`, explicit `--config-dir`, or first-existing generic skills root: `~/.config/agents` when `~/.config/agents/skills` exists or no generic skills root exists yet, otherwise `~/.agents` when `~/.agents/skills` exists and `~/.config/agents/skills` does not; `KIMI_CONFIG_DIR` and `--config-dir` are eCL write-location overrides and arbitrary roots require Kimi-side `--skills-dir` or `extra_skill_dirs` configuration for skill discovery; local `--kimi --local` is guarded and writes no project-level artifacts | eCL owns only generated `skills/ecl-*`, `agents/ecl.*`, `agents/subagents/ecl-*`, installed `evolv-coder-lite/` payload files, and manifest/preservation/migration records. eCL does not own Kimi config files, hooks, settings, rules, statusline, update-banner registration, or non-eCL Kimi skills/agents. Reinstall/update must preserve locally modified generated Kimi artifacts through manifest-backed `ecl-local-patches/`; uninstall removes only eCL-owned Kimi artifacts and preserves non-eCL user content. | [Agent Skills](https://moonshotai.github.io/kimi-cli/en/customization/skills.html), [Agents and Subagents](https://moonshotai.github.io/kimi-cli/en/customization/agents.html), [Tools](https://moonshotai.github.io/kimi-code/en/reference/tools.html); docs checked 2026-06-07 |
| Codex | Skills in `skills/ecl-*/SKILL.md`; agents as source markdown plus per-agent TOML in `agents/`; `[agents.ecl-*]` and hooks in `config.toml` | Global `CODEX_HOME` or `~/.codex`; local `./.codex` | eCL owns generated skills, generated agent TOML, `agents.ecl-*` config sections, `[features].hooks` when added by eCL (canonical; legacy alias `codex_hooks` is recognized and migrated forward, #3566), and eCL hook entries | [Codex config schema](https://developers.openai.com/codex/config-schema.json), [Codex developer docs](https://developers.openai.com/codex/); docs not versioned, checked 2026-05-15; installer compatibility sentinel: Codex 0.130.0 features.hooks key (legacy `codex_hooks` recognized) |
| GitHub Copilot | Skills in `skills/ecl-*/SKILL.md`; agents as `.agent.md`; repository instructions in `copilot-instructions.md` | Global `COPILOT_CONFIG_DIR`, `COPILOT_HOME`, or `~/.copilot`; local `./.github` | eCL owns generated skill/agent files and eCL-authored instruction files; no hook/statusline ownership | [Repository custom instructions](https://docs.github.com/en/copilot/how-tos/configure-custom-instructions/add-repository-instructions), [Copilot CLI custom instructions](https://docs.github.com/en/copilot/how-tos/copilot-cli/add-custom-instructions); GitHub Docs product docs, checked 2026-05-11 |
| Antigravity | Skills in `skills/ecl-*/SKILL.md`; agents in `agents/`; Gemini-style `settings.json` hooks when installed by eCL | Global `ANTIGRAVITY_CONFIG_DIR` or `~/.gemini/antigravity`; local `./.agents` (canonical, #791) or `./.agent` (legacy, recognized for backward-compat) | eCL owns generated skills/agents/hooks and eCL settings entries only | Public Antigravity install/config docs for this file layout were not stable or complete as of 2026-05-11; installer compatibility therefore uses eCL's Gemini-compatible settings policy, documented shim baseline. Fresh installs write to `.agents/` (the Google-Codelabs-documented form); existing `.agent/` installs continue to be detected and served. |
| Cursor | Skills in `skills/ecl-*/SKILL.md`; agents in `agents/`; rule references under `rules/`; lifecycle hooks via `hooks.json` (sessionStart + postToolUse, #777) | Global `CURSOR_CONFIG_DIR` or `~/.cursor`; local `./.cursor` | eCL owns generated skills/agents, eCL rule files or references, and eCL-managed `hooks.json` entries (sentinel `ecl-managed:true`); no statusline ownership | [Cursor rules](https://docs.cursor.com/context/rules); [Cursor hooks](https://docs.cursor.com/context/hooks); docs not versioned, checked 2026-06-07 |
| Windsurf / Devin Desktop | Skills in `skills/ecl-*/SKILL.md`; agents in `agents/`; rule references under `rules/` | Global `WINDSURF_CONFIG_DIR` or `~/.codeium/windsurf`; local `./.devin` (canonical, #1085) or `./.windsurf` (legacy, recognized for backward-compat) | eCL owns generated skills/agents and eCL rule files or references; no hook/statusline ownership | Windsurf has rebranded to Devin Desktop; workspace skills install to `.devin/` per Devin Desktop documented preferred location (#1085). Global `~/.codeium/windsurf/` is unchanged. Windsurf public rule docs were source-limited in search results as of 2026-05-11; installer targets the common workspace rules convention `./.devin/rules` and must be rechecked before migrations rewrite rules |
| Augment Code | Skills in `skills/ecl-*/SKILL.md`; agents in `agents/` | Global `AUGMENT_CONFIG_DIR` or `~/.augment`; local `./.augment` | eCL owns generated skills/agents only; no hook/statusline ownership | [Augment Agent Skills](https://docs.augmentcode.com/cli/skills), [Augment IDE skills](https://docs.augmentcode.com/using-augment/skills); IDE skills public beta in VS Code 0.789.0+, checked 2026-05-11 |
| Trae | Skills in `skills/ecl-*/SKILL.md`; agents in `agents/`; rule references under `rules/` | Global `TRAE_CONFIG_DIR` or `~/.trae`; local `./.trae` | eCL owns generated skills/agents and eCL rule files or references; no hook/statusline ownership | Public Trae docs expose AI settings and `.rules` announcements, but no stable skills/config API was found as of 2026-05-11; migrations must treat this row as source-limited |
| Qwen Code | Claude-compatible skills in `skills/ecl-*/SKILL.md`; agents in `agents/`; optional common hook/settings integration through eCL | Global `QWEN_CONFIG_DIR` or `~/.qwen`; local `./.qwen` | eCL owns generated skills/agents/hooks and eCL settings entries only | [Qwen commands and skills](https://qwenlm.github.io/qwen-code-docs/en/users/features/commands/); docs last updated 2026-05-06 |
| Hermes Agent | Category skills under `skills/ecl/` with `DESCRIPTION.md` plus nested `ecl-*/SKILL.md`; agents in `agents/`; optional common hook/settings integration through eCL | Global `HERMES_HOME` or `~/.hermes`; local `./.hermes` | eCL owns generated `skills/ecl/` category content, generated agents, and eCL settings entries only | [Hermes configuration](https://hermes-agent.nousresearch.com/docs/user-guide/configuration), [Hermes skills](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/skills), [working with skills](https://hermes-agent.nousresearch.com/docs/guides/work-with-skills); docs checked 2026-05-11 |
| CodeBuddy | Skills in `skills/ecl-*/SKILL.md`; agents in `agents/`; optional common hook/settings integration through eCL | Global `CODEBUDDY_CONFIG_DIR` or `~/.codebuddy`; local `./.codebuddy` | eCL owns generated skills/agents/hooks and eCL settings entries only | [CodeBuddy CLI skills](https://www.codebuddy.ai/docs/cli/skills), [CodeBuddy IDE skills](https://www.codebuddy.ai/docs/ide/Features/Skills); docs checked 2026-05-11 |
| Cline | Rule-based integration via `.clinerules` for current installer output | Global `CLINE_CONFIG_DIR` or `~/.cline`; local project root `.clinerules` | eCL owns the generated `.clinerules` file only when it created or manifest-tracked it; no hooks/statusline ownership | [Cline rules](https://docs.cline.bot/customization/cline-rules); docs prefer `.clinerules/` directory and still detect legacy rule files, checked 2026-05-11 |
...
- Preserve unknown user config, even when it sits inside a eCL-managed runtime
  root.
...
4. report stale eCL-looking files that are not in the current manifest
...
- `baseline-preserve-user` for known user-owned files and unknown files that do
  not look like stale eCL-generated artifacts
- `prompt-user` for stale eCL-looking artifacts that are not manifest-proven
...
- How do we prove it is eCL-owned?
...
- unknown file under a eCL directory
...
| `2026-05-11-legacy-orphan-files` | `001-legacy-orphan-files.cts` | 1.50.0 | global, local | Yes | Removes manifest-managed legacy orphan hook files (`hooks/ecl-notify.sh`, `hooks/statusline.js`) retired by the installer. |
| `2026-05-11-codex-legacy-hooks-json` | `002-codex-legacy-hooks-json.cts` | 1.50.0 | global, local | Yes | Removes legacy eCL hook registrations from Codex `hooks.json` after the `config.toml` migration. |
| `2026-06-02-rename-evolv-coder-lite-to-evolv-coder-lite` | `003-rename-evolv-coder-lite-to-evolv-coder-lite.cts` | 1.2.0 | global, local | Yes | Removes managed files from the stale `evolv-coder-lite/` runtime directory after the rename to `evolv-coder-lite/` (#604). User-added files are preserved; emptied directories may remain (framework limitation). <!-- ecl-allow-legacy-name --> |
