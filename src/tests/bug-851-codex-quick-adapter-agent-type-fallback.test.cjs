...
describe('bug #851: Codex adapter documents multi_agent_v1 schema limitation and fallback', () => {
  ...
  //     not equivalent to typed ecl-planner/ecl-executor execution.
  test('(c) not-equivalent label: generic-agent workaround is labeled as NOT equivalent to typed dispatch', () => {
    ...
    const namesTypedAgent =
      sectionC.includes('ecl-planner') ||
      sectionC.includes('ecl-executor') ||
      sectionC.includes('typed eCL agent') ||
      sectionC.includes('typed ecl-');
    ...
    assert.ok(
      namesTypedAgent,
      'Section C must name at least one typed eCL agent (ecl-planner, ecl-executor, or "typed eCL agent")',
    );
    assert.ok(
      hasNotEquivalent,
