 ...
 *   3. Opt-in audit file — when ECL_AUDIT=1 OR config.audit.enabled===true,
 *      appends every event (success + error) as one JSON line to
 *      .planning/.ecl-trace.jsonl relative to `cwd`. Creates .planning/ if absent.
 *   4. Args redaction — args omitted by default; included when ECL_AUDIT_ARGS=1.
 *
...
const AUDIT_FILE_NAME = '.ecl-trace.jsonl';
...
function _isAuditEnabled(config) {
  if (process.env.ECL_AUDIT === '1') return true;
 ...
 * @param {object}  [opts.config]             - eCL config object. config.audit.enabled triggers audit.
 * @returns {{ onEvent(event: object): void }}
 */
