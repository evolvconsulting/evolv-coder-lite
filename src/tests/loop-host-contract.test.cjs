...
const CONTRACT_PATH = path.join(ROOT, 'evolv-coder-lite', 'bin', 'lib', 'loop-host-contract.cjs');
...
describe('crossCheckRoles', () => {
  ...
  test('passes when agent name is present in content', () => {
    const content = 'Agent(subagent_type="ecl-planner") Agent(subagent_type="ecl-phase-researcher") ecl-plan-checker';
  ...
  test('fails when declared role has no agent reference in content', () => {
    const content = 'ecl-phase-researcher ecl-plan-checker'; // planner missing
    const errors = crossCheckRoles(content, ['researcher', 'planner', 'checker'], 'plan-phase.md');
    assert.strictEqual(errors.length, 1, 'expected exactly 1 error for missing planner');
    assert.match(errors[0], /planner.*ecl-planner/);
  });

  test('fails when declared role is unknown (not in ROLE_TO_AGENT)', () => {
    const content = 'ecl-executor ecl-verifier';
...
describe('buildContract from real workflows', () => {
  test('produces a contract matching the inline LOOP_HOST_CONTRACT shape', () => {
    const contract = buildContract(); // reads real evolv-coder-lite/workflows/
...
describe('buildContract cross-check drift guard', () => {
  ...
  test('rejects a block declaring a role whose agent is absent from the workflow', () => {
    // plan-phase.md declares 'researcher' but does NOT mention ecl-phase-researcher
    const files = makeValidWorkflowFiles();
    files['plan-phase.md'] =
      '<!-- ecl:loop-host\n' +
      'step: plan\n' +
      ...
      'ecl-planner ecl-plan-checker\n';

    const tmpDir = makeTempWorkflowsDir(files);
    try {
      assert.throws(
        () => buildContract(tmpDir),
        /ecl-phase-researcher.*not referenced|researcher.*ecl-phase-researcher/,
      );
    } finally {
      cleanup(tmpDir);
    }
...
describe('parseLoopHostBlock multiple-block and duplicate-key detection (FIX 2)', () => {
  test('throws when a file has two ecl:loop-host marker blocks', () => {
    const block =
      '<!-- ecl:loop-host\n' +
      'step: discuss\n' +
    ...
    assert.throws(
      ...
      /expected exactly one ecl:loop-host marker block, found 2/,
    );
  });
  ...
  test('throws when a block has a duplicate "points" key', () => {
    ...
    assert.throws(
      () => parseLoopHostBlock(content, 'discuss-phase.md'),
      /duplicate key 'points' in ecl:loop-host marker/,
    );
  });
  ...
  test('throws when a block has a duplicate "step" key', () => {
    ...
    assert.throws(
      () => parseLoopHostBlock(content, 'discuss-phase.md'),
      /duplicate key 'step' in ecl:loop-host marker/,
    );
  });
...
describe('crossCheckRoles word-boundary match (FIX 3)', () => {
  test('ecl-plan-checker-v2 does NOT satisfy required ecl-plan-checker reference', () => {
    // Content has ecl-plan-checker-v2 but NOT bare ecl-plan-checker
    const content = 'Agent("ecl-phase-researcher") Agent("ecl-planner") ecl-plan-checker-v2';
    const errors = crossCheckRoles(content, ['researcher', 'planner', 'checker'], 'plan-phase.md');
    assert.strictEqual(errors.length, 1, 'expected exactly 1 error for checker missing bare reference');
    assert.match(errors[0], /ecl-plan-checker/);
  });

  test('ecl-plan-checker (bare) still satisfies the checker role', () => {
    const content = 'Agent("ecl-phase-researcher") Agent("ecl-planner") ecl-plan-checker something-else';
    const errors = crossCheckRoles(content, ['researcher', 'planner', 'checker'], 'plan-phase.md');
    assert.deepEqual(errors, []);
  });

  test('ecl-plan-checker immediately followed by newline satisfies the checker role', () => {
    const content = 'ecl-phase-researcher\necl-planner\necl-plan-checker\n';
    const errors = crossCheckRoles(content, ['researcher', 'planner', 'checker'], 'plan-phase.md');
    assert.deepEqual(errors, []);
  });
  ...
  test('buildContract rejects workflow referencing only -v2 agent variant', () => {
    const files = makeValidWorkflowFiles();
    // plan-phase.md refers to ecl-plan-checker-v2 but not ecl-plan-checker
    files['plan-phase.md'] =
      '<!-- ecl:loop-host\n' +
      'step: plan\n' +
      'points: plan:pre, plan:post\n' +
      ...
      'consumes: CONTEXT.md\n' +
      '-->\n' +
      'ecl-phase-researcher ecl-planner ecl-plan-checker-v2\n';
    const tmpDir = makeTempWorkflowsDir(files);
    try {
      assert.throws(
        () => buildContract(tmpDir),
        /ecl-plan-checker.*not referenced|checker.*ecl-plan-checker/,
      );
    } finally {
      cleanup(tmpDir);
    }
...
describe('temp-dir lifecycle', () => {
  test('makeTempWorkflowsDir creates a directory that can be cleaned up', () => {
    const files = { 'dummy.md': '<!-- ecl:loop-host\nstep: discuss\n-->' };
    const tmpDir = makeTempWorkflowsDir(files);
