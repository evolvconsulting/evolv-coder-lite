...
describe('plan-phase commit_docs support (#2399)', () => {
  test('plan-phase.md exists', () => {
    assert.ok(fs.existsSync(PLAN_PHASE_PATH), 'evolv-coder-lite/workflows/plan-phase.md must exist');
  });
  ...
  test('plan-phase.md uses ecl-sdk query commit for the plan commit', () => {
    const content = fs.readFileSync(PLAN_PHASE_PATH, 'utf-8');
    // Must use ecl-sdk query commit (not raw git) so commit_docs guard in ecl-tools is respected
    assert.ok(
      content.includes('ecl-sdk query commit') || content.includes('ecl-tools') || content.includes('ecl-sdk'),
      'plan-phase.md plan commit step must use ecl-sdk query commit (not raw git commit)'
    );
  });
});
