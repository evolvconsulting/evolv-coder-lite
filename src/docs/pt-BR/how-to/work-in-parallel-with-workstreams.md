...
**Pré-requisitos:** Um projeto eCL Core ativo (`.planning/ROADMAP.md` existe). Se não existir, execute `/ecl-new-project` primeiro.
...
Quando um workstream está ativo, todos os comandos eCL — `/ecl-progress`, `/ecl-discuss-phase`, `/ecl-plan-phase`, `/ecl-execute-phase` — leem e escrevem no diretório desse workstream. Alternar workstreams redireciona todos esses comandos para uma subárvore diferente sem tocar na árvore de código-fonte.
...
O eCL cria o diretório do workstream em `.planning/workstreams/backend-api/` e o inicializa com um `STATE.md` e `ROADMAP.md` esqueleto. O workstream não é ativado automaticamente — você precisa alternar para ele explicitamente.
...
A partir deste ponto, todos os comandos de fluxo de trabalho eCL operam no contexto `backend-api`. A alternância é vinculada à sessão: quando múltiplos terminais do Claude Code estão abertos no mesmo repositório, cada sessão pode ter um workstream ativo diferente sem interferir nos demais.
...
Isso ativa o workstream e restaura sua última posição conhecida dentro dele, equivalente a alternar e então executar `/ecl-resume-work`.
...
O eCL marca o workstream como arquivado e o remove da listagem ativa. Os artefatos de planejamento são preservados em `.planning/workstreams/backend-api/` para fins de auditoria.
...
- Você quer executar `/ecl-new-project` independentemente em cada workspace com uma raiz `.planning/` completamente separada, não um subdiretório do `.planning/` do repositório principal
