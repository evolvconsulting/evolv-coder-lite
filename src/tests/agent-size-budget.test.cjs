 ...
 * Agent size budget.
 *
 * Agent definitions in `agents/ecl-*.md` are loaded verbatim into Claude's
 ...
 * that belongs in `evolv-coder-lite/references/`.
 *
 ...
 *
 * See: https://github.com/evolvconsulting/evolv-coder-lite/issues/2361
 */
...
// XL ceiling lowered from 1600 → 1512 (actualMax=1452, ecl-debugger).
  ...
  for (const agent of ALL_AGENTS) {
    ...
    test(`${agent} (${tier}) stays under ${limit} lines`, () => {
      ...
      assert.ok(
        ...
        `Extract shared boilerplate to evolv-coder-lite/references/ or raise the budget ` +
