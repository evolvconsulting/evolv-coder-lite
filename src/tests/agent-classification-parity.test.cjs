 ...
 *   - ### ecl-<name> headings BEFORE "## Advanced and Specialized Agents" → "primary"
 *   - ### ecl-<name> headings INSIDE/AFTER that section → "advanced stub"
 *   - agents/ecl-*.md files with NO heading in AGENTS.md → "inventory only"
 */
 ...
 *   primaryHeadings  — agent names (without "ecl-" prefix) before the advanced section
 *   advancedHeadings — agent names inside "## Advanced and Specialized Agents"
 *
 * We work with the full "ecl-<name>" slug so the names are unambiguous.
 *
 ...
 * ### ecl-* headings are counted: primary if 'before', advanced if 'advanced',
 * ignored if 'past'.
 */
 ...
 * happens to contain a "| ecl-..." row from polluting the result.
 *
...
function parseInventoryMd(raw) {
  ...
  for (const line of lines) {
    ...
    // Data rows: first cell must be a ecl-* slug
    if (!cells[0].startsWith('ecl-')) continue;
...
/**
 * List all agents/ecl-*.md basenames (without .md extension).
 */
function listAgentFiles() {
  return fs
    .readdirSync(AGENTS_DIR)
    .filter((f) => /^ecl-.*\.md$/.test(f))
    .map((f) => f.replace(/\.md$/, ''))
    .sort();
}
...
assert.ok(
  ...
  'AGENTS.md: no ### ecl-* headings found before Advanced section — raw head:\n' + rawAgentsMd.slice(0, 300),
);

assert.ok(
  advancedHeadings.length > 0,
  'AGENTS.md: no ### ecl-* headings found inside Advanced section — raw head:\n' + rawAgentsMd.slice(0, 300),
);

assert.ok(
  inventoryMap.size > 0,
  'INVENTORY.md: no | ecl-* | rows parsed — raw head:\n' + rawInventoryMd.slice(0, 300),
);

assert.ok(
  agentFiles.length > 0,
  'agents/: no ecl-*.md files found — check AGENTS_DIR path: ' + AGENTS_DIR,
);
...
describe('agent-classification-parity: AGENTS.md section structure is the single source of truth', () => {
   ...
   *      (slugs without the "ecl-" prefix, as the prose uses)
   */
  test('AGENTS.md prose counts and advanced-agent parenthetical list are accurate', () => {
    ...
    assert.strictEqual(
      ...
      `AGENTS.md prose says "${prosePrimaryCount} primary agents" but there are ${primaryHeadings.length} ### ecl-* headings before the Advanced section`,
    );
    ...
    assert.strictEqual(
      ...
      `AGENTS.md prose says "${advancedCountRaw} additional shipped agents" but there are ${advancedHeadings.length} ### ecl-* headings in the Advanced section`,
    );

    // --- (c) parenthetical slug list ---
    // The prose lists short slugs without "ecl-" prefix, e.g.:
    ...
    // Derive expected slugs from AGENTS.md headings (strip "ecl-" prefix)
    const expectedSlugs = new Set(advancedHeadings.map((h) => h.replace(/^ecl-/, '')));
   ...
   *
   * Sub-checks:
   *   a) Every agents/ecl-*.md appears exactly once in (primary ∪ advanced ∪ inventoryOnly)
  ...
  test('Roster completeness: every agent file is classified exactly once', () => {
    ...
    assert.deepStrictEqual(
      ...
      [],
      'agents/ecl-*.md files missing from INVENTORY.md: ' + JSON.stringify(missingFromInventory),
    );
    ...
    assert.deepStrictEqual(
      phantomRows,
      [],
      'INVENTORY.md rows reference agents that have no agents/ecl-*.md file: ' + JSON.stringify(phantomRows),
    );
