 ...
 * the YAML frontmatter override format, and is referenced by ecl-verifier.md.
 */
...
describe('verification overrides reference (#1747)', () => {
  ...
  describe('evolv-coder-lite/references/verification-overrides.md', () => {
    const refPath = path.join(ROOT, 'evolv-coder-lite', 'references', 'verification-overrides.md');
    let content;
    ...
    test('documents milestone audit surfacing', () => {
      content = content || fs.readFileSync(refPath, 'utf-8');
      assert.ok(
        content.includes('ecl-audit-milestone') || content.includes('audit-milestone') || content.includes('milestone'),
  ...
  describe('agents/ecl-verifier.md references overrides', () => {
    const verifierPath = path.join(ROOT, 'agents', 'ecl-verifier.md');
    let verifierContent;

    test('ecl-verifier.md exists', () => {
      assert.ok(fs.existsSync(verifierPath), 'ecl-verifier.md should exist');
    ...
    test('references verification-overrides.md in required_reading', () => {
      ...
      assert.ok(
        ...
        'ecl-verifier.md should reference verification-overrides.md'
      );
    });
    ...
    test('verifier includes Step 3b for override check before FAIL', () => {
      ...
      assert.ok(
        ...
        'ecl-verifier.md should include a Step 3b override check'
      );
    });
    ...
    test('verifier frontmatter template includes overrides_applied', () => {
      ...
      assert.ok(
        ...
        'ecl-verifier.md frontmatter template should include overrides_applied counter'
      );
    });
  });
});
