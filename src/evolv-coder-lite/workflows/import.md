...
- **--from**: Import external plan → conflict detection → write PLAN.md → validate via ecl-plan-checker
...
- If `--prd` is present: display message that `--prd` is not yet implemented and exit:
  ```
  eCL > --prd mode is planned for a future release. Use --from to import plan files.
  ```
- If neither flag is found: display usage and exit:

```
Usage: /ecl:import --from <path>

  --from <path>   Import an external plan file into eCL format
```
...
2. Read `.planning/PROJECT.md` — extract project constraints, tech stack, scope boundaries.
   **If PROJECT.md does not exist:** skip constraint checks that rely on it and display:
   ```
   eCL > Note: No PROJECT.md found. Conflict checks against project constraints will be skipped.
   ```
...
- **eCL PLAN.md format**: Has YAML frontmatter with `phase:`, `plan:`, `type:` fields
...
Convert the imported content to eCL PLAN.md format.
...
If the imported plan references PBR plan naming (e.g., `PLAN-01.md`, `plan-01.md`), rename all references to eCL `{NN}-{MM}-PLAN.md` convention during conversion.

Apply eCL naming convention for the output filename:
...
INIT=$($ECL_SDK query init.phase-op "{NN}")
...
Agent({
  ...
  prompt: "Validate: .planning/phases/{phase}/{plan}-PLAN.md — check frontmatter completeness, task structure, and eCL conventions. Report any issues."
})
```
...
```bash
$ECL_SDK query commit "docs({phase}): import plan from {basename FILEPATH}" --files .planning/phases/{phase}/{plan}-PLAN.md .planning/ROADMAP.md
```
...
- Use `pbr:plan-checker` or `pbr:planner` — use `ecl-plan-checker` and `ecl-planner`
- Write `.planning/.active-skill` — this is a PBR pattern with no eCL equivalent
