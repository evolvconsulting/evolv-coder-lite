'use strict';
/**
 * Regression test for bug #891: ecl_run launcher must probe non-Claude
 * runtime homes before emitting the hard error.
 *
 * The last-resort $HOME/.claude/evolv-coder-lite branch is Claude Code-specific.
 * Every non-Claude runtime (Hermes, Cursor, Codex, Copilot, Windsurf, …)
 * installs evolv-coder-lite into a *different* directory that the shim never tried,
 * causing a false-positive fatal ERROR on all non-Claude runtimes when
 * RUNTIME_DIR is not set and ecl-tools is not on PATH.
 *
 * Asserts:
 * (A) Snippet contains all expected non-Claude runtime home probes (structural).
 * (B) HERMES_HOME behavioral: when RUNTIME_DIR misses and ecl-tools is NOT on
 *     PATH, a stub at ${HERMES_HOME}/evolv-coder-lite/bin/ecl-tools.cjs is invoked.
 * (C) Default Hermes path behavioral: stub at $HOME/.hermes/evolv-coder-lite/bin/
 *     ecl-tools.cjs is invoked when HERMES_HOME is not set.
 * (D) Resolution order: non-Claude homes are probed BEFORE the hard error,
 *     and AFTER the $HOME/.claude branch.
 * (E) Propagation: all workflow .md files using ecl_run contain each probe
...
// fallback arms in the ecl_run launcher snippet -- asserts literal substring
...
// evolv-coder-lite install location). Mirrors src/runtime-homes.cts getGlobalConfigDir().
const EXPECTED_RUNTIME_PROBES = {
  hermes:      '.hermes}/evolv-coder-lite/bin/',
  cursor:      '.cursor}/evolv-coder-lite/bin/',
  codex:       '.codex}/evolv-coder-lite/bin/',
  gemini:      '.gemini}/evolv-coder-lite/bin/',
  copilot:     '.copilot}/evolv-coder-lite/bin/',
  windsurf:    '.codeium/windsurf}/evolv-coder-lite/bin/',
  augment:     '.augment}/evolv-coder-lite/bin/',
  trae:        '.trae}/evolv-coder-lite/bin/',
  qwen:        '.qwen}/evolv-coder-lite/bin/',
  codebuddy:   '.codebuddy}/evolv-coder-lite/bin/',
  cline:       '.cline}/evolv-coder-lite/bin/',
  grok:        '.agents}/evolv-coder-lite/bin/',
  antigravity: '.gemini/antigravity}/evolv-coder-lite/bin/',
  opencode:    'opencode}/evolv-coder-lite/bin/',
  kilo:        'kilo}/evolv-coder-lite/bin/',
};
...
/**
 * Build a PATH with no ecl-tools binary so the PATH fallback branch is skipped,
 * while guaranteeing that a bare `node` lookup still resolves regardless of whether
 * the real node binary co-locates with a global ecl-tools shim (e.g. fnm/nvm/Homebrew).
 *
 * Strategy (POSIX only): create a temp dir containing only a `node` symlink →
 * process.execPath, prepend it to the ecl-tools-filtered PATH.  The filtered
 * PATH excludes any directory that contains an executable `ecl-tools`.
 *
 * On Windows the co-location bug does not apply (ecl-tools resolves via .cmd/.ps1,
...
function buildIsolatedPath() {
  ...
  const nodeBinDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-891-node-'));
  try {
...
describe('bug-891: non-Claude runtime home fallback arms', () => {
  ...
  test('(A) snippet contains all non-Claude runtime home probes', () => {
    ...
    assert.deepStrictEqual(
      ...
      `_runtime-launcher.snippet.sh is missing fallback probes for non-Claude runtimes:\n` +
        missing.join('\n') +
        `\n\nAdd elif arms for each runtime home (e.g. "\${HERMES_HOME:-$HOME/.hermes}/evolv-coder-lite/bin/...")` +
        ` before the hard-error else. Current snippet:\n${snippetContent.trim()}`,
    );
  });

  // ── (A2) Structural: probes appear AFTER .claude arm but BEFORE hard error ─
  test('(A2) non-Claude probes appear after .claude/evolv-coder-lite arm and before hard error', () => {
    const snippetContent = fs.readFileSync(SNIPPET_FILE, 'utf8');
    const claudePos = snippetContent.indexOf('.claude/evolv-coder-lite/bin/');
    const errorPos  = snippetContent.indexOf('exit 1');

    assert.ok(claudePos !== -1, 'Snippet must still contain .claude/evolv-coder-lite/bin/ arm (regression guard)');
  ...
  //        ecl-tools co-locate in the same PATH directory.                     ─
  //
  // Machine-independence guarantee: PATH is set to ONLY two controlled dirs —
  // fakeBinDir (holds both fake ecl-tools AND a node symlink) plus a fresh
  ...
  test(
    '(B0) buildIsolatedPath: node is resolvable and ecl-tools is not when they share a PATH dir',
    { skip: process.platform === 'win32' ? 'POSIX-only co-location scenario' : false },
    (t) => {
      // Build a fake bin dir that contains BOTH a ecl-tools executable and a node
      // symlink, simulating a dev setup (fnm/nvm/Homebrew) where both land in the
      // same bin directory.
      const fakeBinDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-891-colocated-'));
      // A second fresh empty dir — contains neither ecl-tools nor node.
      const emptyDir   = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-891-empty-'));
      t.after(() => cleanup(fakeBinDir));
      t.after(() => cleanup(emptyDir));

      // Fake ecl-tools shim (executable file)
      const fakeGsdTools = path.join(fakeBinDir, 'ecl-tools');
      fs.writeFileSync(fakeGsdTools, '#!/bin/sh\necho fake-ecl-tools\n');
      fs.chmodSync(fakeGsdTools, 0o755);

      // node symlink pointing at the real interpreter (co-located with ecl-tools)
      ...
      // (i) ecl-tools must NOT be resolvable on the returned PATH
      const gsdToolsResolvable = returnedDirs.some((dir) => {
        try { fs.accessSync(path.join(dir, 'ecl-tools'), fs.constants.X_OK); return true; }
        catch { return false; }
      });
      assert.equal(
        gsdToolsResolvable,
        false,
        'ecl-tools must not be resolvable on the isolated PATH (home-fallback would be bypassed)',
      );
      ...
      assert.equal(
        nodeResolvable,
        true,
        'node must be resolvable on the isolated PATH (launcher runs: node "$ECL_TOOLS" "$@")',
      );
    },
  );

  // ── (B) Behavioral: HERMES_HOME stub is resolved ──────────────────────────
  test('(B) ecl_run resolves ${HERMES_HOME}/evolv-coder-lite/bin/ stub when set and local+PATH both miss', () => {
    const fakeHome       = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-891-home-b-'));
    const fakeHermesHome = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-891-hermes-'));
    const fakeRuntime    = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-891-rt-'));
    const { isolatedPath, nodeBinDir } = buildIsolatedPath();
    try {
      const hermesBinDir = path.join(fakeHermesHome, 'evolv-coder-lite', 'bin');
      fs.mkdirSync(hermesBinDir, { recursive: true });

      const stubPath = path.join(hermesBinDir, 'ecl-tools.cjs');
      fs.writeFileSync(
        stubPath,
      ...
      assert.ok(
        normStdout.includes('evolv-coder-lite/bin/'),
        `Expected ECL_TOOLS to resolve into hermes evolv-coder-lite/bin/, got:\n${stdout.trim()}`,
      );
      assert.ok(
  ...
  test('(C) ecl_run resolves $HOME/.hermes/evolv-coder-lite/bin/ stub when HERMES_HOME is unset', () => {
    const fakeHome    = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-891-home-'));
    const fakeRuntime = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-891-rt2-'));
    const { isolatedPath, nodeBinDir } = buildIsolatedPath();
    try {
      const hermesBinDir = path.join(fakeHome, '.hermes', 'evolv-coder-lite', 'bin');
      fs.mkdirSync(hermesBinDir, { recursive: true });
      ...
      const normStdout = stdout.replace(/\\/g, '/');
      assert.ok(
        normStdout.includes('.hermes/evolv-coder-lite/bin/'),
        `Expected ECL_TOOLS to resolve into .hermes/evolv-coder-lite/bin/, got:\n${stdout.trim()}`,
      );
      assert.ok(
  ...
  test('(D) resolution order: .claude probe comes before hermes probe, hermes before hard error', () => {
    const snippetContent = fs.readFileSync(SNIPPET_FILE, 'utf8');
    const claudePos  = snippetContent.indexOf('.claude/evolv-coder-lite/bin/');
    const hermesPos  = snippetContent.indexOf('.hermes}/evolv-coder-lite/bin/');
    const errorPos   = snippetContent.indexOf('exit 1');

    assert.ok(claudePos  !== -1, 'Snippet must contain .claude/evolv-coder-lite/bin/ arm');
    assert.ok(hermesPos  !== -1, 'Snippet must contain .hermes}/evolv-coder-lite/bin/ arm');
  ...
  // ── (E) Propagation: workflow .md files using ecl_run contain hermes probe ─
  test('(E) all workflow .md files using ecl_run contain the hermes runtime home probe', () => {
    const HERMES_PROBE = '.hermes}/evolv-coder-lite/bin/';
    ...
    assert.deepStrictEqual(
      missing,
      [],
      `These workflow files use ecl_run but are missing the hermes runtime home probe ("${HERMES_PROBE}"). ` +
