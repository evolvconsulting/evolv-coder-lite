...
describe('orphaned hooks stale detection (#1750)', () => {
  test('stale hook scanner uses an allowlist of managed hooks, not a wildcard', () => {
    ...
    // The scanner MUST NOT use a broad `startsWith('ecl-')` filter that catches
    // orphaned files from removed features (ecl-intel-index.js, ecl-intel-prune.js, etc.)
    // Instead, it should reference a known set of managed hook filenames.
    const hasBroadFilter = /readdirSync\([^)]+\)\.filter\([^)]*startsWith\('ecl-'\)\s*&&[^)]*endsWith\('\.js'\)/s.test(content);
    assert.ok(!hasBroadFilter,
      'scanner must NOT use broad startsWith("ecl-") && endsWith(".js") filter — ' +
      'this catches orphaned hooks from removed features (e.g., ecl-intel-index.js). ' +
