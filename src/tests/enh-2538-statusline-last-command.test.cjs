 ...
 *     <command-name>/ecl-plan-phase</command-name>, output includes "last: /ecl-plan-phase"
 ...
 *   - the config key is registered in the schema so /ecl-settings can surface it
 */
...
const statusline = require('../hooks/ecl-statusline.js');
...
test('default (flag absent) output has no "last:" suffix', () => {
  const transcript =
    JSON.stringify({ type: 'user', message: { content: '<command-name>/ecl-plan-phase</command-name>' } }) + '\n';
...
test('flag=true with recorded command yields "last: /<cmd>"', () => {
  ...
  try {
    const out = statusline.renderStatusline(buildInput(dir, transcriptPath));
    assert.ok(out.includes('last: /ecl-plan-phase'), `expected "last: /ecl-plan-phase" in output; got: ${out}`);
  } finally {
    cleanup();
  }
});
...
test('flag=true picks the MOST RECENT command when multiple are present', () => {
  const transcript =
    JSON.stringify({ type: 'user', message: { content: '<command-name>/ecl-discuss-phase</command-name>' } }) + '\n' +
    JSON.stringify({ type: 'user', message: { content: '<command-name>/ecl-plan-phase</command-name>' } }) + '\n' +
    JSON.stringify({ type: 'user', message: { content: '<command-name>/ecl-execute-phase</command-name>' } }) + '\n';
  const { dir, transcriptPath, cleanup } = makeProject({ flag: true, transcript });
  try {
    const out = statusline.renderStatusline(buildInput(dir, transcriptPath));
    assert.ok(out.includes('last: /ecl-execute-phase'), `expected most-recent "ecl-execute-phase"; got: ${out}`);
    assert.ok(!out.includes('last: /ecl-discuss-phase'), `should not show stale command; got: ${out}`);
  } finally {
    cleanup();
  }
});
