...
describe('release-tarball-smoke', () => {
  ...
  before(async () => {
    ...
    packDir = createTempDir('ecl-smoke-pack-');
    installPrefix = createTempDir('ecl-smoke-prefix-');
    fixtureDir = createTempDir('ecl-smoke-fixture-');
  ...
  // Also verifies that `evolv-coder-lite --local --claude` (init) succeeds in
  ...
  // Verifies that `ecl-sdk query state.json` exits 0 and returns parseable JSON.
  // This is the primary guard for SDK_BINARY_NOT_CALLABLE regressions.
  test('E: sdk binary callable — ecl-sdk query produces parseable JSON', () => {
    const result = runSmoke({
      tarballPath,
      installPrefix,
      expectedVersion: pkg.version,
