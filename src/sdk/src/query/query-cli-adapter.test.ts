...
const resolveGsdToolsPathSeamSpy = vi.hoisted(() => vi.fn(() => '/mock/ecl-tools.cjs'));
...
vi.mock('../query-ecl-tools-path.js', () => ({
...
describe('query-cli-adapter', () => {
  beforeEach(() => {
    ...
    resolveGsdToolsPathSeamSpy.mockReturnValue('/mock/ecl-tools.cjs');
  });
  ...
  it('wires resolveGsdToolsPath from the query seam module', async () => {
    runQueryDispatchSpy.mockImplementationOnce(async (input: any) => {
      ...
      expect(input.resolveGsdToolsPath('/tmp/project')).toBe('/mock/ecl-tools.cjs');
