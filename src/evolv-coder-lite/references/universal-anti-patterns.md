...
10. **NEVER** use non-eCL agent types (`general-purpose`, `Explore`, `Plan`, `Bash`, `feature-dev`, etc.) -- ALWAYS use `subagent_type: "ecl-{agent}"` (e.g., `ecl-phase-researcher`, `ecl-executor`, `ecl-planner`). eCL agents have project-aware prompts, audit logging, and workflow context. Generic agents bypass all of this.
...
15. **No direct Write/Edit to STATE.md or ROADMAP.md for mutations.** Always use `ecl-tools query` for registered state/roadmap handlers (e.g. `state.update`, `state.advance-plan`, `roadmap.update-plan-progress`), or legacy `node …/ecl-tools.cjs` for CLI-only commands. Direct Write tool usage bypasses safe update logic and is unsafe in multi-session environments. Exception: first-time creation of STATE.md from template is allowed.
...
24. **Do not** check for `mode === 'auto'` or `mode === 'autonomous'` -- eCL uses `yolo` config flag. Check `yolo: true` for autonomous mode, absence or `false` for interactive mode.
25. **Prefer `ecl-tools query`** for orchestration when a handler exists; when shelling out to the legacy CLI, use **`ecl-tools.cjs`** (not `ecl-tools.js` or any other filename) — eCL ships the programmatic API as CommonJS for Node.js CLI compatibility.
26. **Plan files MUST follow `{padded_phase}-{NN}-PLAN.md` pattern** (e.g., `01-01-PLAN.md`). Never use `PLAN-01.md`, `plan-01.md`, or any other variation -- ecl-tools detection depends on this exact pattern.
