 ...
 * `~/.ecl/defaults.json`. As a result, the configured per-agent model was
...
const makeTmp = (prefix) => createTempDir(`ecl-2256-${prefix}-`);
...
describe('bug #2256 — readGsdEffectiveModelOverrides', () => {
  ...
  test('reads overrides from ~/.ecl/defaults.json (global only)', () => {
    writeJson(path.join(homeDir, '.ecl', 'defaults.json'), {
      model_overrides: { 'ecl-codebase-mapper': 'gpt-5-mini' },
    });
    const result = readGsdEffectiveModelOverrides(projectDir);
    assert.deepStrictEqual(result, { 'ecl-codebase-mapper': 'gpt-5-mini' });
  });

  test('reads overrides from per-project .planning/config.json', () => {
    writeJson(path.join(projectDir, '.planning', 'config.json'), {
      model_overrides: { 'ecl-codebase-mapper': 'claude-haiku-4-5' },
    });
    const result = readGsdEffectiveModelOverrides(projectDir);
    assert.deepStrictEqual(result, { 'ecl-codebase-mapper': 'claude-haiku-4-5' });
  });

  test('per-project overrides win over global on conflict', () => {
    writeJson(path.join(homeDir, '.ecl', 'defaults.json'), {
      model_overrides: { 'ecl-codebase-mapper': 'global-model', 'ecl-planner': 'opus' },
    });
    writeJson(path.join(projectDir, '.planning', 'config.json'), {
      model_overrides: { 'ecl-codebase-mapper': 'project-model' },
    });
    const result = readGsdEffectiveModelOverrides(projectDir);
    // Per-project wins on conflict; non-conflicting global keys are preserved.
    assert.deepStrictEqual(result, {
      'ecl-codebase-mapper': 'project-model',
      'ecl-planner': 'opus',
    });
  });
...
describe('bug #2256 — Codex adapter embeds per-project override', () => {
  ...
  test('generateCodexAgentToml embeds model when override provided', () => {
    const toml = generateCodexAgentToml(
      'ecl-codebase-mapper',
      agentContent,
      { 'ecl-codebase-mapper': 'gpt-5-mini' },
    );
    assert.match(toml, /^model = "gpt-5-mini"$/m);
  });

  test('generateCodexAgentToml omits model when no override', () => {
    const toml = generateCodexAgentToml('ecl-codebase-mapper', agentContent, null);
...
describe('bug #2256 — Codex skill adapter header documents transport', () => {
  test('Task(model=...) line no longer says "omit" without explanation', () => {
    const header = getCodexSkillAdapterHeader('ecl-plan-phase');
