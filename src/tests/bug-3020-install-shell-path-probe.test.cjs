 ...
 *
 * The installer prints `✓ eCL SDK ready (sdk/dist/cli.js)` whenever
 ...
 *
 *   - Windows cross-shell: ecl-sdk.cmd resolves under PowerShell/cmd
 *     (PATHEXT) but bare `ecl-sdk` does not resolve under Git Bash /
 *     MSYS / WSL bash.
...
describe('bug #3020: isGsdSdkOnPath accepts an explicit PATH string', () => {
  ...
  test('returns true when an executable ecl-sdk exists in the supplied PATH', () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-3020-'));
    try {
      // Create a fake `ecl-sdk` shim with the executable bit set.
  ...
  test('returns false when the supplied PATH has no ecl-sdk', () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-3020-'));
    try {
      const result = isGsdSdkOnPath(tmp);
...
describe('bug #3020: cross-shell PATH mismatch is detectable via the new helpers', () => {
  test('install-time PATH has shim, user-shell PATH does not → mismatch detected', () => {
    const installDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-3020-install-'));
    const userDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-3020-user-'));
    try {
      const shimName = process.platform === 'win32' ? 'ecl-sdk.cmd' : 'ecl-sdk';
