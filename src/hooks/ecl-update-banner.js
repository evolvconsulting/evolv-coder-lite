...
// SessionStart banner that surfaces eCL update availability when eCL's
// statusline isn't installed. Reads the cache that
// ecl-check-update-worker.js writes to ~/.cache/ecl/<updateCacheFileName> (per-package).
//
// Opt-in by design: bin/install.js only registers this hook when the user
// declines to install (or replace) the eCL statusline. The presence of the
...
function buildBannerOutput(state) {
  ...
  if (parseError) {
    ...
    return { systemMessage: 'eCL update check failed.' };
  }
  ...
  return {
    systemMessage: `eCL update available: ${installed} → ${latest}. Run /ecl:update.`,
  };
}
...
function main() {
  const cacheDir = path.join(os.homedir(), '.cache', 'ecl');
  ...
  if (parseError && !suppressFailureWarning) {
    ...
    // where ~/.cache/ecl was created by check-update but the parent dir got
    // wiped between runs.
    try {
