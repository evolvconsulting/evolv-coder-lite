...
const WORKFLOW = path.join(__dirname, '..', 'evolv-coder-lite', 'workflows', 'mvp-phase.md');
...
function parseMvpPhaseContract(content) {
  ...
  const planPhaseStepIndex = lowerLines.findIndex(line => line.includes('## 7. delegate to /ecl plan-phase'));
...
describe('mvp-phase workflow', () => {
  ...
  test('delegates to /ecl plan-phase after ROADMAP write', () => {
