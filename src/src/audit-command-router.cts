'use strict';
/**
 * Audit command routers — CLI dispatchers for `ecl-tools audit-uat` and
 * `ecl-tools audit-open`.
 *
 * ADR-959 (phase 4d-impl-3): audit command family cutover.
 * Extracted from the hardcoded `case 'audit-uat':` and `case 'audit-open':`
 * arms in ecl-tools.cjs.  Behaviour is preserved byte-for-behaviour from the
