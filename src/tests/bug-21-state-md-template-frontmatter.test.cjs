 ...
 * without `ecl_state_version`, `status`, or `progress` keys.
 *
 * Acceptance criteria:
 * 1. The template body extracted from each state.md file's File Template code
 *    block must begin with `---`.
 * 2. The frontmatter must contain at minimum: `ecl_state_version` and `status`.
 */
...
const TEMPLATE_PATHS = [
  path.join(REPO_ROOT, 'evolv-coder-lite', 'templates', 'state.md'),
...
describe('bug #21 — STATE.md template must carry YAML frontmatter', () => {
  for (const templatePath of TEMPLATE_PATHS) {
    ...
    test(`${label} — frontmatter contains ecl_state_version`, () => {
      const content = fs.readFileSync(templatePath, 'utf-8');
      const body = extractFileTemplate(content);
      const keys = parseFrontmatterKeys(body.trimStart());

      assert.ok(
        keys.has('ecl_state_version'),
        `${label}: frontmatter must include 'ecl_state_version', found keys: ${[...keys].join(', ')}`,
      );
    });
  ...
  test('both templates produce byte-equal File Template blocks', () => {
    ...
    assert.strictEqual(
      bodyA,
      bodyB,
      'File Template code blocks in evolv-coder-lite/templates/state.md and ' +
