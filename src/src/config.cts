 ...
 *   2. User-level defaults from ~/.ecl/defaults.json (if present)
 *   3. userChoices — the settings the user explicitly selected during /ecl:new-project
 *
 ...
 * configured during /ecl:new-project). All remaining keys are filled from
 * hardcoded defaults and optional ~/.ecl/defaults.json.
 *
 ...
 * users can run `ecl-tools migrate-config` to apply all four legacy-key
...
function cmdMigrateConfig(cwd: string, raw: boolean): void {
  const ws = process.env['ECL_WORKSTREAM'] || null;
