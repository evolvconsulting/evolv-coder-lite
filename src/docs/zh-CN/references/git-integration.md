...
如果 NO_GIT：静默运行 `git init`。eCL 项目总是有自己的仓库。
</git_check>
...
```bash
ecl-tools.cjs query commit "docs: initialize [project-name] ([N] phases)" --files .planning/
```
...
```bash
ecl-tools.cjs query commit "docs({phase}-{plan}): complete [plan-name] plan" --files .planning/phases/XX-name/{phase}-{plan}-PLAN.md .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/STATE.md .planning/ROADMAP.md
```
...
```bash
ecl-tools.cjs query commit "wip: [phase-name] paused at task [X]/[Y]" --files .planning/
```
