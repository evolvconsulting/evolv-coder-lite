...
describe('#3427 + #3433 — Codex installer avoids duplicate skills and mixed hook representation', { concurrency: false }, () => {
  ...
  beforeEach(() => {
    ...
    tmpRoot = createTempDir('ecl-3427-3433-');
  ...
  test('regenerates managed ecl-* skill copies and preserves unrelated user skills (#3562 reverses prior #3427/#3433 behaviour)', () => {
    ...
    fs.mkdirSync(path.join(codexHome, 'skills', 'ecl-help'), { recursive: true });
    fs.writeFileSync(path.join(codexHome, 'skills', 'ecl-help', 'SKILL.md'), legacySkillBody);
    const legacyHash = crypto.createHash('sha256').update(legacySkillBody).digest('hex');
    fs.writeFileSync(path.join(codexHome, 'ecl-file-manifest.json'), JSON.stringify({
      version: 1,
      files: {
        'skills/ecl-help/SKILL.md': legacyHash,
      },
    }, null, 2));
    ...
    // #3562: $ecl-* commands are discoverable only when skills/ecl-*/SKILL.md
    // exists. The installer must regenerate (not remove) the managed ecl-*
    // directories.
    assert.equal(entries.includes('ecl-help'), true);
    const refreshedBody = fs.readFileSync(path.join(skillsDir, 'ecl-help', 'SKILL.md'), 'utf8');
    ...
    assert.equal(frontmatter.name, 'ecl-help', 'refreshed SKILL.md frontmatter must declare name: ecl-help');

    // Unrelated user skills are preserved — the regen scope is `ecl-*` only.
    assert.equal(entries.includes('custom-user-skill'), true);
  });

  test('stores managed SessionStart update hook in hooks.json and removes inline ecl hook from config.toml', () => {
    const configToml = [
      '[features]',
      'codex_hooks = true',
      '',
      ...
      'type = "command"',
      'command = "node /tmp/legacy/.codex/hooks/ecl-check-update.js"',
      '',
    ].join('\n');
    ...
    assert.equal(tomlCommands.some((cmd) => cmd.includes('ecl-check-update.js')), false);
    ...
    const gsdCommands = sessionStartCommands.filter((cmd) => cmd.includes('ecl-check-update'));
  ...
  test('uninstall removes managed SessionStart hook from hooks.json but preserves user hooks', () => {
    ...
    fs.writeFileSync(path.join(hooksDir, 'ecl-check-update.js'), '// managed hook\n');
    const managedHookPath = path.join(codexHome, 'hooks', 'ecl-check-update.js').replace(/\\/g, '/');
