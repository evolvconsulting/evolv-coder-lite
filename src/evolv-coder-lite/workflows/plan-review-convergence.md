...
CONVERGENCE_ENABLED=$($ECL_SDK query config-get workflow.plan_review_convergence 2>/dev/null || echo "false")
```
...
Agent(
  ...
  prompt="Run /ecl:plan-phase for Phase {PHASE}.

Execute: Skill(skill='ecl-plan-phase', args='{PHASE} {ECL_WS}')
...
Agent(
  ...
  prompt="Run /ecl:plan-phase with --reviews for Phase {PHASE}.

Execute: Skill(skill='ecl-plan-phase', args='{PHASE} --reviews --skip-research {ECL_WS}')
...
IMPORTANT: When ecl-plan-phase outputs '## PLANNING COMPLETE', that means replanning is done. Return at that point.",
  mode="auto"
)
```
...
- [ ] Initial planning via Agent → Skill("ecl-plan-phase") if no plans exist
- [ ] Review via Agent → Skill("ecl-review") — isolated, not inline; {ECL_WS} forwarded
- [ ] Replan via Agent → Skill("ecl-plan-phase --reviews") — isolated, not inline
