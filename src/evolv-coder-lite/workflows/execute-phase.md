...
- **Claude Code:** Uses `Agent(subagent_type="ecl-executor", ...)` — blocks until complete, returns result
...
- **Other runtimes:** If `Agent`/`agent` tool is genuinely unavailable (e.g. a backgrounded
  ...
  Code), you MUST spawn ecl-executor agents — inline execution is not authorized. Check for
...
@~/.claude/evolv-coder-lite/references/context-budget.md
@~/.claude/evolv-coder-lite/references/gates.md
</required_reading>

<available_agent_types>
These are the valid eCL subagent types registered in .claude/agents/ (or equivalent for your runtime).
Always use the exact name from this list — do not fall back to 'general-purpose' or other built-in types:

- ecl-executor — Executes plan tasks, commits, creates SUMMARY.md
- ecl-verifier — Verifies phase completion, checks quality gates
...
- ecl-debugger — Diagnoses and fixes issues
- ecl-codebase-mapper — Maps project structure and dependencies
- ecl-integration-checker — Checks cross-phase integration
- ecl-nyquist-auditor — Validates verification coverage
- ecl-ui-researcher — Researches UI/UX approaches
- ecl-ui-checker — Reviews UI implementation quality
- ecl-ui-auditor — Audits UI against design requirements
</available_agent_types>
...
- Executor agents omit extended deviation rule examples and checkpoint examples from inline prompt — load on-demand via @~/.claude/evolv-coder-lite/references/executor-examples.md
- Planner agents omit extended anti-pattern lists and specificity examples from inline prompt — load on-demand via @~/.claude/evolv-coder-lite/references/planner-antipatterns.md
...
Check if the current runtime is Copilot by testing for the `@ecl-executor` agent pattern
...
EXECUTE_POST_HOOKS_JSON=$(ecl_run loop render-hooks execute:post --raw)
TDD_MODE=$(ecl_run loop render-hooks execute:post --active-cap tdd)
```
...
2. For each plan (sequentially, ignoring wave grouping):
   ...
   c. **If "Execute":** Read and follow `~/.claude/evolv-coder-lite/workflows/execute-plan.md` **inline**
...
- Maintains eCL's planning/tracking structure
- Best for: small phases, bug fixes, verification gaps, learning eCL
...
```bash
DEFAULT_BRANCH=$(ecl_run query git.base-branch 2>/dev/null \
...
tooling and `/ecl:manager`'s background-completion handler can grep partial
   ...
   **Sequential dispatch for parallel execution (waves with 2+ agents):**
   ...
   Agent(
     ...
     prompt="
       ...
       <execution_context>
       @~/.claude/evolv-coder-lite/workflows/execute-plan.md
       @~/.claude/evolv-coder-lite/templates/summary.md
       @~/.claude/evolv-coder-lite/references/checkpoints.md
       @~/.claude/evolv-coder-lite/references/tdd.md
       @~/.claude/evolv-coder-lite/references/worktree-path-safety.md
       ${CONTEXT_WINDOW < 200000 ? '' : '@~/.claude/evolv-coder-lite/references/executor-examples.md'}
       </execution_context>
...
5. **Post-wave hook validation (parallel mode only):** Hooks run on every executor commit by default (#2924); this post-wave run only fires when `workflow.worktree_skip_hooks=true` opted out of per-commit hooks:
   ...
   if [ "$SKIP_HOOKS" = "true" ]; then
     ...
     STASHED=false
     if (! git diff --quiet || ! git diff --cached --quiet) && git stash push -u -m "ecl-post-wave-hook-$$" >/dev/null 2>&1; then STASHED=true; fi
     git hook run pre-commit 2>&1 || echo "⚠ Pre-commit hooks failed — review before continuing"
     [ "$STASHED" = "true" ] && (git stash pop >/dev/null 2>&1 || echo "⚠ Could not pop ecl-post-wave-hook stash — recover manually")
   fi
   ```
...
5.5. **Worktree cleanup (when `isolation="worktree"` was used):**
   ...
   **Cleanup-tail snippet (use after any wave whose merges did not flow through the templated path above):**
   ...
   WT_PATHS_FILE=$(mktemp "${TMPDIR:-/tmp}/ecl-worktree-paths-XXXXXX")
...
5.6. **Post-merge build & test gate:**
   ...
   Read and execute `evolv-coder-lite/workflows/execute-phase/steps/post-merge-gate.md`.
...
5.75. **Execute:wave:post capability dispatch:**
   ...
   ```bash
   WAVE_POST_HOOKS_JSON=$(ecl_run loop render-hooks execute:wave:post --raw)
   ```
   ...
   **For each active entry where `kind == "gate"`** (process in array order), run the gate check:
   ...
   ```bash
   GATE_RESULT=$(ecl_run check ${hook.check.query} "${PHASE_NUMBER}" --raw)
   CHECK_EXIT=$?
   ```
   ...
   **Step 2 — read `GATE_RESULT.block` (boolean).** This step is only reached when the command succeeded.
   ...
   - **Non-blocking gate (`hook.blocking == false`):** never halts. If `GATE_RESULT.block` is `true` (or non-empty `message`), print `⚠ {hook.capId} advisory (wave {N}): {GATE_RESULT.message}`, then:
     - If `GATE_RESULT.spawn_mapper == true` OR `GATE_RESULT.directive == "auto-remap"`: spawn `ecl-codebase-mapper` per `execute-phase/steps/codebase-drift-gate.md`; pass `--paths {GATE_RESULT.affected_paths}`. Continue regardless (wave NOT failed by remap failure).
...
7. **Handle failures:**
   ...
   **Step 7.1 — `class == "quota-exceeded"`:**
   ...
   ```
   Re-run `/ecl:execute-phase` after quota reset for Option 1.
...
**Security gate check:**
```bash
VERIFY_POST_HOOKS_JSON=$(ecl_run loop render-hooks verify:post --raw)
...
/ecl:execute-phase {phase} ${ECL_WS}                # Continue remaining waves
/ecl:execute-phase {phase} --wave {next} ${ECL_WS}  # Run the next wave explicitly
```
...
**Capability gate:**
```bash
EXECUTE_POST_HOOKS_JSON=${EXECUTE_POST_HOOKS_JSON:-$(ecl_run loop render-hooks execute:post --raw)}
```
...
**Invoke review:**
```
Skill(skill="ecl-${ref.skill}", args="${PHASE_NUMBER}")
```
...
/ecl:code-review ${PHASE_NUMBER} --fix
```
...
**TDD review escalation (overrides the advisory default for the `tdd.review-checkpoint` gate only).** The tdd `execute:post` gate is declared `blocking: false`, so by the generic contract above it displays its `message`/table and continues. There is ONE documented exception (see `~/.claude/evolv-coder-lite/references/execute-mvp-tdd.md`): when `MVP_MODE=true` AND `TDD_MODE=true` AND `GATE_RESULT.block == true` (one or more TDD plans miss a RED or GREEN gate commit), the end-of-phase TDD review escalates from advisory to **blocking under MVP+TDD** — refuse to mark the phase complete and present:

```
Phase blocked: {N} TDD plan(s) violate the RED→GREEN gate sequence under MVP+TDD.
Resolve and re-run /ecl execute-phase, or override with /ecl execute-phase {phase} --force-mvp-gate to ship anyway.
```
...
`/ecl:plan-phase {X} --gaps ${ECL_WS}`

Also: `cat {phase_dir}/{phase_num}-VERIFICATION.md` — full report
Also: `/ecl:verify-work {X} ${ECL_WS}` — manual testing first
```

Gap closure cycle: `/ecl:plan-phase {X} --gaps ${ECL_WS}` reads VERIFICATION.md → creates gap plans with `gap_closure: true` → user runs `/ecl:execute-phase {X} --gaps-only ${ECL_WS}` → verifier re-runs.
</step>
...
These items are tracked and will appear in `/ecl:progress` and `/ecl:audit-uat`.
```
...
entries from the completed phase to the global learnings store at `~/.ecl/knowledge/`.
...
**Exception:** If `gaps_found`, the `verify_phase_goal` step already presents the gap-closure path (`/ecl:plan-phase {X} --gaps`). No additional routing needed — skip auto-advance.
...
Read and follow `~/.claude/evolv-coder-lite/workflows/transition.md`, passing through the `--auto` flag so it propagates to the next phase invocation.

**If neither `--auto` nor `AUTO_MODE` is true:**

**STOP. Do not auto-advance. Do not execute transition. Do not plan next phase. Present options to the user and wait.**

**IMPORTANT: There is NO `/ecl-transition` command. Never suggest it. The transition workflow is internal only.**
...
/ecl:progress ${ECL_WS} — see updated roadmap
/ecl:discuss-phase {next} ${ECL_WS} — start here: discuss next phase before planning  ← recommended
/ecl:plan-phase {next} ${ECL_WS} — plan next phase (skip discuss)
/ecl:execute-phase {next} ${ECL_WS} — execute next phase (skip discuss and plan)
```
...
/ecl:progress ${ECL_WS} — see updated roadmap
/ecl:plan-phase {next} ${ECL_WS} — start here: plan next phase (CONTEXT.md already present)  ← recommended
/ecl:discuss-phase {next} ${ECL_WS} — re-discuss next phase
/ecl:execute-phase {next} ${ECL_WS} — execute next phase (skip planning)
```
...
<resumption>
Re-run `/ecl:execute-phase {phase}` → discover_plans finds completed SUMMARYs → skips them → resumes from first incomplete plan → continues wave execution.
