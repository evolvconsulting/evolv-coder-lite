...
`.planning/` 目录是 eCL Core 项目的共享记忆。所有工作流都会读取和写入该目录，并留下可审计的决策记录。本页列出每个文件、其用途，以及哪些命令负责生成或消费它。参见[文档索引](../README.md)。
...
| **生成者** | `/ecl-new-project`（初始创建）；由 `/ecl-complete-milestone` 在决策验证后更新。 |
| **消费者** | 所有规划工作流；`ecl-phase-researcher`、`ecl-planner`（上下文）；`discuss-phase`（历史决策）；`ecl-plan-checker`（项目约束）。 |
...
| **生成者** | `/ecl-new-project`（初始创建）；由 `/ecl-phase --insert` 和 `/ecl-complete-milestone` 更新。 |
| **消费者** | `/ecl-discuss-phase`、`/ecl-plan-phase`、`/ecl-execute-phase`；所有需要阶段信息的编排命令；`ecl-planner`、`ecl-plan-checker`、`ecl-phase-researcher`。 |
...
| **生成者** | `/ecl-new-project`（初始创建）；需求由 `execute-phase` 标记为已完成。 |
| **消费者** | `ecl-planner`（计划必须覆盖所有阶段需求 ID）；`ecl-plan-checker` 维度 1（需求覆盖）；`discuss-phase`（历史需求）。 |
...
| **生成者** | `/ecl-new-project`（初始创建）；由所有阶段工作流、`/ecl-pause-work`、`/ecl-resume-work` 持续更新。 |
| **消费者** | 所有编排工作流；`/ecl-progress`；通过 `/ecl-quick` 执行的临时任务；`ecl-planner` 和 `ecl-phase-researcher`（项目决策）。 |
...
| **生成者** | `/ecl-new-project`（初始创建）；`/ecl-settings`（交互式编辑）。 |
| **消费者** | 每个工作流和子代理——在初始化时通过 `ecl-tools query config-get` 读取。 |
...
| **生成者** | `/ecl-complete-milestone`。 |
| **消费者** | `/ecl-audit-milestone`；人工审查。 |
...
| **生成者** | `/ecl-discuss-phase`（交互式讨论或 PRD/ADR 快速路径）。 |
| **消费者** | `ecl-phase-researcher`（待调查内容）；`ecl-planner`（锁定决策）；`ecl-plan-checker` 维度 7（上下文合规性）。 |
...
| **生成者** | `/ecl-discuss-phase`（`git_commit` 步骤）。 |
| **消费者** | 人工审查；回顾总结。 |
...
| **生成者** | `/ecl-plan-phase` 通过 `ecl-phase-researcher` 代理。 |
| **消费者** | `ecl-planner`（规划输入）；`ecl-plan-checker` 维度 7c（层级合规性）、维度 8（Nyquist）、维度 11（研究解决）；`ecl-pattern-mapper`（文件列表来源）。 |
...
| **生成者** | `/ecl-plan-phase`（步骤 5.5，当 `workflow.nyquist_validation` 已启用且 RESEARCH.md 包含验证架构部分时）。 |
| **消费者** | `ecl-plan-checker` 维度 8（检查 8e 门控——Nyquist 检查进行前必须存在）；`ecl-verifier`。 |

### `<NN>-PATTERNS.md`

| | |
|---|---|
| **用途** | 由 `ecl-pattern-mapper` 生成的代码库类比映射。针对本阶段每个待创建或修改的文件，识别最近似的现有类比，对文件的角色和数据流进行分类，并提取具体代码摘录。引导规划者采用一致的模式。 |
| **生成者** | `/ecl-plan-phase` 通过 `ecl-pattern-mapper` 代理（可选；如果 `workflow.pattern_mapper: false` 则跳过）。 |
| **消费者** | `ecl-planner`（模式指导）；`ecl-plan-checker` 维度 12（模式合规性）。 |
...
| **生成者** | `/ecl-plan-phase` 通过 `ecl-planner` 代理。每个计划一个文件——例如，`03-02-PLAN.md` 是第 3 阶段第 2 个计划。 |
| **消费者** | `/ecl-execute-phase`（执行器代理读取计划并运行任务）；`ecl-plan-checker`（执行前质量审查）；`ecl-verifier`（读取 `must_haves` 进行执行后验证）。 |
...
| **消费者** | `/ecl-progress`（阶段状态）；`ecl-planner`（当后续计划对先前计划输出存在真实依赖时）；`milestone-summary`。 |
...
| **生成者** | `/ecl-verify-work`（或 `/ecl-execute-phase` 内的验证步骤）。 |
| **消费者** | `plan-phase` 已关闭阶段门控（`status: passed` 的 VERIFICATION.md 将阶段标记为 `Complete`，并在没有 `--force` 的情况下阻止重新规划）；`/ecl-progress`；人工审查。 |
...
| **生成者** | `/ecl-audit-uat`（交互式 UAT 会话）。 |
| **消费者** | `/ecl-audit-uat`（恢复先前的 UAT 会话）。 |
