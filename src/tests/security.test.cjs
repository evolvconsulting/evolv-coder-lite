...
// Verify that ecl-context-monitor and ecl-statusline reject session_id values
...
describe('ecl-context-monitor session_id path traversal', () => {
  const monitorPath = path.join(__dirname, '..', 'hooks', 'ecl-context-monitor.js');
...
describe('ecl-statusline session_id path traversal', () => {
  const statuslinePath = path.join(__dirname, '..', 'hooks', 'ecl-statusline.js');
  const tmpDir = os.tmpdir();
  ...
  test('does not write bridge file for session_id with ../ traversal', () => {
    const maliciousId = '../../../etc/ecl-test';
...
describe('validatePromptStructure', () => {
  ...
  test('accepts known valid tags in agent files', () => {
    const text = [
      ...
      '<available_agent_types>ecl-executor</available_agent_types>',
    ].join('\n');
