...
const {
  PROFILING_QUESTIONS,
  CLAUDE_INSTRUCTIONS,
} = require('../evolv-coder-lite/bin/lib/profile-output.cjs');
...
describe('write-profile command', () => {
  ...
  test('#1114: default output resolves the active runtime config home (codex)', () => {
    ...
    const result = runGsdTools(
      ...
      tmpDir,
      { CODEX_HOME: codexHome, ECL_RUNTIME: 'codex' }
    );
    assert.ok(result.success, `Failed: ${result.error}`);
    const out = JSON.parse(result.output);
    // Must land in the Codex home (so Codex advisor-mode finds it), NOT .claude.
    assert.strictEqual(out.profile_path, path.join(codexHome, 'evolv-coder-lite', 'USER-PROFILE.md'));
  ...
  test('#1114: default output is the .claude config home for the claude runtime', () => {
    ...
    // developer's shell (a stray ECL_RUNTIME/CLAUDE_CONFIG_DIR would redirect it).
    const result = runGsdTools(['write-profile', '--input', analysisPath, '--raw'], tmpDir,
      { HOME: tmpDir, ECL_RUNTIME: '', CLAUDE_CONFIG_DIR: '', CODEX_HOME: '' });
    assert.ok(result.success, `Failed: ${result.error}`);
    const out = JSON.parse(result.output);
    ...
    assert.ok(
      out.profile_path.endsWith(path.join('.claude', 'evolv-coder-lite', 'USER-PROFILE.md')),
      `claude profile must be under .claude/evolv-coder-lite; got ${out.profile_path}`
    );
    assert.ok(fs.existsSync(out.profile_path), 'profile should be written to disk');
  });

  test('#1114: config.runtime=codex (no ECL_RUNTIME) also resolves the Codex home', () => {
    const analysis = {
      profile_version: '1.0',
      dimensions: { communication_style: { rating: 'terse-direct', confidence: 'HIGH' } },
    };
    ...
    // No ECL_RUNTIME — the runtime must be read from config.runtime.
    const result = runGsdTools(
      ['write-profile', '--input', analysisPath, '--raw'],
      tmpDir,
      { CODEX_HOME: codexHome, ECL_RUNTIME: '' }
    );
    assert.ok(result.success, `Failed: ${result.error}`);
    const out = JSON.parse(result.output);
    assert.strictEqual(out.profile_path, path.join(codexHome, 'evolv-coder-lite', 'USER-PROFILE.md'));
...
describe('generate-claude-md command', () => {
  ...
  test('does not overwrite existing marker-less CLAUDE.md without --force (#1098)', () => {
    ...
    // No eCL markers in the file → the #1098 guard must leave it untouched.
  ...
  test('overwrites existing marker-less CLAUDE.md with --force (#1098)', () => {
    ...
    assert.ok(content.includes('## eCL Workflow Enforcement'), '--force injects eCL sections');
  });
  ...
  test('skills fallback mentions the normalized project roots', () => {
    ...
    assert.ok(!content.includes('evolv-coder-lite/skills'));
  });
...
describe('generate-dev-preferences command', () => {
  ...
  test('uses runtime-aware skills dir for codex by default', () => {
    ...
    assert.ok(result.success, `Failed: ${result.error}`);
    const out = JSON.parse(result.output);
    assert.strictEqual(out.command_path, path.join(codexHome, 'skills', 'ecl-dev-preferences', 'SKILL.md'));
  ...
  test('canonicalizes codex runtime aliases for skills output path', () => {
    ...
    const result = runGsdTools(
      ['generate-dev-preferences', '--analysis', analysisPath, '--raw'],
      tmpDir,
      { CODEX_HOME: codexHome, ECL_RUNTIME: 'codex-app' }
    );
    assert.ok(result.success, `Failed: ${result.error}`);
    const out = JSON.parse(result.output);
    assert.strictEqual(out.command_path, path.join(codexHome, 'skills', 'ecl-dev-preferences', 'SKILL.md'));
  ...
  test('uses runtime-aware skills dir for cline by default (#782)', () => {
    ...
    const result = runGsdTools(
      ['generate-dev-preferences', '--analysis', analysisPath, '--raw'],
      tmpDir,
      { CLINE_CONFIG_DIR: clineHome, ECL_RUNTIME: 'cline' }
    );
    assert.ok(result.success, `cline skills output should succeed: ${result.error}`);
    const out = JSON.parse(result.output);
    assert.strictEqual(out.command_path, path.join(clineHome, 'skills', 'ecl-dev-preferences', 'SKILL.md'));
  });
});
