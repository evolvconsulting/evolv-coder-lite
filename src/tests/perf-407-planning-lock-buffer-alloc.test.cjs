...
const PLANNING_WORKSPACE_CJS_PATH = path.join(
  __dirname, '..', 'evolv-coder-lite', 'bin', 'lib', 'planning-workspace.cjs'
);

const CLOCK_CJS_PATH = path.join(
  __dirname, '..', 'evolv-coder-lite', 'bin', 'lib', 'clock.cjs'
);
...
function makeTempDir() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-407-'));
