...
Run all remaining phases — or a bounded range of them — unattended, so eCL moves through discuss → plan → execute for each phase without you driving every step.
...
- Any design decisions you care about should already be in `PROJECT.md` or captured via a prior `/ecl-discuss-phase` — autonomous mode can surface grey areas interactively only when you use `--interactive`
...
eCL reads `ROADMAP.md`, discovers every incomplete phase in numeric order, and runs discuss → plan → execute on each one. After all phases complete it automatically runs the milestone lifecycle: audit → complete → cleanup.
...
```bash
/ecl-autonomous --from 3          # phases 3, 4, 5 … (skip already-done phases 1 and 2)
/ecl-autonomous --to 5            # phases up to and including 5
/ecl-autonomous --from 3 --to 5   # exactly phases 3, 4, and 5
```
...
```text
Resume with: /ecl-autonomous --from 6
```
...
Use `--converge` when you want each phase to run the plan-review convergence loop before execution. Both `/ecl-autonomous` and `/ecl-progress --next --auto` support this flag.

```bash
ecl config-set workflow.plan_review_convergence true

# Via autonomous (multi-phase or single-phase):
/ecl-autonomous --only 4 --converge
/ecl-autonomous --from 3 --to 5 --converge --all --max-cycles 5

# Via progress --next --auto (step-chaining with convergence):
/ecl-progress --next --auto --converge
/ecl-progress --next --auto --converge --codex --max-cycles 4
```

`--cross-ai` is accepted as an alias for `--converge`. Reviewer flags supported by `/ecl-plan-review-convergence` pass through unchanged, including `--codex`, `--gemini`, `--claude`, `--opencode`, `--ollama`, `--lm-studio`, `--llama-cpp`, `--all`, and `--max-cycles N`.
...
```bash
/ecl-autonomous --interactive
```

In interactive mode:
- `/ecl-discuss-phase` runs inline and waits for your answers
...
Autonomous mode does not bypass eCL's quality pipeline. Each phase still:
...
Do not use `/ecl-autonomous` when:

- **Phases have unsettled design decisions.** If you have not run `/ecl-discuss-phase` and your `PROJECT.md` does not capture your preferences, smart discuss will make autonomous choices you may not agree with. Run discuss interactively first, or use `--interactive`.

- **You need fine-grained control over a single phase.** For one phase, `/ecl-execute-phase N` gives you step-by-step output and lets you react before continuing. Use `--only N` if you want the autonomous quality pipeline on a single phase but do not need step-by-step interaction.
...
- **You are mid-phase with partial execution.** Autonomous mode picks up incomplete phases but it does not resume a partially-executed wave. Use `/ecl-execute-phase N` to finish a phase that is already in progress.
...
```text
 eCL ► AUTONOMOUS ▸ Phase 3/7: Auth Middleware [████░░░░] 28%
```
...
```bash
/ecl-autonomous --from 4     # replace 4 with the first incomplete phase number
```

eCL skips already-complete phases automatically, so it is safe to re-run from an earlier phase number if you are not sure where the run stopped.
