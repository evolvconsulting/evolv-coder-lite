...
function requireStateCjs() {
  return require(path.join(ROOT, 'evolv-coder-lite', 'bin', 'lib', 'state.cjs'));
}

function makeTempPlanning(stateContent) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-3127-'));
...
const MID_FLIGHT_STATE = `# eCL State
...
const PRE_EXECUTE_STATE = `# eCL State
...
describe('bug #3127: state.begin-phase idempotency guard', () => {
  ...
  test('begin-phase always updates Last Activity date (safe on resume, pinned via ECL_NOW_MS)', () => {
    const stateModule = requireStateCjs();
    const { cmdStateBeginPhase } = stateModule;
    if (!cmdStateBeginPhase) return;
    const dir = makeTempPlanning(MID_FLIGHT_STATE);
    ...
    const origTestMode = process.env.ECL_TEST_MODE;
    const origNowMs = process.env.ECL_NOW_MS;
    ...
    try {
      ...
      if (origTestMode === undefined) delete process.env.ECL_TEST_MODE;
      else process.env.ECL_TEST_MODE = origTestMode;
      if (origNowMs === undefined) delete process.env.ECL_NOW_MS;
      else process.env.ECL_NOW_MS = origNowMs;
      cleanup(dir);
    }
  });
});
