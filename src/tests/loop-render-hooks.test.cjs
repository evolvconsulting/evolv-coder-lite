...
const {
  ...
  CANONICAL_POINTS,
} = require('../evolv-coder-lite/bin/lib/loop-resolver.cjs');
...
before(() => {
  ...
  fs.writeFileSync(
    path.join(tmpUiDisabledConfigDir, '.ecl-surface.json'),
    JSON.stringify({
      baseProfile: 'full',
      disabledClusters: ['ui'],
      explicitAdds: [],
...
describe('renderLoopHooks', () => {
  ...
  test('step hook renders agent ref and inline prompt fragment', () => {
    ...
    const rendered = renderLoopHooks(resolved);
    assert.match(rendered, /agent:ecl-phase-researcher/);
...
// ─── 9. End-to-end cmdLoopRenderHooks (via ecl-tools subprocess) ─────────────

const { spawnSync } = require('node:child_process');
const ROOT = path.resolve(__dirname, '..');
const ECL_TOOLS = path.join(ROOT, 'evolv-coder-lite', 'bin', 'ecl-tools.cjs');

describe('cmdLoopRenderHooks end-to-end (via ecl-tools)', () => {
  test('loop render-hooks plan:pre returns JSON envelope with ui-phase step active', () => {
    const result = spawnSync(
      process.execPath,
      [ECL_TOOLS, 'loop', 'render-hooks', 'plan:pre', '--cwd', tmpProjectDir],
  ...
  test('loop render-hooks plan:pre with no config.json → ui-phase step active by schema default', () => {
    const result = spawnSync(
      process.execPath,
      [ECL_TOOLS, 'loop', 'render-hooks', 'plan:pre', '--cwd', tmpEmptyProjectDir],
      { cwd: ROOT, encoding: 'utf8' },
    );
    assert.strictEqual(result.status, 0, 'Expected exit 0. stderr: ' + (result.stderr || ''));
    const envelope = JSON.parse(result.stdout.trim());
  ...
  test('loop render-hooks plan:pre with ui_phase=false in config.json → ui-phase step absent', () => {
    const result = spawnSync(
      process.execPath,
      [ECL_TOOLS, 'loop', 'render-hooks', 'plan:pre', '--cwd', tmpFalseConfigProjectDir],
      { cwd: ROOT, encoding: 'utf8' },
    );
    assert.strictEqual(result.status, 0, 'Expected exit 0. stderr: ' + (result.stderr || ''));
    const envelope = JSON.parse(result.stdout.trim());
  ...
  test('loop render-hooks invalid-point exits non-zero', () => {
    const result = spawnSync(
      process.execPath,
      [ECL_TOOLS, 'loop', 'render-hooks', 'plan:mid', '--cwd', tmpProjectDir],
      { cwd: ROOT, encoding: 'utf8' },
    );
...
describe('--active-cap flag (loop render-hooks)', () => {
  ...
  test('--active-cap tdd with tdd_mode=true → stdout trimmed === "true", exit 0', () => {
    const result = spawnSync(
      process.execPath,
      [ECL_TOOLS, 'loop', 'render-hooks', 'execute:post', '--active-cap', 'tdd', '--cwd', tddOnDir],
      { cwd: ROOT, encoding: 'utf8' },
    );
    assert.strictEqual(result.status, 0, 'Expected exit 0. stderr: ' + (result.stderr || ''));
  ...
  test('--active-cap tdd with tdd_mode=false → stdout trimmed === "false", exit 0', () => {
    const result = spawnSync(
      process.execPath,
      [ECL_TOOLS, 'loop', 'render-hooks', 'execute:post', '--active-cap', 'tdd', '--cwd', tddOffDir],
      { cwd: ROOT, encoding: 'utf8' },
    );
    assert.strictEqual(result.status, 0, 'Expected exit 0. stderr: ' + (result.stderr || ''));
  ...
  test('--active-cap <nonexistent-cap> → stdout trimmed === "false", exit 0', () => {
    const result = spawnSync(
      process.execPath,
      [ECL_TOOLS, 'loop', 'render-hooks', 'execute:post', '--active-cap', 'no-such-capability-xyz', '--cwd', tddOffDir],
      { cwd: ROOT, encoding: 'utf8' },
    );
  ...
  test('--active-cap with no value → non-zero exit and error message', () => {
    const result = spawnSync(
      process.execPath,
      [ECL_TOOLS, 'loop', 'render-hooks', 'execute:post', '--active-cap', '--cwd', tddOffDir],
      { cwd: ROOT, encoding: 'utf8' },
    );
...
describe('Phase 4 regression: capabilityStatesById gates on active (not enabled) for config-disabled capability', () => {
  test('[regression] enabled=true active=false + no `when` guard → hook NOT rendered (state.active gate)', () => {
    ...
    const registry = makeRegistry({
      point: 'plan:pre',
      steps: [{ capId: 'test-cap', ref: { skill: 'ecl-test-skill' } }],
