...
- [Onboarding an existing codebase](tutorials/onboarding-an-existing-codebase.md) — bring eCL Core to a brownfield repo
...
- [Install a minimal eCL and add skills later](how-to/install-minimal-and-add-skills.md) — install only the core skills, then grow the surface with profiles and `/ecl:surface`
- [Attach a plugin-provided skill to a eCL agent](how-to/attach-a-plugin-skill-to-a-ecl-agent.md) — use the `global:plugin:skill` entry form to load Claude Code plugin skills into agent prompts
...
- [Handle quick and fast tasks](how-to/handle-quick-and-fast-tasks.md) — use `/ecl-quick` and `/ecl-fast` for ad-hoc work outside the phase loop
...
- [Spike and sketch](how-to/spike-and-sketch.md) — use `/ecl-spike` and `/ecl-sketch` for exploratory work before committing to a plan
- [Design a UI phase](how-to/design-a-ui-phase.md) — use the UI phase loop for frontend and visual work
- [Develop a Capability for eCL 1.5+](how-to/develop-a-capability.md) — add feature Capabilities, hook fragments, and registry entries
...
- [Drive eCL from a tracker issue](how-to/drive-ecl-from-a-tracker-issue.md) — start a phase from a GitHub, Linear, or Jira issue
- [Migrate from eCL 2](how-to/migrate-from-ecl-2.md) — upgrade an existing eCL 2 project to eCL Core
- [Update eCL](how-to/update-ecl.md) — re-run the installer to pick up the latest release
- [Clean up evolv-coder-lite-cc](cleanup-evolv-coder-lite-cc.md) — remove leftover old-package artifacts that cause a spurious `⬆ /ecl:update` indicator after migrating to `@evolvconsulting/evolv-coder-lite`
...
- [CLI tools](CLI-TOOLS.md) — `ecl-tools.cjs` programmatic API for workflows and agents
...
- [Context engineering](explanation/context-engineering.md) — how context rot forms and how eCL Core prevents it
...
- [Discuss modes](workflow-discuss-mode.md) — assumptions mode vs interview mode for `/ecl-discuss-phase`
- [Context monitoring](context-monitor.md) — context window monitoring hook architecture
- [Issue-driven orchestration](issue-driven-orchestration.md) — recipe for driving eCL from a tracker issue using existing primitives
