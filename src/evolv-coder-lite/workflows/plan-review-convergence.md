...
ecl-plan-phase N → ecl-review N --codex → ecl-plan-phase N --reviews → ecl-review N --codex → ...
Plan-phase runs inline (bare Skill at depth 0) so it can spawn ecl-planner/ecl-plan-checker at depth 1.
...
@$HOME/.claude/evolv-coder-lite/references/revision-loop.md
@$HOME/.claude/evolv-coder-lite/references/gates.md
@$HOME/.claude/evolv-coder-lite/references/agent-contracts.md
</required_reading>
...
ECL_WS=""
echo "$ARGUMENTS" | grep -qE '\-\-ws\s+\S+' && ECL_WS=$(echo "$ARGUMENTS" | grep -oE '\-\-ws\s+\S+')
```
...
```text
ecl-plan-review-convergence is disabled (workflow.plan_review_convergence=false).
...
Then re-run: /ecl:plan-review-convergence {PHASE}
```
...
```text
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 eCL ► PLAN CONVERGENCE — Phase {phase_number}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
Agent(
  ...
  prompt="Run /ecl:review for Phase {PHASE}.

Execute: Skill(skill='ecl-review', args='--phase {PHASE} {REVIEWER_FLAGS} {ECL_WS}')
...
Definitions:
  ...
  ACTIONABLE — a non-HIGH review finding that would be invisible to /ecl:execute-phase unless it is incorporated into PLAN.md or explicitly deferred/rejected in PLAN.md.
...
4a. **Resolve effective authority** (deterministic — replaces manual `intel.enabled` reasoning):
   ```bash
   EFFECTIVE_AUTHORITY=$(ecl_run drift-guard authority --raw)
   ```
4. **Severity & gating** — classify each symbol's verdict using the seam (do not apply the table manually):
   ```bash
   # For each symbol, e.g.:
   RESULT=$(ecl_run drift-guard severity --status <verdict> --authority "$EFFECTIVE_AUTHORITY")
...
To replan manually:  /ecl:plan-phase {PHASE} --reviews
To restart loop:     /ecl:plan-review-convergence {PHASE} {REVIEWER_FLAGS}
```
Exit workflow.
...
- [ ] Initial planning via inline Skill("ecl-plan-phase") if no plans exist — NOT wrapped in Agent() (bug #936: depth-1 Agent has no Agent tool)
- [ ] Review via Agent → Skill("ecl-review") — isolated Agent is correct; ecl-review is a Bash leaf with no sub-agent spawns; {ECL_WS} forwarded
- [ ] Replan via inline Skill("ecl-plan-phase --reviews") — NOT wrapped in Agent(); inline lets plan-phase spawn ecl-planner/ecl-plan-checker at depth 1
...
- [ ] The review Agent fully completes ecl-review before returning (plan-phase runs inline — no Agent wrap)
