 ...
 * fracture recommendations) no longer emit the unroutable `/ecl:<cmd>` colon
...
// Helper: assert no string field anywhere in `value` (recursive) contains '/ecl:'.
function assertNoColonForm(value, label) {
  ...
  while (stack.length > 0) {
    ...
    if (typeof v === 'string') {
      assert.ok(
        !v.includes('/ecl:'),
        `${p}: must not contain deprecated /ecl: form, got ${JSON.stringify(v)}`,
      );
...
describe('bug-3584: init manager recommendedActions emit hyphen form', () => {
  ...
  test('skills runtime (claude) emits /ecl-<cmd> in recommended_actions[].command', () => {
    // Plan-but-not-executed: directory exists with a PLAN.md → execute is recommended.
    const phaseDir = path.join(tmpDir, '.planning', 'phases', '01-foundation');
    fs.mkdirSync(phaseDir, { recursive: true });
    fs.writeFileSync(path.join(phaseDir, '01-01-PLAN.md'), '# Plan\n');

    const result = runGsdTools('init manager', tmpDir, { ECL_RUNTIME: 'claude' });
    ...
    for (const cmd of commands) {
      assert.ok(
        cmd.startsWith('/ecl-'),
        `recommended_actions command must start with /ecl- for skills-based runtimes, got ${cmd}`,
      );
      assert.ok(
        !cmd.includes('/ecl:'),
        `recommended_actions command must not contain /ecl: colon form, got ${cmd}`,
      );
    }
    assertNoColonForm(payload, 'init.manager payload');
  });

  test('codex runtime emits $ecl-<cmd> in recommended_actions[].command', () => {
    const phaseDir = path.join(tmpDir, '.planning', 'phases', '01-foundation');
    fs.mkdirSync(phaseDir, { recursive: true });
    fs.writeFileSync(path.join(phaseDir, '01-01-PLAN.md'), '# Plan\n');

    const result = runGsdTools('init manager', tmpDir, { ECL_RUNTIME: 'codex' });
    ...
    for (const cmd of commands) {
      assert.ok(
        cmd.startsWith('$ecl-'),
        `codex recommended_actions command must use $ecl- shell-var form, got ${cmd}`,
      );
    }
  });

  test('codex alias runtime emits $ecl-<cmd> in recommended_actions[].command', () => {
    const phaseDir = path.join(tmpDir, '.planning', 'phases', '01-foundation');
    fs.mkdirSync(phaseDir, { recursive: true });
    fs.writeFileSync(path.join(phaseDir, '01-01-PLAN.md'), '# Plan\n');

    const result = runGsdTools('init manager', tmpDir, { ECL_RUNTIME: 'codex-app' });
    ...
    for (const cmd of commands) {
      assert.ok(
        cmd.startsWith('$ecl-'),
        `codex alias recommended_actions command must use $ecl- shell-var form, got ${cmd}`,
      );
    }
  });
});
...
describe('bug-3584: phase add persists hyphen form into ROADMAP.md', () => {
  ...
  test('phase add persists hyphen-form slash command in roadmap get-phase payload', () => {
    ...
    assert.ok(
      !getPayload.section.includes('/ecl:plan-phase'),
      `persisted phase section must not contain /ecl:plan-phase, got: ${getPayload.section}`,
    );
    assert.ok(
      getPayload.section.includes('/ecl-plan-phase'),
      `persisted phase section must contain /ecl-plan-phase, got: ${getPayload.section}`,
    );
  });
});
...
describe('bug-3584: validate health emits hyphen form in fix hints', () => {
  test('validate health on a broken project returns hyphen-form fix strings', (t) => {
    ...
    const fixesWithSlash = allIssues
      .map((i) => i.fix)
      .filter((f) => typeof f === 'string' && /ecl[-:]/.test(f));
    assert.ok(
      fixesWithSlash.length > 0,
      'at least one fix hint should reference a /ecl- slash command',
    );

    for (const fix of fixesWithSlash) {
      assert.ok(
        !fix.includes('/ecl:'),
        `validate health fix hint must not contain /ecl: colon form, got ${JSON.stringify(fix)}`,
      );
    }
...
describe('bug-3584: validate context recommendation uses hyphen form', () => {
  test('warning/critical recommendations emit /ecl-thread', (t) => {
    ...
    assert.ok(
      !payload.recommendation.includes('/ecl:'),
      `recommendation must not contain /ecl:, got ${JSON.stringify(payload.recommendation)}`,
    );
    assert.ok(
      payload.recommendation.includes('/ecl-thread'),
      `recommendation must reference /ecl-thread, got ${JSON.stringify(payload.recommendation)}`,
    );
  });
});

describe('bug-3584: validate health uses formatter for codex runtime too', () => {
  test('validate health under codex emits $ecl-<cmd> in fix strings (positive assertion)', (t) => {
    const tmpDir = createTempDir();
    t.after(() => cleanup(tmpDir));
    ...
    // Collect fixes that mention any ecl slash-command form so we can lock
    ...
    assert.ok(
      ...
      'validate health on a bare tmpdir must produce at least one fix hint referencing a ecl command',
    );

    for (const fix of fixesWithGsdRef) {
      assert.ok(
        fix.includes('$ecl-'),
        `codex validate health fix must use shell-var $ecl- form, got ${JSON.stringify(fix)}`,
      );
      assert.ok(
        !fix.includes('/ecl:'),
        `codex validate health fix must not contain /ecl: colon form, got ${JSON.stringify(fix)}`,
      );
      assert.ok(
        !fix.includes('/ecl-'),
        `codex validate health fix must not contain /ecl- (skills) form, got ${JSON.stringify(fix)}`,
      );
    }
  });
});
