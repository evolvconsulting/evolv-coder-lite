...
/**
 * Bug #2969: /ecl-reapply-patches Step 5 hunk verification gate reports
...
// Script lives at evolv-coder-lite/bin/ so the installer ships it under
// `${ECL_HOME}/evolv-coder-lite/bin/` (issue #2994). The top-level scripts/
...
before(() => {
  tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-2969-'));
...
describe('Bug #2969: deterministic Step 5 verification gate', () => {
  ...
  test('treats ecl-hook-version install-time substitution as upstream-owned, not missing user content (#229)', () => {
    resetFixture();
    const rel = path.join('hooks', 'ecl-statusline.js');
    const pristine = [
    ...
    const backup = [
      '// ecl-hook-version: 1.41.0',
      'console.log("statusline hook");',
      '',
    ].join('\n');
    const installed = [
      '// ecl-hook-version: 1.42.3',
      'console.log("statusline hook");',
      '',
    ].join('\n');
