...
/**
 * eCL Tests — /ecl:sketch --wrap-up silently no-ops (#2949)
 *
 * The --wrap-up flag was documented in commands/ecl/sketch.md but never dispatched.
...
const SKETCH_COMMAND = path.join(ROOT, 'commands/ecl/sketch.md');
const SKETCH_WORKFLOW = path.join(ROOT, 'evolv-coder-lite/workflows/sketch.md');

describe('bug-2949: sketch --wrap-up dispatch wiring', () => {
  test('commands/ecl/sketch.md contains --wrap-up dispatch logic', () => {
  ...
  test('commands/ecl/sketch.md has sketch-wrap-up in execution_context section', () => {
    const content = fs.readFileSync(SKETCH_COMMAND, 'utf8');
  ...
  test('workflows/sketch.md does NOT contain old /ecl-sketch-wrap-up form', () => {
    const content = fs.readFileSync(SKETCH_WORKFLOW, 'utf8');
    assert.ok(
      !content.includes('/ecl-sketch-wrap-up'),
      'workflows/sketch.md must not reference the old /ecl-sketch-wrap-up command'
    );
  });

  test('workflows/sketch.md DOES contain new /ecl:sketch --wrap-up form', () => {
    const content = fs.readFileSync(SKETCH_WORKFLOW, 'utf8');
    assert.ok(
      content.includes('/ecl:sketch --wrap-up'),
      'workflows/sketch.md should reference /ecl:sketch --wrap-up (the new form)'
    );
  });
});
