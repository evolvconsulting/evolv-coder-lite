...
/**
 * Regression tests for #3406 — stale globally-installed `@evolvconsulting/ecl-sdk@0.1.0`
 * shadows the `ecl-sdk` shim that `evolv-coder-lite` installs. The standalone
 * 0.1.0 binary only knows `run | auto | init` (no `query` subcommand), so
 * every workflow that calls `ecl-sdk query <command>` fails until the user
 * runs `npm uninstall -g @evolvconsulting/ecl-sdk`.
 *
 ...
 * `npm ls -g @evolvconsulting/ecl-sdk --json --depth=0`; in tests we hand it a stub
...
describe('#3406: detectStaleStandaloneSdk', () => {
  test('is exported from bin/install.js under ECL_TEST_MODE', () => {
    assert.strictEqual(
  ...
  test('returns { stale: false } when npm ls reports the package is not installed', () => {
    // `npm ls -g @evolvconsulting/ecl-sdk --json --depth=0` exit code 1 with this
  ...
  test('returns { stale: true, version, path? } when @evolvconsulting/ecl-sdk is present', () => {
    const stub = () => JSON.stringify({
      name: 'lib',
      dependencies: {
        '@evolvconsulting/ecl-sdk': {
          version: '0.1.0',
          resolved: 'file:/Users/REDACTED/.nvm/versions/node/v24.15.0/lib/node_modules/@evolvconsulting/ecl-sdk',
        },
      },
    });
    const result = detectStaleStandaloneSdk(stub);
  ...
  test('returns { stale: false } for non-0.1.0 versions (CR #3406)', () => {
    ...
    const stubNewer = () => JSON.stringify({ dependencies: { '@evolvconsulting/ecl-sdk': { version: '1.50.0-canary.0' } } });
    const stubFuture = () => JSON.stringify({ dependencies: { '@evolvconsulting/ecl-sdk': { version: '2.0.0' } } });
...
describe('#3406: install-time wiring stays silent when no stale package is found', () => {
  ...
  // negative case: when @evolvconsulting/ecl-sdk is NOT installed globally (the npm
  ...
  test('install does not emit the stale-SDK warning on a clean npm prefix', () => {
    ...
    const tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-3406-home-'));
    const tmpPrefix = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-3406-npm-'));
    try {
      const stdout = execFileSync(
        ...
        [installScript, '--claude', '--global', '--yes', '--no-sdk'],
        {
          ...
          env: {
            ...
            // ECL_SKIP_STALE_SDK_CHECK here.
          },
          timeout: 60_000,
        }
      );
      assert.ok(
        !stdout.includes('@evolvconsulting/ecl-sdk'),
        'install output must not mention @evolvconsulting/ecl-sdk when the package is absent'
      );
      assert.ok(
...
describe('#3406: formatStaleStandaloneSdkWarning', () => {
  ...
  test('message names the stale package, the version, and the uninstall command', () => {
    ...
    assert.ok(out.includes('@evolvconsulting/ecl-sdk'), 'must name the shadowing package');
    assert.ok(out.includes('0.1.0'), 'must show the stale version');
    assert.ok(
      out.includes('npm uninstall -g @evolvconsulting/ecl-sdk'),
