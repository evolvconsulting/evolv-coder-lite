# Referência de Comandos do eCL Core

> Referência de comandos do eCL Core — sintaxe, flags, opções e exemplos para cada comando estável. Para detalhes sobre funcionalidades, consulte a [Referência de Funcionalidades](FEATURES.md); para tutoriais de fluxo de trabalho, consulte o [Guia do Usuário](USER-GUIDE.md); para o índice de documentação, consulte o [README](README.md).

---

## Sintaxe de Comandos

- **Claude Code / Copilot / OpenCode / Kilo:** `/ecl-command-name [args]` (forma com hífen)
- **Gemini CLI:** `/ecl:command-name [args]` (forma com dois-pontos — o Gemini agrupa comandos sob `ecl:`)
...
Seis roteadores de namespace são incluídos como pontos de entrada de primeiro estágio na v1.40. Eles mantêm o custo de tokens da listagem antecipada de skills baixo (~120 tokens para 6 roteadores vs ~2.150 para uma listagem plana de 86 skills), enquanto toda a superfície permanece invocável diretamente. O modelo seleciona um namespace e então roteia para a sub-skill concreta. Consulte [#2792](https://github.com/evolvconsulting/evolv-coder-lite/issues/2792).

| Comando | Roteia para |
|---------|-------------|
| `/ecl-workflow` | Pipeline de fases — discuss / plan / execute / verify / phase / progress |
| `/ecl-project` | Ciclo de vida do projeto — milestones, auditorias, resumo |
| `/ecl-quality` | Portões de qualidade — revisão de código, debug, auditoria, segurança, eval, ui |
| `/ecl-context` | Inteligência da base de código — map, graphify, docs, learnings |
| `/ecl-manage` | Gerenciamento — config, workspace, workstreams, thread, update, ship, inbox |
| `/ecl-ideate` | Exploração e captura — explore, sketch, spike, spec, capture |

Os skills de namespace são **aditivos** — todo comando concreto existente (por exemplo, `/ecl-plan-phase`, `/ecl-code-review --fix`) ainda pode ser invocado diretamente.
...
```bash
/ecl-new-project                    # Modo interativo
/ecl-new-project --auto @prd.md     # Extração automática a partir de PRD
```
...
Gerencia workspaces do eCL — cria, lista ou remove ambientes de workspace isolados com cópias de repositório e diretórios `.planning/` independentes.

| Flag | Descrição |
|------|-----------|
| `--new` | Cria um novo workspace (use com `--name`, `--repos`, etc.) |
| `--list` | Lista os workspaces eCL ativos e seus status |
...
| `--path /target` | Diretório de destino (padrão: `~/ecl-workspaces/<name>`) |
...
**Casos de uso:**
- Multi-repositório: trabalha em um subconjunto de repositórios com estado eCL isolado
...
/ecl-workspace --new --name feature-b --repos . --strategy worktree  # Isolamento no mesmo repositório
/ecl-workspace --list
...
```bash
/ecl-discuss-phase 1                # Discussão interativa para a fase 1
/ecl-discuss-phase 1 --all          # Discute todas as áreas cinzentas sem etapa de seleção
/ecl-discuss-phase 3 --auto         # Seleciona padrões automaticamente para a fase 3
/ecl-discuss-phase --batch          # Modo em lote para a fase atual
/ecl-discuss-phase 2 --analyze      # Discussão com análise de trade-offs
/ecl-discuss-phase 1 --power        # Respostas em massa a partir de arquivo
/ecl-discuss-phase 3 --assumptions  # Expõe as suposições do Claude antes do planejamento
```
...
```bash
/ecl-ui-phase 2                     # Contrato de design para a fase 2
```
...
```bash
/ecl-plan-phase 1                              # Pesquisa + plano + verificação da fase 1
/ecl-plan-phase 3 --skip-research              # Planejar sem pesquisa (domínio familiar)
/ecl-plan-phase --auto                         # Planejamento não interativo
/ecl-plan-phase 2 --validate                   # Valida estado antes do planejamento
/ecl-plan-phase 1 --bounce                     # Plano + validação de bounce externo
/ecl-plan-phase 2 --ingest docs/adr/0010.md   # Caminho expresso via ADR para síntese de contexto
/ecl-plan-phase 2 --ingest 'docs/adr/00*.md' --ingest-format auto
/ecl-plan-phase --research-phase 4             # Somente pesquisa na fase 4 (solicita se RESEARCH.md existir)
/ecl-plan-phase --research-phase 4 --view      # Imprime RESEARCH.md existente, sem criar agente
/ecl-plan-phase --research-phase 4 --research  # Força atualização da pesquisa, sem prompt
/ecl-plan-phase 1 --mvp                        # Plano em fatias verticais para a fase 1
/ecl-plan-phase 1 --mvp --tdd                  # Fatias verticais + teste falho por tarefa que adiciona comportamento
```
...
```bash
/ecl-plan-review-convergence 3                    # Revisores padrão, 3 ciclos
/ecl-plan-review-convergence 3 --codex            # Revisão somente com Codex
...
**[BETA]** Delega o planejamento da fase para o ultraplan em nuvem do Claude Code; revise no navegador e importe de volta. O rascunho do plano é feito remotamente, liberando o terminal; revise comentários inline no navegador e importe o plano finalizado de volta para `.planning/` via `/ecl-import`.

| Flag | Obrigatório | Descrição |
|------|-------------|-----------|
| `N` | **Sim** | Número da fase a planejar remotamente |

**Isolamento:** Intencionalmente separado de `/ecl-plan-phase` para que mudanças upstream no ultraplan não afetem o pipeline de planejamento principal.

```bash
/ecl-ultraplan-phase 4                  # Delega planejamento para a fase 4
```
...
```bash
/ecl-execute-phase 1                # Executa a fase 1
/ecl-execute-phase 1 --wave 2       # Executa somente a Wave 2
/ecl-execute-phase 1 --validate     # Valida estado antes da execução
/ecl-execute-phase 2 --cross-ai     # Delega a fase 2 para CLI de IA externa
```
...
**Pré-requisitos:** Fase verificada (`/ecl-verify-work` concluído), CLI `gh` instalada e autenticada
**Produz:** PR no GitHub com body rico gerado a partir dos artefatos de planejamento, STATE.md atualizado

```bash
/ecl-ship 4                         # Publica a fase 4
/ecl-ship 4 --draft                 # Publica como PR rascunho
```
...
**Pré-requisitos:** O projeto tem código frontend (funciona de forma autônoma, sem necessidade de projeto eCL)
**Produz:** `{phase}-UI-REVIEW.md`, capturas de tela em `.planning/ui-reviews/`

Para evidência visual mais rica, combine com `ecl-browser` ou outro servidor MCP de navegador, para que a auditoria possa capturar capturas de tela, estado, contexto de console/rede e etapas de interação reproduzíveis.

```bash
/ecl-ui-review                      # Audita a fase atual
/ecl-ui-review 3                    # Audita a fase 3
```
...
```bash
/ecl-milestone-summary                # Resume o milestone atual
/ecl-milestone-summary v1.0           # Resume um milestone específico
```
...
```bash
/ecl-new-milestone                  # Interativo
/ecl-new-milestone "v2.0 Mobile"    # Milestone nomeado
/ecl-new-milestone --reset-phase-numbers "v2.0 Mobile"  # Reinicia numeração de milestone na fase 1
```
...
```bash
/ecl-phase "Add authentication system"          # Acrescenta nova fase com descrição
/ecl-phase --insert 3 "Fix auth race condition" # Insere entre a fase 3 e 4 → cria 3.1
/ecl-phase --remove 7               # Remove a fase 7, renumera 8→7, 9→8, etc.
/ecl-phase --edit 5                 # Edita qualquer campo da fase 5
/ecl-phase --edit 5 --force         # Edita a fase 5 mesmo se em andamento ou concluída
```
...
Planejamento MVP guiado para uma fase — solicita uma história de usuário, executa verificação de divisão SPIDR, escreve `**Mode:** mvp` no ROADMAP.md e então delega para `/ecl-plan-phase` (que detecta o modo MVP automaticamente pelo campo do roadmap).
...
**Pré-requisitos:** A fase já deve existir no ROADMAP.md (criada via `/ecl-new-project`, `/ecl-phase` ou `/ecl-phase --insert`). O comando não cria novas fases — ele converte uma fase existente.

**Comportamento:** Coleta uma história de usuário estruturada, valida o formato, executa uma verificação de divisão SPIDR, escreve `**Goal:**` e `**Mode:** mvp` na seção da fase no ROADMAP.md e então delega para `/ecl-plan-phase <N>`. Consulte [Como planejar uma fase MVP](USER-GUIDE.md#mvp-phase-planning) para um tutorial.
...
**Produz:** ROADMAP.md atualizado, e então todos os artefatos de `/ecl-plan-phase`; `SKELETON.md` quando o modo Walking Skeleton é ativado.

```bash
/ecl-mvp-phase 1                    # Planejamento MVP para a fase 1
/ecl-mvp-phase 2.1                  # Planejamento MVP para uma fase decimal
/ecl-mvp-phase 3 --force            # Converte a fase 3 mesmo se em andamento
```
...
```bash
/ecl-validate-phase 2               # Audita a cobertura de testes para a fase 2
```
...
| `--do "task description"` | Analisa intenção em texto livre e despacha para o comando eCL mais adequado |
...
- Sem projeto → sugere `/ecl-new-project`
- Fase precisa de discussão → executa `/ecl-discuss-phase`
- Fase precisa de planejamento → executa `/ecl-plan-phase`
- Fase precisa de execução → executa `/ecl-execute-phase`
- Fase precisa de verificação → executa `/ecl-verify-work`
- Todas as fases concluídas → sugere `/ecl-complete-milestone`

```bash
/ecl-progress                       # "Onde estou? O que vem a seguir?" com roteamento automático
/ecl-progress --next                # Avança automaticamente para a próxima etapa
/ecl-progress --do "fix the auth bug"  # Despacha intenção em texto livre para o melhor comando eCL
/ecl-progress --forensic            # Relatório padrão + auditoria de integridade
```
...
```bash
/ecl-resume-work                    # Após redefinição de contexto ou nova sessão
```
...
```bash
/ecl-pause-work                     # Cria continue-here.md
/ecl-pause-work --report            # Cria continue-here.md + relatório de sessão
```
...
```bash
/ecl-manager                        # Abre o painel da central de comando
/ecl-manager --analyze-deps         # Analisa as fases do ROADMAP em busca de relações de dependência antes da execução paralela
```
...
Exibe os comandos eCL no nível solicitado. O padrão cabe em uma tela; `--full` é a referência completa; `<topic>` pula diretamente para uma seção.

```bash
/ecl-help                           # Tour de uma página (padrão)
/ecl-help --brief                   # Recapitulação resumida em ~10 linhas dos principais comandos
/ecl-help --full                    # Referência completa (todos os comandos, todas as flags)
/ecl-help <topic>                   # Somente uma seção (por exemplo /ecl-help debug)
/ecl-help --brief <topic>           # Consulta resumida com escopo — assinatura + resumo em uma linha
```

Consulte `evolv-coder-lite/workflows/help/modes/topic.md` para a tabela completa de aliases. Tópicos desconhecidos exibem a lista reconhecida.
...
Sessão de ideação socrática — guia uma ideia por meio de perguntas investigativas, opcionalmente cria pesquisa, e então roteia a saída para o artefato eCL adequado (notas, todos, seeds, perguntas de pesquisa, requisitos ou uma nova fase).

| Argumento | Obrigatório | Descrição |
|-----------|-------------|-----------|
| `topic` | Não | Tópico a explorar (por exemplo, `/ecl-explore authentication strategy`) |

```bash
/ecl-explore                        # Sessão de ideação aberta
/ecl-explore authentication strategy  # Explora um tópico específico
```
...
Reversão segura no git — reverte commits de fase ou plano do eCL usando o manifesto da fase com verificações de dependências e um portão de confirmação.

| Flag | Obrigatório | Descrição |
|------|-------------|-----------|
| `--last N` | (um dos três obrigatórios) | Exibe commits eCL recentes para seleção interativa |
...
```bash
/ecl-undo --last 5                  # Escolhe entre os 5 commits eCL mais recentes
/ecl-undo --phase 03                # Reverte todos os commits da fase 3
/ecl-undo --plan 03-02              # Reverte commits do plano 02 da fase 3
```
...
Ingere um arquivo de plano externo no sistema de planejamento do eCL com detecção de conflitos contra as decisões do `PROJECT.md` antes de escrever qualquer coisa.

| Flag | Obrigatório | Descrição |
|------|-------------|----------|
| `--from <filepath>` | Sim (ou `--from-ecl2`) | Caminho para o arquivo de plano externo a importar |
| `--from-ecl2` | Sim (ou `--from`) | Migração reversa de um projeto eCL-2 (`.ecl/`) de volta para o formato eCL v1 (`.planning/`) |
| `--path <dir>` | Não | Com `--from-ecl2`: caminho para o diretório do projeto eCL-2 (padrão: diretório atual) |

**Processo:** Detecta conflitos → solicita resolução → escreve como eCL PLAN.md → valida via `ecl-plan-checker`

```bash
/ecl-import --from /tmp/team-plan.md    # Importa e valida um plano externo
/ecl-import --from-ecl2                # Migra do eCL-2 de volta para v1 (diretório atual)
/ecl-import --from-ecl2 --path ~/old-project  # Migra a partir de um caminho diferente
```
...
Inicializa ou mescla uma configuração `.planning/` a partir de ADRs, PRDs, SPECs e documentos existentes em um repositório. Executa classificação paralela (`ecl-doc-classifier`) mais síntese com regras de precedência e detecção de ciclos (`ecl-doc-synthesizer`). Produz um relatório de conflitos em três categorias (`INGEST-CONFLICTS.md`: auto-resolvidos, variantes-concorrentes, bloqueadores-não-resolvidos) e bloqueia completamente em contradições ADR LOCKED-vs-LOCKED.
...
**Limites:** v1 suporta no máximo 50 documentos por invocação. Extrai o contrato compartilhado de detecção de conflitos em `references/doc-conflict-engine.md`, que `/ecl-import` também consome.

```bash
/ecl-ingest-docs                            # Varre a raiz do repositório, detecção automática de modo
/ecl-ingest-docs docs/                      # Ingere somente sob docs/
/ecl-ingest-docs --manifest ingest.yaml     # Manifesto explícito de precedência
```
...
Executa tarefa ad-hoc com garantias do eCL.
...
```bash
/ecl-quick                          # Tarefa quick básica
/ecl-quick --discuss --research     # Discussão + pesquisa + planejamento
/ecl-quick --validate               # Somente verificação de plano + verificação
/ecl-quick --full                   # Pipeline completo de qualidade
/ecl-quick list                     # Lista todas as tarefas quick
/ecl-quick status my-task-slug      # Exibe status de uma tarefa quick
/ecl-quick resume my-task-slug      # Retoma uma tarefa quick
```
...
```bash
/ecl-autonomous                     # Executa todas as fases restantes
/ecl-autonomous --from 3            # Inicia a partir da fase 3
/ecl-autonomous --to 5              # Executa até a fase 5, inclusive
/ecl-autonomous --from 3 --to 5     # Executa as fases 3 a 5
```
...
**Subcomandos:**
- `/ecl-debug list` — Lista todas as sessões de debug ativas com status, hipótese e próxima ação
- `/ecl-debug status <slug>` — Imprime resumo completo de uma sessão (contagem de Evidências, Eliminadas, Resolução, checkpoint TDD) sem criar um agente
- `/ecl-debug continue <slug>` — Retoma uma sessão específica pelo slug (exibe Foco Atual e então cria agente de continuação)
- `/ecl-debug [--diagnose] <description>` — Inicia nova sessão de debug (comportamento existente; `--diagnose` para na causa raiz sem aplicar correção)
...
```bash
/ecl-add-tests 2                    # Gera testes para a fase 2
```
...
```bash
/ecl-stats                          # Painel de métricas do projeto
```
...
```bash
/ecl-profile-user                   # Analisa sessões e constrói perfil
/ecl-profile-user --questionnaire   # Alternativa com questionário interativo
/ecl-profile-user --refresh         # Regenera a partir de nova análise
```
...
v1.40.0, [#2792](https://github.com/evolvconsulting/evolv-coder-lite/issues/2792)).
...
```bash
/ecl-health                         # Verifica integridade
/ecl-health --repair                # Verifica e corrige
/ecl-health --context               # Triagem de utilização de contexto
```
...
```bash
/ecl-spike                              # Intake interativo
/ecl-spike "can we stream LLM tokens through SSE"
/ecl-spike --quick websocket-vs-polling
/ecl-spike --wrap-up                    # Empacota descobertas em uma skill reutilizável
```
...
```bash
/ecl-sketch                             # Intake interativo de mood
/ecl-sketch "dashboard layout"
/ecl-sketch --quick "sidebar navigation"
/ecl-sketch --text "onboarding flow"    # Runtime que não é o Claude
/ecl-sketch --wrap-up                   # Empacota o sketch vencedor em uma skill
```
...
Investigação pós-mortem para fluxos de trabalho eCL com falha — diagnostica o que deu errado.
...
```bash
/ecl-forensics                              # Interativo — solicitação de problema
/ecl-forensics "Phase 3 execution stalled"  # Com descrição do problema
```
...
```bash
/ecl-extract-learnings 3                    # Extrai aprendizados da fase 3
/ecl-extract-learnings --all                # Extrai de todas as fases concluídas
```
...
**Pré-requisitos:** Projeto eCL ativo
**Produz:** Diretórios de workstream sob `.planning/`, rastreamento de estado por workstream

```bash
/ecl-workstreams                    # Lista todos os workstreams
/ecl-workstreams create backend-api # Cria novo workstream
/ecl-workstreams switch backend-api # Define workstream ativo
/ecl-workstreams status backend-api # Status detalhado
/ecl-workstreams progress           # Visão geral de progresso entre workstreams
/ecl-workstreams complete backend-api  # Arquiva workstream concluído
/ecl-workstreams resume backend-api    # Retoma trabalho no workstream
```
...
Todas as respostas são mescladas via `ecl-tools query config-set` no caminho de configuração do projeto resolvido (`.planning/config.json` para uma instalação padrão, ou `.planning/workstreams/<active>/config.json` quando um workstream está ativo), preservando chaves não relacionadas. Após a confirmação, o usuário pode salvar o objeto de configurações completo em `~/.ecl/defaults.json` para que execuções futuras de `/ecl-new-project` comecem da mesma linha de base.

```bash
/ecl-settings                       # Configuração interativa
```

### `/ecl-config`

Configura as definições do eCL interativamente — toggles de fluxo de trabalho, controles avançados, integrações e perfil de modelo — com um único comando consolidado.
...
Todas as respostas são mescladas via `ecl-tools query config-set`, preservando chaves não relacionadas. Chaves de API são mascaradas (`****<últimos-4>`) em todas as saídas.

```bash
/ecl-config                         # Configuração interativa de caso comum
/ecl-config --advanced              # Controles para usuários avançados (prompt de seis seções)
/ecl-config --integrations          # Chaves de API, roteamento de CLI de revisão, skills de agente
/ecl-config --profile budget        # Troca para o perfil budget
/ecl-config --profile quality       # Troca para o perfil quality
```
...
```bash
/ecl-surface list                   # Exibe a superfície atual
/ecl-surface profile standard       # Troca para o perfil standard
/ecl-surface disable utility        # Desativa o cluster utility
/ecl-surface reset                  # Restaura o perfil do momento da instalação
```
...
```bash
/ecl-map-codebase                   # Análise completa da base de código (4 agentes paralelos)
/ecl-map-codebase auth              # Foca na área de autenticação
/ecl-map-codebase --fast            # Visão geral rápida de tech + arch (1 agente)
/ecl-map-codebase --fast --focus quality  # Somente qualidade e saúde do código
/ecl-map-codebase --query authentication  # Pesquisa intel por um termo
```
...
```bash
/ecl-graphify build                 # Constrói ou reconstrói o grafo de conhecimento
/ecl-graphify query authentication  # Pesquisa o grafo por um termo
/ecl-graphify status                # Exibe frescor e estatísticas
/ecl-graphify diff                  # Exibe mudanças desde a última construção
```

**Acesso programático:** `node ecl-tools.cjs graphify <build|query|status|diff|snapshot>` — consulte a [Referência de Ferramentas CLI](CLI-TOOLS.md).

### `ecl-tools intel api-surface`

Renderiza o índice `.planning/intel/api-map.json` (construído por `/ecl-map-codebase`) em um `API-SURFACE.md` legível por humanos em `.planning/intel/`. Requer `intel.enabled: true` em `config.json`; quando Intel está desabilitado, o comando imprime uma dica de ativação e sai. O caminho de saída é sempre `.planning/intel/API-SURFACE.md` — não há flag `--out` ou `--format`. Quando `api-map.json` está ausente ou vazio, o comando ainda escreve o arquivo com um banner explícito de "incompleto" para que os consumidores nunca confundam silêncio com "nada existe".

**Produz:** `.planning/intel/API-SURFACE.md`

```bash
node ecl-tools.cjs intel api-surface              # Renderiza api-map.json → API-SURFACE.md
```
...
```bash
/ecl-ai-integration-phase              # Assistente para a fase atual
/ecl-ai-integration-phase 3           # Assistente para uma fase específica
```
...
Audita a cobertura de avaliação de uma fase de IA executada e produz um plano de remediação EVAL-REVIEW.md. Verifica a implementação em relação ao plano de avaliação `AI-SPEC.md` produzido por `/ecl-ai-integration-phase`. Classifica cada dimensão de avaliação como COVERED/PARTIAL/MISSING.
...
```bash
/ecl-eval-review                       # Audita a fase atual
/ecl-eval-review 3                     # Audita uma fase específica
```
...
Atualiza o eCL com prévia do changelog, e opcionalmente sincroniza skills ou reaplicar patches locais.

| Flag | Descrição |
|------|-----------|
| `--sync` | Sincroniza skills do registro eCL após a atualização |
| `--reapply` | Restaura modificações locais (patches) após a atualização |

```bash
/ecl-update                         # Verifica atualizações e instala
/ecl-update --sync                  # Atualiza e sincroniza skills
/ecl-update --reapply               # Atualiza e reaplicar patches locais
```
...
**Cria:** agente `ecl-code-reviewer`; agente `ecl-code-fixer` (com `--fix`)

**Pré-passagem estrutural opcional:** Defina `code_quality.fallow.enabled` como `true` para executar fallow antes da revisão pelo agente. O eCL escreve `{phase}/FALLOW.json` e incorpora uma seção `Structural Findings (fallow)` em `REVIEW.md`. Configure escopo e perfil com `code_quality.fallow.scope` e `code_quality.fallow.profile`.

```bash
/ecl-code-review 3                          # Revisão padrão para a fase 3
/ecl-code-review 2 --depth=deep             # Revisão profunda entre arquivos
/ecl-code-review 4 --files src/auth.ts,src/token.ts  # Lista explícita de arquivos
/ecl-code-review 3 --fix                    # Revisa e corrige descobertas Critical + Warning
/ecl-code-review 3 --fix --all             # Revisa e corrige todas as descobertas incluindo Info
/ecl-code-review 3 --fix --auto            # Revisa, corrige e revisita até estar limpo (máx. 3 iterações)
```
...
```bash
/ecl-audit-fix                              # Executa audit-uat, corrige problemas medium+ (máx. 5)
/ecl-audit-fix --severity high             # Corrige somente problemas de alta gravidade
/ecl-audit-fix --dry-run                   # Prévia de classificação sem correção
/ecl-audit-fix --max 10 --severity all     # Corrige até 10 problemas de qualquer gravidade
```
...
**Não substitui `/ecl-quick`** — use `/ecl-quick` para qualquer coisa que precise de pesquisa, planejamento em múltiplas etapas ou verificação.
...
- Se `review.default_reviewers` estiver **não definido**, `/ecl-review` executa todos os revisores detectados (comportamento padrão atual).
- Se `review.default_reviewers` estiver **definido**, `/ecl-review` executa somente esse subconjunto (por exemplo `["gemini","codex"]`).
...
**Produz:** `{phase}-REVIEWS.md` — consumível por `/ecl-plan-phase --reviews`

```bash
# define revisores padrão do projeto para execuções de /ecl-review sem flag
ecl config-set review.default_reviewers '["gemini","codex"]'

/ecl-review --phase 2             # executa gemini+codex da configuração
/ecl-review --phase 3 --all
/ecl-review --phase 2 --gemini
/ecl-review --phase 2 --cursor    # substituição avulsa
```
...
**Objetivo:** Revisores veem somente mudanças de código, não artefatos de planejamento do eCL.

```bash
/ecl-pr-branch                     # Filtra em relação ao main
/ecl-pr-branch develop             # Filtra em relação ao develop
```
...
**Cria:** agente `ecl-security-auditor`
...
```bash
/ecl-secure-phase                   # Audita a última fase concluída
/ecl-secure-phase 5                 # Audita uma fase específica
```
...
**Cria:** agentes `ecl-doc-writer` (um por tipo de documento), e então agentes `ecl-doc-verifier` para verificação factual
...
```bash
/ecl-docs-update                    # Gera/atualiza documentos interativamente
/ecl-docs-update --force            # Regenera todos os documentos
/ecl-docs-update --verify-only      # Somente verifica documentos existentes
```
...
**Backlog:** A numeração 999.x mantém itens fora da sequência de fases ativas; os diretórios de fases são criados imediatamente para que `/ecl-discuss-phase` e `/ecl-plan-phase` funcionem neles.
**Seeds:** Preservam o POR QUÊ completo, QUANDO expor e rastros de contexto — consumidos por `/ecl-new-milestone`.
...
```bash
/ecl-capture "Consider adding dark mode support"   # Adiciona todo
/ecl-capture --note "Caching strategy idea"        # Nota rápida
/ecl-capture --note list                           # Lista todas as notas
/ecl-capture --note promote 3                      # Promove nota 3 para todo
/ecl-capture --backlog "GraphQL API layer"         # Adiciona ao backlog
/ecl-capture --seed "Add real-time collaboration when WebSocket infra is in place"
/ecl-capture --list                                # Navega e age sobre todos
```
...
Threads são armazenamentos de conhecimento leves entre sessões para trabalho que abrange múltiplas sessões, mas não pertence a nenhuma fase específica. Mais leve que `/ecl-pause-work`.

```bash
/ecl-thread                         # Lista todas as threads
/ecl-thread list --open             # Lista somente threads abertas/em andamento
/ecl-thread list --resolved         # Lista somente threads resolvidas
/ecl-thread status fix-deploy-key   # Exibe status da thread
/ecl-thread close fix-deploy-key    # Marca thread como resolvida
/ecl-thread fix-deploy-key-auth     # Retoma thread
/ecl-thread "Investigate TCP timeout in pasta service"  # Cria nova
```
...
node ecl-tools.cjs roadmap upgrade --convention milestone-prefixed --apply  # aplicar
```
...
```bash
node ecl-tools.cjs state sync             # Reconstrói STATE.md a partir do disco
node ecl-tools.cjs state sync --verify    # Dry-run: exibe mudanças sem escrever
```
...
| Hook | Finalidade |
|------|-----------|
| `ecl-validate-commit.sh` | Impõe o formato Conventional Commits nas mensagens de commit do git |
| `ecl-session-state.sh` | Rastreia transições de estado de sessão |
| `ecl-phase-boundary.sh` | Impõe verificações de limite de fase |
...
Para participar da comunidade eCL no Discord, visite o link no README do eCL ou execute `/ecl-help` e siga o link do Discord exibido lá.

---

## Contribuindo: Padrões de Descrição de Skills

As descrições de skills (o campo `description:` no frontmatter de cada `commands/ecl/*.md`) são
