...
const capabilities = {
  ...
  "audit": {
    ...
    "role": "feature",
    "title": "Audit",
    "description": "Open-artifact audit and UAT-gap audit for milestone close gates; exposes `ecl-tools audit-uat` (cross-phase UAT outstanding items) and `ecl-tools audit-open` (structured open-artifact scan across debug, tasks, threads, todos, seeds, UAT, verification, context-questions).",
    "tier": "full",
    "requires": [],
    "runtimeCompat": {
      "supported": [
  ...
  "drift": {
    ...
    "config": {
      ...
      "workflow.drift_action": {
        ...
        "default": "warn",
        "description": "Action taken by the codebase drift gate when the threshold is exceeded: warn (advisory message) or auto-remap (spawn ecl-codebase-mapper agent to refresh STRUCTURE.md)."
      },
  ...
  "graphify": {
    ...
    "description": "Build, query, and inspect the project knowledge graph in `.planning/graphs/`; exposes graphify CLI subcommands (build, query, status, diff) and the /ecl-graphify skill.",
    "tier": "full",
    "requires": [],
    "runtimeCompat": {
      "supported": [
  ...
  "hermes": {
    ...
    "role": "runtime",
    "title": "Hermes Agent",
    "description": "Hermes Agent (NousResearch) — skills nest under skills/ecl/ category bucket; nested skill layout; settings-json hook surface; Claude hook event dialect; tier-2 support.",
    "tier": "core",
    "requires": [],
    "runtime": {
      "configHome": {
  ...
  "intel": {
    ...
    "description": "Code-intelligence store for codebase querying, diff, snapshot, and API-surface extraction; exposes `ecl-tools intel` subcommands (query, status, update, diff, snapshot, patch-meta, validate, extract-exports, api-surface) and backs `/ecl-map-codebase` and `ecl-intel-updater`.",
    "tier": "full",
    "requires": [],
    "runtimeCompat": {
      "supported": [
  ...
  "mempalace": {
    ...
    "config": {
      ...
      "mempalace.memory_mode": {
        ...
        "default": "augment",
        "description": "How MemPalace relates to eCL native memory. Only 'augment' (additive) is implemented today; 'kg_backend' and 'replace' are forward-declared (routing seam not yet built) and currently behave as 'augment'."
      },
      "mempalace.wing": {
        "type": "string",
        "default": "",
  ...
  "profile-pipeline": {
    ...
    "description": "Developer behavioral profiling from Claude Code session history; scans session JSONL files, extracts and samples user messages, and generates profile artifacts (USER-PROFILE.md, dev-preferences.md, CLAUDE.md sections). Exposes eight `ecl-tools` commands: scan-sessions, extract-messages, profile-sample (pipeline phase) and write-profile, profile-questionnaire, generate-dev-preferences, generate-claude-profile, generate-claude-md (output phase). Backs the /ecl-profile-user skill and ecl-user-profiler agent.",
    "tier": "full",
    "requires": [],
    "runtimeCompat": {
      "supported": [
...
const byAgent = {
  "ecl-framework-selector": "ai-integration",
  "ecl-ai-researcher": "ai-integration",
  "ecl-domain-researcher": "ai-integration",
  "ecl-eval-planner": "ai-integration",
  "ecl-code-reviewer": "code-review",
  "ecl-code-fixer": "code-review",
  "ecl-mempalace-curator": "mempalace",
  "ecl-nyquist-auditor": "nyquist",
  "ecl-pattern-mapper": "pattern-mapper",
  "ecl-user-profiler": "profile-pipeline",
  "ecl-phase-researcher": "research",
  "ecl-security-auditor": "security",
  "ecl-ui-checker": "ui",
  "ecl-ui-auditor": "ui"
};
