...
- Check for MILESTONE-CONTEXT.md (from /ecl-discuss-milestone)
...
ecl_run query state.milestone-switch --milestone "v[X.Y]" --name "[Name]"
```
...
```bash
ecl_run query phases.clear --confirm
```

```bash
ecl_run query commit "docs: start milestone v[X.Y] [Name]" --files .planning/PROJECT.md .planning/STATE.md
```

## 7. Load Context and Resolve Models

```bash
INIT=$(ecl_run query init.new-milestone)
...
- Tell the user to complete/archive the previous milestone first, then rerun `/ecl:new-milestone --reset-phase-numbers ${ECL_WS}`.
...
**IMPORTANT:** Do NOT persist this choice to config.json. The `workflow.research` setting is a persistent user preference that controls plan-phase behavior across the project. Changing it here would silently alter future `/ecl:plan-phase` behavior. To change the default, use `/ecl:settings`.
...
Spawn 4 parallel ecl-project-researcher agents. Each uses this template with dimension-specific fields:
...
Use template: ~/.claude/evolv-coder-lite/templates/research-project/{FILE}
</output>
", subagent_type="ecl-project-researcher", model="{researcher_model}", description="{DIMENSION} research")
```
...
**Commit requirements:**
```bash
ecl_run query commit "docs: define milestone v[X.Y] requirements" --files .planning/REQUIREMENTS.md
```
...
```bash
ecl_run query commit "docs: create milestone v[X.Y] roadmap ([N] phases)" --files .planning/ROADMAP.md .planning/STATE.md .planning/REQUIREMENTS.md
```
...
```bash
ecl_run query commit "docs: tag [count] pending todos with resolves_phase after milestone v[X.Y] roadmap" --files .planning/todos/pending/*.md
```
...
`/ecl:discuss-phase [N] ${ECL_WS}` — gather context and clarify approach

Also: `/ecl:plan-phase [N] ${ECL_WS}` — skip discussion, plan directly
```
...
- [ ] ecl-roadmapper spawned with phase numbering context
...
- [ ] User knows next step: `/ecl:discuss-phase [N] ${ECL_WS}`
