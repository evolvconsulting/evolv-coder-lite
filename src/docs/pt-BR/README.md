...
Documentação abrangente do framework evolv Coder Lite (eCL) — um sistema de meta-prompting, engenharia de contexto e desenvolvimento orientado por especificações para agentes de IA.
...
| [Ferramentas CLI](CLI-TOOLS.md) | Contribuidores, autores de agentes | Superfície CJS `ecl-tools.cjs` + guia guia de `ecl-tools.cjs query` |
...
Perfil de instalação `--minimal` (≥94% de redução no cold-start), `/ecl-phase --edit`, build & test gate pós-merge, `review.models.<cli>` para escolha de modelo de review por runtime, herança de configuração de workstream, workflow manual de canary release, consolidação de skills (86 → 59).

## Links rápidos

- **Começar rápido:** [README principal](../../README.pt-BR.md) -> instalação -> `/ecl-new-project`
