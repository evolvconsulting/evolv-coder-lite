 ...
 *
 * Context: on Windows (and occasionally other platforms), ecl-planner's
...
const PLAN_PHASE = path.join(
  __dirname, '..', 'evolv-coder-lite', 'workflows', 'plan-phase.md'
);

const PLANNER_AGENT = path.join(__dirname, '..', 'agents', 'ecl-planner.md');
const PLANNER_CHUNKED_REF = path.join(__dirname, '..', 'evolv-coder-lite', 'references', 'planner-chunked.md');
const CONFIG_SCHEMA = path.join(__dirname, '..', 'evolv-coder-lite', 'bin', 'lib', 'config-schema.cjs');
...
describe('ecl-planner.md — references planner-chunked.md (#2310)', () => {
  const plannerContent = fs.readFileSync(PLANNER_AGENT, 'utf-8');

  test('ecl-planner.md references planner-chunked.md for chunked return formats', () => {
    assert.ok(
      plannerContent.includes('planner-chunked.md'),
      'ecl-planner.md must reference planner-chunked.md for ## OUTLINE COMPLETE / ## PLAN COMPLETE formats'
    );
  });
});
