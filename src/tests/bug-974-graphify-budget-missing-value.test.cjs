 ...
 *
 * Regression guard: `ecl-tools graphify query <term> --budget` with no value
...
describe('bug #974: --budget missing value → usage error (subprocess)', () => {
  ...
  test('subprocess: --budget last arg → ECL_JSON_ERRORS=1 → usage reason', () => {
  ...
  test('subprocess: --budget foo → ECL_JSON_ERRORS=1 → usage reason', () => {
    const parsed = runJsonErrors(
      ['graphify', 'query', 'AuthService', '--budget', 'foo'],
      tmpDir,
    );
