...
const REVIEW_FAMILY = [
  { file: 'code-review.md', agentTypes: ['ecl-code-reviewer'] },
  { file: 'code-review-fix.md', agentTypes: ['ecl-code-fixer', 'ecl-code-reviewer'] },
  { file: 'eval-review.md', agentTypes: ['ecl-eval-auditor'] },
];
...
describe('agent_skills injection — review-family workflows (#991)', () => {
  for (const { file, agentType } of cases) {
    test(`${file} queries + injects agent_skills for every ${agentType} spawn`, () => {
      ...
      //    the same `ecl_run query agent-skills <type>` idiom as the ~20
      //    sibling workflows (plan-phase, execute-phase, secure-phase, ...).
      const assignRe = new RegExp(
        '([A-Z][A-Z0-9_]*)=\\$\\(\\s*ecl_run query agent-skills ' + escapeRe(agentType) + '\\s*\\)',
      );
      const m = content.match(assignRe);
      assert.ok(
        m,
        `${file}: missing \`VAR=$(ecl_run query agent-skills ${agentType})\` — configured agent_skills are never queried (#991)`,
      );
      const varName = m[1];
