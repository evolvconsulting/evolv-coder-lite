...
describe('init commands', () => {
  ...
  test('init execute-phase respects model_overrides for executor_model', () => {
    ...
    fs.writeFileSync(path.join(tmpDir, '.planning', 'config.json'), JSON.stringify({
      ...
      model_overrides: { 'ecl-executor': 'openai/o4-mini' },
    }));
    ...
    assert.strictEqual(output.executor_model, 'openai/o4-mini',
      'model_overrides["ecl-executor"] must take precedence over profile');
  });
...
describe('init plan-phase zero-padded phase number (bug #2391)', () => {
  ...
  // producing "ecl/phase-01-foundation" rather than "ecl/phase-CK-01-foundation".
  test('branch_name uses normalized phase number when project_code prefixes phase dir (#904)', () => {
    ...
    assert.strictEqual(output.branch_name, 'ecl/phase-01-foundation',
...
describe('cmdInitQuick', () => {
  ...
  test('returns quick branch name when quick_branch_template is configured', () => {
    ...
    assert.ok(output.branch_name.startsWith('ecl/quick-'));
  ...
  test('uses fallback slug in quick branch name when description is omitted', () => {
    fs.writeFileSync(
      ...
      JSON.stringify({
        git: {
          quick_branch_template: 'ecl/quick-{quick}-{slug}',
        },
      }, null, 2)
    );
...
describe('cmdInitMapCodebase', () => {
  ...
  test('map-codebase workflow does not list OpenCode under runtimes without Task tool (#1316)', () => {
    const workflow = fs.readFileSync(
      path.join(__dirname, '..', 'evolv-coder-lite', 'workflows', 'map-codebase.md'), 'utf8'
    );
...
// ─────────────────────────────────────────────────────────────────────────────
// findProjectRoot integration — ecl-tools resolves project root from sub-repo
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// ADR-0006: init handlers honor ECL_WORKSTREAM (planningPaths/planningDir consumption)
// Issue #1189 — regression guard: workstream-scoped paths must be resolved
// through planningDir(cwd) which picks up ECL_WORKSTREAM from env.
// ─────────────────────────────────────────────────────────────────────────────

describe('init handlers honor ECL_WORKSTREAM (ADR-0006 planningPaths consumption)', () => {
  ...
  describe('execute-phase — workstream-scoped path fields', () => {
    ...
    test('execute-phase emits workstream-scoped state/roadmap/config paths (ADR-0006)', () => {
      const result = runGsdTools('init execute-phase 1', tmpDir, { ECL_WORKSTREAM: 'wsx', ECL_PROJECT: '' });
      assert.ok(result.success, `Command failed: ${result.error}`);
    ...
    test('execute-phase WITHOUT ECL_WORKSTREAM resolves flat paths (boundary control)', () => {
      // Flat fixture: the workstream fixture exists but we do NOT pass ECL_WORKSTREAM.
      ...
      const result = runGsdTools('init execute-phase 1', tmpDir, { ECL_WORKSTREAM: '', ECL_PROJECT: '' });
      assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  describe('milestone-op — reads workstream-scoped planning files', () => {
    ...
    beforeEach(() => {
      ...
      const os = require('os');
      tmpDir = fs.mkdtempSync(require('path').join(os.tmpdir(), 'ecl-test-'));
      buildWsFixture(tmpDir, 'wsx');
    });
    ...
    test('milestone-op with ECL_WORKSTREAM finds roadmap/state/phases in workstream scope (ADR-0006)', () => {
      const result = runGsdTools('init milestone-op', tmpDir, { ECL_WORKSTREAM: 'wsx', ECL_PROJECT: '' });
      assert.ok(result.success, `Command failed: ${result.error}`);
    ...
    test('milestone-op WITHOUT ECL_WORKSTREAM misses workstream-only files (negative discrimination)', () => {
      const result = runGsdTools('init milestone-op', tmpDir, { ECL_WORKSTREAM: '', ECL_PROJECT: '' });
      assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  describe('plan-phase — workstream-scoped path resolution', () => {
    ...
    test('plan-phase emits workstream-scoped state/roadmap/requirements paths (ADR-0006)', () => {
      const result = runGsdTools('init plan-phase 1', tmpDir, { ECL_WORKSTREAM: 'wsx', ECL_PROJECT: '' });
      assert.ok(result.success, `Command failed: ${result.error}`);
    ...
    test('plan-phase WITHOUT ECL_WORKSTREAM resolves flat paths (boundary control)', () => {
      const result = runGsdTools('init plan-phase 1', tmpDir, { ECL_WORKSTREAM: '', ECL_PROJECT: '' });
      assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  describe('phase-op — workstream-scoped phase resolution', () => {
    ...
    test('phase-op with ECL_WORKSTREAM finds phase in workstream scope and emits scoped paths (ADR-0006)', () => {
      const result = runGsdTools('init phase-op 1', tmpDir, { ECL_WORKSTREAM: 'wsx', ECL_PROJECT: '' });
      assert.ok(result.success, `Command failed: ${result.error}`);
    ...
    test('phase-op WITHOUT ECL_WORKSTREAM does not find workstream-only phase (negative discrimination)', () => {
      const result = runGsdTools('init phase-op 1', tmpDir, { ECL_WORKSTREAM: '', ECL_PROJECT: '' });
      assert.ok(result.success, `Command failed: ${result.error}`);
