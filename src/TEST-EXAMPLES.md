...
This document shows the kinds of tests eCL expects for high-risk changes. Use it with the testing standards in [`CONTRIBUTING.md`](CONTRIBUTING.md).
...
for (const scenario of cases) {
  test(`ecl-tools rejects ${scenario.name}`, (t) => {
    const projectDir = createTempProject('cli-negative-');
    t.after(() => cleanup(projectDir));

    const result = spawnSync(
      process.execPath,
      [path.join(__dirname, '..', 'evolv-coder-lite', 'bin', 'ecl-tools.cjs'), ...scenario.args, '--json'],
...
test('installer refuses symlink escape outside target root', (t) => {
  ...
  assert.equal(fs.existsSync(path.join(outside, 'ecl-prompt-guard.js')), false);
});
```
