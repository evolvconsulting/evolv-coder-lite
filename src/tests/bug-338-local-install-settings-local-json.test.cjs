 ...
 *  3. Migration: if a prior local install wrote eCL entries to settings.json,
...
describe('#338 case 1: fresh local Claude install writes to settings.local.json', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-338-local-'));
  });
...
describe('#338 case 2: global Claude install continues to write to settings.json', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-338-global-'));
  });
...
// ─── Case 3: migration — prior local install wrote eCL entries to settings.json ─

describe('#338 case 3: migration of prior local install eCL entries from settings.json to settings.local.json', () => {
  let tmpDir;

  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-338-migrate-'));
  });
  ...
  test('eCL hook entries are moved from settings.json to settings.local.json', (t) => {
    const origCwd = process.cwd();
    t.after(() => { process.chdir(origCwd); });
    process.chdir(tmpDir);

    // Pre-populate .claude/settings.json with a eCL-shaped hook block (simulating
        ...
        SessionStart: [
          {
            hooks: [
              {
                type: 'command',
                command: `${process.execPath} ${path.join(claudeDir, 'hooks', 'ecl-check-update.js')}`,
              }
            ]
          }
        ],
        PostToolUse: [
          {
            ...
            hooks: [
              {
                type: 'command',
                command: `${process.execPath} ${path.join(claudeDir, 'hooks', 'ecl-context-monitor.js')}`,
                timeout: 10,
              }
            ]
          }
    ...
    // Verify eCL entries are now in settings.local.json
    ...
    const hasGsdUpdateHook = sessionStartHooks.some(
      entry => entry && entry.hooks && Array.isArray(entry.hooks) &&
        entry.hooks.some(h => h && h.command && h.command.includes('ecl-check-update'))
    );
    assert.ok(
      hasGsdUpdateHook,
      'settings.local.json must contain the migrated ecl-check-update hook after migration'
    );
  });

  test('eCL hook entries are removed from settings.json after migration', (t) => {
    const origCwd = process.cwd();
    t.after(() => { process.chdir(origCwd); });
    process.chdir(tmpDir);
    ...
    const sharedSettingsPath = path.join(claudeDir, 'settings.json');
    const priorSettings = {
      // Include a non-eCL key to verify user content is preserved
      myCustomKey: 'keep-me',
      hooks: {
        SessionStart: [
          {
    ...
    assert.ok(
      fs.existsSync(sharedSettingsPath),
      '.claude/settings.json must still exist after migration (may have non-eCL user content)'
    );
    const sharedSettings = JSON.parse(fs.readFileSync(sharedSettingsPath, 'utf-8'));

    // eCL hooks must be gone from settings.json
    ...
    assert.strictEqual(
      hasGsdHook,
      false,
      'eCL hook entries must be removed from settings.json after migration to settings.local.json'
    );

    // Non-eCL user content must be preserved
    assert.strictEqual(
      sharedSettings.myCustomKey,
      'keep-me',
      'Non-eCL user content in settings.json must be preserved during migration'
    );
  });

  test('settings.json with no eCL entries is left unchanged', (t) => {
    const origCwd = process.cwd();
    t.after(() => { process.chdir(origCwd); });
    process.chdir(tmpDir);
    ...
    // settings.json must be unchanged (no eCL entries to migrate)
    ...
    assert.strictEqual(
      ...
      'user-value',
      'Non-eCL settings.json must be untouched when no eCL entries are present'
    );
