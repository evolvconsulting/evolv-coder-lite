...
**Goal:** Understand why `/ecl-execute-phase` halts with `FATAL: worktree base mismatch` / exit 42 when your branch is ahead of the default branch, and choose the right fix to restore normal — or parallel — execution.

**Prerequisites:** eCL Core is installed and you have an active project. You have run `/ecl-execute-phase` and either seen the exit-42 error or the one-line `⚠ Worktree base mismatch` warning.

---

## What you will see

When you run `/ecl-execute-phase` on a branch that is ahead of the repository's default branch (for example, an unmerged milestone branch, a long-lived feature branch, or a branch with commits not yet in `origin/HEAD`), you may see one of two messages:
...
.claude/settings.local.json (or run: ecl-tools worktree set-baseref). See #683.
```
...
If you saw the `⚠ Worktree base mismatch` warning rather than an exit-42 halt, eCL has already automatically degraded to sequential execution on the main working tree for this run. The phase will complete. No action is required.
...
```bash
node "$HOME/.claude/evolv-coder-lite/bin/ecl-tools.cjs" worktree base-check
```
...
**Note:** Fresh installs and upgrades of eCL Core both set `worktree.baseRef:"head"` automatically in `.claude/settings.local.json` (no-clobber) when `workflow.use_worktrees` is enabled (the default). You can also apply or re-apply it manually at any time with `ecl-tools worktree set-baseref` — for example, if you toggled worktrees on after the initial install.
...
| Still seeing exit 42 after fixes | Run `/ecl-forensics "exit 42 after fix"` |
