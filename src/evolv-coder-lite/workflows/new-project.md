...
INIT=$($ECL_SDK query init.new-project)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_RESEARCHER=$($ECL_SDK query agent-skills ecl-project-researcher)
AGENT_SKILLS_SYNTHESIZER=$($ECL_SDK query agent-skills ecl-research-synthesizer)
AGENT_SKILLS_ROADMAPPER=$($ECL_SDK query agent-skills ecl-roadmapper)
```
...
```bash
mkdir -p .planning
$ECL_SDK query commit "chore: add project config" --files .planning/config.json
```
...
```bash
mkdir -p .planning
$ECL_SDK query commit "docs: initialize project" --files .planning/PROJECT.md
```
...
```bash
$ECL_SDK query commit "docs: define v1 requirements" --files .planning/REQUIREMENTS.md
```
...
```bash
$ECL_SDK query generate-claude-md --output "$INSTRUCTION_FILE"
```
...
```bash
$ECL_SDK query commit "docs: create roadmap ([N] phases)" --files .planning/ROADMAP.md .planning/STATE.md .planning/REQUIREMENTS.md "$INSTRUCTION_FILE"
```
...
```bash
PHASE1_SECTION=$($ECL_SDK query roadmap.get-phase 1 2>/dev/null)
...
- [ ] `$INSTRUCTION_FILE` generated with eCL workflow guidance (AGENTS.md for Codex, CLAUDE.md otherwise)
