 ...
 * ecl-session-state.sh, ecl-validate-commit.sh, and ecl-phase-boundary.sh
...
describe('#1834: installer deploys .sh hooks alongside .js hooks', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('ecl-install-1834-');
  });
  ...
  test('ecl-session-state.sh is present after install', () => {
    const hooksDir = runInstaller(tmpDir);
    const target = path.join(hooksDir, 'ecl-session-state.sh');
    assert.ok(
      fs.existsSync(target),
      'ecl-session-state.sh must be installed to hooks/ — missing file causes SessionStart hook errors'
    );
  });

  test('ecl-validate-commit.sh is present after install', () => {
    const hooksDir = runInstaller(tmpDir);
    const target = path.join(hooksDir, 'ecl-validate-commit.sh');
    assert.ok(
      fs.existsSync(target),
      'ecl-validate-commit.sh must be installed to hooks/ — missing file causes PreToolUse hook errors'
    );
  });

  test('ecl-phase-boundary.sh is present after install', () => {
    const hooksDir = runInstaller(tmpDir);
    const target = path.join(hooksDir, 'ecl-phase-boundary.sh');
    assert.ok(
      fs.existsSync(target),
      'ecl-phase-boundary.sh must be installed to hooks/ — missing file causes PostToolUse hook errors'
    );
  });
