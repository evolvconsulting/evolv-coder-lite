# How to attach a plugin-provided skill to a eCL agent

Use this guide when you have a Claude Code plugin that ships a skill and you want eCL agents to load it automatically at spawn time. The `global:<plugin>:<skill>` entry form makes eCL emit a Skill-tool load directive so the agent picks up the plugin skill alongside any project-local or personal global skills.
...
eCL does not install or manage Claude Code plugins. Install the plugin first:
...
```bash
ecl-tools query config-set agent_skills.ecl-executor '["skills/project-conventions","global:coderabbit:code-review"]'
```
...
- **Plugin must be pre-installed.** eCL only references the skill by namespaced name. If the plugin is not installed in Claude Code, the Skill tool call will fail at agent runtime — eCL cannot validate plugin presence at configuration time.
- **Which agents support it.** All 22 eCL agent types that consume `agent_skills` carry the `Skill` tool, so any of them can load plugin-provided skills. See [Agent Skills Injection — Supported Agent Types](../CONFIGURATION.md#supported-agent-types) for the full list.
...
- **Security boundary.** Plugin skill content is resolved entirely by Claude Code at agent runtime. eCL does not read, cache, or validate the plugin's files.
