...
// ── Env hermeticity (strip ambient ECL_ vars that skew planning dir lookups) ──
const CLEAN_ENV = Object.fromEntries(
  Object.entries(process.env).filter(([k]) => !k.startsWith('ECL_')),
);

/**
 * Invoke ecl-tools CLI with spawnSync and return the parsed result.
 * Always use CLEAN_ENV to avoid ambient ECL_ env vars redirecting planning paths.
 */
