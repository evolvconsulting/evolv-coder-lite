...
// from #621): NO workflow .md may invoke ecl-tools via a hardcoded
// `node "$HOME/.../ecl-tools.cjs"` path. On a global/shim-only install with no
...
// go through the `ecl_run` launcher (defined once per file in the canonical
// preamble, which resolves RUNTIME_DIR → .claude → PATH → $HOME in order).
//
// The parity test (runtime-launcher-parity) guards the retired $ECL_SDK and
// bare /ecl-tools tokens but NOT this hardcoded-node form — which is exactly
...
// references $HOME only inside a `[ -f "$HOME/..." ]` probe / `ECL_TOOLS=`
// assignment and always invokes `node "$ECL_TOOLS"` — never `node "$HOME/..."`.
const HARDCODED_HOME_INVOCATION = /node\s+"\$HOME\/[^"]*ecl-tools\.cjs"/;
...
describe('bug #637: no workflow .md hardcodes a $HOME ecl-tools invocation', () => {
  test('every evolv-coder-lite/workflows/**/*.md resolves ecl-tools via ecl_run, not a hardcoded $HOME path', () => {
    ...
    assert.deepStrictEqual(
      offenders,
      [],
      'Workflow files must invoke ecl-tools via the resolved `ecl_run` launcher, ' +
        'not a hardcoded `node "$HOME/.../ecl-tools.cjs"` path. Offenders:\n' +
        offenders.join('\n'),
    );
  });
});
