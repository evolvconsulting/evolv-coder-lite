...
The **Runtime Surface Module** (`evolv-coder-lite/bin/lib/surface.cjs`, introduced by ADR-0011 Phase 2) re-materializes a resolved Skill Surface profile to disk via `applySurface`. It currently hardcodes two artifact kinds (`commands`, `agents`) and re-derives their source directories via `_findInstallSource` / `_findAgentsSource` walk-up heuristics. The install and uninstall pipelines in `bin/install.js` each encode the same per-runtime artifact layout independently across ~14 install sites and ~6 uninstall sites. Bug #3659 surfaced the resulting drift: `applySurface` omits the `skills` kind for runtimes whose canonical layout is `skills/ecl-<stem>/SKILL.md`, so `ecl-surface profile <name>` leaves ~67 skill directories on disk under the install-time profile's footprint when the resolved profile should have pruned them — roughly 2.7k tokens per session on a measured workstation.
...
- Add a **Runtime Artifact Layout Module** at `evolv-coder-lite/bin/lib/runtime-artifact-layout.cjs` as the single owner of the per-runtime artifact-placement table.
...
- Legacy-layout migrations (`bin/install.js:6710`/`:8402` for the pre-nested `skills/ecl-*/` flat layout; `migrateLegacyDevPreferencesToSkill` for #2973) remain inside the Installer Migration Module (ADR-0008) and run **before** layout-driven copy. The layout module describes only the current canonical target — no historical kinds.

## Initial Scope

Phase 1 should land the module and one consumer (the bug-#3659 fix):

1. New `evolv-coder-lite/bin/lib/runtime-artifact-layout.cjs` — `resolveRuntimeArtifactLayout`, the typed `Layout`/`ArtifactKind` shapes, and the runtime table covering every runtime currently enumerated in `runtime-homes.cjs`.
...
3. `commands/ecl/surface.md` and `tests/surface-apply.test.cjs` updated to construct + pass `Layout` values.
...
### New file
- `evolv-coder-lite/bin/lib/runtime-artifact-layout.cjs` — module body + runtime layout table.

### Files modified (Phase 1)
- `evolv-coder-lite/bin/lib/surface.cjs` — `applySurface` signature change; `_findInstallSource` + `_findAgentsSource` removal.
- `commands/ecl/surface.md` — runbook updates the 3 sites that call `applySurface` to first call `resolveRuntimeArtifactLayout`.
 ...
 * @property {string} prefix                   'ecl-' for all runtimes (incl. Hermes after #947)
 ...
 * @property {ArtifactKind[]} kinds            empty array = runtime has no ecl-* surface
 */
...
```js
// commands/ecl/surface.md (runbook), tests/surface-apply.test.cjs, future install/uninstall
...
- Confirmed bug: `#3659` — `applySurface` doesn't prune `~/.claude/skills/ecl-*/` dirs
...
- Existing canonical sibling: `evolv-coder-lite/bin/lib/runtime-homes.cjs`
...
- `evolv-coder-lite/bin/lib/runtime-artifact-layout.cjs` — 15-runtime layout table (grok intentionally excluded), `resolveRuntimeArtifactLayout(runtime, configDir, scope) → Layout`, walk-up `findInstallSourceRoot` helper.
...
- `evolv-coder-lite/bin/lib/install-profiles.cjs` — new `stageSkillsForRuntimeAsSkills(srcCommandsDir, resolvedProfile, converter, prefix) → stagedDir` helper.
- `evolv-coder-lite/bin/lib/surface.cjs` — `applySurface(runtimeConfigDir, layout, manifest, clusterMap)` signature migration; `_findInstallSource` + `_findAgentsSource` deleted; `_syncGsdDir` extended to handle the `skills` kind via directory iteration.
...
- **Layout:** `case 'qoder'` → `[ skillsKind('skills', 'ecl-', 'convertClaudeCommandToQoderSkill', 'qoder', configDir) ]`; `'qoder'` added to `ALLOWED_RUNTIMES`.
...
**Path note:** since ADR-457's `.cts` migration, the canonical module source is `src/runtime-artifact-layout.cts` (built to `evolv-coder-lite/bin/lib/runtime-artifact-layout.cjs`); the runtime enumeration is owned by `src/runtime-homes.cts` and `src/runtime-config-adapter-registry.cts`. The original body above predates that move; paths read accordingly.
