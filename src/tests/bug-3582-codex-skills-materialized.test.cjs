 ...
 *
 * Background: eCL 1.42.2 reported the user-visible failure
 *   > Skipped Codex skill-copy generation (Codex discovers official skills directly)
 * which left users with a "successful" install but no routable `$ecl-*`
 * entrypoints in Codex CLI 0.130.0. Codex CLI does NOT auto-discover
 * commands from `~/.codex/evolv-coder-lite/workflows/*.md` or `agents/*.md`;
 ...
 * commands/ecl/*.md, with Claude-flavored command frontmatter rewritten
...
// ECL_TEST_MODE neutralizes side-effecting branches (auto-detection, etc.).
...
/**
 * Walk commands/ecl/**\/*.md and return the set of skill names the installer
 ...
 * `ecl-<dir>-<file>` with the .md stripped.
 */
...
function runCodexInstallCaptured() {
  const codexHome = createTempDir('ecl-3582-codex-');
  const logs = [];
  const warnings = [];
...
describe('bug-3582: Codex global install materializes the skill surface', { concurrency: false }, () => {
  ...
  test('writes the exact expected set of ecl-*/SKILL.md skills (deepEqual on name set)', () => {
    ...
    // same-count `ecl-*` directory cannot pass.
    const expected = [...expectedSkillNames()].sort();
    assert.deepStrictEqual(
      [...actualNames].sort(),
      expected,
      `installed Codex skills must exactly match commands/ecl/**/*.md (one skill per command)`,
    );
  ...
  test('representative skills named in the issue report are present', () => {
    ...
    const representative = [
      'ecl-map-codebase',     // the literal command from the bug report
      'ecl-execute-phase',
      'ecl-plan-phase',
      'ecl-new-project',
      'ecl-health',
  ...
  test('installed Codex skills do not ask agents to run bare ecl-tools commands', () => {
    const skillsDir = path.join(installRun.codexHome, 'skills');
    const skillDirs = fs.readdirSync(skillsDir, { withFileTypes: true })
      .filter(e => e.isDirectory() && e.name.startsWith('ecl-'))
      .map(e => e.name);
