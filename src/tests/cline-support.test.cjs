...
describe('Cline runtime support', () => {
  ...
  test('.clinerules references eCL workflow enforcement', () => {
    const content = fs.readFileSync(path.join(__dirname, '..', '.clinerules'), 'utf-8');
    assert.ok(
      content.includes('ecl') || content.includes('eCL') || content.includes('workflow'),
      '.clinerules should mention eCL workflows'
    );
  });
