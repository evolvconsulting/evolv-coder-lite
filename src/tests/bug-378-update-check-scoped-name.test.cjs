 ...
 * querying the SCOPED package name (@evolvconsulting/evolv-coder-lite) when it asks
 * npm for the latest version.
 *
 * Background (#378): the worker once hardcoded the unscoped 'evolv-coder-lite',
 ...
 * Identity seam (`evolv-coder-lite/bin/lib/package-identity.cjs`). The seam's value
 ...
 *      packageName === the scoped '@evolvconsulting/evolv-coder-lite'.
 *
 * Source-grep policy: this test reads hook source via readFileSync. The repo's
 * lint-no-source-grep rule targets bin/lib/evolv-coder-lite — hooks/ is out of
...
const SEAM = require('../evolv-coder-lite/bin/lib/package-identity.cjs');
const { PACKAGE_NAME } = require('../evolv-coder-lite/bin/check-latest-version.cjs');
...
describe('bug #378 / #498: update worker queries the scoped name via the seam', () => {
  ...
  test('package.json name is the scoped @evolvconsulting/evolv-coder-lite', () => {
    const pkg = JSON.parse(fs.readFileSync(PKG_PATH, 'utf8'));
    assert.equal(pkg.name, '@evolvconsulting/evolv-coder-lite');
  });

  test('worker does NOT hardcode the unscoped evolv-coder-lite as a string literal', () => {
    assert.doesNotMatch(
      workerCodeOnly(),
      /['"]evolv-coder-lite['"]/,
      "Worker must not pass the unscoped 'evolv-coder-lite' to npm — it 404s.",
    );
  });
  ...
  test('check-latest-version PACKAGE_NAME is single-sourced from the seam', () => {
    ...
    assert.equal(SEAM.packageName, '@evolvconsulting/evolv-coder-lite');
  });
});
