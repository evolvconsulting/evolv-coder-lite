 ...
 *
 * Seam: evolv-coder-lite/workflows/{execute-phase,execute-plan,quick}.md,
 *       agents/ecl-executor.md, references/git-integration.md
 *
...
const EXECUTE_PHASE_PATH = path.join(REPO_ROOT, 'evolv-coder-lite', 'workflows', 'execute-phase.md');
const EXECUTE_PLAN_PATH = path.join(REPO_ROOT, 'evolv-coder-lite', 'workflows', 'execute-plan.md');
const QUICK_PATH = path.join(REPO_ROOT, 'evolv-coder-lite', 'workflows', 'quick.md');
const EXECUTOR_AGENT_PATH = path.join(REPO_ROOT, 'agents', 'ecl-executor.md');
const GIT_INTEGRATION_PATH = path.join(REPO_ROOT, 'evolv-coder-lite', 'references', 'git-integration.md');
const WORKTREE_BRANCH_CHECK_FRAGMENT = path.join(REPO_ROOT, 'evolv-coder-lite', 'references', 'worktree-branch-check.md');
...
describe('bug #2924: worktree HEAD attachment + destructive recovery', () => {
  ...
  describe('ecl-executor.md prohibits update-ref self-recovery', () => {
    ...
    test('destructive_git_prohibition block exists', () => {
      assert.ok(block, 'ecl-executor.md must contain a <destructive_git_prohibition> block');
    });
  ...
  describe('ecl-executor.md task_commit_protocol enforces worktree-agent-* allow-list', () => {
    const content = fs.readFileSync(EXECUTOR_AGENT_PATH, 'utf-8');
    const block = extractNamedBlock(content, 'task_commit_protocol');

    test('task_commit_protocol block exists', () => {
      assert.ok(block, 'ecl-executor.md must contain a <task_commit_protocol> block');
    });
  ...
  describe('no workflow file performs unconditional update-ref on a protected branch', () => {
    const workflowsDir = path.join(REPO_ROOT, 'evolv-coder-lite', 'workflows');
...
describe('worktree cleanup after executor completes (#1496)', () => {
  const executePhasePath = path.join(__dirname, '..', 'evolv-coder-lite', 'workflows', 'execute-phase.md');
  const quickPath = path.join(__dirname, '..', 'evolv-coder-lite', 'workflows', 'quick.md');
  ...
  test('quick.md includes worktree cleanup after executor returns', () => {
    ...
    assert.ok(
      ...
      'quick cleanup must delegate to ecl_run query worktree.cleanup-wave (#3797)',
    );
  });
...
describe('worktree merge: orchestrator file protection (#1756)', () => {
  ...
  test('execute-phase.md delegates wave cleanup to SDK with fail-closed || exit 1', () => {
    ...
    assert.match(
      ...
      'execute-phase.md must delegate to ecl_run query worktree.cleanup-wave with || exit 1 (#3797)',
    );
  });
  ...
  test('quick.md delegates wave cleanup to SDK with fail-closed || exit 1', () => {
    ...
    assert.match(
      ...
      'quick.md must delegate to ecl_run query worktree.cleanup-wave with || exit 1 (#3797)',
    );
  });
});
...
describe('worktree commit safety hardening (#1977)', () => {
  ...
  test('ecl-executor.md task_commit_protocol includes post-commit deletion verification', () => {
    const content = fs.readFileSync(EXECUTOR_AGENT_PATH, 'utf-8');
...
describe('bug #3384: worktree cleanup workflow contracts', () => {
    ...
    assert.match(content, /WT_PATHS_FILE=.*ecl-worktree-paths-/);
  ...
  test('#1297 ecl-executor self-reports authoritative worktree metadata', () => {
    ...
    assert.match(content, /ECL_WORKTREE_EXPECTED_BASE=\$\(git rev-parse HEAD\)/);
  ...
  test('quick contract requires a cleanup manifest instead of global worktree discovery', () => {
    ...
    assert.match(content, /worktree\.cleanup-wave/);
    assert.match(content, /mktemp "\$\{TMPDIR:-\/tmp\}\/ecl-quick-worktree-/);
    ...
    // QUICK_WORKTREE_MANIFEST node-e code is removed — the ecl_run call with || exit 1 is the
    // only cleanup path, enforcing safety-refusal semantics (#3174/#3384).
    assert.match(content, /ecl_run query worktree\.cleanup-wave --manifest "\$QUICK_WORKTREE_MANIFEST" \|\| exit 1/);
...
test('#3425: helper cleanup path pins orchestrator CWD to primary worktree and checks EXPECTED_BRANCH', () => {
  ...
  assert.match(content, /ecl_run query worktree\.cleanup-wave --manifest "\$WAVE_WORKTREE_MANIFEST"/);
});
