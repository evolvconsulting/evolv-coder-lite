...
describe('formatGsdSlash — descriptor-driven commandStyle (ADR-857 phase 5c)', () => {
  ...
  test('codex (commandStyle=shell-var) → $ecl- with lowercased token', () => {
    ...
    assert.strictEqual(formatGsdSlash('Foo', 'codex'), '$ecl-foo');
  });

  test('slash-hyphen runtime (claude) → /ecl- with case-preserved token', () => {
    // claude descriptor has commandStyle=slash-hyphen. Token case is preserved.
    assert.strictEqual(formatGsdSlash('Foo', 'claude'), '/ecl-Foo');
  });

  test('slash-hyphen runtime (cursor) → /ecl- with case-preserved token', () => {
    // cursor descriptor has commandStyle=slash-hyphen.
    assert.strictEqual(formatGsdSlash('Foo', 'cursor'), '/ecl-Foo');
  });

  test('unknown runtime (no registry entry) → /ecl- default (slash-hyphen fallback)', () => {
    // An unknown runtime has no descriptor entry → runtimes[rt] is undefined →
    // style is undefined → not 'shell-var' → falls through to /ecl- default.
    assert.strictEqual(formatGsdSlash('foo', 'doesnotexist'), '/ecl-foo');
  });
});
...
describe('formatGsdSlash — registry-parity: prefix and casing are a pure function of commandStyle', () => {
  ...
  const { runtimes } = require(
    path.join(ROOT, 'evolv-coder-lite', 'bin', 'lib', 'capability-registry.cjs'),
  );
  ...
  for (const [id, descriptor] of Object.entries(runtimes)) {
    ...
    test(`${id}: commandStyle=${style} → formatGsdSlash prefix and casing match registry`, () => {
      ...
      if (style === 'shell-var') {
        // shell-var runtimes must emit $ecl-<lowercased-token>
        assert.ok(
          result.startsWith('$ecl-'),
          `[${id}] expected $ecl- prefix (commandStyle=shell-var), got: ${result}`,
        );
        assert.strictEqual(
          result,
          `$ecl-${TOKEN_MIXED.toLowerCase()}`,
          `[${id}] shell-var must lowercase the token`,
        );
      } else {
        // slash-hyphen (or any other non-shell-var value) must emit /ecl-<token>
        // with case preserved (no lowercasing)
        assert.ok(
          result.startsWith('/ecl-'),
          `[${id}] expected /ecl- prefix (commandStyle=${style}), got: ${result}`,
        );
        assert.strictEqual(
          result,
          `/ecl-${TOKEN_MIXED}`,
