...
const LIB = path.join(REPO_ROOT, 'evolv-coder-lite', 'bin', 'lib');
...
describe('ADR-857 predicate boundary: substrate loads as core (no capabilities active)', () => {
  test('probe-core.cjs resolves from evolv-coder-lite/bin/lib (core path)', () => {
  ...
  test('edge-probe.cjs resolves from evolv-coder-lite/bin/lib (core path)', () => {
    assert.ok(
...
describe('ADR-857 predicate boundary: substrate lives in core, not in capabilities/', () => {
  test('probe-core.cjs is resolvable from evolv-coder-lite/bin/lib — the core module tier', () => {
    // Must resolve from core lib, not from any capability folder
    const resolved = require.resolve(PROBE_CORE_PATH);
    assert.ok(
      resolved.includes(path.join('evolv-coder-lite', 'bin', 'lib')),
      `probe-core.cjs must resolve from evolv-coder-lite/bin/lib (got: ${resolved})`
    );
    assert.ok(
  ...
  test('edge-probe.cjs is resolvable from evolv-coder-lite/bin/lib — the core module tier', () => {
    const resolved = require.resolve(EDGE_PROBE_PATH);
    assert.ok(
      resolved.includes(path.join('evolv-coder-lite', 'bin', 'lib')),
      `edge-probe.cjs must resolve from evolv-coder-lite/bin/lib (got: ${resolved})`
    );
    assert.ok(
      !resolved.includes('capabilities'),
