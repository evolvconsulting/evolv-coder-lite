/**
 * Integration test — proves PhaseRunner state machine works against real ecl-tools.cjs.
 *
...
const ECL_TOOLS_PATH = resolveGsdToolsPath(process.cwd());
const gsdToolsAvailable = existsSync(ECL_TOOLS_PATH);

async function createTempPlanningDir(): Promise<string> {
  const tmpDir = await mkdtemp(join(tmpdir(), 'ecl-sdk-phase-int-'));
...
describe.skipIf(!gsdToolsAvailable)('Integration: PhaseRunner against real ecl-tools.cjs', () => {
  let tmpDir: string;
  let tools: GSDTools;
  ...
  // ── Test 4: eCL.runPhase() public API delegates correctly ──

  it('eCL.runPhase() creates collaborators and delegates to PhaseRunner', { timeout: 300_000 }, async () => {
    // Import eCL here to test the public API wiring
    const { eCL } = await import('./index.js');

    const ecl = new eCL({ projectDir: tmpDir });
    const events: GSDEvent[] = [];
    ecl.onEvent((e) => events.push(e));

    const result = await ecl.runPhase('01', {
      maxTurnsPerStep: 2,
      maxBudgetPerStep: 0.10,
    });

    // Proves the full wiring works: eCL → PhaseRunner → GSDTools → ecl-tools.cjs
    expect(result.phaseNumber).toBe('01');
    expect(result.phaseName).toBe('integration-test');
    expect(result.steps.length).toBeGreaterThanOrEqual(1);
...
async function createMultiWavePlanningDir(): Promise<string> {
  const tmpDir = await mkdtemp(join(tmpdir(), 'ecl-sdk-wave-int-'));
