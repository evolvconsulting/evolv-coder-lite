...
| Artifact | Value |
|---|---|
| npm registry (`@evolvconsulting/evolv-coder-lite`, `@evolvconsulting/ecl-sdk`) | `1.1.0` — published, immutable |
| git tag | `v1.01.0` |
...
A new `Reject already-published versions` step runs immediately after format validation in the `validate-version` job. It calls `npm view "$pkg@$VERSION" version` for both `@evolvconsulting/evolv-coder-lite` and `@evolvconsulting/ecl-sdk`. If either resolves, the job fails in under 5 seconds with a clear error. No build or test cycle is wasted.
...
```sh
gh run cancel <run-id> --repo evolvconsulting/evolv-coder-lite
```
...
gh release edit v1.01.0 --repo evolvconsulting/evolv-coder-lite --tag v1.1.0
gh release edit v1.1.0  --repo evolvconsulting/evolv-coder-lite --verify-tag
