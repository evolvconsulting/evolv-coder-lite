...
/**
 * Bug #2419: ecl-project-researcher agent type not found
 *
 * When ecl-new-project spawns ecl-project-researcher subagents, it fails with
...
const NEW_PROJECT_PATH = path.join(__dirname, '..', 'evolv-coder-lite', 'workflows', 'new-project.md');
const NEW_MILESTONE_PATH = path.join(__dirname, '..', 'evolv-coder-lite', 'workflows', 'new-milestone.md');
const AGENTS_DIR = path.join(__dirname, '..', 'agents');

describe('ecl-project-researcher agent registration (#2419)', () => {
  test('ecl-project-researcher.md exists in agents source dir', () => {
    const agentFile = path.join(AGENTS_DIR, 'ecl-project-researcher.md');
    assert.ok(
      fs.existsSync(agentFile),
      'agents/ecl-project-researcher.md must exist in the source agents directory'
    );
  });

  test('ecl-project-researcher.md has correct name in frontmatter', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'ecl-project-researcher.md'), 'utf-8');
    assert.ok(
      content.includes('name: ecl-project-researcher'),
      'agents/ecl-project-researcher.md must have name: ecl-project-researcher in frontmatter'
    );
  });
  ...
  test('new-project.md lists ecl-project-researcher in available_agent_types', () => {
    const content = fs.readFileSync(NEW_PROJECT_PATH, 'utf-8');
    ...
    assert.ok(
      agentTypesMatch[1].includes('ecl-project-researcher'),
      'new-project.md <available_agent_types> must list ecl-project-researcher'
    );
  });

  test('new-milestone.md lists ecl-project-researcher in available_agent_types', () => {
    const content = fs.readFileSync(NEW_MILESTONE_PATH, 'utf-8');
    const agentTypesMatch = content.match(/<available_agent_types>([\s\S]*?)<\/available_agent_types>/);
    assert.ok(agentTypesMatch, 'new-milestone.md must have <available_agent_types> section');
    assert.ok(
      agentTypesMatch[1].includes('ecl-project-researcher'),
      'new-milestone.md <available_agent_types> must list ecl-project-researcher'
    );
  });
});
