...
function ensureCodexHooksJsonSessionStart(targetDir, opts = {}) {
  ...
  const scriptPath = path.resolve(targetDir, 'hooks', 'ecl-check-update.js');
...
function convertSlashCommandsToCodexSkillMentions(content) {
  ...
  // Negative lookbehind excludes file paths like bin/ecl-tools.cjs where
  // the slash is preceded by a word char, dot, or another slash.
  converted = converted.replace(/(?<![a-zA-Z0-9./])\/ecl-([a-z0-9-]+)/gi, (_, commandName) => {
    return `$ecl-${String(commandName).toLowerCase()}`;
  });
  return converted;
}
...
function writeHermesCategoryDescription(categoryDir) {
  ...
  const body = [
    ...
    'name: ecl',
    `version: ${pkg.version}`,
    'description: evolv Coder Lite — disciplined planning, execution, and shipping workflows. Use any ecl-* skill in this category to drive a project through new-project → discuss-phase → plan-phase → execute-phase → ship.',
    '---',
    '',
...
function install(isGlobal, runtime = 'claude', options = {}) {
  ...
  if (verifyInstalled(skillDest, 'evolv-coder-lite')) {
    console.log(`  ${green}✓${reset} Installed evolv-coder-lite`);
  } else {
  ...
  // Cache lives at ~/.cache/ecl/ (see hooks/ecl-check-update.js line 35-36)
  const updateCacheFile = path.join(os.homedir(), '.cache', 'ecl', 'ecl-update-check.json');
  ...
  if (isCodex && !isMinimalMode(_effectiveInstallMode)) {
    ...
    // Codex primarily needs ecl-check-update.js for the SessionStart update-check hook.
    ...
    const CODEX_HOOKS_TO_COPY = ['ecl-check-update.js'];
  ...
  if (isCline) {
    // Cline uses .clinerules — generate a rules file with eCL system instructions
