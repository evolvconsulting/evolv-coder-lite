<purpose>
Capture an idea, task, or issue that surfaces during a eCL session as a structured todo for later work. Enables "thought → capture → continue" flow without losing context.
</purpose>
...
- `/ecl-add-todo Add auth token refresh` → title = "Add auth token refresh"
...
```bash
slug=$($ECL_SDK query generate-slug "$title" --raw)
```
...
```bash
$ECL_SDK query commit "docs: capture todo - [title]" --files .planning/todos/pending/[filename] .planning/STATE.md
```
...
2. Add another todo
3. View all todos (/ecl:capture --list)
```
</step>
