/**
 * Tests for ecl-statusline.js eCL state display helpers.
 *
 * Covers:
...
} = require('../hooks/ecl-statusline.js');
...
describe('readGsdState', () => {
  const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-statusline-test-'));
...
describe('context meter respects CLAUDE_CODE_AUTO_COMPACT_WINDOW (#2219)', () => {
  ...
  const hookPath = path.join(__dirname, '..', 'hooks', 'ecl-statusline.js');
  ...
  test('bridge used_pct is raw (CC-consistent) regardless of ACW setting (#2451)', () => {
    ...
    // This ensures ecl-context-monitor warning messages match CC native /context.
...
describe('todo-resolution: resolves in_progress task from the newest matching todos file (#305)', () => {
  ...
  test('resolves in_progress task from the newest matching todos file (#305)', (t) => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-305-'));
    t.after(() => {
      cleanup(tempDir);
    });
