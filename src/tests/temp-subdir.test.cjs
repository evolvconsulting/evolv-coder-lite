/**
 * eCL Tools Tests - dedicated temp subdirectory
 *
 * Tests for issue #1975: eCL temp files should use a dedicated
 * subdirectory (path.join(os.tmpdir(), 'ecl')) instead of writing
...
describe('dedicated ecl temp subdirectory', () => {
  describe('output() temp file placement', () => {
    ...
    test('ecl temp subdirectory path is os.tmpdir()/ecl', () => {
      // The ECL_TEMP_DIR constant should resolve to <tmpdir>/ecl
      assert.strictEqual(ECL_TEMP_DIR, path.join(os.tmpdir(), 'ecl'));
    });
  });
  ...
  describe('reapStaleTempFiles with subdirectory', () => {
    ...
    beforeEach(() => {
      testPrefix = `ecl-tempsub-test-${Date.now()}-`;
      // Ensure the ecl subdirectory exists for test setup
      fs.mkdirSync(ECL_TEMP_DIR, { recursive: true });
    });

    test('removes stale files from ecl subdirectory', () => {
      const stalePath = path.join(ECL_TEMP_DIR, `${testPrefix}stale.json`);
      ...
      assert.ok(!fs.existsSync(stalePath), 'stale file in ecl subdir should be removed');
    });

    test('preserves fresh files in ecl subdirectory', () => {
      const freshPath = path.join(ECL_TEMP_DIR, `${testPrefix}fresh.json`);
      fs.writeFileSync(freshPath, '{}');

      reapStaleTempFiles(testPrefix, { maxAgeMs: 5 * 60 * 1000 });

      assert.ok(fs.existsSync(freshPath), 'fresh file in ecl subdir should be preserved');
      // Clean up
      fs.unlinkSync(freshPath);
    });

    test('removes stale directories from ecl subdirectory', () => {
      const staleDir = path.join(ECL_TEMP_DIR, `${testPrefix}dir`);
      ...
      assert.ok(!fs.existsSync(staleDir), 'stale directory in ecl subdir should be removed');
    });

    test('creates ecl subdirectory if it does not exist', () => {
      // Use a unique nested path to avoid interfering with other tests
      const uniqueSubdir = path.join(os.tmpdir(), `ecl-creation-test-${Date.now()}`);
      ...
      assert.doesNotThrow(() => {
        reapStaleTempFiles(`ecl-creation-test-${Date.now()}-`, { maxAgeMs: 0 });
      });
    });

    test('does not scan system tmpdir root for ecl- files', () => {
      ...
      // because it now only scans the ecl subdirectory
      reapStaleTempFiles(testPrefix, { maxAgeMs: 5 * 60 * 1000 });
