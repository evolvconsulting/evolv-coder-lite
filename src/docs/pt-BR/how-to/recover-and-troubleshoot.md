...
**Pré-requisitos:** eCL Core está instalado. Para problemas específicos de instalação, consulte [Instalar no seu ambiente de execução](install-on-your-runtime.md).
...
O eCL foi projetado em torno de contextos frescos. Cada subagente já recebe uma janela limpa de 200k. A sessão principal se degrada com o tempo — limpá-la e retomar é o remédio correto, não continuar forçando.
...
`.planning/PROJECT.md` já existe. `/ecl-new-project` é uma verificação de segurança. Se você realmente quer começar do zero, delete o diretório `.planning/` primeiro:

```bash
rm -rf .planning/
```

Em seguida, execute novamente `/ecl-new-project`.
...
Verifica a proteção de utilização da janela de contexto. Emite aviso em 60%, crítico em 70%. Se você estiver acima do limite de aviso, execute `/clear` seguido de `/ecl-resume-work` antes de iniciar o próximo comando principal.
...
Os subagentes `ecl-executor` do eCL precisam de acesso Bash com permissão de escrita. Adicione os padrões necessários em `~/.claude/settings.json` sob `permissions.allow`. No mínimo:
...
Isso é causado por múltiplos agentes acionando ferramentas de build simultaneamente. O eCL lida com isso automaticamente desde a v1.26. Se você estiver em uma versão mais antiga, ou ainda vendo contenção, desative a execução paralela:
...
Um bug de classificação conhecido do Claude Code pode reportar falha enquanto o trabalho foi concluído com sucesso. Os orquestradores do eCL verificam a saída real, mas se você vir uma discrepância, os commits são a fonte da verdade.
...
Execute `/ecl-discuss-phase N` antes de planejar. A maioria dos problemas de qualidade do plano vem de suposições que o `CONTEXT.md` teria prevenido:

```bash
/ecl-discuss-phase 1
```

Para ver quais suposições o eCL está fazendo atualmente sem iniciar uma sessão completa:
...
Não execute novamente `/ecl-execute-phase`. Use `/ecl-quick` para correções direcionadas:

```bash
/ecl-quick "Fix the login button not responding on mobile Safari"
```

Ou use `/ecl-verify-work N` para identificar e corrigir problemas sistematicamente por meio de UAT.

### Se um comando parece congelado em "Spawning…"

Aguarde. Os subagentes do eCL são executados em uma janela de contexto separada. O trabalho deles é invisível para a sessão pai enquanto está em andamento. A nota de atividade na linha de spawn confirma que isso é esperado. Agentes de pesquisa e planejamento rotineiramente levam de 1 a 5 minutos; agentes de verificação podem levar mais tempo em fases grandes.
...
`/ecl-forensics` executa uma investigação post-mortem: anomalias no histórico do git, integridade dos artefatos, consistência do STATE.md, trabalho não commitado e worktrees órfãs. Grava um relatório em `.planning/forensics/` e apresenta etapas de remediação recomendadas. É somente leitura e nunca modifica os arquivos do seu projeto.

### Se você precisa reverter uma fase ou plano

```bash
/ecl-undo --phase 03          # Reverte todos os commits da fase 3
/ecl-undo --plan 03-02        # Reverte os commits do plano 02 da fase 3
/ecl-undo --last 5            # Escolhe interativamente entre os 5 commits eCL mais recentes
```

`/ecl-undo` verifica as fases dependentes antes de reverter e sempre apresenta uma confirmação.

---

## Problemas de instalação e atualização

### Se o eCL não é reconhecido após a instalação

Reinicie seu ambiente de execução. O eCL instala comandos slash no diretório de comandos do seu ambiente de execução (por exemplo, `~/.claude/commands/ecl/`). A maioria dos ambientes de execução descobre novos comandos apenas na inicialização.
...
Desde a v1.17, o instalador faz backup dos arquivos modificados localmente em `ecl-local-patches/`. Reaplique suas alterações:
...
Se `npx @evolvconsulting/evolv-coder-lite` falhar devido a interrupções do npm ou restrições de rede, consulte `docs/manual-update.md` para um procedimento de atualização manual passo a passo que funciona sem acesso ao npm.

Para atualizações de rotina, consulte [Atualizar o eCL](update-ecl.md).
...
| Problema | Solução |
|----------|---------|
| Contexto perdido ou nova sessão | `/ecl-resume-work` ou `/ecl-progress` |
| Não sabe qual é o próximo passo | `/ecl-progress --next` |
| Fase deu errado | `/ecl-undo --phase NN`, depois replaneje |
| Algo quebrou | `/ecl-debug "descrição"` (adicione `--diagnose` para análise sem correções) |
| STATE.md fora de sincronia | `state validate` depois `state sync` |
| Integridade de `.planning/` incerta | `/ecl-health`, depois `/ecl-health --repair` |
| Estado do fluxo de trabalho parece corrompido | `/ecl-forensics` |
| Correção direcionada rápida | `/ecl-quick` |
| Plano não corresponde à sua visão | `/ecl-discuss-phase N` depois replaneje |
| Custos elevados | `/ecl-config --profile budget` e `/ecl-settings` para desativar agentes |
| Atualização quebrou alterações locais | `/ecl-update --reapply` |
| Quer resumo da sessão | `/ecl-pause-work --report` |
| Erros de build por execução paralela | Atualize o eCL ou defina `parallelization.enabled: false` |
