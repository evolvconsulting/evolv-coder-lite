...
// ---------------------------------------------------------------------------
// Lazy installer exports (avoids ECL_TEST_MODE env mutation at module load)
// ---------------------------------------------------------------------------
 ...
 * Sets ECL_TEST_MODE only for the duration of the require() call and only if
...
/**
 * Locate the eCL commands/ecl source directory.
 *
 * Resolution order:
 * 1. If runtimeConfigDir provided, check <runtimeConfigDir>/.ecl-source marker.
...
function findInstallSourceRoot(runtimeConfigDir?: string): string {
  // Step 1: marker check
  if (runtimeConfigDir) {
    const markerPath = path.join(runtimeConfigDir, '.ecl-source');
  ...
  let dir = __dirname;
  for (let i = 0; i < 6; i++) {
    const candidate = path.join(dir, 'commands', 'ecl');
  ...
  throw new Error(`findInstallSourceRoot: could not locate commands/ecl from ${__dirname}`);
}

/**
 * Locate the eCL agents source directory.
 *
 * Resolution order:
 * 1. If runtimeConfigDir provided, check <runtimeConfigDir>/.ecl-source marker.
 * 2. Walk up from __dirname using path.dirname (no literal .. segments).
    ...
    if (fs.existsSync(markerPath)) {
      try {
        ...
        if (src && fs.existsSync(src)) {
          // Marker points to commands/ecl; agents/ is a sibling of commands/
 ...
 * @param configDir      runtime config dir (for .ecl-source marker resolution)
 */
function skillsKind(
  destSubpath: string,
  prefix: string,
 ...
 * @param prefix        filename prefix, e.g. 'ecl-'
 * @param converterName name of converter function in bin/install.js exports
 * @param configDir     runtime config dir (for .ecl-source marker resolution)
 */
...
function resolveRuntimeArtifactLayout(runtime: string, configDir: string, scope: 'local' | 'global' = 'global'): Layout {
  ...
  switch (runtime) {
    case 'claude':
      if (scope === 'local') {
        kinds = [
          commandsKind('commands/ecl', 'ecl-', configDir),
          agentsKind('agents', 'ecl-', configDir),
        ];
      } else {
        kinds = [skillsKind('skills', 'ecl-', 'convertClaudeCommandToClaudeSkill', 'claude', configDir)];
      }
      break;

    case 'cursor':
      // Cursor 1.6+ supports two artifact surfaces:
      //   1. skills/ecl-<name>/SKILL.md  — rich skills with frontmatter + adapter header
      //   2. commands/ecl-<name>.md      — plain markdown slash commands (no frontmatter)
      //      accessed via '/' in the Agent input (#785)
      kinds = [
        skillsKind('skills', 'ecl-', 'convertClaudeCommandToCursorSkill', 'cursor', configDir),
        convertedCommandsKind('commands', 'ecl-', 'convertClaudeCommandToCursorCommand', configDir),
      ];
      break;

    case 'gemini':
      kinds = [commandsKind('commands/ecl', 'ecl-', configDir)];
      break;

    case 'codex':
      kinds = [skillsKind('skills', 'ecl-', 'convertClaudeCommandToCodexSkill', 'codex', configDir)];
      break;

    case 'copilot':
      kinds = [skillsKind('skills', 'ecl-', 'convertClaudeCommandToCopilotSkill', 'copilot', configDir)];
      break;

    case 'antigravity':
      kinds = [skillsKind('skills', 'ecl-', 'convertClaudeCommandToAntigravitySkill', 'antigravity', configDir)];
      break;

    case 'windsurf':
      kinds = [skillsKind('skills', 'ecl-', 'convertClaudeCommandToWindsurfSkill', 'windsurf', configDir)];
      break;

    case 'augment':
      kinds = [
        commandsKind('commands', 'ecl-', configDir),
        skillsKind('skills', 'ecl-', 'convertClaudeCommandToAugmentSkill', 'augment', configDir),
      ];
      break;

    case 'trae':
      kinds = [skillsKind('skills', 'ecl-', 'convertClaudeCommandToTraeSkill', 'trae', configDir)];
      break;

    case 'qwen':
      kinds = [skillsKind('skills', 'ecl-', 'convertClaudeCommandToClaudeSkill', 'qwen', configDir)];
      break;

    case 'hermes':
      kinds = [skillsKind('skills/ecl', '', 'convertClaudeCommandToClaudeSkill', 'hermes', configDir)];
      break;

    case 'codebuddy':
      // CodeBuddy (Tencent) reads two user-level surfaces (codebuddy.ai/docs/cli):
      //   1. commands/ecl-<name>.md      — slash commands shown in the '/' menu (#789)
      //   2. skills/ecl-<name>/SKILL.md  — model-invocable skills, emitted with
      //      user-invocable:false so they stay OUT of '/' (the commands surface is
      //      the sole '/' entry point) — avoids a duplicated /ecl-* per workflow.
      // Subagents (~/.codebuddy/agents/) are already emitted by the generic agents
      // block in bin/install.js; MCP is excluded (ecl ships no MCP server).
      kinds = [
        convertedCommandsKind('commands', 'ecl-', 'convertClaudeCommandToCodebuddyCommand', configDir),
        skillsKind('skills', 'ecl-', 'convertClaudeCommandToCodebuddySkill', 'codebuddy', configDir),
      ];
      break;

    case 'cline':
      kinds = scope === 'global' ? [skillsKind('skills', 'ecl-', 'convertClaudeCommandToClineSkill', 'cline', configDir)] : [];
      break;
    ...
    case 'opencode':
      ...
      kinds = [
        commandsKind('command', 'ecl-', configDir),
        skillsKind('skills', 'ecl-', 'convertClaudeCommandToOpencodeSkill', 'opencode', configDir),
      ];
      break;
    ...
    case 'kilo':
      ...
      kinds = [
        commandsKind('command', 'ecl-', configDir),
        skillsKind('skills', 'ecl-', 'convertClaudeCommandToKiloSkill', 'kilo', configDir),
      ];
      break;
