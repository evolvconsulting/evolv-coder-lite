 ...
 *  3. e2e CLI tests via `ecl-tools drift-guard severity/authority`.
 *  4. Structural test that plan-review-convergence.md invokes `ecl_run drift-guard`.
 */
...
} = require('../evolv-coder-lite/bin/lib/plan-drift-guard.cjs');
...
describe('ecl-tools drift-guard — CLI e2e', () => {
  let tmpDir;

  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-drift-guard-'));
...
// ── 5. Structural test: plan-review-convergence.md invokes ecl_run drift-guard

describe('plan-review-convergence.md uses ecl_run drift-guard seam', () => {
  const WORKFLOW_PATH = path.join(
    __dirname, '..', 'evolv-coder-lite', 'workflows', 'plan-review-convergence.md',
  );

  test('workflow contains ecl_run drift-guard authority call', () => {
    const content = fs.readFileSync(WORKFLOW_PATH, 'utf-8');
    assert.ok(
      content.includes('ecl_run drift-guard authority'),
      'plan-review-convergence.md must contain: ecl_run drift-guard authority',
    );
  });
  ...
  test('workflow drift-guard authority call includes --raw (prevents JSON-quoted capture)', () => {
    ...
    assert.match(
      content,
      /ecl_run drift-guard authority --raw/,
  ...
  test('workflow contains ecl_run drift-guard severity call', () => {
    const content = fs.readFileSync(WORKFLOW_PATH, 'utf-8');
    assert.ok(
      content.includes('ecl_run drift-guard severity'),
      'plan-review-convergence.md must contain: ecl_run drift-guard severity',
    );
  });
