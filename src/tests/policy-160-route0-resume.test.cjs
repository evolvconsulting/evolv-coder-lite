...
/**
 * eCL Tools Tests - Route 0 resume-incomplete-phase invariant (#160)
 *
 ...
 * SUMMARY.md files, causing /ecl-next or /ecl-progress to silently skip
...
describe('Route 0: resume_incomplete_phase invariant (#160)', () => {
  const nextMdPath = path.join(__dirname, '..', 'evolv-coder-lite', 'workflows', 'next.md');
  const progressMdPath = path.join(__dirname, '..', 'evolv-coder-lite', 'workflows', 'progress.md');
  ...
  describe('next.md', () => {
    ...
    test('routes to lowest-numbered incomplete phase via execute-phase', () => {
      ...
      assert.ok(
        content.includes('ecl-execute-phase') || content.includes('ecl:execute-phase'),
    ...
    test('scan uses ecl_run (canonical resolver form, not bare ecl-sdk)', () => {
      const content = fs.readFileSync(nextMdPath, 'utf8');
      const route0Start = content.indexOf('name="resume_incomplete_phase"');
      const route0End = content.indexOf('</step>', route0Start);
      const route0Block = content.slice(route0Start, route0End);
      // Must use ecl_run, not bare ecl-sdk
      assert.ok(
        route0Block.includes('ecl_run'),
        'Route 0 scan must use ecl_run (canonical resolver), not bare ecl-sdk'
      );
      // Must NOT use bare ecl-sdk (without $)
      const bareGsdSdkPattern = /(?<!\$)ecl-sdk/;
      assert.ok(
        !bareGsdSdkPattern.test(route0Block),
        'Route 0 must not use bare ecl-sdk — use ecl_run to match the file convention'
      );
    });
  ...
  describe('progress.md', () => {
    ...
    test('scan uses $ROADMAP already loaded (not a fresh bare ecl-sdk call)', () => {
      const content = fs.readFileSync(progressMdPath, 'utf8');
      const route0Start = content.indexOf('Step 0: Resume-incomplete-phase');
      const route0End = content.indexOf('Step 1:', route0Start);
      const route0Block = content.slice(route0Start, route0End);
