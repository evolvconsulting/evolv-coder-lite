#!/usr/bin/env bash
# ecl-graphify-rebuild.sh — detached rebuild runner for hooks/ecl-graphify-update.sh.
#
# Usage:
#   ecl-graphify-rebuild.sh <STATUS_FILE> <LOCK_FILE> <HEAD_SHA> <MS_START> <GRAPHIFY_BIN>
#
...
ECL_STATUS_TS="$TS_END" \
ECL_STATUS_NAME="$STATUS_NAME" \
ECL_EXIT_CODE="$EXIT_CODE" \
ECL_DURATION="$DURATION" \
ECL_HEAD_SHA="$HEAD_SHA" \
ECL_STATUS_FILE="$STATUS_FILE" \
node -e '
  const fs = require("node:fs");
  const status = {
    ts: process.env.ECL_STATUS_TS,
    status: process.env.ECL_STATUS_NAME,
    exit_code: parseInt(process.env.ECL_EXIT_CODE, 10),
    duration_ms: parseInt(process.env.ECL_DURATION, 10),
    head_at_build: process.env.ECL_HEAD_SHA,
    graphify_version: null,
  };
  fs.writeFileSync(process.env.ECL_STATUS_FILE, JSON.stringify(status, null, 2) + "\n");
' 2>/dev/null || true
