...
- [既存コードベースのオンボーディング](tutorials/onboarding-an-existing-codebase.md) — ブラウンフィールドのリポジトリに eCL Core を導入する
...
- [クイックおよびファストタスクを処理する](how-to/handle-quick-and-fast-tasks.md) — フェーズループ外のアドホック作業に `/ecl-quick` と `/ecl-fast` を使用する
...
- [スパイクとスケッチ](how-to/spike-and-sketch.md) — 計画を確定する前の探索的作業に `/ecl-spike` と `/ecl-sketch` を使用する
- [UI フェーズを設計する](how-to/design-a-ui-phase.md) — フロントエンドおよびビジュアル作業に UI フェーズループを使用する
- [トラッカーイシューから eCL を動かす](how-to/drive-ecl-from-a-tracker-issue.md) — GitHub、Linear、または Jira のイシューからフェーズを開始する
- [eCL 2 から移行する](how-to/migrate-from-ecl-2.md) — 既存の eCL 2 プロジェクトを eCL Core にアップグレードする
- [eCL をアップデートする](how-to/update-ecl.md) — インストーラーを再実行して最新リリースを取得する
...
- [CLI ツール](CLI-TOOLS.md) — ワークフローとエージェント向け `ecl-tools.cjs` プログラマティック API
...
- [コンテキストエンジニアリング](explanation/context-engineering.md) — コンテキストの腐敗がどのように形成され、eCL Core がどのように防ぐか
...
- [ディスカスモード](workflow-discuss-mode.md) — `/ecl-discuss-phase` の assumptions モードと interview モード
- [コンテキストモニタリング](context-monitor.md) — コンテキストウィンドウ監視フックのアーキテクチャ
- [イシュー駆動オーケストレーション](issue-driven-orchestration.md) — 既存のプリミティブを使ってトラッカーイシューから eCL を動かすレシピ
