 ...
 * `npx @evolvconsulting/evolv-coder-lite@latest --global --claude`:
 *   ecl-sdk: The term 'ecl-sdk' is not recognized
 *
...
//    ecl-sdk.cmd (using filterNpxFromPath on the passed pathString)
// ---------------------------------------------------------------------------
...
describe('bug #3211-B: isGsdSdkOnPath rejects Windows _npx-only transient PATH', () => {
  ...
  before(() => {
    tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-3211-b-'));
  });
  ...
  test('returns false when a ecl-sdk.cmd exists only in an _npx-style transient dir', () => {
    ...
    // Write a ecl-sdk shim (named .cmd for the Windows scenario — on POSIX
    // isGsdSdkOnPath won't find .cmd; we validate the filter, not the exec check).
    const shimPath = path.join(npxBinDir, 'ecl-sdk.cmd');
    fs.writeFileSync(
      shimPath,
      ['@ECHO OFF', '@node "C:\\path\\to\\ecl-sdk.js" %*', ''].join('\r\n'),
    );
    ...
    assert.equal(
      onPath,
      false,
      'isGsdSdkOnPath(filterNpxFromPath(path)) must return false when only _npx dir has ecl-sdk. persistentPath=' + persistentPath,
    );
  });
});
...
//    NOT on PATH → must NOT print "eCL SDK ready"
// ---------------------------------------------------------------------------
...
describe('bug #3211-D: installSdkIfNeeded — Windows _npx false-positive', () => {
  ...
  beforeEach(() => {
    tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-3211-d-'));
    sdkDir = makeSdkDir(tmpRoot);

    // Simulate: install-time PATH contains only a transient _npx dir
    // with a ecl-sdk shim. The persistent npm prefix dir is separate and
    // NOT in process.env.PATH during the npx run.
    const npxBinDir = path.join(tmpRoot, '_npx', 'abc123', 'node_modules', '.bin');
    fs.mkdirSync(npxBinDir, { recursive: true });
    // Write a ecl-sdk shim in the transient dir (executable on POSIX)
    const shimName = 'ecl-sdk';
    fs.writeFileSync(
      path.join(npxBinDir, shimName),
      ['#!/bin/sh', 'exec node /path/to/ecl-sdk.js "$@"', ''].join('\n'),
      { mode: 0o755 },
    );
  ...
  test('does NOT print "eCL SDK ready" when the only ecl-sdk entry is in a transient _npx dir and npm-prefix is off-PATH', () => {
    ...
    assert.ok(
      ...
      'installer must NOT print "eCL SDK ready" when ecl-sdk is only in a transient _npx dir. Got:\n' + combined,
    );
...
describe('bug #3211-E: isLegacyGsdSdkShim detects legacy marker in .cmd files', () => {
  ...
  before(() => {
    tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-3211-e-'));
  });
  ...
  test('detects @deprecated + ecl-tools.cjs in a .cmd shim', () => {
    assert.equal(typeof isLegacyGsdSdkShim, 'function');

    const legacyCmd = path.join(tmpRoot, 'ecl-sdk.cmd');
    fs.writeFileSync(
      legacyCmd,
      [
        '@ECHO OFF',
        ':: @deprecated — use ecl-tools.cjs directly',
        '@node "C:\\path\\to\\ecl-tools.cjs" %*',
        '',
      ].join('\r\n'),
    );
    assert.equal(isLegacyGsdSdkShim(legacyCmd), true, 'must detect @deprecated ecl-tools.cjs in .cmd shim');
  });

  test('returns false for a modern .cmd shim pointing at ecl-sdk.js', () => {
    const modernCmd = path.join(tmpRoot, 'ecl-sdk-modern.cmd');
    fs.writeFileSync(
      modernCmd,
      [
        '@ECHO OFF',
        '@SETLOCAL',
        '@node "C:\\path\\to\\evolv-coder-lite\\bin\\ecl-sdk.js" %*',
        '',
      ].join('\r\n'),
    );
