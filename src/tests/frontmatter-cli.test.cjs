...
/**
 * eCL Tools Tests - frontmatter CLI integration
 *
 * Integration tests for the 4 frontmatter subcommands (get, set, merge, validate)
 * exercised through ecl-tools.cjs via execSync.
 *
...
function writeTempFile(content) {
  const tmpFile = path.join(os.tmpdir(), `ecl-fm-test-${Date.now()}-${Math.random().toString(36).slice(2)}.md`);
