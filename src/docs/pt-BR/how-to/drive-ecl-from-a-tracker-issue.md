# Como conduzir o eCL Core a partir de uma issue do rastreador

**Objetivo:** Levar uma única issue bem delimitada do GitHub, Linear ou Jira por todo o pipeline do eCL — desde o workspace isolado até o PR mesclado — usando apenas comandos já existentes no eCL Core, sem scripts customizados ou integrações com rastreadores.

**Pré-requisitos:** eCL Core está instalado. A issue tem escopo delimitado, critérios de aceitação observáveis e nenhum bloqueador upstream.
...
```bash
/ecl-phase "Descrição correspondente ao título da issue"
```

- **A issue é urgente e precisa ser inserida entre fases existentes** → insira uma fase decimal:

```bash
/ecl-phase --insert 3 "Fix: descrição da issue"
```
...
O eCL pergunta sobre ambiguidades no escopo da issue — tratamento de erros, casos extremos, contratos de interface, escolhas tecnológicas. Suas respostas moldam o plano que se segue.
...
O eCL cria agentes de pesquisa, lê suas decisões do `CONTEXT.md` (incluindo a URL da issue) e produz arquivos `PLAN.md` atômicos. Um verificador de planos valida cada plano antes de salvá-lo.
...
O eCL percorre os critérios de aceitação do objetivo da fase (que reflete sua issue do rastreador) um de cada vez. Se algo falhar, o eCL diagnostica a causa raiz e cria um plano de correção. Execute novamente e re-verifique até que todas as verificações passem.
...
O eCL monta o corpo do PR a partir dos seus artefatos de planejamento: objetivo da fase, resumo das mudanças, requisitos atendidos, status de verificação e decisões-chave. Inclua `Closes #NNN` ou `Fixes #NNN` no corpo do PR (ou configure via `/ecl-config`) para que a issue do rastreador seja fechada automaticamente quando o PR for mesclado.
...
```bash
/ecl-capture "Acompanhamento: descrição do trabalho descoberto"      # Adicionar como tarefa
/ecl-capture --seed "Ideia que vale uma fase futura"                 # Preservar para o próximo milestone
/ecl-capture --backlog "Não urgente, mas vale registrar"             # Arquivar no backlog
```

O eCL não publica no seu rastreador automaticamente. Criar uma issue no rastreador a partir dos acompanhamentos registrados é uma etapa manual separada — isso mantém a revisão humana no ciclo.
...
|-----------|-----------|
| A issue é muito pequena (typo, mudança de config) | Pule workspace + discuss + plan; use `/ecl-quick` em vez disso |
| A issue tem múltiplas subtarefas independentes | Use `/ecl-manager` para paralelizar a execução entre planos |
| A issue está bloqueada em outra issue | Não inicie até que o bloqueador upstream seja resolvido; o eCL não possui poller automático de dependências |
| O escopo da issue se mostra maior do que o esperado durante a execução | Pare, execute `/ecl-phase --insert N` para adicionar subfases, continue |
| Você quer pular a discussão interativa | Use a flag `--auto` com `/ecl-discuss-phase`, ou defina `workflow.skip_discuss: true` para automação em todo o projeto |
| Múltiplas issues formam uma release coerente | Execute `/ecl-new-milestone` para agrupá-las e `/ecl-autonomous` para executar em sequência |
