 ...
 *
 * Walks workflowsDir, extracts every `${ECL_HOME[...]}/<path>.<cjs|js|sh>`
 * token, and asserts:
...
// Match `${ECL_HOME}` or `${ECL_HOME:-...}` followed by a /-rooted path
// ending in .cjs/.js/.sh. The path is captured verbatim (relative to
// the install root).
const REF_RE = /\$\{ECL_HOME(?::-[^}]*)?\}\/([A-Za-z0-9_./-]+\.(?:cjs|js|sh))/g;
