'use strict';
/**
 * Graphify command router — CLI subcommand dispatcher for `ecl-tools graphify`.
 *
 * ADR-959 (phase 4d-impl-2) pilot: first real capability command cutover.
 * Extracted from the hardcoded `case 'graphify':` arm in ecl-tools.cjs.
...
function routeGraphifyCommand({ args, cwd, raw, error, _graphify }: RouteGraphifyCommandOptions): void {
  ...
  if (subcommand === 'query') {
    const term = args[2];
    if (!term) {
      error('Usage: ecl-tools graphify query <term>', ERROR_REASON.USAGE);
      return;
    }
    ...
    if (budgetIdx !== -1) {
      ...
      if (rawBudget === undefined || Number.isNaN(parseInt(rawBudget, 10))) {
        error('Usage: ecl-tools graphify query <term> [--budget <N>]', ERROR_REASON.USAGE);
        return;
      }
