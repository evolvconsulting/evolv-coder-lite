...
**Prerequisites:** `.planning/ROADMAP.md` exists. A `{phase}-CONTEXT.md` from `/ecl-discuss-phase` is strongly recommended but not required.
...
1. **Research** — A `ecl-phase-researcher` subagent investigates the domain and writes `{phase}-RESEARCH.md`.
2. **Plan** — A `ecl-planner` subagent reads context, research, and requirements, then writes one or more `{phase}-{N}-PLAN.md` files.
3. **Verify** — A `ecl-plan-checker` subagent validates plan quality across eight dimensions and triggers a revision loop (up to three iterations) until quality gates pass.

If no phase number is given, eCL Core targets the next unplanned phase from the roadmap.
...
```bash
/ecl-plan-phase 3 --skip-research
```

**If RESEARCH.md already exists but you want to force a refresh:**

```bash
/ecl-plan-phase 3 --research
```

**If you want to run research only** — write RESEARCH.md and exit before planning:

```bash
/ecl-plan-phase --research-phase 4
```

If RESEARCH.md already exists, you are prompted to update, view, or skip. To force-refresh without the prompt:

```bash
/ecl-plan-phase --research-phase 4 --research
```

To print existing RESEARCH.md to stdout without spawning the researcher:

```bash
/ecl-plan-phase --research-phase 4 --view
```

Note: `--research-phase <N>` is a flag on `/ecl-plan-phase`. There is no standalone research-phase command — the removed standalone research command was retired in favour of this flag.
...
```bash
/ecl-plan-phase 1 --tdd
```

Composable with `--mvp`:

```bash
/ecl-plan-phase 1 --mvp --tdd
```
...
```bash
node ecl-tools.cjs config-set workflow.tdd_mode true
```
...
**If you have run `/ecl-review --phase N` and a `REVIEWS.md` exists:**
...
```bash
/ecl-plan-phase 1 --bounce
```

To skip bounce even if it is enabled in config:

```bash
/ecl-plan-phase 1 --skip-bounce
```
...
The `ecl-plan-checker` validates plans across eight dimensions before allowing execution:
