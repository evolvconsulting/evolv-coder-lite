<purpose>
Check for eCL updates via npm, display changelog for versions between installed and latest, obtain user confirmation, and execute clean installation with cache clearing.
</purpose>
...
Detect the installed eCL version, scope, runtime, and config dir.
...
- If the path contains `/evolv-coder-lite/workflows/update.md`, strip that suffix and store the remainder as `PREFERRED_CONFIG_DIR`.
...
Then resolve the install context via the deterministic projection (#498). **Do NOT re-derive scope, runtime, or version by hand** — `update-context` owns that cascade in tested code (`evolv-coder-lite/bin/lib/update-context.cjs`), the same way `check-latest-version` owns the package name (#2992):

```bash
# Resolve ecl-tools.cjs WITHOUT yet knowing ECL_DIR. The running workflow lives
# at <PREFERRED_CONFIG_DIR>/evolv-coder-lite/workflows/update.md, so its sibling
# bin/ecl-tools.cjs is the authoritative tool for THIS install. Fall back to a
# global copy, then to ecl-tools on PATH.
ECL_TOOLS=""
for cand in \
  "$PREFERRED_CONFIG_DIR/evolv-coder-lite/bin/ecl-tools.cjs" \
  "$HOME/.claude/evolv-coder-lite/bin/ecl-tools.cjs"; do
  if [ -n "$cand" ] && [ -f "$cand" ]; then ECL_TOOLS="$cand"; break; fi
done
# Last resort: the ecl-tools shim on PATH — resolved to its absolute path and
# invoked via the variable (never a bare `ecl-tools` command; see #2851).
if [ -z "$ECL_TOOLS" ] && command -v ecl-tools >/dev/null 2>&1; then
  ECL_TOOLS="$(command -v ecl-tools)"
fi

UC=""
if [ -n "$ECL_TOOLS" ]; then
  case "$ECL_TOOLS" in
    *.cjs) UC="$(node "$ECL_TOOLS" update-context --config-dir "$PREFERRED_CONFIG_DIR" --runtime "$PREFERRED_RUNTIME" --json 2>/dev/null)" ;;
    *)     UC="$("$ECL_TOOLS" update-context --config-dir "$PREFERRED_CONFIG_DIR" --runtime "$PREFERRED_RUNTIME" --json 2>/dev/null)" ;;
  esac
fi
...
if [ -n "$UC" ]; then
  ...
  ECL_DIR="$(printf '%s' "$UC" | jq -r '.gsdDir')"
else
...
- Line 4 = resolved eCL config dir (e.g. `/Users/me/.claude`, `/Users/me/.gemini`); empty if scope is `UNKNOWN`. Capture this as `ECL_DIR` and pass it to subsequent steps so they don't re-derive the runtime path.
...
The `ECL_DIR` value emitted by `get_installed_version` (line 4) resolves to the runtime-specific config dir (`~/.claude/`, `~/.gemini/`, `~/.codex/`, etc.), so the script invocation works for every runtime — not just Claude. If `ECL_DIR` is empty (scope `UNKNOWN`), skip this step and go directly to install.

`LATEST_RESULT` is a JSON document with the documented shape `{ ok: bool, version: string, reason: string, detail?: string }`. Parse via `jq` ONLY when the script actually ran. When `ECL_DIR` is empty (scope `UNKNOWN`), skip the check entirely and seed the parsed fields with their no-op values so downstream logic does not mistake an unset `LATEST_RESULT` for a failed network check (#2993 CR feedback):
...
Running /ecl:update would install the npm release (A.B.C) and downgrade
...
1. Fetch changelog from GitHub raw URL and save to a temp file, e.g. `/tmp/ecl-changelog-$$.md`.
...
```bash
CHANGELOG_TMP="/tmp/ecl-changelog-$$.md"
curl -fsSL "https://raw.githubusercontent.com/evolvconsulting/evolv-coder-lite/main/CHANGELOG.md" -o "$CHANGELOG_TMP" 2>/dev/null \
  || wget -qO "$CHANGELOG_TMP" "https://raw.githubusercontent.com/evolvconsulting/evolv-coder-lite/main/CHANGELOG.md" 2>/dev/null

ECL_CHANGESET_CLI="$ECL_DIR/scripts/changeset/cli.cjs"
if [ ! -f "$ECL_CHANGESET_CLI" ]; then
  CHANGELOG_PREVIEW="(Changelog CLI not found at $ECL_CHANGESET_CLI — reinstall eCL to restore preview. Update will still proceed.)"
else
  EXTRACT_JSON=$(node "$ECL_CHANGESET_CLI" extract \
  ...
  if [ "$EXTRACT_EXIT" -eq 2 ]; then
    ...
    CHANGELOG_PREVIEW=$(node "$ECL_CHANGESET_CLI" extract \
      --from "$INSTALLED_VERSION" \
      --to "$LATEST_VERSION" \
...
⚠️  **Note:** The installer performs a clean install of eCL folders:
- `commands/ecl/` will be wiped and replaced
- `evolv-coder-lite/` will be wiped and replaced
- `agents/ecl-*` files will be replaced
...
- Custom commands not in `commands/ecl/` ✓
- Custom agents not prefixed with `ecl-` ✓
- Custom hooks ✓
- Your CLAUDE.md files ✓

If you've modified any eCL files directly, they'll be automatically backed up to `ecl-local-patches/` and can be reapplied with `/ecl:update --reapply` after the update.
```
...
eCL-managed directories. These are files that exist on disk but are NOT listed
in `ecl-file-manifest.json` — i.e., files the user added themselves that the
...
even when custom files exist (bug #1997). Use `ecl-tools.cjs query detect-custom-files`
or the bundled `ecl-tools.cjs detect-custom-files` path — both resolve paths
...
# get_installed_version emits it as ECL_DIR (LOCAL or GLOBAL install dir, or empty
...
```bash
CUSTOM_JSON=''
if [ -f "$ECL_TOOLS" ] && [ -n "$RUNTIME_DIR" ]; then
  CUSTOM_JSON=$(node "$ECL_TOOLS" detect-custom-files --config-dir "$RUNTIME_DIR" 2>/dev/null)
fi
...
Back up each custom file to `$RUNTIME_DIR/ecl-user-files-backup/` before the
installer wipes the directories:

```bash
BACKUP_DIR="$RUNTIME_DIR/ecl-user-files-backup"
mkdir -p "$BACKUP_DIR"
...
```
⚠️  Found N custom file(s) inside eCL-managed directories.
    These have been backed up to ecl-user-files-backup/ before the update.
...
```
╔═══════════════════════════════════════════════════════════╗
║  eCL Updated: v1.5.10 → v1.5.15                           ║
╚═══════════════════════════════════════════════════════════╝

⚠️  Restart your runtime to pick up the new commands.

[View full changelog](https://github.com/evolvconsulting/evolv-coder-lite/blob/main/CHANGELOG.md)
```
</step>
...
Check for ecl-local-patches/backup-meta.json in the config directory.

**If patches found:**

```
Local patches were backed up before the update.
Run `/ecl:update --reapply` to merge your modifications into the new version.
```
