 ...
 * - If CONTEXT.md is missing: primary suggestion is /ecl-discuss-phase
 * - If CONTEXT.md exists: primary suggestion is /ecl-plan-phase
 */
...
const workflowPath = path.resolve(
  __dirname, '..', 'evolv-coder-lite', 'workflows', 'execute-phase.md'
);
...
describe('bug #2002: offer_next checks CONTEXT.md before suggesting next step', () => {
  ...
  test('offer_next presents /ecl-discuss-phase when CONTEXT.md does not exist', () => {
    content = content || fs.readFileSync(workflowPath, 'utf-8');
    ...
    // "does not exist" and "ecl-discuss-phase" in the offer_next step
    ...
    assert.ok(
      ...
      /ecl-discuss-phase.*recommended|recommended.*ecl-discuss-phase/i.test(offerNextSection),
      'offer_next must present /ecl-discuss-phase as primary when CONTEXT.md does not exist'
    );
  });

  test('offer_next presents /ecl-plan-phase when CONTEXT.md exists', () => {
    content = content || fs.readFileSync(workflowPath, 'utf-8');
    const offerNextIdx = content.indexOf('offer_next');
    assert.ok(offerNextIdx !== -1, 'offer_next step must exist');
    ...
    assert.ok(
      ...
      'offer_next must present /ecl-plan-phase as primary when CONTEXT.md exists'
    );
  });
