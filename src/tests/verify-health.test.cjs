...
/**
 * eCL Tools Tests - Validate Health Command
 *
...
describe('validate health command', () => {
  ...
  // Regression: #3652 — after /ecl:complete-milestone, phase dirs move into
...
describe('validate health --repair command', () => {
  ...
  test('creates config.json with defaults when missing', () => {
    ...
    assert.strictEqual(diskConfig.phase_branch_template, 'ecl/phase-{phase}-{slug}');
    assert.strictEqual(diskConfig.milestone_branch_template, 'ecl/{milestone}-{slug}');
  });
...
// evolv-coder-lite/bin/lib/verify.cjs. These tests fail on old verify.cjs and
