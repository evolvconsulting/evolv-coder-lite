  ...
  const sanitizeFlags = (rawVal: unknown): string => {
    ...
    if (!safe) {
      process.stderr.write(
        `ecl-tools: warning: manager.flags contains invalid tokens, ignoring: ${val}\n`,
      );
      return '';
    }
    return val;
...
function buildSkillManifest(cwd: string, skillsDir: string | null = null): SkillManifest {
  ...
  for (const rootInfo of canonicalRoots) {
    ...
    // (e.g. spec-phase nested under both ecl-ns-workflow and ecl-ns-manage).
    ...
    for (const entry of entries) {
      ...
      // Scope to ecl-ns-* routers only: never vacuum up an unrelated user skill
      // that happens to have its own `skills/` subdirectory.
      if (!entry.name.startsWith('ecl-ns-')) continue;
