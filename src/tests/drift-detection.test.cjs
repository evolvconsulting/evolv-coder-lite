...
/**
 * eCL Tools Tests — Codebase Drift Detection (#2003)
 *
...
describe('last_mapped_commit frontmatter', () => {
  let tmp;
  beforeEach(() => {
    tmp = createTempProject('ecl-drift-');
...
describe('config-set drift validation via CLI', () => {
  let tmp;
  beforeEach(() => {
    tmp = createTempGitProject('ecl-drift-cfg-');
  });
  afterEach(() => cleanup(tmp));
...
describe('ecl-codebase-mapper --paths flag', () => {
  test('agent doc mentions --paths', () => {
    const doc = fs.readFileSync(
      path.join(__dirname, '..', 'agents', 'ecl-codebase-mapper.md'),
      'utf8',
    );
...
// dispatches capability gates via `ecl_run loop render-hooks execute:wave:post`,
...
describe('verify codebase-drift CLI', () => {
  let tmp;
  beforeEach(() => {
    tmp = createTempGitProject('ecl-drift-cli-');
    fs.mkdirSync(path.join(tmp, '.planning', 'codebase'), { recursive: true });
  });
  afterEach(() => cleanup(tmp));
  ...
  test('never exits non-zero when git repo is missing (non-blocking)', () => {
    const nonGit = createTempProject('ecl-drift-nongit-');
    try {
