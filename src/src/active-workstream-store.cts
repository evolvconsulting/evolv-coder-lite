 ...
 * CLI --ws > ECL_WORKSTREAM env > stored active workstream pointer.
 *
...
function resolveActiveWorkstream(
  ...
  } else if (env && typeof env['ECL_WORKSTREAM'] === 'string' && env['ECL_WORKSTREAM'].trim()) {
    ws = env['ECL_WORKSTREAM'].trim();
    source = 'env';
  } else {
...
function applyResolvedWorkstreamEnv(
  ...
  env['ECL_WORKSTREAM'] = resolution.ws;
}
