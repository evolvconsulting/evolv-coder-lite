 ...
 * with eCL marker detection, doc tooling detection, monorepo awareness, and
...
const ECL_MARKER = '<!-- generated-by: ecl-doc-writer -->';
...
/**
 * Check whether a file begins with the eCL doc writer marker.
...
function hasGsdMarker(filePath) {
  try {
    ...
    fs.closeSync(fd);
    return buf.slice(0, bytesRead).toString('utf-8').includes(ECL_MARKER);
  } catch {
    return false;
  }
}
 ...
 *
 * @example
 * node ecl-tools.cjs docs-init --raw
 *
...
function cmdDocsInit(cwd, raw) {
  ...
  const result = {
    doc_writer_model: resolveModelInternal(cwd, 'ecl-doc-writer'),
