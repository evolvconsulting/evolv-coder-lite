...
const LEGACY_PKG_SIGNAL = 'evolv-coder-lite' + '-cc';
...
describe('#607 --dry-run flag: spawned installer exits 0 and mutates nothing', () => {
  ...
  test('exits 0; stdout names artifact and contains "dry"; no install; artifact preserved; no double-print', () => {
    ...
    const legacyHook = path.join(tmpHome, '.gemini', 'hooks', 'ecl-old-update-worker.cjs');
    writeFile(legacyHook, `// installed via ${LEGACY_PKG_SIGNAL}\nconsole.log("old worker");`);

    // Seed the legacy shared cache file
    const legacyCache = path.join(tmpHome, '.cache', 'ecl', 'ecl-update-check.json');
    ...
    const result = spawnSync(
      ...
      [INSTALL_BIN, '--claude', '--global', '--dry-run'],
      {
        env: {
          ...
          ECL_SKIP_STALE_SDK_CHECK: '1',
          // Do NOT set ECL_TEST_MODE — we want the main() block to run
          ECL_TEST_MODE: undefined,
        },
        cwd: REPO_ROOT,
        encoding: 'utf8',
        timeout: 30_000,
    ...
    // No actual install happened — .claude/evolv-coder-lite must not exist
    const installDir = path.join(tmpHome, '.claude', 'evolv-coder-lite');
    assert.equal(
    ...
    const updateCacheFileName = require(
      path.join(REPO_ROOT, 'evolv-coder-lite', 'bin', 'lib', 'package-identity.cjs')
    ).updateCacheFileName;
    const perPkgCacheFile = path.join(tmpHome, '.cache', 'ecl', updateCacheFileName);
  ...
  test('--uninstall --dry-run prints "does not preview --uninstall" warning and exits 0', () => {
    ...
    // No uninstall occurred — .claude/evolv-coder-lite must not have been removed
...
describe('#607 cleanupLegacyGsdCc: exported helper unit tests', () => {
  // ECL_TEST_MODE is already set at the top so requiring install.js is safe.
  ...
  test('dryRun:true — plan lists seeded artifact; nothing removed', () => {
    ...
    const legacyHook = path.join(homeDir, '.gemini', 'hooks', 'ecl-old-update-worker.cjs');
    writeFile(legacyHook, `// installed via ${LEGACY_PKG_SIGNAL}\nconsole.log("old worker");`);
  ...
  test('dryRun:false — seeded leftover removed; dev-preferences.md preserved', () => {
    ...
    const devPrefs = path.join(homeDir, '.gemini', 'evolv-coder-lite', 'dev-preferences.md');
