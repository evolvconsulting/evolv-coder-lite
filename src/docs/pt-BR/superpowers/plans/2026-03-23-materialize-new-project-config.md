...
Este plano formaliza a materialização explícita da configuração do projeto durante `/ecl-new-project`, garantindo que escolhas feitas na inicialização sejam persistidas de forma determinística em `.planning/config.json`.
...
- após `/ecl-new-project`, `config.json` reflete as escolhas feitas
