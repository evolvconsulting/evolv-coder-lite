...
Detect whether eCL is installed locally or globally by checking both locations and validating install integrity.
...
Use `PREFERRED_RUNTIME` as the first runtime checked so `/ecl:update` targets the runtime that invoked it.
...
# runtime directories.
if [ -n "$PREFERRED_CONFIG_DIR" ] && { [ -f "$PREFERRED_CONFIG_DIR/evolv-coder-lite/VERSION" ] || [ -f "$PREFERRED_CONFIG_DIR/evolv-coder-lite/workflows/update.md" ]; }; then
  ...
  if [ -f "$PREFERRED_CONFIG_DIR/evolv-coder-lite/VERSION" ] && grep -Eq '^[0-9]+\.[0-9]+\.[0-9]+' "$PREFERRED_CONFIG_DIR/evolv-coder-lite/VERSION"; then
    INSTALLED_VERSION="$(cat "$PREFERRED_CONFIG_DIR/evolv-coder-lite/VERSION")"
  else
  ...
  # ECL_DIR or downstream check_latest_version misreads the install as
  # UNKNOWN. PREFERRED_CONFIG_DIR is the resolved config dir we just
  # validated above (line 95-96); it is the right ECL_DIR value for
  # this fast path.
...
for entry in "${ORDERED_RUNTIME_DIRS[@]}"; do
  runtime="${entry%%:*}"
  dir="${entry#*:}"
  if [ -f "./$dir/evolv-coder-lite/VERSION" ] || [ -f "./$dir/evolv-coder-lite/workflows/update.md" ]; then
    LOCAL_RUNTIME="$runtime"
    LOCAL_VERSION_FILE="./$dir/evolv-coder-lite/VERSION"
    LOCAL_MARKER_FILE="./$dir/evolv-coder-lite/workflows/update.md"
...
for entry in "${ORDERED_ENV_RUNTIME_DIRS[@]}"; do
  ...
  if [ -f "$dir/evolv-coder-lite/VERSION" ] || [ -f "$dir/evolv-coder-lite/workflows/update.md" ]; then
    ...
    GLOBAL_VERSION_FILE="$dir/evolv-coder-lite/VERSION"
    GLOBAL_MARKER_FILE="$dir/evolv-coder-lite/workflows/update.md"
...
if [ -z "$GLOBAL_RUNTIME" ]; then
  for entry in "${ORDERED_RUNTIME_DIRS[@]}"; do
    runtime="${entry%%:*}"
    dir="${entry#*:}"
    if [ -f "$HOME/$dir/evolv-coder-lite/VERSION" ] || [ -f "$HOME/$dir/evolv-coder-lite/workflows/update.md" ]; then
      GLOBAL_RUNTIME="$runtime"
      GLOBAL_VERSION_FILE="$HOME/$dir/evolv-coder-lite/VERSION"
      GLOBAL_MARKER_FILE="$HOME/$dir/evolv-coder-lite/workflows/update.md"
...
- Line 4 = resolved eCL config dir (e.g. `/Users/me/.claude`, `/Users/me/.gemini`); empty if scope is `UNKNOWN`. Capture this as `ECL_DIR` and pass it to subsequent steps so they don't have to re-derive the runtime path.
...
EXTRACT_JSON=$(node "$ECL_DIR/evolv-coder-lite/scripts/changeset/cli.cjs" extract \
...
if [ "$EXTRACT_EXIT" -eq 2 ]; then
  ...
  CHANGELOG_PREVIEW=$(node "$ECL_DIR/evolv-coder-lite/scripts/changeset/cli.cjs" extract \
    --from "$INSTALLED_VERSION" \
    --to "$LATEST_VERSION" \
...
even when custom files exist (bug #1997). Use `ecl-sdk query detect-custom-files`
when `ecl-sdk` is on `PATH`, or the bundled `ecl-tools.cjs detect-custom-files`
...
```bash
ECL_TOOLS="$RUNTIME_DIR/evolv-coder-lite/bin/ecl-tools.cjs"
CUSTOM_JSON=''
if [ -f "$ECL_TOOLS" ] && [ -n "$RUNTIME_DIR" ]; then
  CUSTOM_JSON=$(node "$ECL_TOOLS" detect-custom-files --config-dir "$RUNTIME_DIR" 2>/dev/null)
elif [ -n "$RUNTIME_DIR" ] && command -v ecl-sdk >/dev/null 2>&1; then
  CUSTOM_JSON=$(ecl-sdk query detect-custom-files --config-dir "$RUNTIME_DIR" 2>/dev/null)
fi
