/**
 * Next slash-command suggestion for `/ecl-next`-style routing (`route.next-action`).
 *
 * Deterministic routing from STATE.md, ROADMAP, and phase directories.
 * See `.planning/research/decision-routing-audit.md` §3.1 and `evolv-coder-lite/workflows/next.md`.
 */
...
export const routeNextAction: QueryHandler = async (_args, projectDir, workstream) => {
  ...
  if (sjd.error) {
    return {
      data: {
        command: '/ecl-new-project',
        args: '',
        reason: 'No STATE.md — initialize a eCL project first',
        current_phase: null,
        phase_name: null,
        gates: {
