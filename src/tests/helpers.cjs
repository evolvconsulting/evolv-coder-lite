...
function runGsdTools(args, cwd = process.cwd(), env = {}) {
  ...
  try {
    ...
    // error) rather than a ecl-tools application error.
...
// Render-on-help shape is `Usage: ecl-tools …\nCommands: …` — both lines
// must be present; structural test, not prose substring matching.
function isUsageOutput(text) {
  return /Usage:\s*ecl-tools/.test(text) && /Commands:/.test(text);
}
...
function resetRuntimeWarningCaches() {
  const configLoader = require('../evolv-coder-lite/bin/lib/config-loader.cjs');
