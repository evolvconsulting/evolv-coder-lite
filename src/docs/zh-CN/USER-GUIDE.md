...
eCL Core 的叙述性辅助指南——从这里开始了解系统全貌，然后按链接进入各专项文档。

> **eCL Core 的文档按照 [Diataxis](https://diataxis.fr) 框架组织。**
...
如需从 GitHub / Linear / Jira issue 直接驱动 eCL，请参阅
[issue-driven-orchestration](issue-driven-orchestration.md) 指南——该指南将跟踪器 issue
映射到工作区 → 讨论 → 计划 → 执行 → 验证 → 审查 → 发布的循环，使用现有的 eCL 基础功能实现。

---

## 斜杠命令形式（连字符 vs 冒号）

eCL 向所有支持的运行时提供**同一套技能**，但有两种斜杠拼写方式：

- **连字符形式** — `/ecl-command-name` — 供 Claude Code、Copilot、OpenCode、Kilo、Cursor、Windsurf、Augment、Antigravity 和 Trae 使用。
- **冒号形式** — `/ecl:command-name` — **仅供 Gemini CLI 使用**。Gemini 将每个插件的命令置于插件 ID 的命名空间下，因此安装时会在 `--gemini` 安装过程中将所有正文引用和命令文件改写为冒号形式。

无需手动选择——安装器会为您所针对的每个运行时写入正确形式。在 Gemini 终端上阅读演示时，将每个斜杠命令中 `ecl` 后的连字符替换为冒号即可。

## 命名空间路由入门（`ecl:<namespace>`，v1.40）
...
| 命名空间 | 路由器 | 路由目标 |
|-----------|--------|-----------|
| 阶段流水线 | `/ecl-workflow` | discuss / plan / execute / verify / phase / progress |
| 项目生命周期 | `/ecl-project` | milestones, audits, summary |
| 质量关卡 | `/ecl-quality` | code review, debug, audit, security, eval, ui |
| 代码库情报 | `/ecl-context` | map, graphify, docs, learnings |
| 管理 | `/ecl-manage` | config, workspace, workstreams, thread, update, ship, inbox |
| 探索与捕获 | `/ecl-ideate` | explore, sketch, spike, spec, capture |

您几乎不需要亲自输入命名空间路由器。它们的价值在于为模型提供发现正确子技能的路由层——其存在使系统提示只需列出 6 条而非 86 条。如果您已经知道具体命令（例如 `/ecl-plan-phase`），可直接调用。

---

## 项目生命周期概览

eCL 核心循环为：**discuss → plan → execute → verify → ship**，每个阶段重复一次。包括示例输出、创建哪些文件以及所有生效标志的完整逐步演练，请参阅专项教程。
...
| 标志 | 命令 | 使用场景 |
| ---- | ------- | ----------- |
| `--auto` | `/ecl-new-project` | 跳过交互式问题，从 PRD 文件导入 |
| `--research` | `/ecl-quick` | 为临时任务添加研究 Agent |
| `--validate` | `/ecl-quick` | 添加计划检查和执行后验证 |
| `--chain` | `/ecl-discuss-phase` | 自动链式运行 discuss → plan → execute 而不中断 |
| `--skip-research` | `/ecl-plan-phase` | 在领域已熟悉时跳过研究 Agent |
| `--draft` | `/ecl-ship` | 创建草稿 PR 而非待审查 PR |
...
在计划阶段研究期间，eCL 会在编写任何代码之前将自动化测试覆盖率映射到每个阶段的需求上。研究者会检测您现有的测试基础设施，将每个需求映射到特定的测试命令，并识别在实施开始前必须创建的测试脚手架（Wave 0 任务）。计划检查器将此作为第 8 个验证维度执行：缺少自动化验证命令的任务计划将不会被批准。

**输出：** `{phase}-VALIDATION.md` — 阶段的反馈契约。

**禁用：** 在 `/ecl-settings` 中将 `workflow.nyquist_validation: false` 设置为 false，适用于测试基础设施不是重点的快速原型阶段。

### 追溯验证（`/ecl-validate-phase`）
...
默认情况下，`/ecl-discuss-phase` 会就您的实现偏好提出开放性问题。假设模式将此倒转：eCL 首先读取您的代码库，提出关于如何构建该阶段的结构化假设，然后仅就修正内容提问。

**启用：** 通过 `/ecl-settings` 将 `workflow.discuss_mode` 设置为 `'assumptions'`。
...
**计划阶段转换关卡（阻塞）。** 计划完成后，eCL 会拒绝将阶段标记为已计划，直到每个可跟踪决策出现在至少一个计划的 `must_haves`、`truths` 或正文中。

**验证阶段验证关卡（非阻塞）。** 在验证期间，eCL 会在计划、SUMMARY.md、修改文件和最近提交消息中搜索每个可跟踪决策。遗漏项以警告章节的形式记录到 VERIFICATION.md；验证状态不变。

**将决策排除在外。** 将其移至 `<decisions>` 内的 `### Claude's Discretion` 标题下，或添加标签：`- **D-08 [informational]:** …`、`- **D-09 [folded]:** …`、`- **D-10 [deferred]:** …`。

**禁用关卡。** 在 `.planning/config.json` 中设置 `workflow.context_coverage_gate: false`（或通过 `/ecl-settings`）。默认值为 `true`。
...
AI 生成的前端在视觉上不一致，原因不在于 Claude Code 在 UI 方面能力不足，而在于执行前没有建立设计契约。`/ecl-ui-phase` 在计划前锁定设计契约；`/ecl-ui-review` 在执行后审计结果。
...
| 命令              | 描述                                              |
| -------------------- | -------------------------------------------------------- |
| `/ecl-ui-phase [N]`  | 为前端阶段生成 UI-SPEC.md 设计契约 |
| `/ecl-ui-review [N]` | 对已实现 UI 进行追溯性六维视觉审计      |

| 设置                   | 默认值 | 描述                                                 |
| ------------------------- | ------- | ----------------------------------------------------------- |
| `workflow.ui_phase`       | `true`  | 为前端阶段生成 UI 设计契约            |
| `workflow.ui_safety_gate` | `true`  | 计划阶段提示为前端阶段运行 /ecl-ui-phase |

---

## 探针与草图

使用 `/ecl-spike` 在计划前验证技术可行性，使用 `/ecl-sketch` 在设计前探索视觉方向。两者均将产物存储在 `.planning/` 中，并通过其配套的收尾工具与项目技能系统集成。
...
待办事项获得完整的阶段目录，因此您可以使用 `/ecl-discuss-phase 999.1` 进一步探索某个想法，或在准备好时使用 `/ecl-plan-phase 999.1`。

**审查和提升**使用 `/ecl-review-backlog`——它显示所有待办事项，并让您选择提升（移至活跃序列）、保留（留在待办事项中）或移除（删除）。
...
`/ecl-new-milestone` 会扫描所有种子并呈现匹配项。**存储位置：** `.planning/seeds/SEED-NNN-slug.md`
...
线程成熟后可提升为阶段（`/ecl-phase`）或待办事项（`/ecl-capture --backlog`）。**存储位置：** `.planning/threads/{slug}.md`
...
| 命令                            | 用途                                              |
| ---------------------------------- | ---------------------------------------------------- |
| `/ecl-workstreams create <name>`   | 创建具有隔离计划状态的新工作流 |
| `/ecl-workstreams switch <name>`   | 将活跃上下文切换到不同的工作流      |
| `/ecl-workstreams list`            | 显示所有工作流及当前活跃的工作流             |
| `/ecl-workstreams complete <name>` | 将工作流标记为完成并归档其状态      |
...
eCL 生成的 Markdown 文件会成为 LLM 系统提示。这意味着流入规划产物的任何用户控制文本都是潜在的间接提示注入向量。v1.27 引入了集中式安全加固：
...
**执行后漂移检测（#2003）。** 每次 `/ecl-execute-phase` 之后，eCL 会检查该阶段是否引入了足够的结构变化，使 `.planning/codebase/STRUCTURE.md` 过时。通过以下方式调整行为：
...
**无需 intel 即可工作。** 默认情况下，守卫使用 `grep`/`ripgrep` 搜索源文件——无需预先索引。如果您已使用 `intel.enabled: true` 运行 `/ecl:map-codebase`，请将 `plan_review.source_grounding_authority: intel` 设置为使用更快的预构建 `api-map.json` 索引。
...
在项目设置时切换（`/ecl:new-project` 在工作流偏好设置期间询问）或随时通过 `/ecl:settings`（计划部分 → 漂移守卫）切换。
...
**在自主模式下跳过讨论阶段：** 以 `yolo` 模式运行时，通过 `/ecl-settings` 设置 `workflow.skip_discuss: true`。
...
### 程序化 CLI（`ecl-tools query` 与 `ecl-tools.cjs`）

对于自动化，优先使用带有已注册子命令的 **`ecl-tools query`**（参见 [CLI-TOOLS.md — SDK 和程序化访问](CLI-TOOLS.md#sdk-and-programmatic-access) 及 QUERY-HANDLERS.md）。旧版 `node $HOME/.claude/evolv-coder-lite/bin/ecl-tools.cjs` CLI 仍受支持。
...
eCL 子 Agent 在单独的上下文窗口中运行——其工作在进行中对父会话不可见。请勿中断会话。等待结果；研究和计划 Agent 通常需要 1–5 分钟。

### 长会话期间上下文退化

在主要命令之间清除上下文窗口：在 Claude Code 中使用 `/clear`。eCL 围绕全新上下文设计——每个子 Agent 获得一个干净的 200K 窗口。清除后使用 `/ecl-resume-work` 或 `/ecl-progress` 恢复状态。

### 计划似乎不正确或不一致

在计划前运行 `/ecl-discuss-phase [N]`。大多数计划质量问题来源于 Claude 在 `CONTEXT.md` 本可避免的情况下做出假设。
...
运行 `/ecl-progress`。它读取所有状态文件，精确告诉您当前所在位置和下一步操作。

### 模型成本过高

切换到预算配置文件：`/ecl-config --profile budget`。如果领域已熟悉，通过 `/ecl-settings` 禁用研究和计划检查 Agent。
...
这是**运行时设置**，不是 eCL 设置。切换项位于 `.claude/settings.json`：
...
完整审计、运行时参考及与 `model_profile` 的组合说明，请参阅捆绑的 `context-budget.md` 参考中的 [MCP 工具模式成本](../../evolv-coder-lite/references/context-budget.md#mcp-tool-schema-cost-harness-concern)。
...
如果您为非 Claude 运行时安装了 eCL，安装器已配置好模型解析。无需手动设置——`resolve_model_ids: "omit"` 会自动设置，告知 eCL 跳过 Anthropic 模型 ID 解析，让运行时选择其默认模型。
...
如果无法运行 eCL 安装器，则无法直接使用 `agents/` 中的源文件——它们采用 Claude Code 的原生 frontmatter 格式。对于 OpenCode，需要进行两项转换：

| 字段 | eCL 源格式 | OpenCode 有效格式 | 操作 |
|---|---|---|---|
...
切换到 `inherit` 配置文件：`/ecl-config --profile inherit`。这使所有 Agent 使用您当前的会话模型。

### 处理敏感/私有项目

在 `/ecl-new-project` 期间或通过 `/ecl-settings` 设置 `commit_docs: false`。将 `.planning/` 添加到您的 `.gitignore`。

### eCL 更新覆盖了我的本地更改

自 v1.17 起，安装器会将本地修改的文件备份到 `ecl-local-patches/`。运行 `/ecl-update --reapply` 将您的更改合并回来。
...
当工作流以不明显的方式失败时，运行 `/ecl-forensics` 生成涵盖 git 历史异常、产物完整性和状态不一致的诊断报告。输出写入 `.planning/forensics/`。
...
eCL 自 v1.26 起自动处理此问题。如果您使用的是旧版本，请在项目的 `CLAUDE.md` 中添加：
...
完全禁用并行执行：`/ecl-settings` → 将 `parallelization.enabled` 设置为 `false`。
...
| 问题                              | 解决方案                                                                 |
| ------------------------------------ | ------------------------------------------------------------------------ |
| 丢失上下文 / 新会话           | `/ecl-resume-work` 或 `/ecl-progress`                                    |
| 阶段出错                     | `git revert` 阶段提交，然后重新计划                             |
| 需要更改范围                 | `/ecl-phase`（默认）、`/ecl-phase --insert` 或 `/ecl-phase --remove`  |
| 出现问题                      | `/ecl-debug "description"`（添加 `--diagnose` 进行分析而不修复） |
| STATE.md 不同步                 | `state validate` 然后 `state sync`                                       |
| 工作流状态似乎损坏       | `/ecl-forensics`                                                         |
| 快速定向修复                   | `/ecl-quick`                                                             |
| 计划与您的愿景不符       | `/ecl-discuss-phase [N]` 然后重新计划                                    |
| 成本持续上涨                   | `/ecl-config --profile budget` 并通过 `/ecl-settings` 关闭 Agent  |
| 更新破坏了本地更改           | `/ecl-update --reapply`                                                  |
| 需要为利益相关者生成会话摘要 | `/ecl-pause-work --report`                                               |
| 不知道下一步是什么         | `/ecl-progress --next`                                                   |
| 并行执行构建错误      | 更新 eCL 或设置 `parallelization.enabled: false`                       |
