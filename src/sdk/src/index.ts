/**
 * eCL SDK — Public API for running eCL plans programmatically.
 *
 * The eCL class composes plan parsing, config loading, prompt building,
 * and session running into a single `executePlan()` call.
 *
 * @example
 * ```typescript
 * import { eCL } from '@evolvconsulting/ecl-sdk';
 *
 * const ecl = new eCL({ projectDir: '/path/to/project' });
 * const result = await ecl.executePlan('.planning/phases/01-auth/01-auth-01-PLAN.md');
 *
 * if (result.success) {
...
import { GSDTools, resolveGsdToolsPath } from './ecl-tools.js';
...
export class eCL {
  ...
  /**
   * Execute a single eCL plan file.
   *
  ...
  /**
   * Subscribe a simple handler to receive all eCL events.
   */
  ...
  /**
   * Subscribe a transport handler to receive all eCL events.
  ...
  /**
   * Load the ecl-executor agent definition if available.
   * Falls back gracefully — returns undefined if not found.
   */
  private async loadAgentDefinition(): Promise<string | undefined> {
    const paths = [
      // Repo-local eCL installation
      join(this.projectDir, '.claude', 'evolv-coder-lite', 'agents', 'ecl-executor.md'),
      // Repo-local agents directory
      join(this.projectDir, '.claude', 'agents', 'ecl-executor.md'),
      // Global home directory
      join(homedir(), '.claude', 'agents', 'ecl-executor.md'),
      join(this.projectDir, 'agents', 'ecl-executor.md'),
    ];
...
export { GSDTools, GSDToolsError, resolveGsdToolsPath } from './ecl-tools.js';
...
// Query registry argv normalization (matches `ecl-sdk query` and `GSDTools` hot path)
