/**
 * eCL Tools Tests - roadmap get-phase fallback to full ROADMAP.md
 *
...
// ECL_PROJECT / ECL_WORKSTREAM. These suites write STATE.md to <tmp>/.planning
// and assume that is where it is read from, so a developer shell inside a eCL
...
beforeEach(() => {
  savedGsdProject = process.env.ECL_PROJECT;
  savedGsdWorkstream = process.env.ECL_WORKSTREAM;
  delete process.env.ECL_PROJECT;
  delete process.env.ECL_WORKSTREAM;
});
afterEach(() => {
  if (savedGsdProject !== undefined) process.env.ECL_PROJECT = savedGsdProject;
  else delete process.env.ECL_PROJECT;
  if (savedGsdWorkstream !== undefined) process.env.ECL_WORKSTREAM = savedGsdWorkstream;
...
`
  ...
  test('extractCurrentMilestone does not truncate on phase heading containing vX.Y (#2619)', () => {
    ...
    const core = require('../evolv-coder-lite/bin/lib/roadmap-parser.cjs');
...
`;
  ...
  test('(7) workstream-aware: STATE.md under ECL_WORKSTREAM is read from the workstream subdir', () => {
    // Regression guard for the env-leak that made these suites pass in clean CI but
    // fail in a developer's ECL_WORKSTREAM shell. planningDir() is workstream-aware,
    ...
    process.env.ECL_WORKSTREAM = 'guard-ws';
    try {
