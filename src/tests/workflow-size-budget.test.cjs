 ...
 * into Claude's context every time the corresponding `/ecl:*` command is
 ...
 * that belongs in `evolv-coder-lite/references/` or a per-mode subdirectory
  ...
  for (const workflow of ALL_WORKFLOWS) {
    ...
    test(`${workflow} (${tier}) stays under ${limit} lines`, () => {
      ...
      assert.ok(
        ...
        `to evolv-coder-lite/references/. See workflows/discuss-phase/ for the pattern.`
      );
    });
  }
});
