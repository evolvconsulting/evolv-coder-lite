...
/ecl-workspace --new --name feature-b --repos . --strategy worktree  # 동일 저장소 격리
/ecl-workspace --new --name spike --repos api,web --strategy clone   # 전체 클론
```
...
활성 eCL 워크스페이스와 상태를 목록으로 표시합니다.

**스캔 위치:** `~/ecl-workspaces/`에서 `WORKSPACE.md` 매니페스트를 탐색합니다.
**표시 항목:** 이름, 저장소 수, 전략, eCL 프로젝트 상태
...
```bash
/ecl-discuss-phase 1                # 페이즈 1 대화형 토론
/ecl-discuss-phase 3 --auto         # 페이즈 3 기본값 자동 선택
/ecl-discuss-phase --batch          # 현재 페이즈 일괄 모드
/ecl-discuss-phase 2 --analyze      # 트레이드오프 분석 포함 토론
```
...
```bash
/ecl-plan-phase 1                   # 페이즈 1 조사 + 계획 + 검증
/ecl-plan-phase 3 --skip-research   # 조사 없이 계획 (익숙한 도메인)
...
```bash
/ecl-execute-phase 1                # 페이즈 1 실행
/ecl-execute-phase 1 --wave 2       # Wave 2만 실행
```
...
- 페이즈 토론 필요 → `/ecl-discuss-phase` 실행
- 페이즈 계획 필요 → `/ecl-plan-phase` 실행
- 페이즈 실행 필요 → `/ecl-execute-phase` 실행
- 페이즈 검증 필요 → `/ecl-verify-work` 실행
...
```bash
/ecl-pause-work --report                 # 세션 종료 후 요약 생성
```
...
**사전 조건:** 페이즈 검증 완료 (`/ecl-verify-work` 통과), `gh` CLI 설치 및 인증
**생성 파일:** 계획 아티팩트 기반의 풍부한 본문이 포함된 GitHub PR, STATE.md 업데이트

```bash
/ecl-ship 4                         # 페이즈 4 출시
/ecl-ship 4 --draft                 # 초안 PR로 출시
```
...
**사전 조건:** 프론트엔드 코드가 있는 프로젝트 (독립 실행 가능, eCL 프로젝트 불필요)
**생성 파일:** `{phase}-UI-REVIEW.md`, `.planning/ui-reviews/`에 스크린샷

```bash
/ecl-ui-review                      # 현재 페이즈 감사
/ecl-ui-review 3                    # 페이즈 3 감사
```
...
```bash
/ecl-new-milestone                  # 대화형 모드
/ecl-new-milestone "v2.0 Mobile"    # 이름이 지정된 마일스톤
...
```bash
/ecl-phase --insert 3                 # 페이즈 3과 4 사이에 삽입 → 3.1 생성
```
...
```bash
/ecl-phase --remove 7                 # 페이즈 7 제거, 8→7, 9→8 등으로 재번호
```
...
```bash
/ecl-discuss-phase --assumptions 2       # 페이즈 2 가정 사항 확인
```


### `/ecl-plan-phase --research-phase`

심층 에코시스템 조사만 수행합니다 (독립 실행 — 일반적으로 `/ecl-plan-phase`를 사용하세요).
...
```bash
/ecl-plan-phase --research-phase 4               # 페이즈 4 도메인 조사
```
...
```bash
/ecl-validate-phase 2               # 페이즈 2 테스트 커버리지 감사
```
...
```bash
/ecl-progress                       # "지금 어디 있나? 다음은 무엇인가?"
```
...
```bash
/ecl-resume-work                    # 컨텍스트 초기화 또는 새 세션 후 실행
```
...
```bash
/ecl-manager --analyze-deps            # 의존성 분석 및 제안
```
...
```bash
/ecl-quick                          # 기본 빠른 작업
/ecl-quick --discuss --research     # 토론 + 조사 + 계획
/ecl-quick --full                   # 계획 검사 및 검증 포함
/ecl-quick --discuss --research --full  # 모든 선택적 단계 포함
```
...
```bash
/ecl-autonomous                     # 남은 모든 페이즈 실행
/ecl-autonomous --from 3            # 페이즈 3부터 시작
/ecl-autonomous --to 5              # 페이즈 5까지만 실행
/ecl-autonomous --from 3 --to 5     # 페이즈 3~5 범위 실행
/ecl-autonomous --only 4            # 페이즈 4만 자율 실행
```

### `/ecl-fast`

자유 형식 텍스트를 적절한 eCL 명령어로 라우팅합니다.

```bash
/ecl-fast                             # 원하는 작업을 설명합니다
```
...
/ecl-profile-user --questionnaire   # 대화형 설문지 대체 방법
/ecl-profile-user --refresh         # 새로운 분석으로 재생성
```
...
실패하거나 멈춘 eCL 워크플로우에 대한 사후 조사를 수행합니다.
...
```bash
/ecl-forensics                              # 대화형 — 문제 입력 프롬프트
/ecl-forensics "Phase 3 execution stalled"  # 문제 설명과 함께 실행
```
...
```bash
/ecl-workstreams                    # 모든 워크스트림 목록
/ecl-workstreams create backend-api # 새 워크스트림 생성
/ecl-workstreams switch backend-api # 활성 워크스트림 설정
/ecl-workstreams status backend-api # 상세 상태 확인
/ecl-workstreams progress           # 교차 워크스트림 진행 상황 개요
/ecl-workstreams complete backend-api  # 완료된 워크스트림 아카이브
/ecl-workstreams resume backend-api    # 워크스트림 작업 재개
```
...
```bash
/ecl-map-codebase                   # 전체 코드베이스 분석
/ecl-map-codebase auth              # 인증 영역에 집중
```
...
eCL 업데이트 후 로컬 수정사항을 복원합니다.

```bash
/ecl-update --reapply               # 로컬 변경사항 병합
```
...
**`/ecl-quick`의 대체가 아닙니다.** 조사, 다단계 계획 또는 검증이 필요한 작업에는 `/ecl-quick`을 사용하세요.
...
**생성 파일:** `{phase}-REVIEWS.md` — `/ecl-plan-phase --reviews`에서 사용 가능
...
**목적:** 리뷰어에게 eCL 계획 아티팩트가 아닌 코드 변경사항만 표시합니다.

```bash
/ecl-pr-branch                     # main을 기준으로 필터링
/ecl-pr-branch develop             # develop을 기준으로 필터링
```
...
**999.x 번호 체계**는 백로그 항목을 활성 페이즈 순서 밖에 유지합니다. 페이즈 디렉터리가 즉시 생성되므로 해당 항목에 대해 `/ecl-discuss-phase`와 `/ecl-plan-phase`를 사용할 수 있습니다.
...
**사용처:** `/ecl-new-milestone` (시드를 스캔하여 일치 항목 제시)
...
스레드는 여러 세션에 걸쳐 이어지지만 특정 페이즈에 속하지 않는 작업을 위한 경량 교차 세션 지식 저장소입니다. `/ecl-pause-work`보다 가볍습니다.

```bash
/ecl-thread                         # 모든 스레드 목록
