...
describe('#1656: community .sh hooks must be present in hooks/dist', () => {
  ...
  test('ecl-session-state.sh exists in hooks/dist', () => {
    const p = path.join(HOOKS_DIST, 'ecl-session-state.sh');
    assert.ok(fs.existsSync(p), 'ecl-session-state.sh must be in hooks/dist/ so the installer can copy it');
  });

  test('ecl-validate-commit.sh exists in hooks/dist', () => {
    const p = path.join(HOOKS_DIST, 'ecl-validate-commit.sh');
    assert.ok(fs.existsSync(p), 'ecl-validate-commit.sh must be in hooks/dist/ so the installer can copy it');
  });

  test('ecl-phase-boundary.sh exists in hooks/dist', () => {
    const p = path.join(HOOKS_DIST, 'ecl-phase-boundary.sh');
    assert.ok(fs.existsSync(p), 'ecl-phase-boundary.sh must be in hooks/dist/ so the installer can copy it');
  });
});
...
// flow that shipped when `@evolvconsulting/ecl-sdk` did not yet exist on npm. The
// package was published at v0.1.0 and is now a hard runtime requirement for
// every /ecl-* command (they all shell out to `ecl-sdk query …`).
//
