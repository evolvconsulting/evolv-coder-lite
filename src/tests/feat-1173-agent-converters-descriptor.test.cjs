...
name: ecl-planner
description: A eCL planning agent.
...
This agent plans eCL phases using ~/.claude/skills.
`;

/**
 * Create a temp agents directory with a .ecl-source marker pointing back to it
 * (so findAgentsSourceRoot finds the fixture dir, not the real agents/ dir).
 * The .ecl-source convention expects the marker to point at a commands/ecl dir;
 * agents/ is resolved as a sibling of commands/. So we set up:
 *   <tmproot>/
 *     commands/ecl/       (empty, satisfies the sibling check)
 *     agents/
 *       ecl-planner.md
 *     .ecl-source         <- points to <tmproot>/commands/ecl
 */
function makeFixtureRoot(agentFiles) {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-1173-root-'));
  const commandsDir = path.join(root, 'commands', 'ecl');
  const agentsDir = path.join(root, 'agents');
  fs.mkdirSync(commandsDir, { recursive: true });
  fs.mkdirSync(agentsDir, { recursive: true });
  // .ecl-source marker must point to commands/ecl so that agentsSourceRoot resolves to agents/
  fs.writeFileSync(path.join(root, '.ecl-source'), commandsDir + '\n', 'utf8');
...
describe('feat-1173: stageAgentsForRuntimeWithConverter', () => {
  test('is exported from install-profiles', () => {
    const installProfiles = require(path.join(ROOT, 'evolv-coder-lite', 'bin', 'lib', 'install-profiles.cjs'));
    assert.strictEqual(
  ...
  test('applies converter to each staged agent file', (t) => {
    const { stageAgentsForRuntimeWithConverter } = require(path.join(ROOT, 'evolv-coder-lite', 'bin', 'lib', 'install-profiles.cjs'));

    const agentsDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-1173-agents-'));
    fs.writeFileSync(path.join(agentsDir, 'ecl-planner.md'), CLAUDE_AGENT_SOURCE, 'utf8');
    fs.writeFileSync(path.join(agentsDir, 'ecl-executor.md'), CLAUDE_AGENT_SOURCE.replace('ecl-planner', 'ecl-executor'), 'utf8');
    ...
    assert.deepStrictEqual(stagedFiles, ['ecl-executor.md', 'ecl-planner.md']);
  ...
  test('non-existent srcAgentsDir returns srcAgentsDir unchanged', () => {
    const { stageAgentsForRuntimeWithConverter } = require(path.join(ROOT, 'evolv-coder-lite', 'bin', 'lib', 'install-profiles.cjs'));
    const ghost = path.join(os.tmpdir(), 'ecl-1173-no-exist-' + Date.now());
    const converter = (c) => c;
  ...
  test('only copies .md files (ignores non-.md)', (t) => {
    ...
    assert.deepStrictEqual(stagedFiles, ['ecl-planner.md'], 'only .md files should be staged');
  });
});
...
describe('feat-1173: dispatchKindEntry agents converter wiring', () => {
  test('agents kind with convertClaudeAgentToCopilotAgent converter applies copilot conversion', (t) => {
    const fixtureRoot = makeFixtureRoot([{ name: 'ecl-planner.md', content: CLAUDE_AGENT_SOURCE }]);
    t.after(() => {
      cleanup(fixtureRoot);
      cleanupStagedSkills();
    });
    ...
    const stagedFile = path.join(stagedDir, 'ecl-planner.md');
  ...
  test('agents kind with convertClaudeAgentToCodexAgent converter applies codex conversion', (t) => {
    ...
    const stagedContent = fs.readFileSync(path.join(stagedDir, 'ecl-planner.md'), 'utf8');
...
describe('feat-1173: real registry claude agents kind has converter=null (backward compat)', () => {
  test('claude local artifacts layout has agents entry with converter=null', () => {
    const registry = require(path.join(ROOT, 'evolv-coder-lite', 'bin', 'lib', 'capability-registry.cjs'));
