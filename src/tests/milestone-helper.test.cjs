...
describe('getMilestoneFromPhaseId', () => {
  ...
  test('strips longer project_code prefix: eCL-10-01 → v10.0', () => {
    assert.strictEqual(getMilestoneFromPhaseId('eCL-10-01'), 'v10.0');
  });
...
describe('getPhaseDirFromPhaseId', () => {
  test('produces zero-padded dir with project code', () => {
    assert.strictEqual(
      getPhaseDirFromPhaseId('2-01', 'Setup Database', 'eCL'),
      'eCL-02-01-setup-database',
    );
  });
  ...
  test('returns null for phase IDs without the M-NN hyphen form', () => {
    assert.strictEqual(
      getPhaseDirFromPhaseId('nohyphen', 'Some Title', 'eCL'),
      null,
    );
  });
});
