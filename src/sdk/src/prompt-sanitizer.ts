 ...
 * patterns from eCL-1 prompts so they're safe for headless SDK use.
 *
 * @-file references (e.g., @~/.claude/evolv-coder-lite/references/foo.md) are
 ...
 * - /ecl-... skill commands (can't invoke skills in Agent SDK)
 ...
 * - @~/.claude/evolv-coder-lite/references/foo.md
 * - @~/.claude/evolv-coder-lite/workflows/bar.md
...
const LINE_PATTERNS: RegExp[] = [
  ...
  // /ecl-command references — entire line containing a skill command
  /^.*\/ecl[:-]\S+.*$/gm,
