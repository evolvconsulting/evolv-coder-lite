 ...
 *
 * `ecl-tools.cjs config-get context_window` (and the SDK equivalent) threw
...
// "Key not found"; now the test passes `--json-errors` to ecl-tools and
...
describe('bug-2943: config-get returns schema default for context_window', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-test-2943-'));
  ...
  function runConfigGet(keyPath, extraArgs = []) {
    const args = [ECL_TOOLS, 'config-get', keyPath, '--raw', '--cwd', tmpDir, ...extraArgs];
    let stdout = '';
    let stderr = '';
    let exitCode = 0;
    try {
