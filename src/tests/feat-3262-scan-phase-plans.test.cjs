...
const scanPhasePlans = require('../evolv-coder-lite/bin/lib/plan-scan.cjs');
...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-plan-scan-'));
});
...
describe('scanPhasePlans — flat layout', () => {
  ...
  test('extended-layout root file (5-PLAN-01-setup.md style)', () => {
    ...
    // e.g. ecl-plan-phase writes "5-PLAN-01-setup.md".
    const dir = phaseDir();
