...
if [ -n "$CODEX_MODEL" ] && [ "$CODEX_MODEL" != "null" ]; then
  cat /tmp/ecl-review-prompt-{phase}.md | codex exec --model "$CODEX_MODEL" --skip-git-repo-check - 2>/dev/null > /tmp/ecl-review-codex-{phase}.md
else
  cat /tmp/ecl-review-prompt-{phase}.md | codex exec --skip-git-repo-check - 2>/dev/null > /tmp/ecl-review-codex-{phase}.md
fi
```
...
**Cursor:**
```bash
cat /tmp/ecl-review-prompt-{phase}.md | cursor agent -p --mode ask --trust 2>/dev/null > /tmp/ecl-review-cursor-{phase}.md
if [ ! -s /tmp/ecl-review-cursor-{phase}.md ]; then
  echo "Cursor review failed or returned empty output." > /tmp/ecl-review-cursor-{phase}.md
fi
```
