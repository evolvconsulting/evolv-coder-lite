...
eCL Core は Gemini CLI、Claude（別セッション）、Codex CLI、CodeRabbit、OpenCode、Qwen Code、Cursor、Antigravity CLI、Ollama、LM Studio、llama.cpp の任意の組み合わせにレビューリクエストをルーティングできます。
...
デフォルトでは `/ecl-review` は検出されたすべての CLI を実行します。プロジェクトのデフォルトとして特定のサブセットを固定するには:
...
eCL は各レビュアーを順番に呼び出し、構造化されたフィードバック（サマリー、強み、HIGH/MEDIUM/LOW の懸念事項、提案、リスク評価）を収集し、結合された出力を `.planning/phases/03-.../03-REVIEWS.md` に書き込みます。
...
デフォルト（`localhost:11434` / `localhost:1234`）が合わない場合は、`/ecl-config --integrations` で `review.*` キーの下にホストアドレスとモデル選択を設定してください。
...
**ストール検知:** サイクルをまたいで HIGH 懸念事項のカウントが減少していない場合、eCL は警告します。サイクル上限に達しても HIGH 懸念事項が残っている場合、エスカレーションゲートが表示され、続行するか手動でレビューするかを確認します。
...
| リリース前に最大限のカバレッジが必要 | `/ecl-plan-review-convergence N --all` |
| 素早く反復して高速なフィードバックが欲しい | 1 つの CLI を選ぶ: `/ecl-review --phase N --gemini` |
