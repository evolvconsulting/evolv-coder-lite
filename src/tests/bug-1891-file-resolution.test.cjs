...
// ecl-tools.cjs @file: resolution is a low-level stdout interception that cannot be
 ...
 *
 * ecl-tools.cjs must transparently resolve @file: references in stdout
...
const ECL_TOOLS_SRC = path.join(__dirname, '..', 'evolv-coder-lite', 'bin', 'ecl-tools.cjs');

describe('bug #1891: @file: resolution in ecl-tools.cjs', () => {
  let src;

  before(() => {
    src = fs.readFileSync(ECL_TOOLS_SRC, 'utf-8');
  });
