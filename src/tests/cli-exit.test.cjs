...
// Paths to the compiled product seam (src/cli-exit.cts → evolv-coder-lite/bin/lib/cli-exit.cjs)
// used for json-error mode regression tests which require io.cjs integration.
const BUILT_CLI_EXIT_PATH = path.resolve(__dirname, '../evolv-coder-lite/bin/lib/cli-exit.cjs');
const IO_PATH = path.resolve(__dirname, '../evolv-coder-lite/bin/lib/io.cjs');
 ...
 *
 * Tests run against the compiled product seam (evolv-coder-lite/bin/lib/cli-exit.cjs)
