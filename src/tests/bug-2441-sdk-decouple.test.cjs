 ...
 *
 * (b) The parent package.json declares a `ecl-sdk` bin entry pointing at
 *     bin/ecl-sdk.js (the back-compat shim), so npm chmods it correctly.
 *
...
const ECL_SDK_SHIM = path.join(__dirname, '..', 'bin', 'ecl-sdk.js');
...
describe('fix #2441: back-compat shim — parent package bin entry', () => {
  test('root package.json declares ecl-sdk bin entry', () => {
    assert.ok(
      rootPkg.bin && rootPkg.bin['ecl-sdk'],
      'root package.json must have a bin["ecl-sdk"] entry for the back-compat shim.'
    );
  });

  test('ecl-sdk bin entry points at bin/ecl-sdk.js', () => {
    assert.equal(
      rootPkg.bin['ecl-sdk'],
      'bin/ecl-sdk.js',
      'bin["ecl-sdk"] must point at bin/ecl-sdk.js'
    );
  });

  test('bin/ecl-sdk.js shim file exists', () => {
    assert.ok(
      fs.existsSync(ECL_SDK_SHIM),
      'bin/ecl-sdk.js must exist as the back-compat PATH shim.'
    );
  });

  test('bin/ecl-sdk.js resolves sdk/dist/cli.js relative to itself', () => {
    const shimContent = fs.readFileSync(ECL_SDK_SHIM, 'utf-8');
    ...
    assert.match(
      ...
      'bin/ecl-sdk.js must call path.resolve(__dirname, "..", "sdk", "dist", "cli.js") to locate the prebuilt CLI.'
    );
  });

  test('bin/ecl-sdk.js invokes cli.js via spawnSync(process.execPath, ...)', () => {
    const shimContent = fs.readFileSync(ECL_SDK_SHIM, 'utf-8');
    ...
    assert.match(
      ...
      'bin/ecl-sdk.js must spawn node via process.execPath so the execute bit on cli.js is irrelevant (#2453).'
    );
    assert.match(
      shimContent,
      /process\.argv\.slice\(\s*2\s*\)/,
      'bin/ecl-sdk.js must forward user args via process.argv.slice(2).'
    );
    assert.match(
      shimContent,
      /process\.exit\(/,
      'bin/ecl-sdk.js must propagate the child exit status via process.exit.'
    );
  });
});
