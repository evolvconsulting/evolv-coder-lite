...
- @~/.claude/evolv-coder-lite/workflows/complete-milestone.md (main workflow)
- @~/.claude/evolv-coder-lite/templates/milestone-archive.md (archive template)
  </execution_context>
...
0. **Check for audit:**
   ...
   - If missing or stale: recommend `/ecl:audit-milestone` first
   - If audit status is `gaps_found`: recommend closing the gaps inline
     ...
     via `/ecl:phase --insert <N>` plus the standard
   ...
   ⚠ No milestone audit found. Run `/ecl:audit-milestone` first to verify
   ...
   a closure phase per gap with `/ecl:phase --insert <N>` and run the
   standard `/ecl:discuss-phase` → `/ecl:plan-phase` → `/ecl:execute-phase`
...
- **Fresh requirements:** Next milestone starts with `/ecl:new-milestone` which includes requirements definition
  </critical_rules>
