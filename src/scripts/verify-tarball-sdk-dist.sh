#!/usr/bin/env bash
# Verify the published evolv-coder-lite tarball actually contains
...
echo "==> Installing runtime deps inside the extracted package and invoking ecl-sdk query --help"
...
echo "==> Also verifying ecl-sdk bin shim resolves ../sdk/dist/cli.js"
SHIM="$EXTRACT_DIR/package/bin/ecl-sdk.js"
if [ ! -f "$SHIM" ]; then
  echo "::error::bin/ecl-sdk.js missing from tarball"
  exit 1
fi
if ! grep -qE "sdk.*dist.*cli\.js" "$SHIM"; then
  echo "::error::bin/ecl-sdk.js does not reference sdk/dist/cli.js"
  exit 1
fi
