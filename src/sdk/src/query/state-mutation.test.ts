...
describe('acquireStateLock / releaseStateLock', () => {
  ...
  beforeEach(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), 'ecl-lock-'));
  });
...
describe('stateUpdate', () => {
  ...
  beforeEach(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), 'ecl-state-update-'));
...
describe('statePatch', () => {
  ...
  beforeEach(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), 'ecl-state-patch-'));
    await setupTestProject(tmpDir);
  });
...
describe('stateBeginPhase', () => {
  ...
  beforeEach(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), 'ecl-state-begin-'));
    await setupTestProject(tmpDir);
  });
...
describe('stateAdvancePlan', () => {
  ...
  beforeEach(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), 'ecl-state-advance-'));
    await setupTestProject(tmpDir);
  });
...
describe('stateAddDecision', () => {
  ...
  beforeEach(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), 'ecl-state-decision-'));
    await setupTestProject(tmpDir);
  });
...
describe('stateAddRoadmapEvolution', () => {
  ...
  beforeEach(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), 'ecl-state-evo-'));
  });
...
describe('stateRecordSession', () => {
  ...
  beforeEach(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), 'ecl-state-session-'));
    await setupTestProject(tmpDir);
  });
...
describe('Bug #2613: STATE.md frontmatter preservation through mutations', () => {
  ...
  beforeEach(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), 'ecl-state-2613-'));
  });
...
describe('stateMilestoneSwitch', () => {
  ...
  beforeEach(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), 'ecl-milestone-switch-'));
  });
...
describe('statePrune current phase extraction (#3471)', () => {
  ...
  beforeEach(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), 'ecl-sdk-stateprune-'));
  });
