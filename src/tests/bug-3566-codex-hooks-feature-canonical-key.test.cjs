 ...
 * `hooks`. The eCL installer was still writing `codex_hooks` on every fresh
 ...
 *   2. Reinstall over a eCL-owned section-form legacy
 ...
 *   3. Reinstall over a eCL-owned root-dotted legacy
 *      `features.codex_hooks = true` migrates forward to
 *      `features.hooks = true`; user-owned legacy entries are preserved.
 *   4. Reinstall over a user-owned `[features].hooks = true` (no eCL
 ...
 *   6. Uninstall removes either eCL-owned `hooks` or eCL-owned legacy
...
describe('#3566 — Codex feature flag is canonical "hooks" (not legacy "codex_hooks")', { concurrency: false }, () => {
  ...
  beforeEach(() => {
    ...
    tmpRoot = createTempDir('ecl-3566-');
  ...
  test('install over a pre-existing legacy [features].codex_hooks line preserves it (user-owned, #2760 defensive)', () => {
    ...
    // forward-compatible without eCL rewriting user-authored content.
    const legacy = [
      '[features]',
      'codex_hooks = true',
      '',
  ...
  test('reinstall preserves user-owned [features].hooks = true (no eCL ownership marker)', () => {
    const userOwned = [
      '[features]',
      'hooks = true',
      '',
  ...
  test('uninstall removes eCL-owned canonical hooks line but preserves user-owned hooks', () => {
    // Phase 1: fresh eCL install — writes eCL-owned hooks line.
    withCodexHome(codexHome, () => install(true, 'codex'));
    ...
    assert.notStrictEqual(
      ...
      true,
      'uninstall must remove eCL-owned canonical hooks line',
    );
  });

  test('uninstall preserves user-owned hooks=true when eCL never owned it', () => {
    const userOwned = [
      '[features]',
      'hooks = true',
      '',
    ...
    assert.strictEqual(
      featuresHooks(parsed),
      true,
      'uninstall must NOT touch a hooks line eCL never claimed ownership of (#2760 defensive principle)',
    );
  });
});
