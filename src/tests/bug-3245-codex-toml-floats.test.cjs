 ...
 *     (integers fail with "invalid type: integer"). eCL's strict-integer-only parser
...
// ECL_TEST_MODE must be set before require('../bin/install.js') so the module
...
describe('#3245 — install succeeds with TOML float in pre-existing config', { concurrency: false }, () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-3245-float-'));
...
describe('#3245 — idempotent rollback reverts skills/, agents/, and VERSION', { concurrency: false }, () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-3245-rollback-'));
    codexHome = path.join(tmpDir, 'codex-home');
  });
  ...
  test('validation failure rolls back skills/, agents/, and VERSION to pre-install state', () => {
    // Start from a clean codexHome with no pre-existing eCL content — the dirs
    ...
    // skills/ — eCL writes ecl-* subdirs here. All must be absent after rollback.
    ...
    if (fs.existsSync(skillsDir)) {
      ...
      assert.strictEqual(
        gsdSkills.length,
        0,
        'rollback must remove all ecl-* skill directories: ' + gsdSkills.map(e => e.name).join(', ')
      );
    }

    // agents/ — eCL writes ecl-*.md and ecl-*.toml here. All must be absent.
    const agentsDir = path.join(codexHome, 'agents');
    if (fs.existsSync(agentsDir)) {
      const gsdAgents = fs.readdirSync(agentsDir)
        .filter(f => f.startsWith('ecl-') && (f.endsWith('.md') || f.endsWith('.toml')));
      assert.strictEqual(
        gsdAgents.length,
        0,
        'rollback must remove all ecl-* agent files: ' + gsdAgents.join(', ')
      );
    }

    // VERSION — eCL writes evolv-coder-lite/VERSION. Must be absent (wasn't there before).
    const versionPath = path.join(codexHome, 'evolv-coder-lite', 'VERSION');
    assert.strictEqual(
  ...
  test('rollback is safe when fired before any snapshots were captured (very early failure)', () => {
    ...
    // Rollback removes all ecl-* skill dirs it wrote. Even if skills/ was
    // created during the install, no ecl-* dirs should survive after rollback.
    const skillsDir = path.join(codexHome, 'skills');
    ...
    assert.deepStrictEqual(
      remainingGsdSkills,
      [],
      'rollback must remove all ecl-* skill dirs even when fired after minimal writes'
    );
  });

  test('rollback does not remove pre-existing user skills that eCL did not write', () => {
    // If the user has a custom skill dir (not ecl-*) it must survive rollback.
    const skillsDir = path.join(codexHome, 'skills');
    ...
    assert.strictEqual(
      ...
      true,
      'pre-existing non-ecl-* skill must survive rollback'
    );
  });
