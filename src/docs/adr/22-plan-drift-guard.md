...
- **Status:** Proposed
- **Date:** 2026-05-29
- **Issue:** evolvconsulting/evolv-coder-lite#22
...
`/ecl:plan-review-convergence` does not catch it because planner and reviewer both read the same channel (other plan files); the drift originates in that channel. The fix must introduce an out-of-band source of truth: the project's own source code.

eCL ships `intel.cjs` (`api-map.json` et al.), but: enabling `intel.enabled` populates nothing (the `ecl-intel-updater` LLM agent is never auto-spawned; population is a manual `/ecl:map-codebase --query refresh`); extraction is regex, JS/CJS/ESM only; intel is stale after 24h with no auto-refresh (`intelUpdate()` is a stub); useful intel therefore costs recurring LLM-agent token spend.
...
1. Ship the **verification pass on by default**, behind a new, additive config key `plan_review.source_grounding` (boolean, default `true`, opt-out). Surface it as one question in `/ecl:new-project` (default Y) and as a toggle in `/ecl:settings`. It stays on permanently with an easy off switch.
...
## References
- Issue: evolvconsulting/evolv-coder-lite#22 (migrated from evolvconsulting/evolv-coder-lite#3813)
