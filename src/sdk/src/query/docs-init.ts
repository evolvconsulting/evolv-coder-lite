 ...
 *
 * Full port of `cmdDocsInit` and helpers from `evolv-coder-lite/bin/lib/docs.cjs`.
 */
...
function hasGsdMarker(filePath: string): boolean {
  try {
    ...
    closeSync(fd);
    return buf.subarray(0, bytesRead).toString('utf-8').includes(ECL_MARKER);
  } catch {
    return false;
  }
}
...
/**
 * Init payload for docs-update workflow — matches `ecl-tools docs-init` JSON.
...
export const docsInit: QueryHandler = async (_args, projectDir) => {
  ...
  const docModelResult = await resolveModel(['ecl-doc-writer'], projectDir);
