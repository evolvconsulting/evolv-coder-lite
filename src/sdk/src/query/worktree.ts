...
function worktreeHandleResult(result: ReturnType<typeof spawnSync>) {
  if (result.error) {
    return { data: { ok: false, reason: (result.error as Error).message || 'ecl-tools invocation failed' } };
  }
  ...
  return {
    data: {
      ok: result.status === 0,
      reason: (result.stderr as string)?.trim() || (result.status === 0 ? 'ok' : 'ecl-tools error'),
    },
  };
}
...
export const worktreeCleanupWave: QueryHandler = async (args, projectDir) => {
  ...
  if (result.error) {
    return { data: { ok: false, reason: result.error.message || 'ecl-tools invocation failed' } };
  }
  ...
  return {
    data: {
      ok: result.status === 0,
      reason: result.stderr?.trim() || (result.status === 0 ? 'ok' : 'ecl-tools error'),
    },
  };
};
