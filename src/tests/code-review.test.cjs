...
/**
 * eCL Code Review Tests
 *
 * Validates all code review artifacts from Phases 1-4:
 * - Agent frontmatter (ecl-code-reviewer, ecl-code-fixer)
...
const PLUGIN_WORKFLOWS_DIR = process.env.ECL_PLUGIN_ROOT || path.join(os.homedir(), '.claude', 'evolv-coder-lite', 'workflows');
...
describe('CR-AGENT: code review agent frontmatter', () => {
  test('ecl-code-reviewer.md has required frontmatter fields', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'ecl-code-reviewer.md'), 'utf-8');
    const frontmatter = content.split('---')[1] || '';

    assert.ok(frontmatter.includes('name:'), 'ecl-code-reviewer missing name:');
    assert.ok(frontmatter.includes('description:'), 'ecl-code-reviewer missing description:');
    assert.ok(frontmatter.includes('tools:'), 'ecl-code-reviewer missing tools:');
    assert.ok(frontmatter.includes('color:'), 'ecl-code-reviewer missing color:');
  });

  test('ecl-code-fixer.md has required frontmatter fields', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'ecl-code-fixer.md'), 'utf-8');
    const frontmatter = content.split('---')[1] || '';

    assert.ok(frontmatter.includes('name:'), 'ecl-code-fixer missing name:');
    assert.ok(frontmatter.includes('description:'), 'ecl-code-fixer missing description:');
    assert.ok(frontmatter.includes('tools:'), 'ecl-code-fixer missing tools:');
    assert.ok(frontmatter.includes('color:'), 'ecl-code-fixer missing color:');
  });

  test('ecl-code-reviewer.md has Read, Bash, Glob, Grep, Write tools', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'ecl-code-reviewer.md'), 'utf-8');
    const frontmatter = content.split('---')[1] || '';

    assert.ok(frontmatter.includes('Read'), 'ecl-code-reviewer missing Read tool');
    assert.ok(frontmatter.includes('Bash'), 'ecl-code-reviewer missing Bash tool');
    assert.ok(frontmatter.includes('Glob'), 'ecl-code-reviewer missing Glob tool');
    assert.ok(frontmatter.includes('Grep'), 'ecl-code-reviewer missing Grep tool');
    assert.ok(frontmatter.includes('Write'), 'ecl-code-reviewer missing Write tool');
  });

  test('ecl-code-fixer.md has Read, Edit, Write, Bash, Grep, Glob tools', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'ecl-code-fixer.md'), 'utf-8');
    const frontmatter = content.split('---')[1] || '';

    assert.ok(frontmatter.includes('Read'), 'ecl-code-fixer missing Read tool');
    assert.ok(frontmatter.includes('Edit'), 'ecl-code-fixer missing Edit tool');
    assert.ok(frontmatter.includes('Write'), 'ecl-code-fixer missing Write tool');
    assert.ok(frontmatter.includes('Bash'), 'ecl-code-fixer missing Bash tool');
  });

  test('ecl-code-reviewer.md does not have skills: in frontmatter', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'ecl-code-reviewer.md'), 'utf-8');
    const frontmatter = content.split('---')[1] || '';

    assert.ok(!frontmatter.includes('skills:'),
      'ecl-code-reviewer has skills: in frontmatter — breaks Gemini CLI');
  });

  test('ecl-code-fixer.md does not have skills: in frontmatter', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'ecl-code-fixer.md'), 'utf-8');
    const frontmatter = content.split('---')[1] || '';

    assert.ok(!frontmatter.includes('skills:'),
      'ecl-code-fixer has skills: in frontmatter — breaks Gemini CLI');
  });

  test('ecl-code-fixer.md rollback uses git checkout (not Write tool)', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'ecl-code-fixer.md'), 'utf-8');
    assert.ok(content.includes('git checkout --'),
      'ecl-code-fixer rollback should use git checkout -- {file} for atomic rollback');
    assert.ok(!content.includes('PRE_FIX_CONTENT'),
      'ecl-code-fixer should not use PRE_FIX_CONTENT in-memory capture (use git checkout instead)');
  });

  test('ecl-code-fixer.md success_criteria consistent with rollback strategy (git checkout)', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'ecl-code-fixer.md'), 'utf-8');
    ...
    assert.ok(successCriteria.includes('git checkout'),
      'ecl-code-fixer success_criteria must reference git checkout rollback');
    assert.ok(!successCriteria.includes('Write tool with captured'),
      'ecl-code-fixer success_criteria must not say Write tool for rollback');
  });

  test('ecl-code-fixer.md flags logic-bug fixes for human review', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'ecl-code-fixer.md'), 'utf-8');
    assert.ok(content.includes('requires human verification'),
      'ecl-code-fixer should flag logic-bug fixes as requiring human verification');
  });

  test('ecl-code-reviewer.md REVIEW.md spec includes files_reviewed_list field', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'ecl-code-reviewer.md'), 'utf-8');
    assert.ok(content.includes('files_reviewed_list'),
      'ecl-code-reviewer REVIEW.md frontmatter spec must include files_reviewed_list for --auto scope persistence');
  });
});
...
describe('CR-CMD: code review command structure', () => {
  test('code-review.md has correct frontmatter name: ecl:code-review', () => {
    const content = fs.readFileSync(path.join(COMMANDS_DIR, 'code-review.md'), 'utf-8');
    const frontmatter = content.split('---')[1] || '';

    assert.ok(frontmatter.includes('name: ecl:code-review'),
...
describe('CR-WORKFLOW: code review workflow structure', () => {
  ...
  test('code-review.md workflow references ecl-code-reviewer agent', () => {
    const content = fs.readFileSync(path.join(WORKFLOWS_DIR, 'code-review.md'), 'utf-8');

    assert.ok(content.includes('ecl-code-reviewer'),
      'code-review.md workflow does not reference ecl-code-reviewer agent');
  });
  ...
  test('code-review-fix.md workflow references ecl-code-fixer agent', () => {
    const content = fs.readFileSync(path.join(WORKFLOWS_DIR, 'code-review-fix.md'), 'utf-8');

    assert.ok(content.includes('ecl-code-fixer'),
      'code-review-fix.md workflow does not reference ecl-code-fixer agent');
  });
...
describe('CR-INTEGRATION: workflow integration points', () => {
  ...
  test('autonomous.md contains ecl-code-review skill invocation', () => {
    ...
    // (ecl-code-review); colon form (ecl:code-review) is the legacy
    ...
    assert.ok(skillNames.includes('ecl-code-review'),
      `autonomous.md must invoke Skill(skill="ecl-code-review", ...); found skills: ${JSON.stringify(skillNames)}`);
    assert.ok(!skillNames.includes('ecl:code-review'),
      'autonomous.md must not use legacy colon form ecl:code-review (canonical is hyphen form)');
  });

  test('autonomous.md auto-fix uses consolidated ecl-code-review --fix invocation (#2790)', () => {
    // After #2790, ecl-code-review-fix was absorbed into ecl-code-review as
    // the --fix flag. The autonomous workflow must invoke the consolidated
    // form, not the deleted ecl-code-review-fix skill.
    const content = fs.readFileSync(path.join(WORKFLOWS_DIR, 'autonomous.md'), 'utf-8');

    const invocations = parseWorkflowSkillInvocations(content);
    const skillNames = invocations.map(inv => inv.skill);
    assert.ok(!skillNames.includes('ecl-code-review-fix'),
      `autonomous.md must not invoke deleted ecl-code-review-fix skill (consolidated into --fix); found: ${JSON.stringify(skillNames)}`);
    assert.ok(!skillNames.includes('ecl:code-review-fix'),
      'autonomous.md must not use legacy colon form ecl:code-review-fix');

    // Find a ecl-code-review invocation that carries the --fix flag (the
    // consolidated auto-fix entry point).
    const fixInvocation = invocations.find(inv => {
      if (inv.skill !== 'ecl-code-review') return false;
      const tokens = new Set((inv.args ?? '').split(/\s+/).filter(Boolean));
      return tokens.has('--fix');
    });
    assert.ok(fixInvocation,
      `autonomous.md must invoke Skill(skill="ecl-code-review", args="... --fix ...") for auto-fix; found: ${JSON.stringify(invocations)}`);
  });

  test('autonomous.md contains --auto flag on consolidated --fix invocation (#2790)', () => {
    const content = fs.readFileSync(path.join(WORKFLOWS_DIR, 'autonomous.md'), 'utf-8');

    // Find the ecl-code-review invocation that carries --fix (the consolidated
    ...
    assert.ok(fixInvocation, 'autonomous.md missing Skill(skill="ecl-code-review", args="... --fix ...") invocation');
    const argTokens = new Set((fixInvocation.args ?? '').split(/\s+/).filter(Boolean));
    assert.ok(argTokens.has('--auto'),
      `autonomous.md ecl-code-review-fix args missing --auto flag; got args="${fixInvocation.args}"`);
  });
});
