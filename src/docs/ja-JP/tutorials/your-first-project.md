...
このチュートリアルでは、eCL Core をインストールし、シンプルなコマンドライン To-Do アプリをゼロから作成します。1フェーズ、1プルリクエスト、完全なループを体験します。終わる頃には、コアフェーズループのすべてのコマンドを少なくとも一度は実行し、各コマンドが生成する計画アーティファクトを確認しているはずです。
...
他のツールは不要です。eCL Core 自体は次のステップでインストールします。
...
プロジェクト内に `.claude/` ディレクトリが作成されます。これが eCL Core のコマンドとエージェントの格納場所です。
...
eCL Core は、ファイルの読み書きを行うサブエージェントを生成します。すべてのファイル操作に対して確認を求められないよう、権限フラグを付けて Claude Code を起動してください:
...
eCL Core がいくつかの確認事項について質問します。自然に答えてください。1つのプランも書く前に、あなたの意図を理解しようとしています。

質問が終わると、ドメインリサーチの実行を提案します。この規模のプロジェクトであればリサーチをスキップできます。プロンプトが表示されたら **Skip research** を選択してください。

次に eCL Core がワークフロー設定（モード、粒度、リサーチエージェント）を選択するよう求めます。それぞれ推奨デフォルトを選択してください。これらの設定は `.planning/config.json` に書き込まれます。

最後に、ロードマッパーのサブエージェントが実行されます（「Spawning roadmapper…」という通知が表示されますが、これは正常です。約1分かかります）。完了すると、eCL Core が提案するロードマップを提示します。単一フェーズのプロジェクトでは次のようになります:
...
eCL Core はフレッシュなコンテキストを前提に設計されています。各フェーズの前にメインセッションウィンドウをクリアしてください:
...
eCL Core がフェーズのゴールを読み取り、実装の方針について質問します。これは「何を」作るかではなく、「どのように」作るかを決める質問です。会話の例:
...
`01-01-PLAN.md` を開いてください。タスク名、対象ファイル、アクションステップ、検証コマンド、完了条件が含まれた `<task>` ブロックがあります。`<verify>` タグに注目してください。eCL Core のエグゼキューターはコードを書いた後にそのコマンドを実行します。
...
eCL Core はプランをウェーブ（独立したプランが並行実行される単位）にグループ化し、プランごとに新しい 200k コンテキストのエグゼキューターを生成し、各タスクをアトミックにコミットします。
...
アイテムが表示され、完了マークを付けた後にアイテム 1 がデフォルトリストから消えているはずです。これが eCL Core によって実現された最初の成果です。
...
eCL Core がフェーズの成功基準を抽出し、それぞれについて確認します:
...
いずれかのチェックが失敗した場合、eCL Core が根本原因を診断して修正プランを作成します。`/ecl-execute-phase 1` を再度実行して修正を適用し、その後 `/ecl-verify-work 1` を再実行してください。
...
eCL Core が自動生成された本文付きのプルリクエストを作成します。PR の本文には常に Summary、Changes、Requirements Addressed、Verification、Key Decisions が含まれます。
...
- `npx @evolvconsulting/evolv-coder-lite@latest` を使った eCL Core のインストール方法。
- `/ecl-new-project` が会話を `.planning/` アーティファクトに裏付けられたロードマップに変換する仕組み。
- `/ecl-discuss-phase` が計画前に実装の意思決定を記録する仕組み。
- `/ecl-plan-phase` が並行リサーチャーを生成してアトミックなタスクプランを作成する仕組み。
- `/ecl-execute-phase` がプランを並行ウェーブで実行し各タスクをコミットする仕組み。
- `/ecl-verify-work` が成功基準を確認し、必要に応じて修正プランを生成する仕組み。
- `/ecl-ship` が検証済みフェーズをプルリクエストに変換する仕組み。

マルチフェーズプロジェクトの場合は、各フェーズでステップ 4〜8 を繰り返し、`/ecl-progress --next` を実行して eCL Core に次のステップを自動検出させてください。
...
- [既存コードベースのオンボーディング](onboarding-an-existing-codebase.md) — ブラウンフィールドリポジトリへの eCL Core の導入
