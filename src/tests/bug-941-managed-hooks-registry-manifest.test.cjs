 ...
 *
 * `managed-hooks-registry.cjs` is shipped alongside ecl-check-update-worker.js
...
function runInstaller(configDir) {
  // Clear ECL_TEST_MODE so the installer's main() block actually runs.
  // The test file sets ECL_TEST_MODE=1 (top of file) to suppress in-process
...
describe('bug #941 — managed-hooks-registry.cjs recorded in file manifest', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('ecl-bug-941-');
  });
  ...
  test('managed-hooks-registry.cjs appears in ecl-file-manifest.json after install', () => {
    runInstaller(tmpDir);
  ...
  test('ecl-file-manifest.json covers the full HOOKS_TO_COPY set (forward-proof)', () => {
    runInstaller(tmpDir);
