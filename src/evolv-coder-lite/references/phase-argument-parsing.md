...
```bash
PHASE_INFO=$(ecl-tools query find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(ecl-tools query roadmap.get-phase "${PHASE}" --pick found)
...
```bash
PHASE_DIR=$(ecl-tools query find-phase "${PHASE}" --raw)
```
