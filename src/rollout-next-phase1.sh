...
#
# Usage:
#   cd /Volumes/Mini\ Me/Users/trekkie/projects/evolv-coder-lite
#   bash /path/to/rollout-next-phase1.sh
#
# Env overrides:
#   TARBALL=/path/to/next-branch-files.tar.gz   (default: ./next-branch-files.tar.gz)
#   REPO=evolvconsulting/evolv-coder-lite           (default: that)
...
# Config
# ───────────────────────────────────────────────────────────
REPO="${REPO:-evolvconsulting/evolv-coder-lite}"
...
[ -d .git ] || die "Not in a git repo. cd to your evolv-coder-lite checkout first."
...
if grep -q "^## Where Do I Open My PR?" "$CONTRIB"; then
  ...
  SECTION_FILE=$(mktemp -t ecl-rollout-section.XXXXXX)
