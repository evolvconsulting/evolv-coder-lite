...
`STATE.md` は eCL Core のプロジェクト記憶ファイルです — プロジェクトの現在地、直近の作業内容、次に実行すべきコマンドを記録する単一の Markdown ドキュメントです。このページではそのスキーマを説明します。[ドキュメントインデックス](../../README.md) も参照してください。

---

## 概要

eCL Core で管理するプロジェクトはすべて `.planning/STATE.md` に `STATE.md` を1つ保持します。このファイルはすべてのワークフロー開始時に読み込まれ、重要なアクションのたびに書き込まれます。ファイルは以下を組み合わせた構成です：

- **YAML フロントマター** — ステータスラインフック（`parseStateMd`）および `ecl-tools state` コマンドが読み取る機械可読フィールド。
...
フロントマターはファイルの先頭にある `---` デリミタの間に記述します。`ecl_state_version` と `status` 以外のフィールドはすべてオプションで、データが未取得の場合は省略できます。
...
| フィールド | 型 | 設定タイミング | 用途 |
|---|---|---|---|
| `ecl_state_version` | string (`'1.0'`) | 常時 | スキーマバージョン。`syncStateFrontmatter` が最初の `state.*` 呼び出し時に書き込む。 |
...
`evolv-coder-lite/bin/lib/state-document.cjs` の `normalizeStateStatus()` が本文の生テキストを以下の正規値にマッピングします：
...
`hooks/ecl-statusline.js` の `formatGsdState()` がパース済みフロントマターを読み取り、**最初にマッチしたシーン** を出力します。新しいライフサイクルフィールドが適用されない場合は、v1.38.x から一切変更なくオリジナルのフォーマットにフォールスルーします。
...
本文（末尾の `---` 以降のすべて）は `evolv-coder-lite/templates/state.md` のテンプレートに従います。標準セクションは以下の通りです：
...
このセクションの `Status:` および `Last activity:` フィールドは、既存の値が既知のテンプレートデフォルト値の場合に eCL ハンドラーによって更新されます（クヌース不変式: エグゼキューター作成値は保存されます）。既知のハンドラーデフォルト値の完全なリストは `evolv-coder-lite/bin/lib/state-document.cjs` の `KNOWN_TEMPLATE_DEFAULTS` に記載されています。
...
**Decisions** — 現在の作業に影響する最近の意思決定のサマリー（完全なログは `PROJECT.md` に保存）。`ecl-tools state add-decision` で追加。

**Pending Todos** — 件数と `.planning/todos/pending/` への参照。`/ecl-capture` で取得。

**Blockers/Concerns** — 今後の作業に影響する課題。起点フェーズのプレフィックス付き。`ecl-tools state add-blocker` で追加し、`ecl-tools state resolve-blocker` で解決。
