...
if [ -f "$ECL_TOOLS" ]; then
  ...
  echo "Run: npx @evolvconsulting/evolv-coder-lite@latest --claude --local" >&2
  exit 1
fi
INIT=$($ECL_SDK query init.quick "$DESCRIPTION")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_PLANNER=$($ECL_SDK query agent-skills ecl-planner)
AGENT_SKILLS_EXECUTOR=$($ECL_SDK query agent-skills ecl-executor)
AGENT_SKILLS_CHECKER=$($ECL_SDK query agent-skills ecl-plan-checker)
AGENT_SKILLS_VERIFIER=$($ECL_SDK query agent-skills ecl-verifier)
```
...
if [ "$USE_WORKTREES" != "false" ]; then
  $ECL_SDK query worktree.reap-orphans 2>/dev/null || true
fi
```
...
```bash
if [ "${USE_WORKTREES}" != "false" ]; then
  COMMIT_DOCS=$($ECL_SDK query config-get commit_docs 2>/dev/null || echo "true")
...
1. **Worktree cleanup:** If the executor ran with `isolation="worktree"`, merge the worktree branch back and clean up:
   ...
   $ECL_SDK query worktree.cleanup-wave --manifest "$QUICK_WORKTREE_MANIFEST" || exit 1
   ```
...
Stage and commit quick task artifacts. This step MUST always run — even if the executor already committed some files (e.g. when running without worktree isolation). The `ecl-sdk query commit` command (or legacy `ecl-tools.cjs` commit) handles already-committed files gracefully.
...
$ECL_SDK query commit "docs(quick-${quick_id}): ${DESCRIPTION}" --files ${file_list}
```
