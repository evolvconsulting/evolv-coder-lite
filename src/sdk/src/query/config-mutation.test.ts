...
beforeEach(async () => {
  tmpDir = await mkdtemp(join(tmpdir(), 'ecl-cfgmut-'));
...
describe('isValidConfigKey', () => {
  ...
  it('accepts wildcard agent_skills.* patterns', async () => {
    const { isValidConfigKey } = await import('./config-mutation.js');
    expect(isValidConfigKey('agent_skills.ecl-planner').valid).toBe(true);
...
describe('configNewProject nested globalDefaults merging (fix #2673)', () => {
  ...
  beforeEach(async () => {
    fakeHome = await mkdtemp(join(tmpdir(), 'ecl-fakehome-'));
  ...
  it('preserves nested workflow keys from globalDefaults', async () => {
    await writeFile(
      join(fakeHome, '.ecl', 'defaults.json'),
      JSON.stringify({
    ...
    expect(raw.git.phase_branch_template).toBe('ecl/phase-{phase}-{slug}');
  });
