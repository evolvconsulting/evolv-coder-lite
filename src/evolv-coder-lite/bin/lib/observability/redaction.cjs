 ...
 * and file audit). Opt-in: set ECL_AUDIT_ARGS=1 to include args verbatim.
 *
 ...
 * Only ECL_AUDIT_ARGS === '1' enables inclusion; any other value (including
 * empty string, 'true', 'yes') keeps the default of omitting args.
 *
 * @returns {boolean}
 */
function shouldIncludeArgs() {
  return process.env.ECL_AUDIT_ARGS === '1';
}
 ...
 * If args should be included (ECL_AUDIT_ARGS=1), passes the event through
