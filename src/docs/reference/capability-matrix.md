...
| Column | Description |
|---|---|
| **id** | Canonical capability identifier; must be unique across first- and third-party capabilities. Reserved prefixes: `ecl-`, `evolv-coder-lite-`, `anthropic-`. |
| **role** | `feature` — extends what the loop does; `runtime` — adapts eCL to a specific AI runtime/IDE. |
...
| **engines.ecl** | Semver range expressing host-version compatibility. A hard gate at install and at load. |
...
| **source** | `first-party` — ships with eCL Core; `third-party` — installed from an external source via `ecl capability install`. |

---

## Native (first-party) capabilities

First-party capabilities are implicitly trusted: they ship as part of the eCL
...
| id | role | tier | version | engines.ecl | extension points | hook kinds | source |
|---|---|---|---|---|---|---|---|
...
Runtime capabilities adapt eCL to a specific AI runtime or IDE — emitting
...
Once a user installs a third-party capability via `ecl capability install
...
install root (`~/.ecl/capabilities/<id>/` for global installs;
`.ecl/capabilities/<id>/` for project-scoped installs), validates it against
...
| Column | Value |
|---|---|
| **id** | As declared in `capability.json`. Must not use reserved prefixes (`ecl-`, `evolv-coder-lite-`, `anthropic-`). |
...
| **engines.ecl** | Range from `capability.json`; verified at install and at each load. |
...
Whether eCL operates or advertises a central community registry of third-party
...
| `engines.ecl` | Recommended | semver range | Host-version compatibility gate. Enforced at install and load. |
| `compatVersions` | No | object: cap-version → min-ecl-version | Graceful downgrade table for sources that enumerate versions (git tags, registry, npm). |
