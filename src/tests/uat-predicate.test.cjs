...
const {
  ...
  analyzeMarkdown,
  evaluateUatPassed,
} = require('../evolv-coder-lite/bin/lib/uat-predicate.cjs');
const { cleanup } = require('./helpers.cjs');

// ─── Helpers ──────────────────────────────────────────────────────────────────

function makeTmpDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-uat-pred-test-'));
}
  ...
  test('fc: inserting result:passed inside wrapper context never flips a failing UAT to passed', () => {
    ...
    fc.assert(
      fc.property(wrappers, fc.nat(3), (wrap, extraCount) => {
        ...
        const fcDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-fc-uat-'));
        try {
