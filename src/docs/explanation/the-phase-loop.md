...
> The central mental model for how eCL Core organises work.

---

## What the loop is

eCL Core structures all development work as a repeating cycle:
...
For work that falls below the threshold where the loop is warranted, eCL Core provides lighter primitives. The phase loop is one tool, not the only tool.
