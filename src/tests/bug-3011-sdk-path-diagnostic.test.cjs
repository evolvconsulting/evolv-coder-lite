 ...
 *
 * Reporter (Windows / PowerShell 7) ran `npx @evolvconsulting/evolv-coder-lite@latest`,
 * upgrade reported success, but `ecl-sdk` could not be resolved by Claude
...
describe('Bug #3011: formatSdkPathDiagnostic — POSIX action lines', () => {
  test('emits a single export PATH line on Linux', () => {
    const ir = formatSdkPathDiagnostic({
      ...
      platform: 'linux',
      runDir: '/home/me/.local/lib/node_modules/evolv-coder-lite/bin',
    });
  ...
  test('emits a single export PATH line on macOS (darwin)', () => {
    const ir = formatSdkPathDiagnostic({
      ...
      platform: 'darwin',
      runDir: '/usr/local/lib/node_modules/evolv-coder-lite/bin',
    });
...
describe('Bug #3011: formatSdkPathDiagnostic — fallback when shimDir is null', () => {
  ...
  test('action lines fall back to npm install -g advice', () => {
    ...
    assert.ok(ir.actionLines.some(l => l.includes('npm install -g @evolvconsulting/evolv-coder-lite')));
  });
});
...
describe('Bug #3011: formatSdkPathDiagnostic — npx-cache detection', () => {
  test('detects POSIX npx cache path', () => {
    const ir = formatSdkPathDiagnostic({
      shimDir: '/home/me/.local/bin',
      platform: 'linux',
      runDir: '/home/me/.npm/_npx/abc123/node_modules/evolv-coder-lite/bin',
    });
