 ...
 *   B. check verify.schema-drift — no-schema/block/ECL_SKIP_SCHEMA_CHECK bypass
...
/**
 * Run ecl-tools and return { status, stdout, stderr, parsed? }.
...
function runTool(args, { cwd, env = {} } = {}) {
  ...
  const r = spawnSync(process.execPath, [ECL_TOOLS, ...args], {
    cwd: cwd || os.tmpdir(),
    encoding: 'utf8',
    env: childEnv,
    timeout: 60000,
...
const tmpDirs = [];
function makeTmpDir() {
  const d = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-wave-post-'));
  tmpDirs.push(d);
  return d;
}
...
describe('B. check verify.schema-drift — CLI route', () => {
  ...
  test('[negative] ECL_SKIP_SCHEMA_CHECK=true → block:false, skipped:true even with schema drift', () => {
    const dir = buildSchemaDriftFixture({ hasSchemaFile: true });

    const r = runTool(['check', 'verify.schema-drift', '1', '--raw'], {
      cwd: dir,
      env: { ECL_SKIP_SCHEMA_CHECK: 'true' },
    });
    assert.strictEqual(r.status, 0, `exit non-zero: ${r.stderr}`);
    assert.ok(r.parsed, `stdout not JSON: ${r.stdout}`);
    const result = r.parsed;
    ...
    assert.strictEqual(result.block, false,
      `block must be false with ECL_SKIP_SCHEMA_CHECK=true; got ${result.block}`);
