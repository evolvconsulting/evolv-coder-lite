...
だからeCLを作りました。複雑さはシステムの中にあり、ワークフローの中にはありません。裏側では、コンテキストエンジニアリング、XMLプロンプトフォーマッティング、サブエージェントのオーケストレーション、状態管理が動いています。あなたが目にするのは、ただ動くいくつかのコマンドだけです。

このシステムは、Claudeが仕事をし、*かつ*検証するために必要なすべてを提供します。私はこのワークフローを信頼しています。ちゃんといい仕事をしてくれます。

これがeCLです。エンタープライズごっこは一切なし。Claude Codeを使って一貫してクールなものを作るための、非常に効果的なシステムです。
...
eCLはそれを解決します。Claude Codeを信頼性の高いものにするコンテキストエンジニアリングレイヤーです。アイデアを説明し、システムに必要なすべてを抽出させ、Claude Codeに仕事をさせましょう。
...
完全なリストは [v1.39.0 リリースノート](https://github.com/evolvconsulting/evolv-coder-lite/releases/tag/v1.39.0) を参照してください。

- **`--minimal` インストールプロファイル** — エイリアス `--core-only`。メインループの6スキル（`new-project`、`discuss-phase`、`plan-phase`、`execute-phase`、`help`、`update`）のみをインストールし、`ecl-*` サブエージェントはゼロ。コールドスタート時のシステムプロンプトのオーバーヘッドを ~12kトークンから ~700トークンへ削減（≥94%減）。32K〜128Kコンテキストのローカル LLM やトークン課金 API に有効。
- **`/ecl-phase --edit`** — `ROADMAP.md` 上の既存フェーズの任意フィールドをその場で編集（番号や位置は変更されない）。`--force` で確認 diff をスキップ、`depends_on` の参照を検証し、書き込み時に `STATE.md` も更新。
...
- **ワークストリーム設定の継承** — `ECL_WORKSTREAM` が設定されている場合、ルートの `.planning/config.json` を先に読み込み、ワークストリーム設定をディープマージ（衝突時はワークストリーム側が優先）。ワークストリーム設定で明示的に `null` を指定するとルート値を上書き可能。
- **手動カナリアリリースワークフロー** — `.github/workflows/canary.yml` が `workflow_dispatch` 経由で `dev` ブランチから `{base}-canary.{N}` ビルドを `@canary` dist-tag に手動公開（`@evolvconsulting/evolv-coder-lite` と `@evolvconsulting/ecl-sdk`）。
...
確認方法：
- Claude Code / Gemini / Copilot / Antigravity: `/ecl-help`
- OpenCode / Kilo / Augment / Trae: `/ecl-help`
- Codex: `$ecl-help`
- Cline: eCLは`.clinerules`経由でインストール — `.clinerules`の存在を確認

> [!NOTE]
> Claude Code 2.1.88+とCodexはスキル（`skills/ecl-*/SKILL.md`）としてインストールされます。Clineは`.clinerules`を使用します。インストーラーがすべての形式を自動的に処理します。
...
eCLは急速に進化しています。定期的にアップデートしてください：
...
```bash
# Claude Code
npx @evolvconsulting/evolv-coder-lite --claude --global   # ~/.claude/ にインストール
npx @evolvconsulting/evolv-coder-lite --claude --local    # ./.claude/ にインストール

# OpenCode
npx @evolvconsulting/evolv-coder-lite --opencode --global # ~/.config/opencode/ にインストール

# Gemini CLI
npx @evolvconsulting/evolv-coder-lite --gemini --global   # ~/.gemini/ にインストール

# Kilo
npx @evolvconsulting/evolv-coder-lite --kilo --global     # ~/.config/kilo/ にインストール
npx @evolvconsulting/evolv-coder-lite --kilo --local      # ./.kilo/ にインストール

# Codex
npx @evolvconsulting/evolv-coder-lite --codex --global    # ~/.codex/ にインストール
npx @evolvconsulting/evolv-coder-lite --codex --local     # ./.codex/ にインストール

# Copilot
npx @evolvconsulting/evolv-coder-lite --copilot --global  # ~/.github/ にインストール
npx @evolvconsulting/evolv-coder-lite --copilot --local   # ./.github/ にインストール

# Cursor CLI
npx @evolvconsulting/evolv-coder-lite --cursor --global      # ~/.cursor/ にインストール
npx @evolvconsulting/evolv-coder-lite --cursor --local       # ./.cursor/ にインストール

# Antigravity
npx @evolvconsulting/evolv-coder-lite --antigravity --global # ~/.gemini/antigravity/ にインストール
npx @evolvconsulting/evolv-coder-lite --antigravity --local  # ./.agent/ にインストール

# Augment
npx @evolvconsulting/evolv-coder-lite --augment --global     # ~/.augment/ にインストール
npx @evolvconsulting/evolv-coder-lite --augment --local      # ./.augment/ にインストール

# Trae
npx @evolvconsulting/evolv-coder-lite --trae --global        # ~/.trae/ にインストール
npx @evolvconsulting/evolv-coder-lite --trae --local         # ./.trae/ にインストール

# Cline
npx @evolvconsulting/evolv-coder-lite --cline --global       # ~/.cline/ にインストール
npx @evolvconsulting/evolv-coder-lite --cline --local        # ./.clinerules にインストール

# 全ランタイム
npx @evolvconsulting/evolv-coder-lite --all --global      # すべてのディレクトリにインストール
```
...
eCLは摩擦のない自動化のために設計されています。Claude Codeを以下のように実行してください：
...
> [!TIP]
> これがeCLの意図された使い方です — `date` や `git commit` を50回も承認するために止まっていては目的が台無しです。
...
> **既存のコードがある場合は？** まず `/ecl-map-codebase` を実行してください。並列エージェントが起動し、スタック、アーキテクチャ、規約、懸念点を分析します。その後 `/ecl-new-project` がコードベースを把握した状態で動作し、質問は追加する内容に焦点を当て、計画時にはパターンが自動的に読み込まれます。
...
> **前提モード：** 質問よりもコードベース分析を優先したい場合は、`/ecl-settings` で `workflow.discuss_mode` を `assumptions` に設定してください。システムがコードを読み、何をなぜそうするかを提示し、間違っている部分だけ修正を求めます。詳しくは[ディスカスモード](docs/ja-JP/workflow-discuss-mode.md)をご覧ください。
...
すべてパスすれば次に進みます。何か壊れていれば、手動でデバッグする必要はありません — 作成された修正プランで `/ecl-execute-phase` を再度実行するだけです。
...
/ecl-execute-phase 2
/ecl-verify-work 2
/ecl-ship 2                  # 検証済みの作業からPRを作成
...
/ecl-complete-milestone
/ecl-new-milestone
```

またはeCLに次のステップを自動判定させます：
...
ディスカッション中のインプットを速くしたい場合は、`/ecl-discuss-phase <n> --batch` で1つずつではなく小さなグループにまとめた質問に一括で回答できます。`--chain` を使うと、ディスカッションからプラン+実行まで途中で止まらずに自動チェインできます。

各フェーズであなたのインプット（discuss）、適切なリサーチ（plan）、クリーンな実行（execute）、人間による検証（verify）が行われます。コンテキストは常にフレッシュ。品質は常に高い。

すべてのフェーズが完了したら、`/ecl-complete-milestone` でマイルストーンをアーカイブしリリースをタグ付けします。

次に `/ecl-new-milestone` で次のバージョンを開始します — `new-project` と同じフローですが既存のコードベース向けです。次に構築したいものを説明し、システムがドメインを調査し、要件をスコーピングし、新しいロードマップを作成します。各マイルストーンはクリーンなサイクルです：定義 → 構築 → シップ。
...
クイックモードはeCLの保証（アトミックコミット、状態トラッキング）をより速いパスで提供します：
...
**`--full` フラグ：** 全フェーズを有効化 — ディスカッション + リサーチ + プランチェック + 検証。クイックタスク形式のフルeCLパイプライン。
...
| コマンド | 説明 |
|---------|--------------|
| `/ecl-new-project [--auto]` | フル初期化：質問 → リサーチ → 要件定義 → ロードマップ |
| `/ecl-discuss-phase [N] [--auto] [--analyze] [--chain]` | 計画前に実装の決定事項をキャプチャ（`--analyze` でトレードオフ分析を追加、`--chain` でプラン+実行へ自動チェイン） |
| `/ecl-plan-phase [N] [--auto] [--reviews]` | フェーズのリサーチ + プラン + 検証（`--reviews` でコードベースレビューの発見事項を読み込み） |
| `/ecl-execute-phase <N>` | 全プランを並列ウェーブで実行し、完了時に検証 |
| `/ecl-verify-work [N]` | 手動ユーザー受入テスト ¹ |
| `/ecl-ship [N] [--draft]` | 検証済みのフェーズ作業から自動生成された本文付きのPRを作成 |
| `/ecl-progress --next` | 次の論理的なワークフローステップに自動的に進む |
| `/ecl-fast <text>` | インラインの軽微タスク — 計画を完全にスキップし即座に実行 |
| `/ecl-audit-milestone` | マイルストーンが完了の定義を達成したか検証 |
| `/ecl-complete-milestone` | マイルストーンをアーカイブし、リリースをタグ付け |
| `/ecl-new-milestone [name]` | 次のバージョンを開始：質問 → リサーチ → 要件定義 → ロードマップ |
| `/ecl-forensics [desc]` | 失敗したワークフロー実行の事後分析（停止ループ、欠落成果物、git異常の診断） |
| `/ecl-milestone-summary [version]` | チームオンボーディングとレビュー向けの包括的なプロジェクトサマリーを生成 |

### ワークストリーム

| コマンド | 説明 |
|---------|--------------|
| `/ecl-workstreams list` | 全ワークストリームとそのステータスを表示 |
| `/ecl-workstreams create <name>` | 並列マイルストーン作業用の名前空間付きワークストリームを作成 |
| `/ecl-workstreams switch <name>` | アクティブなワークストリームを切り替え |
| `/ecl-workstreams complete <name>` | ワークストリームを完了しマージ |

### マルチプロジェクトワークスペース

| コマンド | 説明 |
|---------|--------------|
| `/ecl-workspace --new` | リポジトリのコピー（worktreeまたはクローン）で隔離されたワークスペースを作成 |
| `/ecl-workspace --list` | すべてのeCLワークスペースとそのステータスを表示 |
| `/ecl-workspace --remove` | ワークスペースを削除しworktreeをクリーンアップ |

### UIデザイン

| コマンド | 説明 |
|---------|--------------|
| `/ecl-ui-phase [N]` | フロントエンドフェーズ用のUIデザイン契約（UI-SPEC.md）を生成 |
| `/ecl-ui-review [N]` | 実装済みフロントエンドコードの6つの柱によるビジュアル監査（遡及的） |
...
| `/ecl-progress --next` | 状態を自動検出し次のステップを実行 |
| `/ecl-help` | 全コマンドと使い方ガイドを表示 |
| `/ecl-update` | チェンジログプレビュー付きでeCLをアップデート |
| `/ecl-manager` | 複数フェーズ管理用のインタラクティブコマンドセンター |

### ブラウンフィールド

| コマンド | 説明 |
|---------|--------------|
| `/ecl-map-codebase [area]` | new-project前に既存のコードベースを分析 |

### フェーズ管理

| コマンド | 説明 |
|---------|--------------|
| `/ecl-phase` | ロードマップにフェーズを追加 |
| `/ecl-phase --insert [N]` | フェーズ間に緊急作業を挿入 |
| `/ecl-phase --edit [N] [--force]` | 既存フェーズの任意フィールドをその場で編集 — 番号と位置は変更されない |
| `/ecl-phase --remove [N]` | 将来のフェーズを削除し番号を振り直し |
| `/ecl-discuss-phase --assumptions [N]` | 計画前にClaudeの意図するアプローチを確認 |
| `/ecl-audit-milestone --fix` | 監査で見つかったギャップを埋めるフェーズを作成 |

### セッション

| コマンド | 説明 |
|---------|--------------|
| `/ecl-pause-work` | フェーズ途中で停止する際の引き継ぎを作成（HANDOFF.jsonを書き込み） |
| `/ecl-resume-work` | 前回のセッションから復元 |
| `/ecl-pause-work --report` | 実行した作業と結果のセッションサマリーを生成 |

### ワークストリーム

| コマンド | 説明 |
|---------|--------------|
| `/ecl-workstreams` | 並列ワークストリームを管理（list、create、switch、status、progress、complete） |

### コード品質

| コマンド | 説明 |
|---------|--------------|
| `/ecl-review` | 現在のフェーズまたはブランチのクロスAIピアレビュー |
| `/ecl-pr-branch` | `.planning/` コミットをフィルタリングしたクリーンなPRブランチを作成 |
| `/ecl-audit-uat` | 検証負債を監査 — UATが未実施のフェーズを検出 |

### バックログ & スレッド

| コマンド | 説明 |
|---------|--------------|
| `/ecl-capture --seed <idea>` | トリガー条件付きの将来志向のアイデアをキャプチャ — 適切なマイルストーンで浮上 |
| `/ecl-capture --backlog <desc>` | バックログのパーキングロットにアイデアを追加（999.xナンバリング、アクティブシーケンス外） |
| `/ecl-review-backlog` | バックログ項目をレビューし、アクティブマイルストーンに昇格またはstaleエントリを削除 |
| `/ecl-thread [name]` | 永続コンテキストスレッド — 複数セッションにまたがる作業用の軽量クロスセッション知識 |

### ユーティリティ

| コマンド | 説明 |
|---------|--------------|
| `/ecl-settings` | モデルプロファイルとワークフローエージェントを設定 |
| `/ecl-config --profile <profile>` | モデルプロファイルを切り替え（quality/balanced/budget/inherit） |
| `/ecl-capture [desc]` | 後で取り組むアイデアをキャプチャ |
| `/ecl-capture --list` | 保留中のtodoを一覧表示 |
| `/ecl-debug [desc]` | 永続状態を持つ体系的デバッグ |
| `/ecl-do <text>` | フリーフォームテキストを適切なeCLコマンドに自動ルーティング |
| `/ecl-note <text>` | ゼロフリクションのアイデアキャプチャ — ノートの追加、一覧、todoへの昇格 |
| `/ecl-quick [--full] [--discuss] [--research]` | eCLの保証付きでアドホックタスクを実行（`--full` で全フェーズを有効化、`--discuss` で事前にコンテキストを収集、`--research` で計画前にアプローチを調査） |
| `/ecl-health [--repair]` | `.planning/` ディレクトリの整合性を検証、`--repair` で自動修復 |
| `/ecl-stats` | プロジェクト統計を表示 — フェーズ、プラン、要件、gitメトリクス |
| `/ecl-profile-user [--questionnaire] [--refresh]` | セッション分析から開発者行動プロファイルを生成し、パーソナライズされた応答を提供 |
...
eCLはプロジェクト設定を `.planning/config.json` に保存します。`/ecl-new-project` 実行時に設定するか、後から `/ecl-settings` で更新できます。完全な設定スキーマ、ワークフロートグル、gitブランチオプション、エージェントごとのモデル内訳については、[ユーザーガイド](docs/ja-JP/USER-GUIDE.md#configuration-reference)をご覧ください。
...
これらのトグルには `/ecl-settings` を使用するか、呼び出し時にオーバーライドできます：
- `/ecl-plan-phase --skip-research`
- `/ecl-plan-phase --skip-verify`
...
eCLが実行中にブランチをどう扱うかを制御します。

| 設定 | オプション | デフォルト | 説明 |
|---------|---------|---------|--------------|
| `git.branching_strategy` | `none`, `phase`, `milestone` | `none` | ブランチ作成戦略 |
| `git.phase_branch_template` | string | `ecl/phase-{phase}-{slug}` | フェーズブランチのテンプレート |
| `git.milestone_branch_template` | string | `ecl/{milestone}-{slug}` | マイルストーンブランチのテンプレート |

**戦略：**
- **`none`** — 現在のブランチにコミット（デフォルトのeCL動作）
- **`phase`** — フェーズごとにブランチを作成し、フェーズ完了時にマージ
- **`milestone`** — マイルストーン全体で1つのブランチを作成し、完了時にマージ

マイルストーン完了時、eCLはスカッシュマージ（推奨）または履歴付きマージを提案します。
...
- **PreToolUseプロンプトガードフック** — `ecl-prompt-guard` が `.planning/` への書き込みに埋め込まれたインジェクションベクトルをスキャン（アドバイザリー、ブロッキングではない）
...
> [!NOTE]
> eCLはLLMシステムプロンプトとなるマークダウンファイルを生成するため、計画成果物に流入するユーザー制御テキストは潜在的な間接プロンプトインジェクションベクトルとなります。これらの保護は、そのようなベクトルを複数のレイヤーで捕捉するように設計されています。

### 機密ファイルの保護

eCLのコードベースマッピングおよび分析コマンドは、プロジェクトを理解するためにファイルを読み取ります。**シークレットを含むファイルを保護する**には、Claude Codeの拒否リストに追加してください：
...
> [!IMPORTANT]
> eCLにはシークレットのコミットに対する組み込み保護がありますが、多層防御がベストプラクティスです。防御の第一線として、機密ファイルへの読み取りアクセスを拒否してください。
...
- `~/.claude/commands/ecl/`（グローバル）または `./.claude/commands/ecl/`（ローカル）にファイルが存在するか確認してください
- Codexの場合、`~/.codex/skills/ecl-*/SKILL.md`（グローバル）または `./.codex/skills/ecl-*/SKILL.md`（ローカル）にスキルが存在するか確認してください

**コマンドが期待通りに動作しない？**
- `/ecl-help` を実行してインストールを確認してください
- `npx @evolvconsulting/evolv-coder-lite` を再実行して再インストールしてください
...
```bash
# グローバルインストール
npx @evolvconsulting/evolv-coder-lite --claude --global --uninstall
npx @evolvconsulting/evolv-coder-lite --opencode --global --uninstall
npx @evolvconsulting/evolv-coder-lite --gemini --global --uninstall
npx @evolvconsulting/evolv-coder-lite --kilo --global --uninstall
npx @evolvconsulting/evolv-coder-lite --codex --global --uninstall
npx @evolvconsulting/evolv-coder-lite --copilot --global --uninstall
npx @evolvconsulting/evolv-coder-lite --cursor --global --uninstall
npx @evolvconsulting/evolv-coder-lite --antigravity --global --uninstall
npx @evolvconsulting/evolv-coder-lite --trae --global --uninstall

# ローカルインストール（現在のプロジェクト）
npx @evolvconsulting/evolv-coder-lite --claude --local --uninstall
npx @evolvconsulting/evolv-coder-lite --opencode --local --uninstall
npx @evolvconsulting/evolv-coder-lite --gemini --local --uninstall
npx @evolvconsulting/evolv-coder-lite --kilo --local --uninstall
npx @evolvconsulting/evolv-coder-lite --codex --local --uninstall
npx @evolvconsulting/evolv-coder-lite --copilot --local --uninstall
npx @evolvconsulting/evolv-coder-lite --cursor --local --uninstall
npx @evolvconsulting/evolv-coder-lite --antigravity --local --uninstall
npx @evolvconsulting/evolv-coder-lite --trae --local --uninstall
```

これにより、他の設定を保持しながら、すべてのeCLコマンド、エージェント、フック、設定が削除されます。

---

## コミュニティポート

OpenCode、Gemini CLI、Kilo、Codexは `npx @evolvconsulting/evolv-coder-lite` でネイティブサポートされています。

以下のコミュニティポートがマルチランタイムサポートの先駆けとなりました：

| プロジェクト | プラットフォーム | 説明 |
|---------|----------|-------------|
| [ecl-opencode](https://github.com/rokicool/ecl-opencode) | OpenCode | オリジナルのOpenCode対応版 |
| ecl-gemini（アーカイブ済み） | Gemini CLI | uberfuzzyによるオリジナルのGemini対応版 |
...
**Claude Codeは強力です。eCLはそれを信頼性の高いものにします。**
