...
const SCAN_DIRS = ['agents', 'commands', path.join('evolv-coder-lite', 'references')];

// Fix #3: broaden to catch backtick-delimited and split-quoted forms.
// Matches: node <optional-quote/backtick> ($HOME|${HOME}|~) <any non-newline chars> ecl-tools.cjs
// PREAMBLE_SKIP_RE is still applied before this to exclude preamble lines.
const HARDCODED_RE = /node\s+[`"']?(?:\$HOME|\$\{HOME\}|~)[^\n]*?ecl-tools\.cjs/;
const PREAMBLE_SKIP_RE = /_GSD_SHIM_NAME|ECL_TOOLS=|\[ -f/;
...
test('no hardcoded $HOME ecl-tools.cjs in bash blocks of agents/, commands/, and evolv-coder-lite/references/', () => {
...
test('each scan dir (agents/, commands/, evolv-coder-lite/references/) exists and contains at least one bash block', () => {
