...
/**
 * Tests for evolv-coder-lite/bin/lib/graphify.cjs
 *
...
const {
  ...
  graphifyBuild,
  writeSnapshot,
} = require('../evolv-coder-lite/bin/lib/graphify.cjs');
...
// containing a full-profile .ecl-surface.json with graphify surfaced, and
// clear ECL_RUNTIME / ECL_WORKSTREAM / ECL_PROJECT for hermeticity.
// An EMPTY tmp config dir (no .ecl-surface.json) also works — the resolver
...
function makeSurfacedConfigDir() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-graphify-surface-cfg-'));
  fs.writeFileSync(
...
function saveSurfacedEnv() {
  const saved = {
    ECL_RUNTIME: process.env.ECL_RUNTIME,
  ...
  return {
    restore() {
      if (saved.ECL_RUNTIME === undefined) delete process.env.ECL_RUNTIME;
      else process.env.ECL_RUNTIME = saved.ECL_RUNTIME;
      ...
      if (saved.ECL_WORKSTREAM === undefined) delete process.env.ECL_WORKSTREAM;
      else process.env.ECL_WORKSTREAM = saved.ECL_WORKSTREAM;
      if (saved.ECL_PROJECT === undefined) delete process.env.ECL_PROJECT;
      else process.env.ECL_PROJECT = saved.ECL_PROJECT;
    },
  };
}
...
const { isCapabilityActive } = require('../evolv-coder-lite/bin/lib/capability-state.cjs');
...
describe('status', () => {
  ...
  describe('tri-state graphify gate (isCapabilityActive cutover)', () => {
    ...
    //   but graphify NOT surfaced (disabled cluster in .ecl-surface.json).
    //
    ...
    //     .ecl-surface.json  → full profile, disabledClusters:["graphify"]
    //     (no .ecl-profile   → defaults to 'full' → installedSkills='*' → installed=true)
    //   tmpProjectDir/
    ...
    describe('hermetic: graphify installed + config-enabled but NOT surfaced → disabled', () => {
      ...
      beforeEach(() => {
        tmpConfigDir = createTempDir('ecl-graphify-surface-test-config-');
        tmpProjectDir = createTempDir('ecl-graphify-surface-test-project-');

        // Fixture: .ecl-surface.json — full profile with graphify cluster disabled.
        ...
        // No .ecl-profile written → readActiveProfile returns null → defaults to 'full'
        ...
        fs.writeFileSync(
          path.join(tmpConfigDir, '.ecl-surface.json'),
        ...
        // Clear ECL_WORKSTREAM/ECL_PROJECT — ambient values redirect planningDir()
      ...
      test('graphifyStatus is NOT disabled when installed + SURFACED + config-enabled=true (positive control)', () => {
        // Re-write .ecl-surface.json with graphify surfaced (disabledClusters empty).
  ...
  describe('disabledResponse', () => {
    test('returns disabled:true with enable instructions', () => {
      ...
      assert.strictEqual(result.disabled, true);
      assert.ok(result.message.includes('ecl-tools config-set graphify.enabled true'));
    });
  });
  ...
  describe('graphifyStatus', () => {
    ...
    beforeEach(() => {
      ...
      delete process.env.ECL_RUNTIME;          // use 'claude' default
