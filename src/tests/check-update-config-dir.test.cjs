...
/**
 * Regression test for #1860: detectConfigDir in ecl-check-update.js should
...
describe('detectConfigDir runtime behavior (#1860)', () => {
  ...
  beforeEach(() => {
    tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-test-home-'));
  });
  ...
  test('returns .claude config dir when both .claude and .config/opencode exist', () => {
    ...
    const openCodeVersionDir = path.join(tmpHome, '.config', 'opencode', 'evolv-coder-lite');
    ...
    const claudeVersionDir = path.join(tmpHome, '.claude', 'evolv-coder-lite');
