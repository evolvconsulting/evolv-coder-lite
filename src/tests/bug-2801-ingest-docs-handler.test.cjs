 ...
 *
 * `/ecl-ingest-docs` was broken because:
 * 1. `workflows/ingest-docs.md` called `ecl-sdk query init.ingest-docs` but the
 *    installed binary is `ecl-tools` (not `ecl-sdk`).
 * 2. `ecl-tools init` had no `ingest-docs` case in its dispatch switch.
 *
 * The fix:
 * - Added `case 'ingest-docs'` to the `init` switch in `ecl-tools.cjs`.
 * - Exported `cmdInitIngestDocs` from `init.cjs`.
 * - Updated `workflows/ingest-docs.md` to call `ecl-tools init ingest-docs`.
 *
...
const WORKFLOW_FILE = path.join(REPO_ROOT, 'evolv-coder-lite', 'workflows', 'ingest-docs.md');
  ...
  try {
    stdout = childProc.execFileSync(
      ...
      [TOOLS_PATH, ...args, '--cwd', projectDir],
      {
        ...
        env: { ...process.env, ECL_SESSION_KEY: '' },
      }
    );
  } catch (err) {
...
describe('bug-2801: ecl-tools init ingest-docs handler exists', () => {
  let tmpDir;

  beforeEach(() => {
    tmpDir = createTempProject('ecl-test-2801-');
  });
...
describe('bug-2801: ingest-docs.md workflow calls ecl-tools not ecl-sdk', () => {
  test('no bash code block in ingest-docs.md calls ecl-sdk', () => {
    ...
    // since ecl-sdk can appear in subshell expansions like $(ecl-sdk query ...).
    ...
    assert.deepStrictEqual(
      sdkCalls,
      [],
      `workflow bash blocks still reference ecl-sdk (should use ecl-tools): ${sdkCalls.join(', ')}`
    );
  });

  test('ingest-docs.md init step uses the ecl_run launcher (#637)', () => {
    const content = fs.readFileSync(WORKFLOW_FILE, 'utf-8');
    ...
    const bashLines = [...content.matchAll(codeBlockRe)]
      .flatMap((m) => m[1].split('\n'))
      .filter((l) => !/^\s*#/.test(l));
    // #637 routes ingest-docs through the resolved `ecl_run` launcher instead of
    // the hardcoded `node "$HOME/.../ecl-tools.cjs"` path (which misses global
    // installs). The legacy bare `ecl-tools` form remains the bug and is still
    // rejected by bug-2851's repo-wide guard.
    const initLine = bashLines.find((l) =>
      /\becl_run\s+init\s+ingest-docs\b/.test(l)
    );
    assert.ok(initLine, 'workflow must invoke init ingest-docs via the ecl_run launcher (#637)');
  });

  test('cmdInitIngestDocs is exported from init.cjs', () => {
    const init = require(path.join(REPO_ROOT, 'evolv-coder-lite', 'bin', 'lib', 'init.cjs'));
