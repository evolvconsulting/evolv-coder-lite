...
**Prerequisites:** A phase whose `/ecl-spec-phase` run has passed the ambiguity gate and the edge-completeness probe (Step 5.5). The prohibition probe (Step 5.6) then runs automatically over the same requirements and presents its findings — you do not invoke it separately.
...
Some candidates are **canon** security/compliance constraints a dedicated tool already owns (OWASP / prototype-pollution / path-traversal / injection → `/ecl-secure-phase` + eslint; GDPR retention / consent → `/ecl-secure-phase`). The probe does not surface these as findings to resolve — it emits a one-line breadcrumb and drops them, so the list you triage stays the ~2–3 **bespoke** prohibitions no other tool would catch. You do not act on a breadcrumb here; pick it up in `/ecl-secure-phase`.
...
When you next run `/ecl-plan-phase`, the planner reads the spec's `## Prohibitions` section and:
...
- [`/ecl-spec-phase`](../COMMANDS.md#ecl-spec-phase) — command reference and flags
