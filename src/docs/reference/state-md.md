...
`STATE.md` is eCL Core's living project-memory file — a single Markdown document that records where a project stands, what happened last, and what to run next. This page documents its structure. See [docs index](../README.md).

---

## Overview

Every project managed by eCL Core keeps one `STATE.md` at `.planning/STATE.md`. It is read at the start of every workflow and written after every significant action. The file combines:

- **YAML frontmatter** — machine-readable fields consumed by the status-line hook (`parseStateMd`) and the `ecl-tools state` commands.
...
Frontmatter appears between `---` delimiters at the very start of the file. All fields except `ecl_state_version` and `status` are optional; fields may be absent when their data is not yet available.
...
|---|---|---|---|
| `ecl_state_version` | string (`'1.0'`) | Always | Schema version; written on first `state.*` call by `syncStateFrontmatter`. |
...
`normalizeStateStatus()` in `evolv-coder-lite/bin/lib/state-document.cjs` maps raw body text to these canonical values:
...
`formatGsdState()` in `hooks/ecl-statusline.js` reads the parsed frontmatter and emits the **first matching scene**. If no new lifecycle fields apply, rendering falls through to the original format byte-for-byte unchanged from v1.38.x.
...
The body (everything after the closing `---`) follows the template in `evolv-coder-lite/templates/state.md`. The standard sections are:
...
The `Status:` and `Last activity:` fields in this section are updated by eCL handlers when the existing value is a known template default (Knuth invariant: executor-authored values are preserved). The full list of known handler defaults is in `KNOWN_TEMPLATE_DEFAULTS` inside `evolv-coder-lite/bin/lib/state-document.cjs`.
...
**Decisions** — a summary of recent decisions affecting current work (full log lives in `PROJECT.md`). Added via `ecl-tools state add-decision`.

**Pending Todos** — count and reference to `.planning/todos/pending/`. Captured via `/ecl-capture`.

**Blockers/Concerns** — issues affecting future work, prefixed with the originating phase. Added via `ecl-tools state add-blocker`; resolved via `ecl-tools state resolve-blocker`.
