 ...
 *   1. Third-person framing (no "You are a eCL X" opening in <role>)
...
const AUDIT_AGENTS = [
  'ecl-plan-checker.md',
  'ecl-code-reviewer.md',
  'ecl-security-auditor.md',
  'ecl-verifier.md',
  'ecl-eval-auditor.md',
  'ecl-nyquist-auditor.md',
  'ecl-ui-auditor.md',
  'ecl-integration-checker.md',
  'ecl-doc-verifier.md',
];
...
describe('enh-2427 — sycophancy hardening: audit-class agents', () => {
  ...
  for (const filename of AUDIT_AGENTS) {
    ...
    describe(label, () => {
      ...
      test('(1) third-person framing — <role> does not open with "You are a eCL"', () => {
        content = content || readAgent(AGENTS_DIR, filename);
        role = role || extractRole(content);
        const firstSentence = role.trim().slice(0, 80);
        assert.ok(
          !firstSentence.startsWith('You are a eCL'),
          `${filename}: <role> must not open with "You are a eCL" — use third-person submission framing. Got: "${firstSentence}"`
        );
      });
