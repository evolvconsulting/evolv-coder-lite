 ...
 *
 * Port of evolv-coder-lite/bin/lib/init.cjs (13 of 16 handlers).
 ...
 * in evolv-coder-lite/bin/lib/core.cjs — keep these two implementations behaviour-
 ...
 * the first-touch creation path used by /ecl-discuss-phase and /ecl-plan-phase
 ...
 * (`ECL_RUNTIME` → `config.runtime` → 'claude') and probes that runtime's
 * canonical `agents/` directory. `ECL_AGENTS_DIR` still short-circuits.
 *
...
export const initExecutePhase: QueryHandler = async (args, projectDir, workstream) => {
  ...
  // through to cmdInitExecutePhase. Without parsing here, `ecl-tools init
  ...
  const [executorModel, verifierModel] = configExists
    ? await Promise.all([
        getModelAlias('ecl-executor', projectDir),
        getModelAlias('ecl-verifier', projectDir),
      ])
    : ['', ''];
...
export const initPlanPhase: QueryHandler = async (args, projectDir, workstream) => {
  ...
  const [researcherModel, plannerModel, checkerModel] = configExists
    ? await Promise.all([
        getModelAlias('ecl-phase-researcher', projectDir),
        getModelAlias('ecl-planner', projectDir),
        getModelAlias('ecl-plan-checker', projectDir),
      ])
    : ['', '', ''];
...
export const initNewMilestone: QueryHandler = async (_args, projectDir) => {
  ...
  const [researcherModel, synthesizerModel, roadmapperModel] = await Promise.all([
    getModelAlias('ecl-project-researcher', projectDir),
    getModelAlias('ecl-research-synthesizer', projectDir),
    getModelAlias('ecl-roadmapper', projectDir),
  ]);
...
export const initMapCodebase: QueryHandler = async (_args, projectDir) => {
  ...
  const mapperModel = await getModelAlias('ecl-codebase-mapper', projectDir);
...
export const initNewWorkspace: QueryHandler = async (_args, projectDir) => {
  ...
  const defaultBase = join(home, 'ecl-workspaces');
