...
const firstTimeBaselineMigration = require('../evolv-coder-lite/bin/lib/installer-migrations/000-first-time-baseline.cjs');

function createTempInstall() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-installer-migrations-'));
}
...
function legacyCodexHook(configDir) {
  return {
    hooks: [
      {
        type: 'command',
        command: `node "${path.join(configDir, 'hooks', 'ecl-check-update.js')}"`,
      },
    ],
  };
}
...
test('records a first-time baseline while preserving user-owned artifacts', () => {
  ...
  try {
    writeFile(configDir, 'evolv-coder-lite/workflows/plan.md', 'managed workflow\n');
    writeFile(configDir, 'evolv-coder-lite/USER-PROFILE.md', 'user profile\n');
    writeManifest(configDir, {
      'evolv-coder-lite/workflows/plan.md': sha256('managed workflow\n'),
    });
    ...
    assert.equal(fs.readFileSync(path.join(configDir, 'evolv-coder-lite/workflows/plan.md'), 'utf8'), 'managed workflow\n');
    assert.equal(fs.readFileSync(path.join(configDir, 'evolv-coder-lite/USER-PROFILE.md'), 'utf8'), 'user profile\n');
      ...
      [
        {
          ...
          relPath: 'evolv-coder-lite/workflows/plan.md',
          classification: 'managed-pristine',
        },
        {
          type: 'baseline-preserve-user',
          relPath: 'evolv-coder-lite/USER-PROFILE.md',
...
test('blocks stale eCL-looking baseline artifacts for explicit user choice', () => {
  const configDir = createTempInstall();
  try {
    writeFile(configDir, 'hooks/ecl-retired-hook.js', 'old ecl hook\n');
    writeManifest(configDir, {});
    ...
    assert.equal(fs.readFileSync(path.join(configDir, 'hooks/ecl-retired-hook.js'), 'utf8'), 'old ecl hook\n');
    assert.deepEqual(
      ...
      [
        {
          type: 'prompt-user',
          relPath: 'hooks/ecl-retired-hook.js',
...
test('records known generated agent artifacts so profile cleanup can remove them', () => {
  const configDir = createTempInstall();
  try {
    writeFile(configDir, 'agents/ecl-executor.md', 'old generated agent\n');
    writeFile(configDir, 'agents/ecl-executor.toml', 'old generated agent config\n');
    writeFile(configDir, 'agents/ecl-local-experiment.md', 'user experiment\n');
    writeManifest(configDir, {});
    ...
    assert.deepEqual(
      ...
      [
        {
          type: 'record-baseline',
          relPath: 'agents/ecl-executor.md',
          classification: 'unknown',
        },
        {
          type: 'record-baseline',
          relPath: 'agents/ecl-executor.toml',
          classification: 'unknown',
        },
        {
          type: 'prompt-user',
          relPath: 'agents/ecl-local-experiment.md',
          classification: 'stale-ecl-looking',
        },
      ]
    );
    assert.deepEqual(result.blocked.map((action) => action.relPath), ['agents/ecl-local-experiment.md']);
    assert.equal(fs.readFileSync(path.join(configDir, 'agents/ecl-executor.md'), 'utf8'), 'old generated agent\n');
    assert.equal(fs.readFileSync(path.join(configDir, 'agents/ecl-executor.toml'), 'utf8'), 'old generated agent config\n');
    assert.equal(fs.readFileSync(path.join(configDir, 'agents/ecl-local-experiment.md'), 'utf8'), 'user experiment\n');
  } finally {
    cleanup(configDir);
  }
});
...
test('classifies large files without loading the whole file through readFileSync', (t) => {
  ...
  const relPath = 'skills/ecl-large/SKILL.md';
...
test('applies an unblocked plan with a journal and install-state update', () => {
  ...
  try {
    ...
    assert.match(
      ...
      /^ecl-migration-journal\/2026-05-11T00-00-01-000Z-[0-9a-f]+\.json$/
    );
...
test('stores modified-file backups under the unique migration run journal', (t) => {
  ...
  assert.match(backupRelPath, /^ecl-migration-journal\/2026-05-11T00-00-10-000Z-[0-9a-f]+-backups\/hooks\/old-hook\.js$/);
...
test('successful migration rollback removes run-scoped backup directories', (t) => {
  ...
  assert.equal(
    fs.readdirSync(path.join(configDir, 'ecl-migration-journal')).some((name) => name.includes('backups')),
    false
  );
});
...
test('refuses to run migrations while another installer owns the migration lock', (t) => {
  const configDir = createTempInstall();
  t.after(() => cleanup(configDir));
  fs.writeFileSync(path.join(configDir, 'ecl-install-migration.lock'), 'held by test\n', 'utf8');
...
test('reports lock release failures after migration work completes', (t) => {
  ...
  fs.unlinkSync = (targetPath) => {
    if (path.basename(String(targetPath)) === 'ecl-install-migration.lock') {
      ...
      appliedMigrations: [
        {
          ...
          journal: 'ecl-migration-journal/prior.json',
        },
      ],
    });
...
test('rolls back touched files and leaves state unchanged when apply fails', () => {
  ...
  try {
    ...
    assert.equal(
      fs.existsSync(path.join(configDir, 'ecl-migration-journal')) &&
        fs.readdirSync(path.join(configDir, 'ecl-migration-journal')).some((name) =>
  ...
  try {
    ...
    const plan = planInstallerMigrations({
      ...
      migrations: discoverInstallerMigrations({
        migrationsDir: path.join(__dirname, '..', 'evolv-coder-lite', 'bin', 'lib', 'installer-migrations'),
      }),
      scope: 'global',
      now: () => '2026-05-11T00:00:05.000Z',
    });
...
test('runs a Codex legacy hooks.json cleanup migration without removing user hooks', () => {
  ...
  try {
    writeFile(
      ...
      JSON.stringify({
        SessionStart: [
          ...
          userHook('node "/Users/example/bin/ecl-check-update.js"'),
        ],
      }, null, 2)
    );
    writeManifest(configDir, {});
    ...
    assert.deepEqual(commands, [
      ...
      'node "/Users/example/bin/ecl-check-update.js"',
    ]);
...
test('skips runtime-specific migration records for other runtimes', () => {
  ...
  try {
    ...
    const hooksJson = JSON.parse(fs.readFileSync(path.join(configDir, 'hooks.json'), 'utf8'));
    assert.equal(hooksJson.SessionStart[0].hooks[0].command, `node "${path.join(configDir, 'hooks', 'ecl-check-update.js')}"`);
