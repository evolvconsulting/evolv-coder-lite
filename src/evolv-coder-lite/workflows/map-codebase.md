...
post-execute codebase-drift gate in `/ecl:execute-phase` or by a user running
`/ecl:map-codebase --paths apps/accounting,packages/ui`), the workflow
operates in **incremental-remap mode**:

- Pass `--paths <p1>,<p2>,...` through to each spawned `ecl-codebase-mapper`
...
INIT=$($ECL_SDK query init.map-codebase)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_MAPPER=$($ECL_SDK query agent-skills ecl-codebase-mapper)
```
...
Spawn 4 parallel ecl-codebase-mapper agents.

Use Agent tool with `subagent_type="ecl-codebase-mapper"`, `model="{mapper_model}"`, and `run_in_background=true` for parallel execution.

**CRITICAL:** Use the dedicated `ecl-codebase-mapper` agent, NOT `Explore` or `browser_subagent`. The mapper agent writes documents directly.
...
Use the same document templates as the `ecl-codebase-mapper` agent. Include actual file paths formatted with backticks.
...
```bash
$ECL_SDK query commit "docs: map existing codebase" --files .planning/codebase/*.md
```
...
**Also available:**
- Re-run mapping: `/ecl:map-codebase`
...
- If Agent tool available: 4 parallel ecl-codebase-mapper agents spawned with run_in_background=true
- If Agent tool NOT available: 4 sequential mapping passes performed inline (never using browser_subagent)
- All 7 codebase documents exist
- No empty documents (each should have >20 lines)
- Clear completion summary with line counts
- User offered clear next steps in eCL style
</success_criteria>
