 ...
 * command-router family file and `ecl-tools.cjs` non-family dispatcher
 ...
 * filesystem path, NOT by the `@evolvconsulting/ecl-sdk` package name. The package name
 ...
 * (`node_modules/evolv-coder-lite/sdk/dist/...`) because the `files` array in
 * `package.json` keeps `sdk/dist` at the same path inside the published
 * tarball.
 *
 * The previous implementation used `require('@evolvconsulting/ecl-sdk')`, which always
...
//   <root>/evolv-coder-lite/bin/lib/cjs-sdk-bridge.cjs
