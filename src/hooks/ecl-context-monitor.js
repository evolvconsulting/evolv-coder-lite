...
process.stdin.on('end', () => {
  ...
  try {
    ...
    // Quick sentinel check: skip config read entirely for non-eCL projects (#P2.5).
    ...
    // Detect if eCL is active (has .planning/STATE.md in working directory)
    const isGsdActive = fs.existsSync(path.join(cwd, '.planning', 'STATE.md'));

    // On CRITICAL with active eCL project, auto-record session state as a
    // breadcrumb for /ecl:resume-work (#1974). Fire-and-forget subprocess —
    ...
    if (isCritical && isGsdActive && !warnData.criticalRecorded) {
      try {
        ...
        // and ecl-tools.cjs lives at <runtime-config>/evolv-coder-lite/bin/.
        // Using __dirname makes this work on Claude Code, OpenCode, Gemini,
        // Kilo, etc. without hardcoding ~/.claude/.
        const gsdTools = path.join(__dirname, '..', 'evolv-coder-lite', 'bin', 'ecl-tools.cjs');
    ...
    if (isCritical) {
      message = isGsdActive
        ? `CONTEXT CRITICAL: Usage at ${usedPct}%. Remaining: ${remaining}%. ` +
          ...
          'eCL state is already tracked in STATE.md. Inform the user so they can run ' +
          '/ecl:pause-work at the next natural stopping point.'
        : `CONTEXT CRITICAL: Usage at ${usedPct}%. Remaining: ${remaining}%. ` +
