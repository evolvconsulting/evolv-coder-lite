...
describe('planningPaths', () => {
  ...
  it('uses ECL_PROJECT env when no explicit workstream is provided', () => {
    process.env['ECL_PROJECT'] = 'proj-scope';
    const paths = planningPaths('/proj');
    expect(paths.planning).toContain('/proj/.planning/proj-scope');
  });

  it('explicit workstream overrides ECL_PROJECT env', () => {
    process.env['ECL_PROJECT'] = 'proj-scope';
...
describe('resolvePathUnderProject', () => {
  ...
  beforeEach(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), 'ecl-path-'));
...
const RUNTIME_ENV_VARS = [
  'ECL_AGENTS_DIR', 'ECL_RUNTIME', 'CLAUDE_CONFIG_DIR', 'OPENCODE_CONFIG_DIR',
...
describe('detectRuntime', () => {
  ...
  it('uses ECL_RUNTIME when set to a known runtime', () => {
  ...
  it('normalizes common runtime env aliases to canonical runtime IDs', () => {
    ...
    for (const [alias, runtime] of cases) {
      process.env.ECL_RUNTIME = alias;
      expect(detectRuntime()).toBe(runtime);
    }
  });

  it('falls back to config.runtime when ECL_RUNTIME unset', () => {
  ...
  it('ECL_RUNTIME wins over config.runtime', () => {
    process.env.ECL_RUNTIME = 'codex';
    expect(detectRuntime({ runtime: 'gemini' })).toBe('codex');
  });

  it('unknown ECL_RUNTIME falls through to config then claude', () => {
    process.env.ECL_RUNTIME = 'bogus';
    expect(detectRuntime({ runtime: 'gemini' })).toBe('gemini');
    expect(detectRuntime()).toBe('claude');
  });
...
describe('resolveAgentsDir (runtime-aware)', () => {
  ...
  it('ECL_AGENTS_DIR short-circuits regardless of runtime', () => {
    process.env.ECL_AGENTS_DIR = '/explicit/agents';
...
describe('findProjectRoot (multi-repo .planning resolution)', () => {
  ...
  beforeEach(async () => {
    workspace = await mkdtemp(join(tmpdir(), 'ecl-find-root-'));
  });
