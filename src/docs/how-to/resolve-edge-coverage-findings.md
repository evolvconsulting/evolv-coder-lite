...
**Prerequisites:** A phase whose `/ecl-spec-phase` run has passed the ambiguity gate. The edge-completeness probe (Step 5.5) then runs automatically and presents its findings — you do not invoke it separately.
...
When you next run `/ecl-plan-phase`, the planner reads the spec's `## Edge Coverage` section and:
