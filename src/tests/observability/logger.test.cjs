...
function makeTmpDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-logger-test-'));
}
...
describe('createDefaultLogger — silent on success', () => {
  ...
  beforeEach(() => {
    tmpDir = makeTmpDir();
    savedAudit = process.env.ECL_AUDIT;
    savedAuditArgs = process.env.ECL_AUDIT_ARGS;
    delete process.env.ECL_AUDIT;
    delete process.env.ECL_AUDIT_ARGS;
  });
  ...
  test('no audit file created on ok result (no ECL_AUDIT)', () => {
    const logger = createDefaultLogger({ cwd: tmpDir });
    logger.onEvent(makeOkEvent());
    const auditPath = path.join(tmpDir, '.planning', '.ecl-trace.jsonl');
    assert.ok(!fs.existsSync(auditPath), 'audit file must not be created without ECL_AUDIT=1');
  });
});
...
describe('createDefaultLogger — stderr on error', () => {
  ...
  test('stderr JSON includes args when ECL_AUDIT_ARGS=1', () => {
    process.env.ECL_AUDIT_ARGS = '1';
    const logger = createDefaultLogger({ cwd: tmpDir });
    ...
    const stderrOutput = captureStderr(() => logger.onEvent(errEvent));
    const parsed = JSON.parse(stderrOutput.trim());
    assert.ok('args' in parsed, 'args must appear in stderr when ECL_AUDIT_ARGS=1');
    assert.deepStrictEqual(parsed.args, ['--bad', 'value']);
  });

  test('no stderr output on ok result even when ECL_AUDIT=1', () => {
    process.env.ECL_AUDIT = '1';
    const logger = createDefaultLogger({ cwd: tmpDir });
    const stderrOutput = captureStderr(() => logger.onEvent(makeOkEvent()));
...
describe('createDefaultLogger — audit file', () => {
  ...
  test('creates .planning/.ecl-trace.jsonl when ECL_AUDIT=1 (ok result)', () => {
    process.env.ECL_AUDIT = '1';
    const logger = createDefaultLogger({ cwd: tmpDir });
    logger.onEvent(makeOkEvent());
    const auditPath = path.join(tmpDir, '.planning', '.ecl-trace.jsonl');
    assert.ok(fs.existsSync(auditPath), 'audit file must be created when ECL_AUDIT=1');
  });
  ...
  test('audit file DOES contain args when ECL_AUDIT_ARGS=1', () => {
    process.env.ECL_AUDIT = '1';
    process.env.ECL_AUDIT_ARGS = '1';
    const logger = createDefaultLogger({ cwd: tmpDir });
    ...
    const content = fs.readFileSync(auditPath, 'utf8');
    const parsed = JSON.parse(content.trim());
    assert.ok('args' in parsed, 'args must appear in audit file when ECL_AUDIT_ARGS=1');
    assert.deepStrictEqual(parsed.args, ['visible-arg']);
  });

  test('config.audit.enabled === true triggers audit file (without ECL_AUDIT env)', () => {
    // ECL_AUDIT is not set, but config says enabled
