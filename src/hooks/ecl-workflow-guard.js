...
// eCL Workflow Guard — PreToolUse hook
// Detects when Claude attempts file edits outside a eCL workflow context
// (no active /ecl- skill or Task subagent) and injects an advisory warning.
//
// This is a SOFT guard — it advises, not blocks. The edit still proceeds.
// The warning nudges Claude to use /ecl:quick or /ecl:fast instead of
...
process.stdin.on('end', () => {
  ...
  try {
    ...
    // Check if we're inside a eCL workflow (Task subagent or /ecl- skill)
    ...
    // Allow edits to .planning/ files (eCL state management)
    if (filePath.includes('.planning/') || filePath.includes('.planning\\')) {
      process.exit(0);
    }

    // Allow edits to common config/docs files that don't need eCL tracking
    ...
    if (!isWorkflowGuardEnabled) {
      process.exit(0); // Guard disabled (default) or no eCL project
    }

    // If we get here: eCL project, guard enabled, file edit outside .planning/,
    // not in a subagent context. Inject advisory warning.
    const output = {
      hookSpecificOutput: {
        hookEventName: "PreToolUse",
        additionalContext: `⚠️ WORKFLOW ADVISORY: You're editing ${path.basename(filePath)} directly without a eCL command. ` +
          'This edit will not be tracked in STATE.md or produce a SUMMARY.md. ' +
          'Consider using /ecl:fast for trivial fixes or /ecl:quick for larger changes ' +
