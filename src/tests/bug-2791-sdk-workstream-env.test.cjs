 ...
 *
 * When ECL_WORKSTREAM is set in the environment, `ecl-sdk query` commands must
 * route .planning/ reads to `.planning/workstreams/<name>/` — matching the
 * behaviour of `ecl-tools.cjs` which reads the same env var via planningDir().
 *
 * Before the fix: the SDK CLI only respected `--ws <name>` flag; ECL_WORKSTREAM
 * was ignored, so `ecl-sdk query roadmap.analyze` always read the root
 * `.planning/ROADMAP.md` even when a workstream was active.
 *
 * After the fix: the SDK CLI falls back to ECL_WORKSTREAM when --ws is absent.
 *
 * This test also verifies:
 * - The `ecl-tools` bin alias maps to the same SDK shim as `ecl-sdk` (#2791 Issue 1)
 */
  ...
  try {
    stdout = childProc.execFileSync(
      ...
      [SDK_CLI, 'query', ...args, '--project-dir', projectDir],
      {
        ...
        env: { ...process.env, ECL_SESSION_KEY: '', ...extraEnv },
      }
    );
  } catch (err) {
...
describe('bug-2791: ECL_WORKSTREAM env var respected by ecl-sdk query', () => {
  let tmpDir;

  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-test-2791-'));
  ...
  test('without ECL_WORKSTREAM: roadmap.analyze reads root .planning/ROADMAP.md', () => {
  ...
  test('with ECL_WORKSTREAM set: roadmap.analyze reads workstream ROADMAP.md', () => {
    const result = runSdkQuery(['roadmap.analyze'], tmpDir, { ECL_WORKSTREAM: 'my-ws' });
    assert.strictEqual(result.exitCode, 0, `expected exit 0: ${result.stdout}`);
    assert.ok(result.json !== null, 'expected JSON output');
  ...
  test('--ws flag takes precedence over ECL_WORKSTREAM env var', () => {
    // Set ECL_WORKSTREAM to a non-existent workstream; --ws should override it.
    ...
    assert.strictEqual(
      result.json.phase_count,
      1,
      `expected 1 phase via --ws flag (overriding ECL_WORKSTREAM), got ${result.json.phase_count}`
    );
  });

  test('invalid ECL_WORKSTREAM value is silently ignored and falls back to root', () => {
    const result = runSdkQuery(['roadmap.analyze'], tmpDir, { ECL_WORKSTREAM: '../evil' });
    // Should not crash; invalid name is silently ignored and falls back to root ROADMAP.
    assert.strictEqual(result.exitCode, 0, `expected exit 0 (invalid ECL_WORKSTREAM ignored): ${result.stdout}`);
    assert.ok(result.json !== null, 'expected JSON output after invalid ECL_WORKSTREAM fallback');
    // Root ROADMAP has no phases — confirming root fallback, not an error path.
    assert.strictEqual(
      result.json.phase_count,
      0,
      `expected 0 phases from root ROADMAP fallback (invalid ECL_WORKSTREAM), got ${result.json.phase_count}`
    );
  });
});

describe('bug-2791: package.json declares ecl-tools bin alias (#2791 Issue 1)', () => {
  test('package.json bin has ecl-tools entry', () => {
    const pkg = JSON.parse(fs.readFileSync(PKG_JSON, 'utf-8'));
    assert.ok(
      Object.prototype.hasOwnProperty.call(pkg.bin ?? {}, 'ecl-tools'),
      'package.json bin must include "ecl-tools" to provide collision-free alternative to ecl-sdk'
    );
  });

  test('ecl-tools bin entry points to same file as ecl-sdk', () => {
    const pkg = JSON.parse(fs.readFileSync(PKG_JSON, 'utf-8'));
    assert.strictEqual(
      pkg.bin['ecl-tools'],
      pkg.bin['ecl-sdk'],
      'ecl-tools and ecl-sdk must point to the same bin shim'
    );
  });
});
