 ...
 *
 * Uses the same `loadConfig(cwd)` as `evolv-coder-lite/bin/lib/state.cjs` by resolving
 * `core.cjs` next to a shipped/bundled/user `evolv-coder-lite` install (same probe order
 * as `resolveGsdToolsPath`). This keeps JSON output **byte-compatible** with
 * `node ecl-tools.cjs state load` for monorepo and standard installs.
 *
 ...
 *
 * Port of `cmdStateLoad` from `evolv-coder-lite/bin/lib/state.cjs` lines 44–86.
 */
