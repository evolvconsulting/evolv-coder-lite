...
describe('install-profiles: MINIMAL_SKILL_ALLOWLIST', () => {
  ...
  test('every allowlisted skill exists in commands/ecl/', () => {
    ...
    for (const name of MINIMAL_SKILL_ALLOWLIST) {
      assert.ok(
        ...
        `${name} is allowlisted but commands/ecl/${name}.md does not exist`,
      );
    }
  });
});
...
describe('install-profiles: stageSkillsForMode', () => {
  function createFixtureSkillsDir() {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-stage-fixture-'));
  ...
  test('minimal mode against non-existent source returns source path', () => {
    const ghost = path.join(os.tmpdir(), 'ecl-stage-does-not-exist-' + Date.now());
    assert.strictEqual(stageSkillsForMode(ghost, 'minimal'), ghost);
  });

  test('minimal mode skips non-md files and subdirectories', () => {
    const src = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-stage-mixed-'));
    let staged;
    try {
...
describe('install-profiles: cleanupStagedSkills', () => {
  test('removes staged dirs created during process', () => {
    const src = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-stage-cleanup-'));
    fs.writeFileSync(path.join(src, 'plan-phase.md'), '# plan\n');
    try {
  ...
  test('exit handler registers at most once across many calls', () => {
    cleanupStagedSkills();
    const src = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-stage-exit-handler-'));
    fs.writeFileSync(path.join(src, 'plan-phase.md'), '# plan\n');
    try {
  ...
  test('mid-copy failure removes partial staged dir and re-throws', () => {
    cleanupStagedSkills();
    const src = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-stage-fail-'));
    fs.writeFileSync(path.join(src, 'plan-phase.md'), '# plan\n');
    ...
    fs.mkdtempSync = (prefix, ...rest) => {
      ...
      if (typeof prefix === 'string' && prefix.endsWith('ecl-minimal-skills-')) stagedDir = out;
      return out;
    };
    let copyCount = 0;
    fs.copyFileSync = (s, d) => {
...
describe('install: manifest records mode for both profiles', () => {
  function manifestModeAfterInstall(extraArgs) {
    const targetDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-manifest-mode-'));
    try {
      spawnSync(
        process.execPath,
...
describe('install-minimal-backcompat: --minimal and --profile=core produce same manifest', () => {
  function installAndGetManifest(extraArgs) {
    const targetDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-backcompat-'));
    try {
      spawnSync(
        process.execPath,
        [INSTALL_SCRIPT, '--claude', '--global', '--config-dir', targetDir, ...extraArgs],
      ...
      const markerPath = path.join(targetDir, '.ecl-profile');
  ...
  test('--minimal writes .ecl-profile marker "core"', () => {
    const r = installAndGetManifest(['--minimal']);
    assert.strictEqual(r.profileMarker, 'core');
  });

  test('default install writes .ecl-profile marker "full"', () => {
    const r = installAndGetManifest([]);
    assert.strictEqual(r.profileMarker, 'full');
  });

  test('--profile=core writes .ecl-profile marker "core"', () => {
    const r = installAndGetManifest(['--profile=core']);
    assert.strictEqual(r.profileMarker, 'core');
  });

  test('--profile=standard writes .ecl-profile marker "standard"', () => {
...
describe('install: Codex full → minimal downgrade cleans stale agent state', () => {
  test('--minimal removes stale .toml agents and strips [agents.ecl-*] from config.toml', () => {
    const targetDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-codex-downgrade-'));
    try {
      const agentsDir = path.join(targetDir, 'agents');
      fs.mkdirSync(agentsDir, { recursive: true });
      fs.writeFileSync(path.join(agentsDir, 'ecl-executor.md'), 'stale\n');
      fs.writeFileSync(path.join(agentsDir, 'ecl-planner.md'), 'stale\n');
      fs.writeFileSync(path.join(agentsDir, 'ecl-executor.toml'), 'name = "ecl-executor"\n');
      fs.writeFileSync(path.join(agentsDir, 'ecl-planner.toml'), 'name = "ecl-planner"\n');
      fs.writeFileSync(path.join(agentsDir, 'my-custom-agent.md'), 'user owns this\n');
      const codexConfig = [
        '# user-owned setting',
        'model = "gpt-5"',
        '',
        '# eCL Agent Configuration — managed by evolv-coder-lite installer',
        '[agents.ecl-executor]',
        'cmd = "stale"',
        '',
        '[agents.ecl-planner]',
      ...
      assert.ok(!remaining.includes('ecl-executor.md'));
      assert.ok(!remaining.includes('ecl-planner.md'));
      assert.ok(!remaining.includes('ecl-executor.toml'));
      assert.ok(!remaining.includes('ecl-planner.toml'));
      assert.ok(remaining.includes('my-custom-agent.md'));

      const configPath = path.join(targetDir, 'config.toml');
      if (fs.existsSync(configPath)) {
        const config = fs.readFileSync(configPath, 'utf8');
        assert.ok(!config.includes('[agents.ecl-executor]'));
        assert.ok(!config.includes('[agents.ecl-planner]'));
...
describe('install: Claude full → minimal downgrade removes stale agents', () => {
  test('--minimal removes stale ecl-*.md agents but preserves user-owned agents', () => {
    const targetDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-claude-downgrade-'));
    try {
      const agentsDir = path.join(targetDir, 'agents');
      fs.mkdirSync(agentsDir, { recursive: true });
      fs.writeFileSync(path.join(agentsDir, 'ecl-executor.md'), 'stale\n');
      ...
      assert.ok(!remaining.includes('ecl-planner.md'));
      assert.ok(remaining.includes('my-custom-agent.md'));
      assert.deepStrictEqual(remaining.filter(f => f.startsWith('ecl-')), []);
    } finally {
      fs.rmSync(targetDir, { recursive: true, force: true });
    }
  });
...
describe('#1755: .sh hooks are copied and executable after install', () => {
  let tmpDir;
  beforeEach(() => { tmpDir = createTempDir('ecl-hook-copy-'); });
...
describe('writeManifest includes .sh hooks', () => {
  let tmpDir;
  beforeEach(() => {
    tmpDir = createTempDir('ecl-manifest-');
  ...
  test('manifest contains .sh hook entries', () => {
    ...
    const manifest = JSON.parse(fs.readFileSync(path.join(tmpDir, 'ecl-file-manifest.json'), 'utf8'));
    for (const sh of EXPECTED_SH_HOOKS) {
...
describe('uninstall settings cleanup preserves user hooks', () => {
  const isGsdHook = (cmd) =>
    cmd && (cmd.includes('ecl-check-update') || cmd.includes('ecl-statusline') ||
      cmd.includes('ecl-session-state') || cmd.includes('ecl-context-monitor') ||
      cmd.includes('ecl-phase-boundary') || cmd.includes('ecl-prompt-guard') ||
      cmd.includes('ecl-read-guard') || cmd.includes('ecl-validate-commit') ||
      cmd.includes('ecl-workflow-guard'));
  ...
  test('mixed entry preserves user hooks', () => {
    const entries = [{
      matcher: 'Bash',
      hooks: [
        { type: 'command', command: 'node /path/ecl-prompt-guard.js' },
  ...
  test('entry with only eCL hooks is fully removed', () => {
    const entries = [{
      hooks: [
        { type: 'command', command: 'node /path/ecl-check-update.js' },
        { type: 'command', command: 'node /path/ecl-statusline.js' },
      ],
    }];
  ...
  test('non-array hook entries are preserved (#1825)', () => {
    const entries = [
      ...
      { matcher: 'Bash', hooks: [{ type: 'command', command: 'node /path/ecl-prompt-guard.js' }] },
  ...
  test('all eCL hook names are recognised', () => {
    const cmds = [
      'node /path/ecl-check-update.js', 'node /path/ecl-statusline.js',
      'bash /path/ecl-session-state.sh', 'node /path/ecl-context-monitor.js',
      'bash /path/ecl-phase-boundary.sh', 'node /path/ecl-prompt-guard.js',
      'node /path/ecl-read-guard.js', 'bash /path/ecl-validate-commit.sh',
      'node /path/ecl-workflow-guard.js',
    ];
...
describe('Codex legacy ecl-update-check migration', () => {
  const src = fs.readFileSync(path.join(__dirname, '..', 'bin', 'install.js'), 'utf8');

  test('install.js strips legacy ecl-update-check hook blocks', () => {
    assert.ok(src.includes('ecl-update-check') && src.includes('replace('));
  });

  test('migration regex removes LF legacy hook block', () => {
    const legacyBlock = ['[features]', 'codex_hooks = true', '',
      '# eCL Hooks', '[[hooks]]', 'event = "SessionStart"',
      'command = "node /old/path/ecl-update-check.js"', ''].join('\n');
    let content = legacyBlock.replace(
      /\n# eCL Hooks\n\[\[hooks\]\]\nevent = "SessionStart"\ncommand = "node [^\n]*ecl-update-check\.js"\n/g, '\n',
    );
    assert.ok(!content.includes('ecl-update-check'));
