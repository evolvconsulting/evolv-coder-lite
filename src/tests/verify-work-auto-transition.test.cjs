...
const VERIFY_WORK = path.join(__dirname, '..', 'evolv-coder-lite', 'workflows', 'verify-work.md');
...
describe('verify-work.md — auto-transition after UAT passes with 0 issues', () => {
  ...
  test('transition is NOT suggested when security enforcement is enabled and no SECURITY.md exists', () => {
    const content = fs.readFileSync(VERIFY_WORK, 'utf-8');
    // The workflow should suggest /ecl-secure-phase when security is enabled but no file exists
    assert.ok(
      content.includes('ecl-secure-phase') || content.includes('ecl:secure-phase'),
      'verify-work.md must suggest /ecl:secure-phase when security gate blocks transition'
    );
  });
});
