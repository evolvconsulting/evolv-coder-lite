<purpose>
Create all phases necessary to close gaps identified by `/ecl:audit-milestone`. Reads MILESTONE-AUDIT.md, groups gaps into logical phases, creates phase entries in ROADMAP.md, and offers to plan each phase. One command creates all fix phases — no manual `/ecl-add-phase` per gap.
</purpose>
...
```
No audit gaps found. Run `/ecl:audit-milestone` first.
```
...
HIGHEST=$($ECL_SDK query phases.list --pick directories[-1])
```
...
```bash
$ECL_SDK query commit "docs(roadmap): add gap closure phases {N}-{M}" --files .planning/ROADMAP.md .planning/REQUIREMENTS.md
```
...
**Also available:**
- `/ecl:execute-phase {N}` — if plans already exist
...
`/ecl:audit-milestone` — re-audit to verify gaps closed
`/ecl:complete-milestone {version}` — archive when audit passes
```
...
- [ ] User knows to run `/ecl:plan-phase` next
</success_criteria>
