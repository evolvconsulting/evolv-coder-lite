...
通过 `ecl-tools.cjs query commit` 提交规划工件，它会自动检查 `commit_docs` 配置和 gitignore 状态（与旧版 `ecl-tools.cjs commit` 行为相同）。
...
```bash
ecl-tools.cjs query commit "docs({scope}): {description}" --files .planning/STATE.md .planning/ROADMAP.md
```
...
```bash
ecl-tools.cjs query commit "" --files .planning/codebase/*.md --amend
```
