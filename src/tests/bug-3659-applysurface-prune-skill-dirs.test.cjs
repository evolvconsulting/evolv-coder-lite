 ...
 *
 * applySurface did not prune ~/.claude/skills/ecl-STEM dirs when a cluster
 ...
 *
 * The pruning ran against the wrong (non-existent) dir so stale ecl-Y dirs
 ...
 *   2. Surface state file moves to <configDir>/.ecl-surface.json at the config
 *      root, matching install/uninstall conventions.
 *   3. applySurface is called with scope='global' so the skills kind is active.
 *
 * Tests:
 *   a) disabled cluster ecl-STEM dirs are REMOVED from ~/.claude/skills/
 *   b) ecl-STEM dirs in the retain set are preserved
 *   c) non-ecl dirs are UNTOUCHED (user-owned)
...
const { writeSurface, applySurface } = require('../evolv-coder-lite/bin/lib/surface.cjs');
const { loadSkillsManifest } = require('../evolv-coder-lite/bin/lib/install-profiles.cjs');
const { CLUSTERS } = require('../evolv-coder-lite/bin/lib/clusters.cjs');
 ...
 * skillsDir  — analogous to ~/.claude/skills (contains ecl-* dirs)
 *
 * Pre-populated with:
 *   ecl-explore/SKILL.md  — in research_ideate cluster (will be disabled)
 *   ecl-help/SKILL.md     — in core_loop cluster (will remain enabled)
 *   my-custom-skill/      — user-owned, not ecl-prefixed (must never be touched)
 */
function createFixture() {
  const configDir = createTempDir('ecl-bug3659-');
  const skillsDir = path.join(configDir, 'skills');
  fs.mkdirSync(skillsDir, { recursive: true });

  const gsdExplore = path.join(skillsDir, 'ecl-explore');
...
/**
 * Extended fixture that also includes a user-created ecl-* directory.
 * Used by the all-clusters-disabled counter-test to prove the manifest-membership
 * gate (Finding 1 fix) protects user-owned ecl-* dirs from data loss.
 */
function createFixtureWithUserGsdDir() {
  const base = createFixture();
  const userGsdDir = path.join(base.skillsDir, 'ecl-mything');
  fs.mkdirSync(userGsdDir, { recursive: true });
  fs.writeFileSync(path.join(userGsdDir, 'SKILL.md'), '# user skill\n', 'utf8');
  return { ...base, userGsdDir };
}

describe('bug-3659: applySurface prunes ~/.claude/skills/ecl-*/ on cluster disable', () => {
  test('(a) disabled cluster ecl-* dirs are removed from skills dir', (t) => {
    ...
    // scope='global' gives the skills kind for Claude (destSubpath='skills', prefix='ecl-')
    const layout = resolveRuntimeArtifactLayout('claude', configDir, 'global');
    applySurface(configDir, layout, manifest, CLUSTERS);

    // ecl-explore is in the research_ideate cluster which was disabled:
    // it must be pruned from skillsDir.
    assert.ok(
      !fs.existsSync(gsdExplore),
      'ecl-explore/ must be removed from skills dir when research_ideate cluster is disabled'
    );

    // ecl-help is in core_loop (not disabled) and must survive.
    assert.ok(
      fs.existsSync(gsdHelp),
      'ecl-help/ must be preserved when its cluster is not disabled'
    );
  });

  test('(b) ecl-* dirs in retained clusters are preserved', (t) => {
    ...
    assert.ok(
      fs.existsSync(gsdHelp),
      'ecl-help/ must be preserved — core_loop cluster remains enabled'
    );
  });

  test('(c) non-ecl user dirs are untouched', (t) => {
    ...
    assert.ok(
      ...
      'my-custom-skill/ (non-ecl user dir) must be preserved by applySurface'
    );
    assert.ok(
  ...
  test('(d) idempotence: running applySurface twice produces identical on-disk state', (t) => {
    ...
    assert.ok(
      !fs.existsSync(gsdExplore),
      'ecl-explore/ must remain absent after second applySurface call'
    );
  ...
  test('(e) all-clusters-disabled: all ecl-owned dirs removed; user dirs and user ecl-* dirs survive', (t) => {
    ...
    // Assertions:
    //   1. ecl-explore/ — eCL-owned, disabled cluster → REMOVED
    //   2. ecl-help/    — eCL-owned, disabled cluster → REMOVED
    //   3. my-custom-skill/ — user-owned, no ecl- prefix → PRESERVED
    //   4. ecl-mything/ — prefix match but NOT in manifest → PRESERVED (Finding 1 fix)
    ...
    // 1. eCL-owned dirs in now-disabled clusters must be removed.
    assert.ok(
      !fs.existsSync(gsdExplore),
      'ecl-explore/ must be removed when all clusters are disabled'
    );
    assert.ok(
      !fs.existsSync(gsdHelp),
      'ecl-help/ must be removed when all clusters are disabled'
    );

    // 2. Non-ecl user dir must be preserved regardless.
    assert.ok(
      fs.existsSync(userSkill),
      'my-custom-skill/ (non-ecl user dir) must survive when all clusters are disabled'
    );

    // 3. User-created ecl-* dir NOT in the manifest must be preserved.
    //    This is the critical Finding 1 regression guard: without the manifest-membership
    //    gate, ecl-mything/ would have been silently deleted.
    assert.ok(
      fs.existsSync(userGsdDir),
      'ecl-mything/ (user-created ecl-* dir not in manifest) must be preserved — ' +
