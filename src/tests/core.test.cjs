...
describe('loadConfig', () => {
  ...
  test('returns model_overrides when present (REG-01)', () => {
    writeConfig({ model_overrides: { 'ecl-executor': 'opus' } });
    const config = loadConfig(tmpDir);
    assert.deepStrictEqual(config.model_overrides, { 'ecl-executor': 'opus' });
  });
  ...
  test('known config keys are derived from VALID_CONFIG_KEYS (not hardcoded)', () => {
    ...
    const { VALID_CONFIG_KEYS } = require('../evolv-coder-lite/bin/lib/config.cjs');
...
describe('loadConfig workstream config inheritance (#2714)', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempProject();
    originalEnv = process.env.ECL_WORKSTREAM;
    delete process.env.ECL_WORKSTREAM;
  });

  afterEach(() => {
    if (originalEnv !== undefined) {
      process.env.ECL_WORKSTREAM = originalEnv;
    } else {
      delete process.env.ECL_WORKSTREAM;
    }
    cleanup(tmpDir);
  ...
  test('workstream config inherits model_overrides from root when not defined in workstream', () => {
    writeRootConfig({ model_overrides: { 'ecl-executor': 'opus' } });
    writeWorkstreamConfig('feature-a', { model_profile: 'quality' });
    process.env.ECL_WORKSTREAM = 'feature-a';
    const config = loadConfig(tmpDir);
    assert.deepStrictEqual(config.model_overrides, { 'ecl-executor': 'opus' });
    assert.strictEqual(config.model_profile, 'quality');
  });
  ...
  test('workstream-specific keys override root config values', () => {
    writeRootConfig({ model_profile: 'balanced', model_overrides: { 'ecl-executor': 'opus' } });
    writeWorkstreamConfig('feature-b', { model_profile: 'speed', model_overrides: { 'ecl-executor': 'haiku' } });
    process.env.ECL_WORKSTREAM = 'feature-b';
    const config = loadConfig(tmpDir);
    assert.strictEqual(config.model_profile, 'speed');
    assert.deepStrictEqual(config.model_overrides, { 'ecl-executor': 'haiku' });
  });
  ...
  test('deep merge works for nested workflow.* keys', () => {
    ...
    process.env.ECL_WORKSTREAM = 'feature-c';
    const config = loadConfig(tmpDir);
  ...
  test('explicit null in workstream config overrides root value (PR #2717 null-override bug)', () => {
    writeRootConfig({ model_overrides: { 'ecl-executor': 'opus', 'ecl-planner': 'sonnet' } });
    writeWorkstreamConfig('feature-d', { model_overrides: null });
    process.env.ECL_WORKSTREAM = 'feature-d';
    const config = loadConfig(tmpDir);
...
describe('loadConfig commit_docs gitignore auto-detection (#1250)', () => {
  ...
  test('commit_docs auto-detect works with no config.json', () => {
    ...
    // a project without config.json hasn't been initialized by eCL yet.
...
describe('resolveModelInternal', () => {
  ...
  describe('model profile structural validation', () => {
    test('all known agents resolve to a valid string for each profile', () => {
      const knownAgents = ['ecl-planner', 'ecl-executor', 'ecl-phase-researcher', 'ecl-codebase-mapper'];
  ...
  describe('override precedence', () => {
    test('per-agent override takes precedence over profile', () => {
      writeConfig({
        ...
        model_overrides: { 'ecl-executor': 'haiku' },
      });
      assert.strictEqual(resolveModelInternal(tmpDir, 'ecl-executor'), 'haiku');
    });

    test('opus override resolves to opus', () => {
      writeConfig({
        model_overrides: { 'ecl-executor': 'opus' },
      });
      assert.strictEqual(resolveModelInternal(tmpDir, 'ecl-executor'), 'opus');
    });
    ...
    test('agents not in override fall back to profile', () => {
      ...
      // ecl-planner not overridden, should use quality profile -> opus
      assert.strictEqual(resolveModelInternal(tmpDir, 'ecl-planner'), 'opus');
    });
  });
  ...
  describe('edge cases', () => {
    test('returns sonnet for unknown agent type', () => {
      writeConfig({ model_profile: 'balanced' });
      assert.strictEqual(resolveModelInternal(tmpDir, 'ecl-nonexistent'), 'sonnet');
    });

    test('returns opus for unknown agent type with quality profile', () => {
      writeConfig({ model_profile: 'quality' });
      assert.strictEqual(resolveModelInternal(tmpDir, 'ecl-nonexistent'), 'opus');
    });

    test('returns haiku for unknown agent type with budget profile', () => {
      writeConfig({ model_profile: 'budget' });
      assert.strictEqual(resolveModelInternal(tmpDir, 'ecl-nonexistent'), 'haiku');
    });

    test('returns inherit for unknown agent type with inherit profile', () => {
      writeConfig({ model_profile: 'inherit' });
      assert.strictEqual(resolveModelInternal(tmpDir, 'ecl-nonexistent'), 'inherit');
    });

    test('defaults to balanced profile when model_profile missing', () => {
      writeConfig({});
      // balanced profile, ecl-planner -> opus
      assert.strictEqual(resolveModelInternal(tmpDir, 'ecl-planner'), 'opus');
    });
  });
  ...
  describe('resolve_model_ids: "omit"', () => {
    ...
    test('returns empty string for unknown agents', () => {
      writeConfig({ resolve_model_ids: 'omit' });
      assert.strictEqual(resolveModelInternal(tmpDir, 'ecl-nonexistent'), '');
    });

    test('still respects model_overrides even when omit', () => {
      writeConfig({
        resolve_model_ids: 'omit',
        model_overrides: { 'ecl-planner': 'openai/gpt-5.4' },
      });
      assert.strictEqual(resolveModelInternal(tmpDir, 'ecl-planner'), 'openai/gpt-5.4');
    });
  ...
  describe('resolve_model_ids: true', () => {
    ...
    test('sonnet alias resolves to claude-sonnet-4-6', () => {
      ...
      assert.strictEqual(resolveModelInternal(tmpDir, 'ecl-executor'), 'claude-sonnet-4-6');
    });
...
describe('stale hook filter', () => {
  test('filter should only match ecl-prefixed .js files', () => {
    const files = [
      'ecl-check-update.js',
      ...
      'ecl-check-update.js.bak',         // backup file
      'README.md',                       // non-js file
    ];

    const gsdFilter = f => f.startsWith('ecl-') && f.endsWith('.js');
    ...
    assert.deepStrictEqual(filtered, [
      ...
      'ecl-statusline.js',
      'ecl-workflow-guard.js',
    ], 'should only include ecl-prefixed .js files');

    assert.ok(!filtered.includes('guard-edits-outside-project.js'), 'must not include user hooks');
    assert.ok(!filtered.includes('my-custom-hook.js'), 'must not include non-ecl hooks');
  });
});
...
describe('stale hook path', () => {
  test('ecl-check-update.js checks configDir/hooks/ where hooks are actually installed (#1421)', () => {
    ...
    const content = fs.readFileSync(
      path.join(__dirname, '..', 'hooks', 'ecl-check-update-worker.js'), 'utf-8'
    );
    // Hooks are installed at configDir/hooks/ (e.g. ~/.claude/hooks/),
    // not configDir/evolv-coder-lite/hooks/ which doesn't exist (#1421)
    assert.ok(
...
describe('shared cache directory (#1421)', () => {
  test('ecl-check-update.js writes cache to shared ~/.cache/ecl/ directory', () => {
    const content = fs.readFileSync(
      path.join(__dirname, '..', 'hooks', 'ecl-check-update.js'), 'utf-8'
    );
    ...
    assert.ok(
      content.includes("path.join(homeDir, '.cache', 'ecl')"),
      'check-update must write cache to ~/.cache/ecl/ (shared, tool-agnostic)'
    );
  });
...
describe('resolveWorktreeRoot', () => {
  const { resolveWorktreeRoot } = require('../evolv-coder-lite/bin/lib/core.cjs');
  let tmpDir;
...
describe('resolveWorktreeRoot with linked worktree .planning/', () => {
  ...
  function initBareGitRepo() {
    const dir = normalizePath(fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-wt-main-')));
...
describe('monorepo worktree CWD preservation', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-monorepo-wt-'));
  });
...
describe('withPlanningLock', () => {
  const { withPlanningLock, planningDir } = require('../evolv-coder-lite/bin/lib/core.cjs');
  let tmpDir;
...
describe('detectSubRepos', () => {
  ...
  beforeEach(() => {
    projectRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-detect-test-'));
  });
...
describe('loadConfig sub_repos auto-sync', () => {
  ...
  beforeEach(() => {
    projectRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-sync-test-'));
...
describe('findProjectRoot', () => {
  ...
  beforeEach(() => {
    projectRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-root-test-'));
  });
...
describe('reapStaleTempFiles', () => {
  const gsdTmpDir = path.join(os.tmpdir(), 'ecl');

  test('removes stale ecl-*.json files older than maxAgeMs', () => {
    fs.mkdirSync(gsdTmpDir, { recursive: true });
    const stalePath = path.join(gsdTmpDir, `ecl-reap-test-${Date.now()}.json`);
    fs.writeFileSync(stalePath, '{}');
    // Set mtime to 10 minutes ago
    const oldTime = new Date(Date.now() - 10 * 60 * 1000);
    fs.utimesSync(stalePath, oldTime, oldTime);

    reapStaleTempFiles('ecl-reap-test-', { maxAgeMs: 5 * 60 * 1000 });

    assert.ok(!fs.existsSync(stalePath), 'stale file should be removed');
  });

  test('preserves fresh ecl-*.json files', () => {
    fs.mkdirSync(gsdTmpDir, { recursive: true });
    const freshPath = path.join(gsdTmpDir, `ecl-reap-fresh-${Date.now()}.json`);
    fs.writeFileSync(freshPath, '{}');

    reapStaleTempFiles('ecl-reap-fresh-', { maxAgeMs: 5 * 60 * 1000 });
  ...
  test('removes stale temp directories when present', () => {
    fs.mkdirSync(gsdTmpDir, { recursive: true });
    const staleDir = fs.mkdtempSync(path.join(gsdTmpDir, 'ecl-reap-dir-'));
    fs.writeFileSync(path.join(staleDir, 'data.jsonl'), 'test');
    // Set mtime to 10 minutes ago
    const oldTime = new Date(Date.now() - 10 * 60 * 1000);
    fs.utimesSync(staleDir, oldTime, oldTime);

    reapStaleTempFiles('ecl-reap-dir-', { maxAgeMs: 5 * 60 * 1000 });
  ...
  test('does not throw on empty or missing prefix matches', () => {
    assert.doesNotThrow(() => {
      reapStaleTempFiles('ecl-nonexistent-prefix-xyz-', { maxAgeMs: 0 });
    });
  });
});
...
describe('planningDir', () => {
  ...
  beforeEach(() => {
    savedProject = process.env.ECL_PROJECT;
    savedWorkstream = process.env.ECL_WORKSTREAM;
    delete process.env.ECL_PROJECT;
    delete process.env.ECL_WORKSTREAM;
  });

  afterEach(() => {
    if (savedProject !== undefined) process.env.ECL_PROJECT = savedProject;
    else delete process.env.ECL_PROJECT;
    if (savedWorkstream !== undefined) process.env.ECL_WORKSTREAM = savedWorkstream;
  ...
  test('reads ECL_PROJECT from env when project param is undefined', () => {
    process.env.ECL_PROJECT = 'env-project';
