...
before(() => { tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-changeset-')); });
after(() => { cleanup(tmp); });
...
describe('changeset cli extract: version-range changelog extraction (#3496)', () => {
  ...
  test('F1: workflows/update.md contains concrete extract subcommand invocation', (_t) => {
    const workflowPath = path.join(ROOT, 'evolv-coder-lite', 'workflows', 'update.md');
    const workflowText = fs.readFileSync(workflowPath, 'utf8');
    // The invocation uses either a direct path or an intermediate variable:
    //   node "$ECL_DIR/scripts/changeset/cli.cjs" extract
    //   node "$ECL_CHANGESET_CLI" extract
    ...
    assert.ok(
      ...
      (workflowText.includes('ECL_CHANGESET_CLI') && workflowText.includes('" extract')),
  ...
  // F2: update.md must use the INSTALLED path ($ECL_DIR/scripts/changeset/cli.cjs),
  // NOT the old broken path ($ECL_DIR/evolv-coder-lite/scripts/changeset/cli.cjs).
  // The installer copies scripts/changeset/ into <configDir>/scripts/changeset/,
  // so the runtime path is $ECL_DIR/scripts/changeset/cli.cjs (#935).
  // allow-test-rule: reads a product workflow .md file (not CJS source) to verify
  // the runtime install path contract; there is no behavioural runtime to invoke.
  test('F2: update.md CLI path is $ECL_DIR/scripts/changeset/cli.cjs (not evolv-coder-lite/scripts/…) (#935)', (_t) => {
    const workflowPath = path.join(ROOT, 'evolv-coder-lite', 'workflows', 'update.md');
    const workflowText = fs.readFileSync(workflowPath, 'utf8');
    ...
    // The old broken path ($ECL_DIR/evolv-coder-lite/scripts/changeset/cli.cjs) must not appear
    assert.ok(
      !workflowText.includes('evolv-coder-lite/scripts/changeset/cli.cjs'),
      'update.md must NOT reference the old evolv-coder-lite/scripts/changeset/cli.cjs path (fix for #935)',
    );
  });
  ...
  test('F3: update.md has an explicit guard when changeset CLI is missing (#935)', (_t) => {
    ...
    assert.ok(
      workflowText.includes('ECL_CHANGESET_CLI') && workflowText.includes('! -f'),
      'update.md must guard against a missing changeset CLI with [ ! -f "$ECL_CHANGESET_CLI" ] (#935)',
    );
  });
});
...
describe('changeset cli verify subcommand (not yet implemented — TDD red step)', () => {
  ...
  const FIXTURE_URL_HEADING = [
    '# Changelog',
    '',
    '## [1.3.1](https://www.npmjs.com/package/@evolvconsulting/evolv-coder-lite/v/1.3.1) - 2026-06-04',
    '',
    '### Fixed',
    '',
    '- Some fix. (#42)',
...
describe('changeset cli render --allow-empty', () => {
  ...
  test('--allow-empty with zero fragments writes dated heading + placeholder; verify exits 0', () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-ae-'));
    try {
