...
const FAMILIES = [
  { label: 'Agents',      dir: 'agents',                      filter: (f) => /^ecl-.*\.md$/.test(f) },
  { label: 'Commands',    dir: 'commands/ecl',                filter: (f) => f.endsWith('.md') },
  { label: 'Workflows',   dir: 'evolv-coder-lite/workflows',     filter: (f) => f.endsWith('.md') },
  { label: 'References',  dir: 'evolv-coder-lite/references',    filter: (f) => f.endsWith('.md') },
  { label: 'CLI Modules', dir: 'evolv-coder-lite/bin/lib',       filter: (f) => f.endsWith('.cjs') },
