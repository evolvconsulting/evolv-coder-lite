...
// tests the deployed contract. The commands/ecl/*.md reads in the helper are
 ...
 *   - evolv-coder-lite/workflows/help.md  (deprecation-trail prose)
...
const ALLOWED_HISTORICAL_MENTIONS = new Set([
  path.join(ROOT, 'evolv-coder-lite', 'workflows', 'help.md'),
...
// syntax placeholders — they match the /ecl-* regex but are NOT user-typable
...
const INTERNAL_COMPONENT_SLUGS = new Set([
  ...
  // (e.g. "/ecl-command-name [args]"). It is not a registered command.
  'command-name',
  'command',

  // ecl-tools.cjs — the legacy Node CLI binary (bin/ecl-tools.cjs).
  // Docs reference it as a path component in shell examples, not as a slash command.
  // Example: node "$HOME/.claude/evolv-coder-lite/bin/ecl-tools.cjs" state validate
  'tools',

  // Hook scripts — internal runtime hooks, not user-invocable slash commands.
  //   hooks/ecl-statusline.js       — session statusline hook
  //   hooks/ecl-context-monitor.js  — context-window monitor hook
  //   hooks/ecl-update-banner.js    — update-available banner hook
  //   hooks/ecl-graphify-update.sh  — knowledge-graph auto-update PostToolUse hook (#3347)
  // These appear in docs as file-path references (e.g. "ecl-statusline.js reads
  ...
  // ecl-update-check.json — background update-check CACHE FILE, not a slash command.
  // ARCHITECTURE.md references "~/.cache/ecl/ecl-update-check.json" as a path;
  // the regex captures "/ecl-update-check" from the path component.
  'update-check',

  // Internal agent names referenced in ARCHITECTURE.md tables of agents.
  // These are spawned agents (ecl-planner, etc.), not user-typable slash commands.
  'planner',

  // Malformed token from SDK init reference: "/ecl-init-" appears as a truncated
  // prefix in CLI-TOOLS.md describing the ecl-sdk init command family
  // (e.g., "ecl-sdk query init.phase-op 12"). The regex captures "/ecl-init-"
  ...
  // Docs reference "~/ecl-workspaces/<name>" as the default workspace directory
  // in shell examples and option tables (e.g. "--path /target (default: ~/ecl-workspaces/<name>)").
  // The regex captures "/ecl-workspaces" from the path component. The LIVE slash
  // command is "/ecl-workspace" (singular) — not "/ecl-workspaces" (plural).
  'workspaces',

  // Portuguese translation of "command" — pt-BR/ARCHITECTURE.md uses "/ecl-comando"
  // as the localized equivalent of the "/ecl-command-name" English placeholder
  // in an architecture flow diagram. Not a registered command.
  'comando',

  // GitHub repository name: zh-CN/README.md references "github.com/rokicool/ecl-opencode"
  // as an external community project URL. The regex captures "/ecl-opencode" from
  // the URL path. Not a user-typable slash command in this product.
  'opencode',

  // ecl-sdk — the @evolvconsulting/ecl-sdk npm package and `ecl-sdk query` CLI binary.
  // Docs reference it as a package name (e.g. `@evolvconsulting/ecl-sdk`) and CLI tool
  // (e.g. `ecl-sdk query init phase-op 12`). The regex captures "/ecl-sdk" from
  // the npm scope path separator in `@evolvconsulting/ecl-sdk`. Not a user-typable slash command.
  'sdk',

  // Smoke-test directory path — locale docs reference "/tmp/ecl-smoke-$(date +%s)"
  // as a temporary directory path in bash code-block examples. The regex captures
  // "/ecl-smoke-" from the filesystem path. Not a slash command.
  'smoke-',

  // Template placeholders — zh-CN/references/ui-brand.md used "/ecl-alternative-1"
  // and "/ecl-alternative-2" as unfilled placeholders in a UI template example.
  ...
  // ecl-sync-skills — installed Claude skill directory name (also a workflow
  // under evolv-coder-lite/workflows/sync-skills.md), but NOT a registered
  // slash command (no commands/ecl/sync-skills.md). Docs reference it as a
  // filesystem path component, e.g. "~/.agents/skills/ecl-sync-skills/" in
...
function extractCommandTokens(content) {
  ...
  function isInternal(token) {
    // Strip the /ecl- or /ecl: or $ecl- prefix to get the slug
    const slug = token.replace(/^(?:\/ecl[:-]|\$ecl-)/, '');
...
describe('getLiveCommandTokens() — helper contract', () => {
  ...
  test('returns a non-empty set (commands/ecl/ has registered commands)', () => {
    const result = getLiveCommandTokens();
    assert.ok(result.size > 0, 'live registry must contain at least one token');
  });

  test('contains /ecl-help (from commands/ecl/help.md name: ecl:help)', () => {
    const result = getLiveCommandTokens();
    assert.ok(result.has('/ecl-help'), 'registry must contain /ecl-help');
  });

  test('contains /ecl:help (Gemini form)', () => {
    const result = getLiveCommandTokens();
    assert.ok(result.has('/ecl:help'), 'registry must contain /ecl:help');
  });

  test('contains $ecl-help (Codex form)', () => {
    const result = getLiveCommandTokens();
    assert.ok(result.has('$ecl-help'), 'registry must contain $ecl-help');
  });

  test('contains /ecl-plan-phase (from commands/ecl/plan-phase.md)', () => {
    const result = getLiveCommandTokens();
    assert.ok(result.has('/ecl-plan-phase'), 'registry must contain /ecl-plan-phase');
  });
  ...
  test('contains exactly 3 tokens per slug (slash, colon, dollar)', () => {
    ...
    for (const slash of slashTokens) {
      ...
      assert.ok(
        result.has(`/ecl:${slug}`),
        `registry must contain Gemini form /ecl:${slug} for slash form ${slash}`
      );
      assert.ok(
        result.has(`$ecl-${slug}`),
        `registry must contain Codex form $ecl-${slug} for slash form ${slash}`
      );
    }
  });

  test('does NOT contain removed /ecl-reapply-patches', () => {
    const result = getLiveCommandTokens();
    assert.ok(!result.has('/ecl-reapply-patches'), 'registry must NOT contain removed /ecl-reapply-patches');
  });

  test('does NOT contain removed /ecl-code-review-fix', () => {
    const result = getLiveCommandTokens();
    assert.ok(!result.has('/ecl-code-review-fix'), 'registry must NOT contain removed /ecl-code-review-fix');
  });

  test('does NOT contain removed /ecl-status', () => {
    const result = getLiveCommandTokens();
    assert.ok(!result.has('/ecl-status'), 'registry must NOT contain removed /ecl-status');
  });
...
describe('getLiveCommandTokens() — fixture contract', () => {
  test('parses ecl:foo frontmatter and emits 3 canonical tokens', () => {
    // This test validates the parsing logic against a known-good fixture
    // by inspecting the live registry for commands/ecl/help.md (name: ecl:help).
    // Fixture file tests are done inline since the helper reads commands/ecl/ only.
    // The canonical token contract:
    //   name: ecl:foo → /ecl-foo, /ecl:foo, $ecl-foo
    const registry = getLiveCommandTokens();
    // We know help.md has name: ecl:help
    const slug = 'help';
    assert.ok(registry.has(`/ecl-${slug}`), `must have /ecl-${slug}`);
    assert.ok(registry.has(`/ecl:${slug}`), `must have /ecl:${slug}`);
    assert.ok(registry.has(`$ecl-${slug}`), `must have $ecl-${slug}`);
  });

  test('parses ecl-slug frontmatter (ns-* commands) and emits 3 tokens', () => {
    // ns-context.md has name: ecl-context (dash-style, no colon)
    const registry = getLiveCommandTokens();
    assert.ok(registry.has('/ecl-context'), 'must have /ecl-context (from ns-context.md)');
    assert.ok(registry.has('/ecl:context'), 'must have /ecl:context (Gemini form)');
    assert.ok(registry.has('$ecl-context'), 'must have $ecl-context (Codex form)');
  });
});
...
describe('adversarial: polarity inversion catches drift deny-list misses', () => {
  test('renaming a live command without updating docs would fail this test (demonstrated via token absence)', () => {
    // If /ecl-progress were renamed to /ecl-status-new, the old /ecl-progress
    // token would not appear in the live registry, and any doc referencing
    // /ecl-progress would fail. The deny-list shape would have passed silently
    ...
    const registry = getLiveCommandTokens();
    assert.ok(registry.has('/ecl-progress'), '/ecl-progress must be live (not renamed to /ecl-status)');
    assert.ok(!registry.has('/ecl-status'), '/ecl-status must be absent (was deleted, replaced by /ecl-progress)');
  });

  test('freshly-deleted command /ecl-check-todos is absent from registry', () => {
    const registry = getLiveCommandTokens();
    assert.ok(!registry.has('/ecl-check-todos'), '/ecl-check-todos must not be in the live registry');
  });

  test('freshly-deleted command /ecl-new-workspace is absent from registry', () => {
    const registry = getLiveCommandTokens();
    assert.ok(!registry.has('/ecl-new-workspace'), '/ecl-new-workspace must not be in the live registry');
  });

  test('freshly-deleted command /ecl-plan-milestone-gaps is absent from registry', () => {
    const registry = getLiveCommandTokens();
    assert.ok(!registry.has('/ecl-plan-milestone-gaps'), '/ecl-plan-milestone-gaps must not be in the live registry');
  });
