 ...
 *
 * Ported from evolv-coder-lite/bin/lib/roadmap.cjs and core.cjs.
...
export async function getMilestoneInfo(projectDir: string, workstream?: string): Promise<{ version: string; name: string }> {
  try {
    ...
    // List-format: in flight / active (eCL ROADMAP template uses 🟡 for current milestone)
...
export async function extractCurrentMilestone(content: string, projectDir: string, workstream?: string): Promise<string> {
  ...
  // matches the literal `Phase Details` label (canonical per eCL ROADMAP
...
function searchPhaseInContent(content: string, escapedPhase: string, phaseNum: string): PhaseSection | null {
  ...
  if (!headerMatch) {
    ...
    if (plainBulletMatch) {
      // Strip trailing annotations like `— pending /ecl:spec-phase 7`
 ...
 *
 * Delegates to ecl-tools.cjs which holds the full annotation logic.
...
export const roadmapAnnotateDependencies: QueryHandler = async (args, projectDir) => {
  ...
  // CRITICAL: set ECL_SDK_NESTED=1 so the CJS router in the child process
  ...
  // SDK→spawn(ecl-tools)→router→SDK→spawn(ecl-tools)→… loops until the
  // synckit 15s timeout fires and bug-3537's annotate test surfaces a
  // misleading "code=null" failure.
  const childEnv: NodeJS.ProcessEnv = { ...process.env, ECL_SDK_NESTED: '1' };
  ...
  if (result.error) {
    return { data: { updated: false, reason: result.error.message || 'ecl-tools invocation failed' } };
  }

  if (result.status !== 0) {
    return { data: { updated: false, reason: result.stderr?.trim() || 'ecl-tools error' } };
  }

  try {
    return { data: JSON.parse(result.stdout.trim()) };
  } catch {
    return { data: { updated: false, reason: 'failed to parse ecl-tools output' } };
  }
};
