...
-->
<purpose>
Create executable phase prompts (PLAN.md files) for a roadmap phase with integrated research and verification. Default flow: Research (if needed) -> Plan -> Verify -> Done. Orchestrates ecl-phase-researcher, ecl-planner, and ecl-plan-checker agents with a revision loop (max 3 iterations).
</purpose>
...
@~/.claude/evolv-coder-lite/references/revision-loop.md
@~/.claude/evolv-coder-lite/references/gate-prompts.md
@~/.claude/evolv-coder-lite/references/agent-contracts.md
@~/.claude/evolv-coder-lite/references/gates.md
</required_reading>

<available_agent_types>
Valid eCL subagent types (use exact names — do not fall back to 'general-purpose'):
- ecl-phase-researcher — Researches technical approaches for a phase
- ecl-pattern-mapper — Analyzes codebase for existing patterns, produces PATTERNS.md
- ecl-planner — Creates detailed plans from phase scope
- ecl-plan-checker — Reviews plan quality before execution
</available_agent_types>

<runtime_compatibility>
**Subagent spawning — top-level Claude Code:**
The Agent tool IS available in a top-level Claude Code session. Always spawn
ecl-phase-researcher, ecl-planner, and ecl-plan-checker as separate Agent() calls.
...
Agent() call for ecl-phase-researcher, ecl-planner, and ecl-plan-checker. Only
...
MVP_MODE_CFG=$(ecl_run query config-get workflow.mvp_mode 2>/dev/null || echo "false")
```
...
**If `planning_exists` is false:** Error — run `/ecl:new-project` first.
...
/ecl:plan-phase --reviews cannot replan a closed phase. If the review surfaced
...
with: /ecl:plan-phase ${phase_number} --force
...
if [[ "$ARGUMENTS" =~ (^|[[:space:]])--tdd([[:space:]]|$) ]]; then
  ecl_run query config-set workflow.tdd_mode true 2>/dev/null || true
fi
```
...
if [ "$MVP_MODE" = "true" ] && [ "$padded_phase" = "01" ]; then
  PRIOR_SUMMARIES=$(ecl_run query phases.list --pick summaries_total 2>/dev/null || echo "0")
...
```bash
CHUNKED_CFG=$(ecl_run query config-get workflow.plan_chunked 2>/dev/null || echo "false")
CHUNKED_MODE=false
...
Then re-run /ecl:plan-phase {N} --reviews
```
Exit workflow.
...
```bash
MVP_MODE=$(ecl_run query phase.mvp-mode "${PHASE}" $MVP_FLAG_ARG --pick active)
```
...
5. Commit:
```bash
ecl_run query commit "docs(${padded_phase}): generate context from PRD" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
1. Display banner: `eCL ► ADR Ingest Express Path` with `{INGEST_PATH}` and `{INGEST_FORMAT}`.
2. Parse each resolved ADR through `evolv-coder-lite/bin/lib/adr-parser.cjs` (`--input`, `--format`) and collect normalized records.
3. Status gate: reject `superseded`/`rejected`/`deprecated`; warn on `proposed`; missing status defaults to `accepted`.
4. Empty-decisions fallback: if all parsed ADRs have zero `decisions[]`, emit `ADR ingest produced no locked decisions; fall back to discuss-phase for this phase.` and exit with `/ecl:discuss-phase {N}` guidance.
...
6. Commit with `ecl-tools.cjs query commit "docs(${padded_phase}): generate context from ADR ingest" --files "${phase_dir}/${padded_phase}-CONTEXT.md"` and set `context_content`; continue to step 5.
...
If "Run discuss-phase first":
  **IMPORTANT:** Do NOT invoke discuss-phase as a nested Skill/Task call — AskUserQuestion
  ...
  ```
  Run this command first, then re-run /ecl:plan-phase {X} ${ECL_WS}:

  /ecl:discuss-phase {X} ${ECL_WS}
  ```
...
If `AI_SPEC_FILE` is non-empty, pass `AI_SPEC_PATH` and `FRAMEWORK_LINE` to the planner in step 8 so it can reference the AI design contract. If it is empty, the active `ai-integration` capability hook in step 5.6 handles any AI-system nudge or `/ecl:ai-integration-phase` dispatch.
...
if ecl_run query teams-status --active >/dev/null 2>&1; then
  echo "⚠️  CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS detected. eCL's multi-agent orchestration is not validated under claude-code agent-teams and may stall (a subagent's completion can fail to route to the orchestrator). Recommend disabling agent-teams for eCL workflows. See https://github.com/evolvconsulting/evolv-coder-lite/issues/1355" >&2
fi
```

```bash
PHASE_DESC=$(ecl_run query roadmap.get-phase "${PHASE}" --pick section)
if [ -z "${PLAN_PRE_HOOKS_JSON:-}" ]; then
  PLAN_PRE_HOOKS_JSON=$(ecl_run loop render-hooks plan:pre --raw)
fi
```
...
**If `RESEARCH_ONLY=true`:** the user invoked `/ecl:plan-phase --research-phase <N>` for research-only mode. Do **not** continue to Section 5.5+ (validation strategy, planner, plan-checker, verification, gaps, bounce, post-planning-gaps). Print the research-complete summary and exit cleanly:
...
Re-run /ecl:plan-phase ${PHASE} to plan the phase using this research,
or /ecl:plan-phase ${PHASE} --research to refresh research and plan.
```
...
**If found:**
1. Read template: `~/.claude/evolv-coder-lite/templates/VALIDATION.md`
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 eCL ► SECURITY THREAT MODEL REQUIRED (ASVS L{SECURITY_ASVS})
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
```bash
PLAN_PRE_HOOKS_JSON=${PLAN_PRE_HOOKS_JSON:-$(ecl_run loop render-hooks plan:pre --raw)}
...
- If `ref.skill` is set, dispatch with `Skill(skill="ecl-${ref.skill}", args="${PHASE} --auto ${ECL_WS}")` when pipeline mode allows auto-chaining. Prepend `ecl-` to `ref.skill` — `ui-phase` → `ecl-ui-phase`.
...
- In pipeline / `--auto` mode, invoke the hook's `ref.skill` via `Skill(skill="ecl-${ref.skill}", args="${PHASE} --auto ${ECL_WS}")`.
- In manual mode, display the existing non-blocking `/ecl:ai-integration-phase {N}` recommendation and let the user continue planning without AI-SPEC or stop to run the capability workflow first.
...
```bash
GATE=$(ecl_run check ui-plan-gate "${PHASE}" --raw)
```
...
```bash
AUTO_CHAIN=$(ecl_run query check auto-mode --pick auto_chain_active 2>/dev/null || echo "false")
```
...
```
Skill(skill="ecl-${ref.skill}", args="${PHASE} --auto ${ECL_WS}")
```
...
`/ecl:ui-phase {N} ${ECL_WS}` — generate UI design contract before planning
───────────────────────────────────────────────
Also available:
- `/ecl:plan-phase {N} --skip-ui ${ECL_WS}` — plan without UI-SPEC (not recommended for frontend phases)
```
...
1. Re-run: `/ecl:plan-phase {PHASE} --research ${ECL_WS}`
2. Disable Nyquist with the exact command:
   `ecl-tools.cjs query config-set workflow.nyquist_validation false`
3. Continue anyway (plans fail Dimension 8)

Proceed to Step 7.8 (or Step 8 if pattern mapper is disabled) only if user selects 2 or 3.

## 7.8. Spawn ecl-pattern-mapper Agent (Optional)

Pattern mapper activation is owned by the `pattern-mapper` capability's `plan:pre` step hook. Read `PLAN_PRE_HOOKS_JSON` and skip if no active step hook has `capId == "pattern-mapper"` and `ref.agent == "ecl-pattern-mapper"`. Also skip if no CONTEXT.md and no RESEARCH.md exist for this phase (nothing to extract file lists from).
...
- {context_path} (USER DECISIONS from /ecl:discuss-phase)
...
**If Mode is reviews:** REVIEWS.md is feedback input, not a hidden execution contract. /ecl:execute-phase primarily consumes PLAN.md plus the normal phase context, so every current actionable review finding must become visible in the relevant PLAN.md before planning can pass.
...
<downstream_consumer>
Output consumed by /ecl:execute-phase. Plans need:
...
`/ecl:plan-phase {N} --chunked` resumes from the last successfully committed plan.

**Intended for new or in-progress chunked runs.** To recover plans already written by a prior
*non-chunked* run, use step 6's "Add more plans" or proceed directly to `/ecl:execute-phase`
...
5. **Commit per-plan:**
   ```bash
   ecl_run query commit "docs(${PADDED_PHASE}): plan ${plan_id} (chunked)" --files "${PHASE_DIR}/${plan_id}-PLAN.md"
   ```
...
3. **Stop** — exit; user can re-run `/ecl:plan-phase {N}` to resume
...
**If "Split":** Use `/ecl:phase --insert` to create the sub-phases, then replan each.
...
**Stall detection:** If `issue_count >= prev_issue_count`:
  ...
  **If `stall_reentry_count >= 2`:**
    ...
    Suggest: "Consider resolving these issues manually or running `/ecl:debug` to investigate root causes."
...
**Read pass count:**
```bash
BOUNCE_PASSES=$(ecl_run query config-get workflow.plan_bounce_passes 2>/dev/null || echo "2")
BOUNCE_SCRIPT=$(ecl_run query config-get workflow.plan_bounce_script 2>/dev/null | jq -r '.' 2>/dev/null || true)
```

Display banner:
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 eCL ► BOUNCING PLANS (External Refinement)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
5. **Re-run plan checker on bounced plans:** Spawn ecl-plan-checker (same as step 10) on all modified plans. If a bounced plan fails the checker, restore original from its pre-bounce.md backup:
```
...
```bash
ecl_run query commit "refactor(${padded_phase}): bounce plans through external refinement" --files "${PHASE_DIR}/*-PLAN.md"
```
...
```bash
GATE_CFG=$(ecl_run query config-get workflow.context_coverage_gate 2>/dev/null || echo "true")
if [ "$GATE_CFG" != "false" ]; then
  GATE_RESULT=$(ecl_run query check.decision-coverage-plan "${PHASE_DIR}" "${CONTEXT_PATH}")
...
```bash
ecl_run query state.planned-phase --phase "${PHASE_NUMBER}" --name "${PHASE_NAME}" --plans "${PLAN_COUNT}"
```
...
```bash
ecl_run query roadmap.annotate-dependencies "${PHASE_NUMBER}"
```
...
```bash
ecl_run query commit "docs(${PADDED_PHASE}): create phase plan" --files "${PHASE_DIR}"/*-PLAN.md .planning/STATE.md .planning/ROADMAP.md
```
...
```bash
PLAN_POST_HOOKS_JSON=$(ecl_run loop render-hooks plan:post --raw)
PHASE_REQ_IDS=$(ecl_run query init.plan-phase "$PHASE" --pick phase_req_ids 2>/dev/null || echo TBD)
```
...
```bash
GATE_RESULT=$(ecl_run check ${hook.check.query} "${PHASE_DIR}" "${PHASE_REQ_IDS}" --raw)
CHECK_EXIT=$?
```
...
```
Skill(skill="ecl-execute-phase", args="${PHASE} --auto --no-transition ${ECL_WS}")
```
...
- **PHASE COMPLETE** → Display final summary:
  ...
  Next: /ecl:discuss-phase ${NEXT_PHASE} --auto ${ECL_WS}
  ```
- **GAPS FOUND / VERIFICATION FAILED** → Display result, stop chain:
  ...
  /ecl:execute-phase ${PHASE} ${ECL_WS}
  ```
...
/ecl:execute-phase {X} ${ECL_WS}

───────────────────────────────────────────────────────────────

**Also available:**
- cat .planning/phases/{phase-dir}/*-PLAN.md — review plans
- /ecl:plan-phase {X} --research — re-research first
- /ecl:review --phase {X} --all — peer review plans with external AIs
- /ecl:plan-phase {X} --reviews — replan incorporating review feedback
...
5. **Retry:** Restart Claude Code and run `/ecl:plan-phase` again

If freezes persist, try `--skip-research` to reduce the agent chain from 3 to 2 agents:
```
/ecl:plan-phase N --skip-research
```
</windows_troubleshooting>
...
- [ ] ecl-phase-researcher spawned with CONTEXT.md
- [ ] Existing plans checked
- [ ] ecl-planner spawned with CONTEXT.md + RESEARCH.md
- [ ] Plans created (PLANNING COMPLETE or CHECKPOINT handled)
- [ ] ecl-plan-checker spawned with CONTEXT.md
