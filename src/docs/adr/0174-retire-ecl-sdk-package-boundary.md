# ADR-0174: Retire @evolvconsulting/ecl-sdk package boundary — single-runtime collapse

- **Status:** Accepted (2026-05-23)
- **Date:** 2026-05-23
- **Tracking issue:** [#174](https://github.com/evolvconsulting/evolv-coder-lite/issues/174) — sub-issues #175–#197

## Supersedes

| ADR | What it said | Why it is superseded |
|-----|-------------|----------------------|
| [ADR-0005](0005-sdk-architecture-seam-map.md) | Established the SDK as a composition of explicit seam Modules with thin Adapters, with the SDK package itself as the composition root. | The SDK package boundary is being retired; the seam-Module vocabulary survives intact under a single `src/` tree inside `evolv-coder-lite-cc`. |
| [ADR-0007](0007-sdk-package-seam-module.md) | Defined one explicit SDK Package Seam Module for the `@evolvconsulting/ecl-sdk` → `@evolvconsulting/evolv-coder-lite` compatibility transition. | The transition scaffolding this Module owned (install-layout probing, legacy-asset discovery, compatibility diagnostics) is deleted when the SDK package boundary is retired. |
...
5. **External programmatic API surface is not a user requirement.** The user does not need `@evolvconsulting/ecl-sdk` to be installable by external consumers. The typed contract is internal. Retaining the package boundary would mean paying for SDK scaffolding in perpetuity to support a use case that does not exist.

## Decision

### 1. Single npm package

`evolv-coder-lite-cc` is the sole npm package. `@evolvconsulting/ecl-sdk` is retired. No external programmatic API is exposed. The typed contract is internal to `evolv-coder-lite-cc`.

### 2. Source shape — TypeScript canonical in `src/`, compiled to CJS in `dist/`

`src/` is the hand-authored source of truth for all Modules. `tsc` on `prepublishOnly` compiles `src/` to CJS in `dist/`. `bin/ecl-tools.cjs` becomes a thin shim that `require`s from `dist/`. No `synckit`, no worker pool, no generator scripts replacing tsc. The `.generated.cjs` artifacts in `bin/lib/` are deleted; their callers are updated to require from `dist/`.
...
| `src/cli/` | `ecl-tools` entrypoint code |
...
- **Opt-in file audit** — `ECL_AUDIT=1` env var or `config.audit.enabled: true` writes `DispatchEvent` records to `.planning/.ecl-trace.jsonl`. Args are excluded by default (privacy); `ECL_AUDIT_ARGS=1` opts in.
...
- **`tsc` build step added to `prepublishOnly`.** `evolv-coder-lite-cc` currently has no TypeScript compilation at the root. The build step is net-simpler than the existing generator infrastructure, but it is a new step in the publish path.
...
| 5 — Retire SDK package | Delete `sdk/` directory; delete `bin/ecl-sdk.js` shim and `bin/ecl-sdk` wrapper; delete `release-sdk.yml` and SDK steps in `release.yml`, `hotfix.yml`, `install-smoke.yml`. | ~2 |
...
Implementation is tracked in [#174 — sub-issues #175–#197](https://github.com/evolvconsulting/evolv-coder-lite/issues/174).
