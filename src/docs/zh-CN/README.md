...
- [接入现有代码库](tutorials/onboarding-an-existing-codebase.md) — 将 eCL Core 引入已有项目的代码库
...
- [处理快速临时任务](how-to/handle-quick-and-fast-tasks.md) — 使用 `/ecl-quick` 和 `/ecl-fast` 处理阶段循环之外的临时工作
...
- [探索与草图](how-to/spike-and-sketch.md) — 在提交计划之前，使用 `/ecl-spike` 和 `/ecl-sketch` 进行探索性工作
- [设计 UI 阶段](how-to/design-a-ui-phase.md) — 使用 UI 阶段循环处理前端和视觉工作
- [从追踪器 Issue 驱动 eCL](how-to/drive-ecl-from-a-tracker-issue.md) — 从 GitHub、Linear 或 Jira issue 启动一个阶段
- [从 eCL 2 迁移](how-to/migrate-from-ecl-2.md) — 将现有的 eCL 2 项目升级到 eCL Core
- [更新 eCL](how-to/update-ecl.md) — 重新运行安装程序以获取最新版本
...
- [CLI 工具](CLI-TOOLS.md) — `ecl-tools.cjs` 用于工作流和代理的编程式 API
...
- [上下文工程](explanation/context-engineering.md) — 上下文腐化如何形成，以及 eCL Core 如何防止它
...
- [讨论模式](workflow-discuss-mode.md) — `/ecl-discuss-phase` 的假设模式与访谈模式
- [上下文监控](context-monitor.md) — 上下文窗口监控钩子架构
- [Issue 驱动编排](issue-driven-orchestration.md) — 使用现有原语从追踪器 issue 驱动 eCL 的方案
