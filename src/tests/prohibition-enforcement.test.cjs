...
// "heavy half"). Requires the BUILT evolv-coder-lite/bin/lib/prohibition-enforcement.cjs — authored as
...
const ENFORCEMENT_LIB = path.join(__dirname, '..', 'evolv-coder-lite', 'bin', 'lib', 'prohibition-enforcement.cjs');
...
describe('prohibition-enforcement REAL runner end-to-end (#1259)', () => {
  ...
  test('a genuine non-vacuous passing node-test proven fail-first greens via the real runner + real prover', (t) => {
    ...
    // ECL_PROHIB_SUBJECT convention — it asserts its subject is clean. The clean runCheck run reads
    ...
    fs.writeFileSync(tf,
      ...
      "  const subject = fs.readFileSync(process.env.ECL_PROHIB_SUBJECT, 'utf-8');\n" +
  ...
  test('an eslint-IGNORED target does NOT green the lint-rule kind (vacuous-green guard, NEW-BL-01)', () => {
    ...
    const result = enforce.runProhibitionEnforcement(
      TEST_TIER,
      { kind: 'lint-rule', rule: 'local/no-source-grep', target: 'evolv-coder-lite/bin/lib/prohibition-enforcement.cjs', failFirst: true },
      { cwd: process.cwd() },
    );
  ...
  // behavior (eslint plugin load, ECL_PROHIB_SUBJECT convention, fail-first proof) was unproven at the
  ...
  test('FULL producer (real prover + real runner): node-test greens via ECL_PROHIB_SUBJECT — red on bad fixture, clean pass on clean subject (FF-03/FF-10)', (t) => {
    const enforce = require(ENFORCEMENT_LIB);
    const dir = createTempDir('prohib-full-node-green-');
    t.after(() => cleanup(dir));
    // A REAL negative test that honors the ECL_PROHIB_SUBJECT convention: it reads its subject and
    // asserts it is CLEAN. The default runner runs it with NO ECL_PROHIB_SUBJECT set -> the fixture
    // defaults to a clean in-dir subject -> passes non-vacuously. The default prover runs it with
    // ECL_PROHIB_SUBJECT=<bad fixture> -> the assertion fails -> RED -> fail-first proven.
    ...
    fs.writeFileSync(negTest,
      ...
      "const path = require('node:path');\n" +
      "test('guards the must-NOT: subject is clean', () => {\n" +
      "  const subjectPath = process.env.ECL_PROHIB_SUBJECT || path.join(__dirname, 'clean-subject.txt');\n" +
  ...
  test('COMPOSE (#1346): a prohibition projected WITH check_violation_fixture greens end-to-end through the DEFAULT prover+runner (zero hand-authoring)', (t) => {
    const enforce = require(ENFORCEMENT_LIB);
    const pc = require(path.join(__dirname, '..', 'evolv-coder-lite', 'bin', 'lib', 'probe-core.cjs'));
...
// demonstrate the `ECL_PROHIB_SUBJECT` subject-injection convention. Typed-result assertions only.
describe('prohibition-enforcement defaultProveFailFirst REAL prover (#1279)', () => {
  ...
  test('node-test: a negative test that goes RED against a known-bad ECL_PROHIB_SUBJECT is proven (FF-03 red direction)', (t) => {
    const enforce = require(ENFORCEMENT_LIB);
    const dir = createTempDir('prohib-ff-node-red-');
    t.after(() => cleanup(dir));
    // A negative test that HONORS the ECL_PROHIB_SUBJECT convention: it reads the subject path and
  ...
  test('node-test: an HONEST test + a non-existent violationFixture path is NOT proven (FF-05 fail-OPEN guard, #1314 Major 1)', (t) => {
    ...
    // loads cleanly) reads ECL_PROHIB_SUBJECT and fs.readFileSync's it. Point violationFixture at a
  ...
  test('node-test: a RELATIVE violationFixture is resolved against cwd (existence guard matches the child, #1314 Major 1)', (t) => {
    ...
    t.after(() => cleanup(dir));
    // The fixture is named RELATIVELY; the prover runs with cwd=dir and sets ECL_PROHIB_SUBJECT to the
