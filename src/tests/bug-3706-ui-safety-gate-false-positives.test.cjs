 ...
 * Invoked as: printf '%s' "$PHASE_SECTION" | node "${ECL_REPO_ROOT}/bin/lib/ui-safety-gate.cjs"
...
const AUTONOMOUS_PATH = path.join(__dirname, '..', 'evolv-coder-lite', 'workflows', 'autonomous.md');
...
describe('Workflow .md structural guard (#3718)', () => {
  ...
  for (const [label, filePath] of [
    ...
    test(`${label} must invoke ui-safety-gate.cjs via stdin, anchored to the eCL install dir`, () => {
      ...
      // gate's OWN resolution, not §1's unrelated RUNTIME_DIR usage for ecl-tools.
      const gi = content.indexOf('ui-safety-gate.cjs');
      const region = content.slice(Math.max(0, gi - 800), gi + 200);

      // #448: the helper ships inside the eCL package, so it must be resolved
      // against the eCL install dir (RUNTIME_DIR), NOT the consuming project's
      // git root — otherwise the node call fails and the gate silently no-ops.
      assert.ok(
        region.includes('RUNTIME_DIR'),
        `${label}: UI gate must resolve ui-safety-gate.cjs against RUNTIME_DIR (the eCL install dir), not the consuming project's git root (#448)`
      );
      assert.ok(
        !region.includes('ECL_REPO_ROOT'),
        `${label}: UI gate must NOT anchor the helper to ECL_REPO_ROOT (the consuming project's git root) — that silently no-ops in installed repos (#448)`
      );
...
describe('UI gate resolves the helper against RUNTIME_DIR, not the consuming repo (#448)', () => {
  ...
  test('UI text is detected (HAS_UI=0) from a project without bin/lib', () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-consuming-'));
    try {
  ...
  test('UI gate found via evolv-coder-lite/bin/lib/ in installed layout (no root bin/lib/)', () => {
    // Regression for #448: installed RUNTIME_DIR has evolv-coder-lite/bin/lib/ but NOT root bin/lib/.
    // The probe must find the helper at the installed path, not silently no-op to HAS_UI=0.
    const fakeRuntime = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-installed-rt-'));
    const consumingProject = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-consuming-'));
    try {
      const installedLibDir = path.join(fakeRuntime, 'evolv-coder-lite', 'bin', 'lib');
      fs.mkdirSync(installedLibDir, { recursive: true });
      fs.copyFileSync(
        path.join(REPO_ROOT, 'evolv-coder-lite', 'bin', 'lib', 'ui-safety-gate.cjs'),
      ...
      assert.strictEqual(res.status, 0, `bash failed: ${res.stderr}`);
      assert.strictEqual(res.stdout.trim(), '0',
        'helper must be found via evolv-coder-lite/bin/lib/ in installed layout and report UI present');
    } finally {
      cleanup(fakeRuntime);
