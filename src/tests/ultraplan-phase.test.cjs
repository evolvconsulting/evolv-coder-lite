 ...
 * This command offloads eCL plan phase to Claude Code's ultraplan cloud infrastructure.
 */
...
const CMD_PATH = path.join(__dirname, '..', 'commands', 'ecl', 'ultraplan-phase.md');
const WF_PATH = path.join(__dirname, '..', 'evolv-coder-lite', 'workflows', 'ultraplan-phase.md');

// ─── File Existence ────────────────────────────────────────────────────────────

describe('ultraplan-phase file existence', () => {
  test('command file exists', () => {
    assert.ok(fs.existsSync(CMD_PATH), 'commands/ecl/ultraplan-phase.md should exist');
  });

  test('workflow file exists', () => {
    assert.ok(fs.existsSync(WF_PATH), 'evolv-coder-lite/workflows/ultraplan-phase.md should exist');
  });
});
...
describe('ultraplan-phase command frontmatter', () => {
  ...
  test('has correct name field', () => {
    assert.match(content, /^name:\s*ecl:ultraplan-phase$/m);
  });
...
describe('ultraplan-phase command references', () => {
  ...
  test('references the ultraplan-phase workflow', () => {
    assert.ok(
      content.includes('@~/.claude/evolv-coder-lite/workflows/ultraplan-phase.md'),
...
describe('ultraplan-phase workflow runtime gate', () => {
  ...
  test('error message references /ecl-plan-phase as local alternative', () => {
    assert.ok(
      content.includes('ecl:plan-phase') || content.includes('ecl-plan-phase'),
      'error message should direct users to /ecl-plan-phase as the local alternative'
    );
  });
});
...
describe('ultraplan-phase workflow initialization', () => {
  ...
  test('handles missing .planning directory', () => {
    assert.ok(
      content.includes('ecl-new-project') || content.includes('/ecl-new-project') || content.includes('ecl:new-project'),
      'workflow should direct user to /ecl-new-project when .planning is missing'
    );
  });
});
...
describe('ultraplan-phase workflow return path', () => {
  ...
  test('directs user to run /ecl-import --from after ultraplan completes', () => {
    assert.ok(content.includes('ecl-import') || content.includes('ecl:import'), 'workflow must direct user to run /ecl:import --from with the saved file path');
  });

  test('mentions the --from flag for ecl-import', () => {
    assert.ok(content.includes('--from'), 'workflow should reference /ecl-import --from <file-path>');
  });

  test('return-path instructions appear before the /ultraplan trigger', () => {
    const ultraplanTriggerIndex = content.indexOf('/ultraplan');
    const importIndex = content.indexOf('ecl-import');
    assert.ok(
      importIndex < ultraplanTriggerIndex,
      'return-path instructions (ecl-import) must appear before /ultraplan trigger so they are visible in scroll-back'
    );
  });
});
...
describe('ultraplan-phase workflow isolation', () => {
  ...
  test('does NOT directly write PLAN.md files', () => {
    assert.ok(
      ...
      'workflow must NOT directly write PLAN.md — delegates to /ecl-import --from'
    );
  });
