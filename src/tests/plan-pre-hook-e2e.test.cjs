...
// ─── Subprocess helper (isolated env, no ambient ECL_ vars) ───────────────────
...
/**
 * Run ecl-tools via spawnSync. Returns { status, stdout, stderr }.
...
describe('plan:pre surface cluster filter — ui disabled, tdd+intel active', () => {
  ...
  before(() => {
    ...
    // .ecl-surface.json disabling UI cluster in same dir (config-dir = tmpDir)
    fs.writeFileSync(
      path.join(tmpDir, '.ecl-surface.json'),
      JSON.stringify({
        baseProfile: 'full',
        disabledClusters: ['ui'],
        explicitAdds: [],
