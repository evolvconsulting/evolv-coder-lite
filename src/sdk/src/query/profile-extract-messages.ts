/**
 * `extract-messages` — parity with `evolv-coder-lite/bin/lib/profile-pipeline.cjs` `cmdExtractMessages`.
...
/**
 * Port of `cmdExtractMessages` — same JSON result as `ecl-tools extract-messages` (stdout object;
...
export async function runExtractMessages(
  ...
  const tmpDir = mkdtempSync(join(tmpdir(), 'ecl-pipeline-'));
