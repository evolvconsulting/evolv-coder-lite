...
/**
 * Governance tests for the evolv-coder-lite repository root layout.
 *
 ...
 * AGENTS.md to process.cwd() when `ecl install copilot` is run inside a repo
 * checkout. If that file is ever committed, editors and AI tools that auto-load
 * repo-root instruction files will silently pick up eCL's installer-generated
...
test('repo-layout: root AGENTS.md is absent — no ad-hoc AI instruction file committed alongside CONTEXT.md', () => {
  ...
  assert.equal(
    ...
    [
      ...
      'This file is written by `ecl install copilot` (bin/install.js, local Copilot path, issue #786)',
      ...
      'Run `ecl uninstall copilot` to remove the artefact, then verify it is gitignored',
