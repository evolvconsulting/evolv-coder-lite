...
// assert the flat layout (>= 60 top-level ecl-* entries, concrete skills discoverable).
...
describe('issue-69: applySurface preserves nested skill layout (no re-flatten)', () => {
  ...
  test('cline global full: applySurface keeps 6 router dirs and nested ecl-ns-manage/skills/help/SKILL.md', (t) => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-69-surface-'));
    ...
    assert.strictEqual(
      ...
      6,
      `Install must produce exactly 6 ecl-* top-level dirs (routers). Got ${topLevelAfterInstall.length}: [${topLevelAfterInstall.join(', ')}]`,
    );
    assert.ok(
      fs.existsSync(path.join(skillsDir, 'ecl-ns-workflow', 'skills', 'plan-phase', 'SKILL.md')),
      'After install: ecl-ns-workflow/skills/plan-phase/SKILL.md must exist',
    );
    ...
    assert.strictEqual(
      ...
      6,
      `After applySurface: expected exactly 6 ecl-* top-level dirs (routers only). Got ${topLevelAfterSurface.length}: [${topLevelAfterSurface.join(', ')}]. ` +
    ...
    assert.ok(
      fs.existsSync(path.join(skillsDir, 'ecl-ns-workflow', 'skills', 'plan-phase', 'SKILL.md')),
      'After applySurface: ecl-ns-workflow/skills/plan-phase/SKILL.md must still exist (nested layout preserved)',
    );
    ...
    assert.ok(
      ...
      'After applySurface: ecl-plan-phase/ must NOT exist at top level (#69 re-flatten regression guard)',
    );
  });
  ...
  test('claude global full: install produces flat layout and applySurface preserves it (#924)', (t) => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-924-69-'));
    t.after(() => { try { cleanup(tmpDir); } catch { /* best-effort */ } });
    ...
    // Install must produce FLAT layout (>= 60 ecl-* dirs)
    const topLevelAfterInstall = fs.readdirSync(skillsDir).filter((n) => n.startsWith('ecl-'));
    assert.ok(
      topLevelAfterInstall.length >= 60,
      `Claude install must produce >= 60 ecl-* top-level dirs (flat, #924). Got ${topLevelAfterInstall.length}.`,
    );

    // ecl-plan-phase must be directly at top level
    assert.ok(
      fs.existsSync(path.join(skillsDir, 'ecl-plan-phase', 'SKILL.md')),
      'After claude install: ecl-plan-phase/SKILL.md must be at top level (flat layout, #924)',
    );

    // No nested skills/ subdirs under ecl-ns-* in Claude
    assert.ok(
      !fs.existsSync(path.join(skillsDir, 'ecl-ns-workflow', 'skills')),
      'After claude install: ecl-ns-workflow/skills/ must NOT exist (flat layout, no nesting, #924)',
    );
    ...
    assert.ok(
      ...
      `After applySurface: claude must still have >= 60 ecl-* dirs (flat preserved). Got ${topLevelAfterSurface.length}.`,
    );

    assert.ok(
      fs.existsSync(path.join(skillsDir, 'ecl-plan-phase', 'SKILL.md')),
      'After applySurface: ecl-plan-phase/SKILL.md must remain at top level (#924)',
    );
  });
});
