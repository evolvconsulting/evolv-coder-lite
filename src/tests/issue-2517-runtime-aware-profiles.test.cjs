 ...
 * developer's real `~/.ecl/defaults.json` cannot bleed into assertions
...
// Without this, a developer's real `~/.ecl/defaults.json` (e.g. one with
...
function isolateHome() {
  ...
  _origGsdHome = process.env.ECL_HOME;
  _isolatedHome = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-home-iso-'));
  process.env.HOME = _isolatedHome;
  process.env.ECL_HOME = _isolatedHome;
}
function restoreHome() {
  if (_origHome === undefined) delete process.env.HOME; else process.env.HOME = _origHome;
  if (_origGsdHome === undefined) delete process.env.ECL_HOME; else process.env.ECL_HOME = _origGsdHome;
...
describe('issue #2517: backwards compat — no runtime key set', () => {
  ...
  test('adaptive profile still works without runtime (#1713/#1806)', () => {
    ...
    assert.strictEqual(resolveModelInternal(tmpDir, 'ecl-planner'), 'opus');
    assert.strictEqual(resolveModelInternal(tmpDir, 'ecl-codebase-mapper'), 'haiku');
  });
});
...
describe('issue #2517: runtime "codex" — Codex tier resolution', () => {
  ...
  test('sonnet tier -> gpt-5.4 model; heavy-tier agent -> xhigh effort on codex', () => {
    ...
    assert.strictEqual(resolveModelInternal(tmpDir, 'ecl-roadmapper'), 'gpt-5.4');
...
describe('issue #2517: precedence chain', () => {
  ...
  test('per-agent model_overrides wins over runtime tier resolution', () => {
    writeConfig(tmpDir, {
      runtime: 'codex',
      model_profile: 'quality',
      model_overrides: { 'ecl-planner': 'gpt-5.4-mini' },
    });
    assert.strictEqual(resolveModelInternal(tmpDir, 'ecl-planner'), 'gpt-5.4-mini');
  });
  ...
  test('model_profile_overrides[runtime][tier] beats built-in defaults', () => {
    ...
    // ecl-planner quality -> opus -> overridden to gpt-5-pro
    assert.strictEqual(resolveModelInternal(tmpDir, 'ecl-planner'), 'gpt-5-pro');
    // haiku not overridden — fall back to spec defaults
    // ecl-codebase-mapper quality -> sonnet -> gpt-5.4
    assert.strictEqual(resolveModelInternal(tmpDir, 'ecl-codebase-mapper'), 'gpt-5.4');
  });
  ...
  test('partial profile_overrides — only opus overridden, sonnet uses default', () => {
    ...
    // ecl-planner balanced -> opus -> overridden to gpt-5-pro
    assert.strictEqual(resolveModelInternal(tmpDir, 'ecl-planner'), 'gpt-5-pro');
    // ecl-roadmapper balanced -> sonnet -> spec default
    assert.strictEqual(resolveModelInternal(tmpDir, 'ecl-roadmapper'), 'gpt-5.4');
  });
  ...
  test('per-agent override beats profile override beats default', () => {
    writeConfig(tmpDir, {
      ...
      model_overrides: { 'ecl-planner': 'custom-model' },
    });
...
describe('issue #2517: unknown runtime + safe fallback', () => {
  ...
  test('unknown runtime falls back to Claude-alias safe default (no Codex IDs leaked)', () => {
    ...
    const resolved = resolveModelInternal(tmpDir, 'ecl-planner');
...
describe('issue #2517: install end-to-end — per-project config reaches Codex TOML (finding #1)', () => {
  ...
  const prevTestMode = process.env.ECL_TEST_MODE;
  process.env.ECL_TEST_MODE = '1';
  const installMod = require('../bin/install.js');
  if (prevTestMode === undefined) delete process.env.ECL_TEST_MODE;
  else process.env.ECL_TEST_MODE = prevTestMode;
  ...
  test('readGsdRuntimeProfileResolver picks up runtime from .planning/config.json', () => {
    // No ~/.ecl/defaults.json (HOME is isolated tmpdir). Per-project config alone
  ...
  test('per-project config wins over global ~/.ecl/defaults.json', () => {
    fs.mkdirSync(path.join(_isolatedHome, '.ecl'), { recursive: true });
    fs.writeFileSync(
      path.join(_isolatedHome, '.ecl', 'defaults.json'),
  ...
  test('inline require paths resolve relative to install.js __dirname (finding #6)', () => {
    ...
    const libDir = path.join(installDir, '..', 'evolv-coder-lite', 'bin', 'lib');
...
describe('issue #2612: runtime "gemini" — Gemini tier resolution', () => {
  ...
  test('opus tier -> gemini-3.1-pro-preview', () => {
    ...
    assert.strictEqual(resolveModelInternal(tmpDir, 'ecl-planner'), 'gemini-3.1-pro-preview');
  });

  test('sonnet tier -> gemini-3-flash', () => {
    writeConfig(tmpDir, { runtime: 'gemini', model_profile: 'balanced' });
    assert.strictEqual(resolveModelInternal(tmpDir, 'ecl-roadmapper'), 'gemini-3-flash');
  });
...
describe('issue #2612: runtime "qwen" — Qwen tier resolution', () => {
  ...
  test('opus tier -> qwen3-max-2026-01-23', () => {
    ...
    assert.strictEqual(resolveModelInternal(tmpDir, 'ecl-planner'), 'qwen3-max-2026-01-23');
  });

  test('sonnet tier -> qwen3-coder-plus', () => {
    writeConfig(tmpDir, { runtime: 'qwen', model_profile: 'balanced' });
    assert.strictEqual(resolveModelInternal(tmpDir, 'ecl-roadmapper'), 'qwen3-coder-plus');
  });
...
describe('issue #2612: runtime "opencode" — OpenCode tier resolution', () => {
  ...
  test('sonnet tier -> anthropic/claude-sonnet-4-6', () => {
    ...
    assert.strictEqual(resolveModelInternal(tmpDir, 'ecl-roadmapper'), 'anthropic/claude-sonnet-4-6');
  });
...
describe('issue #2612: runtime "copilot" — Copilot tier resolution', () => {
  ...
  test('sonnet tier -> claude-sonnet-4-6', () => {
    ...
    assert.strictEqual(resolveModelInternal(tmpDir, 'ecl-roadmapper'), 'claude-sonnet-4-6');
  });
...
describe('issue #2612: partial override merge for new Group A runtimes', () => {
  ...
  test('gemini.opus override wins; sonnet and haiku use built-in defaults', () => {
    ...
    // opus is overridden
    assert.strictEqual(resolveModelInternal(tmpDir, 'ecl-planner'), 'gemini-3-ultra');
    // sonnet not overridden — built-in default (quality -> sonnet for ecl-codebase-mapper)
    assert.strictEqual(resolveModelInternal(tmpDir, 'ecl-codebase-mapper'), 'gemini-3-flash');
  });
  ...
  test('qwen.opus override wins; sonnet and haiku use built-in defaults', () => {
    ...
    // opus is overridden
    assert.strictEqual(resolveModelInternal(tmpDir, 'ecl-planner'), 'qwen3-max-custom');
    // sonnet not overridden — quality -> sonnet for ecl-codebase-mapper
    assert.strictEqual(resolveModelInternal(tmpDir, 'ecl-codebase-mapper'), 'qwen3-coder-plus');
  });
  ...
  test('copilot.haiku override wins; opus and sonnet still use built-in defaults', () => {
    ...
    // ecl-codebase-mapper budget -> haiku -> overridden
    assert.strictEqual(resolveModelInternal(tmpDir, 'ecl-codebase-mapper'), 'claude-haiku-4-6');
    // ecl-planner budget -> sonnet -> built-in default
    assert.strictEqual(resolveModelInternal(tmpDir, 'ecl-planner'), 'claude-sonnet-4-6');
  });
});
