...
eCL follows [Semantic Versioning 2.0.0](https://semver.org/) with three release tiers mapped to npm dist-tags.

## Release Tiers

| Tier | What ships | Version format | npm tag | Branch | Install |
|------|-----------|---------------|---------|--------|---------|
| **Patch** | Bug fixes only | `1.27.1` | `latest` | `hotfix/1.27.1` | `npx @evolvconsulting/evolv-coder-lite@latest` |
| **Minor** | Fixes + enhancements | `1.28.0` | `latest` (after RC) | `release/1.28.0` | `npx @evolvconsulting/evolv-coder-lite@next` (RC) |
| **Major** | Fixes + enhancements + features | `2.0.0` | `latest` (after beta) | `release/2.0.0` | `npx @evolvconsulting/evolv-coder-lite@next` (beta) |
...
|-----|---------|-------------|
| `latest` | Stable production release | `npm install @evolvconsulting/evolv-coder-lite` (default) |
| `next` | Pre-release (RC or beta) | `npm install @evolvconsulting/evolv-coder-lite@next` (opt-in) |
...
3. Trigger `hotfix.yml` with `action=finalize`. The workflow:
   ...
   - Builds SDK, bundles `sdk-bundle/ecl-sdk.tgz` inside the CC tarball (parity with `release-sdk.yml`).
...
Active while the `@evolvconsulting/ecl-sdk` npm token is unavailable; bundles the SDK inside the CC tarball.
...
4. Test the RC: `npx @evolvconsulting/evolv-coder-lite@next`
...
npm dist-tag ls @evolvconsulting/evolv-coder-lite
```
