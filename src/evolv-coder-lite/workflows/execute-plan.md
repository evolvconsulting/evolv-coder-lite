...
@~/.claude/evolv-coder-lite/references/git-integration.md
</required_reading>
...
INIT=$(ecl_run query init.execute-phase "${PHASE}")
...
# config settings can be fetched via ecl-tools.cjs query config-get if needed
```
...
INLINE_THRESHOLD=$(ecl_run query config-get workflow.inline_plan_threshold 2>/dev/null || echo "2")
...
2. Per segment:
   - Subagent route: spawn ecl-executor for assigned tasks only. Prompt: task range, plan path, read full plan for context, execute assigned tasks, track deviations, NO SUMMARY/commit. Track via agent protocol.
...
```bash
ecl_run query phases.list --type summaries --raw
...
Apply deviation rules from the ecl-executor agent definition (single source of truth):
...
See `~/.claude/evolv-coder-lite/references/tdd.md` for structure.
</tdd_plan_execution>
...
See ~/.claude/evolv-coder-lite/references/checkpoints.md for details.
</step>
...
```bash
NODE_REPAIR=$(ecl_run query config-get workflow.node_repair 2>/dev/null || echo "true")
```

If `NODE_REPAIR` is `true`: invoke `@./.claude/evolv-coder-lite/workflows/node-repair.md` with:
...
If user_setup exists: create `{phase}-USER-SETUP.md` using template `~/.claude/evolv-coder-lite/templates/user-setup.md`. Per service: env vars table, account setup checklist, dashboard config, local dev notes, verification commands. Status "Incomplete". Set `USER_SETUP_CREATED=true`. If empty/missing: skip.
</step>
...
Create `{phase}-{plan}-SUMMARY.md` at `.planning/phases/XX-name/`. Use `~/.claude/evolv-coder-lite/templates/summary.md`.
...
Update STATE.md using ecl-tools.cjs query (or legacy ecl-tools) state mutations:
...
if [ "$IS_WORKTREE" != "true" ]; then
  ...
  ecl_run query state.advance-plan

  # Recalculate progress bar from disk state
  ecl_run query state.update-progress

  # Record execution metrics
  ecl_run query state.record-metric \
...
ecl_run query state.add-decision \
  --phase "${PHASE}" --summary-file "${DECISION_TEXT_FILE}" --rationale-file "${RATIONALE_FILE}"

# Add blockers if any found
ecl_run query state.add-blocker --text-file "${BLOCKER_TEXT_FILE}"
```
</step>

<step name="update_session_continuity">
Update session info using ecl-tools.cjs query (or legacy ecl-tools):
...
if [ "$IS_WORKTREE" != "true" ]; then
  ...
  ecl_run query roadmap.update-plan-progress "${PHASE}"
fi
```
...
```bash
ecl_run query requirements.mark-complete ${REQ_IDS}
```
...
if [ "$IS_WORKTREE" = "true" ]; then
  ecl_run query commit "docs({phase}-{plan}): complete [plan-name] plan" --files .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/REQUIREMENTS.md
else
  ecl_run query commit "docs({phase}-{plan}): complete [plan-name] plan" --files .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/STATE.md .planning/ROADMAP.md .planning/REQUIREMENTS.md
fi
```
</step>
...
```bash
ecl_run query commit "" --files .planning/codebase/*.md --amend
```
</step>
...
|-----------|-------|--------|
| summaries < plans | **A: More plans** | Find next PLAN without SUMMARY — skip any plan whose `plan_id` matches a non-terminal async-job manifest (`external_job_waiting`; see `identify_plan`). Yolo: auto-continue. Interactive: show next plan, suggest `/ecl:execute-phase {phase}` + `/ecl:verify-work`. STOP here. |
| summaries = plans, current < highest phase | **B: Phase done** | Show completion, suggest `/ecl:plan-phase {Z+1}` + `/ecl:verify-work {Z}` + `/ecl:discuss-phase {Z+1}` |
| summaries = plans, current = highest phase | **C: Milestone done** | Show banner, suggest `/ecl:complete-milestone` + `/ecl:verify-work` + `/ecl-add-phase` |
