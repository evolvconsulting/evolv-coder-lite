 ...
 *
 * Seam scope per #3596:
 *   - hooks/ecl-prompt-guard.js          — stdin/stdout JSON contract
 *   - hooks/ecl-read-injection-scanner.js
 *   - evolv-coder-lite/bin/lib/security.cjs      — sanitizer + validators
 *   - evolv-coder-lite/bin/lib/workstream-name-policy.cjs
 *   - evolv-coder-lite/bin/ecl-tools.cjs CLI     — full-stack contract
 *
 ...
 *      because eCL itself uses that tag as legitimate prompt scaffolding.
...
const PROMPT_GUARD_HOOK = path.join(REPO_ROOT, 'hooks', 'ecl-prompt-guard.js');
const READ_SCANNER_HOOK = path.join(REPO_ROOT, 'hooks', 'ecl-read-injection-scanner.js');
...
const {
  ...
  validatePhaseNumber,
  validateFieldName,
} = require('../evolv-coder-lite/bin/lib/security.cjs');
const {
  toWorkstreamSlug,
  hasInvalidPathSegment,
  isValidActiveWorkstreamName,
} = require('../evolv-coder-lite/bin/lib/workstream-name-policy.cjs');
...
function sentinelPath(label) {
  return path.join(
    os.tmpdir(),
    `ecl-3596-sentinel-${label}-${process.pid}-${Date.now()}`,
  );
}
...
describe('CLI: hostile workstream names cannot escape or execute', () => {
  let tmpDir;
  beforeEach(() => { tmpDir = createTempGitProject('ecl-3596-ws-'); });
  ...
  test('--ws traversal value is rejected before any planning IO', () => {
    const escape = path.join(tmpDir, '..', '..', '..', 'ecl-3596-traverse-marker');
...
describe('CLI: fake-token env values are never echoed back', () => {
  let tmpDir;
  beforeEach(() => { tmpDir = createTempGitProject('ecl-3596-secret-'); });
  afterEach(() => { cleanup(tmpDir); });
  ...
  test('unknown subcommand error contains no env token values', () => {
    ...
    const r = runCli(['phase', 'this-sub-does-not-exist'], {
      ...
      env: {
        ...
        ECL_SECRET_AAAK: 'aaak_v1_should_never_appear',
      },
    });
...
// ─── Hook: ecl-prompt-guard advisory contract ───────────────────────────────

describe('ecl-prompt-guard: hostile .planning/ writes are advised, not blocked', () => {
...
// ─── Hook: ecl-read-injection-scanner advisory contract ─────────────────────

describe('ecl-read-injection-scanner: hostile reads are flagged with severity', () => {
  ...
  test('REGRESSION GUARD: bare <instructions> tag is NOT flagged (intentional whitelist)', () => {
    ...
    //   "Note: <instructions> is excluded — eCL uses it as legitimate prompt structure"
...
describe('sanitizeForPrompt: fake boundary tags are replaced, not echoed', () => {
  ...
  test('REGRESSION GUARD: <instructions> tag survives sanitization (legitimate use)', () => {
    // Mirrors the read-scanner whitelist: <instructions> is eCL's own
    ...
    assert.match(out, /<instructions>do the work<\/instructions>/,
      '<instructions> is eCL prompt scaffolding — must survive sanitizer (PINNED)');
  });
});
...
describe('validatePath: hostile path values are rejected before write', () => {
  let tmpDir;
  beforeEach(() => { tmpDir = createTempGitProject('ecl-3596-path-'); });
  afterEach(() => { cleanup(tmpDir); });
  ...
  test('symlink escaping the base is rejected', () => {
    ...
    const outside = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-3596-escape-'));
...
// and hooks/ecl-read-injection-scanner.js (inlined for hook independence).
  ...
  test('every canonical MARKDOWN_LINK_PATTERN source string appears in the hook source', () => {
    ...
    for (const entry of MARKDOWN_LINK_PATTERNS) {
      ...
      assert.ok(
        ...
        `Hook ecl-read-injection-scanner.js must contain pattern source: ${src}`,
      );
    }
  });
});
