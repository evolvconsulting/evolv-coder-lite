 ...
 *
 * @param {string} skillsDir        directory that contains the ecl-STEM sub-dirs
 * @param {Set<string>} retainedNames set of directory names to keep (e.g. 'ecl-help')
 * @param {string} prefix           eCL dir prefix, e.g. 'ecl-' (or '' for Hermes)
...
function _syncGsdDir(stagedDir, destDir, kind, manifest) {
  ...
  if (kindName === 'skills') {
    ...
    // Remove ecl-only files from dest that aren't in staged set
    // For commands dir: all .md files are ecl skills
    // For agents dir: only ecl-* files
