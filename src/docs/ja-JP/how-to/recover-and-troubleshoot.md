...
**前提条件:** eCL Core がインストール済みであること。インストールの問題については、[ランタイムへのインストール](install-on-your-runtime.md)を参照してください。
...
eCL は新鮮なコンテキストを前提に設計されています。すべてのサブエージェントはすでにクリーンな 200k ウィンドウを取得します。メインセッションは時間とともに劣化します。プッシュし続けるのではなく、クリアして再開することが正しい対処法です。
...
`.planning/PROJECT.md` がすでに存在します。`/ecl-new-project` は安全チェックです。本当に最初からやり直したい場合は、まず `.planning/` ディレクトリを削除します。

```bash
rm -rf .planning/
```

その後 `/ecl-new-project` を再実行します。
...
コンテキストウィンドウ使用率ガードを調査します。60% で警告、70% でクリティカル。警告閾値を超えている場合は、次の主要なコマンドを開始する前に `/clear` を実行してから `/ecl-resume-work` を実行してください。
...
eCL の `ecl-executor` サブエージェントには書き込み可能な Bash アクセスが必要です。`~/.claude/settings.json` の `permissions.allow` に必要なパターンを追加します。最低限：
...
これは複数のエージェントが同時にビルドツールをトリガーすることで発生します。eCL は v1.26 以降、これを自動的に処理します。古いバージョンを使用している場合、またはまだ競合が見られる場合は、並行実行を無効にします。
...
Claude Code の既知の分類バグで、作業が成功したのに失敗と報告される場合があります。eCL のオーケストレーターは実際の出力をスポットチェックしますが、不一致が見られる場合はコミットが真実です。
...
計画前に `/ecl-discuss-phase N` を実行します。プランの品質問題のほとんどは、`CONTEXT.md` があれば防げた前提から生じます。

```bash
/ecl-discuss-phase 1
```

完全なセッションを開始せずに eCL が現在行っている前提を確認するには：
...
`/ecl-execute-phase` を再実行しないでください。対象を絞った修正には `/ecl-quick` を使用します。

```bash
/ecl-quick "Fix the login button not responding on mobile Safari"
```

または `/ecl-verify-work N` を使って UAT を通じて体系的に問題を特定・修正します。

### コマンドが「Spawning…」でフリーズしているように見える場合

待ってください。eCL サブエージェントは別のコンテキストウィンドウで動作します。その作業は進行中の間、親セッションからは見えません。スポーン行の liveness ノートがこれが期待される動作であることを確認しています。リサーチと計画エージェントは通常 1〜5 分かかります。大きなフェーズでは検証エージェントがさらに時間がかかる場合があります。
...
`/ecl-forensics` はポストモーテム調査を実行します：git 履歴の異常、成果物の整合性、STATE.md の一貫性、未コミットの作業、孤立したワークツリー。レポートを `.planning/forensics/` に書き込み、推奨される修復手順を提示します。読み取り専用であり、プロジェクトファイルを変更することはありません。

### フェーズまたはプランをロールバックする必要がある場合

```bash
/ecl-undo --phase 03          # フェーズ 3 のすべてのコミットをロールバックする
/ecl-undo --plan 03-02        # フェーズ 3 のプラン 02 のコミットをロールバックする
/ecl-undo --last 5            # 最近の 5 件の eCL コミットからインタラクティブに選ぶ
```

`/ecl-undo` はロールバック前に依存するフェーズを確認し、常に確認ゲートを表示します。

---

## インストールとアップデートの問題

### インストール後に eCL が認識されない場合

ランタイムを再起動してください。eCL はランタイムのコマンドディレクトリ（例：`~/.claude/commands/ecl/`）にスラッシュコマンドをインストールします。ほとんどのランタイムは起動時にのみ新しいコマンドを検出します。
...
v1.17 以降、インストーラはローカルで変更されたファイルを `ecl-local-patches/` にバックアップします。変更を再適用します。
...
npm の障害やネットワーク制限のために `npx @evolvconsulting/evolv-coder-lite` が失敗する場合は、`docs/manual-update.md` に npm アクセスなしで動作するステップバイステップの手動アップデート手順があります。

定期的なアップデートについては、[eCL のアップデート](update-ecl.md)を参照してください。
...
| 問題 | 解決策 |
|---------|---------|
| コンテキストを失った、または新しいセッション | `/ecl-resume-work` または `/ecl-progress` |
| 次のステップがわからない | `/ecl-progress --next` |
| フェーズがうまくいかなかった | `/ecl-undo --phase NN`、その後再計画する |
| 何かが壊れた | `/ecl-debug "description"`（修正なしの分析は `--diagnose` を追加） |
| STATE.md が同期していない | `state validate` その後 `state sync` |
| `.planning/` の整合性が不確か | `/ecl-health`、その後 `/ecl-health --repair` |
| ワークフロー状態が破損しているように見える | `/ecl-forensics` |
| 対象を絞った素早い修正 | `/ecl-quick` |
| プランがビジョンと一致しない | `/ecl-discuss-phase N` その後再計画する |
| コストが高くなっている | `/ecl-config --profile budget` と `/ecl-settings` でエージェントをオフにする |
| アップデートがローカルの変更を壊した | `/ecl-update --reapply` |
| セッションのサマリーが必要 | `/ecl-pause-work --report` |
| 並行実行のビルドエラー | eCL をアップデートするか `parallelization.enabled: false` を設定する |
