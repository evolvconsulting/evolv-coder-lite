 ...
 *
 * The schema (evolv-coder-lite/bin/shared/model-catalog.json `profiles` array) defines 5 valid
...
const SETTINGS_PATH = path.join(REPO_ROOT, 'evolv-coder-lite', 'workflows', 'settings.md');
const CATALOG_PATH = path.join(REPO_ROOT, 'evolv-coder-lite', 'bin', 'shared', 'model-catalog.json');
