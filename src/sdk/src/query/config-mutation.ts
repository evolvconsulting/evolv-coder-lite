 ...
 *
 * Ported from evolv-coder-lite/bin/lib/config.cjs.
...
// with evolv-coder-lite/bin/lib/config-schema.cjs by a CI parity test (#2653).
 ...
 * @returns QueryResult matching ecl-tools `config-set` JSON: `{ updated, key, value, previousValue }`
...
export const configSet: QueryHandler = async (args, projectDir, workstream) => {
  ...
  // config.cjs:362-370 — without this, `ecl-sdk query config-set brave_search XXX`
...
export const configNewProject: QueryHandler = async (args, projectDir, workstream) => {
  ...
  // D11: Load global defaults from ~/.ecl/defaults.json if present
  const homeDir = homedir();
  let globalDefaults: Record<string, unknown> = {};
  try {
    const defaultsPath = join(homeDir, '.ecl', 'defaults.json');
  ...
  const hasBraveSearch = !!(process.env.BRAVE_API_KEY || existsSync(join(homeDir, '.ecl', 'brave_api_key')));
  const hasFirecrawl = !!(process.env.FIRECRAWL_API_KEY || existsSync(join(homeDir, '.ecl', 'firecrawl_api_key')));
  const hasExaSearch = !!(process.env.EXA_API_KEY || existsSync(join(homeDir, '.ecl', 'exa_api_key')));
