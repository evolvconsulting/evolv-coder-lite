...
describe('MODEL_PROFILES', () => {
  test('contains every shipped ecl agent file on disk (#3229)', () => {
...
describe('VALID_PROFILES', () => {
  ...
  test('includes all MODEL_PROFILES keys plus inherit', () => {
    const fromData = Object.keys(MODEL_PROFILES['ecl-planner']);
...
describe('getAgentToModelMapForProfile', () => {
  test('returns correct models for balanced profile', () => {
    ...
    assert.strictEqual(map['ecl-planner'], 'opus');
    assert.strictEqual(map['ecl-codebase-mapper'], 'haiku');
    assert.strictEqual(map['ecl-verifier'], 'sonnet');
  });

  test('returns correct models for budget profile', () => {
    const map = getAgentToModelMapForProfile('budget');
    assert.strictEqual(map['ecl-planner'], 'sonnet');
    assert.strictEqual(map['ecl-phase-researcher'], 'haiku');
  });

  test('returns correct models for quality profile', () => {
    const map = getAgentToModelMapForProfile('quality');
    assert.strictEqual(map['ecl-planner'], 'opus');
    assert.strictEqual(map['ecl-executor'], 'opus');
  });

  test('returns correct models for adaptive profile', () => {
    const map = getAgentToModelMapForProfile('adaptive');
    assert.strictEqual(map['ecl-planner'], 'opus', 'planner should use opus in adaptive');
    assert.strictEqual(map['ecl-debugger'], 'opus', 'debugger should use opus in adaptive');
    assert.strictEqual(map['ecl-executor'], 'sonnet', 'executor should use sonnet in adaptive');
    assert.strictEqual(map['ecl-codebase-mapper'], 'haiku', 'mapper should use haiku in adaptive');
    assert.strictEqual(map['ecl-plan-checker'], 'haiku', 'checker should use haiku in adaptive');
  });

  // ─── resolution order: override > profile > default ─────────────────────────
  // Uses ecl-phase-researcher because it has visibly distinct values at every
  ...
  describe('resolution order: override > profile > default', () => {
    ...
    const AGENT = 'ecl-phase-researcher';
...
describe('formatAgentToModelMapAsTable', () => {
  test('produces a table with header and separator', () => {
    const map = { 'ecl-planner': 'opus', 'ecl-executor': 'sonnet' };
    ...
    assert.ok(table.includes('ecl-planner'), 'should list agent');
