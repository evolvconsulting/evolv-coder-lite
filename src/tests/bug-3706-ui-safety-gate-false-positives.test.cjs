...
describe('Workflow .md structural guard (#3718)', () => {
  ...
  for (const [label, filePath] of [
    ...
    test(`${label} must invoke ui-safety-gate.cjs via stdin, anchored to ECL_REPO_ROOT`, () => {
      ...
      assert.ok(
        ...
        `${label}: must anchor path to ECL_REPO_ROOT to avoid CWD-sensitive failure (#3718)`
      );
      assert.ok(
        content.includes('git rev-parse --show-toplevel'),
        `${label}: must derive ECL_REPO_ROOT from git rev-parse --show-toplevel`
      );
