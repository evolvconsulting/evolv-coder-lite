...
AGENT_SKILLS_RESEARCHER=$($ECL_SDK query agent-skills ecl-phase-researcher)
...
MVP_MODE_CFG=$($ECL_SDK query config-get workflow.mvp_mode 2>/dev/null || echo "false")
```

When `TDD_MODE` is `true`, the planner agent is instructed to apply `type: tdd` to eligible tasks using heuristics from `references/tdd.md`. The planner's `<required_reading>` is extended to include `@~/.claude/evolv-coder-lite/references/tdd.md` so gate enforcement rules are available during planning.
...
if [ "$MVP_MODE" = "true" ] && [ "$padded_phase" = "01" ]; then
  PRIOR_SUMMARIES=$($ECL_SDK query phases.list --pick summaries_total 2>/dev/null || echo "0")
...
```bash
CHUNKED_CFG=$($ECL_SDK query config-get workflow.plan_chunked 2>/dev/null || echo "false")
CHUNKED_MODE=false
...
```bash
MVP_MODE=$($ECL_SDK query phase.mvp-mode "${PHASE}" $MVP_FLAG_ARG --pick active)
```
...
5. Commit:
```bash
$ECL_SDK query commit "docs(${padded_phase}): generate context from PRD" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
6. Commit with `ecl-sdk query commit "docs(${padded_phase}): generate context from ADR ingest" --files "${phase_dir}/${padded_phase}-CONTEXT.md"` and set `context_content`; continue to step 5.
...
AI_PHASE_CFG=$($ECL_SDK query config-get workflow.ai_integration_phase 2>/dev/null || echo "true")
```
...
◆ Note: This phase appears to involve AI system development.
  Consider running /ecl:ai-integration-phase {N} before planning to:
  - Select the right framework for your use case
  - Research its docs and best practices
  - Design an evaluation strategy

  Continue planning without AI-SPEC? (non-blocking — /ecl:ai-integration-phase can be run after)
```

Use AskUserQuestion with options:
- "Continue — plan without AI-SPEC"
- "Stop — I'll run /ecl:ai-integration-phase {N} first"

If "Stop": Exit with `/ecl:ai-integration-phase {N}` reminder.
...
```bash
PHASE_DESC=$($ECL_SDK query roadmap.get-phase "${PHASE}" --pick section)
```
...
Agent(
  ...
  subagent_type="ecl-phase-researcher",
...
SECURITY_ASVS=$($ECL_SDK query config-get workflow.security_asvs_level --raw 2>/dev/null || echo "1")
SECURITY_BLOCK=$($ECL_SDK query config-get workflow.security_block_on --raw 2>/dev/null || echo "high")
```
...
```bash
UI_PHASE_CFG=$($ECL_SDK query config-get workflow.ui_phase 2>/dev/null || echo "true")
UI_GATE_CFG=$($ECL_SDK query config-get workflow.ui_safety_gate 2>/dev/null || echo "true")
```
...
```bash
PHASE_SECTION=$($ECL_SDK query roadmap.get-phase "${PHASE}" 2>/dev/null)
...
```bash
AUTO_CHAIN=$($ECL_SDK query check auto-mode --pick auto_chain_active 2>/dev/null || echo "false")
```
...
```
Skill(skill="ecl-ui-phase", args="${PHASE} --auto ${ECL_WS}")
```
After `ecl-ui-phase` returns, re-read:
```bash
UI_SPEC_FILE=$(ls "${PHASE_DIR}"/*-UI-SPEC.md 2>/dev/null | head -1)
...
```bash
PHASE_SECTION=$($ECL_SDK query roadmap.get-phase "${PHASE}" --pick section 2>/dev/null)
```
...
2. Disable Nyquist with the exact command:
   `ecl-sdk query config-set workflow.nyquist_validation false`
...
Check config:
```bash
PATTERN_MAPPER_CFG=$($ECL_SDK query config-get workflow.pattern_mapper 2>/dev/null || echo "true")
```
...
Agent(
  prompt="{above}",
  subagent_type="ecl-pattern-mapper",
  model="{researcher_model}",
)
```
...
${TDD_MODE === 'true' ? `
<tdd_mode_active>
**TDD Mode is ENABLED.** Apply TDD heuristics from @~/.claude/evolv-coder-lite/references/tdd.md to all eligible tasks:
...
5. **Commit per-plan:**
   ```bash
   $ECL_SDK query commit "docs(${PADDED_PHASE}): plan ${plan_id} (chunked)" --files "${PHASE_DIR}/${plan_id}-PLAN.md"
   ```
...
**Read pass count:**
```bash
BOUNCE_PASSES=$($ECL_SDK query config-get workflow.plan_bounce_passes 2>/dev/null || echo "2")
BOUNCE_SCRIPT=$($ECL_SDK query config-get workflow.plan_bounce_script 2>/dev/null | jq -r '.' 2>/dev/null || true)
```
...
```bash
$ECL_SDK query commit "refactor(${padded_phase}): bounce plans through external refinement" --files "${PHASE_DIR}/*-PLAN.md"
```
...
```bash
GATE_CFG=$($ECL_SDK query config-get workflow.context_coverage_gate 2>/dev/null || echo "true")
if [ "$GATE_CFG" != "false" ]; then
  GATE_RESULT=$($ECL_SDK query check.decision-coverage-plan "${PHASE_DIR}" "${CONTEXT_PATH}")
...
```bash
$ECL_SDK query state.planned-phase --phase "${PHASE_NUMBER}" --name "${PHASE_NAME}" --plans "${PLAN_COUNT}"
```
...
```bash
$ECL_SDK query roadmap.annotate-dependencies "${PHASE_NUMBER}"
```
...
```bash
$ECL_SDK query commit "docs(${PADDED_PHASE}): create phase plan" --files "${PHASE_DIR}"/*-PLAN.md .planning/STATE.md .planning/ROADMAP.md
```
...
```bash
POST_PLANNING_GAPS=$($ECL_SDK query config-get workflow.post_planning_gaps --default true 2>/dev/null || echo true)
if [ "$POST_PLANNING_GAPS" = "true" ]; then
  node "$HOME/.claude/evolv-coder-lite/bin/ecl-tools.cjs" gap-analysis --phase-dir "${PHASE_DIR}"
fi
```

(`ecl-tools.cjs gap-analysis` reads `.planning/REQUIREMENTS.md`, `${PHASE_DIR}/CONTEXT.md`,
...
re-run `/ecl:plan-phase --gaps` to add plans, or proceed to execute-phase as-is.
