'use strict';
// allow-test-rule: reads markdown product files (ecl-executor.md, worktree-path-safety.md) to verify structural protocol — not source-grep

// Regression guards for bug #3097 and #3099.
//
// #3097: ecl-executor's worktree HEAD guard used `if [ -f .git ]` to detect
...
const executorSrc = fs.readFileSync(
  path.join(ROOT, 'agents', 'ecl-executor.md'), 'utf8',
);
const executePhaseSrc = fs.readFileSync(
  path.join(ROOT, 'evolv-coder-lite', 'workflows', 'execute-phase.md'), 'utf8',
);

describe('bug #3097: cwd-drift sentinel in ecl-executor.md', () => {
  test('task_commit_protocol has cwd-drift assertion step (0a)', () => {
    ...
    assert.ok(
      protocol.includes('cwd') || protocol.includes('drift') || protocol.includes('ecl-spawn-toplevel'),
  ...
  test('cwd-drift check precedes HEAD assertion', () => {
    ...
    const protocolEnd = executorSrc.indexOf('</task_commit_protocol>');
    const protocol = executorSrc.slice(protocolIdx, protocolEnd);
    const driftIdx = protocol.search(/cwd.drift|ecl-spawn-toplevel|drift.*assertion/i);
...
describe('bug #3099: absolute-path safety guidance in ecl-executor.md', () => {
  ...
  test('worktree-path-safety.md reference file exists', () => {
    assert.ok(
      fs.existsSync(path.join(ROOT, 'evolv-coder-lite', 'references', 'worktree-path-safety.md')),
      'evolv-coder-lite/references/worktree-path-safety.md does not exist',
    );
  });

  test('worktree-path-safety.md contains cwd-drift and absolute-path guards', () => {
    const safetySrc = fs.readFileSync(
      path.join(ROOT, 'evolv-coder-lite', 'references', 'worktree-path-safety.md'), 'utf8',
    );
    assert.ok(safetySrc.includes('ecl-spawn-toplevel') || safetySrc.includes('cwd-drift'),
