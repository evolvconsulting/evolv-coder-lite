...
import { GSDTools, GSDToolsError, resolveGsdToolsPath } from './ecl-tools.js';
import { setTransportPolicy, clearTransportPolicy } from './ecl-transport-policy.js';
...
const BUNDLED_GSD_TOOLS_PATH = fileURLToPath(
  new URL('../../evolv-coder-lite/bin/ecl-tools.cjs', import.meta.url),
);
...
describe('GSDTools', () => {
  ...
  beforeEach(async () => {
    tmpDir = join(tmpdir(), `ecl-tools-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
  ...
  describe('exec()', () => {
    ...
    it('throws GSDToolsError with context when ecl-tools.cjs not found', async () => {
      const tools = new GSDTools({
        projectDir: tmpDir,
        gsdToolsPath: '/nonexistent/path/ecl-tools.cjs',
  ...
  describe('initNewProject()', () => {
    ...
    it('propagates errors from ecl-tools', async () => {
      const scriptPath = await createScript(
        'init-fail.cjs',
  ...
  describe('resolveGsdToolsPath()', () => {
    it('prefers bundled ecl-tools over project .claude when the bundled file exists', async () => {
      const localBinDir = join(tmpDir, '.claude', 'evolv-coder-lite', 'bin');
      await mkdir(localBinDir, { recursive: true });
      await writeFile(join(localBinDir, 'ecl-tools.cjs'), '// stub');

      const result = resolveGsdToolsPath(tmpDir);
      if (existsSync(BUNDLED_GSD_TOOLS_PATH)) {
        expect(result).toBe(BUNDLED_GSD_TOOLS_PATH);
      } else {
        expect(result).toBe(join(localBinDir, 'ecl-tools.cjs'));
      }
    });
    ...
    it('uses explicit gsdToolsPath when provided (overrides bundled / .claude resolution)', async () => {
      const localBinDir = join(tmpDir, '.claude', 'evolv-coder-lite', 'bin');
      await mkdir(localBinDir, { recursive: true });
      const scriptPath = join(localBinDir, 'ecl-tools.cjs');
      await writeFile(
        scriptPath,
