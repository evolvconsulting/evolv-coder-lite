...
describe('install/uninstall — hermes (nested skills/ecl/ layout)', () => {
  let tmpDir;
  let previousCwd;
  ...
  test('installs eCL into ./.hermes and removes it cleanly', () => {
    ...
    assert.ok(Object.keys(manifest.files).some(f => f.startsWith('skills/ecl/help/')),
      JSON.stringify(manifest.files));

    uninstall(false, 'hermes');

    assert.ok(!fs.existsSync(path.join(targetDir, 'skills', 'ecl', 'help')));
...
describe('install/uninstall — qwen (flat skills/ecl-* layout)', () => {
  let tmpDir;
  let previousCwd;
...
describe('install/uninstall — trae (flat skills/ecl-* layout)', () => {
  let tmpDir;
  let previousCwd;
