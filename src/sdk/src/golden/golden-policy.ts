...
const MUTATION_DEFERRED_REASON =
  'Listed in QUERY_MUTATION_COMMANDS — mutates `.planning/`, git, or profile files. Subprocess golden vs ecl-tools.cjs is covered where a tmp fixture or `--dry-run` exists in golden.integration.test.ts; otherwise handler parity lives in sdk/src/query/*-mutation.test.ts, commit.test.ts, phase-lifecycle.test.ts, workstream.test.ts, intel.test.ts, profile.test.ts, template.test.ts, docs-init.ts, or uat.test.ts as applicable.';

/** Registry commands with no `ecl-tools.cjs` analogue — cannot have subprocess JSON parity. */
const NO_CJS_SUBPROCESS_REASON: Record<string, string> = {
  'phases.archive':
    'No `ecl-tools.cjs` command for `phases archive` (SDK-only). Covered in sdk/src/query/phase-lifecycle.test.ts.',
  'check.config-gates':
  ...
  'phase.list-plans':
    'SDK-only listing helper for agents (no `ecl-tools.cjs` mirror). Covered in sdk/src/query/phase-list-queries.test.ts.',
  ...
  'commands':
    'SDK-only registry introspection (no ecl-tools.cjs equivalent — the CJS layer has no self-describing verb). Covered in sdk/src/query/commands-list.test.ts. Closes #3121.',
  'phase.mvp-mode':
...
/**
 * Canonical commands with an explicit subprocess JSON check vs ecl-tools.cjs
