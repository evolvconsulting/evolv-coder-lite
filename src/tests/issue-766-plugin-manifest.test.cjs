...
describe('D: always-on hook contract drift guard', () => {
  ...
  test('SessionStart: exactly one no-matcher group with ecl-check-update.js and no timeout', () => {
    ...
    assert.equal(h.script, 'ecl-check-update.js', 'SessionStart hook must be ecl-check-update.js');
    assert.equal(h.timeout, undefined, 'ecl-check-update.js must NOT have a timeout field');
  });
