...
**前置条件：** 该阶段至少有一个 `PLAN.md` 文件。如果规划尚未完成，请先运行 `/ecl-plan-phase N` —— 参见[规划阶段](plan-a-phase.md)。
...
eCL Core 读取阶段的计划文件，将其按依赖关系分组为若干波次，并为每个计划生成独立的执行器代理。每个执行器在下一波次开始前以原子方式提交其工作。

在分发任何代理之前，eCL Core 会打印波次表：
...
eCL Core 仅执行第 2 波次的计划。它会首先检查所有较早波次是否已完成；如果任何第 1 波次计划仍标记为未完成，则会停止并提示你先完成较早波次。
...
eCL Core 在生成任何执行器之前运行状态一致性检查。检测到的偏差会被上报，你可以在继续之前接受或纠正。

---

## 恢复停滞的执行

如果执行中途停止——配额错误、网络断开或会话崩溃——波次级别的进度会被保留。eCL Core 会检查每个计划的 `SUMMARY.md` 文件；已有该文件的计划在重新运行时会自动跳过：

```bash
/ecl-execute-phase 1
```

eCL Core 会跳过 `SUMMARY.md` 已存在的计划，并从第一个未完成的计划继续。

**如果提交存在但 `SUMMARY.md` 缺失**（执行器已提交，但在会话结束前未写入摘要），eCL Core 会弹出一个安全恢复门并提供三个选项：
