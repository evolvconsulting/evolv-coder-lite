 ...
 * CLI commands (ecl-tools dispatch layer).
 *
 * Conventions:
 ...
 *   - HOME is overridden via runGsdTools env param to sandbox ~/.ecl/ writes
 */
...
function makeTempDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-research-test-'));
}
...
describe('research-plan: cache hit via pre-seeded store', () => {
  test('returns cache.hit:true for pre-seeded question; no fetch property', () => {
    ...
    try {
      ...
      const researchStore = require('../evolv-coder-lite/bin/lib/research-store.cjs');
      ...
      // Seed the cache directly, passing homeDir so it writes into tmpDir/.ecl/
...
describe('FINDING-1: package-legitimacy check flag parser correctness', () => {
  ...
  test('--bad-flag pkgA pkgB → unknown-flag error on stderr, non-zero exit, pkgA and pkgB not consumed as flag values', () => {
    ...
    try {
      ...
      //
      // Green proof: the current parser (ecl-tools.cjs lines 1951-1952) hits
