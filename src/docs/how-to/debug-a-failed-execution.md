...
**Prerequisites:** You have run `/ecl-execute-phase N` and the execution stopped before writing `VERIFICATION.md`, or you see unexpected output, missing files, or a stalled spinner.
...
This is normal, not a freeze. eCL subagents run in an isolated context window. The liveness note on the spawn line confirms this. Do not interrupt the session.
...
eCL checks for `SUMMARY.md` files before dispatching executors. Plans that already have one are skipped automatically.
...
eCL detects this at the next run and surfaces a safe-resume gate with three options:
...
eCL gathers symptoms, runs a structured investigation using the scientific method, and proposes a fix. If `tdd_mode: true` is set in your config, it requires a failing test before applying any fix.
...
## Run a post-mortem with `/ecl-forensics`
...
eCL analyses git history, `.planning/` artifact completeness, STATE.md consistency, uncommitted work, and orphaned worktrees. It writes a structured report to `.planning/forensics/report-<timestamp>.md` and surfaces recommended remediation steps.

`/ecl-forensics` is read-only — it never modifies your project files.
...
eCL skips plans whose `SUMMARY.md` already exists and dispatches executors only for the remaining plans.
...
Reverts all commits for plan `02` of phase `3`. eCL shows a confirmation gate before writing any change.
...
Reverts all commits for phase `3`. eCL checks whether any subsequent phases depend on this phase and warns you before proceeding.
...
Shows the five most recent eCL commits and lets you select which to revert.
