 ...
 * opt-in env `ECL_ENABLE_E2E=1`. Skips if env unset or CLI unavailable.
 */
...
import { eCL, parsePlanFile, GSDEventType } from './index.js';
...
const e2eEnabled = process.env.ECL_ENABLE_E2E === '1';
...
describe.skipIf(!canRunE2E)('E2E: Single plan execution', () => {
  ...
  beforeAll(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), 'ecl-sdk-e2e-'));
  ...
  it('executes a single plan and returns a valid PlanResult', async () => {
    const ecl = new eCL({ projectDir: tmpDir, maxBudgetUsd: 1.0, maxTurns: 20 });
    const result = await ecl.executePlan('sample-plan.md');
    ...
    expect(outputContent).toContain('hello from ecl-sdk');
  ...
  it('proves session isolation (R014) — different session IDs for sequential runs', async () => {
    ...
    const tmpDir2 = await mkdtemp(join(tmpdir(), 'ecl-sdk-e2e-'));
    await cp(fixturesDir, tmpDir2, { recursive: true });

    try {
      const gsd1 = new eCL({ projectDir: tmpDir, maxBudgetUsd: 1.0, maxTurns: 20 });
      const ecl2 = new eCL({ projectDir: tmpDir2, maxBudgetUsd: 1.0, maxTurns: 20 });

      const result1 = await gsd1.executePlan('sample-plan.md');
      const result2 = await ecl2.executePlan('sample-plan.md');
...
describe.skipIf(!canRunE2E)('E2E: Event stream during plan execution (R007)', () => {
  ...
  beforeAll(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), 'ecl-sdk-e2e-stream-'));
    await cp(fixturesDir, tmpDir, { recursive: true });
  });
...
describe('E2E: Error handling', () => {
  it('returns failure for nonexistent plan path', async () => {
    const tmpDir = await mkdtemp(join(tmpdir(), 'ecl-sdk-e2e-err-'));

    try {
      const ecl = new eCL({ projectDir: tmpDir });
      await expect(ecl.executePlan('nonexistent-plan.md')).rejects.toThrow();
    } finally {
      await rm(tmpDir, { recursive: true, force: true });
    }
  });
