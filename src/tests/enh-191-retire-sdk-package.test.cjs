...
test('enhancement #191: sdk package artifacts are removed from repository layout', () => {
  ...
  const shimPath = path.join(ROOT, 'bin', 'ecl-sdk.js');

  assert.equal(fs.existsSync(sdkDir), false, 'sdk/ directory must be deleted');
  assert.equal(fs.existsSync(shimPath), false, 'bin/ecl-sdk.js must be deleted');
});

test('enhancement #191: published package no longer exposes ecl-sdk artifacts', () => {
  const pkg = readPackageJson();

  assert.equal(Object.prototype.hasOwnProperty.call(pkg.bin || {}, 'ecl-sdk'), false,
    'package.json bin must not expose ecl-sdk');
  assert.equal(pkg.bin && pkg.bin['ecl-tools'], 'evolv-coder-lite/bin/ecl-tools.cjs',
    'package.json bin.ecl-tools must point to evolv-coder-lite/bin/ecl-tools.cjs');
...
test('enhancement #191: installer does not maintain ecl-sdk shim compatibility path', () => {
