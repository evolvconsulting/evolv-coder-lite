...
const EXECUTE_PHASE = path.join(__dirname, '..', 'evolv-coder-lite', 'workflows', 'execute-phase.md');
const EXECUTOR_AGENT = path.join(__dirname, '..', 'agents', 'ecl-executor.md');
const PLANNER_AGENT = path.join(__dirname, '..', 'agents', 'ecl-planner.md');
const EXECUTOR_EXAMPLES_REF = path.join(__dirname, '..', 'evolv-coder-lite', 'references', 'executor-examples.md');
const PLANNER_ANTIPATTERNS_REF = path.join(__dirname, '..', 'evolv-coder-lite', 'references', 'planner-antipatterns.md');
...
describe('prompt thinning — sub-200K context window support (#1978)', () => {
  ...
  describe('ecl-executor.md — reference to extracted examples', () => {
    test('references executor-examples.md for extended examples', () => {
      const content = fs.readFileSync(EXECUTOR_AGENT, 'utf-8');
      assert.ok(
        content.includes('executor-examples.md'),
        'ecl-executor.md must reference executor-examples.md for extended deviation/checkpoint examples'
      );
    });
  });

  describe('ecl-planner.md — reference to extracted anti-patterns', () => {
    test('references planner-antipatterns.md for extended anti-patterns', () => {
      ...
      assert.ok(
        ...
        'ecl-planner.md must reference planner-antipatterns.md for extended checkpoint anti-patterns and specificity examples'
      );
    });
  });
  ...
  describe('executor-examples.md — extracted reference file', () => {
    test('file exists', () => {
      assert.ok(
        ...
        'evolv-coder-lite/references/executor-examples.md must exist'
      );
    });
  ...
  describe('planner-antipatterns.md — extracted reference file', () => {
    test('file exists', () => {
      assert.ok(
        ...
        'evolv-coder-lite/references/planner-antipatterns.md must exist'
      );
    });
