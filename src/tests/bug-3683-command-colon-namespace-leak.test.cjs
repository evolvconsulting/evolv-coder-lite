...
// Claude Code. Asserting that staged bodies lack `/ecl:<cmd>` colon refs is
// a behavioral test of the install transform, not source-grep theater.

/**
 * Regression for #3683 — installed command bodies leak `/ecl:<cmd>` colon refs
 ...
 * PR #3677. Static prose in `commands/ecl/*.md` (e.g. plan-phase.md referencing
 * `/ecl:execute-phase`) therefore reaches the model verbatim, causing the model
...
/**
 * Build the roster regex that matches `/ecl:<known-cmd>` or `ecl:<known-cmd>`
...
describe('bug #3683 — command body colon-namespace leak (Claude local install)', () => {
  ...
  describe('B — normalizeAgentBodyForRuntime rewrites colon refs in command-body prose', () => {
    ...
    test('B1: claude — rewrites /ecl:<cmd> colon refs in command-body prose to hyphen form', () => {
      const input = [
        '## After planning',
        '',
        'Run `/ecl:execute-phase 1 --tdd` to begin execution.',
        'Then use `/ecl:verify-work 1` when done.',
      ].join('\n');
      ...
      assert.ok(!out.includes('/ecl:execute-phase'), 'colon form for execute-phase must be absent');
      assert.ok(!out.includes('/ecl:verify-work'), 'colon form for verify-work must be absent');
    });

    test('B2: gemini — colon refs preserved (Gemini intentionally uses colon namespace)', () => {
      const input = 'Run `/ecl:execute-phase 1` to begin.';
      const out = normalizeAgentBodyForRuntime(input, 'gemini', cmdNames);
      assert.ok(out.includes('/ecl:execute-phase'), 'Gemini must keep colon form');
      assert.ok(!out.includes('/ecl-execute-phase'), 'Gemini must not have hyphen form injected');
    });
  });
  ...
  // ---------------------------------------------------------------------------
  describe('E — integration: staged commands/ecl/*.md files contain no colon-namespace refs', () => {
    let tmpDir;
    const cmdNames = readCmdNames();
    const rosterRegex = buildRosterRegex(cmdNames);

    before(() => {
      tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-3683-'));
    ...
    test('E0: staged commands/ecl/ directory exists after install', () => {
      const commandsDir = path.join(tmpDir, '.claude', 'commands', 'ecl');
      assert.ok(
        fs.existsSync(commandsDir),
        `commands/ecl/ must be created by local claude install at ${commandsDir}`,
      );
    });

    test('E1: no staged command body contains /ecl:<known-cmd> colon refs', () => {
      const commandsDir = path.join(tmpDir, '.claude', 'commands', 'ecl');
      assert.ok(fs.existsSync(commandsDir), 'commands/ecl/ must exist for this check to be meaningful');
      ...
      assert.deepEqual(
        offenders,
        [],
        `Staged command bodies still contain roster colon refs (e.g. /ecl:execute-phase). ` +
        `Install must normalize these to /ecl-<cmd> for claude runtime. Offenders: ${offenders.join(', ')}`,
      );
    });
    ...
    test('E2: idempotent — re-running install does not double-mangle already-hyphenated refs', () => {
      ...
      // produce garbled output like /ecl--execute-phase. Verify the directory
      // still passes the same cleanliness check after a second install.
      runClaudeLocalInstall(tmpDir);

      const commandsDir = path.join(tmpDir, '.claude', 'commands', 'ecl');
      const doubleRewriteRegex = /\/ecl--[a-z]/;
      const garbled = [];
      ...
      assert.deepEqual(
        garbled,
        [],
        `Re-install produced double-hyphen artifacts (/ecl--cmd) — normalizer is not idempotent. Garbled files: ${garbled.join(', ')}`,
      );
    });
  });
});
