 ...
 *
 * Ported from evolv-coder-lite/bin/lib/phase.cjs and commands.cjs.
...
export const phaseInsert: QueryHandler = async (args, projectDir, workstream) => {
  ...
  await readModifyWriteRoadmapMd(projectDir, async (rawContent) => {
    ...
    // Build phase entry
    const phaseEntry = `\n### Phase ${decimalPhase}: ${description} (INSERTED)\n\n**Goal:** [Urgent work - to be planned]\n**Requirements**: TBD\n**Depends on:** Phase ${afterPhase}\n**Plans:** 0 plans\n\nPlans:\n- [ ] TBD (run /ecl-plan-phase ${decimalPhase} to break down)\n`;
 ...
 *
 * @param args - Positional `[type, phase, name?]` **or** ecl-tools style
...
export const phaseScaffold: QueryHandler = async (args, projectDir, workstream) => {
  ...
  switch (type) {
    case 'context': {
      ...
      content = `---\nphase: "${padded}"\nname: "${phaseName}"\ncreated: ${today}\n---\n\n# Phase ${phase}: ${phaseName} — Context\n\n## Decisions\n\n_Decisions will be captured during /ecl-discuss-phase ${phase}_\n\n## Discretion Areas\n\n_Areas where the executor can use judgment_\n\n## Deferred Ideas\n\n_Ideas to consider later_\n`;
      break;
    }
    case 'uat': {
...
export const milestoneComplete: QueryHandler = async (args, projectDir, workstream) => {
  ...
  if (version === '--help' || version === '-h') {
    throw new GSDError(
      `"${version}" is not a valid milestone version; see \`ecl-sdk query --help\` for command list`,
      ErrorClassification.Validation,
    );
  }
  assertNoNullBytes(version, 'version');
  ...
  if (existsSync(statePath)) {
    await readModifyWriteStateMdFull(projectDir, (stateContent) => {
      ...
      if (operatorPattern.test(next)) {
        next = next.replace(
          operatorPattern,
          `$1\n- Start the next milestone with /ecl-new-milestone\n\n`,
        );
      } else {
        next = `${next.trimEnd()}\n\n## Operator Next Steps\n\n- Start the next milestone with /ecl-new-milestone\n`;
      }
