/**
 * Golden test helpers — run `ecl-tools.cjs` as a subprocess and capture JSON or raw stdout.
 *
...
import { resolveGsdToolsPath } from '../ecl-tools.js';
    ...
    execFile(
      ...
      (err, stdout, stderr) => {
        if (err) {
          ...
          reject(
            new Error(
              `ecl-tools failed (exit ${code}): ${stderrStr || (err instanceof Error ? err.message : String(err))}`,
            ),
          );
          return;
        }
...
async function parseGsdToolsJson(raw: string, projectDir: string): Promise<unknown> {
  ...
  if (jsonStr.startsWith('@file:')) {
    ...
    try {
      ...
      throw new Error(`Failed to read ecl-tools @file: indirection at "${filePath}": ${reason}`);
    }
  }
...
/**
 * Run `node ecl-tools.cjs <command> [...args]` in `projectDir` and parse stdout as JSON.
 */
...
/**
 * Run `node ecl-tools.cjs <command> [...args]` and return raw stdout (no JSON parse).
 */
