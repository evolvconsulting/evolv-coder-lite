...
eCL Core 是一个**元提示框架**，位于用户与 AI 编码 Agent（Claude Code、Gemini CLI、OpenCode、Kilo、Codex、Copilot、Antigravity、Trae、Cline、Augment Code）之间。它提供：
...
工作流文件（`evolv-coder-lite/workflows/*.md`）不承担繁重工作。它们：

- 通过 `ecl-tools.cjs init <workflow>` 加载上下文
- 以聚焦的提示词派生专用 Agent
- 收集结果并路由到下一步
- 在步骤之间更新状态
...
- **Claude Code：** 自定义斜线命令（连字符形式，`/ecl-command-name`）
- **OpenCode / Kilo：** 斜线命令（连字符形式，`/ecl-command-name`）
- **Codex：** 技能（`$ecl-command-name`）
- **Copilot：** 斜线命令（连字符形式，`/ecl-command-name`）
- **Gemini CLI：** 在 `ecl:` 命名空间下的斜线命令（冒号形式，`/ecl:command-name`）——Gemini 将所有自定义命令置于其插件 id 的命名空间下，因此安装路径会将正文中的每个引用改写为冒号形式
- **Antigravity：** 技能

**命令总数：** 请参阅 [`docs/INVENTORY.md`](INVENTORY.md#commands) 获取权威数量及完整列表。

#### 两阶段层级路由（v1.40，[#2792](https://github.com/evolvconsulting/evolv-coder-lite/issues/2792)）

为控制急于列举技能的 token 开销，v1.40 引入了六个命名空间**元技能**（`ecl-workflow`、`ecl-project`、`ecl-quality`、`ecl-context`、`ecl-manage`、`ecl-ideate`——源自 `commands/ecl/ns-*.md`，但可调用的 `name:` 为此处显示的简短形式），位于具体子技能之上。模型看到的是 6 个命名空间路由器（约 120 个 token），而非扁平的 86 个技能列表（约 2,150 个 token），选择命名空间后通过嵌入在命名空间路由器主体中的路由表路由到具体子技能。命名空间技能是**可叠加的**——每个具体命令仍可直接调用。

路由器描述使用管道分隔的关键词标签（≤ 60 个字符），符合工具注意力研究的结论：关键词密集的标签在路由效果上优于散文，且 token 开销仅约 40%。

#### MCP token 预算交互

急于列举技能是每轮两种反复出现的 token 开销之一。另一种是 `.claude/settings.json` 中每个已启用 MCP 服务器注入的 MCP 工具 schema。重型 MCP 服务器（browser/playwright、Mac-tools、Windows-tools）每轮各自可消耗 20k+ token——通常远超 `model_profile` 调优所节省的量。该开关位于 Claude Code 框架中（`.claude/settings.json` 中的 `enabledMcpjsonServers` / `disabledMcpjsonServers`），**不属于** eCL 的关注范围。两阶段路由层（#2792）和严格的 MCP 启用管理是每轮最大的成本杠杆。请参阅 [`docs/USER-GUIDE.md`](USER-GUIDE.md) 和 `references/context-budget.md` 了解审计清单。

### 工作流（`evolv-coder-lite/workflows/*.md`）

命令所引用的编排逻辑，包含逐步流程：

- 通过 `ecl-tools.cjs init` 处理程序加载上下文
- 带有模型解析的 Agent 派生指令
- 关卡/检查点定义
- 状态更新模式
- 错误处理与恢复
...
工作流文件在每次调用对应的 `/ecl-*` 命令时会被完整加载到 Claude 的上下文中。为控制该成本，`tests/workflow-size-budget.test.cjs` 强制执行的工作流大小预算与 #2361 中的 Agent 预算保持一致：
...
根据 issue #2551，`workflows/discuss-phase.md` 须严格遵守 <500 行上限。当工作流超出其层级时，应将各模式的主体提取到 `workflows/<workflow>/modes/<mode>.md`，将模板提取到 `workflows/<workflow>/templates/`，将共享知识提取到 `evolv-coder-lite/references/`。父文件成为轻量级调度器，仅读取当前调用所需的模式和模板文件。
...
### 参考文档（`evolv-coder-lite/references/*.md`）
...
将思考类模型（o3、o4-mini、Gemini 2.5 Pro）集成到 eCL 工作流的参考文档：
...
规划器 Agent（`agents/ecl-planner.md`）已从单一整体文件分解为一个核心 Agent 加参考模块，以遵守部分运行时强加的 50K 字符限制：

- `planner-gap-closure.md` — 缺口修复模式行为（读取 VERIFICATION.md，针对性重规划）
- `planner-reviews.md` — 跨 AI 审查集成（从 `/ecl-review` 读取 REVIEWS.md）
- `planner-revision.md` — 用于迭代细化的计划修订模式

### 模板（`evolv-coder-lite/templates/`）

所有规划产物的 Markdown 模板。由 `ecl-tools.cjs template fill` / `phase.scaffold`（以及顶级 `scaffold`）使用，以创建预结构化文件：
...
| Hook | 事件 | 用途 |
|------|-------|---------|
| `ecl-statusline.js` | `statusLine` | 显示模型、任务、目录及上下文使用量进度条 |
| `ecl-context-monitor.js` | `PostToolUse` / `AfterTool` | 在剩余上下文为 35%/25% 时向 Agent 注入上下文警告 |
| `ecl-check-update.js` | `SessionStart` | 触发后台更新检查的前台触发器 |
| `ecl-check-update-worker.js` | （辅助程序） | 由 `ecl-check-update.js` 派生的后台工作进程；不直接注册事件 |
| `ecl-prompt-guard.js` | `PreToolUse` | 扫描 `.planning/` 写入内容中的提示词注入模式（建议性） |
| `ecl-read-injection-scanner.js` | `PostToolUse` | 扫描 Read 工具输出中不受信任内容里的注入指令 |
| `ecl-workflow-guard.js` | `PreToolUse` | 检测 eCL 工作流上下文之外的文件编辑（建议性，通过 `hooks.workflow_guard` 选择启用） |
| `ecl-read-guard.js` | `PreToolUse` | 建议性防护，防止对本会话中尚未读取的文件执行 Edit/Write |
| `ecl-session-state.sh` | `PostToolUse` | 基于 shell 的运行时的会话状态跟踪 |
| `ecl-validate-commit.sh` | `PostToolUse` | 用于规范提交格式执行的提交验证 |
| `ecl-phase-boundary.sh` | `PostToolUse` | 工作流转换的阶段边界检测 |

请参阅 [`docs/INVENTORY.md`](INVENTORY.md#hooks-11-shipped) 获取权威的 11 个 hook 列表。

### 命令路由中枢（`evolv-coder-lite/bin/lib/command-routing-hub.cjs`）

CJS 命令族路由器通过 `CommandRoutingHub` 进行调度。中枢拥有不抛出异常的纯结果契约（`hub.dispatch()` 捕获内部异常并返回 `{ ok: false, kind, ...typedPayload }`）以及封闭的运行时错误分类（`UnknownCommand`、`InvalidArgs`、`HandlerRefusal`、`HandlerFailure`）。路由器适配器保持为轻量级 CLI 转换器——它们构建中枢、调用 `dispatch`，然后将结果映射到 `output()`/`error()` 调用。运行时为单路径（无双运行时模式选择）。参见 `docs/adr/0174-retire-ecl-sdk-package-boundary.md`。

### CLI 工具（`evolv-coder-lite/bin/`）

Node.js CLI 工具（`ecl-tools.cjs`），其领域模块分布在 `evolv-coder-lite/bin/lib/` 中（请参阅 [`docs/INVENTORY.md`](INVENTORY.md#cli-modules-33-shipped) 获取权威列表）：
...
21 个主要 Agent 的概念派生模式分类。完整的 31 个 Agent 权威列表（包括 10 个高级/专用 Agent，如 `ecl-pattern-mapper`、`ecl-code-reviewer`、`ecl-code-fixer`、`ecl-ai-researcher`、`ecl-domain-researcher`、`ecl-eval-planner`、`ecl-eval-auditor`、`ecl-framework-selector`、`ecl-debug-session-manager`、`ecl-intel-updater`），请参阅 [`docs/INVENTORY.md`](INVENTORY.md#agents-31-shipped)。


| 类别             | Agent                                                                                   | 并行性                                                                                    |
| ---------------- | --------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------- |
| **研究者**       | ecl-project-researcher, ecl-phase-researcher, ecl-ui-researcher, ecl-advisor-researcher | 4 路并行（技术栈、功能、架构、陷阱）；advisor 在 discuss-phase 期间派生 |
| **综合者**       | ecl-research-synthesizer                                                                | 串行（研究者完成后）                                                                       |
| **规划者**       | ecl-planner, ecl-roadmapper                                                             | 串行                                                                                      |
| **检查者**       | ecl-plan-checker, ecl-integration-checker, ecl-ui-checker, ecl-nyquist-auditor          | 串行（验证循环，最多 3 次迭代）                                                             |
| **执行者**       | ecl-executor                                                                            | 波次内并行，波次间串行                                                                     |
| **验证者**       | ecl-verifier                                                                            | 串行（所有执行者完成后）                                                                    |
| **映射者**       | ecl-codebase-mapper                                                                     | 4 路并行（技术、架构、质量、关注点）                                                         |
| **调试者**       | ecl-debugger                                                                            | 串行（交互式）                                                                             |
| **审计者**       | ecl-ui-auditor, ecl-security-auditor                                                    | 串行                                                                                      |
| **文档写作者**   | ecl-doc-writer, ecl-doc-verifier                                                        | 串行（写作者后接验证者）                                                                    |
| **分析者**       | ecl-user-profiler                                                                       | 串行                                                                                      |
| **假设分析者**   | ecl-assumptions-analyzer                                                                | 串行（discuss-phase 期间）                                                                 |
...
编排器从配置中读取 `context_window`（`ecl-tools.cjs config-get context_window`），当该值 >= 500,000 时，条件性地包含更丰富的上下文。对于标准 200K 窗口，提示词使用截断版本并以缓存友好的顺序排列，以最大化上下文效率。
...
├── skills/ecl-*/SKILL.md           # Global skills (authoritative roster: docs/INVENTORY.md)
...
在 `/ecl-execute-phase` 最后一个波次提交后，工作流运行一个非阻塞性的 `codebase_drift_gate` 步骤（位于 `schema_drift_gate` 和 `verify_phase_goal` 之间）。它将 diff `last_mapped_commit..HEAD` 与 `.planning/codebase/STRUCTURE.md` 进行对比，并统计四类结构性元素：
...
若数量达到 `workflow.drift_threshold`（默认为 3），关卡将**警告**（默认）并显示建议的 `/ecl-map-codebase --paths …` 命令，或**自动重新映射**（`workflow.drift_action = auto-remap`），方法是派生 `ecl-codebase-mapper` 并将其范围限定为受影响的路径。检测或重新映射过程中的任何错误都会被记录，阶段继续执行——漂移检测不会导致验证失败。
...
4. **运行时适配** — 按运行时转换文件内容：
  ...
  - Hermes Agent：在 `skills/ecl/` 下按类别分组的技能
...
7. **补丁备份** — 自 v1.17 起，将本地修改的文件备份到 `ecl-local-patches/`，供 `/ecl-update --reapply` 使用
8. **清单跟踪** — 写入 `ecl-file-manifest.json` 以支持干净卸载
9. **卸载模式** — `--uninstall` 移除所有 eCL 文件、hook 和设置
...
Runtime Engine (Claude Code / Gemini CLI)
    ...
    │
    └── SessionStart event ──► ecl-check-update.js
        Reads: VERSION file
...
**威胁模型：** eCL 将从"研究者命名一个软件包"到"执行者运行 `npm install`"的完整路径自动化。一个通过 `npm view`（仅证明已注册，而非合法性）的幻觉名称此前可能未被检测到而流入。约 20% 的 AI 生成软件包引用是幻觉；其中约 43% 的名称在不同提示词中反复出现，使攻击者的预先注册在经济上可行。

**关卡层次：**

| 层次 | 组件 | 操作 |
|-------|-----------|--------|
| 研究 | `ecl-phase-researcher` | 运行 `slopcheck install <pkgs> --json`；向 RESEARCH.md 写入 `## Package Legitimacy Audit` 表格；在写入 RESEARCH.md 之前剥离 `[SLOP]` 软件包 |
| 规划 | `ecl-planner` | 读取审计表；在任何 `[ASSUMED]` 或 `[SUS]` 安装任务之前插入 `checkpoint:human-verify`；向 `<threat_model>` 添加 `T-{phase}-SC` STRIDE 供应链行 |
| 执行 | `ecl-executor` | 规则 3 将软件包安装排除在自动修复范围之外；失败的安装以检查点形式呈现，而非静默替换 |
...
**工作流防护**（`ecl-workflow-guard.js`）：

- 触发于对非 `.planning/` 文件的 Write/Edit
- 检测 eCL 工作流上下文之外的编辑（无活动的 `/ecl-` 命令或任务子 Agent）
- 建议使用 `/ecl-quick` 或 `/ecl-fast` 进行状态跟踪的变更
...
eCL 通过统一的命令/工作流架构支持多种 AI 编码运行时：
...
| --- | --- | --- | --- | --- | --- |
| Claude Code | `~/.claude` | `./.claude` | 全局 `skills/ecl-*/SKILL.md`；本地 `commands/ecl/*.md` | `agents/ecl-*.md` | `settings.json` hook 和 statusLine 条目 |
| OpenCode | `~/.config/opencode` | `./.opencode` | `command/ecl-*.md` | `agents/ecl-*.md` | `opencode.json` 或 `opencode.jsonc`；无 eCL hook |
| Kilo | `~/.config/kilo` | `./.kilo` | `command/ecl-*.md` | `agents/ecl-*.md` | `kilo.json` 或 `kilo.jsonc`；无 eCL hook |
| Gemini CLI | `~/.gemini` | `./.gemini` | `commands/ecl/*.toml` | `agents/ecl-*.md` | `settings.json` 功能标志、hook 和 statusline |
| Codex | `~/.codex` | `./.codex` | `skills/ecl-*/SKILL.md` | `agents/` 源 markdown 加每个 Agent 的 TOML | `config.toml` `[agents.ecl-*]`、`[features].hooks`（规范；遗留别名 `codex_hooks` 在重新安装时被识别并迁移到新版本，#3566）以及 hook 表 |
| GitHub Copilot | `~/.copilot` | `./.github` | `skills/ecl-*/SKILL.md` 和 `copilot-instructions.md` | `.agent.md` 文件 | 无 eCL hook 或 statusline |
| Antigravity | 自动检测：`~/.gemini/antigravity`、`~/.gemini/antigravity-ide` 或 `~/.gemini/antigravity-cli` | `./.agent` | `skills/ecl-*/SKILL.md` | `agents/ecl-*.md` | eCL 安装时的 Gemini 风格 `settings.json` hook 条目 |
| Cursor | `~/.cursor` | `./.cursor` | `skills/ecl-*/SKILL.md` | `agents/ecl-*.md` | `rules/` 下的规则引用；无 eCL hook |
| Windsurf | `~/.codeium/windsurf` | `./.windsurf` | `skills/ecl-*/SKILL.md` | `agents/ecl-*.md` | `rules/` 下的规则引用；无 eCL hook |
| Augment Code | `~/.augment` | `./.augment` | `skills/ecl-*/SKILL.md` | `agents/ecl-*.md` | 无 eCL hook 或 statusline |
| Trae | `~/.trae` | `./.trae` | `skills/ecl-*/SKILL.md` | `agents/ecl-*.md` | `rules/` 下的规则引用；无 eCL hook |
| Qwen Code | `~/.qwen` | `./.qwen` | `skills/ecl-*/SKILL.md` | `agents/ecl-*.md` | 通用 eCL 设置及在支持时的 hook 条目 |
| Hermes Agent | `~/.hermes` | `./.hermes` | `skills/ecl/DESCRIPTION.md` 加 `skills/ecl/ecl-*/SKILL.md` | `agents/ecl-*.md` | 通用 eCL 设置及在支持时的 hook 条目 |
| CodeBuddy | `~/.codebuddy` | `./.codebuddy` | `skills/ecl-*/SKILL.md` | `agents/ecl-*.md` | 通用 eCL 设置及在支持时的 hook 条目 |
| Cline | `~/.cline` | 项目根目录 | `.clinerules` | 仅规则 | 无 eCL hook 或 statusline |
...
5. **模型引用** — `inherit` 配置文件让 eCL 推迟到运行时的模型选择
