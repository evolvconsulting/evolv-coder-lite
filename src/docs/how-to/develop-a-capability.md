# Develop a Capability for eCL 1.5+

This guide shows you how to add or change a first-party eCL Capability after the ADR-857 cutover. In eCL terms, the extension unit is a **Capability**. A plugin is a packaging or host-runtime term, for example a Claude Code plugin or Gemini extension, not the unit that owns a eCL feature.
...
```json
{
  ...
  "skills": [],
  "agents": ["ecl-example-agent"],
  "hooks": [],
  "config": {},
  "steps": [
    {
      "point": "plan:pre",
      "ref": { "agent": "ecl-example-agent" },
...
Every feature Capability must declare `runtimeCompat`. This is part of the eCL 1.5+ developer contract: a Capability says which runtime descriptors it can surface through, and the generator validates that declaration before the central registry is written.
...
- `steps` add a sequenced unit of work, such as running `ecl-phase-researcher` or `ecl-pattern-mapper`.
...
```bash
node evolv-coder-lite/bin/ecl-tools.cjs loop render-hooks plan:pre --raw
```

In installed workflow prose, the same resolver surface appears as `ecl-tools loop render-hooks plan:pre`.
...
```bash
node evolv-coder-lite/bin/ecl-tools.cjs loop render-hooks plan:pre --config-dir ~/.claude --raw
node evolv-coder-lite/bin/ecl-tools.cjs capability state --config-dir ~/.claude --raw
```
...
Do not add migrated Capability keys to `evolv-coder-lite/bin/shared/config-schema.manifest.json`. The central schema remains for host/core keys; Capability-owned keys validate through the generated registry and are merged into `loadConfig` by the federated config overlay. Existing nested `.planning/config.json` values such as `{ "workflow": { "example_enabled": false } }` continue to override the Capability default.
...
- `research` registers a `plan:pre` step that invokes `ecl-phase-researcher`.
- `ai-integration` owns the AI-SPEC planning activation and config key.
- `pattern-mapper` registers a `plan:pre` step that invokes `ecl-pattern-mapper`.
...
From eCL 1.6.0, capabilities are versioned (the `version` field is required in `capability.json`) and can be installed directly from a URL, a git ref, an npm package, or a local path — without modifying the core repo.
...
- **How-to** — [Version and update a capability](../how-to/version-a-capability.md): manage `version`, `engines.ecl`, and `compatVersions`; use `ecl capability update`.
- **How-to** — [Remove a capability](../how-to/remove-a-capability.md): uninstall cleanly with `ecl capability remove`, including the `--purge-data` option.
