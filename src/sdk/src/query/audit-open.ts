/**
 * Open Artifact Audit — full TypeScript port of `evolv-coder-lite/bin/lib/audit.cjs`.
 *
 * Scans `.planning/` artifact categories for unresolved items (same JSON as ecl-tools `audit-open`).
 */
...
/**
 * Same structured result as `ecl-tools.cjs audit-open` (JSON).
 */
...
/**
 * Human-readable report (same text as ecl-tools without `--json`).
 */
