 ...
 * evolv-coder-lite/bin/lib/legacy-cleanup.cjs using real temp dirs so the
...
const { planLegacyCleanup, applyLegacyCleanup } = require(
  path.join(__dirname, '..', 'evolv-coder-lite', 'bin', 'lib', 'legacy-cleanup.cjs')
);
...
const OLD_PACKAGE_SIGNAL = 'evolv-coder-lite' + '-cc';

// ─── Helpers ─────────────────────────────────────────────────────────────────

function mkTmpDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-607-'));
}
...
describe('issue-607 legacy-cleanup: planLegacyCleanup', () => {
  ...
  test('flags a hook file in hooks/ whose content contains the old package signal', () => {
    const hookFile = path.join(configDir, 'hooks', 'ecl-check-update-worker.js');
  ...
  test('does NOT flag a file whose content does not reference the old package', () => {
    const hookFile = path.join(configDir, 'hooks', 'ecl-check-update-worker.js');
    writeFile(hookFile, '// installed via @evolvconsulting/evolv-coder-lite\nconsole.log("ok");');
  ...
  test('regression #607 (data-loss): user custom ecl-*.js with NO old-package content must NOT appear in plan', () => {
    // Previously the orphaned-hook-by-name rule would flag any ecl-*.js not in
    // MANAGED_HOOKS — deleting user-authored hooks. This is the key regression test.
    const userHook = path.join(configDir, 'hooks', 'ecl-my-custom.js');
    writeFile(userHook, '// my custom hook — does not reference the old package');

    const plan = planLegacyCleanup([configDir], { homeDir });
    const entry = plan.find((p) => p.path === userHook);
    assert.equal(entry, undefined, 'user custom ecl-*.js with no old-package content must NOT be in plan');
  });

  test('regression #607 (data-loss): user custom ecl-*.sh with NO old-package content must NOT appear in plan', () => {
    const userHook = path.join(configDir, 'hooks', 'ecl-my-custom.sh');
    writeFile(userHook, '#!/bin/sh\n# my custom shell hook, clean');

    const plan = planLegacyCleanup([configDir], { homeDir });
    const entry = plan.find((p) => p.path === userHook);
    assert.equal(entry, undefined, 'user custom ecl-*.sh with no old-package content must NOT be in plan');
  });

  // ── self-deletion regression: evolv-coder-lite/ subtree must NOT be scanned ──

  test('regression #607 (self-deletion): a code file under evolv-coder-lite/bin/lib/ containing old-package signal must NOT be flagged', () => {
    // The subtree 'evolv-coder-lite' is no longer scanned — the current package's
    // own infra lives there and would falsely match if scanned.
    const libFile = path.join(configDir, 'evolv-coder-lite', 'bin', 'lib', 'legacy-cleanup.cjs');
    writeFile(libFile, "'use strict';\nconst SIG = 'evolv-coder-lite' + '-cc';\nmodule.exports = {};");

    const plan = planLegacyCleanup([configDir], { homeDir });
    const entry = plan.find((p) => p.path === libFile);
    assert.equal(entry, undefined, 'code file in evolv-coder-lite/ subtree must NOT appear in plan (subtree not scanned)');
  });
  ...
  test('regression #607: CHANGELOG.md containing old-package signal must NOT be flagged', () => {
    ...
    const changelogFile = path.join(configDir, 'evolv-coder-lite', 'CHANGELOG.md');
    writeFile(changelogFile, '# Changelog\n\nMigrated from ' + OLD_PACKAGE_SIGNAL + ' to @evolvconsulting/evolv-coder-lite.');
  ...
  test('regression #607: a workflow .md file containing old-package signal must NOT be flagged', () => {
    const workflowMd = path.join(configDir, 'evolv-coder-lite', 'workflows', 'update.md');
  ...
  test('NEVER flags dev-preferences.md even if its content contains old-package signal', () => {
    // Place dev-preferences.md inside a eCL-managed subtree
  ...
  test('flags the legacy shared cache when it exists with reason legacy-shared-cache', () => {
    const cachePath = path.join(homeDir, '.cache', 'ecl', 'ecl-update-check.json');
  ...
  test('de-duplicates candidates when two configDirs share same absolute path (same dir listed twice)', () => {
    const signalFile = path.join(configDir, 'hooks', 'ecl-old-feature.sh');
  ...
  test('plan entries are sorted by path', () => {
    writeFile(path.join(configDir, 'hooks', 'ecl-zzz-last.js'), '// ' + OLD_PACKAGE_SIGNAL);
    writeFile(path.join(configDir, 'hooks', 'ecl-aaa-first.js'), '// ' + OLD_PACKAGE_SIGNAL);
  ...
  test('plan entries only ever have reason content-references-old-package or legacy-shared-cache', () => {
    writeFile(path.join(configDir, 'hooks', 'ecl-worker.js'), '// ' + OLD_PACKAGE_SIGNAL);
    const cachePath = path.join(homeDir, '.cache', 'ecl', 'ecl-update-check.json');
    writeFile(cachePath, '{}');
    // User custom hook — should NOT appear
    writeFile(path.join(configDir, 'hooks', 'ecl-my-custom.js'), '// user hook, clean');
...
describe('issue-607 legacy-cleanup: applyLegacyCleanup', () => {
  ...
  test('dryRun:true removes nothing and returns dryRun:true + all paths in skipped', () => {
    ...
    const signalHook = path.join(configDir, 'hooks', 'ecl-check-update-worker.js');
    writeFile(signalHook, '// ' + OLD_PACKAGE_SIGNAL);
    const cacheFile  = path.join(homeDir, '.cache', 'ecl', 'ecl-update-check.json');
    writeFile(cacheFile, '{}');
  ...
  test('apply removes flagged files and returns them in removed[]', () => {
    ...
    writeFile(managedHook, '// @evolvconsulting/evolv-coder-lite only');
