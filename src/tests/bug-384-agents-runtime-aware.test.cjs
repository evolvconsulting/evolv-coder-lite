 ...
 *
 * After the fix:
 *  - ECL_RUNTIME=opencode + OPENCODE_CONFIG_DIR pointing at a temp dir →
 *    agents_installed=true, agent_runtime='opencode', agents_dir under the
 *    opencode config dir
 *  - No ECL_RUNTIME + ECL_AGENTS_DIR pointing at a temp dir →
 *    agents_installed=true, agent_runtime='claude'
 *  - ECL_RUNTIME=opencode but agents dir empty →
...
describe('bug #384 — getAgentsDir() is runtime-aware', () => {
  ...
  beforeEach(() => {
    ...
    opencodeConfigDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-test-opencode-'));
  });
  ...
  test('ECL_RUNTIME=opencode finds agents under OPENCODE_CONFIG_DIR/agents', () => {
    ...
    const result = runGsdTools(
      ...
      tmpDir,
      {
        ECL_RUNTIME: 'opencode',
        OPENCODE_CONFIG_DIR: opencodeConfigDir,
        // Ensure the process HOME does NOT have a conflicting ~/.claude/agents
        // that might accidentally produce a false positive via ECL_AGENTS_DIR
        // (we must NOT set ECL_AGENTS_DIR here — the whole point is that the fix
        // uses the runtime-aware path without needing ECL_AGENTS_DIR).
      }
    );
    ...
    assert.strictEqual(output.agent_runtime, 'opencode',
      'agent_runtime must be "opencode" when ECL_RUNTIME=opencode');
  ...
  // ── Test 2: claude fallback via ECL_AGENTS_DIR ──────────────────────────────

  test('default runtime (no ECL_RUNTIME) with ECL_AGENTS_DIR → agents_installed=true, agent_runtime=claude', () => {
    // Classic ECL_AGENTS_DIR override: no runtime set, use the env shortcut
    createAgentsInConfigDir(tmpDir);
    // ECL_AGENTS_DIR points directly at the agents dir (not the config dir)
    ...
    const result = runGsdTools(
      ...
      tmpDir,
      {
        ECL_AGENTS_DIR: directAgentsDir,
        // Explicitly unset ECL_RUNTIME so no runtime override applies
        ECL_RUNTIME: '',
      }
    );
    assert.ok(result.success, `Command failed: ${result.error}`);
    ...
    assert.strictEqual(output.agents_installed, true,
      `agents_installed must be true when ECL_AGENTS_DIR points at a populated agents dir. ` +
      `agents_dir=${output.agents_dir}`);

    assert.strictEqual(output.agent_runtime, 'claude',
      'agent_runtime must be "claude" when no ECL_RUNTIME is set');

    assert.strictEqual(output.agents_dir, directAgentsDir,
      `agents_dir must match ECL_AGENTS_DIR override`);
  });

  // ── Test 3 (negative): opencode runtime, empty agents dir ───────────────────

  test('ECL_RUNTIME=opencode with empty agents dir → agents_installed=false, agent_runtime still surfaced', () => {
