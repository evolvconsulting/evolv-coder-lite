...
> generated from `.ts` via `tsc`", `evolv-coder-lite/src/` / `sdk/src/` source trees,
...
- `evolv-coder-lite/bin/lib/` holds **84** `.cjs` files. **Exactly one** carries a
...
- There is **no** `evolv-coder-lite/src/` or `sdk/src/` TypeScript tree. There is
...
2. **Build at publish (recommended).** `bin/lib/*.cjs` becomes a gitignored
   ...
   built output. **Feasible today:** `package.json` already ships `evolv-coder-lite`
...
- Single-runtime collapse (**accepted**): `0174-retire-ecl-sdk-package-boundary.md`
- Superseded shared-module seam: `3524-cjs-sdk-hard-seam.md`
- Tracking issue: [#457](https://github.com/evolvconsulting/evolv-coder-lite/issues/457)
