 ...
 *
 * Ported from evolv-coder-lite/bin/lib/verify.cjs.
...
export const validateHealth: QueryHandler = async (args, projectDir, workstream) => {
  ...
  if (!existsSync(planBase)) {
    addIssue('error', 'E001', '.planning/ directory not found', 'Run /ecl-new-project to initialize');
    return {
      data: {
        status: 'broken',
        errors,
  ...
  if (!existsSync(projectPath)) {
    addIssue('error', 'E002', 'PROJECT.md not found', 'Run /ecl-new-project to create');
  } else {
    try {
  ...
  if (!existsSync(roadmapPath)) {
    addIssue('error', 'E003', 'ROADMAP.md not found', 'Run /ecl-new-milestone to create roadmap');
  }

  // ─── Check 4: STATE.md exists and references valid phases ─────────────────
  if (!existsSync(statePath)) {
    addIssue('error', 'E004', 'STATE.md not found', 'Run /ecl-health --repair to regenerate', true);
    ...
    try {
      ...
      // milestones. After /ecl:complete-milestone, phases are moved into
  ...
  if (!existsSync(configPath)) {
    addIssue('warning', 'W003', 'config.json not found', 'Run /ecl-health --repair to create with defaults', true);
    ...
    try {
      ...
      addIssue('error', 'E005', `config.json: JSON parse error - ${msg}`, 'Run /ecl-health --repair to reset to defaults', true);
  ...
  if (existsSync(configPath)) {
    try {
      ...
      if (workflow && workflow.nyquist_validation === undefined) {
        addIssue('warning', 'W008', 'config.json: workflow.nyquist_validation absent (defaults to enabled but agents may skip)', 'Run /ecl-health --repair to add key', true);
        ...
        if (researchFile) {
          try {
            ...
            if (researchContent.includes('## Validation Architecture')) {
              addIssue('warning', 'W009', `Phase ${e.name}: has Validation Architecture in RESEARCH.md but no VALIDATION.md`, 'Re-run /ecl-plan-phase with --research to regenerate');
            }
          } catch { /* intentionally empty */ }
        }
      }
        ...
        if (phaseCheckboxRe.test(roadmapContentFull)) {
          ...
          if (statusVal !== 'complete' && statusVal !== 'done') {
            addIssue('warning', 'W011',
              ...
              'Run /ecl-progress to re-derive current position, or manually update STATE.md');
          }
        }
      }
    } catch { /* intentionally empty */ }
      ...
      try {
        switch (repair) {
          ...
          case 'regenerateState': {
            ...
            stateContent += `- ${new Date().toISOString().split('T')[0]}: STATE.md regenerated by /ecl-health --repair\n`;
...
/**
 * Default agents directory — mirrors `getAgentsDir` in `evolv-coder-lite/bin/lib/core.cjs`:
 * `ECL_AGENTS_DIR`, else `../../../agents` relative to this module (`sdk/dist/query` → monorepo
 * root), matching `core.cjs` (`evolv-coder-lite/bin/lib` → same repo `agents/`).
 */
function getAgentsDirForValidateAgents(): string {
  if (process.env.ECL_AGENTS_DIR) return process.env.ECL_AGENTS_DIR;
  return resolveBundledAgentsDir();
}

/**
 * Validate eCL agent file installation under the managed agents directory.
 *
 ...
 *   < 60%   healthy
 *   60–70%  warning   → recommend /ecl-thread
 ...
 *
 * Mirror of evolv-coder-lite/bin/lib/context-utilization.cjs (the legacy
 * ecl-tools.cjs path uses the CJS module). Keep both in sync.
 */
...
const CONTEXT_RECOMMENDATIONS: Record<string, string | null> = {
  healthy: null,
  warning: 'Context is approaching the fracture zone — consider /ecl-thread to continue in a fresh window.',
  critical: 'Reasoning quality may degrade past 70% utilization (fracture point). Run /ecl-thread now to preserve output quality.',
};
