# Orquestração multi-agente no eCL Core

> **Explicação** — Este documento descreve *por que* o eCL Core foi projetado em torno da
...
O design multi-agente do eCL Core é uma resposta direta a esse problema. Em vez de
...
Todos os workflows em `evolv-coder-lite/workflows/` seguem a mesma estrutura:
...
Os agentes do eCL Core se enquadram em categorias funcionais que mapeiam o
pipeline pesquisa → planejamento → execução → verificação:

| Categoria | Agentes | Paralelismo típico |
|---|---|---|
| Pesquisadores | `ecl-project-researcher`, `ecl-phase-researcher`, `ecl-ui-researcher`, `ecl-advisor-researcher` | 4 em paralelo (stack, funcionalidades, arquitetura, armadilhas) |
...
| Verificadores | `ecl-plan-checker`, `ecl-integration-checker`, `ecl-ui-checker`, `ecl-nyquist-auditor` | Sequencial, até 3 iterações de revisão |
| Executores | `ecl-executor` | Paralelo dentro de uma onda, sequencial entre ondas |
| Validadores | `ecl-verifier` | Sequencial, após a conclusão de todos os executores |
| Mapeadores | `ecl-codebase-mapper` | 4 sub-sondas em paralelo |
...
A expressão mais visível do design multi-agente é como o `/ecl-execute-phase`
...
de nenhum agente), os workflows do eCL sobrevivem a resets de contexto (`/clear`), trocas de
...
trabalhando em um contexto terminaria mais rápido para tarefas simples. O eCL mitiga
...
Este é o custo do isolamento. O handler `ecl-tools.cjs init`
