<purpose>
Auto-fix issues from REVIEW.md. Validates phase, checks config gate, verifies REVIEW.md exists and has fixable issues, spawns ecl-code-fixer agent, handles --auto iteration loop (capped at 3), commits REVIEW-FIX.md once at the end, and presents results.
</purpose>
...
- ecl-code-fixer: Applies fixes to code review findings
- ecl-code-reviewer: Reviews source files for bugs and issues
</available_agent_types>
...
AGENT_SKILLS_FIXER=$(ecl_run query agent-skills ecl-code-fixer)
...
if [ ! -f "${REVIEW_PATH}" ]; then
  echo "Error: No REVIEW.md found for Phase ${PHASE_ARG}. Run /ecl:code-review ${PHASE_ARG} first."
  exit 1
fi
```
...
You can retry with /ecl:code-review ${PHASE_ARG} --fix.
```
...
if [ ! -f "${FIX_REPORT_PATH}" ]; then
  ...
  echo ""
  echo "Retry: /ecl:code-review ${PHASE_ARG} --fix"
  echo ""
  echo "═══════════════════════════════════════════════════════════════"
  exit 1
fi
...
if [ "$FIX_STATUS" = "all_fixed" ]; then
  ...
  echo ""
  echo "Next step:"
  echo "  /ecl:verify-work  — Verify phase completion"
  echo ""
fi
```
...
if [ "$FIX_STATUS" = "partial" ] || [ "$FIX_STATUS" = "none_fixed" ]; then
  ...
  echo "  /ecl:code-review ${PHASE_NUMBER}           — Re-review code"
  echo "  /ecl:verify-work                           — Verify phase completion"
  echo ""
fi
```
