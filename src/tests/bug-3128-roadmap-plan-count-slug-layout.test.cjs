...
// This misses the {N}-PLAN-{NN}-{slug}.md layout that ecl-plan-phase
...
const roadmapLib = path.join(ROOT, 'evolv-coder-lite', 'bin', 'lib', 'roadmap.cjs');
const planScanLib = path.join(ROOT, 'evolv-coder-lite', 'bin', 'lib', 'plan-scan.cjs');
...
function makeTempPhase(files) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-3128-'));
