/**
 * Contract test: every `ecl-sdk query agent-skills <slug>` invocation in
 * `evolv-coder-lite/workflows/**\/*.md` must reference a slug that exists as
 ...
 *
 * Related: https://github.com/evolvconsulting/evolv-coder-lite/issues/2615
 */
...
const workflowsDir = join(repoRoot, 'evolv-coder-lite', 'workflows');
const agentsDir = join(repoRoot, 'agents');

/**
 * Matches a full `ecl-sdk query agent-skills <slug>` invocation and captures
 * the slug. Requires a token boundary before `ecl-sdk` and a word boundary
...
/**
 * Extracts every `ecl-sdk query agent-skills <slug>` usage from the given
