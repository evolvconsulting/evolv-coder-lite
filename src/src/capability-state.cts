 ...
 * `ecl-tools capability state` diagnostic share the same enablement answer.
 *
 ...
 *   - ./runtime-slash.cjs      (resolveRuntime — ECL_RUNTIME > config.runtime > 'claude' precedence)
...
/**
 * Derive the commands/ecl path from __dirname (which resolves to
 * evolv-coder-lite/bin/lib/ at runtime). The source tree is:
 *   <repo>/evolv-coder-lite/bin/lib/capability-state.cjs
 *   <repo>/commands/ecl/*.md
 * So we walk up three levels: lib/ → bin/ → evolv-coder-lite/ → <repo>/, then
 * into commands/ecl/.
 */
function _resolveCommandsGsdDir(): string {
  // __dirname = evolv-coder-lite/bin/lib/
  const repoRoot = path.resolve(__dirname, '..', '..', '..');
  return path.join(repoRoot, 'commands', 'ecl');
}

/**
 * Build a skill dependency manifest from an INSTALLED runtime's skills directory.
 *
 * In an installed runtime (e.g. Codex at ~/.codex), ecl skills live as
 * configDir/skills/ecl-STEM/SKILL.md. There is no commands/ecl source tree.
 * This function scans that installed layout and builds the same
 * Map shape that loadSkillsManifest produces from sources.
 *
 * Stem extraction: a directory named ecl-secure-phase maps to stem secure-phase.
 * Only directories whose names start with ecl- are included so user-created
 * skills (without the ecl- prefix) are not accidentally pulled in.
 *
...
function _loadInstalledSkillsManifest(configDir: string): Map<string, string[]> {
  ...
  for (const entry of entries) {
    ...
    // Strip the 'ecl-' prefix to get the skill stem
    const stem = entry.name.slice(4); // 'ecl-'.length === 4
    if (!stem) continue;
    ...
    // no file exists. Mirror that here: a stale ecl-<stem>/ directory with no
 ...
 *   2. Otherwise, fall back to installed skills at configDir/skills/ecl-[stem]/SKILL.md.
 *
 * In an installed runtime the commands/ecl source tree is absent; only the
...
function resolveCapabilityRuntimeState(
  ...
  // precedence:  process.env.ECL_RUNTIME → config.runtime → 'claude'
  ...
  if (!resolvedConfigDir) {
    try {
      ...
      // Detect the active runtime via ECL_RUNTIME → config.runtime → 'claude'.
  ...
  try {
    ...
    // commands/ecl is absent) fall back to <configDir>/skills/ecl-*/SKILL.md.
