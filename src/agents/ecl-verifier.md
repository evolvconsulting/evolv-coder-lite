...
ecl-sdk query roadmap.get-phase "$PHASE_NUM"
...
```bash
PHASE_DATA=$(ecl-sdk query roadmap.get-phase "$PHASE_NUM" --raw)
```
...
Use `ecl-sdk query` for artifact verification against must_haves in PLAN frontmatter:

```bash
ARTIFACT_RESULT=$(ecl-sdk query verify.artifacts "$PLAN_PATH")
```
...
Use `ecl-sdk query` for key link verification against must_haves in PLAN frontmatter:

```bash
LINKS_RESULT=$(ecl-sdk query verify.key-links "$PLAN_PATH")
```
...
SUMMARY_FILES=$(ecl-sdk query summary-extract "$PHASE_DIR"/*-SUMMARY.md --fields key-files)

# Option 2: Verify commits exist (if commit hashes documented)
COMMIT_HASHES=$(grep -oE "[a-f0-9]{7,40}" "$PHASE_DIR"/*-SUMMARY.md | head -10)
if [ -n "$COMMIT_HASHES" ]; then
  COMMITS_VALID=$(ecl-sdk query verify.commits $COMMIT_HASHES)
fi
...
```bash
ROADMAP_DATA=$(ecl-sdk query roadmap.analyze --raw)
```
...
```bash
USER_STORY_VALID=$(ecl-sdk query user-story.validate --story "$PHASE_GOAL" --pick valid)
```
