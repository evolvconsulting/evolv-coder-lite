...
Maps the five canonical triage roles to the actual label strings in `evolvconsulting/evolv-coder-lite`.
...
| `needs-version` | `needs-version` | Applied by the Version gate workflow when a bug report is auto-closed for missing a valid eCL Version. Edit the issue to add the version and reopen it. |
...
1. **Gate on open** — When an issue is opened, `version-gate.yml` checks whether it is a bug report (has the `bug` label, or its body contains a `### eCL Version` heading from the bug template). Non-bug issues are skipped.
2. **Version check** — The gate extracts the value under the `### eCL Version` heading. A value is considered valid if it contains a semver-ish token (e.g. `1.18.0`, `v1.4.1`, `1.18.0-dev`) or a git commit SHA (7-40 hex chars). Missing, blank, `_No response_`, or junk values like `idk` are treated as absent.
