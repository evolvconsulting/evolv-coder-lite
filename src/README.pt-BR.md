...
Então eu criei o eCL. A complexidade fica no sistema, não no seu fluxo. Por trás: engenharia de contexto, formatação XML de prompts, orquestração de subagentes, gerenciamento de estado. O que você vê: alguns comandos que simplesmente funcionam.
...
O eCL corrige isso. É a camada de engenharia de contexto que torna o Claude Code confiável.
...
Lista completa nas [notas de release v1.39.0](https://github.com/evolvconsulting/evolv-coder-lite/releases/tag/v1.39.0).

- **Perfil de instalação `--minimal`** — alias `--core-only`. Instala apenas os 6 skills do loop principal (`new-project`, `discuss-phase`, `plan-phase`, `execute-phase`, `help`, `update`) e nenhum subagente `ecl-*`. Reduz o overhead do system prompt no cold-start de ~12k para ~700 tokens (≥94% de redução). Útil para LLMs locais com contexto de 32K–128K e APIs cobradas por token.
- **`/ecl-phase --edit`** — edita qualquer campo de uma fase existente em `ROADMAP.md` no lugar, sem alterar o número ou a posição. `--force` pula o diff de confirmação; referências em `depends_on` são validadas e o `STATE.md` é atualizado na escrita.
...
- **Herança de configuração de workstream** — quando `ECL_WORKSTREAM` está definido, o `.planning/config.json` raiz é carregado primeiro e merge-deep com o config da workstream (workstream vence em conflito). Um `null` explícito no config da workstream sobrescreve corretamente o valor raiz.
- **Workflow manual de canary release** — `.github/workflows/canary.yml` publica builds `{base}-canary.{N}` de `@evolvconsulting/evolv-coder-lite` e `@evolvconsulting/ecl-sdk` na dist-tag `@canary` a partir de `dev`, sob demanda via `workflow_dispatch`.
...
- Codex: `$ecl-help`
- Cline: eCL instala via `.clinerules` — verifique se `.clinerules` existe

> [!NOTE]
> Claude Code 2.1.88+ e Codex instalam como skills (`skills/ecl-*/SKILL.md`). Cline usa `.clinerules`. O instalador lida com todos os formatos automaticamente.
...
# OpenCode
npx @evolvconsulting/evolv-coder-lite --opencode --global
...
npx @evolvconsulting/evolv-coder-lite --kilo --local

# Codex
npx @evolvconsulting/evolv-coder-lite --codex --global
npx @evolvconsulting/evolv-coder-lite --codex --local
...
# Augment
npx @evolvconsulting/evolv-coder-lite --augment --global     # Install to ~/.augment/
npx @evolvconsulting/evolv-coder-lite --augment --local      # Install to ./.augment/

# Trae
npx @evolvconsulting/evolv-coder-lite --trae --global        # Install to ~/.trae/
npx @evolvconsulting/evolv-coder-lite --trae --local         # Install to ./.trae/

# Cline
npx @evolvconsulting/evolv-coder-lite --cline --global       # Install to ~/.cline/
npx @evolvconsulting/evolv-coder-lite --cline --local        # Install to ./.clinerules
...
> [!TIP]
> Esse é o modo pensado para o eCL: aprovar `date` e `git commit` 50 vezes mata a produtividade.

---

## Como funciona

> **Já tem código?** Rode `/ecl-map-codebase` primeiro para analisar stack, arquitetura, convenções e riscos.
...
| Comando | O que faz |
|---------|-----------|
| `/ecl-new-project [--auto]` | Inicializa projeto completo |
| `/ecl-discuss-phase [N] [--auto] [--analyze] [--chain]` | Captura decisões antes do plano (`--chain` encadeia automaticamente em plan+execute) |
| `/ecl-plan-phase [N] [--auto] [--reviews]` | Pesquisa + plano + validação |
| `/ecl-execute-phase <N>` | Executa planos em ondas paralelas |
| `/ecl-verify-work [N]` | UAT manual |
| `/ecl-ship [N] [--draft]` | Cria PR da fase validada |
| `/ecl-progress --next` | Avança automaticamente para o próximo passo |
| `/ecl-fast <text>` | Tarefas triviais sem planejamento |
| `/ecl-complete-milestone` | Fecha o marco e marca release |
| `/ecl-new-milestone [name]` | Inicia próximo marco |
...
| `/ecl-pr-branch` | Cria branch limpa para PR |
| `/ecl-settings` | Configura perfis e agentes |
| `/ecl-config --profile <profile>` | Troca perfil (quality/balanced/budget/inherit) |
| `/ecl-quick [--full] [--discuss] [--research]` | Execução rápida com garantias do eCL (`--full` ativa todas as etapas, `--validate` ativa apenas verificação) |
| `/ecl-health [--repair]` | Verifica e repara `.planning/` |

> Para a lista completa de comandos e opções, use `/ecl-help`.
...
Você pode configurar no `/ecl-new-project` ou ajustar depois com `/ecl-settings`.
...
- Rode `/ecl-help`
- Reinstale com `npx @evolvconsulting/evolv-coder-lite@latest`
...
OpenCode, Gemini CLI, Kilo e Codex agora são suportados nativamente via `npx @evolvconsulting/evolv-coder-lite`.

| Projeto | Plataforma | Descrição |
|---------|------------|-----------|
| [ecl-opencode](https://github.com/rokicool/ecl-opencode) | OpenCode | Adaptação original para OpenCode |
| ecl-gemini (archived) | Gemini CLI | Adaptação original para Gemini por uberfuzzy |
...
**Claude Code é poderoso. O eCL o torna confiável.**
