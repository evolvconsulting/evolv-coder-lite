'use strict';
/**
 * Intel command router — CLI subcommand dispatcher for `ecl-tools intel`.
 *
 * ADR-959 (phase 4d-impl-4): intel command family cutover — last first-party
 * command cutover in the initial capability rollout.
 * Extracted from the hardcoded `case 'intel':` arm in ecl-tools.cjs.
...
function routeIntelCommand({ args, cwd, raw, error, _intel, _core }: RouteIntelCommandOptions): void {
  ...
  if (subcommand === 'query') {
    const term = args[2];
    if (!term) {
      error('Usage: ecl-tools intel query <term>', ERROR_REASON.USAGE);
      return;
    }
    ...
    if (!filePath) {
      error('Usage: ecl-tools intel patch-meta <file-path>', ERROR_REASON.USAGE);
      return;
    }
    ...
    const filePath = args[2];
    if (!filePath) {
      error('Usage: ecl-tools intel extract-exports <file-path>', ERROR_REASON.USAGE);
      return;
    }
