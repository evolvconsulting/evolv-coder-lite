    ...
    if (!agentStatus.agents_installed) {
      if ((agentStatus.installed_agents).length === 0) {
        addIssue(
          'warning',
          'W010',
          `No eCL agents found in ${agentStatus.agents_dir} — Task(subagent_type="ecl-*") will fall back to general-purpose`,
          `Run the eCL installer: npx ${PACKAGE_NAME}@latest`,
        );
        ...
        addIssue(
          ...
          `Re-run the eCL installer to complete the install: npx ${PACKAGE_NAME}@latest`,
        );
      } else if ((agentStatus.incomplete_agents).length > 0) {
        addIssue(
          'warning',
          'W010',
          `Missing ${(agentStatus.missing_agents).length} eCL agents: ${(agentStatus.missing_agents).join(', ')}; incomplete agent installs (missing generated file): ${(agentStatus.incomplete_agents).join(', ')} — affected workflows will fall back to general-purpose`,
          `Run the eCL installer: npx ${PACKAGE_NAME}@latest`,
        );
      } else {
        addIssue(
          'warning',
          'W010',
          `Missing ${(agentStatus.missing_agents).length} eCL agents: ${(agentStatus.missing_agents).join(', ')} — affected workflows will fall back to general-purpose`,
          `Run the eCL installer: npx ${PACKAGE_NAME}@latest`,
        );
      }
    }
      ...
      if (fs.existsSync(roadmapPath)) {
        ...
        for (const mm of mismatches) {
          addIssue(
            ...
            'Run `ecl-tools roadmap upgrade --convention milestone-prefixed` to migrate (dry-run by default)',
          );
        }
      }
    }
    ...
    for (const entry of entries) {
      ...
      if (!isCanonicalPlanningFile(entry.name)) {
        addIssue(
          'warning',
          'W019',
          `Unrecognized .planning/ file: ${entry.name} — not a canonical eCL artifact`,
  ...
  output(
    {
      ...
      // When skipCheck is true (ECL_SKIP_SCHEMA_CHECK=true), the gate is bypassed —
