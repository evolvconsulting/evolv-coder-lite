/**
 * Core type definitions for eCL-1 PLAN.md structures.
 *
 * These types model the YAML frontmatter + XML task bodies
 * that make up a eCL plan file.
 */
...
/**
 * JSON output from `ecl-tools.cjs init new-project`.
 * Describes project state and model configuration for the init workflow.
 */
export interface InitNewProjectInfo {
  /** Model resolved for the ecl-project-researcher agent. */
  researcher_model: string;
  /** Model resolved for the ecl-research-synthesizer agent. */
  synthesizer_model: string;
  /** Model resolved for the ecl-roadmapper agent. */
  ...
  /** Allow additional fields from ecl-tools evolution. */
  [key: string]: unknown;
}
...
/**
 * Options for creating a eCL instance.
 */
export interface GSDOptions {
  /** Root directory of the project. */
  projectDir: string;
  /** Path to ecl-tools.cjs. Falls back to the bundled repo path, then <projectDir>/.claude/, then ~/.claude/. */
  gsdToolsPath?: string;
  /**
   * Optional session correlation id for query mutation events when using {@link eCL.createTools}.
   */
  sessionId?: string;
...
/**
 * Phase types for eCL execution workflow.
 */
...
/**
 * Event types emitted by the eCL event stream.
...
/**
 * Base fields present on every eCL event.
 */
...
/**
 * Single phase entry from `ecl-tools.cjs roadmap analyze`.
 */
...
/**
 * Structured output from `ecl-tools.cjs roadmap analyze`.
 */
...
export interface InitConfig {
  /** Model for research sessions (overrides ecl-tools detected model). */
...
/**
 * Discriminated union of all eCL events.
 */
...
/**
 * Transport handler interface for consuming eCL events.
...
/**
 * Structured output from `ecl-tools.cjs init phase-op <N>`.
