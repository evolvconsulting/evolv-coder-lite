 ...
 *      so ECL_NOW_MS pins the written date deterministically in subprocess tests.
...
describe('bug-474: state date-stamping is pinned by ECL_NOW_MS', () => {
  let tmpDir;
  ...
  test('state advance-plan writes pinned date (not real today) when ECL_NOW_MS is set', () => {
    // AAA — Act: run advance-plan with a pinned historical timestamp
    const result = runGsdTools('state advance-plan', tmpDir, {
      ECL_TEST_MODE: '1',
      ECL_NOW_MS: String(PINNED_MS),
    });
...
// ─────────────────────────────────────────────────────────────────────────────
// §2  realClock ECL_NOW_MS invalid-input hardening
// ─────────────────────────────────────────────────────────────────────────────

/**
 * Verify that malformed / out-of-range ECL_NOW_MS values fall back to the real
...
describe('bug-474: realClock ECL_NOW_MS invalid-input hardening', () => {
  const realClock = require('../evolv-coder-lite/bin/lib/clock.cjs').realClock;
  ...
  before(() => {
    savedTestMode = process.env.ECL_TEST_MODE;
    savedNowMs = process.env.ECL_NOW_MS;
  ...
  after(() => {
    if (savedTestMode === undefined) {
      ...
      process.env.ECL_TEST_MODE = savedTestMode;
    }
    if (savedNowMs === undefined) {
      delete process.env.ECL_NOW_MS;
    } else {
      process.env.ECL_NOW_MS = savedNowMs;
    }
  });
  ...
  for (const [label, value] of INVALID_INPUTS) {
    test(`ECL_NOW_MS='${value}' (${label}) falls back to real clock — no crash, valid ISO`, () => {
      // AAA — Arrange
      process.env.ECL_NOW_MS = value;

      // AAA — Act + Assert: must not throw
      assert.doesNotThrow(
        () => realClock.nowIso(),
        `realClock.nowIso() must not throw for ECL_NOW_MS='${value}'`,
      );
      ...
      assert.ok(
        ...
        `realClock.nowIso() must return a valid ISO date for ECL_NOW_MS='${value}', got: ${iso}`,
      );
    });
  }

  test('ECL_NOW_MS valid decimal integer pins the clock', () => {
    // AAA — Arrange: a known historical epoch
    process.env.ECL_NOW_MS = String(PINNED_MS);
...
describe('bug-474: installer-migrations lock-loop timeout is deterministic via clock seam', () => {
  ...
  test('acquireInstallMigrationLock timeout path fires deterministically via makeFakeClock', (t) => {
    // AAA — Arrange
    const os = require('os');
    const { acquireInstallMigrationLock } = require('../evolv-coder-lite/bin/lib/installer-migrations.cjs');
