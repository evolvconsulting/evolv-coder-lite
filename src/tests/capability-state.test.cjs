...
const {
  ...
  _resolveManifest,
} = require('../evolv-coder-lite/bin/lib/capability-state.cjs');

// The real capability registry
const realRegistry = require('../evolv-coder-lite/bin/lib/capability-registry.cjs');
...
// ─── cmdCapabilityState — end-to-end via ecl-tools CLI ──────────────────────
//
// Because cmdCapabilityState destructures `output` at module load time, patching
// core.cjs after the fact is ineffective. We instead invoke ecl-tools via
...
describe('cmdCapabilityState — end-to-end via ecl-tools CLI', () => {
  let tmpConfigDir;
  let tmpConfigDirCore;
  let tmpConfigDirUiDisabled;

  before(() => {
    // Tmp runtime config dir without .ecl-profile (defaults to 'full')
    ...
    fs.writeFileSync(path.join(tmpConfigDirCore, '.ecl-profile'), 'core\n', 'utf8');

    tmpConfigDirUiDisabled = fs.mkdtempSync(path.join(os.tmpdir(), 'cap-state-cfg-ui-disabled-'));
    fs.writeFileSync(
      path.join(tmpConfigDirUiDisabled, '.ecl-surface.json'),
      JSON.stringify({
        baseProfile: 'full',
        disabledClusters: ['ui'],
        explicitAdds: [],
...
//
// In an installed runtime (e.g. Codex) the commands/ecl source tree is absent.
// Only <configDir>/skills/ecl-*/SKILL.md files exist. Prior to this fix,
...
// FAIL before the fix and PASS after, regardless of whether commands/ecl
...
describe('regressions: installed-runtime capability surface (#1160)', () => {
  ...
  test('_loadInstalledSkillsManifest: scans ecl-<stem>/SKILL.md dirs and produces stems', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'cap-ism-scan-'));
    try {
      // Create installed skill dirs
      const secureDir = path.join(tmpDir, 'skills', 'ecl-secure-phase');
      const validateDir = path.join(tmpDir, 'skills', 'ecl-validate-phase');
      ...
      // Add a non-ecl- dir that should be ignored
      ...
      assert.ok(!manifest.has('user-custom'), 'non-ecl- dir must not appear');
    } finally {
      cleanup(tmpDir);
    }
  });
  ...
  test('_loadInstalledSkillsManifest: parses requires from SKILL.md frontmatter', () => {
    ...
    try {
      const skillDir = path.join(tmpDir, 'skills', 'ecl-my-skill');
  ...
  test('_loadInstalledSkillsManifest: directory with no SKILL.md is NOT registered (parity with loadSkillsManifest)', () => {
    ...
    try {
      fs.mkdirSync(path.join(tmpDir, 'skills', 'ecl-nodoc'), { recursive: true });
  ...
  test('_resolveManifest: uses commandsGsdDir when it exists', () => {
    const realCommandsGsdDir = path.resolve(__dirname, '..', 'commands', 'ecl');
    ...
    try {
      ...
      assert.ok(manifest instanceof Map);
      // The real commands/ecl has many skills; manifest should be non-empty
  ...
  // These tests use a config dir that has NO .ecl-source and where _resolveCommandsGsdDir
  ...
  describe('end-to-end CLI with installed skill layout', () => {
    ...
    before(() => {
      ...
      fs.writeFileSync(path.join(tmpInstalledConfigDir, '.ecl-profile'), 'full\n', 'utf8');
      const secureDir = path.join(tmpInstalledConfigDir, 'skills', 'ecl-secure-phase');
      const validateDir = path.join(tmpInstalledConfigDir, 'skills', 'ecl-validate-phase');
      fs.mkdirSync(secureDir, { recursive: true });
      fs.mkdirSync(validateDir, { recursive: true });
      fs.writeFileSync(path.join(secureDir, 'SKILL.md'), makeSkillMd('secure-phase'), 'utf8');
      fs.writeFileSync(path.join(validateDir, 'SKILL.md'), makeSkillMd('validate-phase'), 'utf8');
    ...
    test('security capability: enabled=true in installed runtime with security_enforcement=true', () => {
      ...
      assert.strictEqual(result.status, 0, `ecl-tools exited ${result.status}:\nstdout: ${result.stdout}\nstderr: ${result.stderr}`);
      const envelope = JSON.parse(result.stdout);
  ...
  // ── TRUE installed-runtime layout: ecl-tools runs where commands/ecl is unreachable ──
  // The block above runs the repo's ecl-tools.cjs, where commands/ecl source IS
  // reachable by walk-up, so it cannot exercise the bug. This block copies the
  // runtime executable tree (evolv-coder-lite/bin + scripts + package.json) into a temp
  ...
  describe('true installed layout (commands/ecl unreachable)', () => {
    let installRoot;
    ...
    before(() => {
      ...
      fs.mkdirSync(path.join(installRoot, 'evolv-coder-lite'), { recursive: true });
      fs.cpSync(
        path.join(repoRoot, 'evolv-coder-lite', 'bin'),
        path.join(installRoot, 'evolv-coder-lite', 'bin'),
        { recursive: true },
      );
      fs.cpSync(
      ...
      installedGsdTools = path.join(installRoot, 'evolv-coder-lite', 'bin', 'ecl-tools.cjs');

      // 2. Installed runtime config dir: full profile + skills/ecl-*/SKILL.md only.
      installedConfigDir = fs.mkdtempSync(path.join(os.tmpdir(), 'cap-state-instcfg-'));
      fs.writeFileSync(path.join(installedConfigDir, '.ecl-profile'), 'full\n', 'utf8');
      for (const stem of ['secure-phase', 'validate-phase']) {
        const skillDir = path.join(installedConfigDir, 'skills', `ecl-${stem}`);
        fs.mkdirSync(skillDir, { recursive: true });
    ...
    test('commands/ecl is genuinely unreachable from the installed ecl-tools', () => {
      // Sanity: no commands/ecl anywhere under the install root.
      const probe = path.join(installRoot, 'commands', 'ecl');
      assert.strictEqual(fs.existsSync(probe), false, 'install root must have no commands/ecl');
    });
...
describe('isCapabilityActive — convenience predicate (Phase 2)', () => {
  ...
  test('isCapabilityActive returns same value as the entry active field (e2e)', () => {
    ...
    const { resolveCapabilityRuntimeState } = require('../evolv-coder-lite/bin/lib/capability-state.cjs');
...
describe('resolveCapabilityRuntimeState configOverride — honors caller snapshot instead of reading disk', () => {
  ...
  before(() => {
    ...
    savedEnv = {
      ECL_WORKSTREAM: process.env.ECL_WORKSTREAM,
      ECL_PROJECT: process.env.ECL_PROJECT,
      CLAUDE_CONFIG_DIR: process.env.CLAUDE_CONFIG_DIR,
    };
    delete process.env.ECL_WORKSTREAM;
    delete process.env.ECL_PROJECT;
  ...
  after(() => {
    // Restore env vars
    if (savedEnv.ECL_WORKSTREAM !== undefined) process.env.ECL_WORKSTREAM = savedEnv.ECL_WORKSTREAM;
    else delete process.env.ECL_WORKSTREAM;
    if (savedEnv.ECL_PROJECT !== undefined) process.env.ECL_PROJECT = savedEnv.ECL_PROJECT;
    else delete process.env.ECL_PROJECT;
...
// ─── Cross-runtime runtime detection (HIGH fix — ECL_RUNTIME → config.runtime → 'claude') ──────

describe('isCapabilityActive cross-runtime detection (ECL_RUNTIME → config.runtime → claude)', () => {
  // Verifies the HIGH bug fix: when ECL_RUNTIME='codex', resolveCapabilityRuntimeState
  // must consult the CODEX config dir (via CODEX_HOME), not ~/.claude.
  //
  // Fixture layout:
  //   CODEX_HOME        → tmpCodexDir/   ← .ecl-surface.json: full profile, graphify SURFACED
  //   CLAUDE_CONFIG_DIR → tmpClaudeDir/ ← .ecl-surface.json: full profile, graphify NOT surfaced
  //   ECL_RUNTIME=codex
  ...
  // Post-fix (detects 'codex' via ECL_RUNTIME): reads tmpCodexDir → surfaced → true (CORRECT).
  ...
  before(() => {
    ...
    fs.writeFileSync(
      path.join(tmpCodexDir, '.ecl-surface.json'),
    ...
    fs.writeFileSync(
      path.join(tmpClaudeDir, '.ecl-surface.json'),
  ...
  test('ECL_RUNTIME=codex → resolver consults CODEX_HOME, not CLAUDE_CONFIG_DIR (HIGH cross-runtime fix)', () => {
    prevGsdRuntime = process.env.ECL_RUNTIME;
    prevCodexHome = process.env.CODEX_HOME;
    prevClaudeConfigDir = process.env.CLAUDE_CONFIG_DIR;
    prevGsdWorkstream = process.env.ECL_WORKSTREAM;
    prevGsdProject = process.env.ECL_PROJECT;
    ...
    try {
      ...
      // With ECL_RUNTIME=codex, the resolver must look at CODEX_HOME (graphify surfaced)
      ...
      assert.strictEqual(
        active,
        true,
        'isCapabilityActive must return true when ECL_RUNTIME=codex and graphify is surfaced in CODEX_HOME — ' +
        'the pre-fix code hardcoded getGlobalConfigDir("claude") regardless of ECL_RUNTIME, returning false (BUG)',
      );
      ...
      assert.strictEqual(
        activeSwapped,
        false,
        'isCapabilityActive must return false when ECL_RUNTIME=codex and graphify is NOT surfaced in CODEX_HOME — ' +
        'even though claude dir has graphify surfaced, the codex dir is the authoritative surface',
      );
    } finally {
      // Restore env vars
      if (prevGsdRuntime === undefined) delete process.env.ECL_RUNTIME;
      else process.env.ECL_RUNTIME = prevGsdRuntime;
      ...
      if (prevGsdWorkstream === undefined) delete process.env.ECL_WORKSTREAM;
      else process.env.ECL_WORKSTREAM = prevGsdWorkstream;
      if (prevGsdProject === undefined) delete process.env.ECL_PROJECT;
      else process.env.ECL_PROJECT = prevGsdProject;
