...
const RULES = [
  ...
  },
  {
    ...
    // src/*.cts compiles into evolv-coder-lite/bin/lib/*.cjs; a source-only edit must
  ...
  },
  {
    ...
    match: path => path.startsWith('bin/') ||
      path.startsWith('evolv-coder-lite/bin/') ||
  ...
  },
  {
    ...
    match: path => path.startsWith('evolv-coder-lite/workflows/'),
    tests: [
...
function classify(files) {
  ...
  for (const file of files) {
    ...
    if (
      ['bin/', 'src/', 'evolv-coder-lite/', 'agents/', 'commands/', 'hooks/', 'tests/', 'scripts/'].some(p => file.startsWith(p)) ||
