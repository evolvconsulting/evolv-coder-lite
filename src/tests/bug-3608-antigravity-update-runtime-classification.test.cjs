/**
 * Bug #3608: /ecl:update must model Antigravity as a first-class runtime, so an
 ...
 * `evolv-coder-lite/bin/lib/update-context.cjs` (resolveUpdateContext). The
...
} = require(path.join(ROOT, 'evolv-coder-lite', 'bin', 'lib', 'update-context.cjs'));
const UPDATE_MD = path.join(ROOT, 'evolv-coder-lite', 'workflows', 'update.md');
...
describe('bug #3608 / #498: update-context models Antigravity as a first-class runtime', () => {
  ...
  test('behavioral: an Antigravity install resolves to runtime "antigravity", not "gemini"', () => {
    ...
    const verFile = normKey(path.join(agDir, 'evolv-coder-lite', 'VERSION'));
    const markerFile = normKey(path.join(agDir, 'evolv-coder-lite', 'workflows', 'update.md'));
    const fakeFs = {
