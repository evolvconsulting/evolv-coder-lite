...
/**
 * eCL Tools Tests - settings.json JSONC (JSON with comments) support
 *
...
//
// install.js prints a banner at module-load time (outside its ECL_TEST_MODE
// guard) — silence stdout for the duration of the require() so test output
// stays clean.  The main-logic block IS gated on ECL_TEST_MODE, so no
    ...
    "SessionStart": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "node ~/.claude/hooks/ecl-statusline.js"
          }
        ]
      }
    ]
...
describe('readSettings: JSON null coalesced to empty, malformed warns (#1191)', () => {
  ...
  test('valid JSON null content returns empty object with no malformed warning (real function)', () => {
    ...
    const tmpFile = path.join(os.tmpdir(), `ecl-settings-test-null-${process.pid}.json`);
  ...
  test('malformed content returns null and emits malformed warning (real function)', () => {
    ...
    const tmpFile = path.join(os.tmpdir(), `ecl-settings-test-broken-${process.pid}.json`);
  ...
  test('valid object content returns parsed object with no warning (real function)', () => {
    const tmpFile = path.join(os.tmpdir(), `ecl-settings-test-valid-${process.pid}.json`);
  ...
  test('absent file returns empty object with no warning (real function)', () => {
    const tmpFile = path.join(os.tmpdir(), `ecl-settings-test-absent-${process.pid}.json`);
