...
This is the authoritative roster of every shipped eCL Core surface. See the [docs index](README.md) to navigate by topic.
...
| `help.md` | Display the complete eCL Core command reference. | `/ecl-help` |
...
| `planner-load-graph-context.md` | Planner's load_graph_context step: knowledge-graph freshness + dependency-context query via the ecl_run launcher (extracted from ecl-planner.md). |
...
> **Subdirectory:** `evolv-coder-lite/references/few-shot-examples/` contains additional few-shot examples (`plan-checker.md`, `verifier.md`) that are referenced from specific agents. These are not counted in the 64 top-level references.
...
| `agent-command-router.cjs` | Thin CJS subcommand router adapter for `ecl-tools agent` |
| `artifacts.cjs` | Canonical artifact registry — known `.planning/` root file names; used by `ecl-health` W019 lint |
| `audit.cjs` | Audit dispatch, audit open sessions, audit storage helpers |
| `check-command-router.cjs` | Thin CJS subcommand router adapter for `ecl-tools check` |
...
| `legacy-cleanup.cjs` | Detect and remove leftover evolv-coder-lite-cc artifacts; exports `planLegacyCleanup` (pure scan) and `applyLegacyCleanup` (thin IO applier) that root out stale files from the old package across every eCL-managed runtime config directory (#607) |
...
| `package-identity.cjs` | Generated single source for eCL's published-package coordinates (npm name, bin name, repo slug, changelog URL, manual-install command), derived from package.json; read by the update worker, `check-latest-version`, and installer (#498) |
...
| `research-store.cjs` | Content-addressed research cache: sha256 keys, per-source TTL staleness, two-tier (user ~/.ecl / project .planning) store |
...
| `task-command-router.cjs` | Thin CJS subcommand router adapter for `ecl-tools task` |
| `template.cjs` | Template selection and filling with variable substitution |
| `uat.cjs` | UAT file parsing, verification debt tracking, audit-uat support |
| `ui-safety-gate.cjs` | Shell-free word-boundary UI token detector (#3706, #3718); reads phase-section text from stdin, exits 0 (UI found) or 1 (no UI); also deployed to `evolv-coder-lite/bin/lib/` so the eCL installer ships it to `$RUNTIME_DIR` (#448) |
| `update-context.cjs` | Pure install-context resolver for `/ecl:update` — runtime/scope/config-dir/version detection (LOCAL/GLOBAL/UNKNOWN) ported from update.md bash; backs `ecl-tools update-context` (#498) |
...
| `verification-command-router.cjs` | Thin CJS subcommand router adapter for `ecl-tools verification` |
...
| `ecl-cursor-session-start.js` | Cursor `sessionStart` | Cursor-native context injection at session start (issue #777) |
| `ecl-cursor-post-tool.js` | Cursor `postToolUse` | Cursor-native STATE.md update monitor after tool calls (issue #777) |
...
| `ecl-worktree-path-guard.js` | `PreToolUse` | Hard-blocks Edit/Write/MultiEdit with absolute paths outside the worktree root (PR #579, #260) |
| `ecl-config-reload.js` | `FileChanged` | Hot-reloads eCL config context when `.planning/config.json` changes mid-session (#770) |
