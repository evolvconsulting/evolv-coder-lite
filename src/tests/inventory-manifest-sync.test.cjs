...
const FAMILIES = [
  { name: 'agents',      dir: path.join(ROOT, 'agents'),                           filter: (f) => /^ecl-.*\.md$/.test(f),  toName: (f) => f.replace(/\.md$/, '') },
  { name: 'commands',    dir: path.join(ROOT, 'commands', 'ecl'),                  filter: (f) => f.endsWith('.md'),        toName: (f) => '/ecl-' + f.replace(/\.md$/, '') },
  { name: 'workflows',   dir: path.join(ROOT, 'evolv-coder-lite', 'workflows'),        filter: (f) => f.endsWith('.md'),        toName: (f) => f },
  { name: 'references',  dir: path.join(ROOT, 'evolv-coder-lite', 'references'),       filter: (f) => f.endsWith('.md'),        toName: (f) => f },
  { name: 'cli_modules', dir: path.join(ROOT, 'evolv-coder-lite', 'bin', 'lib'),       filter: (f) => f.endsWith('.cjs'),       toName: (f) => f },
