...
-->
<purpose>
Validate built features through conversational testing with persistent state. Creates UAT.md that tracks test progress, survives /clear, and feeds gaps into /ecl:plan-phase --gaps.
...
<template>
@~/.claude/evolv-coder-lite/templates/UAT.md
</template>
...
Provide a phase number to start testing (e.g., /ecl:verify-work 4)
```
...
```bash
PLAN_HOOKS_JSON=$(ecl_run loop render-hooks plan:pre --raw)
...
**MVP-mode UAT framing.** When `MVP_MODE=true`, follow the rules in `@~/.claude/evolv-coder-lite/references/verify-mvp-mode.md`. Briefly:
...
```
Skill(skill="ecl-${ref.skill}", args="{phase}")
```
...
```
⚠ Security enforcement enabled — /ecl:secure-phase {phase} did not produce SECURITY.md.
...
- `/ecl:secure-phase {phase}` — security review (required before advancing)
- `/ecl:plan-phase {next}` — Plan next phase
- `/ecl:execute-phase {next}` — Execute next phase
- `/ecl:ui-review {phase}` — visual quality audit (if frontend files were modified)
```
...
Read and follow `~/.claude/evolv-coder-lite/workflows/transition.md`.
...
- `/ecl:plan-phase {next}` — Plan next phase
- `/ecl:execute-phase {next}` — Execute next phase
- `/ecl:secure-phase {phase}` — security review
- `/ecl:ui-review {phase}` — visual quality audit (if frontend files were modified)
```
</step>
...
If user declines: stop. User resolves items and re-runs `/ecl:verify-work`.
...
- Follow @~/.claude/evolv-coder-lite/workflows/diagnose-issues.md
...
Spawn ecl-planner in --gaps mode:
...
<downstream_consumer>
Output consumed by /ecl:execute-phase
...
Spawn ecl-planner with revision context:
...
3. Abandon (exit, user runs /ecl:plan-phase manually)
...
`/clear` then `/ecl:execute-phase {phase} --gaps-only`
...
- [ ] If issues: ecl-planner creates fix plans (gap_closure mode)
- [ ] If issues: ecl-plan-checker verifies fix plans
- [ ] If issues: revision loop until plans pass (max 3 iterations)
- [ ] Ready for `/ecl:execute-phase --gaps-only` when complete
</success_criteria>
