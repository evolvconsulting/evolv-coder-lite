 ...
 *
 * Enforces the commands/ecl/*.md contract across all 65 command files:
 *
 *   1. name:        present, non-empty, matches ecl: or ecl- prefix
...
function check(filePath) {
  ...
  // 1. name: present + ecl: / ecl- prefix
  if (!fm.name || !fm.name.trim()) {
    violations.push('name: field missing or empty');
  } else if (!/^ecl[:-]/.test(fm.name.trim())) {
    violations.push(`name: must start with "ecl:" or "ecl-", got "${fm.name.trim()}"`);
  }
  ...
  for (const { token, normalized, trailingProse } of refs) {
    const absPath = path.join(ECL_ROOT, normalized);
