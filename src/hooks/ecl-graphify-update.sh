...
#      the exact `ecl-sdk query commit` command shape; the SDK command invokes
...
# Gate 2 — HEAD-advancing git op (shell-direct or exact `ecl-sdk query commit`)
case "$COMMAND" in
  *"git commit"*|*"git merge"*|*"git pull"*|*"git rebase --continue"*|*"git cherry-pick"*) ;;
  *"ecl-sdk query commit"|*"ecl-sdk query commit "*) ;;
  *) exit 0 ;;
esac
