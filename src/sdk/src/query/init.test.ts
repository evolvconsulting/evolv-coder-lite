...
beforeEach(async () => {
  tmpDir = await mkdtemp(join(tmpdir(), 'ecl-init-'));
...
describe('withProjectRoot', () => {
  ...
  // directory (~/.claude/evolv-coder-lite/agents) while the installer writes to
  // ~/.claude/agents, causing agents_installed: false even on clean installs.
  it('reports agents_installed: true when all expected agents exist in ECL_AGENTS_DIR', async () => {
    ...
    const prev = process.env.ECL_AGENTS_DIR;
    process.env.ECL_AGENTS_DIR = agentsDir;
    try {
      const enriched = withProjectRoot(tmpDir, {});
      expect(enriched.agents_installed).toBe(true);
      expect(enriched.missing_agents).toEqual([]);
    } finally {
      if (prev === undefined) delete process.env.ECL_AGENTS_DIR;
      else process.env.ECL_AGENTS_DIR = prev;
    }
  });

  it('reports missing agents when ECL_AGENTS_DIR is empty', async () => {
  ...
  it('honors CLAUDE_CONFIG_DIR when ECL_AGENTS_DIR is unset', async () => {
    const { MODEL_PROFILES } = await import('./config-query.js');
    ...
    const prevAgents = process.env.ECL_AGENTS_DIR;
    ...
    try {
      ...
      if (prevAgents === undefined) delete process.env.ECL_AGENTS_DIR;
      else process.env.ECL_AGENTS_DIR = prevAgents;
      if (prevClaude === undefined) delete process.env.CLAUDE_CONFIG_DIR;
      else process.env.CLAUDE_CONFIG_DIR = prevClaude;
    }
  });

  // #2402 — runtime-aware resolution: ECL_RUNTIME selects which runtime's
  // config-dir env chain to consult, so non-Claude installs stop
  // false-negating.
  it('ECL_RUNTIME=codex resolves agents under CODEX_HOME/agents', async () => {
    const { MODEL_PROFILES } = await import('./config-query.js');
    ...
    const prevAgents = process.env.ECL_AGENTS_DIR;
    const prevRuntime = process.env.ECL_RUNTIME;
    ...
    try {
      ...
      if (prevRuntime === undefined) delete process.env.ECL_RUNTIME;
      else process.env.ECL_RUNTIME = prevRuntime;
      if (prevCodex === undefined) delete process.env.CODEX_HOME;
      else process.env.CODEX_HOME = prevCodex;
    }
  });

  it('config.runtime drives detection when ECL_RUNTIME is unset', async () => {
    const { MODEL_PROFILES } = await import('./config-query.js');
  ...
  it('ECL_RUNTIME wins over config.runtime', async () => {
    const { MODEL_PROFILES } = await import('./config-query.js');
  ...
  it('unknown ECL_RUNTIME falls through to config/Claude default', () => {
    const prevAgents = process.env.ECL_AGENTS_DIR;
    const prevRuntime = process.env.ECL_RUNTIME;
    delete process.env.ECL_AGENTS_DIR;
    process.env.ECL_RUNTIME = 'not-a-runtime';
    try {
  ...
  it('ECL_AGENTS_DIR takes precedence over CLAUDE_CONFIG_DIR', async () => {
    const { MODEL_PROFILES } = await import('./config-query.js');
    ...
    const prevAgents = process.env.ECL_AGENTS_DIR;
    const prevClaude = process.env.CLAUDE_CONFIG_DIR;
    process.env.ECL_AGENTS_DIR = winningDir;
...
describe('initExecutePhase', () => {
  ...
  it('keeps same-milestone archived phase directory instead of nulling it (#3469)', async () => {
    const tmp = await mkdtemp(join(tmpdir(), 'ecl-init-3469-'));
    try {
...
describe('initPlanPhase', () => {
  ...
  // /ecl-plan-phase workflow can short-circuit on closed phases instead of
...
describe('initMilestoneOp', () => {
  ...
  it('derives phase_count from ROADMAP current milestone, not on-disk dirs (#2633)', async () => {
    ...
    const fresh = await mkdtemp(join(tmpdir(), 'ecl-init-2633-'));
    try {
