 ...
 *   - SDK callers that require() ecl-tools.cjs as a module
...
const state = require('../evolv-coder-lite/bin/lib/state.cjs');
...
describe('buildStateFrontmatter cache invalidation (#1967)', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-1967-cache-'));
