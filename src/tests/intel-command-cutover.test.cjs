 ...
 * `case 'intel':` arm in ecl-tools.cjs to the capability registry dispatch
...
const { routeIntelCommand } = require('../evolv-coder-lite/bin/lib/intel-command-router.cjs');
...
function runJsonErrors(args, tmpDir, env = {}) {
  const result = runGsdTools(args, tmpDir, { ...env, ECL_JSON_ERRORS: '1' });
  assert.strictEqual(result.success, false,
    `Expected failure with ECL_JSON_ERRORS=1 for args: ${args.join(' ')}\n` +
  ...
  try {
    ...
    throw new Error(
      `ECL_JSON_ERRORS=1 must emit valid JSON on stderr.\n` +
...
describe('intel router: unit tests via recording mocks', () => {
  ...
  test('routeIntelCommand query: missing term calls error(USAGE)', () => {
    ...
    assert.ok(errFn.calls[0].msg.includes('ecl-tools intel query <term>'),
  ...
  test('routeIntelCommand patch-meta: missing filePath calls error(USAGE)', () => {
    ...
    assert.ok(errFn.calls[0].msg.includes('ecl-tools intel patch-meta <file-path>'),
      'usage error must mention correct usage');
    assert.strictEqual(errFn.calls[0].reason, 'usage',
      'reason must be "usage" (ERROR_REASON.USAGE)');
    assert.strictEqual(c.outputCalls.length, 0, 'core.output must not be called on usage error');
  ...
  test('routeIntelCommand extract-exports: missing filePath calls error(USAGE)', () => {
    ...
    assert.strictEqual(errFn.calls.length, 1, 'error must be called for missing filePath');
    assert.ok(errFn.calls[0].msg.includes('ecl-tools intel extract-exports <file-path>'),
      'usage error must mention correct usage');
    assert.strictEqual(errFn.calls[0].reason, 'usage',
      'reason must be "usage" (ERROR_REASON.USAGE)');
    assert.strictEqual(c.outputCalls.length, 0, 'core.output must not be called on usage error');
...
describe('intel cutover: dispatch path (default-case → capability registry)', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempProject('ecl-intel-cutover-');
  });
...
describe('intel cutover: output shape equivalence', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempProject('ecl-intel-behavior-');
  });
...
describe('intel cutover: error paths (exit non-zero + correct messages)', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempProject('ecl-intel-err-');
  });
  ...
  test('intel query (missing term): exits non-zero, emits usage message', () => {
    ...
    assert.ok(
      output.includes('ecl-tools intel query <term>'),
  ...
  test('intel patch-meta (missing filePath): exits non-zero, emits usage message', () => {
    ...
    const output = result.error + result.output;
    assert.ok(
      output.includes('ecl-tools intel patch-meta <file-path>'),
  ...
  test('intel extract-exports (missing filePath): exits non-zero, emits usage message', () => {
    ...
    const output = result.error + result.output;
    assert.ok(
      output.includes('ecl-tools intel extract-exports <file-path>'),
      `Must emit usage hint for missing filePath. Got: ${output}`,
    );
  });
...
// ─── 5. JSON-ERRORS — ECL_JSON_ERRORS mode ───────────────────────────────────

describe('intel cutover: ECL_JSON_ERRORS structured error payloads', () => {
  let tmpDir;

  beforeEach(() => {
    tmpDir = createTempProject('ecl-intel-jsonerr-');
  });
  ...
  test('intel query (missing term) with ECL_JSON_ERRORS=1 emits {ok:false,reason:"usage"}', () => {
    const parsed = runJsonErrors(['intel', 'query'], tmpDir);
    assertTypedError(parsed, 'usage', 'intel query missing term');
    assert.ok(parsed.message.includes('ecl-tools intel query <term>'),
      'usage message must include the usage hint');
  });

  test('intel patch-meta (missing filePath) with ECL_JSON_ERRORS=1 emits {ok:false,reason:"usage"}', () => {
    const parsed = runJsonErrors(['intel', 'patch-meta'], tmpDir);
    assertTypedError(parsed, 'usage', 'intel patch-meta missing filePath');
    assert.ok(parsed.message.includes('ecl-tools intel patch-meta <file-path>'),
      'usage message must include the usage hint');
  });

  test('intel extract-exports (missing filePath) with ECL_JSON_ERRORS=1 emits {ok:false,reason:"usage"}', () => {
    ...
    assert.ok(parsed.message.includes('ecl-tools intel extract-exports <file-path>'),
      'usage message must include the usage hint');
  });

  test('intel unknown subcommand with ECL_JSON_ERRORS=1 emits {ok:false,reason:"sdk_unknown_command"}', () => {
  ...
  test('intel status (disabled gate) with ECL_JSON_ERRORS=1 does NOT emit error payload (succeeds)', () => {
    // The disabled gate is not a CLI error — it exits 0 with a JSON payload
    const result = runGsdTools(['intel', 'status'], tmpDir, { ECL_JSON_ERRORS: '1' });
    assert.ok(result.success,
      `intel status disabled gate must exit 0 with ECL_JSON_ERRORS=1. stderr: ${result.error}`);
  });
});
