...
/**
 * Regression test for issue #4 (evolvconsulting/evolv-coder-lite):
 ...
 *   - Issue #4 (evolvconsulting/evolv-coder-lite)
 */
...
const phaseModule = require('../evolv-coder-lite/bin/lib/phase.cjs');
 ...
 */
function createFixture(prefix = 'ecl-4-regression-') {
...
function capturePhaseComplete(cwd, phaseNum) {
  // We invoke ecl-tools directly for the full CJS path, but with ECL_DISABLE_SDK_BRIDGE=1
...
function create4ColFixture(existingDate, alreadyComplete = true) {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-1161-4col-'));
...
function create5ColFixture(existingDate, alreadyComplete = true) {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-1161-5col-'));
  const planDir = path.join(tmpDir, '.planning');
  const phasesDir = path.join(planDir, 'phases');
  fs.mkdirSync(phasesDir, { recursive: true });
...
const PINNED_CLOCK_ENV = {
  ECL_TEST_MODE: '1',
  ECL_NOW_MS: String(PINNED_MS_1161),
};
...
function createVerificationFixture(verFmStatus) {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-1159-verif-'));
  const planDir = path.join(tmpDir, '.planning');
  const phasesDir = path.join(planDir, 'phases');
  const phase01Dir = path.join(phasesDir, '01-foundation');
  fs.mkdirSync(phase01Dir, { recursive: true });
...
function createDeferredReqFixture({ includeMissingActive = false } = {}) {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-1159-deferred-'));
  const planDir = path.join(tmpDir, '.planning');
  const phasesDir = path.join(planDir, 'phases');
  const phase01Dir = path.join(phasesDir, '01-foundation');
  fs.mkdirSync(phase01Dir, { recursive: true });
...
describe('issue #1159 (Defect B): deferred/future requirement IDs must not trigger traceability warning', () => {
  ...
  test(
    ...
    () => {
      ...
      tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-1159-subhead-'));
      const planDir = path.join(tmpDir, '.planning');
      const phasesDir = path.join(planDir, 'phases');
      const phase01Dir = path.join(phasesDir, '01-foundation');
      fs.mkdirSync(phase01Dir, { recursive: true });
