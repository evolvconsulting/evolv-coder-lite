...
const SCAN_DIRS = [
  'agents',
  'commands',
  'evolv-coder-lite/workflows',
  'evolv-coder-lite/bin/lib',
  'hooks',
];
...
const ALLOWLIST = new Set([
  'evolv-coder-lite/bin/lib/security.cjs',        // The security module itself
  'evolv-coder-lite/workflows/discuss-phase.md',  // Large workflow (~50K) with power mode + i18n
  'evolv-coder-lite/workflows/new-project.md',     // Large workflow (~50K) — agent install, runtime detect, brownfield map, #3491 worktree gating
  'evolv-coder-lite/workflows/execute-phase.md',  // Large orchestration workflow (~51K) with wave execution + code-review gate
  'evolv-coder-lite/workflows/plan-phase.md',      // Large orchestration workflow (~51K) with TDD mode integration
  'hooks/ecl-prompt-guard.js',                  // The prompt guard hook
...
describe('codebase prompt injection scan', () => {
  ...
  test('agent definition files are clean (injection patterns)', () => {
    ...
    for (const file of agentFiles) {
      ...
      // (path.relative returns 'evolv-coder-lite\bin\...' on win32; allowlist
      // keys are POSIX 'evolv-coder-lite/bin/...').
