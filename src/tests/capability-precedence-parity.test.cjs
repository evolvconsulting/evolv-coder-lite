...
const LIB = path.join(__dirname, '..', 'evolv-coder-lite', 'bin', 'lib');
...
describe('capability-precedence-parity: behavioral matrix', () => {
  ...
  function withCleanEnv(fn) {
    ...
    try {
      ...
      if (saved.ECL_WORKSTREAM !== undefined) process.env.ECL_WORKSTREAM = saved.ECL_WORKSTREAM;
      else delete process.env.ECL_WORKSTREAM;
      if (saved.ECL_PROJECT !== undefined) process.env.ECL_PROJECT = saved.ECL_PROJECT;
  ...
  // Without ECL_WORKSTREAM, planningDir === planningRoot, so levels 2 and 3 point
  // to the same file. Level-3 distinction is only exercisable by setting ECL_WORKSTREAM.
  // We test the "paths same → no double-read" path (covered by level-2 test above)
  // and the "paths differ" path via ECL_WORKSTREAM.
  test('level-3: root config.json read when workstream path differs, parity holds', () => {
    // Use ECL_WORKSTREAM to force planningDir to differ from planningRoot.
    withCleanEnv(() => {
      ...
      process.env.ECL_WORKSTREAM = wsName;
