...
/**
 * Tests for modular decomposition of agents/ecl-planner.md
 *
 * Verifies that:
 *   1. ecl-planner.md stays under the 100K agent file threshold
 *   2. ecl-planner.md is under 45K chars (proving the three mode sections were extracted)
 *   3. The three reference files exist
 *   4. ecl-planner.md contains reference pointers to each extracted file
...
const PLANNER_EXTRACTED_LIMIT = 50 * 1024;  // 50K — proves extraction happened (eCL: +2K vs upstream 48K to absorb rebrand-expansion of the product name)

// ─── File paths ──────────────────────────────────────────────────────────────

const PLANNER_PATH = path.join(PROJECT_ROOT, 'agents', 'ecl-planner.md');
const GAP_CLOSURE_REF = path.join(PROJECT_ROOT, 'evolv-coder-lite', 'references', 'planner-gap-closure.md');
const REVISION_REF = path.join(PROJECT_ROOT, 'evolv-coder-lite', 'references', 'planner-revision.md');
const REVIEWS_REF = path.join(PROJECT_ROOT, 'evolv-coder-lite', 'references', 'planner-reviews.md');

// ─── ecl-planner.md size ─────────────────────────────────────────────────────

describe('ecl-planner.md size constraints', () => {
  ...
  test('planner is under 100K chars (agent file threshold)', () => {
    ...
    assert.ok(
      ...
      `ecl-planner.md is ${content.length} chars, exceeds 100K agent threshold`
    );
  });
  ...
  test('planner is under 45K chars (proves mode sections were extracted)', () => {
    ...
    assert.ok(
      ...
      `ecl-planner.md is ${content.length} chars, expected < 45K after extracting mode sections`
    );
  });
});
...
// ─── ecl-planner.md contains reference pointers ──────────────────────────────

describe('ecl-planner.md contains reference pointers to extracted files', () => {
  let plannerContent;
  ...
  test('planner references planner-gap-closure.md', () => {
    ...
    assert.ok(
      ...
      'ecl-planner.md must reference planner-gap-closure.md'
    );
  });
  ...
  test('planner references planner-revision.md', () => {
    ...
    assert.ok(
      ...
      'ecl-planner.md must reference planner-revision.md'
    );
  });
  ...
  test('planner references planner-reviews.md', () => {
    ...
    assert.ok(
      ...
      'ecl-planner.md must reference planner-reviews.md'
    );
  });
});
