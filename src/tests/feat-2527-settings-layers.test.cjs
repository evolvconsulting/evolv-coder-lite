...
/**
 * Feature test for #2527 — /ecl-settings expands to 22 settings grouped into
 ...
 * step, the confirmation table, the ~/.ecl/defaults.json save step, and
 * VALID_CONFIG_KEYS.
 *
 * Closes: #2527
 */
...
const SETTINGS_PATH = path.join(__dirname, '..', 'evolv-coder-lite', 'workflows', 'settings.md');
    ...
    test('save_as_defaults step references every new field', () => {
      ...
      for (const field of NEW_FIELDS) {
        assert.ok(
          ...
          `save_as_defaults step must persist "${field}" into ~/.ecl/defaults.json`
        );
      }
    });
  });
