...
const { resolveUpdateContext } = require(
  path.join(ROOT, 'evolv-coder-lite', 'bin', 'lib', 'update-context.cjs'),
);
...
function ver(dir) { return `${dir}/evolv-coder-lite/VERSION`; }
function marker(dir) { return `${dir}/evolv-coder-lite/workflows/update.md`; }
...
describe('ecl-tools update-context (CLI): emits the JSON contract', () => {
  test('--config-dir fixture resolves to the documented 4-field JSON', () => {
    const tmp = nodeFs.mkdtempSync(path.join(os.tmpdir(), 'ecl-uc-'));
    try {
      nodeFs.mkdirSync(path.join(tmp, 'evolv-coder-lite', 'workflows'), { recursive: true });
      nodeFs.writeFileSync(path.join(tmp, 'evolv-coder-lite', 'VERSION'), '1.42.0\n');
      nodeFs.writeFileSync(path.join(tmp, 'evolv-coder-lite', 'workflows', 'update.md'), 'x');
      const out = execFileSync(
        process.execPath,
        [ECL_TOOLS, 'update-context', '--config-dir', tmp, '--runtime', 'kilo', '--json'],
        { encoding: 'utf8', env: { ...process.env, ECL_TEST_MODE: '1' } },
      );
...
describe('resolveUpdateContext: parity with the old inline bash (adversarial-review)', () => {
  test('preferredConfigDir with a leading ~/ is expanded before the fast path', () => {
    ...
    // custom --config-dir like ~/custom-ecl must resolve, not fall to UNKNOWN.
    const fs = fakeFs({
      [ver(`${HOME}/custom-ecl`)]: '1.41.0\n',
      [marker(`${HOME}/custom-ecl`)]: 'x',
    });
    const r = resolveUpdateContext({
      home: HOME, cwd: CWD, env: {}, fs, preferredConfigDir: '~/custom-ecl',
    });
    assert.equal(r.installedVersion, '1.41.0');
    assert.equal(r.scope, 'GLOBAL');
    assert.ok(sameDir(r.gsdDir, `${HOME}/custom-ecl`), `gsdDir was ${r.gsdDir}`);
  });
