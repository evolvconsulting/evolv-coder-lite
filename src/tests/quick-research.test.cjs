/**
 * eCL Quick Research Flag Tests
 *
 * Validates the --research flag for /ecl-quick:
 ...
 * - Workflow spawns ecl-phase-researcher for research
 */
...
describe('quick command: --research in frontmatter', () => {
  ...
  test('quick.md exists', () => {
    assert.ok(fs.existsSync(commandPath), 'commands/ecl/quick.md should exist');
  });
...
describe('quick workflow: research step', () => {
  ...
  test('research step spawns ecl-phase-researcher', () => {
    content = fs.readFileSync(workflowPath, 'utf-8');
    ...
    assert.ok(
      researchSection.includes('subagent_type="ecl-phase-researcher"'),
      'research step should spawn ecl-phase-researcher agent'
    );
  });
