# `ecl capability` Command Reference

> **Slash form:** `ecl:capability` (surfaced as a slash command on slash-command runtimes)
> **CLI form:** `ecl capability`
...
The `capability` family manages the installation, upgrade, removal, and inspection of eCL capabilities — both first-party and third-party overlays. A row for this command also appears in [docs/COMMANDS.md](../COMMANDS.md) (that file is not edited here).
...
```
ecl capability install <spec> [--integrity sha512-<hash>] [--scope global|project] [--yes]
```
...
| `--scope` | `global` \| `project` | `global` | Installation root. `global` writes to `~/.ecl/capabilities/<id>/`; `project` writes to `.ecl/capabilities/<id>/` in the current working directory. |
...
The ledger file (`~/.claude/.ecl-capabilities.json` for the global scope on the Claude runtime, and analogues per runtime) records the installed version, source, integrity hash, owned files, and any fragments written into shared files (e.g. hook registrations in `settings.json`).
...
```
ecl capability remove <id> [--purge-data]
```
...
First-party capabilities (shipped with eCL) cannot be removed via this subcommand; the entire product uninstall path (`ecl --uninstall`) handles first-party removal.
...
Clears the disabled flag in the ledger entry for `<id>`. On the next eCL invocation, the capability is included in the runtime overlay subject to its `engines.ecl` range. If the eCL version has changed since the capability was disabled, the `engines.ecl` check is re-evaluated at load time; an incompatible capability is skipped with a warning.
...
Lists all capabilities visible to the current eCL session: first-party capabilities (shipped with eCL) and installed overlay capabilities in both global and project scopes. Disabled capabilities are included with a `disabled` status.
...
| `disabled` | Present in the ledger but excluded via `ecl capability disable`. |
| `incompatible` | `engines.ecl` range does not satisfy the current eCL version; skipped with a warning at load time. |
...
|---|---|---|
| Registry name | `my-cap@ecl-registry` | Registry — fetches the capability bundle from the named registry; `integrity` is populated from the registry catalogue. |
...
| npm package | `npm:@org/ecl-capability-foo@^1.0.0` | npm — resolves via `npm dist-tags` / semver range; installs with `--ignore-scripts`. |
...
All forms pass through the same pipeline: fetch → verify integrity or SHA pin → check `engines.ecl` → obtain consent → validate → extract → record ledger.
...
|---|---|---|
| `global` | `~/.ecl/capabilities/<id>/` | Per-runtime, e.g. `~/.claude/.ecl-capabilities.json` |
| `project` | `.ecl/capabilities/<id>/` (CWD) | Per-runtime, adjacent to project root |

The ledger is the commit point for installs and upgrades. Its entries record the installed version, original source URL, integrity hash, owned files, and shared-file edits. A reconciliation sweep on the next eCL run resolves crash orphans (files on disk without a ledger entry, or ledger entries with missing files).
