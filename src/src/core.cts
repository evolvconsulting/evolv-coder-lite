 ...
 * @param prefix - filename prefix to match (e.g., 'ecl-')
 * @param opts
 ...
 */
function reapStaleTempFiles(prefix = 'ecl-', { maxAgeMs = 5 * 60 * 1000, dirsOnly = false }: ReapOptions = {}): void {
  try {
    ensureGsdTempDir();
    const now = Date.now();
 ...
 *
 * Priority:
 *   1. ECL_AGENTS_DIR env var (explicit override, any runtime)
 *   2. For claude runtime: __dirname-relative path (agents/ sibling of evolv-coder-lite/)
 *      This is correct for both repo runs and real installs (the runtime config dir's
 *      agents/ folder) because ecl-tools.cjs lives inside evolv-coder-lite/bin/ in both cases.
 *   3. For non-claude runtimes: getGlobalConfigDir(runtime)/agents
 *
 * @param runtime - the active runtime name; defaults to ECL_RUNTIME env, then 'claude'
 */
function getAgentsDir(runtime?: string): string {
  if (process.env['ECL_AGENTS_DIR']) {
    return process.env['ECL_AGENTS_DIR'];
  }
  const resolved = runtime ?? (process.env['ECL_RUNTIME'] || 'claude');
  if (resolved === 'claude') {
...
function checkAgentsInstalled(runtime?: string): AgentsInstalledResult {
  const resolvedRuntime = runtime ?? (process.env['ECL_RUNTIME'] || 'claude');
  ...
  if (policy && typeof policy === 'object' && !Array.isArray(policy)) {
    ...
    if (provider && typeof provider === 'string' &&
      ...
      if (!_warnedConfigKeys.has(pkey)) {
        ...
        try {
          process.stderr.write(
            `ecl: warning — model_policy.provider has unknown value "${provider}". ` +
      ...
      for (const [pruntime, tierMap] of Object.entries(rtOverrides as Record<string, unknown>)) {
        if (!(KNOWN_RUNTIMES).has(pruntime)) {
          ...
          if (!_warnedConfigKeys.has(key)) {
            ...
            try {
              process.stderr.write(
                `ecl: warning — model_policy.runtime_tiers.${pruntime}.* uses ` +
        ...
        for (const tierName of Object.keys(tierMap)) {
          if (!RUNTIME_OVERRIDE_TIERS.has(tierName)) {
            ...
            if (!_warnedConfigKeys.has(key)) {
              ...
              try {
                process.stderr.write(
                  `ecl: warning — model_policy.runtime_tiers.${pruntime}.${tierName} ` +
...
function getMilestonePhaseFilter(cwd: string, versionOverride?: string | null): MilestonePhaseFilter {
  ...
  try {
    ...
    if (!hasVersionedMilestonesGlobal && hasPhaseHeadings) {
      console.warn(
        '[ecl] Deprecated: free-form ROADMAP.md detected (no versioned milestone headings). ' +
