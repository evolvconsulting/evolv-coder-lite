...
const { isCentralConfigKey } = require('../evolv-coder-lite/bin/lib/config-schema.cjs');
...
describe('ADR-857 Phase 6 capstone conformance (#1139)', () => {
  ...
  test('every declared capability hook point has a render-hooks call site in the host loop (#1168)', () => {
    ...
    assert.deepEqual(
      ...
      `but no host-loop workflow calls \`ecl_run loop render-hooks <point>\`, so the hooks ` +
  ...
  test('host loop bodies are materially smaller than the pre-phase-6 baseline (#1168)', () => {
    ...
    const PRE_PHASE6 = { 'plan-phase.md': 94519, 'execute-phase.md': 93800 };  // eCL: execute-phase frozen +rebrand byte-expansion over upstream 93166 (rebranded body is 93,613)
    const notShrunk = [];
    for (const [file, frozen] of Object.entries(PRE_PHASE6)) {
      const now = lfByteCount(path.join(ROOT, 'evolv-coder-lite', 'workflows', file));
...
describe('ADR-857 phase 6 — capabilities must not bake install paths into the registry', () => {
  // Matches eCL install paths that LEAK when copied verbatim to non-Claude runtimes.
  // (~/.claude/projects is a legit runtime feature and is intentionally NOT matched.)
  const LEAK = /\.claude[/\\](?:evolv-coder-lite|commands|agents|hooks)\b/;
  ...
  test('generated capability-registry.cjs contains no ~/.claude install path', () => {
    const reg = fs.readFileSync(path.join(__dirname, '..', 'evolv-coder-lite', 'bin', 'lib', 'capability-registry.cjs'), 'utf8');
  ...
  test('every plan:pre planner contribution is injected generically (not per-capId hardcode)', () => {
    ...
    // delivery mechanism.
    const planPhase = readRepoFile('evolv-coder-lite/workflows/plan-phase.md');
  ...
  test('every declared gate check.query returns a uniform boolean `block` field', () => {
    ...
    const gsdTools = path.join(ROOT, 'evolv-coder-lite', 'bin', 'ecl-tools.cjs');
    const failures = [];
