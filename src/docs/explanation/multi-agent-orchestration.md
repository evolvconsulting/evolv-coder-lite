# Multi-agent orchestration in eCL Core

> **Explanation** — This document describes *why* eCL Core is designed around
...
eCL Core's multi-agent design is a direct response to that problem. Instead of
...
Every workflow in `evolv-coder-lite/workflows/` follows the same shape:

```text
Orchestrator (workflow .md file)
    │
    ├── Load context
    │   ecl-tools.cjs init <workflow> <phase>
    │   → JSON: project info, config, state, phase details
    │
    ├── Resolve model
    │   ecl-tools.cjs resolve-model <agent-name>
    ...
    │
    └── Update state
        ecl-tools.cjs state update / state patch / state advance-plan
```
...
eCL Core's agents fall into functional categories that map onto the
research → plan → execute → verify pipeline:

| Category | Agents | Typical parallelism |
|---|---|---|
| Researchers | `ecl-project-researcher`, `ecl-phase-researcher`, `ecl-ui-researcher`, `ecl-advisor-researcher` | 4 parallel (stack, features, architecture, pitfalls) |
| Synthesisers | `ecl-research-synthesizer` | Sequential, after researchers complete |
| Planners | `ecl-planner`, `ecl-roadmapper` | Sequential |
| Checkers | `ecl-plan-checker`, `ecl-integration-checker`, `ecl-ui-checker`, `ecl-nyquist-auditor` | Sequential, up to 3 revision iterations |
| Executors | `ecl-executor` | Parallel within a wave, sequential across waves |
| Verifiers | `ecl-verifier` | Sequential, after all executors complete |
| Mappers | `ecl-codebase-mapper` | 4 parallel sub-probes |
...
The most visible expression of multi-agent design is how `/ecl-execute-phase`
...
window), eCL workflows survive context resets (`/clear`), tab switches, and
...
working in one context would finish faster for simple tasks. eCL mitigates
...
payloads. This is the cost of isolation. The `ecl-tools.cjs init` handler
