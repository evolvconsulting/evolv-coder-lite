 ...
 *
 * /ecl-update cache-clear step only cleared per-runtime cache paths
 * (e.g. ~/.claude/cache/ecl-update-check.json) but the SessionStart hook
 * (hooks/ecl-check-update.js) writes to the shared tool-agnostic path
 * ~/.cache/ecl/ecl-update-check.json. After a successful update, the statusline
 * kept showing the stale "⬆ /ecl-update" indicator because the actual cache
 * file was never deleted.
 *
 * Fix: add `rm -f "$HOME/.cache/ecl/ecl-update-check.json"` to the
 * run_update step's cache-clear block in evolv-coder-lite/workflows/update.md.
 */
...
const CHECK_UPDATE_HOOK = path.join(REPO_ROOT, 'hooks', 'ecl-check-update.js');

describe('bug-2784: update.md cache-clear covers shared cache path', () => {
  test('ecl-check-update.js hook constructs cache dir from .cache and ecl path segments', () => {
    ...
    assert.ok(
      segments.includes('ecl'),
      `hook cacheDir path.join() must include 'ecl' segment; got: ${JSON.stringify(segments)}`
    );
  });

  test('update.md run_update bash commands include rm for shared ecl cache file', () => {
    ...
    const sharedCacheClearCmds = bashLines.filter(
      (line) => /^rm\b/.test(line) && line.includes('.cache/ecl/ecl-update-check') && line.includes('*.json')
    );
    assert.ok(
      sharedCacheClearCmds.length > 0,
      [
        'run_update step bash blocks must include an `rm` command targeting .cache/ecl/ecl-update-check*.json (glob form clearing legacy + per-package variants).',
