...
/**
 * Remove stale ecl-* temp files/dirs older than maxAgeMs (default: 5 minutes).
 * Runs opportunistically before each new temp file write to prevent unbounded accumulation.
 * @param {string} prefix - filename prefix to match (e.g., 'ecl-')
 * @param {object} opts
 ...
 */
/**
 * Dedicated eCL temp directory: path.join(os.tmpdir(), 'ecl').
 * Created on first use. Keeps eCL temp files isolated from the system
 * temp directory so reap scans only eCL files (#1975).
 */
const ECL_TEMP_DIR = path.join(require('os').tmpdir(), 'ecl');

function ensureGsdTempDir() {
  platformEnsureDir(ECL_TEMP_DIR);
}

function reapStaleTempFiles(prefix = 'ecl-', { maxAgeMs = 5 * 60 * 1000, dirsOnly = false } = {}) {
  try {
    ensureGsdTempDir();
    const now = Date.now();
    const entries = fs.readdirSync(ECL_TEMP_DIR);
    for (const entry of entries) {
      if (!entry.startsWith(prefix)) continue;
      const fullPath = path.join(ECL_TEMP_DIR, entry);
      try {
...
function output(result, raw, rawValue) {
  ...
  if (raw && rawValue !== undefined) {
    ...
    if (json.length > 50000) {
      reapStaleTempFiles();
      ensureGsdTempDir();
      const tmpPath = path.join(ECL_TEMP_DIR, `ecl-${Date.now()}.json`);
 ...
 * instead of plain "Error: <message>" text. Set by ecl-tools.cjs when the
...
function loadConfig(cwd, options = {}) {
  const activeWorkstream = Object.prototype.hasOwnProperty.call(options, 'workstream')
    ? options.workstream
    : (process.env.ECL_WORKSTREAM || null);
  // When ECL_WORKSTREAM is set, load root config first so workstream config
  ...
  try {
    ...
    if (unknownKeys.length > 0) {
      ...
      if (!_warnedUnknownConfigKeys.has(warnKey)) {
        ...
        process.stderr.write(
          `ecl-tools: warning: unknown config key(s) in .planning/config.json: ${unknownKeys.join(', ')} — these will be ignored\n`
        );
      }
    }
    ...
    // Fall back to ~/.ecl/defaults.json only for truly pre-project contexts (#1683)
    // If .planning/ exists, the project is initialized — just missing config.json.
    // When ECL_WORKSTREAM is set and root config was loaded, the workstream config
    ...
    try {
      const home = process.env.ECL_HOME || os.homedir();
      const globalDefaultsPath = path.join(home, '.ecl', 'defaults.json');
...
function pruneOrphanedWorktrees(repoRoot) {
  try {
    ...
    if (pruneResult && pruneResult.timedOut) {
      ...
      // Uses process.stderr.write to match the [ecl-tools] WARNING prefix style.
      process.stderr.write(
        '[ecl-tools] WARNING: worktree health check degraded' +
...
/**
 * Resolve the agents directory from the eCL install location.
 * ecl-tools.cjs lives at <configDir>/evolv-coder-lite/bin/ecl-tools.cjs,
 * so agents/ is at <configDir>/agents/.
 *
 * ECL_AGENTS_DIR env var overrides the default path. Used in tests and for
 * installs where the agents directory is not co-located with ecl-tools.cjs.
 *
 * @returns {string} Absolute path to the agents directory
 */
function getAgentsDir() {
  if (process.env.ECL_AGENTS_DIR) {
    return process.env.ECL_AGENTS_DIR;
  }
  // __dirname is evolv-coder-lite/bin/lib/ → go up 3 levels to configDir
  return path.join(__dirname, '..', '..', '..', 'agents');
}

/**
 * Check which eCL agents are installed on disk.
 * Returns an object with installation status and details.
 *
 * Recognises both standard format (ecl-planner.md) and Copilot format
 * (ecl-planner.agent.md). Copilot renames agent files during install (#1512).
 *
...
function _warnUnknownProfileOverrides(parsed, configLabel) {
  ...
  if (runtime && typeof runtime === 'string' && !KNOWN_RUNTIMES.has(runtime)) {
    ...
    if (!_warnedConfigKeys.has(key)) {
      ...
      try {
        process.stderr.write(
          `ecl: warning — config key "runtime" has unknown value "${runtime}". ` +
  ...
  for (const [overrideRuntime, tierMap] of Object.entries(overrides)) {
    if (!KNOWN_RUNTIMES.has(overrideRuntime)) {
      ...
      if (!_warnedConfigKeys.has(key)) {
        ...
        try {
          process.stderr.write(
            `ecl: warning — model_profile_overrides.${overrideRuntime}.* uses ` +
    ...
    for (const tierName of Object.keys(tierMap)) {
      if (!RUNTIME_OVERRIDE_TIERS.has(tierName)) {
        ...
        if (!_warnedConfigKeys.has(key)) {
          ...
          try {
            process.stderr.write(
              `ecl: warning — model_profile_overrides.${overrideRuntime}.${tierName} ` +
 ...
 * @param {string} agentType - Agent name (e.g. 'ecl-verifier').
