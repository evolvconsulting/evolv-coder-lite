...
// commands/ecl/*.md, evolv-coder-lite/workflows/*.md, and agents/ecl-*.md are
 ...
 * tracker — a distinct tool set. eCL workflows were partially migrated and still
...
describe('#3168 — commands/ecl: allowed-tools must use Agent not Task', () => {
...
describe('#3168 — agents: tools frontmatter must use Agent not Task', () => {
  const agents = readMdFiles(AGENTS_DIR, 'ecl-');
