...
describe('generate-claude-md', () => {
  ...
  test('creates CLAUDE.md with workflow enforcement section', () => {
    ...
    assert.ok(content.includes('## eCL Workflow Enforcement'));
    ...
    assert.ok(content.includes('/ecl-quick'));
    assert.ok(content.includes('/ecl-debug'));
    assert.ok(content.includes('/ecl-execute-phase'));
    assert.ok(!content.includes('/ecl:quick'));
    assert.ok(!content.includes('/ecl:execute-phase'));
    assert.ok(content.includes('Do not make direct repo edits outside a eCL workflow'));
  });
  ...
  test('adds workflow enforcement section when force-updating an existing marker-less CLAUDE.md', () => {
    ...
    // #1098: a hand-crafted file (no eCL markers) is only overwritten with --force.
  ...
  test('#1098: does NOT clobber an existing marker-less CLAUDE.md without --force', () => {
    ...
    assert.ok(!content.includes('## eCL Workflow Enforcement'), 'eCL sections must NOT be injected');
  });

  test('#1098: updates an existing eCL-managed CLAUDE.md (has markers) without --force', () => {
    fs.writeFileSync(
      path.join(tmpDir, '.planning', 'PROJECT.md'),
      '# Test Project\n\n## What This Is\n\nA small test project.\n'
    );
    // First generation creates a eCL-managed file (with markers).
...
describe('new-project workflow includes CLAUDE.md generation', () => {
  const workflowPath = path.join(__dirname, '..', 'evolv-coder-lite', 'workflows', 'new-project.md');
...
describe('generate-claude-md skills section', () => {
  ...
  test('includes skills fallback when no skills directories exist', () => {
    ...
    const content = fs.readFileSync(path.join(tmpDir, '.claude', 'CLAUDE.md'), 'utf-8');
    assert.ok(content.includes('<!-- eCL:skills-start'));
    assert.ok(content.includes('<!-- eCL:skills-end -->'));
  ...
  test('discovers skills from .codex/skills/ directory and ignores deprecated import-only roots', () => {
    ...
    const homeDir = fs.mkdtempSync(path.join(require('os').tmpdir(), 'ecl-claude-skills-home-'));
    fs.mkdirSync(path.join(homeDir, '.claude', 'evolv-coder-lite', 'skills', 'import-only'), { recursive: true });
    fs.writeFileSync(
      path.join(homeDir, '.claude', 'evolv-coder-lite', 'skills', 'import-only', 'SKILL.md'),
  ...
  test('skips ecl- prefixed skill directories', () => {
    const gsdSkillDir = path.join(tmpDir, '.claude', 'skills', 'ecl-plan-phase');
    ...
    fs.writeFileSync(
      ...
      '---\nname: ecl-plan-phase\ndescription: eCL internal skill.\n---\n'
    );
    fs.writeFileSync(
    ...
    const content = fs.readFileSync(path.join(tmpDir, '.claude', 'CLAUDE.md'), 'utf-8');
    assert.ok(!content.includes('ecl-plan-phase'));
  ...
  test('skills section appears between architecture and workflow', () => {
    ...
    const workflowIdx = content.indexOf('## eCL Workflow Enforcement');
