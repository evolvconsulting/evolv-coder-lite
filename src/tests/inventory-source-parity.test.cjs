...
/** Extract agent names from `| ecl-xxx | ...` rows (no backticks). */
function agentNames(text) {
  const re = /^\|\s*(ecl-[a-z0-9-]+)\s*\|/gm;
  const names = [];
  let m;
  while ((m = re.exec(text)) !== null) names.push(m[1]);
  return names;
...
function commandSourcePaths(text) {
  const re = /\[commands\/ecl\/[^\]]+\]\(\.\.\/(commands\/ecl\/[^)]+)\)/g;
  const paths = [];
  let m;
...
describe('INVENTORY.md declared artifacts exist on the filesystem (ghost-entry guard)', () => {
  ...
  describe('Workflows', () => {
    ...
    for (const name of names) {
      test(name, () => {
        const p = path.join(ROOT, 'evolv-coder-lite', 'workflows', name);
        assert.ok(
          fs.existsSync(p),
          'INVENTORY.md declares workflow "' + name + '" but evolv-coder-lite/workflows/' + name + ' does not exist — remove the ghost row or restore the file',
        );
      });
    }
  });
  ...
  describe('References', () => {
    ...
    for (const name of names) {
      test(name, () => {
        const p = path.join(ROOT, 'evolv-coder-lite', 'references', name);
        assert.ok(
          fs.existsSync(p),
          'INVENTORY.md declares reference "' + name + '" but evolv-coder-lite/references/' + name + ' does not exist — remove the ghost row or restore the file',
        );
      });
    }
  });
  ...
  describe('CLI Modules', () => {
    ...
    for (const name of names) {
      test(name, () => {
        const p = path.join(ROOT, 'evolv-coder-lite', 'bin', 'lib', name);
        assert.ok(
          fs.existsSync(p),
          'INVENTORY.md declares CLI module "' + name + '" but evolv-coder-lite/bin/lib/' + name + ' does not exist — remove the ghost row or restore the file',
        );
      });
    }
  });
