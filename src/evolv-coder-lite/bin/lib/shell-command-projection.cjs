 ...
 * that eCL writes into runtime config or prints for copy/paste. This module
...
function buildWindowsShimTriple(shimSrc) {
  ...
  return {
    invocation,
    eol: { cmd: '\r\n', ps1: '\n', sh: '\n' },
    fileNames: { cmd: 'ecl-sdk.cmd', ps1: 'ecl-sdk.ps1', sh: 'ecl-sdk' },
...
function formatSdkPathDiagnostic({ shimDir, platform, runDir }) {
  ...
  if (shimDir) {
    ...
    actionLines.push('npm install -g @evolvconsulting/evolv-coder-lite');
  }
  const npxNoteLines = isNpx
    ? [
        "Note: you're running via npx. For a persistent shim,",
        'install globally instead: npm install -g @evolvconsulting/evolv-coder-lite',
      ]
    : [];
