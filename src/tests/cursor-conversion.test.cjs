...
describe('_applyRuntimeRewrites(cursor) — bare-form ~/.claude regression (#1356)', () => {
  ...
  const COMPOUND_INPUT = [
    'Config dir: ~/.claude',
    'Also: $HOME/.claude',
    'Slash form: ~/.claude/evolv-coder-lite/foo.md',
  ...
  test('trailing-slash ~/.claude/evolv-coder-lite/foo.md is rewritten exactly once (no doubling)', () => {
    const input = '~/.claude/evolv-coder-lite/foo.md';
    const result = _applyRuntimeRewrites(input, 'cursor', CURSOR_PATH_PREFIX);
    assert.ok(result.includes('~/.cursor/evolv-coder-lite/foo.md'), `slash form must be rewritten; got: ${result}`);
