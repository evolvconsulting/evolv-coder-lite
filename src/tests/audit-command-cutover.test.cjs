 ...
 * `case 'audit-uat':` and `case 'audit-open':` arms in ecl-tools.cjs to the
...
const { routeAuditUat, routeAuditOpen } = require('../evolv-coder-lite/bin/lib/audit-command-router.cjs');
...
describe('audit cutover: dispatch path (default-case → capability registry)', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempProject('ecl-audit-cutover-');
  });
...
describe('audit cutover: output shape equivalence', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempProject('ecl-audit-behavior-');
  });
  ...
  test('audit-uat --raw flag passes through (does not break dispatch)', () => {
    ...
    // --raw is a ecl-tools global flag; it modifies output encoding but
...
// ─── 4. JSON-ERRORS — ECL_JSON_ERRORS mode passes through cleanly ────────────

describe('audit cutover: ECL_JSON_ERRORS mode (both commands succeed without structured error)', () => {
  let tmpDir;

  beforeEach(() => {
    tmpDir = createTempProject('ecl-audit-jsonerr-');
  });
  ...
  test('audit-open --json with ECL_JSON_ERRORS=1 succeeds (no spurious error payload)', () => {
    // Successful commands must not emit JSON error payloads; verify exit 0.
    const result = runGsdTools(['audit-open', '--json'], tmpDir, { ECL_JSON_ERRORS: '1' });
    assert.ok(result.success,
      `audit-open --json must succeed even with ECL_JSON_ERRORS=1; stderr: ${result.error}`);
  });

  test('audit-open text with ECL_JSON_ERRORS=1 succeeds (no spurious error payload)', () => {
    const result = runGsdTools(['audit-open'], tmpDir, { ECL_JSON_ERRORS: '1' });
    assert.ok(result.success,
      `audit-open text mode must succeed even with ECL_JSON_ERRORS=1; stderr: ${result.error}`);
  });

  test('audit-uat with ECL_JSON_ERRORS=1 succeeds (no spurious error payload)', () => {
    const result = runGsdTools(['audit-uat'], tmpDir, { ECL_JSON_ERRORS: '1' });
    assert.ok(result.success,
      `audit-uat must succeed even with ECL_JSON_ERRORS=1; stderr: ${result.error}`);
  });
});
