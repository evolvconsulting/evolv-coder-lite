/**
 * Installer migration: remove legacy Codex hooks.json eCL hook registrations.
 *
...
const migration: InstallerMigration = {
  ...
  title: 'Remove legacy Codex hooks.json eCL hook registrations',
  description: 'Remove legacy Codex hooks.json eCL hook registrations after config.toml migration.',
  introducedIn: '1.50.0',
  runtimes: ['codex'],
  scopes: ['global', 'local'],
  destructive: true,
    ...
    return [
      {
        ...
        value: pruned.value,
        deleteIfEmpty: true,
        reason: 'legacy Codex hooks.json eCL registration retired by installer migration',
        ownershipEvidence: 'pruned command matches generated eCL hook command under the install hooks directory',
      },
    ];
  },
};
