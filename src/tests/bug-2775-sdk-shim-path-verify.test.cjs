 ...
 *
 * `npx @evolvconsulting/evolv-coder-lite@latest --global` runs the installer, which prints
 * `✓ eCL SDK ready` even though the secondary `ecl-sdk` bin is not on the
 ...
 * — a strictly weaker invariant than `command -v ecl-sdk` resolving.
 *
 * The fix tightens the success gate: after confirming the dist is present,
 * the installer must verify `ecl-sdk` resolves on PATH. If it does not, the
 * installer attempts to materialize the shim into a user-writable PATH
 * location (`~/.local/bin/ecl-sdk`) and re-checks. Only when the PATH probe
 * succeeds does it print `✓ eCL SDK ready`. Otherwise it emits a clear
 ...
 * with a `ecl-sdk` shim. The legacy code printed the success line in this
...
describe('bug #2775: installSdkIfNeeded must verify ecl-sdk on PATH before reporting ready',
  { skip: isWindows ? 'POSIX-only: asserts ~/.local/bin shebang shim with mode 0o755; Windows uses ecl-sdk.cmd + PATHEXT + registry Path' : false },
  () => {
  let tmpRoot;
  let sdkDir;
  let pathDir;
  ...
  beforeEach(() => {
    tmpRoot = createTempDir('ecl-2775-');
    ...
    // PATH does NOT contain anything with a ecl-sdk shim — simulates npx-cache.
  ...
  test('does NOT print "eCL SDK ready" when ecl-sdk is not callable on PATH and cannot be linked', () => {
    // Make ~/.local/bin not on PATH and not creatable-friendly: PATH stays
    // as a single dir with no ecl-sdk. The installer may attempt to create
    // ~/.local/bin/ecl-sdk, but that location isn't on PATH either, so the
    ...
    const hasReady = /eCL SDK ready/.test(combined);
    const mentionsPath = /not on (your )?PATH|ecl-sdk.*PATH|PATH.*ecl-sdk/i.test(combined);
    assert.ok(
      !hasReady,
      `installer must not print "eCL SDK ready" when ecl-sdk is not on PATH. Output:\n${combined}`,
    );
    assert.ok(
      mentionsPath,
      `installer must surface a PATH-related warning when ecl-sdk is not callable. Output:\n${combined}`,
    );
  });

  test('DOES print "eCL SDK ready" after self-linking into a directory that IS on PATH', () => {
    ...
    assert.ok(
      ...
      `installer must print "eCL SDK ready" after self-linking to a dir on PATH. Output:\n${combined}`,
    );
    // And the link must actually exist + resolve back to the shim.
    const linkPath = path.join(localBin, 'ecl-sdk');
  ...
  test('symlink-fallback writes a wrapper that require()s the real shim by absolute path (preserves __dirname)', () => {
    ...
    // The fallback must NOT copy bin/ecl-sdk.js into ~/.local/bin (which would
    ...
    const realShimSrc = path.resolve(__dirname, '..', 'bin', 'ecl-sdk.js');
    ...
    const target = path.join(localBin, 'ecl-sdk');
    assert.ok(fs.existsSync(target), `fallback must materialize ${target}`);
    // Critical: it must NOT be a verbatim copy of bin/ecl-sdk.js.
    const targetContent = fs.readFileSync(target, 'utf8');
    const realShimContent = fs.readFileSync(realShimSrc, 'utf8');
    assert.notStrictEqual(
      targetContent,
      realShimContent,
      'fallback must not copyFileSync bin/ecl-sdk.js — that breaks __dirname-based CLI resolution',
    );
  ...
  test('self-link prefers a PATH-backed HOME dir over ~/.local/bin when ~/.local/bin is off-PATH', () => {
    ...
    assert.ok(
      fs.existsSync(path.join(homeBin, 'ecl-sdk')),
      `installer must materialize the link in the on-PATH HOME dir (~/bin), not ~/.local/bin`,
    );
  });

  test('DOES print "eCL SDK ready" when ecl-sdk is already resolvable on PATH', () => {
    // Pre-populate PATH with a `ecl-sdk` shim so the probe finds one.
    const preexisting = path.join(pathDir, 'ecl-sdk');
    ...
    assert.ok(
      ...
      `installer must print "eCL SDK ready" when ecl-sdk is already on PATH. Output:\n${stdout}`,
    );
  });
});
