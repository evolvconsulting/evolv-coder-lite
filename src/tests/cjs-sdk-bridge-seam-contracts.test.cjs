 ...
 *
 *   • `evolv-coder-lite/bin/lib/cjs-sdk-bridge.cjs` — load + cache + surface
 ...
 *     verify-command-router.cjs`) + top-level `ecl-tools.cjs` dispatch —
 ...
 *     fall back to CJS when `ECL_WORKSTREAM` is set (the gate the
...
const BRIDGE_PATH = path.join(REPO_ROOT, 'evolv-coder-lite', 'bin', 'lib', 'cjs-sdk-bridge.cjs');
...
describe('phase 6: CLI family-router dispatch emits structured JSON', () => {
  ...
  test('state json emits a frontmatter object with progress', () => {
    ...
    const payload = JSON.parse(result.output);
    assert.strictEqual(payload.ecl_state_version, '1.0');
...
// CJS routers still force CJS fallback when `ECL_WORKSTREAM` is set in the
...
describe('phase 6: ECL_WORKSTREAM gate routes through CJS fallback consistently', () => {
  let tmpDir;
  ...
  test('roadmap get-phase produces identical structured output with and without ECL_WORKSTREAM unset', () => {
    ...
    // When ECL_WORKSTREAM is set, the router falls through to CJS.  For the
    ...
    const cjsPath = runGsdTools(['roadmap', 'get-phase', '1'], tmpDir, { ECL_WORKSTREAM: '' });
