...
describe('withIsolatedProcessState', () => {
  test('restores env, cwd, and exitCode after callback', () => {
    ...
    const originalMarker = process.env.ECL_TEST_ISOLATION_MARKER;

    const tempCwd = path.dirname(originalCwd);

    withIsolatedProcessState(() => {
      process.env.ECL_TEST_ISOLATION_MARKER = 'changed';
      process.exitCode = 73;
    ...
    assert.strictEqual(process.env.ECL_TEST_ISOLATION_MARKER, originalMarker);
  });
