...
> `ecl-tools` CLI（`evolv-coder-lite/bin/ecl-tools.cjs`）参考文档。斜杠命令与用户流程请参见[命令参考](COMMANDS.md)。返回[文档索引](README.md)。

---

## 概述

`ecl-tools.cjs` 集中处理配置解析、模型解析、阶段查找、Git 提交、摘要验证、状态管理以及模板操作，供 eCL 命令、工作流和代理使用。


|                    |                                                                                                                                                                                                        |
| ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **发布路径**   | `evolv-coder-lite/bin/ecl-tools.cjs`                                                                                                                                                                      |
| **实现**       | `evolv-coder-lite/bin/lib/` 下的 20 个领域模块（以该目录为准）                                                                                                                                              |
...
代理名称：`ecl-planner`、`ecl-executor`、`ecl-phase-researcher`、`ecl-project-researcher`、`ecl-research-synthesizer`、`ecl-verifier`、`ecl-plan-checker`、`ecl-integration-checker`、`ecl-roadmapper`、`ecl-debugger`、`ecl-codebase-mapper`、`ecl-nyquist-auditor`
...
`validate context` 输出包含 `utilization`、`status`（在 60% / 70% 阈值处分别为 `ok` / `warn` / `critical`）以及 `suggestion` 字符串的结构化信封。相同数据支撑 `/ecl-health --context`。
...
**大载荷处理：** 当输出超过约 50KB 时，CLI 会将内容写入临时文件并返回 `@file:/tmp/ecl-init-XXXXX.json`。工作流检查 `@file:` 前缀并从磁盘读取：
...
返回所有可用 eCL 技能的 JSON 映射，包含其元数据（名称、描述、文件路径、参数提示）。由安装程序和会话启动钩子使用，以避免重复的文件系统扫描。
...
# 将 eCL-2 项目反向迁移到当前结构（支撑 `/ecl-import --from-ecl2`）
...
用户入口：`/ecl-graphify`（参见[命令参考](COMMANDS.md#ecl-graphify)）。
...
| Graphify | `lib/graphify.cjs` | 知识图谱构建/查询/状态/差异/快照（支撑 `/ecl-graphify`） |
| 学习记录 | `lib/learnings.cjs` | 从阶段/SUMMARY 制品中提取学习记录（支撑 `/ecl-extract-learnings`） |
| 审计 | `lib/audit.cjs` | 阶段/里程碑审计队列处理器；`audit-open` 助手 |
| GSD2 导入 | `lib/ecl2-import.cjs` | 从 eCL-2 项目反向迁移导入（支撑 `/ecl-import --from-ecl2`） |
| Intel | `lib/intel.cjs` | 可查询的代码库智能索引（支撑 `/ecl-map-codebase --query`） |

---

## 审阅器 CLI 路由

`review.models.<cli>` 将审阅器类型映射到代码审查工作流调用的 shell 命令。通过 [`/ecl-config --integrations`](COMMANDS.md#ecl-config) 或直接设置：
...
node ecl-tools.cjs config-set review.models.claude   ""   # 清除——回退到会话模型
```
...
通过 `/ecl-settings` 配置的 API 密钥（`brave_search`、`firecrawl`、`exa_search`）以明文形式写入 `.planning/config.json`，但在所有 `config-set` / `config-get` 输出、确认表格和交互式提示中均会被遮蔽（`****<last-4>`）。遮蔽实现请参见 `evolv-coder-lite/bin/lib/secrets.cjs`。`config.json` 文件本身是安全边界——请通过文件系统权限保护它，并将其排除在 git 之外（`.planning/` 默认已被 gitignore）。
