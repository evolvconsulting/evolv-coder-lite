...
**前提条件：** 已激活的 eCL Core 项目（`.planning/ROADMAP.md` 存在）。若尚未创建，请先运行 `/ecl-new-project`。
...
当某个工作流处于激活状态时，所有 eCL 命令——`/ecl-progress`、`/ecl-discuss-phase`、`/ecl-plan-phase`、`/ecl-execute-phase`——都将从该工作流的目录读取并写入。切换工作流会将所有这些命令重定向到另一个子树，而不会影响源代码树。
...
eCL 会在 `.planning/workstreams/backend-api/` 下创建工作流目录，并初始化一个框架 `STATE.md` 和 `ROADMAP.md`。工作流不会自动激活——需要显式切换。
...
从此时起，所有 eCL 工作流命令均在 `backend-api` 上下文中运行。切换是会话范围的：当多个 Claude Code 终端同时打开同一仓库时，每个会话可以持有不同的激活工作流，互不干扰。
...
此命令会激活该工作流并恢复上次已知位置，等价于切换后再运行 `/ecl-resume-work`。
...
eCL 会将该工作流标记为已归档，并将其从活跃列表中移出。规划产物将保留在 `.planning/workstreams/backend-api/` 下以供审计。
...
- 您希望在每个工作区中独立运行 `/ecl-new-project`，拥有完全独立的 `.planning/` 根目录，而不是主仓库 `.planning/` 的子目录
