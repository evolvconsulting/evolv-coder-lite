 ...
 * package across every eCL-managed runtime config directory.
 *
 * Issue: #607
 *
...
/**
 * Subtrees within a configDir that eCL actively scans for old-package content.
 * Deliberately excludes 'evolv-coder-lite' — the current package's own infra and
 ...
 * IS scanned.
 */
const ECL_MANAGED_SUBTREES = ['hooks', 'commands'];

/**
 * Extensions eligible for the content-reference scan.
 *
 * WHY: The current @evolvconsulting/evolv-coder-lite package ships ZERO references to the old
 ...
 * installer to delete the freshly-installed evolv-coder-lite/CHANGELOG.md,
...
function planLegacyCleanup(configDirs, opts = {}) {
  ...
  for (const configDir of configDirs) {
    for (const subtree of ECL_MANAGED_SUBTREES) {
  ...
  const legacyCachePath = path.join(homeDir, '.cache', 'ecl', 'ecl-update-check.json');
  try {
