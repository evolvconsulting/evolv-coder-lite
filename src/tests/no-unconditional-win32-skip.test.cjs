...
describe('Windows parity guard for trySelfLinkGsdSdk (#2962)', () => {
  ...
  before(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-win32-guard-'));
  ...
  test('trySelfLinkGsdSdk dispatches to the Windows handler and writes shims (does NOT silently return null)', () => {
    ...
    assert.ok(
      fs.existsSync(path.join(tmpDir, 'ecl-sdk.cmd')),
      'Windows dispatch must materialize ecl-sdk.cmd in the npm global bin',
    );
  });
});
