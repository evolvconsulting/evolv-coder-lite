...
> eCL Core의 명령어 참조 — 모든 안정 명령어의 구문, 플래그, 옵션, 예시. 기능 세부 사항은 [기능 참조](FEATURES.md)를, 워크플로 안내는 [사용자 가이드](USER-GUIDE.md)를, 문서 목록은 [README](README.md)를 참조하세요.

---

## 명령어 구문

- **Claude Code / Copilot / OpenCode / Kilo:** `/ecl-command-name [args]` (하이픈 형식)
- **Gemini CLI:** `/ecl:command-name [args]` (콜론 형식 — Gemini는 `ecl:` 네임스페이스로 명령어를 분류합니다)
...
v1.40에서 여섯 개의 네임스페이스 라우터가 1단계 진입점으로 제공됩니다. 이를 통해 즉시 스킬 목록을 로드하는 토큰 비용을 낮추면서(라우터 6개에 ~120 토큰 대 86개 스킬 전체 목록에 ~2,150 토큰) 전체 기능을 직접 호출할 수 있습니다. 모델이 네임스페이스를 선택한 후 구체적인 하위 스킬로 라우팅합니다. [#2792](https://github.com/evolvconsulting/evolv-coder-lite/issues/2792)를 참조하세요.

| 명령어 | 라우팅 대상 |
|---------|-----------|
| `/ecl-workflow` | 단계 파이프라인 — discuss / plan / execute / verify / phase / progress |
| `/ecl-project` | 프로젝트 수명 주기 — 마일스톤, 감사, 요약 |
| `/ecl-quality` | 품질 게이트 — 코드 리뷰, 디버그, 감사, 보안, 평가, UI |
| `/ecl-context` | 코드베이스 인텔리전스 — 맵, 그래프화, 문서, 학습 내용 |
| `/ecl-manage` | 관리 — config, workspace, workstreams, thread, update, ship, inbox |
| `/ecl-ideate` | 탐색 및 캡처 — explore, sketch, spike, spec, capture |

네임스페이스 스킬은 **추가적** 방식으로 동작합니다 — 기존의 모든 구체적인 명령어(예: `/ecl-plan-phase`, `/ecl-code-review --fix`)는 여전히 직접 호출할 수 있습니다.
...
```bash
/ecl-new-project                    # 대화형 모드
/ecl-new-project --auto @prd.md     # PRD에서 자동 추출
```
...
/ecl-workspace --new --name feature-b --repos . --strategy worktree  # 동일 리포지토리 격리
/ecl-workspace --list
...
```bash
/ecl-discuss-phase 1                # 단계 1의 대화형 논의
/ecl-discuss-phase 1 --all          # 선택 단계 없이 모든 불확실한 영역 논의
/ecl-discuss-phase 3 --auto         # 단계 3의 기본값 자동 선택
/ecl-discuss-phase --batch          # 현재 단계의 배치 모드
/ecl-discuss-phase 2 --analyze      # 트레이드오프 분석과 함께 논의
/ecl-discuss-phase 1 --power        # 파일로부터 대량 답변
/ecl-discuss-phase 3 --assumptions  # 계획 전 Claude의 가정 표시
```
...
```bash
/ecl-plan-phase 1                              # 단계 1 리서치 + 계획 + 검증
/ecl-plan-phase 3 --skip-research              # 리서치 없이 계획 (친숙한 도메인)
/ecl-plan-phase --auto                         # 비대화형 계획 수립
/ecl-plan-phase 2 --validate                   # 계획 전 상태 검증
/ecl-plan-phase 1 --bounce                     # 계획 + 외부 바운스 검증
/ecl-plan-phase 2 --ingest docs/adr/0010.md   # 컨텍스트 합성을 위한 ADR 익스프레스 경로
/ecl-plan-phase 2 --ingest 'docs/adr/00*.md' --ingest-format auto
/ecl-plan-phase --research-phase 4             # 단계 4만 리서치 (RESEARCH.md 있으면 프롬프트)
/ecl-plan-phase --research-phase 4 --view      # 기존 RESEARCH.md 출력, 생성 없음
/ecl-plan-phase --research-phase 4 --research  # 강제 리서치 새로 고침, 프롬프트 없음
/ecl-plan-phase 1 --mvp                        # 단계 1의 수직 슬라이스 계획
/ecl-plan-phase 1 --mvp --tdd                  # 수직 슬라이스 + 동작 추가 작업당 실패 테스트
```
...
```bash
/ecl-plan-review-convergence 3                    # 기본 리뷰어, 3 사이클
/ecl-plan-review-convergence 3 --codex            # Codex 전용 리뷰
...
**[BETA]** 계획 단계를 Claude Code의 ultraplan 클라우드로 오프로드; 브라우저에서 리뷰하고 가져오기. 계획이 원격으로 작성되는 동안 터미널은 자유롭게 유지됩니다; 브라우저에서 인라인 댓글을 리뷰한 후 `/ecl-import`를 통해 최종 계획을 `.planning/`으로 가져옵니다.

| 플래그 | 필수 | 설명 |
|------|----------|-------------|
| `N` | **예** | 원격으로 계획할 단계 번호 |

**격리:** 업스트림 ultraplan 변경이 핵심 계획 파이프라인에 영향을 미치지 않도록 `/ecl-plan-phase`와 의도적으로 분리됩니다.

```bash
/ecl-ultraplan-phase 4                  # 단계 4의 계획을 오프로드
```
...
```bash
/ecl-execute-phase 1                # 단계 1 실행
/ecl-execute-phase 1 --wave 2       # 웨이브 2만 실행
/ecl-execute-phase 1 --validate     # 실행 전 상태 검증
/ecl-execute-phase 2 --cross-ai     # 단계 2를 외부 AI CLI에 위임
```
...
브라우저 기반 UAT의 경우 구성된 브라우저 MCP 서버를 사용하세요. 현재 Open eCL 컴패니언은 `ecl-browser`(`ecl-browser mcp`)이며, 결정론적 탐색, 버전 관리된 참조, 어설션, 스크린샷, 시각적 비교, 녹화, 사용자 인수 기능을 제공합니다. 이미 구성되어 있는 레거시 Playwright MCP 서버도 계속 사용할 수 있습니다.

```bash
/ecl-verify-work 1                  # 단계 1의 UAT
```
...
**전제 조건:** 단계 검증 완료 (`/ecl-verify-work` 통과), `gh` CLI 설치 및 인증됨
**생성 결과:** 계획 아티팩트로부터 풍부한 본문이 포함된 GitHub PR, STATE.md 업데이트

```bash
/ecl-ship 4                         # 단계 4 배포
/ecl-ship 4 --draft                 # 초안 PR로 배포
```
...
**전제 조건:** 프로젝트에 프론트엔드 코드 포함 (독립형으로 작동, eCL 프로젝트 불필요)
**생성 결과:** `{phase}-UI-REVIEW.md`, `.planning/ui-reviews/`의 스크린샷

더 풍부한 시각적 증거를 위해 `ecl-browser` 또는 다른 브라우저 MCP 서버와 함께 사용하면 감사에서 스크린샷, 상태, 콘솔/네트워크 컨텍스트, 재현 가능한 상호작용 단계를 캡처할 수 있습니다.

```bash
/ecl-ui-review                      # 현재 단계 감사
/ecl-ui-review 3                    # 단계 3 감사
```
...
```bash
/ecl-milestone-summary                # 현재 마일스톤 요약
/ecl-milestone-summary v1.0           # 특정 마일스톤 요약
```
...
```bash
/ecl-new-milestone                  # 대화형
/ecl-new-milestone "v2.0 Mobile"    # 이름 있는 마일스톤
/ecl-new-milestone --reset-phase-numbers "v2.0 Mobile"  # 마일스톤 번호를 1부터 재시작
```
...
```bash
/ecl-phase "Add authentication system"          # 설명과 함께 새 단계 추가
/ecl-phase --insert 3 "Fix auth race condition" # 단계 3과 4 사이에 삽입 → 3.1 생성
/ecl-phase --remove 7               # 단계 7 삭제, 8→7, 9→8 등으로 번호 재정렬
/ecl-phase --edit 5                 # 단계 5의 모든 필드 편집
/ecl-phase --edit 5 --force         # 진행 중이거나 완료된 경우에도 단계 5 편집
```
...
단계에 대한 안내형 MVP 계획 — 사용자 스토리를 입력받고, SPIDR 분할 확인을 실행하고, ROADMAP.md에 `**Mode:** mvp`를 작성한 후 `/ecl-plan-phase`에 위임합니다 (로드맵 필드를 통해 MVP 모드를 자동 감지).
...
**전제 조건:** 단계가 ROADMAP.md에 이미 존재해야 합니다 (`/ecl-new-project`, `/ecl-phase`, 또는 `/ecl-phase --insert`를 통해 생성). 이 명령어는 새 단계를 생성하지 않습니다 — 기존 단계를 전환합니다.

**동작:** 구조화된 사용자 스토리를 수집하고, 형식을 검증하고, SPIDR 분할 확인을 실행하고, 단계의 ROADMAP.md 섹션에 `**Goal:**`과 `**Mode:** mvp`를 작성한 후 `/ecl-plan-phase <N>`에 위임합니다. 안내는 [MVP 단계 계획 방법](USER-GUIDE.md#mvp-phase-planning)을 참조하세요.

**Walking Skeleton:** 이전 단계 요약이 없는 새 프로젝트의 단계 1에서 `--mvp`(또는 `mode: mvp`)가 사용될 때 자동으로 트리거됩니다. 계획자는 `PLAN.md`와 함께 `SKELETON.md`를 생성합니다.

**생성 결과:** 업데이트된 ROADMAP.md, 그 후 `/ecl-plan-phase`의 모든 아티팩트; Walking Skeleton 모드 실행 시 `SKELETON.md`.

```bash
/ecl-mvp-phase 1                    # 단계 1의 MVP 계획
/ecl-mvp-phase 2.1                  # 소수 단계의 MVP 계획
/ecl-mvp-phase 3 --force            # 진행 중이어도 단계 3 전환
```
...
```bash
/ecl-validate-phase 2               # 단계 2의 테스트 커버리지 감사
```
...
| `--do "task description"` | 자유 형식 의도를 분석하고 가장 적합한 eCL 명령어로 디스패치 |
| `--forensic` | 표준 보고서 후 6개 검사 무결성 감사 추가 (STATE 일관성, 고아 핸드오프, 연기된 범위 드리프트, 메모리 플래그 보류 작업, 블로킹 할일, 커밋되지 않은 코드) |

**자동 라우팅 동작 (`--next`):**
- 프로젝트 없음 → `/ecl-new-project` 제안
- 단계 논의 필요 → `/ecl-discuss-phase` 실행
- 단계 계획 필요 → `/ecl-plan-phase` 실행
- 단계 실행 필요 → `/ecl-execute-phase` 실행
- 단계 검증 필요 → `/ecl-verify-work` 실행
- 모든 단계 완료 → `/ecl-complete-milestone` 제안

```bash
/ecl-progress                       # "현재 어디에 있나? 다음은?" 자동 라우팅 포함
/ecl-progress --next                # 자동으로 다음 단계로 진행
/ecl-progress --do "fix the auth bug"  # 자유 형식 의도를 최적의 eCL 명령어로 디스패치
/ecl-progress --forensic            # 표준 보고서 + 무결성 감사
```
...
```bash
/ecl-resume-work                    # 컨텍스트 재설정 또는 새 세션 후
```
...
```bash
/ecl-pause-work                     # continue-here.md 생성
/ecl-pause-work --report            # continue-here.md + 세션 보고서 생성
```
...
```bash
/ecl-manager                        # 명령 센터 대시보드 열기
/ecl-manager --analyze-deps         # 병렬 실행 전 ROADMAP 단계의 의존성 관계 스캔
```
...
요청한 수준에서 eCL 명령어를 표시합니다. 기본값은 화면 하나에 맞습니다; `--full`은 전체 참조; `<topic>`은 특정 섹션으로 바로 이동합니다.

```bash
/ecl-help                           # 한 페이지 개요 (기본값)
/ecl-help --brief                   # 주요 명령어의 ~10줄 요약
/ecl-help --full                    # 전체 참조 (모든 명령어, 모든 플래그)
/ecl-help <topic>                   # 하나의 섹션만 (예: /ecl-help debug)
/ecl-help --brief <topic>           # 압축된 범위 지정 조회 — 시그니처 + 한 줄 요약
```

전체 별칭 테이블은 `evolv-coder-lite/workflows/help/modes/topic.md`를 참조하세요. 알 수 없는 주제는 인식된 목록을 출력합니다.
...
소크라테스식 아이디어 발상 세션 — 탐색 질문을 통해 아이디어를 안내하고, 선택적으로 리서치를 생성한 후 적절한 eCL 아티팩트(노트, 할일, 시드, 리서치 질문, 요구사항 또는 새 단계)로 출력을 라우팅합니다.

| 인수 | 필수 | 설명 |
|----------|----------|-------------|
| `topic` | 아니요 | 탐색할 주제 (예: `/ecl-explore authentication strategy`) |

```bash
/ecl-explore                        # 개방형 아이디어 발상 세션
/ecl-explore authentication strategy  # 특정 주제 탐색
```
...
안전한 git 되돌리기 — 의존성 확인 및 확인 게이트를 통해 단계 매니페스트를 사용하여 eCL 단계 또는 계획 커밋 롤백.

| 플래그 | 필수 | 설명 |
|------|----------|-------------|
| `--last N` | (세 가지 중 하나 필수) | 대화형 선택을 위한 최근 eCL 커밋 표시 |
...
```bash
/ecl-undo --last 5                  # 최근 eCL 커밋 5개에서 선택
/ecl-undo --phase 03                # 단계 3의 모든 커밋 되돌리기
/ecl-undo --plan 03-02              # 단계 3의 계획 02 커밋 되돌리기
```
...
외부 계획 파일을 eCL 계획 시스템에 수집하고, 작성 전에 `PROJECT.md` 결정과의 충돌을 감지합니다.

| 플래그 | 필수 | 설명 |
|------|----------|--------------|
| `--from <filepath>` | 예 (`--from-ecl2`와 중 하나) | 가져올 외부 계획 파일 경로 |
| `--from-ecl2` | 예 (`--from`과 중 하나) | eCL-2 (`.ecl/`) 프로젝트를 eCL v1 (`.planning/`) 형식으로 역 마이그레이션 |
| `--path <dir>` | 아니요 | `--from-ecl2` 사용 시: eCL-2 프로젝트 디렉토리 경로 (기본값: 현재 디렉토리) |

**처리:** 충돌 감지 → 해결 프롬프트 → eCL PLAN.md로 작성 → `ecl-plan-checker`를 통한 검증

```bash
/ecl-import --from /tmp/team-plan.md    # 외부 계획 가져오기 및 검증
/ecl-import --from-ecl2                # eCL-2에서 v1으로 마이그레이션 (현재 디렉토리)
/ecl-import --from-ecl2 --path ~/old-project  # 다른 경로에서 마이그레이션
```
...
리포지토리의 기존 ADR, PRD, SPEC, 문서에서 .planning/ 설정을 부트스트랩하거나 병합합니다. 병렬 분류(`ecl-doc-classifier`)와 우선순위 규칙 및 순환 감지를 통한 합성(`ecl-doc-synthesizer`)을 실행합니다. 세 가지 버킷 충돌 보고서(`INGEST-CONFLICTS.md`: 자동 해결됨, 경쟁 변형, 미해결 차단자)를 생성하고 LOCKED 대 LOCKED ADR 모순에서 하드 블록합니다.
...
**제한:** v1은 호출당 최대 50개 문서. 공유 충돌 감지 계약을 `references/doc-conflict-engine.md`로 추출하며, `/ecl-import`도 이를 사용합니다.

```bash
/ecl-ingest-docs                            # 리포지토리 루트 스캔, 모드 자동 감지
/ecl-ingest-docs docs/                      # docs/ 아래만 수집
/ecl-ingest-docs --manifest ingest.yaml     # 명시적 우선순위 매니페스트
```
...
```bash
/ecl-quick                          # 기본 빠른 작업
/ecl-quick --discuss --research     # 논의 + 리서치 + 계획
/ecl-quick --validate               # 계획 확인 + 검증만
/ecl-quick --full                   # 완전한 품질 파이프라인
/ecl-quick list                     # 모든 빠른 작업 나열
/ecl-quick status my-task-slug      # 빠른 작업 상태 표시
/ecl-quick resume my-task-slug      # 빠른 작업 재개
```
...
```bash
/ecl-autonomous                     # 나머지 모든 단계 실행
/ecl-autonomous --from 3            # 단계 3부터 시작
/ecl-autonomous --to 5              # 단계 5까지 실행
/ecl-autonomous --from 3 --to 5     # 단계 3부터 5까지 실행
```
...
**서브커맨드:**
- `/ecl-debug list` — 상태, 가설, 다음 조치와 함께 모든 활성 디버그 세션 나열
- `/ecl-debug status <slug>` — 에이전트를 생성하지 않고 세션의 전체 요약(증거 수, 제거 수, 해결책, TDD 체크포인트) 출력
- `/ecl-debug continue <slug>` — 슬러그로 특정 세션 재개 (현재 포커스 표시 후 계속 에이전트 생성)
- `/ecl-debug [--diagnose] <description>` — 새 디버그 세션 시작 (기존 동작; `--diagnose`는 수정 적용 없이 근본 원인에서 중지)
...
```bash
/ecl-profile-user                   # 세션 분석 및 프로필 구축
/ecl-profile-user --questionnaire   # 대화형 설문지 대안
/ecl-profile-user --refresh         # 새 분석에서 재생성
```
...
[#2792](https://github.com/evolvconsulting/evolv-coder-lite/issues/2792)).
...
```bash
/ecl-health                         # 무결성 확인
/ecl-health --repair                # 확인 및 수정
/ecl-health --context               # 컨텍스트 활용 트리아지
```
...
/ecl-spike --wrap-up                    # 결과를 재사용 가능한 스킬로 패키징
```
...
```bash
/ecl-sketch                             # 대화형 분위기 입력
/ecl-sketch "dashboard layout"
/ecl-sketch --quick "sidebar navigation"
/ecl-sketch --text "onboarding flow"    # 비 Claude 런타임
/ecl-sketch --wrap-up                   # 채택된 스케치를 스킬로 패키징
```
...
실패한 eCL 워크플로에 대한 사후 조사 — 무엇이 잘못되었는지 진단합니다.
...
```bash
/ecl-forensics                              # 대화형 — 문제에 대한 프롬프트
/ecl-forensics "Phase 3 execution stalled"  # 문제 설명과 함께
```
...
```bash
/ecl-extract-learnings 3                    # 단계 3에서 학습 내용 추출
/ecl-extract-learnings --all                # 모든 완료된 단계에서 추출
```
...
```bash
/ecl-workstreams                    # 모든 워크스트림 나열
/ecl-workstreams create backend-api # 새 워크스트림 생성
/ecl-workstreams switch backend-api # 활성 워크스트림 설정
/ecl-workstreams status backend-api # 상세 상태
/ecl-workstreams progress           # 크로스 워크스트림 진행 개요
/ecl-workstreams complete backend-api  # 완료된 워크스트림 아카이브
/ecl-workstreams resume backend-api    # 워크스트림에서 작업 재개
```
...
모든 응답은 `ecl-tools query config-set`을 통해 해결된 프로젝트 설정 경로(일반 설치의 경우 `.planning/config.json`, 워크스트림이 활성화된 경우 `.planning/workstreams/<active>/config.json`)에 병합되며 관련 없는 키는 보존됩니다. 확인 후 사용자는 전체 설정 객체를 `~/.ecl/defaults.json`에 저장할 수 있으므로 향후 `/ecl-new-project` 실행이 동일한 기준으로 시작됩니다.
...
eCL 설정을 대화형으로 구성합니다 — 워크플로 토글, 고급 노브, 통합, 모델 프로필 — 단일 통합 명령어로.
...
모든 응답은 `ecl-tools query config-set`을 통해 관련 없는 키를 보존하며 병합됩니다. API 키는 모든 출력에서 마스킹됩니다 (`****<last-4>`).

```bash
/ecl-config                         # 일반적인 대화형 설정
/ecl-config --advanced              # 파워 유저 노브 (6섹션 프롬프트)
/ecl-config --integrations          # API 키, 리뷰 CLI 라우팅, 에이전트 스킬
/ecl-config --profile budget        # 예산 프로필로 전환
/ecl-config --profile quality       # 품질 프로필로 전환
```
...
```bash
/ecl-surface list                   # 현재 표면 표시
/ecl-surface profile standard       # 표준 프로필로 전환
/ecl-surface disable utility        # 유틸리티 클러스터 비활성화
/ecl-surface reset                  # 설치 시 프로필 복원
```
...
```bash
/ecl-map-codebase                   # 전체 코드베이스 분석 (4개 병렬 에이전트)
/ecl-map-codebase auth              # auth 영역에 집중
/ecl-map-codebase --fast            # 빠른 기술 + 아키텍처 개요 (1 에이전트)
/ecl-map-codebase --fast --focus quality  # 품질 및 코드 건강도만
/ecl-map-codebase --query authentication  # 인텔에서 용어 검색
```
...
```bash
/ecl-graphify build                 # 지식 그래프 구축 또는 재구축
/ecl-graphify query authentication  # 그래프에서 용어 검색
/ecl-graphify status                # 신선도 및 통계 표시
/ecl-graphify diff                  # 마지막 빌드 이후의 변경사항 표시
```

**프로그래밍 방식 접근:** `node ecl-tools.cjs graphify <build|query|status|diff|snapshot>` — [CLI 도구 참조](CLI-TOOLS.md) 참조.

### `ecl-tools intel api-surface`

`/ecl-map-codebase`가 구축한 `.planning/intel/api-map.json` 인덱스를 `.planning/intel/`의 사람이 읽을 수 있는 `API-SURFACE.md`로 렌더링합니다. `config.json`에서 `intel.enabled: true`로 게이팅되며; Intel이 비활성화된 경우 명령어가 활성화 힌트를 출력하고 종료합니다. 출력 경로는 항상 `.planning/intel/API-SURFACE.md` — `--out` 또는 `--format` 플래그가 없습니다. `api-map.json`이 없거나 비어 있으면 명령어는 여전히 명시적인 "불완전" 배너와 함께 파일을 작성하므로 소비자가 침묵을 "아무것도 없음"으로 혼동하지 않습니다.

**생성 결과:** `.planning/intel/API-SURFACE.md`

```bash
node ecl-tools.cjs intel api-surface              # api-map.json → API-SURFACE.md 렌더링
```
...
```bash
/ecl-ai-integration-phase              # 현재 단계의 마법사
/ecl-ai-integration-phase 3           # 특정 단계의 마법사
```
...
실행된 AI 단계의 평가 커버리지를 감사하고 EVAL-REVIEW.md 개선 계획을 생성합니다. `/ecl-ai-integration-phase`가 생성한 `AI-SPEC.md` 평가 계획에 대한 구현을 확인합니다. 각 평가 차원을 COVERED/PARTIAL/MISSING으로 점수를 매깁니다.

**전제 조건:** 단계가 실행되었고 `AI-SPEC.md`가 있음
**생성 결과:** 결과, 갭, 개선 지침이 담긴 `{phase}-EVAL-REVIEW.md`

```bash
/ecl-eval-review                       # 현재 단계 감사
/ecl-eval-review 3                     # 특정 단계 감사
```
...
변경 로그 미리보기와 함께 eCL를 업데이트하고, 선택적으로 스킬을 동기화하거나 로컬 패치를 재적용합니다.

| 플래그 | 설명 |
|------|-------------|
| `--sync` | 업데이트 후 eCL 레지스트리에서 스킬 동기화 |
| `--reapply` | 업데이트 후 로컬 수정사항(패치) 복원 |

```bash
/ecl-update                         # 업데이트 확인 및 설치
/ecl-update --sync                  # 업데이트 및 스킬 동기화
/ecl-update --reapply               # 업데이트 및 로컬 패치 재적용
```
...
**생성 에이전트:** `ecl-code-reviewer` 에이전트; `--fix` 사용 시 `ecl-code-fixer` 에이전트

**선택적 구조적 사전 통과:** `code_quality.fallow.enabled`를 `true`로 설정하면 에이전트 검토 전에 fallow를 실행합니다. eCL는 `{phase}/FALLOW.json`을 작성하고 `REVIEW.md`에 `Structural Findings (fallow)` 섹션을 포함합니다. `code_quality.fallow.scope`와 `code_quality.fallow.profile`로 범위와 프로필을 설정합니다.

```bash
/ecl-code-review 3                          # 단계 3의 표준 검토
/ecl-code-review 2 --depth=deep             # 딥 크로스 파일 검토
/ecl-code-review 4 --files src/auth.ts,src/token.ts  # 명시적 파일 목록
/ecl-code-review 3 --fix                    # 검토 후 Critical + Warning 결과 수정
/ecl-code-review 3 --fix --all             # 검토 후 Info 포함 모든 결과 수정
/ecl-code-review 3 --fix --auto            # 검토, 수정, 깨끗해질 때까지 재검토 (최대 3회 반복)
```
...
```bash
/ecl-audit-fix                              # audit-uat 실행, medium+ 문제 수정 (최대 5개)
/ecl-audit-fix --severity high             # 고심각도 문제만 수정
/ecl-audit-fix --dry-run                   # 수정 없이 분류 미리보기
/ecl-audit-fix --max 10 --severity all     # 모든 심각도의 최대 10개 문제 수정
```
...
**`/ecl-quick`의 대안이 아닙니다** — 리서치, 다단계 계획, 또는 검증이 필요한 작업에는 `/ecl-quick`을 사용하세요.
...
**기본 리뷰어 동작 (플래그 없음):**
- `review.default_reviewers`가 **설정되지 않은** 경우, `/ecl-review`는 감지된 모든 리뷰어를 실행합니다 (현재 기본 동작).
- `review.default_reviewers`가 **설정된** 경우, `/ecl-review`는 해당 하위 집합만 실행합니다 (예: `["gemini","codex"]`).
- `--all`은 항상 설정을 재정의하고 전체 감지된 집합을 실행합니다.
- 명시적 플래그 (예: `--cursor`)는 해당 실행에 대해 `--all`과 설정 기본값 모두를 재정의합니다.

**생성 결과:** `{phase}-REVIEWS.md` — `/ecl-plan-phase --reviews`에서 사용 가능

```bash
# 플래그 없는 /ecl-review 실행을 위한 프로젝트 기본 리뷰어 설정
ecl config-set review.default_reviewers '["gemini","codex"]'

/ecl-review --phase 2             # 설정에서 gemini+codex 실행
/ecl-review --phase 3 --all
/ecl-review --phase 2 --gemini
/ecl-review --phase 2 --cursor    # 일회성 재정의
```
...
**목적:** 리뷰어는 eCL 계획 아티팩트가 아닌 코드 변경사항만 봅니다.

```bash
/ecl-pr-branch                     # main 기준으로 필터링
/ecl-pr-branch develop             # develop 기준으로 필터링
```
...
**생성 에이전트:** `ecl-security-auditor` 에이전트
...
```bash
/ecl-secure-phase                   # 마지막 완료 단계 감사
/ecl-secure-phase 5                 # 특정 단계 감사
```
...
**생성 에이전트:** `ecl-doc-writer` 에이전트 (문서 유형당 하나), 그 후 사실 검증을 위한 `ecl-doc-verifier` 에이전트

각 문서 작성자는 코드베이스를 직접 탐색합니다 — 환각된 경로나 오래된 서명 없음. 문서 검증자는 라이브 파일시스템에 대한 주장을 확인합니다.

```bash
/ecl-docs-update                    # 대화형으로 문서 생성/업데이트
/ecl-docs-update --force            # 모든 문서 재생성
/ecl-docs-update --verify-only      # 기존 문서만 검증
```
...
**백로그:** 999.x 번호 체계는 활성 단계 시퀀스 외부에 항목을 유지합니다; 단계 디렉토리는 즉시 생성되므로 `/ecl-discuss-phase`와 `/ecl-plan-phase`가 작동합니다.
**시드:** 전체 WHY, 언제 표시할지, 이동 경로를 보존합니다 — `/ecl-new-milestone`이 사용합니다.

**생성 결과:** `.planning/todos/` (기본값), 노트 파일 (--note), ROADMAP.md 백로그 섹션 (--backlog), `.planning/seeds/SEED-NNN-slug.md` (--seed)

```bash
/ecl-capture "Consider adding dark mode support"   # 할일 추가
/ecl-capture --note "Caching strategy idea"        # 빠른 노트
/ecl-capture --note list                           # 모든 노트 나열
/ecl-capture --note promote 3                      # 노트 3을 할일로 승격
/ecl-capture --backlog "GraphQL API layer"         # 백로그에 추가
/ecl-capture --seed "Add real-time collaboration when WebSocket infra is in place"
/ecl-capture --list                                # 할일 탐색 및 실행
```
...
스레드는 여러 세션에 걸쳐 작업하지만 특정 단계에 속하지 않는 작업을 위한 경량 크로스 세션 지식 저장소입니다. `/ecl-pause-work`보다 더 가볍습니다.

```bash
/ecl-thread                         # 모든 스레드 나열
/ecl-thread list --open             # 열린/진행 중인 스레드만 나열
/ecl-thread list --resolved         # 해결된 스레드만 나열
/ecl-thread status fix-deploy-key   # 스레드 상태 표시
/ecl-thread close fix-deploy-key    # 스레드를 해결됨으로 표시
/ecl-thread fix-deploy-key-auth     # 스레드 재개
/ecl-thread "Investigate TCP timeout in pasta service"  # 새 스레드 생성
```
...
```bash
node ecl-tools.cjs roadmap upgrade --convention milestone-prefixed         # 드라이런
node ecl-tools.cjs roadmap upgrade --convention milestone-prefixed --apply  # 적용
```
...
```bash
node ecl-tools.cjs state sync             # 디스크에서 STATE.md 재구성
node ecl-tools.cjs state sync --verify    # 드라이런: 작성 없이 변경사항 표시
```
...
| 훅 | 목적 |
|------|---------|
| `ecl-validate-commit.sh` | git 커밋 메시지에 Conventional Commits 형식 강제 |
| `ecl-session-state.sh` | 세션 상태 전환 추적 |
| `ecl-phase-boundary.sh` | 단계 경계 확인 강제 |
...
eCL Discord 커뮤니티에 참여하려면 eCL README의 링크를 방문하거나 `/ecl-help`를 실행하고 거기에 표시된 Discord 링크를 따르세요.

---

## 기여: 스킬 설명 표준

스킬 설명(`commands/ecl/*.md` 프론트매터의 `description:` 필드)은
