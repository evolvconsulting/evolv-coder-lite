...
describe('bug #969 A — ensureBuiltArtifacts rebuilds stale artifacts', () => {
   ...
   * evolv-coder-lite/bin/lib/*.cjs or the real tsbuildinfo. (Regression from #996 fixed here.)
   */
  function makeTempProject() {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-bug969-'));
   ...
   * touches evolv-coder-lite/bin/lib/core.cjs or the real tsbuildinfo. (Fixed from #996.)
   */
   ...
   * the real evolv-coder-lite/bin/lib tree.
   */
...
describe('bug #969 B — runGsdTools kill-signal discrimination', () => {
  const TOOLS_PATH = path.join(__dirname, '..', 'evolv-coder-lite', 'bin', 'ecl-tools.cjs');
    ...
    try {
      ...
      if (err.killed || err.signal != null || err.code === 'ETIMEDOUT') {
        throw new Error(
          ...
          `ecl-tools was killed before completion ` +
   ...
   *   ecl-tools output from it, causing a confusing downstream assertion fail.
   *
   * PASS-AFTER (fix):
   ...
   * real ecl-tools invocation (even `--help` takes >1ms to start node).
   */
  test('throws a resource-starvation error when subprocess is killed/times out', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-969-'));
    try {
      // 1ms timeout guarantees ETIMEDOUT / killed before ecl-tools can respond.
      assert.throws(
  ...
  test('returns { success: true } for a normal fast command with generous timeout', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-969-'));
    try {
      // 30s timeout; ecl-tools --help completes in well under 1s.
  ...
  /**
   * Verify that a clean non-zero exit (a real ecl-tools application error, not
