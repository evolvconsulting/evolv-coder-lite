...
/**
 * Parse ECL_NOW_MS to a valid pinned epoch millisecond value, or return null.
 *
 ...
 * Returns null when ECL_TEST_MODE is not set.
 */
function _pinnedNowMs(): number | null {
  if (!process.env.ECL_TEST_MODE) return null;
  const raw = process.env.ECL_NOW_MS;
...
export const realClock: Clock = {
   ...
   *
   * When both ECL_TEST_MODE and ECL_NOW_MS are set (subprocess time-pin adapter,
