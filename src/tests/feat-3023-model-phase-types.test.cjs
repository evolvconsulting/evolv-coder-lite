...
describe('#3023 resolver: models.<phase_type> overrides profile-based tier', () => {
  ...
  test('phase-type alone — research agents get the phase-type tier, planner gets profile default', () => {
    ...
    // ecl-phase-researcher is a research agent — should pick up 'haiku'
    // from the phase-type slot, not 'sonnet' from the balanced profile.
    assert.equal(resolveModelInternal(projectDir, 'ecl-phase-researcher'), 'haiku');
    // ecl-codebase-mapper is also research → haiku
    assert.equal(resolveModelInternal(projectDir, 'ecl-codebase-mapper'), 'haiku');
    // ecl-planner is planning, no models.planning set → falls through to
    // profile (balanced → opus per MODEL_PROFILES).
    assert.equal(resolveModelInternal(projectDir, 'ecl-planner'), 'opus');
  });
  ...
  test('per-agent override beats phase-type (acceptance criterion b)', () => {
    writeConfig(projectDir, {
      model_profile: 'balanced',
      models: { research: 'haiku' },
      model_overrides: { 'ecl-phase-researcher': 'opus' },
    });
    // The targeted per-agent override wins for that one agent.
    assert.equal(resolveModelInternal(projectDir, 'ecl-phase-researcher'), 'opus');
    // Other research agents still pick up the phase-type tier.
    assert.equal(resolveModelInternal(projectDir, 'ecl-codebase-mapper'), 'haiku');
    assert.equal(resolveModelInternal(projectDir, 'ecl-research-synthesizer'), 'haiku');
  });
  ...
  test('phase-type beats profile (acceptance criterion c)', () => {
    ...
    assert.equal(resolveModelInternal(projectDir, 'ecl-phase-researcher'), 'haiku');
    assert.equal(resolveModelInternal(projectDir, 'ecl-codebase-mapper'), 'haiku');
    // ecl-planner is planning, no slot set, profile=quality → opus.
    assert.equal(resolveModelInternal(projectDir, 'ecl-planner'), 'opus');
  });
  ...
  test('issue example: opus for planning/discuss/execution, sonnet for research/verification/completion', () => {
    ...
    assert.equal(resolveModelInternal(projectDir, 'ecl-executor'), 'opus');
    // Research agents → sonnet
    assert.equal(resolveModelInternal(projectDir, 'ecl-phase-researcher'), 'sonnet');
    // Verification agents → sonnet
    assert.equal(resolveModelInternal(projectDir, 'ecl-verifier'), 'sonnet');
  });
  ...
  test('phase-type "inherit" is honored (preserves existing inherit semantics)', () => {
    ...
    assert.equal(resolveModelInternal(projectDir, 'ecl-phase-researcher'), 'inherit');
  });
  ...
  test('unrecognized tier value falls through to profile (typo safety) — CR follow-up', () => {
    ...
    assert.equal(resolveModelInternal(projectDir, 'ecl-phase-researcher'), 'sonnet');
    assert.equal(resolveModelInternal(projectDir, 'ecl-codebase-mapper'), 'haiku',
      'ecl-codebase-mapper at balanced is haiku per profile, unaffected by typo');
  });
  ...
  test('phase-type override wins over profile=inherit (CR Major) — model resolver', () => {
    ...
    // ecl-executor (execution) must get the phase-type opus, not inherit.
    assert.equal(resolveModelInternal(projectDir, 'ecl-executor'), 'opus');
  });
  ...
  test('phase-type "haiku" wins over profile=inherit; agents without a slot still inherit', () => {
    ...
    assert.equal(resolveModelInternal(projectDir, 'ecl-planner'), 'inherit');
  });
  ...
  test('profile=inherit with no models block still returns inherit (no regression)', () => {
    ...
    assert.equal(resolveModelInternal(projectDir, 'ecl-executor'), 'inherit');
    assert.equal(resolveModelInternal(projectDir, 'ecl-phase-researcher'), 'inherit');
  });
  ...
  test('profile=inherit with models block but agent has no slot → inherit', () => {
    ...
    // ecl-executor (execution slot) is not set → falls through to inherit.
    assert.equal(resolveModelInternal(projectDir, 'ecl-executor'), 'inherit');
  });
});
...
const { resolveReasoningEffortInternal } = require('../evolv-coder-lite/bin/lib/core.cjs');
...
describe('#3023 + #3030 CR: resolveReasoningEffortInternal honors phase-type tier (Codex)', () => {
  ...
  test('phase-type override flips both model AND reasoning_effort to the same tier (Codex)', () => {
    ...
    // ecl-executor's profile tier under balanced is sonnet, so without
    ...
    const effort = resolveReasoningEffortInternal(projectDir, 'ecl-executor');
    ...
    const sonnetEffort = (() => {
      ...
      try {
        ...
        return resolveReasoningEffortInternal(sonnetDir, 'ecl-executor');
      } finally {
        rmr(sonnetDir);
      }
    })();
    const opusEffort = (() => {
      ...
      try {
        ...
        return resolveReasoningEffortInternal(opusDir, 'ecl-executor');
      } finally {
        rmr(opusDir);
      }
    })();
  ...
  test('per-agent override still bypasses phase-type for reasoning_effort', () => {
    writeConfig(projectDir, {
      ...
      model_profile: 'balanced',
      models: { execution: 'opus' },
      model_overrides: { 'ecl-executor': 'openai/gpt-5' },
    });
    ...
    assert.equal(resolveReasoningEffortInternal(projectDir, 'ecl-executor'), null);
  });
  ...
  test('phase-type override wins over profile=inherit for effort (CR Major #3030)', () => {
    ...
    // Compute the expected effort by reading what ecl-executor would
    ...
    const expected = (() => {
      ...
      try {
        ...
        return resolveReasoningEffortInternal(dir, 'ecl-executor');
      } finally {
        rmr(dir);
      }
    })();
    const actual = resolveReasoningEffortInternal(projectDir, 'ecl-executor');
...
describe('#3023 config-schema: models.<phase_type> validation', () => {
  ...
  test('unknown phase-type is rejected (acceptance criterion d)', () => {
    ...
    assert.equal(isValidConfigKey('models.ecl-planner'), false,
