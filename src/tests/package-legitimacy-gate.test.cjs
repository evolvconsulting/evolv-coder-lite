...
const RESEARCHER = path.join(AGENTS, 'ecl-phase-researcher.md');
const PLANNER = path.join(AGENTS, 'ecl-planner.md');
const EXECUTOR = path.join(AGENTS, 'ecl-executor.md');
...
describe('ecl-phase-researcher.md — package-legitimacy seam invocation', () => {
  let model;
  ...
  test('invokes ecl-tools query package-legitimacy check inside a fenced code block', () => {
...
describe('ecl-phase-researcher.md — Package Legitimacy Audit section in template', () => {
  ...
  test('Package Legitimacy Audit table has required columns', () => {
    ...
    // 'slopcheck' column renamed to 'Verdict' to reflect the code seam (ecl-tools query package-legitimacy)
...
describe('ecl-phase-researcher.md — ecosystem-specific package verification', () => {
  let model;
...
describe('ecl-phase-researcher.md — no npx --yes auto-download', () => {
  let model;
...
describe('ecl-phase-researcher.md — WebSearch-origin package tagging', () => {
  let model;
...
describe('ecl-planner.md — checkpoint gate for [ASSUMED]/[SUS] packages', () => {
  let model;
...
describe('ecl-planner.md — supply-chain row in threat_model template', () => {
  let planTemplate;
...
describe('ecl-planner.md — no npx --yes auto-download', () => {
  test('does not invoke npx --yes inside a code block', () => {
    const model = readModel(PLANNER);
    const found = model.codeBlocks.some((block) => hasAllTokens(block, ['npx', '--yes']));
...
describe('ecl-executor.md — package installs excluded from RULE 3 auto-fix', () => {
  let model;
