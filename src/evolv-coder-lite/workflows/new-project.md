...
- ecl-project-researcher — Researches project-level technical decisions
- ecl-research-synthesizer — Synthesizes findings from parallel research agents
- ecl-roadmapper — Creates phased execution roadmaps
</available_agent_types>
...
- File reference: `/ecl:new-project --auto @prd.md`
...
Usage:
  /ecl:new-project --auto @your-idea.md
  /ecl:new-project --auto [paste or write your idea here]
...
INIT=$(ecl_run query init.new-project)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_RESEARCHER=$(ecl_run query agent-skills ecl-project-researcher)
AGENT_SKILLS_SYNTHESIZER=$(ecl_run query agent-skills ecl-research-synthesizer)
AGENT_SKILLS_ROADMAPPER=$(ecl_run query agent-skills ecl-roadmapper)
```
...
```text
⚠ eCL agents not installed. The following agents are missing from your agents directory:
...
Subagent spawns (ecl-project-researcher, ecl-research-synthesizer, ecl-roadmapper) will fail
with "agent type not found" if `required_agents_installed` is false. Run the installer with --global to make agents available:

  npx @evolvconsulting/evolv-coder-lite@latest --global
...
**If `project_exists` is true:** Error — project already initialized. Use `/ecl:progress`.
...
- options:
  - "Map codebase first" — Run /ecl:map-codebase to understand existing architecture (Recommended)
  - "Skip mapping" — Proceed with project initialization

**If "Map codebase first":**

```
Run `/ecl:map-codebase` first, then return to `/ecl:new-project`
```
...
Ask which optional PRD-style sections `/ecl:ship` should append to generated PR bodies. These map to `ship.pr_body_sections`; selected sections are written with `"enabled": true`, unselected seeded sections are written with `"enabled": false` so the project can enable them later without editing `ship.md`.
...
AskUserQuestion([
  {
    header: "PR Body",
    question: "Which optional PRD-style sections should /ecl:ship include in PR bodies?",
    multiSelect: true,
    options: [
...
```bash
mkdir -p .planning
ecl_run query commit "chore: add project config" --files .planning/config.json
```
...
{if HAS_SPIKES && !SPIKE_SKILL}  ◆ Raw spikes in .planning/spikes/ — consider `/ecl:spike --wrap-up` to package findings
{if HAS_SKETCHES && !SKETCH_SKILL}  ◆ Raw sketches in .planning/sketches/ — consider `/ecl:sketch --wrap-up` to package findings
...
**After each phase transition** (via `/ecl-transition`):
...
**After each milestone** (via `/ecl:complete-milestone`):
...
```bash
mkdir -p .planning
ecl_run query commit "docs: initialize project" --files .planning/PROJECT.md
```
...
**Check for global defaults** at `~/.ecl/defaults.json`. If the file exists, read and display its contents before asking:

```bash
DEFAULTS_RAW=$(cat ~/.ecl/defaults.json 2>/dev/null)
```
...
```text
Your saved defaults (~/.ecl/defaults.json):
  • Mode: [value]
  • Granularity: [value]
...
**If "Configure fresh" or `~/.ecl/defaults.json` doesn't exist:** proceed with the questions below.
...
**PR body onboarding:** Ask which optional PRD-style sections `/ecl:ship` should append to generated PR bodies. Use the same `ship.pr_body_sections` mapping as Step 2a: selected sections get `enabled: true`, seeded-but-unselected sections get `enabled: false`, and selecting none writes an empty list. Prefer lean/agile PRD sections that make user value, acceptance criteria, Definition of Done, and stakeholder traceability explicit.
...
```bash
mkdir -p .planning
ecl_run query config-new-project '{"mode":"[yolo|interactive]","granularity":"[selected]","parallelization":true|false,"commit_docs":true|false,"model_profile":"quality|balanced|budget|inherit","workflow":{"research":true|false,"plan_check":true|false,"verifier":true|false,"nyquist_validation":[false if granularity=coarse, true otherwise]},"plan_review":{"source_grounding":true|false},"ship":{"pr_body_sections":[{"heading":"User Stories & Acceptance Criteria","enabled":true|false,"source":"REQUIREMENTS.md ## User Stories || REQUIREMENTS.md ## Acceptance Criteria","fallback":"- Acceptance criteria are covered by the linked requirements and verification evidence."},{"heading":"Risks & Dependencies","enabled":true|false,"source":"PLAN.md ## Risks || PLAN.md ## Dependencies","fallback":"- No known high-risk rollout dependencies."},{"heading":"Success Metrics & Release Criteria","enabled":true|false,"source":"REQUIREMENTS.md ## Definition of Done || VERIFICATION.md ## Release Criteria","fallback":"- Release when automated verification and required manual checks pass."},{"heading":"Stakeholder Review & Approval","enabled":true|false,"template":"- Product owner approval pending for {phase_name}."}]}}'
```

**Note:** Run `/ecl:settings` anytime to update model profile, workflow agents, branching strategy, and other preferences.
...
- question: "I detected separate git repos in this workspace. Which directories contain code that eCL should commit to?"
- multiSelect: true
...
Spawn 4 parallel ecl-project-researcher agents with path references:
...
Use template: ~/.claude/evolv-coder-lite/templates/research-project/STACK.md
</output>
", subagent_type="ecl-project-researcher", model="{researcher_model}", description="Stack research")
...
Use template: ~/.claude/evolv-coder-lite/templates/research-project/FEATURES.md
</output>
", subagent_type="ecl-project-researcher", model="{researcher_model}", description="Features research")
...
Use template: ~/.claude/evolv-coder-lite/templates/research-project/ARCHITECTURE.md
</output>
", subagent_type="ecl-project-researcher", model="{researcher_model}", description="Architecture research")
...
Use template: ~/.claude/evolv-coder-lite/templates/research-project/PITFALLS.md
</output>
", subagent_type="ecl-project-researcher", model="{researcher_model}", description="Pitfalls research")
```
...
Use template: ~/.claude/evolv-coder-lite/templates/research-project/SUMMARY.md
Commit after writing.
</output>
", subagent_type="ecl-research-synthesizer", model="{synthesizer_model}", description="Synthesize research")
```

> **ORCHESTRATOR RULE — CODEX RUNTIME**: After calling Agent() above, stop working on this task immediately. Do not read more files, edit code, or run tests related to this task while the subagent is active. Wait for the subagent to return its result. This prevents duplicate work, conflicting edits, and wasted context. Only resume when the subagent result is available.

**Synthesizer output self-heal (#222) — verify SUMMARY.md materialized:** The synthesizer's canonical output is `.planning/research/SUMMARY.md` on disk; its brief structured return (`## SYNTHESIS COMPLETE` plus a few `###` confirmation lines) is NOT the file content. A known LLM false-refusal (issue #222) sometimes makes the agent return the full SUMMARY.md document inline — fabricating a write restriction (e.g. "the runtime is blocking file writes") — instead of writing the file. Prompt hardening alone does not fully eliminate it, so the orchestrator MUST absorb the failure deterministically before spawning `ecl-roadmapper`:

1. Verify `.planning/research/SUMMARY.md` exists AND is substantive — non-empty, and free of any leftover `<!-- ecl:write-continue -->` continuation sentinel (which marks a truncated/incomplete write). You may validate with `ecl-tools verify-summary .planning/research/SUMMARY.md` — it exits 0 regardless, so check its JSON `passed` field (`"passed": false` means missing or invalid), not the process exit code. If it passes, continue normally.
2. If it is MISSING or invalid AND the synthesizer's return message contains the FULL SUMMARY.md document — recognizable by the template's top-level markers `# Project Research Summary`, `## Key Findings`, `## Implications for Roadmap`, and `## Sources`, not merely the brief `## SYNTHESIS COMPLETE` confirmation — the false-refusal fired: write that returned document to `.planning/research/SUMMARY.md` with the Write tool, then commit ALL research artifacts the synthesizer owns (it commits on behalf of the four researchers) with `ecl-tools query commit "docs: complete project research" --files .planning/research/` unless they are already committed. Log `⚠ #222 self-heal: synthesizer returned SUMMARY.md inline without writing it; orchestrator persisted the file.`
3. If it is MISSING or invalid AND the return is only a brief confirmation (no full SUMMARY document to recover), the synthesizer genuinely failed — surface the error and stop; do NOT spawn `ecl-roadmapper` against a missing or incomplete SUMMARY.md.

This guarantees `ecl-roadmapper` (which lists SUMMARY.md as required reading) never runs against a missing or truncated SUMMARY.md.

Display research complete banner and key findings:

```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 eCL ► RESEARCH COMPLETE ✓
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
```bash
ecl_run query commit "docs: define v1 requirements" --files .planning/REQUIREMENTS.md
```
...
Spawn ecl-roadmapper agent with path references:
...
", subagent_type="ecl-roadmapper", model="{roadmapper_model}", description="Create roadmap")
```
...
- Re-spawn roadmapper with revision context:
  ...
  ", subagent_type="ecl-roadmapper", model="{roadmapper_model}", description="Revise roadmap")
  ```
...
```bash
ecl_run query generate-claude-md --output "$INSTRUCTION_FILE"
```

This ensures new projects get the default eCL workflow-enforcement guidance and current project context in `$INSTRUCTION_FILE`.

**Commit roadmap (after approval or auto mode):**

```bash
ecl_run query commit "docs: create roadmap ([N] phases)" --files .planning/ROADMAP.md .planning/STATE.md .planning/REQUIREMENTS.md "$INSTRUCTION_FILE"
```
...
Exit skill and invoke SlashCommand("/ecl:discuss-phase 1 --auto")
...
```bash
PHASE1_SECTION=$(ecl_run query roadmap.get-phase 1 2>/dev/null)
...
/ecl:discuss-phase 1 — gather context and clarify approach

---

**Also available:**
- /ecl:ui-phase 1 — generate UI design contract (recommended for frontend phases)
- /ecl:plan-phase 1 — skip discussion, plan directly
...
- [ ] ecl-roadmapper spawned with context
- [ ] Roadmap files written immediately (not draft)
- [ ] User feedback incorporated (if any)
- [ ] ROADMAP.md created with phases, requirement mappings, success criteria
- [ ] STATE.md initialized
- [ ] REQUIREMENTS.md traceability updated
- [ ] `$INSTRUCTION_FILE` generated with eCL workflow guidance (AGENTS.md for Codex, `.claude/CLAUDE.md` otherwise; an existing hand-crafted file without eCL markers is left untouched unless `--force`)
- [ ] User knows next step is `/ecl:discuss-phase 1`
