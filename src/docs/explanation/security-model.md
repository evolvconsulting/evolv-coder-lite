# eCL Core security model

> **Explanation** — This document describes *why* eCL Core has the security
> posture it does and *how the layers fit together*. It is not a reference for
> every hook parameter. For the `/ecl-secure-phase` command and its options,
...
eCL Core does. The research → plan → execute pipeline automates the full path
...
eCL Core's security model is built around one organising principle:
...
Without a gate, a hallucinated name would flow undetected through eCL's
...
**Research stage.** When `ecl-phase-researcher` recommends external packages,
...
**Planning stage.** `ecl-planner` reads the Audit table. For any package
...
**Execution stage.** If an install fails, `ecl-executor` **surfaces a
...
research time), eCL applies the strictest possible fallback: **every
...
eCL Core generates Markdown files that become LLM system prompts. The
...
eCL Core addresses prompt injection at three levels.

**Input validation (`security.cjs`).** The `evolv-coder-lite/bin/lib/security.cjs`
...
**Runtime hook: `ecl-prompt-guard.js`.** This hook fires on every Write or
...
**Runtime hook: `ecl-read-injection-scanner.js`.** This hook fires on the
...
has embedded instructions in a file that eCL is about to incorporate into an
agent's context.
...
This catches injection attempts in the eCL source itself — for example, a
supply-chain attack that modified a workflow file to add a role-override
instruction.

### Read Injection Scanner vs Prompt Guard

The two hooks cover complementary surfaces. `ecl-prompt-guard.js` watches
*writes to planning artifacts* — it catches injection being planted. 
`ecl-read-injection-scanner.js` watches *reads of any file* — it catches
...
Upstream of eCL's runtime behaviour, the `evolvconsulting` organisation enforces
...
confusion and typosquatting attacks against eCL's own dependencies.
...
`https://github.com/evolvconsulting/evolv-coder-lite/security/advisories/new`. Do not open
...
- [Commands](../COMMANDS.md) — includes `/ecl-secure-phase` and
  `/ecl-code-review` with security-relevant flags
