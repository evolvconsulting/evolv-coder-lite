/**
 * Tests for src/core-utils.cts (compiled to evolv-coder-lite/bin/lib/core-utils.cjs).
 *
...
const coreUtils = require('../evolv-coder-lite/bin/lib/core-utils.cjs');
...
describe('detectSubRepos', () => {
  ...
  test('returns empty array for directory with no sub-repos', () => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-cu-test-'));
