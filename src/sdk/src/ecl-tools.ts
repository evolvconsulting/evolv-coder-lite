/**
 * eCL Tools Bridge — programmatic access to eCL planning operations.
 *
 * By default routes commands through the SDK **query registry** (same handlers as
 * `ecl-sdk query`) so `PhaseRunner`, `InitRunner`, and `eCL` share contracts with
 ...
 * repeated argv resolution. When a workstream is set, dispatches to `ecl-tools.cjs` so
...
import { GSDToolsError } from './ecl-tools-error.js';
import type { QueryCommandResolution } from './query/query-command-resolution-strategy.js';
import { resolveGsdToolsPath } from './query-ecl-tools-path.js';
import { createGSDToolsRuntime } from './query-ecl-tools-runtime.js';
...
export { GSDToolsError } from './ecl-tools-error.js';
...
export class GSDTools {
  ...
  constructor(opts: {
    ...
    timeoutMs?: number;
    workstream?: string;
    /** When set, mutation handlers emit the same events as `ecl-sdk query`. */
  ...
  /**
   * Execute a ecl-tools command and return parsed JSON output.
  ...
  /**
   * Execute a ecl-tools command and return raw stdout without JSON parsing.
  ...
  /**
   * Query phase state from ecl-tools.cjs `init phase-op`.
  ...
  /**
   * Begin phase state tracking in ecl-tools.cjs.
   */
  ...
  /**
   * Query new-project init state from ecl-tools.cjs `init new-project`.
  ...
  /**
   * Set a config value via ecl-tools.cjs `config-set`.
   * Handles type coercion (booleans, numbers, JSON) on the ecl-tools side.
...
export { resolveGsdToolsPath } from './query-ecl-tools-path.js';
