 ...
 *
 * Also covers: `ecl-tools roadmap validate` subcommand shape.
 *
 ...
 *   Each milestone maps to a `## [eCL] vX.Y — Label` section; each phase maps
...
describe('W021 — milestone-prefixed phase ID convention', () => {
  ...
  test("'ecl-tools roadmap validate' subcommand returns JSON with warnings array", () => {
    buildRoadmap(tmpDir, [
      {
        version: 'v1.0',
        label: 'Foundation',
  ...
  test('W021 warning text includes the migration command', () => {
    ...
    const migrationCmd = 'ecl-tools roadmap upgrade --convention milestone-prefixed';
