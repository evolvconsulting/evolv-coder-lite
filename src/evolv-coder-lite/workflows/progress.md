...
Run /ecl:new-project to start a new project.
```

Exit.

If missing STATE.md: suggest `/ecl:new-project`.

**If ROADMAP.md missing but PROJECT.md exists:**

This means a milestone was completed and archived. Go to **Route F** (between milestones).

If missing both ROADMAP.md and PROJECT.md: suggest `/ecl:new-project`.
</step>
...
## Pending Todos
- [count] pending — /ecl:capture --list to review

## Active Debug Sessions
- [count] active — /ecl:debug to continue
...
Review: `/ecl:audit-uat ${ECL_WS}` — full cross-phase audit
Resume testing: `/ecl:verify-work {phase} ${ECL_WS}` — retest specific phase
```
...
`/ecl:plan-phase {phase-number} ${ECL_WS}`
...
`/ecl:discuss-phase {phase}` — gather context and clarify approach

---

**Also available:**
- `/ecl:ui-phase {phase}` — generate UI design contract (recommended for frontend phases)
- `/ecl:plan-phase {phase}` — skip discussion, plan directly
- `/ecl:discuss-phase {phase}` — include assumptions check before planning
...
`/ecl:discuss-phase {phase} ${ECL_WS}` — gather context and clarify approach

---

**Also available:**
- `/ecl:plan-phase {phase} ${ECL_WS}` — skip discussion, plan directly
- `/ecl:discuss-phase {phase} ${ECL_WS}` — include assumptions check before planning
...
`/ecl:plan-phase {phase} --gaps ${ECL_WS}`

---

**Also available:**
- `/ecl:execute-phase {phase} ${ECL_WS}` — execute phase plans
- `/ecl:verify-work {phase} ${ECL_WS}` — run more UAT testing
...
`/ecl:verify-work {phase} ${ECL_WS}` — resume testing from where you left off

---

**Also available:**
- `/ecl:audit-uat ${ECL_WS}` — full cross-phase UAT audit
- `/ecl:execute-phase {phase} ${ECL_WS}` — execute phase plans
...
`/ecl:verify-work {phase} ${ECL_WS}` — resume human verification
...
`/ecl:discuss-phase {Z+1}` — gather context and clarify approach

---

**Also available:**
- `/ecl:ui-phase {Z+1}` — generate UI design contract (recommended for frontend phases)
- `/ecl:plan-phase {Z+1}` — skip discussion, plan directly
- `/ecl:verify-work {Z}` — user acceptance test before continuing
...
`/ecl:discuss-phase {Z+1} ${ECL_WS}` — gather context and clarify approach

---

**Also available:**
- `/ecl:plan-phase {Z+1} ${ECL_WS}` — skip discussion, plan directly
- `/ecl:verify-work {Z} ${ECL_WS}` — user acceptance test before continuing
...
`/ecl:complete-milestone ${ECL_WS}`

---

**Also available:**
- `/ecl:verify-work ${ECL_WS}` — user acceptance test before completing milestone
...
- Phase complete but next phase not planned → offer `/ecl:plan-phase [next] ${ECL_WS}`
- All work complete → offer milestone completion
- Blockers present → highlight before offering to continue
- Handoff file exists → mention it, offer `/ecl:resume-work ${ECL_WS}`
</step>
...
- Check 2 (orphaned handoff): `Read the handoff file(s) and resume from where work was paused: /ecl:resume-work ${ECL_WS}`
...
- Check 1 (STATE inconsistency): `Run /ecl:verify-work ${PHASE} ${ECL_WS} to reconcile state`
</step>
...
- [ ] Smart routing: /ecl:execute-phase if plans exist, /ecl:plan-phase if not
- [ ] User confirms before any action
- [ ] Seamless handoff to appropriate ecl command
      </success_criteria>
