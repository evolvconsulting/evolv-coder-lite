...
describe('config-get --default flag (#1893)', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-config-default-'));
  ...
  function run(...args) {
    return execFileSync('node', [ECL_TOOLS, ...args, '--cwd', tmpDir], {
      encoding: 'utf-8',
      timeout: 5000,
    }).trim();
  }
  ...
  function runExpectError(...args) {
    try {
      execFileSync('node', [ECL_TOOLS, ...args, '--cwd', tmpDir], {
        encoding: 'utf-8',
        timeout: 5000,
