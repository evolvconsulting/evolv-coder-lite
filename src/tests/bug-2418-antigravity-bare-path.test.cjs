 ...
 *
 * Files affected: agents/ecl-debugger.md (configDir = ~/.claude) and
 * evolv-coder-lite/workflows/update.md (comment with e.g. ~/.claude).
 */
...
describe('convertClaudeToAntigravityContent bare path replacement (#2418)', () => {
  describe('global install', () => {
    ...
    test('still replaces ~/.claude/ (with trailing slash) correctly', () => {
      const input = 'See ~/.claude/evolv-coder-lite/workflows/';
      const result = convertClaudeToAntigravityContent(input, true);
      assert.ok(
        result.includes('~/.gemini/antigravity/evolv-coder-lite/workflows/'),
    ...
    test('does not double-replace ~/.claude/ paths', () => {
      const input = 'See ~/.claude/evolv-coder-lite/';
      const result = convertClaudeToAntigravityContent(input, true);
  ...
  describe('installed files contain no bare ~/.claude references after conversion', () => {
    ...
    test('ecl-debugger.md has no leaked ~/.claude after global Antigravity conversion', () => {
      const debuggerPath = path.join(repoRoot, 'agents', 'ecl-debugger.md');
      ...
      assert.strictEqual(
        matches, null,
        `ecl-debugger.md still contains leaked .claude paths after Antigravity conversion: ${matches}`
      );
    });

    test('update.md has no leaked ~/.claude after global Antigravity conversion', () => {
      const updatePath = path.join(repoRoot, 'evolv-coder-lite', 'workflows', 'update.md');
