...
`.planning/spikes/WRAP-UP-SUMMARY.md`. Companion to `/ecl:spike`.
</purpose>
...
Run `/ecl:spike` first to create experiments.
```
Exit.
...
```bash
$ECL_SDK query commit "docs(spike-wrap-up): package [N] spike findings into project skill" --files .planning/spikes/WRAP-UP-SUMMARY.md .planning/spikes/CONVENTIONS.md
```
</step>
...
`/ecl:spike` (run with no argument — its frontier mode analyzes the spike landscape and proposes integration and frontier spikes)

───────────────────────────────────────────────────────────────

**Also available:**
- `/ecl:plan-phase` — start planning the real implementation
- `/ecl:spike [idea]` — spike a specific new idea
...
- [ ] Summary presented
- [ ] Next-step options presented (including frontier spike exploration via `/ecl:spike`)
</success_criteria>
