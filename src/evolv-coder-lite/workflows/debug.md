...
Invoked by `/ecl:debug` (`commands/ecl/debug.md`).

Systematic debugging using the scientific method with subagent isolation.
Orchestrates symptom gathering, session creation, and delegation to `ecl-debug-session-manager`.

<available_agent_types>
Valid eCL subagent types (use exact names — do not fall back to 'general-purpose'):
- ecl-debug-session-manager — manages debug checkpoint/continuation loop in isolated context
- ecl-debugger — investigates bugs using scientific method
</available_agent_types>
...
─────────────────────────────────────────────
Run `/ecl:debug continue <slug>` to resume a session.
No sessions? `/ecl:debug <description>` to start.
```

If no files exist or the glob returns nothing: print "No active debug sessions. Run `/ecl:debug <issue description>` to start one."
...
**Sanitize SLUG first:** strip whitespace, reject unless it matches `^[a-z0-9][a-z0-9-]*$`, enforce max 30 chars, reject any `..`, `/`, or `\`. If invalid, print "No active debug session found with slug: {SLUG}. Check `/ecl:debug list` for active sessions." and stop.

Check `.planning/debug/{SLUG}.md` exists. If not, print "No active debug session found with slug: {SLUG}. Check `/ecl:debug list` for active sessions." and stop.
...
""",
  subagent_type="ecl-debug-session-manager",
...
## 4. Session Management (delegated to ecl-debug-session-manager)

After initial context setup, spawn the session manager to handle the full checkpoint/continuation loop. The session manager handles specialist_hint dispatch internally: when ecl-debugger returns ROOT CAUSE FOUND it extracts the specialist_hint field and invokes the matching skill (e.g. typescript-expert, swift-concurrency) before offering fix options.
...
If summary shows `ABANDONED`: note session saved at `.planning/debug/{slug}.md` for later `/ecl:debug continue {slug}`.
...
- [ ] ecl-debug-session-manager spawned with security-hardened session_params
