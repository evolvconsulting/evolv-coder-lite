---
name: ecl:workspace
description: Manage eCL workspaces — create, list, or remove isolated workspace environments
...
<objective>
Manage eCL workspaces with a single consolidated command.

Mode routing:
- **--new**: Create an isolated workspace with repo copies and independent .planning/ → new-workspace workflow
- **--list**: List active eCL workspaces and their status → list-workspaces workflow
- **--remove**: Remove a eCL workspace and clean up worktrees → remove-workspace workflow
</objective>
...
| --list | Scan ~/ecl-workspaces/, show summary table | list-workspaces |
| --remove | Confirm and remove workspace directory | remove-workspace |

</routing>

<execution_context>
@~/.claude/evolv-coder-lite/workflows/new-workspace.md
@~/.claude/evolv-coder-lite/workflows/list-workspaces.md
@~/.claude/evolv-coder-lite/workflows/remove-workspace.md
