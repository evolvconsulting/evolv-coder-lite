...
`;
    ...
    const result = convertClaudeCommandToWindsurfSkill(input, 'ecl-quick');
...
`;
    ...
    const result = convertClaudeCommandToWindsurfSkill(input, 'ecl-plan-phase');
...
`;
    ...
    const result = convertClaudeCommandToWindsurfSkill(input, 'ecl-test');
...
describe('convertClaudeToWindsurfMarkdown', () => {
  ...
  test('replaces $ARGUMENTS with {{ECL_ARGS}}', () => {
    const input = 'Pass $ARGUMENTS to the command.';
    const result = convertClaudeToWindsurfMarkdown(input);
    assert.ok(result.includes('{{ECL_ARGS}}'), '$ARGUMENTS replaced');
  });
