 ...
 *   - commands/ecl-<stem>.md  (slash command definitions)
 *   - skills/ecl-<stem>/SKILL.md  (existing skill definitions)
 *
 * mcpServers in settings.json is explicitly excluded: ecl ships no MCP server
...
describe('enh-790 — augment layout has commands + skills kinds', () => {
  ...
  test('augment commands kind targets commands/ with ecl- prefix', () => {
    const layout = resolveRuntimeArtifactLayout('augment', '/tmp/fake-augment-dir');
  ...
  test('augment skills kind targets skills/ with ecl- prefix', () => {
    const layout = resolveRuntimeArtifactLayout('augment', '/tmp/fake-augment-dir');
...
describe('enh-790 — installRuntimeArtifacts augment emits both commands and skills', () => {
  test('global augment install: commands/ecl-help.md and skills/ecl-help/SKILL.md exist', (t) => {
    const configDir = createTempDir('ecl-enh790-augment-');
  ...
  test('commands/ecl-help.md has Augment-compatible content (no raw ~/.claude/ refs)', (t) => {
    const configDir = createTempDir('ecl-enh790-content-');
    t.after(() => cleanup(configDir));

    installRuntimeArtifacts('augment', configDir, 'global', RESOLVED_CORE);

    const helpCmd = path.join(configDir, 'commands', 'ecl-help.md');
    assert.ok(fs.existsSync(helpCmd), 'ecl-help.md must exist');
  ...
  test('command count matches skill count (profile parity)', (t) => {
    const configDir = createTempDir('ecl-enh790-parity-');
    t.after(() => cleanup(configDir));
    ...
    const commandsDir = path.join(configDir, 'commands');
    const skillsDir = path.join(configDir, 'skills');
    const cmdCount = fs.readdirSync(commandsDir).filter(f => f.startsWith('ecl-') && f.endsWith('.md')).length;
  ...
  test('full profile install does NOT mutate source commands/ecl/ files', (t) => {
    ...
    const { resolveProfile } = require('../evolv-coder-lite/bin/lib/install-profiles.cjs');
    ...
    const configDir = createTempDir('ecl-enh790-full-');
    t.after(() => cleanup(configDir));

    // Record source file content before install
    const srcHelpPath = path.join(__dirname, '..', 'commands', 'ecl', 'help.md');
    ...
    assert.strictEqual(srcContentBefore, srcContentAfter,
      'source commands/ecl/help.md must not be mutated by the install');

    // Installed command file must have rewrites applied (Augment path substitution)
    const installedHelp = path.join(configDir, 'commands', 'ecl-help.md');
    assert.ok(fs.existsSync(installedHelp), 'installed ecl-help.md must exist');
...
describe('enh-790 — installRuntimeArtifacts does not leak temp dirs', () => {
  test('install cleans up ecl-cmd-rewrites-* temp dirs (no leak) — #856', (t) => {
    const { resolveProfile } = require('../evolv-coder-lite/bin/lib/install-profiles.cjs');
    const RESOLVED_FULL = resolveProfile({ modes: ['full'], manifest: MANIFEST });
    ...
    const isolatedTmp = createTempDir('ecl-enh790-tmproot-');
    ...
    const configDir = createTempDir('ecl-enh790-leak-');
    t.after(() => {
    ...
    // The install creates its ecl-cmd-rewrites-* temp dirs under the isolated root;
    // after the fix none must remain.
    const leaked = fs.readdirSync(isolatedTmp).filter(n => n.startsWith('ecl-cmd-rewrites-'));
    assert.ok(
      leaked.length === 0,
      `installer must not leak ecl-cmd-rewrites-* temp dirs; leaked: ${leaked.join(', ')}`
    );
  });
});
...
describe('enh-790 — uninstallRuntimeArtifacts removes augment commands', () => {
  test('uninstall removes ecl-* commands but preserves user commands', (t) => {
    const configDir = createTempDir('ecl-enh790-uninstall-');
    t.after(() => cleanup(configDir));

    const { uninstallRuntimeArtifacts } = require('../bin/install.js');

    // Pre-create: a eCL command + a user-owned command
    const commandsDir = path.join(configDir, 'commands');
...
describe('enh-790 — mcpServers excluded (ecl ships no MCP server)', () => {
  test('augment install does not write settings.json mcpServers', (t) => {
    const configDir = createTempDir('ecl-enh790-mcp-excluded-');
    t.after(() => cleanup(configDir));
    ...
    if (fs.existsSync(settingsPath)) {
      ...
      assert.ok(!settings.mcpServers, 'settings.json must not contain mcpServers (ecl ships no MCP server)');
    }
