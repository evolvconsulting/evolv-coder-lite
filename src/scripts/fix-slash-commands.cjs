'use strict';
/**
 * One-shot script + library: bidirectional eCL slash-command namespace normalizer.
 *
 * - Default direction (transformContent): retired /ecl-<cmd> → /ecl:<cmd>
 *   (keeps monorepo sources, docs, and workflows in the active colon form).
 * - Reverse direction (transformContentToHyphen): /ecl:<cmd> / ecl:<cmd> → ecl-<cmd>
 *   (used during skill installation for runtimes that register skills under the
 *   canonical hyphen form established in #2808).
 *
 * Both directions only rewrite known commands from `commands/ecl/*.md` (longest-first
 * matching + word-boundary safety). Non-commands (ecl-sdk, ecl-tools, etc.) are
...
const SEARCH_DIRS = [
  path.join(__dirname, '..', 'evolv-coder-lite', 'bin', 'lib'),
  path.join(__dirname, '..', 'evolv-coder-lite', 'workflows'),
  path.join(__dirname, '..', 'evolv-coder-lite', 'references'),
  path.join(__dirname, '..', 'evolv-coder-lite', 'templates'),
  path.join(__dirname, '..', 'evolv-coder-lite', 'contexts'),
...
function buildPattern(cmdNames) {
  // Empty input would compile `/ecl-()(?=[^a-zA-Z0-9_-]|$)/g`, which the regex
  // engine still matches at any `/ecl-` token followed by a non-word boundary
  // (e.g. EOL, whitespace, punctuation) — rewriting it to a stray `/ecl:`.
  ...
  return new RegExp(`/ecl-(${sorted.join('|')})(?=[^a-zA-Z0-9_-]|$)`, 'g');
}

/**
 * Pure transform: rewrite retired `/ecl-<cmd>` to `/ecl:<cmd>` for the given command names.
 * Returns the rewritten string. Identifiers not in `cmdNames` (e.g. `/ecl-sdk`,
 * `/ecl-tools`) are left untouched.
 */
function transformContent(src, cmdNames) {
  const pattern = buildPattern(cmdNames);
  if (!pattern) return src;
  return src.replace(pattern, (_, cmd) => `/ecl:${cmd}`);
}

/**
 * Build regex for the reverse direction (colon form → hyphen form).
 * Matches both "ecl:cmd" and "/ecl:cmd" (the leading / is preserved automatically
...
function buildColonPattern(cmdNames) {
  ...
  return new RegExp(`(?<![a-zA-Z0-9_-])ecl:(${sorted.join('|')})(?=[^a-zA-Z0-9_-]|$)`, 'g');
}

/**
 * Pure transform (reverse): rewrite `/ecl:<cmd>` / `ecl:<cmd>` to hyphen form
 * for known eCL commands.
 *
 * Non-command identifiers (e.g. ecl-sdk, ecl-tools) are left untouched, matching
...
function readCmdNames() {
  try {
    ...
    // global Claude installs (no local commands/ecl/ directory). Return [] so
