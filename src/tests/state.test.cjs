...
`
  ...
  describe('--cwd override', () => {
    ...
    beforeEach(() => {
      outsideDir = fs.mkdtempSync(path.join(require('os').tmpdir(), 'ecl-test-outside-'));
    });
...
`
    ...
    const output = JSON.parse(result.output);
    assert.strictEqual(output.ecl_state_version, '1.0', 'should have version 1.0');
...
`
    ...
    const output = JSON.parse(result.output);
    assert.strictEqual(output.ecl_state_version, '1.0', 'version from frontmatter');
...
`
    ...
    assert.ok(content.includes('ecl_state_version: 1.0'), 'should have version field');
...
const { stateExtractField, stateReplaceField, stateReplaceFieldWithFallback } = require('../evolv-coder-lite/bin/lib/state.cjs');
...
describe('cmdStateRecordSession (state record-session)', () => {
  ...
  test('updates session fields with stopped-at and resume-file', () => {
    ...
    const result = runGsdTools(
      ...
      tmpDir,
      { ECL_TEST_MODE: '1', ECL_NOW_MS: String(PINNED_MS) },
    );
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('state planned-phase command', () => {
  ...
  test('after call: Last Activity is the pinned date (deterministic via ECL_NOW_MS)', () => {
    fs.writeFileSync(
      path.join(tmpDir, '.planning', 'STATE.md'),
      `# Project State\n\n**Status:** Planning\n**Total Plans in Phase:** 0\n**Last Activity:** 2024-01-01\n**Current Phase:** 1\n`
    );
...
// command that just failed ("...is SDK-only. Use: ecl-tools query
// state.add-roadmap-evolution ..."), and no CJS handler existed after the SDK
// retirement (ADR-0174). Every `/ecl:phase insert` and `/ecl:phase --edit` run
...
`);
  ...
  test('shell metacharacters in --note are stored literally, never executed', () => {
    ...
    const probe = path.join(tmpDir, 'ecl-pwn-1140');
...
describe('regressions: table-format STATE.md (#1162)', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempProject('ecl-1162-');
  ...
  test('missing STATE.md returns updated:false gracefully', () => {
    ...
    const missingDir = createTempProject('ecl-1162-missing-');
    try {
...
describe('regressions: table-format STATE.md (#1162) — updateCurrentPositionFields preserve-authored invariants', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempProject('ecl-1162-f2-');
    statePath = path.join(tmpDir, '.planning', 'STATE.md');
  });
...
function make1255TempProject(stateContent) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-1255-'));
...
function make1257TempProject(stateContent) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-1257-'));
  const planningDir = path.join(dir, '.planning');
  fs.mkdirSync(planningDir, { recursive: true });
