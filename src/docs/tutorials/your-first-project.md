...
In this tutorial you will install eCL Core and build a small command-line to-do app from scratch — one phase, one PR, the full loop. By the end you will have run every command in the core phase loop at least once, and you will have seen the planning artefacts that each command produces.
...
No other tools are required. eCL Core itself is installed in the next step.

---

## Step 1 — Install eCL Core
...
Notice that a `.claude/` directory now exists in your project. That is where eCL Core's commands and agents live.
...
eCL Core spawns sub-agents that read and write files. Start Claude Code with the permissions flag so it does not pause to ask about every file operation:
...
eCL Core will open a conversation. It asks one question first:
...
eCL Core follows up with a handful of clarifying questions. Answer them naturally. It is learning what you care about before it writes a single plan.
...
eCL Core then asks you to pick workflow settings (mode, granularity, research agents). Choose the recommended defaults for each. These are written to `.planning/config.json`.

Finally, a roadmapper sub-agent runs (you'll see the "Spawning roadmapper…" notice — this is normal and takes roughly a minute). When it returns, eCL Core presents a proposed roadmap. For a single-phase project it will look something like:
...
eCL Core is designed around fresh contexts. Clear the main session window before each phase:
...
eCL Core reads the phase goal and asks about your implementation preferences. These are the decisions that shape *how* it builds, not just *what* it builds. Example exchange:
...
When the discussion closes, eCL Core writes:
...
Open `01-01-PLAN.md`. You'll see a `<task>` block with a name, the files it touches, the action steps, a verify command, and a done condition. Notice the `<verify>` tag — eCL Core's executor will run that command after writing the code.
...
eCL Core groups the plans into waves (independent plans run in parallel), spawns a fresh 200k-context executor per plan, and commits each task atomically.
...
You should see items appear, and item 1 disappear from the default list after marking it done. That is your first visible result delivered by eCL Core.
...
eCL Core extracts the phase's success criteria and walks you through each one:
...
If any check fails, eCL Core diagnoses the root cause and creates a fix plan. Run `/ecl-execute-phase 1` again to apply it, then re-run `/ecl-verify-work 1`.
...
eCL Core creates a pull request with a generated body. The PR body always includes: Summary, Changes, Requirements Addressed, Verification, and Key Decisions.
...
- How to install eCL Core with `npx @evolvconsulting/evolv-coder-lite@latest`.
- How `/ecl-new-project` turns a conversation into a roadmap backed by `.planning/` artefacts.
- How `/ecl-discuss-phase` captures implementation decisions before any planning happens.
- How `/ecl-plan-phase` spawns parallel researchers and produces atomic task plans.
- How `/ecl-execute-phase` runs those plans in parallel waves and commits each task.
- How `/ecl-verify-work` walks through success criteria and generates fix plans when needed.
- How `/ecl-ship` turns a verified phase into a pull request.

For a multi-phase project, repeat Steps 4–8 for each phase, then run `/ecl-progress --next` to let eCL Core detect the next step automatically.
...
- [Onboarding an existing codebase](onboarding-an-existing-codebase.md) — bring eCL Core to a brownfield repo
