 ...
 *
 * Cline is listed in eCL documentation as a supported runtime but was
 * completely absent from bin/install.js. Running `npx @evolvconsulting/evolv-coder-lite`
 ...
 * - Installs evolv-coder-lite/ engine with path replacement
 */
...
describe('Cline markdown conversion', () => {
  ...
  test('replaces .claude/ paths with .cline/', () => {
    const result = convertClaudeToCliineMarkdown('See ~/.claude/evolv-coder-lite/');
  ...
  test('replaces .claude/skills/ with .cline/skills/', () => {
    const result = convertClaudeToCliineMarkdown('skills at .claude/skills/ecl-executor');
...
describe('Cline install (local)', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('ecl-cline-test-');
  ...
  test('install creates .clinerules directory with ecl.md (#787 directory form)', () => {
    install(false, 'cline');
    ...
    assert.ok(fs.existsSync(path.join(clinerulesDir, 'ecl.md')), '.clinerules/ecl.md must exist');
  });

  test('.clinerules/ecl.md contains eCL instructions', () => {
    install(false, 'cline');
    const ruleFile = path.join(tmpDir, '.clinerules', 'ecl.md');
    const content = fs.readFileSync(ruleFile, 'utf8');
    assert.ok(content.includes('eCL') || content.includes('ecl'), '.clinerules/ecl.md must reference eCL');
  });

  test('install creates evolv-coder-lite engine directory', () => {
    install(false, 'cline');
    const engineDir = path.join(tmpDir, 'evolv-coder-lite');
    assert.ok(fs.existsSync(engineDir), 'evolv-coder-lite directory must exist after install');
  });
    ...
    function scanDir(dir) {
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
          ...
          // Check for eCL install paths that should have been substituted.
          ...
          const hasLeaked = /~\/\.claude\/(?:evolv-coder-lite|commands|agents|hooks)|HOME\/\.claude\/(?:evolv-coder-lite|commands|agents|hooks)/.test(content);
          assert.ok(!hasLeaked, `Found leaked eCL .claude install path in ${fullPath}`);
        }
      }
    }
    scanDir(engineDir);
