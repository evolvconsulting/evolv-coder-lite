...
| `resume_command` | string | Exact command to resume eCL reconciliation (re-enter the loop to re-check the job), e.g. `/ecl:execute-phase <phase>`. This is a eCL reconciliation entry point, not a scheduler resubmit. |
