...
INIT=$(ecl_run init phase-op "${PHASE}")
...
Use /ecl:progress to see available phases.
```
Exit.
...
If "Skip": Exit with message: "Existing SPEC.md unchanged. Run /ecl:discuss-phase [X] to continue."
...
ones). Reference: @~/.claude/evolv-coder-lite/references/edge-probe.md.

**Runtime coverage compute — resolve and invoke edge-probe.cjs:**

```bash
# Resolve the compiled edge-probe.cjs against the eCL install dir via RUNTIME_DIR (#448)
...
_ECL_RT="${RUNTIME_DIR:-$(git rev-parse --show-toplevel 2>/dev/null || pwd)}"
EDGE_PROBE_JS=$(for _c in \
  "$_ECL_RT/evolv-coder-lite/bin/lib/edge-probe.cjs" \
  "$_ECL_RT/bin/lib/edge-probe.cjs" \
  "$_ECL_RT/.claude/bin/lib/edge-probe.cjs" \
  "$HOME/.claude/evolv-coder-lite/bin/lib/edge-probe.cjs" \
  "$HOME/.claude/bin/lib/edge-probe.cjs"; do
  [ -f "$_c" ] && { echo "$_c"; break; }
done)

# Graceful degradation — never silent skip (RR-04). Build ONLY when $_ECL_RT is a verified
# eCL source checkout (has tsconfig.build.json + src/edge-probe.cts), and pin npm to it with
...
# prepublishOnly, so this build path only matters in a eCL dev checkout (review High).
if [ -z "$EDGE_PROBE_JS" ]; then
  if [ -f "$_ECL_RT/tsconfig.build.json" ] && [ -f "$_ECL_RT/src/edge-probe.cts" ]; then
    npm --prefix "$_ECL_RT" run build:lib 2>/dev/null || true
    EDGE_PROBE_JS=$(for _c in \
      "$_ECL_RT/evolv-coder-lite/bin/lib/edge-probe.cjs" \
      "$_ECL_RT/bin/lib/edge-probe.cjs" \
      "$_ECL_RT/.claude/bin/lib/edge-probe.cjs" \
  ...
  if [ -z "$EDGE_PROBE_JS" ]; then
    echo "ERROR: edge-probe.cjs not found — reinstall eCL or run \`npm run build:lib\` in your eCL checkout." >&2
    exit 1
  fi
fi
...
requirement list). Reference: @~/.claude/evolv-coder-lite/references/prohibition-probe.md — the
...
3. **Canon-referral (ADR-550 D6, PROB-13).** A kept candidate that is canon security/compliance
   ...
   /ecl:secure-phase + eslint; not minted here"*) and DROP it. Minting canon items duplicates
   /ecl:secure-phase and drowns the bespoke signal.
   ...
   - **Keep it** → write a NEGATIVE acceptance criterion (a must-NOT line) into Acceptance
     ...
     - **Capture the wired-check descriptor on `test`-tier (#1278, SOFT).** When a prohibition is
       ...
       - `check_violation_fixture` (#1346) — path to a KNOWN-BAD subject the wired check is run
         ...
         read its subject from the `ECL_PROHIB_SUBJECT` env var so the prover can inject this fixture.
...
Use the SPEC.md template from @~/.claude/evolv-coder-lite/templates/spec.md.
...
- User directed to /ecl:discuss-phase as next step
