...
describe('#14: /ecl:progress --next --auto flag must be documented and propagated', () => {
  test('progress.md <flags> section documents --auto flag', () => {
    const command = fs.readFileSync(
      path.join(ROOT, 'commands', 'ecl', 'progress.md'),
      'utf8'
    );
  ...
  test('next.md show_and_execute step handles --auto to chain steps', () => {
    const workflow = fs.readFileSync(
      path.join(ROOT, 'evolv-coder-lite', 'workflows', 'next.md'),
      'utf8'
    );
  ...
  test('next.md --auto chaining re-invokes /ecl:progress --next after step completion', () => {
    const workflow = fs.readFileSync(
      path.join(ROOT, 'evolv-coder-lite', 'workflows', 'next.md'),
      'utf8'
    );

    // The workflow must contain instructions to re-invoke /ecl:progress --next --auto
    // after the determined step completes, enabling the chain.
    assert.ok(
      workflow.includes('--next --auto'),
      'next.md must instruct re-invocation of /ecl:progress --next --auto after step completion to enable chaining'
    );
  });
});
