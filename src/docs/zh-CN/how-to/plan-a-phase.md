...
**前提条件：** `.planning/ROADMAP.md` 已存在。强烈建议（但非必须）先通过 `/ecl-discuss-phase` 生成 `{phase}-CONTEXT.md`。
...
1. **研究** — `ecl-phase-researcher` 子代理调查相关领域并写入 `{phase}-RESEARCH.md`。
2. **规划** — `ecl-planner` 子代理读取上下文、研究成果和需求，然后写入一个或多个 `{phase}-{N}-PLAN.md` 文件。
3. **验证** — `ecl-plan-checker` 子代理从八个维度验证计划质量，并触发修订循环（最多三次迭代），直至质量门控通过。

若未指定阶段编号，eCL Core 将自动定位 ROADMAP.md 中下一个未规划的阶段。
...
注意：`--research-phase <N>` 是 `/ecl-plan-phase` 上的标志。不存在独立的研究阶段命令——原来的独立研究命令已被弃用，以此标志取而代之。
...
**如果已运行 `/ecl-review --phase N` 且存在 `REVIEWS.md`：**
...
`ecl-plan-checker` 在允许执行前从八个维度验证计划：
