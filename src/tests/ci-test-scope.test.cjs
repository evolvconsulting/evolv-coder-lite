...
describe('ci-test-scope.cjs', () => {
  ...
  test('product code (evolv-coder-lite/bin/lib/foo.cjs) — product_changed true', () => {
    const result = scopeFor(['evolv-coder-lite/bin/lib/foo.cjs']);
    assert.strictEqual(result.code_changed, true);
    assert.strictEqual(result.product_changed, true,
      `expected product_changed=true for evolv-coder-lite/ change, got: ${JSON.stringify(result)}`);
  });
  ...
  test('mixed docs + code — escalates to product_changed true', () => {
    // Use bin/ecl (installer rule, fullMatrix:true) to get a code file that reliably triggers full matrix.
    const result = scopeFor(['docs/x.md', 'bin/ecl']);
    assert.strictEqual(result.code_changed, true);
    assert.strictEqual(result.product_changed, true,
  ...
  test('command changes request command tests without full parity matrix', () => {
    const result = scopeFor(['commands/ecl/plan-phase.md']);
    assert.strictEqual(result.code_changed, true);
    assert.strictEqual(result.full_matrix, false);
  ...
  test('installer-sensitive changes request full matrix and install tests', () => {
    const result = scopeFor(['bin/ecl']);
    assert.strictEqual(result.code_changed, true);
    assert.strictEqual(result.product_changed, true,
      `expected product_changed=true for bin/ecl, got: ${JSON.stringify(result)}`);
    assert.strictEqual(result.full_matrix, true);
  ...
  test('bug-408: code change with no rule match falls back to unit suite token', () => {
    // A plain source file that matches no RULES entry but is under evolv-coder-lite/ (code path)
    const result = scopeFor(['evolv-coder-lite/src/some-util.js']);
    assert.strictEqual(result.code_changed, true);
