...
`.planning/` 디렉터리는 프로젝트를 위한 eCL Core의 공유 메모리입니다. 모든 워크플로가 여기서 읽고 쓰며, 감사 가능한 결정 추적 기록을 남깁니다. 이 페이지는 모든 파일, 그 목적, 그리고 어떤 명령이 생성하거나 소비하는지를 매핑합니다. [문서 인덱스](../../README.md)를 참조하세요.
...
| **생성자** | `/ecl-new-project` (최초 생성); 결정이 검증됨에 따라 `/ecl-complete-milestone`에 의해 업데이트됩니다. |
| **소비자** | 모든 플래닝 워크플로; `ecl-phase-researcher`, `ecl-planner` (컨텍스트); `discuss-phase` (이전 결정); `ecl-plan-checker` (프로젝트 제약 사항). |
...
| **생성자** | `/ecl-new-project` (최초 생성); `/ecl-phase --insert`와 `/ecl-complete-milestone`에 의해 업데이트됩니다. |
| **소비자** | `/ecl-discuss-phase`, `/ecl-plan-phase`, `/ecl-execute-phase`; 페이즈 정보가 필요한 모든 오케스트레이션 명령; `ecl-planner`, `ecl-plan-checker`, `ecl-phase-researcher`. |
...
| **생성자** | `/ecl-new-project` (최초 생성); `execute-phase`에 의해 요구사항이 완료로 표시됩니다. |
| **소비자** | `ecl-planner` (플랜은 모든 페이즈 요구사항 ID를 처리해야 함); `ecl-plan-checker` Dimension 1 (요구사항 커버리지); `discuss-phase` (이전 요구사항). |
...
| **생성자** | `/ecl-new-project` (최초 생성); 모든 페이즈 워크플로, `/ecl-pause-work`, `/ecl-resume-work`에 의해 지속적으로 업데이트됩니다. |
| **소비자** | 모든 오케스트레이션 워크플로; `/ecl-progress`; `/ecl-quick`을 통한 임시 태스크 실행; `ecl-planner` 및 `ecl-phase-researcher` (프로젝트 결정). |
...
| **생성자** | `/ecl-new-project` (최초 생성); `/ecl-settings` (대화형 편집). |
| **소비자** | 모든 워크플로 및 서브에이전트 — `ecl-tools query config-get`을 통해 초기화 시점에 읽습니다. |
...
| **생성자** | `/ecl-complete-milestone`. |
| **소비자** | `/ecl-audit-milestone`; 인간 검토. |
...
| **생성자** | `/ecl-discuss-phase` (대화형 토론 또는 PRD/ADR 익스프레스 경로). |
| **소비자** | `ecl-phase-researcher` (무엇을 조사할지); `ecl-planner` (잠긴 결정); `ecl-plan-checker` Dimension 7 (컨텍스트 준수). |
...
| **생성자** | `/ecl-discuss-phase` (`git_commit` 단계). |
| **소비자** | 인간 검토; 회고. |
...
| **생성자** | `/ecl-plan-phase` (via `ecl-phase-researcher` 에이전트). |
| **소비자** | `ecl-planner` (플래닝 입력); `ecl-plan-checker` Dimension 7c (계층 준수), Dimension 8 (Nyquist), Dimension 11 (리서치 해결); `ecl-pattern-mapper` (파일 목록 소스). |
...
| **생성자** | `/ecl-plan-phase` (Step 5.5, `workflow.nyquist_validation`이 활성화되고 RESEARCH.md에 Validation Architecture 섹션이 있는 경우). |
| **소비자** | `ecl-plan-checker` Dimension 8 (Check 8e 게이트 — Nyquist 확인이 진행되기 전에 반드시 존재해야 함); `ecl-verifier`. |

### `<NN>-PATTERNS.md`

| | |
|---|---|
| **목적** | `ecl-pattern-mapper`가 생성한 코드베이스 유사 맵. 이 페이즈에서 생성하거나 수정할 각 파일에 대해, 가장 가까운 기존 유사 파일을 식별하고, 파일의 역할과 데이터 흐름을 분류하며, 구체적인 코드 발췌를 추출합니다. 플래너가 일관된 패턴을 사용하도록 안내합니다. |
| **생성자** | `/ecl-plan-phase` (via `ecl-pattern-mapper` 에이전트, 선택; `workflow.pattern_mapper: false`이면 건너뜀). |
| **소비자** | `ecl-planner` (패턴 안내); `ecl-plan-checker` Dimension 12 (패턴 준수). |
...
| **생성자** | `/ecl-plan-phase` (via `ecl-planner` 에이전트). 플랜당 하나의 파일 — 예: `03-02-PLAN.md`는 Phase 3, Plan 2. |
| **소비자** | `/ecl-execute-phase` (실행기 에이전트가 플랜을 읽고 태스크를 실행); `ecl-plan-checker` (실행 전 품질 검토); `ecl-verifier` (실행 후 검증을 위해 `must_haves`를 읽음). |
...
| **소비자** | `/ecl-progress` (페이즈 상태); `ecl-planner` (후속 플랜이 이전 플랜 출력에 대한 실질적인 의존성이 있는 경우); `milestone-summary`. |
...
| **생성자** | `/ecl-verify-work` (또는 `/ecl-execute-phase` 내의 verify 단계). |
| **소비자** | `plan-phase` 종료된 페이즈 게이트(`status: passed`인 VERIFICATION.md는 페이즈를 `Complete`로 표시하고 `--force` 없이 재플래닝을 차단함); `/ecl-progress`; 인간 검토. |
...
| **생성자** | `/ecl-audit-uat` (대화형 UAT 세션). |
| **소비자** | `/ecl-audit-uat` (이전 UAT 세션 재개). |
