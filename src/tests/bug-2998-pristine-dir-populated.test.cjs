...
/**
 * Bug #2998: ecl-pristine/ snapshot is documented but never populated by
 * the installer. saveLocalPatches declared a pristineDir variable and
 * promised "saves pristine copies (from manifest) to ecl-pristine/ to
 * enable three-way merge during reapply-patches" -- but no code ever
 * wrote to that directory. Effect: the /ecl-reapply-patches Step 5
 ...
 * out the modified-file paths into ecl-pristine/. saveLocalPatches now
...
describe('Bug #2998: populatePristineDir is exported and writes pristine for modified files', () => {
  ...
  test('returns 0 when no files are modified (no-op)', () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-2998-'));
    try {
      const written = INSTALL.populatePristineDir({
        packageSrc: ROOT,
        pristineDir: path.join(tmp, 'ecl-pristine'),
        modified: [],
        runtime: 'claude',
  ...
  test('writes one pristine file per modified path that exists in source', () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-2998-'));
    const pristineDir = path.join(tmp, 'ecl-pristine');
    try {
      // Pick a real installed-side relPath from the package source. The
      // install transforms map source `evolv-coder-lite/<rel>` to installed
      // `evolv-coder-lite/<rel>` for skills-aware runtimes (like claude),
      // so the relPath is the same on both sides.
      const candidate = path.join('evolv-coder-lite', 'workflows', 'reapply-patches.md');
  ...
  test('skips paths not present in source (does not corrupt pristine with stale data)', () => {
    ...
    try {
      const written = INSTALL.populatePristineDir({
        packageSrc: ROOT,
        pristineDir,
        modified: ['evolv-coder-lite/this-path-does-not-exist.md'],
        runtime: 'claude',
        pathPrefix: '$HOME/.claude/',
        isGlobal: true,
      });
      assert.equal(written, 0, 'expected zero pristine files for non-existent source paths');
      const out = path.join(pristineDir, 'evolv-coder-lite/this-path-does-not-exist.md');
  ...
  test('pristine files have stable content (transformations are deterministic)', () => {
    ...
    const tmp1 = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-2998-d1-'));
    const tmp2 = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-2998-d2-'));
    try {
      const candidate = path.join('evolv-coder-lite', 'workflows', 'reapply-patches.md');
      const ctx = {
        packageSrc: ROOT,
      ...
      INSTALL.populatePristineDir(Object.assign({ pristineDir: path.join(tmp1, 'ecl-pristine') }, ctx));
      INSTALL.populatePristineDir(Object.assign({ pristineDir: path.join(tmp2, 'ecl-pristine') }, ctx));
      const a = fs.readFileSync(path.join(tmp1, 'ecl-pristine', candidate));
      const b = fs.readFileSync(path.join(tmp2, 'ecl-pristine', candidate));
...
describe('Bug #2998 (#3004 CR): pristine expansion covers every manifest install root', () => {
  test('paths under agents/ are staged via copyWithPathReplacement, not silently skipped', () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-2998-multi-'));
    const pristineDir = path.join(tmp, 'ecl-pristine');
    try {
      const candidate = path.join('agents', 'ecl-planner.md');
      const sourcePath = path.join(ROOT, candidate);
      assert.equal(fs.existsSync(sourcePath), true,
        `precondition: source file exists at ${candidate}`);
      const written = INSTALL.populatePristineDir({
  ...
  test('a mix of evolv-coder-lite/ and agents/ paths in modified list are all staged', () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-2998-mix-'));
    const pristineDir = path.join(tmp, 'ecl-pristine');
    try {
      const a = path.join('evolv-coder-lite', 'workflows', 'reapply-patches.md');
      const b = path.join('agents', 'ecl-planner.md');
