 ...
 *
 * Owns the lifecycle writer functions for hook surfaces managed by eCL on four
 * runtimes:
...
function isManagedCursorHookEntry(entry: unknown): boolean {
  return Boolean(entry && typeof entry === 'object' && (entry as Record<string, unknown>)[ECL_CURSOR_HOOK_MARKER]);
}
...
//
// MUTATES `settings` by reference — registers all eCL-managed hook entries
...
function applySettingsJsonHooks(settings: any, opts: ApplySettingsJsonHooksOpts): void {
  ...
  if (hooksSurface !== 'none') {
    ...
    const hasGsdUpdateHook = settings.hooks.SessionStart.some((entry: HookGroup) =>
      entry.hooks && entry.hooks.some((h: HookEntry) => referencesHook(h as Record<string, unknown>, 'ecl-check-update'))
    );
    ...
    const hasContextMonitorHook = settings.hooks[postToolEvent].some((entry: HookGroup) =>
      entry.hooks && entry.hooks.some((h: HookEntry) => referencesHook(h as Record<string, unknown>, 'ecl-context-monitor'))
    );
    ...
    if (!hasContextMonitorHook && fs.existsSync(contextMonitorFile) && contextMonitorCommand) {
      ...
      for (const entry of settings.hooks[postToolEvent]) {
        if (entry.hooks && entry.hooks.some((h: HookEntry) => referencesHook(h as Record<string, unknown>, 'ecl-context-monitor'))) {
          let migrated = false;
          if (!entry.matcher) {
          ...
          for (const h of entry.hooks) {
            if (referencesHook(h as Record<string, unknown>, 'ecl-context-monitor') && !h.timeout) {
              h.timeout = 10;
              migrated = true;
            }
          }
    ...
    const hasPromptGuardHook = settings.hooks[preToolEvent].some((entry: HookGroup) =>
      entry.hooks && entry.hooks.some((h: HookEntry) => referencesHook(h as Record<string, unknown>, 'ecl-prompt-guard'))
    );
    ...
    const hasReadGuardHook = settings.hooks[preToolEvent].some((entry: HookGroup) =>
      entry.hooks && entry.hooks.some((h: HookEntry) => referencesHook(h as Record<string, unknown>, 'ecl-read-guard'))
    );
    ...
    const hasReadInjectionScannerHook = settings.hooks[postToolEvent].some((entry: HookGroup) =>
      entry.hooks && entry.hooks.some((h: HookEntry) => referencesHook(h as Record<string, unknown>, 'ecl-read-injection-scanner'))
    );
    ...
    const workflowGuardHookEntry = settings.hooks[preToolEvent].find((entry: HookGroup) =>
      entry.hooks && entry.hooks.some((h: HookEntry) => referencesHook(h as Record<string, unknown>, 'ecl-workflow-guard'))
    );
    ...
    const hasWorktreePathGuardHook = settings.hooks[preToolEvent].some((entry: HookGroup) =>
      entry.hooks && entry.hooks.some((h: HookEntry) => referencesHook(h as Record<string, unknown>, 'ecl-worktree-path-guard'))
    );
    ...
    const hasValidateCommitHook = settings.hooks[preToolEvent].some((entry: HookGroup) =>
      entry.hooks && entry.hooks.some((h: HookEntry) => referencesHook(h as Record<string, unknown>, 'ecl-validate-commit'))
    );
    ...
    const hasGraphifyUpdateHook = settings.hooks[postToolEvent].some((entry: HookGroup) =>
      entry.hooks && entry.hooks.some((h: HookEntry) => referencesHook(h as Record<string, unknown>, 'ecl-graphify-update'))
    );
    ...
    const hasSessionStateHook = settings.hooks.SessionStart.some((entry: HookGroup) =>
      entry.hooks && entry.hooks.some((h: HookEntry) => referencesHook(h as Record<string, unknown>, 'ecl-session-state'))
    );
    ...
    const hasPhaseBoundaryHook = settings.hooks[postToolEvent].some((entry: HookGroup) =>
      entry.hooks && entry.hooks.some((h: HookEntry) => referencesHook(h as Record<string, unknown>, 'ecl-phase-boundary'))
    );
    ...
    if (extendedEvents.includes('FileChanged')) {
      ...
      const alreadyHasConfigReload = settings.hooks.FileChanged.some((entry: HookGroup) =>
        entry.hooks && entry.hooks.some((h: HookEntry) => referencesHook(h as Record<string, unknown>, 'ecl-config-reload'))
      );
...
// hook by name.  Covers all three registration shapes used by eCL:
