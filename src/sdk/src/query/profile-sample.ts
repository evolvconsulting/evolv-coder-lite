/**
 * `profile-sample` — parity with `evolv-coder-lite/bin/lib/profile-pipeline.cjs` `cmdProfileSample`.
 */
...
/**
 * Port of `cmdProfileSample` — same JSON + JSONL file shape as `ecl-tools profile-sample`.
 */
export async function runProfileSample(
  ...
  const tmpDir = mkdtempSync(join(tmpdir(), 'ecl-profile-'));
