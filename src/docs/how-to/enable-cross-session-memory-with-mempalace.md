...
Give eCL durable memory across sessions and projects. When MemPalace is connected, eCL recalls prior decisions and patterns before you plan, and captures phase artifacts verbatim at each phase boundary.

**What you need:** [MemPalace](https://github.com/MemPalace/mempalace) installed locally. eCL never installs MemPalace for you — `pip install mempalace` is your action. A working MemPalace install is not required to complete eCL work; the capability is `onError: skip` at every hook, so an absent or unreachable MemPalace leaves the loop unchanged.
...
The `mempalace.memory_mode` key controls how tightly MemPalace couples to eCL's native memory. **Only `augment` is implemented today.** The other modes are declared for future use — selecting them today has no additional effect beyond `augment`.
...
| `replace` | Intended to make the palace the durable store; eCL memory reads would resolve through it. | Future use — not yet functional today. | **Declared; not yet functional** |
...
**Discuss phase (`/ecl-discuss-phase`):**
...
**Verify phase (`/ecl-verify-work`):**
5. At `verify:post` — `SUMMARY.md` is filed into `milestones` in MemPalace.

**Ship phase (`/ecl-ship`):**
6. At `ship:post` — the `ecl-mempalace-curator` agent writes a diary entry and (if enabled) proposes cross-project tunnels.

**Execute phase (`/ecl-execute-phase`, each wave):**
...
ecl-tools query config-set mempalace.recall_on_discuss false

# Disable the MEMORY-RECALL.md step at plan:pre
ecl-tools query config-set mempalace.recall_on_plan false

# Disable artifact capture at phase boundaries
ecl-tools query config-set mempalace.capture_artifacts false
```
...
# ecl-tools query config-set mempalace.auto_capture_hooks true
```
...
```bash
ecl-tools query config-set mempalace.wing my-project-name
```
