 ...
 *  7. PARITY: VERIFIER_STATUSES covered by routing table; ecl-verifier.md emitted statuses covered
...
} = require('../evolv-coder-lite/bin/lib/verification.cjs');
...
function mkPhaseDir(suffix) {
  return fs.mkdtempSync(path.join(os.tmpdir(), `ecl-651-${suffix}-`));
}
...
describe('verification-status', () => {
  ...
  test('status: gaps_found in "03-foo" dir → next_command includes phase token 03', () => {
    ...
    const baseDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-651-parent-'));
    ...
    try {
      ...
      assert.equal(result.next_command, '/ecl:plan-phase 03 --gaps');
    } finally {
      cleanup(baseDir);
    }
  });
  ...
  test('no *-VERIFICATION.md file → status missing, next_command execute-phase', () => {
    ...
    try {
      ...
      assert.equal(result.next_command, '/ecl:execute-phase');
  ...
  // (b) Parse agents/ecl-verifier.md for emitted statuses via /→ \*\*status:\s*([a-z_]+)\*\*/g,
  ...
  test('PARITY: ecl-verifier.md emitted statuses all have routing table entries', () => {
    const verifierPath = path.join(__dirname, '..', 'agents', 'ecl-verifier.md');
    ...
    assert.ok(
      ...
      'No emitted statuses found in ecl-verifier.md — regex or file path may be wrong. ' +
    ...
    for (const s of emittedStatuses) {
      assert.ok(
        s in VERIFICATION_ROUTING_TABLE,
        `ecl-verifier.md emits status '${s}' but VERIFICATION_ROUTING_TABLE has no entry for it. ` +
          'Add a route or remove/rename the status in ecl-verifier.md.',
      );
    }
  });
  ...
  test('nonexistent phase directory → missing', () => {
    const nonexistent = path.join(os.tmpdir(), 'ecl-651-nonexistent-' + Date.now());
  ...
  test('ship.md still emits the PHASE_VERIFICATION_INCOMPLETE gate sentinel (contract anchor for #651 consolidation)', () => {
    const shipMdPath = path.join(__dirname, '..', 'evolv-coder-lite', 'workflows', 'ship.md');
