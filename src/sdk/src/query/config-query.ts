 ...
 *
 * Ported from evolv-coder-lite/bin/lib/config.cjs and commands.cjs.
 ...
 * // { data: true }
 *
 * const model = await resolveModel(['ecl-planner'], '/project');
 ...
 *
 * Mirrors the CJS table at evolv-coder-lite/bin/lib/config.cjs:505-510 byte-for-
 ...
 * Does NOT merge with defaults (matches ecl-tools.cjs behavior).
 *
...
export const configGet: QueryHandler = async (args, projectDir, workstream) => {
  ...
  // key is absent, mirroring ecl-tools.cjs config-get behavior from #1893.
  ...
  for (const key of keys) {
    if (current === undefined || current === null || typeof current !== 'object') {
      ...
      // See issue #2544 — callers use `if ! ecl-sdk query config-get k; then` patterns.
  ...
  // config.cjs:440-441 — without this, `ecl-sdk query config-get brave_search`
 ...
 *
 * @param args - args[0] is the agent type (e.g., 'ecl-planner')
 * @param projectDir - Project root directory
...
export const resolveModel: QueryHandler = async (args, projectDir, workstream) => {
  ...
  // the tier alias. Mirror the CJS branch at evolv-coder-lite/bin/lib/core.cjs
