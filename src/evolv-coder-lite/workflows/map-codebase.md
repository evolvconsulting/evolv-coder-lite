...
INIT=$(ecl_run query init.map-codebase)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_MAPPER=$(ecl_run query agent-skills ecl-codebase-mapper)
```
...
```bash
ecl_run query commit "docs: map existing codebase" --files .planning/codebase/*.md
```
