...
//   2. Verify every src/*.cts (non-.d.cts) maps to a non-empty evolv-coder-lite/bin/lib/*.cjs.
...
// it works regardless of ECL_TEST_DIR / temp-dir cwd.
function ensureBuiltArtifacts(overrides = {}) {
  ...
  const outDir = overrides.outDir || join(root, 'evolv-coder-lite', 'bin', 'lib');
  ...
  // Handles subdirectories (e.g. src/installer-migrations/*.cts → evolv-coder-lite/bin/lib/installer-migrations/*.cjs).
  ...
  // #996 placed the tsbuildinfo inside evolv-coder-lite/bin/ (a copied/shipped tree), which
  ...
  const legacyTsBuildInfo = join(root, 'evolv-coder-lite', 'bin', 'tsconfig.build.tsbuildinfo');
...
function main() {
  ...
  if (selected.length === 0) {
    ...
    // Allow ECL_ALLOW_EMPTY_SUITE=1 as an escape hatch (downgrades to a warning).
    if (process.env.ECL_ALLOW_EMPTY_SUITE === '1') {
      console.error(`run-tests: WARNING: 0 test files selected for suite "${suite || 'all'}" — discovery or suite filter may be broken (ECL_ALLOW_EMPTY_SUITE=1 suppressed the error)`);
      return 0;
    }
  ...
  // honours ECL_PROJECT/ECL_WORKSTREAM. A developer shell inside a eCL workstream
  // exports ECL_WORKSTREAM, which would redirect fixture STATE.md reads away from
