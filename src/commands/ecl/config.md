---
name: ecl:config
description: Configure eCL settings — workflow toggles, advanced knobs, integrations, and model profile
...
<objective>
Configure eCL settings interactively with a single consolidated command.
...
| --profile &lt;name&gt; | Switch model profile without interactive prompt | ecl-sdk config-set-model-profile |

</routing>

<execution_context>
@~/.claude/evolv-coder-lite/workflows/settings.md
@~/.claude/evolv-coder-lite/workflows/settings-advanced.md
@~/.claude/evolv-coder-lite/workflows/settings-integrations.md
</execution_context>
...
- If it starts with `--profile`: extract the profile name (remainder after `--profile`), then:
  1. **Pre-flight check (#2439):** verify `ecl-sdk` is on PATH via `command -v ecl-sdk`.
     If absent, emit the install hint `Install eCL via 'npm i -g evolv-coder-lite'` and stop —
     do NOT invoke `ecl-sdk` directly (avoids the opaque `command not found: ecl-sdk` failure).
  2. Run: `ecl-sdk query config-set-model-profile <profile-name> --raw` and display the output verbatim.
