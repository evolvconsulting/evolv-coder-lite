...
const {
  ...
  getActiveWorkstream,
  setActiveWorkstream,
} = require('../evolv-coder-lite/bin/lib/planning-workspace.cjs');

const planningWorkspaceDirect = require('../evolv-coder-lite/bin/lib/planning-workspace.cjs');
...
describe('planning-workspace: session adapter precedence', () => {
  ...
  beforeEach(() => {
    savedSession = process.env.ECL_SESSION_KEY;
  });

  afterEach(() => {
    if (savedSession !== undefined) process.env.ECL_SESSION_KEY = savedSession;
    else delete process.env.ECL_SESSION_KEY;
  });

  test('uses session adapter over shared adapter when session key exists', () => {
    process.env.ECL_SESSION_KEY = 'session-123';

    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-planning-precedence-'));
    try {
...
describe('planning-workspace: self-heal behavior', () => {
  ...
  test('clears stale pointers when workstream directory is gone', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-planning-workspace-'));
    try {
...
describe('planning-workspace: lock seam', () => {
  test('exports withPlanningLock and acquires/release lock', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-planning-lock-'));
    try {
  ...
  test('does not retry errors thrown by locked work', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-planning-lock-work-error-'));
    let attempts = 0;
    try {
      assert.throws(() => {
...
describe('planning-workspace direct: functions expose matching behavior', () => {
  ...
  test('planning-workspace functions work consistently', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'evolv-coder-lite-compat-'));
    try {
