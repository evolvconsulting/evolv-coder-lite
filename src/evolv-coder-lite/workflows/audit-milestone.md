...
AGENT_SKILLS_CHECKER=$(ecl_run query agent-skills ecl-integration-checker)
```
...
```bash
integration_checker_model=$(ecl_run query resolve-model ecl-integration-checker --raw)
```
...
PHASE_INFO=$(ecl_run query find-phase 01 --raw)
...
${AGENT_SKILLS_CHECKER}",
  subagent_type="ecl-integration-checker",
...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  [ -e "$summary" ] || continue
  ecl_run query summary-extract "$summary" --fields requirements_completed --pick requirements_completed
done
```
...
```bash
NYQUIST_CONFIG=$(ecl_run query config-get workflow.nyquist_validation --raw --default true 2>/dev/null)
```
...
Discovery only — never auto-calls `/ecl:validate-phase`.
...
|-------|---------------|-----------|--------|
| {phase} | exists/missing | true/false/partial | `/ecl:validate-phase {N}` |

Phases needing validation: run `/ecl:validate-phase {N}` for each flagged phase.
...
/ecl:phase --insert <N> "Close gap: <REQ-ID> — <description>"
/ecl:discuss-phase <N>
/ecl:plan-phase <N>
/ecl:execute-phase <N>

For Nyquist-coverage gaps flagged in the table above, prefer running
`/ecl:validate-phase <N>` for each flagged phase (and `/ecl:secure-phase
...
- /ecl:complete-milestone {version} — proceed anyway (accept tech debt)
...
/ecl:phase --insert <N> "Address tech debt: <area>"
/ecl:discuss-phase <N>
/ecl:plan-phase <N>
/ecl:execute-phase <N>
