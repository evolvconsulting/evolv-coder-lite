<purpose>
Interactive configuration of eCL workflow agents (research, plan_check, verifier) and model profile selection via multi-question prompt. Updates .planning/config.json with user preferences. Optionally saves settings as global defaults (~/.ecl/defaults.json) for future projects.
</purpose>
...
if [[ -z "${ECL_CONFIG_PATH:-}" ]]; then
  if [[ -f .planning/active-workstream ]]; then
    ...
    ECL_CONFIG_PATH=".planning/workstreams/${WS}/config.json"
  else
    ECL_CONFIG_PATH=".planning/config.json"
  fi
fi
```
...
Store `$ECL_CONFIG_PATH` — all subsequent reads and writes use this path, not a hardcoded `.planning/config.json`, so active-workstream installs target the correct file (#2282).
</step>
...
- `workflow.ui_safety_gate` — prompt to run /ecl:ui-phase before planning frontend phases (default: true if absent)
...
- `workflow.code_review` — enable /ecl:code-review and /ecl:code-review --fix commands (default: true if absent)
- `workflow.code_review_depth` — default depth for /ecl:code-review: `quick`, `standard`, or `deep` (default: `"standard"` if absent; only relevant when `code_review` is on)
- `workflow.ui_review` — run visual quality audit (/ecl:ui-review) in autonomous mode (default: true if absent)
- `commit_docs` — whether `.planning/` files are committed to git (default: true if absent)
- `intel.enabled` — enable queryable codebase intelligence (/ecl:map-codebase --query) (default: false if absent)
- `graphify.enabled` — enable project knowledge graph (/ecl:graphify) (default: false if absent)
...
- `model_policy.provider` — provider slug for model policy (default: `null`; known values: anthropic, openai, google, qwen; set via /ecl:config --advanced)
- `model_policy.budget` — budget level for model policy (default: `null`; known values: high, medium, low; set via /ecl:config --advanced)
- `model_policy.high` — model ID for high-cost tier (default: `null`; set via /ecl:config --advanced)
- `model_policy.medium` — model ID for medium-cost tier (default: `null`; set via /ecl:config --advanced)
- `model_policy.low` — model ID for low-cost tier (default: `null`; set via /ecl:config --advanced)
</step>
...
AskUserQuestion([
  ...
  {
    question: "Enable Code Review? (/ecl:code-review and /ecl:code-review --fix commands)",
    header: "Code Review",
    multiSelect: false,
    options: [
      { label: "Yes (Recommended)", description: "Enable /ecl:code-review commands for reviewing source files changed during a phase." },
  ...
  {
    question: "Code Review Depth? (default depth for /ecl:code-review — override per-run with --depth=)",
    header: "Review Depth",
    multiSelect: false,
    options: [
  ...
  {
    question: "Enable UI Review? (visual quality audit via /ecl:ui-review in autonomous mode)",
    header: "UI Review",
    multiSelect: false,
    options: [
  ...
  },
  {
    ...
    multiSelect: false,
    options: [
      { label: "Yes (Recommended)", description: "ecl-pattern-mapper runs between research and plan steps. Surfaces conventions so new code follows house style." },
  ...
  {
    question: "Enable UI Safety Gate? (prompts to run /ecl:ui-phase before planning frontend phases)",
    header: "UI Gate",
    multiSelect: false,
    options: [
      { label: "Yes (Recommended)", description: "plan-phase asks to run /ecl:ui-phase first when frontend indicators detected." },
  ...
  },
  {
    ...
    multiSelect: false,
    options: [
      { label: "Yes (Recommended)", description: "Run /ecl:ai-integration-phase before planning AI system phases. Surfaces the right framework, researches its docs, and designs the evaluation strategy." },
  ...
  },
  {
    ...
    options: [
      ...
      { label: "Per Phase", description: "Create branch for each phase (ecl/phase-{N}-{name})" },
      { label: "Per Milestone", description: "Create branch for entire milestone (ecl/{version}-{name})" }
    ]
  },
  {
  ...
  },
  {
    ...
    options: [
      ...
      { label: "Yes", description: "Skip discuss in /ecl:autonomous — chain directly to plan. Best for backend/pipeline work where phase descriptions are the spec." }
    ]
  },
  {
  ...
  {
    question: "Enable Intel? (queryable codebase intelligence via /ecl:map-codebase --query — builds a JSON index in .planning/intel/)",
    header: "Intel",
    multiSelect: false,
    options: [
      { label: "No (Recommended)", description: "Skip intel indexing. Use when codebase is small or intel queries are not needed." },
      { label: "Yes", description: "Enable /ecl:map-codebase --query commands. Builds and queries a JSON index of the codebase." }
    ]
  },
  {
    question: "Enable Graphify? (project knowledge graph via /ecl:graphify — builds a graph in .planning/graphs/)",
    header: "Graphify",
    multiSelect: false,
    options: [
      { label: "No (Recommended)", description: "Skip knowledge graph. Use when dependency graphs are not needed." },
      { label: "Yes", description: "Enable /ecl:graphify commands. Builds and queries a project knowledge graph." }
    ]
  },
  {
    ...
    multiSelect: false,
    options: [
      { label: "No (Recommended)", description: "Manual /ecl:graphify build only. Conservative default — opt in if you want fresh context on every /ecl:quick or /ecl:plan-phase." },
...
```json
{
  ...
  "model_policy": {
    // Read-only in this flow — written only by /ecl:config --advanced (Section 8).
...
- **Capability hook-gate keys** (owned by a capability in the registry — see `registry.configSchema`): write via the capability writer:
  ```bash
  ecl_run capability set <owner> --gate <key>=<value> [--config-dir "$RUNTIME_CONFIG_DIR"]
  ```
...
AskUserQuestion([
  {
    ...
    multiSelect: false,
    options: [
      { label: "Yes", description: "New projects start with these settings (saved to ~/.ecl/defaults.json)" },
...
If "Yes": write the same config object (minus project-specific fields like `brave_search`) to `~/.ecl/defaults.json`:

```bash
mkdir -p ~/.ecl
```

Write `~/.ecl/defaults.json` with:
```json
{
  "mode": <current>,
  "granularity": <current>,
...
These settings apply to future /ecl:plan-phase and /ecl:execute-phase runs.

Quick commands:
- /ecl:config --integrations — configure API keys (Brave/Firecrawl/Exa), review.models CLI routing, and agent_skills injection
- /ecl:config --profile <profile> — switch model profile
- /ecl:plan-phase --research — force research
- /ecl:plan-phase --skip-research — skip research
- /ecl:plan-phase --skip-verify — skip plan check
- /ecl:config --advanced — power-user tuning (plan bounce, timeouts, branch templates, cross-AI, context window, model policy)
```
</step>
