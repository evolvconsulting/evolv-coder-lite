...
This guide is for capability authors who want to distribute their work so other eCL users can install it with `ecl capability install`. It covers preparing the manifest, validating locally, and releasing through each supported distribution channel.
...
Use [Semantic Versioning](https://semver.org/). Every published capability must carry a version; eCL will reject installation of a manifest that omits it.

### `engines.ecl` (required)
...
Declare the minimum eCL version your capability requires. eCL checks this constraint at both install time and load time and refuses to activate the capability on an incompatible installation. Be as permissive as correctness allows — a tighter range blocks more users.

If you need to offer a graceful downgrade path for users on older eCL versions, you can also declare `compatVersions`:
...
"homepage": "https://github.com/your-org/ecl-cap-example",
"repository": "https://github.com/your-org/ecl-cap-example",
"license": "MIT"
```
...
The prefixes `ecl-`, `evolv-coder-lite-`, and `anthropic-` are reserved for first-party capabilities. Do not use them as the `id` or package name of a third-party capability.
...
If you are developing outside the core repo, use the standalone validator when it is available, or install your capability locally and check that `ecl capability list` shows it without errors:

```bash
ecl capability install ./path/to/your-capability --scope project
ecl capability list
```
...
```bash
ecl capability install https://github.com/your-org/ecl-cap-example.git#v1.0.0
```

For a reproducible pin that cannot be moved, consumers can use a commit SHA instead:

```bash
ecl capability install https://github.com/your-org/ecl-cap-example.git#sha:abc123def456...
```
...
```bash
ecl capability install npm:@your-org/ecl-cap-example@1.0.0
```
...
```bash
tar -czf ecl-cap-example-1.0.0.tgz capabilities/example/
```

Consumers install using the tarball URL:

```bash
ecl capability install https://github.com/your-org/ecl-cap-example/releases/download/v1.0.0/ecl-cap-example-1.0.0.tgz
```
...
```bash
openssl dgst -sha512 -binary ecl-cap-example-1.0.0.tgz | openssl base64 -A | sed 's/^/sha512-/'
```

Publish the resulting string in your release notes. Consumers pass it at install time:

```bash
ecl capability install https://example.com/ecl-cap-example-1.0.0.tgz \
  --integrity sha512-<hash>
```

eCL verifies the hash before extracting the archive and aborts if it does not match.
...
```json
"provenance": {
  "sourceRepo": "https://github.com/your-org/ecl-cap-example",
  "commit": "abc123def456..."
}
```
...
- [Version and update a capability](./version-a-capability.md) — manage `version`, `engines.ecl`, and `compatVersions` across releases.
- [Capability manifest](../reference/capability-manifest.md) — full field reference.
- [Capability trust model](../explanation/capability-trust-model.md) — how eCL treats third-party capabilities at install time.
