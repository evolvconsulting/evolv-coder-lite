...
beforeEach(async () => {
  tmpDir = await mkdtemp(join(tmpdir(), 'ecl-cfg-'));
...
describe('resolveModel', () => {
  it('returns model and profile for known agent', async () => {
    ...
    const result = await resolveModel(['ecl-planner'], tmpDir);
  ...
  it('resolves shipped-but-previously-missing agents without unknown_agent (#3229)', async () => {
    ...
    const result = await resolveModel(['ecl-code-reviewer'], tmpDir);
    const data = result.data as Record<string, unknown>;
  ...
  it('runtime codex model_profile_overrides beat resolve_model_ids omit (#3358)', async () => {
    ...
    const planner = (await resolveModel(['ecl-planner'], tmpDir)).data as Record<string, unknown>;
    const executor = (await resolveModel(['ecl-executor'], tmpDir)).data as Record<string, unknown>;
  ...
  // evolv-coder-lite/bin/lib/core.cjs:1348-1350 has the missing guard.
  it('#3643: runtime:claude + resolve_model_ids:true + balanced returns full sonnet id', async () => {
    ...
    const result = await resolveModel(['ecl-executor'], tmpDir);
  ...
  it('#3643: runtime:claude + resolve_model_ids:true + budget returns full haiku id', async () => {
    ...
    // ecl-verifier maps to 'haiku' under budget profile per model-catalog.json.
    const result = await resolveModel(['ecl-verifier'], tmpDir);
  ...
  it('#3643 regression-guard: model_overrides[agent] beats claude+resolve_model_ids:true', async () => {
    ...
    await writeFile(
      ...
      JSON.stringify({
        ...
        runtime: 'claude',
        resolve_model_ids: true,
        model_overrides: { 'ecl-executor': 'custom-anthropic-id' },
      }),
    );
    const result = await resolveModel(['ecl-executor'], tmpDir);
  ...
  it('resolveModel uses workstream config when --ws is specified', async () => {
    const { resolveModel } = await import('./config-query.js');
    // Root config: balanced profile → ecl-executor resolves to 'sonnet'
    await writeFile(
      join(tmpDir, '.planning', 'config.json'),
      JSON.stringify({ model_profile: 'balanced' }),
    );
    // Workstream config: quality profile → ecl-executor resolves to 'opus'
    ...
    const rootResult = await resolveModel(['ecl-executor'], tmpDir);
    ...
    const wsResult = await resolveModel(['ecl-executor'], tmpDir, 'frontend');
...
describe('MODEL_PROFILES', () => {
  it('contains every shipped ecl agent file on disk (#3229)', async () => {
