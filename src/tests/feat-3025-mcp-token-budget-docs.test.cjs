 ...
 *
 * Verifies that evolv-coder-lite/references/context-budget.md contains the
 ...
 *      project-specific)
 *   4. An explicit note that eCL does NOT manage MCP enablement — this is
...
const CONTEXT_BUDGET_MD = path.join(ROOT, 'evolv-coder-lite', 'references', 'context-budget.md');
...
function parseMcpBudgetSection(section) {
  ...
  // backticks before phrase-matching so e.g. "eCL does **not** manage"
  // is caught by the primary `ecl does not manage` alternative below.
  ...
  // (5) Harness vs eCL distinction — must explicitly state eCL doesn't
  // own this knob and point at the harness
  const explainsHarnessNotGsd =
    /(ecl does(?:n[''’]t| not) (own|manage|control)|harness (concern|setting|controlled)|not a ecl (setting|knob))/i.test(stripped);
...
describe('#3025 context-budget.md: MCP token-budget section exists with required content', () => {
  ...
  test('explains eCL does not own MCP enablement — harness concern (#3025 requirement 4)', () => {
    const section = extractSection(CONTEXT_BUDGET_MD, 'mcp');
    const parsed = parseMcpBudgetSection(section);
    assert.equal(parsed.explainsHarnessNotGsd, true,
      'section must explicitly state eCL does not manage MCP enablement (harness concern)');
  });
