...
/**
 * Extracts text from the four canonical XML tags that ecl-planner emits for
 ...
 *
 * Reference: agents/ecl-planner.md line 66:
...
function extractPlanSections(planContent: string): PlanSections {
  ...
  // <task>, <action>). The planner spec (agents/ecl-planner.md line 66) directs
...
async function loadGateConfig(projectDir: string, workstream?: string): Promise<boolean> {
  try {
    ...
    if (typeof v === 'string') {
      ...
      console.warn(
        `[ecl] workflow.context_coverage_gate is a string "${v}" — expected boolean. Defaulting to ON.`,
      );
      return true;
    }
    if (v !== undefined && v !== null) {
      console.warn(
        `[ecl] workflow.context_coverage_gate has invalid type ${typeof v} (value: ${JSON.stringify(v)}); expected boolean. Defaulting to ON.`,
      );
    }
    ...
    for (const blockMatch of blockMatches) {
      ...
      for (const f of files) {
        ...
        if (!isInsideRoot(f, projectDir)) {
          console.warn(
            `[ecl] decision-coverage: skipping files_modified entry "${f}" — outside project root`,
          );
          continue;
        }
