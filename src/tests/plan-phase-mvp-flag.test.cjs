...
const WORKFLOW = path.join(__dirname, '..', 'evolv-coder-lite', 'workflows', 'plan-phase.md');
...
function parseWorkflowContract(content) {
  ...
  const plannerLines = lines.filter(line => line.includes('planner') || line.includes('ecl-planner'));
...
describe('plan-phase workflow — --mvp flag', () => {
  ...
  test('planner spawn passes MVP_MODE to ecl-planner', () => {
