...
describe('active-workstream-store', () => {
  ...
  test('getActiveWorkstream uses session adapter over shared adapter when session key exists', () => {
    const savedSession = process.env.ECL_SESSION_KEY;
    process.env.ECL_SESSION_KEY = 'session-123';

    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-active-store-precedence-'));
    try {
  ...
  test('getActiveWorkstream self-heals stale pointers', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-active-store-stale-'));
    try {
