...
describe('generateBaseline', () => {
  ...
  let outPath;
  beforeEach(() => {
    dir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-gen-baseline-'));
  ...
  test('predicate filters which files are baselined (agent path)', () => {
    ...
    fs.writeFileSync(path.join(agentDir, 'ecl-one.md'), 'a\n');
    fs.writeFileSync(path.join(agentDir, 'ecl-two.md'), 'bb\n');
    fs.writeFileSync(path.join(agentDir, 'README.md'), 'not an agent\n');

    const result = generateBaseline({
      dir: agentDir,
      outPath,
      predicate: (f) => f.startsWith('ecl-'),
    });
    assert.strictEqual(result.count, 2, 'only ecl-*.md files should be baselined');
    const written = JSON.parse(fs.readFileSync(outPath, 'utf-8'));
    assert.deepStrictEqual(Object.keys(written), ['ecl-one.md', 'ecl-two.md']);
  });
});
