...
describe('GSDOptions.workstream', () => {
  it('eCL class accepts workstream option', async () => {
...
describe('GSDTools workstream injection', () => {
  ...
  beforeEach(async () => {
    tmpDir = join(tmpdir(), `ecl-ws-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
  ...
  it('passes --ws flag to ecl-tools.cjs when workstream is set', async () => {
    const { GSDTools } = await import('./ecl-tools.js');
...
describe('loadConfig with workstream', () => {
  ...
  beforeEach(async () => {
    tmpDir = join(tmpdir(), `ecl-config-ws-${Date.now()}-${Math.random().toString(36).slice(2)}`);
...
describe('ContextEngine with workstream', () => {
  ...
  beforeEach(async () => {
    tmpDir = join(tmpdir(), `ecl-ctx-ws-${Date.now()}-${Math.random().toString(36).slice(2)}`);
    await mkdir(tmpDir, { recursive: true });
  });
