/**
 * runtime-slash.cts — single source of truth for emitting eCL slash-command
...
export function formatGsdSlash(commandName: unknown, runtime: unknown): unknown {
  ...
  if (style === 'shell-var') {
    // shell-var runtimes (currently: codex) use $ecl-<cmd> syntax. The command
...
export function resolveRuntime(projectDir: string | null | undefined): string {
  const envRuntime = resolveRuntimeNameFromCandidates(process.env['ECL_RUNTIME']);
