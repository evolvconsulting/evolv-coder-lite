...
const WORKFLOW = path.join(__dirname, '../evolv-coder-lite/workflows/sync-skills.md');
const COMMAND = path.join(__dirname, '../commands/ecl/sync-skills.md');
  ...
  for (const { runtime, expected } of CASES) {
    test(`resolves correct skills root for ${runtime}`, () => {
      const result = spawnSync(process.execPath, [INSTALL_JS, '--skills-root', runtime], {
        encoding: 'utf-8',
        env: { ...process.env, ECL_TEST_MODE: undefined }, // ensure not in test mode
      });
...
describe('sync-skills.md — required behavioral specs', () => {
  ...
  test('only ecl-* directories are touched (non-eCL preservation)', () => {
    content = content || readWorkflow();
    assert.ok(
      content.includes('ecl-*') && (content.includes('non-eCL') || content.includes('Non-eCL') || content.includes('not starting with')),
      'workflow must document that only ecl-* dirs are modified'
    );
  });
...
// ── commands/ecl/sync-skills.md ───────────────────────────────────────────────
// #2790: sync-skills.md was consolidated into update.md as the --sync flag.

describe('commands/ecl/sync-skills.md', () => {
  test('sync-skills is now --sync flag on update.md (#2790)', () => {
    const updateCmd = path.join(__dirname, '../commands/ecl/update.md');
    assert.ok(fs.existsSync(updateCmd), 'commands/ecl/update.md must exist');
  ...
  test('sync-skills.md command file is deleted (#2790)', () => {
    assert.ok(!fs.existsSync(COMMAND), 'commands/ecl/sync-skills.md should be deleted (consolidated into update.md)');
  });
});
...
describe('INVENTORY sync', () => {
  test('INVENTORY.md lists /ecl-update --sync command (#2790: absorbed /ecl-sync-skills)', () => {
    const inventory = fs.readFileSync(path.join(__dirname, '../docs/INVENTORY.md'), 'utf-8');
    assert.ok(inventory.includes('/ecl-update --sync'), 'INVENTORY.md must list /ecl-update --sync (absorbed /ecl-sync-skills in #2790)');
  });
  ...
  test('INVENTORY-MANIFEST.json includes /ecl-update (#2790: sync-skills absorbed into update.md --sync)', () => {
    // #2790: /ecl-sync-skills was absorbed into /ecl-update as the --sync flag.
    // The manifest now records /ecl-update instead of /ecl-sync-skills.
    const manifest = JSON.parse(
      fs.readFileSync(path.join(__dirname, '../docs/INVENTORY-MANIFEST.json'), 'utf-8')
    );
    assert.ok(
      manifest.families.commands.includes('/ecl-update'),
      'INVENTORY-MANIFEST.json must include /ecl-update in commands (absorbed /ecl-sync-skills via #2790)'
    );
  });
