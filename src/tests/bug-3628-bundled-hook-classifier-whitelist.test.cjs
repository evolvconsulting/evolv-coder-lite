...
/**
 * Bug #3628: `bundled-ecl-hook` classifier (added in #3610) uses a shape
 * regex (`/^hooks\/ecl-[^/]+\.(?:js|sh|cjs|mjs)$/`) that matches ANY file
 * named `hooks/ecl-<name>.{js,sh,cjs,mjs}`, not only the 13 hook files
 ...
 *
 *   - User-authored custom hooks (e.g. `hooks/ecl-personal-experiment.js`)
 *   - Retired bundled hooks from prior eCL versions
 *
 ...
 * exported from the classifier module). Any `hooks/ecl-<name>` file NOT in
...
describe('bug #3628: BUNDLED_GSD_HOOK_FILES is an explicit whitelist', () => {
  ...
  test('every entry is a hooks/-prefixed posix path', () => {
    for (const relPath of BUNDLED_GSD_HOOK_FILES) {
      ...
      assert.ok(
        relPath.includes('ecl-'),
        `entry ${JSON.stringify(relPath)} must contain the "ecl-" prefix`,
      );
    }
  });
  ...
  test('every ecl-*.{js,sh,cjs,mjs} file in hooks/ is in BUNDLED_GSD_HOOK_FILES (no shipping drift)', () => {
    const hooksDir = path.join(__dirname, '..', 'hooks');
    const onDisk = fs
      .readdirSync(hooksDir, { withFileTypes: true })
      .filter((e) => e.isFile() && /^ecl-[^/]+\.(?:js|sh|cjs|mjs)$/.test(e.name))
      .map((e) => `hooks/${e.name}`);
...
describe('bug #3628: classifyPromptUserAction whitelists shipped bundled hooks', () => {
  test('classifies every entry in BUNDLED_GSD_HOOK_FILES as bundled-ecl-hook → remove', () => {
    for (const relPath of BUNDLED_GSD_HOOK_FILES) {
      const result = classifyPromptUserAction({ relPath });
      assert.deepStrictEqual(
        result,
        { category: 'bundled-ecl-hook', choice: 'remove' },
        `${relPath} should classify as bundled-ecl-hook`,
      );
    }
  });

  const USER_OWNED_OR_RETIRED = [
    'hooks/ecl-personal-experiment.js',
    'hooks/ecl-my-custom-guard.sh',
    'hooks/ecl-team-policy.cjs',
    'hooks/ecl-retired-hook.js',
    'hooks/ecl-old-statusline.js',
    'hooks/ecl-experimental.mjs',
  ];
  ...
  test('still does NOT classify nested ecl-* directories (existing #3610 boundary preserved)', () => {
    assert.strictEqual(
      classifyPromptUserAction({ relPath: 'hooks/ecl-helpers/index.js' }),
      null,
    );
  });

  test('still does NOT classify non-ecl hooks (existing boundary preserved)', () => {
    assert.strictEqual(
