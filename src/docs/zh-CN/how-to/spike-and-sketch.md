...
**前提条件：** 无。`/ecl-spike` 和 `/ecl-sketch` 会自行创建所需的存储目录，不要求已初始化 eCL 项目。
...
| 你想回答的问题… | 使用 |
|---|---|
| "这个技术方案真的可行吗？" | `/ecl-spike` |
| "这个布局 / 交互 / 视觉处理感觉对吗？" | `/ecl-sketch` |
...
eCL 会询问技术问题，将其分解为 2–5 个独立实验，以 **Given / When / Then** 假设形式呈现，并在开始构建前请求确认。
...
此命令会写入 `.claude/skills/spike-findings-[project]/`。该技能会被自动发现，并在后续的 `/ecl-sketch`、`/ecl-ui-phase` 和 `/ecl-plan-phase` 运行时加载——无需显式引用。
...
eCL 会开启一段简短对话，在编写任何代码之前探索感觉、视觉参考和核心用户操作。它每次只问一个问题，只有在你说"开始"后才动手构建。
...
此命令会写入 `.claude/skills/sketch-findings-[project]/`。该技能由 `/ecl-ui-phase` 自动获取——经过预验证的决策（布局、色彩方案、排版、间距）被视为已锁定，不会再次询问。
...
spike 调查结果会为 sketch 提供参考（真实数据结构、真实交互状态、实际约束）。两次 wrap-up 均会持久化决策，规划器和 UI 研究员会自动加载，因此在 `/ecl-discuss-phase` 或 `/ecl-ui-phase` 期间无需重新解释选择。

---

## spike 或 sketch 如何流入某个阶段

Spike 和 sketch 的产物不需要手动引用。eCL 会在以下两个时间点自动读取它们：

1. **`/ecl-sketch`** — 在构建原型前加载 `.claude/skills/spike-findings-*/`，使变体反映已验证的约束（流式状态、真实字段名等）
2. **`/ecl-ui-phase N`** — 在生成 UI 设计契约前加载 `.claude/skills/sketch-findings-*/`；经过预验证的设计决策被视为已锁定
