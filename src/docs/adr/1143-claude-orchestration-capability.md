...
- **Status:** Proposed
- **Date:** 2026-06-12
- **Issue:** [#1143](https://github.com/evolvconsulting/evolv-coder-lite/issues/1143)
...
- **Blocked by:** [#857](https://github.com/evolvconsulting/evolv-coder-lite/issues/857) being **released** (Proposed → Accepted + capability infrastructure shipped). Not actionable until then.
- **Relates to:** [#853](https://github.com/evolvconsulting/evolv-coder-lite/issues/853) (Claude Code backgrounded agents cannot nest subagents), existing BETA skill `ecl-ultraplan-phase`

## Context

Claude Code ships two orchestration primitives eCL does not yet treat as first-class:

1. **`ultraplan`** (`/ultraplan <objective>`, `claude --teleport`) — hands a *planning* task to a Claude Code web session running in plan mode; the plan is drafted in the cloud, reviewed/commented in the browser, then approved back to the terminal (which archives the web session). eCL **already** wraps this as the BETA skill `ecl-ultraplan-phase` (`evolv-coder-lite/workflows/ultraplan-phase.md`), which constructs a plan prompt, triggers `/ultraplan`, and relies on the user manually running `/ecl:import --from <file>` to bring the plan back.

2. **`ultracode`** (`/effort ultracode`, or an `ultracode:` prompt prefix) — sets `xhigh` reasoning **plus automatic Workflow orchestration**. It is the trigger for the **Workflow tool** (Agent SDK ≥ v0.3.149): a deterministic JavaScript orchestrator that fans subagents out with `agent()`, `parallel()` (barrier), `pipeline()` (no barrier), and `phase()`, supporting `isolation: 'worktree'`, a shared token `budget`, custom `agentType`, and `resumeFromRunId`. It can be activated per-session via `/effort`, `--settings`, or an Agent SDK control request (`"ultracode": true`). **eCL does not use the Workflow tool at all.**

### Why this matters for the loop

eCL's `execute-phase` is wave-based: plans carry a `wave` number, waves run sequentially, and plans *within* a wave run in parallel when their `files_modified` sets do not overlap. Today eCL realizes that by dispatching, one `Agent` call per message, a backgrounded `ecl-executor` in a worktree.

On **Claude Code** this degrades. Backgrounded agents on Claude Code have no `Agent`/`Task` tool, so they cannot nest subagents (#853). The autonomous loop therefore falls back to **inline sequential execution** — and with it silently drops wave parallelism, the plan-checker, and the verifier — on the single runtime most eCL users run. The degradation is structural and outside eCL's control to fix at the agent layer.

The Workflow tool sidesteps #853 precisely because it is invoked **from the main loop**, not from a backgrounded agent: the script *is* the orchestrator and spawns the subagents itself. Its primitives map almost 1:1 onto eCL's existing wave model, and it adds three things eCL's hand-rolled fan-out lacks: determinism, a shared token budget, and resume.

Post ADR-857, eCL now has a home for exactly this kind of runtime-specific, preview-grade, toggleable feature: a **Capability** with `runtimeCompat`, a federated config slice, a `tier`, and loop-hook registration at the stable extension points. Before 857 there was nowhere clean to put runtime-gated loop behavior without forking core prose; after 857 it is a data manifest plus a registry regen.

## Decision

Introduce a **Claude orchestration Capability** — `capabilities/claude-orchestration/`, `role: feature`, `runtimeCompat: claude`, **default-off, BETA** — that adopts Claude Code's orchestration primitives as optional, runtime-gated eCL surfaces. It is **blocked on #857 being released** and ships only after the capability infrastructure is Accepted.

### 1. Workflow tool as an optional loop execution backend (primary, NEW)

When the active runtime exposes the Workflow tool, `execute-phase` may emit a generated **Workflow script** in place of manual one-agent-per-message dispatch. The translation preserves eCL's existing model:

| eCL execute-phase concept | Workflow primitive |
|---|---|
| Wave (barrier between waves) | `parallel()` barrier; or `pipeline()` when plans flow through stages without a cross-plan barrier |
| Plan executor | `agent(brief, { agentType: 'ecl-executor', isolation: 'worktree' })` |
...
| Resume / `ecl-undo` | `resumeFromRunId` keyed to the phase manifest |
...
Net effect on Claude Code: **wave parallelism, the plan-checker, and the verifier are restored** — the capability path is *not* a backgrounded agent, so #853 does not apply — and eCL gains determinism, a shared budget, and resume it does not have today.

### 2. Registration at ADR-857 loop extension points

The capability registers hooks at `execute:wave:pre`, `execute:wave:post`, and `execute:post`, resolved through `ecl_run loop render-hooks <point>`. Activation is gated by:
...
The existing `ecl-ultraplan-phase` BETA skill and `commands/ecl/ultraplan-phase.md` become capability-owned at `plan:*`. Plan-offload (`ultraplan`) and execute-orchestration (`ultracode`/Workflow) then share one runtime gate, one federated config slice, and one BETA boundary — instead of `ultraplan` living as a stray BETA skill while a second preview surface is added elsewhere. This also lets a future iteration close `ultraplan`'s manual `/ecl:import --from` round-trip behind the same capability without touching core.
...
The capability tracks two moving Claude-Code preview surfaces. Containment: one capability + BETA gate (mirroring today's `ecl-ultraplan-phase` BETA isolation), a pinned minimum Agent-SDK/Workflow-tool version in the runtime gate, and inline fallback on any detection miss. A preview-API change can degrade the capability to inline; it cannot break the core loop.
...
- **Positive:** restores loop parallelism + plan-checker + verifier on Claude Code (undoing the #853 inline degradation); adds determinism, shared token budget, and resume to eCL execution; consolidates two Claude preview surfaces behind one gated capability; sets a reusable pattern for the other fan-out points.
