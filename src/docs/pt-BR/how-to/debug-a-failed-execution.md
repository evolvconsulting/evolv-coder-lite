...
**Pré-requisitos:** Você executou `/ecl-execute-phase N` e a execução parou antes de gravar `VERIFICATION.md`, ou você vê saída inesperada, arquivos ausentes ou um indicador de progresso travado.
...
Isso é normal, não é um travamento. Os subagentes eCL são executados em uma janela de contexto isolada. A nota de atividade na linha de spawn confirma isso. Não interrompa a sessão.
...
O eCL verifica a existência de arquivos `SUMMARY.md` antes de despachar os executores. Planos que já possuem um são ignorados automaticamente.
...
O eCL detecta isso na próxima execução e apresenta uma porta de retomada segura com três opções:
...
```bash
/ecl-debug --diagnose "Phase 2 executor produced stubs instead of real code"
```
...
```bash
/ecl-debug "Login middleware not handling 401 correctly after phase 3"
```

O eCL coleta sintomas, executa uma investigação estruturada usando o método científico e propõe uma correção. Se `tdd_mode: true` estiver definido na sua configuração, ele exige um teste com falha antes de aplicar qualquer correção.
...
```bash
/ecl-debug continue <slug>
```

---

## Executar uma análise post-mortem com `/ecl-forensics`
...
O eCL analisa o histórico do git, a completude dos artefatos em `.planning/`, a consistência de STATE.md, o trabalho não commitado e as worktrees órfãs. Ele grava um relatório estruturado em `.planning/forensics/report-<timestamp>.md` e apresenta as etapas de remediação recomendadas.

`/ecl-forensics` é somente leitura — ele nunca modifica os arquivos do seu projeto.
...
O eCL ignora planos cujo `SUMMARY.md` já existe e despacha executores apenas para os planos restantes.

Se precisar executar novamente apenas uma onda específica:

```bash
/ecl-execute-phase 1 --wave 2
```

Se quiser validar a integridade de `.planning/` antes de despachar:

```bash
/ecl-execute-phase 1 --validate
```

---

## Reverter com `/ecl-undo`
...
Reverte todos os commits do plano `02` da fase `3`. O eCL exibe uma porta de confirmação antes de gravar qualquer alteração.
...
Reverte todos os commits da fase `3`. O eCL verifica se alguma fase subsequente depende desta fase e avisa você antes de prosseguir.
...
Mostra os cinco commits eCL mais recentes e permite que você selecione quais reverter.
