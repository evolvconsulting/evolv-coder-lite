 ...
 *  Must strip ECL_TEST_MODE so the child runs the real install, not the no-op guard. */
...
function runMinimalInstall({ runtime, scope, extraArgs = [] }) {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `ecl-${runtime}-${scope}-`));
  try {
...
function manifestSkillSet(manifest) {
  ...
  for (const key of Object.keys(manifest.files)) {
    if (key.startsWith('skills/')) {
      const seg = key.split('/')[1].replace(/^ecl-/, '').replace(/\.md$/, '');
      out.add(seg);
    } else if (key.startsWith('command/')) {
      const file = key.split('/')[1];
      out.add(file.replace(/^ecl-/, '').replace(/\.md$/, ''));
    } else if (key.startsWith('commands/ecl/')) {
...
function collectSkillBasenamesOnDisk(configDir) {
  ...
  if (fs.existsSync(skillsDir)) {
    for (const entry of fs.readdirSync(skillsDir, { withFileTypes: true })) {
      if (entry.isDirectory() && entry.name.startsWith('ecl-')) {
        out.add(entry.name.replace(/^ecl-/, ''));
      } else if (entry.isFile() && entry.name.startsWith('ecl-') && entry.name.endsWith('.md')) {
        out.add(entry.name.replace(/^ecl-/, '').replace(/\.md$/, ''));
      }
    }
  }
  ...
  const commandsGsdDir = path.join(configDir, 'commands', 'ecl');
