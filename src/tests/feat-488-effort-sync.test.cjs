// Tests for ecl-tools effort sync command (#488)
...
const ECL_TOOLS = path.resolve(__dirname, '../evolv-coder-lite/bin/ecl-tools.cjs');

function runCli(args, env = {}) {
  const result = spawnSync(process.execPath, [ECL_TOOLS, ...args], {
    encoding: 'utf8',
    env: { ...process.env, ECL_TEST_MODE: '1', ...env },
  });
  return result;
}
...
name: ecl-planner
description: Plans phases for eCL milestones
effort: medium
---
Body of the agent.
`;
...
name: ecl-executor
description: Executes eCL phase plans
---
Body of the agent.
`;
...
describe('feat-488: effort sync command', () => {
  test('dry-run mode reports pending changes without writing files', () => {
    ...
    const agentPath = path.join(agentsDir, 'ecl-planner.md');
    fs.writeFileSync(agentPath, AGENT_WITH_EFFORT);
    writePlanningConfig(tmpDir, { default: 'high', agent_overrides: { 'ecl-planner': 'xhigh' } });

    const { cmdEffortSync } = require('../evolv-coder-lite/bin/lib/commands.cjs');
    ...
    assert.equal(result.changes[0].agent, 'ecl-planner');
  ...
  test('--apply mode rewrites effort: frontmatter to new config value', () => {
    ...
    const agentPath = path.join(agentsDir, 'ecl-planner.md');
    fs.writeFileSync(agentPath, AGENT_WITH_EFFORT);
    writePlanningConfig(tmpDir, { default: 'low', agent_overrides: { 'ecl-planner': 'xhigh' } });
  ...
  test('skips agents where effort: already matches config', () => {
    ...
    writePlanningConfig(tmpDir, { agent_overrides: { 'ecl-planner': 'xhigh' } });
  ...
  test('injects effort: into agent files that lack the frontmatter key', () => {
    ...
    const agentsDir = makeAgentsDir(tmpDir);
    const agentPath = path.join(agentsDir, 'ecl-executor.md');
  ...
  test('home-default effort config gap: applies home-level effort when project config has no effort section', () => {
    // The key #488 scenario: user changed ~/.ecl/defaults.json effort settings
    ...
    // ECL_HOME (~/.ecl/defaults.json). Redirect ECL_HOME to our fake home.
    ...
    // Since ECL_HOME isn't redirected here, the result depends on the real home.
  ...
  test('CLI dispatcher: effort sync --apply routes through ecl-tools correctly', () => {
    ...
    // ecl-tools may print a startup banner before the JSON payload — parse from the first `{`.
