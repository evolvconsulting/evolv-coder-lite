...
const {
  ...
  readGsdCommandNames,
} = require('../evolv-coder-lite/bin/lib/command-roster.cjs');
const {
...
const { resolveInstallPlan } = require('../evolv-coder-lite/bin/lib/runtime-config-adapter-registry.cjs');
const runtimeArtifactConversion = require('../evolv-coder-lite/bin/lib/runtime-artifact-conversion.cjs');
...
const hooksSurface = require('../evolv-coder-lite/bin/lib/runtime-hooks-surface.cjs');
...
// applySettingsJsonHooks: mutates settings.hooks.* in place with all eCL-managed
...
// output file so they deny only Edit,MultiEdit. ecl-nyquist-auditor is intentionally
// excluded (it legitimately uses Write AND Edit to create/patch test files).
const READONLY_AGENT_DISALLOWED_TOOLS = {
  'ecl-plan-checker': 'Write, Edit, MultiEdit',
  'ecl-integration-checker': 'Write, Edit, MultiEdit',
  'ecl-ui-checker': 'Write, Edit, MultiEdit',
  'ecl-verifier': 'Edit, MultiEdit',
  'ecl-doc-verifier': 'Edit, MultiEdit',
  'ecl-eval-auditor': 'Edit, MultiEdit',
  'ecl-ui-auditor': 'Edit, MultiEdit',
};
...
// Tool name mapping from Claude/eCL agents to Kimi CLI module paths.
...
/**
 * Convert a Claude/eCL tool name to a Kimi CLI module path.
...
function normalizeKimiSkillName(skillName) {
  ...
  if (text.startsWith('$')) text = text.slice(1);
  text = text.replace(/^ecl:/, 'ecl-');
  if (!text.startsWith('ecl-')) text = `ecl-${text}`;
  text = text.replace(/[^a-z0-9-]+/g, '-').replace(/-+/g, '-').replace(/^-|-$/g, '');
  return text || 'ecl-command';
}
...
function convertGsdCommandReferencesToKimiSkillInvocations(content, cmdNames) {
  ...
  const colonPattern = new RegExp(`(?<![A-Za-z0-9_/:.-])/?ecl:(${commandGroup})(?=[^A-Za-z0-9_-]|$)`, 'g');
  const hyphenPattern = new RegExp(`(?:/|\\$)ecl-(${commandGroup})(?=[^A-Za-z0-9_-]|$)`, 'g');

  return content
    .replace(colonPattern, (_, cmd) => `/skill:ecl-${cmd}`)
    .replace(hyphenPattern, (_, cmd) => `/skill:ecl-${cmd}`);
}
...
function convertClaudeCommandToKimiSkill(content, skillName, _runtime = null, cmdNames = null) {
  ...
  const description = frontmatter
    ? extractFrontmatterField(frontmatter, 'description') || `Run eCL workflow ${kimiSkillName}.`
    : `Run eCL workflow ${kimiSkillName}.`;
...
const KIMI_CANONICAL_GSD_AGENT_RE = /^ecl-[a-z0-9-]+$/;
...
function neutralizeKimiAgentPrompt(content) {
  ...
  prompt = prompt.replace(/~\/\.claude\/evolv-coder-lite\b/g, 'eCL core');
  prompt = prompt.replace(/\$HOME\/\.claude\/evolv-coder-lite\b/g, 'eCL core');
...
function buildKimiRootAgentYaml({ description, tools, subagents }) {
  const lines = [
    ...
    'agent:',
    '  name: ecl',
    `  description: ${yamlQuote(toSingleLine(description || 'Run eCL workflows in Kimi CLI.'))}`,
    '  extend: default',
    '  system_prompt_path: ./ecl.md',
  ];
...
function buildKimiAgentArtifacts({
  ...
  const rootDescription = rootFrontmatter
    ? extractFrontmatterField(rootFrontmatter, 'description') || 'Run eCL workflows in Kimi CLI.'
    : 'Run eCL workflows in Kimi CLI.';
  ...
  for (const source of subagentSources) {
    ...
    if (!name || !KIMI_CANONICAL_GSD_AGENT_RE.test(name)) {
      addKimiAgentDiagnostic(
        ...
        'Subagent source does not use a canonical ecl-* Kimi agent name.',
        name || '(missing)',
        parsed.path
      );
      continue;
  ...
  const rootTools = mapKimiAgentContractTools(parseFrontmatterTools(rootFrontmatter), diagnostics, 'ecl');
  ...
  return {
    root: {
      name: 'ecl',
      yamlPath: 'agents/ecl.yaml',
      promptPath: 'agents/ecl.md',
...
const CODEX_GSD_TOOLS_INVOCATION = 'node "$HOME/.codex/evolv-coder-lite/bin/ecl-tools.cjs"';

function rewriteBareGsdToolsCommandsForCodex(content) {
  return content
    .replace(/(^[ \t]*)ecl-tools(?=\s)/gm, `$1${CODEX_GSD_TOOLS_INVOCATION}`)
    .replace(/(\$\(\s*)ecl-tools(?=\s)/g, `$1${CODEX_GSD_TOOLS_INVOCATION}`)
    .replace(/(`\s*)ecl-tools(?=\s)/g, `$1${CODEX_GSD_TOOLS_INVOCATION}`)
    .replace(/((?:&&|\|\||[;|])\s*)ecl-tools(?=\s)/g, `$1${CODEX_GSD_TOOLS_INVOCATION}`);
}
...
function convertClaudeToCodexMarkdown(content) {
  ...
  // bare `ecl-tools` binary on PATH. Keep resolver probes such as
  // `command -v ecl-tools` intact; rewrite only command invocations.
...
- **agent_type-capable schema** (e.g. \`multi_agent_v2\`): \`spawn_agent\` accepts \`agent_type\`, \`message\`, \`reasoning_effort\`, \`fork_context\`, etc. — typed eCL agent dispatch is available.
- **Generic schema** (\`multi_agent_v1\`): \`spawn_agent\` accepts only \`message\`, \`items\`, \`fork_context\` — there is **no \`agent_type\` field**. Typed eCL agent dispatch is unavailable in this session.
...
When only the generic \`multi_agent_v1\` schema is available, typed eCL agent dispatch
(\`ecl-planner\`, \`ecl-executor\`, etc.) is NOT possible. This is a known Codex limitation
(openai/codex#15250). **This workaround is NOT equivalent to typed ecl-planner/ecl-executor
execution** — eCL agents carry project-aware prompts, audit logging, and workflow context
...
function generateCodexAgentToml(agentName, agentContent, modelOverrides = null, runtimeResolver = null, effortCfg = null, sandboxTier = 'codex-agent-sandbox') {
  ...
  // follows eCL. Keep those knobs coupled unless eCL also pins the model.
  if (hasPinnedModel) {
...
/**
 * Remove stale agents/openai.yaml sidecar files from eCL-managed Codex skill dirs.
 *
 * Prior to #1326, eCL's Codex install path wrote an agents/openai.yaml file
 * alongside each ecl-* SKILL.md. Recent Codex builds index BOTH SKILL.md and
 * the sidecar, causing each eCL skill to appear twice in autocomplete. This
 ...
 *   - Only touches directories whose names start with "ecl-".
 *   - Skips user-owned dirs (ecl-dev-preferences) — their agents/ content is
 *     never modified, mirroring the same USER_OWNED_SKILL_DIRS guard used by
 *     installOpencodeFamilySkills.
 *   - For each managed ecl-* dir, if agents/openai.yaml exists, deletes it.
 *   - If agents/ is now empty, removes the directory; if it still contains
 *     other files (e.g. user-added content), leaves it in place.
 *   - Non-ecl-* dirs and their agents/ content are never touched.
...
function cleanupCodexSkillMetadataSidecars(skillsDir) {
  ...
  // never be modified by eCL's install path.
...
function _removeGsdEntries(destDir, kind) {
  ...
  if (kind.kind === 'kimi-agents') {
    for (const fileName of ['ecl.yaml', 'ecl.md']) {
    ...
    if (fs.existsSync(subagentsDir)) {
      for (const entry of fs.readdirSync(subagentsDir, { withFileTypes: true })) {
        if (!entry.isFile()) continue;
        if (!entry.name.startsWith('ecl-')) continue;
...
function _runLegacyInstallMigrations(runtime, configDir, scope = 'global') {
  ...
  if (runtime === 'hermes') {
    ...
    // Hermes: bare-stem skills/ecl/<stem>/ cleanup is deferred to AFTER the
    // layout-driven install loop in installRuntimeArtifacts, where the exact set
    // of staged ecl-<stem>/ dirs is known. Removing here (before staging) would
...
function _runLegacyUninstallCleanup(runtime, configDir, scope = 'global') {
  ...
  if (runtime === 'hermes') {
    ...
    // Hermes: pre-#947 bare-stem skills/ecl/<stem>/ entries (dirs that do NOT
    // start with 'ecl-') — the #3664 layout used prefix='' so eCL-owned skills
    // had bare names (e.g. skills/ecl/help/). These are stale on uninstall.
    const nestedGsdDirForUninstall = path.join(configDir, 'skills', 'ecl');
...
/**
 * After the layout-driven install loop writes new ecl-<stem>/ dirs to
 * skills/ecl/, remove any pre-existing bare-stem dirs (skills/ecl/<stem>/)
 * that correspond to the newly installed ecl-<stem> entries.
 *
 * The removal set is derived from the ACTUAL installed skill dirs (every
 * entry starting with 'ecl-' that is a directory), so it covers ALL shipped
 * eCL skills — including 'dev-preferences' and future additions — without
 ...
 *
 * Safety: a bare dir is ONLY removed when a corresponding ecl-<stem>/ dir was
 * installed this run. A user-owned dir 'skills/ecl/my-workflow/' that has no
 * matching 'skills/ecl/ecl-my-workflow/' is never touched.
 *
 * @param {string} nestedGsdDir  absolute path to skills/ecl/ category dir
 */
function _removeHermesBareStemDirs(nestedGsdDir) {
  ...
  // Collect the set of stems that were installed as ecl-<stem>/ this run.
  const installedStems = new Set();
  for (const entry of entries) {
    if (entry.isDirectory() && entry.name.startsWith('ecl-')) {
      installedStems.add(entry.name.slice('ecl-'.length)); // e.g. 'quick', 'dev-preferences'
    }
  }

  // Remove any bare <stem>/ dir for which ecl-<stem>/ was just installed.
  for (const entry of entries) {
    if (entry.isDirectory() && !entry.name.startsWith('ecl-') && installedStems.has(entry.name)) {
...
function installRuntimeArtifacts(runtime, configDir, scope, resolvedProfile) {
  ...
  for (const kind of layout.kinds) {
    ...
    // os.tmpdir() (ecl-cmd-rewrites-*); remove it once copied so it does not accumulate (#856).
    ...
    try {
      ...
      if (kind.kind === 'skills' && fs.existsSync(dest)) {
        ...
        // install (#2973 / #3664).
        //
        // All runtimes (incl. Hermes after #947) use prefix='ecl-'.
        // _removeGsdEntries removes only ecl-* entries; non-ecl-* user dirs are
        // untouched. Preserve the explicit user-owned eCL-prefixed skill
        // ecl-dev-preferences, which eCL does not reinstall from source but must
        // survive the prune (#2973).
        const toPreserve = new Map(); // dirName -> Map<relPath, Buffer>

        {
          // Preserve explicitly user-owned eCL-prefixed skill dirs.
  ...
  // Hermes: after the install loop has written all ecl-<stem>/ dirs to
  // skills/ecl/, remove any stale bare-stem dirs (skills/ecl/<stem>/) that
  // correspond to the newly installed ecl-<stem> entries. This is the robust
  ...
  // every ecl-<stem>/ present now was written this run (or was there before
  // with the same prefix). User-owned bare dirs with no ecl-<stem> counterpart
  // are untouched.
  if (runtime === 'hermes') {
    const nestedGsdDirForCleanup = path.join(configDir, 'skills', 'ecl');
...
function uninstallRuntimeArtifacts(runtime, configDir, scope) {
  ...
  // Hermes: after removing ecl-* skill dirs from skills/ecl/, also remove
  // the eCL-managed DESCRIPTION.md and then the category dir itself if it
  // contains no user content (#947). _removeGsdEntries removed ecl-* dirs
  ...
  if (runtime === 'hermes') {
    ...
    if (fs.existsSync(nestedGsdDir)) {
      // Remove eCL-owned DESCRIPTION.md (written by writeHermesCategoryDescription)
...
function uninstall(isGlobal, runtime = 'claude') {
  ...
  // eCL-managed files only: enumerate the exact set the installer writes.
  // Any file NOT in this set is user-owned and must survive uninstall.
  // After removing eCL files, attempt to rmdir — if the directory is still
  // non-empty (user has custom helpers) it stays; otherwise it goes cleanly.
  const ECL_CHANGESET_FILES = [
  ...
  const ECL_SCRIPTS_LIB_FILES = ['cli-exit.cjs', 'allowlist-ratchet.cjs'];
  ...
  if (fs.existsSync(changesetUninstallDir)) {
    ...
    for (const file of ECL_CHANGESET_FILES) {
    ...
    if (removedChangeset > 0) {
      removedCount++;
      console.log(`  ${green}✓${reset} Removed scripts/changeset/ eCL files`);
    }
  }
  ...
  if (fs.existsSync(scriptsLibUninstallDir)) {
    ...
    for (const file of ECL_SCRIPTS_LIB_FILES) {
    ...
    if (removedScriptsLib > 0) {
      removedCount++;
      console.log(`  ${green}✓${reset} Removed scripts/lib/ eCL files`);
    }
  }
...
function writeManifest(configDir, runtime = 'claude', options = {}) {
  ...
  if ((isCodex || isCopilot || isAntigravity || isCursor || isWindsurf || isTrae || (!isOpencode && !isGemini)) && fs.existsSync(codexSkillsDir)) {
    // All runtimes (including Hermes post-#947) use the canonical 'ecl-' prefix.
    const skillListPrefix = 'ecl-';
  ...
  if (isKimi && fs.existsSync(agentsDir)) {
    ...
    for (const [rel, hash] of Object.entries(agentHashes)) {
      const isRootAgent = rel === 'ecl.yaml' || rel === 'ecl.md';
      const isSubagent = /^subagents\/ecl-[^/]+\.(yaml|md)$/.test(rel);
  ...
  if (!isCodex && !isCopilot && !isCline && !isKimi) {
    ...
    if (fs.existsSync(hooksDir)) {
      ...
      // `file.startsWith('ecl-') && (file.endsWith('.js') || file.endsWith('.sh'))`
...
function install(isGlobal, runtime = 'claude', options = {}) {
  ...
  // but NOT auto-removed here; legacy .agent/ ecl artifacts are recognized but not
  ...
  if (_isSkillsRuntime) {
    ...
    // ecl-* skill dirs. Prior installs wrote these files so Codex would show a
    // display name and description in the /skills TUI popup. Recent Codex builds
    // index BOTH SKILL.md and the sidecar, causing each eCL skill to appear twice
    ...
    if (isHermes) {
      ...
      if (fs.existsSync(hermesSkillsDir)) {
        // Hermes layout uses prefix: 'ecl-' (#947) — skill dirs have ecl-<stem> names
      ...
      const rootAgentPath = path.join(targetDir, 'agents', 'ecl.yaml');
  ...
  // so that `node "$ECL_DIR/scripts/changeset/cli.cjs"` resolves at runtime.
  //
  // The changeset CLI (scripts/changeset/cli.cjs) is invoked by the update
  // workflow (evolv-coder-lite/workflows/update.md) to extract changelog ranges for
  // the /ecl-update preview step. It was previously only present in the npm
  ...
  //   - ../../evolv-coder-lite/bin/lib/semver-compare.cjs  (already installed under evolv-coder-lite/)
  //   - ../../evolv-coder-lite/bin/lib/package-identity.cjs (already installed under evolv-coder-lite/)
  //
  // All runtimes that use the update workflow need this, so we copy unconditionally
  // (same scope as evolv-coder-lite/ itself — every runtime that installs workflows gets it).
  ...
  // Copy scripts/fix-slash-commands.cjs — required by evolv-coder-lite/bin/lib/command-roster.cjs
  // at load time via require('../../../scripts/fix-slash-commands.cjs'). Without this file
  // every ecl-tools command crashes with MODULE_NOT_FOUND (#1223).
  ...
  // Register all eCL-managed hook entries into settings.hooks.* for runtimes
...
function finishInstall(settingsPath, settings, statuslineCommand, shouldInstallStatusline, runtime = 'claude', isGlobal = true, configDir = null, bannerOpts = {}) {
  ...
  if (shouldInstallBanner && settings && plan.writesSharedSettings && !isOpencode) {
    if (!bannerCommand) {
      ...
      const alreadyRegistered = settings.hooks.SessionStart.some(entry =>
        entry && entry.hooks && entry.hooks.some(h => h && referencesHook(h, 'ecl-update-banner'))
      );
  ...
  if (plan.finishPermissionWriter === 'opencode' && !process.env.ECL_TEST_MODE) {
  ...
  if (runtime === 'kimi') command = '/skill:ecl-new-project';
...
`);
  ...
  if (runtime === 'kimi') {
    const agentPath = configDir ? path.join(configDir, 'agents', 'ecl.yaml') : 'agents/ecl.yaml';
    console.log(`
