 ...
 * evolv-coder-lite/bin/lib/verification.cjs (gitignored).
 *
...
/** The set of status values that the ecl-verifier agent emits. */
 ...
 * internally — the verifier (ecl-verifier.md) never emits them. The verifier only
...
const VERIFICATION_ROUTING_TABLE: Record<string, VerificationRoute> = {
  ...
  missing: {
    ...
    next_command: '/ecl:execute-phase',
  },
  ...
  if (rawStatus in VERIFICATION_ROUTING_TABLE && rawStatus !== 'missing' && rawStatus !== 'unknown') {
    ...
    const next_command =
      rawStatus === 'gaps_found'
        ? `/ecl:plan-phase ${phaseNumber} --gaps`
        : entry.next_command;
    return {
      status: entry.status,
