 ...
 *
 * Mirrors the default structure from `evolv-coder-lite/bin/lib/config.cjs`
...
export interface WorkflowConfig {
  ...
  /** Mirrors ecl-tools flat `config.tdd_mode` (from `workflow.tdd_mode`). */
  tdd_mode: boolean;
  /**
  ...
  /** Subagent timeout in ms (matches `evolv-coder-lite/bin/lib/core.cjs` default 300000). */
...
export interface GSDConfig {
  ...
  /** Project slug for branch templates; mirrors ecl-tools `config.project_code`. */
  project_code?: string | null;
  /** Interactive vs headless; mirrors ecl-tools flat `config.mode`. */
  mode?: string;
  [key: string]: unknown;
}
 ...
 * (built-in defaults only; no `~/.ecl/defaults.json` layering).
...
export async function loadConfig(projectDir: string, workstream?: string): Promise<GSDConfig> {
  ...
  // `buildNewProjectConfig` already baked them into config.json at /ecl-new-project.
