...
/** Agent install scan differs between ecl-tools subprocess vs in-process (paths / env); compare the rest. */
...
describe('Golden file tests', () => {
  describe('generate-slug', () => {
    it('SDK output matches ecl-tools.cjs and checked-in golden fixture (fixture must track CLI, not SDK alone)', async () => {
  ...
  describe('config-get', () => {
    ...
    beforeEach(async () => {
      tmpDir = join(tmpdir(), `ecl-golden-cfgget-${Date.now()}-${Math.random().toString(36).slice(2)}`);
    ...
    it('SDK output matches ecl-tools.cjs for top-level key', async () => {
  ...
  describe('find-phase', () => {
    it('SDK output matches ecl-tools.cjs for core fields', async () => {
  ...
  describe('roadmap.analyze', () => {
    it('SDK JSON matches ecl-tools.cjs', async () => {
  ...
  describe('roadmap parity (subprocess parity)', () => {
    async function withFreshRoadmapProjects(): Promise<{ gsdDir: string; sdkDir: string }> {
      ...
      const gsdDir = join(tmpdir(), `ecl-golden-roadmap-ecl-${suffix}`);
      const sdkDir = join(tmpdir(), `ecl-golden-roadmap-sdk-${suffix}`);
      await setupPhasesFixture(gsdDir);
      await setupPhasesFixture(sdkDir);
      return { gsdDir, sdkDir };
    }

    it('roadmap.get-phase matches ecl-tools.cjs on fixture', async () => {
    ...
    it('roadmap.update-plan-progress matches ecl-tools.cjs on fixture', async () => {
      const { gsdDir, sdkDir } = await withFreshRoadmapProjects();
      try {
  ...
  describe('progress', () => {
    it('SDK JSON matches ecl-tools.cjs (`progress json`)', async () => {
  ...
  describe('frontmatter.validate (mutation)', () => {
    it('SDK JSON matches ecl-tools.cjs (plan schema)', async () => {
  ...
  describe('config-set (mutation)', () => {
    ...
    beforeEach(async () => {
      tmpDir = join(tmpdir(), `ecl-golden-config-${Date.now()}-${Math.random().toString(36).slice(2)}`);
      await mkdir(join(tmpDir, '.planning'), { recursive: true });
    ...
    it('SDK config-set JSON matches ecl-tools.cjs (fresh tree per capture)', async () => {
      const registry = createRegistry();
  ...
  describe('state mutations (subprocess parity)', () => {
    ...
    beforeEach(async () => {
      tmpDir = join(tmpdir(), `ecl-golden-state-${Date.now()}-${Math.random().toString(36).slice(2)}`);
    ...
    it('state.update matches ecl-tools.cjs', async () => {
    ...
    it('state.patch matches ecl-tools.cjs', async () => {
    ...
    it('state.begin-phase matches ecl-tools.cjs', async () => {
    ...
    it('state.sync --verify matches ecl-tools.cjs', async () => {
    ...
    it('state.advance-plan matches ecl-tools.cjs', async () => {
    ...
    it('state.update-progress matches ecl-tools.cjs', async () => {
    ...
    it('state.add-decision matches ecl-tools.cjs', async () => {
    ...
    it('state.add-blocker matches ecl-tools.cjs', async () => {
    ...
    it('state.resolve-blocker matches ecl-tools.cjs', async () => {
    ...
    it('state.record-session matches ecl-tools.cjs', async () => {
    ...
    it('state.signal-waiting matches ecl-tools.cjs', async () => {
    ...
    it('state.signal-resume matches ecl-tools.cjs', async () => {
      // First signal so resume has something to remove
      const gsdDir2 = join(tmpdir(), `ecl-golden-state-resume-ecl-${Date.now()}`);
      const sdkDir2 = join(tmpdir(), `ecl-golden-state-resume-sdk-${Date.now()}`);
      try {
    ...
    it('state.planned-phase matches ecl-tools.cjs', async () => {
    ...
    it('state.milestone-switch matches ecl-tools.cjs', async () => {
      const gsdDir2 = join(tmpdir(), `ecl-golden-state-ms-ecl-${Date.now()}`);
      const sdkDir2 = join(tmpdir(), `ecl-golden-state-ms-sdk-${Date.now()}`);
      try {
        await setupMinimalStateProject(gsdDir2);
        await setupMinimalStateProject(sdkDir2);
    ...
    it('state.prune dry-run matches ecl-tools.cjs', async () => {
      ...
      const gsdDir2 = join(tmpdir(), `ecl-golden-prune-ecl-${Date.now()}`);
      const sdkDir2 = join(tmpdir(), `ecl-golden-prune-sdk-${Date.now()}`);
      try {
        await setupMinimalStateProject(gsdDir2);
        await setupMinimalStateProject(sdkDir2);
    ...
    it('state.record-metric matches ecl-tools.cjs (no-metrics-section → SDK auto-creates like CJS)', async () => {
      ...
      const gsdDir2 = join(tmpdir(), `ecl-golden-state-metric-ecl-${Date.now()}`);
      const sdkDir2 = join(tmpdir(), `ecl-golden-state-metric-sdk-${Date.now()}`);
      try {
  ...
  describe('phase mutations (subprocess parity)', () => {
    async function withFreshPhaseProjects(): Promise<{ gsdDir: string; sdkDir: string }> {
      const suffix = `${Date.now()}-${Math.random().toString(36).slice(2)}`;
      const gsdDir = join(tmpdir(), `ecl-golden-phase-ecl-${suffix}`);
      const sdkDir = join(tmpdir(), `ecl-golden-phase-sdk-${suffix}`);
      await setupMinimalStateProject(gsdDir);
      await setupMinimalStateProject(sdkDir);
      return { gsdDir, sdkDir };
    }

    it('phase.add matches ecl-tools.cjs', async () => {
    ...
    it('phase.add-batch matches ecl-tools.cjs', async () => {
      const { gsdDir, sdkDir } = await withFreshPhaseProjects();
      try {
    ...
    it('phase.insert matches ecl-tools.cjs', async () => {
      const { gsdDir, sdkDir } = await withFreshPhaseProjects();
      try {
  ...
  describe('phases parity (subprocess parity)', () => {
    async function withFreshPhasesProjects(): Promise<{ gsdDir: string; sdkDir: string }> {
      const suffix = `${Date.now()}-${Math.random().toString(36).slice(2)}`;
      const gsdDir = join(tmpdir(), `ecl-golden-phases-ecl-${suffix}`);
      const sdkDir = join(tmpdir(), `ecl-golden-phases-sdk-${suffix}`);
      await setupPhasesFixture(gsdDir);
      await setupPhasesFixture(sdkDir);
      return { gsdDir, sdkDir };
    }

    it('phases.list matches ecl-tools.cjs', async () => {
    ...
    it('phases.list --type plans matches ecl-tools.cjs', async () => {
      const { gsdDir, sdkDir } = await withFreshPhasesProjects();
      try {
    ...
    it('phases.list --type summaries matches ecl-tools.cjs', async () => {
      const { gsdDir, sdkDir } = await withFreshPhasesProjects();
      try {
    ...
    it('phases.list --phase 10 matches ecl-tools.cjs', async () => {
      const { gsdDir, sdkDir } = await withFreshPhasesProjects();
      try {
    ...
    it('phases.list --include-archived matches ecl-tools.cjs', async () => {
      const { gsdDir, sdkDir } = await withFreshPhasesProjects();
      try {
    ...
    it('phases.clear --confirm matches ecl-tools.cjs', async () => {
      const { gsdDir, sdkDir } = await withFreshPhasesProjects();
      try {
  ...
  describe('current-timestamp', () => {
    it('SDK full format matches ecl-tools.cjs output structure', async () => {
    ...
    it('SDK date format matches ecl-tools.cjs output structure', async () => {
    ...
    it('SDK filename format matches ecl-tools.cjs (same subprocess round-trip)', async () => {
  ...
  describe('init.quick', () => {
    it('SDK JSON matches ecl-tools.cjs except clock-derived quick fields', async () => {
  ...
  describe('state.validate', () => {
    it('SDK output matches ecl-tools.cjs', async () => {
  ...
  describe('state.sync --verify', () => {
    it('SDK dry-run output matches ecl-tools.cjs', async () => {
  ...
  describe('detect-custom-files', () => {
    ...
    beforeEach(async () => {
      tmpDir = join(tmpdir(), `ecl-golden-dcf-${Date.now()}-${Math.random().toString(36).slice(2)}`);
      await mkdir(join(tmpDir, 'agents'), { recursive: true });
      await writeFile(join(tmpDir, 'ecl-file-manifest.json'), JSON.stringify({ version: 1, files: {} }), 'utf-8');
    ...
    it('SDK output matches ecl-tools.cjs for manifest + custom file', async () => {
  ...
  describe('docs-init', () => {
    it('SDK output matches ecl-tools.cjs (normalized existing_docs order)', async () => {
  ...
  describe('intel.update', () => {
    it('SDK JSON matches ecl-tools.cjs (`intel update`)', async () => {
  ...
  describe('roadmap.annotate-dependencies', () => {
    it('roadmap.annotate-dependencies matches ecl-tools.cjs on fixture', async () => {
      const suffix = `${Date.now()}-${Math.random().toString(36).slice(2)}`;
      const gsdDir = join(tmpdir(), `ecl-golden-roadmap-annotate-ecl-${suffix}`);
      const sdkDir = join(tmpdir(), `ecl-golden-roadmap-annotate-sdk-${suffix}`);
      try {
        await setupPhasesFixture(gsdDir);
        await setupPhasesFixture(sdkDir);
  ...
  describe('phase.next-decimal', () => {
    it('phase.next-decimal matches ecl-tools.cjs on fixture', async () => {
      const suffix = `${Date.now()}-${Math.random().toString(36).slice(2)}`;
      const gsdDir = join(tmpdir(), `ecl-golden-phase-nd-ecl-${suffix}`);
      const sdkDir = join(tmpdir(), `ecl-golden-phase-nd-sdk-${suffix}`);
      try {
        await setupMinimalStateProject(gsdDir);
        await setupMinimalStateProject(sdkDir);
  ...
  describe('phase.remove and phase.complete', () => {
    it('phase.remove matches ecl-tools.cjs on fixture', async () => {
      const suffix = `${Date.now()}-${Math.random().toString(36).slice(2)}`;
      const gsdDir = join(tmpdir(), `ecl-golden-phase-rm-ecl-${suffix}`);
      const sdkDir = join(tmpdir(), `ecl-golden-phase-rm-sdk-${suffix}`);
      try {
        await setupPhasesFixture(gsdDir);
        await setupPhasesFixture(sdkDir);
    ...
    it('phase.complete matches ecl-tools.cjs on fixture', async () => {
      const suffix = `${Date.now()}-${Math.random().toString(36).slice(2)}`;
      const gsdDir = join(tmpdir(), `ecl-golden-phase-complete-ecl-${suffix}`);
      const sdkDir = join(tmpdir(), `ecl-golden-phase-complete-sdk-${suffix}`);
      try {
        await setupPhasesFixture(gsdDir);
        await setupPhasesFixture(sdkDir);
