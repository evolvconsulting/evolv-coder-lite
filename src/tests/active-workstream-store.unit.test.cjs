...
} = require('../evolv-coder-lite/bin/lib/active-workstream-store.cjs');

// ── Helpers ───────────────────────────────────────────────────────────────────

const SESSION_ENV_KEYS = [
  'ECL_SESSION_KEY', 'CODEX_THREAD_ID', 'CLAUDE_SESSION_ID', 'CLAUDE_CODE_SSE_PORT',
...
describe('createSharedPointerAdapter', () => {
  let tmpDir;
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-unit-shared-'));
...
describe('createSessionScopedPointerAdapter', () => {
  ...
  let saved;
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-unit-session-'));
  ...
  test('uses env session key when no fixed key provided', () => {
    process.env.ECL_SESSION_KEY = 'env-session';
    const adapter = createSessionScopedPointerAdapter(tmpDir);
    assert.notEqual(adapter, null);
...
describe('pickActiveWorkstreamAdapter', () => {
  ...
  test('returns session adapter from adapters when session key exists', () => {
    process.env.ECL_SESSION_KEY = 'some-session';
  ...
  test('creates shared pointer adapter when no opts and no session', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-pick-'));
    try {
      fs.mkdirSync(path.join(tmpDir, '.planning'), { recursive: true });
  ...
  test('creates session scoped adapter when session key exists and no adapters given', () => {
    process.env.ECL_SESSION_KEY = 'my-session';
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-pick-sess-'));
    try {
      const picked = pickActiveWorkstreamAdapter(tmpDir, {});
      ...
      cleanup(tmpDir);
      delete process.env.ECL_SESSION_KEY;
    }
  });
...
describe('getWorkstreamSessionKey', () => {
  ...
  test('returns ecl-session-key prefixed key for ECL_SESSION_KEY', () => {
    process.env.ECL_SESSION_KEY = 'mysession';
    const key = getWorkstreamSessionKey();
    assert.equal(key, 'ecl-session-key-mysession');
  });
  ...
  test('ECL_SESSION_KEY takes priority over CODEX_THREAD_ID', () => {
    process.env.ECL_SESSION_KEY = 'first';
    process.env.CODEX_THREAD_ID = 'second';
    const key = getWorkstreamSessionKey();
    assert.equal(key, 'ecl-session-key-first');
  });
  ...
  test('empty ECL_SESSION_KEY falls through', () => {
    process.env.ECL_SESSION_KEY = '';
    process.env.CODEX_THREAD_ID = 'fallback';
    const key = getWorkstreamSessionKey();
    assert.equal(key, 'codex-thread-id-fallback');
  });

  test('whitespace-only ECL_SESSION_KEY falls through', () => {
    process.env.ECL_SESSION_KEY = '   ';
...
describe('getActiveWorkstream', () => {
  ...
  let saved;
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-unit-get-'));
    saved = saveSessionEnv();
    clearSessionEnv();
  });
  afterEach(() => {
...
describe('setActiveWorkstream', () => {
  ...
  let saved;
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-unit-set-'));
    saved = saveSessionEnv();
    clearSessionEnv();
  });
  afterEach(() => {
...
describe('clearActiveWorkstream', () => {
  ...
  test('uses session adapter branch when session key present', () => {
    process.env.ECL_SESSION_KEY = 'clear-session';
    const session = createMemoryPointerAdapter('session-ws');
    const shared = createMemoryPointerAdapter('shared-ws');
...
describe('resolveActiveWorkstream', () => {
  test('cli source overrides env and store', () => {
    const r = resolveActiveWorkstream('/repo', ['--ws', 'cli-ws'], { ECL_WORKSTREAM: 'env-ws' }, { getStored: () => 'store-ws' });
  ...
  test('env source overrides store', () => {
    const r = resolveActiveWorkstream('/repo', [], { ECL_WORKSTREAM: 'env-ws' }, { getStored: () => 'store-ws' });
    assert.equal(r.ws, 'env-ws');
    assert.equal(r.source, 'env');
  });

  test('env with whitespace is trimmed', () => {
    const r = resolveActiveWorkstream('/repo', [], { ECL_WORKSTREAM: '  trimmed  ' }, { getStored: () => null });
    assert.equal(r.ws, 'trimmed');
    assert.equal(r.source, 'env');
  });

  test('env whitespace-only falls to store', () => {
    const r = resolveActiveWorkstream('/repo', [], { ECL_WORKSTREAM: '   ' }, { getStored: () => 'store-ws' });
  ...
  test('throws for invalid name from env', () => {
    assert.throws(
      () => resolveActiveWorkstream('/repo', [], { ECL_WORKSTREAM: 'bad name' }, { getStored: () => null }),
      /Invalid workstream name/
    );
  });
  ...
  test('ECL_WORKSTREAM non-string falls to store', () => {
    const r = resolveActiveWorkstream('/repo', [], { ECL_WORKSTREAM: 42 }, { getStored: () => 'store-ws' });
    assert.equal(r.ws, 'store-ws');
    assert.equal(r.source, 'store');
  });
  ...
  test('source is exactly "env" string', () => {
    const r = resolveActiveWorkstream('/repo', [], { ECL_WORKSTREAM: 'myws' }, { getStored: () => null });
    assert.equal(r.source, 'env');
  });
...
describe('applyResolvedWorkstreamEnv', () => {
  ...
  test('overwrites existing ECL_WORKSTREAM value', () => {
    const env = { ECL_WORKSTREAM: 'old' };
    applyResolvedWorkstreamEnv({ ws: 'new-ws', source: 'store', args: [] }, env);
    assert.equal(env.ECL_WORKSTREAM, 'new-ws');
  });

  test('uses process.env by default (does not throw)', () => {
    const saved = process.env.ECL_WORKSTREAM;
    try {
      applyResolvedWorkstreamEnv({ ws: 'default-env-ws', source: 'cli', args: [] });
      assert.equal(process.env.ECL_WORKSTREAM, 'default-env-ws');
    } finally {
      if (saved === undefined) delete process.env.ECL_WORKSTREAM;
      else process.env.ECL_WORKSTREAM = saved;
    }
  });
