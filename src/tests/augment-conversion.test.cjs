...
`;
    ...
    const result = convertClaudeCommandToAugmentSkill(input, 'ecl-quick');
...
`;
    ...
    const result = convertClaudeCommandToAugmentSkill(input, 'ecl-plan-phase');
    // Slash commands: /ecl:execute-phase -> /ecl-execute-phase
    assert.ok(result.includes('/ecl-execute-phase 17'), 'slash command ecl: -> ecl-');
    assert.ok(result.includes('/ecl-help'), '/ecl-help preserved');
    assert.ok(result.includes('ecl-progress'), 'bare ecl: -> ecl-');
  });
...
`;
    ...
    const result = convertClaudeCommandToAugmentSkill(input, 'ecl-test');
...
`;
    ...
    assert.ok(nameMatch, 'frontmatter contains name field');
    assert.strictEqual(nameMatch[1], 'ecl-bugfix', 'agent name is plain scalar');
    assert.ok(!result.includes('name: "ecl-bugfix"'), 'quoted agent name is not emitted');
  });
...
`;
    ...
    assert.ok(result.includes('name: ecl-test'), 'name preserved');
...
describe('convertClaudeToAugmentMarkdown', () => {
  ...
  test('normalizes ecl: to ecl-', () => {
    const input = 'Run /ecl:new-project for new projects.';
    const result = convertClaudeToAugmentMarkdown(input);
    assert.strictEqual(result, 'Run /ecl-new-project for new projects.');
  });
});