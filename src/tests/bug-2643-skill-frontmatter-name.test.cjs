 ...
 *
 * Original (#2643): workflows emitted Skill(skill="ecl:<cmd>") and the
 * installer registered colon form in SKILL.md name: to match.
 *
 * Updated (#2808): workflows now use Skill(skill="ecl-<cmd>") (hyphen),
 * and the installer emits name: ecl-<cmd> (hyphen). Claude Code autocomplete
 * now shows the canonical hyphen form instead of the deprecated colon form.
 * The directory name (ecl-<cmd>) is unchanged.
 */
 ...
 *      `Skill(skill="ecl:extract_learnings")` is surfaced rather than
 *      silently skipped by an over-strict character class.
 *
 * Returns `[{ name, raw }]` per call. Filtering by namespace (ecl- vs ecl:)
...
function extractSkillCalls(content) {
  ...
  // (e.g. `Skill(skill=\"ecl-plan-phase\", …)` written inside a string
...
describe('skill frontmatter name parity (#2643 / #2808)', () => {
  test('skillFrontmatterName helper emits hyphen form (#2808)', () => {
    ...
    assert.strictEqual(skillFrontmatterName('ecl-execute-phase'), 'ecl-execute-phase');
    assert.strictEqual(skillFrontmatterName('ecl-plan-phase'), 'ecl-plan-phase');
    assert.strictEqual(skillFrontmatterName('ecl-next'), 'ecl-next');
  });

  test('convertClaudeCommandToClaudeSkill emits name: ecl-<cmd> (hyphen)', () => {
    const input = '---\nname: old\ndescription: test\n---\n\nBody.';
    const result = convertClaudeCommandToClaudeSkill(input, 'ecl-execute-phase');
    ...
    assert.strictEqual(
      nameValue,
      'ecl-execute-phase',
      `frontmatter name: must be 'ecl-execute-phase' (hyphen form), got '${nameValue}'`
    );
  });

  test('no workflow uses deprecated Skill(skill="ecl:<cmd>") colon form', () => {
  ...
  test('every workflow Skill(skill="ecl-<cmd>") resolves to an emitted skill name', () => {
    const workflowFiles = collectFiles(WORKFLOWS_DIR);
    ...
    for (const f of workflowFiles) {
      ...
      for (const n of extractSkillNamesHyphen(src)) {
        // Skip template expressions (e.g. `ecl-${ref.skill}`): these are
    ...
    assert.ok(
      referenced.size > 0,
      `expected at least one literal Skill(skill="ecl-<cmd>") reference in workflows under ${WORKFLOWS_DIR}`
    );
