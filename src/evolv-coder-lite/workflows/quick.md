<purpose>
Execute small, ad-hoc tasks with eCL guarantees (atomic commits, STATE.md tracking). Quick mode spawns ecl-planner (quick mode) + ecl-executor(s), tracks tasks in `.planning/quick/`, and updates STATE.md's "Quick Tasks Completed" table.
...
- ecl-code-reviewer — Reviews source files for bugs, security issues, and code quality
</available_agent_types>
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 eCL ► QUICK TASK (DISCUSS + VALIDATE)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 eCL ► QUICK TASK (DISCUSS + RESEARCH)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 eCL ► QUICK TASK (RESEARCH + VALIDATE)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
INIT=$(ecl_run query init.quick "$DESCRIPTION")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_PLANNER=$(ecl_run query agent-skills ecl-planner)
AGENT_SKILLS_EXECUTOR=$(ecl_run query agent-skills ecl-executor)
AGENT_SKILLS_CHECKER=$(ecl_run query agent-skills ecl-plan-checker)
AGENT_SKILLS_VERIFIER=$(ecl_run query agent-skills ecl-verifier)
```
...
if [ "$USE_WORKTREES" != "false" ]; then
  ecl_run query worktree.reap-orphans 2>/dev/null || true
fi
```
...
**If `roadmap_exists` is false:** Error — Quick mode requires an active project with ROADMAP.md. Run `/ecl:new-project` first.
...
if [ "${USE_WORKTREES:-true}" != "false" ]; then
  QUICK_WORKTREE_MANIFEST=$(mktemp "${TMPDIR:-/tmp}/ecl-quick-worktree-XXXXXX.json")
  printf '{"worktrees":[]}\n' > "$QUICK_WORKTREE_MANIFEST"
  export QUICK_WORKTREE_MANIFEST
fi
```

Spawn ecl-executor with plan reference:
...
ORCHESTRATOR build-time embed (NOT a sub-agent runtime step): before this dispatch, read \`evolv-coder-lite/references/worktree-branch-check.md\`, substitute \`{EXPECTED_BASE}\` with the base SHA captured above (${EXPECTED_BASE}), substitute \`{EXPECTED_BASE_ALTERNATE}\` with \`${QUICK_PLAN_PARENT}\` when it differs from \`${EXPECTED_BASE}\` (otherwise empty), and replace this note with that fragment's \`<worktree_branch_check>\` block so the dispatched prompt carries the runnable guard verbatim — do not pass this instruction through in its place.
</worktree_branch_check>
...
1. **Worktree cleanup:** If the executor ran with `isolation="worktree"`, merge the worktree branch back and clean up:
   ...
   ecl_run query worktree.cleanup-wave --manifest "$QUICK_WORKTREE_MANIFEST" || exit 1
   ```
...
Agent(
  ...
  Depth: quick",
  subagent_type="ecl-code-reviewer",
  model="{executor_model}"
)
```
...
Stage and commit quick task artifacts. This step MUST always run — even if the executor already committed some files (e.g. when running without worktree isolation). The `ecl-tools.cjs query commit` command (or legacy `ecl-tools.cjs` commit) handles already-committed files gracefully.
...
ecl_run query commit "docs(quick-${quick_id}): ${DESCRIPTION}" --files ${file_list}
```
...
eCL > QUICK TASK COMPLETE (VALIDATED)
...
Ready for next task: /ecl:quick ${ECL_WS}
```
