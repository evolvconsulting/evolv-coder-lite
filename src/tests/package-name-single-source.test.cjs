 ...
 *
 * Scans runtime files (bin/install.js, evolv-coder-lite/bin/**, scripts/*.cjs)
 * and FAILS if the literal `@evolvconsulting/evolv-coder-lite` appears in a
...
const LITERAL = '@evolvconsulting/evolv-coder-lite';
const IDENTITY_MODULE = path.join(ROOT, 'evolv-coder-lite', 'bin', 'lib', 'package-identity.cjs');

// Files to scan: bin/install.js + everything under evolv-coder-lite/bin/ + touched scripts
function getRuntimeFiles() {
  ...
  // evolv-coder-lite/bin/**/*.cjs and *.js (recursive)
  ...
  collectDir(path.join(ROOT, 'evolv-coder-lite', 'bin'));
...
test('no hardcoded @evolvconsulting/evolv-coder-lite literals in runtime non-comment code lines (#516)', () => {
  ...
  assert.deepEqual(
    violations,
    [],
    `Found ${violations.length} hardcoded @evolvconsulting/evolv-coder-lite literal(s) in non-comment code lines:\n` +
    violations.map(v => `  ${v}`).join('\n') +
    '\n\nReplace each with the PACKAGE_NAME imported from evolv-coder-lite/bin/lib/package-identity.cjs'
  );
});
