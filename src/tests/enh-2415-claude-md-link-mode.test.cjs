...
const { cmdGenerateClaudeMd } = require('../evolv-coder-lite/bin/lib/profile-output.cjs');

function makeTempProject(files = {}) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-2415-'));
...
test('link mode falls back to embed for workflow section (no linkable source)', () => {
  ...
  assert.ok(!content.includes('@eCL defaults'), 'workflow should not write @eCL defaults');
  assert.ok(content.includes('eCL Workflow Enforcement'), 'workflow content should be embedded inline');
});
