 ...
 * commands/ecl/ for the slash menu to read on older configurations.
 *
...
const SETTINGS_PATH = path.join(os.tmpdir(), `ecl-test-settings-${process.pid}.json`);
...
describe('Bug #2957: claude+global post-install message', () => {
  test('claude+global message tells the user to restart and offers skill-name fallback', () => {
    ...
    assert.match(output, /\/ecl-new-project/, 'should still mention /ecl-new-project');
    assert.match(output, /ecl-new-project skill/i, 'should mention the skill name fallback');
    assert.doesNotMatch(
      output,
  ...
  test('claude+local message keeps the original /ecl-new-project instruction', () => {
