/**
 * Regression test for #2829: `command not found: ecl-sdk` with local-mode install.
 *
 * Repro: a fresh `npx @evolvconsulting/evolv-coder-lite@latest` install with the runtime set
 * to local mode left every `ecl-sdk query …` call site unable to resolve the
 ...
 * actually carries `sdk/dist/cli.js` and `bin/ecl-sdk.js` regardless of mode,
 ...
 * `ecl-sdk` is on PATH and self-link the shim into a user-writable PATH dir
 * if not, so `/ecl-plan` and friends can call `ecl-sdk query …` directly.
 *
...
describe('bug #2829: local-mode install must materialize ecl-sdk on PATH',
  { skip: isWindows ? 'POSIX-only: asserts ~/.local/bin shebang shim; Windows uses ecl-sdk.cmd + USERPROFILE + PATHEXT' : false },
  () => {
  let tmpRoot;
  let sdkDir;
  let pathDir;
  ...
  beforeEach(() => {
    tmpRoot = createTempDir('ecl-2829-');
  ...
  test('isLocal=true with sdk/dist/cli.js present self-links ecl-sdk into an on-PATH HOME bin dir', () => {
    ...
    // The shim must be materialized so `ecl-sdk query …` resolves.
    const linkPath = path.join(localBin, 'ecl-sdk');
    assert.ok(
      fs.existsSync(linkPath),
      `local install must self-link ecl-sdk into ${linkPath}. Output:\n${combined}`,
    );
    // And the installer must report ready (matches the global-mode UX).
    assert.ok(
      /eCL SDK ready/.test(combined),
      `local install must print "eCL SDK ready" once ecl-sdk is on PATH. Output:\n${combined}`,
    );
  ...
  test('isLocal=true with dist present and no on-PATH HOME bin still warns rather than lying about readiness', () => {
    ...
    // probe still cannot find ecl-sdk on PATH.
    const { stdout, stderr } = captureConsole(() => {
      installSdkIfNeeded({ sdkDir, isLocal: true });
    });
    const combined = `${stdout}\n${stderr}`;
    assert.ok(
      !/eCL SDK ready/.test(combined),
      `installer must not falsely claim ready when ecl-sdk is not callable. Output:\n${combined}`,
    );
    assert.ok(
