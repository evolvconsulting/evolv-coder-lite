 ...
 *
 * The ecl-autonomous skill spawns background agents via Agent(..., run_in_background=true).
...
// commands/ecl/autonomous.md is the installed command — its frontmatter is what Claude Code
// reads at runtime to enforce allowed-tools. Checking text content IS checking the contract.
describe('commands/ecl/autonomous.md allowed-tools', () => {
  test('includes Agent in allowed-tools list', () => {
    const filePath = path.join(__dirname, '..', 'commands', 'ecl', 'autonomous.md');
