...
} = require('../evolv-coder-lite/bin/lib/shell-command-projection.cjs');

// Bidirectional eCL slash-command namespace transformer (#3583).
...
} = require('../evolv-coder-lite/bin/lib/worktree-base-ref.cjs');
const { resolveRuntimeConfigIntent } = require('../evolv-coder-lite/bin/lib/runtime-config-adapter-registry.cjs');
 ...
 * `/ecl:<cmd>` colon refs leak into installed agent prose. Sibling fixes
...
/**
 * #3677 predicate — true when an agent body needs `/ecl:<cmd>` → `/ecl-<cmd>`
...
const ECL_CODEX_MARKER = '# eCL Agent Configuration \u2014 managed by evolv-coder-lite installer';
const ECL_CODEX_HOOKS_OWNERSHIP_PREFIX = '# eCL codex_hooks ownership: ';
...
// for evolv-coder-lite's own known-safe tool calls, and to add defense-in-depth deny
// entries for common credential files.
//
// Format: each string uses Claude Code's documented permission rule syntax \u2014
//   "Tool(pattern)"  e.g. "Bash(npx evolv-coder-lite *)", "Read(.planning/*)"
//   "Tool"           (bare tool name, no pattern)
//
// Merge policy: additive, non-destructive \u2014 existing user entries are preserved;
// eCL entries are appended only when not already present (idempotent).
const ECL_CLAUDE_ALLOW_PERMISSIONS = Object.freeze([
  'Bash(npx evolv-coder-lite *)',
  'Read(.planning/*)',
  'Write(.planning/*)',
  'Read(STATE.md)',
  'Write(STATE.md)',
]);
const ECL_CLAUDE_DENY_PERMISSIONS = Object.freeze([
  'Read(.env)',
  'Read(.env.*)',
  'Read(.secrets)',
]);
...
/**
 * Merge eCL-owned permission entries into a Claude Code settings object.
 *
 * Additive and idempotent: existing allow/deny entries are preserved; eCL
...
const ECL_COPILOT_INSTRUCTIONS_MARKER = '<!-- eCL Configuration \u2014 managed by evolv-coder-lite installer -->';
const ECL_COPILOT_INSTRUCTIONS_CLOSE_MARKER = '<!-- /eCL Configuration -->';
...
// hook runs an INLINE shell command (bash / powershell), so the eCL hook is fully
...
const ECL_COPILOT_HOOK_FILE = 'ecl-session.json';
...
const ECL_COPILOT_SESSION_MSG_PRESENT =
  'eCL: .planning/STATE.md present - review the current phase and any blockers before acting.';
const ECL_COPILOT_SESSION_MSG_ABSENT =
  'eCL: no .planning/ workflow found - run /ecl-new-project to start a tracked workflow.';
const ECL_COPILOT_SESSION_HOOK_BASH =
  'if [ -f .planning/STATE.md ]; then ' +
  `printf '%s' '{"additionalContext":"${ECL_COPILOT_SESSION_MSG_PRESENT}"}'; else ` +
  `printf '%s' '{"additionalContext":"${ECL_COPILOT_SESSION_MSG_ABSENT}"}'; fi`;
const ECL_COPILOT_SESSION_HOOK_PWSH =
  'if (Test-Path .planning/STATE.md) ' +
  `{ '{"additionalContext":"${ECL_COPILOT_SESSION_MSG_PRESENT}"}' } ` +
  `else { '{"additionalContext":"${ECL_COPILOT_SESSION_MSG_ABSENT}"}' }`;
...
// A `command` hook entry runs an external script. eCL registers two managed hooks:
//   sessionStart → ecl-cursor-session-start.js  (context injection)
//   postToolUse  → ecl-cursor-post-tool.js       (STATE.md update monitor)
// Cursor docs: https://cursor.com/docs/hooks
const ECL_CURSOR_SESSION_HOOK_SCRIPT = 'ecl-cursor-session-start.js';
const ECL_CURSOR_POST_TOOL_HOOK_SCRIPT = 'ecl-cursor-post-tool.js';
// Marker comment embedded in managed hook entries so eCL can find+remove them.
const ECL_CURSOR_HOOK_MARKER = 'ecl-managed';

// eCL-managed files under hooks/lib/ (helpers required by ecl-*.sh hooks).
// git-cmd.js does not start with "ecl-" (shared classifier for #3129), ecl-graphify-rebuild.sh does.
const ECL_HOOK_LIB_FILES = ['git-cmd.js', 'ecl-graphify-rebuild.sh'];

const CODEX_AGENT_SANDBOX = {
  'ecl-executor': 'workspace-write',
  'ecl-planner': 'workspace-write',
  'ecl-phase-researcher': 'workspace-write',
  'ecl-project-researcher': 'workspace-write',
  'ecl-research-synthesizer': 'workspace-write',
  'ecl-verifier': 'workspace-write',
  'ecl-codebase-mapper': 'workspace-write',
  'ecl-roadmapper': 'workspace-write',
  'ecl-debugger': 'workspace-write',
  'ecl-plan-checker': 'read-only',
  'ecl-integration-checker': 'read-only',
};
...
// Hoisted to top with absolute __dirname-based paths so `ecl install codex` works
// when invoked via npm global install (cwd is the user's project, not the ecl repo
// root). Inline `require('../evolv-coder-lite/...')` from inside install functions
...
const { MODEL_PROFILES: ECL_MODEL_PROFILES } = require(path.join(_gsdLibDir, 'model-profiles.cjs'));
const {
...
const {
  planLegacyCleanup,
  applyLegacyCleanup,
} = require(path.join(__dirname, '..', 'evolv-coder-lite', 'bin', 'lib', 'legacy-cleanup.cjs'));
const {
  updateCacheFileName,
} = require(path.join(__dirname, '..', 'evolv-coder-lite', 'bin', 'lib', 'package-identity.cjs'));
...
const hasPortableHooks = args.includes('--portable-hooks') || process.env.ECL_PORTABLE_HOOKS === '1';
...
// Note: when re-running as `ecl update` the marker is read later (after
...
const banner = '\n' +
  ...
  '   ▀████▀     ▀██▀     ▀████▀   ██    ▀██▀' + reset + '\n' +
  '\n' +
  '  eCL Core ' + dim + 'v' + pkg.version + reset + '\n' +
  '  Git. Ship. Done.\n' +
...
/**
 * Rewrite legacy `node .../ecl-*.js` command strings in settings.hooks to use
...
function resolveBashRunner(opts) {
  ...
  const candidates = [];
  if (env.ECL_BASH_PATH) candidates.push(env.ECL_BASH_PATH);
  ...
  for (const entries of Object.values(settings.hooks)) {
    ...
    for (const entry of entries) {
      ...
      for (const h of entry.hooks) {
        ...
        // /home/me/scripts/wraps-ecl-check-update.js-and-more.js). #3002 CR.
...
/**
 * Build the eCL-managed Codex SessionStart hook block for config.toml.
 *
...
function buildCodexHookBlock(targetDir, opts) {
  ...
  const platform = (opts && opts.platform) || process.platform;
  const updateCheckScript = path.resolve(targetDir, 'hooks', 'ecl-check-update.js');
  const commandValue = projectCodexHookTomlCommand({
    absoluteRunner,
    scriptPath: updateCheckScript,
    platform,
  });
  return `${eol}# eCL Hooks${eol}` +
...
function rewriteLegacyCodexHookBlock(content, absoluteRunner, opts) {
  ...
  //   command = "node /Users/x/.codex/hooks/ecl-check-update.js"
  //   command = "node \"/Users/x/.codex/hooks/ecl-check-update.js\""
  // Examples we must leave alone:
  //   command = "\"/usr/local/bin/node\" \"/path/to/ecl-check-update.js\""  ← already absolute
...
/**
 * Generic reconcile helper: ensure hooks.json contains exactly one managed eCL
...
/**
 * Reconcile the eCL-managed SessionStart hook entry in hooks.json.
 ...
 *
 *   "C:/Program Files/nodejs/node.exe" "C:/path/.codex/hooks/ecl-check-update.js"
 *
 ...
 * ecl-check-update hook entry, while preserving user-owned entries.
 *
 * Codex accepts hook config from hooks.json and config.toml. To avoid the
 * startup warning for mixed representations in the same layer, eCL now stores
...
function ensureCodexHooksJsonSessionStart(targetDir, opts = {}) {
  ...
  const scriptPath = path.resolve(targetDir, 'hooks', 'ecl-check-update.js').replace(/\\/g, '/');
...
/**
 * Ensure hooks.json contains exactly one managed eCL hook entry for the given
 * Codex event, wired to ecl-context-monitor.js. Preserves user-owned entries.
 *
 * Used for the new Codex events added in #772:
 *   SubagentStart — inject context / ECL_AGENT_NAME awareness at subagent open
 *   Stop          — post-session context headroom tracking
 *   PostToolUse   — mirror the Claude Code PostToolUse context monitor
 *
 * All three events are routed through ecl-context-monitor.js — the same hook
 ...
 *
 * On Windows (#3426): writes a ecl-context-monitor.cmd shim alongside the .js
 * file and uses the .cmd path as the hook command — exactly the same fix as
 * SessionStart uses for ecl-check-update — to avoid the bash.exe POSIX-exec
...
function ensureCodexHooksJsonEvent(targetDir, eventName, opts = {}) {
  ...
  const scriptPath = path.resolve(targetDir, 'hooks', 'ecl-context-monitor.js').replace(/\\/g, '/');
  ...
  if (platform === 'win32') {
    ...
    // ecl-check-update.js also affects ecl-context-monitor.js.
    const shimIR = buildCodexHookWindowsShimIR(scriptPath, absoluteRunner);
    if (!shimIR) return { changed: false, wrote: false, path: hooksJsonPath };
    try {
      atomicWriteFileSync(shimIR.cmdPath, shimIR.render.cmd(), 'utf8');
...
/**
 * Remove a eCL-managed event entry from hooks.json. Called during uninstall.
 *
 * @param {string} targetDir
 ...
 * @param {string} hookName - Hook filename (e.g. 'ecl-statusline.js')
...
/**
 * Read model_overrides from ~/.ecl/defaults.json at install time.
...
function readGsdGlobalModelOverrides() {
  try {
    const defaultsPath = path.join(os.homedir(), '.ecl', 'defaults.json');
 ...
 *
 * Merges `~/.ecl/defaults.json` (global) with per-project
 ...
 * described — a per-project override for `ecl-codebase-mapper` in
...
/**
 * #2517 — Read a single eCL config file (defaults.json or per-project
...
function _readGsdConfigFile(absPath, label) {
  ...
  try {
    ...
    process.stderr.write(`ecl: warning — could not read ${label} (${absPath}): ${err.message}\n`);
    return null;
  }
  try {
    return JSON.parse(raw);
  } catch (err) {
    process.stderr.write(`ecl: warning — invalid JSON in ${label} (${absPath}): ${err.message}\n`);
    return null;
  }
}
 ...
 * `~/.ecl/defaults.json`, with per-project keys winning over global. This
...
function readGsdRuntimeProfileResolver(targetDir = null) {
  ...
  // Per-project wins. Only fall back to ~/.ecl/defaults.json when the project
  ...
  return {
    ...
    resolve(agentName) {
      const agentModels = ECL_MODEL_PROFILES[agentName];
...
function convertClaudeToCopilotContent(content, isGlobal = false) {
  ...
  c = c.replace(/\.\/\.claude\//g, './.github/');
  c = c.replace(/\.claude\//g, '.github/');
  // CONV-07: Command name conversion (all ecl: references → ecl-)
  c = c.replace(/ecl:/g, 'ecl-');
...
/**
 * Map a skill directory name (ecl-<cmd>) to the frontmatter `name:` used
 * by Claude Code as the skill identity. Emits the hyphen form (ecl-<cmd>)
 * so Claude Code autocomplete shows the canonical invocation form, not the
 * deprecated colon form. See #2808.
 *
 * Historical note: this previously returned `ecl:<cmd>` (colon) because
 * workflows called Skill(skill="ecl:<cmd>"). Those calls have been updated
 * to use hyphen form (#2808) so the colon rewrite is no longer needed.
 *
 * Codex must NOT use this helper: its adapter invokes skills as `$ecl-<cmd>`
...
function skillFrontmatterName(skillDirName) {
  ...
  // Return the hyphen form as-is (ecl-<cmd>) — canonical since #2808.
  return skillDirName;
}
 ...
 *
 * We assign descending priorities to eCL's main-loop commands so the most-used
 ...
 * Keyed by command stem (skill dir is `ecl-<stem>`).
 */
 ...
 * Emits `name: ecl-<cmd>` (hyphen) so Skill(skill="ecl-<cmd>") calls and
...
function convertClaudeCommandToClaudeSkill(content, skillName, runtime = null, cmdNames = null) {
  ...
  // #3583: rewrite any /ecl:<cmd> or ecl:<cmd> in the body to the canonical
  // hyphen form (ecl-<cmd>) so installed SKILL.md bodies match the hyphen
  ...
  // Track eCL's package version so Hermes' skill_view() reports a stable
  ...
  // we keep their output byte-stable). skillName is the `ecl-<stem>` dir name.
  if (runtime === 'qwen') {
    const stem = typeof skillName === 'string' && skillName.startsWith('ecl-')
      ? skillName.slice(4)
      : skillName;
...
function convertClaudeToAntigravityContent(content, isGlobal = false) {
  ...
  c = c.replace(/\.\/\.claude\//g, './.agent/');
  c = c.replace(/\.claude\//g, '.agent/');
  // Command name conversion (all ecl: references → ecl-)
  c = c.replace(/ecl:/g, 'ecl-');
  // Runtime-neutral agent name replacement (#766)
...
function convertSlashCommandsToCursorSkillMentions(content) {
  // Keep leading "/" for slash commands; only normalize ecl: -> ecl-.
  // This preserves rendered "next step" commands like "/ecl-execute-phase 17".
  return content.replace(/ecl:/gi, 'ecl-');
}
...
function convertClaudeToCursorMarkdown(content) {
  ...
  converted = converted.replace(/\$ARGUMENTS\b/g, '{{ECL_ARGS}}');
...
- Treat all user text after the skill mention as \`{{ECL_ARGS}}\`.
- If no arguments are present, treat \`{{ECL_ARGS}}\` as empty.
...
## C. Tool Usage
Use these Cursor tools when executing eCL workflows:
...
function convertClaudeCommandToCursorSkill(content, skillName) {
  ...
  const { frontmatter, body } = extractFrontmatterAndBody(converted);
  let description = `Run eCL workflow ${skillName}.`;
  if (frontmatter) {
 ...
 * becomes the command name (e.g. `ecl-help.md` → `/ecl-help`).
 *
...
## C. Tool Usage
Use these Windsurf tools when executing eCL workflows:
- \`Shell\` for running commands (terminal operations)
- \`StrReplace\` for editing existing files
- \`Read\`, \`Write\`, \`Glob\`, \`Grep\`, \`Task\`, \`WebSearch\`, \`WebFetch\`, \`TodoWrite\` as needed
...
## C. Tool Usage
Use these Augment tools when executing eCL workflows:
...
function convertSlashCommandsToTraeSkillMentions(content) {
  return content.replace(/\/ecl:([a-z0-9-]+)/g, (_, commandName) => {
    return `/ecl-${commandName}`;
  });
}
...
function convertClaudeCommandToCodebuddySkill(content, skillName) {
  ...
  // avoiding a duplicated /ecl-* entry per workflow.
 ...
 * command name (ecl-help.md → /ecl-help), so the Claude-specific `name: ecl:<x>`
 ...
 * @param {string} commandName  installed command name (e.g. 'ecl-help')
 * @returns {string}
 */
function convertClaudeCommandToCodebuddyCommand(content, commandName) {
  const converted = convertClaudeToCodebuddyMarkdown(content);
  const { frontmatter, body } = extractFrontmatterAndBody(converted);
  let description = `Run eCL workflow ${commandName}.`;
  let argumentHint = '';
  if (frontmatter) {
    const maybeDescription = extractFrontmatterField(frontmatter, 'description');
...
function convertClaudeCommandToClineSkill(content, skillName, runtime = null, cmdNames = null) {
  ...
  // Hyphen-normalise /ecl:<cmd> → ecl-<cmd> references in the body, then
  ...
  if (!description) description = `Run eCL workflow ${skillName}.`;
  description = toSingleLine(description);
...
function convertSlashCommandsToCodexSkillMentions(content) {
  // Colon-style /ecl: never appears as a filesystem path segment, so no boundary guard is needed (unlike the hyphen-style below).
  let converted = content.replace(/\/ecl:([a-z0-9-]+)/gi, (_, commandName) => {
    return `$ecl-${String(commandName).toLowerCase()}`;
  });
  // Convert hyphen-style command references (workflow output) to Codex $ prefix.
  // A real /ecl-<cmd> MENTION is defined positively by two boundaries, so any
  ...
  //      delimiter (backtick/quote/paren/bracket) — e.g. `/ecl-execute-phase`.
  //   2. Right boundary: the command token is NOT followed by a path separator
  //      `/` (a path continues: `/evolv-coder-lite/bin/...`; a command does not). The
  //      `(?![a-z0-9/-])` also blocks regex backtracking to a shorter command.
  // This converts backtick-wrapped MENTIONS (`/ecl-foo`) while leaving backtick-
  // wrapped PATHS (`/evolv-coder-lite/workflows/update.md`) untouched (#712).
  converted = converted.replace(/(?<=^|[\s`"'([])\/ecl-([a-z0-9-]+)(?![a-z0-9/-])/gi, (_, commandName) => {
    return `$ecl-${String(commandName).toLowerCase()}`;
  });
  return converted;
}
...
- Treat all user text after \`${invocation}\` as \`{{ECL_ARGS}}\`.
- If no arguments are present, treat \`{{ECL_ARGS}}\` as empty.

## B. AskUserQuestion → request_user_input Mapping
eCL workflows use \`AskUserQuestion\` (Claude Code syntax). Translate to Codex \`request_user_input\`:
...
eCL workflows use \`Task(...)\` (Claude Code syntax). Translate to Codex collaboration tools:
...
- \`Task(model="...")\` → omit. \`spawn_agent\` has no inline \`model\` parameter;
  ...
  \`~/.ecl/defaults.json\` are honored automatically by Codex's agent router.
- Resolved \`reasoning_effort="low|medium|high|xhigh"\` (\`xhigh\` is a eCL/Codex tier, not a generic runtime enum) → pass \`reasoning_effort\`
  to \`spawn_agent\` when the runtime/tool supports it. Omit missing, empty,
  inherited, or unsupported values; do not invent one-off effort literals in
  workflow prose.
- \`fork_context: false\` by default — eCL agents load their own context via \`<files_to_read>\` blocks
...
function generateCodexAgentToml(agentName, agentContent, modelOverrides = null, runtimeResolver = null, effortCfg = null) {
  ...
  const resolvedDescription = toSingleLine(
    extractFrontmatterField(frontmatterText, 'description') || `eCL agent ${resolvedName}`
  );
  ...
  // Embed model override when configured in ~/.ecl/defaults.json so that
 ...
 *   display_name: short human-readable skill name (strip ecl- prefix)
 *   short_description: 1-2 sentence description for TUI chip, ≤180 chars
 *
 * @param {string} skillName - Full skill name e.g. "ecl-plan-phase"
...
function generateCodexSkillMetadataYaml(skillName, shortDescription) {
  // Display name: strip "ecl-" prefix and convert hyphens to spaces for readability.
  const displayName = skillName.replace(/^ecl-/, '').replace(/-/g, ' ');
...
/**
 * Write agents/openai.yaml TUI chip metadata for each ecl-* skill directory.
 *
 * Called after layout-driven skill install for Codex. Iterates every ecl-*
 ...
 *
 * User-owned skill directories (e.g. ecl-dev-preferences) are explicitly
...
function writeCodexSkillMetadataFiles(skillsDir) {
  ...
  // never be overwritten by eCL's install path.
  const _userOwnedSkillDirs = new Set(['ecl-dev-preferences']);
  for (const entry of fs.readdirSync(skillsDir, { withFileTypes: true })) {
    if (!entry.isDirectory() || !entry.name.startsWith('ecl-')) continue;
    ...
    try {
      ...
      if (!shortDesc) {
        shortDesc = `Run eCL workflow ${entry.name}.`;
      }
...
/**
 * Generate the eCL config block for Codex config.toml.
...
/**
 * Strip any managed eCL agent sections from a TOML string.
 *
 * Used by the uninstall path (`stripGsdFromCodexConfig`). Removes only what eCL
 * owns; user-authored `[agents.<name>]` and `[[agents]]` entries are preserved
 * so uninstall returns the file to its pre-eCL shape.
 *
 * Handles BOTH shapes so reinstall self-heals configs from all eCL versions:
 *   - Current (#2727): `[agents.ecl-*]` struct tables (Codex 0.120.0+).
 *   - Legacy (#2645): `[[agents]]` array-of-tables whose `name = "ecl-*"`.
 *
...
function stripCodexGsdAgentSections(content) {
  ...
  const sections = getTomlTableSections(content).filter((section) => {
    // Current `[agents.ecl-<name>]` struct tables (#2727, Codex 0.120.0+).
    if (!section.array && /^agents\.ecl-/.test(section.path)) {
      return true;
    }

    // Legacy `[[agents]]` array-of-tables (#2645) — only strip blocks whose
    // `name = "ecl-..."`, preserving user-authored [[agents]] entries.
    if (section.array && section.path === 'agents') {
      ...
      return Boolean(nameMatch && /^ecl-/.test(nameMatch[1]));
    }
...
function stripGsdFromCodexConfig(content) {
  ...
  // No marker but may have eCL-injected feature keys
  let cleaned = content;
  ...
  // Remove [agents.ecl-*] sections (from header to next section or EOF)
...
function getManagedCodexHooksOwnership(content) {
  ...
  const afterMarker = content.slice(markerIndex + ECL_CODEX_MARKER.length);
  const match = afterMarker.match(/^\r?\n# eCL codex_hooks ownership: (section|root_dotted)\r?\n/);
...
function setManagedCodexHooksOwnership(content, ownership) {
  ...
  const eol = detectLineEnding(content);
  const markerEnd = markerIndex + ECL_CODEX_MARKER.length;
  ...
  return content.slice(0, markerEnd) +
    eol +
    `${ECL_CODEX_HOOKS_OWNERSHIP_PREFIX}${ownership}${eol}` +
    remainder;
}
...
function stripLeakedGsdCodexSections(content) {
  ...
  // managed `ecl-*` names, and the legacy bare-table and sequence forms
  ...
  // purges them regardless of eCL marker presence. Users who had legitimate
  ...
  const leakedSections = getTomlTableSections(content)
    .filter((section) => {
      // Legacy [agents.ecl-<name>] map tables (pre-#2645).
      if (!section.array && section.path.startsWith('agents.ecl-')) return true;
      ...
      // `name = "ecl-..."` which preserved user-authored entries that are
...
/**
 * Strip eCL-managed legacy Codex hook blocks from a config.toml string
 ...
 * `command = "...ecl-(check-update|update-check).js"` value, remove its
 ...
 * The leading `# eCL Hooks` header line is swallowed by extending the
 ...
 * if no eCL-managed hook section is present.
 */
function stripStaleGsdHookBlocks(configContent) {
  ...
  // A section is eCL-managed if any structural `command` key inside its
  // body parses to a string whose basename matches `ecl-(check-update|
  ...
  // preceding line is the eCL marker comment `# eCL Hooks` (and is not part
  ...
  for (const section of stale) {
    ...
    if (prev && !prev.startsInMultilineString && prev.text.trim() === '# eCL Hooks') {
      start = prev.start;
    }
...
function migrateCodexHooksMapFormat(content) {
  ...
  // Insert flat-AoT conversions before the eCL managed marker (if present) so
  ...
  if (flatAotBlocks.length > 0) {
    ...
    const markerIdx = result.indexOf(ECL_CODEX_MARKER);
    if (markerIdx !== -1) {
 ...
 * the eCL-managed hook block must be emitted in the same shape so it
 ...
 *
 * Supports the value forms eCL emits or that real Codex configs commonly use:
  ...
  if (ch === '"') {
    ...
    while (j < text.length) {
      ...
      if (c === '\\') {
        ...
        if (next === '"') { out += '"'; j += 2; continue; }
        if (next === '/') { out += '/'; j += 2; continue; }
        // Pass-through unrecognized escape (Codex/eCL don't use these).
...
/**
 * Merge eCL config block into an existing or new config.toml.
 * Three cases: new file, existing with eCL marker, existing without marker.
 *
...
function mergeCodexConfig(configPath, gsdBlock) {
  ...
  const markerIndex = existing.indexOf(ECL_CODEX_MARKER);

  // Case 2: Has eCL marker — truncate and re-append
  if (markerIndex !== -1) {
    let before = existing.substring(0, markerIndex).trimEnd();
    if (before) {
      // Strip any eCL-managed sections that leaked above the marker from previous installs
  ...
  // Case 3: No marker — append eCL block
...
/**
 * Repair config.toml files corrupted by pre-#1346 eCL installs.
...
/**
 * Merge eCL instructions into copilot-instructions.md.
...
function mergeCopilotInstructions(filePath, gsdContent) {
  const gsdBlock = ECL_COPILOT_INSTRUCTIONS_MARKER + '\n' +
    gsdContent.trim() + '\n' +
  ...
  const openIndex = existing.indexOf(ECL_COPILOT_INSTRUCTIONS_MARKER);
  const closeIndex = existing.indexOf(ECL_COPILOT_INSTRUCTIONS_CLOSE_MARKER);

  // Case 2: Has eCL markers — replace between markers
  if (openIndex !== -1 && closeIndex !== -1) {
    const before = existing.substring(0, openIndex).trimEnd();
    const after = existing.substring(closeIndex + ECL_COPILOT_INSTRUCTIONS_CLOSE_MARKER.length).trimStart();
    let newContent = '';
...
/**
 * Strip eCL section from copilot-instructions.md content.
 * Returns cleaned content, or null if file should be deleted (was eCL-only).
 * @param {string} content - File content
 * @returns {string|null} - Cleaned content or null if empty
 */
function stripGsdFromCopilotInstructions(content) {
  const openIndex = content.indexOf(ECL_COPILOT_INSTRUCTIONS_MARKER);
  const closeIndex = content.indexOf(ECL_COPILOT_INSTRUCTIONS_CLOSE_MARKER);

  if (openIndex !== -1 && closeIndex !== -1) {
    const before = content.substring(0, openIndex).trimEnd();
    const after = content.substring(closeIndex + ECL_COPILOT_INSTRUCTIONS_CLOSE_MARKER.length).trimStart();
...
// form (`.clinerules/ecl.md`). Sources adjudicated:
//   - https://cline.bot/blog/cline-v3-36-hooks
//   - https://docs.cline.bot/customization/cline-rules

const ECL_AGENTS_MD_MARKER = '<!-- eCL Configuration — managed by evolv-coder-lite installer -->';
const ECL_AGENTS_MD_CLOSE_MARKER = '<!-- End eCL Configuration -->';

/**
 * The eCL instruction body shared by the Cline directory-form rules file and
 * the cross-tool AGENTS.md block. Self-contained — references only the evolv-coder-lite
 * engine layout, not the (separate) #782 Cline skills directory.
 */
function buildClineRulesBody() {
  return [
    '# eCL Core — Git. Ship. Done.',
    '',
    '- eCL workflows live in `evolv-coder-lite/workflows/`. Load the relevant workflow when',
    '  the user runs a `/ecl-*` command.',
    '- eCL agents live in `agents/`. Use the matching agent when spawning subagents.',
    '- eCL tools are at `evolv-coder-lite/bin/ecl-tools.cjs`. Run with `node`.',
    '- Planning artifacts live in `.planning/`. Never edit them outside a eCL workflow.',
    '- Do not apply eCL workflows unless the user explicitly asks for them.',
    '- When a eCL command triggers a deliverable (feature, fix, docs), offer the next',
 ...
 * tool calls that target `.planning/` (eCL-owned artifacts), and otherwise
...
'use strict';
/* eCL-managed Cline PreToolUse hook — evolv-coder-lite issue #787.
  ...
  try {
    ...
    if (isWrite && paths.some(isPlanningPath)) {
      return process.stdout.write(JSON.stringify({
        cancel: true,
        errorMessage:
          'eCL: .planning/ artifacts are managed by eCL workflows. Edit them only through a /ecl-* command, not directly.',
      }));
    }
...
/**
 * Merge the eCL AGENTS.md block into an existing file (or create it), preserving
...
function mergeGsdAgentsMd(filePath, gsdContent) {
  const gsdBlock = ECL_AGENTS_MD_MARKER + '\n' + gsdContent.trim() + '\n' + ECL_AGENTS_MD_CLOSE_MARKER;
  ...
  const existing = fs.readFileSync(filePath, 'utf8');
  const openIndex = existing.indexOf(ECL_AGENTS_MD_MARKER);
  const closeIndex = existing.indexOf(ECL_AGENTS_MD_CLOSE_MARKER);

  if (openIndex !== -1 && closeIndex !== -1) {
    const before = existing.substring(0, openIndex).trimEnd();
    const after = existing.substring(closeIndex + ECL_AGENTS_MD_CLOSE_MARKER.length).trimStart();
    let newContent = '';
    if (before) newContent += before + '\n\n';
    newContent += gsdBlock;
    if (after) newContent += '\n\n' + after;
...
/**
 * Strip the eCL block from AGENTS.md content. Returns null if the file became
 * empty (was eCL-only), the unchanged content if no markers were found, or the
 * cleaned content otherwise.
 */
function stripGsdFromAgentsMd(content) {
  const openIndex = content.indexOf(ECL_AGENTS_MD_MARKER);
  const closeIndex = content.indexOf(ECL_AGENTS_MD_CLOSE_MARKER);
  if (openIndex !== -1 && closeIndex !== -1) {
    const before = content.substring(0, openIndex).trimEnd();
    const after = content.substring(closeIndex + ECL_AGENTS_MD_CLOSE_MARKER.length).trimStart();
    const cleaned = (before + (before && after ? '\n\n' : '') + after).trim();
    if (!cleaned) return null;
    return cleaned + '\n';
  }
...
function writeClineArtifacts(targetDir, isGlobalInstall) {
  ...
  // file is eCL-authored (the installer wrote its full contents with no user
  ...
  // followed (which would write eCL files through the link into an external dir).
  try {
  ...
  fs.writeFileSync(path.join(clinerulesDir, 'ecl.md'), buildClineRulesBody());
  written.push('.clinerules/ecl.md');
  console.log(`  ${green}✓${reset} Wrote .clinerules/ecl.md`);
  ...
  if (isGlobalInstall) {
    try {
      ...
      console.log(`  ${green}✓${reset} Merged eCL instructions into ~/.agents/AGENTS.md`);
    } catch (err) {
...
//
// Cursor v2.4+ supports a hooks.json lifecycle hook system. eCL registers two
...
//
// eCL entries are identified by a top-level `"ecl-managed": true` field on
// each hook entry. Non-eCL entries are preserved. The reconciler is idempotent
...
function buildCursorHookEntry(scriptPath) {
  return {
    ...
    [ECL_CURSOR_HOOK_MARKER]: true,
  };
}

/**
 * Return true if a Cursor hook entry is eCL-managed.
 * Detection: presence of the ECL_CURSOR_HOOK_MARKER sentinel field.
 *
 * @param {object} entry - A hooks array element from hooks.json
 * @returns {boolean}
 */
function isManagedCursorHookEntry(entry) {
  return Boolean(entry && typeof entry === 'object' && entry[ECL_CURSOR_HOOK_MARKER]);
}

/**
 * Reconcile the eCL-managed entries in a Cursor hooks.json file.
 *
 * Supports both known hooks.json shapes:
 *   1) { "version": 1, "hooks": { "sessionStart": [...], "postToolUse": [...] } }
 *   2) { "sessionStart": [...], "postToolUse": [...] }   (no wrapper object)
 *
 * Managed entries (those with ECL_CURSOR_HOOK_MARKER) are removed then
...
function reconcileCursorHooksJson(hooksJsonPath, managedEntries) {
  ...
  // eCL always writes (and migrates to) the nested shape so Cursor reads it correctly.
  ...
  for (const event of MANAGED_EVENTS) {
    ...
    // Strip all prior eCL-managed entries for this event.
...
/**
 * #777 — Write eCL-managed Cursor lifecycle hooks into <targetDir>/hooks.json.
 *
 * Both managed hook scripts (ecl-cursor-session-start.js, ecl-cursor-post-tool.js)
 * are copied from the eCL hooks/ source to <targetDir>/hooks/ first, so the
 ...
 * @param {string} src       - The eCL install source root (for copying hook scripts)
...
function writeCursorHooksJson(targetDir, src, opts) {
  ...
  // Copy the two eCL-managed hook scripts from the eCL source hooks/ directory.
  // Apply the same /ecl:/gi → ecl- rewrite used by copyWithPathReplacement for Cursor
  // JS files, so the installed hook scripts contain no /ecl: colon refs (bug-376 2b).
  ...
  const hookScripts = [ECL_CURSOR_SESSION_HOOK_SCRIPT, ECL_CURSOR_POST_TOOL_HOOK_SCRIPT];
  ...
  for (const script of hookScripts) {
    ...
    if (fs.existsSync(srcPath)) {
      ...
      // Rewrite /ecl:<cmd> → ecl-<cmd> so installed hook scripts are consistent
  ...
  // buildHookCommand('ecl-cursor-session-start.js', ...): sessionStart → context injection
  // Only register the hook entry if the script was actually installed (dangling guard).
  const sessionStartCmd = installedScripts.has('ecl-cursor-session-start.js')
    ? buildHookCommand(targetDir, 'ecl-cursor-session-start.js', hookOpts)
    : null;
  // buildHookCommand('ecl-cursor-post-tool.js', ...): postToolUse → STATE.md update monitor
  const postToolCmd = installedScripts.has('ecl-cursor-post-tool.js')
    ? buildHookCommand(targetDir, 'ecl-cursor-post-tool.js', hookOpts)
    : null;
...
/**
 * Remove all eCL-managed Cursor lifecycle hook entries from hooks.json.
...
/**
 * #786 — Build the eCL-managed GitHub Copilot lifecycle hook config object.
 *
 ...
 * the agent toward the project's eCL planning state at session start.
 *
  ...
  return {
    ...
    hooks: {
      sessionStart: [
        {
          type: 'command',
          bash: ECL_COPILOT_SESSION_HOOK_BASH,
          powershell: ECL_COPILOT_SESSION_HOOK_PWSH,
          timeoutSec: 10,
        },
      ],
    },
...
/**
 * #786 — Write the eCL-managed Copilot lifecycle hook config under the runtime
 * config dir (`<targetDir>/hooks/ecl-session.json`). For local installs
 * targetDir is `.github` (→ `.github/hooks/`); for global installs it is
 * `~/.copilot` (→ `~/.copilot/hooks/`) — both are valid Copilot hook locations.
 *
 * The managed file is fully owned by eCL, so it is overwritten wholesale on
...
function writeCopilotHookConfig(targetDir) {
  const hooksDir = path.join(targetDir, 'hooks');
  fs.mkdirSync(hooksDir, { recursive: true });
  const hookPath = path.join(hooksDir, ECL_COPILOT_HOOK_FILE);
...
function installCodexConfig(targetDir, agentsSrc) {
  ...
  const agentEntries = fs.readdirSync(agentsSrc).filter(f => f.startsWith('ecl-') && f.endsWith('.md'));
  const agents = [];

  // Compute the Codex eCL install path (absolute, so subagents with empty $HOME work — #820)
  const codexGsdPath = `${path.resolve(targetDir, 'evolv-coder-lite').replace(/\\/g, '/')}/`;

  for (const file of agentEntries) {
    let content = fs.readFileSync(path.join(agentsSrc, file), 'utf8');
    // Replace full .claude/evolv-coder-lite prefix so path resolves to the Codex
    // eCL install before generic .claude → .codex conversion rewrites it.
    content = content.replace(/~\/\.claude\/evolv-coder-lite\//g, codexGsdPath);
    content = content.replace(/\$HOME\/\.claude\/evolv-coder-lite\//g, codexGsdPath);
    ...
    // $ARGUMENTS → {{ECL_ARGS}}, /clear removal, anchored and bare .claude/
    ...
    // `~/.ecl/defaults.json` (project wins on conflict) so Codex TOML files
    ...
    // per-project override the reporter had set for ecl-codebase-mapper.
    ...
    // ~/.ecl/defaults.json — without this, the PR's headline claim that
...
/**
 * Get the list of known eCL commands from the source directory.
 ...
 * to prevent. The warning is gated on ECL_TEST_MODE to keep test output clean.
...
function getGsdCommandRoster() {
  ...
  const gsdSrc = path.join(baseDir, '..', 'commands', 'ecl');
  if (fs.existsSync(gsdSrc)) {
    ...
    _gsdCommandRoster = new Set();
    if (!_gsdCommandRosterWarned && !process.env.ECL_TEST_MODE) {
      _gsdCommandRosterWarned = true;
      console.warn(
        `WARNING: eCL command roster not found at ${gsdSrc}. ` +
        `Gemini /ecl- → /ecl: conversion will be a no-op. ` +
        `This usually means the package was installed without commands/ecl/.`
      );
    }
  }
...
// Test-only: reset the cached roster. Exported via ECL_TEST_MODE bundle below.
...
function convertSlashCommandsToGeminiMentions(content) {
  ...
  //
  // - Lookbehind `(?<![A-Za-z0-9./])` rejects URLs (`example.com/ecl-…`),
  //   sub-paths (`bin/ecl-…`), and root-relative file paths preceded by a
  //   path char. Without it the roster alone is insufficient: a URL like
  //   `https://example.com/ecl-plan-phase` ends in a known command name and
  //   would convert incorrectly.
  // - `(?!\/)` rejects sub-path continuation (`/ecl-foo/bar`).
  // - `(?!\.[a-z])` rejects file extensions (`.cjs`, `.md`) but PERMITS
  //   sentence-ending punctuation like `/ecl-help.` because `.` at end of
  ...
  //   `ecl-planner` (no leading slash anyway) and unknown tokens pass through.
  //
  // eCL commands are always lowercase, so no case-insensitive flag.
  return content.replace(/(?<![A-Za-z0-9./])\/ecl-([a-z0-9-]+)(?!\/)(?!\.[a-z])/g, (match, commandName) => {
    return commands.has(commandName) ? `/ecl:${commandName}` : match;
  });
}
...
function convertClaudeToGeminiAgent(content) {
  ...
  // when they can't be resolved. eCL agents use ${PHASE}, ${PLAN}, etc. as
...
function convertClaudeToOpencodeFrontmatter(content, { isAgent = false, modelOverride = null } = {}) {
  ...
  // Replace /ecl-command colon variant with /ecl-command for opencode (flat command structure)
  convertedContent = convertedContent.replace(/\/ecl:/g, '/ecl-');
  ...
  if (isAgent) {
    ...
    // Embed model override from ~/.ecl/defaults.json so model_overrides is
...
function convertClaudeToKiloFrontmatter(content, { isAgent = false } = {}) {
  ...
  convertedContent = convertedContent.replace(/\bSlashCommand\b/g, 'skill');
  convertedContent = convertedContent.replace(/\bTodoWrite\b/g, 'todowrite');
  // Replace /ecl-command colon variant with /ecl-command for Kilo (flat command structure)
  convertedContent = convertedContent.replace(/\/ecl:/g, '/ecl-');
 ...
 * path, and `/ecl:`→`/ecl-` body rewrites, then rebuilds a minimal skill
 ...
 * @param {string} skillName - Skill directory name (e.g. ecl-help)
...
function convertClaudeToGeminiToml(content, { commandName = null } = {}) {
  ...
  // `progress` command (eCL's status/dashboard surface). Inject the live
 ...
 * OpenCode expects: command/ecl-help.md (invoked as /ecl-help)
 * Source structure: commands/ecl/help.md
 * 
 * @param {string} srcDir - Source directory (e.g., commands/ecl/)
 * @param {string} destDir - Destination directory (e.g., command/)
 * @param {string} prefix - Prefix for filenames (e.g., 'ecl')
...
function copyFlattenedCommands(srcDir, destDir, prefix, pathPrefix, runtime) {
  ...
  // Remove old ecl-*.md files before copying new ones
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      // e.g., commands/ecl/debug/start.md -> command/ecl-debug-start.md
      copyFlattenedCommands(srcPath, destDir, `${prefix}-${entry.name}`, pathPrefix, runtime);
    } else if (entry.name.endsWith('.md')) {
      // Flatten: help.md -> ecl-help.md
...
function listCodexSkillNames(skillsDir, prefix = 'ecl-') {
  if (!fs.existsSync(skillsDir)) return [];
 ...
 * @param {string} prefix          skill name prefix without trailing dash (e.g. 'ecl')
 ...
 * Claude Code 2.1.88+ uses skills/xxx/SKILL.md instead of commands/ecl/xxx.md.
 ...
 * @param {string} prefix - Skill name prefix (e.g. 'ecl')
 * @param {string} pathPrefix - Path prefix for file references
...
/**
 * Write the Hermes "ecl" category DESCRIPTION.md.
 ...
 * reach for that category. Per spec in #2841 we collapse all 86 eCL commands
 * under a single "ecl" category to keep system-prompt overhead bounded.
 */
function writeHermesCategoryDescription(categoryDir) {
  ...
  const body = [
    ...
    'name: ecl',
    `version: ${pkg.version}`,
    'description: eCL Core — Git. Ship. Done. Disciplined planning, execution, and shipping workflows. Use any ecl-* skill in this category to drive a project through new-project → discuss-phase → plan-phase → execute-phase → ship.',
    '---',
    '',
    '# eCL Core',
    '',
    'eCL is a structured development workflow. Skills in this category cover',
    'project initialization, phase planning, execution, code review, and shipping.',
    '',
    'Invoke any `ecl-*` skill in this category to drive the corresponding step.',
    '',
  ].join('\n');
...
/**
 * Recursively install eCL commands as Antigravity skills.
...
/**
 * Single source of truth for user-owned artifacts inside evolv-coder-lite/.
 *
 * These files are created/refreshed by user-facing workflows (e.g.
 * /ecl-profile-user) and must be preserved across reinstalls. Critically, they
 * MUST be excluded from ecl-file-manifest.json — otherwise saveLocalPatches()
 * will compare a refreshed file against a stale manifest hash and emit a
 * spurious "locally modified eCL file" warning (bug #2771).
 *
 ...
 *
 * Paths are relative to the evolv-coder-lite/ directory.
 */
...
/**
 * Migrate a legacy dev-preferences.md (saved from commands/ecl/) into the
 * runtime-aware SKILL.md location used by the writer after #2973.
 *
 * For runtimes with a nested skills layout (e.g. Hermes: skills/ecl/<stem>/),
 * the target is <configDir>/skills/ecl/dev-preferences/SKILL.md.
 * For runtimes with a flat skills layout (prefix='ecl-'), the target is
 * <configDir>/skills/ecl-dev-preferences/SKILL.md.
 *
...
function migrateLegacyDevPreferencesToSkill(targetDir, saved, runtime, scope = 'global') {
  ...
  if (runtime) {
    ...
    const stemName = skillsKindEntry.prefix === '' ? 'dev-preferences' : 'ecl-dev-preferences';
    ...
    skillDir = path.join(targetDir, 'skills', 'ecl-dev-preferences');
  }
...
function applyRuntimeContentRewritesForCommandsInPlace(stagedDir, runtime, pathPrefix) {
  ...
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-cmd-rewrites-'));
  try {
 ...
 *     encodes the eCL namespace as its last segment (e.g. `commands/ecl`), in
 *     which case write as `${stem}.md` (directory IS the namespace).
 *   - agents: write as-is (files already carry their own `ecl-` prefix).
 */
function _copyStaged(stagedDir, destDir, kind) {
  ...
  if (kind.kind === 'skills') {
    // Each child of stagedDir is a prefixed skill directory: ecl-help/, etc.
    for (const entry of fs.readdirSync(stagedDir, { withFileTypes: true })) {
  ...
  // represents the eCL namespace (e.g. 'commands/ecl' → last segment 'ecl').
  ...
  for (const entry of entries) {
    ...
    let destName;
    if (kind.kind === 'agents') {
      // Agent files already carry the ecl- prefix in the source dir
      destName = entry.name;
...
/**
 * Remove eCL-prefixed entries from destDir matching kind.prefix.
 * For Hermes nested case (prefix === ''): the destSubpath IS the namespace
 * (skills/ecl) — remove the entire destDir.
 */
function _removeGsdEntries(destDir, kind) {
  if (!fs.existsSync(destDir)) return;
  if (kind.prefix === '') {
    // Whole-namespace removal (Hermes nested case — destSubpath is skills/ecl)
    // The directory itself is the eCL namespace, so remove it entirely.
 ...
 *
 * - Claude/Qwen/Hermes: migrate legacy commands/ecl/dev-preferences.md →
 *   skills/ecl-dev-preferences/SKILL.md if the old file is present.
 *   Also removes the legacy commands/ecl/ directory.
 * - Hermes: remove flat skills/ecl-STAR directories (pre-2841 layout) before
 *   writing the new nested skills/ecl/ layout.
 *
 * @param {string} runtime
...
function _runLegacyInstallMigrations(runtime, configDir, scope = 'global') {
  const legacyCommandsGsd = path.join(configDir, 'commands', 'ecl');

  // Claude / Qwen / Hermes: clean up legacy commands/ecl/ and preserve dev-preferences
  // for migration. The actual migration call is deferred to after all layout cleanup so
  // that for Hermes the flat skills/ecl-*/ removal (below) does not delete the freshly
  // created skills/ecl-dev-preferences/ skill dir.
  ...
  // Hermes: remove pre-#2841 flat skills/ecl-*/ entries that lived alongside
  // the new skills/ecl/ nested layout.
  if (runtime === 'hermes') {
    ...
    if (fs.existsSync(flatSkillsDir)) {
      for (const entry of fs.readdirSync(flatSkillsDir, { withFileTypes: true })) {
        if (entry.isDirectory() && entry.name.startsWith('ecl-')) {
          fs.rmSync(path.join(flatSkillsDir, entry.name), { recursive: true });
        }
      }
    }

    // Hermes: remove intermediate-layout skills/ecl/ecl-*/ entries that existed
    // between #2841 and #3664. Phase 2 (#3664) uses prefix='' producing bare-stem
    // names (skills/ecl/<stem>/SKILL.md); the intermediate layout had the ecl-
    // prefix inside the nested dir (skills/ecl/ecl-<stem>/SKILL.md). Only
    // children whose name starts with ecl- are removed — the parent skills/ecl/
    // directory and any non-ecl- siblings (user content) are preserved.
 ...
 *
 * - Claude global/Qwen: remove legacy commands/ecl/ directory if present.
 *   For Claude LOCAL, commands/ecl/ is the current primary location (not
 *   legacy), so we skip removal here and let _removeGsdEntries handle it
 *   with ecl- prefix filtering (preserving user files like dev-preferences.md).
 * - Hermes: remove pre-2841 flat skills/ecl-STAR entries.
 *
 * @param {string} runtime
 * @param {string} configDir  resolved runtime config directory
 * @param {'global'|'local'} [scope]
...
function _runLegacyUninstallCleanup(runtime, configDir, scope = 'global') {
  // Claude global / Qwen: commands/ecl/ is a legacy location (global Claude
  // uses skills/ now; Qwen always uses skills/). Remove whole directory.
  // Claude local: commands/ecl/ is the primary current location — skip here,
  // let layout's _removeGsdEntries handle ecl-prefixed file removal.
  // #2973 / Codex review (bd1f06c9): preserve user-owned dev-preferences.md
  // before destructive wipe. Migration to skills/ecl-dev-preferences/SKILL.md
  // is deferred and returned so the caller can apply it AFTER layout-driven
  // removal — this prevents the layout's ecl-* prefix removal from wiping the
  // freshly created skill dir (same pattern as _runLegacyInstallMigrations).
  let savedLegacyArtifacts = null;
  // commands/ecl/ is a legacy location for Qwen, Hermes, and Claude-global.
  // Claude-local commands/ecl/ is the primary current location — skip here.
  ...
  // Hermes: pre-#2841 flat skills/ecl-*/ entries
  if (runtime === 'hermes') {
    const flatSkillsDir = path.join(configDir, 'skills');
    if (fs.existsSync(flatSkillsDir)) {
      for (const entry of fs.readdirSync(flatSkillsDir, { withFileTypes: true })) {
...
function installRuntimeArtifacts(runtime, configDir, scope, resolvedProfile) {
  ...
  for (const kind of layout.kinds) {
    ...
    if (kind.kind === 'skills' && fs.existsSync(dest)) {
      ...
      // install (#2973 / #3664).
      //
      // For prefix='' (Hermes): _removeGsdEntries wipes the entire dest dir (skills/ecl/).
      // Preserve every subdir that is NOT in the staged set — those are user-added dirs
      // (e.g. user-content/) that eCL does not manage.
      //
      // For prefix='ecl-' (others): _removeGsdEntries removes only ecl-* entries.
      // Non-ecl-* user dirs (e.g. my-custom-skill/) are untouched. Only preserve the
      // explicit user-owned eCL-prefixed skill ecl-dev-preferences, which eCL does not
      ...
      if (kind.prefix === '') {
        ...
        // Non-Hermes: only preserve explicitly user-owned eCL-prefixed skill dirs.
        // ecl-dev-preferences is the sole user-customisable skill in this category.
        const USER_OWNED_SKILL_DIRS = ['ecl-dev-preferences'];
      ...
      // just prune stale ecl-* entries and copy new ones.
      _removeGsdEntries(dest, kind);
      _copyStaged(stagedForCopy, dest, kind);
    }
  }
 ...
 * @returns {number} number of ecl-* skill directories written
 */
function installOpencodeFamilySkills(runtime, targetDir, rawCommandsDir, pathPrefix) {
  ...
  // Preserve user-owned eCL-prefixed skill dirs across the ecl-* prune.
  // ecl-dev-preferences is generated by the user (via generate-dev-preferences)
  // and lives at <configDir>/skills/ecl-dev-preferences — _removeGsdEntries
 ...
 * determine which eCL-owned entries to remove.
 *
 * @param {string} runtime             canonical runtime ID
 * @param {string} configDir           resolved runtime config directory
 * @param {'global'|'local'} scope
...
function uninstallRuntimeArtifacts(runtime, configDir, scope) {
  ...
  // removing Claude local commands/ecl/ which is the primary install dir).
  // Returns saved user artifacts so we can migrate AFTER layout removal
  // (the layout's ecl-* prefix pass would wipe a skill dir created here).
  ...
  // complete. Do NOT restore to commands/ecl/ — the user is uninstalling.
  if (savedLegacyArtifacts) {
    migrateLegacyDevPreferencesToSkill(configDir, savedLegacyArtifacts, runtime, scope);
  }
}
...
function copyWithPathReplacement(srcDir, destDir, pathPrefix, runtime, isCommand = false, isGlobal = false) {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      // #3683 — normalize /ecl:<cmd> → /ecl-<cmd> in any body passing through
      ...
      jsContent = jsContent.replace(/ecl:/gi, 'ecl-');
      ...
      let jsContent = fs.readFileSync(srcPath, 'utf8');
      jsContent = jsContent.replace(/\/ecl:([a-z0-9-]+)/g, (_, commandName) => {
        return `/ecl-${commandName}`;
      });
...
function cleanupOrphanedHooks(settings) {
  const orphanedHookPatterns = [
    'ecl-notify.sh',  // Removed in v1.6.x
    'hooks/statusline.js',  // Renamed to ecl-statusline.js in v1.9.0
    'ecl-intel-index.js',  // Removed in v1.9.2
    'ecl-intel-session.js',  // Removed in v1.9.2
    'ecl-intel-prune.js',  // Removed in v1.9.2
  ];
  ...
  // Fix #330: Update statusLine if it points to old eCL statusline.js path
  // Only match the specific old eCL path pattern (hooks/statusline.js),
  ...
  if (settings.statusLine && settings.statusLine.command &&
    ...
    console.log(`  ${green}✓${reset} Updated statusline path (hooks/statusline.js → hooks/ecl-statusline.js)`);
  }
...
function uninstall(isGlobal, runtime = 'claude') {
  ...
  console.log(`  Uninstalling eCL from ${cyan}${runtimeLabel}${reset} at ${cyan}${locationLabel}${reset}\n`);
  ...
  if (isCopilot && !isGlobal) {
    ...
    if (fs.existsSync(agentsMdPath)) {
      ...
      if (cleaned === null) {
        ...
        console.log(`  ${green}✓${reset} Removed AGENTS.md (was eCL-only)`);
      } else if (cleaned !== content) {
        fs.writeFileSync(agentsMdPath, cleaned);
        console.log(`  ${green}✓${reset} Cleaned eCL section from AGENTS.md`);
      }
    }
  }
  ...
  try {
    fs.unlinkSync(path.join(targetDir, '.ecl-profile'));
    removedCount++;
  } catch {}

  // 1. Remove eCL commands/skills (layout-driven)
  ...
  if (isCodex) {
    ...
    if (fs.existsSync(codexAgentsDir)) {
      ...
      for (const file of tomlFiles) {
        if (file.startsWith('ecl-') && file.endsWith('.toml')) {
    ...
    // Codex: clean eCL sections from config.toml
    ...
    if (fs.existsSync(codexConfigPath)) {
      ...
      if (cleaned === null) {
        ...
        removedCount++;
        console.log(`  ${green}✓${reset} Removed config.toml (was eCL-only)`);
      } else if (cleaned !== content) {
        fs.writeFileSync(codexConfigPath, cleaned);
        removedCount++;
        console.log(`  ${green}✓${reset} Cleaned eCL sections from config.toml`);
      }
    }
  ...
  if (isCopilot) {
    ...
    if (fs.existsSync(instructionsPath)) {
      ...
      if (cleaned === null) {
        ...
        removedCount++;
        console.log(`  ${green}✓${reset} Removed copilot-instructions.md (was eCL-only)`);
      } else if (cleaned !== content) {
        fs.writeFileSync(instructionsPath, cleaned);
        removedCount++;
        console.log(`  ${green}✓${reset} Cleaned eCL section from copilot-instructions.md`);
      }
    }

    // #786: remove the eCL-managed Copilot lifecycle hook config and prune the
    // hooks dir if we left it empty.
    const hookPath = path.join(targetDir, 'hooks', ECL_COPILOT_HOOK_FILE);
    if (fs.existsSync(hookPath)) {
      fs.unlinkSync(hookPath);
      removedCount++;
      console.log(`  ${green}✓${reset} Removed Copilot lifecycle hook (${ECL_COPILOT_HOOK_FILE})`);
      try {
        const hooksDir = path.join(targetDir, 'hooks');
  ...
  // directory-form rules + PreToolUse hook, and strip the eCL block from the
  // global cross-tool ~/.agents/AGENTS.md target.
  if (runtime === 'cline') {
    const clinerulesDir = path.join(targetDir, '.clinerules');
    for (const rel of ['ecl.md', path.join('hooks', 'PreToolUse')]) {
    ...
    // Prune now-empty eCL-created directories (leave any user-added rule files).
      ...
      try {
        if (fs.existsSync(agentsPath)) {
          ...
          if (cleaned === null) {
            ...
            removedCount++;
            console.log(`  ${green}✓${reset} Removed ~/.agents/AGENTS.md (was eCL-only)`);
          } else if (cleaned !== content) {
            fs.writeFileSync(agentsPath, cleaned);
            removedCount++;
            console.log(`  ${green}✓${reset} Cleaned eCL section from ~/.agents/AGENTS.md`);
          }
        }
      } catch { /* best-effort */ }
    }
  ...
  // 1b-cursor. Non-layout Cursor side-effects (issue #777): remove eCL-managed
  // hook entries from hooks.json and clean up the managed hook scripts.
  if (isCursor) {
    const hooksJsonCleanup = removeCursorHooksJson(targetDir);
    if (hooksJsonCleanup.changed) {
      removedCount++;
      console.log(`  ${green}✓${reset} Removed eCL-managed Cursor hooks from hooks.json`);
    }
    // Remove the managed hook scripts (session-start + post-tool).
    const hooksDir = path.join(targetDir, 'hooks');
    for (const script of [ECL_CURSOR_SESSION_HOOK_SCRIPT, ECL_CURSOR_POST_TOOL_HOOK_SCRIPT]) {
  ...
  // 1c. Claude local: remove commands/ecl/ (primary local install location).
  //     The layout's _removeGsdEntries uses the 'ecl-' prefix which applies to
  ...
  if (!isGlobal && runtime === 'claude') {
    const gsdCommandsDir = path.join(targetDir, 'commands', 'ecl');
    if (fs.existsSync(gsdCommandsDir)) {
      ...
      removedCount++;
      console.log(`  ${green}✓${reset} Removed commands/ecl/`);
      if (preservedDevPrefs) {
        try {
          fs.mkdirSync(gsdCommandsDir, { recursive: true });
          fs.writeFileSync(devPrefsPath, preservedDevPrefs);
          console.log(`  ${green}✓${reset} Preserved commands/ecl/dev-preferences.md`);
        } catch (err) {
  ...
  // 1d. Gemini: remove commands/ecl/ with dev-preferences.md preservation.
  //     The layout removes ecl-*.toml files but not the directory itself.
  ...
  // 1d. Qwen/Hermes: migrate dev-preferences.md from legacy commands/ecl/ location
  ...
  if (isQwen || isHermes) {
    ...
    if (fs.existsSync(legacyDir)) {
      ...
      if (migrateLegacyDevPreferencesToSkill(targetDir, savedLegacyArtifacts, runtime, _uninstallScope)) {
        ...
        const _stem = _sk && _sk.prefix === '' ? 'dev-preferences' : 'ecl-dev-preferences';
        const _skillRelPath = _sk ? `${_sk.destSubpath}/${_stem}/SKILL.md` : 'skills/ecl-dev-preferences/SKILL.md';
  ...
  // 2. Remove evolv-coder-lite directory
  const gsdDir = path.join(targetDir, 'evolv-coder-lite');
  if (fs.existsSync(gsdDir)) {
    ...
    removedCount++;
    console.log(`  ${green}✓${reset} Removed evolv-coder-lite/`);
    ...
    if (preservedProfile) {
      try {
        ...
        console.log(`  ${green}✓${reset} Preserved evolv-coder-lite/USER-PROFILE.md`);
      } catch (err) {
  ...
  // 3. Remove eCL agents (ecl-*.md files only)
  const agentsDir = path.join(targetDir, 'agents');
  if (fs.existsSync(agentsDir)) {
    const files = fs.readdirSync(agentsDir);
    let agentCount = 0;
    for (const file of files) {
      if (file.startsWith('ecl-') && file.endsWith('.md')) {
    ...
    if (agentCount > 0) {
      removedCount++;
      console.log(`  ${green}✓${reset} Removed ${agentCount} eCL agents`);
    }
  }
  ...
  if (fs.existsSync(hooksDir)) {
    ...
    if (hookCount > 0) {
      removedCount++;
      console.log(`  ${green}✓${reset} Removed ${hookCount} eCL hooks`);
    }

    // Remove only the eCL-managed files from hooks/lib/ (git-cmd.js + ecl-graphify-rebuild.sh).
    ...
    if (fs.existsSync(hooksLibDir)) {
      ...
      for (const file of ECL_HOOK_LIB_FILES) {
  ...
  // 5. Remove eCL package.json (CommonJS mode marker)
  ...
  if (fs.existsSync(pkgJsonPath)) {
    try {
      ...
      if (content === '{"type":"commonjs"}') {
        ...
        removedCount++;
        console.log(`  ${green}✓${reset} Removed eCL package.json`);
      }
    } catch (e) {
      // Ignore read errors
    }
  ...
  // 6. Clean up settings.json (remove eCL hooks and statusline)
  ...
  if (fs.existsSync(settingsPath)) {
    ...
    // Remove eCL statusline if it references our hook
    if (settings.statusLine && settings.statusLine.command &&
      settings.statusLine.command.includes('ecl-statusline')) {
      delete settings.statusLine;
      settingsModified = true;
      console.log(`  ${green}✓${reset} Removed eCL statusline from settings`);
    }

    // Remove eCL hooks from settings — per-hook granularity to preserve
    // user hooks that share an entry with a eCL hook (#1755 followup).
      ...
      if (settings.hooks && settings.hooks[eventName]) {
        ...
        settings.hooks[eventName] = settings.hooks[eventName]
          .map(entry => {
            ...
            // Filter out individual eCL hooks, keep user hooks
    ...
    if (settingsModified) {
      console.log(`  ${green}✓${reset} Removed eCL hooks from settings`);
    }
    ...
    // #768 — Remove eCL-owned Claude permissions from settings.json.
    // Applies only to Claude uninstalls. Filter only the exact eCL-owned entries
    // to preserve any user-added allow/deny entries.
    // Uses a local flag to avoid the shared `settingsModified` producing a false
    // "Removed eCL permissions" message when only hooks/statusline changed.
    if (runtime === 'claude' && settings.permissions) {
      let permissionsModified = false;
      if (Array.isArray(settings.permissions.allow)) {
        const before = settings.permissions.allow.length;
        settings.permissions.allow = settings.permissions.allow.filter(
          (e) => !ECL_CLAUDE_ALLOW_PERMISSIONS.includes(e)
        );
      ...
      if (Array.isArray(settings.permissions.deny)) {
        ...
        settings.permissions.deny = settings.permissions.deny.filter(
          (e) => !ECL_CLAUDE_DENY_PERMISSIONS.includes(e)
        );
      ...
      if (permissionsModified) {
        settingsModified = true;
        console.log(`  ${green}✓${reset} Removed eCL permissions from settings.json`);
      }
    }
  ...
  if (isOpencode) {
    ...
    if (fs.existsSync(configPath)) {
      try {
        ...
        // Remove eCL permission entries
        if (config.permission) {
          for (const permType of ['read', 'external_directory']) {
            if (config.permission[permType]) {
              const keys = Object.keys(config.permission[permType]);
              for (const key of keys) {
                if (key.includes('evolv-coder-lite')) {
        ...
        if (modified) {
          ...
          removedCount++;
          console.log(`  ${green}✓${reset} Removed eCL permissions from ${path.basename(configPath)}`);
        }
      } catch (e) {
  ...
  console.log(`
  ${green}Done!${reset} eCL has been uninstalled from ${runtimeLabel}.
...
/**
 * Configure OpenCode permissions to allow reading eCL reference docs
 * This prevents permission prompts when eCL accesses the evolv-coder-lite directory
...
function configureOpencodePermissions(isGlobal = true, configDir = null) {
  ...
  // Build the eCL path using the actual config directory
  // Use ~ shorthand if it's in the default location, otherwise use full path
  const defaultConfigDir = path.join(os.homedir(), '.config', 'opencode');
  const gsdPath = opencodeConfigDir === defaultConfigDir
    ? '~/.config/opencode/evolv-coder-lite/*'
    : `${opencodeConfigDir.replace(/\\/g, '/')}/evolv-coder-lite/*`;
  ...
  // Write config back
  fs.writeFileSync(configPath, JSON.stringify(config, null, 2) + '\n');
  console.log(`  ${green}✓${reset} Configured read permission for eCL docs`);
}

/**
 * Configure Kilo permissions to allow reading eCL reference docs
 * This prevents permission prompts when eCL accesses the evolv-coder-lite directory
 * @param {boolean} isGlobal - Whether this is a global or local install
 * @param {string|null} configDir - Resolved config directory when already known
 */
function configureKiloPermissions(isGlobal = true, configDir = null) {
  ...
  const gsdPath = kiloConfigDir === defaultConfigDir
    ? '~/.config/kilo/evolv-coder-lite/*'
    : `${kiloConfigDir.replace(/\\/g, '/')}/evolv-coder-lite/*`;
...
const PATCHES_DIR_NAME = 'ecl-local-patches';
const MANIFEST_NAME = 'ecl-file-manifest.json';
...
function writeManifest(configDir, runtime = 'claude', options = {}) {
  ...
  const isCline = runtime === 'cline';
  const isHermes = runtime === 'hermes';
  const gsdDir = path.join(configDir, 'evolv-coder-lite');
  const commandsDir = path.join(configDir, 'commands', 'ecl');
  const opencodeCommandDir = path.join(configDir, 'command');
  // Hermes nests eCL skills under skills/ecl/ as a single category (#2841).
  // All other runtimes that use the Codex-style skills layout use a flat skills/ root.
  const codexSkillsDir = isHermes
    ? path.join(configDir, 'skills', 'ecl')
    : path.join(configDir, 'skills');
  const codexSkillsManifestPrefix = isHermes ? 'skills/ecl/' : 'skills/';
  ...
  for (const [rel, hash] of Object.entries(gsdHashes)) {
    ...
    manifest.files['evolv-coder-lite/' + rel] = hash;
  }
  // Record commands/ecl/ for any runtime that emits it (Gemini globally,
  ...
  if (fs.existsSync(commandsDir)) {
    ...
    for (const [rel, hash] of Object.entries(cmdHashes)) {
      manifest.files['commands/ecl/' + rel] = hash;
    }
  }
  ...
  if ((isCodex || isCopilot || isAntigravity || isCursor || isWindsurf || isTrae || (!isOpencode && !isGemini)) && fs.existsSync(codexSkillsDir)) {
    // Hermes uses prefix '' (bare stem names); all others use 'ecl-'
    const skillListPrefix = isHermes ? '' : 'ecl-';
    ...
    if (isHermes) {
      ...
      if (fs.existsSync(descPath)) {
        manifest.files['skills/ecl/DESCRIPTION.md'] = fileHash(descPath);
      }
    }
  }
  if (fs.existsSync(agentsDir)) {
    for (const file of fs.readdirSync(agentsDir)) {
      if (file.startsWith('ecl-') && (file.endsWith('.md') || file.endsWith('.toml'))) {
  ...
  if (isCline) {
    for (const rel of ['.clinerules/ecl.md', '.clinerules/hooks/PreToolUse']) {
  ...
  if (!isCodex && !isCopilot && !isCline) {
    ...
    if (fs.existsSync(hooksDir)) {
      for (const file of fs.readdirSync(hooksDir)) {
        if (file.startsWith('ecl-') && (file.endsWith('.js') || file.endsWith('.sh'))) {
      ...
      // to git-cmd.js (validate-commit classifier) and ecl-graphify-rebuild.sh.
      const hooksLibDir = path.join(hooksDir, 'lib');
      if (fs.existsSync(hooksLibDir)) {
        for (const file of fs.readdirSync(hooksLibDir)) {
          if (ECL_HOOK_LIB_FILES.includes(file)) {
...
/**
 * Populate ecl-pristine/ with the transformed pristine versions of every
 ...
 * /ecl-reapply-patches Step 5 verifier (#2972) uses as the diff base
...
function populatePristineDir({ packageSrc, pristineDir, modified, runtime, pathPrefix, isGlobal }) {
  ...
  // install roots: evolv-coder-lite/, commands/ecl/, command/, skills/, agents/,
  ...
  // path replacement).
  const stageRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-pristine-stage-'));
  let written = 0;
  try {
...
/**
 * Detect user-modified eCL files by comparing against install manifest.
 * Backs up modified files to ecl-local-patches/ for reapply after update.
 * Also saves pristine copies (from manifest) to ecl-pristine/ to enable
 ...
 * needed to populate ecl-pristine/. If omitted (legacy callers), pristine
...
function saveLocalPatches(configDir, pristineCtx) {
  ...
  if (manifest.files) {
    for (const artifact of USER_OWNED_ARTIFACTS) {
      delete manifest.files[`evolv-coder-lite/${artifact}`];
    }
  }

  const patchesDir = path.join(configDir, PATCHES_DIR_NAME);
  const pristineDir = path.join(configDir, 'ecl-pristine');
  const modified = [];
  ...
  // if the user had not modified it" — used by /ecl-reapply-patches Step 5
  ...
  if (modified.length > 0) {
    ...
    console.log('  ' + yellow + 'i' + reset + '  Found ' + modified.length + ' locally modified eCL file(s) — backed up to ' + PATCHES_DIR_NAME + '/');
    for (const f of modified) {
      console.log('     ' + dim + f + reset);
    }

    // #2998 / #3407: maintain ecl-pristine/ as the diff baseline for the
    // reapply-patches verifier (#2972).
    //
    // #3407 root-cause fix: the prior approach (#3004 CR) wiped ecl-pristine/
    ...
    //
    // Correct approach: `ecl-pristine/` is populated lazily by saveLocalPatches'
    ...
    //
    // Per-file decision:
    //   - sha256(ecl-pristine/X) === originalHash  →  correct; keep it
    //   - ecl-pristine/X exists but hash mismatch  →  stale from a previous
    //     buggy run (#3407); remove so verifier falls back cleanly
    //   - ecl-pristine/X absent                    →  attempt hash-validated
    ...
    if (pristineCtx) {
      ...
      for (const relPath of modified) {
        ...
        // File absent from ecl-pristine/ (or just removed above as stale):
      ...
      if (missingPaths.length > 0) {
        ...
        try {
          tempPristineDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-pristine-regen-'));
          populatePristineDir({
          ...
          console.warn(`ecl-pristine regen skipped: ${err.message}`);
        } finally {
          if (tempPristineDir) {
      ...
      if (preserved > 0) {
        console.log('  ' + green + '✓' + reset + '  Preserved ' + cyan + 'ecl-pristine/' + reset + ' (' + preserved + ' file(s)) for three-way merge');
      }
      if (regenerated > 0) {
        console.log('  ' + green + '✓' + reset + '  Regenerated ' + cyan + 'ecl-pristine/' + reset + ' (' + regenerated + ' file(s)) via hash-validated new-release source');
      }
      if (removed > 0) {
        console.log('  ' + yellow + 'i' + reset + '  Removed ' + removed + ' stale ecl-pristine/ snapshot(s); regenerated ' + regenerated + ' of those — falls back to over-broad verify heuristic for the rest');
      }
    }
  }
  return modified;
    ...
    const reapplyCommand = (runtime === 'opencode' || runtime === 'kilo' || runtime === 'copilot')
      ...
      : runtime === 'gemini'
        ...
        : runtime === 'codex'
          ? '$ecl-update --reapply'
        : runtime === 'cursor'
          ? 'ecl-update --reapply (mention the skill name)'
          : '/ecl-update --reapply';
    console.log('');
...
function install(isGlobal, runtime = 'claude', options = {}) {
  ...
  // Reusable helper to copy hooks/lib/ (git-cmd.js + ecl-graphify-rebuild.sh).
  ...
  // keeping install scope aligned with ECL_HOOK_LIB_FILES (which uninstall/manifest manage).
  ...
  // ecl update marker re-application (ADR-0010 Deviation 2):
  ...
  const _commandsDir = path.join(src, 'commands', 'ecl');
  ...
  // Save any locally modified eCL files before they get wiped.
  // The pristine context lets saveLocalPatches populate ecl-pristine/ via
  ...
  // directories and files eCL will mutate so that any post-install validation
  ...
  // reflect the true pre-eCL state. Non-Codex runtimes skip this block.
  //
  // Snapshot contents:
  //   codexPreInstallSkillNames  — Set of ecl-* skill dir names that existed
  //   codexPreInstallSkillContents — Map<skillName, Map<relPath, Buffer>> of
  //       the full file tree of each pre-existing ecl-* skill dir, so that
  //       overwritten dirs can be fully restored on rollback (not just removed).
  //   codexPreInstallAgentFiles  — Set of ecl-*.{md,toml} filenames in agents/
  ...
  //   codexPreInstallVersionBytes — Buffer (or null) of evolv-coder-lite/VERSION
  //
  ...
  // pre-existing ecl-* skill directory. Best-effort: read errors are silently
  ...
  // Map<filename, Buffer> — content snapshot of each pre-existing ecl-* agent file.
  ...
  if (isCodex && !isMinimalMode(_effectiveInstallMode)) {
    ...
    const _preVersionPath = path.join(targetDir, 'evolv-coder-lite', 'VERSION');
  ...
  // config.toml is touched: skills/, agents/, evolv-coder-lite/VERSION, and orphaned
  ...
  const _codexPreConfigRollback = !isCodex || isMinimalMode(_effectiveInstallMode) ? null : () => {
    ...
    // skills/ecl-* — pass 1: restore snapshot entries (may be absent if deleted mid-install).
    ...
    // skills/ecl-* — pass 2: remove any newly-created dirs not in the snapshot.
    if (fs.existsSync(_earlySkillsDir)) {
      try {
        for (const entry of fs.readdirSync(_earlySkillsDir, { withFileTypes: true })) {
          if (entry.isDirectory() && entry.name.startsWith('ecl-') && !codexPreInstallSkillNames.has(entry.name)) {
    ...
    // agents/ecl-* — pass 1: restore snapshot entries.
    ...
    // agents/ecl-* — pass 2: remove any newly-created files not in the snapshot.
    if (fs.existsSync(_earlyAgentsDir)) {
      try {
        for (const file of fs.readdirSync(_earlyAgentsDir)) {
          if (file.startsWith('ecl-') && (file.endsWith('.md') || file.endsWith('.toml')) && !codexPreInstallAgentFiles.has(file)) {
            try { fs.unlinkSync(path.join(_earlyAgentsDir, file)); } catch (_) { /* best-effort */ }
          }
        }
      } catch (_) { /* best-effort */ }
    }
    // evolv-coder-lite/VERSION
    const _earlyVersionPath = path.join(targetDir, 'evolv-coder-lite', 'VERSION');
  ...
  const priorInstallExisted = fs.existsSync(path.join(targetDir, 'evolv-coder-lite', 'VERSION'));
  try {
  ...
  // #3541: non-interactive runs (typical /ecl-update via Claude Code) have
  ...
  // artifacts → remove; user-facing skills → keep; bundled eCL hooks →
  ...
  // For unambiguous categories (e.g. `hooks/ecl-*` bundled hooks left
  // behind by a previous version), there is no actual "user choice" to
  // make — the file is a known eCL-managed artifact and the installer is
  // about to write the fresh bundled version. Gating the resolver on
  // `!isTTY` made `npx @evolvconsulting/evolv-coder-lite@latest --codex` hard-abort with
  // 12 blocked bundled hooks. The env-override branch (operator-supplied
  // ECL_INSTALLER_MIGRATION_RESOLVE) still applies only in non-TTY mode.
  ...
  if (_isSkillsRuntime) {
    ...
    // installed skill so the /skills popup shows name + description for each ecl-* skill.
    ...
    // Hermes only: write DESCRIPTION.md for the ecl/ category after layout install
    if (isHermes) {
      writeHermesCategoryDescription(path.join(targetDir, 'skills', 'ecl'));
    }

    // Verify installed artifacts and report
    if (isHermes) {
      const hermesSkillsDir = path.join(targetDir, 'skills', 'ecl');
      if (fs.existsSync(hermesSkillsDir)) {
        // Hermes layout uses prefix: '' — skill dirs have bare stem names (no ecl- prefix)
        const count = fs.readdirSync(hermesSkillsDir, { withFileTypes: true })
          .filter(e => e.isDirectory()).length;
        if (count > 0) {
          console.log(`  ${green}✓${reset} Installed ${count} skills to skills/ecl/`);
        } else {
          failures.push('skills/ecl/*');
        }
      } else {
        failures.push('skills/ecl/*');
      }
    ...
    // Copy commands/ecl/*.md as command/ecl-*.md (flatten structure)
    const gsdSrc = _stageSkills(_commandsDir);
    copyFlattenedCommands(gsdSrc, commandDir, 'ecl', pathPrefix, runtime);
    if (verifyInstalled(commandDir, 'command/ecl-*')) {
      const count = fs.readdirSync(commandDir).filter(f => f.startsWith('ecl-')).length;
      console.log(`  ${green}✓${reset} Installed ${count} commands to command/`);
    } else {
      failures.push('command/ecl-*');
    }
    ...
    // #3037: when running --local --gemini and a eCL-managed user-scope
    // command directory already exists at ~/.gemini/commands/ecl/, skip
    // the local copy. Gemini conflict-detects by command name across
    // scopes and renames every overlapping /ecl:* command to
    // /workspace.ecl:* and /user.ecl:*, breaking the documented namespace.
    ...
    // eCL content, not just "directory is non-empty". A user who hand-
    // dropped a single override (e.g. ~/.gemini/commands/ecl/my-override
    // .toml) would otherwise be unable to run a local install at all.
    // Detection rule: at least 3 of the canonical eCL command files
    // ('help.toml', 'progress.toml', 'new-project.toml') must be present.
    // These three ship in every eCL Gemini install (minimal mode included
    ...
    // produce by accident.
    const homeGeminiGsd = path.join(os.homedir(), '.gemini', 'commands', 'ecl');
    const ECL_MANAGED_CANARIES = ['help.toml', 'progress.toml', 'new-project.toml'];
    const userScopeHasGsd =
      !isGlobal &&
      path.resolve(targetDir) !== path.resolve(path.join(os.homedir(), '.gemini')) &&
      fs.existsSync(homeGeminiGsd) &&
      ECL_MANAGED_CANARIES.every((f) =>
        fs.existsSync(path.join(homeGeminiGsd, f))
      );

    if (userScopeHasGsd) {
      console.log(
        `  ${yellow}⚠${reset}  Skipping commands/ecl/ for local install — eCL is already installed at user scope (${homeGeminiGsd}).`
      );
      console.log(
        `      Gemini conflict-detects across scopes and would rename every /ecl:* command to /workspace.ecl:* and /user.ecl:*.`
      );
      console.log(
        `      The user-scope install already provides /ecl:* commands in this project; no local copy is needed.`
      );
    } else {
      const commandsDir = path.join(targetDir, 'commands');
      fs.mkdirSync(commandsDir, { recursive: true });
      const gsdSrc = _stageSkills(_commandsDir);
      const gsdDest = path.join(commandsDir, 'ecl');
      copyWithPathReplacement(gsdSrc, gsdDest, pathPrefix, runtime, true, isGlobal);
      if (verifyInstalled(gsdDest, 'commands/ecl')) {
        console.log(`  ${green}✓${reset} Installed commands/ecl`);
      } else {
        failures.push('commands/ecl');
      }
    }
  } else {
    // Claude Code local: commands/ecl/ format — Claude Code reads local project
    // commands from .claude/commands/ecl/, not .claude/skills/
    const commandsDir = path.join(targetDir, 'commands');
    fs.mkdirSync(commandsDir, { recursive: true });
    const gsdSrc = _stageSkills(_commandsDir);
    const gsdDest = path.join(commandsDir, 'ecl');
    ...
    if (verifyInstalled(gsdDest, 'commands/ecl')) {
      ...
      console.log(`  ${green}✓${reset} Installed ${count} commands to commands/ecl/`);
    } else {
      failures.push('commands/ecl');
    }
    ...
    if (fs.existsSync(staleSkillsDir)) {
      ...
      if (staleGsd.length > 0) {
        console.log(`  ${green}✓${reset} Removed ${staleGsd.length} stale eCL skill(s) from skills/`);
      }
    }
  }

  // Copy evolv-coder-lite skill with path replacement
  // Preserve user-generated files before the wipe-and-copy so they survive re-install
  const skillSrc = path.join(src, 'evolv-coder-lite');
  const skillDest = path.join(targetDir, 'evolv-coder-lite');
  ...
  if (verifyInstalled(skillDest, 'evolv-coder-lite')) {
  ...
  // Skipped under --minimal: ecl-* subagent descriptions are eagerly loaded
  // into the runtime's Agent tool schema, costing ~6k tokens per turn even
  // when no eCL workflow is active. See evolvconsulting/evolv-coder-lite#2762.
  ...
  // Always remove stale ecl-* agents first so re-installing with
  ...
  if (isMinimalMode(_effectiveInstallMode)) {
    // Codex registers agents in `config.toml` via `[agents.ecl-*]` sections.
    ...
    console.log(`  ${dim}↳${reset} Skipping agents (minimal install — run \`ecl update\` without \`--minimal\` to add full surface)`);
  ...
  const changelogDest = path.join(targetDir, 'evolv-coder-lite', 'CHANGELOG.md');
  ...
  // Write VERSION file
  const versionDest = path.join(targetDir, 'evolv-coder-lite', 'VERSION');
  ...
  if (!isCodex && !isCopilot && !isCursor && !isWindsurf && !isTrae && !isCline) {
    // Write package.json to force CommonJS mode for eCL scripts
    ...
    if (fs.existsSync(hooksSrc)) {
      ...
      for (const entry of hookEntries) {
        ...
        if (fs.statSync(srcFile).isFile()) {
          ...
          // hooks/dist/lib/ecl-graphify-rebuild.sh and must land at the
      ...
      if (verifyInstalled(hooksDest, 'hooks')) {
        ...
        const expectedShHooks = ['ecl-session-state.sh', 'ecl-validate-commit.sh', 'ecl-phase-boundary.sh', 'ecl-graphify-update.sh'];
  ...
  if (!isCodex && !isCopilot && !isCursor && !isWindsurf && !isTrae && !isCline && fs.existsSync(hooksLibSrc)) {
    ...
    copyLibDir(hooksLibSrc, hooksLibDest, ECL_HOOK_LIB_FILES);
    console.log(`  ${green}✓${reset} Installed hooks/lib/ helpers (git-cmd, graphify-rebuild, ...)`);
  }

  // Remove legacy evolv-coder-lite-cc artifacts and stale update caches (#607).
  ...
  if (configIntent.installSurface === 'codex-toml' && !isMinimalMode(_effectiveInstallMode)) {
    // Capture pre-install snapshots before ANY eCL mutation
    ...
    //   skills/ecl-* — restore pre-existing dirs from content snapshot; remove
    //                   newly-created dirs (i.e. those not in the pre-install Set)
    //   agents/ecl-* — restore pre-existing files from content snapshot; remove
    //                   newly-created files
    //   evolv-coder-lite/VERSION — restore or remove
    ...
    // default to empty Set / null). Does NOT touch non-ecl-* user content.
    const restoreCodexSnapshot = () => {
      ...
      //     (copyCommandsAsCodexSkills removes pre-existing ecl-* dirs before re-writing)
      ...
      // Pass 2 — remove any newly-created ecl-* dirs (not in the pre-install snapshot).
      ...
      // Pass 2 — remove any newly-created ecl-* agent files (not in the pre-install snapshot).
      if (fs.existsSync(_rollbackAgentsDir)) {
        try {
          for (const file of fs.readdirSync(_rollbackAgentsDir)) {
            if (!file.startsWith('ecl-') || (!file.endsWith('.md') && !file.endsWith('.toml'))) continue;
      ...
      // 4. evolv-coder-lite/VERSION
      const _rollbackVersionPath = path.join(targetDir, 'evolv-coder-lite', 'VERSION');
      if (codexPreInstallVersionBytes !== null) {
    ...
    if (!isMinimalMode(_effectiveInstallMode)) {
      ...
      // not include agents/ecl-*.toml because those files did not yet exist.
      writeManifest(targetDir, runtime, { mode: _effectiveInstallMode });
    } else {
    ...
    // #772: added ecl-context-monitor.js for the new SubagentStart/Stop/PostToolUse events.
    // We deliberately do *not* copy ecl-graphify-update.sh or hooks/lib/ for Codex
    // in this change (graphify auto-update support for Codex is out of scope for #3579).
    const CODEX_HOOKS_TO_COPY = ['ecl-check-update.js', 'ecl-context-monitor.js'];
    ...
    if (fs.existsSync(codexHooksSrc)) {
      ...
      for (const entry of fs.readdirSync(codexHooksSrc)) {
        ...
        if (entry.endsWith('.js')) {
          ...
          // #2136: any .sh hook reaching this loop must have {{ECL_VERSION}}
          ...
          // extended later (e.g. to ship ecl-graphify-update.sh for Codex).
          let content = fs.readFileSync(srcFile, 'utf8');
          content = content.replace(/\{\{ECL_VERSION\}\}/g, pkg.version);
          fs.writeFileSync(destFile, content);
          try { fs.chmodSync(destFile, 0o755); } catch (e) { /* Windows */ }
    ...
    // restore returns the file to its true pre-eCL state on validation
    ...
    try {
      ...
      // Strip ALL prior eCL-managed hook blocks BEFORE migration so the migration
      // only touches user-authored hooks, not eCL-owned stale entries. Running
      // strip after migration causes Shape 1 (legacy ecl-update-check filename)
      // to be converted by migration before the strip regex can match it (#2698).
      //
      // Historical shapes stripped, in order:
      //   Shape 1 — legacy ecl-update-check filename (pre-#1755): flat [[hooks]] + event
      ...
      // namespaced [[hooks.<EVENT>]] form after stripping eCL-managed stale blocks.
      // Running migration after strip ensures only user-authored hooks are migrated
      // (#2698 regression: migration before strip converts stale eCL blocks before
      ...
      // eCL-managed Codex hook payloads now live in hooks.json to avoid mixed
      ...
      if (hasEnabledCodexHooksFeature(configContent)) {
        const checkUpdateFile = path.join(targetDir, 'hooks', 'ecl-check-update.js');
        if (!fs.existsSync(checkUpdateFile)) {
          console.warn(`  ${yellow}⚠${reset}  Skipped Codex SessionStart hook registration — ecl-check-update.js not found at target`);
        ...
        // ecl-context-monitor.js so context-headroom warnings surface at:
        ...
        //
        // Note: UserPromptSubmit is NOT wired — ecl-prompt-guard exits unless
        ...
        const contextMonitorFile = path.join(targetDir, 'hooks', 'ecl-context-monitor.js');
  ...
  if (configIntent.installSurface === 'copilot-instructions') {
    ...
    const templatePath = path.join(targetDir, 'evolv-coder-lite', 'templates', 'copilot-instructions.md');
    const instructionsPath = path.join(targetDir, 'copilot-instructions.md');
  ...
  if (configIntent.installSurface === 'cline-rules') {
    // Cline uses the `.clinerules/` directory form (issue #787): eCL rules live
    // at .clinerules/ecl.md and a PreToolUse lifecycle hook at
  ...
  // #3002 CR: rewrite legacy `node .../ecl-*.js` command strings carried over
  ...
  const updateCheckCommand = isGlobal
    ? buildHookCommand(targetDir, 'ecl-check-update.js', hookOpts)
    : localCmd('ecl-check-update.js');
  const contextMonitorCommand = isGlobal
    ? buildHookCommand(targetDir, 'ecl-context-monitor.js', hookOpts)
    : localCmd('ecl-context-monitor.js');
  const promptGuardCommand = isGlobal
    ? buildHookCommand(targetDir, 'ecl-prompt-guard.js', hookOpts)
    : localCmd('ecl-prompt-guard.js');
  const readGuardCommand = isGlobal
    ? buildHookCommand(targetDir, 'ecl-read-guard.js', hookOpts)
    : localCmd('ecl-read-guard.js');
  const readInjectionScannerCommand = isGlobal
    ? buildHookCommand(targetDir, 'ecl-read-injection-scanner.js', hookOpts)
    : localCmd('ecl-read-injection-scanner.js');
  const configReloadCommand = isGlobal
    ? buildHookCommand(targetDir, 'ecl-config-reload.js', hookOpts)
    : localCmd('ecl-config-reload.js');
  ...
  if (!isOpencode && !isKilo) {
    ...
    const hasGsdUpdateHook = settings.hooks.SessionStart.some(entry =>
      entry.hooks && entry.hooks.some(h => h.command && h.command.includes('ecl-check-update'))
    );
    ...
    if (!hasGsdUpdateHook && fs.existsSync(checkUpdateFile) && updateCheckCommand) {
      ...
      console.warn(`  ${yellow}⚠${reset}  Skipped update check hook — ecl-check-update.js not found at target`);
    }
    ...
    const hasContextMonitorHook = settings.hooks[postToolEvent].some(entry =>
      entry.hooks && entry.hooks.some(h => h.command && h.command.includes('ecl-context-monitor'))
    );
    ...
    if (!hasContextMonitorHook && fs.existsSync(contextMonitorFile) && contextMonitorCommand) {
      ...
      console.warn(`  ${yellow}⚠${reset}  Skipped context monitor hook — ecl-context-monitor.js not found at target`);
    } else {
      // Migrate existing context monitor hooks: add matcher and timeout if missing
      for (const entry of settings.hooks[postToolEvent]) {
        if (entry.hooks && entry.hooks.some(h => h.command && h.command.includes('ecl-context-monitor'))) {
          let migrated = false;
          if (!entry.matcher) {
          ...
          for (const h of entry.hooks) {
            if (h.command && h.command.includes('ecl-context-monitor') && !h.timeout) {
              h.timeout = 10;
              migrated = true;
            }
          }
    ...
    const hasPromptGuardHook = settings.hooks[preToolEvent].some(entry =>
      entry.hooks && entry.hooks.some(h => h.command && h.command.includes('ecl-prompt-guard'))
    );

    const promptGuardFile = path.join(targetDir, 'hooks', 'ecl-prompt-guard.js');
    if (!hasPromptGuardHook && fs.existsSync(promptGuardFile) && promptGuardCommand) {
      ...
      console.warn(`  ${yellow}⚠${reset}  Skipped prompt guard hook — ecl-prompt-guard.js not found at target`);
    }
    ...
    const hasReadGuardHook = settings.hooks[preToolEvent].some(entry =>
      entry.hooks && entry.hooks.some(h => h.command && h.command.includes('ecl-read-guard'))
    );

    const readGuardFile = path.join(targetDir, 'hooks', 'ecl-read-guard.js');
    if (!hasReadGuardHook && fs.existsSync(readGuardFile) && readGuardCommand) {
      ...
      console.warn(`  ${yellow}⚠${reset}  Skipped read guard hook — ecl-read-guard.js not found at target`);
    }
    ...
    const hasReadInjectionScannerHook = settings.hooks[postToolEvent].some(entry =>
      entry.hooks && entry.hooks.some(h => h.command && h.command.includes('ecl-read-injection-scanner'))
    );

    const readInjectionScannerFile = path.join(targetDir, 'hooks', 'ecl-read-injection-scanner.js');
    if (!hasReadInjectionScannerHook && fs.existsSync(readInjectionScannerFile) && readInjectionScannerCommand) {
      ...
      console.warn(`  ${yellow}⚠${reset}  Skipped read injection scanner hook — ecl-read-injection-scanner.js not found at target`);
    }
    ...
    if (hasWorkflowGuardHook && workflowGuardHookEntry.matcher !== workflowGuardMatcher) {
      ...
      console.warn(`  ${yellow}⚠${reset}  Skipped workflow guard hook — ecl-workflow-guard.js not found at target`);
    }
    ...
    const worktreePathGuardCommand = isGlobal
      ? buildHookCommand(targetDir, 'ecl-worktree-path-guard.js', hookOpts)
      : localCmd('ecl-worktree-path-guard.js');
    const hasWorktreePathGuardHook = settings.hooks[preToolEvent].some(entry =>
      entry.hooks && entry.hooks.some(h => h.command && h.command.includes('ecl-worktree-path-guard'))
    );
    const worktreePathGuardFile = path.join(targetDir, 'hooks', 'ecl-worktree-path-guard.js');
    if (!hasWorktreePathGuardHook && fs.existsSync(worktreePathGuardFile) && worktreePathGuardCommand) {
      ...
      console.warn(`  ${yellow}⚠${reset}  Skipped worktree path guard hook — ecl-worktree-path-guard.js not found at target`);
    }

    // Configure commit validation hook (Conventional Commits enforcement, opt-in)
    const validateCommitCommand = isGlobal
      ? buildHookCommand(targetDir, 'ecl-validate-commit.sh', hookOpts)
      : localShellCmd('ecl-validate-commit.sh');
    const hasValidateCommitHook = settings.hooks[preToolEvent].some(entry =>
      entry.hooks && entry.hooks.some(h => h.command && h.command.includes('ecl-validate-commit'))
    );
    ...
    const validateCommitFile = path.join(targetDir, 'hooks', 'ecl-validate-commit.sh');
    if (!hasValidateCommitHook && fs.existsSync(validateCommitFile) && validateCommitCommand) {
      ...
      console.warn(`  ${yellow}⚠${reset}  Skipped commit validation hook — ecl-validate-commit.sh not found at target`);
    ...
    const graphifyUpdateCommand = isGlobal
      ? buildHookCommand(targetDir, 'ecl-graphify-update.sh', hookOpts)
      : localShellCmd('ecl-graphify-update.sh');
    const hasGraphifyUpdateHook = settings.hooks[postToolEvent].some(entry =>
      entry.hooks && entry.hooks.some(h => h.command && h.command.includes('ecl-graphify-update'))
    );
    const graphifyUpdateFile = path.join(targetDir, 'hooks', 'ecl-graphify-update.sh');
    if (!hasGraphifyUpdateHook && fs.existsSync(graphifyUpdateFile) && graphifyUpdateCommand) {
      ...
      console.warn(`  ${yellow}⚠${reset}  Skipped graphify auto-update hook — ecl-graphify-update.sh not found at target`);
    ...
    const sessionStateCommand = isGlobal
      ? buildHookCommand(targetDir, 'ecl-session-state.sh', hookOpts)
      : localShellCmd('ecl-session-state.sh');
    const hasSessionStateHook = settings.hooks.SessionStart.some(entry =>
      entry.hooks && entry.hooks.some(h => h.command && h.command.includes('ecl-session-state'))
    );
    const sessionStateFile = path.join(targetDir, 'hooks', 'ecl-session-state.sh');
    if (!hasSessionStateHook && fs.existsSync(sessionStateFile) && sessionStateCommand) {
      ...
      console.warn(`  ${yellow}⚠${reset}  Skipped session state hook — ecl-session-state.sh not found at target`);
    ...
    const phaseBoundaryCommand = isGlobal
      ? buildHookCommand(targetDir, 'ecl-phase-boundary.sh', hookOpts)
      : localShellCmd('ecl-phase-boundary.sh');
    const hasPhaseBoundaryHook = settings.hooks[postToolEvent].some(entry =>
      entry.hooks && entry.hooks.some(h => h.command && h.command.includes('ecl-phase-boundary'))
    );
    const phaseBoundaryFile = path.join(targetDir, 'hooks', 'ecl-phase-boundary.sh');
    if (!hasPhaseBoundaryHook && fs.existsSync(phaseBoundaryFile) && phaseBoundaryCommand) {
      ...
      console.warn(`  ${yellow}⚠${reset}  Skipped phase boundary hook — ecl-phase-boundary.sh not found at target`);
    ...
    // three lifecycle events.  Wire ecl-context-monitor so agents get context-
    ...
    // user prompt text, not a tool invocation, so ecl-prompt-guard (which
    ...
    if (isQwen || runtime === 'claude') {
      ...
      for (const event of ['SubagentStop', 'Stop', 'PreCompact']) {
        ...
        if (!alreadyHasContextMonitor && fs.existsSync(contextMonitorFile) && contextMonitorCommand) {
          ...
          console.warn(`  ${yellow}⚠${reset}  Skipped ${event} hook — ecl-context-monitor.js not found at target`);
        }
      }
    }
    ...
    // ecl previously did not register.  Three high-value events are added here:
    //
    //   BeforeAgent  — fires after user submits a prompt, before the agent
    //                  plans.  Wire ecl-context-monitor for context headroom
    //                  awareness at prompt time.
    //   AfterAgent   — fires once per turn after the model generates its final
    //                  response.  Wire ecl-context-monitor to track headroom
    //                  after each agent turn completes.
    //   BeforeModel  — fires before each LLM call (per-turn, not per-session).
    //                  Wire ecl-context-monitor for per-turn context injection
    //                  — more precise than session-start-only injection.
    //
    // All three reuse ecl-context-monitor.js — no new hook files needed.
    // The `decision:"deny"` retry capability of AfterAgent is intentionally
    // left to the hook script to implement when triggered (ecl-context-monitor
    ...
    // ecl hook use case at this time; deferred to a follow-on issue.
    //
    ...
    if (isGemini) {
      for (const geminiEvent of ['BeforeAgent', 'AfterAgent', 'BeforeModel']) {
        ...
        if (!alreadyHasContextMonitor && fs.existsSync(contextMonitorFile) && contextMonitorCommand) {
          ...
          console.warn(`  ${yellow}⚠${reset}  Skipped ${geminiEvent} hook — ecl-context-monitor.js not found at target`);
        }
      }
    }
    // ── end Gemini-only extended hook events ──────────────────────────────────

    // ── FileChanged hook: hot-reload ecl config on .planning/config.json edits ─
    // Claude Code fires FileChanged when a watched file changes on disk.  Wire
    // ecl-config-reload.js to reload the ecl config context whenever the user
    ...
    // when the changed file is not the ecl config.
    //
    ...
    if (runtime === 'claude') {
      ...
      const configReloadFile = path.join(targetDir, 'hooks', 'ecl-config-reload.js');
      const alreadyHasConfigReload = settings.hooks.FileChanged.some(entry =>
        entry.hooks && entry.hooks.some(h => h.command && h.command.includes('ecl-config-reload'))
      );
      if (!alreadyHasConfigReload && fs.existsSync(configReloadFile) && configReloadCommand) {
        ...
        console.warn(`  ${yellow}⚠${reset}  Skipped FileChanged hook — ecl-config-reload.js not found at target`);
  ...
  // disables ALL hook execution — ecl hooks are registered but will never run.
  ...
  if (isGemini && settings && settings.hooksConfig && settings.hooksConfig.enabled === false) {
    console.warn(
      ...
      `     evolv-coder-lite hooks are registered but will NOT run until you set\n` +
  ...
  const updateBannerCommand = isOpencode || isKilo
    ? null
    : (isGlobal
      ? buildHookCommand(targetDir, 'ecl-update-banner.js', hookOpts)
      : localCmd('ecl-update-banner.js'));
...
function finishInstall(settingsPath, settings, statuslineCommand, shouldInstallStatusline, runtime = 'claude', isGlobal = true, configDir = null, bannerOpts = {}) {
  ...
  if (shouldInstallBanner && settings && !isOpencode && !isKilo && !isCodex && !isCopilot && !isCursor && !isWindsurf && !isTrae && !isCline) {
    if (!bannerCommand) {
      ...
      const alreadyRegistered = settings.hooks.SessionStart.some(entry =>
        entry && entry.hooks && entry.hooks.some(h => h && h.command && h.command.includes('ecl-update-banner'))
      );
      const bannerHookFile = configDir ? path.join(configDir, 'hooks', 'ecl-update-banner.js') : null;
      ...
      if (alreadyRegistered) {
        ...
        console.warn(`  ${yellow}⚠${reset}  Skipped update banner — ecl-update-banner.js not found at target`);
      } else {
  ...
  // Merges eCL-owned entries non-destructively (preserves existing user permissions).
  ...
  if (configIntent.finishPermissionWriter === 'opencode' && !process.env.ECL_TEST_MODE) {
  ...
  if (runtime !== 'claude' && !process.env.ECL_TEST_MODE) {
    ...
    try {
      ...
      if (defaults.resolve_model_ids !== 'omit') {
        ...
        console.log(`  ${green}✓${reset} Set resolve_model_ids: "omit" in ~/.ecl/defaults.json`);
      }
    } catch (e) {
      console.log(`  ${yellow}⚠${reset} Could not write ~/.ecl/defaults.json: ${e.message}`);
    }
  }
  ...
  let command = '/ecl-new-project';
  if (runtime === 'opencode') command = '/ecl-new-project';
  if (runtime === 'kilo') command = '/ecl-new-project';
  if (runtime === 'gemini') command = '/ecl:new-project';
  if (runtime === 'codex') command = '$ecl-new-project';
  if (runtime === 'copilot') command = '/ecl-new-project';
  if (runtime === 'antigravity') command = '/ecl-new-project';
  if (runtime === 'cursor') command = 'ecl-new-project (mention the skill name)';
  if (runtime === 'windsurf') command = '/ecl-new-project';
  if (runtime === 'augment') command = '/ecl-new-project';
  if (runtime === 'trae') command = '/ecl-new-project';
  if (runtime === 'cline') command = '/ecl-new-project';
  if (runtime === 'qwen') command = '/ecl-new-project';
  if (runtime === 'hermes') command = '/ecl-new-project';
...
function handleStatusline(settings, isInteractive, callback) {
  ...
  eCL includes a statusline showing:
    • Model name
    • Current task (from todo list)
    • Context window usage (color-coded)

  ${cyan}1${reset}) Keep existing
  ${cyan}2${reset}) Replace with eCL statusline
`);
...
function buildUpdateBannerPromptText() {
  return `
  ${yellow}Optional: eCL update banner${reset}
  Without eCL's statusline, update notifications won't be visible. You can
  install a SessionStart banner that surfaces a one-line message when a new
  eCL release is available. The banner appears only at session start and
 ...
 * update banner. Used by `installAllRuntimes` only when eCL's statusline
...
// Runtime subdir names to scan for legacy evolv-coder-lite-cc artifacts (#607).
...
/**
 * Detect and remove leftover evolv-coder-lite-cc artifacts across ALL known
...
function cleanupLegacyGsdCc({ homeDir = os.homedir(), dryRun = false, logger = console } = {}) {
  ...
  // shared cache (ecl-update-check.json) as a plan entry.
  ...
  const perPkgCacheFile = path.join(homeDir, '.cache', 'ecl', updateCacheFileName);
  if (dryRun) {
...
/**
 * Install eCL for all selected runtimes
 */
...
// The main() block below is gated on !ECL_TEST_MODE, as before.
module.exports = {
    ...
    ECL_CLAUDE_ALLOW_PERMISSIONS,
    ECL_CLAUDE_DENY_PERMISSIONS,
    ECL_CODEX_MARKER,
    CODEX_AGENT_SANDBOX,
    getDirName,
    ...
    writeClineArtifacts,
    mergeGsdAgentsMd,
    ECL_CURSOR_SESSION_HOOK_SCRIPT,
    ECL_CURSOR_POST_TOOL_HOOK_SCRIPT,
    ECL_CURSOR_HOOK_MARKER,
...
if (require.main === module && !process.env.ECL_TEST_MODE) {
  if (hasDryRun) {
    ...
    if (hasUninstall) {
      console.log('Note: --dry-run previews legacy evolv-coder-lite-cc cleanup only; it does not preview --uninstall.');
    }
    ...
    if (plan.length === 0) {
      console.log('  (no legacy evolv-coder-lite-cc artifacts found)');
    }
    process.exit(0);
    ...
    const skillsRoot = runtimeArg === 'hermes'
      ? path.join(globalDir, 'skills', 'ecl')
...
} // end of !ECL_TEST_MODE main logic block
