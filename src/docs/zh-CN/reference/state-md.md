...
`STATE.md` 是 eCL Core 的动态项目记忆文件——一个记录项目当前状态、最近发生的事情以及下一步操作的单一 Markdown 文档。本页面记录其结构。参见[文档索引](../README.md)。

---

## 概述

由 eCL Core 管理的每个项目在 `.planning/STATE.md` 处保存一个 `STATE.md`。该文件在每次工作流开始时被读取，并在每次重要操作后被写入。该文件包含：

- **YAML 前置数据** — 机器可读字段，由状态行钩子（`parseStateMd`）和 `ecl-tools state` 命令使用。
...
前置数据出现在文件最开头的 `---` 分隔符之间。除 `ecl_state_version` 和 `status` 外，所有字段均为可选；当相关数据尚不可用时，字段可以缺失。
...
| 字段 | 类型 | 填充时机 | 用途 |
|---|---|---|---|
| `ecl_state_version` | 字符串（`'1.0'`） | 始终 | 架构版本；在第一次 `state.*` 调用时由 `syncStateFrontmatter` 写入。 |
...
`evolv-coder-lite/bin/lib/state-document.cjs` 中的 `normalizeStateStatus()` 将原始正文文本映射到以下规范值：
...
`hooks/ecl-statusline.js` 中的 `formatGsdState()` 读取已解析的前置数据并输出**第一个匹配的场景**。如果没有新的生命周期字段适用，渲染将回退到与 v1.38.x 完全一致的原始格式。
...
正文（结束 `---` 之后的所有内容）遵循 `evolv-coder-lite/templates/state.md` 中的模板。标准章节为：
...
当现有值为已知模板默认值时，该章节中的 `Status:` 和 `Last activity:` 字段由 eCL 处理器更新（Knuth 不变式：执行器编写的值被保留）。已知处理器默认值的完整列表位于 `evolv-coder-lite/bin/lib/state-document.cjs` 中的 `KNOWN_TEMPLATE_DEFAULTS`。
...
**决策** — 影响当前工作的近期决策摘要（完整日志在 `PROJECT.md` 中）。通过 `ecl-tools state add-decision` 添加。

**待处理的待办事项** — 数量及对 `.planning/todos/pending/` 的引用。通过 `/ecl-capture` 捕获。

**阻碍/关切** — 影响未来工作的问题，以发起阶段为前缀。通过 `ecl-tools state add-blocker` 添加；通过 `ecl-tools state resolve-blocker` 解决。
