# Orquestração Orientada a Issues com o eCL
...
assistida por IA através dos primitivos existentes do eCL.

## O que é este guia

Uma receita para combinar comandos que o eCL já inclui em um loop
...
rastreador — cada comando referenciado abaixo já existe no eCL hoje.
...
([repositório](https://github.com/openai/symphony)). O eCL não vende nem
encapsula o Symphony. Os *conceitos* de orquestração se mapeiam claramente
nos primitivos que o eCL já expõe; este guia apenas descreve esse mapeamento
para que você possa adotar o padrão sem escrever código de integração ou
contornar os controles de segurança do eCL.

## Por que isso existe

O eCL tem os blocos de construção para desenvolvimento de IA orientado a issues —
`/ecl-workspace --new`, `/ecl-manager`, `/ecl-autonomous`, `/ecl-verify-work`,
`/ecl-review`, `/ecl-ship`, além de `STATE.md` e o conjunto de artefatos de fase
...
- Subutilização: desenvolvedores executam discuss/plan/execute manualmente e
  nunca recorrem a `/ecl-manager` ou `/ecl-autonomous`, mesmo quando seu padrão
...
primitivo do eCL que já o serve. Use esta tabela como chave de tradução ao
ler documentações do Symphony, posts de blog ou descrições de orquestração
de terceiros.

| Conceito Symphony | Primitivo eCL |
|---|---|
...
| Um workspace isolado de agente por tarefa | `/ecl-workspace --new --strategy worktree` |
| Despacho e concorrência de agentes | `/ecl-manager` (painel interativo), `/ecl-autonomous` (sem supervisão) |
| Etapas de discussão e planejamento por fase | `/ecl-discuss-phase` → `/ecl-plan-phase` → `/ecl-execute-phase` |
| Prova de trabalho / evidência de testes | `/ecl-verify-work` (UAT.md persistido entre `/clear`) |
| Revisão adversarial | `/ecl-review` (revisão por pares entre IAs do plano) |
| Controle humano de merge | `/ecl-ship` (cria PR, revisão de código opcional, prepara merge) |
| Captura de trabalho subsequente | `/ecl-capture`, `/ecl-capture --seed`, `/ecl-new-milestone`, ou uma issue aberta manualmente no rastreador |
| Controle de concorrência | Semântica de agente gerenciador / segundo plano (sem poller sempre ativo) |

O mapeamento é unidirecional: o eCL é responsável pelos controles de segurança
...
2. **Mapeie para uma fase do eCL.** Se a issue se mapear para uma fase
   existente em `ROADMAP.md`, selecione-a. Caso contrário, execute
   `/ecl-new-milestone` (para um novo marco de issues relacionadas) ou abra
   uma fase via `/ecl-phase` / `/ecl-phase --insert`. Capture a URL da issue
   do rastreador no `CONTEXT.md` da fase para que a rastreabilidade sobreviva
   à compactação.
3. **Crie um workspace isolado.** Execute
   `/ecl-workspace --new --strategy worktree <slug>` para criar uma git
   ...
   ficam fora do `main`.
4. **Execute discuss → plan → execute através do eCL.** De dentro do
   workspace, execute `/ecl-discuss-phase` para esclarecer ambiguidades,
   `/ecl-plan-phase` para produzir `PLAN.md`, e `/ecl-manager`
   (painel interativo) ou `/ecl-execute-phase` / `/ecl-autonomous`
   (sem supervisão) para implementar. Evite conduzir invocações brutas de
   `claude` de fora do eCL — isso contorna as atualizações de `STATE.md`
   e o manifesto de fases.
5. **Exija prova de trabalho.** Execute `/ecl-verify-work` para conduzir o
   ...
   `/ecl-plan-phase --gaps` quando a verificação revela escopo não coberto.
6. **Passe pelos controles de revisão e envio.** Execute `/ecl-review` para
   obter revisão por pares adversarial do plano por IAs independentes (detecta
   pontos cegos modelo a modelo), depois `/ecl-ship` para abrir o PR com um
   ...
   repositório remoto.
7. **Capture trabalho subsequente explicitamente.** Use `/ecl-capture` para
   notas inline, `/ecl-capture --seed` para ideias que valem uma fase futura,
   ou `/ecl-new-milestone` para um grupo coerente de trabalhos subsequentes.
   Criar uma issue no rastreador a partir de um trabalho subsequente
   descoberto requer confirmação explícita do usuário — o eCL não publica em
...
- **Worktrees isoladas.** Cada issue roda em uma worktree de
  `/ecl-workspace --new`, para que trabalho parcial, planos abandonados e
  commits exploratórios nunca toquem o `main`. `ecl-local-patches/` é a
  superfície de recuperação se edições manuais de uma worktree precisarem
  voltar após uma atualização.
- **Revisão humana explícita.** `/ecl-review` e `/ecl-ship` ambos param para
  ...
  fila de merge — não algo que o eCL decide por você.
- **Nenhuma publicação automática.** O eCL nunca abre, comenta ou fecha uma
  ...
  separada.
- **Verificação antes do envio.** O `UAT.md` do `/ecl-verify-work` deve
  registrar evidências antes que `/ecl-ship` seja executado. A disciplina
...
diretamente na worktree, pular `/ecl-verify-work`, ou criar issues via a API
...
- **Sem venda ou cópia do código Symphony.** O eCL reutiliza seus próprios
  primitivos. O mapeamento acima é conceitual; nenhum código derivado do
  Symphony está incluído neste repositório.
- **Sem daemon de longa execução.** O eCL não faz polling no GitHub ou Linear.
...
- **Sem dependência obrigatória de rastreador.** O loop funciona sem qualquer
  ...
  humana* — a URL vai para `CONTEXT.md`. O eCL não tem opinião sobre qual
  rastreador você usa, ou se você usa algum.
- **Sem contorno dos controles de verificação, revisão ou decisão humana.**
  Mesmo ao executar `/ecl-autonomous`, os controles de verificação e revisão
...
- Importar uma issue do GitHub ou Linear para um workspace / fase do eCL.
...
- [Índice de documentação](README.md) — sumário completo da documentação do eCL Core.
- [docs/USER-GUIDE.md](./USER-GUIDE.md) — guias orientados a tarefas dos comandos individuais referenciados acima.
- [docs/COMMANDS.md](COMMANDS.md) — referência completa dos comandos `/ecl-*`.
