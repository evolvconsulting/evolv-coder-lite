...
const TOOLS_PATH = path.join(ROOT, 'evolv-coder-lite', 'bin', 'ecl-tools.cjs');
...
function makeTempPlanning(stateContent) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-397-'));
...
const STATE_EXECUTOR_RESUME_FILE = `# eCL State
...
const STATE_DEFAULT_RESUME_FILE = `# eCL State
...
const STATE_EXECUTOR_STATUS = `# eCL State
...
const STATE_DEFAULT_STATUS = `# eCL State
...
const STATE_EXECUTOR_LAST_ACTIVITY = `# eCL State
...
const STATE_EXECUTOR_CURRENT_POSITION = `# eCL State
