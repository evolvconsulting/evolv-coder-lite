...
const { comparePhaseNum, normalizePhaseName } = require('../evolv-coder-lite/bin/lib/core.cjs');
...
describe('bug #1998: phase complete updates overview checkbox', () => {
  ...
  test('checkbox updated when no archived milestones exist', () => {
    ...
    try {
      execFileSync('node', [ECL_TOOLS_BIN, 'phase', 'complete', '1'], { cwd: tmpDir, timeout: 10000 });
    } catch {
...
describe('bug #2526: phase complete warns about unregistered REQ-IDs', () => {
  ...
  test('emits warning for REQ-IDs in body but missing from Traceability table', () => {
    ...
    let stderr = '';
    try {
      const result = execFileSync('node', [ECL_TOOLS_BIN, 'phase', 'complete', '1'], {
        cwd: tmpDir,
        timeout: 10000,
        encoding: 'utf-8',
      });
