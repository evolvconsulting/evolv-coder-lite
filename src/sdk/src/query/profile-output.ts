 ...
 * Ported from `evolv-coder-lite/bin/lib/profile-output.cjs` (`cmdWriteProfile`,
...
const CLAUDE_MD_FALLBACKS = {
  project: 'Project not yet initialized. Run /ecl-new-project to set up.',
...
function buildSection(sectionName: string, sourceFile: string, content: string): string {
  return [`<!-- eCL:${sectionName}-start source:${sourceFile} -->`, content, `<!-- eCL:${sectionName}-end -->`].join('\n');
}
...
function generateWorkflowSection(): { content: string; source: string; hasFallback: boolean } {
  return { content: CLAUDE_MD_WORKFLOW_ENFORCEMENT, source: 'eCL defaults', hasFallback: false };
}
...
export const generateDevPreferences: QueryHandler = async (args, projectDir) => {
  ...
  if (analysis.data_source === 'questionnaire') {
    stackBlock =
      'Stack preferences not available (questionnaire-only profile). Run `/ecl-profile-user --refresh` with session data to populate.';
  } else if (stackOpt) {
    stackBlock = stackOpt;
  } else {
  ...
  if (!outPath) {
    ...
    const defaultSkillPath = resolveGlobalSkillMarkdownPath(runtime, 'ecl-dev-preferences');
    if (!defaultSkillPath) {
      throw new GSDError(
  ...
  return {
    data: {
      ...
      command_name: '/ecl-dev-preferences',
...
export const generateClaudeProfile: QueryHandler = async (args, projectDir) => {
  ...
  const sectionLines = [
    ...
    '## Developer Profile',
    '',
    `> Generated by eCL from ${dataSource}. Run \`/ecl-profile-user --refresh\` to update.`,
    '',
...
export const generateClaudeMd: QueryHandler = async (args, projectDir) => {
  ...
  if (existingContent === null) {
    ...
    if (!autoFlag && fileContent.indexOf('<!-- eCL:profile-start') === -1) {
  ...
  if (profileStatus === 'placeholder_added') message += ' Run /ecl-profile-user to unlock Developer Profile.';
