'use strict';
/**
 * Regression test for bug #444: ecl_run resolver must probe
 * <repo-root>/.claude/evolv-coder-lite/bin/ecl-tools.cjs (the project-local
 ...
 * (B) Behavioral: when RUNTIME_DIR/evolv-coder-lite/bin/ misses, but a stub
 *     exists ONLY at <repo-root>/.claude/evolv-coder-lite/bin/ecl-tools.cjs,
 *     ecl_run resolves to that stub (no PATH stub, no HOME stub).
...
// arm in the ecl_run launcher snippet -- asserts literal substring presence and exercises
...
const LOCAL_CLAUDE_PROBE = '_GSD_RUNTIME_ROOT}/.claude/evolv-coder-lite/bin/';

/**
 * Build a PATH that strips ecl-tools but keeps node and system binaries.
 * Accepts additional bin dirs to prepend.
 *
 * We cannot simply remove the whole directory that contains ecl-tools because
 ...
 * having no evolv-coder-lite/bin/ sub-path, so the resolver's first two checks
 * (RUNTIME_DIR/evolv-coder-lite/bin/ and RUNTIME_DIR/.claude/evolv-coder-lite/bin/)
 * are the only ones exercised before we hit our stub.
 *
 * The extra extraBefore dirs (e.g. noToolsBin) sit first but have no ecl-tools
 * binary, so command -v ecl-tools still falls back to PATH lookup. However,
 * the snippet's elif arm that uses `command -v ecl-tools` will find the real
 * installed one unless we mask it. To mask it without losing node, we create
 * a noToolsBin dir that shadows ecl-tools with a sentinel that must NOT be
...
function makeIsolatedPath(extraBefore = []) {
  ...
  // before command -v ecl-tools — so the real ecl-tools on PATH is never reached.
...
describe('bug-444: resolver finds repo-local .claude install', () => {
  ...
  test('(A) snippet file contains the repo-local .claude/ check arm before $HOME/.claude/', () => {
    ...
    const homeClaudeIdx = content.indexOf('$HOME/.claude/evolv-coder-lite/bin/');
    assert.ok(
      homeClaudeIdx !== -1,
  ...
  test('(B) ecl_run resolves repo-local .claude/evolv-coder-lite/bin/ stub when no other locations present', () => {
    // Create a fake repo root with a stub ONLY at .claude/evolv-coder-lite/bin/ecl-tools.cjs
    // NO stub at evolv-coder-lite/bin/, NOT on PATH, NOT in $HOME/.claude
    const fakeRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-444-root-'));
    const fakeHome = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-444-home-'));
    ...
    try {
      ...
      const localClaudeBinDir = path.join(fakeRoot, '.claude', 'evolv-coder-lite', 'bin');
      fs.mkdirSync(localClaudeBinDir, { recursive: true });
      const stubPath = path.join(localClaudeBinDir, 'ecl-tools.cjs');
      fs.writeFileSync(
        stubPath,
      ...
      // Keep node in PATH (needed to run the .cjs stub); remove ecl-tools
      ...
      assert.ok(
        normStdout.includes('.claude/evolv-coder-lite/bin/ecl-tools.cjs'),
        `Expected ECL_TOOLS to resolve to .claude/evolv-coder-lite/bin/ecl-tools.cjs, got:\n${stdout.trim()}`,
      );
  ...
  test('(C) repo-local .claude/ install wins over $HOME/.claude/ when both exist', () => {
    const fakeRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-444-prec-root-'));
    const fakeHome = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-444-prec-home-'));
    const noToolsBin = path.join(fakeRoot, 'nobin');
    fs.mkdirSync(noToolsBin, { recursive: true });
    ...
    try {
      ...
      const localClaudeBinDir = path.join(fakeRoot, '.claude', 'evolv-coder-lite', 'bin');
      fs.mkdirSync(localClaudeBinDir, { recursive: true });
      const localStubPath = path.join(localClaudeBinDir, 'ecl-tools.cjs');
      fs.writeFileSync(
        localStubPath,
      ...
      const homeClaudeBinDir = path.join(fakeHome, '.claude', 'evolv-coder-lite', 'bin');
      fs.mkdirSync(homeClaudeBinDir, { recursive: true });
      const homeStubPath = path.join(homeClaudeBinDir, 'ecl-tools.cjs');
      fs.writeFileSync(
        homeStubPath,
