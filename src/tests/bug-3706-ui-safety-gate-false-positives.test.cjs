...
describe('Workflow .md structural guard (#3718)', () => {
  ...
  // `ecl_run check ui-plan-gate` (a CLI command that internally calls checkUiPresence
...
describe('UI gate resolves the helper against RUNTIME_DIR, not the consuming repo (#448)', () => {
  ...
  const GATE_SNIPPET = [
    ...
    'UI_GATE_JS=$(for _c in "$_ECL_RT/evolv-coder-lite/bin/lib/ui-safety-gate.cjs" "$_ECL_RT/bin/lib/ui-safety-gate.cjs" "$_ECL_RT/.claude/bin/lib/ui-safety-gate.cjs" "$HOME/.claude/evolv-coder-lite/bin/lib/ui-safety-gate.cjs" "$HOME/.claude/bin/lib/ui-safety-gate.cjs"; do [ -f "$_c" ] && { echo "$_c"; break; }; done)',
