...
function cmdValidateHealth(cwd, options, raw) {
  ...
  if (!fs.existsSync(statePath)) {
    ...
    // the active one. After /ecl:complete-milestone, historical phase dirs
  ...
  // Verify eCL agents are installed. Missing agents cause Task(subagent_type=...)
    ...
    if (!agentStatus.agents_installed) {
      if (agentStatus.installed_agents.length === 0) {
        addIssue('warning', 'W010',
          ...
          'Run the eCL installer: npx @evolvconsulting/evolv-coder-lite@latest');
      } else {
        addIssue('warning', 'W010',
