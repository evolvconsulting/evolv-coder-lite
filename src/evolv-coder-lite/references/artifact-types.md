...
This reference documents all artifact types in the eCL planning taxonomy. Each type has a defined
...
- **Location**: `~/.claude/evolv-coder-lite/USER-PROFILE.md`
...
### Spike README.md / MANIFEST.md (per-spike, via /ecl:spike)
- **Shape**: YAML frontmatter (spike, name, validates, verdict, related, tags) + run instructions + results
- **Lifecycle**: Created by `/ecl:spike` → Verified → Wrapped up by `/ecl-spike-wrap-up`
- **Location**: `.planning/spikes/NNN-name/README.md`, `.planning/spikes/MANIFEST.md`
- **Consumed by**: `/ecl-spike-wrap-up` for curation; `pause-work` for spike context handoff
