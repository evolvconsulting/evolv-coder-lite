 ...
 *
 * Ported from evolv-coder-lite/bin/lib/intel.cjs.
 ...
 * Returns `{ file, exports, method }` with `file` as a resolved absolute path (matches `ecl-tools.cjs`).
 */
...
/**
 * `ecl-tools intel update` entry point: returns the same JSON as `intel.cjs` `intelUpdate`.
 * Does not run the full graph refresh in-process — that work is done by the
 * **ecl-intel-updater** agent after spawn. When `.planning/intel/` is disabled in config,
...
export const intelUpdate: QueryHandler = async (_args, projectDir, _workstream) => {
  ...
  return {
    data: {
      action: 'spawn_agent',
      message: 'Run ecl-tools intel update or spawn ecl-intel-updater agent for full refresh',
    },
  };
};
