...
describe('hasVersionHeading', () => {
  test('true for "### eCL Version" heading', () => {
    assert.equal(hasVersionHeading(bugBody('1.18.0')), true);
  });

  test('false for "### Version" (without eCL prefix — bare Version no longer matches)', () => {
    assert.equal(hasVersionHeading('### Version\n\n1.0.0'), false);
  });

  test('true for h1 "# eCL Version"', () => {
    assert.equal(hasVersionHeading('# eCL Version\n\n1.0.0'), true);
  });
...
describe('extractVersion', () => {
  ...
  test('returns null when section is absent', () => {
    // bugBody with no arg produces no ### eCL Version section
...
describe('isBugReport', () => {
  ...
  test('true when no bug label but body has "### eCL Version" heading', () => {
    // API-filed bug: copied template body but omitted labels
    const body = bugBody('1.18.0');
    assert.equal(isBugReport({ labels: [], body }), true);
  });

  test('false when no bug label and body has bare "### Version" heading (no eCL prefix — not matched)', () => {
...
describe('evaluateVersionGate', () => {
  ...
  test('bug label + no version section → close / missing-version', () => {
    // bug label present but body has no ### eCL Version section
  ...
  test('bug label + ### eCL Version section present but blank → close / invalid-version', () => {
    const result = evaluateVersionGate({
      labels: ['bug'],
      body: '### eCL Version\n\n\n### What happened?\n\nSomething broke.',
    });
  ...
  test('no bug label and no version heading (body omits eCL Version section entirely) → skip / not-a-bug', () => {
    // bugBody(undefined) produces NO ### eCL Version section, so isBugReport
  ...
  test('no bug label but body has eCL Version heading with invalid value "idk" → isBugReport true via heading fallback → close / invalid-version', () => {
    // API-filed bug: body includes ### eCL Version heading (isBugReport = true)
  ...
  test('API-filed bug: version heading present but empty, no bug label → close / invalid-version', () => {
    // Body has the ### eCL Version heading but the value is empty (heading present → invalid, not missing)
    const body = '### eCL Version\n\n\n### What happened?\n\nSomething broke.';
    const result = evaluateVersionGate({ labels: [], body });
    assert.deepEqual(result, { action: 'close', reason: 'invalid-version' });
  });
  ...
  test('CRLF body with eCL Version 1.18.0 → skip / valid-version', () => {
    const body =
      '### eCL Version\r\n\r\n1.18.0\r\n\r\n### What happened?\r\n\r\nBoom.';
  ...
  test('eCL Version is the last section with value on final line (no trailing newline) → skip / valid-version', () => {
...
describe('isBugReport — bare Version heading (no eCL prefix)', () => {
...
describe('isBugReport — labels are authoritative', () => {
  test('labels ["enhancement"] + body with "### eCL Version" heading → false (labels win)', () => {
    const body = '### eCL Version\n\n_No response_\n\n### What happened?\n\nSomething broke.';
    assert.equal(isBugReport({ labels: ['enhancement'], body }), false);
  });

  test('labels ["enhancement","needs-review"] + body with "### eCL Version" heading → false', () => {
    const body = '### eCL Version\n\n_No response_\n\n### What happened?\n\nSomething broke.';
    assert.equal(isBugReport({ labels: ['enhancement', 'needs-review'], body }), false);
  });

  test('labels [] (unlabeled) + body with "### eCL Version" heading → true (heading fallback)', () => {
    const body = '### eCL Version\n\n1.18.0\n\n### What happened?\n\nSomething broke.';
    assert.equal(isBugReport({ labels: [], body }), true);
  });

  test('labels ["bug"] + body with "### eCL Version" heading → true (bug label fast-path)', () => {
    const body = '### eCL Version\n\n1.18.0\n\n### What happened?\n\nSomething broke.';
    assert.equal(isBugReport({ labels: ['bug'], body }), true);
  });
});
...
describe('renderCloseComment', () => {
  ...
  test('contains the words "eCL Version"', () => {
    const comment = renderCloseComment();
    assert.ok(comment.includes('eCL Version'), `expected "eCL Version" in comment: ${comment}`);
  });
...
describe('evaluateVersionGate — labels authoritative', () => {
  test('labels ["enhancement"] + body with "### eCL Version" heading → skip / not-a-bug', () => {
    const body = '### eCL Version\n\nidk\n\n### What happened?\n\nSomething broke.';
...
describe('extractVersion — double heading', () => {
  test('two "### eCL Version" headings: first section "_No response_", second "1.18.0" → returns "_No response_" (first wins)', () => {
    const body =
      '### eCL Version\n\n_No response_\n\n### eCL Version\n\n1.18.0\n\n### What happened?\n\nBoom.';
    assert.equal(extractVersion(body), '_No response_');
  });
