...
# ecl-graphify-update.sh — PostToolUse hook (Bash matcher) that auto-rebuilds
...
#      the exact `ecl-tools query commit` command shape; the SDK command invokes
...
#   - Detaches hooks/lib/ecl-graphify-rebuild.sh which copies graphify-out/* to
...
# Gate 2 — HEAD-advancing git op (shell-direct or exact `ecl-tools query commit`)
case "$COMMAND" in
  *"git commit"*|*"git merge"*|*"git pull"*|*"git rebase --continue"*|*"git cherry-pick"*) ;;
  *"ecl-tools query commit"|*"ecl-tools query commit "*) ;;
  *) exit 0 ;;
esac
...
node -e '
  ...
  const status = {
    ts: process.env.ECL_TS,
    status: "running",
    exit_code: null,
    duration_ms: null,
    head_at_build: process.env.ECL_HEAD,
...
REBUILD_SCRIPT="$HOOK_DIR/lib/ecl-graphify-rebuild.sh"
