...
In this tutorial you will build a tiny, fully declarative eCL capability from scratch and watch it act inside your project's loop. By the end you will have a working capability installed, visible in `ecl capability list`, and firing at the `plan:pre` extension point.
...
- eCL 1.6.0 or later (`ecl --version`).
...
- `engines.ecl` is a hard gate: eCL will refuse to install or load this capability on any version older than 1.6.0.
...
```bash
ecl capability install ./capabilities/hello-note --scope project
```
...
Because `hello-note` declares no executable surfaces (no hook scripts, no MCP servers, no command modules) eCL copies the files to the project capability ledger without displaying a consent prompt. That is intentional — declarative capabilities are safe to install without reviewing runnable code.
...
```bash
ecl loop render-hooks plan:pre --raw
```
...
```bash
ecl capability disable hello-note
```

Run `ecl capability list` again. The `status` column will now show `disabled`. Run `ecl loop render-hooks plan:pre --raw` and you will see that `hello-note` is absent from the active hook set. Disabled capabilities are removed from the resolver output by construction — there is nothing feature-specific for the loop to run.

To re-enable it:

```bash
ecl capability enable hello-note
```
