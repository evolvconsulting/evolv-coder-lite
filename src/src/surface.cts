 ...
 *
 * Manages the runtime enable/disable surface state (the `.ecl-surface.json` marker in
 * each runtime's config dir root (e.g., ~/.claude)) independently of the install-time profile marker
 * (`.ecl-profile`). Runtime config locations are resolved by callers.
 *
...
const SURFACE_FILE_NAME = '.ecl-surface.json';
 ...
 *  - parsed ecl-file-manifest.json object ({ files: { ... } }) by rebuilding
...
function normalizeSkillManifest(runtimeConfigDir: string, manifest: Map<string, string[]> | object | null | undefined): Map<string, string[]> {
  ...
  // Parsed ecl-file-manifest.json shape: rebuild the dependency map from source.
...
function resolveSurface(runtimeConfigDir: string, manifest: Map<string, string[]> | object, clusterMap?: ClusterMap | Record<string, string[]>, registry?: { capabilityClusters?: Record<string, string[]>; profileMembership?: Record<string, { tier: string; profiles: string[] }> }): { name: string; skills: Set<string>; agents: Set<string> } {
  ...
  // Determine base profile name: from surface state or from .ecl-profile marker
...
/**
 * Prune eCL-managed skill directories from a skills directory.
 *
 * Removes every directory in `skillsDir` that is eCL-owned but NOT listed
 * in `retainedNames`. User-owned dirs (not matching the eCL ownership criteria)
 * are always preserved.
 *
 * Ownership criteria:
 *   - Non-empty prefix (e.g. 'ecl-'): dir name starts with that prefix AND
 ...
 *     preserved — this prevents data loss for user-created ecl-* directories.
 ...
 *
 * @param skillsDir        directory that contains the ecl-STEM sub-dirs
 * @param retainedNames    set of directory names to keep (e.g. 'ecl-help')
 * @param prefix           eCL dir prefix, e.g. 'ecl-' (or '' for Hermes)
...
function pruneSkillDirs(skillsDir: string, retainedNames: Set<string>, prefix: string, manifest?: Map<string, string[]>): void {
  ...
  for (const entry of fs.readdirSync(skillsDir)) {
    ...
    if (prefix !== '') {
      ...
      // The dir must also be in the manifest to be considered eCL-owned.
      // A user-created ecl-* dir that isn't in the manifest is preserved with a warning.
      if (!canonicalStems.has(entry.slice(prefix.length))) {
        process.stderr.write(
          `[ecl] Warning: ${entry} matches eCL prefix '${prefix}' but is not in the manifest — preserving (user-owned or unknown)\n`
        );
        continue;
      }
      isGsdOwned = true;
    } else if (canonicalStems) {
      // Hermes: eCL-owned iff the directory name appears in the canonical manifest.
    ...
    if (retainedNames.has(entry)) continue; // eCL-owned and in retain set
    try {
 ...
 * For 'agents' kind: same, but only remove files starting with 'ecl-' prefix.
 ...
 *
 * For Hermes (empty prefix): uses manifest membership to discriminate eCL-owned vs
 * user-owned dirs. eCL-owned = stem in manifest; removal targets = in manifest AND
...
function _syncGsdDir(stagedDir: string, destDir: string, kind: ArtifactKind | string, manifest?: Map<string, string[]>): void {
  ...
  const kindPrefix = (typeof kind === 'object' && kind !== null) ? kind.prefix : 'ecl-';
  ...
  if (kindName === 'skills') {
    ...
    // Prune eCL-owned dirs that are no longer in the staged set.
    ...
    // the ecl- prefix on copy; namespaced command dirs (commands/ecl) and agents
    // keep their staged names. Copying staged names verbatim previously diverged
    // from install and orphaned the installed ecl-*.md files, and the unscoped
    ...
    // Prune stale eCL-owned files not in the staged set, preserving user-owned files
    // (mirrors install's prefix-scoped _removeGsdEntries):
    //   - agents: only ecl-* are eCL-owned
    //   - flat command dirs: only `${kindPrefix}`-prefixed are eCL-owned
    //   - namespaced command dirs: the whole dir is eCL-owned
    for (const file of fs.readdirSync(destDir).filter(f => f.endsWith('.md'))) {
      if (kindName === 'agents' && !file.startsWith('ecl-')) continue;
