/**
 * Integration tests for ecl-context-monitor.js auto-record on CRITICAL (#1974).
 *
 * Verifies:
 * 1. On CRITICAL + active eCL project, the hook sets criticalRecorded in the
...
const HOOK_PATH = path.resolve(__dirname, '..', 'hooks', 'ecl-context-monitor.js');
...
/**
 * Run ecl-tools state record-session synchronously.
...
function runRecordSession(cwd, stoppedAt) {
  const result = spawnSync(
    process.execPath,
    [ECL_TOOLS, 'state', 'record-session', '--stopped-at', stoppedAt, '--cwd', cwd],
...
describe('#1974 context exhaustion auto-record', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-1974-'));
    ...
    // Minimal config.json required by ecl-tools
  ...
  test('does NOT spawn subprocess when .planning/STATE.md is absent', () => {
    // Delete STATE.md to simulate non-eCL project
  ...
  test('hook uses __dirname-based path (runtime-agnostic)', () => {
    ...
    assert.match(
      hookSource,
      /path\.join\(__dirname,\s*'\.\.',\s*'evolv-coder-lite'/,
      'hook must use __dirname-based path resolution for ecl-tools.cjs'
    );
    assert.doesNotMatch(
      hookSource,
      /process\.env\.HOME.*\.claude.*evolv-coder-lite.*ecl-tools\.cjs/,
