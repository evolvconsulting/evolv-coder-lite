...
このチュートリアルでは、すでにコードが存在するリポジトリに eCL Core を導入します。コードベースをマッピングし、*追加する*内容を説明するプロジェクトを作成して、小さな変更に対して最初の議論・計画サイクルを実行します。最終的に、eCL Core の計画パイプラインがあなたのスタック、規約、懸念事項を把握し、計画するたびにその知識を活用できる状態になります。

---

## 作るもの

既存の Express アプリケーションに `GET /health` エンドポイントを1つ追加します。変更は小さく、本来のレッスン — eCL Core が計画前にコードベースを学習する仕組み — から注意がそれることはありません。
...
プロジェクトを作成する前に、eCL Core に既存のコードを学習させてください。これがブラウンフィールドの計画を正確にするステップです。

```text
/ecl-map-codebase
```

eCL Core が4つの並行マッパーサブエージェントを生成します（「Spawning 4 parallel codebase mapper agents…」という通知が表示されます。1〜5分かかりますので中断しないでください）。各エージェントはそれぞれ異なる観点に注目します:
...
`.planning/codebase/STACK.md` を開いてください。eCL Core が検出した言語、ランタイム、フレームワークのバージョン、主要な依存関係が表示されます。これは推測ではなく、実際に読み込んだファイルに基づいています。

`.planning/codebase/CONVENTIONS.md` を開いてください。ソースコードから観察した命名規則、エラーハンドリングパターン、コードスタイルのルールが表示されます。このリポジトリで eCL Core が生成するすべてのプランは、これらの規約に自動的に従います。
...
プロジェクトを作成します。前のステップで eCL Core が既存のコードを見つけているため、これがブラウンフィールドプロジェクトであることをすでに把握しています。`/ecl-new-project` を実行すると、既存のものを再説明するのではなく、*追加する*内容に焦点を当てた質問がされます:

```text
/ecl-new-project
```

eCL Core が何を作りたいかを尋ねます。コードベース全体の説明ではなく、追加する機能で答えてください:
...
eCL Core が少数の確認質問をした後、要件とロードマップの作成に進みます。すでに `ARCHITECTURE.md` と `STACK.md` を読み込んでいるため、既存の機能を `PROJECT.md` の **Validated** セクションに自動的にマッピングします。既存の API サーフェスを説明する必要はありません。
...
`.planning/codebase/` はすでにステップ 3 から存在しています。eCL Core は `PROJECT.md` を書く際にこれらのファイルを読み込んでいるため、あなたが説明しなくても Validated 要件を入力できたのです。
...
eCL Core があなたの `CONVENTIONS.md` と `ARCHITECTURE.md` を読み込んでいるため、質問は汎用的なアドバイスではなく、実際のコードベースに基づいています。以下のような内容が表示される場合があります:
...
議論が終了すると、eCL Core が以下のファイルを書き込みます:
...
`01-01-PLAN.md` を開いてください。`<files>` タグが `src/routes/health.js` を参照していることに注目してください。これは議論で指定した正確なパスであり、eCL Core がコードベースマップで観察したルーティングパターンと一致しています。これがコードベースマップの効果です。
...
今後の機能追加ごとに、構造が大幅に変わった際は `/ecl-map-codebase` を再実行してコードベースマップを最新の状態に保ってください。

---

## 学んだこと

- `/ecl-map-codebase` が4つの並行エージェントを実行して `.planning/codebase/` に `STACK.md`、`ARCHITECTURE.md`、`CONVENTIONS.md`、`CONCERNS.md`、`STRUCTURE.md`、`TESTING.md`、`INTEGRATIONS.md` を生成する仕組み。
- ブラウンフィールドリポジトリで `/ecl-new-project` を実行すると、*追加する*内容に焦点を当てた質問がされ、既存コードから Validated 要件が自動入力される仕組み。
- コードベースマップが `/ecl-discuss-phase` のすべての質問を形成する方法 — ファイルパス、パターン、規約が実際のコードから導出される。
...
- [コマンドによるコードベースマッピング](../COMMANDS.md) — `/ecl-map-codebase` のすべてのフラグとサブコマンド
