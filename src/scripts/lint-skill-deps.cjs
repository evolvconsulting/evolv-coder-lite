 ...
 *    For each commands/ecl/*.md, parse requires: and walk the body for
 *    references to other eCL skills (pattern: /ecl:<stem> or ecl:<stem>).
 ...
 *
 * Usage:
 *   node scripts/lint-skill-deps.cjs               # scans commands/ecl/
...
const PROFILES_MODULE = path.join(__dirname, '..', 'evolv-coder-lite', 'bin', 'lib', 'install-profiles.cjs');
...
let commandsDir = path.join(__dirname, '..', 'commands', 'ecl');
    ...
    for (const ref of referenced) {
      ...
      if (!allStems.has(ref)) {
        violations.push({
          ...
          filePath,
          undeclared: ref,
          message: 'body references unknown skill ecl:' + ref,
        });
        continue;
      }
      if (!declaredSet.has(ref)) {
        violations.push({
          ...
          filePath,
          undeclared: ref,
          message: 'body references ecl:' + ref + ' but requires: does not list it',
        });
      }
    }
  }
