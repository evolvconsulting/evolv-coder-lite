...
describe('#443 effort cascade', () => {
  ...
  test('no config -> ecl-planner (heavy) defaults to "xhigh" via tier default', () => {
    // ecl-planner is heavy tier; manifest default for heavy is xhigh
    assert.strictEqual(resolveEffortInternal(tmpDir, 'ecl-planner'), 'xhigh');
  });

  test('routing_tier_defaults: light (ecl-codebase-mapper) -> "low"', () => {
    // ecl-codebase-mapper routingTier=light, default for light is "low"
    assert.strictEqual(resolveEffortInternal(tmpDir, 'ecl-codebase-mapper'), 'low');
  });

  test('routing_tier_defaults: standard (ecl-executor) -> "high"', () => {
    // ecl-executor routingTier=standard, default for standard is "high"
    assert.strictEqual(resolveEffortInternal(tmpDir, 'ecl-executor'), 'high');
  });

  test('routing_tier_defaults: heavy (ecl-planner) -> "xhigh"', () => {
    // ecl-planner routingTier=heavy, default for heavy is "xhigh"
    assert.strictEqual(resolveEffortInternal(tmpDir, 'ecl-planner'), 'xhigh');
  });
  ...
  test('effort.routing_tier_defaults override beats tier default', () => {
    ...
    assert.strictEqual(resolveEffortInternal(tmpDir, 'ecl-planner'), 'medium');
  });
  ...
  test('effort.agent_overrides beats routing_tier_defaults', () => {
    writeConfig(tmpDir, {
      effort: {
        ...
        agent_overrides: { 'ecl-planner': 'low' },
      },
    });
    assert.strictEqual(resolveEffortInternal(tmpDir, 'ecl-planner'), 'low');
  });

  test('opts.override beats agent_overrides', () => {
    writeConfig(tmpDir, {
      effort: { agent_overrides: { 'ecl-planner': 'low' } },
    });
    assert.strictEqual(resolveEffortInternal(tmpDir, 'ecl-planner', { override: 'minimal' }), 'minimal');
  });
  ...
  test('invalid override falls through to agent_overrides', () => {
    ...
    assert.strictEqual(resolveEffortInternal(tmpDir, 'ecl-planner', { override: 'turbo' }), 'low');
  });

  test('invalid agent_overrides value falls through to routing_tier_defaults', () => {
    writeConfig(tmpDir, {
      effort: {
        agent_overrides: { 'ecl-planner': 123 },
        routing_tier_defaults: { heavy: 'medium' },
      },
    });
    assert.strictEqual(resolveEffortInternal(tmpDir, 'ecl-planner'), 'medium');
  ...
  test('invalid effort.default falls through to hardcoded "high" (no routing_tier_defaults set)', () => {
    ...
    assert.strictEqual(resolveEffortInternal(tmpDir, 'ecl-planner'), 'high');
  });
  ...
  test('effort block is non-object (string) -> effortCfg=null -> uses manifest tier default xhigh', () => {
    ...
    // Non-object effort => effortCfg=null; ecl-planner heavy tier manifest default = xhigh
    assert.strictEqual(resolveEffortInternal(tmpDir, 'ecl-planner'), 'xhigh');
  });
...
describe('#443 fast_mode cascade', () => {
  ...
  test('no config -> defaults to false', () => {
    assert.strictEqual(resolveFastModeInternal(tmpDir, 'ecl-planner'), false);
  });
  ...
  test('fast_mode.enabled=true -> true when no tier/agent overrides', () => {
    ...
    // For ecl-planner (heavy), tier default is false — falls through to enabled=true
    assert.strictEqual(resolveFastModeInternal(tmpDir, 'ecl-planner'), true);
  });
  ...
  test('fast_mode.routing_tier_defaults.light=true -> light agent gets true', () => {
    ...
    assert.strictEqual(resolveFastModeInternal(tmpDir, 'ecl-codebase-mapper'), true);
  });
  ...
  test('fast_mode.agent_overrides beats routing_tier_defaults', () => {
    writeConfig(tmpDir, {
      fast_mode: {
        ...
        agent_overrides: { 'ecl-codebase-mapper': true },
      },
    });
    assert.strictEqual(resolveFastModeInternal(tmpDir, 'ecl-codebase-mapper'), true);
  });
  ...
  test('opts.override beats agent_overrides', () => {
    writeConfig(tmpDir, {
      fast_mode: { agent_overrides: { 'ecl-planner': true } },
    });
    assert.strictEqual(resolveFastModeInternal(tmpDir, 'ecl-planner', { override: false }), false);
  });
  ...
  test('string "true" NOT accepted as fast_mode override', () => {
    ...
    const result = resolveFastModeInternal(tmpDir, 'ecl-planner');
    assert.strictEqual(typeof result, 'boolean');
  });

  test('string "true" in opts.override NOT accepted', () => {
    // opts.override must be strict boolean — string falls through
    const result = resolveFastModeInternal(tmpDir, 'ecl-planner', { override: 'true' });
...
describe('#443 resolveEffortForTier escalation', () => {
  ...
  test('dynamic_routing disabled -> attempt ignored, returns base effort', () => {
    // ecl-planner heavy -> xhigh baseline
    const base = resolveEffortForTier(tmpDir, 'ecl-planner', 0);
    const attempt1 = resolveEffortForTier(tmpDir, 'ecl-planner', 1);
  ...
  test('dynamic_routing enabled, escalate_on_failure=false -> attempt ignored', () => {
    ...
    const base = resolveEffortForTier(tmpDir, 'ecl-codebase-mapper', 0);
    const attempt1 = resolveEffortForTier(tmpDir, 'ecl-codebase-mapper', 1);
  ...
  test('dynamic_routing enabled, attempt=1 -> one step up from base', () => {
    ...
    // ecl-codebase-mapper: light -> effort 'low'; attempt=1 -> 'medium'
    assert.strictEqual(resolveEffortForTier(tmpDir, 'ecl-codebase-mapper', 0), 'low');
    assert.strictEqual(resolveEffortForTier(tmpDir, 'ecl-codebase-mapper', 1), 'medium');
  });
  ...
  test('escalation clamps at "max"', () => {
    ...
    // xhigh -> max -> max (clamp)
    const result = resolveEffortForTier(tmpDir, 'ecl-planner', 99);
  ...
  test('respects max_escalations cap', () => {
    ...
    const at1 = resolveEffortForTier(tmpDir, 'ecl-codebase-mapper', 1);
    const at2 = resolveEffortForTier(tmpDir, 'ecl-codebase-mapper', 2);
...
describe('#443 resolve-execution CLI command', () => {
  ...
  beforeEach(() => {
    ...
    // HOME isolation to prevent ~/.ecl/defaults.json bleed
  ...
  test('default (claude) runtime -> effort present, effort_param=output_config.effort, fast_mode_supported=false', () => {
    const result = runGsdTools(['resolve-execution', 'ecl-planner'], tmpDir, { HOME: tmpDir });
  ...
  test('--effort flag overrides config effort', () => {
    const result = runGsdTools(
      ['resolve-execution', 'ecl-planner', '--effort', 'low'],
      tmpDir,
      { HOME: tmpDir }
    );
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('--fast-mode flag honored', () => {
    const result = runGsdTools(
      ['resolve-execution', 'ecl-planner', '--fast-mode', 'true'],
      tmpDir,
      { HOME: tmpDir }
    );
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('--attempt flag triggers escalation', () => {
    ...
    const result0 = runGsdTools(
      ['resolve-execution', 'ecl-codebase-mapper', '--attempt', '0'],
      tmpDir,
      { HOME: tmpDir }
    );
    const result1 = runGsdTools(
      ['resolve-execution', 'ecl-codebase-mapper', '--attempt', '1'],
      tmpDir,
      { HOME: tmpDir }
    );
  ...
  test('--raw prints effort string', () => {
    const result = runGsdTools(
      ['resolve-execution', 'ecl-planner', '--raw'],
      tmpDir,
      { HOME: tmpDir }
    );
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('#443 resolve-model emits effort (unified)', () => {
  ...
  test('resolve-model on claude runtime emits effort (not null)', () => {
    const result = runGsdTools(['resolve-model', 'ecl-planner'], tmpDir, { HOME: tmpDir });
    assert.ok(result.success, `Command failed: ${result.error}`);
    const output = JSON.parse(result.output);
...
describe('#443 QA matrix — malformed effort/fast_mode configs', () => {
  ...
  test('effort.default=123 (numeric) -> gracefully falls through', () => {
    ...
    // ecl-planner is heavy, tier default xhigh is used instead
    const result = resolveEffortInternal(tmpDir, 'ecl-planner');
...
describe('#443 config schema: new effort/fast_mode keys valid', () => {
  ...
  test('effort.agent_overrides.<agent-id> is valid (dynamic pattern)', () => {
    assert.ok(isValidConfigKey('effort.agent_overrides.ecl-planner'));
  ...
  test('fast_mode.agent_overrides.<agent-id> is valid', () => {
    assert.ok(isValidConfigKey('fast_mode.agent_overrides.ecl-planner'));
  });
...
describe('#443 resolve-execution: deterministic arg parsing (flags-first ordering)', () => {
  ...
  test('flags-first: --effort low ecl-planner resolves ecl-planner (NOT "low" as agent)', () => {
    // BUG: before fix, agentTypeArg = 'low' (first non-dash token) -> unknown_agent:true
    const result = runGsdTools(
      ['resolve-execution', '--effort', 'low', 'ecl-planner'],
      tmpDir,
      { HOME: tmpDir }
    );
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('flags-first: --attempt 1 ecl-codebase-mapper resolves ecl-codebase-mapper (NOT "1" as agent)', () => {
    // BUG: before fix, agentTypeArg = '1' -> unknown_agent:true
    const result = runGsdTools(
      ['resolve-execution', '--attempt', '1', 'ecl-codebase-mapper'],
      tmpDir,
      { HOME: tmpDir }
    );
    assert.ok(result.success, `Command failed: ${result.error}`);
    const output = JSON.parse(result.output);
    assert.ok(!output.unknown_agent, `ecl-codebase-mapper must be resolved, got: ${JSON.stringify(output)}`);
  });

  test('agent-first parity: ecl-planner --effort low produces same effort as flags-first', () => {
  ...
  test('error: two positional agents -> non-zero exit', () => {
    const result = runGsdTools(
      ['resolve-execution', 'ecl-planner', 'ecl-executor'],
      tmpDir,
      { HOME: tmpDir }
    );
  ...
  test('error: --attempt notanumber -> non-zero exit, clear error', () => {
    const result = runGsdTools(
      ['resolve-execution', '--attempt', 'notanumber', 'ecl-planner'],
      tmpDir,
      { HOME: tmpDir }
    );
  ...
  test('error: trailing --effort (no value) -> non-zero exit', () => {
    const result = runGsdTools(
      ['resolve-execution', 'ecl-planner', '--effort'],
      tmpDir,
      { HOME: tmpDir }
    );
