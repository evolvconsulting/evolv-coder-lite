...
const installProfiles = require('../evolv-coder-lite/bin/lib/install-profiles.cjs');
...
describe('resolveRuntimeArtifactLayout — claude local', () => {
  test('returns correct layout for claude scope=local', () => {
    ...
    assert.strictEqual(layout.kinds[0].destSubpath, 'commands/ecl');
    assert.strictEqual(layout.kinds[0].prefix, 'ecl-');
    ...
    assert.strictEqual(layout.kinds[1].prefix, 'ecl-');
...
describe('resolveRuntimeArtifactLayout — hermes', () => {
  test('returns correct layout for hermes', () => {
    ...
    assert.strictEqual(layout.kinds.length, 1);
    assert.strictEqual(layout.kinds[0].kind, 'skills');
    assert.strictEqual(layout.kinds[0].destSubpath, 'skills/ecl');
    assert.strictEqual(layout.kinds[0].prefix, 'ecl-'); // #947: restored canonical prefix
    assert.strictEqual(typeof layout.kinds[0].stage, 'function');
  });
});
...
describe('resolveRuntimeArtifactLayout — opencode', () => {
  test('returns commands + skills layout for opencode (#784)', () => {
    ...
    assert.strictEqual(commands.prefix, 'ecl-');
    ...
    assert.strictEqual(skills.prefix, 'ecl-');
...
describe('resolveRuntimeArtifactLayout edge-cases', () => {
  test('hermes has destSubpath skills/ecl and ecl- prefix (#947: restored from bare-stem)', () => {
    const layout = resolveRuntimeArtifactLayout('hermes', '/tmp/x');
    assert.strictEqual(layout.kinds[0].destSubpath, 'skills/ecl');
    assert.strictEqual(layout.kinds[0].prefix, 'ecl-'); // #947: bare-stem prefix='' reversed
  });
...
const CORE_AGENTS = new Set(['ecl-planner']);
...
describe('stage — skills kind (claude global)', () => {
  test('stage returns a directory containing ecl-<stem>/SKILL.md entries', () => {
    ...
    for (const entry of entries) {
      assert.ok(entry.startsWith('ecl-'), `entry should start with ecl-: ${entry}`);
  ...
  test('stage with skills="*" produces flat layout for claude (#924: reverted from nested)', () => {
    ...
    // #924: Claude is reverted to FLAT. Full profile produces >= 60 top-level ecl-* dirs.
    // (Previously nested: exactly 6 ecl-ns-* router dirs. That broke Skill-tool discovery.)
...
describe('stage — skills kind (kimi global)', () => {
  test('stage returns Kimi SKILL.md dirs and agent YAML/prompt artifacts', () => {
    ...
    const stagedDir = skillsKind.stage(PROFILE_CORE);
    assert.ok(fs.existsSync(stagedDir), 'stagedDir must exist');
    const skillMd = path.join(stagedDir, 'ecl-new-project', 'SKILL.md');
    assert.ok(fs.existsSync(skillMd), 'ecl-new-project/SKILL.md must exist');
    ...
    const rootYamlPath = path.join(stagedAgentsDir, 'ecl.yaml');
    const rootPromptPath = path.join(stagedAgentsDir, 'ecl.md');
    const executorYamlPath = path.join(stagedAgentsDir, 'subagents', 'ecl-executor.yaml');
    const executorPromptPath = path.join(stagedAgentsDir, 'subagents', 'ecl-executor.md');
    assert.ok(fs.existsSync(rootYamlPath), 'agents/ecl.yaml must be staged');
    assert.ok(fs.existsSync(rootPromptPath), 'agents/ecl.md must be staged');
    assert.ok(fs.existsSync(executorYamlPath), 'agents/subagents/ecl-executor.yaml must be staged');
    assert.ok(fs.existsSync(executorPromptPath), 'agents/subagents/ecl-executor.md must be staged');
    ...
    assert.match(rootYaml, /system_prompt_path: \.\/ecl\.md/);
    ...
    assert.match(executorYaml, /system_prompt_path: \.\/ecl-executor\.md/);
  ...
  test('tracks Kimi agent staging dir before writing artifacts', () => {
    ...
    try {
      fs.writeFileSync = function writeFileSyncWithInjectedFailure(file, ...args) {
        ...
        if (filePath.includes('ecl-kimi-agents-') && path.basename(filePath) === 'ecl.yaml') {
      ...
      added = [...installProfiles.STAGED_DIRS]
        .filter(dir => !before.has(dir) && path.basename(dir).startsWith('ecl-kimi-agents-'));
...
describe('stage — opencode/kilo skills kind (#784)', () => {
  for (const runtime of ['opencode', 'kilo']) {
    test(`${runtime} skills stage writes ecl-<stem>/SKILL.md with name + description`, () => {
      ...
      for (const entry of entries) {
        ...
        assert.ok(!/\/ecl:/.test(content), 'body must not contain /ecl: colon refs');
      }
    });
  }
});
