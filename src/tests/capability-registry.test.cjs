...
const { LOOP_HOST_CONTRACT } = require('../evolv-coder-lite/bin/lib/loop-host-contract.cjs');
...
describe('UI pilot capability', () => {
  ...
  test('UI pilot generates a registry with correct shape', () => {
    ...
    // byAgent maps ecl-ui-checker and ecl-ui-auditor to 'ui'
    assert.strictEqual(registry.byAgent['ecl-ui-checker'], 'ui');
    assert.strictEqual(registry.byAgent['ecl-ui-auditor'], 'ui');
      ...
      gates: [
        {
          ...
          check: { agentVerdict: { ref: 'ecl-ui-checker', prompt: 'check' } },
          blocking: true,
          onError: 'halt',
        },
      ],
...
describe('validateCrossCapability adversarial cases', () => {
  ...
  test('tier-monotone violation: core requires full rejected', () => {
    const coreCap = {
      ...
      skills: ['core-skill'], agents: ['evolv-coder-lite-agent'], hooks: [], config: {},
      steps: [], contributions: [], gates: [],
    };
    const fullCap = {
      id: 'full-cap', role: 'feature', title: 'Full', tier: 'full', requires: [],
      skills: ['full-skill'], agents: ['ecl-full-agent'], hooks: [], config: {},
      steps: [], contributions: [], gates: [],
    };
  ...
  test('config key owned by two capabilities rejected', () => {
    ...
    const cap2 = {
      ...
      skills: ['other-skill'], agents: ['ecl-other-agent'], hooks: [],
...
const REGISTRY_PATH = path.join(ROOT, 'evolv-coder-lite', 'bin', 'lib', 'capability-registry.cjs');
...
describe('committed evolv-coder-lite/bin/lib/capability-registry.cjs is not stale', () => {
...
describe('registry structure', () => {
  ...
  test('requiresClosure works for a cap with transitive requires', () => {
    const capA = {
      ...
      skills: ['a-skill'], agents: ['ecl-a-agent'], hooks: [], config: {},
      steps: [], contributions: [], gates: [],
    };
    const capB = {
      id: 'cap-b', role: 'feature', title: 'B', tier: 'standard', requires: ['cap-c'],
      skills: ['b-skill'], agents: ['ecl-b-agent'], hooks: [], config: {},
      steps: [], contributions: [], gates: [],
    };
    const capC = {
      id: 'cap-c', role: 'feature', title: 'C', tier: 'standard', requires: [],
      skills: ['c-skill'], agents: ['ecl-c-agent'], hooks: [], config: {},
      steps: [], contributions: [], gates: [],
    };
  ...
  test('step.ref with both skill and agent is rejected', () => {
    const cap = {
      ...
      steps: [
        {
          point: 'plan:pre',
          ref: { skill: 'ui-phase', agent: 'ecl-ui-checker' },  // BOTH keys — invalid
          produces: ['UI-SPEC.md'],
          consumes: ['CONTEXT.md'],
          when: 'workflow.ui_phase',
          onError: 'skip',
...
describe('double-prefix guard: step.ref.skill must not start with "ecl-"', () => {
  // ref.skill is an unprefixed stem (e.g. "ui-review"). Workflow dispatch prepends
  // "ecl-" at runtime. A stem already starting with "ecl-" would produce "ecl-ecl-..."
  // at dispatch time, silently invoking a non-existent skill.

  test('ref.skill starting with "ecl-" is rejected', () => {
    const cap = {
      ...UI_CAP,
      steps: [
        {
          point: 'verify:post',
          ref: { skill: 'ecl-ui-review' },  // wrong: stem must NOT have ecl- prefix
          produces: ['UI-REVIEW.md'],
          consumes: ['UI-SPEC.md'],
          when: 'workflow.ui_review',
          onError: 'skip',
    ...
    const errors = validateCapability(cap, 'ui');
    assert.ok(errors.length > 0, 'Expected an error for ecl-prefixed ref.skill');
    assert.ok(
      errors.some((e) => e.includes('ecl-') && (e.includes('double') || e.includes('unprefixed') || e.includes('must not start'))),
      'Error should mention the double-prefix problem, got: ' + JSON.stringify(errors),
    );
  });

  test('ref.skill without "ecl-" prefix is accepted (stem only)', () => {
    const cap = {
      ...UI_CAP,
      steps: [
        {
    ...
    const prefixErrors = validateCapability(cap, 'ui').filter((e) => e.includes('ecl-') && e.includes('stem'));
...
describe('ref membership check: step.ref.agent must be in cap.agents', () => {
  // A capability declares agents: ["ecl-ui-checker", "ecl-ui-auditor"].
  // A step with ref.agent "ecl-unknown-agent" (not in agents) must be rejected.
  ...
  test('step.ref.agent NOT in cap.agents is rejected', () => {
    const cap = {
      ...
      steps: [
        {
          point: 'plan:pre',
          ref: { agent: 'ecl-unknown-agent' },  // not in agents: ["ecl-ui-checker", "ecl-ui-auditor"]
          produces: ['UI-SPEC.md'],
          consumes: ['CONTEXT.md'],
          when: 'workflow.ui_phase',
          onError: 'skip',
    ...
    assert.ok(
      errors.some((e) => e.includes('ecl-unknown-agent') && e.includes('not declared')),
      'Error should mention "ecl-unknown-agent" and "not declared", got: ' + JSON.stringify(errors),
    );
  });
  ...
  test('step.ref.agent IN cap.agents is accepted', () => {
    const cap = {
      ...
      steps: [
        {
          point: 'plan:pre',
          ref: { agent: 'ecl-ui-checker' },  // declared in agents
          produces: ['UI-SPEC.md'],
          consumes: ['CONTEXT.md'],
          when: 'workflow.ui_phase',
          onError: 'skip',
    ...
    const errors = validateCapability(cap, 'ui');
    const membershipErrors = errors.filter((e) => e.includes('not declared') && e.includes('ecl-ui-checker'));
    assert.deepEqual(
      membershipErrors, [],
...
describe('Fix: 3-node requires cycle (A→B→C→A) is detected', () => {
  test('three-node requires cycle is reported as an error', () => {
    const capA = {
      ...
      skills: ['cyc-a-skill'], agents: ['ecl-cyc-a'], hooks: [], config: {},
      steps: [], contributions: [], gates: [],
    };
    const capB = {
      id: 'cyc-b', role: 'feature', title: 'CycB', tier: 'standard', requires: ['cyc-c'],
      skills: ['cyc-b-skill'], agents: ['ecl-cyc-b'], hooks: [], config: {},
      steps: [], contributions: [], gates: [],
    };
    const capC = {
      id: 'cyc-c', role: 'feature', title: 'CycC', tier: 'standard', requires: ['cyc-a'],
      skills: ['cyc-c-skill'], agents: ['ecl-cyc-c'], hooks: [], config: {},
      steps: [], contributions: [], gates: [],
    };
  ...
  test('agentVerdict gate with blocking:false generates zero errors', () => {
    ...
    const cap = {
      ...
      gates: [
        {
          point: 'execute:wave:post',
          check: { agentVerdict: { ref: 'ecl-ui-checker', prompt: 'check ui' } },
...
describe('S1: fragment.path traversal guard', () => {
  ...
  test('fragment.path is materialized into inline registry content', (t) => {
    const capsDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-cap-fragment-'));
  ...
  test('step fragment.path is materialized into inline registry content', (t) => {
    const capsDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-cap-step-fragment-'));
    t.after(() => cleanup(capsDir));
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        runtimeCompat: { supported: ['*'], unsupported: [] },
        skills: [],
        agents: ['ecl-phase-researcher'],
        hooks: [],
        config: {},
        steps: [{
          point: 'plan:pre',
          ref: { agent: 'ecl-phase-researcher' },
          fragment: { path: 'fragments/plan-pre.md' },
...
describe('FIX 1: self-consume rejection in validateConsumesGlobal', () => {
  test('a step produces:["SELF.md"] and consumes:["SELF.md"] with no other producer is rejected', () => {
    const cap = {
      ...
      tier: 'standard', requires: [],
      skills: ['self-skill'], agents: ['ecl-self-agent'], hooks: [], config: {},
      steps: [
        {
          point: 'plan:pre',
  ...
  test('a step produces:["SELF.md"] and consumes:["SELF.md"] but another capability produces SELF.md at an earlier point is accepted', () => {
    const producerCap = {
      ...
      tier: 'standard', requires: [],
      skills: ['producer-skill'], agents: ['ecl-producer-agent'], hooks: [], config: {},
      steps: [
        {
...
describe('configSchema emission (ADR-857 phase 3b)', () => {
  ...
  test('committed capability-registry.cjs has configSchema with correct shape', () => {
    const registry = require('../evolv-coder-lite/bin/lib/capability-registry.cjs');
...
describe('ADR-857 phase 4a: pending-reconciliation warnings (SOFT gate)', () => {
  ...
  test('ui reconciled: profileMembership.ui.profiles is ["full"] after tier:full reconciliation', () => {
    ...
    const { resolveProfile: rp } = require('../evolv-coder-lite/bin/lib/install-profiles.cjs');
...
describe('ADR-1016 phase 5a: closed-vocab rejection — tightened validateRuntimeBody', () => {
  ...
  test('bad ArtifactKind.nesting → validation error', () => {
    const cap = makeRuntimeCap({
      ...
      runtime: {
        artifactLayout: {
          global: [{ kind: 'skills', destSubpath: 'skills', prefix: 'ecl-', nesting: 'deep', recursive: false, converter: null }],
          local: [],
        },
      },
    });
...
describe('ADR-1016 phase 5a: validateArtifactLayout unit tests', () => {
  ...
  test('valid skills entry in global → no errors', () => {
    const errors = validateArtifactLayout('test', {
      global: [{ kind: 'skills', destSubpath: 'skills', prefix: 'ecl-', nesting: 'flat', recursive: false, converter: null }],
      local: [],
    });
    assert.deepEqual(errors, []);
  });
  ...
  test('kimi-agents kind → no errors', () => {
    const errors = validateArtifactLayout('test', {
      global: [{ kind: 'kimi-agents', destSubpath: 'agents', prefix: 'ecl', nesting: 'flat', recursive: false, converter: null }],
      local: [],
    });
    assert.deepEqual(errors, []);
  });
...
describe('FIX 3: tightened runtime validator — ArtifactKind field type checks', () => {
  test('ArtifactKind with recursive: "yes" (non-boolean) → validation error', () => {
    const cap = makeRuntimeCap({
      ...
      runtime: {
        artifactLayout: {
          global: [{ kind: 'skills', destSubpath: 'skills', prefix: 'ecl-', nesting: 'flat', recursive: 'yes', converter: null }],
          local: [],
        },
      },
    });
  ...
  test('ArtifactKind with recursive: true (boolean) → no error', () => {
    const cap = makeRuntimeCap({
      ...
      runtime: {
        artifactLayout: {
          global: [{ kind: 'skills', destSubpath: 'skills', prefix: 'ecl-', nesting: 'flat', recursive: true, converter: null }],
          local: [],
        },
      },
    });
  ...
  test('ArtifactKind with converter: 123 (non-string, non-null) → validation error', () => {
    const cap = makeRuntimeCap({
      ...
      runtime: {
        artifactLayout: {
          global: [{ kind: 'skills', destSubpath: 'skills', prefix: 'ecl-', nesting: 'flat', recursive: false, converter: 123 }],
          local: [],
        },
      },
    });
  ...
  test('ArtifactKind with converter: "convertClaudeCommandToKiloSkill" (string) → no error', () => {
    const cap = makeRuntimeCap({
      ...
      runtime: {
        artifactLayout: {
          global: [{ kind: 'skills', destSubpath: 'skills', prefix: 'ecl-', nesting: 'flat', recursive: true, converter: 'convertClaudeCommandToKiloSkill' }],
          local: [],
        },
      },
    });
...
describe('#1196 — discuss loop wiring + wired-point guard', () => {
  ...
  describe('Defect 1: discuss is a wireable loop host', () => {
    test('HOST_LOOP_FILES includes discuss-phase.md', () => {
      ...
      assert.ok(
        HOST_LOOP_FILES.includes('evolv-coder-lite/workflows/discuss-phase.md'),
        `HOST_LOOP_FILES must include 'evolv-coder-lite/workflows/discuss-phase.md'. Got: ${JSON.stringify(HOST_LOOP_FILES)}`,
      );
    });
  ...
  describe('Anti-pattern parity: host-file set has a single source of truth', () => {
    test('every STEP_WORKFLOWS entry file exists on disk and contains a ecl:loop-host marker', () => {
      for (const { file, step } of STEP_WORKFLOWS) {
        const absPath = path.join(ROOT, 'evolv-coder-lite', 'workflows', file);
        assert.ok(
        ...
        assert.match(
          content,
          /<!--\s*ecl:loop-host/,
          `${file} (step: ${step}) is listed in STEP_WORKFLOWS but lacks a ecl:loop-host marker`,
        );
      }
    });

    test('HOST_LOOP_FILES matches STEP_WORKFLOWS (single source of truth)', () => {
      const expectedFromStepWorkflows = STEP_WORKFLOWS.map((w) => 'evolv-coder-lite/workflows/' + w.file);
      assert.deepEqual(
        HOST_LOOP_FILES,
    ...
    test('every workflow file carrying a ecl:loop-host marker is present in STEP_WORKFLOWS', () => {
      const workflowsDir = path.join(ROOT, 'evolv-coder-lite', 'workflows');
      ...
      for (const mdFile of allMdFiles) {
        ...
        const content = fs.readFileSync(absPath, 'utf8');
        if (/<!--\s*ecl:loop-host/.test(content) && !stepWorkflowFiles.has(mdFile)) {
      ...
      assert.deepEqual(
        ...
        `These workflow files have a ecl:loop-host marker but are absent from STEP_WORKFLOWS: ` +
  ...
  describe('Property: scanWiredPoints is a correct extractor', () => {
      ...
      fc.assert(
        fc.property(
          ...
          (subset) => {
            ...
            const lines = subset.map((p) => `HOOKS_JSON=$(ecl_run loop render-hooks ${p} --raw)`);
            // Add some noise to exercise robustness
            const noise = ['# comment', 'echo hello', `ecl_run loop some-other-command`, ''];
    ...
    test('NIT-02: scanWiredPoints does not match an incomplete occurrence (no point token after render-hooks)', () => {
      ...
      const incompleteText = 'HOOKS_JSON=$(ecl_run loop render-hooks\n)';
