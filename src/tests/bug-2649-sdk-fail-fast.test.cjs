...
function makeTempSdk({ npxCache = false } = {}) {
  let root;
  if (npxCache) {
    root = path.join(os.tmpdir(), `ecl-npx-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`, 'npm-cache', '_npx', 'deadbeefcafe0001', 'node_modules', 'evolv-coder-lite');
    fs.mkdirSync(root, { recursive: true });
  } else {
    root = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-clone-'));
  }
...
describe('installer SDK dist-missing fail-fast (#2649)', () => {
  ...
  test('missing dist in npx cache: fail fast, no nested npm install', () => {
    ...
    try {
      ...
      assert.strictEqual(ir.fix_command, 'npm install -g @evolvconsulting/evolv-coder-lite@latest');
