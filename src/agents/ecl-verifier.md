...
ecl_run query roadmap.get-phase "$PHASE_NUM"
...
```bash
PHASE_DATA=$(ecl_run query roadmap.get-phase "$PHASE_NUM" --raw)
```
...
```bash
ARTIFACT_RESULT=$(ecl_run query verify.artifacts "$PLAN_PATH")
```
...
```bash
LINKS_RESULT=$(ecl_run query verify.key-links "$PLAN_PATH")
```
...
SUMMARY_FILES=$(ecl_run query summary-extract "$PHASE_DIR"/*-SUMMARY.md --fields key-files)

# Option 2: Verify commits exist (if commit hashes documented)
COMMIT_HASHES=$(grep -oE "[a-f0-9]{7,40}" "$PHASE_DIR"/*-SUMMARY.md | head -10)
if [ -n "$COMMIT_HASHES" ]; then
  COMMITS_VALID=$(ecl_run query verify.commits $COMMIT_HASHES)
fi
...
```bash
ROADMAP_DATA=$(ecl_run query roadmap.analyze --raw)
```
