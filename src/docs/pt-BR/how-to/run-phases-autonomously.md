...
Execute todas as fases restantes — ou um intervalo delimitado delas — sem supervisão, para que o eCL avance por discuss → plan → execute em cada fase sem que você precise conduzir cada etapa.
...
- Qualquer decisão de design que você se importe já deve estar em `PROJECT.md` ou registrada via um `/ecl-discuss-phase` anterior — o modo autônomo só consegue apresentar áreas cinzentas de forma interativa quando você usa `--interactive`
...
O eCL lê o `ROADMAP.md`, descobre cada fase incompleta em ordem numérica e executa discuss → plan → execute em cada uma. Após todas as fases serem concluídas, ele executa automaticamente o ciclo de vida do milestone: audit → complete → cleanup.
...
```bash
/ecl-autonomous --from 3          # fases 3, 4, 5 … (ignora as fases 1 e 2 já concluídas)
/ecl-autonomous --to 5            # fases até e incluindo a 5
/ecl-autonomous --from 3 --to 5   # exatamente as fases 3, 4 e 5
```
...
No modo interativo:
- `/ecl-discuss-phase` é executado inline e aguarda suas respostas
...
O modo autônomo não ignora o pipeline de qualidade do eCL. Cada fase ainda:
...
Não use `/ecl-autonomous` quando:

- **As fases têm decisões de design não resolvidas.** Se você não executou `/ecl-discuss-phase` e seu `PROJECT.md` não registra suas preferências, o smart discuss fará escolhas autônomas com as quais você pode não concordar. Execute o discuss de forma interativa primeiro, ou use `--interactive`.

- **Você precisa de controle detalhado sobre uma única fase.** Para uma fase, `/ecl-execute-phase N` fornece saída passo a passo e permite que você reaja antes de continuar. O modo autônomo é projetado para execuções em lote sem supervisão.
...
- **Você está no meio de uma fase com execução parcial.** O modo autônomo retoma fases incompletas, mas não retoma uma onda parcialmente executada. Use `/ecl-execute-phase N` para concluir uma fase que já está em andamento.
...
```bash
/ecl-autonomous --from 4     # substitua 4 pelo número da primeira fase incompleta
```

O eCL ignora automaticamente as fases já concluídas, portanto é seguro executar novamente a partir de um número de fase anterior caso não tenha certeza de onde a execução parou.
