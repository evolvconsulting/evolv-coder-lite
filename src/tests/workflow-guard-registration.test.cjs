...
/**
 * Regression guard for #1767: ecl-workflow-guard.js must be registered in settings.json
 *
...
describe('workflow-guard hook registration (#1767)', () => {
  test('install.js constructs a command path variable for ecl-workflow-guard.js', () => {
    ...
    const commandConstructionLines = lines.filter(line =>
      line.includes('ecl-workflow-guard.js') &&
      (line.includes('buildHookCommand') || line.includes("'node '"))
    );
    assert.ok(
      commandConstructionLines.length > 0,
      [
        'install.js must construct a command path for ecl-workflow-guard.js',
...
describe('hook registration completeness anti-pattern guard', () => {
  test('every JS hook in gsdHooks (except module-owned) has a command construction in install.js', () => {
    ...
    // Cursor hooks (ecl-cursor-*.js) are registered by writeCursorHooksJson in
    ...
    const jsHooks = ECL_UNINSTALL_HOOKS.filter(h => h.endsWith('.js') && !MODULE_OWNED_HOOKS.has(h) && !h.endsWith('-worker.js'));
...
describe('atomic write temp-tracking — shared Set parity guard', () => {
  ...
  test('writeCursorHooksJson atomic write registers temp path in shared __atomicWrittenTmps', (t) => {
    ...
    fs.writeFileSync(path.join(srcHooksDir, 'ecl-cursor-session-start.js'), '// stub\n');
    fs.writeFileSync(path.join(srcHooksDir, 'ecl-cursor-post-tool.js'), '// stub\n');
...
describe('cursor hook registration — behavioral guard (module-owned hooks)', () => {
  ...
  test('writeCursorHooksJson writes both ecl-cursor-session-start and ecl-cursor-post-tool to hooks.json', (t) => {
    ...
    const { writeCursorHooksJson, ECL_CURSOR_HOOK_MARKER } = require('../bin/install.js');
    ...
    assert.ok(
      ...
      'hooks.json must contain a sessionStart entry (ecl-cursor-session-start.js)'
    );
    assert.ok(
      Array.isArray(hookTable.postToolUse) && hookTable.postToolUse.length > 0,
      'hooks.json must contain a postToolUse entry (ecl-cursor-post-tool.js)'
    );

    // Both entries must be eCL-managed (not user-authored stubs).
    assert.equal(
      hookTable.sessionStart[0][ECL_CURSOR_HOOK_MARKER], true,
      'sessionStart entry must carry the eCL managed marker'
    );
    assert.equal(
      hookTable.postToolUse[0][ECL_CURSOR_HOOK_MARKER], true,
      'postToolUse entry must carry the eCL managed marker'
    );
    ...
    assert.ok(
      sessionCmd.includes('ecl-cursor-session-start.js'),
      `sessionStart command must reference ecl-cursor-session-start.js, got: ${sessionCmd}`
    );
    assert.ok(
      postToolCmd.includes('ecl-cursor-post-tool.js'),
      `postToolUse command must reference ecl-cursor-post-tool.js, got: ${postToolCmd}`
    );
  });
});
