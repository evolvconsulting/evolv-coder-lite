...
eCL Core can route review requests to any combination of: Gemini CLI, Claude (separate session), Codex CLI, CodeRabbit, OpenCode, Qwen Code, Cursor, Antigravity CLI, Ollama, LM Studio, and llama.cpp.
...
By default, `/ecl-review` runs all detected CLIs. To pin a subset as project defaults:
...
Alternatively, set it directly with `ecl-tools`:
...
eCL invokes each reviewer in sequence, collects structured feedback (Summary, Strengths, Concerns at HIGH/MEDIUM/LOW, Suggestions, Risk Assessment), and writes the combined output to `.planning/phases/03-.../03-REVIEWS.md`.
...
Configure the host addresses and model selection under `review.*` keys via `/ecl-config --integrations` if the defaults (`localhost:11434` / `localhost:1234`) do not apply.
...
**Stall detection:** if the HIGH-concern count is not decreasing across cycles, eCL warns you. When the cycle cap is reached with open HIGH concerns, an escalation gate asks whether to proceed or review manually.
...
| You need maximum coverage before a release | `/ecl-plan-review-convergence N --all` |
| You're iterating quickly and want fast feedback | pick one CLI: `/ecl-review --phase N --gemini` |
