 ...
 * migrations for eCL runtime config directories.
 *
...
const INSTALL_STATE_NAME = 'ecl-install-state.json';
const INSTALL_MIGRATION_LOCK_NAME = 'ecl-install-migration.lock';
...
function applyInstallerMigrationPlan({
  ...
  const journalRelPath = path.posix.join('ecl-migration-journal', `${runId}.json`);
  const journalPath = path.join(configDir, journalRelPath);
  const rollbackRootRelPath = path.posix.join('ecl-migration-journal', `${runId}-rollback`);
  const rollbackRoot = path.join(configDir, rollbackRootRelPath);
  const backupRootRelPath = path.posix.join('ecl-migration-journal', `${runId}-backups`);
