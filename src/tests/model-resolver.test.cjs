...
const modelResolver = require('../evolv-coder-lite/bin/lib/model-resolver.cjs');
...
function makeTempProject(prefix = 'ecl-model-resolver-test-') {
...
describe('resolveModelInternal', () => {
  ...
  test('no config -> balanced profile -> ecl-planner resolves to a string', () => {
    const model = resolveModelInternal(tmpDir, 'ecl-planner');
  ...
  test('model_overrides takes precedence over everything else', () => {
    writeConfig(tmpDir, { model_overrides: { 'ecl-planner': 'my-custom-model' } });
    assert.strictEqual(resolveModelInternal(tmpDir, 'ecl-planner'), 'my-custom-model');
  });

  test('model_profile=quality -> opus-class model for ecl-planner', () => {
  ...
  test('model_profile=budget -> haiku-class model for ecl-planner', () => {
  ...
  test('models config per phase type overrides profile tier', () => {
    ...
    // ecl-planner maps to planning phase type; config says opus
  ...
  test('runtime non-claude + model_profile_overrides for runtime tier', () => {
    ...
    // ecl-codebase-mapper is light tier -> haiku in balanced profile
    const model = resolveModelInternal(tmpDir, 'ecl-codebase-mapper');
    assert.ok(typeof model === 'string' && model.length > 0);
  });
});
...
describe('resolveEffortInternal', () => {
  ...
  test('invocation override beats everything', () => {
    writeConfig(tmpDir, { effort: { agent_overrides: { 'ecl-planner': 'low' } } });
...
describe('resolveFastModeInternal', () => {
  ...
  test('opts.override=true beats config', () => {
    writeConfig(tmpDir, { fast_mode: { agent_overrides: { 'ecl-planner': false } } });
    assert.strictEqual(resolveFastModeInternal(tmpDir, 'ecl-planner', { override: true }), true);
  });
  ...
  test('agent_overrides beats enabled', () => {
    writeConfig(tmpDir, {
      fast_mode: { enabled: true, agent_overrides: { 'ecl-planner': false } },
    });
    assert.strictEqual(resolveFastModeInternal(tmpDir, 'ecl-planner'), false);
  });
...
describe('resolveEffortForTier', () => {
  ...
  test('dynamic_routing disabled -> attempt has no effect', () => {
    ...
    const at1 = resolveEffortForTier(tmpDir, 'ecl-planner', 1);
  ...
  test('escalation clamps at "max"', () => {
    ...
    assert.strictEqual(resolveEffortForTier(tmpDir, 'ecl-planner', 99), 'max');
  });
});
...
describe('resolveModelForTier', () => {
  ...
  test('no dynamic_routing -> falls back to resolveModelInternal', () => {
    const fromForTier = resolveModelForTier(tmpDir, 'ecl-planner');
    const fromInternal = resolveModelInternal(tmpDir, 'ecl-planner');
    assert.strictEqual(fromForTier, fromInternal);
  });

  test('model_overrides wins before dynamic routing logic', () => {
    writeConfig(tmpDir, {
      model_overrides: { 'ecl-planner': 'override-model' },
      dynamic_routing: {
        enabled: true,
        tier_models: { light: 'haiku', standard: 'sonnet', heavy: 'opus' },
        escalate_on_failure: true,
    ...
    assert.strictEqual(resolveModelForTier(tmpDir, 'ecl-planner'), 'override-model');
  });
  ...
  test('dynamic_routing + tier_models + attempt=0 -> default tier model', () => {
    ...
    // ecl-codebase-mapper is 'light' tier
    assert.strictEqual(resolveModelForTier(tmpDir, 'ecl-codebase-mapper', 0), 'haiku-custom');
  });
  ...
  test('dynamic_routing + attempt=1 escalates tier', () => {
    ...
    // ecl-codebase-mapper light -> attempt=1 -> standard
    assert.strictEqual(resolveModelForTier(tmpDir, 'ecl-codebase-mapper', 1), 'sonnet-custom');
  });
});
...
describe('ADVERSARIAL: edge cases', () => {
  ...
  test('resolveEffortInternal: invalid override "turbo" falls through to tier default', () => {
    const result = resolveEffortInternal(tmpDir, 'ecl-planner', { override: 'turbo' });
    // ecl-planner is heavy -> tier default xhigh
  ...
  test('resolveEffortForTier: max_escalations=0 caps escalation', () => {
    ...
    const at0 = resolveEffortForTier(tmpDir, 'ecl-codebase-mapper', 0);
