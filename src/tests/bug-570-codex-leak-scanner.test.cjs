 ...
 *   Fix: scan only files listed in ecl-file-manifest.json.
 *
...
describe('#570 — Codex leak scanner sub-bugs', { concurrency: false }, () => {
  ...
  test('convertClaudeToCodexMarkdown replaces bare ~/.claude (no trailing slash)', () => {
    ...
    const out = convertClaudeCommandToCodexSkill(input, 'ecl-test');
  ...
  test('writeManifest includes .toml agent files for Codex', () => {
    ...
    const tomlFiles = fs.existsSync(agentsDir)
      ? fs.readdirSync(agentsDir).filter((f) => f.startsWith('ecl-') && f.endsWith('.toml'))
      : [];
    assert.ok(
      tomlFiles.length > 0,
      `Precondition: Codex install must write at least one ecl-*.toml in agents/; found none in ${agentsDir}`,
    );

    const manifestPath = path.join(codexHome, 'ecl-file-manifest.json');
    assert.ok(
      fs.existsSync(manifestPath),
      `ecl-file-manifest.json must exist after install; not found at ${manifestPath}`,
    );
    ...
    const tomlManifestKeys = manifestKeys.filter(
      (k) => k.startsWith('agents/ecl-') && k.endsWith('.toml'),
    );
    assert.ok(
      tomlManifestKeys.length > 0,
      `Expected at least one 'agents/ecl-*.toml' key in manifest.files, but found none.\n` +
