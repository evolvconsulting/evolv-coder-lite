...
/**
 * eCL Tools Tests - Phase
 *
 * Consolidated from 20 → 4 test files (issue #3740). This file covers the
 * CJS phase CLI layer (phase.cjs + ecl-tools.cjs): add, remove, complete,
...
const ECL_TOOLS_BIN = path.resolve(__dirname, '..', 'evolv-coder-lite', 'bin', 'ecl-tools.cjs');
...
`
  ...
  test('CJS scanner [999, 1000] fixture: skips exactly 999 and returns 1001 (regression #3774)', () => {
    ...
    //
    // ECL_WORKSTREAM=ws1 forces the CJS fallback in phase-command-router.cjs.
    // When ECL_WORKSTREAM is set, planningDir resolves to
    ...
    const result = runGsdTools('phase add After One Thousand', tmpDir, { ECL_WORKSTREAM: ws });
...
const { comparePhaseNum, normalizePhaseName } = require('../evolv-coder-lite/bin/lib/phase-id.cjs');
...
/**
 * Run `ecl-tools phase complete <phase>` for the phase-complete regression
...
function runPhaseComplete(tmpDir, { phase = '1', tolerateExit = false } = {}) {
  try {
    return execFileSync('node', [ECL_TOOLS_BIN, 'phase', 'complete', phase], {
      cwd: tmpDir,
      timeout: 60000,
      encoding: 'utf-8',
    ...
    if (err.killed || err.signal != null || err.code === 'ETIMEDOUT') {
      throw new Error(
        `ecl-tools phase complete ${phase} was killed before completion ` +
...
describe('bug #1998: phase complete updates overview checkbox', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-1998-'));
...
describe('bug #2005: phase complete updates plan count when milestone is inside <details>', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-2005-'));
    planningDir = path.join(tmpDir, '.planning');
    fs.mkdirSync(planningDir, { recursive: true });
...
describe('bug #2526: phase complete warns about unregistered REQ-IDs', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-2526-'));
    planningDir = path.join(tmpDir, '.planning');
    fs.mkdirSync(planningDir, { recursive: true });
...
{
  const PMG_WF = path.join(__dirname, '..', 'evolv-coder-lite', 'workflows', 'plan-milestone-gaps.md');
  const IMPORT_WF = path.join(__dirname, '..', 'evolv-coder-lite', 'workflows', 'import.md');
  const BACKLOG_WF = path.join(__dirname, '..', 'evolv-coder-lite', 'workflows', 'add-backlog.md');
  ...
  describe('bug-3298 — import.md must not construct bare {NN}-{slug} phase dirs', () => {
    ...
    test('plan_convert step must call init.phase-op to resolve the prefixed dir', () => {
      ...
      assert.ok(
        ...
        'import.md must call ecl-sdk query init.phase-op to get expected_phase_dir with project_code prefix',
      );
    });
  });
...

{
  ...
  describe('bug #3517: phase.complete leaves STATE.md with stale fields', () => {
    ...
    beforeEach(() => {
      tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-3517-'));
    });
    ...
    test('prose-block STATE keeps next phase name without field-miss warnings (#1316)', () => {
      ...
      const result = spawnSync(process.execPath, [ECL_TOOLS_BIN, 'phase', 'complete', '32'], {
        cwd: tmpDir,
        encoding: 'utf8',
        env: process.env,
      });
