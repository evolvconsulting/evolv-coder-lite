...
/**
 * Bug #2410 — /ecl:manager background execute-phase Task fails with
...
describe('bug #2410: checkpoint heartbeat format is user-documented', () => {
  ...
  test('COMMANDS.md documents the [checkpoint] format under /ecl-manager', () => {
    const managerIdx = commandsDoc.indexOf('### `/ecl-manager`');
    assert.ok(managerIdx !== -1, '/ecl-manager section should exist');
    const section = commandsDoc.slice(managerIdx, managerIdx + 4000);
    assert.ok(
      /\[checkpoint\]/.test(section),
      'COMMANDS.md /ecl-manager section should document [checkpoint] heartbeat markers'
    );
    assert.ok(
