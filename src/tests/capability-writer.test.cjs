 ...
 * Tests run against the REAL registry and compiled .cjs in evolv-coder-lite/bin/lib/.
 */
...
const { setCapabilityState } = require('../evolv-coder-lite/bin/lib/capability-writer.cjs');
const { readSurface } = require('../evolv-coder-lite/bin/lib/surface.cjs');

// ─── Helpers ──────────────────────────────────────────────────────────────────

/**
 * Create a temp runtimeConfigDir and cwd pair for a test.
 * - rcd has no .ecl-profile file → default 'full' profile → installedSkills='*'
...
describe('capability-writer: setCapabilityState', () => {
  ...
  test('CLI: capability set ui --off exits 0; capability set ui --on exits 0; unknown id exits non-zero', () => {
    ...
    try {
      ...
      const gsdToolsBin = path.resolve(__dirname, '../evolv-coder-lite/bin/ecl-tools.cjs');
  ...
  test('Fix A: disable when explicitAdds holds cap skill stems → skill stems removed, ui.enabled===false, no errors', () => {
    ...
    try {
      ...
      const surfacePath = path.join(rcd, '.ecl-surface.json');
