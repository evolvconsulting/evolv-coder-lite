...
const COMMAND_PATH = path.join(ROOT, 'commands', 'ecl', 'phase.md');
const WORKFLOW_PATH = path.join(ROOT, 'evolv-coder-lite', 'workflows', 'edit-phase.md');

// ─── File existence ──────────────────────────────────────────────────────────

describe('edit-phase: file existence', () => {
  test('commands/ecl/phase.md exists (absorbed edit-phase in #2790)', () => {
    assert.ok(fs.existsSync(COMMAND_PATH), 'commands/ecl/phase.md should exist (consolidates edit-phase)');
  });

  test('evolv-coder-lite/workflows/edit-phase.md exists', () => {
    assert.ok(fs.existsSync(WORKFLOW_PATH), 'evolv-coder-lite/workflows/edit-phase.md should exist');
  });
});
...
describe('edit-phase: command file structure', () => {
  test('consolidated phase.md has correct name frontmatter (#2790)', () => {
    ...
    assert.ok(/^name:\s*ecl:phase/m.test(content), 'name should be ecl:phase (consolidated)');
  });
...
describe('edit-phase: documentation registration', () => {
  test('INVENTORY.md routes edit-phase workflow through consolidated /ecl-phase --edit (#2790)', () => {
    // #2790 absorbed /ecl-edit-phase into /ecl-phase as the --edit flag. The
    ...
    // "Invoked by" column documents /ecl-phase --edit (not the deleted form).
    ...
    assert.ok(
      /\/ecl-phase\s+--edit/.test(invokedBy),
      `edit-phase.md row must list "/ecl-phase --edit" as caller; got: "${invokedBy}"`
    );
    assert.ok(
      !/\/ecl-edit-phase\b/.test(invokedBy),
      `edit-phase.md row must not still cite the deleted /ecl-edit-phase command; got: "${invokedBy}"`
    );
  });
  ...
  test('INVENTORY-MANIFEST.json contains /ecl-phase in commands (#2790: edit-phase absorbed into phase.md)', () => {
    // #2790: /ecl-edit-phase was absorbed into /ecl-phase as the --edit flag.
    // The manifest now records /ecl-phase instead of /ecl-edit-phase.
    const manifest = JSON.parse(
      fs.readFileSync(path.join(ROOT, 'docs', 'INVENTORY-MANIFEST.json'), 'utf-8')
    );
    assert.ok(
      manifest.families.commands.includes('/ecl-phase'),
      'INVENTORY-MANIFEST.json must list /ecl-phase in commands (absorbed /ecl-edit-phase via #2790)'
    );
  });
  ...
  test('docs/COMMANDS.md documents /ecl-phase (absorbed /ecl-edit-phase via --edit flag, #2790)', () => {
    const commands = fs.readFileSync(
      path.join(ROOT, 'docs', 'COMMANDS.md'),
      'utf-8'
    );
    assert.ok(
      commands.includes('/ecl-phase'),
      'docs/COMMANDS.md must document /ecl-phase (which absorbed /ecl-edit-phase via --edit flag in #2790)'
    );
  });
});
