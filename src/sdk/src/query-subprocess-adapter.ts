...
export class QuerySubprocessAdapter {
          ...
          try {
            ...
            reject(
              this.deps.createFailureError(
                `Failed to parse ecl-tools output for "${command}": ${parseErr instanceof Error ? parseErr.message : String(parseErr)}\nRaw output: ${raw.slice(0, 500)}`,
                command,
                args,
                0,
                stderrStr,
  ...
  private processExecutionError(
    ...
    return this.deps.createFailureError(
      `ecl-tools exited with code ${error.code ?? 'unknown'}: ${command} ${args.join(' ')}${stderrStr ? `\n${stderrStr}` : ''}`,
      command,
      args,
  ...
  private processSpawnError(command: string, args: string[], err: Error) {
    return this.deps.createFailureError(`Failed to execute ecl-tools: ${err.message}`, command, args, null, '');
  }
  ...
  private async parseOutput(raw: string): Promise<unknown> {
    ...
    if (jsonStr.startsWith('@file:')) {
      ...
      try {
        ...
        throw new Error(`Failed to read ecl-tools @file: indirection at "${resolvedPath}": ${reason}`);
      }
    }
