...
function searchPhaseInContent(content: string, escapedPhase: string, phaseNum: string): PhaseSearchResult | null {
  ...
  // Also matches bracket-prefixed headings like ### [eCL] Phase 2-01:.
...
function cmdRoadmapAnalyze(cwd: string, raw: boolean): void {
  ...
  while ((match = phasePattern.exec(content)) !== null) {
    ...
    // Phases completed before eCL tracking (or via external tools) may lack
...
function cmdRoadmapUpdatePlanProgress(cwd: string, phaseNum: string | null | undefined, raw: boolean): void {
  ...
  withPlanningLock(cwd, () => {
    ...
    //   `**Plans**: N plans`  — bold word + outer colon (evolv-coder-lite/templates/roadmap.md)
    ...
    if (missingPlans.length > 0) {
      ...
      //
      // Canonical template (evolv-coder-lite/templates/roadmap.md) uses BOTH lines:
