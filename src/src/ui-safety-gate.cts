 ...
 *   echo "$PHASE_SECTION" | node evolv-coder-lite/bin/lib/ui-safety-gate.cjs
 *   echo $?   → 0 if UI tokens found, 1 if not, 2 on usage error
 *
 * Exit codes mirror grep: 0 = match found, 1 = no match, 2 = usage error.
 *
 * Canonical location: evolv-coder-lite/bin/lib/ui-safety-gate.cjs (#448)
 * This path is deployed by the eCL installer to $RUNTIME_DIR/evolv-coder-lite/bin/lib/.
