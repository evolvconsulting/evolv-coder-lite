...
/**
 * Deterministic latest-version check for /ecl-update (#2992).
 *
 * The /ecl-update workflow's check_latest_version step was previously
 * prescribed in LLM-driven prose ("run `npm view evolv-coder-lite
 * version`"). The executing model could shortcut the prescription and
 * invent npm queries against wrong-shaped names (`@evolv-coder-lite/cli`,
 * `evolv-coder-lite-cli`, `ecl`), all of which 404 or — worse — return an
 ...
 * `npm view @evolvconsulting/evolv-coder-lite@next version` resolves the RC channel (#660).
 */
...
function checkLatestVersion(opts = {}) {
  ...
  // Bounded at 15s so a hung registry doesn't block /ecl-update (#2993 CR).
  const defaultSpawn = () => {
