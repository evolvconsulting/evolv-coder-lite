...
/**
 * eCL Secure-Phase Tests
 *
 * Validates the security-first enforcement layer:
 * - ecl-security-auditor agent frontmatter and structure
...
// ─── 1. Agent frontmatter — ecl-security-auditor.md ─────────────────────────

describe('SECURE: ecl-security-auditor agent', () => {
  const agentPath = path.join(AGENTS_DIR, 'ecl-security-auditor.md');

  test('agent file exists', () => {
    assert.ok(
      fs.existsSync(agentPath),
      'ecl-security-auditor.md must exist in agents/'
    );
  });
  ...
  test('name is ecl-security-auditor', () => {
    const content = fs.readFileSync(agentPath, 'utf-8');
    const frontmatter = content.split('---')[1] || '';
    assert.ok(
      frontmatter.includes('name: ecl-security-auditor'),
      'name must be ecl-security-auditor'
    );
  });
...
describe('SECURE: secure-phase command file', () => {
  ...
  test('command file exists', () => {
    assert.ok(
      ...
      'secure-phase.md must exist in commands/ecl/'
    );
  });

  test('has valid frontmatter with name ecl:secure-phase', () => {
    const content = fs.readFileSync(cmdPath, 'utf-8');
    const frontmatter = content.split('---')[1] || '';
    assert.ok(
      frontmatter.includes('name: ecl:secure-phase'),
      'name must be ecl:secure-phase'
    );
  });
...
describe('SECURE: secure-phase workflow file', () => {
  ...
  test('workflow file exists', () => {
    assert.ok(
      ...
      'secure-phase.md must exist in evolv-coder-lite/workflows/'
    );
  });

  test('contains ecl-security-auditor reference', () => {
    const content = fs.readFileSync(wfPath, 'utf-8');
    assert.ok(
      content.includes('ecl-security-auditor'),
      'must reference ecl-security-auditor agent'
    );
  });
...
describe('SECURE: SECURITY.md template', () => {
  ...
  test('template exists', () => {
    assert.ok(
      ...
      'SECURITY.md must exist in evolv-coder-lite/templates/'
    );
  });
...
describe('SECURE: config.json security defaults', () => {
  ...
  test('config template exists', () => {
    assert.ok(
      ...
      'config.json must exist in evolv-coder-lite/templates/'
    );
  });
...
describe('SECURE: VALIDATION.md security columns', () => {
  ...
  test('VALIDATION.md template exists', () => {
    assert.ok(
      ...
      'VALIDATION.md must exist in evolv-coder-lite/templates/'
    );
  });
