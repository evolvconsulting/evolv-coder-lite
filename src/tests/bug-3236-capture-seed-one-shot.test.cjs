...
const PLANT_SEED = path.join(ROOT, 'evolv-coder-lite', 'workflows', 'plant-seed.md');
...
function readPlantSeed() {
  try {
    ...
    throw new Error('evolv-coder-lite/workflows/plant-seed.md not found: ' + err.message);
  }
}
...
describe('#3236: plant-seed.md one-shot capture contract', () => {
  test('plant-seed.md exists', () => {
    assert.ok(
      ...
      'evolv-coder-lite/workflows/plant-seed.md does not exist',
    );
  });
