...
interface CmdGenerateClaudeMdOptions {
  output?: string;
  auto?: boolean;
  // #1098: overwrite an existing instruction file that has no eCL section
...
function extractSectionContent(fileContent: string, sectionName: string): string | null {
  const startMarker = `<!-- eCL:${sectionName}-start`;
  const endMarker = `<!-- eCL:${sectionName}-end -->`;
...
function buildSection(sectionName: string, sourceFile: string, content: string): string {
  return [
    `<!-- eCL:${sectionName}-start source:${sourceFile} -->`,
    content,
    `<!-- eCL:${sectionName}-end -->`,
  ].join('\n');
}
...
function generateSkillsSection(cwd: string): SectionResult {
  ...
  for (const dir of SKILL_SEARCH_DIRS) {
    ...
    for (const entry of entries) {
      ...
      // Skip eCL's own installed skills — only surface project-specific skills
...
function cmdWriteProfile(cwd: string, options: CmdWriteProfileOptions, raw: boolean): void {
  ...
  if (!outputPath) {
    ...
    outputPath = path.join(getGlobalConfigDir(effectiveRuntime), 'evolv-coder-lite', 'USER-PROFILE.md');
...
function cmdGenerateDevPreferences(cwd: string, options: CmdGenerateDevPreferencesOptions, raw: boolean): void {
  ...
  // commands/ecl subdirectory in favor of skills/<skill>/SKILL.md under
  // the runtime config dir. This writer was missed in the migration
  // (PR #1540 targeted eCL-shipped command files; dev-preferences is a
  // runtime-generated user artifact). Default now points at the skills/
  // location so /ecl:profile-user --refresh stops re-creating the legacy
  ...
  if (!outputPath) {
    ...
    const skillDir = getGlobalSkillDir(effectiveRuntime, 'ecl-dev-preferences');
    if (!skillDir) {
...
function cmdGenerateClaudeProfile(cwd: string, options: CmdGenerateClaudeProfileOptions, raw: boolean): void {
  ...
  const sectionLines = [
    ...
    '## Developer Profile',
    '',
    `> Generated by eCL from ${dataSource}. Run \`${String(formatGsdSlash('profile-user', resolveRuntime(cwd)))}\` to update.`,
    '',
  ...
  if (existingContent !== null) {
    const startMarker = '<!-- eCL:profile-start -->';
    const endMarker = '<!-- eCL:profile-end -->';
...
function cmdGenerateClaudeMd(cwd: string, options: CmdGenerateClaudeMdOptions, raw: boolean): void {
  ...
  const sectionHeadings: Record<string, string> = {
    ...
    workflow: '## eCL Workflow Enforcement',
  };
  ...
  // repo-root `CLAUDE.md`, so generated eCL content does not land next to — or
  ...
  try {
    ...
    // ECL_RUNTIME env var takes precedence over config.runtime, mirroring detectRuntime().
  ...
  if (existingContent === null) {
  ...
  } else if (!/<!-- eCL:[a-z]+-start/.test(existingContent) && !options.force) {
    // #1098: the target instruction file already exists and contains NO eCL
    // section markers — it is a hand-crafted CLAUDE.md/AGENTS.md, not a
    // eCL-managed one. Do NOT clobber it with generated project documentation
    // (broad project detail belongs in PROJECT.md / REQUIREMENTS.md, which eCL
    // already owns). Leave the file untouched and report a skip; `--force`
    // overwrites intentionally.
    output({
      claude_md_path: outputPath,
      action: 'skipped',
      reason: 'existing instruction file has no eCL markers (hand-crafted); not overwriting. Pass --force to overwrite.',
      sections_generated: [],
      sections_fallback: [],
      sections_skipped: MANAGED_SECTIONS,
      sections_total: MANAGED_SECTIONS.length,
      profile_status: 'skipped',
      message: `Left existing ${path.basename(outputPath)} untouched (no eCL markers found). Broad project context lives in PROJECT.md / REQUIREMENTS.md; pass --force to overwrite this file with eCL-managed sections.`,
    }, raw, undefined);
    return;
  } else {
    action = 'updated';
    ...
    for (const name of MANAGED_SECTIONS) {
      ...
      const hasMarkers = fileContent.indexOf(`<!-- eCL:${name}-start`) !== -1;
    ...
    if (!options.auto && fileContent.indexOf('<!-- eCL:profile-start') === -1) {
  ...
  if (finalContent && finalContent.indexOf('<!-- eCL:profile-start') !== -1) {
    if (action === 'created' || existingContent.indexOf('<!-- eCL:profile-start') === -1) {
