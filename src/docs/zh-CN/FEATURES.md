...
> eCL Core 的功能索引与参考文档。架构细节请参见[架构文档](ARCHITECTURE.md)。命令语法请参见[命令参考](COMMANDS.md)。返回[文档索引](README.md)。
...
- [v1.34.0 功能](#v1340-features)
  ...
  - [/ecl-progress --next 中的硬停止安全门控](#101-hard-stop-safety-gates-in-ecl-progress---next)
...
- [v1.35.0 功能](#v1350-features)
  ...
  - [eCL-2 反向迁移](#105-ecl-2-reverse-migration)
...
**命令：** `/ecl-new-project [--auto @file.md]`
...
- 如果存在来自 `/ecl-map-codebase` 的代码库上下文，将自动加载

---

### 2. 阶段讨论

**命令：** `/ecl-discuss-phase [N] [--auto] [--batch]`
...
**命令：** `/ecl-plan-phase [N] [--auto] [--skip-research] [--skip-verify]`
...
- REQ-PLAN-07：当检测到前端阶段且不存在 UI-SPEC.md 时，系统必须提示用户运行 `/ecl-ui-phase`（UI 安全门控）
...
**命令：** `/ecl-audit-milestone`、`/ecl-complete-milestone`、`/ecl-new-milestone [name]`
...
**命令：** `/ecl-phase`、`/ecl-phase --insert [N]`、`/ecl-phase --remove [N]`
...
**命令：** `/ecl-quick [--full] [--discuss] [--research]`

**目的：** 临时任务执行，具备 eCL 保证但路径更快。
...
**命令：** `/ecl-progress --do`（另见 `/ecl-manager` 用于交互式路由）

**目的：** 分析自由文本并路由到适当的 eCL 命令。

**需求：**
- REQ-DO-01：系统必须从自然语言输入中解析用户意图
- REQ-DO-02：系统必须将意图映射到最匹配的 eCL 命令
...
- REQ-NEXT-04：如果不存在项目，系统必须建议 `/ecl-new-project`
- REQ-NEXT-05：当所有阶段完成时，系统必须建议 `/ecl-complete-milestone`

**状态检测逻辑：**
| 状态 | 操作 |
|-------|--------|
| 无 `.planning/` 目录 | 建议 `/ecl-new-project` |
| 阶段无 CONTEXT.md | 运行 `/ecl-discuss-phase` |
| 阶段无 PLAN.md 文件 | 运行 `/ecl-plan-phase` |
| 阶段有计划但无 SUMMARY.md | 运行 `/ecl-execute-phase` |
| 阶段已执行但无 VERIFICATION.md | 运行 `/ecl-verify-work` |
| 所有阶段完成 | 建议 `/ecl-complete-milestone` |
...
- REQ-POSTVER-03：系统必须记录问题供 `/ecl-verify-work` 处理
...
**触发时机：** 在 `/ecl-execute-phase` 期间，在验证器步骤之前自动运行。
...
**触发时机：** 在 `/ecl-plan-phase` 结束时，在计划检查器循环之后自动运行。
...
- REQ-CTX-06：上下文监控器必须区分 eCL 激活与非 eCL 激活项目
...
**命令：** `/ecl-pause-work`、`/ecl-resume-work`、`/ecl-progress`
...
**命令：** `/ecl-config --profile <quality|balanced|budget|adaptive|inherit>`
...
**引入版本：** #2003
**触发条件：** 在每次 `/ecl-execute-phase` 结束时自动运行
**配置：**
- `workflow.drift_threshold`（整数，默认 `3`）— 门控触发前的最小新增结构元素数。
- `workflow.drift_action`（`warn` | `auto-remap`，默认 `warn`）— 仅警告或派生 `ecl-codebase-mapper` 并将 `--paths` 限定到受影响的子树。
...
- REQ-DRIFT-05：检测/重映射失败对 `/ecl-execute-phase` 必须是非阻塞的
...
**命令：** `/ecl-capture [desc]`、`/ecl-capture --list`
...
**目的：** 使用变更日志预览将 eCL 更新至最新版本。

**需求：**
- REQ-UPDATE-01：系统必须通过 npm 检查新版本
- REQ-UPDATE-02：系统必须在更新前显示新版本的变更日志
- REQ-UPDATE-03：系统必须感知运行时并针对正确的目录
- REQ-UPDATE-04：系统必须将本地修改的文件备份到 `ecl-local-patches/`
- REQ-UPDATE-05：`/ecl-update --reapply` 必须在更新后恢复本地修改
...
- REQ-SETTINGS-03：系统必须支持保存为全局默认值（`~/.ecl/defaults.json`）
...
- REQ-RUNTIME-05：卸载必须干净地移除所有 eCL 文件，不影响其他配置
...
**更新横幅（选项，当未使用 eCL 状态行时）：**

当用户拒绝（或保留非 eCL）状态行时，安装器提供一个 SessionStart 横幅，在不占用状态行空间的情况下显示更新可用性。横幅读取 `~/.cache/ecl/ecl-update-check.json`（由 `ecl-check-update-worker.js` 写入），仅在有可用更新时输出一行：

```text
eCL update available: 1.39.0 → 1.40.0. Run /ecl-update.
```

无更新时横幅保持静默，"检查失败"诊断每 24 小时限流一次。通过 `npx @evolvconsulting/evolv-coder-lite --uninstall` 或删除引用 `ecl-update-banner.js` 的 SessionStart 条目可干净移除。

### 38. 开发者画像

**命令：** `/ecl-profile-user [--questionnaire] [--refresh]`
...
- `ecl-user-profiler` 智能体 — 从会话数据进行行为分析
...
每次 `/ecl-progress` 调用都会扫描当前里程碑中的所有阶段，查找未处理项目（pending、skipped、blocked、human_needed）。显示带可操作链接的非阻塞警告节区。
...
**需求：**
- REQ-DEBT-01：系统必须在 `/ecl-progress` 中呈现所有先前阶段的未处理 UAT/验证项目
...
- REQ-DEBT-06：`/ecl-audit-uat` 必须扫描所有阶段，按可测试性分类项目，并生成人工测试计划
...
**何时使用 vs `/ecl-quick`：**
- `/ecl-fast` — 可在 2 分钟内完成的一句话任务（拼写错误、配置更改、小型添加）
- `/ecl-quick` — 任何需要研究、多步骤规划或验证的事项

---

### 42. 跨 AI 同行评审

**命令：** `/ecl-review --phase N [--gemini] [--claude] [--codex] [--coderabbit] [--opencode] [--qwen] [--cursor] [--agy] [--ollama] [--lm-studio] [--llama-cpp] [--all]`
...
- REQ-REVIEW-05：审查结果必须可被 `/ecl-plan-phase --reviews` 使用
...
**用户配置说明：**
- 在 `.planning/config.json` 中（或通过 `ecl config-set`）设置 `review.default_reviewers`，控制无标志 `/ecl-review` 的扇出。
...
**命令：** `/ecl-capture --backlog <description>`、`/ecl-review-backlog`、`/ecl-capture --seed <idea>`

**目的：** 捕获尚未准备好进行主动规划的想法。待办事项使用 999.x 编号，保持在活跃阶段序列之外。种子是具有触发条件的前瞻性想法，在适当的里程碑时自动浮现。

**需求：**
- REQ-BACKLOG-01：待办事项必须使用 999.x 编号，保持在活跃阶段序列之外
- REQ-BACKLOG-02：必须立即创建阶段目录，以便 `/ecl-discuss-phase` 和 `/ecl-plan-phase` 可以在其上运行
- REQ-BACKLOG-03：`/ecl-review-backlog` 必须支持每个项目的提升、保留和删除操作
- REQ-BACKLOG-04：提升的项目必须重新编号进入活跃里程碑序列
- REQ-SEED-01：种子必须捕获完整的原因和浮现时机条件
- REQ-SEED-02：`/ecl-new-milestone` 必须扫描种子并呈现匹配项
...
**命令：** `/ecl-thread [name | description]`

**目的：** 跨会话的轻量级知识存储，用于跨多个会话但不属于任何特定阶段的工作。比 `/ecl-pause-work` 更轻量 — 无阶段状态，无计划上下文。
...
**命令：** `/ecl-pr-branch [target branch]`

**目的：** 通过过滤掉 `.planning/` 提交，创建适合拉取请求的干净分支。审查者只看到代码更改，而不是 eCL 规划构件。
...
**目的：** eCL 规划构件的纵深防御安全机制。由于 eCL 生成的 Markdown 文件会成为 LLM 系统提示，流入这些文件的用户控制文本是潜在的间接提示注入向量。
...
**2. 提示注入守护钩子**（`ecl-prompt-guard.js`）
...
**目的：** 在 `/ecl-discuss-phase` 期间自动生成 `DISCUSSION-LOG.md`，提供讨论期间做出决策的完整审计追踪。
...
**命令：** `/ecl-forensics [description]`

**目的：** 对失败或卡住的 eCL 工作流进行事后调查。
...
**命令：** `/ecl-milestone-summary [version]`
...
**命令：** `/ecl-discuss-phase` 配合 `workflow.discuss_mode: 'assumptions'`
...
**属于：** `/ecl-new-project` 和 `/ecl-progress`

**目的：** 自动检测 UI 密集型项目并呈现 `/ecl-ui-phase` 建议。

**需求：**
- REQ-UI-DETECT-01：系统必须检测项目描述中的 UI 信号（关键字、框架引用）
- REQ-UI-DETECT-02：当适用时，系统必须在 ROADMAP.md 阶段中添加 `ui_hint` 注释
- REQ-UI-DETECT-03：系统必须在 UI 密集型阶段的后续步骤中建议 `/ecl-ui-phase`
- REQ-UI-DETECT-04：系统不得将 `/ecl-ui-phase` 设为强制性

**流程：**
1. **检测** — 扫描项目描述和技术栈中的 UI 信号（关键字、框架引用）
2. **标注** — 在 ROADMAP.md 中为适用阶段添加 `ui_hint` 标记
3. **呈现** — 在 UI 密集型阶段的后续步骤中包含 `/ecl-ui-phase` 建议

---

### 55. 多运行时安装选择

**属于：** `npx @evolvconsulting/evolv-coder-lite`
...
**目的：** 将 Windsurf 添加为 eCL 安装和执行支持的 AI CLI 运行时。

**需求：**
- REQ-WINDSURF-01：安装器必须检测 Windsurf 运行时并将其作为目标提供
- REQ-WINDSURF-02：eCL 命令必须在 Windsurf 会话中正确运行

**流程：**
1. **检测** — 识别系统上 Windsurf 运行时的可用性
2. **安装** — 为 Windsurf 环境配置 eCL 技能和钩子
...
- REQ-SCHEMA-04：系统必须支持 `ECL_SKIP_SCHEMA_CHECK` 环境变量以覆盖检测
...
**配置：** `ECL_SKIP_SCHEMA_CHECK` 环境变量，用于绕过检测。

---

### 60. 安全强制执行

**命令：** `/ecl-secure-phase <N>`
...
- REQ-SEC-04：系统必须派生 `ecl-security-auditor` 智能体进行分析
...
**流程：**
1. **建模** — 从阶段实现上下文构建威胁模型
2. **审计** — 派生 `ecl-security-auditor` 根据威胁模型进行验证
...
**需求：**
- REQ-DOCS-01：系统必须派生 `ecl-doc-writer` 智能体生成文档
- REQ-DOCS-02：系统必须派生 `ecl-doc-verifier` 智能体检查准确性
...
**流程：**
1. **生成** — 派生 `ecl-doc-writer` 从实现创建或更新文档
2. **验证** — 派生 `ecl-doc-verifier` 根据代码库检查文档准确性
3. **输出** — 生成带准确性注释的已验证文档
...
**属于：** `/ecl-plan-phase --research-phase <N>`
...
**目的：** 将 eCL 命令迁移到 Claude Code 2.1.88+ 技能格式，同时保持向后兼容性。

**需求：**
- REQ-SKILLS-01：安装器必须为 Claude Code 2.1.88+ 写入 `skills/ecl-*/SKILL.md`
- REQ-SKILLS-02：安装器必须自动清理旧版 `commands/ecl/` 目录
- REQ-SKILLS-03：安装器必须通过 Gemini 路径维护与旧版 Claude Code 的向后兼容性

**流程：**
1. **检测** — 检查 Claude Code 版本以确定技能支持情况
2. **迁移** — 为每个 eCL 命令写入 `skills/ecl-*/SKILL.md` 文件
3. **清理** — 如果已安装技能，则删除旧版 `commands/ecl/` 目录
...
**命令：** `/ecl-manager --analyze-deps`

**目的：** 在运行 `/ecl-manager` 之前检测阶段依赖关系，并建议在 ROADMAP.md 中添加 `Depends on` 条目。
...
**目的：** 将 eCL 安装扩展到 Trae IDE、Cline 和 Augment Code 运行时。
...
**钩子：** `ecl-validate-commit.sh`、`ecl-session-state.sh`、`ecl-phase-boundary.sh`

**目的：** eCL 项目的可选 git 和会话钩子，在配置中通过 `hooks.community: true` 门控。

**需求：**
- REQ-COMMUNITY-01：所有社区钩子在 `.planning/config.json` 中 `hooks.community` 为 `true` 之前必须为无操作
- REQ-COMMUNITY-02：`ecl-validate-commit.sh` 必须对 git commit 消息强制执行常规提交格式
- REQ-COMMUNITY-03：`ecl-session-state.sh` 必须跟踪会话状态转换
- REQ-COMMUNITY-04：`ecl-phase-boundary.sh` 必须强制执行阶段边界检查
...
**命令：** `/ecl-map-codebase --query [<term>|status|diff|refresh]`
**配置：** `intel.enabled`
...
**参考：** `evolv-coder-lite/references/gates.md`
...
**命令：** `/ecl-code-review`、`/ecl-code-review --fix`

**目的：** 对阶段期间更改的源文件进行结构化审查，并通过单独的自动修复过程，每次修复以原子化提交。

**需求：**
- REQ-REVIEW-01：`ecl-code-review` 必须使用 SUMMARY.md 和 git diff 回退将文件范围限定到阶段
- REQ-REVIEW-02：审查必须支持三个深度级别：`quick`、`standard`、`deep`
- REQ-REVIEW-03：发现结果必须按严重性分类：Critical、Warning、Info
- REQ-REVIEW-04：`ecl-code-review --fix` 必须读取 REVIEW.md 并默认修复 Critical + Warning 发现
...
**命令：** `/ecl-explore [topic]`

**目的：** 在提交计划之前，通过苏格拉底式探究性问题引导开发者探索想法。将输出路由到适当的 eCL 构件：笔记、待办事项、种子、研究问题、需求更新或新阶段。

**需求：**
- REQ-EXPLORE-01：探索必须使用苏格拉底式探究 — 在提出解决方案之前提问
- REQ-EXPLORE-02：会话必须提供将输出路由到适当 eCL 构件的选项
...
**命令：** `/ecl-undo --last N | --phase NN | --plan NN-MM`

**目的：** 使用阶段清单和 git 日志安全回滚 eCL 阶段或计划提交，进行依赖性检查，并在应用任何回滚之前设置硬确认门控。

**需求：**
- REQ-UNDO-01：`--phase` 模式必须通过清单和 git 日志回退识别阶段的所有提交
- REQ-UNDO-02：`--plan` 模式必须识别特定计划的所有提交
- REQ-UNDO-03：`--last N` 模式必须显示最近的 eCL 提交供交互式选择
...
**命令：** `/ecl-import --from <filepath>`

**目的：** 将外部计划文件摄入 eCL 规划系统，检测与 `PROJECT.md` 决策的冲突，将其转换为有效的 eCL PLAN.md，并通过 plan-checker 进行验证。

**需求：**
- REQ-IMPORT-01：导入器必须检测外部计划与现有 PROJECT.md 决策之间的冲突
- REQ-IMPORT-02：所有检测到的冲突必须在写入之前呈现给用户解决
- REQ-IMPORT-03：导入的计划必须以有效的 eCL PLAN.md 格式写入
- REQ-IMPORT-04：写入的计划必须通过 `ecl-plan-checker` 验证

---

### 97. 快速代码库扫描

**命令：** `/ecl-map-codebase --fast [--focus tech|arch|quality|concerns]`

**目的：** `/ecl-map-codebase` 的轻量级替代方案，为一两个组合的焦点区域派生单个映射智能体，在 `.planning/codebase/` 中生成有针对性的输出，无需 4 个并行智能体的开销。
...
- REQ-SCAN-03：输出必须以与 `/ecl-map-codebase` 相同的格式写入 `.planning/codebase/`

---

### 98. 自主审计修复

**命令：** `/ecl-audit-fix [--source <audit>] [--severity high|medium|all] [--max N] [--dry-run]`
...
### 101. /ecl-progress --next 中的硬停止安全门控

**命令：** `/ecl-progress --next`

**目的：** 通过添加硬停止安全门控和连续调用守护来阻止 `/ecl-progress --next` 进入失控循环，该守护在检测到重复的相同步骤时中断自主链式操作。

**需求：**
- REQ-NEXT-GATE-01：`/ecl-progress --next` 必须跟踪连续的相同步骤调用
...
- REQ-ADAPTIVE-02：`adaptive` 必须可通过 `/ecl-config --profile adaptive` 选择

---

### 103. 合并后 Hunk 验证

**命令：** `/ecl-update --reapply`
...
**目的：** 将 eCL 安装扩展到 Cline、CodeBuddy 和 Qwen Code 运行时。

**需求：**
- REQ-CLINE-02：Cline 安装必须将 `.clinerules` 写入 `~/.cline/`（全局）或 `./.cline/`（本地）。无自定义斜杠命令 — 仅基于规则的集成。标志：`--cline`。
- REQ-CODEBUDDY-01：CodeBuddy 安装必须将技能部署到 `~/.codebuddy/skills/ecl-*/SKILL.md`。标志：`--codebuddy`。
- REQ-QWEN-01：Qwen Code 安装必须将技能部署到 `~/.qwen/skills/ecl-*/SKILL.md`，遵循 Claude Code 2.1.88+ 使用的开放标准。`QWEN_CONFIG_DIR` 环境变量覆盖默认路径。标志：`--qwen`。
...
**命令：** `/ecl-import --from-ecl2 [--dry-run] [--force] [--path <dir>]`

**目的：** 将项目从 eCL-2 格式（带里程碑→切片→任务层次结构的 `.ecl/` 目录）迁移回 v1 `.planning/` 格式，恢复与所有 eCL v1 命令的完整兼容性。

**需求：**
- REQ-FROM-GSD2-01：导入器必须从指定或当前目录读取 `.ecl/`
...
| `--path <dir>` | 指定 eCL-2 根目录 |

---

### 106. AI 集成阶段向导

**命令：** `/ecl-ai-integration-phase [N]`
...
**目的：** 在 `/ecl-ship` 的手动审查步骤之前，如果已配置，自动运行外部代码审查命令。命令通过 stdin 接收 diff 和阶段上下文，并返回 JSON 判决（`APPROVED` 或 `REVISE`）。无论结果如何，都进入现有的手动审查流程。
...
**命令：** `/ecl-execute-phase N --cross-ai`
...
**可选集成 — `capture_thought`：** `capture_thought` 是**一种约定，而非捆绑工具**。eCL 不附带一个，也不要求一个。工作流检查当前会话中是否有任何 MCP 服务器暴露名为 `capture_thought` 的工具，如果有，则为每个提取的学习调用一次，签名如下。如果不存在此类工具，则该步骤静默跳过，`LEARNINGS.md` 仍然是主要输出。
...
运行内存/知识库 MCP 服务器（例如 ExoCortex 风格服务器、`claude-mem` 或 `mem0` 风格服务器）的用户可以实现此工具名称，以便学习成果自动路由到其知识库，附带 `project`、`phase` 和 `source` 元数据。其他用户可以在不进行任何额外设置的情况下使用 `/ecl-extract-learnings` — `LEARNINGS.md` 构件就是该功能。
...
**目的：** 允许项目将其 CLAUDE.md 存储在非根位置。`claude_md_path` 配置键控制 `/ecl-profile-user` 和相关命令写入生成的 CLAUDE.md 文件的位置。
...
**命令：** `/ecl-spike [idea] [--quick]`

**目的：** 在提交实现方案之前运行 2–5 个专注的可行性实验。每个实验使用 Given/When/Then 框架，生成可执行代码，并返回 VALIDATED / INVALIDATED / PARTIAL 判决。配套的 `/ecl-spike --wrap-up` 将发现结果打包为项目本地技能。
...
- REQ-SPIKE-06：`/ecl-spike --wrap-up` 必须将发现结果打包到 `.claude/skills/spike-findings-[project]/`
...
| `.claude/skills/spike-findings-[project]/` | 打包的发现结果（通过 `/ecl-spike --wrap-up`） |

---

### 118. Sketch 命令

**命令：** `/ecl-sketch [idea] [--quick] [--text]`

**目的：** 在提交实现之前通过一次性 HTML 模型探索设计方向。每个设计问题生成 2–3 个交互式变体，无需构建步骤即可直接在浏览器中查看。配套的 `/ecl-sketch --wrap-up` 将获胜决策打包为项目本地技能。
...
- REQ-SKETCH-08：`/ecl-sketch --wrap-up` 必须将获胜决策打包到 `.claude/skills/sketch-findings-[project]/`
...
| `.claude/skills/sketch-findings-[project]/` | 打包的决策（通过 `/ecl-sketch --wrap-up`） |
...
**需求：**
- REQ-BUDGET-01：`agents/ecl-*.md` 文件分为三个层级：XL（≤ 1 600 行）、Large（≤ 1 000 行）、Default（≤ 500 行）
...
**目的：** 在 `.planning/graphs/` 中构建、查询和检查项目的轻量级知识图谱。按项目选项启用。作为 `/ecl-graphify` 用户界面命令和 `ecl-tools.cjs graphify …` 程序化动词族公开。通过图谱视图补充 `/ecl-map-codebase --query`（快照导向），覆盖命令、智能体、工作流和阶段的节点和边。

**需求：**
- REQ-GRAPH-01：通过 `.planning/config.json` 中的 `graphify.enabled: true` 选项启用。禁用时，`/ecl-graphify` 打印激活提示并停止，不写入任何内容。
- REQ-GRAPH-02：斜杠命令 `/ecl-graphify` 公开子命令 `build`、`query <term>`、`status`、`diff`。程序化 CLI `node ecl-tools.cjs graphify …` 额外公开 `snapshot`，也在 `graphify build` 的最后一步自动调用。
...
- REQ-GRAPH-05：Graphify 通过 `ecl-tools.cjs graphify ...` 命令处理器调用。

**配置：** `graphify.enabled`、`graphify.build_timeout`
**参考文件：** `commands/ecl/graphify.md`、`bin/lib/graphify.cjs`
...
**目的：** 通过将 31 个微技能折叠到 4 个新的分组父技能和 6 个现有父技能（作为标志吸收子操作）中来降低急切技能列表开销。零功能损失 — 每个删除的微技能的行为通过整合父技能上的标志保留。整合后，`commands/ecl/*.md` 包含 59 个子技能（加上 6 个命名空间元技能，见 #123）。

**需求：**
- REQ-CONSOLIDATE-01：四个新的分组技能替换微技能集群：
  - `/ecl-capture` — 折叠 add-todo（默认）、note（`--note`）、add-backlog（`--backlog`）、plant-seed（`--seed`）、check-todos（`--list`）
  - `/ecl-phase` — 折叠 add-phase（默认）、insert-phase（`--insert`）、remove-phase（`--remove`）、edit-phase（`--edit`）
  - `/ecl-config` — 折叠 settings-advanced（`--advanced`）、settings-integrations（`--integrations`）、set-profile（`--profile`）
  - `/ecl-workspace` — 折叠 new-workspace（`--new`）、list-workspaces（`--list`）、remove-workspace（`--remove`）
- REQ-CONSOLIDATE-02：六个现有父技能将 wrap-up / 子操作作为标志吸收：`/ecl-update --sync`、`/ecl-update --reapply`、`/ecl-sketch --wrap-up`、`/ecl-spike --wrap-up`、`/ecl-map-codebase --fast`、`/ecl-map-codebase --query`、`/ecl-code-review --fix`、`/ecl-progress --do`、`/ecl-progress --next`。
- REQ-CONSOLIDATE-03：删除的微技能斜杠形式（裸 `ecl-add-todo`、`ecl-add-backlog`、`ecl-plant-seed`、`ecl-check-todos`、`ecl-add-phase`、`ecl-insert-phase`、`ecl-remove-phase`、`ecl-edit-phase`、`ecl-new-workspace`、`ecl-list-workspaces`、`ecl-remove-workspace`、`ecl-settings-advanced`、`ecl-settings-integrations`、`ecl-set-profile`、`ecl-sketch-wrap-up`、`ecl-spike-wrap-up`、`ecl-reapply-patches`、`ecl-code-review-fix`、…）必须解析为"未知命令" — 无影子存根。
- REQ-CONSOLIDATE-04：`autonomous.md` 调用 `/ecl-code-review --fix`（之前调用已删除的 `ecl-code-review-fix`）。

**参考 issue：** [#2790](https://github.com/evolvconsulting/evolv-coder-lite/issues/2790)
...
**命令：**
- `/ecl-workflow` — 阶段流水线路由器（讨论/规划/执行/验证/阶段/进度）
- `/ecl-project` — 项目生命周期（里程碑、审计、摘要）
- `/ecl-quality` — 质量门控（代码审查、调试、审计、安全、评估、UI）
- `/ecl-context` — 代码库智能（映射、graphify、文档、学习）
- `/ecl-manage` — 配置/工作区/工作流/线程/更新/发布/收件箱
- `/ecl-ideate` — 探索与捕获（探索、sketch、spike、规范、捕获）
...
**需求：**
- REQ-NS-01：六个 `commands/ecl/ns-*.md` 命名空间路由器带管道分隔的关键字标签描述（≤ 60 个字符）。
- REQ-NS-02：现有子技能保持不变，仍可直接调用 — 命名空间技能是附加的，不是替换直接斜杠形式的。
- REQ-NS-03：每个命名空间路由器的正文包含一个路由表，将用户意图映射到 #2790 后整合界面上正确的具体子技能。

**参考 issue：** [#2792](https://github.com/evolvconsulting/evolv-coder-lite/issues/2792)

---

### 124. 上下文窗口利用率守护

**命令：** `/ecl-health --context`

**目的：** 上下文窗口饱和的质量守护。两个阈值：60% 利用率警告（"考虑使用 `/ecl-thread`"），70% 为临界（"推理质量可能下降"；根据最近的上下文注意力研究，与断裂点匹配）。

**需求：**
- REQ-CTX-GUARD-01：`/ecl-health --context` 打印带当前利用率、阈值层级（`ok` / `warn` / `critical`）和补救建议的结构化状态行。
- REQ-CTX-GUARD-02：相同的分类以 `ecl-tools.cjs validate context --tokens-used <int> --context-window <int>` 公开 — 状态行和钩子调用者的结构化封装（#125）。两个标志都是必需的；处理器返回与 REQ-CTX-GUARD-03 中纯分类器相同的 `{ percent, state }` 封装。
...
**参考 issue：** [#2833](https://github.com/evolvconsulting/evolv-coder-lite/issues/2833) — 完整字段参考和渲染规则见 [`docs/STATE-MD-LIFECYCLE.md`](reference/state-md.md)。
...
**参考 issue：** [#3023](https://github.com/evolvconsulting/evolv-coder-lite/pull/3030)
...
**参考 issue：** [#3024](https://github.com/evolvconsulting/evolv-coder-lite/pull/3031)

---

### 128. 更新横幅选项

**目的：** 向已拒绝或绕过 eCL 状态行的用户呈现更新可用性，无需状态行。

**行为：**
- 安装时，如果安装器检测到没有 eCL 状态行，它提供一个选项 `SessionStart` 钩子。
- 钩子读取现有的 `~/.cache/ecl/ecl-update-check.json` 缓存 — 与状态行使用的相同缓存 — 仅在有可用更新时打印横幅。
- 无更新时保持静默。
- 失败诊断每 24 小时限流一次。
- 通过 `npx @evolvconsulting/evolv-coder-lite --uninstall` 干净移除。
...
**参考 issue：** [#2795](https://github.com/evolvconsulting/evolv-coder-lite/pull/2795)

---

### 129. Issue 驱动编排指南

**目的：** 记录从 GitHub / Linear / Jira issue 驱动完整 eCL 工作流的方法，将跟踪器中心概念映射到现有 eCL 原语。

**文档：** [`docs/issue-driven-orchestration.md`](issue-driven-orchestration.md)

**覆盖的工作流：**
1. 为每个 issue 创建隔离的工作区（`/ecl-workspace --new`）
2. 运行管理仪表板以了解情况（`/ecl-manager`）
3. 自主执行（`/ecl-autonomous`）
4. 验证和审查（`/ecl-verify-work`、`/ecl-review`）
5. 发布并关闭 issue（`/ecl-ship`）

无新命令或守护进程 — 纯粹是将现有原语映射到跟踪器驱动工作流的文档构件。

**参考 issue：** [#2840](https://github.com/evolvconsulting/evolv-coder-lite/pull/2840)
...
**参考 issue：** [#3170](https://github.com/evolvconsulting/evolv-coder-lite/issues/3170)
...
**运行时控制：** `/ecl:surface` 列出配置文件状态，无需重新安装即可启用、禁用或重置技能集群。

**需求：**
- REQ-SURFACE-01：安装器必须解析 `--profile=<name>` 并将活跃配置文件持久化在 `.ecl-profile` 中。
...
- 用户拥有的构件被保留。
- 模糊的 eCL 风格文件通过清晰的报告阻止，而不是被静默覆盖。
- 迁移计划支持演习报告和回滚保护。
...
**目的：** 在不编辑 eCL 工作流文件的情况下，将项目特定的 PRD 风格节区添加到生成的 PR 正文中。
...
**目的：** 让团队为无标志 `/ecl-review` 运行选择默认的审查者子集。
...
**参考：** [配置参考](CONFIGURATION.md#reviewer-defaults-for-ecl-review)
...
**行为：** 启用后，eCL 解析 `fallow` 二进制文件，运行有界审计，写入 `FALLOW.json`，并将结构性发现嵌入 `REVIEW.md`。
...
**参考：** [检查点参考](../../evolv-coder-lite/references/checkpoints.md)

---

### 139. 配额与速率限制失败分类

**命令：** `/ecl-execute-phase`
