...
describe('staleness', () => {
  ...
  describe('git-aware', () => {
    ...
    test('built_at_commit absent (pre-v0.7 graph): all four new fields null', () => {
      // No built_at_commit on the graph -- eCL must not fabricate one.
...
describe('mvp-viz', () => {
  // Contract: commands/ecl/graphify.md documents MVP visual differentiation.
  ...
  const CMD = path.join(__dirname, '..', 'commands', 'ecl', 'graphify.md');
...
describe('regressions', () => {
  // ── Regression for #3166 ────────────────────────────────────────────────────
  // /ecl-graphify build lost artifacts because the skill spawned a Task
  ...
  const SKILL_PATH = path.join(__dirname, '..', 'commands', 'ecl', 'graphify.md');
  ...
  //   Gap 2: hooks/lib/ecl-graphify-rebuild.sh not copied by installer
  ...
  describe('#3579 Gap 1: build-hooks.js packages every top-level hooks/*.sh into dist', () => {
    ...
    test('hooks/dist/ecl-graphify-update.sh exists after build', () => {
      assert.ok(
        fs.existsSync(path.join(DIST_DIR_3579, 'ecl-graphify-update.sh')),
        'expected hooks/dist/ecl-graphify-update.sh to exist after build (Gap 1)'
      );
    });

    test('hooks/dist/lib/ecl-graphify-rebuild.sh exists after build', () => {
      assert.ok(
        fs.existsSync(path.join(DIST_DIR_3579, 'lib', 'ecl-graphify-rebuild.sh')),
        'expected hooks/dist/lib/ecl-graphify-rebuild.sh to exist after build (Gap 2)'
      );
    });
  });
  ...
  describe('#3579: installer deploys graphify hook + lib helper to target', () => {
    ...
    before(() => {
      execFileSync(process.execPath, [BUILD_SCRIPT_3579], { encoding: 'utf-8', stdio: 'pipe' });
      tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-3579-install-'));
    ...
    test('hooks/ecl-graphify-update.sh present at install target', () => {
      const dest = path.join(tmpDir, 'hooks', 'ecl-graphify-update.sh');
      assert.ok(fs.existsSync(dest), `expected ${dest} to exist after install`);
    });

    test('hooks/lib/ecl-graphify-rebuild.sh present at install target', () => {
      const dest = path.join(tmpDir, 'hooks', 'lib', 'ecl-graphify-rebuild.sh');
      assert.ok(fs.existsSync(dest), `expected ${dest} to exist after install`);
    });

    test('installer does not warn about missing ecl-graphify-update.sh', () => {
      assert.ok(
        !installStdout.includes('Missing expected hook: ecl-graphify-update.sh'),
        `installer output must not warn about missing graphify hook; got:\n${installStdout}`
      );
      assert.ok(
        !installStdout.includes(
          'Skipped graphify auto-update hook — ecl-graphify-update.sh not found'
        ),
