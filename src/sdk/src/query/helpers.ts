 ...
 *
 * Ported from evolv-coder-lite/bin/lib/core.cjs and state.cjs.
...
export function detectRuntime(config?: { runtime?: unknown }): Runtime {
  const envRuntime = canonicalizeRuntimeName(process.env.ECL_RUNTIME);
...
/**
 * Resolve the eCL agents directory for a given runtime.
 *
 * Precedence:
 *   1. `ECL_AGENTS_DIR` — explicit SDK override (wins over runtime selection)
 *   2. `<projectDir>/.claude/agents` — project-local install (Claude Code `--local`,
 *      `npx evolv-coder-lite-cc@latest`); checked first to match Claude Code's own
...
export function planningPaths(projectDir: string, workstream?: string): PlanningPaths {
  ...
  // Validate env workstream before use: invalid ECL_WORKSTREAM falls back to
 ...
 * from evolv-coder-lite/bin/lib/frontmatter.cjs (lines 323, 340, 354, 369).
