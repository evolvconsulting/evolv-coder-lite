 ...
 * are correctly enforced across ecl-update-banner.js, ecl-statusline.js,
...
const { PACKAGE_NAME, updateCacheFileName } = require('../evolv-coder-lite/bin/lib/package-identity.cjs');
const { buildBannerOutput } = require('../hooks/ecl-update-banner.js');
const { evaluateUpdateCache } = require('../hooks/ecl-statusline.js');

// ─── Package identity constants ──────────────────────────────────────────────

describe('package-identity exports', () => {
  test('PACKAGE_NAME is @evolvconsulting/evolv-coder-lite', () => {
    assert.equal(PACKAGE_NAME, '@evolvconsulting/evolv-coder-lite');
  });
...
describe('buildBannerOutput lineage guard', () => {
  test('returns null when package_name is present but foreign', () => {
    const out = buildBannerOutput({
      cache: {
        ...
        installed: '1.2.0',
        latest: '1.42.3',
        package_name: 'evolv-coder-lite-cc',
      },
      parseError: false,
  ...
  test('returns banner when package_name matches PACKAGE_NAME', () => {
    ...
    assert.ok(out.systemMessage.includes('/ecl:update'));
  });
...
describe('evaluateUpdateCache lineage guard', () => {
  ...
  test('returns staleWarning=stale when stale_hooks present and matching package_name', () => {
    const r = evaluateUpdateCache({
      ...
      latest: '1.3.0',
      package_name: '@evolvconsulting/evolv-coder-lite',
      stale_hooks: [{ file: 'ecl-statusline.js', hookVersion: '1.2.0', installedVersion: '1.3.0' }],
    });
  ...
  test('returns staleWarning=dev when installed > latest (dev install) and matching package_name', () => {
    const r = evaluateUpdateCache({
      ...
      latest: '1.3.0',
      package_name: '@evolvconsulting/evolv-coder-lite',
      stale_hooks: [{ file: 'ecl-statusline.js', hookVersion: '1.2.0', installedVersion: '2.0.0' }],
    });
