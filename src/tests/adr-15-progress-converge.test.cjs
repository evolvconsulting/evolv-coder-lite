...
const COMMAND_PATH = path.join(REPO_ROOT, 'commands', 'ecl', 'progress.md');
const WORKFLOW_PATH = path.join(REPO_ROOT, 'evolv-coder-lite', 'workflows', 'next.md');
const FULL_MD_PATH = path.join(REPO_ROOT, 'evolv-coder-lite', 'workflows', 'help', 'modes', 'full.md');
...
describe('ADR-15: /ecl:progress --next --auto --converge (#1190)', () => {
  ...
  test('next workflow routes Route 3 through plan-review-convergence when PLAN_STRATEGY=converge', () => {
    ...
    assert.match(
      ...
      'next workflow should reference /ecl:plan-review-convergence for the converge route',
    );
    assert.match(
      workflow,
  ...
  test('next workflow preserves --auto re-invocation chaining', () => {
    ...
    assert.match(
      workflow,
      /\/ecl:progress --next --auto/,
      'workflow should re-invoke /ecl:progress --next --auto for chaining',
    );
  ...
  test('full.md documents --converge, --auto, and --cross-ai for /ecl:progress', () => {
    const fullMd = read(FULL_MD_PATH);

    assert.match(
      fullMd,
      /\/ecl:progress --next --auto --converge/,
      'full.md should show /ecl:progress --next --auto --converge usage',
    );
    assert.match(
      fullMd,
      /--auto/,
  ...
  test('COMMANDS.md documents progress convergence flags and usage', () => {
    ...
    assert.match(
      commandsDoc,
      /\/ecl-progress --next --auto --converge/,
      'COMMANDS.md should show /ecl-progress --next --auto --converge usage example',
    );
  ...
  test('how-to shows /ecl-progress --next --auto --converge usage', () => {
    const howTo = read(HOW_TO_PATH);

    assert.match(
      howTo,
      /\/ecl-progress --next --auto --converge/,
      'how-to should show /ecl-progress --next --auto --converge usage',
    );
  });
});
