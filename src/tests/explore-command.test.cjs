...
describe('explore command', () => {
  test('command file exists', () => {
    const p = path.join(__dirname, '..', 'commands', 'ecl', 'explore.md');
    assert.ok(fs.existsSync(p), 'commands/ecl/explore.md should exist');
  });

  test('command file has required frontmatter', () => {
    const p = path.join(__dirname, '..', 'commands', 'ecl', 'explore.md');
    const content = fs.readFileSync(p, 'utf-8');
    assert.ok(content.includes('name: ecl:explore'), 'Command must have name frontmatter');
  ...
  test('workflow file exists', () => {
    const p = path.join(__dirname, '..', 'evolv-coder-lite', 'workflows', 'explore.md');
