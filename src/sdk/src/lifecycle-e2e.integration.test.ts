/**
 * E2E lifecycle integration test — proves eCL.runPhase() drives
 * the full phase lifecycle: discuss → research → plan → execute → verify → advance
 * after bootstrapping a real project via InitRunner.
 *
 * This is the capstone proof that `ecl-sdk auto` works end-to-end
 * without human intervention. InitRunner bootstraps the project,
 * then eCL.runPhase() drives Phase 1 through the complete lifecycle.
 *
...
describe.skipIf(!cliAvailable || !gsdToolsAvailable)('E2E Lifecycle: InitRunner → eCL.runPhase() full lifecycle', () => {
  let tmpDir: string;
  ...
  beforeAll(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), 'ecl-sdk-lifecycle-e2e-'));
  ...
  it('eCL.runPhase() drives Phase 1 through the full lifecycle without human intervention', async () => {
    ...
    // Construct eCL with autoMode: true
    const ecl = new eCL({
      projectDir: tmpDir,
      autoMode: true,
    });
    ecl.onEvent((e: GSDEvent) => events.push(e));

    // Run the discovered first phase with tight budget to minimize cost
    const result: PhaseRunnerResult = await ecl.runPhase(phase1Number!, {
      maxTurnsPerStep: 10,
