<purpose>
Interactive configuration of eCL power-user knobs — plan bounce, node repair, subagent timeouts,
...
This is a companion to `/ecl:settings` — the common-case prompt there covers model profile,
...
All subsequent reads and writes go through `$ECL_CONFIG_PATH`. Never hardcode
...
- `git.phase_branch_template` (default: `ecl/phase-{phase}-{slug}`)
- `git.milestone_branch_template` (default: `ecl/{milestone}-{slug}`)
...
AskUserQuestion([
  ...
  {
    question: "Phase branch template? (current: <value or ecl/phase-{phase}-{slug}>)",
  ...
  {
    question: "Milestone branch template? (current: <value or ecl/{milestone}-{slug}>)",
...
This section lets the user inspect and override the built-in model IDs eCL resolves for each
...
defines which AI models eCL uses at each cost tier (low / medium / high), independently
of the `runtime` and `model_profile` selections above. Two paths are offered:

- **Known provider:** choose a provider and a budget level; eCL materializes the canonical
...
```bash
cat "$ECL_CONFIG_PATH" | python3 -c "import sys,json; c=json.load(sys.stdin); mp=c.get('model_policy',{}); print(json.dumps(mp,indent=2))" 2>/dev/null || echo "{}"
```
...
AskUserQuestion([
  {
    ...
    multiSelect: false,
    options: [
      { label: "Known provider", description: "Choose a provider (Claude / OpenAI / Gemini / Qwen) and a budget level — eCL writes the canonical tier mapping automatically." },
...
ecl_run query config-set model_policy.low      "<low-id>"

# Generic-provider path — write only tiers the user changed ("Keep current" skipped):
ecl_run query config-set model_policy.provider "custom"
ecl_run query config-set model_policy.budget   null
# Per-tier writes for each non-"Keep current" answer:
ecl_run query config-set model_policy.high     "<high-id>"   # omit if user chose "Keep current"
ecl_run query config-set model_policy.medium   "<medium-id>" # omit if user chose "Keep current"
ecl_run query config-set model_policy.low      "<low-id>"    # omit if user chose "Keep current"
```
...
UI/AI phase gates), use /ecl:settings.
```
</step>
...
<success_criteria>
- [ ] Current config read from resolved `$ECL_CONFIG_PATH`
