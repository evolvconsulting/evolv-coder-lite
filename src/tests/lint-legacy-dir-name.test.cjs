...
function createTempRepo() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-lint-legacy-test-'));
...
function runGuard(cwd) {
  ...
  return spawnSync(process.execPath, [GUARD_SCRIPT], {
    cwd,
    encoding: 'utf8',
    env: { ...process.env, ECL_LINT_LEGACY_REPO_ROOT: cwd },
  });
}

// ---------------------------------------------------------------------------
// The guard script needs to respect ECL_LINT_LEGACY_REPO_ROOT for testing.
...
// the guard, add ECL_LINT_LEGACY_REPO_ROOT env var support to it. We test
// against that.
...
describe('lint-legacy-dir-name — slug variant allowed', () => {
  test('exits 0 when token is a hyphenated slug variant (not bare)', () => {
    const dir = createTempRepo();
    try {
      // Slug variants like evolv-coder-lite, evolv-coder-lite-cli should be allowed.
...
describe('lint-legacy-dir-name — ecl-allow-legacy-name marker', () => {
  test('exits 0 when forbidden token is on a line with the allow marker', () => {
    ...
    try {
      ...
      writeFile(dir, 'src/migration.js', `const legacyRoot = '${bare}'; // ecl-allow-legacy-name\n`);
