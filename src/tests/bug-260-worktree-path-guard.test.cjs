/**
 * Regression tests for bug #260 — ecl-worktree-path-guard.js
 *
 ...
 * worktree. The prose guard in ecl-executor.md step 0b is skipped under load,
 ...
 *   7. install.js has an fs.existsSync guard for ecl-worktree-path-guard.js
 */
...
const HOOK_PATH = path.join(__dirname, '..', 'hooks', 'ecl-worktree-path-guard.js');
...
function makeMainRepo() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-260-main-'));
  git(dir, ['init', '-q']);
...
function makeWorktree(mainRepo, branchName) {
  ...
  const wtDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-260-wt-'));
...
describe('bug #260: ecl-worktree-path-guard.js', () => {
  ...
  describe('sibling path is blocked', () => {
    test('path that shares prefix with worktree root but is a sibling exits 2', () => {
      ...
      const base = realp(fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-260-sib-base-')));
...
// ---------------------------------------------------------------------------
// #1342 — eCL-activity gate + fail-open for no-repo targets
// ---------------------------------------------------------------------------

describe('#1342 — eCL-activity gate + fail-open for no-repo targets', () => {
  // Fixtures: one non-agent linked worktree (plain user branch) + one agent worktree
  let mainRepo1342;
  let nonAgentWorktree;   // on branch 'feature-x' — non-eCL
  let agentWorktree;       // on branch 'worktree-agent-foo' — eCL-managed
  ...
  test('(1) non-agent linked worktree: Write to a path outside all git repos exits 0 (no block)', () => {
    ...
    const plansDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-1342-plans-'));
    try {
  ...
  test('(2) non-agent linked worktree: Edit targeting main repo root exits 0 (gate no-op, not #260 block)', () => {
    ...
    const result = runHook(nonAgentWorktree, payload);
    assert.strictEqual(result.status, 0,
      `Non-agent linked worktree must exit 0 (eCL-activity gate fires before #260 check). ` +
      `Got exit ${result.status}. stderr: ${result.stderr}`
    );
    assert.strictEqual(result.stdout, '', 'Expected no block output');
  });

  // Test 3 — eCL-managed worktree (worktree-agent-foo): Edit targeting main repo root → exit 2 (block)
  test('(3) eCL-managed worktree: Edit targeting main repo root exits 2 with block decision', () => {
    const payload = {
      cwd: agentWorktree,
      tool_name: 'Edit',
      tool_input: { file_path: path.join(mainRepo1342, 'src', 'index.ts') },
    ...
    assert.strictEqual(result.status, 2,
      `eCL-managed worktree targeting main repo root must be blocked (exit 2). ` +
      `Got exit ${result.status}. stderr: ${result.stderr}`
    );
    let parsed;
    assert.doesNotThrow(() => { parsed = JSON.parse(result.stdout); }, 'stdout must be valid JSON');
  ...
  // Test 4 — eCL-managed worktree: absolute target INSIDE the active worktree → exit 0
  test('(4) eCL-managed worktree: absolute target inside the active worktree exits 0', () => {
    const payload = {
      cwd: agentWorktree,
      tool_name: 'Edit',
    ...
    const result = runHook(agentWorktree, payload);
    assert.strictEqual(result.status, 0,
      `eCL-managed worktree targeting its own subtree must pass. ` +
      `Got exit ${result.status}. stderr: ${result.stderr}`
    );
    assert.strictEqual(result.stdout, '', 'Expected no block output');
  });

  // Test 5 — eCL-managed worktree: target OUTSIDE all git repos (tmpdir) → exit 0 (fail open)
  test('(5) eCL-managed worktree: target outside all git repos exits 0 (fail open, not #260 vector)', () => {
    ...
    const externalDir = realp(fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-1342-ext-')));
    try {
      ...
      const result = runHook(agentWorktree, payload);
      assert.strictEqual(result.status, 0,
        `eCL-managed worktree writing to a path outside all git repos must fail open (exit 0). ` +
  ...
  // Test 6 — eCL-managed worktree: Write to .git/config of the MAIN repo → exit 2 (block)
  test('(6) blocks absolute writes into the main repo .git internals from a eCL worktree (#1342)', () => {
    ...
    const result = runHook(agentWorktree, payload);
    assert.strictEqual(result.status, 2,
      `eCL-managed worktree targeting .git/config of another repo must be blocked (exit 2). ` +
      `Got exit ${result.status}. stderr: ${result.stderr}`
    );
    let parsed;
    assert.doesNotThrow(() => { parsed = JSON.parse(result.stdout); }, 'stdout must be valid JSON');
...
describe('install.js guard for ecl-worktree-path-guard.js', () => {
  let src;
  ...
  test('install.js checks fs.existsSync before registering ecl-worktree-path-guard.js', () => {
    ...
    assert.ok(
      ...
      'ecl-worktree-path-guard.js in settings.json. Without this guard, the hook ' +
      'is registered even when the .js file was never copied (root cause of #1754).'
    );
  });

  test('install.js emits a skip warning when ecl-worktree-path-guard.js is missing', () => {
    const anchorIdx = src.indexOf('hasWorktreePathGuardHook');
    assert.ok(anchorIdx !== -1, 'hasWorktreePathGuardHook not found in install.js');
    ...
    assert.ok(
      block.includes('Skipped') && block.includes('ecl-worktree-path-guard'),
      'install.js must emit a skip warning mentioning ecl-worktree-path-guard when the file is not found'
    );
  });
});
