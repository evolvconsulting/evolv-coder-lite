...
eCL Core is a **meta-prompting framework** that sits between the user and AI coding agents (Claude Code, Gemini CLI, Kimi CLI, OpenCode, Kilo, Codex, Copilot, Antigravity, Trae, Cline, Augment Code). It provides:
...
┌─────────────────────▼────────────────────────────────┐
│              COMMAND LAYER                            │
│   commands/ecl/*.md — Prompt-based command files      │
│   (Claude Code custom commands / Codex skills)        │
└─────────────────────┬────────────────────────────────┘
                      │
┌─────────────────────▼────────────────────────────────┐
│              WORKFLOW LAYER                           │
│   evolv-coder-lite/workflows/*.md — Orchestration logic  │
...
### Commands (`commands/ecl/*.md`)

User-facing entry points. Each file contains YAML frontmatter (name, description, allowed-tools) and a prompt body that bootstraps the workflow. Commands are installed as:

- **Claude Code:** Custom slash commands (hyphen form, `/ecl-command-name`)
- **OpenCode / Kilo:** Slash commands (hyphen form, `/ecl-command-name`)
- **Codex:** Skills (`$ecl-command-name`)
- **Copilot:** Slash commands (hyphen form, `/ecl-command-name`)
- **Gemini CLI:** Slash commands under the `ecl:` namespace (colon form, `/ecl:command-name`) — Gemini namespaces all custom commands under their plugin id, so the install path rewrites every body-text reference to colon form
- **Kimi CLI:** Agent Skills (`/skill:ecl-command-name`) plus an explicit custom agent launch with `kimi --agent-file`
- **Antigravity:** Skills

**Total commands:** see [`docs/INVENTORY.md`](INVENTORY.md#commands) for the authoritative count and full roster.

#### Two-stage hierarchical routing (v1.40, [#2792](https://github.com/evolvconsulting/evolv-coder-lite/issues/2792))

To keep the eager skill-listing token cost low, v1.40 introduces six namespace **meta-skills** (`ecl-workflow`, `ecl-project`, `ecl-quality`, `ecl-context`, `ecl-manage`, `ecl-ideate` — sourced from `commands/ecl/ns-*.md`, but the invocable `name:` is the bare form shown here) layered above the concrete sub-skills. On runtimes with non-recursive skill loaders (claude global, cline, qwen, hermes, augment, trae, antigravity) the installer now realizes this fully: it emits only the 6 namespace router bundles as top-level skills and nests the ~61 concrete skills under `<router>/skills/<name>/SKILL.md`, so the eager listing is ≈6 entries instead of ≈67. The model selects a namespace router, which instructs it to read the nested concrete skill file via a routing table embedded in the router body. On these runtimes concrete skills are **not** directly invocable by bare name via the Skill tool; they are reachable through the router. Slash commands (`/ecl-*`, via the separate commands surface) are unaffected where the runtime has one. On runtimes with recursive or unconfirmed skill loaders (cursor, codex, copilot, windsurf, codebuddy, opencode, kilo) the layout remains flat — all skills emitted at the top level as before.
...
The eager skill listing is one of two recurring per-turn token costs. The other is the MCP tool schema injected by every enabled MCP server in `.claude/settings.json`. Heavyweight MCP servers (browser/playwright, Mac-tools, Windows-tools) can each cost 20 k+ tokens per turn — often dwarfing what `model_profile` tuning saves. The toggle lives in the Claude Code harness (`enabledMcpjsonServers` / `disabledMcpjsonServers` in `.claude/settings.json`) and is **not** a eCL concern. Together, the two-stage routing layer (#2792) and disciplined MCP enablement are the largest cost levers per turn. See [`docs/USER-GUIDE.md`](USER-GUIDE.md) and `references/context-budget.md` for the audit checklist.
...
### References (`evolv-coder-lite/references/*.md`)
...
The planner agent (`agents/ecl-planner.md`) was decomposed from a single monolithic file into a core agent plus reference modules to stay under the 50K character limit imposed by some runtimes:
...
| Hook | Event | Purpose |
|------|-------|---------|
| `ecl-statusline.js` | `statusLine` | Displays model, task, directory, and context usage bar |
| `ecl-context-monitor.js` | `PostToolUse` / `AfterTool` | Injects agent-facing context warnings at 35%/25% remaining |
| `ecl-check-update.js` | `SessionStart` | Foreground trigger for the background update check |
| `ecl-ensure-canonical-path.js` | `SessionStart` | For Claude Code plugin installs, symlinks `~/.claude/evolv-coder-lite/{bin,contexts,references,templates,workflows}` to the plugin's bundled tree so `@~/.claude/evolv-coder-lite/...` includes resolve; runs first in `SessionStart`, no-op in classic installs, self-heals after `claude plugin update` (#997) |
| `ecl-check-update-worker.js` | (helper) | Background worker spawned by `ecl-check-update.js`; no direct event registration |
| `ecl-prompt-guard.js` | `PreToolUse` | Scans `.planning/` writes for prompt injection patterns (advisory) |
| `ecl-read-injection-scanner.js` | `PostToolUse` | Scans Read tool output for injected instructions in untrusted content |
| `ecl-workflow-guard.js` | `PreToolUse` | Detects file edits outside eCL workflow context (advisory, opt-in via `hooks.workflow_guard`) |
| `ecl-read-guard.js` | `PreToolUse` | Advisory guard preventing Edit/Write on files not yet read in the session |
...
Node.js CLI utility (`ecl-tools.cjs`) with domain modules split across `evolv-coder-lite/bin/lib/` (see [`docs/INVENTORY.md`](INVENTORY.md#cli-modules) for the authoritative roster):
...
| `loop-resolver.cjs`        | Loop Extension Point resolver — ADR-857 phase 3c registry-consuming query; consumes resolved Capability State, filters `byLoopPoint` by capability enablement plus config activation, renders active hooks as markdown, emits `{ point, activeHooks, rendered }` envelope; `ecl-tools loop render-hooks <point> [--config-dir <path>]` |
| `capability-state.cjs`     | Unified capability-state resolver — ADR-857 phase 4b/6; composes install profile, runtime surface, and config activation into one per-capability view consumed by workflow hook rendering; pure `resolveCapabilityState`, reusable `resolveCapabilityRuntimeState`, I/O `cmdCapabilityState`, and convenience predicate `isCapabilityActive(capId, cwd)`; `ecl-tools capability state [--config-dir <path>]` emits `{ runtimeConfigDir, capabilities[] }` where each entry carries `enabled` (installed && surfaced) and `active` (enabled && configActivation via the capability's `activationKey`; absent key → active===enabled) |
| `graphify-command-router.cjs` | ADR-959 capability command router — first real capability command cutover (phase 4d-impl-2); extracted from the `case 'graphify':` arm in `ecl-tools.cjs`; dispatches build/query/status/diff subcommands; discovered via `commandFamilies` in the capability registry |
| `audit-command-router.cjs` | ADR-959 capability command router (phase 4d-impl-3); extracted from the `case 'audit-uat':` and `case 'audit-open':` arms in `ecl-tools.cjs`; `routeAuditUat` → `uat.cjs:cmdAuditUat`, `routeAuditOpen` → `audit.cjs:{auditOpenArtifacts,formatAuditReport}`; discovered via `commandFamilies` in the capability registry |
| `intel-command-router.cjs` | ADR-959 capability command router (phase 4d-impl-4, last first-party cutover); extracted from the `case 'intel':` arm in `ecl-tools.cjs`; `routeIntelCommand` → all 9 intel subcommands via lazy `require('./intel.cjs')`; preserves non-raw `timeAgo` transform on `status.files[*].updated_at`; discovered via `commandFamilies` in the capability registry |
...
Conceptual spawn-pattern taxonomy for the primary agents. For the authoritative agent roster (including the advanced/specialized agents such as `ecl-pattern-mapper`, `ecl-code-reviewer`, `ecl-code-fixer`, `ecl-ai-researcher`, `ecl-domain-researcher`, `ecl-eval-planner`, `ecl-eval-auditor`, `ecl-framework-selector`, `ecl-debug-session-manager`, `ecl-intel-updater`), see [`docs/INVENTORY.md`](INVENTORY.md#agents).


| Category         | Agents                                                                                  | Parallelism                                                                               |
| ---------------- | --------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------- |
| **Researchers**  | ecl-project-researcher, ecl-phase-researcher, ecl-ui-researcher, ecl-advisor-researcher | 4 parallel (stack, features, architecture, pitfalls); advisor spawns during discuss-phase |
| **Synthesizers** | ecl-research-synthesizer                                                                | Sequential (after researchers complete)                                                   |
| **Planners**     | ecl-planner, ecl-roadmapper                                                             | Sequential                                                                                |
| **Checkers**     | ecl-plan-checker, ecl-integration-checker, ecl-ui-checker, ecl-nyquist-auditor          | Sequential (verification loop, max 3 iterations)                                          |
| **Executors**    | ecl-executor                                                                            | Parallel within waves, sequential across waves                                            |
| **Verifiers**    | ecl-verifier                                                                            | Sequential (after all executors complete)                                                 |
| **Mappers**      | ecl-codebase-mapper                                                                     | 4 parallel (tech, arch, quality, concerns)                                                |
| **Debuggers**    | ecl-debugger                                                                            | Sequential (interactive)                                                                  |
| **Auditors**     | ecl-ui-auditor, ecl-security-auditor                                                    | Sequential                                                                                |
| **Doc Writers**  | ecl-doc-writer, ecl-doc-verifier                                                        | Sequential (writer then verifier)                                                         |
| **Profilers**    | ecl-user-profiler                                                                       | Sequential                                                                                |
| **Analyzers**    | ecl-assumptions-analyzer                                                                | Sequential (during discuss-phase)                                                         |
...
├── skills/ecl-ns-*/SKILL.md        # Global skills — nesting runtimes: 6 namespace routers (authoritative roster: docs/INVENTORY.md)
│   └── skills/<name>/SKILL.md     #   concrete skills nested under each router
│   (flat runtimes: skills/ecl-*/SKILL.md — all ~67 skills at top level)
├── commands/ecl/*.md               # Local Claude installs use slash commands instead of global skills
├── evolv-coder-lite/
│   ├── bin/ecl-tools.cjs           # CLI utility
...
│   ├── ARCHITECTURE.md
│   └── PITFALLS.md
├── codebase/               # Brownfield mapping (from /ecl-map-codebase)
...
├── threads/               # Persistent context threads (from /ecl-thread)
├── seeds/                 # Forward-looking ideas (from /ecl-capture --seed)
...
After the last wave of `/ecl-execute-phase` commits, the workflow runs a
...
**warns** (default) with the suggested `/ecl-map-codebase --paths …` command,
or **auto-remaps** (`workflow.drift_action = auto-remap`) by spawning
`ecl-codebase-mapper` scoped to the affected paths. Any error in detection
...
4. **Runtime adaptation** — Transforms file content per runtime:
  ...
  - Kimi CLI: Generates Agent Skills under `skills/ecl-*/SKILL.md`, custom agent YAML/prompt files, and explicit `kimi_cli.tools.*` module paths
  ...
  - Hermes Agent: Category-based skills under `skills/ecl/`
...
7. **Patch backup** — Since v1.17, backs up locally modified files to `ecl-local-patches/` for `/ecl-update --reapply`
8. **Manifest tracking** — Writes `ecl-file-manifest.json` for clean uninstall
9. **Uninstall mode** — `--uninstall` removes all eCL files, hooks, and settings
...
Runtime Engine (Claude Code / Gemini CLI)
    ...
    │
    └── SessionStart event
        ├──► ecl-ensure-canonical-path.js   (runs first)
        │    Reads:  ${CLAUDE_PLUGIN_ROOT}/evolv-coder-lite/ (plugin installs only)
        │    Writes: ~/.claude/evolv-coder-lite/{bin,contexts,references,templates,workflows} symlinks
        │            (no-op in classic installs; preserves user files; self-heals)
        └──► ecl-check-update.js
             Reads:  VERSION file
             Writes: ~/.claude/cache/ecl-update-check.json (spawns background process)
```
...
**Threat model:** eCL automates the full path from "researcher names a package" to "executor runs `npm install`". A hallucinated name that passes `npm view` (proving only registration, not legitimacy) would previously flow through undetected. ~20% of AI-generated package references are hallucinated; ~43% of those names recur consistently across prompts, making pre-registration economically viable for attackers.

**Gate layers:**

| Layer | Component | Action |
|-------|-----------|--------|
| Research | `ecl-phase-researcher` | Runs `slopcheck install <pkgs> --json`; writes `## Package Legitimacy Audit` table to RESEARCH.md; strips `[SLOP]` packages before RESEARCH.md is written |
| Planning | `ecl-planner` | Reads Audit table; inserts `checkpoint:human-verify` before any `[ASSUMED]` or `[SUS]` install task; adds `T-{phase}-SC` STRIDE supply-chain row to `<threat_model>` |
| Execution | `ecl-executor` | RULE 3 excludes package installation from auto-fix scope; failed installs surface as checkpoints, never silent substitutions |
...
- Detects edits outside eCL workflow context (no active `/ecl-` command or Task subagent)
- Advises using `/ecl-quick` or `/ecl-fast` for state-tracked changes
...
eCL supports multiple AI coding runtimes through a unified command/workflow architecture:
...
| --- | --- | --- | --- | --- | --- |
| Claude Code | `~/.claude` | `./.claude` | Global `skills/ecl-ns-*/SKILL.md` (6 routers) + `skills/ecl-ns-*/skills/<name>/SKILL.md` (nested concretes); local `commands/ecl/*.md` | `agents/ecl-*.md` | `settings.json` hook and statusLine entries |
| OpenCode | `~/.config/opencode` | `./.opencode` | `command/ecl-*.md` | `agents/ecl-*.md` | `opencode.json` or `opencode.jsonc`; no eCL hooks |
...
| Kimi CLI | First-existing generic root: `~/.config/agents` recommended, then `~/.agents` when `~/.agents/skills` exists and `~/.config/agents/skills` does not | Deferred and guarded | `skills/ecl-*/SKILL.md` (flat) invoked as `/skill:ecl-*` | `agents/ecl.yaml`, `agents/ecl.md`, and `agents/subagents/ecl-*` YAML/prompt pairs | Explicit `kimi --agent-file <configRoot>/agents/ecl.yaml`; no eCL hooks or statusline |
| Codex | `~/.codex` | `./.codex` | `skills/ecl-*/SKILL.md` (flat) | `agents/` source markdown plus per-agent TOML | `config.toml` `[agents.ecl-*]`, `[features].hooks` (canonical; legacy alias `codex_hooks` is recognized and migrated forward on reinstall, #3566), and hook tables |
| GitHub Copilot | `~/.copilot` | `./.github` | `skills/ecl-*/SKILL.md` (flat), `copilot-instructions.md`, and `AGENTS.md` (repo root, local) | `.agent.md` files | Self-contained `sessionStart` hook (`hooks/ecl-session.json`, inline `command` type); no statusline |
| Antigravity | auto-detected: `~/.gemini/antigravity`, `~/.gemini/antigravity-ide`, or `~/.gemini/antigravity-cli` | `./.agent` | `skills/ecl-ns-*/SKILL.md` (6 routers) + `skills/ecl-ns-*/skills/<name>/SKILL.md` (nested concretes) | `agents/ecl-*.md` | Gemini-style `settings.json` hook entries when installed by eCL |
| Cursor | `~/.cursor` | `./.cursor` | `skills/ecl-*/SKILL.md` (flat) | `agents/ecl-*.md` | Rule references under `rules/`; `hooks.json` with sessionStart context injection and postToolUse STATE.md monitor (#777) |
| Windsurf | `~/.codeium/windsurf` | `./.devin` (canonical, #1085); `./.windsurf` legacy recognized | `skills/ecl-*/SKILL.md` (flat) | `agents/ecl-*.md` | Rule references under `rules/`; no eCL hooks |
| Augment Code | `~/.augment` | `./.augment` | `skills/ecl-ns-*/SKILL.md` (6 routers) + `skills/ecl-ns-*/skills/<name>/SKILL.md` (nested concretes) | `agents/ecl-*.md` | No eCL hooks or statusline |
| Trae | `~/.trae` | `./.trae` | `skills/ecl-ns-*/SKILL.md` (6 routers) + `skills/ecl-ns-*/skills/<name>/SKILL.md` (nested concretes) | `agents/ecl-*.md` | Rule references under `rules/`; no eCL hooks |
| Qwen Code | `~/.qwen` | `./.qwen` | `skills/ecl-ns-*/SKILL.md` (6 routers) + `skills/ecl-ns-*/skills/<name>/SKILL.md` (nested concretes) | `agents/ecl-*.md` | Common eCL settings and hook entries where supported |
| Hermes Agent | `~/.hermes` | `./.hermes` | `skills/ecl/ns-*/SKILL.md` (6 routers, prefix='') + `skills/ecl/ns-*/skills/<name>/SKILL.md` (nested concretes) | `agents/ecl-*.md` | Common eCL settings and hook entries where supported |
| CodeBuddy | `~/.codebuddy` | `./.codebuddy` | `skills/ecl-*/SKILL.md` (flat, `user-invocable: false`) | `agents/ecl-*.md` | `/ecl-*` slash commands under `commands/`; common eCL settings and hook entries where supported |
| Cline | `~/.cline` | project root | `skills/ecl-ns-*/SKILL.md` (6 routers) + `skills/ecl-ns-*/skills/<name>/SKILL.md` (nested concretes) + `.clinerules` | Rules only | No eCL hooks or statusline |
