...
- The machine-readable roster lives in `docs/INVENTORY-MANIFEST.json` (regenerated by `scripts/gen-inventory-manifest.cjs --write`). For live counts, run `ls agents/ecl-*.md | wc -l` etc. against the checkout.
...
| ecl-mempalace-curator | Ship-time MemPalace curation — diary entry, cross-project tunnel proposals, wing-scoped sync pruning, and extract-learnings → KG mirroring with provenance. | MemPalace capability at `ship:post` | advanced stub |

**Coverage note.** `docs/AGENTS.md` gives full role cards for the primary agents plus concise stubs for the advanced agents. The Agent Tool Permissions Summary in that file covers only the primary agents; the advanced agents' tool lists are captured in their per-agent frontmatter in `agents/ecl-*.md`.
...
| `/ecl-plan-review-convergence` | Cross-AI plan convergence loop — replan with review feedback until no HIGH concerns or actionable non-HIGH findings remain (max 3 cycles). | [commands/ecl/plan-review-convergence.md](../commands/ecl/plan-review-convergence.md) |
...
| `/ecl-mempalace-recall` | Recall prior decisions, patterns, and surprises from MemPalace into MEMORY-RECALL.md before planning. | [commands/ecl/mempalace-recall.md](../commands/ecl/mempalace-recall.md) |
| `/ecl-mempalace-capture` | File a phase artifact (CONTEXT/PLAN/SUMMARY) verbatim into MemPalace and mirror decision facts into its temporal KG. | [commands/ecl/mempalace-capture.md](../commands/ecl/mempalace-capture.md) |
...
| `plan-review-convergence.md` | Cross-AI plan convergence loop — replan with review feedback until no HIGH concerns or actionable non-HIGH findings remain. | `/ecl-plan-review-convergence` |
...
| `loop-hook-dispatch.md` | Generic dispatch contract for consuming `ecl_run loop render-hooks <point> --raw` output in any host-loop workflow — envelope shape, per-kind dispatch rules (contribution/step/gate), and liveness banner. |
...
> **Subdirectory:** `evolv-coder-lite/references/few-shot-examples/` contains additional few-shot examples (`plan-checker.md`, `verifier.md`) that are referenced from specific agents. These are not among the top-level references.
...
| `audit-command-router.cjs` | ADR-959 capability command router for `ecl-tools audit-uat` and `ecl-tools audit-open` — extracted from hardcoded cases in `ecl-tools.cjs`; dispatches to `uat.cjs:cmdAuditUat` and `audit.cjs:{auditOpenArtifacts,formatAuditReport}`; phase 4d-impl-3 |
...
| `command-roster.cjs` | Read-only discovery of canonical `commands/ecl/*.md` command stems for runtime artifact conversion and namespace rewrites |
...
| `git-base-branch.cjs` | Single base-branch resolver (`ecl_run query git.base-branch`) with full precedence ladder: config override → origin/HEAD symref → `git remote show origin` → local branch presence → "main". Eliminates per-workflow duplicated bash detection (#1146) |
| `graphify.cjs` | Knowledge-graph build/query/status/diff for `/ecl-graphify` |
| `graphify-command-router.cjs` | ADR-959 capability command router for `ecl-tools graphify` — dispatches build/query/status/diff subcommands; first real capability command cutover (phase 4d-impl-2) |
...
| `intel-command-router.cjs` | ADR-959 capability command router for `ecl-tools intel` — extracted from the `case 'intel':` arm in `ecl-tools.cjs`; dispatches query/status/diff/snapshot/patch-meta/validate/extract-exports/update/api-surface subcommands; preserves `timeAgo` transform on `status.files[*].updated_at` in non-raw mode; phase 4d-impl-4 (last first-party cutover) |
...
| `ecl-ensure-canonical-path.js` | `SessionStart` | Symlinks `~/.claude/evolv-coder-lite/{bin,contexts,references,templates,workflows}` to the plugin's bundled tree so `@~/.claude/evolv-coder-lite/...` includes resolve in marketplace plugin installs; no-op in classic installs, self-heals after `claude plugin update` (#997) |
