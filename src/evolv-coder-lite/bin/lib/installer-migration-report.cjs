...
// runs without a TTY (typical /ecl:update path via Claude Code or any
// scripted update), prompt-user migration actions cannot be answered
// interactively. We resolve them by classification:
//   - Stale SDK build artifacts (evolv-coder-lite/sdk/{dist,src}/ecl-*):
//     default `remove`. Fresh install supplies replacements.
//   - User-facing skill anchors (skills/ecl-*/SKILL.md): default `keep`.
...
const RESOLUTION_ENV_VAR = 'ECL_INSTALLER_MIGRATION_RESOLVE';
...
// set — a shape regex like `^hooks/ecl-[^/]+\.(?:js|sh|cjs|mjs)$` also
...
// migration journal under ecl-migration-journal/<runId>-backups/).
