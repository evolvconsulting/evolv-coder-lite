...
AI_PHASE_CFG=$(ecl_run query config-get workflow.ai_integration_phase 2>/dev/null || echo "true")
```
...
SECURITY_ASVS=$(ecl_run query config-get workflow.security_asvs_level --raw 2>/dev/null || echo "1")
SECURITY_BLOCK=$(ecl_run query config-get workflow.security_block_on --raw 2>/dev/null || echo "high")
```
...
```bash
UI_PHASE_CFG=$(ecl_run query config-get workflow.ui_phase 2>/dev/null || echo "true")
UI_GATE_CFG=$(ecl_run query config-get workflow.ui_safety_gate 2>/dev/null || echo "true")
```
...
```bash
PHASE_SECTION=$(ecl_run query roadmap.get-phase "${PHASE}" --pick section 2>/dev/null)
```
...
Check config:
```bash
PATTERN_MAPPER_CFG=$(ecl_run query config-get workflow.pattern_mapper 2>/dev/null || echo "true")
```
...
```bash
INTEL_CFG=$(ecl_run query config-get intel.enabled 2>/dev/null || echo "false")
