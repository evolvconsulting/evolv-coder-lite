...
const HOOK_PATH = path.join(__dirname, '..', 'hooks', 'ecl-update-banner.js');
const {
  buildBannerOutput,
  shouldSuppressFailureWarning,
  RATE_LIMIT_SECONDS,
} = require('../hooks/ecl-update-banner.js');
const { updateCacheFileName } = require('../evolv-coder-lite/bin/lib/package-identity.cjs');
...
describe('buildBannerOutput', () => {
  ...
  test('returns banner envelope when update_available is true', () => {
    const out = buildBannerOutput({
      cache: { update_available: true, installed: '1.39.0', latest: '1.40.0', package_name: '@evolvconsulting/evolv-coder-lite' },
      parseError: false,
      suppressFailureWarning: false,
    });
    ...
    assert.ok(
      out.systemMessage.includes('/ecl:update'),
      'banner should reference /ecl:update command'
    );
  });
  ...
  test('falls back to "unknown" when installed/latest missing', () => {
    const out = buildBannerOutput({
      cache: { update_available: true, package_name: '@evolvconsulting/evolv-coder-lite' },
      parseError: false,
      suppressFailureWarning: false,
    });
    assert.ok(out);
...
describe('shouldSuppressFailureWarning', () => {
  function tmpDir() {
    return fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-banner-supp-'));
  }
...
describe('ecl-update-banner.js end-to-end', () => {
  function setupHome() {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-banner-home-'));
    fs.mkdirSync(path.join(home, '.cache', 'ecl'), { recursive: true });
    return home;
  }
  ...
  function writeCache(home, contents) {
    fs.writeFileSync(
      path.join(home, '.cache', 'ecl', updateCacheFileName),
  ...
  test('emits valid SessionStart JSON when update_available=true', () => {
    ...
    try {
      writeCache(home, {
        ...
        installed: '1.39.0',
        latest: '1.40.0',
        package_name: '@evolvconsulting/evolv-coder-lite',
      });
      const r = runHook(home);
      ...
      assert.ok(parsed.systemMessage.includes('/ecl:update'));
    } finally {
      cleanup(home);
    }
  });
  ...
  test('suppresses repeat failure diagnostic within 24h via sentinel', () => {
    ...
    try {
      ...
      const sentinel = path.join(home, '.cache', 'ecl', 'banner-failure-warned-at');
...
//
// These tests load bin/install.js as a module via ECL_TEST_MODE and assert on
...
describe('install.js update-banner wiring', () => {
  ...
  test('buildUpdateBannerHookEntry produces a SessionStart hook entry', () => {
    ...
    const entry = installExports.buildUpdateBannerHookEntry(
      '"/usr/local/bin/node" "/home/u/.claude/hooks/ecl-update-banner.js"'
    );
    ...
    assert.ok(
      entry.hooks[0].command.includes('ecl-update-banner.js'),
