---
name: ecl:plan-review-convergence
...
<objective>
Cross-AI plan convergence loop — an outer revision gate around ecl-review and ecl-planner.
...
**Flow:** Skill("ecl-plan-phase") → Agent→Skill("ecl-review") → check unresolved HIGH + actionable non-HIGH → Skill("ecl-plan-phase --reviews") → Agent→Skill("ecl-review") → ... → Converge or escalate

Replaces ecl-plan-phase's internal ecl-plan-checker with external AI reviewers (codex, gemini, etc.). Plan-phase runs **inline** (bare Skill at depth 0) so it can spawn ecl-planner/ecl-plan-checker at depth 1. Review runs inside an isolated Agent (ecl-review is a Bash leaf — no sub-agents needed). Orchestrator only does loop control.

**Orchestrator role:** Parse arguments, validate phase, run plan-phase inline (Skill at depth 0), spawn an Agent for ecl-review, check unresolved HIGH and actionable non-HIGH counts, stall detection, escalation gate.
</objective>

<execution_context>
@$HOME/.claude/evolv-coder-lite/workflows/plan-review-convergence.md
