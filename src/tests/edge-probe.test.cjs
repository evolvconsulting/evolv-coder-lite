 ...
 * the BUILT artifact (`evolv-coder-lite/bin/lib/edge-probe.cjs`), which
...
const BUILT_SCRIPT = path.join(__dirname, '..', 'evolv-coder-lite', 'bin', 'lib', 'edge-probe.cjs');
...
describe('edge-probe: golden fixtures', () => {
  const root = path.join(__dirname, '..', 'evolv-coder-lite', 'references', 'edge-probe-fixtures');
