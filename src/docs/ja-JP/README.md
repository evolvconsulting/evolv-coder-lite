...
evolv Coder Lite（eCL）フレームワークの包括的なドキュメントです。eCL は、AI コーディングエージェント向けのメタプロンプティング、コンテキストエンジニアリング、仕様駆動開発システムです。
...
| [CLI ツールリファレンス](CLI-TOOLS.md) | コントリビューター、エージェント作成者 | CJS `ecl-tools.cjs` と `ecl-tools.cjs query` 가이드 のガイド |
...
- **v1.39 の新機能:** `--minimal` インストールプロファイル（≥94% コールドスタート削減）、`/ecl-phase --edit`、マージ後ビルド & テストゲート、`review.models.<cli>` ランタイム別レビューモデル、ワークストリーム設定の継承、手動カナリアリリースワークフロー、スキル統合（86 → 59）
- **はじめに:** [README](../README.md) → インストール → `/ecl-new-project`
- **ワークフロー完全ガイド:** [ユーザーガイド](USER-GUIDE.md)
- **コマンド一覧:** [コマンドリファレンス](COMMANDS.md)
- **eCL の設定:** [設定リファレンス](CONFIGURATION.md)
