...
**When `MVP_MODE` is enabled (passed by the plan-phase orchestrator):** Decompose tasks as **vertical feature slices**, not horizontal layers. Required reading: `@~/.claude/evolv-coder-lite/references/planner-mvp-mode.md` (loaded conditionally by the orchestrator).
...
1. Frame the phase goal as a user story at the top of `PLAN.md`. The user story is sourced from the `**Goal:**` line in ROADMAP.md (set by `mvp-phase`). Emit it with bolded keywords:
   ...
   **As a** [user role], **I want to** [capability], **so that** [outcome].
   ...
   Format rules from `@~/.claude/evolv-coder-lite/references/user-story-template.md`:
...
```bash
node "$HOME/.claude/evolv-coder-lite/bin/ecl-tools.cjs" graphify status
```
...
```bash
node "$HOME/.claude/evolv-coder-lite/bin/ecl-tools.cjs" graphify query "<phase-goal-keyword>" --budget 2000
```

(graphify is not exposed on `ecl-tools query` yet; use `ecl-tools.cjs` for graphify only.)
