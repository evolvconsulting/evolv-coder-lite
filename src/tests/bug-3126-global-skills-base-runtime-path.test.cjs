...
//
// Fix introduces evolv-coder-lite/bin/lib/runtime-homes.cjs with first-class
// support for all 15 supported runtimes, including:
//   - hermes: nested skills/ecl/<skillName>/ layout (#2841)
...
describe('bug #3126: runtime-homes getGlobalConfigDir — defaults', () => {
  ...
  for (const [runtime, expected] of defaults) {
    test(`${runtime} default configDir`, () => {
      ...
      const { runtimes: _reg3126 } = require(path.join(ROOT, 'evolv-coder-lite', 'bin', 'lib', 'capability-registry.cjs'));
...
describe('bug #3126: runtime-homes env-var overrides', () => {
  ...
  test('antigravity detects 2.x IDE dir when legacy dir is absent', () => {
    const home = require('node:fs').mkdtempSync(path.join(os.tmpdir(), 'ecl-antigravity-home-'));
    try {
...
describe('bug #3126: runtime-homes getGlobalSkillsBase', () => {
  ...
  test('hermes: skills at <configDir>/skills/ecl (nested layout #2841)', () => {
    withEnv('HERMES_HOME', undefined, () => {
      assert.strictEqual(
        getGlobalSkillsBase('hermes'),
        path.join(os.homedir(), '.hermes', 'skills', 'ecl'),
      );
    });
  });
...
describe('bug #3126: runtime-homes getGlobalSkillDir', () => {
  test('cursor: <configDir>/skills/<skillName>', () => {
    withEnv('CURSOR_CONFIG_DIR', undefined, () => {
      assert.strictEqual(
        getGlobalSkillDir('cursor', 'ecl-executor'),
        path.join(os.homedir(), '.cursor', 'skills', 'ecl-executor'),
      );
    });
  });
  test('hermes: <configDir>/skills/ecl/<skillName>', () => {
    withEnv('HERMES_HOME', undefined, () => {
      assert.strictEqual(
        getGlobalSkillDir('hermes', 'ecl-executor'),
        path.join(os.homedir(), '.hermes', 'skills', 'ecl', 'ecl-executor'),
      );
    });
  });
  test('cline: returns ~/.cline/skills/ecl-executor (skills-capable since v3.48.0 — #782)', () => {
    withEnv('CLINE_CONFIG_DIR', undefined, () => {
      assert.strictEqual(
        getGlobalSkillDir('cline', 'ecl-executor'),
        path.join(os.homedir(), '.cline', 'skills', 'ecl-executor'),
      );
    });
  });
});
...
describe('bug #3126: init.cjs uses runtime-homes not hardcoded .claude', () => {
  test('init.cjs has no hardcoded globalSkillsBase assignment to ~/.claude/skills', () => {
    ...
    const src = fs.readFileSync(
      path.join(ROOT, 'evolv-coder-lite', 'bin', 'lib', 'init.cjs'),
      'utf8',
    );
    assert.ok(
