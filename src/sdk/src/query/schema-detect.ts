/**
 * Schema drift detection — ports `evolv-coder-lite/bin/lib/schema-detect.cjs`.
 * Used by `verify.schema-drift` to match ecl-tools.cjs JSON output.
 */
...
export function checkSchemaDrift(
  ...
  const message = [
    ...
    pushCommands,
    '',
    'Run the appropriate push command, or set ECL_SKIP_SCHEMA_CHECK=true to bypass this gate.',
  ].join('\n');
  ...
  if (skipCheck) {
    return {
      ...
      orms: detection.orms,
      unpushedOrms,
      message: 'Schema drift detected but check was skipped (ECL_SKIP_SCHEMA_CHECK=true).',
    };
  }
