// Migrated to typed-IR (#2974): the ecl-session-state.sh and
// ecl-phase-boundary.sh hooks now emit Claude Code SessionStart/PostToolUse
...
// plain text. ecl-validate-commit.sh already emitted JSON ({ decision,
// reason }). Tests parse the JSON and assert on typed fields.

/**
 * eCL Tools Tests - Community Hooks (opt-in)
 *
...
function createTempProject(prefix = 'ecl-hook-test-') {
...
describe('hook file validation', () => {
  test('ecl-session-state.sh exists', () => {
    const hookPath = path.join(HOOKS_DIR, 'ecl-session-state.sh');
    assert.ok(fs.existsSync(hookPath), 'ecl-session-state.sh should exist');
  });

  test('ecl-validate-commit.sh exists', () => {
    const hookPath = path.join(HOOKS_DIR, 'ecl-validate-commit.sh');
    assert.ok(fs.existsSync(hookPath), 'ecl-validate-commit.sh should exist');
  });

  test('ecl-phase-boundary.sh exists', () => {
    const hookPath = path.join(HOOKS_DIR, 'ecl-phase-boundary.sh');
    assert.ok(fs.existsSync(hookPath), 'ecl-phase-boundary.sh should exist');
  });

  test('ecl-session-state.sh is executable', { skip: isWindows ? 'Windows has no POSIX file permissions' : false }, () => {
    const hookPath = path.join(HOOKS_DIR, 'ecl-session-state.sh');
    const stat = fs.statSync(hookPath);
    assert.ok((stat.mode & 0o111) !== 0, 'ecl-session-state.sh should be executable');
  });

  test('ecl-validate-commit.sh is executable', { skip: isWindows ? 'Windows has no POSIX file permissions' : false }, () => {
    const hookPath = path.join(HOOKS_DIR, 'ecl-validate-commit.sh');
    const stat = fs.statSync(hookPath);
    assert.ok((stat.mode & 0o111) !== 0, 'ecl-validate-commit.sh should be executable');
  });

  test('ecl-phase-boundary.sh is executable', { skip: isWindows ? 'Windows has no POSIX file permissions' : false }, () => {
    const hookPath = path.join(HOOKS_DIR, 'ecl-phase-boundary.sh');
    const stat = fs.statSync(hookPath);
    assert.ok((stat.mode & 0o111) !== 0, 'ecl-phase-boundary.sh should be executable');
  });
});
...
describe('opt-in gating behavior', { skip: isWindows ? 'bash hooks require unix shell' : false }, () => {
  ...
  test('validate-commit is a no-op when config.json is absent', (t) => {
    ...
    const bareDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-hook-bare-'));
...
describe('hook execution when enabled', { skip: isWindows ? 'bash hooks require unix shell' : false }, () => {
  ...
  test('session-state exits 0 without .planning/ (in enabled project)', (t) => {
    ...
    const noStateDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-hook-nostate-'));
