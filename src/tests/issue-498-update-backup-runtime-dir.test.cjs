 ...
 * update.md must derive RUNTIME_DIR from ECL_DIR.
 *
 * The get_installed_version step was rewritten to call `ecl-tools update-context`
 * and now emits ECL_DIR (the resolved config dir) instead of the old probe-loop
 ...
 * (commands/ecl, evolv-coder-lite), user-added files inside those dirs could be
 * deleted without the intended backup.
 *
 * This locks the fix: RUNTIME_DIR comes from ECL_DIR, and the dead LOCAL_DIR /
 * GLOBAL_DIR references are gone.
 *
 * Source-text-is-the-product: update.md's bash blocks ARE the deployed /ecl:update
...
const UPDATE_MD = path.join(__dirname, '..', 'evolv-coder-lite', 'workflows', 'update.md');
...
describe('#498 regression: update.md backup uses ECL_DIR, not the removed LOCAL_DIR/GLOBAL_DIR', () => {
  const src = codeOnly(UPDATE_MD);

  test('RUNTIME_DIR is assigned from ECL_DIR', () => {
    assert.match(
      src,
      /RUNTIME_DIR="\$ECL_DIR"/,
      'backup_custom_files must set RUNTIME_DIR="$ECL_DIR" (the resolved config dir from update-context)',
    );
  });
