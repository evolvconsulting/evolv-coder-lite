...
// commands/ecl/*.md text IS what the runtime loads — testing description
 ...
 *
 * Verifies:
 * 1. All skill descriptions in commands/ecl/*.md are <= 100 chars
...
/**
 * Get all .md files in commands/ecl/ with their descriptions.
 */
...
describe('description length budget', () => {
  test('all commands/ecl/*.md descriptions are <= 100 chars', () => {
...
describe('lint-descriptions.cjs', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-lint-desc-test-'));
  });
