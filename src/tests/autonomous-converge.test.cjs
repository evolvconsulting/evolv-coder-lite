...
const COMMAND_PATH = path.join(REPO_ROOT, 'commands', 'ecl', 'autonomous.md');
const WORKFLOW_PATH = path.join(REPO_ROOT, 'evolv-coder-lite', 'workflows', 'autonomous.md');
...
describe('autonomous --converge flag (#711)', () => {
  ...
  test('workflow routes planning through plan-review-convergence when enabled', () => {
    ...
    assert.match(
      ...
      'non-interactive converge mode should call ecl-plan-review-convergence',
    );
    assert.match(
      workflow,
      /Run plan convergence for phase \$\{PHASE_NUM\}: Skill\(skill=\\"ecl-plan-review-convergence\\"/,
  ...
  test('docs show autonomous convergence usage', () => {
    ...
    assert.match(howTo, /\/ecl-autonomous --only 4 --converge/, 'how-to should show single-phase converge usage');
  });
});
