...
- ecl-security-auditor — Verifies threat mitigation coverage
</available_agent_types>
...
AGENT_SKILLS_AUDITOR=$(ecl_run query agent-skills ecl-security-auditor)
```

Parse: `phase_dir`, `phase_number`, `phase_name`, `phase_slug`, `padded_phase`.

```bash
AUDITOR_MODEL=$(ecl_run query resolve-model ecl-security-auditor --raw)
...
If no active secure-phase step hook exists: exit with "Security enforcement disabled. Enable via /ecl:settings."

Display banner: `eCL > SECURE PHASE {N}: {name}`
...
- **State C** (`SUMMARY_FILES` empty): Exit — "Phase {N} not executed. Run /ecl:execute-phase {N} first."
...
```
Agent(
  prompt="Read ~/.claude/agents/ecl-security-auditor.md for instructions.\n\n" +
  ...
  subagent_type="ecl-security-auditor",
  model="{AUDITOR_MODEL}",
...
**State B (create):**
1. Read template from `~/.claude/evolv-coder-lite/templates/SECURITY.md`
...
```
eCL > PHASE {N} SECURITY BLOCKED
{K} threats open — phase advancement blocked until threats_open: 0
▶ Fix mitigations then re-run: /ecl:secure-phase {N}
...
```bash
ecl_run query commit "docs(phase-${PHASE}): add/update security threat verification"
```
...
```
eCL > PHASE {N} THREAT-SECURE
threats_open: 0 — all threats have dispositions.
▶ /ecl:validate-phase {N}    validate test coverage
▶ /ecl:verify-work {N}       run UAT
```
