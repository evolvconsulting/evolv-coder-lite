...
describe('AUDIT-FIX: command file', () => {
  ...
  test('command file exists', () => {
    assert.ok(
      ...
      'audit-fix.md must exist in commands/ecl/'
    );
  });

  test('has valid frontmatter with name ecl:audit-fix', () => {
    const content = fs.readFileSync(cmdPath, 'utf-8');
    const frontmatter = content.split('---')[1] || '';
    assert.ok(
      frontmatter.includes('name: ecl:audit-fix'),
      'name must be ecl:audit-fix'
    );
  });
  ...
  test('has argument-hint in frontmatter for /ecl-help discoverability', () => {
    const content = fs.readFileSync(cmdPath, 'utf-8');
    const frontmatter = content.split('---')[1] || '';
    assert.ok(
      frontmatter.includes('argument-hint:'),
      'must have argument-hint in frontmatter for /ecl-help discoverability'
    );
  });
...
describe('AUDIT-FIX: workflow file', () => {
  ...
  test('workflow file exists', () => {
    assert.ok(
      ...
      'audit-fix.md must exist in evolv-coder-lite/workflows/'
    );
  });
  ...
  test('has <available_agent_types> listing ecl-executor', () => {
    const content = fs.readFileSync(wfPath, 'utf-8');
    assert.ok(
    ...
    assert.ok(
      content.includes('ecl-executor'),
      'must list ecl-executor as available agent type'
    );
  });
});
