 ...
 *
 * Background: every installed `ecl-*` skill costs eager system-prompt tokens
 * because runtimes (Claude Code, opencode, etc.) enumerate skill descriptions
 * in `<available_skills>` on every turn. With 66 skills + 33 agents eCL alone
 ...
 *  - loadSkillsManifest: parse requires: frontmatter from commands/ecl/*.md
 ...
 *  - readActiveProfile / writeActiveProfile: .ecl-profile marker persistence
 *  - resolveEffectiveProfile: explicit flag > .ecl-profile marker > full
 *  - mostRestrictiveProfile: resolve multi-runtime disagreement to smallest set
 *
 * Companion module (Phase 2): evolv-coder-lite/bin/lib/surface.cjs owns the
 * runtime /ecl:surface command. It reuses stageSkillsForProfile and
 * stageAgentsForProfile from this module for cluster-level enable/disable
 * without reinstall, persisting state in <runtimeConfigDir>/.ecl-surface.json.
 *
 ...
 * since eCL enforces flow-style arrays for requires:.
 *
 ...
 * Scans the full content for `ecl-<stem>` patterns that correspond to
 * real agent files. Returns all unique `ecl-*` stems found in the body.
 *
 * The caller is responsible for filtering by which agents actually exist —
 * this function returns all syntactically valid `ecl-*` matches.
 *
 * @param {string} content full file content
 * @returns {string[]} deduplicated agent stems like ['ecl-planner', 'ecl-executor']
 */
...
/**
 * Load the requires: dependency graph from a commands/ecl directory.
 * Also derives calls_agents for each skill by scanning the body text for
 * `ecl-*` agent name references. Agent stems are stored under the special
 * key `_calls_agents_<stem>` so they don't conflict with skill stems.
 *
 * @param {string} commandsDir absolute path to commands/ecl/
...
/**
 * Stage a filtered copy of commands/ecl for a resolved profile.
 * In full mode (skills === '*') returns srcDir unchanged (no-op).
 *
 * @param {string} srcDir absolute path to commands/ecl
...
function stageSkillsForProfile(srcDir, resolvedProfile) {
  ...
  const stageDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-profile-skills-'));
  try {
 ...
 * For tiered profiles, copies only agents whose full stem (e.g. 'ecl-planner')
...
function stageAgentsForProfile(srcAgentsDir, resolvedProfile) {
  ...
  const stageDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-profile-agents-'));
  try {
    if (resolvedProfile.agents instanceof Set && resolvedProfile.agents.size > 0) {
      ...
      for (const entry of entries) {
        if (!entry.isFile()) continue;
        if (!entry.name.endsWith('.md')) continue;
        // Agent stem is the full filename without extension, e.g. "ecl-planner"
        const stem = entry.name.slice(0, -3);
...
function stageSkillsForRuntimeAsSkills(srcCommandsDir, resolvedProfile, converter, prefix) {
  ...
  const stageDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-profile-runtime-skills-'));
  try {
...
const PROFILE_MARKER_NAME = '.ecl-profile';
...
function resolveEffectiveProfile({ requestedProfileName, targetDir }) {
  ...
  // 2. Marker-driven (ecl update path)
...
/**
 * Stage a filtered copy of the source commands/ecl directory.
...
function stageSkillsForMode(srcDir, mode) {
  ...
  const stageDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-minimal-skills-'));
  try {
    const entries = fs.readdirSync(srcDir, { withFileTypes: true });
    for (const entry of entries) {
      if (!entry.isFile()) continue;
