 ...
 * (#1259, ADR-550 Decision 5d "heavy half"; the D1 seam — a NEW deterministic ecl-tools
 ...
 * `evolv-coder-lite/bin/lib/prohibition-enforcement.cjs`. Do NOT hand-write the `.cjs`; it is emitted.
 *
...
export interface CheckDescriptor {
   ...
   * test exercises via the `ECL_PROHIB_SUBJECT` env convention, expected to drive the test RED.
 ...
 *   - node-test: spawn the negative test (TAP) with `ECL_PROHIB_SUBJECT` set to the `violationFixture`
...
function defaultProveFailFirst(check: CheckDescriptor, cwd: string, timeoutMs?: number): FailFirstProof {
  try {
    ...
    if (check.kind === 'node-test') {
      ...
      // `ECL_PROHIB_SUBJECT` point at a missing file; an honest negative test reading that subject
      ...
      // `ECL_PROHIB_SUBJECT` is set (rather than because the subject's CONTENT violates the must-NOT)
      ...
      // `ECL_PROHIB_SUBJECT` and resolves a relative subject against `cwd`, so the existence check must
      ...
      try {
        out = execFileSync(process.execPath, buildNodeTestArgs(check), {
          ...
          env: { ...childEnv(), ECL_PROHIB_SUBJECT: fixture },
          timeout: posTimeout(timeoutMs, NODE_TEST_TIMEOUT_MS),
          maxBuffer: CHECK_MAX_BUFFER,
        });
      } catch (e) {
...
/**
 * CLI surface: `ecl_run check prohibition-enforcement <request.json>` (or `--json '<inline>'`).
