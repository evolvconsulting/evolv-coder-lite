 ...
 * strings.
 *
 * This lint makes those literals *value-checked*: every eCL package/repo
...
/**
 * Pure: find every eCL coordinate literal in `text` that does not match the
...
const SCAN_DIRS = ['bin', 'hooks', 'scripts', 'evolv-coder-lite'];
...
const EXEMPT = new Set([
  path.join('evolv-coder-lite', 'bin', 'lib', 'package-identity.cjs'),
...
function scanRepo(root) {
  const seam = require(path.join(root, 'evolv-coder-lite', 'bin', 'lib', 'package-identity.cjs'));
...
function main() {
  ...
  if (violations.length === 0) {
    process.stdout.write('ok identity-drift: all eCL coordinate literals match the seam\n');
    return;
  }
  process.stderr.write('identity-drift: stale eCL coordinate literal(s) found.\n');
