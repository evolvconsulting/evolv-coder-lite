...
Set `model_profile` in `.planning/config.json` or via `/ecl-config --profile <name>`:
...
```bash
/ecl-config --profile balanced   # Normal development
/ecl-config --profile budget     # Prototyping or high-cost phases
/ecl-config --profile quality    # Production release
/ecl-config --profile inherit    # OpenRouter, local models
```
...
`model_overrides` can be set per-project in `.planning/config.json` or globally in `~/.ecl/defaults.json`. Per-project entries win on conflict; non-conflicting global entries are preserved.
...
```bash
npx @evolvconsulting/evolv-coder-lite@latest --codex --global   # or --opencode, --kilo, etc.
```
...
All five research agents resolve to `sonnet` *except* `ecl-codebase-mapper`, which is pinned to `haiku`.
...
Each agent has a default tier (`light`, `standard`, or `heavy`). On the first attempt, eCL picks `tier_models[default_tier]`. If the orchestrator detects a soft failure (verification inconclusive, plan-check flagged, etc.), it re-spawns the agent one tier up. `max_escalations` caps the total retries.
...
## Using eCL on non-Anthropic runtimes

If you installed eCL for Codex, OpenCode, Gemini CLI, or Kilo, the installer already set `resolve_model_ids: "omit"` in your config. This tells eCL to skip Anthropic model ID resolution and let the runtime choose its own default model. No manual setup is needed for the basic case.
...
eCL resolves each tier alias to the Codex-native model and reasoning effort defined in the runtime tier map.
