 ...
 * Keeps ecl-tools.cjs thin while preserving existing command semantics.
 *
 ...
 * - ECL_WORKSTREAM is active (workstream-scoped requests fall through to CJS).
...
function routePhaseCommand({ phase, args, cwd, raw, error }) {
  const activeWorkstream = process.env.ECL_WORKSTREAM;
  ...
  const UNSUPPORTED = {
    'list-plans': 'phase list-plans is SDK-only. Use: ecl-sdk query phase.list-plans ...',
    'list-artifacts': 'phase list-artifacts is SDK-only. Use: ecl-sdk query phase.list-artifacts ...',
