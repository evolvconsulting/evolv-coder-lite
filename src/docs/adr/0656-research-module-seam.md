...
All three modules are reachable via `ecl-tools query research-plan | research-store | package-legitimacy`.
...
- The `ecl-tools query` interface is the test surface — behavioral tests can assert typed JSON output without source-grep.
