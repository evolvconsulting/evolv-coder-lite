...
// commands/ecl/*.md files ARE the deployed registry — reading their frontmatter
 ...
 * registry: commands/ecl/*.md (one file per registered command).
 *
 * Each command file has YAML frontmatter with a `name:` field:
 *   name: ecl:slug    (colon-style — most commands)
 *   name: ecl-slug    (dash-style — ns-* namespace commands)
 *
 * For each slug, three canonical token forms are emitted:
 *   /ecl-slug   — Claude / non-Gemini runtimes
 *   /ecl:slug   — Gemini runtime
 *   $ecl-slug   — Codex runtime
 *
 ...
 *   name: ecl:slug     → slug = "slug"
 *   name: ecl-slug     → slug = "slug"
 *   name: "ecl:slug"   → slug = "slug"  (quoted)
 *   name: "ecl-slug"   → slug = "slug"  (quoted)
 */
function parseSlug(content, filePath) {
  ...
  // The value must be one of: ecl:<slug> or ecl-<slug>
  ...
  if (!nameMatch) {
    throw new Error(
      ...
      `(expected "name: ecl:<slug>" or "name: ecl-<slug>")`
    );
  }

  return nameMatch[2]; // the slug after "ecl:" or "ecl-"
}

/**
 * Returns the Set<string> of all canonical slash-command tokens derived from
 * commands/ecl/*.md. Memoized — safe to call repeatedly without extra fs I/O.
 *
...
function getLiveCommandTokens() {
  ...
  for (const fileName of entries) {
    ...
    tokens.add(`/ecl-${slug}`);   // Claude / non-Gemini
    tokens.add(`/ecl:${slug}`);   // Gemini
    tokens.add(`$ecl-${slug}`);   // Codex
  }
