 ...
 * `/ecl-update` unrecoverable when leftover `ecl-*` files were classified
 * as `stale-ecl-looking`.
 *
 * Fix shape (per triage brief):
 *   A. Classify-and-default for safe categories - stale SDK build
 *      artifacts default to "remove"; user-facing skills/ecl-asterisk/SKILL.md
...
describe('#3541: installer migration prompt-user non-TTY resolution', { concurrency: false }, () => {
  ...
  beforeEach(() => {
    configDir = createTempDir('ecl-3541-');
  });
  ...
  test('Test A: non-TTY default resolution removes stale SDK artifacts and keeps user skills', () => {
    ...
    // 24 stale `evolv-coder-lite/sdk/{dist,src}/ecl-*` files leaked into the
    // baseline because the new manifest no longer classifies them as managed.
    writeFile(configDir, 'evolv-coder-lite/sdk/dist/ecl-old-bundle.js', 'stale sdk bundle\n');
    // User-facing skill: replicates `skills/ecl-roadmap/SKILL.md` from the
    // same incident — user-owned content that must be preserved.
    writeFile(configDir, 'skills/ecl-roadmap/SKILL.md', '# Roadmap skill\nuser content\n');

    // Plant an empty manifest so both files classify as `stale-ecl-looking`
    // (they look like eCL artifacts but are not manifest-managed).
    ...
    assert.deepEqual(
      blockedPaths,
      ['evolv-coder-lite/sdk/dist/ecl-old-bundle.js', 'skills/ecl-roadmap/SKILL.md'],
    ...
    const sdkResolution = byPath.get('evolv-coder-lite/sdk/dist/ecl-old-bundle.js');
    const skillResolution = byPath.get('skills/ecl-roadmap/SKILL.md');
  ...
  test('Test B: error message groups paths by reason and suggests a resolution path', () => {
    ...
    const blocked = [
      {
        type: 'prompt-user',
        relPath: 'evolv-coder-lite/sdk/dist/ecl-a.js',
      ...
      {
        type: 'prompt-user',
        relPath: 'evolv-coder-lite/sdk/dist/ecl-b.js',
        reason: 'eCL-looking file is not proven manifest-managed and needs explicit user choice',
        classification: 'stale-ecl-looking',
        prompt: 'Choose whether to remove this stale-looking eCL artifact or keep it as user-owned.',
        choices: ['keep', 'remove'],
    ...
    assert.match(
      message,
      /ECL_INSTALLER_MIGRATION_RESOLVE/,
  ...
  test('Test C: non-TTY env override resolves otherwise-unclassified prompt-user actions', () => {
    const result = {
      blocked: [
        {
          type: 'prompt-user',
          relPath: 'skills/ecl-custom/SKILL.toml',
    ...
    const resolved = resolveInstallerMigrationPromptsForNonTty(result, {
      isTty: false,
      env: { ECL_INSTALLER_MIGRATION_RESOLVE: 'keep' },
    });
    ...
    assert.equal(resolved.resolutions[0].source, 'ECL_INSTALLER_MIGRATION_RESOLVE');
