...
// commands/ecl/*.md files are the deployed skill surface. Their frontmatter
// IS the runtime contract. Checking frontmatter fields checks deployed behaviour.

/**
 * #3156 — plan-phase auto-dispatches to ecl-planner subagent on OpenCode,
 * losing Task tool access.
 *
 * Root cause: commands/ecl/plan-phase.md had `agent: ecl-planner` in its
 ...
 *
 * This test parses the YAML frontmatter of every commands/ecl/*.md file and
 * asserts:
...
// the main agent context (i.e., all eCL commands) must omit this directive.
