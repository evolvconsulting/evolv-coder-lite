...
> **说明文档** — 本文档阐述 eCL Core *为何*围绕多智能体编排进行设计，以及*各组件如何协同工作*。这不是操作指南。有关配置，请参阅
...
eCL Core 的多智能体设计正是对这一问题的直接回应。与其让一个
...
`evolv-coder-lite/workflows/` 中的每个工作流都遵循相同的结构：
...
eCL Core 的智能体按功能类别划分，对应
研究 → 规划 → 执行 → 验证的流水线：

| 类别 | 智能体 | 典型并行度 |
|---|---|---|
| 研究员 | `ecl-project-researcher`、`ecl-phase-researcher`、`ecl-ui-researcher`、`ecl-advisor-researcher` | 4 个并行（技术栈、功能、架构、潜在问题） |
| 综合员 | `ecl-research-synthesizer` | 顺序执行，在研究员完成后运行 |
| 规划员 | `ecl-planner`、`ecl-roadmapper` | 顺序执行 |
| 检查员 | `ecl-plan-checker`、`ecl-integration-checker`、`ecl-ui-checker`、`ecl-nyquist-auditor` | 顺序执行，最多 3 次修订迭代 |
| 执行员 | `ecl-executor` | 波次内并行，波次间顺序 |
| 验证员 | `ecl-verifier` | 顺序执行，在所有执行员完成后运行 |
| 映射员 | `ecl-codebase-mapper` | 4 个并行子探针 |
| 审计员 | `ecl-ui-auditor`、`ecl-security-auditor` | 顺序执行 |
...
多智能体设计最直观的体现是 `/ecl-execute-phase`
如何处理一组可能相互依赖的计划。
...
单个能力强大的智能体在一个上下文中工作会更快完成。eCL 通过
...
这是隔离的代价。`ecl-tools.cjs init` 处理器
