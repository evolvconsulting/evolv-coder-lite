...
describe('sanitizePrompt', () => {
  ...
  describe('@file: references', () => {
    ...
    it('strips @file: with various path formats', () => {
      const input = [
        ...
        '@file:~/.claude/evolv-coder-lite/workflows/execute-plan.md',
      ].join('\n');
  ...
  describe('/ecl- skill commands', () => {
    it('strips lines containing /ecl- commands', () => {
      const input = 'Before\nRun /ecl-execute-plan to proceed\nAfter';
      const result = sanitizePrompt(input);
      expect(result).not.toContain('/ecl-');
      expect(result).toContain('Before');
      expect(result).toContain('After');
    });

    it('strips various /ecl- skill formats', () => {
      const input = [
        'Use /ecl-research-phase',
        'Then /ecl-plan-phase --auto',
        'Finally /ecl-verify-phase',
      ].join('\n');
      expect(sanitizePrompt(input)).toBe('');
    });

    it('strips legacy /ecl: slash commands for backward compatibility', () => {
      const input = 'Before\nRun /ecl:execute-plan to proceed\nAfter';
      const result = sanitizePrompt(input);
      expect(result).not.toContain('/ecl:');
      expect(result).toContain('Before');
    ...
    it('strips various legacy /ecl: command formats', () => {
      const input = [
        'Use /ecl:research-phase',
        'Then /ecl:plan-phase --auto',
        'Finally /ecl:verify-phase',
  ...
  describe('multiple patterns in one string', () => {
    it('strips all pattern types from a mixed prompt', () => {
      const input = [
        ...
        '',
        'When done, run /ecl-plan-phase to proceed.',
        '',
