/**
 * eCL Agent Installation Validation Tests (#1371)
 *
 * Validates that eCL detects missing or incomplete agent installations and
 * surfaces warnings through init commands and health checks. When agents are
 * not installed, Task(subagent_type="ecl-*") silently falls back to
...
const MODEL_PROFILES = require('../evolv-coder-lite/bin/lib/model-profiles.cjs').MODEL_PROFILES;
const EXPECTED_AGENTS = Object.keys(MODEL_PROFILES);
const ROOT = path.join(__dirname, '..');
const INSTALL_SCRIPT = path.join(ROOT, 'bin', 'install.js');

/**
 * Create a fake eCL install directory structure that mirrors what the installer
 * produces. ecl-tools.cjs lives at <configDir>/evolv-coder-lite/bin/ecl-tools.cjs,
 * so the agents dir is at <configDir>/agents/.
 *
 * We use --cwd to point at the project, and ECL_INSTALL_DIR env to override
...
describe('init commands: agents_installed field (#1371)', () => {
  ...
  // Point the SDK at the repo's agents/ dir (sibling of evolv-coder-lite/) via the
  // ECL_AGENTS_DIR override. The SDK side of init resolves agents from
  // ECL_AGENTS_DIR or the runtime config dir (~/.claude/agents for Claude); it
  ...
  test('init execute-phase includes agents_installed=true when agents exist', () => {
    ...
    const result = runGsdTools('init execute-phase 1 --raw', tmpDir, { ECL_AGENTS_DIR: REPO_AGENTS_DIR });
    ...
    assert.strictEqual(output.agents_installed, true,
      'agents_installed should be true when ECL_AGENTS_DIR has ecl-*.md files');
  });
  ...
  test('init plan-phase includes agents_installed=true when agents exist', () => {
    ...
    const result = runGsdTools('init plan-phase 1 --raw', tmpDir, { ECL_AGENTS_DIR: REPO_AGENTS_DIR });
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('validate health: agent installation check W010 (#1371)', () => {
  ...
  test('health check reports healthy when agents are installed (repo layout)', () => {
    // In the repo, agents/ exists as a sibling of evolv-coder-lite/, so the
    // health check should find them via the ecl-tools.cjs path resolution
...
describe('checkAgentsInstalled: Copilot .agent.md format (#1512)', () => {
  ...
  test('agents_installed=true when agents exist as .agent.md (Copilot format)', () => {
    // Simulate a Copilot install: agents are named ecl-*.agent.md, not ecl-*.md
    // Use ECL_AGENTS_DIR to point at an isolated dir with ONLY .agent.md files,
    ...
    const result = runGsdTools('validate agents --raw', tmpDir, { ECL_AGENTS_DIR: agentsDir });
    assert.ok(result.success, `Command failed: ${result.error}`);

    const output = JSON.parse(result.output);
    // Must report the custom dir, not the default repo agents dir
    assert.strictEqual(output.agents_dir, agentsDir,
      'agents_dir must be the ECL_AGENTS_DIR override, not the repo default');
  ...
  test('init new-workspace includes agents_installed=true with Copilot .agent.md files', () => {
    ...
    const result = runGsdTools('init new-workspace --raw', tmpDir, { ECL_AGENTS_DIR: agentsDir });
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('ECL_AGENTS_DIR env var overrides default agents directory', () => {
    ...
    const result = runGsdTools('validate agents --raw', tmpDir, { ECL_AGENTS_DIR: customAgentsDir });
    assert.ok(result.success, `Command failed: ${result.error}`);

    const output = JSON.parse(result.output);
    // The custom dir path should be reported
    assert.strictEqual(output.agents_dir, customAgentsDir,
      'agents_dir must reflect ECL_AGENTS_DIR override');
  });
});
...
describe('checkAgentsInstalled: Kimi agents/subagents layout', () => {
  test('Kimi install is detected by init, validate agents, and health checks', () => {
    const tmpDir = createTempProject('ecl-kimi-agent-status-project-');
    const tmpConfig = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-kimi-agent-status-config-'));
    const tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-kimi-agent-status-home-'));
    ...
    try {
      ...
      const agentWarnings = (healthOutput.warnings || []).filter(
        (warning) => warning.code === 'W010' || /eCL agents/i.test(warning.message || ''),
      );
...
describe('bug #1058: validate agents detects manifest-backed .md/.toml pair drift', () => {
  // All real agents/ecl-*.md files in the repo (used to populate fixtures)
  const REPO_AGENTS_DIR_1058 = path.resolve(__dirname, '..', 'agents');

  /**
   * Copy all ecl-*.md files from the repo agents dir into destDir.
   */
  function copyAgentMdFiles(destDir) {
    const files = fs.readdirSync(REPO_AGENTS_DIR_1058).filter(f => /^ecl-.*\.md$/.test(f));
  ...
  /**
   * Build a ecl-file-manifest.json whose files map includes both .md and .toml
  ...
  test('agents_found=false and incomplete non-empty when .toml files are absent but manifest expects them', () => {
    ...
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-bug-1058-missing-toml-'));
    const agentsDir = path.join(tmpDir, 'agents');
    fs.mkdirSync(agentsDir, { recursive: true });
    ...
    fs.writeFileSync(
      path.join(tmpDir, 'ecl-file-manifest.json'),
  ...
  test('agents_found=false and incomplete non-empty when .md files are absent but manifest expects them', () => {
    ...
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-bug-1058-missing-md-'));
    const agentsDir = path.join(tmpDir, 'agents');
    fs.mkdirSync(agentsDir, { recursive: true });
...
`);
  ...
  test('agents_found=true and incomplete=[] when both .md and .toml files are present', () => {
    ...
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-bug-1058-complete-pair-'));
    const agentsDir = path.join(tmpDir, 'agents');
    fs.mkdirSync(agentsDir, { recursive: true });
  ...
  test('no spurious incomplete flags when no manifest is present (protects claude/bundled installs)', () => {
    ...
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-bug-1058-no-manifest-'));
    const agentsDir = path.join(tmpDir, 'agents');
    fs.mkdirSync(agentsDir, { recursive: true });
    ...
    // Explicitly do NOT write ecl-file-manifest.json
