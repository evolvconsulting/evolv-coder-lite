...
> Tracked by issue [#3597](https://github.com/evolvconsulting/evolv-coder-lite/issues/3597).

## Suites

| Suite | Filename pattern | What goes here |
|---|---|---|
| `unit` | `*.test.cjs` (no other marker) | Default fast lane. Pure logic, no network, no external processes beyond `ecl-tools`. Most tests live here. |
...
> Tracked by issue [#1074](https://github.com/evolvconsulting/evolv-coder-lite/issues/1074).
> Bytes (not lines) per [#717](https://github.com/evolvconsulting/evolv-coder-lite/issues/717);
> LF-normalized per [#683](https://github.com/evolvconsulting/evolv-coder-lite/issues/683).

Workflow files (`evolv-coder-lite/workflows/*.md`) and agent files (`agents/ecl-*.md`)
...
(issue [#2551](https://github.com/evolvconsulting/evolv-coder-lite/issues/2551)).
...
from the separate 45 KB-*char* extraction-evidence threshold on `ecl-planner`
...
3. **Or shrink it instead of baselining.** Prefer extraction when the growth is
   ...
   `evolv-coder-lite/references/`; for an agent, lift shared boilerplate into
   `evolv-coder-lite/references/` and `@`-reference it — then load it **LAZILY**. Do *not* convert them to eager `@-required_reading`
...
| `tests/agent-size-baseline.json` | The committed per-agent snapshot (one entry per `ecl-*` agent). |
...
`evolv-coder-lite/bin/lib/*.cjs` only — `scripts/`, `hooks/`, and `bin/` are
...
```bash
npm run ci:test-scope -- --files "commands/ecl/plan-phase.md"
...
**What:** The `ecl-tools resolve-execution <agent>` command emits a JSON object
...
**What:** `ecl-tools config-set` accepts all new key namespaces
...
The integration tests verify that eCL resolves and renders effort values
