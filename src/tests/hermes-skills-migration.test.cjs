...
/**
 * eCL Tools Tests - Hermes Agent Skills Migration
 *
 * Tests for installing eCL for Hermes Agent using the standard
...
describe('Hermes Agent: installRuntimeArtifacts', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-hermes-test-'));
  });
  ...
  test('removes stale ecl- skills before installing new ones', () => {
    ...
    // Pre-create a stale flat skill (skills/ecl-old-skill) — legacy Hermes layout
    const staleFlatSkillDir = path.join(configDir, 'skills', 'ecl-old-skill');
...
describe('Hermes Agent: SKILL.md format validation', () => {
  test('SKILL.md frontmatter parses with required Hermes fields', () => {
    ...
    const result = convertClaudeCommandToClaudeSkill(input, 'ecl-review', 'hermes');
    const fm = parseFrontmatter(result);

    assert.strictEqual(fm.name, 'ecl-review', 'name uses hyphen form');
    ...
    assert.strictEqual(fm.agent, 'ecl-code-reviewer', 'agent preserved');
