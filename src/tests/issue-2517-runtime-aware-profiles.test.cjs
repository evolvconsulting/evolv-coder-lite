...
describe('issue #2517: precedence chain', () => {
  ...
  test('model_profile_overrides[runtime][tier] beats built-in defaults', () => {
    ...
    // ecl-codebase-mapper quality -> sonnet -> gpt-5.3-codex
    assert.strictEqual(resolveModelInternal(tmpDir, 'ecl-codebase-mapper'), 'gpt-5.3-codex');
  });
...
describe('issue #2612: runtime "gemini" — Gemini tier resolution', () => {
  ...
  test('opus tier -> gemini-3-pro', () => {
    ...
    assert.strictEqual(resolveModelInternal(tmpDir, 'ecl-planner'), 'gemini-3-pro');
  });
