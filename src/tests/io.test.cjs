/**
 * Tests for src/io.cts (compiled to evolv-coder-lite/bin/lib/io.cjs).
 *
...
const io = require('../evolv-coder-lite/bin/lib/io.cjs');
...
describe('output()', () => {
  const ioPath = path.resolve(__dirname, '../evolv-coder-lite/bin/lib/io.cjs');
...
// ─── ECL_TEMP_DIR / reapStaleTempFiles ───────────────────────────────────────

describe('ECL_TEMP_DIR', () => {
  test('resolves to <tmpdir>/ecl', () => {
    assert.strictEqual(io.ECL_TEMP_DIR, path.join(os.tmpdir(), 'ecl'));
  });
});

describe('reapStaleTempFiles (via io)', () => {
  const TEST_PREFIX = 'ecl-io-test-';

  afterEach(() => {
    // clean up any test files we created
    try {
      const entries = fs.readdirSync(io.ECL_TEMP_DIR);
      for (const e of entries) {
        if (e.startsWith(TEST_PREFIX)) {
          const p = path.join(io.ECL_TEMP_DIR, e);
          try { fs.unlinkSync(p); } catch { /* ignore */ }
        }
      }
    } catch { /* ignore */ }
  ...
  test('removes stale files beyond maxAgeMs', () => {
    fs.mkdirSync(io.ECL_TEMP_DIR, { recursive: true });
    const stalePath = path.join(io.ECL_TEMP_DIR, TEST_PREFIX + 'stale.json');
  ...
  test('keeps fresh files within maxAgeMs', () => {
    fs.mkdirSync(io.ECL_TEMP_DIR, { recursive: true });
    const freshPath = path.join(io.ECL_TEMP_DIR, TEST_PREFIX + 'fresh.json');
  ...
  test('does not throw when ECL_TEMP_DIR does not exist yet', () => {
    // reap against a non-existent prefix — must not throw
    assert.doesNotThrow(() => {
      io.reapStaleTempFiles('ecl-io-nonexistent-prefix-xyz-', { maxAgeMs: 0 });
    });
  });
});
