 ...
 * eCL Workspace Tests
 *
 * Tests for /ecl-new-workspace, /ecl-list-workspaces, /ecl-remove-workspace
 * init functions and integration with ecl-tools routing.
 */
...
const { detectChildRepos } = require('../evolv-coder-lite/bin/lib/init.cjs');
...
describe('detectChildRepos', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('ecl-ws-test-');
  });
...
// ─── cmdInitNewWorkspace via ecl-tools ──────────────────────────────────────
...
// ─── cmdInitListWorkspaces via ecl-tools ────────────────────────────────────
...
describe('init list-workspaces', () => {
  ...
  test('finds workspaces with WORKSPACE.md', () => {
    const wsBase = path.join(tmpDir, 'ecl-workspaces');
...
// ─── cmdInitRemoveWorkspace via ecl-tools ───────────────────────────────────
...
describe('workspace worktree integration', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('ecl-ws-integ-');
...
describe('workspace command files', () => {
   ...
   * Each line of the form `@~/.claude/evolv-coder-lite/workflows/foo.md` becomes
  ...
  function executionContextIncludes(body) {
    ...
    for (const blk of blocks) {
      for (const line of blk.split('\n')) {
        ...
        // Normalize away the home-prefix and the `.claude/evolv-coder-lite/` root
  ...
  test('consolidated workspace.md command declares correct frontmatter contract (#2790)', () => {
    ...
    const { fm } = parseCommandFile(path.join(baseDir, 'commands/ecl/workspace.md'));
    assert.equal(fm.name, 'ecl:workspace', `workspace.md frontmatter name must be "ecl:workspace"; got "${fm.name}"`);
  ...
  test('workspace.md @-includes the new-workspace workflow', () => {
    const { body } = parseCommandFile(path.join(baseDir, 'commands/ecl/workspace.md'));
  ...
  test('new-workspace workflow exists', () => {
    const content = fs.readFileSync(path.join(baseDir, 'evolv-coder-lite/workflows/new-workspace.md'), 'utf8');
    assert.ok(
      content.includes('init new-workspace') || content.includes('init.new-workspace'),
      'expected init new-workspace (CJS) or ecl-sdk query init.new-workspace'
    );
  ...
  test('list-workspaces workflow exists', () => {
    const content = fs.readFileSync(path.join(baseDir, 'evolv-coder-lite/workflows/list-workspaces.md'), 'utf8');
    assert.ok(
      content.includes('init list-workspaces') || content.includes('init.list-workspaces'),
      'expected init list-workspaces or ecl-sdk query init.list-workspaces'
    );
  });

  test('remove-workspace workflow exists', () => {
    const content = fs.readFileSync(path.join(baseDir, 'evolv-coder-lite/workflows/remove-workspace.md'), 'utf8');
    assert.ok(
      content.includes('init remove-workspace') || content.includes('init.remove-workspace'),
      'expected init remove-workspace or ecl-sdk query init.remove-workspace'
    );
    assert.ok(content.includes('git worktree remove'));
  });
});

// ─── Routing in ecl-tools ───────────────────────────────────────────────────

describe('workspace routing in ecl-tools', () => {
  let tmpDir;
