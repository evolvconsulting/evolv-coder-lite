<purpose>
Analyze ROADMAP.md phases for dependency relationships before execution. Detect file overlap between phases, semantic API/data-flow dependencies, and suggest `Depends on` entries to prevent merge conflicts during parallel execution by `/ecl:manager`.
</purpose>
...
Read `.planning/ROADMAP.md`. If it does not exist, error: "No ROADMAP.md found — run `/ecl:new-project` first."
...
After applying: "ROADMAP.md updated. Run `/ecl:manager` to execute phases in the correct order."
