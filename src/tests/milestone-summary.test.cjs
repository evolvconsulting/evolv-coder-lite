...
/**
 * eCL Milestone Summary + Audit Tests
 *
...
const workflowPath = path.join(repoRoot, 'evolv-coder-lite', 'workflows', 'milestone-summary.md');

describe('milestone-summary command', () => {
  test('command file exists', () => {
    assert.ok(fs.existsSync(commandPath), 'commands/ecl/milestone-summary.md should exist');
  });

  test('command has correct frontmatter name', () => {
    const content = fs.readFileSync(commandPath, 'utf-8');
    assert.ok(content.includes('name: ecl:milestone-summary'), 'should have correct command name');
  });
...
describe('milestone-summary fixture-based artifact discovery', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-ms-test-'));
  });
...
describe('audit.cjs module (#2158)', () => {
  ...
  test('auditOpenArtifacts returns structured result with counts', () => {
    const { auditOpenArtifacts } = require('../evolv-coder-lite/bin/lib/audit.cjs');
  ...
  test('formatAuditReport returns string with header', () => {
    const { auditOpenArtifacts, formatAuditReport } = require('../evolv-coder-lite/bin/lib/audit.cjs');
...
describe('complete-milestone workflow has pre-close audit gate (#2158)', () => {
  const completeMilestoneContent = fs.readFileSync(
    path.join(__dirname, '..', 'evolv-coder-lite', 'workflows', 'complete-milestone.md'),
    'utf8',
  );
...
describe('state.md template has Deferred Items section (#2158)', () => {
  const stateTemplate = fs.readFileSync(
    path.join(__dirname, '..', 'evolv-coder-lite', 'templates', 'state.md'),
    'utf8',
  );
