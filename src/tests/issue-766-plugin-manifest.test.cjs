...
const identity = require(path.join(ROOT, 'evolv-coder-lite', 'bin', 'lib', 'package-identity.cjs'));
...
describe('A: .claude-plugin/plugin.json', () => {
  ...
  test('name equals identity.binName ("evolv-coder-lite")', (t) => {
  ...
  test('commands field is "./commands/ecl/" and that dir exists with at least one .md file', (t) => {
    if (!manifest) { t.skip('manifest could not be parsed'); return; }
    assert.equal(manifest.commands, './commands/ecl/', 'commands must be "./commands/ecl/"');
...
describe('B: hooks/hooks.json', () => {
  ...
  test('ecl-context-monitor.js entry has timeout === 10', (t) => {
    if (!hooksConfig) { t.skip('hooks.json could not be parsed'); return; }
    let found = false;
    for (const eventEntries of Object.values(hooksConfig.hooks)) {
      for (const entry of eventEntries) {
        for (const hook of entry.hooks) {
          if (hook.command && hook.command.includes('ecl-context-monitor.js')) {
            found = true;
            assert.equal(hook.timeout, 10, 'ecl-context-monitor.js must have timeout === 10');
          }
        }
      }
    }
    assert.ok(found, 'ecl-context-monitor.js entry was not found in hooks.json');
  });
});
...
describe('C: plugin.json schema validation', () => {
  ...
  test(
    ...
    () => {
      const pluginRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-plugin-validate-'));
      try {
...
describe('D: always-on hook contract drift guard', () => {
  ...
  test('SessionStart: one no-matcher group with ecl-ensure-canonical-path.js then ecl-check-update.js', () => {
    // #997: ecl-ensure-canonical-path.js is wired alongside ecl-check-update.js
    // in the single SessionStart no-matcher group. It must run FIRST so the
    // canonical ~/.claude/evolv-coder-lite path (and its @-include targets) exist before
    ...
    assert.equal(
      noMatcherHooks[0].script, 'ecl-ensure-canonical-path.js',
      'ecl-ensure-canonical-path.js must be the FIRST SessionStart hook (#997)'
    );
    assert.equal(
      noMatcherHooks[0].timeout, 5,
      'ecl-ensure-canonical-path.js must have a small timeout (5s) — symlink setup is fast'
    );
    assert.equal(
      noMatcherHooks[1].script, 'ecl-check-update.js',
      'ecl-check-update.js must remain a SessionStart hook'
    );
    assert.equal(noMatcherHooks[1].timeout, undefined, 'ecl-check-update.js must NOT have a timeout field');
  });

  test('PreToolUse Write|Edit group: ecl-prompt-guard.js (timeout 5) + ecl-read-guard.js (timeout 5)', () => {
    const map = buildHookMap();
    ...
    assert.equal(hooks[0].script, 'ecl-prompt-guard.js', 'first hook must be ecl-prompt-guard.js');
    assert.equal(hooks[0].timeout, 5, 'ecl-prompt-guard.js must have timeout 5');
    assert.equal(hooks[1].script, 'ecl-read-guard.js', 'second hook must be ecl-read-guard.js');
    assert.equal(hooks[1].timeout, 5, 'ecl-read-guard.js must have timeout 5');
  });

  test('PreToolUse Write|Edit|MultiEdit group: ecl-worktree-path-guard.js (timeout 5)', () => {
    const map = buildHookMap();
    const groups = map['PreToolUse'];
    assert.ok(groups, 'PreToolUse must be present in hooks.json');
    ...
    assert.equal(hooks[0].script, 'ecl-worktree-path-guard.js', 'hook must be ecl-worktree-path-guard.js');
    assert.equal(hooks[0].timeout, 5, 'ecl-worktree-path-guard.js must have timeout 5');
  });

  test('PostToolUse Bash|Edit|Write|MultiEdit|Agent|Task group: ecl-context-monitor.js (timeout 10)', () => {
    const map = buildHookMap();
    ...
    assert.equal(hooks[0].script, 'ecl-context-monitor.js', 'hook must be ecl-context-monitor.js');
    assert.equal(hooks[0].timeout, 10, 'ecl-context-monitor.js must have timeout 10');
  });

  test('PostToolUse Read group: ecl-read-injection-scanner.js (timeout 5)', () => {
    const map = buildHookMap();
    const groups = map['PostToolUse'];
    assert.ok(groups, 'PostToolUse must be present in hooks.json');
    ...
    assert.equal(hooks[0].script, 'ecl-read-injection-scanner.js', 'hook must be ecl-read-injection-scanner.js');
    assert.equal(hooks[0].timeout, 5, 'ecl-read-injection-scanner.js must have timeout 5');
  });
});
...
//
// ecl-ensure-canonical-path.js must be shipped + wired so plugin installs get a
// real ~/.claude/evolv-coder-lite directory (with the immutable bundled subdirs
// symlinked) — otherwise every `@~/.claude/evolv-coder-lite/...` include in agents /
// commands / templates resolves to nothing and agents fail (#997).
describe('F: #997 ecl-ensure-canonical-path.js is shipped and wired', () => {
  const HOOK_BASENAME = 'ecl-ensure-canonical-path.js';
...
describe('G: #997 ensureCanonicalPath() behavioural regression', () => {
  const { ensureCanonicalPath, dirLinkType, MANAGED_SUBDIRS } =
    require(path.join(ROOT, 'hooks', 'ecl-ensure-canonical-path.js'));
  ...
  beforeEach(() => {
    tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-997-'));
  });
  afterEach(() => {
  ...
  // Build a fake plugin layout: <tmp>/plugin/evolv-coder-lite/<subdir>/marker.md and a
  // separate fake home <tmp>/home with an (initially absent) .claude dir.
  function makePluginLayout(subdirs = MANAGED_SUBDIRS) {
    const pluginRoot = path.join(tmp, 'plugin');
    const bundled = path.join(pluginRoot, 'evolv-coder-lite');
  ...
  test('plugin layout: creates ~/.claude/evolv-coder-lite with all subdirs symlinked to the bundle', () => {
    ...
    const canonical = path.join(homeDir, '.claude', 'evolv-coder-lite');
  ...
  test('no plugin context: CLAUDE_PLUGIN_ROOT unset → no-op (classic/npm install path)', () => {
    ...
    assert.ok(!fs.existsSync(path.join(homeDir, '.claude', 'evolv-coder-lite')), 'must not create canonical dir');
  });

  test('boundary: bundled evolv-coder-lite dir missing under plugin root → no-op', () => {
    const pluginRoot = path.join(tmp, 'plugin-empty');
    fs.mkdirSync(pluginRoot, { recursive: true }); // no evolv-coder-lite/ inside
    const homeDir = path.join(tmp, 'home');
    fs.mkdirSync(path.join(homeDir, '.claude'), { recursive: true });
    const result = ensureCanonicalPath({ pluginRoot, homeDir, platform: 'linux', env: {} });
    assert.equal(result.status, 'noop');
  ...
  test('self-heal: a stale symlink (pointing at a removed prior plugin version) is pruned and recreated', () => {
    ...
    const stalePrior = path.join(tmp, 'plugin-OLD', 'evolv-coder-lite', 'references');
    ...
    assert.equal(
      fs.realpathSync(linkPath),
      fs.realpathSync(path.join(pluginRoot, 'evolv-coder-lite', 'references')),
  ...
  test('self-heal: a managed symlink pointing at the wrong (but existing) target is repointed', () => {
    ...
    const otherDir = path.join(tmp, 'plugin-OTHER', 'evolv-coder-lite', 'workflows');
    ...
    assert.equal(
      ...
      fs.realpathSync(path.join(pluginRoot, 'evolv-coder-lite', 'workflows')),
      'workflows must be repointed to the current bundle'
    );
  });

  test('security: bundled evolv-coder-lite that symlinks OUTSIDE the plugin root is rejected', () => {
    const pluginRoot = path.join(tmp, 'plugin-evil');
    fs.mkdirSync(pluginRoot, { recursive: true });
    // Attacker places a symlink at <pluginRoot>/evolv-coder-lite pointing outside root.
    const outside = path.join(tmp, 'OUTSIDE');
    fs.mkdirSync(path.join(outside, 'references'), { recursive: true });
    fs.symlinkSync(outside, path.join(pluginRoot, 'evolv-coder-lite'), 'dir');
    const homeDir = path.join(tmp, 'home');
    fs.mkdirSync(path.join(homeDir, '.claude'), { recursive: true });
    const result = ensureCanonicalPath({ pluginRoot, homeDir, platform: 'linux', env: {} });
    ...
    assert.equal(result.reason, 'no-plugin-bundle');
    assert.ok(
      !fs.existsSync(path.join(homeDir, '.claude', 'evolv-coder-lite', 'references')),
  ...
  test('CLAUDE_CONFIG_DIR honoured: canonical path is created under the custom config dir', () => {
    ...
    assert.equal(result.status, 'ensured');
    assert.equal(result.canonicalDir, path.join(customCfg, 'evolv-coder-lite'));
    assert.ok(fs.lstatSync(path.join(customCfg, 'evolv-coder-lite', 'references')).isSymbolicLink());
  });

  test('canonical path is itself a symlink → no-op (never writes links through a user-pointed symlink)', () => {
    // A user pointed ~/.claude/evolv-coder-lite at some other directory via a symlink.
    // The hook must NOT create managed links through it into a dir it does not
    // own — it bails as a no-op.
    const { pluginRoot, homeDir } = makePluginLayout();
    const userTarget = path.join(tmp, 'user-ecl');
