...
// Mock the heavy dependencies that eCL constructor + runPhase pull in
...
vi.mock('./ecl-tools.js', () => ({
  ...
  resolveGsdToolsPath: vi.fn().mockReturnValue('/mock/ecl-tools.cjs'),
}));

import { eCL } from './index.js';
import { GSDTools } from './ecl-tools.js';
...
describe('eCL.run()', () => {
  ...
  beforeEach(() => {
    ...
    ecl = new eCL({ projectDir: '/tmp/test-project' });
    events = [];

    // Capture emitted events
    (ecl.eventStream.emitEvent as ReturnType<typeof vi.fn>).mockImplementation(
  ...
  it('skips phases where roadmap_complete === true', async () => {
    ...
    const result = await ecl.run('build it');
  ...
  it('sorts phases numerically, not lexicographically', async () => {
    ...
    vi.spyOn(ecl, 'runPhase').mockImplementation(async (phaseNumber: string) => {
