 ...
 *   5. isDirInMilestone works for new-style dirs ('eCL-02-01-setup') against
 *      ROADMAP entries 'Phase 2-01:'.
 *   6. Heading regex matches both '### Phase 2-01: Setup' and
 *      '### [eCL] Phase 2-01: Setup'.
 *
...
const { getMilestonePhaseFilter } = require('../evolv-coder-lite/bin/lib/roadmap-parser.cjs');
...
describe('backwards-compat: legacy Phase N roadmap entries', () => {
  ...
  // ── test 5: new-style dirs ('eCL-02-01-setup') match 'Phase 2-01:' ───────

  test('isDirInMilestone: new-style dir "eCL-02-01-setup" matches ROADMAP "Phase 2-01:"', () => {
    writeRoadmap(tmpDir, [
      '## Roadmap v1.0: Current',
      '',
      '### Phase 2-01: Setup',
    ...
    writeConfig(tmpDir, { project_code: 'eCL' });

    const filter = getMilestonePhaseFilter(tmpDir);
    assert.strictEqual(
      filter('eCL-02-01-setup'),
      true,
      '"eCL-02-01-setup" must match "Phase 2-01:"'
    );
    assert.strictEqual(
      filter('02-01-setup'),
      true,
  ...
  // ── test 6: heading regex matches both plain and [eCL]-prefixed headings ──

  test('phase heading regex matches "### Phase 2-01: Setup" and "### [eCL] Phase 2-01: Setup"', () => {
    const plain = '### Phase 2-01: Setup';
    const bracketed = '### [eCL] Phase 2-01: Setup';
    ...
    assert.strictEqual(
      ...
      true,
      '"### [eCL] Phase 2-01:" must also be matched by the heading regex'
    );
  });
});
