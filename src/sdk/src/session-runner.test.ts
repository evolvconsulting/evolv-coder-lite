...
describe('runPhaseStepSession', () => {
  ...
  describe('resolveModel runtime awareness (#2832)', () => {
    ...
    it('respects ECL_RUNTIME env precedence over config (no Claude id when env=codex)', async () => {
      const prev = process.env.ECL_RUNTIME;
      ...
      try {
        ...
        if (prev === undefined) delete process.env.ECL_RUNTIME;
        else process.env.ECL_RUNTIME = prev;
      }
    });
