...
**목표:** `/ecl-new-project`가 `.planning/config.json`을 생성할 때, 파일에 사용자가 선택한 6개 키만이 아닌 모든 유효한 기본값이 포함되도록 하여 개발자가 소스 코드를 읽지 않고도 모든 설정을 확인할 수 있게 합니다.
...
**기술 스택:** Node.js/CommonJS, 기존 ecl-tools CLI, 테스트에는 `node:test`.
...
- `git.phase_branch_template: "ecl/phase-{phase}-{slug}"`
- `git.milestone_branch_template: "ecl/{milestone}-{slug}"`
...
| 파일 | 액션 | 목적 |
|------|--------|---------|
| `evolv-coder-lite/bin/lib/config.cjs` | 수정 | `buildNewProjectConfig()` + `cmdConfigNewProject()` 추가 |
| `evolv-coder-lite/bin/ecl-tools.cjs` | 수정 | `config-new-project` case 등록 + usage 문자열 업데이트 |
| `evolv-coder-lite/workflows/new-project.md` | 수정 | Steps 2a + 5: 인라인 JSON 작성을 CLI 호출로 교체 |
...
- 수정: `evolv-coder-lite/bin/lib/config.cjs`
...
describe('config-new-project command', () => {
  ...
  test('creates full config with all expected top-level and nested keys', () => {
    ...
    assert.strictEqual(config.git.phase_branch_template, 'ecl/phase-{phase}-{slug}');
    assert.strictEqual(config.git.milestone_branch_template, 'ecl/{milestone}-{slug}');
...
```bash
cd /Users/diego/Dev/evolv-coder-lite
...
`evolv-coder-lite/bin/lib/config.cjs`에서 `validateKnownConfigKeyPath` 함수 뒤(약 35번째 줄)와 `ensureConfigFile` 앞에 다음을 추가하세요:
 ...
 * 다음 우선순위 순서로 병합합니다:
 *   1. 하드코딩된 기본값
 *   2. ~/.ecl/defaults.json의 사용자 수준 기본값(있는 경우)
...
function buildNewProjectConfig(cwd, userChoices) {
  ...
  const braveKeyFile = path.join(homedir, '.ecl', 'brave_api_key');
  const hasBraveSearch = !!(process.env.BRAVE_API_KEY || fs.existsSync(braveKeyFile));

  // 사용 가능한 경우 ~/.ecl/defaults.json에서 사용자 수준 기본값 로드
  const globalDefaultsPath = path.join(homedir, '.ecl', 'defaults.json');
  let userDefaults = {};
  try {
 ...
 *
 * 사용자가 선택한 설정을 JSON 문자열로 받습니다(/ecl-new-project 중 명시적으로
 * 구성한 키들). 나머지 키들은 하드코딩된 기본값과 선택적 ~/.ecl/defaults.json에서 채워집니다.
 *
...
```bash
cd /Users/diego/Dev/evolv-coder-lite
git add evolv-coder-lite/bin/lib/config.cjs tests/config.test.cjs
git commit -m "feat: add config-new-project command for full config materialization"
```

---

## 작업 2: ecl-tools.cjs에 `config-new-project` 등록

**파일.**

- 수정: `evolv-coder-lite/bin/ecl-tools.cjs`

- [ ] **Step 2.1: ecl-tools.cjs의 switch에 case 추가**
...
```bash
cd /Users/diego/Dev/evolv-coder-lite
node evolv-coder-lite/bin/ecl-tools.cjs config-new-project '{"mode":"interactive","granularity":"standard"}' --cwd /tmp/ecl-smoke-$(date +%s)
```

예상 결과: `{"created":true,"path":".planning/config.json"}` (또는 유사한 형태)가 출력됩니다.

정리: `rm -rf /tmp/ecl-smoke-*`
...
```bash
cd /Users/diego/Dev/evolv-coder-lite
git add evolv-coder-lite/bin/ecl-tools.cjs
git commit -m "feat: register config-new-project in ecl-tools CLI router"
```
...
- 수정: `evolv-coder-lite/workflows/new-project.md`
...
```bash
mkdir -p .planning
node "$HOME/.claude/evolv-coder-lite/bin/ecl-tools.cjs" config-new-project "$(cat <<'CHOICES'
{
  "mode": "yolo",
...
```bash
cd /Users/diego/Dev/evolv-coder-lite
grep -n "config-new-project\|config\.json\|CHOICES" evolv-coder-lite/workflows/new-project.md
```
...
```bash
cd /Users/diego/Dev/evolv-coder-lite
git add evolv-coder-lite/workflows/new-project.md
...
node /Users/diego/Dev/evolv-coder-lite/evolv-coder-lite/bin/ecl-tools.cjs init new-project --cwd "$TMP"

# Step 5 시뮬레이션: 전체 config 생성
node /Users/diego/Dev/evolv-coder-lite/evolv-coder-lite/bin/ecl-tools.cjs config-new-project '{
  "mode": "interactive",
  "granularity": "standard",
  "parallelization": true,
  "commit_docs": true,
...
node /Users/diego/Dev/evolv-coder-lite/evolv-coder-lite/bin/ecl-tools.cjs config-new-project "$CHOICES" --cwd "$TMP"
...
node /Users/diego/Dev/evolv-coder-lite/evolv-coder-lite/bin/ecl-tools.cjs config-get workflow.plan_check --cwd "$TMP"
# 예상: false

node /Users/diego/Dev/evolv-coder-lite/evolv-coder-lite/bin/ecl-tools.cjs config-get git.branching_strategy --cwd "$TMP"
# 예상: "none"
...
**문제:**
`/ecl-new-project`는 온보딩 중 사용자가 명시적으로 선택한 6개 키만으로
...
2. **암묵적 확장**: `/ecl-settings` 또는 `config-set`이 처음으로 config에
   기록할 때도 해당 키들이 추가되지 않습니다. config는 유효한 구성의
   일부만 반영합니다.

**해결책:**
`ecl-tools.cjs`에 `config-new-project` CLI 명령어를 추가합니다. 이 명령어는:
...
`git.branching_strategy: "none"`을 확인하고 eCL 소스를 읽지 않고도
