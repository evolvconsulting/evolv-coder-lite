 ...
 *
 * Verifies that the ecl-verifier agent filters gaps addressed in later
...
describe('verifier deferred-items filtering (#1624)', () => {
  ...
  describe('agents/ecl-verifier.md', () => {
    ...
    test('contains Step 9b for filtering deferred items', () => {
      ...
      assert.ok(
        ...
        'ecl-verifier.md should contain Step 9b or "Filter Deferred" section'
      );
    });
    ...
    test('Step 9b references roadmap analyze for cross-referencing', () => {
      ...
      assert.ok(
        ...
        'Step 9b should reference roadmap analyze (CJS or ecl-sdk query) for loading full milestone data'
      );
    });
  ...
  describe('evolv-coder-lite/workflows/verify-phase.md', () => {
    const workflowPath = path.join(ROOT, 'evolv-coder-lite', 'workflows', 'verify-phase.md');
    let workflowContent;
  ...
  describe('evolv-coder-lite/references/planner-gap-closure.md', () => {
    const closurePath = path.join(ROOT, 'evolv-coder-lite', 'references', 'planner-gap-closure.md');
    let closureContent;
