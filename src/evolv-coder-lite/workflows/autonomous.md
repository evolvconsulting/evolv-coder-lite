...
When `PLAN_STRATEGY=converge`, the planning step MUST invoke the plan-review convergence workflow instead of `ecl-plan-phase`. `--cross-ai` is an alias for `--converge`. Forward `CONVERGENCE_ARGS` exactly as parsed so reviewer flags and `--max-cycles N` retain the same meaning as they have on `/ecl:plan-review-convergence`.
...
INIT=$(ecl_run query init.milestone-op)
...
if [ "$PLAN_STRATEGY" = "converge" ]; then
  CONVERGENCE_ENABLED=$(ecl_run query config-get workflow.plan_review_convergence 2>/dev/null || echo "false")
  if [ "$CONVERGENCE_ENABLED" != "true" ]; then
    printf '%s\n' \
      'ecl-autonomous --converge is disabled (workflow.plan_review_convergence=false).' \
      '' \
...
**If `roadmap_exists` is false:** Error — "No ROADMAP.md found. Run `/ecl:new-milestone` first."
**If `state_exists` is false:** Error — "No STATE.md found. Run `/ecl:new-milestone` first."
...
```bash
DETAIL=$(ecl_run query roadmap.get-phase ${PHASE_NUM})
```
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 eCL ► AUTONOMOUS ▸ Phase {N}/{T}: {Name} [████░░░░] {P}%
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
...
```bash
PHASE_STATE=$(ecl_run query init.phase-op ${PHASE_NUM})
```
...
```bash
SKIP_DISCUSS=$(ecl_run query config-get workflow.skip_discuss 2>/dev/null || echo "false")
```
...
```bash
ecl_run query commit "docs(${PADDED_PHASE}): auto-generated context (discuss skipped)" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```
Skill(skill="ecl-discuss-phase", args="${PHASE_NUM}")
```
...
HOOKS_JSON=$(ecl_run loop render-hooks plan:pre --raw)
```
...
```bash
GATE=$(ecl_run check ui-plan-gate "${PHASE_NUM}" --raw)
```
...
```
Skill(skill="ecl-${ref.skill}", args="${PHASE_NUM}")
```

(Prepend `ecl-` to `ref.skill` — so `ui-phase` → `ecl-ui-phase`. Bare `${PHASE_NUM}` args — autonomous style, same pattern as the verify:post dispatch.) Entries where `kind == "gate"` are silently ignored — autonomous is always pipeline mode, there is no blocking gate here.
...
- **On Claude Code (`RUNTIME` is `claude`):** Run plan **inline** (do NOT background) so the plan-checker runs. The next phase's discuss does not overlap planning here — correctness over overlap.
  ...
  ```
  Skill(skill="ecl-plan-review-convergence", args="${PHASE_NUM} ${CONVERGENCE_ARGS}")
  ```

  - Otherwise (local planning):

  ```
  Skill(skill="ecl-plan-phase", args="${PHASE_NUM}")
  ```
...
- **On other runtimes:** Dispatch plan as a background agent to keep the main context lean. While plan runs, the workflow can immediately start discussing the next phase (see step 4).
  ...
  Agent(
    ...
    prompt="Run plan convergence for phase ${PHASE_NUM}: Skill(skill=\"ecl-plan-review-convergence\", args=\"${PHASE_NUM} ${CONVERGENCE_ARGS}\")"
  )
  ```
  ...
  Agent(
    ...
    run_in_background=true,
    prompt="Run plan-phase for phase ${PHASE_NUM}: Skill(skill=\"ecl-plan-phase\", args=\"${PHASE_NUM}\")"
  )
  ```
...
```
Skill(skill="ecl-execute-phase", args="${PHASE_NUM} --no-transition")
```
...
Agent(
  ...
  run_in_background=true,
  prompt="Run execute-phase for phase ${PHASE_NUM}: Skill(skill=\"ecl-execute-phase\", args=\"${PHASE_NUM} --no-transition\")"
)
```
...
```
Skill(skill="ecl-code-review", args="${PHASE_NUM} --fix --auto")
```
...
```
Skill(skill="ecl-plan-phase", args="${PHASE_NUM} --gaps")
```
...
```bash
UI_SPEC_FILE=$(ls "${PHASE_DIR}"/*-UI-SPEC.md 2>/dev/null | head -1)
HOOKS_JSON=$(ecl_run loop render-hooks verify:post --raw)
```
...
(i.e. prepend `ecl-` to `ref.skill` — so `ui-review` → `ecl-ui-review`.)
...
> Full instructions are in `evolv-coder-lite/references/autonomous-smart-discuss.md`. Read that file now and follow it exactly.

Smart discuss is an autonomous-optimized variant of `ecl-discuss-phase`. It proposes grey area answers in batch tables — the user accepts or overrides per area — and writes an identical CONTEXT.md to what discuss-phase produces.

**Inputs:** `PHASE_NUM` from execute_phase.

Read and execute: `$HOME/.claude/evolv-coder-lite/references/autonomous-smart-discuss.md`
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 eCL ► AUTONOMOUS ▸ --to ${TO_PHASE} REACHED
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

 Completed through phase ${TO_PHASE} as requested.
 Remaining phases were not executed.

 Resume with: /ecl:autonomous --from ${next_incomplete_phase}
```
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 eCL ► AUTONOMOUS ▸ PHASE ${ONLY_PHASE} COMPLETE ✓
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

 Phase ${ONLY_PHASE}: ${PHASE_NAME} — Done
 Mode: Single phase (--only)

 Lifecycle skipped — run /ecl:autonomous without --only
...
```
Skill(skill="ecl-audit-milestone")
```
...
On **"Stop"**: Go to handle_blocker with "User stopped — audit gaps remain. Run /ecl:audit-milestone to review, then /ecl:complete-milestone when ready."
...
On **"Stop"**: Go to handle_blocker with "User stopped — tech debt to address. Run /ecl:audit-milestone to review details."

**5b. Complete Milestone**

```
Skill(skill="ecl-complete-milestone", args="${milestone_version}")
```
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 ...
 Resume with: /ecl:autonomous ${ONLY_PHASE ? "--only " + ONLY_PHASE : "--from " + next_phase}${TO_PHASE ? " --to " + TO_PHASE : ""}
```
...
- [ ] Audit invoked via Skill(skill="ecl-audit-milestone")
...
- [ ] `--interactive` runs discuss inline via ecl-discuss-phase (asks questions, waits for user)
...
- [ ] `--converge` routes planning through `ecl-plan-review-convergence`
...
- [ ] Default autonomous planning remains `ecl-plan-phase` when convergence is not requested
</success_criteria>
