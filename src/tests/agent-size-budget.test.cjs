 ...
 *
 * Agent definitions in `agents/ecl-*.md` are loaded verbatim into the agent's
 ...
 *      extracting shared boilerplate to `evolv-coder-lite/references/`, not a +N bump.
 ...
 *   - https://github.com/evolvconsulting/evolv-coder-lite/issues/717  (bytes, not lines)
...
const isGsdAgent = (f) => f.startsWith('ecl-');
...
//   XL      56 KiB — high-water ecl-debugger 51,043 → ~6.3 KB headroom
//   LARGE   48 KiB — high-water ecl-executor 42,342 → ~6.8 KB headroom
//   DEFAULT 24 KiB — high-water ecl-ui-researcher 19,095 → ~5.5 KB headroom
const XL_CAP = 57344;       // 56 KiB
const LARGE_CAP = 51200;    // 50 KiB (eCL: +2 KiB over upstream 48 KiB — rebrand byte-expansion pushes ecl-verifier.md to 49,227)
...
describe('SIZE: agent tier hard caps (issue #1074)', () => {
  ...
  // it — crossing it means extract shared boilerplate to evolv-coder-lite/references/.
  for (const agent of ALL_AGENTS) {
    ...
    test(`${agent} (${tier}) stays within the ${tier} hard cap (${cap} bytes)`, () => {
      ...
      assert.ok(
        ...
        `to evolv-coder-lite/references/ and load it lazily.`
      );
    });
  }
});
  ...
  test('every agent matches its committed baseline', () => {
    ...
    assertFileBaseline({
      ...
      updateHint:
        ...
        'then justify any growth in your PR (or extract shared boilerplate to evolv-coder-lite/references/).',
    });
  });
});
