...
| --profile &lt;name&gt; | Switch model profile without interactive prompt | ecl-tools query config-set-model-profile |
...
- If it starts with `--profile`: extract the profile name (remainder after `--profile`), then:
  1. Verify `ecl-tools` is on PATH via `command -v ecl-tools`; if absent, emit the install hint `Install eCL via 'npm i -g @evolvconsulting/evolv-coder-lite'` and stop.
  2. Run: `ecl-tools query config-set-model-profile <profile-name> --raw` and display the output verbatim.
