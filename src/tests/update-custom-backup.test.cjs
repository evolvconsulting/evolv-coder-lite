...
describe('detect-custom-files — update workflow backup detection (#1997)', () => {
  ...
  test('detects custom files added inside agents/', () => {
    ...
    // Add a user's custom agent (not prefixed with ecl-)
  ...
  // update. skills/ is now a eCL-managed directory and must be scanned so that
  // user-added skill directories are backed up before the wipe (#2942).
  // eCL-owned skills (tracked in manifest) must NOT be flagged as custom.
