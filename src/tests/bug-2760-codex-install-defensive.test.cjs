 ...
 *     `[[hooks.SessionStart]]` (namespaced AoT) entries in their config, eCL
 *     used to append a `[[hooks]]` (top-level AoT) block that confuses
 *     round-trip writers and produces a config Codex refuses to load.
 *     Fix: detect the user's preferred shape and emit eCL's hook in the same
 ...
 *     invalid in current Codex schema and break Codex even though eCL now
 *     emits the correct `[agents.<name>]` struct form. Fix: install-time
 *     stripping always purges these forms regardless of eCL marker presence
...
// Scope ECL_TEST_MODE to module load only — restore prior value (or unset) so
// downstream tests in the same node process never see test-only behaviour
// leak through (#2760 CR4 finding 5).
const previousGsdTestMode = process.env.ECL_TEST_MODE;
...
if (previousGsdTestMode === undefined) {
  ...
  process.env.ECL_TEST_MODE = previousGsdTestMode;
}
...
describe('#2760 defect 3 — Hooks AoT preservation across install/uninstall/reinstall', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-2760-d3-'));
  ...
  test('fresh install emits the two-level nested AoT schema (#2773)', () => {
    ...
    const managed = sessionStartCommands.filter((cmd) => /ecl-check-update/.test(cmd));
    assert.equal(managed.length, 1, 'hooks.json must contain exactly one managed ecl-check-update command');
    assert.ok(
      !parsed.hooks || !Array.isArray(parsed.hooks.SessionStart),
      'config.toml should not carry managed SessionStart hooks for eCL'
    );
  });

  test('preserves user [[hooks.SessionStart]] entries and registers managed eCL handler in hooks.json', () => {
    // Users may have their own [[hooks.SessionStart]] entries using the new schema.
    // eCL must append its own two-level block without disturbing theirs.
    const userConfig = [
      '[[hooks.SessionStart]]',
      '',
    ...
    assert.ok(
      hooksJsonCommands.some((cmd) => typeof cmd === 'string' && /ecl-check-update/.test(cmd)),
      'eCL handler must appear in hooks.json SessionStart entries: ' + JSON.stringify(hooksJsonCommands)
    );
  ...
  test('reinstall replaces flat [[hooks]] + event form with nested schema', () => {
    // Upgrade path: user has a config written by eCL 1.38.x (flat [[hooks]] form).
    const legacyConfig = [
      '[features]',
      'codex_hooks = true',
      '',
      ...
      '[[hooks]]',
      'event = "SessionStart"',
      'command = "node /old/path/to/ecl-check-update.js"',
      '',
    ].join('\n');
    ...
    // Only one eCL hook entry must exist (no duplication) in hooks.json.
    const hooksJsonCommands = readHooksSessionStartCommands(codexHome);
    const gsdHandlers = hooksJsonCommands.filter((cmd) => /ecl-check-update/.test(cmd));
...
describe('#2760 fix 2 — Strip purges invalid legacy [agents] / [[agents]] regardless of marker', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-2760-f2-'));
    codexHome = path.join(tmpDir, 'codex-home');
  });
  ...
  test('strips bare [agents] single-bracket block (no eCL marker, arbitrary user keys)', () => {
    writeCodexConfig(codexHome, [
      '[agents]',
    ...
    // must itself be a table (the ecl-* struct form).
    assert.ok(
    ...
    const gsdAgents = Object.keys(parsed.agents).filter((k) => k.startsWith('ecl-'));
    assert.ok(
      gsdAgents.length > 0 && gsdAgents.every((k) => typeof parsed.agents[k] === 'object'),
      'agents.ecl-* struct form must be present: ' + JSON.stringify(Object.keys(parsed.agents))
    );
  ...
  test('strips [[agents]] sequence-form block without eCL marker (third-party / marker-edited-out)', () => {
    writeCodexConfig(codexHome, [
      '[[agents]]',
      'name = "user-helper"',
    ...
    // a table-of-tables with ecl-* struct keys.
    assert.ok(
      parsed.agents && typeof parsed.agents === 'object' && !Array.isArray(parsed.agents),
...
describe('#2760 fix 3 — Post-write Codex schema validation', { concurrency: false }, () => {
  test('passes a clean config produced by eCL install', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-2760-f3a-'));
    try {
      const codexHome = path.join(tmpDir, 'codex-home');
      runCodexInstall(codexHome);
      const content = readCodexConfig(codexHome);
      const result = validateCodexConfigSchema(content);
      assert.equal(result.ok, true, 'eCL-emitted config passes schema validation');
    } finally {
      fs.rmSync(tmpDir, { recursive: true, force: true });
    }
  });
  ...
  test('aborts install and restores pre-install backup when post-write validation fails', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-2760-f3b-'));
...
describe('#2760 fix 4 — Write-failure rollback (atomic write + snapshot restore)', { concurrency: false }, () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-2760-f4-'));
    codexHome = path.join(tmpDir, 'codex-home');
  ...
  test('pre-install config bytes survive when fs.renameSync throws over configPath', () => {
    ...
    // user's [model] section, not a half-written eCL block.
    ...
    assert.equal(parsed.agents, undefined,
      'no eCL agents block may have leaked into the surviving file');
...
describe('#2760 CR4 finding 2 — Legacy flat [[hooks]] block migrates to namespaced AoT on reinstall', { concurrency: false }, () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-2760-cr4-f2-'));
    codexHome = path.join(tmpDir, 'codex-home');
  });
  ...
  test('pre-install legacy flat [[hooks]] ecl-check-update + user namespaced [[hooks.SessionStart]] → post-install converges on namespaced AoT', () => {
    // Reproduce the upgrade scenario:
    //   - User has [[hooks.SessionStart]] entry of their own (signal that eCL
    //     should emit in the namespaced shape).
    //   - A previous eCL install left the legacy flat [[hooks]] managed block
    //     for ecl-check-update. The pre-CR4 strip step would short-circuit
    ...
    const userPlusLegacy = [
      ...
      '[[hooks]]',
      'event = "SessionStart"',
      'command = "node /old/path/hooks/ecl-check-update.js"',
      '',
    ].join('\n');
    ...
    // and the eCL entry is re-emitted in the namespaced AoT shape so the two
    ...
    assert.ok(
      hooksJsonCommands.some((cmd) => typeof cmd === 'string' && /ecl-check-update/.test(cmd)),
      'eCL entry must appear in hooks.json SessionStart entries: '
    ...
    // No duplicate ecl-check-update entries — exactly one managed entry.
    const gsdEntries = hooksJsonCommands.filter((cmd) => typeof cmd === 'string' && /ecl-check-update/.test(cmd));
    assert.equal(gsdEntries.length, 1,
      'exactly one ecl-check-update entry after migration, got: ' + gsdEntries.length);
  });
});
...
describe('#2760 CR4 finding 3 / #3245 — parseTomlToObject handles edge-case value types (floats accepted; dates/trailing-garbage rejected)', () => {
  ...
  // requires f64 for tool_timeout_sec/startup_timeout_sec, so eCL's parser
...
describe('#2760 CR4 finding 1 — atomicWriteFileSync failure aborts install (post-write fatal)', { concurrency: false }, () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-2760-cr4-f1-'));
    codexHome = path.join(tmpDir, 'codex-home');
    originalRenameSync = fs.renameSync;
    ...
    fs.renameSync = (src, dst) => {
      if (dst === configPath) {
        ...
        try {
          ...
          isHookWrite = /eCL codex_hooks ownership/.test(data);
        } catch (_) { /* ignore */ }
        if (isHookWrite) {
...
describe('#2760 CR5 finding 1 — pre-write failures abort install (outer catch fatal)', { concurrency: false }, () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-2760-cr5-f1-'));
    codexHome = path.join(tmpDir, 'codex-home');
    originalConsoleLog = console.log;
    consoleOutput = [];
    console.log = (...args) => { consoleOutput.push(args.join(' ')); };
...
describe('#2760 CR5 finding 3 — migration emits namespaced AoT (no flat/namespaced mixing)', { concurrency: false }, () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-2760-cr5-f3-'));
    codexHome = path.join(tmpDir, 'codex-home');
  });
  ...
  test('user has [[hooks.AfterTool]] AND legacy [hooks.SessionStart] → post-install both namespaced, no flat AoT', () => {
    ...
    //    eCL-managed entry converges on namespaced too.
    const userPlusLegacy = [
      '[[hooks.AfterTool]]',
      'command = "x"',
      '',
    ...
    // eCL's managed ecl-check-update entry also lives in the namespaced array.
    const hooksJsonCommands = readHooksSessionStartCommands(codexHome);
    assert.ok(
      hooksJsonCommands.some((cmd) => typeof cmd === 'string' && /ecl-check-update/.test(cmd)),
      'managed ecl-check-update entry must appear in hooks.json SessionStart entries: ' +
        JSON.stringify(hooksJsonCommands)
    );
