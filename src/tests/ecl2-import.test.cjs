...
const {
  ...
  slugify,
  zeroPad,
} = require('../evolv-coder-lite/bin/lib/ecl2-import.cjs');

// ─── Fixture Builders ──────────────────────────────────────────────────────

/** Build a minimal but complete eCL-2 .ecl/ directory in tmpDir. */
function makeGsd2Project(tmpDir, opts = {}) {
  const gsdDir = path.join(tmpDir, '.ecl');
...
/** Build a two-milestone eCL-2 project. */
...
describe('parseGsd2', () => {
  let tmpDir;
  beforeEach(() => { tmpDir = createTempDir('ecl2-parse-'); });
  ...
  test('handles missing .ecl/milestones/ gracefully', () => {
    const gsdDir = path.join(tmpDir, '.ecl');
...
describe('buildPlanningArtifacts', () => {
  let tmpDir;
  beforeEach(() => { tmpDir = createTempDir('ecl2-artifacts-'); });
  afterEach(() => { cleanup(tmpDir); });
  ...
  test('SUMMARY.md strips eCL-2 frontmatter and adds v1 frontmatter', () => {
    const gsdDir = makeGsd2Project(tmpDir);
    const data = parseGsd2(gsdDir);
    const artifacts = buildPlanningArtifacts(data);
    ...
    // eCL-2 frontmatter field should not appear
...
describe('ecl-tools from-ecl2 CLI', () => {
  let tmpDir;
  beforeEach(() => { tmpDir = createTempDir('ecl2-cli-'); });
  afterEach(() => { cleanup(tmpDir); });

  test('--dry-run returns preview without writing files', () => {
    makeGsd2Project(tmpDir);
    const result = runGsdTools(['from-ecl2', '--dry-run', '--raw'], tmpDir);
  ...
  test('writes .planning/ directory with correct structure', () => {
    makeGsd2Project(tmpDir);
    const result = runGsdTools(['from-ecl2', '--raw'], tmpDir);
    assert.ok(result.success, result.error);
    const parsed = JSON.parse(result.output);
  ...
  test('errors when no .ecl/ directory present', () => {
    const result = runGsdTools(['from-ecl2', '--raw'], tmpDir);
    const parsed = JSON.parse(result.output);
    assert.strictEqual(parsed.success, false);
    assert.ok(parsed.error.includes('No .ecl/'));
  });
  ...
  test('--force overwrites existing .planning/', () => {
    ...
    const result = runGsdTools(['from-ecl2', '--force', '--raw'], tmpDir);
    const parsed = JSON.parse(result.output);
    assert.strictEqual(parsed.success, true);
    assert.ok(fs.existsSync(path.join(tmpDir, '.planning', 'ROADMAP.md')));
  });
  ...
  test('--path resolves target directory', () => {
    ...
    const result = runGsdTools(['from-ecl2', '--path', projectDir, '--dry-run', '--raw'], tmpDir);
    assert.ok(result.success, result.error);
    const parsed = JSON.parse(result.output);
    assert.strictEqual(parsed.dryRun, true);
    assert.ok(parsed.preview.includes('PROJECT.md'));
  ...
  test('completion state: S01 done → [x] in ROADMAP.md', () => {
    makeGsd2Project(tmpDir);
    runGsdTools(['from-ecl2', '--raw'], tmpDir);
