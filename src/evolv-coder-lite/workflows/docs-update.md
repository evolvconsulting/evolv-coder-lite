...
Spawn 3 parallel ecl-doc-writer agents for Wave 1 docs: README, ARCHITECTURE, CONFIGURATION.
...
1. Spawn the `ecl-doc-verifier` agent (or invoke sequentially if Task tool is unavailable) with a `<verify_assignment>` block:
   ```xml
   <verify_assignment>
...
1. Spawn the `ecl-doc-verifier` agent with the same `<verify_assignment>` block as above.
