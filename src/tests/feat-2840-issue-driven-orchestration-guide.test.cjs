 ...
 *   - One guide explaining issue-driven orchestration using existing eCL
 *     commands.
...
function parseGuide() {
  ...
  // ecl-new-workspace, .planning/, etc. must survive).
  const stripped = content.replace(/\*{1,3}|~{2}/g, '');

  // Concept-mapping table: rows that pair a Symphony-style concept with a
  // eCL primitive. Test asserts on presence of each required pair, not on
  ...
  // guide. This prevents drift if /ecl-* command names are renamed.
  ...
  // eCL primitive that must appear in the same paragraph/row of the
  ...
  const conceptPairs = conceptMappingSection
    ? {
        ...
        workspaceCommand: /\/ecl-workspace\s+--new/.test(conceptMappingSection),
        executionCommand:
          /\/ecl-manager/.test(conceptMappingSection) ||
          /\/ecl-autonomous/.test(conceptMappingSection),
        verifyCommand: /\/ecl-verify-work/.test(conceptMappingSection),
        reviewCommand: /\/ecl-review/.test(conceptMappingSection),
        shipCommand: /\/ecl-ship/.test(conceptMappingSection),
      }
    : null;
...
describe('issue-driven-orchestration guide (#2840)', () => {
  ...
  test('every required eCL command is referenced at least once', () => {
    const ir = parseGuide();
  ...
  test('concept mapping section exists and pairs Symphony concepts with eCL primitives', () => {
    const ir = parseGuide();
    ...
    const expected = {
      ...
      workspaceCommand: '/ecl-workspace --new must appear in the concept mapping',
      executionCommand:
        '/ecl-manager or /ecl-autonomous must appear in the concept mapping',
      verifyCommand: '/ecl-verify-work must appear in the concept mapping',
      reviewCommand: '/ecl-review must appear in the concept mapping',
      shipCommand: '/ecl-ship must appear in the concept mapping',
    };
