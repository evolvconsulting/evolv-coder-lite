 ...
 * checkout sdk/shared path only, which breaks installed ecl-tools.cjs because
 * runtime installs copy evolv-coder-lite/ but not sdk/.
 */
...
const CONFIGURATION_CJS = path.join(REPO_ROOT, 'evolv-coder-lite', 'bin', 'lib', 'configuration.cjs');
const SHARED_DIR = path.join(REPO_ROOT, 'evolv-coder-lite', 'bin', 'shared');
...
describe('bug #3571: configuration generated manifests resolve in install layout', () => {
  ...
  test('co-located bin/shared manifests let configuration.cjs load without sdk/shared', () => {
    const gsdBinDir = path.join(tmpRoot, '.codex', 'evolv-coder-lite', 'bin');
  ...
  test('post-install: install() copies configuration manifests to co-located bin/shared', () => {
    ...
    const sharedDir = path.join(tmpRoot, '.codex', 'evolv-coder-lite', 'bin', 'shared');
