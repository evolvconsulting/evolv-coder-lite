...
describe('enh-770: Claude install registers SubagentStop / Stop / PreCompact context hooks', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('ecl-770-claude-ctx-');
  ...
  test('SubagentStop / Stop / PreCompact all use ecl-context-monitor', () => {
    for (const event of ['SubagentStop', 'Stop', 'PreCompact']) {
      const cmds = hooksForEvent(settings, event);
      assert.ok(
        cmds.some(c => c.includes('ecl-context-monitor')),
        `Event ${event} should use ecl-context-monitor; got commands: ${JSON.stringify(cmds)}`
      );
    }
  });
});
...
describe('enh-770: Claude install registers FileChanged hook for .planning/config.json', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('ecl-770-filechanged-');
    previousCwd = process.cwd();
    process.chdir(tmpDir);
    stubHooksIntoTarget(path.join(tmpDir, '.claude'));
  ...
  test('FileChanged hook uses ecl-config-reload', () => {
    const cmds = hooksForEvent(settings, 'FileChanged');
    assert.ok(
      cmds.some(c => c.includes('ecl-config-reload')),
      `FileChanged should use ecl-config-reload; got commands: ${JSON.stringify(cmds)}`
    );
  });
...
describe('enh-770: Claude install is idempotent for the new hook events', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('ecl-770-idem-');
    previousCwd = process.cwd();
    process.chdir(tmpDir);
    stubHooksIntoTarget(path.join(tmpDir, '.claude'));
  });
...
describe('enh-770: Uninstall removes new hook event entries', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('ecl-770-uninstall-');
    previousCwd = process.cwd();
    process.chdir(tmpDir);
    stubHooksIntoTarget(path.join(tmpDir, '.claude'));
...
// ─── Suite 5: ecl-config-reload.js hook script exists and has correct shape ───

describe('enh-770: ecl-config-reload.js hook script', () => {
  const reloadScript = path.join(__dirname, '..', 'hooks', 'ecl-config-reload.js');

  test('ecl-config-reload.js exists in hooks/', () => {
    assert.ok(fs.existsSync(reloadScript),
      `ecl-config-reload.js must exist at ${reloadScript}`);
  });

  test('ecl-config-reload.js contains the ecl-hook-version stamp', () => {
    ...
    assert.ok(
      ...
      'ecl-config-reload.js must contain the ecl-hook-version stamp for installer stamping'
    );
  });

  test('ecl-config-reload.js reads from stdin and emits JSON output', () => {
    ...
    assert.ok(
      ...
      'ecl-config-reload.js must read stdin and emit JSON output per hook protocol'
    );
  });

  test('ecl-config-reload.js targets the FileChanged hook event', () => {
    ...
    assert.ok(
      ...
      'ecl-config-reload.js must reference FileChanged in its hookSpecificOutput'
    );
  });
});
...
// ─── Suite 7: managed-hooks-registry includes ecl-config-reload.js ───────────

describe('enh-770: managed-hooks-registry includes ecl-config-reload.js', () => {
  test('MANAGED_HOOKS array includes ecl-config-reload.js', () => {
    const { MANAGED_HOOKS } = require('../hooks/managed-hooks-registry.cjs');
    assert.ok(
      MANAGED_HOOKS.includes('ecl-config-reload.js'),
      `MANAGED_HOOKS must include ecl-config-reload.js; got: ${JSON.stringify(MANAGED_HOOKS)}`
    );
  });
});
