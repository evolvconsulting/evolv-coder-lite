'use strict';
/**
 * E2E content tests for the eCL capability engine — ADR-857 phase 6
 *
...
const { resolveLoopHooks, renderLoopHooks } = require('../evolv-coder-lite/bin/lib/loop-resolver.cjs');

// ─── Fixture helpers ──────────────────────────────────────────────────────────

/** Create a bare temp dir with .planning/ layout (no config.json) */
function makeTempProject() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-loop-e2e-'));
...
function makeBareDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-loop-bare-'));
}

/** Spawn ecl-tools via raw spawnSync; returns { status, stdout, stderr } */
function spawnGsd(args, cwd) {
  return spawnSync(process.execPath, [ECL_TOOLS, ...args], {
    cwd: cwd || os.tmpdir(),
    encoding: 'utf8',
    timeout: 60000,
  });
