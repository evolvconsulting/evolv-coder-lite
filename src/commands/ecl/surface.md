...
<objective>
Manage the runtime skill surface without reinstall. Reads/writes `~/.claude/.ecl-surface.json`
(sibling to `~/.claude/.ecl-profile`) and re-stages the active skills directory in place.
Skill dirs live at `~/.claude/skills/ecl-*/`.
...
| `reset` | Delete `.ecl-surface.json`, return to install-time profile |
| *(none)* | Treat as `list` |
...
`evolv-coder-lite/bin/lib/surface.cjs`. The registry is loaded via:
```js
...
```
Base profile:   standard  (from .ecl-surface.json)
Install profile: standard  (from .ecl-profile)
```
...
1. Check if `.ecl-surface.json` exists.
2. Delete it.
...
`~/.claude/skills/ecl-*/` because the `claude global` layout has `destSubpath =
...
Surface state is stored at `${RUNTIME_CONFIG_DIR}/.ecl-surface.json`
(i.e. `~/.claude/.ecl-surface.json`).
...
- Missing `surface.cjs` → prompt: "Run `npm i -g evolv-coder-lite` to reinstall eCL."

<execution_context>
Surface state file: `~/.claude/.ecl-surface.json`
Install profile marker: `~/.claude/.ecl-profile`
Skill dirs: `~/.claude/skills/ecl-*/`
Engine module: `~/.claude/evolv-coder-lite/bin/lib/surface.cjs`
Cluster definitions: `~/.claude/evolv-coder-lite/bin/lib/clusters.cjs`
</execution_context>
