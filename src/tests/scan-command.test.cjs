...
describe('scan command', () => {
  test('scan is now a --fast flag on map-codebase.md (#2790)', () => {
    const p = path.join(__dirname, '..', 'commands', 'ecl', 'map-codebase.md');
    assert.ok(fs.existsSync(p), 'commands/ecl/map-codebase.md should exist');
  ...
  test('workflow file exists', () => {
    const p = path.join(__dirname, '..', 'evolv-coder-lite', 'workflows', 'scan.md');
    assert.ok(fs.existsSync(p), 'evolv-coder-lite/workflows/scan.md should exist');
  });
  ...
  test('workflow references ecl-codebase-mapper', () => {
    const p = path.join(__dirname, '..', 'evolv-coder-lite', 'workflows', 'scan.md');
    const content = fs.readFileSync(p, 'utf-8');
    assert.ok(content.includes('ecl-codebase-mapper'),
      'Workflow should reference the ecl-codebase-mapper agent');
  });
});
