# Temporal context as a first-class eCL signal

**Source:** [#2756](https://github.com/evolvconsulting/evolv-coder-lite/issues/2756)
...
eCL. Three flavors floated across the issue:
...
Framed initially as a `claude-inject-idle-time` plugin, with a request that eCL treat
the pattern as core.
...
- **Statusline / TUI re-entry is platform-level, not eCL-level.** A statusline showing
  idle time belongs in Claude Code itself or in a thin user plugin, not in eCL's phase
  machinery.
...
- `evolv-coder-lite/references/` — where any future plugin-interface doc would live
