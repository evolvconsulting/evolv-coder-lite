...
const commandPath = path.join(repoRoot, 'commands', 'ecl', 'forensics.md');
const workflowPath = path.join(repoRoot, 'evolv-coder-lite', 'workflows', 'forensics.md');

describe('forensics command', () => {
  test('command file exists', () => {
    assert.ok(fs.existsSync(commandPath), 'commands/ecl/forensics.md should exist');
  });

  test('command has correct frontmatter', () => {
    const content = fs.readFileSync(commandPath, 'utf-8');
    assert.ok(content.includes('name: ecl:forensics'), 'should have correct command name');
...
describe('forensics workflow', () => {
  ...
  test('workflow submits issues to evolvconsulting/evolv-coder-lite, not the current repo', () => {
    const content = fs.readFileSync(workflowPath, 'utf-8');
    ...
    assert.match(
      content,
      /gh issue create[\s\S]{0,250}--repo\s+evolvconsulting\/evolv-coder-lite/,
      'gh issue create must use --repo evolvconsulting/evolv-coder-lite to avoid submitting to the user\'s current project repo'
    );
  });

  test('workflow checks bug label in evolvconsulting/evolv-coder-lite, not the current repo', () => {
    const content = fs.readFileSync(workflowPath, 'utf-8');
    // Regex is more robust than a fixed-length slice to formatting changes
    assert.match(
      content,
      /gh label list[\s\S]{0,250}--repo\s+evolvconsulting\/evolv-coder-lite/,
      'gh label list must target evolvconsulting/evolv-coder-lite'
    );
  });
  ...
  test('workflow updates STATE.md', () => {
    ...
    assert.ok(
      ...
      'should update STATE.md via state record-session (CJS or ecl-sdk query)'
    );
  });
...
describe('forensics fixture-based tests', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-forensics-test-'));
  });
