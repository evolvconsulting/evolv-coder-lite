...
| c | `normalizeAgentBodyForRuntime` (colon→hyphen `/ecl:` refs) | 10240 | claude, qwen, hermes | cross-cutting (gated set) |
...
| j | Stale-agent cleanup: remove pre-existing `agents/ecl-*` before (re)install; minimal-mode Codex strips stale `[agents.ecl-*]` TOML (the full→minimal shrink) | ~10111, ~10126 | all (file cleanup); codex (toml) | cross-cutting pre-step |
...
AgentConverterContext = {
  agentName: string;      // 'ecl-planner' — for effort, disallowedTools, model overrides
