...
**Pré-requisitos:** `.planning/ROADMAP.md` deve existir. Caso contrário, execute `/ecl-new-project` primeiro.

---

## Escolha seu modo de discussão

eCL Core oferece dois modos. Escolha com base em quão bem compreendida é a base de código.
...
```bash
node ecl-tools.cjs config-set workflow.discuss_mode assumptions
/ecl-discuss-phase 2
```
...
```bash
node ecl-tools.cjs config-set workflow.discuss_mode discuss
```
...
Quando você executar `/ecl-plan-phase` em seguida, o planejador lê CONTEXT.md para saber quais decisões estão confirmadas. Ele não vai refazer perguntas já respondidas aqui. O pesquisador o lê primeiro para saber o que investigar.

**Se o CONTEXT.md estiver ausente quando você executar `/ecl-plan-phase`**, você terá a opção de continuar sem contexto (os planos usam apenas pesquisa e requisitos, sem suas preferências de design) ou executar `/ecl-discuss-phase` primeiro.
...
```bash
/ecl-plan-phase 1 --prd path/to/prd.md
```
