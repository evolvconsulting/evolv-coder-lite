 ...
 * the ECL_AUDIT_ARGS env var. Tests use real env manipulation and
...
const {
  shouldIncludeArgs,
  redactEvent,
} = require('../../evolv-coder-lite/bin/lib/observability/redaction.cjs');

describe('shouldIncludeArgs', () => {
  let originalEnv;

  beforeEach(() => {
    originalEnv = process.env.ECL_AUDIT_ARGS;
  ...
  afterEach(() => {
    if (originalEnv === undefined) {
      ...
      process.env.ECL_AUDIT_ARGS = originalEnv;
    }
  });

  test('returns false when ECL_AUDIT_ARGS is not set', () => {
    assert.strictEqual(shouldIncludeArgs(), false);
  });

  test('returns false when ECL_AUDIT_ARGS is empty string', () => {
    process.env.ECL_AUDIT_ARGS = '';
    assert.strictEqual(shouldIncludeArgs(), false);
  });

  test('returns false when ECL_AUDIT_ARGS is "0"', () => {
    process.env.ECL_AUDIT_ARGS = '0';
    assert.strictEqual(shouldIncludeArgs(), false);
  });

  test('returns true when ECL_AUDIT_ARGS is "1"', () => {
    process.env.ECL_AUDIT_ARGS = '1';
    assert.strictEqual(shouldIncludeArgs(), true);
  });

  test('returns false for any other non-1 value', () => {
    process.env.ECL_AUDIT_ARGS = 'yes';
    assert.strictEqual(shouldIncludeArgs(), false);

    process.env.ECL_AUDIT_ARGS = 'true';
    assert.strictEqual(shouldIncludeArgs(), false);
  });
});
...
describe('redactEvent', () => {
  ...
  test('strips args from event when ECL_AUDIT_ARGS is not set', () => {
  ...
  test('includes args when ECL_AUDIT_ARGS=1', () => {
    process.env.ECL_AUDIT_ARGS = '1';
    const event = Object.freeze({
      traceId: 'abc',
      command: 'plan',
    ...
    const redacted = redactEvent(event);
    assert.ok('args' in redacted, 'args must be present when ECL_AUDIT_ARGS=1');
