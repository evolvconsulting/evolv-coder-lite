...
/**
 * Deterministic verifier for the /ecl-reapply-patches Step 5 "Hunk Verification
 ...
 *     --patches-dir <path>        \  # ecl-local-patches/
 *     --config-dir <path>         \  # ~/.claude (or runtime equivalent)
 *     [--pristine-dir <path>]        # ecl-pristine/; if absent, falls back to
...
const ECL_HOOK_VERSION_LINE_RE = /^(?:\/\/|#)\s*ecl-hook-version:\s*\S+\s*$/i;
...
function normalizeUpstreamOwnedLine(line) {
  ...
  if (ECL_HOOK_VERSION_LINE_RE.test(trimmed)) {
    const prefix = trimmed.startsWith('#') ? '#' : '//';
    return `${prefix} ecl-hook-version: __GSD_VERSION_TOKEN__`;
  }
  return line;
}
...
const REASON = Object.freeze({
  ...
  // Bug #3657: the on-disk ecl-pristine/ file's SHA-256 does not match the
  // hash recorded in backup-meta.json.pristine_hashes.  This means the
  // pristine snapshot was refreshed to a newer eCL version after the backup
  ...
  // ecl-pristine/ file is absent from disk.  This happens on post-#604-rename
  ...
  if (pristineDir) {
    ...
    try {
      ...
      if (stat.isFile()) {
        ...
        // means the installer refreshed ecl-pristine/ to a newer eCL version
        ...
        if (recordedHash) {
          if (sha256(candidate) === recordedHash) {
            ...
            // Hash mismatch: the on-disk ecl-pristine/ was refreshed to a newer
            // eCL version after the backup was captured.  Using it as the diff
    ...
    // ecl-pristine/ file (the only candidate was discarded because it was from
