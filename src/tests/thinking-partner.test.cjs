...
describe('Thinking Partner Integration (#1726)', () => {
  ...
  describe('Reference document', () => {
    const refPath = path.join(ECL_ROOT, 'references', 'thinking-partner.md');
  ...
  describe('Discuss-phase integration', () => {
    function readDiscussFamily() {
      const candidates = [
        path.join(ECL_ROOT, 'workflows', 'discuss-phase.md'),
        path.join(ECL_ROOT, 'workflows', 'discuss-phase', 'modes', 'default.md'),
        path.join(ECL_ROOT, 'workflows', 'discuss-phase', 'modes', 'advisor.md'),
      ];
      return candidates
  ...
  describe('Plan-phase integration', () => {
    test('plan-phase.md contains thinking partner conditional block', () => {
      const content = fs.readFileSync(
        path.join(ECL_ROOT, 'workflows', 'plan-phase.md'),
        'utf-8'
      );
      assert.ok(
