/**
 * Bug #2767: Workflows pass paths positionally to `ecl-sdk query commit`.
 *
 ...
 *
 * Primary test: invoke the actual `ecl-sdk query commit[-to-subrepo]` binary
...
/**
 * Invoke `ecl-sdk query <subcommand> <...args>` against a project dir.
...
describe('bug #2767 (behavioral): ecl-sdk query commit --files', () => {
  let tmpDir;

  beforeEach(() => {
    tmpDir = createTempGitProject('ecl-2767-');
...
describe('bug #2767 (behavioral): commit-to-subrepo requires --files', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempGitProject('ecl-2767-sub-');
    fs.writeFileSync(
...
// rendered into chat, not invoked by ecl-tools.cjs), so a textual guard is
// the only available enforcement layer.

const SCAN_DIRS = ['agents', 'commands', 'evolv-coder-lite', 'docs', 'scripts'];
...
function extractInvocations(filePath) {
  ...
  for (let i = 0; i < lines.length; i++) {
    const m = lines[i].match(/(ecl-sdk\s+query\s+commit(?:-to-subrepo)?\b.*)$/);
    if (!m) continue;
    const tail = m[1].match(/^ecl-sdk\s+query\s+commit(?:-to-subrepo)?(.?)/);
...
describe('bug #2767 (supplementary doc-lint): no positional invocations in shipped .md', () => {
  ...
  test('every ecl-sdk query commit invocation either uses --files or has no path args', () => {
    ...
    if (broken.length > 0) {
      ...
      assert.fail(
        `${broken.length} \`ecl-sdk query commit\` invocation(s) pass paths positionally.\n` +
