...
**目标：** 创建一个完全隔离的 eCL 环境——独立的 git worktree、独立的 `.planning/` 根目录，以及可选的多仓库支持——适用于功能分支或多仓库工作场景。
...
工作区默认存放在 `~/ecl-workspaces/<name>/` 下。

```
~/ecl-workspaces/
└── feature-b/
    ├── WORKSPACE.md        ← 清单文件
    ├── .planning/          ← 完全独立的 eCL 状态
    │   ├── PROJECT.md
    │   ├── ROADMAP.md
    │   └── ...
...
eCL 会在 `~/ecl-workspaces/feature-b/` 中创建 `hr-ui` 和 `ZeymoAPI` 的 worktree，在每个仓库中检出 `workspace/feature-b` 分支，写入 `WORKSPACE.md`，并创建一个空的 `.planning/` 目录，准备好供 `/ecl-new-project` 使用。
...
工作区内的 `.planning/` 目录是从该目录运行所有后续 eCL 命令的根目录。它与源仓库中存在的任何 `.planning/` 完全独立。
...
eCL 会移除 git worktree 并清理工作区目录。此操作不会从远程仓库删除分支——仅删除本地 worktree 和工作区目录。
...
- 你需要跨**多个仓库**协同工作，且这些仓库需要在同一个 eCL 项目下进行协调（例如，一个 API 仓库和一个 UI 仓库需要一起发布）
- 你需要每个功能拥有**独立的 git worktree**，带有各自的分支、锁文件和构建产物——以确保一个环境中的构建和依赖安装不会影响另一个环境
- 你希望拥有**完全独立的 `.planning/` 根目录**，而非主仓库 `.planning/` 的子目录
- 你正在采用 Issue 驱动的工作流，将每个跟踪器 Issue 映射到一个工作区（参见[从跟踪器 Issue 驱动 eCL](drive-ecl-from-a-tracker-issue.md)）

选择[工作流](work-in-parallel-with-workstreams.md)的场景：

- 所有工作都在**单一仓库**中进行，共享相同的 git 历史
- 你希望在不同关注领域（API、UI、基础设施）上并发运行 `/ecl-plan-phase` 或 `/ecl-discuss-phase`，且各自的 `STATE.md` 文件之间互不干扰
...
- [从跟踪器 Issue 驱动 eCL](drive-ecl-from-a-tracker-issue.md)
- [命令](../COMMANDS.md)
- [文档索引](../README.md)
