 ...
 * CJS side: require('../evolv-coder-lite/bin/lib/plan-scan.generated.cjs')
 */
 ...
 */
function uniqueMissingPath(prefix = 'ecl-missing') {
...
const cjs = requireFromRoot('../evolv-coder-lite/bin/lib/plan-scan.generated.cjs');
...
describe('plan-scan-generator parity: scanPhasePlans (non-existent dir)', async () => {
  ...
  test('returns zero counts for non-existent directory', () => {
    const nonExistent = uniqueMissingPath('ecl-plan-scan-nonexistent');
...
describe('plan-scan-generator parity: scanPhasePlans (flat layout)', async () => {
  ...
  test('detects flat plan and summary files', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-plan-scan-test-'));
    try {
...
describe('plan-scan-generator parity: module.exports call style', async () => {
  test('default export is callable as function (CJS caller pattern)', () => {
    ...
    const result = cjs(uniqueMissingPath('ecl-plan-scan-cjs-default'));
