/**
 * eCL Tools Tests — ~/.ecl/defaults.json fallback (#1683)
 *
 * When .planning/ does not exist (pre-project context), loadConfig() should
 * consult ~/.ecl/defaults.json before returning hardcoded defaults.
...
function createBareTmpDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-test-'));
}

describe('loadConfig ~/.ecl/defaults.json fallback (#1683)', () => {
  test('pre-project, no defaults.json → hardcoded defaults', (t) => {
    const tmpDir = createBareTmpDir();
    process.env.ECL_HOME = tmpDir;
    t.after(() => { delete process.env.ECL_HOME; cleanup(tmpDir); });
  ...
  test('pre-project, defaults.json exists → merges with hardcoded defaults', (t) => {
    ...
    // Create ~/.ecl/defaults.json under fake ECL_HOME
