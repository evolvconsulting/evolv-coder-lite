/**
 * eCL Tools Tests - docs-update
 *
 * Integration tests for the docs-init ecl-tools subcommand.
 * Covers: JSON output shape, project type detection, existing doc scanning,
 * eCL marker detection, and doc tooling detection.
 *
...
describe('existing doc scanning', () => {
  ...
  test('detects eCL marker in existing docs', () => {
    fs.writeFileSync(
      path.join(tmpDir, 'README.md'),
      '<!-- generated-by: ecl-doc-writer -->\n# README\n',
      'utf-8'
    );
    ...
    assert.strictEqual(readmeEntry.has_gsd_marker, true, 'README.md should have eCL marker');

    const notesEntry = data.existing_docs.find(d => d.path === 'NOTES.md');
    assert.ok(notesEntry, 'NOTES.md should appear in existing_docs');
    assert.strictEqual(notesEntry.has_gsd_marker, false, 'NOTES.md should not have eCL marker');
  });
});
