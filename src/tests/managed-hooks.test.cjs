 ...
 *
 * ecl-check-update-worker.js uses a MANAGED_HOOKS array (now in the shared
 * managed-hooks-registry.cjs module) to detect stale hooks after a eCL update.
 * It must list every hook file that eCL ships so that all deployed hooks are
