#!/usr/bin/env node
/**
 * CLI entry point for ecl-sdk.
 *
 * Usage: ecl-sdk run "<prompt>" [--project-dir <dir>] [--ws-port <port>]
...
export interface ParsedCliArgs {
   ...
   * Extra flags are kept so handlers that share ecl-tools-style argv (e.g. `--pick`) still receive them.
   */
  queryArgv?: string[];
}

/**
 * Parse `ecl-sdk query …` without rejecting unknown flags (query argv is forwarded to the registry).
 */
function parseCliArgsQueryPermissive(argv: string[]): ParsedCliArgs {
  ...
  while (i < argv.length) {
    ...
    // flag onto queryArgv lets the registered handler (or the ecl-tools.cjs
    ...
    // `ecl-sdk query --help` → top-level USAGE while letting
    // `ecl-sdk query phase add --help` reach the handler.
    if (a === '-h' || a === '--help') {
      help = true;
      queryArgv.push(a);
      i += 1;
...
export const USAGE = `
Usage: ecl-sdk <command> [args] [options]
...
async function readStdin(): Promise<string> {
  ...
  if (stdin.isTTY) {
    throw new Error(
      ...
      '  ecl-sdk init @path/to/prd.md\n' +
      '  ecl-sdk init "build a todo app"\n' +
      '  cat prd.md | ecl-sdk init'
    );
  }
...
export async function main(argv: string[] = process.argv.slice(2)): Promise<void> {
  ...
  // Fall back to ECL_WORKSTREAM env var when --ws is not supplied (#2791).
  // ecl-tools.cjs resolves the active workstream via this env var; parity
  // means ecl-sdk command paths see the same .planning/ path as ecl-tools.
  if (args.ws === undefined && process.env.ECL_WORKSTREAM) {
    const envWs = process.env.ECL_WORKSTREAM;
  ...
  if (args.command !== 'run' && args.command !== 'init' && args.command !== 'auto') {
    console.error('Error: Expected "ecl-sdk run <prompt>", "ecl-sdk auto", "ecl-sdk init [input]", or "ecl-sdk query <command>"');
    console.error(USAGE);
    process.exitCode = 1;
    return;
  }
  ...
  if (args.command === 'run' && !args.prompt) {
    console.error('Error: "ecl-sdk run" requires a prompt');
    console.error(USAGE);
    process.exitCode = 1;
    return;
  }
  ...
  if (args.command === 'init') {
    ...
    // Build eCL instance for tools and event stream
    const ecl = new eCL({
      projectDir: args.projectDir,
      model: args.model,
  ...
  if (args.command === 'auto') {
    ...
    // defaults when missing) and gate before constructing eCL/InitRunner.
    try {
  ...
  try {
    const result = await ecl.run(args.prompt!);
