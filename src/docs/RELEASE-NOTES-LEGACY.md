...
> **Archived history — frozen.** These notes cover the project's first lifecycle under its earlier package names, **`evolv-coder-lite-cc`** and later **`@evolvconsulting/evolv-coder-lite`** (versions `1.0.0` → `1.42.x`, plus pre-release and canary builds up to `1.50.0-canary.1`). The project now ships as **`@evolvconsulting/evolv-coder-lite`**, whose version numbering restarts at `1.0.0`. Current release notes live in [`CHANGELOG.md`](../CHANGELOG.md).
>
> Because the `@evolvconsulting/evolv-coder-lite` line reuses low version numbers (`1.0.0`, `1.1.0`, `1.2.0`, …), the legacy `1.x` numbers below **will collide** with current ones. Always read a legacy version number as belonging to the retired `evolv-coder-lite-cc` / `evolv-coder-lite` lineage.
>
> **Install commands have been removed.** The retired packages are no longer maintained; historical `npx @evolvconsulting/evolv-coder-lite@…` instructions have been stripped so they cannot be copied by mistake. To install the current tool, see the project README.

## About this archive

This document is a condensed, read-only record of every release published before the project was renamed to `@evolvconsulting/evolv-coder-lite`. It exists so that the old version history is preserved without its `1.x` numbers colliding with the current package's release notes.
...
| 1.17.0 | 2026-02-08 | ecl-tools verification suite, frontmatter CRUD, and state progression |
| 1.16.0 | 2026-02-08 | Ten new ecl-tools CLI commands replacing manual AI orchestration |
...
| 1.13.0 | 2026-02-08 | History digest, phases list, and structured ecl-tools commands |
| 1.12.1 | 2026-02-08 | Consolidated workflow initialization for reduced token usage |
| 1.12.0 | 2026-02-07 | Thin orchestrator pattern and centralized ecl-tools utility |
...
| 1.5.17 | 2026-01-15 | Add /ecl:update command for in-place version management |
...
| 1.5.7 | 2025-01-15 | Add ecl-executor and ecl-verifier goal-backward subagents |
...
| 1.4.3 | 2026-01-13 | Added /ecl:debug command for systematic persistent debugging |
...
| 1.3.34 | 2026-01-11 | Added /ecl:add-todo and /ecl:check-todos for mid-session capture |
| 1.3.33 | 2026-01-11 | Fixed zero-padding for decimal phase numbers; removed .claude-plugin |
| 1.3.32 | 2026-01-10 | Added /ecl:resume-task for resuming interrupted subagent executions |
...
| 1.3.29 | 2026-01-08 | Added /ecl:verify-work, /ecl:plan-fix, and UAT issues template |
| 1.3.28 | 2026-01-07 | Added --config-dir argument and /ecl:remove-phase command |
...
| 1.3.1 | 2025-12-17 | Added /ecl:map-codebase documentation in help and README |
| 1.3.0 | 2025-12-17 | Added /ecl:map-codebase command for brownfield project analysis |
...
| 1.0.0 | 2025-12-14 | Initial release of eCL Core meta-prompting system |
...
### 1.42.3 — —
- Codex CLI 0.130.0 compatibility restored: installer writes `~/.codex/skills/ecl-<name>/SKILL.md` for every command; the previous build left zero routable entrypoints.
- `runtime-slash.cjs` introduced: emits `/ecl-<cmd>` for skills-based runtimes and `$ecl-<cmd>` for Codex; the deprecated colon form is no longer emitted at runtime.
...
### 1.42.1 — 2026-05-15
- **Fixed:** `/ecl-discuss-phase` and `/ecl-plan-phase` first-touch creation now apply `project_code` prefix consistently, eliminating two-headed phase directory naming. (#3287)
...
### 1.39.1 — 2026-05-01
- **Fixed:** `ecl-sdk query agent-skills` emits the raw `<agent_skills>` block instead of a JSON-quoted string, restoring subagent skill injection. (#2917)
...
- **Fixed:** `--sdk` install on Windows writes callable `ecl-sdk.cmd`, `.ps1`, and Bash shim triples so the binary resolves across all shells. (#2962)
...
- **Added:** `/ecl-map-codebase` (arch focus) produces a richer `ARCHITECTURE.md` with ASCII diagrams, component tables, and data-flow traces. (#2500)
...
- **Added:** `/ecl-ingest-docs` command bootstraps or merges a full `.planning/` setup from mixed ADRs, PRDs, SPECs, and DOCs in a single pass. (#2387)
- **Fixed:** `ecl-read-injection-scanner` hook included in the build allowlist and ships to users; was silently omitted since 1.37.0. (#2406)

### 1.37.1 — 2026-04-17
- **Fixed:** UI-phase researcher loads sketch findings skills, preventing repeated questions already answered during `/ecl-sketch`.

### 1.37.0 — 2026-04-17
- **Added:** `/ecl-spike` and `/ecl-sketch` first-class commands for feasibility spiking and UI design sketching with full eCL planning integration.
- **Added:** `/ecl-spec-phase` Socratic spec refinement with ambiguity scoring; produces a SPEC.md with falsifiable requirements before planning begins. (#2213)
- **Added:** `ecl-read-injection-scanner` PostToolUse hook scans for prompt injection in read file contents. (#2201)
...
### 1.36.0 — 2026-04-14
- **Added:** `@evolvconsulting/ecl-sdk` query layer Phases 1 & 2 — `ecl-sdk query` replaces raw `ecl-tools.cjs` calls with a supported, registry-backed CLI. (#2118, #2122)
- **Added:** `/ecl-graphify` knowledge graph integration for richer context connections between planning artifacts. (#2164)
...
- **Added:** `/ecl-from-ecl2` reverse migration from eCL-2 `.ecl/` format back to v1 `.planning/` format with `--dry-run` and `--force` flags.
- **Added:** Statusline surfaces eCL milestone, phase, and status when no active todo is present. (#628)
...
### 1.33.0 — 2026-04-05
- **Added:** Queryable codebase intelligence system — persistent `.planning/intel/` store with structured JSON; query via `ecl-tools intel` subcommands. (#1688)
...
- **Added:** `--to N` flag for autonomous mode, `--power` flag for discuss-phase, `/ecl-analyze-dependencies`, and research gate blocking planning on unresolved questions. (#1644, #1513, #1607, #1618)
...
### 1.31.0 — 2026-04-01
- **Added:** Claude Code 2.1.88+ skills migration — commands install as `skills/ecl-*/SKILL.md`; legacy directory auto-cleaned on install.
- **Added:** `/ecl:docs-update`, `/ecl:secure-phase`, `--chain` flag for discuss-phase, and `--only N` flag for autonomous mode.
...
### 1.30.0 — 2026-03-26
- **Added:** Headless TypeScript SDK (`ecl-sdk`) with `init` and `auto` CLI commands for autonomous project execution.
...
### 1.28.0 — 2026-03-22
- **Added:** Workstream namespacing for parallel milestone work, multi-project workspace commands, and `/ecl:forensics` post-mortem workflow.
...
### 1.27.0 — 2026-03-20
- **Added:** Advisor mode for discuss-phase, Cursor CLI runtime support, and seven new slash commands including `/ecl:fast`, `/ecl:review`, and `/ecl:pr-branch`.
...
### 1.26.0 — 2026-03-18
- **Added:** Developer profiling pipeline via `/ecl:profile-user` generating `USER-PROFILE.md` and `CLAUDE.md` profile sections.
- **Added:** `/ecl:ship` for PR creation from verified phase work, `/ecl:next` for automatic workflow advancement, and structured session handoff via `HANDOFF.json`.
...
- **Fixed:** Phantom `/ecl:transition` command replaced, PROJECT.md drift corrected, and hook lifecycle issues resolved.

### 1.25.0 — 2026-03-16
- **Added:** Antigravity AI runtime support, `/ecl:do` natural language router, and `/ecl:note` for zero-friction idea capture.
...
### 1.24.0 — 2026-03-15
- **Added:** `--research` flag for `/ecl:quick`, `inherit` model profile for OpenCode, and persistent debug knowledge base appended to `.planning/debug/knowledge-base.md`.
- **Fixed:** ROADMAP searches scoped to current milestone, OpenCode agent frontmatter conversion, Windows installer EPERM crash, and absolute paths in `ecl-tools.cjs`.

### 1.23.0 — 2026-03-15
- **Added:** `/ecl:ui-phase` and `/ecl:ui-review` for UI design contracts and visual audits; `/ecl:stats` project statistics dashboard; Copilot CLI runtime support.
...
### 1.22.4 — 2026-03-03
- **Added:** `--discuss` flag for `/ecl:quick` to gather context before quick tasks.
...
- **Added:** Code-aware discuss phase — `/ecl:discuss-phase` scouts relevant source files before asking questions.
...
### 1.21.0 — 2026-02-25
- **Added:** YAML frontmatter sync to STATE.md; `/ecl:add-tests` command; Codex runtime support.
- **Changed:** Installer suggests `/ecl:new-project`; requirements propagate `phase_req_ids` to agents.
...
### 1.20.5 — 2026-02-19
- **Fixed:** `/ecl:health --repair` creates a timestamped backup before regenerating STATE.md.
...
### 1.20.0 — 2026-02-15
- **Added:** `/ecl:health` command with `--repair` flag; `--full` flag for `/ecl:quick`; `--auto` flag wired through full phase chain.
...
### 1.19.2 — 2026-02-15
- **Added:** User-level defaults via `~/.ecl/defaults.json`; per-agent model overrides.
...
### 1.18.0 — 2026-02-08
- **Added:** `--auto` flag for `/ecl:new-project` runs research → requirements → roadmap automatically.
- **Fixed:** Windows SessionStart hook spawns detached process; research decision persists to config.json.

### 1.17.0 — 2026-02-08
- **Added:** ecl-tools verification suite (6 verify commands), frontmatter CRUD, template fill, and state progression commands.
- **Added:** Local patch preservation — installer backs up modified eCL files to `ecl-local-patches/`; `/ecl:reapply-patches` command added.
- **Changed:** Agents use ecl-tools for state updates and verification instead of manual markdown parsing.

### 1.16.0 — 2026-02-08
- **Added:** 10 new ecl-tools CLI commands covering phase add/insert/remove/complete, roadmap analyze, milestone complete, validate, progress, todo complete, and scaffold.
- **Changed:** Workflows delegate deterministic operations to ecl-tools, reducing token usage; execute-phase spawns `ecl-executor` subagents correctly.

### 1.15.0 — 2026-02-08
- **Changed:** Optimized workflow context loading eliminates redundant file reads, saving ~5,000–10,000 tokens per execution.

### 1.14.0 — 2026-02-08
- **Added:** Context-optimizing ecl-tools commands (`phase-plan-index`, `state-snapshot`, `summary-extract`) returning structured JSON.
- **Fixed:** Installer no longer deletes opencode.json on JSONC parse errors; handles comments, trailing commas, and BOM.

### 1.13.0 — 2026-02-08
- **Added:** `history-digest`, `phases list`, `roadmap get-phase`, `phase next-decimal`, `state get/patch`, and `template select` ecl-tools commands.
- **Changed:** Planner uses two-step context assembly; agents migrated from bash patterns to structured ecl-tools commands.
...
- **Added:** `ecl-tools.cjs` CLI utility with 11 functions replacing repetitive bash patterns across 50+ files.

### 1.11.2 — 2026-02-05
- **Fixed (CRITICAL):** Prevent API keys from being committed via `/ecl:map-codebase`.
...
- **Fixed:** CONTEXT.md from `/ecl:discuss-phase` now flows to all downstream agents.
...
### 1.9.12 — 2025-01-23
- **Removed:** `/ecl:whats-new` command — superseded by `/ecl:update`.
...
### 1.9.9 — 2026-01-23
- **Added:** `/ecl:join-discord` command for quick access to the eCL Discord invite link.

### 1.9.8 — 2025-01-22
- **Added:** `--uninstall` flag to cleanly remove eCL from global or local installations.
...
### 1.9.2 — 2025-01-21
- **Removed:** Codebase Intelligence System removed — deleted `/ecl:analyze-codebase`, `/ecl:query-intel`, SQLite graph database, sql.js (21 MB), and all intel hooks.

### 1.9.0 — 2025-01-20
- **Added:** Model Profiles via `/ecl:set-profile` for quality/balanced/budget configurations.
- **Added:** `/ecl:settings` command for toggling workflow behaviors interactively.

### 1.8.0 — 2026-01-19
- **Added:** `planning.commit_docs: false` config option keeps `.planning/` local-only, not committed to git.
- **Added:** `/ecl:new-project` asks about git tracking during initial setup.

### 1.7.1 — 2026-01-19
- **Fixed:** Quick task PLAN and SUMMARY files use numbered prefix (`001-PLAN.md`, `001-SUMMARY.md`) matching regular phase convention.

### 1.7.0 — 2026-01-19
- **Added:** `/ecl:quick` executes small ad-hoc tasks with eCL guarantees, skipping optional agents; tasks live in `.planning/quick/`.
...
- **Fixed:** Installation verifies copied files before showing success; orphaned `ecl-notify.sh` hook removed automatically.

### 1.6.3 — 2025-01-17
- **Added:** `--gaps-only` flag for `/ecl:execute-phase` executes only gap closure plans, eliminating redundant state discovery.
...
### 1.6.1 — 2025-01-17
- **Changed:** Installer performs clean install of eCL folders, removing orphaned files from previous versions.
- **Changed:** `/ecl:update` shows changelog and requests confirmation before updating.

### 1.6.0 — 2026-01-17
- **Breaking:** `/ecl:new-milestone` now mirrors `/ecl:new-project` in a single unified flow.
- **Breaking Removed:** `/ecl:discuss-milestone`, `/ecl:create-roadmap`, `/ecl:define-requirements`, `/ecl:research-project` consolidated into project/milestone flows.
- **Added:** `/ecl:verify-work` includes next-step routing after verification completes.
...
- **Fixed:** Next-step suggestions consistently recommend `/ecl:discuss-phase` before `/ecl:plan-phase`.
...
### 1.5.28 — 2026-01-16
- **Breaking Removed:** `/ecl:execute-plan` command; use `/ecl:execute-phase` instead.
...
### 1.5.22 — 2025-01-16
- **Added:** Statusline update indicator shows `⬆ /ecl:update` when a new version is available.
- **Fixed:** Planner updates ROADMAP.md placeholders after planning completes.

### 1.5.21 — 2026-01-16
- **Added:** eCL brand system for consistent UI; research synthesizer agent consolidates parallel research into SUMMARY.md.
- **Changed:** `/ecl:new-project` unified into a single command handling questions → research → requirements → roadmap.
...
### 1.5.19 — 2026-01-16
- **Changed:** `/ecl:discuss-phase` redesigned with intelligent gray area analysis and multi-select user control; CONTEXT.md template restructured.
- **Changed:** `/ecl:plan-phase` spawns `ecl-phase-researcher` before planning unless research exists or `--skip-research` used.

### 1.5.18 — 2026-01-16
- **Added:** Plan verification loop — `ecl-plan-checker` agent validates plans across six dimensions with up to three revision iterations.
- **Added:** Dedicated `ecl-planner` agent (1,319 lines) with full planning methodology and TDD integration.
- **Changed:** `/ecl:plan-phase` refactored to thin orchestrator pattern spawning planner and checker agents.

### 1.5.17 — 2026-01-15
- **Added:** `/ecl:update` command to check for updates, install, and display changelog of changes.

### 1.5.16 — 2026-01-15
- **Added:** `ecl-researcher` agent with four research modes (ecosystem, feasibility, implementation, comparison).
- **Added:** `ecl-debugger` agent with scientific debugging methodology and seven investigation techniques.
- **Added:** `ecl-codebase-mapper` agent for brownfield codebase analysis.
- **Changed:** `/ecl:research-phase` and `/ecl:research-project` refactored to thin orchestrators spawning `ecl-researcher`.

### 1.5.15 — 2025-01-15
- **Fixed:** `agents/` folder (ecl-executor, ecl-verifier, ecl-integration-checker, ecl-milestone-auditor) was missing from npm package; now included.
- **Changed:** `/ecl:plan-fix` consolidated into `/ecl:plan-phase --gaps`.

### 1.5.14 — 2025-01-15
- **Fixed:** Plan-phase always routes to `/ecl:execute-phase` after planning, including single-plan phases.

### 1.5.13 — 2026-01-15
- **Fixed:** `/ecl:new-milestone` presents research and requirements paths as equal options matching `/ecl:new-project` format.
...
### 1.5.9 — 2025-01-15
- **Added:** `/ecl:audit-milestone` with parallel verification agents for milestone completion checking.
...
### 1.5.7 — 2025-01-15
- **Added:** `ecl-executor` subagent for plan execution and `ecl-verifier` subagent for goal-backward phase verification.
...
- **Removed:** Dead `/ecl:status` command, unused `agent-history.md` template, and old `_archive/` directory.
...
### 1.5.0 — 2026-01-14
- **Added:** `/ecl:research-project` spawns parallel agents to investigate stack, features, architecture, and pitfalls pre-roadmap.
- **Added:** `/ecl:define-requirements` transforms research findings into scoped v1 requirements with phase traceability.
...
### 1.4.25 — 2026-01-14
- **Added:** `/ecl:whats-new` command shows changes since installed version; VERSION file and CHANGELOG.md included in package.
...
### 1.4.3 — 2026-01-13
- **Added:** `/ecl:debug` command for systematic debugging with persistent state.

### 1.4.2 — 2026-01-13
- **Fixed:** Installation verification step clarification.

### 1.4.1 — 2026-01-13
- **Added:** Parallel phase execution via `/ecl:execute-phase` with wave-based dependency graphs and `/ecl:status` monitoring.
...
### 1.3.34 — 2026-01-11
- **Added:** `/ecl:add-todo` and `/ecl:check-todos` commands for mid-session idea capture.
...
### 1.3.32 — 2026-01-10
- **Added:** `/ecl:resume-task` command for resuming interrupted subagent executions.
...
### 1.3.29 — 2026-01-08
- **Added:** `/ecl:verify-work` for conversational UAT validation, `/ecl:plan-fix` for fixing UAT issues, and UAT issues template.

### 1.3.28 — 2026-01-07
- **Added:** `--config-dir` CLI argument for multi-account setups and `/ecl:remove-phase` command.
...
### 1.3.1 — 2025-12-17
- **Added:** `/ecl:map-codebase` documentation added to help and README.

### 1.3.0 — 2025-12-17
- **Added:** `/ecl:map-codebase` command for brownfield project analysis with parallel Explore agent orchestration.
...
- **Changed:** Research workflow implemented as Claude Code context injection. (Renamed from `1.2.0` to avoid colliding with `@evolvconsulting/evolv-coder-lite@1.2.0`.)
...
### 1.1.0 — 2025-12-15
- **Added:** Pre-roadmap research workflow with `/ecl:research-phase`, `/ecl:research-project`, and `/ecl:create-roadmap` commands.
- **Changed:** new-project split to create only PROJECT.md and config.json; questioning rewritten as thinking partner.

### 1.0.11 — 2025-12-15
- **Added:** `/ecl:research-phase` command for niche domain ecosystem discovery.
...
### 1.0.0 — 2025-12-14
- **Added:** Initial release of the eCL Core meta-prompting system with core slash commands, PROJECT.md/STATE.md templates, phase-based workflow, YOLO mode, and interactive mode with checkpoints.
...
### 1.50.0-canary.1
- `/ecl plan-phase --mvp` flag enables vertical-slice planning mode; suppresses horizontal-layer language in favour of user-flow-driven decomposition.
- `/ecl mvp-phase <N>` new top-level command frames a phase as a vertical MVP slice using "As a / I want to / So that" user stories, with SPIDR splitting for oversized stories.
...
- `/ecl new-project` prompts for Vertical MVP vs Horizontal Layers mode; `/ecl-progress`, `/ecl-stats`, and `/ecl-graphify` gain MVP-mode awareness.
...
### 1.40.0-rc.1
- Six namespace meta-skills (`ecl:workflow`, `ecl:project`, `ecl:review`, `ecl:context`, `ecl:manage`, `ecl:ideate`) replace the flat 86-skill listing; drops cold-start overhead from ~2,150 to ~120 tokens.
...
- `/ecl-health --context` utilization guard warns at 60% and raises critical at 70% context-window utilization.
- Gemini slash commands use the correct `/ecl:<cmd>` namespace form; previous `/ecl-<cmd>` references were unexecutable in Gemini CLI.
...
- `ecl-sdk auto` detects Codex runtime correctly; previously ignored `runtime: codex` and routed through the Claude SDK, producing `[FAILED] $0.00 0.1s`.
