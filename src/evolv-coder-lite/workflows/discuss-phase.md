...
<required_reading>
@~/.claude/evolv-coder-lite/references/domain-probes.md
@~/.claude/evolv-coder-lite/references/gate-prompts.md
@~/.claude/evolv-coder-lite/references/universal-anti-patterns.md
</required_reading>
...
1. **ecl-phase-researcher** — Reads CONTEXT.md to know WHAT to research
2. **ecl-planner** — Reads CONTEXT.md to know WHAT decisions are locked
...
**Express path available:** If you already have a PRD or acceptance criteria document, use `/ecl:plan-phase {phase} --prd path/to/prd.md` to skip this discussion and go straight to planning.
...
INIT=$(ecl_run query init.phase-op "${PHASE}"); [[ "$INIT" == @file:* ]] && INIT=$(cat "${INIT#@file:}")
AGENT_SKILLS_ADVISOR=$(ecl_run query agent-skills ecl-advisor-researcher)
```
...
Use /ecl:progress ${ECL_WS} to see available phases.
```
Exit workflow.
...
if [ -f "$HOME/.claude/evolv-coder-lite/USER-PROFILE.md" ]; then
  ADVISOR_MODE=true
else
  ADVISOR_MODE=false
fi
...
Check if a SPEC.md (from `/ecl:spec-phase`) exists for this phase. SPEC.md locks requirements before implementation decisions.
...
**Note:** SPEC.md files named `AI-SPEC.md` (from `/ecl:ai-integration-phase`) are excluded — different purpose.
</step>
...
If raw spikes/sketches exist but no findings skill, note: `⚠ Unpackaged spikes/sketches detected — run /ecl:spike --wrap-up or /ecl:sketch --wrap-up to make findings available.`
...
```bash
TODO_MATCHES=$(ecl_run query todo.match-phase "${PHASE_NUMBER}")
```
...
Read `@~/.claude/evolv-coder-lite/references/scout-codebase.md` — it contains the phase-type→map selection table, single-read rule, no-maps fallback, and `<codebase_context>` output schema. Then execute:
...
```bash
DISCUSS_PRE_HOOKS_JSON=$(ecl_run loop render-hooks discuss:pre --raw)
```
Apply each entry in `activeHooks` per @~/.claude/evolv-coder-lite/references/loop-hook-dispatch.md. Empty list → continue to `analyze_phase`.
</step>
...
```bash
DISCUSS_POST_HOOKS_JSON=$(ecl_run loop render-hooks discuss:post --raw)
```
Apply each entry in `activeHooks` per @~/.claude/evolv-coder-lite/references/loop-hook-dispatch.md. Empty list → continue to `confirm_creation`.
</step>
...
`/ecl:plan-phase ${PHASE} ${ECL_WS}`

---

**Also available:** `--chain` for auto plan+execute after; `/ecl:plan-phase ${PHASE} --skip-research ${ECL_WS}` to plan without research; `/ecl:ui-phase ${PHASE} ${ECL_WS}` for UI design contracts; review/edit CONTEXT.md before continuing.
```
</step>
...
```bash
ecl_run query commit "docs(${padded_phase}): capture phase context" --files "${phase_dir}/${padded_phase}-CONTEXT.md" "${phase_dir}/${padded_phase}-DISCUSSION-LOG.md"
```
...
ecl_run query commit "docs(state): record phase ${PHASE} context session" --files .planning/STATE.md
```
</step>
