 ...
 *
 * Structural assertions for /ecl-ingest-docs (#2387). Agents and workflows
...
const { extractFrontmatter } = require('../evolv-coder-lite/bin/lib/frontmatter.cjs');

const ROOT = path.join(__dirname, '..');
const CMD_PATH = path.join(ROOT, 'commands', 'ecl', 'ingest-docs.md');
const WF_PATH = path.join(ROOT, 'evolv-coder-lite', 'workflows', 'ingest-docs.md');
const CLASSIFIER_PATH = path.join(ROOT, 'agents', 'ecl-doc-classifier.md');
const SYNTHESIZER_PATH = path.join(ROOT, 'agents', 'ecl-doc-synthesizer.md');
const CONFLICT_ENGINE_PATH = path.join(ROOT, 'evolv-coder-lite', 'references', 'doc-conflict-engine.md');

// ─── File Existence ────────────────────────────────────────────────────────────

describe('ingest-docs file structure (#2387)', () => {
  test('command file exists', () => {
    assert.ok(fs.existsSync(CMD_PATH), 'commands/ecl/ingest-docs.md should exist');
  });
  test('workflow file exists', () => {
    assert.ok(fs.existsSync(WF_PATH), 'evolv-coder-lite/workflows/ingest-docs.md should exist');
  });
  test('classifier agent exists', () => {
    assert.ok(fs.existsSync(CLASSIFIER_PATH), 'agents/ecl-doc-classifier.md should exist');
  });
  test('synthesizer agent exists', () => {
    assert.ok(fs.existsSync(SYNTHESIZER_PATH), 'agents/ecl-doc-synthesizer.md should exist');
  });
...
describe('ingest-docs command frontmatter', () => {
  ...
  test('has name field', () => {
    assert.match(content, /^name:\s*ecl:ingest-docs$/m);
  });
...
describe('ingest-docs command references', () => {
  ...
  test('references the ingest-docs workflow', () => {
    assert.ok(
      content.includes('@~/.claude/evolv-coder-lite/workflows/ingest-docs.md'),
      'command must @-reference its workflow'
    );
  });
  test('references the doc-conflict-engine', () => {
    assert.ok(
      content.includes('@~/.claude/evolv-coder-lite/references/doc-conflict-engine.md'),
      'command must load the shared conflict-engine contract'
    );
  });
  test('references gate-prompts', () => {
    assert.ok(
      content.includes('@~/.claude/evolv-coder-lite/references/gate-prompts.md'),
...
describe('ingest-docs workflow content', () => {
  ...
  test('spawns ecl-doc-classifier and ecl-doc-synthesizer', () => {
    assert.ok(
      content.includes('ecl-doc-classifier'),
      'workflow must spawn ecl-doc-classifier'
    );
    assert.ok(
      content.includes('ecl-doc-synthesizer'),
      'workflow must spawn ecl-doc-synthesizer'
    );
  });
  ...
  test('routes to ecl-roadmapper in new mode', () => {
    assert.ok(
      content.includes('ecl-roadmapper'),
      'new mode must delegate to ecl-roadmapper'
    );
  });
...
describe('ecl-doc-classifier agent', () => {
...
describe('ecl-doc-synthesizer agent', () => {
...
describe('import command adopts shared conflict-engine', () => {
  const cmdContent = fs.readFileSync(path.join(ROOT, 'commands', 'ecl', 'import.md'), 'utf-8');
  const wfContent = fs.readFileSync(path.join(ROOT, 'evolv-coder-lite', 'workflows', 'import.md'), 'utf-8');

  test('import command loads doc-conflict-engine reference', () => {
    assert.ok(
      cmdContent.includes('@~/.claude/evolv-coder-lite/references/doc-conflict-engine.md'),
      '/ecl-import must load the shared conflict-engine contract'
    );
  });
