...
describe('trace correlation — end-to-end parentTraceId propagation', () => {
  before(() => {
    ...
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-trace-correlation-test-'));
    auditPath = path.join(tmpDir, '.planning', '.ecl-trace.jsonl');
  ...
  after(() => {
    // Restore ECL_AUDIT to whatever it was before this test group
  ...
  test('JS filter on parentTraceId returns exactly the 3 children (jq-style)', () => {
    const rootTraceId = capturedEvents[0].traceId;
    // Simulates: jq 'select(.parentTraceId == $rootTraceId)' .ecl-trace.jsonl
  ...
  test('invalid parentTraceId in a child dispatch breaks the correlation tree for that child but does not poison sibling traces', () => {
    ...
    const isolatedTmp = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-poison-test-'));
    let isolatedSavedAudit;
    try {
      isolatedSavedAudit = process.env.ECL_AUDIT;
      process.env.ECL_AUDIT = '1';
      ...
      const isolatedAuditPath = path.join(isolatedTmp, '.planning', '.ecl-trace.jsonl');
