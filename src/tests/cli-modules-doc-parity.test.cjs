...
/**
 * For every `evolv-coder-lite/bin/lib/*.cjs`, assert the module name
...
const LIB_DIR = path.join(ROOT, 'evolv-coder-lite', 'bin', 'lib');
  ...
  for (const file of moduleFiles) {
    test(file, () => {
      assert.ok(
        ...
        `evolv-coder-lite/bin/lib/${file} has no row in docs/INVENTORY.md CLI Modules table — add one`,
      );
    });
  }
});
