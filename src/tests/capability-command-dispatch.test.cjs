...
const { dispatchCapabilityCommand } = require('../evolv-coder-lite/bin/ecl-tools.cjs');
...
const { ExitError } = require('../evolv-coder-lite/bin/lib/cli-exit.cjs');
...
describe('dispatchCapabilityCommand — non-ExitError from router → structured error via error()', () => {
  test('router throws TypeError → injected error() called with attributed message, raw error does NOT propagate', () => {
    ...
    const { ERROR_REASON } = require('../evolv-coder-lite/bin/lib/io.cjs');
