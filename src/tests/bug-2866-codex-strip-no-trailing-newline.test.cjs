 ...
 *
 * The cleanup regexes in `bin/install.js` matched stale eCL hook blocks
 ...
 * saw `ecl-check-update` already present, skipped writing the new
...
const SHAPES = {
  'Shape 1 (legacy ecl-update-check)': [
    '# eCL Hooks',
    '[[hooks]]',
    'event = "SessionStart"',
    'command = "node /Users/USER/.codex/hooks/ecl-update-check.js"',
  ].join('\n'),
  'Shape 2 (flat [[hooks]] + ecl-check-update)': [
    '# eCL Hooks',
    '[[hooks]]',
    'event = "SessionStart"',
    'command = "node /Users/USER/.codex/hooks/ecl-check-update.js"',
  ].join('\n'),
...
describe('bug-2866: stripStaleGsdHookBlocks handles end-of-file without trailing newline', () => {
  ...
  function assertStripped(out, shape, scenario) {
    ...
    const staleCmd = [...shape_.keys.entries()].find(([_, v]) =>
      /ecl-(update-check|check-update)/.test(v));
    assert.strictEqual(staleCmd, undefined,
      `(${shape}, ${scenario}) no key may carry a stale ecl-*-update command, got: ${staleCmd && staleCmd.join('=')}`);
  ...
  test('returns input unchanged when no eCL hook block is present', () => {
    const benign = '[history]\npersistence = "save-all"\n';
    const out = stripStaleGsdHookBlocks(benign);
    assert.strictEqual(out, benign, 'helper must be a no-op when no eCL reference exists');
  ...
  test('user-authored [[hooks.UserPromptSubmit]] is preserved', () => {
    ...
    // eCL-managed `ecl-(check-update|update-check).js` command.
    const input = [
      '[history]',
      'persistence = "save-all"',
