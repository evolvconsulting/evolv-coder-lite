 ...
 *
 * ALLOWED:
 *   - require('../evolv-coder-lite/bin/lib/foo.cjs')  -- runs the module, not text inspection
...
// Requires a source-dir indicator ('bin', 'lib', 'evolv-coder-lite') to avoid
// flagging temp files like path.join(tmpDir, 'example.cjs').
//   const CONFIG_PATH = path.join(__dirname, '..', 'evolv-coder-lite', 'bin', 'lib', 'config-schema.cjs');
const CJS_PATH_CONST_RE = /(?:const|let|var)\s+(\w+)\s*=\s*path\.join\s*\([^)]*(?:'bin'|"bin"|'lib'|"lib"|'evolv-coder-lite'|"evolv-coder-lite")[^)]*['"][^'"]*\.cjs['"]/gm;
...
const READ_WITH_INLINE_CJS_RE = /readFileSync\s*\([^,)]*path\.join\s*\([^)]*(?:'bin'|"bin"|'lib'|"lib"|'evolv-coder-lite'|"evolv-coder-lite")[^)]*['"][^'"]*\.cjs['"]/;
