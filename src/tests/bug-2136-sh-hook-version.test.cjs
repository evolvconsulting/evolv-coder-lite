 ...
 *
 * Root cause: three bash hooks (ecl-phase-boundary.sh, ecl-session-state.sh,
 * ecl-validate-commit.sh) shipped without a ecl-hook-version header, and the
 * stale-hook detector in ecl-check-update.js only matched JavaScript comment
 * syntax (//) — not bash comment syntax (#).
 *
 * Result: every session showed "⚠ stale hooks — run /ecl-update" immediately
 ...
 *   1. Bash hooks ship with "# ecl-hook-version: {{ECL_VERSION}}"
 *   2. install.js substitutes {{ECL_VERSION}} in .sh files at install time
 *   3. ecl-check-update.js regex matches both "//" and "#" comment styles
 *
 * Neither fix alone is sufficient:
 *   - Headers + regex fix only (no install.js fix): installed hooks contain
 *     literal "{{ECL_VERSION}}" — the {{-guard silently skips them, making
...
// NOTE: Do NOT set ECL_TEST_MODE here — the E2E install tests spawn the
// real installer subprocess, which skips all install logic when ECL_TEST_MODE=1.
...
const CHECK_UPDATE_FILE = path.join(HOOKS_DIR, 'ecl-check-update.js');
const WORKER_FILE = path.join(HOOKS_DIR, 'ecl-check-update-worker.js');
...
describe('bug #2136 part 1: bash hook sources carry ecl-hook-version placeholder', () => {
  for (const sh of SH_HOOKS) {
    test(`${sh} contains "# ecl-hook-version: {{ECL_VERSION}}"`, () => {
      const content = fs.readFileSync(path.join(HOOKS_DIR, sh), 'utf8');
      assert.ok(
        content.includes('# ecl-hook-version: {{ECL_VERSION}}'),
        `${sh} must include "# ecl-hook-version: {{ECL_VERSION}}" so the ` +
        `installer can stamp it and ecl-check-update.js can detect staleness`
      );
    });
  }
  ...
  test('version header is on line 2 (immediately after shebang)', () => {
    ...
    for (const sh of SH_HOOKS) {
      ...
      assert.ok(
        lines[1].startsWith('# ecl-hook-version:'),
        `${sh} line 2 must be the ecl-hook-version header (got: "${lines[1]}")`
      );
    }
  });
});
...
// ─────────────────────────────────────────────────────────────────────────────
// Part 2: ecl-check-update-worker.js regex handles bash "#" comment syntax
...
describe('bug #2136 part 2: stale-hook detector handles bash comment syntax', () => {
  ...
  test('version regex in source matches "#" comment syntax in addition to "//"', () => {
    ...
    assert.ok(
      hasBashAlternative,
      'ecl-check-update-worker.js version regex must include an alternative for bash "#" comments. ' +
  ...
  test('version regex does not use the old JS-only form as the sole pattern', () => {
    ...
    //   /\\/\\/ ecl-hook-version:\\s*(.+)/
    // which, when evaluated in the subprocess, produced: /\/\/ ecl-hook-version:\s*(.+)/
    // That only matched JS "//" comments — never bash "#".
    // We verify that the old exact string no longer appears.
    assert.ok(
      !src.includes('\\/\\/ ecl-hook-version'),
      'ecl-check-update-worker.js must not use the old JS-only (\\/\\/ ecl-hook-version) ' +
  ...
  test('version regex correctly matches both bash and JS hook version headers', () => {
    ...
    assert.ok(
      src.includes('ecl-hook-version'),
      'ecl-check-update-worker.js must contain a ecl-hook-version version check'
    );

    // The fixed regex that must be present: matches both comment styles
    const fixedRegex = /(?:\/\/|#) ecl-hook-version:\s*(.+)/;

    assert.ok(
      fixedRegex.test('# ecl-hook-version: 1.36.0'),
      'bash-style "# ecl-hook-version: X" must be matchable by the required regex'
    );
    assert.ok(
      fixedRegex.test('// ecl-hook-version: 1.36.0'),
      'JS-style "// ecl-hook-version: X" must still match (no regression)'
    );
    assert.ok(
      !fixedRegex.test('ecl-hook-version: 1.36.0'),
...
// ─────────────────────────────────────────────────────────────────────────────
// Part 3a: install.js bundled path substitutes {{ECL_VERSION}} in .sh hooks
// ─────────────────────────────────────────────────────────────────────────────

describe('bug #2136 part 3a: install.js bundled path substitutes {{ECL_VERSION}} in .sh hooks', () => {
  let src;
  ...
  test('.sh branch in bundled hook copy loop reads file and substitutes ECL_VERSION', () => {
    ...
    assert.ok(
      region.includes('ECL_VERSION'),
      'bundled .sh branch must reference ECL_VERSION substitution. Without this, ' +
      'installed .sh hooks contain the literal "{{ECL_VERSION}}" placeholder and ' +
...
// ─────────────────────────────────────────────────────────────────────────────
// Part 3b: install.js Codex path also substitutes {{ECL_VERSION}} in .sh hooks
// ─────────────────────────────────────────────────────────────────────────────

describe('bug #2136 part 3b: install.js Codex path substitutes {{ECL_VERSION}} in .sh hooks', () => {
  let src;
  ...
  test('.sh branch in Codex hook copy block substitutes ECL_VERSION', () => {
    ...
    assert.ok(
      region.includes('ECL_VERSION'),
      'Codex .sh branch must substitute {{ECL_VERSION}}. The bundled path was fixed ' +
...
describe('bug #2136 part 4: installed .sh hooks contain stamped concrete version', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('ecl-2136-install-');
  });
  ...
  test('installed .sh hooks contain a concrete version string, not the template placeholder', () => {
    ...
    for (const sh of SH_HOOKS) {
      ...
      assert.ok(
        content.includes('# ecl-hook-version:'),
        `installed ${sh} must contain a "# ecl-hook-version:" header`
      );
      assert.ok(
        !content.includes('{{ECL_VERSION}}'),
        `installed ${sh} must not contain literal "{{ECL_VERSION}}" — ` +
        `install.js must substitute it with the concrete package version`
      );

      const versionMatch = content.match(/# ecl-hook-version:\s*(\S+)/);
  ...
  test('stale-hook detector reports zero stale bash hooks immediately after fresh install', () => {
    ...
    // version-check logic (extracted from ecl-check-update.js) against the
    ...
    const checkScript = `
      ...
      // Use the same regex that the fixed ecl-check-update.js uses
      const versionRe = /(?:\\/\\/|#) ecl-hook-version:\\s*(.+)/;
