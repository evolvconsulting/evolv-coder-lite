 ...
 * /ecl:<cmd> command literals because the hook-copy loop in install.js had
 * no /ecl: → /ecl- rewrite for the claude runtime.
 *
 * Fix: the `.js` branch of the hook-copy loop now applies
 * `content.replace(/ecl:/gi, 'ecl-')` when
 ...
 *      no user-facing /ecl: literals (// comment occurrences exempted).
 ...
 * ECL_TEST_MODE is cleared so the install() main block executes.
 */
...
/**
 * Return lines (from nonCommentLines) that contain a user-facing /ecl: ref.
 */
function colonRefs(content) {
  return nonCommentLines(content).filter(line => /\/ecl:/.test(line));
}
...
describe('bug #376 — prerequisite: hooks/dist is present', () => {
  ...
  test('hooks/dist contains at least one .js hook file with a /ecl: literal', () => {
    ...
    assert.ok(
      ...
      'Expected at least one hooks/dist .js file with a non-comment /ecl: literal ' +
...
// ---------------------------------------------------------------------------
// Suite 1 — Claude install: no /ecl: colon refs in installed .js hook files
// ---------------------------------------------------------------------------
describe('bug #376 — Suite 1: Claude install rewrites /ecl: → /ecl- in hook .js files', () => {
  let tmpDir;

  before(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-376-claude-'));
  ...
  test('1b: installed .js hook files contain no user-facing /ecl: colon refs', () => {
    const hooksDir = path.join(tmpDir, '.claude', 'hooks');
    ...
    assert.deepEqual(
      offenders,
      [],
      'Installed Claude hook .js files must not contain /ecl:<cmd> colon refs ' +
      '(non-comment occurrences). The install-time rewriter must replace these with /ecl-<cmd>. ' +
      'Offenders: ' + JSON.stringify(offenders, null, 2),
    );
  });

  test('1c: installed .js hook files DO contain the hyphen form /ecl- (rewrite happened)', () => {
    const hooksDir = path.join(tmpDir, '.claude', 'hooks');
    ...
    const withHyphen = jsFiles.filter(({ full }) => {
      const content = fs.readFileSync(full, 'utf-8');
      return /\/ecl-/.test(content);
    });

    assert.ok(
      withHyphen.length > 0,
      'At least one installed .js hook file must contain /ecl- (confirming rewrite ran). ' +
      `Files checked: ${jsFiles.map(f => f.rel).join(', ')}`,
    );
  });
});
...
// ---------------------------------------------------------------------------
// Suite 2 — Cursor install regression: /ecl: → /ecl- still works (pre-existing)
//
// Note: Cursor installs its own hooks (ecl-cursor-session-start.js and
// ecl-cursor-post-tool.js) via the cursor-hooks-json installSurface (issue #777).
// It does NOT install the bundled Claude-style hooks/dist files (no ecl-session-state.sh
// etc.). The Cursor /ecl: rewrite applies in `copyWithPathReplacement` to JS files
// under the agent/skill tree (.cursor/evolv-coder-lite/*.js etc). We verify that Cursor's
// installed .js files under .cursor/ have no /ecl: colon refs, and that the hooks/
// directory contains only the Cursor-specific managed hooks.
// ---------------------------------------------------------------------------
describe('bug #376 — Suite 2: Cursor install still rewrites /ecl: → /ecl- (regression)', () => {
  let tmpDir;

  before(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-376-cursor-'));
  ...
  test('2b: Cursor-installed .js files contain no user-facing /ecl: colon refs', () => {
    const cursorDir = path.join(tmpDir, '.cursor');
    if (!fs.existsSync(cursorDir)) return;

    // Infrastructure files whose /ecl: occurrences are intentional implementation
    ...
    // by evolv-coder-lite/bin/lib/command-roster.cjs on ALL runtimes (including Cursor).
    // It must be installed verbatim and must NOT be content-rewritten: it needs to
    // emit `/ecl:${cmd}` for non-Cursor runtimes, and its /ecl: strings are internal
    ...
    const offenders = [];
    for (const { rel, full } of jsFiles) {
      // Skip infrastructure files whose /ecl: strings are intentional (see above).
    ...
    assert.deepEqual(
      offenders,
      [],
      'Cursor-installed .js files must not contain /ecl:<cmd> colon refs. ' +
      'The existing Cursor branch in copyWithPathReplacement must still apply /ecl:/gi → ecl- rewrite. ' +
      'Offenders: ' + JSON.stringify(offenders, null, 2),
    );
  });

  test('2c: Cursor install creates a hooks/ directory with only Cursor-specific managed hooks', () => {
    // Since issue #777, Cursor installs ecl-cursor-session-start.js and
    // ecl-cursor-post-tool.js into <configDir>/hooks/. These are Cursor-native
    // hooks — NOT the bundled Claude-style hooks (no ecl-session-state.sh etc.).
    ...
    const CLAUDE_BUNDLED_HOOKS = ['ecl-session-state.sh', 'ecl-context-monitor.js', 'ecl-statusline.js'];
    ...
    assert.ok(
      fs.existsSync(path.join(hooksDir, 'ecl-cursor-session-start.js')),
      'ecl-cursor-session-start.js must be installed in .cursor/hooks/ (#777)',
    );
    assert.ok(
      fs.existsSync(path.join(hooksDir, 'ecl-cursor-post-tool.js')),
      'ecl-cursor-post-tool.js must be installed in .cursor/hooks/ (#777)',
    );
  });
});
...
describe('bug #376 — Suite 3: hooks/ source files are unchanged by install', () => {
  ...
  test('3a: hooks/dist .js source files still contain /ecl: literals (not mutated)', () => {
    ...
    assert.ok(
      withColonRef.length > 0,
      'hooks/dist .js files must still contain /ecl: literals after install — ' +
