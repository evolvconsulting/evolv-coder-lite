...
```bash
USER_STORY_RESULT=$(ecl_run query user-story.validate --story "$USER_STORY")
...
```bash
NEW_MODE=$(ecl_run query roadmap.get-phase "${PHASE}" --pick mode)
NEW_GOAL=$(ecl_run query roadmap.get-phase "${PHASE}" --pick goal)
```
