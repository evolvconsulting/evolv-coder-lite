...
```bash
INIT=$(ecl-sdk query init.plan-phase "${PHASE}")
...
(graphify is not exposed on `ecl-sdk query` yet; use `ecl-tools.cjs` for graphify only.)
...
Validate each created PLAN.md using `ecl-sdk query`:

```bash
VALID=$(ecl-sdk query frontmatter.validate "$PLAN_PATH" --schema plan)
```
...
```bash
STRUCTURE=$(ecl-sdk query verify.plan-structure "$PLAN_PATH")
```
...
```bash
ecl-sdk query commit "docs($PHASE): create phase plan" --files \
