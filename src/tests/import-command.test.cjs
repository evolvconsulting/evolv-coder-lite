 ...
 *
 * Structural assertions for the /ecl-import command and workflow files.
 */
...
const CMD_PATH = path.join(__dirname, '..', 'commands', 'ecl', 'import.md');
const WF_PATH = path.join(__dirname, '..', 'evolv-coder-lite', 'workflows', 'import.md');

// ─── File Existence ────────────────────────────────────────────────────────────

describe('import command file structure', () => {
  test('command file exists', () => {
    assert.ok(fs.existsSync(CMD_PATH), 'commands/ecl/import.md should exist');
  });

  test('workflow file exists', () => {
    assert.ok(fs.existsSync(WF_PATH), 'evolv-coder-lite/workflows/import.md should exist');
  });
});
...
describe('import command frontmatter', () => {
  ...
  test('has name field', () => {
    assert.match(content, /^name:\s*ecl:import$/m);
  });
...
describe('import command references', () => {
  ...
  test('references the import workflow', () => {
    assert.ok(
      content.includes('@~/.claude/evolv-coder-lite/workflows/import.md'),
      'command should reference the workflow via @~/.claude/evolv-coder-lite/workflows/import.md'
    );
  });
});
...
describe('import workflow content', () => {
  ...
  test('includes plan-checker validation gate', () => {
    assert.ok(
      content.includes('ecl-plan-checker'),
      'workflow should delegate validation to ecl-plan-checker'
    );
  });

  test('no-args usage display is present', () => {
    assert.ok(
      content.includes('Usage: /ecl:import') || content.includes('Usage: /ecl-import'),
