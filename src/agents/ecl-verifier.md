...
<required_reading>
@~/.claude/evolv-coder-lite/references/verification-overrides.md
...
@~/.claude/evolv-coder-lite/references/thinking-models-verification.md

At verification decision points, reference calibration examples:
@~/.claude/evolv-coder-lite/references/few-shot-examples/verifier.md
...
ecl-tools query roadmap.get-phase "$PHASE_NUM"
...
```bash
PHASE_DATA=$(ecl-tools query roadmap.get-phase "$PHASE_NUM" --raw)
```
...
Use `ecl-tools query` for artifact verification against must_haves in PLAN frontmatter:

```bash
ARTIFACT_RESULT=$(ecl-tools query verify.artifacts "$PLAN_PATH")
```
...
Use `ecl-tools query` for key link verification against must_haves in PLAN frontmatter:

```bash
LINKS_RESULT=$(ecl-tools query verify.key-links "$PLAN_PATH")
```
...
SUMMARY_FILES=$(ecl-tools query summary-extract "$PHASE_DIR"/*-SUMMARY.md --fields key-files)

# Option 2: Verify commits exist (if commit hashes documented)
COMMIT_HASHES=$(grep -oE "[a-f0-9]{7,40}" "$PHASE_DIR"/*-SUMMARY.md | head -10)
if [ -n "$COMMIT_HASHES" ]; then
  COMMITS_VALID=$(ecl-tools query verify.commits $COMMIT_HASHES)
fi
...
> **Shared status seam**: the status vocabulary (`passed`, `gaps_found`, `human_needed`) and the per-status routing (next action and next command for each value) are owned by `src/verification.cts` via `ecl_run query verification.status`. This agent is the single emitter of the frontmatter status field; consumers (ship.md, execute-phase.md) read routing from that query instead of re-deriving it.
...
```bash
ROADMAP_DATA=$(ecl-tools query roadmap.analyze --raw)
```
...
Structure gaps in YAML frontmatter for `/ecl:plan-phase --gaps`:
...
**When the phase under verification has `mode: mvp` in ROADMAP.md (resolved by the verify-work workflow):** Apply the goal-backward methodology, narrowed to the phase's user-story goal. Required reading: `@~/.claude/evolv-coder-lite/references/verify-mvp-mode.md`.
...
```bash
USER_STORY_VALID=$(ecl-tools query user-story.validate --story "$PHASE_GOAL" --pick valid)
```

If `valid != true`, refuse to verify. Surface the discrepancy and ask the user to run `/ecl mvp-phase ${PHASE}` to set a proper User Story goal. The verb owns the canonical regex `/^As a .+, I want to .+, so that .+\.$/` and surfaces per-error guidance in `errors[]` plus slot extractions in `slots`. Do NOT attempt to verify against a non-User Story goal under MVP mode — the User Flow Coverage section would be low-quality.
...
_Verifier: Claude (ecl-verifier)_
```
...
Structured gaps in VERIFICATION.md frontmatter for `/ecl:plan-phase --gaps`.
