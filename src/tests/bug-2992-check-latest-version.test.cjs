...
const { checkLatestVersion, CHECK_REASON, PACKAGE_NAME } = require(
  path.join(ROOT, 'evolv-coder-lite', 'bin', 'check-latest-version.cjs'),
);
...
describe('Bug #2992: deterministic latest-version check', () => {
  test('PACKAGE_NAME is the constant @evolvconsulting/evolv-coder-lite (no callers can override)', () => {
...
describe('Bug #2992: error paths', () => {
  const { checkLatestVersion, CHECK_REASON } = require(require('node:path').join(__dirname, '..', 'evolv-coder-lite', 'bin', 'check-latest-version.cjs'));
...
describe('Issue #815: --next dist-tag support', () => {
  ...
  test('buildViewArgs() defaults to the bare latest spec (byte-for-byte unchanged)', () => {
    assert.deepEqual(buildViewArgs(), ['view', '@evolvconsulting/evolv-coder-lite', 'version']);
    assert.deepEqual(buildViewArgs('latest'), ['view', '@evolvconsulting/evolv-coder-lite', 'version']);
  });

  test('buildViewArgs("next") targets the @next dist-tag', () => {
    assert.deepEqual(buildViewArgs('next'), ['view', '@evolvconsulting/evolv-coder-lite@next', 'version']);
  });
