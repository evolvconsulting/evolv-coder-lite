...
Todos are captured during work sessions with /ecl-add-todo.

---

Would you like to:

1. Continue with current phase (/ecl:progress)
2. Add a todo now (/ecl-add-todo)
```
...
- `/ecl:capture --list` → show all
- `/ecl:capture --list api` → filter to area:api only
</step>
...
- `/ecl:capture --list [area]` to filter by area
- `q` to exit
```
...
- options:
  - "Work on it now" — move to done, start working
  - "Create a phase" — /ecl-add-phase with this scope
  - "Brainstorm approach" — think through before deciding
  - "Put it back" — return to list
</step>
...
**Create a phase:**
Display: `/ecl-add-phase [description from todo]`
...
$ECL_SDK query commit "docs: start work on todo - [title]" --files .planning/todos/completed/[filename] .planning/STATE.md
```
