...
function disabledResponse() {
  return { disabled: true, message: 'graphify is not enabled. Enable with: ecl-tools config-set graphify.enabled true' };
}
...
function graphifyStatus(cwd) {
  ...
  // hooks/ecl-graphify-update.sh PostToolUse hook (opt-in via graphify.auto_update,
  // default false). When the most recent auto-build is "failed" or still "running",
  // fold that into the existing `stale: true` signal so consumers (ecl-planner,
  // ecl-phase-researcher) surface the standard "treat semantic relationships as
