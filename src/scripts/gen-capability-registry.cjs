...
/**
 * gen-capability-registry.cjs — generates evolv-coder-lite/bin/lib/capability-registry.cjs
...
const CONFIG_SCHEMA_PATH = path.join(ROOT, 'evolv-coder-lite', 'bin', 'shared', 'config-schema.manifest.json');
...
// Require the committed evolv-coder-lite/bin/lib/loop-host-contract.cjs artifact so the
...
function validateStep(step, prefix, declaredSkills, declaredAgents) {
  ...
  if (typeof step.ref !== 'object' || step.ref === null) {
    ...
    } else if (hasSkill && typeof step.ref.skill === 'string' && step.ref.skill.startsWith('ecl-')) {
      // Double-prefix guard: ref.skill is an unprefixed stem (e.g. "ui-review").
      // Workflow dispatch prepends "ecl-" at runtime → "ecl-ui-review".
      // A stem that already starts with "ecl-" would produce "ecl-ecl-..." at dispatch.
      errors.push(
        prefix + '.ref.skill "' + step.ref.skill + '" must not start with "ecl-" ' +
        '(it is an unprefixed stem; the workflow prepends "ecl-" at dispatch — ' +
        'starting with "ecl-" would produce "ecl-' + step.ref.skill + '")',
      );
...
const INSTALL_PROFILES_PATH = path.join(ROOT, 'evolv-coder-lite', 'bin', 'lib', 'install-profiles.cjs');
const CLUSTERS_PATH = path.join(ROOT, 'evolv-coder-lite', 'bin', 'lib', 'clusters.cjs');
...
function runConsistencyGate(capabilityClusters, profileMembership, capMap) {
  ...
  const commandsGsdDir = path.join(ROOT, 'commands', 'ecl');
...
function main() {
  ...
  if (flag === '--check') {
    ...
    if (!fs.existsSync(REGISTRY_PATH)) {
      process.stderr.write(
        'evolv-coder-lite/bin/lib/capability-registry.cjs does not exist. Run:\n' +
    ...
    if (normalizeLineEndings(stripGeneratedComment(committed)) !== normalizeLineEndings(stripGeneratedComment(live))) {
      process.stderr.write(
        'evolv-coder-lite/bin/lib/capability-registry.cjs is stale. Run:\n' +
        '  node scripts/gen-capability-registry.cjs --write\n',
      );
      throw new ExitError(1);
    }

    process.stdout.write('evolv-coder-lite/bin/lib/capability-registry.cjs is up to date.\n');
  } else if (flag === '--write') {
    // Fix #3: read the REAL central config keys so collision detection fires and is visible.
    const centralKeys = loadCentralConfigKeys();
    const { capMap, errors } = loadAndValidate(centralKeys);
