...
> eCL Core 命令参考手册 — 所有稳定命令的语法、标志、选项及示例。功能详情请参阅[功能参考](FEATURES.md)；工作流程演示请参阅[用户指南](USER-GUIDE.md)；文档索引请参阅 [README](README.md)。

---

## 命令语法

- **Claude Code / Copilot / OpenCode / Kilo：** `/ecl-command-name [args]`（连字符形式）
- **Gemini CLI：** `/ecl:command-name [args]`（冒号形式 — Gemini 将命令置于 `ecl:` 命名空间下）
...
v1.40 中，六个命名空间路由器作为第一阶段入口点随附发布。与平铺式 86 个技能列表（约 2150 个 token）相比，它们将预加载技能列表的 token 开销保持在较低水平（6 个路由器约 120 个 token），同时完整功能仍可直接调用。模型先选择命名空间，再路由到具体子技能。详见 [#2792](https://github.com/evolvconsulting/evolv-coder-lite/issues/2792)。

| 命令 | 路由至 |
|---------|-----------|
| `/ecl-workflow` | 阶段流水线 — discuss / plan / execute / verify / phase / progress |
| `/ecl-project` | 项目生命周期 — 里程碑、审计、摘要 |
| `/ecl-quality` | 质量关卡 — 代码审查、调试、审计、安全、评估、界面 |
| `/ecl-context` | 代码库智能 — 映射、图谱、文档、学习记录 |
| `/ecl-manage` | 管理 — 配置、工作区、工作流、线程、更新、发布、收件箱 |
| `/ecl-ideate` | 探索与捕捉 — 探索、草图、实验、规格、捕捉 |

命名空间技能是**叠加式**的 — 每个现有的具体命令（例如 `/ecl-plan-phase`、`/ecl-code-review --fix`）仍可直接调用。
...
```bash
/ecl-new-project                    # 交互模式
/ecl-new-project --auto @prd.md     # 从 PRD 自动提取
```
...
/ecl-workspace --new --name feature-b --repos . --strategy worktree  # 同仓库隔离
/ecl-workspace --list
...
```bash
/ecl-discuss-phase 1                # 阶段 1 的交互式讨论
/ecl-discuss-phase 1 --all          # 不经选择步骤讨论所有灰色地带
/ecl-discuss-phase 3 --auto         # 自动为阶段 3 选择默认值
/ecl-discuss-phase --batch          # 当前阶段的批量模式
/ecl-discuss-phase 2 --analyze      # 含权衡分析的讨论
/ecl-discuss-phase 1 --power        # 从文件批量解答
/ecl-discuss-phase 3 --assumptions  # 在规划前呈现 Claude 的假设
```
...
```bash
/ecl-plan-phase 1                              # 研究 + 规划 + 验证阶段 1
/ecl-plan-phase 3 --skip-research              # 无需研究直接规划（熟悉的领域）
/ecl-plan-phase --auto                         # 非交互式规划
/ecl-plan-phase 2 --validate                   # 规划前验证状态
/ecl-plan-phase 1 --bounce                     # 规划 + 外部弹回验证
/ecl-plan-phase 2 --ingest docs/adr/0010.md   # 使用 ADR 快速通道进行上下文综合
/ecl-plan-phase 2 --ingest 'docs/adr/00*.md' --ingest-format auto
/ecl-plan-phase --research-phase 4             # 仅研究阶段 4（RESEARCH.md 存在时提示）
/ecl-plan-phase --research-phase 4 --view      # 打印现有 RESEARCH.md，不生成新报告
/ecl-plan-phase --research-phase 4 --research  # 强制刷新研究，不提示
/ecl-plan-phase 1 --mvp                        # 阶段 1 的垂直切片规划
/ecl-plan-phase 1 --mvp --tdd                  # 垂直切片 + 每个行为添加任务以失败测试开始
```
...
```bash
/ecl-plan-review-convergence 3                    # 默认审查者，3 个循环
/ecl-plan-review-convergence 3 --codex            # 仅 Codex 审查
...
**[测试版]** 将阶段规划卸载到 Claude Code 的 ultraplan 云端；在浏览器中审查并导入回来。计划在远程起草，终端保持空闲；在浏览器中审查内联评论，然后通过 `/ecl-import` 将最终计划导入 `.planning/`。

| 标志 | 必填 | 描述 |
|------|----------|-------------|
| `N` | **是** | 要远程规划的阶段编号 |

**隔离性：** 有意与 `/ecl-plan-phase` 分开，以防上游 ultraplan 变更影响核心规划流水线。

```bash
/ecl-ultraplan-phase 4                  # 卸载阶段 4 的规划
```
...
```bash
/ecl-execute-phase 1                # 执行阶段 1
/ecl-execute-phase 1 --wave 2       # 仅执行第 2 波
/ecl-execute-phase 1 --validate     # 执行前验证状态
/ecl-execute-phase 2 --cross-ai     # 将阶段 2 委托给外部 AI CLI
```
...
如需基于浏览器的 UAT，请使用已配置的浏览器 MCP 服务器。当前的 Open eCL 配套工具是 `ecl-browser`（`ecl-browser mcp`），提供确定性导航、版本化引用、断言、截图、视觉差异对比、录制和人工接管功能。已配置的旧版 Playwright MCP 服务器仍可使用。

```bash
/ecl-verify-work 1                  # 阶段 1 的 UAT
```
...
**前提条件：** 阶段已验证（`/ecl-verify-work` 通过），`gh` CLI 已安装并完成身份验证
**产出：** 带有规划产物丰富正文的 GitHub PR，STATE.md 已更新

```bash
/ecl-ship 4                         # 发布阶段 4
/ecl-ship 4 --draft                 # 作为草稿 PR 发布
```
...
**前提条件：** 项目有前端代码（可独立运行，无需 eCL 项目）
**产出：** `{phase}-UI-REVIEW.md`，截图保存在 `.planning/ui-reviews/`

如需更丰富的视觉证据，可将此命令与 `ecl-browser` 或其他浏览器 MCP 服务器配合使用，以便审计可以捕获截图、状态、控制台/网络上下文和可重现的交互步骤。

```bash
/ecl-ui-review                      # 审计当前阶段
/ecl-ui-review 3                    # 审计阶段 3
```
...
```bash
/ecl-milestone-summary                # 摘要当前里程碑
/ecl-milestone-summary v1.0           # 摘要特定里程碑
```
...
```bash
/ecl-new-milestone                  # 交互式
/ecl-new-milestone "v2.0 Mobile"    # 命名里程碑
/ecl-new-milestone --reset-phase-numbers "v2.0 Mobile"  # 从 1 重新开始里程碑编号
```
...
```bash
/ecl-phase "Add authentication system"          # 追加带描述的新阶段
/ecl-phase --insert 3 "Fix auth race condition" # 在阶段 3 和 4 之间插入 → 创建 3.1
/ecl-phase --remove 7               # 移除阶段 7，8→7、9→8 等重新编号
/ecl-phase --edit 5                 # 编辑阶段 5 的任意字段
/ecl-phase --edit 5 --force         # 即使阶段 5 进行中或已完成也进行编辑
```
...
阶段的引导式 MVP 规划 — 提示输入用户故事，运行 SPIDR 拆分检查，将 `**Mode:** mvp` 写入 ROADMAP.md，然后委托给 `/ecl-plan-phase`（通过路线图字段自动检测 MVP 模式）。
...
**前提条件：** 阶段必须已存在于 ROADMAP.md 中（通过 `/ecl-new-project`、`/ecl-phase` 或 `/ecl-phase --insert` 创建）。该命令不创建新阶段 — 它转换现有阶段。

**行为：** 收集结构化用户故事，验证格式，运行 SPIDR 拆分检查，将 `**Goal:**` 和 `**Mode:** mvp` 写入阶段的 ROADMAP.md 章节，然后委托给 `/ecl-plan-phase <N>`。演示请参阅[如何规划 MVP 阶段](USER-GUIDE.md#mvp-phase-planning)。

**行走骨架：** 当在无先前阶段摘要的新项目第 1 阶段使用 `--mvp`（或 `mode: mvp`）时自动触发。规划器在 `PLAN.md` 旁边生成 `SKELETON.md`。

**产出：** 已更新的 ROADMAP.md，以及 `/ecl-plan-phase` 的所有产物；行走骨架模式触发时生成 `SKELETON.md`。

```bash
/ecl-mvp-phase 1                    # 阶段 1 的 MVP 规划
/ecl-mvp-phase 2.1                  # 小数阶段的 MVP 规划
/ecl-mvp-phase 3 --force            # 即使阶段 3 进行中也进行转换
```
...
```bash
/ecl-validate-phase 2               # 审计阶段 2 的测试覆盖率
```
...
| `--do "task description"` | 分析自由形式的意图并分派到最合适的 eCL 命令 |
| `--forensic` | 在标准报告后附加 6 项完整性审计（STATE 一致性、孤立切换、延期范围漂移、内存标记的待处理工作、阻塞性 todo、未提交代码） |

**自动路由行为（`--next`）：**
- 无项目 → 建议 `/ecl-new-project`
- 阶段需要讨论 → 运行 `/ecl-discuss-phase`
- 阶段需要规划 → 运行 `/ecl-plan-phase`
- 阶段需要执行 → 运行 `/ecl-execute-phase`
- 阶段需要验证 → 运行 `/ecl-verify-work`
- 所有阶段已完成 → 建议 `/ecl-complete-milestone`

```bash
/ecl-progress                       # "我在哪里？下一步是什么？"（含自动路由）
/ecl-progress --next                # 自动推进至下一步
/ecl-progress --do "fix the auth bug"  # 将自由形式意图分派到最佳 eCL 命令
/ecl-progress --forensic            # 标准报告 + 完整性审计
```
...
```bash
/ecl-pause-work                     # 创建 continue-here.md
/ecl-pause-work --report            # 创建 continue-here.md + 会话报告
```
...
```bash
/ecl-manager                        # 打开命令中心仪表板
/ecl-manager --analyze-deps         # 在并行执行前扫描 ROADMAP 阶段的依赖关系
```
...
按请求层级显示 eCL 命令。默认适合单屏显示；`--full` 为完整参考；`<topic>` 直接跳转到某一章节。

```bash
/ecl-help                           # 单页导览（默认）
/ecl-help --brief                   # 约 10 行的顶级命令简明摘要
/ecl-help --full                    # 完整参考（每个命令，每个标志）
/ecl-help <topic>                   # 仅一个章节（例如 /ecl-help debug）
/ecl-help --brief <topic>           # 简洁的范围查找 — 签名 + 单行摘要
```

完整别名表请参阅 `evolv-coder-lite/workflows/help/modes/topic.md`。未知主题将打印已识别的列表。
...
苏格拉底式构思会话 — 通过深度提问引导某个想法，可选择生成研究内容，然后将输出路由到正确的 eCL 产物（笔记、待办、种子、研究问题、需求或新阶段）。

| 参数 | 必填 | 描述 |
|----------|----------|-------------|
| `topic` | 否 | 要探索的主题（例如 `/ecl-explore authentication strategy`） |

```bash
/ecl-explore                        # 开放式构思会话
/ecl-explore authentication strategy  # 探索特定主题
```
...
安全 git 回退 — 使用阶段清单并通过依赖检查和确认门回滚 eCL 阶段或计划提交。

| 标志 | 必填 | 描述 |
|------|----------|-------------|
| `--last N` | （三选一必填） | 显示最近的 eCL 提交以供交互式选择 |
...
```bash
/ecl-undo --last 5                  # 从最近 5 个 eCL 提交中选择
/ecl-undo --phase 03                # 回退阶段 3 的所有提交
/ecl-undo --plan 03-02              # 回退阶段 3 第 02 号计划的提交
```
...
将外部计划文件导入 eCL 规划系统，在写入任何内容之前检测与 `PROJECT.md` 决策的冲突。

| 标志 | 必填 | 描述 |
|------|----------|--------------|
| `--from <filepath>` | 是（或 `--from-ecl2`） | 要导入的外部计划文件路径 |
| `--from-ecl2` | 是（或 `--from`） | 将 eCL-2（`.ecl/`）项目反向迁移回 eCL v1（`.planning/`）格式 |
| `--path <dir>` | 否 | 与 `--from-ecl2` 配合：eCL-2 项目目录路径（默认为当前目录） |

**流程：** 检测冲突 → 提示解决 → 写入为 eCL PLAN.md → 通过 `ecl-plan-checker` 验证

```bash
/ecl-import --from /tmp/team-plan.md    # 导入并验证外部计划
/ecl-import --from-ecl2                # 从 eCL-2 迁移回 v1（当前目录）
/ecl-import --from-ecl2 --path ~/old-project  # 从不同路径迁移
```
...
从仓库中现有的 ADR、PRD、规格和文档引导或合并 `.planning/` 设置。运行并行分类（`ecl-doc-classifier`）以及带优先级规则和循环检测的综合（`ecl-doc-synthesizer`）。生成三分桶冲突报告（`INGEST-CONFLICTS.md`：自动解决、竞争变体、未解决阻塞项），并对 LOCKED-vs-LOCKED ADR 矛盾实施硬性阻止。
...
**限制：** v1 每次调用上限为 50 个文档。将共享冲突检测契约提取到 `references/doc-conflict-engine.md`，`/ecl-import` 也会使用。

```bash
/ecl-ingest-docs                            # 扫描仓库根目录，自动检测模式
/ecl-ingest-docs docs/                      # 仅摄取 docs/ 下的内容
/ecl-ingest-docs --manifest ingest.yaml     # 显式优先级清单
```
...
```bash
/ecl-quick                          # 基本快速任务
/ecl-quick --discuss --research     # 讨论 + 研究 + 规划
/ecl-quick --validate               # 仅计划检查 + 验证
/ecl-quick --full                   # 完整质量流水线
/ecl-quick list                     # 列出所有快速任务
/ecl-quick status my-task-slug      # 显示快速任务的状态
/ecl-quick resume my-task-slug      # 恢复快速任务
```
...
```bash
/ecl-autonomous                     # 运行所有剩余阶段
/ecl-autonomous --from 3            # 从阶段 3 开始
/ecl-autonomous --to 5              # 运行到阶段 5（含）
/ecl-autonomous --from 3 --to 5     # 运行阶段 3 到 5
```
...
**子命令：**
- `/ecl-debug list` — 列出所有活动调试会话及状态、假设和下一步操作
- `/ecl-debug status <slug>` — 打印会话的完整摘要（证据数量、已排除数量、解决方案、TDD 检查点），不生成代理
- `/ecl-debug continue <slug>` — 通过 slug 恢复特定会话（显示当前焦点后生成延续代理）
- `/ecl-debug [--diagnose] <description>` — 开始新调试会话（现有行为；`--diagnose` 在找到根本原因后停止，不应用修复）
...
```bash
/ecl-profile-user                   # 分析会话并构建档案
/ecl-profile-user --questionnaire   # 交互式问卷回退方案
/ecl-profile-user --refresh         # 从新分析中重新生成
```

### `/ecl-health`

验证 `.planning/` 目录完整性。使用 `--context` 时，针对 60% / 70% 阈值探测上下文窗口使用率保护（v1.40.0 新增，[#2792](https://github.com/evolvconsulting/evolv-coder-lite/issues/2792)）。
...
```bash
/ecl-health                         # 检查完整性
/ecl-health --repair                # 检查并修复
/ecl-health --context               # 上下文使用率分类
```
...
/ecl-sketch --text "onboarding flow"    # 非 Claude 运行时
/ecl-sketch --wrap-up                   # 将获胜草图打包为技能
```
...
```bash
/ecl-forensics                              # 交互式 — 提示输入问题
/ecl-forensics "Phase 3 execution stalled"  # 带问题描述
```
...
```bash
/ecl-extract-learnings 3                    # 提取阶段 3 的学习记录
/ecl-extract-learnings --all                # 从所有已完成阶段提取
```
...
```bash
/ecl-workstreams                    # 列出所有工作流
/ecl-workstreams create backend-api # 创建新工作流
/ecl-workstreams switch backend-api # 设置活动工作流
/ecl-workstreams status backend-api # 详细状态
/ecl-workstreams progress           # 跨工作流进度概览
/ecl-workstreams complete backend-api  # 归档已完成的工作流
/ecl-workstreams resume backend-api    # 在工作流中恢复工作
```
...
所有答案通过 `ecl-tools query config-set` 合并到已解析的项目配置路径（标准安装为 `.planning/config.json`，工作流处于活动状态时为 `.planning/workstreams/<active>/config.json`），保留不相关的键。确认后，用户可以将完整设置对象保存到 `~/.ecl/defaults.json`，以便未来运行 `/ecl-new-project` 时从相同的基线开始。
...
通过单一合并命令交互式配置 eCL 设置 — 工作流切换、高级参数、集成和模型配置。
...
所有答案通过 `ecl-tools query config-set` 合并，保留不相关的键。API 密钥在所有输出中以掩码显示（`****<last-4>`）。

```bash
/ecl-config                         # 常用交互式配置
/ecl-config --advanced              # 高级用户参数（六章节提示）
/ecl-config --integrations          # API 密钥、审查 CLI 路由、代理技能
/ecl-config --profile budget        # 切换到 budget 配置
/ecl-config --profile quality       # 切换到 quality 配置
```
...
```bash
/ecl-surface list                   # 显示当前表面
/ecl-surface profile standard       # 切换到 standard 配置
/ecl-surface disable utility        # 禁用 utility 集群
/ecl-surface reset                  # 恢复安装时的配置
```
...
```bash
/ecl-map-codebase                   # 完整代码库分析（4 个并行代理）
/ecl-map-codebase auth              # 聚焦 auth 区域
/ecl-map-codebase --fast            # 快速技术 + 架构概览（1 个代理）
/ecl-map-codebase --fast --focus quality  # 仅质量和代码健康状况
/ecl-map-codebase --query authentication  # 搜索 intel 中的某个术语
```
...
```bash
/ecl-graphify build                 # 构建或重建知识图谱
/ecl-graphify query authentication  # 在图谱中搜索某个术语
/ecl-graphify status                # 显示新鲜度和统计信息
/ecl-graphify diff                  # 显示自上次构建以来的变更
```

**编程访问：** `node ecl-tools.cjs graphify <build|query|status|diff|snapshot>` — 参阅 [CLI 工具参考](CLI-TOOLS.md)。

### `ecl-tools intel api-surface`

将 `.planning/intel/api-map.json` 索引（由 `/ecl-map-codebase` 构建）渲染为 `.planning/intel/` 中人类可读的 `API-SURFACE.md`。以 `config.json` 中 `intel.enabled: true` 为门控；当 Intel 被禁用时，命令打印激活提示并退出。输出路径始终为 `.planning/intel/API-SURFACE.md` — 没有 `--out` 或 `--format` 标志。当 `api-map.json` 不存在或为空时，命令仍会写入文件并附带明确的"不完整"横幅，以便使用者不会将沉默误认为"什么都不存在"。

**产出：** `.planning/intel/API-SURFACE.md`

```bash
node ecl-tools.cjs intel api-surface              # 渲染 api-map.json → API-SURFACE.md
```
...
```bash
/ecl-ai-integration-phase              # 当前阶段的向导
/ecl-ai-integration-phase 3           # 特定阶段的向导
```
...
审计已执行 AI 阶段的评估覆盖率并生成 EVAL-REVIEW.md 修复计划。根据 `/ecl-ai-integration-phase` 生成的 `AI-SPEC.md` 评估计划检查实现情况。将每个评估维度评分为 COVERED/PARTIAL/MISSING。

**前提条件：** 阶段已被执行且有 `AI-SPEC.md`
**产出：** `{phase}-EVAL-REVIEW.md`，包含发现结果、差距和修复指南

```bash
/ecl-eval-review                       # 审计当前阶段
/ecl-eval-review 3                     # 审计特定阶段
```
...
更新 eCL，预览变更日志，并可选择同步技能或重新应用本地补丁。

| 标志 | 描述 |
|------|-------------|
| `--sync` | 更新后从 eCL 注册表同步技能 |
| `--reapply` | 更新后恢复本地修改（补丁） |

```bash
/ecl-update                         # 检查更新并安装
/ecl-update --sync                  # 更新并同步技能
/ecl-update --reapply               # 更新并重新应用本地补丁
```
...
**生成：** `ecl-code-reviewer` 代理；使用 `--fix` 时生成 `ecl-code-fixer` 代理

**可选结构预检：** 将 `code_quality.fallow.enabled` 设置为 `true` 可在代理审查前运行 fallow。eCL 写入 `{phase}/FALLOW.json` 并在 `REVIEW.md` 中嵌入 `Structural Findings (fallow)` 章节。使用 `code_quality.fallow.scope` 和 `code_quality.fallow.profile` 配置范围和配置文件。

```bash
/ecl-code-review 3                          # 阶段 3 的标准审查
/ecl-code-review 2 --depth=deep             # 深度跨文件审查
/ecl-code-review 4 --files src/auth.ts,src/token.ts  # 显式文件列表
/ecl-code-review 3 --fix                    # 审查后修复 Critical + Warning 发现
/ecl-code-review 3 --fix --all             # 审查后修复所有发现（包括 Info）
/ecl-code-review 3 --fix --auto            # 审查、修复并重新审查直到清洁（最多 3 次迭代）
```
...
```bash
/ecl-audit-fix                              # 运行 audit-uat，修复 medium+ 级别的问题（最多 5 个）
/ecl-audit-fix --severity high             # 仅修复高严重性问题
/ecl-audit-fix --dry-run                   # 预览分类而不修复
/ecl-audit-fix --max 10 --severity all     # 修复任意严重性的最多 10 个问题
```
...
**不是 `/ecl-quick` 的替代品** — 任何需要研究、多步骤规划或验证的事项请使用 `/ecl-quick`。
...
**默认审查者行为（无标志）：**
- 如果 `review.default_reviewers` **未设置**，`/ecl-review` 运行所有检测到的审查者（当前默认行为）。
- 如果 `review.default_reviewers` **已设置**，`/ecl-review` 仅运行该子集（例如 `["gemini","codex"]`）。
- `--all` 始终覆盖配置并运行完整的检测集。
- 显式标志（例如 `--cursor`）在该次运行中覆盖 `--all` 和配置默认值。

**产出：** `{phase}-REVIEWS.md` — 可供 `/ecl-plan-phase --reviews` 使用

```bash
# 设置项目默认审查者，用于无标志的 /ecl-review 运行
ecl config-set review.default_reviewers '["gemini","codex"]'

/ecl-review --phase 2             # 使用配置中的 gemini+codex 运行
/ecl-review --phase 3 --all
/ecl-review --phase 2 --gemini
/ecl-review --phase 2 --cursor    # 一次性覆盖
```
...
**目的：** 审查者只看到代码变更，而非 eCL 规划产物。

```bash
/ecl-pr-branch                     # 相对于 main 进行过滤
/ecl-pr-branch develop             # 相对于 develop 进行过滤
```
...
```bash
/ecl-secure-phase                   # 审计最后完成的阶段
/ecl-secure-phase 5                 # 审计特定阶段
```
...
**生成：** `ecl-doc-writer` 代理（每种文档类型一个），然后是用于事实验证的 `ecl-doc-verifier` 代理

每个文档写作代理直接探索代码库 — 不存在幻觉路径或过时签名。文档验证代理对照实时文件系统检查声明。

```bash
/ecl-docs-update                    # 交互式生成/更新文档
/ecl-docs-update --force            # 重新生成所有文档
/ecl-docs-update --verify-only      # 仅验证现有文档
```
...
**待办停车场：** 999.x 编号使条目保持在活动阶段序列之外；阶段目录立即创建，以便 `/ecl-discuss-phase` 和 `/ecl-plan-phase` 可以在其上运行。
**种子：** 保留完整的原因、触发时机和面包屑 — 由 `/ecl-new-milestone` 使用。

**产出：** `.planning/todos/`（默认）、笔记文件（--note）、ROADMAP.md 待办章节（--backlog）、`.planning/seeds/SEED-NNN-slug.md`（--seed）

```bash
/ecl-capture "Consider adding dark mode support"   # 添加待办事项
/ecl-capture --note "Caching strategy idea"        # 快速笔记
/ecl-capture --note list                           # 列出所有笔记
/ecl-capture --note promote 3                      # 将笔记 3 提升为待办事项
/ecl-capture --backlog "GraphQL API layer"         # 添加到待办停车场
/ecl-capture --seed "Add real-time collaboration when WebSocket infra is in place"
/ecl-capture --list                                # 浏览并处理待办事项
```
...
线程是用于跨多个会话但不属于任何特定阶段的工作的轻量级跨会话知识存储。比 `/ecl-pause-work` 更轻量。

```bash
/ecl-thread                         # 列出所有线程
/ecl-thread list --open             # 仅列出开放/进行中的线程
/ecl-thread list --resolved         # 仅列出已解决的线程
/ecl-thread status fix-deploy-key   # 显示线程状态
/ecl-thread close fix-deploy-key    # 将线程标记为已解决
/ecl-thread fix-deploy-key-auth     # 恢复线程
/ecl-thread "Investigate TCP timeout in pasta service"  # 创建新线程
```
...
```bash
node ecl-tools.cjs roadmap upgrade --convention milestone-prefixed         # 演练
node ecl-tools.cjs roadmap upgrade --convention milestone-prefixed --apply  # 应用
```
...
```bash
node ecl-tools.cjs state sync             # 从磁盘重建 STATE.md
node ecl-tools.cjs state sync --verify    # 演练：显示变更而不写入
```
...
| 钩子 | 用途 |
|------|---------|
| `ecl-validate-commit.sh` | 对 git 提交信息强制执行 Conventional Commits 格式 |
| `ecl-session-state.sh` | 跟踪会话状态转换 |
| `ecl-phase-boundary.sh` | 执行阶段边界检查 |
...
加入 eCL Discord 社区，请访问 eCL README 中的链接，或运行 `/ecl-help` 并点击其中显示的 Discord 链接。

---

## 贡献：技能描述标准

技能描述（每个 `commands/ecl/*.md` frontmatter 中的 `description:` 字段）会被注入到每个会话的系统提示中。为保持每会话开销较低，描述必须不超过 100 个字符，且不得重复 `argument-hint:` 中已有的标志文档。
