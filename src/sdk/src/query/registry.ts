 ...
 * The `ecl-sdk query` CLI resolves argv with `resolveQueryArgv()` before dispatch;
 * there is no automatic delegation to `ecl-tools.cjs`.
 *
 * Also exports `extractField` — a TypeScript port of the `--pick` field
 * extraction logic from ecl-tools.cjs (lines 365-382).
 *
 * @example
 * ```typescript
 ...
 *
 * Direct port of `extractField()` from ecl-tools.cjs (lines 365-382).
 ...
 *
 * `dispatch()` throws `GSDError` for unknown command keys. The `ecl-sdk query`
 * CLI uses `resolveQueryArgv()` first; when no handler matches, it may shell out
 * to `ecl-tools.cjs` (see `cli.ts` and `QUERY-HANDLERS.md` fallback policy).
 */
...
/**
 * Map argv after `ecl-sdk query` to a registered handler key and remaining args.
