...
// ---------------------------------------------------------------------------
// Allowlisted set of user-invocable skills (commands/ecl/*.md, ns-* excluded).
...
describe('absorbed skills are removed', () => {
  ...
  for (const [name, reason] of absorbed) {
    test(`commands/ecl/${name}.md does NOT exist (${reason})`, () => {
      assert.ok(
...
describe('outright deleted dead skills are removed', () => {
  const deleted = [
    ...
    // from-ecl2          → import --from-ecl2 (pending #3131)
  ];

  for (const name of deleted) {
    test(`commands/ecl/${name}.md does NOT exist`, () => {
      assert.ok(
        !fs.existsSync(skillPath(name)),
...
describe('#3131 re-wired workflows: standalone command files must not exist', () => {
  const rewired = [
    ...
    ['from-ecl2',              'absorbed into import.md --from-ecl2'],
  ];
...
describe('#3131 re-wired workflows: parent command argument-hints advertise the new flags', () => {
  ...
  test('import.md argument-hint contains --from-ecl2', () => {
    const fm = parseFrontmatter(skillPath('import'));
    assert.ok(
      (fm['argument-hint'] || '').includes('--from-ecl2'),
      'import.md argument-hint does not contain --from-ecl2. got: ' + (fm['argument-hint'] || '(none)'),
    );
  });
});
...
describe('#3131 re-wired workflows: parent command bodies dispatch to workflow files', () => {
  ...
  test('import.md body references from-ecl2', () => {
    assert.ok(
      bodyContains('import', 'from-ecl2'),
      'import.md body does not reference from-ecl2 — --from-ecl2 flag dispatch is missing',
    );
  });
});
...
describe('settings.md is kept (merged into config entry point or remains standalone)', () => {
  test('commands/ecl/settings.md still exists', () => {
    assert.ok(
...
describe('skill set', () => {
  test('user-invocable skill set is allowlisted (consolidating toward ~58)', () => {
    ...
    assertWithinAllowlist({
      label: 'user-invocable skills (commands/ecl)',
