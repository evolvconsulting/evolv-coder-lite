 ...
 *
 * Regression tests for #2832: ecl-sdk auto silently routed Codex (and other
 ...
 * runtime or fall back to the in-session eCL slash commands.
 */
...
describe('assertRuntimeSupportsAutoMode', () => {
  ...
  beforeEach(() => {
    prevEnv = process.env.ECL_RUNTIME;
    delete process.env.ECL_RUNTIME;
  });
  afterEach(() => {
    if (prevEnv === undefined) delete process.env.ECL_RUNTIME;
    else process.env.ECL_RUNTIME = prevEnv;
  });
  ...
  it('throws for ECL_RUNTIME=codex even when config says claude', () => {
  ...
  it('error message references issue #2832 and slash-command workaround', () => {
    ...
    expect(caught!.message).toMatch(/ecl-discuss-phase|ecl-plan-phase|ecl-execute-phase/);
  });
  ...
  it('attributes source to config when ECL_RUNTIME is set to an unsupported value', () => {
    ...
    process.env.ECL_RUNTIME = 'unsupported-env';
    let caught: Error | undefined;
    try {
      assertRuntimeSupportsAutoMode({ runtime: 'codex' });
    } catch (err) {
