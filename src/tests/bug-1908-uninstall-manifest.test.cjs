 ...
 *
 * `--uninstall` did not remove `ecl-file-manifest.json` from the target
 ...
 * after cleaning up the rest of the eCL artefacts.
 */
...
function createFakeInstall(prefix = 'ecl-uninstall-test-') {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), prefix));

  // Simulate the minimum directory/file layout produced by the installer:
  // evolv-coder-lite/ directory, agents/ directory, and the manifest file.
  fs.mkdirSync(path.join(dir, 'evolv-coder-lite', 'workflows'), { recursive: true });
  fs.writeFileSync(path.join(dir, 'evolv-coder-lite', 'workflows', 'execute-phase.md'), '# stub');

  fs.mkdirSync(path.join(dir, 'agents'), { recursive: true });
  fs.writeFileSync(path.join(dir, 'agents', 'ecl-executor.md'), '# stub');

  const manifest = {
    version: '1.34.0',
    timestamp: new Date().toISOString(),
    files: {
      'evolv-coder-lite/workflows/execute-phase.md': 'abc123',
      'agents/ecl-executor.md': 'def456',
    },
  };
...
describe('uninstall — manifest cleanup (#1908)', () => {
  ...
  test('ecl-file-manifest.json is removed after global uninstall', () => {
  ...
  test('ecl-file-manifest.json is removed after local uninstall', () => {
    const manifestPath = path.join(tmpDir, MANIFEST_NAME);
    ...
    fs.mkdirSync(path.join(localDir, 'evolv-coder-lite', 'workflows'), { recursive: true });
    fs.writeFileSync(path.join(localDir, 'evolv-coder-lite', 'workflows', 'execute-phase.md'), '# stub');
