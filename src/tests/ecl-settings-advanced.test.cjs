...
/**
 * Tests for `/ecl-settings-advanced` — power-user configuration command (#2528).
 *
 * Covers:
 ...
 *   - /ecl-settings confirmation output advertises /ecl-settings-advanced
...
const COMMAND_PATH = path.join(ROOT, 'commands', 'ecl', 'config.md');
const WORKFLOW_PATH = path.join(ROOT, 'evolv-coder-lite', 'workflows', 'settings-advanced.md');
const SETTINGS_WORKFLOW_PATH = path.join(ROOT, 'evolv-coder-lite', 'workflows', 'settings.md');
...
const SPEC_FIELDS = {
  ...
  git: [
    ...
    { key: 'git.phase_branch_template',      default: 'ecl/phase-{phase}-{slug}' },
    { key: 'git.milestone_branch_template',  default: 'ecl/{milestone}-{slug}' },
  ],
  runtime: [
...
describe('ecl-settings-advanced — file scaffolding', () => {
  ...
  test('workflow file exists at evolv-coder-lite/workflows/settings-advanced.md', () => {
  ...
  test('command frontmatter has name, description, allowed-tools', () => {
    ...
    const fm = fmMatch[1];
    assert.match(fm, /name:\s*ecl:config/, 'frontmatter missing name (ecl:config)');
...
describe('ecl-settings-advanced — workflow structure', () => {
  let workflow;
  try {
...
describe('ecl-settings-advanced — VALID_CONFIG_KEYS coverage', () => {
  for (const key of ALL_SPEC_KEYS) {
    test(`VALID_CONFIG_KEYS contains "${key}"`, () => {
      assert.ok(
        VALID_CONFIG_KEYS.has(key),
        `VALID_CONFIG_KEYS missing ${key} — add it to evolv-coder-lite/bin/lib/config-schema.cjs`
      );
    });
  }
});

// ─── /ecl-settings mentions /ecl-settings-advanced ────────────────────────────

describe('/ecl-settings advertises /ecl-settings-advanced', () => {
  test('settings workflow mentions canonical /ecl-config --advanced', () => {
    const text = fs.readFileSync(SETTINGS_WORKFLOW_PATH, 'utf-8');
    assert.ok(
      text.includes('/ecl:config --advanced'),
      'evolv-coder-lite/workflows/settings.md must mention /ecl:config --advanced'
    );
    assert.ok(
      !text.includes('ecl-settings-advanced') && !text.includes('ecl:settings-advanced'),
      'evolv-coder-lite/workflows/settings.md must not mention legacy /ecl-settings-advanced variants'
    );
  });
});
...
describe('ecl-settings-advanced — config merge preserves unrelated keys', () => {
...
describe('ecl-settings-advanced — negative scenarios', () => {
  ...
  // The /ecl-settings-advanced workflow promises non-numeric input is never
  ...
  test('config-set on a numeric key stores non-numeric input verbatim as string (workflow layer must reject before reaching here)', (t) => {
    ...
    // /ecl-settings-advanced workflow. If a future change adds a numeric
