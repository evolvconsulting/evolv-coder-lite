...
const RUNTIME_INSTALL_CONTRACTS = {
  ...
  gemini: { surface: 'commands-ecl', settings: true, packageJson: true },
...
function assertFreshInstallContract(runtime, targetDir) {
  ...
  if (contract.workflowPayload !== false) {
    assert.equal(
      fs.readFileSync(path.join(targetDir, 'evolv-coder-lite', 'VERSION'), 'utf8'),
      pkg.version,
      `${runtime} should install the package VERSION`
    );
    assert.ok(
      fs.existsSync(path.join(targetDir, 'evolv-coder-lite', 'bin', 'ecl-tools.cjs')),
      `${runtime} should install the eCL tool payload`
    );
    assert.ok(
      fs.existsSync(path.join(targetDir, 'ecl-file-manifest.json')),
      `${runtime} should write the install manifest`
    );

    const manifest = JSON.parse(fs.readFileSync(path.join(targetDir, 'ecl-file-manifest.json'), 'utf8'));
    ...
    assert.ok(
      manifest.files['evolv-coder-lite/VERSION'],
      `${runtime} manifest should track the installed VERSION file`
    );
  } else {
    assert.equal(
      fs.existsSync(path.join(targetDir, 'evolv-coder-lite')),
      false,
      `${runtime} should not install the eCL workflow payload`
    );
  }

  if (contract.surface === 'flat-skills') {
    // Pre-#3562: codex was special-cased to expect zero ecl-* skill dirs
    ...
    const hermesGsdDir = path.join(targetDir, 'skills', 'ecl');
    const hermesSkillCount = fs.existsSync(hermesGsdDir)
      ? fs.readdirSync(hermesGsdDir, { withFileTypes: true })
          ...
          .length
      : 0;
    assert.ok(hermesSkillCount > 0, `skills/ecl should contain generated eCL entries (got ${hermesSkillCount})`);
    assert.ok(
      fs.existsSync(path.join(targetDir, 'skills', 'ecl', 'DESCRIPTION.md')),
      'Hermes should install the nested eCL category description'
    );
  } else if (contract.surface === 'flat-command') {
    assert.ok(
      listDirNames(targetDir, 'command').some((name) => name.startsWith('ecl-') && name.endsWith('.md')),
      `${runtime} should install flattened command markdown files`
    );
  } else if (contract.surface === 'commands-ecl') {
    assert.ok(
      listDirNames(targetDir, path.join('commands', 'ecl')).length > 0,
      `${runtime} should install commands/ecl entries`
    );
  } else if (contract.surface === 'kimi-skills-agents') {
    assertHasGsdDirectory(targetDir, 'skills');
    assert.ok(
      fs.existsSync(path.join(targetDir, 'agents', 'ecl.yaml')),
      'Kimi should install the root agent YAML'
    );
    assert.ok(
      fs.existsSync(path.join(targetDir, 'agents', 'ecl.md')),
      'Kimi should install the root agent prompt'
    );
    assert.ok(
      fs.existsSync(path.join(targetDir, 'agents', 'subagents', 'ecl-executor.yaml')),
      'Kimi should install eCL subagent YAML'
    );
  ...
  if (contract.surface !== 'kimi-skills-agents') {
    assert.ok(
      listDirNames(targetDir, 'agents').some((name) => name.startsWith('ecl-')),
  ...
  if (contract.codexConfig) {
    assert.match(
      ...
      /eCL Agent Configuration/,
      'Codex should install config.toml with the eCL marker'
    );
  }
...
describe('installer migration install integration', { concurrency: false }, () => {
  ...
  beforeEach(() => {
    tmpRoot = createTempDir('ecl-install-migrations-');
  ...
  test('blocks install before materialization when baseline needs explicit user choice', () => {
    writeFile(codexHome, 'hooks/ecl-retired-hook.txt', 'old ecl hook\n');
    ...
    assert.equal(fs.readFileSync(path.join(codexHome, 'hooks/ecl-retired-hook.txt'), 'utf8'), 'old ecl hook\n');
    assert.equal(fs.existsSync(path.join(codexHome, 'skills')), false);
    assert.equal(fs.existsSync(path.join(codexHome, 'evolv-coder-lite', 'VERSION')), false);
  });
  ...
  test('rolls back applied migrations when package materialization fails for non-Codex installs', () => {
    ...
    assert.throws(
      () => captureConsole(() =>
        withEnv('CLAUDE_CONFIG_DIR', claudeHome, () =>
          withWriteFailure(path.join(claudeHome, 'evolv-coder-lite', 'VERSION'), () => install(true, 'claude'))
        )
      ),
    ...
    assert.equal(fs.existsSync(path.join(claudeHome, 'ecl-install-state.json')), false);
  });
  ...
  test('rolls back completed runtime migrations when a later runtime install fails', () => {
      ...
      () => captureConsole(() =>
        withEnv('CLAUDE_CONFIG_DIR', claudeHome, () =>
          withEnv('CODEX_HOME', codexHome, () =>
            withWriteFailure(path.join(codexHome, 'evolv-coder-lite', 'VERSION'), () =>
    ...
    assert.equal(fs.existsSync(path.join(codexHome, 'ecl-install-state.json')), false);
  });
  ...
  for (const runtime of SUPPORTED_RUNTIMES) {
    test(`runs a full end-to-end install for ${runtime}`, () => {
      ...
      const installState = JSON.parse(fs.readFileSync(path.join(targetDir, 'ecl-install-state.json'), 'utf8'));
      assert.ok(
    ...
    test(`blocks ambiguous eCL-looking user-choice artifacts for ${runtime}`, () => {
      const targetDir = path.join(tmpRoot, `.${runtime}-blocked`);
      fs.mkdirSync(targetDir, { recursive: true });
      writeFile(targetDir, 'evolv-coder-lite/ecl-retired-tool.cjs', 'old ambiguous artifact\n');
      ...
      const output = stripAnsi(`${result.stdout}\n${result.stderr}`);
      assert.match(output, /Installer migrations/);
      assert.match(output, /blocked\s+evolv-coder-lite\/ecl-retired-tool\.cjs/);
      assert.match(output, /installer migration blocked/);
      assert.equal(
        fs.readFileSync(path.join(targetDir, 'evolv-coder-lite/ecl-retired-tool.cjs'), 'utf8'),
        'old ambiguous artifact\n'
      );
      assert.equal(fs.existsSync(path.join(targetDir, 'evolv-coder-lite', 'VERSION')), false);
    });
  }
});
