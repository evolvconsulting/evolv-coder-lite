...
// ecl-config-reload.js — FileChanged hook: hot-reload eCL config context
...
process.stdin.on('end', () => {
  ...
  try {
    ...
    // Only handle the eCL planning config — verify both basename and that the
    ...
    if (event === 'unlink') {
      process.stdout.write(JSON.stringify({
        hookSpecificOutput: {
          ...
          additionalContext:
            'eCL config (.planning/config.json) was deleted. ' +
    ...
    try {
      ...
      process.stdout.write(JSON.stringify({
        hookSpecificOutput: {
          hookEventName: 'FileChanged',
          additionalContext:
            'eCL config (.planning/config.json) was modified but could not be parsed. ' +
    ...
    const lines = ['eCL config reloaded (.planning/config.json updated):'];
