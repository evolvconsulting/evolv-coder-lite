 ...
 * created under ECL_TEMP_DIR (path.join(os.tmpdir(), 'ecl')), not directly
...
describe('bug-866: profile-pipeline temp dirs under ECL_TEMP_DIR', () => {
  let tmpDir;
  let isolatedSysTmp;

  beforeEach(() => {
    tmpDir = createTempDir('ecl-866-');
  ...
  test('extract-messages output_file is under ECL_TEMP_DIR, not os.tmpdir() root', () => {
    ...
    // The expected ECL_TEMP_DIR from the child's perspective: isolatedSysTmp/ecl
    const expectedGsdTempDir = path.join(isolatedSysTmp, 'ecl');
    ...
    assert.ok(
      ...
      `output_file "${outputFile}" must be under ECL_TEMP_DIR "${expectedGsdTempDir}" (realpath: ${expectedRoot}); got dir "${actualDir}"`
    );

    // Must NOT be directly under the isolated systmp root (i.e., no ecl-pipeline-*
    // at depth 1 of isolatedSysTmp).
    const rel = path.relative(isolatedSysTmp, outputFile);
    const depth1Dir = rel.split(path.sep)[0];
    assert.ok(
      !depth1Dir.startsWith('ecl-pipeline-'),
      `output_file must not be in isolatedSysTmp/ecl-pipeline-* but got depth-1 dir: "${depth1Dir}"`
    );
  });

  test('profile-sample output_file is under ECL_TEMP_DIR, not os.tmpdir() root', () => {
    const sessionsDir = createSessions(tmpDir);
    ...
    const depth1Dir = rel.split(path.sep)[0];
    assert.ok(
      !depth1Dir.startsWith('ecl-profile-'),
      `output_file must not be in isolatedSysTmp/ecl-profile-* but got depth-1 dir: "${depth1Dir}"`
    );
  });
});
