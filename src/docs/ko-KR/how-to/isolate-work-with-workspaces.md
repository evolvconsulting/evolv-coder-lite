...
**목표:** 피처 브랜치나 멀티 저장소 작업을 위해 별도의 git 워크트리, 독립적인 `.planning/` 루트, 그리고 선택적으로 여러 저장소를 포함하는 완전히 격리된 eCL 환경을 만듭니다.
...
워크스페이스는 기본적으로 `~/ecl-workspaces/<name>/` 아래에 위치합니다.

```
~/ecl-workspaces/
└── feature-b/
    ├── WORKSPACE.md        ← 매니페스트
    ├── .planning/          ← 완전히 독립된 eCL 상태
    │   ├── PROJECT.md
    │   ├── ROADMAP.md
    │   └── ...
...
eCL는 `~/ecl-workspaces/feature-b/` 내에 `hr-ui`와 `ZeymoAPI`의 워크트리를 생성하고, 각각에서 `workspace/feature-b` 브랜치를 체크아웃하며, `WORKSPACE.md`를 작성하고 `/ecl-new-project`를 위한 빈 `.planning/` 디렉터리를 생성합니다.
...
`.`은 eCL에게 현재 저장소의 워크트리를 생성하도록 지시합니다. 워크트리는 `workspace/payments-rework`로 체크아웃됩니다.
...
eCL는 프롬프트 없이 모든 기본값을 수락합니다.

---

## 워크스페이스 내에서 eCL 초기화

워크스페이스를 생성한 후 그 안으로 이동하고 eCL 프로젝트를 초기화합니다:
...
워크스페이스 내의 `.planning/` 디렉터리가 해당 디렉터리에서 실행되는 모든 후속 eCL 명령의 루트입니다. 이것은 소스 저장소에 존재하는 어떠한 `.planning/`과도 완전히 분리되어 있습니다.
...
eCL는 git 워크트리를 제거하고 워크스페이스 디렉터리를 정리합니다. 이 작업은 원격 저장소에서 브랜치를 삭제하지 않으며 로컬 워크트리와 워크스페이스 디렉터리만 제거합니다.
...
- **여러 저장소**(예: API 저장소와 함께 출시되는 UI 저장소)를 하나의 eCL 프로젝트 아래서 조율해야 할 때
...
- 각 트래커 이슈가 워크스페이스에 매핑되는 이슈 기반 워크플로를 따를 때([트래커 이슈에서 eCL 구동](drive-ecl-from-a-tracker-issue.md) 참조)

[워크스트림](work-in-parallel-with-workstreams.md)을 선택할 때:

- 모든 작업이 **하나의 저장소**에 있고 같은 git 히스토리를 공유할 때
- 서로의 `STATE.md` 파일 간의 컨텍스트 유입 없이 서로 다른 관심 영역(API, UI, 인프라)에 대해 동시에 `/ecl-plan-phase` 또는 `/ecl-discuss-phase`를 실행하고 싶을 때
...
- [트래커 이슈에서 eCL 구동](drive-ecl-from-a-tracker-issue.md)
- [명령 참조](../COMMANDS.md)
- [문서 인덱스](../README.md)
