...
Defina `model_profile` em `.planning/config.json` ou via `/ecl-config --profile <name>`:
...
```bash
/ecl-config --profile balanced   # Desenvolvimento normal
/ecl-config --profile budget     # Prototipagem ou fases de alto custo
/ecl-config --profile quality    # Lançamento em produção
/ecl-config --profile inherit    # OpenRouter, modelos locais
```
...
`model_overrides` pode ser definido por projeto em `.planning/config.json` ou globalmente em `~/.ecl/defaults.json`. Entradas por projeto têm precedência em conflitos; entradas globais sem conflito são preservadas.
...
```bash
npx @evolvconsulting/evolv-coder-lite@latest --codex --global   # ou --opencode, --kilo, etc.
```
...
Todos os cinco agentes de pesquisa resolvem para `sonnet` *exceto* `ecl-codebase-mapper`, que está fixado em `haiku`.
...
Cada agente possui um nível padrão (`light`, `standard` ou `heavy`). Na primeira tentativa, o eCL escolhe `tier_models[default_tier]`. Se o orquestrador detectar uma falha suave (verificação inconclusiva, verificação de plano sinalizada, etc.), ele reinicia o agente um nível acima. `max_escalations` limita o total de novas tentativas.
...
## Usando o eCL em runtimes não-Anthropic

Se você instalou o eCL para Codex, OpenCode, Gemini CLI ou Kilo, o instalador já definiu `resolve_model_ids: "omit"` na sua configuração. Isso instrui o eCL a pular a resolução de IDs de modelo Anthropic e deixar o runtime escolher seu próprio modelo padrão. Nenhuma configuração manual é necessária para o caso básico.
...
O eCL resolve cada alias de nível para o modelo nativo do Codex e o esforço de raciocínio definido no mapa de nível do runtime.
