 ...
 *   - enabled axis: mutates .ecl-surface.json via readSurface/writeSurface
  ...
  for (const entry of desired) {
    if (entry.enabled === true) {
      ...
      if (afterCap && afterCap.enabled !== true) {
        errors.push(
          `cannot enable "${entry.id}": not in the active surface/profile (widen the profile or use /ecl:surface enable)`,
        );
      }
    }
...
/**
 * CLI command entry point for `ecl-tools capability set`.
 *
