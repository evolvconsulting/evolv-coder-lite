...
const JS_HOOKS = [
  { name: 'ecl-check-update.js',      registrationAnchor: 'hasGsdUpdateHook' },
  { name: 'ecl-context-monitor.js',   registrationAnchor: 'hasContextMonitorHook' },
...
describe('bug #1754: .js hook registration guards', () => {
  ...
  test('all .js hooks use the same guard pattern as .sh hooks', () => {
    ...
    // Count unique hook file existence checks (pattern: path.join(targetDir, 'hooks', 'ecl-*.js'))
    const jsGuards = (registrationSection.match(/ecl-[\w-]+\.js.*not found at target/g) || []);
    const shGuards = (registrationSection.match(/ecl-[\w-]+\.sh.*not found at target/g) || []);
