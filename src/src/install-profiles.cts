 ...
 */
const DEFAULT_COMMANDS_DIR = path.resolve(__dirname, '..', '..', '..', 'commands', 'ecl');
 ...
 * whose final cell carries a `ecl-<stem>` token (optionally with `--flag`
...
function transformRouterBodyToNested(converted: string): string {
  ...
  const out = lines.map((line) => {
    ...
    const m = cell.match(/ecl-([a-z0-9-]+)((?:\s+--[a-z0-9-]+)*)/i);
    if (!m) return line;
    const stem = m[1];
 ...
 * added here — the prefix is already embedded in agent stems, e.g. `ecl-planner.md`).
 *
...
function stageAgentsForRuntimeWithConverter(
  ...
  const stageDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-profile-runtime-agents-'));
  try {
    const entries = fs.readdirSync(srcAgentsDir, { withFileTypes: true });
    for (const entry of entries) {
      if (!entry.isFile()) continue;
...
function stageCommandsForRuntimeFlat(
  ...
  const stageDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-profile-runtime-commands-'));
  try {
    const entries = fs.readdirSync(srcCommandsDir, { withFileTypes: true });
    for (const entry of entries) {
      if (!entry.isFile()) continue;
