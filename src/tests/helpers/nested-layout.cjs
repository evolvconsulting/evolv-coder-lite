 ...
 * each commands/ecl/ns-*.md router's `requires:` frontmatter list with the
...
const { parseRequires } = require('../../evolv-coder-lite/bin/lib/install-profiles.cjs');

const COMMANDS_ECL = path.join(__dirname, '..', '..', 'commands', 'ecl');

// Router stems (ns-*.md basenames), discovered from disk and sorted.
const ROUTER_STEMS = fs
  .readdirSync(COMMANDS_ECL)
  .filter((f) => f.startsWith('ns-') && f.endsWith('.md'))
  .map((f) => f.replace(/\.md$/, ''))
  .sort();

/**
 * Read a router's `requires:` child list from commands/ecl/<routerStem>.md
 * using the production frontmatter parser.
 */
function routerChildren(routerStem) {
  const srcFile = path.join(COMMANDS_ECL, `${routerStem}.md`);
 ...
 * prefix is '' for runtimes that nest under a skills/ecl parent dir (hermes),
 * or 'ecl-' for flat-prefixed runtimes (claude, cline, qwen, …).
 */
