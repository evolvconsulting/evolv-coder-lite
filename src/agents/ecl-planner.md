...
**Test:** Could a different Claude instance execute without asking clarifying questions? If not, add specificity. See @~/.claude/evolv-coder-lite/references/planner-antipatterns.md for vague-vs-specific comparison table.
...
```bash
ecl-tools query history-digest
```
...
```bash
VALID=$(ecl-tools query frontmatter.validate "$PLAN_PATH" --schema plan)
```
...
```bash
STRUCTURE=$(ecl-tools query verify.plan-structure "$PLAN_PATH")
```
...
```bash
ecl-tools query commit "docs($PHASE): create phase plan" --files \
...
Execute: `/ecl:execute-phase {phase}`
...
Execute: `/ecl:execute-phase {phase} --gaps-only`
```
