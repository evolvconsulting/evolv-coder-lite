...
describe('loadConfig', () => {
  ...
  beforeEach(async () => {
    tmpDir = join(tmpdir(), `ecl-config-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
    await mkdir(join(tmpDir, '.planning'), { recursive: true });
    // Isolate ~/.ecl/defaults.json by pointing HOME at an empty tmp dir.
    fakeHome = join(tmpdir(), `ecl-home-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
    await mkdir(fakeHome, { recursive: true });
    prevHome = process.env.HOME;
    process.env.HOME = fakeHome;
    // Also isolate ECL_HOME (loadUserDefaults prefers it over HOME).
    prevGsdHome = process.env.ECL_HOME;
  ...
  afterEach(async () => {
    ...
    if (prevGsdHome === undefined) delete process.env.ECL_HOME;
    else process.env.ECL_HOME = prevGsdHome;
  });

  async function writeUserDefaults(defaults: unknown) {
    await mkdir(join(fakeHome, '.ecl'), { recursive: true });
    await writeFile(join(fakeHome, '.ecl', 'defaults.json'), JSON.stringify(defaults));
  }
  ...
  it('partial config merges correctly for nested objects', async () => {
    ...
    expect(config.git.phase_branch_template).toBe('ecl/phase-{phase}-{slug}');
  ...
  // ─── User-level defaults (~/.ecl/defaults.json) ─────────────────────────
  ...
  // had `resolve_model_ids: "omit"` in ~/.ecl/defaults.json.
  //
  // Mirrors current CJS parity expectations for SDK loadConfig + resolveModel:
  // in pre-project context, loadConfig ignores ~/.ecl/defaults.json so
  ...
  it('pre-project: ignores user defaults and uses built-in defaults', async () => {
    ...
    // default ('omit'), confirming that user-level ~/.ecl/defaults.json is still ignored.
  ...
  it('project config is authoritative over user defaults (CJS parity)', async () => {
    ...
    // ~/.ecl/defaults.json is ignored — buildNewProjectConfig already baked
  ...
  it('ignores malformed ~/.ecl/defaults.json', async () => {
    await mkdir(join(fakeHome, '.ecl'), { recursive: true });
    await writeFile(join(fakeHome, '.ecl', 'defaults.json'), '{not json');
