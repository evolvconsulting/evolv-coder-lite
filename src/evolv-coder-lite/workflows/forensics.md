...
Post-mortem investigation for failed or stuck eCL workflows. Analyzes git history,
...
3. {Recovery command if applicable — e.g., `/ecl:resume-work`, `/ecl:execute-phase N`}

---

*Report generated by `/ecl:forensics`. All paths redacted for portability.*
```
...
BUG_LABEL=$(gh label list --repo evolvconsulting/evolv-coder-lite --search "bug" --json name -q '.[0].name' 2>/dev/null)
LABEL_FLAG=""
if [ -n "$BUG_LABEL" ]; then
...
gh issue create \
  --repo evolvconsulting/evolv-coder-lite \
