...
function planningDir(cwd, ws, project) {
  if (project === undefined) project = process.env.ECL_PROJECT || null;
  if (ws === undefined) ws = process.env.ECL_WORKSTREAM || null;

  // Reject path separators and traversal components in project/workstream names
  const BAD_SEGMENT = /[/\\]|\.\./;
  if (project && BAD_SEGMENT.test(project)) {
    throw new Error(`ECL_PROJECT contains invalid path characters: ${project}`);
  }
  if (ws && BAD_SEGMENT.test(ws)) {
    throw new Error(`ECL_WORKSTREAM contains invalid path characters: ${ws}`);
  }
...
function getSessionScopedWorkstreamFile(cwd, fixedSessionKey) {
  ...
  const dirPath = path.join(os.tmpdir(), 'ecl-workstream-sessions', projectId);
  return {
    sessionKey,
    dirPath,
 ...
 * `NN.N-CONTEXT.md`, etc.) used by ecl-discuss-phase output.
 *
