 ...
 *    that was previously prose-only in `references/execute-mvp-tdd.md`. The ecl-executor
 ...
 *    prose. Consumed by the verifier (phase-goal guard) and by `/ecl-mvp-phase`
 *    (interactive-prompt validation).
 *
 * Domain terms: see CONTEXT.md → MVP Mode, User Story, Behavior-Adding Task.
 * Concept index: evolv-coder-lite/references/mvp-concepts.md.
 */
 ...
 *
 * @example
 *   ecl-sdk query phase.mvp-mode 1                    # roadmap + config check
 *   ecl-sdk query phase.mvp-mode 1 --cli-flag         # caller saw --mvp on CLI
 */
 ...
 *
 * Canonical specification: evolv-coder-lite/references/execute-mvp-tdd.md.
 *
 * @example
 *   ecl-sdk query task.is-behavior-adding ./plans/01-PLAN-auth.md
 *   ecl-sdk query task.is-behavior-adding --task-content "<task>...</task>"
 */
 ...
 * Used by `ecl-verifier` (phase-goal guard) and `/ecl-mvp-phase` (interactive prompting).
 *
 * @example
 *   ecl-sdk query user-story.validate "As a user, I want to log in, so that I can see my data."
 *   ecl-sdk query user-story.validate --story "<text>"
 */
