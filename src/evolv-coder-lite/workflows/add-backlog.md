...
Invoked by `/ecl:capture --backlog` (`commands/ecl/capture.md`).
...
NEXT=$($ECL_SDK query phase.next-decimal 999 --raw)
```
...
Plans:
- [ ] TBD (promote with /ecl:review-backlog when ready)
```
...
PROJECT_CODE=$($ECL_SDK query config-get project_code --raw 2>/dev/null || echo "")
...
```bash
$ECL_SDK query commit "docs: add backlog item ${NEXT} — ${ARGUMENTS}" --files .planning/ROADMAP.md "${PHASE_DIR}/.gitkeep"
```
...
Use /ecl:discuss-phase {NEXT} to explore it further.
Use /ecl:review-backlog to promote items to active milestone.
```
...
- Phase directories are created immediately so /ecl:discuss-phase and /ecl:plan-phase work on them
- No `Depends on:` field — backlog items are unsequenced by definition
- Sparse numbering is fine (999.1, 999.3) — always uses next-decimal
- Promote backlog items to the active milestone with /ecl:review-backlog
</notes>
