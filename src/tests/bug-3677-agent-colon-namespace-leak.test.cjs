...
/**
 * Regression for #3677 — installed agent bodies leak `/ecl:<cmd>` colon refs
 ...
 *     does no namespace work). The retired `/ecl:<cmd>` colon refs leak.
 *
...
describe('bug #3677 — agent body colon-namespace leak (Claude / Qwen / Hermes)', () => {
  ...
  describe('C — normalizeAgentBodyForRuntime applies transformContentToHyphen iff predicate is true', () => {
    ...
    const inputBody = [
      '# Agent prose',
      '',
      'Run `/ecl:execute-phase 1 --tdd` to execute the phase.',
      'Then `/ecl:verify-work 1` to verify.',
      'Reference unchanged: `ecl-sdk query commit` (this is a CLI binary, not a slash command).',
    ].join('\n');
    // Only known commands from commands/ecl/*.md should be rewritten; ecl-sdk
    ...
    test('C1: claude — rewrites both colon refs to hyphen', () => {
      ...
      assert.ok(out.includes('/ecl-execute-phase'), 'execute-phase must be rewritten to hyphen form');
      assert.ok(out.includes('/ecl-verify-work'), 'verify-work must be rewritten to hyphen form');
      assert.ok(!out.includes('/ecl:execute-phase'), 'colon form for execute-phase must be gone');
      assert.ok(!out.includes('/ecl:verify-work'), 'colon form for verify-work must be gone');
      assert.ok(out.includes('ecl-sdk query commit'), 'ecl-sdk (CLI binary) must not be touched');
    });
    ...
    test('C4: gemini — colon refs preserved (intentional namespace)', () => {
      ...
      assert.ok(out.includes('/ecl:execute-phase'), 'Gemini intentionally uses colon namespace; do not rewrite');
      assert.ok(!out.includes('/ecl-execute-phase'), 'Gemini agents must NOT have hyphen form');
    });
  ...
  describe('D — sanity check: the underlying transform actually works against real cmd names', () => {
    test('D1: transformContentToHyphen rewrites /ecl:<cmd> to /ecl-<cmd> for known cmds only', () => {
      const out = transformContentToHyphen(
        'A /ecl:execute-phase B /ecl:unknown-cmd C /ecl-sdk D',
        ['execute-phase'],
      );
      assert.ok(out.includes('/ecl-execute-phase'), 'known cmd rewritten');
      assert.ok(out.includes('/ecl:unknown-cmd'), 'unknown cmd preserved (longest-first matcher only rewrites registered names)');
      assert.ok(out.includes('/ecl-sdk'), 'ecl-sdk (binary, not slash command) preserved');
    });
  });
  ...
  // worth keeping: real-source efficacy against every `agents/ecl-*.md` (the
  ...
  describe('E — real-source efficacy + idempotence (ported from #3681, credit: johnzilla)', () => {
    ...
    // Roster regex matches any registered command in `ecl:<cmd>` form with a
    ...
    const roster = () => new RegExp(
      `(?<![a-zA-Z0-9_-])ecl:(${[...cmdNames].sort((a, b) => b.length - a.length).join('|')})(?=[^a-zA-Z0-9_-]|$)`,
    );
    ...
    test('E1: every agents/ecl-*.md transforms clean — no roster colon refs survive', () => {
      const agentsDir = path.join(REPO_ROOT, 'agents');
      const offenders = [];
      for (const f of fs.readdirSync(agentsDir)) {
        if (!f.startsWith('ecl-') || !f.endsWith('.md')) continue;
    ...
    test('E2: idempotent — transform of already-hyphenated input is a no-op', () => {
      const input = 'use /ecl-plan-phase next, then /ecl-execute-phase';
      assert.strictEqual(
    ...
    test('E3: word boundary — /ecl:plan-phase-extra is not a roster match', () => {
      assert.strictEqual(
        transformContentToHyphen('/ecl:plan-phase-extra', cmdNames),
        '/ecl:plan-phase-extra',
      );
    });

    test('E4: rewrites bare `ecl:<cmd>` shorthand (no leading slash)', () => {
      const out = transformContentToHyphen(
        'Spawned by the ecl:execute-phase orchestrator.',
        cmdNames,
      );
      assert.strictEqual(out, 'Spawned by the ecl-execute-phase orchestrator.');
    });
  });
});
