 ...
 *   `~/.claude/evolv-coder-lite/bin/lib/model-catalog.cjs` tries:
 *     require(path.join(__dirname, '..', '..', '..', 'sdk', 'shared', 'model-catalog.json'))
 *   which resolves to `~/.claude/sdk/shared/model-catalog.json`.
 *   The installer copies `evolv-coder-lite/` but never copies `sdk/shared/`,
 ...
 *      evolv-coder-lite/bin/shared/model-catalog.json (co-located inside the
 *      evolv-coder-lite/ payload).
 *
...
const MODEL_CATALOG_CJS = path.join(REPO_ROOT, 'evolv-coder-lite', 'bin', 'lib', 'model-catalog.cjs');
const MODEL_CATALOG_JSON = path.join(REPO_ROOT, 'evolv-coder-lite', 'bin', 'shared', 'model-catalog.json');
...
//   <tmp>/.claude/evolv-coder-lite/bin/lib/model-catalog.cjs  (copy of real file)
...
describe('bug #3288: model-catalog.cjs install-layout resolution', () => {
  ...
  beforeEach(() => {
    ...
    savedExplicitConfigDir = process.env.ECL_EXPLICIT_CONFIG_DIR;
    delete process.env.ECL_EXPLICIT_CONFIG_DIR;
  });
  ...
  test('OLD layout (3-level __dirname, no co-located json) fails to require', () => {
    ...
    //   <tmpRoot>/.claude/evolv-coder-lite/bin/lib/model-catalog.cjs  (copy of the real CJS)
    //   sdk/shared/model-catalog.json                              ABSENT
    const gsdLibDir = path.join(tmpRoot, '.claude', 'evolv-coder-lite', 'bin', 'lib');
...
`;
  ...
  test('NEW layout (co-located bin/shared/model-catalog.json) resolves correctly', () => {
    ...
    //   <tmpRoot>/.claude/evolv-coder-lite/bin/lib/model-catalog.cjs (copy of real CJS)
    //   <tmpRoot>/.claude/evolv-coder-lite/bin/shared/model-catalog.json (co-located copy)
    const gsdBinDir = path.join(tmpRoot, '.claude', 'evolv-coder-lite', 'bin');
