 ...
 *
 * Seam: evolv-coder-lite/bin/lib/agent-install-check.cjs
 * Interface: getAgentsDir, checkAgentsInstalled
 *
 * Verifies:
 *   1. getAgentsDir behaviour: ECL_AGENTS_DIR override, claude path, non-claude path
 *   2. checkAgentsInstalled behaviour against temp dirs via ECL_AGENTS_DIR:
...
const AGENT_INSTALL_CHECK_PATH = path.join(
  __dirname, '..', 'evolv-coder-lite', 'bin', 'lib', 'agent-install-check.cjs'
);
const RUNTIME_HOMES_PATH = path.join(
  __dirname, '..', 'evolv-coder-lite', 'bin', 'lib', 'runtime-homes.cjs'
);
...
const MODEL_PROFILES = require(path.join(__dirname, '..', 'evolv-coder-lite', 'bin', 'lib', 'model-profiles.cjs')).MODEL_PROFILES;
...
beforeEach(() => {
  savedAgentsDir = process.env['ECL_AGENTS_DIR'];
  savedRuntime = process.env['ECL_RUNTIME'];
...
afterEach(() => {
  if (savedAgentsDir === undefined) {
    ...
    process.env['ECL_AGENTS_DIR'] = savedAgentsDir;
  }
  if (savedRuntime === undefined) {
    delete process.env['ECL_RUNTIME'];
  } else {
    process.env['ECL_RUNTIME'] = savedRuntime;
  }
});
...
describe('getAgentsDir', () => {
  test('ECL_AGENTS_DIR override takes priority', () => {
    process.env['ECL_AGENTS_DIR'] = '/tmp/x';
  ...
  test('ECL_RUNTIME env var is respected when no argument provided', () => {
  ...
  test('defaults to claude when no arg and no ECL_RUNTIME', () => {
...
describe('checkAgentsInstalled', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('ecl-agent-check-');
    // Point ECL_AGENTS_DIR at a path we control
    process.env['ECL_AGENTS_DIR'] = path.join(tmpDir, 'agents');
  });
  ...
  test('agents_dir and agent_runtime are returned in result', () => {
    ...
    // ECL_AGENTS_DIR overrides, so agents_dir = our tmp path
