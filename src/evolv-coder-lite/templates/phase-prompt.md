...
> **Note:** Planning methodology is in `agents/ecl-planner.md`.
...
<!-- For checkpoint task examples and patterns, see @~/.claude/evolv-coder-lite/references/checkpoints.md -->
...
**Wave is pre-computed:** Wave numbers are assigned during `/ecl:plan-phase`. Execute-phase reads `wave` directly from frontmatter and groups plans by wave number. No runtime dependency analysis needed.
...
See `~/.claude/evolv-coder-lite/references/tdd.md` for TDD plan structure.
...
See `~/.claude/evolv-coder-lite/templates/user-setup.md` for full schema and examples
...
See `~/.claude/evolv-coder-lite/workflows/verify-phase.md` for verification logic.
