...
export type LegacySdkAsset = 'ecl-tools' | 'core-cjs';
...
export const BUNDLED_CORE_CJS_PATH = fileURLToPath(
  new URL('../../evolv-coder-lite/bin/lib/core.cjs', import.meta.url),
);

export const BUNDLED_GSD_TEMPLATES_DIR = fileURLToPath(
  new URL('../../evolv-coder-lite/templates', import.meta.url),
);
...
function legacyAssetProbes(asset: LegacySdkAsset, projectDir: string, homeDir: string): string[] {
  ...
  return [
    ...
    join(projectDir, '.claude', 'evolv-coder-lite', 'bin', suffix),
    join(homeDir, '.claude', 'evolv-coder-lite', 'bin', suffix),
  ];
}
...
/**
 * Resolve the legacy `ecl-tools.cjs` executable path through the SDK Package Seam Module.
 *
...
export function resolveGsdToolsPath(projectDir: string, deps: LegacySdkCompatibilityDeps = {}): string {
  const resolution = probeLegacySdkAsset('ecl-tools', projectDir, deps);
...
function missingLegacyCoreMessage(resolution: LegacySdkAssetResolution): string {
  return [
    'state load: evolv-coder-lite/bin/lib/core.cjs not found.',
    `Checked: ${resolution.probes.join(', ')}`,
    'Install eCL (e.g. npm i -g @evolvconsulting/evolv-coder-lite) or clone with evolv-coder-lite next to the SDK.',
  ].join(' ');
}
