...
/**
 * Regression test for #3197 — ecl-tools config-set rejects workflow._auto_chain_active.
 *
 * Root cause: RUNTIME_STATE_KEYS was added to sdk/src/query/config-schema.ts in #3162
 * but not to evolv-coder-lite/bin/lib/config-schema.cjs, so ecl-tools.cjs users still hit
...
describe('#3197 — ecl-tools.cjs config-set workflow._auto_chain_active', () => {
  test('config-set workflow._auto_chain_active true succeeds via ecl-tools.cjs (CJS path)', (t) => {
