...
} = require(path.join(ROOT, 'evolv-coder-lite', 'bin', 'lib', 'runtime-name-policy.cjs'));
...
describe('runtime-name-policy windsurf alias parity — manifest vs FALLBACK_ALIASES (#792)', () => {
  ...
  const manifestPath = path.join(ROOT, 'evolv-coder-lite', 'bin', 'shared', 'runtime-aliases.manifest.json');
