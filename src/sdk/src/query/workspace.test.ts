...
describe('resolveWorkspaceContext', () => {
  ...
  it('reads ECL_WORKSTREAM from env', () => {
    process.env['ECL_WORKSTREAM'] = 'backend';
    const ctx = resolveWorkspaceContext();
    expect(ctx.workstream).toBe('backend');
  });

  it('reads ECL_PROJECT from env', () => {
    process.env['ECL_PROJECT'] = 'api-server';
    const ctx = resolveWorkspaceContext();
    expect(ctx.project).toBe('api-server');
  });

  it('reads both vars when both are set', () => {
    process.env['ECL_WORKSTREAM'] = 'ws1';
    process.env['ECL_PROJECT'] = 'proj1';
    const ctx = resolveWorkspaceContext();
