...
> Por que o eCL Core existe e qual problema ele foi projetado para resolver.
...
Considere o que isso significa na prática. Quando você executa `/ecl-plan-phase`, o orquestrador:
...
A engenharia de contexto por si só não é suficiente. Se um agente começa do zero mas recebe instruções vagas, ele vai produzir saídas vagas. O eCL Core combina subagentes com contexto limpo com duas disciplinas complementares:
...
**Meta-prompting** significa que as próprias definições de agentes são prompts cuidadosamente engenheirados, não instruções ad-hoc. Os arquivos em `evolv-coder-lite/workflows/` e `agents/` codificam conhecimento conquistado a duras penas sobre como delimitar tarefas, o que verificar e quando escalar para um checkpoint humano. O usuário não precisa reexplicar esse conhecimento a cada sessão; ele está integrado aos próprios prompts do sistema.
...
A engenharia de contexto exige que o conhecimento sobreviva a reinicializações de contexto. O eCL Core usa o sistema de arquivos para isso. Toda saída significativa é escrita em `.planning/` como Markdown ou JSON legível por humanos. Isso significa que:
...
**Sobrecarga.** O ciclo de fases introduz atrito real. Executar `/ecl-discuss-phase`, `/ecl-plan-phase` e `/ecl-execute-phase` como etapas separadas leva mais tempo que digitar "escreva esse recurso" em uma sessão simples. Para uma mudança pequena e bem compreendida, essa sobrecarga não se justifica.
