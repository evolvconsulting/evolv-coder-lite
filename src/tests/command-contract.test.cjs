// allow-test-rule: source-text-is-the-product — commands/ecl/*.md files ARE the
 ...
 *
 * Authoritative behavioral contract for every commands/ecl/*.md file.
 ...
 *
 * Contract:
 *   1. name:          present, non-empty, starts with ecl: or ecl-
...
const ECL_ROOT     = path.join(ROOT, 'evolv-coder-lite');
...
describe('command contract: name field (ADR-0002)', () => {
  for (const { name, full } of commandFiles) {
    test(`${name}: name: present and starts with ecl: or ecl-`, () => {
      const fm = parseFrontmatter(fs.readFileSync(full, 'utf-8'));
      assert.ok(fm.name && fm.name.trim(), `${name}: name: field missing or empty`);
      assert.ok(
        /^ecl[:-]/.test(fm.name.trim()),
        `${name}: name: must start with "ecl:" or "ecl-", got "${fm.name.trim()}"`,
      );
    });
  }
});
  ...
  for (const { name, full } of commandFiles) {
    test(`${name}: all execution_context @-refs exist on disk`, () => {
      ...
      for (const { normalized } of refs) {
        assert.ok(
          fs.existsSync(path.join(ECL_ROOT, normalized)),
