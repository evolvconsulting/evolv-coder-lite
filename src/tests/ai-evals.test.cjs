...
/**
 * eCL AI Evals Framework Tests
 *
 * Validates the /ecl-ai-integration-phase + /ecl-eval-review contribution:
...
const COMMANDS_DIR   = path.join(REPO_ROOT, 'commands', 'ecl');
const WORKFLOWS_DIR  = path.join(REPO_ROOT, 'evolv-coder-lite', 'workflows');
const TEMPLATES_DIR  = path.join(REPO_ROOT, 'evolv-coder-lite', 'templates');
const REFERENCES_DIR = path.join(REPO_ROOT, 'evolv-coder-lite', 'references');
...
describe('COMMAND: ai-integration-phase and eval-review frontmatter', () => {
  ...
  for (const cmd of commands) {
    test(`${cmd}.md exists`, () => {
      ...
      assert.ok(fs.existsSync(p), `commands/ecl/${cmd}.md should exist`);
    });
  ...
  test('ai-integration-phase.md name is ecl:ai-integration-phase', () => {
    const content = fs.readFileSync(path.join(COMMANDS_DIR, 'ai-integration-phase.md'), 'utf-8');
    assert.ok(content.includes('name: ecl:ai-integration-phase'), 'ai-integration-phase command name mismatch');
  });

  test('eval-review.md name is ecl:eval-review', () => {
    const content = fs.readFileSync(path.join(COMMANDS_DIR, 'eval-review.md'), 'utf-8');
    assert.ok(content.includes('name: ecl:eval-review'), 'eval-review command name mismatch');
  });
});
...
describe('WORKFLOW: plan-phase.md AI nudge integration', () => {
  ...
  test('plan-phase.md references /ecl-ai-integration-phase nudge', () => {
    const content = fs.readFileSync(planPhasePath, 'utf-8');
    assert.ok(
...
describe('WORKFLOW: ai-integration-phase and eval-review workflow files', () => {
  ...
  test('ai-integration-phase.md orchestrates 4 agents', () => {
    ...
    for (const agent of ['ecl-framework-selector', 'ecl-ai-researcher', 'ecl-domain-researcher', 'ecl-eval-planner']) {
      assert.ok(content.includes(agent), `ai-integration-phase.md should reference ${agent}`);
    }
  });

  test('eval-review.md references ecl-eval-auditor', () => {
    const content = fs.readFileSync(path.join(WORKFLOWS_DIR, 'eval-review.md'), 'utf-8');
    assert.ok(content.includes('ecl-eval-auditor'), 'eval-review.md should reference ecl-eval-auditor');
  });
