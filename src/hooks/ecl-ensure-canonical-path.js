...
//
// ecl-ensure-canonical-path — SessionStart hook (#997)
//
// PROBLEM: eCL agents/commands/templates use markdown `@`-file-includes that
// hardcode the canonical path `@~/.claude/evolv-coder-lite/...` (references, workflows,
...
// `~/.claude/evolv-coder-lite/` is never created and every @-include resolves to
...
// set and a bundled `evolv-coder-lite/` tree found beneath it), ensure
// `~/.claude/evolv-coder-lite/` exists and its immutable subdirs (bin, contexts,
...
// are the directories `@~/.claude/evolv-coder-lite/<subdir>/...` includes point into.
 ...
 * ecl-check-update.js detectConfigDir), else falls back to ~/.claude. The
 * canonical eCL tree always lives at `<configDir>/evolv-coder-lite`.
 */
...
/**
 * Locate the bundled `evolv-coder-lite/` tree beneath a plugin root.
 *
 * Claude Code unpacks the package so the bundled tree sits at
 * `<pluginRoot>/evolv-coder-lite/`. Returns the absolute, realpath-normalised path to
...
function resolveBundledTree(pluginRoot) {
  ...
  const bundled = path.join(root, 'evolv-coder-lite');
  let bundledReal;
  try {
    ...
    // ENOTDIR if <pluginRoot>/evolv-coder-lite is absent, so no separate existence
  ...
  // root. A crafted symlink at <pluginRoot>/evolv-coder-lite pointing outside the root
...
/**
 * Ensure the canonical `~/.claude/evolv-coder-lite/` path exposes the bundled subdirs.
 *
...
function ensureCanonicalPath(opts = {}) {
  ...
  const canonicalDir = path.join(configDir, 'evolv-coder-lite');
  ...
  for (const sub of MANAGED_SUBDIRS) {
    ...
    if (existing) {
      ...
      // junctions, so this single predicate identifies every eCL-managed link.
      if (existing.isSymbolicLink()) {
        // A eCL-managed link that is stale or points elsewhere (e.g. previous
