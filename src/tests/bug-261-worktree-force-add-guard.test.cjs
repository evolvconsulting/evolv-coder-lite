...
const HOOK_PATH = path.join(__dirname, '..', 'hooks', 'ecl-workflow-guard.js');
...
function makeRepo(branch) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-bug-261-'));
  git(dir, ['init', '-q']);
...
describe('bug #261: workflow guard blocks forced git add on worktree-agent branches', () => {
  ...
  test('allows git add -f on worktree-agent branch when no eCL config exists', () => {
