...
function tmpDir(prefix) {
  return fs.mkdtempSync(path.join(os.tmpdir(), prefix || 'ecl-ip-'));
}

function createFixtureSkillsDir() {
  const tmp = createTempDir('ecl-stage-profile-');
...
function createFixtureAgentsDir() {
  const tmp = createTempDir('ecl-agents-profile-');
  for (const name of ['ecl-planner', 'ecl-executor', 'ecl-code-reviewer']) {
    fs.writeFileSync(path.join(tmp, `${name}.md`), `# ${name}\n`);
  }
  return tmp;
}
...
describe('stageSkillsForRuntimeAsSkills', () => {
  ...
  test('registers stagedDir in STAGED_DIRS after staging', (t) => {
    const src = createTempDir('ecl-rta-src-');
    let stagedDir;
    t.after(() => {
      cleanup(src);
    ...
    stagedDir = stageSkillsForRuntimeAsSkills(src, { skills: '*' }, converter, 'ecl-');
  ...
  test('non-existent srcCommandsDir returns srcCommandsDir unchanged', () => {
    const ghost = path.join(os.tmpdir(), 'ecl-rta-no-exist-' + Date.now());
    const converter = (content, _skillName) => content;
    const result = stageSkillsForRuntimeAsSkills(ghost, { skills: '*' }, converter, 'ecl-');
  ...
  test('skills Set filters: only matching stems land in stagedDir', (t) => {
    ...
    const converter = (content, _skillName) => content;
    stagedDir = stageSkillsForRuntimeAsSkills(src, { skills: new Set(['phase']) }, converter, 'ecl-');
    const entries = fs.readdirSync(stagedDir).sort();
    assert.deepStrictEqual(entries, ['ecl-phase']);
  });
  ...
  test('skills === "*" stages all md files as <prefix><stem>/SKILL.md', (t) => {
    ...
    stagedDir = stageSkillsForRuntimeAsSkills(src, { skills: '*' }, converter, 'ecl-');
    const entries = fs.readdirSync(stagedDir).sort();
    assert.deepStrictEqual(entries, ['ecl-alpha', 'ecl-beta', 'ecl-gamma']);
    for (const name of ['alpha', 'beta', 'gamma']) {
      const content = fs.readFileSync(path.join(stagedDir, `ecl-${name}`, 'SKILL.md'), 'utf8');
...
describe('stageSkillsForProfile', () => {
  ...
  test('non-existent srcDir returns srcDir unchanged', () => {
    const ghost = path.join(os.tmpdir(), 'ecl-no-exist-' + Date.now());
...
describe('stageAgentsForProfile', () => {
  ...
  test('non-full profile with agents Set copies only member agent files', (t) => {
    ...
    const agents = new Set(['ecl-planner']);
    staged = stageAgentsForProfile(src, { skills: new Set(['plan-phase']), agents });
    const files = fs.readdirSync(staged).sort();
    assert.deepStrictEqual(files, ['ecl-planner.md']);
  });

  test('non-existent srcAgentsDir returns srcAgentsDir unchanged', () => {
    const ghost = path.join(os.tmpdir(), 'ecl-agents-no-exist-' + Date.now());
  ...
  test('standard profile — stageAgentsForProfile copies exactly the agents in resolvedProfile.agents', (t) => {
    ...
    assert.ok(resolved.agents instanceof Set && resolved.agents.size > 0,
      'standard profile must have >0 agents (plan-phase calls ecl-planner etc)');
    let staged;
    t.after(() => {
      if (staged) cleanupStagedSkills();
    });
...
describe('resolveProfile', () => {
  ...
  test('standard — agents Set is non-empty; ecl-planner and ecl-plan-checker present', () => {
    const manifest = loadSkillsManifest(REAL_COMMANDS_DIR);
    ...
    assert.ok(result.agents.has('ecl-planner'), 'standard should include ecl-planner');
    assert.ok(result.agents.has('ecl-plan-checker'), 'standard should include ecl-plan-checker');
  });
  ...
  test('agents are derived from real manifest body text (ecl-planner from plan-phase)', () => {
    const realManifest = loadSkillsManifest(REAL_COMMANDS_DIR);
    const result = resolveProfile({ modes: ['standard'], manifest: realManifest });
    assert.ok(result.agents.has('ecl-planner'), 'ecl-planner should be derived from plan-phase body');
  });

  test('agents transitively closed — plan-phase in standard brings its agents', () => {
    const manifest = loadSkillsManifest(REAL_COMMANDS_DIR);
    const result = resolveProfile({ modes: ['standard'], manifest });
    assert.ok(result.agents.has('ecl-planner'));
  });
});
...
describe('loadSkillsManifest', () => {
  test('returns a Map', () => {
    const dir = tmpDir('ecl-manifest-fixture-');
    try {
  ...
  test('skill with no requires: frontmatter maps to empty array', () => {
    const dir = tmpDir('ecl-manifest-fixture-');
    try {
      writeSkill(dir, 'help', 'name: ecl:help\ndescription: Help text');
      const m = loadSkillsManifest(dir);
  ...
  test('skill with requires: single value maps to array of one', () => {
    const dir = tmpDir('ecl-manifest-fixture-');
    try {
      writeSkill(dir, 'add-tests', 'name: ecl:add-tests\ndescription: Add tests\nrequires: [phase]');
      const m = loadSkillsManifest(dir);
  ...
  test('skill with requires: multiple values maps to full array', () => {
    const dir = tmpDir('ecl-manifest-fixture-');
    try {
      writeSkill(dir, 'plan-phase', 'name: ecl:plan-phase\ndescription: Plan\nrequires: [discuss-phase, phase, review, update]');
      const m = loadSkillsManifest(dir);
  ...
  test('skill with requires: empty array maps to empty array', () => {
    const dir = tmpDir('ecl-manifest-fixture-');
    try {
      writeSkill(dir, 'explore', 'name: ecl:explore\ndescription: Explore\nrequires: []');
      const m = loadSkillsManifest(dir);
  ...
  test('loads real commands/ecl/ directory: >=60 skills, discuss-phase deps correct, help has no requires', () => {
...
describe('readActiveProfile / writeActiveProfile', () => {
  ...
  test('non-existent directory returns null (not throws)', () => {
    const ghost = path.join(os.tmpdir(), 'ecl-marker-no-exist-' + Date.now());
  ...
  test('corrupt marker content (invalid chars) returns null', () => {
    const dir = tmpDir('ecl-marker-');
    try {
      fs.writeFileSync(path.join(dir, '.ecl-profile'), 'profile with spaces and !!!\n');
      const result = readActiveProfile(dir);
      assert.strictEqual(result, null);
    } finally {
      fs.rmSync(dir, { recursive: true, force: true });
  ...
  test('empty marker file returns null', () => {
    const dir = tmpDir('ecl-marker-');
    try {
      fs.writeFileSync(path.join(dir, '.ecl-profile'), '');
      const result = readActiveProfile(dir);
      assert.strictEqual(result, null);
    } finally {
      fs.rmSync(dir, { recursive: true, force: true });
  ...
  test('writeActiveProfile creates the directory if it does not exist', () => {
    const base = tmpDir('ecl-marker-base-');
