...
/**
 * eCL Tools Tests - autonomous --interactive flag
 *
...
describe('autonomous --interactive flag (#1413)', () => {
  const workflowPath = path.join(__dirname, '..', 'evolv-coder-lite', 'workflows', 'autonomous.md');
  const commandPath = path.join(__dirname, '..', 'commands', 'ecl', 'autonomous.md');
  ...
  test('workflow uses discuss-phase skill in interactive mode', () => {
    // Per #2697 the user-facing form is the hyphen invariant ecl-discuss-phase;
    ...
    // ecl-discuss-phase mention in prose) and falsely pass. Instead, isolate
    ...
    const skillCall = /Skill\(\s*skill\s*=\s*['"]ecl-discuss-phase['"]/.test(branch);
    assert.ok(
      skillCall,
      `INTERACTIVE branch must invoke Skill(skill="ecl-discuss-phase"). Got branch:\n${branch}`,
    );
  });
