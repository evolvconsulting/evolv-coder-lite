 ...
 * so `npx @evolvconsulting/evolv-coder-lite@latest --sdk` produced a misleading "✓ eCL SDK ready"
...
describe('bug #3033: --sdk flag (opts.forceSdk) must be wired into installSdkIfNeeded',
  { skip: isWindows ? 'POSIX-only: forces shebang ecl-sdk shim into ~/.local/bin and asserts mode 0o755' : false },
  () => {
  let tmpRoot;
  let sdkDir;
  let pathDir;
  ...
  beforeEach(() => {
    tmpRoot = createTempDir('ecl-3033-');
  ...
  test('forceSdk=true + isLocal=true + dist present: self-links ecl-sdk into PATH dir', () => {
    ...
    assert.ok(
      ...
      `forceSdk=true must materialize ecl-sdk shim at ${linkPath}. Output:\n${combined}`,
    );

    // Must report "eCL SDK ready" — not the legacy "Skipping SDK check" message.
    assert.ok(
      /eCL SDK ready/.test(combined),
      `forceSdk=true must print "eCL SDK ready" once shim is on PATH. Output:\n${combined}`,
    );
    assert.ok(
  ...
  test('throwOnFailure=true converts missing SDK dist into catchable error', () => {
    ...
    assert.throws(
      ...
      (error) => {
        assert.equal(error.code, 'ECL_SDK_MISSING_DIST');
