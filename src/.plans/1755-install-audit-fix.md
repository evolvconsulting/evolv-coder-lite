...
**Line 5485** — Change `ecl-update-check.js` to `ecl-check-update.js` and fix path from `evolv-coder-lite/hooks/` to `hooks/`.
**Line 5492** — Update dedup check to use `ecl-check-update`.

### Fix 3: Fix stale cache invalidation path (CRITICAL)
**Line 5406** — Change from `path.join(path.dirname(targetDir), 'cache', ...)` to `path.join(os.homedir(), '.cache', 'ecl', 'ecl-update-check.json')`.
...
### Fix 5: Add ecl-workflow-guard.js to uninstall hook list (MEDIUM)
**Line 4404** — Add `'ecl-workflow-guard.js'` to the `gsdHooks` array.

### Fix 6: Add community hooks to uninstall settings.json cleanup (MEDIUM)
**Lines 4453-4520** — Add filters for `ecl-session-state`, `ecl-validate-commit`, `ecl-phase-boundary` in the appropriate event cleanup blocks (SessionStart, PreToolUse, PostToolUse).

### Fix 7: Remove phantom ecl-check-update.sh from uninstall list (LOW)
**Line 4404** — Remove `'ecl-check-update.sh'` from `gsdHooks` array.
...
- uninstall removes ecl-workflow-guard.js
