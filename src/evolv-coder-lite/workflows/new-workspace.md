...
INIT=$($ECL_SDK query init.new-workspace)
...
Run this command from a directory containing git repos, or specify repos explicitly:
  /ecl:workspace --new --name my-workspace --repos /path/to/repo1,/path/to/repo2
```
Exit.
...
Usage:
  /ecl:workspace --new --name my-workspace --repos repo1,repo2 --auto
```
Exit.
...
Next steps:
  cd "$TARGET_PATH"
  /ecl:new-project    # Initialize eCL in the workspace
```
...
**Offer to initialize eCL (if not `--auto`):**

Use AskUserQuestion:
- header: "Initialize eCL"
- question: "Would you like to initialize a eCL project in the new workspace?"
- options:
  - "Yes — run /ecl:new-project" → tell user to `cd "$TARGET_PATH"` first, then run `/ecl:new-project`
