...
describe('workflow.discuss_mode config', () => {
  test('config template includes discuss_mode default', () => {
    const template = JSON.parse(
      fs.readFileSync(path.join(__dirname, '..', 'evolv-coder-lite', 'templates', 'config.json'), 'utf8')
    );
  ...
  test('discuss-phase command references both workflow files', () => {
    const command = fs.readFileSync(
      path.join(__dirname, '..', 'commands', 'ecl', 'discuss-phase.md'), 'utf8'
    );
  ...
  test('assumptions workflow file exists and has required steps', () => {
    const workflow = fs.readFileSync(
      path.join(__dirname, '..', 'evolv-coder-lite', 'workflows', 'discuss-phase-assumptions.md'), 'utf8'
    );
    const requiredSteps = [
  ...
  test('plan-phase gate references discuss_mode config', () => {
    const planPhase = fs.readFileSync(
      path.join(__dirname, '..', 'evolv-coder-lite', 'workflows', 'plan-phase.md'), 'utf8'
    );
  ...
  test('plan-phase command argument-hint includes --text', () => {
    const command = fs.readFileSync(
      path.join(__dirname, '..', 'commands', 'ecl', 'plan-phase.md'), 'utf8'
    );
  ...
  test('plan-phase init exposes text_mode in workflow flags', () => {
    const initSrc = fs.readFileSync(
      path.join(__dirname, '..', 'evolv-coder-lite', 'bin', 'lib', 'init.cjs'), 'utf8'
    );
  ...
  test('progress workflow references discuss_mode', () => {
    const progress = fs.readFileSync(
      path.join(__dirname, '..', 'evolv-coder-lite', 'workflows', 'progress.md'), 'utf8'
    );
  ...
  test('discuss-phase command mode-routing uses ecl_run (shim-safe) not bare ecl-tools', () => {
    const command = fs.readFileSync(
      path.join(__dirname, '..', 'commands', 'ecl', 'discuss-phase.md'), 'utf8'
    );
    ...
    // Must use ecl_run for the config lookup
    assert.ok(
      command.includes('ecl_run query config-get workflow.discuss_mode'),
      'discuss-phase.md must use ecl_run (not bare ecl-tools) for discuss_mode lookup'
    );
    // Must NOT contain the bare footgun pattern: ecl-tools immediately before the silent default
    assert.ok(
      !command.includes('ecl-tools query config-get workflow.discuss_mode 2>/dev/null || echo'),
      'discuss-phase.md must NOT use bare ecl-tools binary for discuss_mode lookup (shim-only install footgun)'
    );
  });
});
