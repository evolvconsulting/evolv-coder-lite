...
/**
 * Tests for ecl-read-guard.js PreToolUse hook.
 *
...
const HOOK_PATH = path.join(__dirname, '..', 'hooks', 'ecl-read-guard.js');
...
describe('ecl-read-guard hook', () => {
  let tmpDir;

  beforeEach(() => {
    tmpDir = createTempDir('ecl-read-guard-');
  });
  ...
  test('hook is registered in build-hooks.js HOOKS_TO_COPY', () => {
    ...
    assert.ok(
      content.includes('ecl-read-guard.js'),
      'ecl-read-guard.js must be in HOOKS_TO_COPY so it ships in hooks/dist/'
    );
  });
  ...
  test('hook is registered in install.js uninstall hook list', () => {
    ...
    assert.ok(
      content.includes("'ecl-read-guard.js'"),
      'ecl-read-guard.js must be in the uninstall gsdHooks list'
    );
  });
