...
Resolve and re-run /ecl execute-phase, or override with
/ecl execute-phase {phase} --force-mvp-gate to ship anyway.
```
...
**Invoke review:**
```
Skill(skill="ecl-code-review", args="${PHASE_NUMBER}")
```
...
Check for override:
```bash
SKIP_SCHEMA=$(echo "${ECL_SKIP_SCHEMA_CHECK:-false}")
```
...
Display:
```
⚠ Schema drift detected but ECL_SKIP_SCHEMA_CHECK=true — bypassing gate.
...
2. Skip schema check (ECL_SKIP_SCHEMA_CHECK=true) — bypass this gate
...
`evolv-coder-lite/workflows/execute-phase/steps/codebase-drift-gate.md` —
