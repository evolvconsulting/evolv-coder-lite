...
- **Issue:** [evolvconsulting/evolv-coder-lite#2302](https://github.com/evolvconsulting/evolv-coder-lite/issues/2302) — Phase 3 SDK query parity, registry, docs (umbrella #2007).
- **Workspace:** `D:\Repos\evolv-coder-lite` (PBR backport). **Upstream:** `evolvconsulting/evolv-coder-lite`. Confirm branch with `git branch` (typical: `feat/sdk-phase3-query-layer`).
...
| Role                                    | GitHub                                                                                 | Notes                                                                                                                                                                                                                                       |
| --------------------------------------- | -------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Product / requirements anchor**       | [#2007](https://github.com/evolvconsulting/evolv-coder-lite/issues/2007)                        | Problem statement, user stories, and target architecture for the SDK-first migration. **Do not** treat its original acceptance-checklist boxes as proof of what is merged upstream; work was split into phased PRs after maintainer review. |
| **Phase 3 execution scope**             | [#2302](https://github.com/evolvconsulting/evolv-coder-lite/issues/2302) **+ this handover**    | What this branch is actually doing now: registry/CLI parity, docs, harness gaps, runner alignment follow-ups as listed below.                                                                                                               |
| **Patch mine (if local tree is short)** | [PR #2008](https://github.com/evolvconsulting/evolv-coder-lite/pull/2008) and matching branches | Large pre-phasing PR; cherry-pick or compare when something looks missing vs that line of work.                                                                                                                                             |
...
- **”Golden parity: coverage and exceptions”** — How `golden.integration.test.ts` compares SDK vs `ecl-tools.cjs` (full `toEqual`, subset, normalized `docs-init`, `intel.update` CJS parity, time-dependent fields, etc.).
- **”CJS command surface vs SDK registry”** — Naming aliases, CLI-only rows, SDK-only rows, and a **top-level `ecl-tools` command → SDK** matrix.
...
### 2. `ecl-sdk query` tokenization (`normalizeQueryCommand`)

- **Problem:** `ecl-sdk query` used only argv[0] as the registry key, so `query state json` dispatched `state` (unregistered) instead of `state.json`.
- **Fix:** `sdk/src/query/normalize-query-command.ts` merges the same **command + subcommand** patterns as `ecl-tools` `runCommand()` (e.g. `state json` → `state.json`, `init execute-phase 9` → `init.execute-phase`, `scaffold …` → `phase.scaffold`, `progress bar` → `progress.bar`). Wired in `sdk/src/cli.ts` before `registry.dispatch()`.
- **Tests:** `sdk/src/query/normalize-query-command.test.ts`.

### 3. `phase add-batch` in the registry

- **Implementation:** `phaseAddBatch` in `sdk/src/query/phase-lifecycle.ts` — port of `cmdPhaseAddBatch` from `evolv-coder-lite/bin/lib/phase.cjs` (batch append under one roadmap lock; sequential or `phase_naming: custom`).
...
- `stateProjectLoad` — `sdk/src/query/state-project-load.ts`; registry key `state.load`. Uses `createRequire` to call `core.cjs` `loadConfig(projectDir)` from the same resolution paths as a normal install (bundled monorepo path, `projectDir/.claude/evolv-coder-lite/...`, `~/.claude/evolv-coder-lite/...`). `GSDTools.stateLoad()` and `formatRegistryRawStdout` for `--raw` no longer force a subprocess solely for this command.
- **Risk:** If `core.cjs` is absent (e.g. some `@evolvconsulting/ecl-sdk`-only layouts), `state.load` throws `GSDError` — document; future option is a TS `loadConfig` port or bundling.
...
| Status                   | Surface                                                                                          |
| ------------------------ | ------------------------------------------------------------------------------------------------ |
| **Registered**           | Essentially all `ecl-tools.cjs` `runCommand` surfaces, including `phase.add-batch`.          |
| **CLI-only (by design)** | `graphify`, `from-ecl2` — not in `createRegistry()`; documented in `QUERY-HANDLERS.md`.  |
| **SDK-only extra**       | `phases.archive` — no `ecl-tools phases archive` subcommand (CJS has `list` / `clear` only). |


**Programmatic API:** `createRegistry()` / `registry.dispatch('dotted.name', args, projectDir)`.

**CLI:** `ecl-sdk query …` — apply `normalizeQueryCommand` semantics (or pass dotted names explicitly).

**Still not unified:** `GSDTools` (`sdk/src/ecl-tools.ts`) shells out to `ecl-tools.cjs` for plan/session flows; migrating callers to the registry is separate #2302 / runner work. `state load` is **not** among the subprocess-only exceptions anymore (it uses the registry like other native query handlers when native query is active).
...
| `sdk/src/cli.ts`                            | `ecl-sdk query` path (uses `normalizeQueryCommand`).                                   |
...
Work that moves **deterministic** orchestration out of AI/bash and into **SDK queries** (historically `ecl-tools.cjs`) has **two layers**. Do not confuse them:


| Layer                    | Goal                                                                                                                                                                         | What “done” looks like                                                                  |
| ------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------- |
| **Parity / migration**   | Existing CLI behavior is **stable and testable** in the registry so callers can use `ecl-sdk query` instead of `node …/ecl-tools.cjs` without silent drift.                  | Goldens + `QUERY-HANDLERS.md`; same JSON/`--raw` contracts as CJS.                      |
...
### Decision-routing audit (proposed `ecl-tools` / SDK queries)
...
|---|------------------|------|--------|
| 3.1 | `route next-action` | **1** | Next slash-command from `/ecl-next`-style routing. |
...
**SDK registry (Tier 1):** **Done** — `check.config-gates`, `check.phase-ready`, `route.next-action` in `createRegistry()` (`sdk/src/query/index.ts`). Documented in `sdk/src/query/QUERY-HANDLERS.md` § Decision routing (**SDK-only** until/unless mirrored in `ecl-tools.cjs`).

**Simple roadmap (execute in order):**

1. **Harden parity** for surfaces workflows already depend on (registry dispatch, goldens, docs) so swaps from CJS to `ecl-sdk query` stay safe.
...
2. ~~**Re-export `normalizeQueryCommand`**~~ — Done: exported from `sdk/src/query/index.ts` and `sdk/src/index.ts` (`@evolvconsulting/ecl-sdk`).
3. **Issue #2302 follow-ups** — Runner alignment (`GSDTools` → registry where appropriate). **`configGet`** now uses `dispatchNativeJson` with canonical `config-get` (fixes subprocess argv vs real `ecl-tools.cjs`, which has no `config` + `get` top-level). Keep `graphify` / `from-ecl2` out of scope unless product reopens.
...
- `ecl-sdk query` understands two-token command patterns like `ecl-tools`.
- `phase add-batch` implemented and registered; **only** intentional CLI-only gaps remain (**graphify**, **from-ecl2**).
