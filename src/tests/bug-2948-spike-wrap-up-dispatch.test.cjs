 ...
 *
 * `/ecl:spike --wrap-up` was silently no-oping because:
 * 1. `commands/ecl/spike.md` listed `--wrap-up` as a flag but had no dispatch block.
 * 2. `workflows/spike.md` still referenced the deleted `/ecl-spike-wrap-up` entry-point
 *    instead of the correct `/ecl:spike --wrap-up` form.
 *
 * Fix:
 * - `commands/ecl/spike.md` now has a dispatch block that routes `--wrap-up` to
 *   spike-wrap-up.md, and spike-wrap-up.md is listed in execution_context so the
 *   runtime can find it.
 * - `workflows/spike.md` companion references updated from `/ecl-spike-wrap-up` to
 *   `/ecl:spike --wrap-up`.
 */
...
const SPIKE_CMD_PATH = path.join(__dirname, '..', 'commands', 'ecl', 'spike.md');
const SPIKE_WORKFLOW_PATH = path.join(__dirname, '..', 'evolv-coder-lite', 'workflows', 'spike.md');
...
describe('bug-2948: /ecl:spike --wrap-up dispatch wiring', () => {
  describe('commands/ecl/spike.md — frontmatter and section contract', () => {
    test('spike.md command file exists and has valid frontmatter', () => {
      assert.ok(fs.existsSync(SPIKE_CMD_PATH), 'commands/ecl/spike.md should exist');
  ...
  describe('evolv-coder-lite/workflows/spike.md — companion references', () => {
    test('spike workflow file exists', () => {
      assert.ok(fs.existsSync(SPIKE_WORKFLOW_PATH), 'evolv-coder-lite/workflows/spike.md should exist');
    });

    test('does NOT reference the deleted /ecl-spike-wrap-up entry-point', () => {
      const fm = parseFrontmatter(fs.readFileSync(SPIKE_WORKFLOW_PATH, 'utf-8'));
      assert.ok(
        !fm._body.includes('/ecl-spike-wrap-up'),
        'workflows/spike.md must not reference the deleted /ecl-spike-wrap-up command; use /ecl:spike --wrap-up instead'
      );
    });

    test('references /ecl:spike --wrap-up as the canonical wrap-up invocation', () => {
      const fm = parseFrontmatter(fs.readFileSync(SPIKE_WORKFLOW_PATH, 'utf-8'));
      assert.ok(
        fm._body.includes('/ecl:spike --wrap-up'),
        'workflows/spike.md must reference /ecl:spike --wrap-up as the canonical wrap-up command'
      );
    });
  });
});
