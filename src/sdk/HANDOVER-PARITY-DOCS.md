...
1. **Parity “exceptions” documentation** — A clear, maintainable description of where **full JSON equality** between `ecl-tools.cjs` and `createRegistry()` is **not** expected or not attempted, and why (stubs, structural-only tests, environment-dependent fields, ordering, etc.). Map this to **#2007 / #2302** expectations: no *undocumented* gap.
2. **CJS-only matrix** — A **single authoritative table**: each relevant `ecl-tools.cjs` surface (top-level command or documented cluster) → **registered in SDK** vs **permanent CLI-only** vs **alias / naming difference**, with a **one-line justification** where not registered.

## Parent tracking

- **Issue:** [evolvconsulting/evolv-coder-lite#2302](https://github.com/evolvconsulting/evolv-coder-lite/issues/2302) — Phase 3 SDK query parity, registry, docs (parent umbrella #2007).
...
- `sdk/src/golden/capture.ts` — `captureGsdToolsOutput()` spawns `evolv-coder-lite/bin/ecl-tools.cjs`.
...
Build the table by **diffing** `evolv-coder-lite/bin/ecl-tools.cjs` `switch (command)` top-level cases against `createRegistry()` registrations in `sdk/src/query/index.ts`.

**Already documented as product-out-of-scope for registry:** **graphify**, **from-ecl2** / **ecl2-import**.
...
- Adding `@deprecated` headers to `ecl-tools.cjs` — separate task.
...
- A reader can answer: **“Which `ecl-tools` top-level commands are not registered and why?”** from one table.
