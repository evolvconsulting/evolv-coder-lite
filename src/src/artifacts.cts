/**
 * Canonical eCL artifact registry (ADR-457 build-at-publish: the hand-written
 ...
 * are added.
 *
 * Enumerates the file names that ecl workflows officially produce at the
 * .planning/ root level. Used by ecl-health (W019) to flag unrecognized files
...
export const CANONICAL_PATTERNS: ReadonlyArray<RegExp> = [
  /^v\d+\.\d+(?:\.\d+)?-MILESTONE-AUDIT\.md$/i, // ecl-complete-milestone (pre-archive)
