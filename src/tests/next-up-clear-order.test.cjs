...
const CONTINUATION_FORMAT = path.join(ECL_ROOT, 'references', 'continuation-format.md');
const WORKFLOWS_DIR = path.join(ECL_ROOT, 'workflows');
...
describe('workflow files — no <sub>/clear patterns in Next Up blocks', () => {
  ...
  test('no workflow file contains <sub> with /clear', () => {
    ...
    for (const filePath of workflowFiles) {
      ...
      if (matches) {
        failures.push({
          file: path.relative(ECL_ROOT, filePath),
...
describe('reference files — no <sub>/clear patterns', () => {
  const referencesDir = path.join(ECL_ROOT, 'references');
