...
// `.md` bodies. Those files ARE the runtime contract that eCL loads into agent
...
// `ecl-tools commit` against a temp project and assert on its structured
// JSON return envelope plus the git index state. No raw-text matching on
// rendered output.

/**
 * Regression for #3678 — ecl-executor force-commits .planning/ files when
 * commit_docs is false.
 *
 * Root cause: the executor agent prompt (agents/ecl-executor.md) tells the
 * agent to call `ecl-sdk query commit "docs(...)" --files .planning/...`
 ...
 *      workflow bodies entirely (no eCL-managed surface should force-stage
 *      gitignored content).
 */
...
describe('bug #3678 — executor must respect commit_docs:false', () => {
  ...
  describe('A — agent prompt teaches the agent how to handle commit_docs:false', () => {
    test('A1: agent body explicitly references the SDK skipped envelope', () => {
      ...
      assert.ok(
        ...
        'agents/ecl-executor.md must teach the agent how to recognize the '
        + 'skipped envelope from `ecl-sdk query commit` (one of: '
    ...
    test('A2: agent body explicitly forbids raw git fallback when SDK skips', () => {
      ...
      assert.ok(
        ...
        'agents/ecl-executor.md must contain an explicit "do not fall back to '
  ...
  describe('B — SDK behavior: commit_docs:false leaves repo state untouched', () => {
    ...
    test('B1: commit returns committed:false with skipped envelope', () => {
      ...
      assert.ok(result.success, `ecl-tools commit should exit 0 even when skipped: ${result.error || ''}`);
  ...
  test('checklist carve-out preserved for intentional skip', () => {
    ...
    assert.ok(
      checklistLine,
      'agents/ecl-executor.md must contain a "Final metadata commit made" checklist line',
    );
    assert.ok(
  ...
  describe('C — structural ban on raw force-add in eCL-managed bodies', () => {
    ...
    test('C2: no workflow body contains `git add -f` / `git add --force`', () => {
      const offenders = scanForForceAdd(path.join(REPO_ROOT, 'evolv-coder-lite', 'workflows'));
      assert.deepStrictEqual(
        offenders,
        [],
