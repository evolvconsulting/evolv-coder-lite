# Referência de Configuração do eCL
...
O eCL armazena as configurações do projeto em `.planning/config.json`. Criado durante `/ecl-new-project`, atualizado via `/ecl-settings`.
...
| `phase_id_convention` | enum | `"milestone-prefixed"`, `null` | `null` | Convenção de nomenclatura para IDs de fase. `null` = IDs numéricos legados (`Phase 1`, `Phase 2`). `"milestone-prefixed"` = IDs globalmente únicos que codificam o marco envolvente (`Phase 1-01`, `Phase 1-02`). Execute `ecl-tools roadmap upgrade --convention milestone-prefixed` para migrar um ROADMAP.md existente. |
...
| `brave_search` | boolean | `true`/`false` | detectado automaticamente | Substitui a detecção automática de disponibilidade da API Brave Search. Quando não definido, o eCL verifica a variável de ambiente `BRAVE_API_KEY` ou o arquivo `~/.ecl/brave_api_key` |
| `firecrawl` | boolean | `true`/`false` | detectado automaticamente | Substitui a detecção automática de disponibilidade da API Firecrawl. Quando não definido, o eCL verifica a variável de ambiente `FIRECRAWL_API_KEY` ou o arquivo `~/.ecl/firecrawl_api_key` |
| `exa_search` | boolean | `true`/`false` | detectado automaticamente | Substitui a detecção automática de disponibilidade da API Exa Search. Quando não definido, o eCL verifica a variável de ambiente `EXA_API_KEY` ou o arquivo `~/.ecl/exa_api_key` |
...
Configuradas interativamente via [`/ecl-config --integrations`](COMMANDS.md#ecl-config). Estas são configurações de *conectividade* — chaves de API e roteamento entre ferramentas — e são mantidas intencionalmente separadas de `/ecl-settings` (controles de fluxo de trabalho).

### Chaves de API de Busca

Os campos de chave de API aceitam um valor string (a própria chave). Também podem ser definidos como os valores especiais `true`/`false`/`null` para substituir a detecção automática de variáveis de ambiente / arquivos `~/.ecl/*_api_key` (comportamento legado, consulte as linhas acima).
...
### Revisores Padrão para `/ecl-review`

Use `review.default_reviewers` para limitar a execução de `/ecl-review` sem flags a um subconjunto de revisores detectados.

| Configuração | Tipo | Padrão | Descrição |
|---------|------|---------|-------------|
| `review.default_reviewers` | string[] \| null | `null` (todos os revisores detectados) | Subconjunto padrão opcional para `/ecl-review` sem flags, por exemplo `["gemini","codex"]`. Precedência: flags de revisor explícitas > `--all` > `review.default_reviewers` > todos detectados. Slugs desconhecidos são ignorados com aviso; slugs conhecidos mas não detectados são ignorados com uma nota informativa; arrays vazios são rejeitados pelo `config-set`. |
...
`agent_skills.<agent-type>` estende o mapa `agent_skills` documentado abaixo. O slug é validado contra `[a-zA-Z0-9_-]+` — sem separadores de caminho, sem espaços em branco, sem metacaracteres shell. Configurado interativamente via `/ecl-config --integrations`.
...
| `workflow.ui_safety_gate` | boolean | `true` | Solicita executar /ecl-ui-phase para fases de frontend durante a fase de planejamento |
| `workflow.ui_review` | boolean | `true` | Executa auditoria de qualidade visual (`/ecl-ui-review`) após execução de fase no modo autônomo. Quando `false`, o passo de auditoria de UI é ignorado. |
...
| `workflow.discuss_mode` | string | `'discuss'` | Controla como `/ecl-discuss-phase` coleta contexto. `'discuss'` (padrão) faz perguntas uma a uma. `'assumptions'` lê a base de código primeiro, gera premissas estruturadas com níveis de confiança e só pede para corrigir o que está errado. Adicionado na v1.28 |
...
| `workflow.code_review` | boolean | `true` | Habilita os comandos `/ecl-code-review` e `/ecl-code-review --fix`. Quando `false`, os comandos saem com uma mensagem de gate de configuração. Adicionado na v1.34 |
| `workflow.code_review_depth` | string | `standard` | Profundidade de revisão padrão para `/ecl-code-review`: `quick` (somente correspondência de padrão), `standard` (análise por arquivo) ou `deep` (entre arquivos com grafos de importação). Pode ser substituído por execução com `--depth=`. Adicionado na v1.34 |
...
| `workflow.human_verify_mode` | string | `'end-of-phase'` | Controla os pontos de verificação humana. `'end-of-phase'` (padrão desde #3309) suprime as tasks `checkpoint:human-verify` e incorpora verificações nos blocos `<verify><human-check>` para revisão ao final da fase. `'mid-flight'` restaura as tasks de checkpoint bloqueantes. `checkpoint:decision` e `checkpoint:human-action` não são afetados. Consulte [Referência de Checkpoints](../../evolv-coder-lite/references/checkpoints.md#checkpoint_types). |
...
| `workflow.ai_integration_phase` | boolean | `true` | Habilita o comando `/ecl-ai-integration-phase`. Quando `false`, o comando sai com uma mensagem de gate de configuração |
...
| `workflow.pattern_mapper` | boolean | `true` | Executa o agente `ecl-pattern-mapper` entre pesquisa e planejamento para mapear novos arquivos para análogos existentes na base de código |
...
| `workflow.drift_threshold` | number | `3` | Número mínimo de novos elementos estruturais (novos diretórios, exportações barrel, migrações, módulos de rota) introduzidos durante uma fase antes que o gate de deriva pós-execução da base de código tome ação. Consulte [#2003](https://github.com/evolvconsulting/evolv-coder-lite/issues/2003). Adicionado na v1.39 |
| `workflow.drift_action` | string | `warn` | O que fazer quando `workflow.drift_threshold` é excedido após `/ecl-execute-phase`. `warn` imprime uma mensagem sugerindo `/ecl-map-codebase --paths …`; `auto-remap` gera `ecl-codebase-mapper` com escopo para os caminhos afetados. Adicionado na v1.39 |
...
O namespace `code_quality.*` controla ferramentas opcionais de análise estrutural que complementam `/ecl-code-review`. As configurações são aditivas: cada ferramenta é habilitada independentemente e está desativada por padrão.

| Configuração | Tipo | Padrão | Descrição |
|---------|------|---------|-------------|
| `code_quality.fallow.enabled` | boolean | `false` | Habilita a pré-passagem estrutural fallow para `/ecl-code-review`. Quando `false`, nenhuma sondagem de binário fallow ou artefato JSON é produzido. |
...
`ship.pr_body_sections` adiciona seções adicionais ao corpo do PR para conteúdo de PRD/corpo do PR específico do projeto em `/ecl-ship` sem editar `evolv-coder-lite/workflows/ship.md`.
...
Pelo menos um de `source`, `template` ou `fallback` é obrigatório para cada seção. O padrão é `[]`, portanto projetos existentes mantêm sua saída atual de `/ecl-ship` até que a integração adicione entradas habilitadas.
...
| `planning.sub_repos` | array de strings | `[]` | Caminhos de sub-repositórios aninhados relativos à raiz do projeto. Quando definido, as ferramentas com reconhecimento de eCL limitam a busca de fase, resolução de caminho e operações de commit por sub-repo em vez de tratar o repositório externo como um monorepo |

### Resolução da Raiz do Projeto em Workspaces Multi-Repositório

Quando `sub_repos` está definido e `ecl-tools.cjs` ou `ecl-tools query` é invocado de dentro de um repositório filho listado, ambas as CLIs sobem até o workspace pai que possui `.planning/` antes de despachar os manipuladores. Ordem de resolução (verificada em cada ancestral até 10 níveis, nunca acima de `$HOME`):
...
| `hooks.workflow_guard` | boolean | `false` | Avisa quando edições de arquivo ocorrem fora do contexto do fluxo de trabalho eCL (aconselha usar `/ecl-quick` ou `/ecl-fast`) |
...
O hook guardião de injeção de prompt (`ecl-prompt-guard.js`) está sempre ativo e não pode ser desabilitado — é uma funcionalidade de segurança, não um controle de fluxo de trabalho.

### Configuração de Planejamento Privado

Quando `planning.commit_docs` é `false` e `.planning/` está listado em `.gitignore`, o eCL trata os artefatos de planejamento como locais apenas. `planning.search_gitignored: true` garante que buscas amplas ainda incluam o diretório `.planning/` nesta configuração. Consulte [Configurar planejamento privado](how-to/configure-model-profiles.md) para os passos de configuração.

---

## Injeção de Habilidades de Agente

Injeta arquivos de habilidades personalizados nos prompts de subagentes eCL. As habilidades são lidas pelos agentes no momento do spawn, fornecendo instruções específicas do projeto além do que o CLAUDE.md oferece.
...
Qualquer tipo de agente eCL pode receber habilidades. Tipos comuns:

- `ecl-executor` -- executa planos de implementação
- `ecl-planner` -- cria planos de fase
- `ecl-checker` -- verifica a qualidade do plano
- `ecl-verifier` -- verificação pós-execução
- `ecl-researcher` -- pesquisa de fase
- `ecl-project-researcher` -- pesquisa de novo projeto
- `ecl-debugger` -- agentes de diagnóstico
- `ecl-codebase-mapper` -- análise da base de código
- `ecl-advisor` -- consultores da fase de discussão
- `ecl-ui-researcher` -- criação de contrato de design de UI
- `ecl-ui-checker` -- verificação de especificação de UI
- `ecl-roadmapper` -- criação de roadmap
- `ecl-synthesizer` -- síntese de pesquisa

### Como Funciona

No momento do spawn, os fluxos de trabalho chamam `ecl-tools query agent-skills <type>` (ou o legado `node ecl-tools.cjs agent-skills <type>`) para carregar as habilidades configuradas. Se existirem habilidades para o tipo de agente, elas são injetadas como um bloco `<agent_skills>` no prompt de Task():
...
| `intel.enabled` | boolean | `false` | Habilita o sistema de inteligência consultável da base de código. Quando `true`, os comandos `/ecl-map-codebase --query` constroem e consultam um índice JSON em `.planning/intel/`. Adicionado na v1.34 |
...
| Configuração | Tipo | Padrão | Descrição |
|---------|------|---------|-------------|
| `plan_review.source_grounding` | boolean | `true` | Habilita o guardião de deriva de plano. Quando `true` (padrão), a revisão de plano resolve cada referência de símbolo citada em um PLAN.md em relação à árvore de fontes ativa. Planos que citam uma função, classe, decorador ou flag CLI inexistente produzem um aviso `needs-acknowledgement` antes do plano ser aprovado. Desabilite com `false` para ignorar completamente a verificação de símbolo. Ative durante a configuração (`/ecl:new-project`) ou a qualquer momento via `/ecl:settings`. |
| `plan_review.source_grounding_authority` | enum | `grep` | Seleciona o adaptador de resolução usado para verificar a existência de símbolos. Valores permitidos: `grep` (padrão — busca ripgrep/grep de arquivos de fonte, funciona em qualquer projeto sem ferramental adicional), `intel` (consulta o índice `.planning/intel/api-map.json` construído por `/ecl:map-codebase`; requer `intel.enabled: true`), `treesitter` (reservado para adaptador tree-sitter futuro), `lsp` (reservado para adaptador LSP futuro), `scip` (reservado para adaptador SCIP/LSIF futuro). Use `intel` quando tiver executado `/ecl:map-codebase` e quiser a busca mais rápida e pré-indexada. Todos os outros valores além de `grep` e `intel` são reservados e não têm efeito na versão atual. |
...
| `git.phase_branch_template` | string | `ecl/phase-{phase}-{slug}` | Template de nome de branch para estratégia de fase |
| `git.milestone_branch_template` | string | `ecl/{milestone}-{slug}` | Template de nome de branch para estratégia de marco |
| `git.quick_branch_template` | string ou null | `null` | Template opcional de nome de branch para tasks `/ecl-quick` |
...
| Configuração | Tipo | Padrão | Descrição |
|---------|------|---------|-------------|
| `workflow.security_enforcement` | boolean | `true` | Habilita verificação de segurança ancorada em modelo de ameaças via `/ecl-secure-phase`. Quando `false`, as verificações de segurança são completamente ignoradas |
...
Configure a seleção de modelo por CLI para `/ecl-review`. Quando definido, substitui o modelo padrão da CLI para aquele revisor.
...
| `review.models.ollama` | string | (padrão do servidor) | Nome do modelo passado ao Ollama quando o revisor `--ollama` é invocado. Se não definido, o primeiro modelo disponível reportado pelo servidor é usado (por exemplo `llama3`). Defina para uma tag específica: `ecl config-set review.models.ollama codellama` |
...
| `review.default_reviewers` | string[] \| null | (todos os revisores detectados) | Subconjunto de revisores padrão para `/ecl-review` sem flags. Exemplo: `["gemini","codex"]`. Flags explícitas e `--all` substituem esta configuração. |
...
| `review.ollama_host` | string | `http://localhost:11434` | URL base do servidor Ollama. Substitua quando executar o Ollama em uma porta não padrão ou host remoto: `ecl config-set review.ollama_host http://192.168.1.10:11434` |
...
Configure flags por etapa que `/ecl-manager` acrescenta a cada comando despachado. Isso permite personalizar como o manager executa as etapas de discuss, plan e execute sem entrada manual de flags.
...
Tokens de flag inválidos são sanitizados e registrados como avisos. Apenas flags eCL reconhecidas são repassadas.
...
|-------|-----------|------------|----------|------------|-----------|
| ecl-planner | Opus | Opus | Sonnet | Opus | Inherit |
| ecl-roadmapper | Opus | Sonnet | Sonnet | Opus | Inherit |
| ecl-executor | Opus | Sonnet | Sonnet | Sonnet | Inherit |
| ecl-phase-researcher | Opus | Sonnet | Haiku | Sonnet | Inherit |
| ecl-project-researcher | Opus | Sonnet | Haiku | Sonnet | Inherit |
| ecl-research-synthesizer | Sonnet | Sonnet | Haiku | Haiku | Inherit |
| ecl-debugger | Opus | Sonnet | Sonnet | Opus | Inherit |
| ecl-codebase-mapper | Sonnet | Haiku | Haiku | Haiku | Inherit |
| ecl-verifier | Sonnet | Sonnet | Haiku | Sonnet | Inherit |
| ecl-plan-checker | Sonnet | Sonnet | Haiku | Haiku | Inherit |
| ecl-integration-checker | Sonnet | Sonnet | Haiku | Haiku | Inherit |
| ecl-nyquist-auditor | Sonnet | Sonnet | Haiku | Haiku | Inherit |
| ecl-pattern-mapper | Sonnet | Sonnet | Haiku | Haiku | Inherit |
| ecl-ui-researcher | Opus | Sonnet | Haiku | Sonnet | Inherit |
| ecl-ui-checker | Sonnet | Sonnet | Haiku | Haiku | Inherit |
| ecl-ui-auditor | Sonnet | Sonnet | Haiku | Haiku | Inherit |
| ecl-doc-writer | Opus | Sonnet | Haiku | Sonnet | Inherit |
| ecl-doc-verifier | Sonnet | Sonnet | Haiku | Haiku | Inherit |

> **Todos os 33 agentes incluídos possuem atribuições explícitas de nível por perfil** no catálogo (`sdk/shared/model-catalog.json`). A tabela acima mostra um subconjunto representativo dos agentes mais usados. Para agentes não listados aqui, `model_overrides` aceita qualquer nome de agente incluído. Os dados autoritativos de perfil são derivados de `sdk/shared/model-catalog.json` via `evolv-coder-lite/bin/lib/model-catalog.cjs` e `sdk/src/model-catalog.ts`.
...
ou `~/.ecl/defaults.json` (global). Entradas por projeto ganham em conflito e
...
executar `ecl install <runtime>` após editar `model_overrides` é obrigatório
para que a alteração entre em vigor. Consulte a issue #2256.

### Modelos Por Tipo de Fase (`models`) — adicionado na v1.41

> Expresse ajuste no nível de **fase** (planning, research, execution, verification) sem precisar conhecer a taxonomia de agentes. Adicionado em [#3023](https://github.com/evolvconsulting/evolv-coder-lite/pull/3030).

`model_overrides` é por **agente** (preciso mas verboso; você precisa saber que `ecl-codebase-mapper` é pesquisa e `ecl-doc-writer` é execução). O bloco `models` permite dizer "Opus para planejamento e execução, Sonnet para o restante" em duas linhas:
...
```bash
$ ecl config-set models.deployment opus
Error: 'models.deployment' is not a valid config key

# Válido:
$ ecl config-set models.research sonnet
```
...
> Comece barato, escale apenas quando o agente falhar no gate. Adicionado em [#3024](https://github.com/evolvconsulting/evolv-coder-lite/pull/3031).
...
|---|---|---|
| `light` | ecl-codebase-mapper, ecl-doc-classifier, ecl-doc-verifier, ecl-integration-checker, ecl-intel-updater, ecl-nyquist-auditor, ecl-pattern-mapper, ecl-plan-checker, ecl-research-synthesizer, ecl-ui-auditor, ecl-ui-checker | Barato/rápido — mapeadores puros, scanners, auditorias de baixo risco |
| `standard` | ecl-advisor-researcher, ecl-ai-researcher, ecl-code-fixer, ecl-code-reviewer, ecl-doc-synthesizer, ecl-doc-writer, ecl-domain-researcher, ecl-eval-auditor, ecl-executor, ecl-phase-researcher, ecl-project-researcher, ecl-ui-researcher, ecl-verifier | Motor padrão — pesquisa, escrita, verificação primária |
| `heavy` | ecl-assumptions-analyzer, ecl-debug-session-manager, ecl-debugger, ecl-eval-planner, ecl-framework-selector, ecl-planner, ecl-roadmapper, ecl-security-auditor, ecl-user-profiler | Raciocínio profundo — já no topo, não pode escalar mais |
...
> Controle de esforço unificado entre provedores. Adicionado em [#443](https://github.com/evolvconsulting/evolv-coder-lite/issues/443).
...
> Controle de propagação de fast_mode por agente. Adicionado em [#443](https://github.com/evolvconsulting/evolv-coder-lite/issues/443).
...
Use `node ecl-tools.cjs resolve-execution <agent-type> [--effort <level>] [--fast-mode <true|false>] [--attempt <n>]` para obter o contexto completo de execução resolvido para um agente:
...
>
> O [Codex CLI 0.130.0](https://github.com/openai/codex/releases/tag/rust-v0.130.0) (lançado em 2026-05-08) removeu a descoberta de extra-skills-roots via [openai/codex#21485](https://github.com/openai/codex/pull/21485). A partir desta versão, o Codex CLI só verifica `~/.codex/skills/<name>/SKILL.md`, `<project>/.codex/skills/` e raízes de plugin registradas para habilidades invocáveis. O eCL instala a superfície `$ecl-*` como `~/.codex/skills/ecl-<name>/SKILL.md` para que os comandos resolvam após uma reinicialização do Codex. Versões anteriores do Codex CLI podem mostrar uma listagem duplicada (a varredura legada de extra-roots mais as cópias da raiz do usuário) — reinicie o Codex e atualize para ≥ 0.130.0 ou aceite as duplicatas até fazê-lo.

Quando o eCL é instalado para um runtime não-Claude, o instalador automaticamente define `resolve_model_ids: "omit"` em `~/.ecl/defaults.json`. Isso faz o eCL retornar um parâmetro de modelo vazio para todos os agentes, para que cada agente use o modelo com que o runtime está configurado. Nenhuma configuração adicional é necessária para o caso padrão.
...
Isso resolve `ecl-planner` → `gpt-5.5` (xhigh), `ecl-executor` → `gpt-5.3-codex` (medium), `ecl-codebase-mapper` → `gpt-5.4-mini` (medium). O instalador do Codex incorpora `model = "..."` e `model_reasoning_effort = "..."` em cada TOML de agente gerado.
...
**Runtimes desconhecidos.** Se `runtime` for definido para um valor sem mapa de nível integrado e sem `model_profile_overrides[<runtime>]`, o eCL cai de volta para o padrão seguro de alias Claude em vez de emitir um ID de modelo que o runtime não pode aceitar. Para suportar um novo runtime, popule `model_profile_overrides.<runtime>.{opus,sonnet,haiku}` com IDs válidos.
...
> **[#49](https://github.com/evolvconsulting/evolv-coder-lite/issues/49)** — superfície de configuração de política de modelo neutra em relação ao provedor. Resolve antes do legado `model_profile_overrides`.

`model_policy` fornece uma maneira mais simples e neutra em relação ao provedor de configurar níveis de modelo entre runtimes. É a superfície preferida para runtimes não-Anthropic onde `model_profile_overrides` exigiria conhecer manualmente os IDs de modelo corretos. Configure via `/ecl:settings` → Seção 8 (Model Policy).

### Predefinição de provedor conhecido

Escolha um provedor e nível de orçamento via o fluxo de configurações; o eCL escreve os IDs de modelo canônicos para aquela combinação de provedor/orçamento:
...
Use `provider: "generic"` (ou `"custom"`) para OpenRouter, LiteLLM, gateways locais ou qualquer runtime onde você fornece IDs de modelo exatos. O eCL trata IDs de modelo como strings opacas — sem inferência de prefixo, sem padrões específicos do provedor:
...
| `ECL_AUDIT` | Defina como `1` para habilitar o arquivo de auditoria de despacho (`.planning/.ecl-trace.jsonl`) |
| `ECL_AUDIT_ARGS` | Defina como `1` para incluir args de comando nos eventos de auditoria/erro (omitidos por padrão) |
| `ECL_PROJECT` | Substitui a raiz do projeto para suporte a workspace multi-projeto (v1.32) |
| `ECL_SKIP_SCHEMA_CHECK` | Ignora a detecção de deriva de esquema durante a fase de execução (v1.31) |
...
**Localização:** `~/.ecl/defaults.json`

Quando `/ecl-new-project` cria um novo `config.json`, ele lê os padrões globais e os mescla como configuração inicial. Configurações por projeto sempre substituem os globais.
...
O campo `kind` corresponde a uma das variantes de erro do Hub: `UnknownCommand`, `InvalidArgs`, `HandlerRefusal` ou `HandlerFailure`. Args são omitidos por padrão (privacidade); consulte `ECL_AUDIT_ARGS` abaixo.
...
**Localização do arquivo de auditoria:** `.planning/.ecl-trace.jsonl` (gitignored)
...
`ECL_AUDIT_ARGS` aplica-se simultaneamente tanto à linha de erro do stderr quanto ao arquivo de auditoria.
