...
//   - COMMANDS.md mentions ecl-tools intel api-surface
...
const NEW_PROJECT_PATH = path.join(ROOT, 'evolv-coder-lite', 'workflows', 'new-project.md');
const SETTINGS_PATH = path.join(ROOT, 'evolv-coder-lite', 'workflows', 'settings.md');
...
describe('feat-22-surfacing-docs', () => {
  ...
  test('COMMANDS.md mentions intel api-surface', () => {
    ...
    assert.ok(
      ...
      'COMMANDS.md must document the ecl-tools intel api-surface command'
    );
  });
