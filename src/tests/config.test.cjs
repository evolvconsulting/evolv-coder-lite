/**
 * eCL Tools Tests - config.cjs
 *
 * CLI integration tests for config-ensure-section, config-set, and config-get
 * commands exercised through ecl-tools.cjs via execSync.
 *
 ...
 *
 * `config-ensure-section` is invoked through a spawned `ecl-tools.cjs` child.
 ...
 * stderr (an OS-level kill, not a ecl-tools application error; see the
...
describe('config-ensure-section command', () => {
  ...
  test('creates config.json with expected structure and types', () => {
    ...
    // Verify structure and types — exact values may vary if ~/.ecl/defaults.json exists
...
describe('config-set/config-get context', () => {
  ...
  test('all three context profile files exist', () => {
    const contextsDir = path.join(__dirname, '..', 'evolv-coder-lite', 'contexts');
...
describe('config-path command (#2282)', () => {
  ...
  test('returns workstream config path when ECL_WORKSTREAM is set', () => {
    const result = runGsdTools('config-path', tmpDir, { ECL_WORKSTREAM: 'my-stream' });
    assert.ok(result.success, `config-path failed: ${result.error}`);
