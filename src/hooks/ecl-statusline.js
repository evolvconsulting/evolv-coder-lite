...
// Claude Code Statusline - eCL Edition
// Shows: model | current task (or eCL state) | directory | context usage
...
function readLastSlashCommand(transcriptPath) {
  ...
  // Command names in Claude Code transcripts are plain identifiers like "ecl-plan-phase"
...
/**
 * Format eCL state into display string.
 *
...
function runStatusline() {
  ...
  try {
    ...
    // eCL state (milestone · status · phase) — shown when no todo task
    ...
    // backward compatibility with older ecl-check-update.js versions.
    let gsdUpdate = '';
    const sharedCacheFile = path.join(homeDir, '.cache', 'ecl', 'ecl-update-check.json');
    const legacyCacheFile = path.join(claudeDir, 'cache', 'ecl-update-check.json');
    const cacheFile = fs.existsSync(sharedCacheFile) ? sharedCacheFile : legacyCacheFile;
    if (fs.existsSync(cacheFile)) {
      try {
        const cache = JSON.parse(fs.readFileSync(cacheFile, 'utf8'));
        if (cache.update_available) {
          gsdUpdate = '\x1b[33m⬆ /ecl:update\x1b[0m │ ';
        }
        if (cache.stale_hooks && cache.stale_hooks.length > 0) {
          // If installed version is ahead of npm latest, this is a dev install.
          // Running /ecl:update would downgrade — show a contextual warning instead.
          const isDevInstall = (() => {
          ...
          if (isDevInstall) {
            ...
            gsdUpdate += '\x1b[31m⚠ stale hooks — run /ecl:update\x1b[0m │ ';
          }
        }
      } catch (e) {}
    }
 ...
 * @param {string|null} [opts.middle=null]  - middle segment (todo task or eCL state), null = absent
