...
**前提条件:** `/ecl-execute-phase N` を実行したが、`VERIFICATION.md` が書き込まれる前に実行が停止した、または予期しない出力・ファイル不足・スピナーの停止が確認された状態であること。
...
これはフリーズではなく正常な動作です。eCL のサブエージェントは独立したコンテキストウィンドウで動作します。スポーン行の liveness ノートがこれを確認しています。セッションを中断しないでください。
...
eCL はエグゼキューターを dispatching する前に `SUMMARY.md` ファイルを確認します。すでに `SUMMARY.md` があるプランは自動的にスキップされます。
...
eCL は次回の実行時にこれを検出し、3 つの選択肢を持つ安全な再開ゲートを表示します。
...
eCL は症状を収集し、科学的手法を使って体系的な調査を行い、修正案を提案します。設定で `tdd_mode: true` が有効な場合、修正を適用する前に失敗するテストが必要です。
...
eCL は git 履歴、`.planning/` 成果物の完全性、STATE.md の一貫性、未コミットの作業、孤立したワークツリーを分析します。構造化されたレポートを `.planning/forensics/report-<timestamp>.md` に書き込み、推奨される修復手順を提示します。

`/ecl-forensics` は読み取り専用です。プロジェクトファイルを変更することはありません。
...
eCL はすでに `SUMMARY.md` が存在するプランをスキップし、残りのプランのみにエグゼキューターを dispatch します。
...
フェーズ `3` のプラン `02` に関するすべてのコミットを revert します。eCL は変更を書き込む前に確認ゲートを表示します。
...
フェーズ `3` に関するすべてのコミットを revert します。eCL は後続フェーズがこのフェーズに依存しているかどうかを確認し、続行前に警告を表示します。
...
最近の 5 件の eCL コミットを表示し、revert するものを選択できます。
