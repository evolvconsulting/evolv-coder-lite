...
const { VALID_CONFIG_KEYS, isValidConfigKey } = require('../evolv-coder-lite/bin/lib/config-schema.cjs');
...
describe('VALID_CONFIG_KEYS correctness', () => {
  ...
  test('#2531: hooks.workflow_guard must be in VALID_CONFIG_KEYS (used by hook, documented)', () => {
    assert.ok(
      ...
      'hooks.workflow_guard is read by ecl-workflow-guard.js hook and documented in CONFIGURATION.md'
    );
  });
  ...
  test('#2532: workflow.ui_review must be in VALID_CONFIG_KEYS (used in autonomous.md)', () => {
    assert.ok(
      ...
      'workflow.ui_review is read in autonomous.md via ecl-sdk query config-get'
    );
  });
  ...
  test('#2533: workflow.max_discuss_passes must be in VALID_CONFIG_KEYS (used in discuss-phase.md)', () => {
    assert.ok(
      ...
      'workflow.max_discuss_passes is read in discuss-phase.md via ecl-sdk query config-get'
    );
  });
