...
**전제 조건:** `.planning/ROADMAP.md`가 존재해야 합니다. `/ecl-discuss-phase`에서 생성된 `{phase}-CONTEXT.md`를 강력히 권장하지만 필수는 아닙니다.
...
1. **리서치** — `ecl-phase-researcher` 서브에이전트가 도메인을 조사하고 `{phase}-RESEARCH.md`를 작성합니다.
2. **기획** — `ecl-planner` 서브에이전트가 컨텍스트, 리서치, 요구 사항을 읽고 하나 이상의 `{phase}-{N}-PLAN.md` 파일을 작성합니다.
3. **검증** — `ecl-plan-checker` 서브에이전트가 8개 차원에서 계획 품질을 검증하고 품질 게이트를 통과할 때까지 최대 3회의 수정 루프를 실행합니다.

페이즈 번호가 없으면 eCL Core는 로드맵에서 다음 미기획 페이즈를 대상으로 합니다.
...
참고: `--research-phase <N>`은 `/ecl-plan-phase`의 플래그입니다. 독립형 리서치 페이즈 명령은 없습니다. 이전의 독립형 리서치 명령은 이 플래그로 대체되었습니다.
...
**`/ecl-review --phase N`을 실행하여 `REVIEWS.md`가 존재하는 경우:**
...
`ecl-plan-checker`는 실행을 허용하기 전에 8개 차원에서 계획을 검증합니다:
