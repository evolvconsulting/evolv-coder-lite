 ...
 * `evolv-coder-lite` now that the package has been renamed to `evolv-coder-lite` (#604).
 *
 ...
 *   (evolv-coder-lite-OLD, evolv-coder-lite-classic, evolv-coder-lite-cli, etc.)
 ...
 *   path like `~/.claude/evolv-coder-lite/` or `'evolv-coder-lite'` in code.
 *
 * Allowlist:
 ...
 *   - Lines containing the marker `ecl-allow-legacy-name` (intentional uses)
...
const ALLOW_MARKER = 'ecl-allow-legacy-name';
const SELF_PATH = path.resolve(__filename);
// ECL_LINT_LEGACY_REPO_ROOT is used by tests to redirect the guard to a
// temporary fixture git repo without touching the real working tree.
const REPO_ROOT = process.env.ECL_LINT_LEGACY_REPO_ROOT
  ? path.resolve(process.env.ECL_LINT_LEGACY_REPO_ROOT)
...
function main() {
  ...
  process.stderr.write('\n');
  process.stderr.write('Fix: rename to evolv-coder-lite, or add `ecl-allow-legacy-name` marker on the line if the\n');
