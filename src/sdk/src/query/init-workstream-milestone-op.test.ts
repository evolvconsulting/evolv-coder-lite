...
describe('initMilestoneOp workstream resolution (#3196)', () => {
  ...
  beforeEach(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), 'ecl-ws-milestone-op-'));
  ...
  it('reads from active-workstream file when no explicit --ws is passed', async () => {
    ...
    // Resolve context as the CLI would (no --ws arg, no ECL_WORKSTREAM env)
    const prev = process.env.ECL_WORKSTREAM;
    ...
    try {
      ...
      if (prev === undefined) delete process.env.ECL_WORKSTREAM;
      else process.env.ECL_WORKSTREAM = prev;
    }
  });
  ...
  it('ECL_WORKSTREAM env overrides active-workstream file', async () => {
    // File says other-ws, env says test-ws
    await writeFile(join(tmpDir, '.planning', 'active-workstream'), 'other-ws\n');

    const prev = process.env.ECL_WORKSTREAM;
    process.env.ECL_WORKSTREAM = 'test-ws';
    try {
      const ctx = resolveQueryRuntimeContext({ projectDir: tmpDir });
      expect(ctx.ws).toBe('test-ws');
    } finally {
...
describe('roadmapAnalyze workstream resolution (#3196)', () => {
  ...
  beforeEach(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), 'ecl-ws-roadmap-analyze-'));
...
describe('resolveQueryRuntimeContext active-workstream file fallback (#3196)', () => {
  ...
  beforeEach(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), 'ecl-rtctx-'));
  ...
  it('reads ws from active-workstream file when no --ws and no ECL_WORKSTREAM', async () => {
  ...
  it('ECL_WORKSTREAM env takes priority over active-workstream file', async () => {
    await writeFile(join(tmpDir, '.planning', 'active-workstream'), 'my-ws\n');
    await mkdir(join(tmpDir, '.planning', 'workstreams', 'env-ws'), { recursive: true });

    const prev = process.env.ECL_WORKSTREAM;
    process.env.ECL_WORKSTREAM = 'env-ws';
    try {
      const ctx = resolveQueryRuntimeContext({ projectDir: tmpDir });
