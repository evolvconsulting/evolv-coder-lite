...
} = require('../evolv-coder-lite/bin/lib/configuration.cjs');
...
describe('auto-update', () => {
  ...
  const HOOK = path.join(__dirname, '..', 'hooks', 'ecl-graphify-update.sh');

  function createTempGitRepo(opts = {}) {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-3347-'));
  ...
  describe('hook — HEAD-advancing command matchers',
    ...
    for (const cmd of [
      ...
      // #3653 — `ecl-tools query commit` invokes git via spawnSync('git', [...]),
      ...
      'ecl-tools query commit "docs: probe" --files .planning/STATE.md',
      'npx ecl-tools query commit "docs: probe" --files .planning/STATE.md',
    ]) {
    ...
    test('does NOT dispatch on SDK commit-to-subrepo prefix collision', (t) => {
      ...
      const r = runHook(
        tmpDir,
        {
          tool_name: 'Bash',
          tool_input: {
            command: 'ecl-tools query commit-to-subrepo "msg" --files packages/foo',
          },
        },
        { pathPrepend: mockBin },
      );
    ...
    // `ecl-tools query` verbs (phase.complete, roadmap.update-plan-progress,
    // state.begin-phase) mutate .md files but do NOT advance HEAD; matching
    // them would cause a spurious rebuild per state mutation.
    for (const cmd of [
      'ecl-tools query phase.complete 109',
      'ecl-tools query roadmap.update-plan-progress 109 W001',
      'ecl-tools query state.begin-phase 110',
    ]) {
