...
O eCL Core pode encaminhar solicitações de revisão para qualquer combinação de: Gemini CLI, Claude (sessão separada), Codex CLI, CodeRabbit, OpenCode, Qwen Code, Cursor, Antigravity CLI, Ollama, LM Studio e llama.cpp.
...
Por padrão, `/ecl-review` executa todos os CLIs detectados. Para fixar um subconjunto como padrões do projeto:

```bash
/ecl-config --integrations
```
...
Como alternativa, defina diretamente com `ecl-tools`:
...
O eCL invoca cada revisor em sequência, coleta feedback estruturado (Resumo, Pontos Fortes, Preocupações em ALTA/MÉDIA/BAIXA, Sugestões, Avaliação de Risco) e grava a saída combinada em `.planning/phases/03-.../03-REVIEWS.md`.

### Selecionar um único revisor para uma execução pontual

```bash
/ecl-review --phase 3 --gemini
/ecl-review --phase 3 --codex
/ecl-review --phase 3 --cursor
```
...
```bash
/ecl-review --phase 3 --ollama
/ecl-review --phase 3 --lm-studio
```

Configure os endereços de host e a seleção de modelo nas chaves `review.*` via `/ecl-config --integrations` se os padrões (`localhost:11434` / `localhost:1234`) não se aplicarem.
...
```bash
/ecl-plan-review-convergence 3 --codex
/ecl-plan-review-convergence 3 --gemini
```
...
**Detecção de estagnação:** se a contagem de preocupações ALTAS não estiver diminuindo entre os ciclos, o eCL avisa você. Quando o limite de ciclos é atingido com preocupações ALTAS em aberto, um portão de escalação pergunta se você deseja prosseguir ou revisar manualmente.
...
| Você precisa de cobertura máxima antes de um lançamento | `/ecl-plan-review-convergence N --all` |
| Você está iterando rapidamente e quer feedback rápido | escolha um CLI: `/ecl-review --phase N --gemini` |
