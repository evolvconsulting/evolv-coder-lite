...
// Layout:
//   <tmpDir>/
//     evolv-coder-lite/bin/lib/<cjsName>.cjs
...
function createFixture({ cjsName, tsName, tsInQuery = true, allowlistExtra = {} }) {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-lint-handsync-'));

  const cjsDir = path.join(tmpDir, 'evolv-coder-lite', 'bin', 'lib');
...
describe('lint-shared-module-handsync: allowlist entry honored', () => {
  test('exits 0 when pair is in cooperatingSiblings allowlist', () => {
    ...
    const tmpDir = createFixture({
      ...
      allowlistExtra: {
        cooperatingSiblings: [
          {
            cjs: `evolv-coder-lite/bin/lib/${cjsName}.cjs`,
  ...
  test('reports unallowlisted ts sibling when another ts sibling for the same cjs IS allowlisted (#3632)', () => {
    ...
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-lint-multi-'));
    try {
      const cjsDir = path.join(tmpDir, 'evolv-coder-lite', 'bin', 'lib');
      fs.mkdirSync(cjsDir, { recursive: true });
  ...
  test('generated .cjs files are excluded from pair detection', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-lint-gen-'));
    try {
      const cjsDir = path.join(tmpDir, 'evolv-coder-lite', 'bin', 'lib');
      fs.mkdirSync(cjsDir, { recursive: true });
