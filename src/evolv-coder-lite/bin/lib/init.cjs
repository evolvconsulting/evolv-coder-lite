...
function cmdInitExecutePhase(cwd, phase, raw, options = {}) {
  ...
  const result = {
    // Models
    executor_model: resolveModelInternal(cwd, 'ecl-executor'),
    verifier_model: resolveModelInternal(cwd, 'ecl-verifier'),
...
function cmdInitPlanPhase(cwd, phase, raw, options = {}) {
  ...
  // the first-touch mkdir in /ecl:plan-phase stays consistent with phase.add.
  ...
  const result = {
    // Models
    researcher_model: resolveModelInternal(cwd, 'ecl-phase-researcher'),
    planner_model: resolveModelInternal(cwd, 'ecl-planner'),
    checker_model: resolveModelInternal(cwd, 'ecl-plan-checker'),
    ...
    // #3569: surface phase lifecycle status so /ecl:plan-phase can short-circuit
...
function cmdInitNewProject(cwd, raw) {
  ...
  const firecrawlKeyFile = path.join(homedir, '.ecl', 'firecrawl_api_key');
  const hasFirecrawl = !!(process.env.FIRECRAWL_API_KEY || fs.existsSync(firecrawlKeyFile));

  // Detect Exa API key availability
  const exaKeyFile = path.join(homedir, '.ecl', 'exa_api_key');
  ...
  const result = {
    // Models
    researcher_model: resolveModelInternal(cwd, 'ecl-project-researcher'),
    synthesizer_model: resolveModelInternal(cwd, 'ecl-research-synthesizer'),
    roadmapper_model: resolveModelInternal(cwd, 'ecl-roadmapper'),
...
function cmdInitPhaseOp(cwd, phase, raw) {
  ...
  // #3287: compute the canonical directory name with project_code prefix so
  // the first-touch mkdir in /ecl:discuss-phase stays consistent with phase.add.
  ...
  const result = {
    ...
    // string API keys (when user did `ecl-tools config-set brave_search XXX`).
...
function cmdInitMapCodebase(cwd, raw) {
  ...
  const result = {
    // Models
    mapper_model: resolveModelInternal(cwd, 'ecl-codebase-mapper'),
...
function cmdInitManager(cwd, raw) {
  ...
  // runtimes, $ecl-<cmd> shell-var for codex.
  ...
  const sanitizeFlags = (raw) => {
    ...
    if (!safe) {
      process.stderr.write(`ecl-tools: warning: manager.flags contains invalid tokens, ignoring: ${val}\n`);
      return '';
    }
    return val;
  };
...
function cmdInitNewWorkspace(cwd, raw) {
  ...
  const defaultBase = path.join(homedir, 'ecl-workspaces');
 ...
 * @param {string} agentType - The agent type (e.g., 'ecl-executor', 'ecl-planner')
 ...
 * Used by workflows: SKILLS=$(node "$TOOLS" agent-skills ecl-executor 2>/dev/null)
 */
 ...
 *   installation: { ecl_skills_installed: boolean, legacy_claude_commands_installed: boolean },
...
function buildSkillManifest(cwd, skillsDir = null) {
  ...
  const canonicalRoots = skillsDir ? [{
    ...
    },
    {
      ...
      path: path.join(os.homedir(), '.claude', 'evolv-coder-lite', 'skills'),
      scope: 'import-only',
      kind: 'skills',
      deprecated: true,
    },
    {
      root: '.claude/commands/ecl',
      path: path.join(os.homedir(), '.claude', 'commands', 'ecl'),
  ...
  const gsdSkillsInstalled = skills.some(skill => skill.name.startsWith('ecl-'));

  return {
    skills,
    roots,
    installation: {
      ecl_skills_installed: gsdSkillsInstalled,
