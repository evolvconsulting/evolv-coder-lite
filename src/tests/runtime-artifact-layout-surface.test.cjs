...
describe('applySurface', () => {
  ...
  // Regression guard for #816: applySurface must write ecl-prefixed command files
  ...
  // `commands/`, augment `commands/`, kilo `command/`) with kind.prefix='ecl-'.
  // _copyStaged names files `ecl-<stem>.md` but the buggy _syncGsdDir copies
  // them as `<stem>.md` (unprefixed) and also deletes ALL .md files not in the
  // staged set, including user files.
  test('applySurface writes ecl-prefixed command files matching install and preserves user commands (#816)', (t) => {
    const configDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-surface-816-'));
    ...
    // (a) At least one ecl-prefixed command file must exist — on buggy code only
    //     unprefixed files like 'help.md' are written, so this assertion fails.
    assert.ok(
      files.some(f => f.startsWith('ecl-') && f.endsWith('.md')),
      '#816: applySurface must write at least one ecl-prefixed command file ' +
      '(e.g. ecl-help.md) to match installRuntimeArtifacts/_copyStaged behaviour. ' +
      `Actual files: [${files.join(', ')}]`
    );

    // (b) Every eCL-owned command file must be prefixed — no bare <stem>.md files
    //     allowed among eCL-owned output (excluding the user file).
    const gsdFiles = files.filter(f => f !== 'my-user-cmd.md');
    const unprefixed = gsdFiles.filter(f => !f.startsWith('ecl-'));
    assert.deepStrictEqual(
      unprefixed,
      [],
      '#816: all eCL-owned command files must start with ecl- to match install. ' +
    ...
    assert.ok(
      ...
      'present before sync — _syncGsdDir must not prune files not owned by eCL. ' +
      `Actual files: [${files.join(', ')}]`
    );
  });
  ...
  //
  // Matrix: opencode/kilo = flat command/ + prefix ecl-;
  //         cursor/augment = flat commands/ + prefix ecl-;
  //         gemini = namespaced commands/ecl/ (no file prefix — dir is namespace).
  ...
  test('applySurface command-dir filenames match a fresh install for every command runtime (#816 parity)', async (t) => {
    ...
    for (const runtime of PARITY_RUNTIMES) {
      ...
      const installDir = fs.mkdtempSync(path.join(os.tmpdir(), `ecl-816-install-${runtime}-`));
      const surfaceDir = fs.mkdtempSync(path.join(os.tmpdir(), `ecl-816-surface-${runtime}-`));
  ...
  for (const runtime of ['cursor', 'codex']) {
    test(`applySurface rewrites ${runtime} skill bodies to the install pathPrefix, not the converter default ~/.claude path (#813)`, (t) => {
      ...
      const configDir = fs.mkdtempSync(path.join(os.tmpdir(), `ecl-surface-813-${runtime}-`));
      t.after(() => cleanup(configDir));
      ...
      const manifest = loadSkillsManifest(REAL_COMMANDS_DIR);
      // skills/ecl-<name>/SKILL.md (destSubpath 'skills', prefix 'ecl-')
      ...
      // is cross-platform (Windows CI leg is not covered by local ecl-test) (#813).
...
describe('listSurface', () => {
  test('accepts parsed ecl-file-manifest JSON objects without crashing (#322)', () => {
