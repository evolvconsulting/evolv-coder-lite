...
并非每项工作都需要完整的阶段流程。eCL 提供了两个轻量级命令，适用于不需要完整的讨论 → 计划 → 执行 → 验证循环的工作。
...
| 场景 | 命令 |
|-----------|---------|
| 修复 Bug、添加小功能，或任何无法概括为单一琐碎编辑的任务 | `/ecl-quick` |
| 修复错别字、更新配置值、添加 `.gitignore` 条目，或任何涉及 ≤ 3 个文件且耗时不到一分钟的更改 | `/ecl-fast` |
| 任务有未知因素、需要调研，或将涉及超过几个文件 | `/ecl-quick` 加 `--research` |

**经验法则：** 如果你哪怕有一刻犹豫该任务是否属于琐碎操作，就使用 `/ecl-quick`。当范围看起来不够简单时，`/ecl-fast` 会自动将你重定向到 `/ecl-quick`。

---

## `/ecl-quick` — 带有 eCL 保证的临时任务

`/ecl-quick` 运行一个规划器和执行器，提供与完整阶段相同的原子提交和 STATE.md 跟踪保证，但无需阶段开销（无 ROADMAP 条目、无讨论阶段、无跨多个计划的波次协调）。
...
eCL 会提示你输入任务描述，然后进行规划和执行。产出物保存在 `.planning/quick/` 中。
...
`/ecl-fast` 直接在当前上下文中完成工作。没有子代理、没有 `PLAN.md`，也没有调研。它仅适用于你自己在一分钟内即可完成的更改。
...
`/ecl-fast` 在继续操作前会检查任务是否确实属于琐碎操作。如果判断范围过大，它会停止并重定向你：
...
完成更改后，`/ecl-fast` 以原子方式提交，并且如果 `.planning/STATE.md` 中存在 `Quick Tasks Completed` 表格，则向其追加一行。

---

## `/ecl-quick` 相比 `/ecl-fast` 多提供的能力

| 能力 | `/ecl-fast` | `/ecl-quick` |
|------------|------------|--------------|
| 子代理规划器 | 否 | 是 |
| 子代理执行器 | 否 | 是 |
| 调研代理 | 否 | 可选（`--research`） |
...
关键区别在于子代理隔离。`/ecl-quick` 在独立的上下文窗口中启动全新的规划器和执行器，这意味着工作会被妥善规划，提交按任务原子化，且编排器可验证结果。`/ecl-fast` 仅使用当前上下文窗口，有意限制于无需上述任何流程的琐碎更改。
