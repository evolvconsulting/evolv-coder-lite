...
function cmdWriteProfile(cwd, options, raw) {
  ...
  if (!outputPath) {
    outputPath = path.join(os.homedir(), '.claude', 'evolv-coder-lite', 'USER-PROFILE.md');
...
function cmdGenerateDevPreferences(cwd, options, raw) {
  ...
  if (!outputPath) {
    ...
    try {
      ...
      effectiveRuntime = process.env.ECL_RUNTIME || config.runtime || 'claude';
    } catch {
      effectiveRuntime = process.env.ECL_RUNTIME || 'claude';
    }
...
function cmdGenerateClaudeProfile(cwd, options, raw) {
  ...
  const sectionLines = [
    ...
    '## Developer Profile',
    '',
    `> Generated by eCL from ${dataSource}. Run \`${formatGsdSlash('profile-user', resolveRuntime(cwd))} --refresh\` to update.`,
    '',
...
function cmdGenerateClaudeMd(cwd, options, raw) {
  ...
  try {
    ...
    const effectiveRuntime = process.env.ECL_RUNTIME || config.runtime || null;
