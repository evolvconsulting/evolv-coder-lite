...
describe('bug-2851: workflow files must not call bare `ecl-tools` (#2245 sweep regression)', () => {
  ...
  test('plan-phase.md §13e gap-analysis uses the ecl_run launcher (resolvable invocation, #621)', () => {
    ...
    for (const blk of blocks) {
      for (const line of blk.lines) {
        if (/gap-analysis/.test(line) && !/^\s*#/.test(line)) {
          ...
          assert.match(
            line,
            /\becl_run\s+gap-analysis\b/,
            `gap-analysis must use the ecl_run launcher (not a hardcoded $HOME path), got: ${line.trim()}`,
          );
        }
      }
    }
