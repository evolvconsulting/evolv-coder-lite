 ...
 *
 * #2046 (macOS/Linux): The three .sh hooks (ecl-validate-commit.sh,
 * ecl-session-state.sh, ecl-phase-boundary.sh) were registered in
...
// buildHookCommand was extracted to evolv-coder-lite/bin/lib/runtime-hooks-surface.cjs
...
const SH_HOOKS = [
  { name: 'ecl-validate-commit.sh' },
  { name: 'ecl-session-state.sh' },
  { name: 'ecl-phase-boundary.sh' },
];
...
describe('bugs #2045 #2046: .sh hook paths must be absolute and quoted', () => {
  ...
  describe('buildHookCommand', () => {
    test('returns a bash command for .sh hookName', () => {
      ...
      const cmd = buildHookCommand(TEST_CONFIG_DIR, 'ecl-validate-commit.sh', HOOK_OPTS);
    ...
    test('buildHookCommand produces bash runner for .sh and node runner for .js', () => {
      ...
      const shCmd = buildHookCommand(TEST_CONFIG_DIR, 'ecl-validate-commit.sh', HOOK_OPTS);
      assert.ok(
      ...
      const jsCmd = buildHookCommand(TEST_CONFIG_DIR, 'ecl-something.js', HOOK_OPTS);
      assert.ok(
    ...
    describe(`${name} — buildHookCommand output`, () => {
      ...
      for (const platform of ['linux', 'win32']) {
        test(`${platform}: configDir is absolute and double-quoted (fixes #2045 spaces)`, () => {
          ...
          // e.g. bash "/home/first last/.claude/hooks/ecl-validate-commit.sh"
          //       or  "/home/first last/.claude/hooks/ecl-validate-commit.sh"
          assert.ok(
