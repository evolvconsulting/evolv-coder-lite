...
import { GSDTools, GSDToolsError } from './ecl-tools.js';
...
describe('PhaseRunner', () => {
  ...
  beforeEach(async () => {
    tempProjectDirs = [];
    defaultProjectDir = await mkdtemp(join(tmpdir(), 'ecl-phase-runner-default-'));
  ...
  describe('research gate (#1602)', () => {
    ...
    beforeEach(async () => {
      tempPhaseDir = await mkdtemp(join(tmpdir(), 'ecl-research-gate-'));
    });
...
1. **Something** — needs decision`, 'utf-8');
  ...
  describe('verification routing', () => {
    ...
    it('keeps phase pending when changed phase files contain unresolved TBD/FIXME/XXX markers', async () => {
      const projectDir = await mkdtemp(join(tmpdir(), 'ecl-architectural-debt-'));
    ...
    it('allows changed-file debt markers when they reference tracked follow-up work', async () => {
      const projectDir = await mkdtemp(join(tmpdir(), 'ecl-tracked-debt-'));
      tempProjectDirs.push(projectDir);
      const phaseDir = join(projectDir, '.planning', 'phases', '01-auth');
      const sourceDir = join(projectDir, 'scripts', 'upstream');
      await mkdir(phaseDir, { recursive: true });
    ...
    it('allows changed-file entries for files deleted by the phase', async () => {
      const projectDir = await mkdtemp(join(tmpdir(), 'ecl-deleted-file-debt-scan-'));
      tempProjectDirs.push(projectDir);
      const phaseDir = join(projectDir, '.planning', 'phases', '01-auth');
      await mkdir(phaseDir, { recursive: true });
    ...
    it('allows dotted lowercase xxx placeholder text when scanning debt markers', async () => {
      const projectDir = await mkdtemp(join(tmpdir(), 'ecl-lowercase-placeholder-'));
      tempProjectDirs.push(projectDir);
      const phaseDir = join(projectDir, '.planning', 'phases', '01-auth');
      const sourceDir = join(projectDir, 'scripts', 'upstream');
      await mkdir(phaseDir, { recursive: true });
    ...
    it('keeps phase pending when changed files contain lowercase debt markers', async () => {
      const projectDir = await mkdtemp(join(tmpdir(), 'ecl-lowercase-debt-'));
      tempProjectDirs.push(projectDir);
      const phaseDir = join(projectDir, '.planning', 'phases', '01-auth');
      const sourceDir = join(projectDir, 'scripts', 'upstream');
      await mkdir(phaseDir, { recursive: true });
    ...
    it('keeps phase pending when debt markers are followed by punctuation', async () => {
      const projectDir = await mkdtemp(join(tmpdir(), 'ecl-punctuated-debt-'));
      tempProjectDirs.push(projectDir);
      const phaseDir = join(projectDir, '.planning', 'phases', '01-auth');
      const sourceDir = join(projectDir, 'scripts', 'upstream');
      await mkdir(phaseDir, { recursive: true });
    ...
    it('allows bare hash issue references when they read as references', async () => {
      const projectDir = await mkdtemp(join(tmpdir(), 'ecl-bare-hash-ref-'));
      tempProjectDirs.push(projectDir);
      const phaseDir = join(projectDir, '.planning', 'phases', '01-auth');
      const sourceDir = join(projectDir, 'scripts', 'upstream');
      await mkdir(phaseDir, { recursive: true });
    ...
    it('does not treat quoted numeric fragments as debt references', async () => {
      const projectDir = await mkdtemp(join(tmpdir(), 'ecl-hex-fragment-debt-'));
      tempProjectDirs.push(projectDir);
      const phaseDir = join(projectDir, '.planning', 'phases', '01-auth');
      const sourceDir = join(projectDir, 'scripts', 'upstream');
      await mkdir(phaseDir, { recursive: true });
    ...
    it('does not allow unrelated earlier issue text to satisfy a later debt marker', async () => {
      const projectDir = await mkdtemp(join(tmpdir(), 'ecl-unrelated-debt-ref-'));
      tempProjectDirs.push(projectDir);
      const phaseDir = join(projectDir, '.planning', 'phases', '01-auth');
      const sourceDir = join(projectDir, 'scripts', 'upstream');
      await mkdir(phaseDir, { recursive: true });
    ...
    it('reports one unresolved debt finding per line', async () => {
      const projectDir = await mkdtemp(join(tmpdir(), 'ecl-duplicate-debt-'));
      tempProjectDirs.push(projectDir);
      const phaseDir = join(projectDir, '.planning', 'phases', '01-auth');
      const sourceDir = join(projectDir, 'scripts', 'upstream');
      await mkdir(phaseDir, { recursive: true });
    ...
    it('keeps phase pending when a declared file resolves through a symlink outside the project', async () => {
      const projectDir = await mkdtemp(join(tmpdir(), 'ecl-symlink-project-'));
      const externalDir = await mkdtemp(join(tmpdir(), 'ecl-symlink-external-'));
    ...
    it('keeps phase pending when plan files cannot be listed for the debt scan', async () => {
      const projectDir = await mkdtemp(join(tmpdir(), 'ecl-debt-missing-plans-'));
      tempProjectDirs.push(projectDir);
      const phaseDir = join(projectDir, '.planning', 'phases', '01-auth');
      const logger = { warn: vi.fn(), info: vi.fn(), debug: vi.fn() } as any;
      const phaseOp = makePhaseOp({ phase_dir: phaseDir, has_context: true, has_plans: true, plan_count: 1 });
  ...
  describe('advance step', () => {
    ...
    it('captures phaseComplete errors without throwing', async () => {
      ...
      (deps.tools.phaseComplete as ReturnType<typeof vi.fn>).mockRejectedValue(
        new Error('ecl-tools commit failed'),
      );
...
describe('GSDTools typed methods', () => {
  ...
  beforeEach(async () => {
    tmpDir = join(tmpdir(), `ecl-tools-phase-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
  ...
  describe('initPhaseOp()', () => {
    it('returns typed PhaseOpInfo from ecl-tools output', async () => {
  ...
  describe('configGet()', () => {
    it('returns string value from ecl-tools config', async () => {
      const scriptPath = await createScript(
        'config-get.cjs',
        `
        const args = process.argv.slice(2);
...
async function createPromptTempDir(): Promise<string> {
  return mkdtemp(join(tmpdir(), 'ecl-prompt-'));
}
...
describe('PromptFactory', () => {
  ...
  describe('buildPrompt', () => {
    it('assembles research prompt with role + purpose + process + context', async () => {
      ...
      await writeFile(
        join(agentsDir, 'ecl-phase-researcher.md'),
        makeAgentDef('ecl-phase-researcher', 'Read, Grep, Bash', 'You are a researcher.'),
      );
    ...
    it('assembles plan prompt with all context files', async () => {
      ...
      await writeFile(
        join(agentsDir, 'ecl-planner.md'),
        makeAgentDef('ecl-planner', 'Read, Write, Bash', 'You are a planner.'),
      );
    ...
    it('delegates execute phase with plan to buildExecutorPrompt', async () => {
      await writeFile(
        join(agentsDir, 'ecl-executor.md'),
        makeAgentDef('ecl-executor', 'Read, Write, Edit, Bash', 'You are an executor.'),
      );
    ...
    it('assembles verify prompt with phase instructions', async () => {
      ...
      await writeFile(
        join(agentsDir, 'ecl-verifier.md'),
        makeAgentDef('ecl-verifier', 'Read, Bash, Grep', 'You are a verifier.'),
      );
    ...
    it('handles missing workflow file gracefully', async () => {
      await writeFile(
        join(agentsDir, 'ecl-phase-researcher.md'),
        makeAgentDef('ecl-phase-researcher', 'Read, Bash', 'You are a researcher.'),
      );
  ...
  describe('loadAgentDef', () => {
    ...
    it('falls back to project agents dir', async () => {
      ...
      await writeFile(
        join(projectAgentsDir, 'ecl-executor.md'),
    ...
    it('prefers user agents dir over project agents dir', async () => {
      const projectAgentsDir = join(tempDir, 'project-agents');
      await mkdir(projectAgentsDir, { recursive: true });
      await writeFile(join(agentsDir, 'ecl-executor.md'), 'user agent');
      await writeFile(join(projectAgentsDir, 'ecl-executor.md'), 'project agent');
  ...
  describe('headless prompt loading', () => {
    it('loadWorkflowFile prefers installed eCL over sdkPromptsDir', async () => {
      const sdkDir = join(tempDir, 'sdk-prompts');
      await mkdir(join(sdkDir, 'workflows'), { recursive: true });

      await writeFile(join(workflowsDir, 'research-phase.md'), 'eCL-1 original');
      ...
      const content = await factory.loadWorkflowFile(PhaseType.Research);
      expect(content).toBe('eCL-1 original');
    });

    it('loadWorkflowFile falls back to eCL-1 when sdkPromptsDir file missing', async () => {
      const sdkDir = join(tempDir, 'sdk-prompts');
      await mkdir(join(sdkDir, 'workflows'), { recursive: true });
    ...
    it('loadAgentDef prefers installed agents over sdkPromptsDir', async () => {
      ...
      await writeFile(join(agentsDir, 'ecl-executor.md'), 'user agent');
      await writeFile(join(sdkDir, 'agents', 'ecl-executor.md'), 'SDK bundled agent');
    ...
    it('buildPrompt sanitizes interactive patterns from output', async () => {
      ...
      expect(prompt).not.toContain('/ecl:');
