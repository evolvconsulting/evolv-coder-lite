...
eCL accepts three types of contributions. Each type has a different process and a different bar for acceptance. **Read this section before opening anything.**
...
**Process:**
1. Open a [Bug Report issue](https://github.com/evolvconsulting/evolv-coder-lite/issues/new?template=bug_report.yml) — fill it out completely.
...
**Process:**
1. Open an [Enhancement issue](https://github.com/evolvconsulting/evolv-coder-lite/issues/new?template=enhancement.yml) with the full proposal.  The issue template requires: the problem being solved, the concrete benefit, the scope of changes, and alternatives considered.
...
**Process:**
1. **Discuss first** — check [Discussions](https://github.com/evolvconsulting/evolv-coder-lite/discussions) to see if the idea has been raised. If it has and was declined, don't open a new issue.
2. Open a [Feature Request issue](https://github.com/evolvconsulting/evolv-coder-lite/issues/new?template=feature_request.yml) with the complete spec. The template requires: the solo-developer problem being solved, what is being added, full scope of affected files and systems, user stories, acceptance criteria, and assessment of maintenance burden.
3. **Wait for maintainer approval.** A maintainer must label the issue `approved-feature` before you write a single line of code. Approval is not guaranteed — eCL is intentionally lean and many valid ideas are declined because they conflict with the project's design philosophy.
...
**Rejection reasons:** Issue not labeled `approved-feature`, spec is incomplete, scope exceeds what was approved, feature conflicts with eCL's solo-developer focus, maintenance burden too high.
...
eCL uses two long-lived branches: `main` (production, what's on npm `@latest`)
...
# ... commit, push
gh pr create --base next --repo evolvconsulting/evolv-coder-lite
```
...
npm run changeset -- --type Fixed --pr <YOUR_PR_NUMBER> \
  --body "**\`/ecl-foo\` no longer drops trailing slashes** — explain the user-visible change."
```
...
Fragments are consolidated into `CHANGELOG.md` at release time by the release workflow. See [`.changeset/README.md`](.changeset/README.md) for the format spec and [#2975](https://github.com/evolvconsulting/evolv-coder-lite/issues/2975) for the rationale.

**CI enforcement:** the `Changeset Required` workflow (`scripts/changeset/lint.cjs`) fails any PR that touches `bin/`, `evolv-coder-lite/`, `agents/`, `commands/`, `hooks/`, or `sdk/src/` without a `.changeset/*.md` fragment. The gate also **validates the content** of every changed fragment: a fragment whose frontmatter does not parse (e.g. a `pr: 0` placeholder that was never backfilled to the real PR number) fails the gate with `fail_invalid_fragment`, naming the offending file. This stops a malformed fragment from merging to `next` and only detonating later in the release job's CHANGELOG render.
...
node scripts/release-notes/format-github-release-notes.cjs \
  --tag vX.Y.Z --repo evolvconsulting/evolv-coder-lite --apply
```
...
If your PR adds, changes, deprecates, or removes user-visible behavior, you **must** update the relevant documentation in `docs/`. CI will fail any PR whose changeset fragment is typed `Added`, `Changed`, `Deprecated`, or `Removed` without also modifying at least one file under `docs/` ([#3213](https://github.com/evolvconsulting/evolv-coder-lite/issues/3213)).
...
|--------|---------|----------|
| `createTempProject(prefix?)` | tmpDir with `.planning/phases/` | Testing eCL tools that need planning structure |
...
| `runGsdTools(args, cwd, env?)` | Executes ecl-tools.cjs | Testing CLI commands |
...
Changes to CLI parsing, command dispatch, query dispatch, command routers, `ecl-tools`, or `ecl-sdk` must include a negative input matrix for the affected command family.
...
const configSrc = fs.readFileSync(
  path.join(ECL_ROOT, 'evolv-coder-lite', 'bin', 'lib', 'config-schema.cjs'), 'utf-8'
);
assert.ok(
...
const cmdContent = fs.readFileSync(path.join(tmpDir, 'ecl-sdk.cmd'), 'utf8');
...
- Setup guide: [ecl-test-runner getting started](https://github.com/evolvconsulting/ecl-test-runner/blob/main/docs/getting-started.md)
...
if git diff --cached --name-only | grep -Eq "^sdk/src/query/command-manifest\.|^sdk/src/query/command-aliases\.generated\.ts$|^evolv-coder-lite/bin/lib/command-aliases\.generated\.cjs$|^sdk/scripts/gen-command-aliases\.ts$"; then
  npm run check:alias-drift
fi
EOF
...
export ECL_BLOCKED_AUTHOR_REGEX='@example-corp\\.com$'
...
blocked_regex="${ECL_BLOCKED_AUTHOR_REGEX:-}"
...
- **No external dependencies in core** — `ecl-tools.cjs` and all lib files use only Node.js built-ins
...
Only `agents/` at the repo root is tracked by git. The following directories may exist on a developer machine with eCL installed and **must not be edited** — they are install-sync outputs and will be overwritten:
...
| `.github/agents/ecl-*` | Yes (`.gitignore:37`) | Local CI-surface bundle |
