...
- [v1.34.0 Features](#v1340-features)
  ...
  - [Hard Stop Safety Gates in /ecl-progress --next](#101-hard-stop-safety-gates-in-ecl-progress---next)
...
- [v1.35.0 Features](#v1350-features)
  ...
  - [eCL-2 Reverse Migration](#105-ecl-2-reverse-migration)
...
**Command:** `/ecl-new-project [--auto @file.md]`
...
**Command:** `/ecl-discuss-phase [N] [--auto] [--batch]`
...
**Command:** `/ecl-plan-phase [N] [--auto] [--skip-research] [--skip-verify]`
...
- REQ-PLAN-07: System MUST prompt user to run `/ecl-ui-phase` if frontend phase detected and no UI-SPEC.md exists (UI safety gate)
...
**Commands:** `/ecl-audit-milestone`, `/ecl-complete-milestone`, `/ecl-new-milestone [name]`
...
**Commands:** `/ecl-phase`, `/ecl-phase --insert [N]`, `/ecl-phase --remove [N]`
...
**Command:** `/ecl-quick [--full] [--discuss] [--research]`

**Purpose:** Ad-hoc task execution with eCL guarantees but a faster path.
...
**Command:** `/ecl-progress --do` (see also `/ecl-manager` for interactive routing)

**Purpose:** Analyze freeform text and route to the appropriate eCL command.

**Requirements:**
- REQ-DO-01: System MUST parse user intent from natural language input
- REQ-DO-02: System MUST map intent to the best matching eCL command
...
- REQ-NEXT-04: System MUST suggest `/ecl-new-project` if no project exists
- REQ-NEXT-05: System MUST suggest `/ecl-complete-milestone` when all phases are complete

**State Detection Logic:**
| State | Action |
|-------|--------|
| No `.planning/` directory | Suggest `/ecl-new-project` |
| Phase has no CONTEXT.md | Run `/ecl-discuss-phase` |
| Phase has no PLAN.md files | Run `/ecl-plan-phase` |
| Phase has plans but no SUMMARY.md | Run `/ecl-execute-phase` |
| Phase executed but no VERIFICATION.md | Run `/ecl-verify-work` |
| All phases complete | Suggest `/ecl-complete-milestone` |
...
- REQ-NYQ-05: System MUST support retroactive validation via `/ecl-validate-phase`
...
**Retroactive Validation (`/ecl-validate-phase [N]`):**
...
- REQ-POSTVER-03: System MUST log issues for `/ecl-verify-work` to address
...
**When:** Runs automatically during `/ecl-execute-phase` before the verifier step.
...
**When:** Runs automatically at the end of `/ecl-plan-phase` after the plan checker loop.
...
- REQ-CTX-06: Context monitor MUST differentiate eCL-active vs non-eCL-active projects
...
**Commands:** `/ecl-pause-work`, `/ecl-resume-work`, `/ecl-progress`
...
**Command:** `/ecl-config --profile <quality|balanced|budget|adaptive|inherit>`
...
| ecl-nyquist-auditor | Sonnet | Sonnet | Haiku | Inherit |
...
**Command:** `/ecl-map-codebase [area]`

**Purpose:** Analyze an existing codebase before starting a new project, so eCL understands what exists.
...
- REQ-MAP-04: Subsequent `/ecl-new-project` MUST load codebase mapping and focus questions on what's being added
...
**Introduced by:** #2003
**Trigger:** Runs automatically at the end of every `/ecl-execute-phase`
**Configuration:**
...
- `workflow.drift_action` (`warn` | `auto-remap`, default `warn`) —
  warn-only or spawn `ecl-codebase-mapper` with `--paths` scoped to
  affected subtrees.
...
- REQ-DRIFT-05: Detection/remap failure MUST be non-blocking for `/ecl-execute-phase`
...
**Commands:** `/ecl-capture [desc]`, `/ecl-capture --list`
...
**Purpose:** Update eCL to the latest version with changelog preview.
...
- REQ-SETTINGS-03: System MUST support saving as global defaults (`~/.ecl/defaults.json`)
...
**Purpose:** Run eCL across multiple AI coding agent runtimes.
...
- REQ-RUNTIME-05: Uninstall MUST cleanly remove all eCL files without affecting other configurations
...
- `~/.cursor/commands/ecl-<name>.md` — plain markdown slash commands (no frontmatter) invocable via `/` in the Agent input (Cursor 1.6+)

**Native skills emission (1.4.0):** Three runtimes now emit eCL as on-demand native skills (`skills/<name>/SKILL.md`) at install time, in addition to their existing command and agent surfaces. Skills respect the active install profile and are removed on uninstall.
...
- **CodeBuddy** — `/ecl-*` slash commands written to `~/.codebuddy/commands/`
- **Augment** — `commands/ecl-<name>.md` written to `~/.augment/commands/`
- **Cursor** (Cursor >= 1.6) — `.cursor/commands/ecl-<name>.md` so eCL appears in the `/` command menu
...
- Codex emits `service_tier: flex` for light-tier agents; eCL skills appear in the Codex `/skills` picker via `SKILL.md` (no `agents/openai.yaml` sidecar is emitted — doing so caused duplicate autocomplete entries, #1326)
- Gemini commands use native `{{args}}` interpolation

**Native packaging:**
- **Claude Code:** eCL Core ships a `.claude-plugin/plugin.json` manifest, enabling installation and lifecycle management via `claude plugin install|enable|disable|update evolv-coder-lite`. Commands load under the `/evolv-coder-lite:` namespace (e.g. `/evolv-coder-lite:plan-phase`), avoiding slash-command collisions with the classic npm installer which uses `/ecl:`. Always-on guard and update hooks are wired automatically via `hooks/hooks.json`. The plugin path is additive — the npm installer (`npx @evolvconsulting/evolv-coder-lite`) remains fully supported.
...
**Statusline Display:**
```text
[⬆ /ecl-update │] model │ [current task │] directory [█████░░░░░ 50%]
```

Color coding: <50% green, <65% yellow, <80% orange, ≥80% red with skull emoji

**Update Banner (opt-in, when eCL statusline isn't used):**

When the user declines (or keeps a non-eCL) statusline, the installer offers a SessionStart banner that surfaces update availability without occupying statusline real estate. The banner reads `~/.cache/ecl/ecl-update-check.json` (written by `ecl-check-update-worker.js`) and emits one line only when an update is available:

```text
eCL update available: 1.39.0 → 1.40.0. Run /ecl-update.
```

The banner is silent when up-to-date and rate-limits "check failed" diagnostics to once per 24 hours. Removed cleanly by `npx @evolvconsulting/evolv-coder-lite --uninstall` or by deleting the SessionStart entry that references `ecl-update-banner.js`.

### 38. Developer Profiling

**Command:** `/ecl-profile-user [--questionnaire] [--refresh]`
...
- `ecl-user-profiler` agent — Behavioral analysis from session data
...
Every `/ecl-progress` call scans ALL phases in the current milestone for outstanding items (pending, skipped, blocked, human_needed). Displays a non-blocking warning section with actionable links.
...
**Requirements:**
- REQ-DEBT-01: System MUST surface outstanding UAT/verification items from ALL prior phases in `/ecl-progress`
...
- REQ-DEBT-06: `/ecl-audit-uat` MUST scan all phases, categorize items by testability, and produce a human test plan
...
**Command:** `/ecl-fast [task description]`
...
**When to use vs `/ecl-quick`:**
- `/ecl-fast` — One-sentence tasks executable in under 2 minutes (typo, config change, small addition)
- `/ecl-quick` — Anything needing research, multi-step planning, or verification
...
- REQ-REVIEW-05: Reviews MUST be consumable by `/ecl-plan-phase --reviews`
...
- Set `review.default_reviewers` in `.planning/config.json` (or via `ecl config-set`) to control no-flag `/ecl-review` fan-out.
...
**Commands:** `/ecl-capture --backlog <description>`, `/ecl-review-backlog`, `/ecl-capture --seed <idea>`
...
- REQ-BACKLOG-02: Phase directories MUST be created immediately so `/ecl-discuss-phase` and `/ecl-plan-phase` work on them
- REQ-BACKLOG-03: `/ecl-review-backlog` MUST support promote, keep, and remove actions per item
...
- REQ-SEED-02: `/ecl-new-milestone` MUST scan seeds and present matches
...
**Command:** `/ecl-thread [name | description]`

**Purpose:** Lightweight cross-session knowledge stores for work that spans multiple sessions but doesn't belong to any specific phase. Lighter weight than `/ecl-pause-work` — no phase state, no plan context.
...
**Command:** `/ecl-pr-branch [target branch]`

**Purpose:** Create a clean branch suitable for pull requests by filtering out `.planning/` commits. Reviewers see only code changes, not eCL planning artifacts.
...
**Purpose:** Defense-in-depth security for eCL's planning artifacts. Because eCL generates markdown files that become LLM system prompts, user-controlled text flowing into these files is a potential indirect prompt injection vector.
...
**2. Prompt Injection Guard Hook** (`ecl-prompt-guard.js`)
...
**3. Workflow Guard Hook** (`ecl-workflow-guard.js`)
PreToolUse hook that detects when Claude attempts file edits outside a eCL workflow context. Advises using `/ecl-quick` or `/ecl-fast` instead of direct edits. Configurable via `hooks.workflow_guard` (default: false).
...
**Purpose:** Auto-generate `DISCUSSION-LOG.md` during `/ecl-discuss-phase` for full audit trail of decisions made during discussion.
...
**Command:** `/ecl-forensics [description]`

**Purpose:** Post-mortem investigation of failed or stuck eCL workflows.
...
**Command:** `/ecl-milestone-summary [version]`
...
2. **Switch** — Change active workstream context for subsequent eCL commands
...
**Command:** `/ecl-discuss-phase` with `workflow.discuss_mode: 'assumptions'`
...
**Part of:** `/ecl-new-project` and `/ecl-progress`

**Purpose:** Automatically detect UI-heavy projects and surface `/ecl-ui-phase` recommendation.
...
- REQ-UI-DETECT-03: System MUST suggest `/ecl-ui-phase` in next steps for UI-heavy phases
- REQ-UI-DETECT-04: System MUST NOT make `/ecl-ui-phase` mandatory
...
3. **Surface** — Include `/ecl-ui-phase` recommendation in next steps for UI-heavy phases

---

### 55. Multi-Runtime Installer Selection

**Part of:** `npx @evolvconsulting/evolv-coder-lite`
...
3. **Install** — Configure eCL for all selected runtimes in a single session
...
**Purpose:** Add Windsurf as a supported AI CLI runtime for eCL installation and execution.

**Requirements:**
- REQ-WINDSURF-01: Installer MUST detect Windsurf runtime and offer it as a target
- REQ-WINDSURF-02: eCL commands MUST function correctly within Windsurf sessions

**Process:**
1. **Detect** — Identify Windsurf runtime availability on the system
2. **Install** — Configure eCL skills and hooks for the Windsurf environment
...
**Purpose:** Provide eCL documentation in Portuguese, Korean, and Japanese.
...
**Command:** Automatic during `/ecl-execute-phase`
...
- REQ-SCHEMA-04: System MUST support `ECL_SKIP_SCHEMA_CHECK` env var to override detection
...
**Config:** `ECL_SKIP_SCHEMA_CHECK` environment variable to bypass detection.

---

### 60. Security Enforcement

**Command:** `/ecl-secure-phase <N>`
...
- REQ-SEC-04: System MUST spawn `ecl-security-auditor` agent for analysis
...
2. **Audit** — Spawn `ecl-security-auditor` to verify against threat model
...
**Command:** `/ecl-docs-update`

**Purpose:** Generate and verify project documentation with accuracy checks.

**Requirements:**
- REQ-DOCS-01: System MUST spawn `ecl-doc-writer` agent to generate documentation
- REQ-DOCS-02: System MUST spawn `ecl-doc-verifier` agent to check accuracy
...
**Process:**
1. **Generate** — Spawn `ecl-doc-writer` to create or update documentation from implementation
2. **Verify** — Spawn `ecl-doc-verifier` to check documentation accuracy against codebase
...
**Flag:** `/ecl-discuss-phase <N> --chain`
...
**Part of:** `/ecl-plan-phase --research-phase <N>`
...
**Purpose:** Migrate eCL commands to Claude Code 2.1.88+ skills format with backward compatibility.

**Requirements:**
- REQ-SKILLS-01: Installer MUST write `skills/ecl-*/SKILL.md` for Claude Code 2.1.88+
- REQ-SKILLS-02: Installer MUST auto-clean legacy `commands/ecl/` directory
...
2. **Migrate** — Write `skills/ecl-*/SKILL.md` files for each eCL command
3. **Clean** — Remove legacy `commands/ecl/` directory if skills are installed
...
**Part of:** `/ecl-execute-phase` (verifier step)
...
**Command:** `/ecl-manager --analyze-deps`

**Purpose:** Detect phase dependencies and suggest `Depends on` entries for ROADMAP.md before running `/ecl-manager`.
...
**Part of:** `/ecl-verify-work` (optional)
...
**Purpose:** Extend eCL installation to Trae IDE, Cline, and Augment Code runtimes.
...
**Flag:** `/ecl-autonomous --interactive`
...
**Hooks:** `ecl-validate-commit.sh`, `ecl-session-state.sh`, `ecl-phase-boundary.sh`

**Purpose:** Optional git and session hooks for eCL projects, gated behind `hooks.community: true` in config.

**Requirements:**
- REQ-COMMUNITY-01: All community hooks MUST be no-ops unless `hooks.community` is `true` in `.planning/config.json`
- REQ-COMMUNITY-02: `ecl-validate-commit.sh` MUST enforce Conventional Commits format on git commit messages
- REQ-COMMUNITY-03: `ecl-session-state.sh` MUST track session state transitions
- REQ-COMMUNITY-04: `ecl-phase-boundary.sh` MUST enforce phase boundary checks
...
**Command:** `/ecl-map-codebase --query [<term>|status|diff|refresh]`
...
**References:** `evolv-coder-lite/references/gates.md`
...
**Commands:** `/ecl-code-review`, `/ecl-code-review --fix`
...
**Requirements:**
- REQ-REVIEW-01: `ecl-code-review` MUST scope files to the phase using SUMMARY.md and git diff fallback
...
- REQ-REVIEW-04: `ecl-code-review --fix` MUST read REVIEW.md and fix Critical + Warning findings by default
...
**Command:** `/ecl-explore [topic]`

**Purpose:** Guide a developer through exploring an idea via Socratic probing questions before committing to a plan. Routes outputs to the appropriate eCL artifact: notes, todos, seeds, research questions, requirements updates, or a new phase.

**Requirements:**
- REQ-EXPLORE-01: Exploration MUST use Socratic probing — ask questions before proposing solutions
- REQ-EXPLORE-02: Session MUST offer to route outputs to the appropriate eCL artifact
...
**Command:** `/ecl-undo --last N | --phase NN | --plan NN-MM`

**Purpose:** Roll back eCL phase or plan commits safely using the phase manifest and git log, with dependency checks and a hard confirmation gate before any revert is applied.
...
- REQ-UNDO-03: `--last N` mode MUST display recent eCL commits for interactive selection
...
**Command:** `/ecl-import --from <filepath>`

**Purpose:** Ingest an external plan file into the eCL planning system with conflict detection against `PROJECT.md` decisions, converting it to a valid eCL PLAN.md and validating it through the plan-checker.
...
- REQ-IMPORT-03: Imported plan MUST be written as a valid eCL PLAN.md format
- REQ-IMPORT-04: Written plan MUST pass `ecl-plan-checker` validation

---

### 97. Rapid Codebase Scan

**Command:** `/ecl-map-codebase --fast [--focus tech|arch|quality|concerns]`

**Purpose:** Lightweight alternative to `/ecl-map-codebase` that spawns a single mapper agent for one or two combined focus areas, producing targeted output in `.planning/codebase/` without the overhead of 4 parallel agents.
...
- REQ-SCAN-03: Output MUST be written to `.planning/codebase/` in the same format as `/ecl-map-codebase`

---

### 98. Autonomous Audit-to-Fix

**Command:** `/ecl-audit-fix [--source <audit>] [--severity high|medium|all] [--max N] [--dry-run]`
...
### 101. Hard Stop Safety Gates in /ecl-progress --next

**Command:** `/ecl-progress --next`

**Purpose:** Prevent `/ecl-progress --next` from entering runaway loops by adding hard stop safety gates and a consecutive-call guard that interrupts autonomous chaining when repeated identical steps are detected.

**Requirements:**
- REQ-NEXT-GATE-01: `/ecl-progress --next` MUST track consecutive same-step calls
...
- REQ-ADAPTIVE-02: `adaptive` MUST be selectable via `/ecl-config --profile adaptive`

---

### 103. Post-Merge Hunk Verification

**Command:** `/ecl-update --reapply`
...
**Purpose:** Extend eCL installation to Cline, CodeBuddy, and Qwen Code runtimes.
...
### 105. eCL-2 Reverse Migration

**Command:** `/ecl-import --from-ecl2 [--dry-run] [--force] [--path <dir>]`

**Purpose:** Migrate a project from eCL-2 format (`.ecl/` directory with Milestone→Slice→Task hierarchy) back to the v1 `.planning/` format, restoring full compatibility with all eCL v1 commands.

**Requirements:**
- REQ-FROM-GSD2-01: Importer MUST read `.ecl/` from the specified or current directory
...
| `--path <dir>` | Specify the eCL-2 root directory |

---

### 106. AI Integration Phase Wizard

**Command:** `/ecl-ai-integration-phase [N]`
...
**Command:** `/ecl-plan-phase N --bounce`
...
**Command:** `/ecl-ship` (enhanced)

**Purpose:** Before the manual review step in `/ecl-ship`, automatically run an external code review command if configured. The command receives the diff and phase context via stdin and returns a JSON verdict (`APPROVED` or `REVISE`). Falls through to the existing manual review flow regardless of outcome.
...
**Command:** `/ecl-execute-phase N --cross-ai`
...
**Command:** `/ecl-plan-phase` (enhanced research step)
...
**Command:** `/ecl-extract-learnings N`
...
**Purpose:** Allow projects to store their CLAUDE.md in a non-root location. The `claude_md_path` config key controls where `/ecl-profile-user` and related commands write the generated CLAUDE.md file.

**Requirements:**
- REQ-CMDPATH-01: `claude_md_path` defaults to `./.claude/CLAUDE.md` (a valid project-scoped memory location; changed from `./CLAUDE.md` in v1.5 per [#1098](https://github.com/evolvconsulting/evolv-coder-lite/issues/1098) so generated content does not pollute a hand-crafted repo-root `CLAUDE.md`)
...
- REQ-CMDPATH-04: `generate-claude-md` never overwrites an existing instruction file that lacks eCL section markers (a hand-crafted file) unless `--force` is passed
...
**Command:** `/ecl-spike [idea] [--quick]`

**Purpose:** Run 2–5 focused feasibility experiments before committing to an implementation approach. Each experiment uses Given/When/Then framing, produces executable code, and returns a VALIDATED / INVALIDATED / PARTIAL verdict. Companion `/ecl-spike --wrap-up` packages findings into a project-local skill.
...
- REQ-SPIKE-06: `/ecl-spike --wrap-up` MUST package findings into `.claude/skills/spike-findings-[project]/`
...
| `.claude/skills/spike-findings-[project]/` | Packaged findings (via `/ecl-spike --wrap-up`) |

---

### 118. Sketch Command

**Command:** `/ecl-sketch [idea] [--quick] [--text]`

**Purpose:** Explore design directions through throwaway HTML mockups before committing to implementation. Produces 2–3 interactive variants per design question, all viewable directly in a browser with no build step. Companion `/ecl-sketch --wrap-up` packages winning decisions into a project-local skill.
...
- REQ-SKETCH-08: `/ecl-sketch --wrap-up` MUST package winning decisions into `.claude/skills/sketch-findings-[project]/`
...
| `.claude/skills/sketch-findings-[project]/` | Packaged decisions (via `/ecl-sketch --wrap-up`) |
...
**Requirements:**
- REQ-BUDGET-01: `agents/ecl-*.md` files are classified into three tiers: XL (≤ 1 600 lines), Large (≤ 1 000 lines), Default (≤ 500 lines)
...
**Purpose:** Build, query, and inspect a lightweight knowledge graph of the project in `.planning/graphs/`. Opt-in per project. Exposed as the `/ecl-graphify` user-facing command and the `ecl-tools.cjs graphify …` programmatic verb family. Complements `/ecl-map-codebase --query` (snapshot-oriented) with a graph-oriented view of nodes and edges across commands, agents, workflows, and phases.

**Requirements:**
- REQ-GRAPH-01: Opt-in via `graphify.enabled: true` in `.planning/config.json`. When disabled, `/ecl-graphify` prints an activation hint and stops without writing.
- REQ-GRAPH-02: Slash-command `/ecl-graphify` exposes subcommands `build`, `query <term>`, `status`, `diff`. The programmatic CLI `node ecl-tools.cjs graphify …` additionally exposes `snapshot`, which is also invoked automatically as the final step of `graphify build`.
...
- REQ-GRAPH-05: Graphify is invoked through `ecl-tools.cjs graphify ...` command handlers.

**Configuration:** `graphify.enabled`, `graphify.build_timeout`
**Reference files:** `commands/ecl/graphify.md`, `bin/lib/graphify.cjs`
...
**Purpose:** Cut the eager skill-listing overhead by folding 31 micro-skills into 4 new grouped parents and 6 existing parents that absorb sub-operations as flags. Zero functional loss — every removed micro-skill's behavior survives via a flag on a consolidated parent. After consolidation, `commands/ecl/*.md` ships 59 sub-skills (plus 6 namespace meta-skills, see #123).

**Requirements:**
- REQ-CONSOLIDATE-01: Four new grouped skills replace clusters of micro-skills:
  - `/ecl-capture` — folds add-todo (default), note (`--note`), add-backlog (`--backlog`), plant-seed (`--seed`), check-todos (`--list`)
  - `/ecl-phase` — folds add-phase (default), insert-phase (`--insert`), remove-phase (`--remove`), edit-phase (`--edit`)
  - `/ecl-config` — folds settings-advanced (`--advanced`), settings-integrations (`--integrations`), set-profile (`--profile`)
  - `/ecl-workspace` — folds new-workspace (`--new`), list-workspaces (`--list`), remove-workspace (`--remove`)
- REQ-CONSOLIDATE-02: Six existing parents absorb wrap-up / sub-operations as flags: `/ecl-update --sync`, `/ecl-update --reapply`, `/ecl-sketch --wrap-up`, `/ecl-spike --wrap-up`, `/ecl-map-codebase --fast`, `/ecl-map-codebase --query`, `/ecl-code-review --fix`, `/ecl-progress --do`, `/ecl-progress --next`.
- REQ-CONSOLIDATE-03: Deleted micro-skill slash forms (the bare `ecl-add-todo`, `ecl-add-backlog`, `ecl-plant-seed`, `ecl-check-todos`, `ecl-add-phase`, `ecl-insert-phase`, `ecl-remove-phase`, `ecl-edit-phase`, `ecl-new-workspace`, `ecl-list-workspaces`, `ecl-remove-workspace`, `ecl-settings-advanced`, `ecl-settings-integrations`, `ecl-set-profile`, `ecl-sketch-wrap-up`, `ecl-spike-wrap-up`, `ecl-reapply-patches`, `ecl-code-review-fix`, …) MUST resolve to "Unknown command" — no shadow stubs.
- REQ-CONSOLIDATE-04: `autonomous.md` invokes `/ecl-code-review --fix` (was previously calling the deleted `ecl-code-review-fix`).

**Reference issue:** [#2790](https://github.com/evolvconsulting/evolv-coder-lite/issues/2790)
...
**Commands:**
- `/ecl-workflow` — phase pipeline router (discuss / plan / execute / verify / phase / progress)
- `/ecl-project` — project lifecycle (milestones, audits, summary)
- `/ecl-quality` — quality gates (code review, debug, audit, security, eval, ui)
- `/ecl-context` — codebase intelligence (map, graphify, docs, learnings)
- `/ecl-manage` — config / workspace / workstreams / thread / update / ship / inbox
- `/ecl-ideate` — exploration & capture (explore, sketch, spike, spec, capture)
...
**Requirements:**
- REQ-NS-01: Six `commands/ecl/ns-*.md` namespace routers ship with pipe-separated keyword-tag descriptions (≤ 60 chars).
...
**Reference issue:** [#2792](https://github.com/evolvconsulting/evolv-coder-lite/issues/2792)

---

### 124. Context-Window Utilization Guard

**Command:** `/ecl-health --context`

**Purpose:** Quality guard against context-window saturation. Two thresholds: 60 % utilization warns ("consider `/ecl-thread`"), 70 % is critical ("reasoning quality may degrade"; matches the fracture-point per recent context-attention research).

**Requirements:**
- REQ-CTX-GUARD-01: `/ecl-health --context` prints a structured status line with current utilization, threshold tier (`ok` / `warn` / `critical`), and a remediation suggestion.
- REQ-CTX-GUARD-02: The same triage is exposed as `ecl-tools.cjs validate context --tokens-used <int> --context-window <int>` — a structured envelope for status-line and hook callers (#125). Both flags are required; the handler returns the same `{ percent, state }` envelope as the pure classifier in REQ-CTX-GUARD-03.
...
**Reference issue:** [#3023](https://github.com/evolvconsulting/evolv-coder-lite/pull/3030)
...
**Reference issue:** [#3024](https://github.com/evolvconsulting/evolv-coder-lite/pull/3031)

---

### 128. Update Banner Opt-In

**Purpose:** Surface update availability to users who have declined or bypassed the eCL statusline, without requiring the statusline.

**Behavior:**
- At install time, if the installer detects no eCL statusline, it offers an opt-in `SessionStart` hook.
- The hook reads the existing `~/.cache/ecl/ecl-update-check.json` cache — the same cache used by the statusline — and prints a banner only when an update is available.
- Silent when up-to-date.
- Failure diagnostics rate-limited to once per 24 h.
- Cleanly removed by `npx @evolvconsulting/evolv-coder-lite --uninstall`.
...
**Reference issue:** [#2795](https://github.com/evolvconsulting/evolv-coder-lite/pull/2795)

---

### 129. Issue-Driven Orchestration Guide

**Purpose:** Document a recipe for driving the full eCL workflow from a GitHub / Linear / Jira issue, mapping tracker-centric concepts onto existing eCL primitives.

**Document:** [`docs/issue-driven-orchestration.md`](issue-driven-orchestration.md)

**Covered workflow:**
1. Create an isolated workspace per issue (`/ecl-workspace --new`)
2. Run the manager dashboard to get oriented (`/ecl-manager`)
3. Execute autonomously (`/ecl-autonomous`)
4. Verify and review (`/ecl-verify-work`, `/ecl-review`)
5. Ship and close the issue (`/ecl-ship`)
...
**Reference issue:** [#2840](https://github.com/evolvconsulting/evolv-coder-lite/pull/2840)
...
**Reference issue:** [#3170](https://github.com/evolvconsulting/evolv-coder-lite/issues/3170)
...
**Runtime control:** `/ecl:surface` lists profile state and enables, disables, or resets skill clusters without reinstalling.

**Requirements:**
- REQ-SURFACE-01: Installer MUST resolve `--profile=<name>` and persist the active profile in `.ecl-profile`.
...
- Ambiguous eCL-looking files block with a clear report instead of being silently overwritten.
...
**Purpose:** Add project-specific PRD-style sections to generated PR bodies without editing eCL workflow files.
...
**Purpose:** Let teams choose the default reviewer subset for no-flag `/ecl-review` runs.
...
**Reference:** [Configuration Reference](CONFIGURATION.md#reviewer-defaults-for-ecl-review)

---

### 137. Fallow Structural Review Pre-Pass

**Command:** `/ecl-code-review`

**Config keys:** `code_quality.fallow.*`

**Purpose:** Add an optional structural analysis pass before the agent review.

**Behavior:** When enabled, eCL resolves a `fallow` binary, runs a bounded audit, writes `FALLOW.json`, and embeds structural findings in `REVIEW.md`.
...
**Reference:** [Checkpoints Reference](../evolv-coder-lite/references/checkpoints.md)

---

### 139. Quota and Rate-Limit Failure Classification

**Command:** `/ecl-execute-phase`
...
**Command:** `/ecl-complete-milestone`
...
**CLI:** `node ecl-tools.cjs phase uat-passed <N> [--require-verification]`
...
**Reference:** [Edge Probe](../evolv-coder-lite/references/edge-probe.md)
...
**Commands:** `/ecl-mempalace-recall`, `/ecl-mempalace-capture`
...
- REQ-MP-04: At `ship:post`, agent `ecl-mempalace-curator` writes a diary entry, proposes cross-project tunnels (when `mempalace.cross_project_tunnels: true`), and runs wing-scoped sync pruning.
- REQ-MP-05: `mempalace.memory_mode` declares three values: `augment` (default, **implemented** — palace is an additional recall layer alongside eCL native memory), `kg_backend` (**forward-declared; routing seam not yet implemented** — selecting this today behaves identically to `augment`), `replace` (**forward-declared; not yet functional** — selecting this today behaves identically to `augment`). Only `augment` has effect in the current release.
