 ...
 *   5. Source agents/ecl-planner.md has NO effort: key (injection is
 ...
 * directly (e.g. ~/.cache/ecl update-check deletion, ~/.ecl/defaults.json
 ...
 * $HOME/.claude/evolv-coder-lite/bin/ecl-tools.cjs is absent).
 *
 * ECL_SKIP_STALE_SDK_CHECK=1 is set to suppress the `npm ls -g` subprocess
...
function runGlobalInstall(runtime, tmpHome) {
  ...
  // never touches the real $HOME/.claude / $HOME/.cache / $HOME/.ecl.
  const isolatedHome = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-443-home-'));
...
// ecl-planner: heavy → xhigh
// ecl-codebase-mapper: light → low
// ecl-executor: standard → high
...
describe('#443 Claude install: effort: injected into frontmatter', () => {
  ...
  beforeEach(() => {
    tmpDir = makeTmpDir('ecl-443-claude-');
  ...
  test('ecl-planner.md contains effort: xhigh (heavy tier default)', () => {
    runGlobalInstall('claude', claudeHome);
    const fm = readFrontmatter(path.join(claudeHome, 'agents', 'ecl-planner.md'));
    assert.match(fm, /^effort:\s*xhigh$/m,
      `ecl-planner frontmatter should have effort: xhigh\nActual:\n${fm}`);
  });

  test('ecl-codebase-mapper.md contains effort: low (light tier default)', () => {
    runGlobalInstall('claude', claudeHome);
    const fm = readFrontmatter(path.join(claudeHome, 'agents', 'ecl-codebase-mapper.md'));
    assert.match(fm, /^effort:\s*low$/m,
      `ecl-codebase-mapper frontmatter should have effort: low\nActual:\n${fm}`);
  });

  test('ecl-executor.md contains effort: high (standard tier default)', () => {
    runGlobalInstall('claude', claudeHome);
    const fm = readFrontmatter(path.join(claudeHome, 'agents', 'ecl-executor.md'));
    assert.match(fm, /^effort:\s*high$/m,
      `ecl-executor frontmatter should have effort: high\nActual:\n${fm}`);
  });
});
...
describe('#443 Gemini install: effort: absent (Gemini-safe)', () => {
  ...
  beforeEach(() => {
    tmpDir = makeTmpDir('ecl-443-gemini-');
  ...
  test('ecl-planner.md does NOT contain effort: (Gemini install)', () => {
    runGlobalInstall('gemini', geminiHome);
    const fm = readFrontmatter(path.join(geminiHome, 'agents', 'ecl-planner.md'));
    assert.doesNotMatch(fm, /^effort:/m,
      `ecl-planner (Gemini) frontmatter must NOT have effort:\nActual:\n${fm}`);
  });

  test('ecl-executor.md does NOT contain effort: (Gemini install)', () => {
    runGlobalInstall('gemini', geminiHome);
    const fm = readFrontmatter(path.join(geminiHome, 'agents', 'ecl-executor.md'));
    assert.doesNotMatch(fm, /^effort:/m,
      `ecl-executor (Gemini) frontmatter must NOT have effort:\nActual:\n${fm}`);
  });
});
...
describe('#838 Codex install: inherited model omits model_reasoning_effort', () => {
  ...
  beforeEach(() => {
    tmpDir = makeTmpDir('ecl-443-codex-');
  ...
  test('ecl-planner.toml omits both model and model_reasoning_effort when model is inherited', () => {
    runGlobalInstall('codex', codexHome);
    const tomlContent = fs.readFileSync(
      path.join(codexHome, 'agents', 'ecl-planner.toml'), 'utf8'
    );
    assert.doesNotMatch(tomlContent, /^model\s*=/m,
      `ecl-planner.toml should omit model when inheriting Codex chat model\nActual:\n${tomlContent.slice(0, 500)}`);
    assert.doesNotMatch(tomlContent, /^model_reasoning_effort\s*=/m,
      `ecl-planner.toml should omit model_reasoning_effort when model is inherited\nActual:\n${tomlContent.slice(0, 500)}`);
  });
});
...
describe('#443 Config-driven: effort.agent_overrides drives install-time effort', () => {
  ...
  beforeEach(() => {
    ...
    // Write a project config with effort.agent_overrides overriding ecl-planner to 'low'.
  ...
  test('Claude .md gets effort: low when agent_overrides.ecl-planner=low', () => {
    ...
    const fm = readFrontmatter(path.join(claudeHome, 'agents', 'ecl-planner.md'));
    assert.match(fm, /^effort:\s*low$/m,
      `ecl-planner should have effort: low from config override\nActual:\n${fm}`);
  });

  test('Codex .toml gets model_reasoning_effort = "low" when agent_overrides.ecl-planner=low', () => {
    runGlobalInstall('codex', codexHome);
    const tomlContent = fs.readFileSync(
      path.join(codexHome, 'agents', 'ecl-planner.toml'), 'utf8'
    );
    assert.match(tomlContent, /^model\s*=\s*"gpt-5.5"$/m,
      `ecl-planner.toml should pin Codex model when runtime:"codex" is configured\nActual:\n${tomlContent.slice(0, 500)}`);
    assert.match(tomlContent, /^model_reasoning_effort\s*=\s*"low"$/m,
      `ecl-planner.toml should have model_reasoning_effort = "low" from config override\nActual:\n${tomlContent.slice(0, 500)}`);
  });

  test('Codex .toml clamps effort max → xhigh when agent_overrides.ecl-planner=max', () => {
    ...
    assert.match(tomlContent, /^model_reasoning_effort\s*=\s*"xhigh"$/m,
      `ecl-planner.toml should clamp max → xhigh for Codex\nActual:\n${tomlContent.slice(0, 500)}`);
...
describe('#443 resolveInstallTimeEffort: invalid tokens fall through to valid effort', () => {
  ...
  beforeEach(() => {
    ...
    //           .claude/          <-- claudeHome
    //           .codex/           <-- codexHome
    tmpDir = makeTmpDir('ecl-443-invalid-effort-');
    const projectDir = path.join(tmpDir, 'project');
    claudeHome = path.join(projectDir, '.claude');
    codexHome = path.join(projectDir, '.codex');
  ...
  test('effort.agent_overrides.ecl-planner="bogus" (invalid) with valid default -> falls through to valid default', () => {
    // BUG before fix: "bogus" is returned and written verbatim
    writeProjectConfig({
      effort: {
        agent_overrides: { 'ecl-planner': 'bogus' },
        default: 'medium',
      },
    });
    runGlobalInstall('claude', claudeHome);
...
describe('#443 Source purity: agents/ecl-planner.md has no effort: key', () => {
  test('source agents/ecl-planner.md frontmatter does not contain effort:', () => {
    const fm = readFrontmatter(path.join(SOURCE_AGENTS_DIR, 'ecl-planner.md'));
    assert.doesNotMatch(fm, /^effort:/m,
      `Source agents/ecl-planner.md must NOT contain effort: (injection is install-only)`);
  });

  test('source agents/ecl-executor.md frontmatter does not contain effort:', () => {
    const fm = readFrontmatter(path.join(SOURCE_AGENTS_DIR, 'ecl-executor.md'));
    assert.doesNotMatch(fm, /^effort:/m,
      `Source agents/ecl-executor.md must NOT contain effort: (injection is install-only)`);
  });

  test('source agents/ecl-codebase-mapper.md frontmatter does not contain effort:', () => {
    const fm = readFrontmatter(path.join(SOURCE_AGENTS_DIR, 'ecl-codebase-mapper.md'));
    assert.doesNotMatch(fm, /^effort:/m,
      `Source agents/ecl-codebase-mapper.md must NOT contain effort: (injection is install-only)`);
  });
});
