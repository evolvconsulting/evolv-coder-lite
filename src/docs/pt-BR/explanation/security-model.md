# Modelo de segurança do eCL Core

> **Explicação** — Este documento descreve *por que* o eCL Core possui a
> postura de segurança que possui e *como as camadas se articulam*. Não é uma
> referência para todos os parâmetros de hook. Para o comando `/ecl-secure-phase`
...
O eCL Core sim. O pipeline pesquisa → plano → execução automatiza o caminho
...
O modelo de segurança do eCL Core é construído em torno de um princípio
...
pesquisador → planejador → executor do eCL e eventualmente seria executado como
...
**Estágio de pesquisa.** Quando `ecl-phase-researcher` recomenda pacotes
...
**Estágio de planejamento.** `ecl-planner` lê a tabela de auditoria. Para
...
**Estágio de execução.** Se uma instalação falhar, `ecl-executor` **exibe um
...
pip falhar no momento da pesquisa), o eCL aplica o fallback mais restrito
...
O eCL Core gera arquivos Markdown que se tornam prompts de sistema de LLMs. O
...
O eCL Core trata a injeção de prompt em três níveis.

**Validação de entrada (`security.cjs`).** O módulo
`evolv-coder-lite/bin/lib/security.cjs` é o utilitário central de segurança.
Ele fornece:
...
**Hook de runtime: `ecl-prompt-guard.js`.** Este hook é acionado a cada
...
**Hook de runtime: `ecl-read-injection-scanner.js`.** Este hook é acionado na
...
eCL está prestes a incorporar ao contexto de um agente.
...
código-fonte do eCL — por exemplo, um ataque de cadeia de suprimentos que
modificou um arquivo de workflow para adicionar uma instrução de sobrescrita de
papel.

### Read Injection Scanner vs Prompt Guard

Os dois hooks cobrem superfícies complementares. `ecl-prompt-guard.js` monitora
*gravações em artefatos de planejamento* — ele detecta injeções sendo plantadas.
`ecl-read-injection-scanner.js` monitora *leituras de qualquer arquivo* — ele
...
Acima do comportamento de runtime do eCL, a organização `evolvconsulting` aplica
...
`https://github.com/evolvconsulting/evolv-coder-lite/security/advisories/new`. Não abra
...
- [Comandos](../COMMANDS.md) — inclui `/ecl-secure-phase` e
  `/ecl-code-review` com flags relevantes para segurança
- [Arquitetura § Sistema de Hooks](../ARCHITECTURE.md#hook-system) —
