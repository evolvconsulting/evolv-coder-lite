...
beforeEach(async () => {
  previousGsdAgentsDir = process.env.ECL_AGENTS_DIR;
  tmpDir = await mkdtemp(join(tmpdir(), 'ecl-init-complex-'));
...
afterEach(async () => {
  if (previousGsdAgentsDir === undefined) delete process.env.ECL_AGENTS_DIR;
  else process.env.ECL_AGENTS_DIR = previousGsdAgentsDir;
...
describe('initNewProject', () => {
  ...
  it('separates required agent registration from skill payload availability (#3388)', async () => {
    ...
    process.env.ECL_AGENTS_DIR = emptyAgentsDir;
    ...
    await writeFile(join(tmpDir, '.planning', 'config.json'), JSON.stringify({
      ...
      commit_docs: false,
      agent_skills: {
        'ecl-project-researcher': ['.claude/skills/ecl-project-researcher'],
        'ecl-research-synthesizer': ['.claude/skills/ecl-research-synthesizer'],
        'ecl-roadmapper': ['.claude/skills/ecl-roadmapper'],
      },
      workflow: { research: true, plan_check: true, verifier: true, nyquist_validation: true },
    }));
...
describe('initProgress', () => {
  ...
  it('derives completed_count from ROADMAP [x] checkboxes when phases/ is absent', async () => {
    ...
    const tmp = await mkdtemp(join(tmpdir(), 'ecl-init-complex-2646-'));
    try {
  ...
  it('treats terminal heading labels as complete when selecting next_phase (#3472)', async () => {
    const tmp = await mkdtemp(join(tmpdir(), 'ecl-init-complex-3472-'));
    try {
      await mkdir(join(tmp, '.planning'), { recursive: true });
      await writeFile(join(tmp, '.planning', 'config.json'), JSON.stringify({
        model_profile: 'balanced',
...
describe('initProgress workstream (#2731)', () => {
  it('scans phases from workstream subdirectory, not root', async () => {
    const tmp = await mkdtemp(join(tmpdir(), 'ecl-ws-progress-'));
    try {
...
describe('initManager workstream (#2731)', () => {
  it('reads ROADMAP.md from workstream subdirectory, not root', async () => {
    const tmp = await mkdtemp(join(tmpdir(), 'ecl-ws-manager-'));
    try {
      const wsBase = join(tmp, '.planning', 'workstreams', 'production-support');
...
// reported `pending` (disk-only policy). This mismatch meant /ecl-manager and
// /ecl-progress disagreed on the same data. Post-fix: both apply the
...
describe('initProgress + initManager precedence (#2674)', () => {
  ...
  beforeEach(async () => {
    precedenceDir = await mkdtemp(join(tmpdir(), 'ecl-2674-'));
