# ADR-1239: eCL as an Embeddable Orchestration Engine

- **Status:** Proposed
- **Date:** 2026-06-14
- **Issue:** [#1239](https://github.com/evolvconsulting/evolv-coder-lite/issues/1239)
- **Epic:** [#857](https://github.com/evolvconsulting/evolv-coder-lite/issues/857) (Capability system)
...
- **Distinct from:** [#956](https://github.com/evolvconsulting/evolv-coder-lite/issues/956) (third-party *feature* plugins / Connected Capabilities)

## Context

eCL is a **standalone installer that projects onto a host** — `npx @evolvconsulting/evolv-coder-lite --codex` writes artifacts into `~/.codex` via a per-runtime descriptor (ADR-1016). That answers only "*how do we write our files onto a CLI we already know*." It does not let eCL be **embedded as an orchestration engine** a host loads as a plugin: a new host (a "pi console") has no path, and the dependency points the wrong way (eCL reaches into the host instead of the host embedding eCL).

We want the inversion: **eCL is the engine; the host loads it through a stable, negotiated interface; a third party writes the thin host-plugin.** This is "turn the CLIs into Capabilities like we did for the loop."

### The six interface points (the integration surface, already implicit in the code)

1. **Command / workflow invocation** — `ecl-tools.cjs` Command Routing Hub (ADR-0012) + the workflow/slash surface.
2. **Agent dispatch** — eCL spawns sub-agents through the host's Agent/Task primitive.
3. **Model invocation** — eCL tiers → host model ids.
...
6. **Artifact surface** — how the host renders eCL's commands/agents/skills.
...
- **Declarative** (files only, "no in-process extension API"): Gemini CLI (TOML commands + `.md` agents + 10 hook scripts), Cursor (`.mdc`/`.md` + 19 hook events via `hooks.json`), Codex (AGENTS.md prose + `/skills` menu, **no custom slash commands**), Cline-via-rules (`.clinerules` text — the surface eCL uses today, **0 programmatic events**).
...
- **`embeddingMode`**: `imperative` | `declarative` — does the host run eCL as code or interpret eCL's artifacts?
- **`commandSurface`**: `slash-file` (Claude/OpenCode, `ecl:`-namespaced) | `slash-programmatic` (pi/VS-Code-chat) | `slash-toml` (Gemini, `ecl.`-namespaced) | `palette` (VS Code) | `prose-only` (Codex). Drives how interface point 1 binds; `prose-only` is a real degradation.
- **`dispatch`**: `{ namedDispatch: bool, nested: bool, maxDepth: int, background: bool, subagentToolkit: 'full'|'read-only' }`. eCL's orchestration **flattens** when `maxDepth`/`nested` are insufficient (run plan/execute inline) — the #853 rule, generalized and tested.
- **`modelMode`**: `active` (host exposes `sendRequest`/provider registration → eCL calls the model: VS Code, pi) | `passive` (eCL can only inject prompts/instructions: Gemini/Cursor/Cline/Codex/OpenCode). Two model-layer adapters; `passive` means eCL expresses orchestration declaratively.
- **`hookBus`**: `host` (host fires events eCL subscribes to: Gemini/Cursor/Hermes/Codex/pi/OpenCode) | `engine` (host has no bus → eCL owns it internally and fires its own: VS Code) | `none` (no bus → degrade lifecycle gating to rule-text instructions: Cline-rules). Plus the **portable event floor** (`SessionStart`/`PreToolUse`/`PostToolUse`/`Stop`/`SessionEnd` — the "claude dialect" all hook-capable hosts share) and negotiated extended events.
...
- **`transport`**: `mcp` (near-universal — Claude/Codex/OpenCode/VS-Code/Gemini/Cursor/Cline/Hermes all consume MCP) | `native-extension` (pi: MCP needs a community extension) — eCL may ship a **companion MCP server** binding interface points 1+5 (the MemPalace pattern, already shipping).
...
|---|---|---|---|
| 1 Command | `slash-file`/`slash-programmatic` (Claude, OpenCode, pi, Gemini, Cursor) | `slash-toml` namespacing (Gemini `ecl.`-prefixed) | `prose-only` (Codex): commands become AGENTS.md prose + skills menu |
...
**Positive:** eCL embeds into any host with a plugin mechanism (Codex/OpenCode/pi today; a new "pi console" tomorrow) with no eCL source change; projection and imperative embedding unify under one contract; the agent-nesting bug class becomes a declared, tested capability; the engine gains a clean boundary; per-host degradation is explicit and testable rather than scattered `runtime === '…'` checks.
...
- **Phase B — Engine ↔ host boundary**: separate orchestration core (loop, `ecl-tools`, state) from install/projection; fold per-runtime residue into descriptors (absorbs #1173/ADR-1235 + the `install.js` residue list); add `destSubpath` write-confinement.
...
3. **Expose only `ecl-tools.cjs` as "the API"** — rejected: the engine is the *loop* (dispatch + hooks + model + state), not just deterministic CLI ops; the six points are irreducible.
...
|---|---|---|---|---|---|---|---|
| Claude Code | imperative | slash-file (`ecl:`-ns) | nested fg ∞ / bg depth-5; `Agent()` | passive (per-subagent `model`) | host (30) | yes (bundle) | node |
...
| Gemini CLI | declarative | slash-toml (`ecl.`-ns) | **flat** (no nesting) | passive (sub-agent `model:`) | host (10: BeforeAgent/Model/Tool) | yes | node |
| Cursor | declarative | slash-file (`ecl-`-ns) | host sub-agents; 19 hook events | passive | host (19) | yes | node |
