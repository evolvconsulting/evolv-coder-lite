 ...
 *
 * Ported from evolv-coder-lite/bin/lib/state.cjs.
 ...
 *
 * @param args - Either `--field value` pairs (CLI / ecl-tools) or a single JSON object string (SDK).
 * @param projectDir - Project root directory
 ...
 *
 * Accepts ecl-tools-style argv: `--phase N [--name S] [--plans C]` or positional
 ...
 *
 * @param args - ecl-tools argv: `--phase`, `--plan`, `--duration`, `--tasks`, `--files`
 * @param projectDir - Project root directory
 ...
 * argv matches `ecl-tools.cjs`: `--phase`, `--summary`, `--rationale`, etc.
 */
 ...
 * argv: `--text`, `--text-file` (see `ecl-tools.cjs`).
 */
 ...
 *
 * The `/ecl-new-milestone` workflow only rewrote STATE.md's body (Current
 ...
 *
 * Args (named, matches ecl-tools style):
 ...
 * `state.milestone-switch` from `/ecl-new-milestone` Step 5 in place of the
 * manual body rewrite.
 */
export const stateMilestoneSwitch: QueryHandler = async (args, projectDir, workstream) => {
  // NOTE: the CLI flag is `--milestone` (not `--version`). ecl-tools reserves
  ...
  try {
    ...
    if (existingFm.ecl_state_version) {
      fm.ecl_state_version = existingFm.ecl_state_version;
    }
...
// ─── parseNamedArgs (matches ecl-tools.cjs) ───────────────────────────────
 ...
 *
 * Writes `WAITING.json` under both `.ecl/` and `.planning/` so readers that only
...
export const stateSignalWaiting: QueryHandler = async (args, projectDir, _workstream) => {
  ...
  const waitingPaths = [
    join(projectDir, '.ecl', 'WAITING.json'),
  ...
  try {
    ...
    mkdirSync(join(projectDir, '.ecl'), { recursive: true });
