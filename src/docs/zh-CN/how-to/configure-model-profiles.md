...
在 `.planning/config.json` 中设置 `model_profile`，或通过 `/ecl-config --profile <name>` 设置：
...
`model_overrides` 可在 `.planning/config.json` 中按项目设置，也可在 `~/.ecl/defaults.json` 中全局设置。项目级条目在冲突时优先；不冲突的全局条目会被保留。
...
全部五个研究代理解析为 `sonnet`，*除* `ecl-codebase-mapper` 被固定为 `haiku` 之外。
...
每个代理都有一个默认层级（`light`、`standard` 或 `heavy`）。第一次尝试时，eCL Core 选择 `tier_models[default_tier]`。如果编排器检测到软失败（验证不确定、计划检查被标记等），则将代理提升一级重新启动。`max_escalations` 限制总重试次数。
...
## 在非 Anthropic 运行时上使用 eCL Core

如果您为 Codex、OpenCode、Gemini CLI 或 Kilo 安装了 eCL Core，安装程序已在您的配置中设置了 `resolve_model_ids: "omit"`。这告知 eCL Core 跳过 Anthropic 模型 ID 解析，让运行时选择其自己的默认模型。基本情况下无需手动设置。
...
eCL Core 将每个层级别名解析为运行时层级映射中定义的 Codex 原生模型和推理力度。
