...
eCLは、ユーザーとAIコーディングエージェント（Claude Code、Gemini CLI、OpenCode、Kilo、Codex、Copilot、Antigravity、Trae、Cline、Augment Code）の間に位置する**メタプロンプティングフレームワーク**です。以下の機能を提供します：
...
- **Claude Code:** カスタムスラッシュコマンド（`/ecl-command-name`）
- **OpenCode / Kilo:** スラッシュコマンド（`/ecl-command-name`）
- **Codex:** スキル（`$ecl-command-name`）
- **Copilot:** スラッシュコマンド（`/ecl-command-name`）
- **Antigravity:** スキル
...
### CLIツール（`evolv-coder-lite/bin/`）

17のドメインモジュールを持つNode.js CLIユーティリティ（`ecl-tools.cjs`）：
...
| カテゴリ | エージェント | 並列実行 |
|----------|--------|-------------|
| **リサーチャー** | ecl-project-researcher, ecl-phase-researcher, ecl-ui-researcher, ecl-advisor-researcher | 4並列（stack、features、architecture、pitfalls）; advisorはdiscuss-phase中に起動 |
| **シンセサイザー** | ecl-research-synthesizer | 逐次（リサーチャー完了後） |
| **プランナー** | ecl-planner, ecl-roadmapper | 逐次 |
| **チェッカー** | ecl-plan-checker, ecl-integration-checker, ecl-ui-checker, ecl-nyquist-auditor | 逐次（検証ループ、最大3回反復） |
| **エグゼキューター** | ecl-executor | ウェーブ内は並列、ウェーブ間は逐次 |
| **ベリファイアー** | ecl-verifier | 逐次（全エグゼキューター完了後） |
| **マッパー** | ecl-codebase-mapper | 4並列（tech、arch、quality、concerns） |
| **デバッガー** | ecl-debugger | 逐次（インタラクティブ） |
| **オーディター** | ecl-ui-auditor | 逐次 |
...
├── commands/ecl/*.md               # 37 slash commands
├── evolv-coder-lite/
...
├── hooks/
│   ├── ecl-statusline.js           # Statusline hook
│   ├── ecl-context-monitor.js      # Context warning hook
│   └── ecl-check-update.js         # Update check hook
...
7. **パッチバックアップ** — v1.17以降、ローカルで変更されたファイルを `/ecl-update --reapply` 用に `ecl-local-patches/` へバックアップ
8. **マニフェスト追跡** — クリーンアンインストールのために `ecl-file-manifest.json` を書き込み
9. **アンインストールモード** — `--uninstall` ですべてのeCLファイル、フック、設定を削除
...
- eCLワークフローコンテキスト外での編集を検出（アクティブな `/ecl-` コマンドやTaskサブエージェントがない場合）
...
eCLは統一されたコマンド/ワークフローアーキテクチャを通じて複数のAIコーディングランタイムをサポートしています：

| ランタイム | コマンド形式 | エージェントシステム | 設定場所 |
|---------|---------------|--------------|-----------------|
| Claude Code | `/ecl-command` | Task起動 | `~/.claude/` |
| OpenCode | `/ecl-command` | サブエージェントモード | `~/.config/opencode/` |
| Kilo | `/ecl-command` | サブエージェントモード | `~/.config/kilo/` |
| Gemini CLI | `/ecl-command` | Task起動 | `~/.gemini/` |
| Codex | `$ecl-command` | スキル | `~/.codex/` |
| Copilot | `/ecl-command` | エージェント委譲 | `~/.github/` |
...
5. **モデル参照** — `inherit` プロファイルにより、eCLはランタイムのモデル選択に委譲
