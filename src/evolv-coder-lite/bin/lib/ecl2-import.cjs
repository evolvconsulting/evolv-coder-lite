...
function buildPreview(gsd2Data, artifacts, projectDir) {
  ...
  lines.push(`  - eCL-2 database state (rebuilt from files on first ${formatGsdSlash('health', resolveRuntime(projectDir))})`);
...
function cmdFromGsd2(args, cwd, raw) {
  ...
  if (!gsdDir) {
    return output({ success: false, error: `No .ecl/ directory found in ${projectDir}` }, raw);
  }
