/**
 * eCL Tools Tests - Agent Skills Injection
 *
...
/**
 * Run ecl-tools and capture BOTH stdout and stderr on success.
...
const { loadTrustedGlobalRoots, validatePath } = require('../evolv-coder-lite/bin/lib/security.cjs');
...
describe('agent-skills command', () => {
  ...
  test('returns empty block when no config exists', () => {
    const r = runAgentSkillsJson(['agent-skills', 'ecl-executor'], tmpDir);
    assert.ok(r.success, `Command failed: ${r.error}`);
    assert.strictEqual(r.ir.agent_type, 'ecl-executor');
  ...
  test('returns empty block for unconfigured agent type', () => {
    writeConfig(tmpDir, {
      agent_skills: {
        'ecl-executor': ['skills/test-skill'],
      },
    });
    const r = runAgentSkillsJson(['agent-skills', 'ecl-planner'], tmpDir);
    assert.ok(r.success, `Command failed: ${r.error}`);
    assert.strictEqual(r.ir.agent_type, 'ecl-planner');
    assert.strictEqual(r.ir.block, '');
  });
  ...
  test('handles multiple skill paths', () => {
    ...
    writeConfig(tmpDir, {
      agent_skills: {
        'ecl-executor': ['skills/skill-a', 'skills/skill-b'],
      },
    });
  ...
  test('warns for nonexistent skill path but does not error', () => {
    writeConfig(tmpDir, {
      agent_skills: {
        'ecl-executor': ['skills/nonexistent'],
      },
    });
...
describe('config-set agent_skills', () => {
  ...
  test('can set agent_skills via dot notation', () => {
    const result = runGsdTools(
      ['config-set', 'agent_skills.ecl-executor', '["skills/my-skill"]'],
      tmpDir,
    ...
    assert.deepStrictEqual(
      config.agent_skills['ecl-executor'],
      ['skills/my-skill'],
...
describe('agent-skills global: prefix', () => {
  ...
  beforeEach(() => {
    ...
    fakeHome = fs.mkdtempSync(path.join(require('os').tmpdir(), 'ecl-1992-home-'));
  ...
  test('global:valid-skill resolves to $HOME/.claude/skills/valid-skill/SKILL.md', () => {
    ...
    writeConfig(tmpDir, {
      agent_skills: { 'ecl-executor': ['global:valid-skill'] },
    });

    const r = runAgentSkillsJson(
      ['agent-skills', 'ecl-executor'], tmpDir, { HOME: fakeHome, USERPROFILE: fakeHome }
    );
    assert.ok(r.success, `Command failed: ${r.error}`);
  ...
  test('global:invalid!name is rejected by regex and skipped', () => {
    writeConfig(tmpDir, {
      agent_skills: { 'ecl-executor': ['global:invalid!name'] },
    });
  ...
  test('global:missing-skill is skipped when directory is absent', () => {
    writeConfig(tmpDir, {
      agent_skills: { 'ecl-executor': ['global:missing-skill'] },
    });
  ...
  test('mix of global: and project-relative paths both resolve correctly', () => {
    ...
    writeConfig(tmpDir, {
      agent_skills: { 'ecl-executor': ['global:shadcn', 'skills/local-skill'] },
    });
  ...
  test('global: with empty name produces clear warning and skips', () => {
    writeConfig(tmpDir, {
      agent_skills: { 'ecl-executor': ['global:'] },
    });
...
describe('loadTrustedGlobalRoots', () => {
  ...
  test('drops non-string entries from the array', () => {
    ...
    const realDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-tgr-ns-'));
    try {
  ...
  test('keeps absolute paths — real dirs are kept and canonicalized', () => {
    ...
    const dir1 = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-tgr-d1-'));
    const dir2 = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-tgr-d2-'));
    try {
  ...
  test('expands leading ~/ to os.homedir() — kept only if the dir exists', () => {
    ...
    const subdir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-tgr-tilde-'));
    try {
  ...
  test('non-existent absolute root is dropped (returns [])', () => {
    const config = { agent_skills_security: { trusted_global_roots: ['/nonexistent-ecl-root-12345xyz'] } };
  ...
  test('trusted root that is a symlink is canonicalized to the link target', () => {
    const realTarget = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-tgr-symtgt-'));
    const symlinkPath = path.join(os.tmpdir(), `ecl-tgr-symlink-${Date.now()}`);
  ...
  test('de-duplicates entries by canonical path', () => {
    ...
    const realDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-tgr-dedup-'));
    try {
      const realPath = fs.realpathSync(realDir);
  ...
  test('expands ~/ before absolute check — non-existent ~/x is dropped after expansion', () => {
    ...
    const expandedX = path.join(os.homedir(), 'x-ecl-nonexistent-12345');
    // Ensure it really doesn't exist
    if (fs.existsSync(expandedX)) {
      // Cannot test non-existence reliably — skip assertion
      return;
    }
    const config = { agent_skills_security: { trusted_global_roots: ['~/x-ecl-nonexistent-12345'] } };
    const result = loadTrustedGlobalRoots(config);
...
// triggers a symlink escape scenario through ecl-tools subprocess invocation).
...
describe('trusted_global_roots guard logic', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-52-trusted-'));
    externalDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-52-external-'));
  ...
  test('with unrelated trusted root — skill still rejected', () => {
    const skillMd = path.join(externalDir, 'SKILL.md');
    const unrelatedDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-52-unrelated-'));
    try {
...
//
// These tests exercise the full CLI path (runAgentSkillsJson → ecl-tools →
...
describe('trusted_global_roots e2e CLI (#52)', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempProject();
    fakeHome = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-52-e2e-home-'));
    globalSkillsDir = path.join(fakeHome, '.claude', 'skills');
    fs.mkdirSync(globalSkillsDir, { recursive: true });
    sharedRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-52-e2e-shared-'));
  ...
  test('REGRESSION: symlinked-escape skill with NO agent_skills_security in config → block is empty', (t) => {
    ...
    writeConfig(tmpDir, {
      runtime: 'claude',
      agent_skills: { 'ecl-executor': ['global:shared-skill'] },
    });
  ...
  test('FEATURE NOTE: accepted-via-trusted-root emits NOTE on stderr', (t) => {
    ...
    const r = runGsdToolsWithStderr(
      ['agent-skills', '--json', 'ecl-executor'],
      tmpDir,
  ...
  test('NEGATIVE: symlink escape with unrelated trusted root (existing dir) → block is empty', (t) => {
    ...
    const unrelatedRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-52-e2e-unrelated-'));
...
describe('bug #1243: plugin-namespaced agent skills', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempProject();
    fakeHome = fs.mkdtempSync(path.join(require('os').tmpdir(), 'ecl-1243-home-'));
    globalSkillsDir = path.join(fakeHome, '.claude', 'skills');
    fs.mkdirSync(globalSkillsDir, { recursive: true });
  });
  ...
  test('happy: global:coderabbit:code-review (claude) emits directive naming coderabbit:code-review, no @-line, no path', () => {
    writeConfig(tmpDir, {
      runtime: 'claude',
      agent_skills: { 'ecl-executor': ['global:coderabbit:code-review'] },
    });
  ...
  test('mixed: path-resolvable global + namespaced → @-include AND directive in block', () => {
    ...
    writeConfig(tmpDir, {
      runtime: 'claude',
      agent_skills: {
        'ecl-executor': ['global:my-local-skill', 'global:vendor:remote-skill'],
      },
    });
  ...
  test('precedence: bare global:foo not-on-disk → not found/skipped, no directive', () => {
    ...
    writeConfig(tmpDir, {
      runtime: 'claude',
      agent_skills: { 'ecl-executor': ['global:foo'] },
    });
  ...
  test('negative: global:../evil rejected', () => {
    writeConfig(tmpDir, {
      runtime: 'claude',
      agent_skills: { 'ecl-executor': ['global:../evil'] },
    });
    const r = runAgentSkillsJson(
      ['agent-skills', 'ecl-executor'], tmpDir, { HOME: fakeHome, USERPROFILE: fakeHome }
    );
  ...
  test('negative: global:a::b rejected (empty segment)', () => {
    writeConfig(tmpDir, {
      runtime: 'claude',
      agent_skills: { 'ecl-executor': ['global:a::b'] },
    });
    const r = runAgentSkillsJson(
      ['agent-skills', 'ecl-executor'], tmpDir, { HOME: fakeHome, USERPROFILE: fakeHome }
    );
  ...
  test('negative: global::x rejected (leading colon)', () => {
    writeConfig(tmpDir, {
      runtime: 'claude',
      agent_skills: { 'ecl-executor': ['global::x'] },
    });
    const r = runAgentSkillsJson(
      ['agent-skills', 'ecl-executor'], tmpDir, { HOME: fakeHome, USERPROFILE: fakeHome }
    );
  ...
  test('cross-runtime: namespaced + codex runtime → no directive (skipped/warned)', () => {
    writeConfig(tmpDir, {
      runtime: 'codex',
      agent_skills: { 'ecl-executor': ['global:vendor:remote-skill'] },
    });
  ...
  test('HYRUM regression: include-only block is BYTE-IDENTICAL to expected format', () => {
    ...
    writeConfig(tmpDir, {
      runtime: 'claude',
      agent_skills: { 'ecl-executor': ['global:shadcn', 'skills/local'] },
    });
  ...
  test('grant: exact set of agents with Skill equals the 22 consumers (drift guard)', () => {
    ...
    // allow-test-rule: source-text-is-the-product (#1243)
    const AGENTS_DIR = path.join(__dirname, '..', 'agents');
    const agentFiles = fs.readdirSync(AGENTS_DIR).filter((f) => f.startsWith('ecl-') && f.endsWith('.md'));
