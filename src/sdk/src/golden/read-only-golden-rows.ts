/**
 * Read-only subprocess golden rows: SDK `registry.dispatch` vs `ecl-tools.cjs` JSON on stdout.
...
/** Repo-relative fixtures (cwd = evolv-coder-lite repo root). */
 ...
 * (Expand as more handlers are aligned with `ecl-tools.cjs`.)
 */
export const READ_ONLY_JSON_PARITY_ROWS: JsonParityRow[] = [
  { canonical: 'resolve-model', sdkArgs: ['ecl-planner'], cjs: 'resolve-model', cjsArgs: ['ecl-planner'] },
  ...
  { canonical: 'intel.query', sdkArgs: ['ecl'], cjs: 'intel', cjsArgs: ['query', 'ecl'] },
  {
