...
> **Explanation** — This document describes *why* eCL draws its trust
...
eCL 1.6.0 opens the capability platform to third-party authors with **full
artifact parity**: a third-party capability may ship the same executable
surfaces that eCL Core ships — hooks, MCP servers, command modules. This is a
...
eCL grants full artifact parity. It does not grant symmetric trust. First-party
capabilities are implicitly trusted because they *are* the shipped package —
their provenance is the eCL Core release process itself. Third-party
...
eCL's trust model is not designed in isolation. It is informed by failures in
...
eCL's response: auto-update is **off by default** for third-party capabilities.
...
VS Code also has no signature check on VSIX packages. eCL requires an
...
eCL takes a stronger position: **install never executes capability code**,
...
specific source repository. eCL emits provenance for first-party capabilities
...
eCL operates a central registry (see the PRD).
...
eCL borrows two things from Obsidian. First, the honesty: **there is no
...
community plugins are safe" message. eCL does not use restricted mode, but its
...
eCL does not borrow Obsidian's centralised review model. Requiring a
...
for maintainers. eCL ships decentralised URL import precisely to avoid that.
...
for eCL's integrity field.

eCL mirrors the allowlist mechanic as `strictKnownRegistries`, and mirrors the
...
The most powerful thing eCL can say to a user about a third-party capability
...
eCL presents a pre-install summary that names every executable surface the
...
of the capability bundle. When present, eCL verifies this digest before
...
The `ecl-`, `evolv-coder-lite-`, and `anthropic-` id prefixes are reserved for
...
actor cannot publish a capability called `ecl-security` and exploit a user's
implicit trust in the eCL namespace.
...
eCL does not sandbox third-party capability code. The honest reason: Node-level
...
installed, runs with the same permissions eCL Core itself runs with. It is not
...
2. **Integrity** — the bundle you consented to is the bundle that ran (SHA
   verified).
3. **Reversibility** — `ecl capability remove <id>` removes exactly what the
...
command modules. A third-party author could extend *what eCL describes* but not
*what it executes*.
...
in the parts of eCL where participation matters most. Full parity was the
explicit scope.
...
of eCL. The trust model is a forever commitment, not a one-time effort.

### Centralised-registry-only distribution

The alternative to decentralised URL/git import is requiring all third-party
capabilities to go through a eCL-operated curated registry — one PR per
...
registry. Whether eCL later operates or advertises a community registry is an
...
eCL can offer authors the full power of the platform while making users'
...
capability completely. eCL does not pretend this is the same as not running
the code at all.
