 ...
 * `workflows/update.md`. That logic resolved the installed eCL version, the
...
function versionFile(dir) { return path.join(dir, 'evolv-coder-lite', 'VERSION'); }
function markerFile(dir) { return path.join(dir, 'evolv-coder-lite', 'workflows', 'update.md'); }

// Detection: a dir "has eCL" if it carries a VERSION file or the update.md
// workflow marker.
...
function resolveUpdateContext({ home, cwd, env = {}, fs, preferredConfigDir = '', preferredRuntime = '' }) {
  ...
  // tilde-expands, so a custom --config-dir like `~/custom-ecl` must resolve
