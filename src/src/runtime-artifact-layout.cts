...
function kimiAgentsKind(destSubpath: string, prefix: string, configDir: string): ArtifactKind {
  return {
    ...
    stage: (resolved) => {
      ...
      const rootAgent = `---\nname: ecl\ndescription: Run eCL workflows in Kimi CLI.\ntools: Agent\n---\n\n# eCL for Kimi CLI\n\nCoordinate installed /skill:ecl-* workflows and route work to generated eCL subagents when a workflow requires an agent handoff.\n`;
      const artifacts = buildKimiAgentArtifacts({ rootAgent, subagents });
      const stageDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-kimi-agents-'));
      installProfiles.STAGED_DIRS.add(stageDir);
      fs.writeFileSync(path.join(stageDir, 'ecl.yaml'), artifacts.root.yaml);
      fs.writeFileSync(path.join(stageDir, 'ecl.md'), artifacts.root.prompt);
...
//                   at the top level so Skill(skill="ecl-plan-phase") succeeds)
//
