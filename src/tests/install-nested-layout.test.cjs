...
const { COMMANDS_ECL, ROUTER_STEMS, routerChildren } = require('./helpers/nested-layout.cjs');
...
const NEST = [
  ...
  { runtime: 'cline',       scope: 'global', skillsSub: 'skills',     prefix: 'ecl-' },
  { runtime: 'qwen',        scope: 'global', skillsSub: 'skills',     prefix: 'ecl-' },
  { runtime: 'hermes',      scope: 'global', skillsSub: 'skills/ecl', prefix: 'ecl-' }, // #947: restored canonical prefix
  { runtime: 'augment',     scope: 'global', skillsSub: 'skills',     prefix: 'ecl-' },
  { runtime: 'trae',        scope: 'global', skillsSub: 'skills',     prefix: 'ecl-' },
  { runtime: 'antigravity', scope: 'global', skillsSub: 'skills',     prefix: 'ecl-' },
];
...
function runInstall(runtime, scope, resolved) {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), `ecl-nest-test-${runtime}-`));
...
for (const { runtime, scope, skillsSub, prefix } of NEST) {
  describe(`${runtime} (nested layout)`, () => {
    ...
    test(`${runtime}: exactly 6 top-level router bundles, no concrete skill at top level`, () => {
      ...
      // Total eCL-owned top-level entries must be EXACTLY 6 (only the routers).
      // All nested runtimes (incl. Hermes after #947) use prefix='ecl-'.
      ...
      assert.strictEqual(
        gsdTopLevelCount,
        6,
        `Expected exactly 6 total eCL-owned top-level skill dirs for ${runtime} (only routers), got ${gsdTopLevelCount}: [${topLevel.join(', ')}]`,
      );
    });
...
// ---------------------------------------------------------------------------
// claude extra: total top-level ecl- count must be >= 60 (FLAT, #924)
//
// Pre-#924 (nested) this block asserted exactly 6 (only routers).
// Post-#924 (flat) Claude has all concrete skills at the top level.
// ---------------------------------------------------------------------------

describe('claude: total top-level ecl- entries >= 60 (flat layout, #924)', () => {
  let tmpDir;
  ...
  test('claude: >= 60 ecl-* top-level skill entries (concrete flat layout, not nested)', () => {
    ...
    assert.ok(
      topLevel.length >= 60,
      `Expected >= 60 ecl-* top-level entries under claude/skills (flat layout after #924 fix). ` +
...
for (const { runtime, scope, skillsSub } of FLAT) {
  describe(`${runtime} (flat layout)`, () => {
    ...
    test(`${runtime}: stays flat — concrete skills remain top-level, no nesting`, () => {
      ...
      const topLevel = fs.readdirSync(skillsDir);
      const gsdEntries = topLevel.filter((n) => n.startsWith('ecl-'));

      // For flat runtimes, there should be many more than 6 top-level ecl- entries
      assert.ok(
        gsdEntries.length >= 60,
        `Flat runtime ${runtime} must have >= 60 ecl-* top-level entries (concrete skills), got ${gsdEntries.length}`,
      );

      // No router dir should contain a skills/ subdirectory (nesting must not have been applied)
      const routerDirsPresent = topLevel.filter((n) => n.startsWith('ecl-ns-'));
