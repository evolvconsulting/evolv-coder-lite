...
const {
  ...
  putResearch,
  getResearch,
} = require('../evolv-coder-lite/bin/lib/research-store.cjs');
...
describe('research-store: getResearch missing key', () => {
  ...
  beforeEach(() => {
    tmpCwd = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-rs-cwd-'));
    tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-rs-home-'));
  });
...
describe('research-store: resolveStorePath tiers', () => {
  ...
  test("source 'curated' → under injected homeDir/.ecl/research-cache", () => {
    const p = resolveStorePath(FAKE_CWD, 'curated', { homeDir: FAKE_HOME });
    assert.equal(p, path.join(FAKE_HOME, '.ecl', 'research-cache'));
  });
...
describe('research-store: I1 regression — source is the tier axis', () => {
  ...
  beforeEach(() => {
    tmpCwd = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-rs-i1-cwd-'));
    tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-rs-i1-home-'));
  });
  ...
  test('source:web + kind:docs → file in PROJECT dir, NOT user dir', () => {
    ...
    const userFile = path.join(tmpHome, '.ecl', 'research-cache', `${key}.json`);

    assert.ok(fs.existsSync(projectFile), 'file should exist in project dir (.planning/research/.cache)');
    assert.ok(!fs.existsSync(userFile), 'file should NOT exist in user dir (~/.ecl/research-cache)');
  });
  ...
  test('source:curated + kind:web → file in USER dir, NOT project dir', () => {
    ...
    assert.ok(fs.existsSync(userFile), 'file should exist in user dir (~/.ecl/research-cache)');
...
describe('research-store: W4a regression — prefer fresh over stale across tiers', () => {
  ...
  beforeEach(() => {
    tmpCwd = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-rs-w4-cwd-'));
    tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-rs-w4-home-'));
  });
  ...
  test('fresh project entry wins over stale curated entry for same key', () => {
    ...
    const userDir = path.join(tmpHome, '.ecl', 'research-cache');
...
describe('research-store: W4b regression — blank version caps TTL', () => {
  ...
  beforeEach(() => {
    tmpCwd = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-rs-w4b-cwd-'));
    tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-rs-w4b-home-'));
  });
...
describe('research-store: isValidResearchKey exported', () => {
  const { isValidResearchKey } = require('../evolv-coder-lite/bin/lib/research-store.cjs');
...
describe('research-store: putResearch rejects traversal key', () => {
  ...
  beforeEach(() => {
    tmpCwd = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-rs-trav-cwd-'));
    tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-rs-trav-home-'));
  });
...
describe('research-store: getResearch rejects traversal key', () => {
  ...
  beforeEach(() => {
    tmpCwd = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-rs-trav2-cwd-'));
    tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-rs-trav2-home-'));
  });
...
describe('research-store: getResearch rejects malformed cache metadata', () => {
  ...
  beforeEach(() => {
    tmpCwd = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-rs-malformed-cwd-'));
    tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-rs-malformed-home-'));
  });
