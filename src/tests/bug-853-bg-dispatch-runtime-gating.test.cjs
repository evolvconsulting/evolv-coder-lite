'use strict';
/**
 * Regression guard — bug(#853): /ecl-manager and /ecl-autonomous --interactive
...
describe('bug-853 — manager/autonomous gate background dispatch by runtime', () => {
  ...
  test('manager.md runs plan/execute inline on Claude Code', () => {
    assert.match(MANAGER, /If `RUNTIME` is `claude`[\s\S]{0,400}?Skill\(skill="ecl-plan-phase"/);
    assert.match(MANAGER, /If `RUNTIME` is `claude`[\s\S]{0,400}?Skill\(skill="ecl-execute-phase"/);
  });
  ...
  test('autonomous.md runs plan/execute inline on Claude Code in interactive mode', () => {
    assert.match(AUTONOMOUS, /On Claude Code \(`RUNTIME` is `claude`\)[\s\S]{0,400}?Skill\(skill="ecl-plan-phase"/);
    assert.match(AUTONOMOUS, /On Claude Code \(`RUNTIME` is `claude`\)[\s\S]{0,400}?Skill\(skill="ecl-execute-phase"/);
  });
});
