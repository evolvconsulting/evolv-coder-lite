...
const EXECUTE_PHASE = path.join(ROOT, 'evolv-coder-lite', 'workflows', 'execute-phase.md');
  ...
  return [...content.matchAll(/<step name="([^"]+)"[^>]*>([\s\S]*?)<\/step>/g)]
    .map((match) => {
      ...
      return {
        ...
        readsRuntimeConfig: body.includes('RUNTIME=$($ECL_SDK query config-get runtime --default claude') || body.includes('RUNTIME=$(ecl-sdk query config-get runtime --default claude'),
...
describe('#3360 — Codex execute-phase fails closed for unsupported worktree isolation', () => {
  ...
  test('Codex adapter documents that worktree isolation has no direct spawn_agent mapping', () => {
    const header = getCodexSkillAdapterHeader('ecl-execute-phase');
