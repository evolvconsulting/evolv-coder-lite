...
describe('bug #1829: model_profile "inherit" — resolveModelInternal', () => {
  ...
  test('returns "inherit" for ecl-planner when model_profile is "inherit"', () => {
    writeConfig(tmpDir, { model_profile: 'inherit' });
    assert.strictEqual(resolveModelInternal(tmpDir, 'ecl-planner'), 'inherit');
  });

  test('returns "inherit" for ecl-executor when model_profile is "inherit"', () => {
    writeConfig(tmpDir, { model_profile: 'inherit' });
    assert.strictEqual(resolveModelInternal(tmpDir, 'ecl-executor'), 'inherit');
  });

  test('returns "inherit" for ecl-phase-researcher when model_profile is "inherit"', () => {
    writeConfig(tmpDir, { model_profile: 'inherit' });
    assert.strictEqual(resolveModelInternal(tmpDir, 'ecl-phase-researcher'), 'inherit');
  });

  test('returns "inherit" for ecl-codebase-mapper when model_profile is "inherit"', () => {
    writeConfig(tmpDir, { model_profile: 'inherit' });
    assert.strictEqual(resolveModelInternal(tmpDir, 'ecl-codebase-mapper'), 'inherit');
  });

  test('returns "inherit" for ecl-verifier when model_profile is "inherit"', () => {
    writeConfig(tmpDir, { model_profile: 'inherit' });
    assert.strictEqual(resolveModelInternal(tmpDir, 'ecl-verifier'), 'inherit');
  });
...
describe('bug #1829: model_profile "inherit" — resolve-model CLI', () => {
  ...
  test('CLI resolve-model returns "inherit" for ecl-executor with inherit profile', () => {
    fs.writeFileSync(
      path.join(tmpDir, '.planning', 'config.json'),
      JSON.stringify({ model_profile: 'inherit' }, null, 2)
    );

    const result = runGsdTools('resolve-model ecl-executor', tmpDir);
  ...
  test('CLI resolve-model returns "inherit" for ecl-planner with inherit profile', () => {
    fs.writeFileSync(
      path.join(tmpDir, '.planning', 'config.json'),
      JSON.stringify({ model_profile: 'inherit' }, null, 2)
    );

    const result = runGsdTools('resolve-model ecl-planner', tmpDir);
    assert.ok(result.success, `resolve-model failed: ${result.error}`);
