...
describe('bug-2808: SKILL.md name: uses hyphen form', () => {
  ...
  test('generated autocomplete skill surface uses hyphen names without underscores', (t) => {
    ...
    // With the full profile (#924 fix), skills are FLAT: ecl-<stem>/SKILL.md
    ...
    // the kind of drift this test exists to catch (a `ecl:extract-learnings`
    ...
    for (const skillMdPath of allSkillMdPaths) {
      ...
      assert.ok(name.startsWith('ecl-'), `${relPath}: autocomplete name must start with ecl-, got ${name}`);
    ...
    assert.ok(allNames.includes('ecl-extract-learnings'), 'autocomplete surface must include ecl-extract-learnings');
    assert.ok(!allNames.includes('ecl-extract_learnings'), 'autocomplete surface must not include ecl-extract_learnings');
  });
