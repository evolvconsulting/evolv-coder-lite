...
const SPEC_PHASE_PATH = path.join(__dirname, '..', 'evolv-coder-lite', 'workflows', 'spec-phase.md');
...
// scripts. Every executable `build:lib` invocation must be pinned to the eCL dir with
// `npm --prefix`, and the build must be gated behind a verified eCL source checkout. A bare
...
test('review High: Step 5.5 build:lib is --prefix-pinned to the eCL dir and gated on a source checkout', () => {
  const content = readSpecPhase();
  const block = extractStep55Block(content);
  assert.ok(block.length > 0, 'Step 5.5 block must be extractable from spec-phase.md');
  ...
  for (const line of buildInvocations) {
    assert.match(
      line,
      /npm\s+--prefix\s+"?\$?\{?_?ECL_RT/,
      `build:lib must be pinned with \`npm --prefix "$_ECL_RT"\` so it never runs the consuming project's scripts — offending line: ${line.trim()}`
    );
  }

  // The build must be gated behind a verified eCL source checkout (tsconfig.build.json present),
  // so it cannot fire inside a plain consumer repo where the artifact merely happens to be absent.
  assert.match(
    block,
    /tsconfig\.build\.json/,
    'Step 5.5 must gate the build behind a eCL source checkout (e.g. test -f "$_ECL_RT/tsconfig.build.json")'
  );
});
