 ...
 *
 * When ecl-tools is invoked with --json-errors, all error() calls emit a
 ...
 *   6. ECL_JSON_ERRORS=1 env var   → same structured output without --flag
...
// Helper: run ecl-tools with --json-errors and parse the structured stderr.
...
describe('feat #3255: --json-errors mode emits structured error objects', () => {
  ...
  // ── 6. ECL_JSON_ERRORS=1 env var activates structured mode ───────────────

  test('ECL_JSON_ERRORS=1 env var produces same structured error as --json-errors flag', () => {
