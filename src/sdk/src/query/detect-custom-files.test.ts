...
async function writeManifest(configDir: string, files: Record<string, string>): Promise<void> {
  ...
  await writeFile(
    join(configDir, 'ecl-file-manifest.json'),
...
describe('detectCustomFiles — skills/ parity with CJS port (#3317)', () => {
  ...
  beforeEach(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), 'ecl-3317-skills-'));
  });
  ...
  it('does not flag eCL-owned skill listed in manifest', async () => {
    await writeManifest(tmpDir, {
      'skills/ecl-planner/SKILL.md': '# eCL Planner Skill\n',
    });
    ...
    expect(result.custom_files).not.toContain('skills/ecl-planner/SKILL.md');
  });

  it('still detects custom files in evolv-coder-lite/workflows/ (regression guard)', async () => {
    await writeManifest(tmpDir, {
      'evolv-coder-lite/workflows/plan-phase.md': '# Plan Phase\n',
      'skills/ecl-planner/SKILL.md': '# eCL Planner Skill\n',
    });
    await writeCustomFile(tmpDir, 'evolv-coder-lite/workflows/custom-workflow.md', '# Custom\n');

    const { data } = await detectCustomFiles(['--config-dir', tmpDir], tmpDir);
    const result = data as DetectResult;

    expect(result.custom_files).toContain('evolv-coder-lite/workflows/custom-workflow.md');
  });
