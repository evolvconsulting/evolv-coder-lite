# sync-skills — Cross-Runtime eCL Skill Sync

**Command:** `/ecl-sync-skills`

Sync managed `ecl-*` skill directories from one canonical runtime's skills root to one or more destination runtime skills roots. Keeps multi-runtime installs aligned after a `ecl-update` on one runtime.
...
```bash
INSTALL_JS="$(dirname "$0")/../evolv-coder-lite/bin/install.js"
# If running from a global install, resolve relative to the eCL package
INSTALL_JS_GLOBAL="$HOME/.claude/evolv-coder-lite/bin/install.js"
...
error: source skills root not found: <path>
       Is eCL installed globally for the '<runtime>' runtime?
       Run: node ~/.claude/evolv-coder-lite/bin/install.js --global --<runtime>
```
Then exit.
...
```bash
# List ecl-* subdirectories in source
SRC_SKILLS=$(ls -1 "$SRC_SKILLS_ROOT" 2>/dev/null | grep '^ecl-')

# List ecl-* subdirectories in destination (may not exist yet)
DST_SKILLS=$(ls -1 "$DEST_ROOT" 2>/dev/null | grep '^ecl-')

# Diff:
# CREATE  — in SRC but not in DST
# UPDATE  — in both; content differs (compare recursively via checksums)
# REMOVE  — in DST but not in SRC (stale eCL skill no longer in source)
# SKIP    — in both; content identical (already up to date)
```

**Non-eCL preservation:** Only `ecl-*` entries are ever created, updated, or removed. Entries in the destination that do not start with `ecl-` are never touched.
...
CREATE: ecl-help
UPDATE: ecl-update
REMOVE: ecl-old-command
SKIP:   ecl-plan-phase (up to date)
(N changes)
...
1. **Only `ecl-*` directories** are created, updated, or removed. Any directory not starting with `ecl-` in a destination root is untouched.
2. **Dry-run is the default.** `--apply` must be passed explicitly to write anything.
3. **Source root must exist.** Never create the source root; it must have been created by a prior `ecl-update` or installer run.
...
- Sync copies files verbatim and does not apply runtime-specific content transformations. Use the eCL installer directly for runtimes that require format conversion.
