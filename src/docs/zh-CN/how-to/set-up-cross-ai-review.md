...
eCL Core 可将评审请求路由至以下任意组合：Gemini CLI、Claude（独立会话）、Codex CLI、CodeRabbit、OpenCode、Qwen Code、Cursor、Antigravity CLI、Ollama、LM Studio 以及 llama.cpp。
...
默认情况下，`/ecl-review` 会运行所有检测到的 CLI。若要将特定子集固定为项目默认值：
...
eCL 会依次调用每位评审者，收集结构化反馈（摘要、优点、HIGH/MEDIUM/LOW 级别问题、建议、风险评估），并将合并后的输出写入 `.planning/phases/03-.../03-REVIEWS.md`。
...
如果默认值（`localhost:11434` / `localhost:1234`）不适用，请通过 `/ecl-config --integrations` 在 `review.*` 键下配置主机地址和模型选择。
...
**停滞检测：** 如果 HIGH 级别问题数量在各轮次间未减少，eCL 会向您发出警告。当循环上限已达但仍存在未解决的 HIGH 级别问题时，升级门控会询问是否继续或手动审查。
...
| 发布前需要最大覆盖率 | `/ecl-plan-review-convergence N --all` |
| 快速迭代并希望获得快速反馈 | 选择一个 CLI：`/ecl-review --phase N --gemini` |
