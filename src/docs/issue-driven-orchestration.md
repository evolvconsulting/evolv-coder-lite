# Issue-Driven Orchestration with eCL
...
through eCL's existing primitives.

## What this guide is

A recipe for combining commands eCL already ships into an issue-tracker
...
referenced below already exists in eCL today.
...
([repository](https://github.com/openai/symphony)). eCL does not vendor or
wrap Symphony. The orchestration *concepts* map cleanly onto primitives
eCL already exposes; this guide just spells the mapping out so you can
adopt the pattern without writing glue code or bypassing eCL's safety
gates.

## Why this exists

eCL has the building blocks for issue-driven AI development —
`/ecl-workspace --new`, `/ecl-manager`, `/ecl-autonomous`, `/ecl-verify-work`,
`/ecl-review`, `/ecl-ship`, plus `STATE.md` and the phase artifact suite
...
- Underuse: developers run discuss/plan/execute manually and never reach
  for `/ecl-manager` or `/ecl-autonomous` even when their work pattern
  fits.
...
Each row maps a Symphony-style orchestration concept to the eCL primitive
...
| Symphony concept | eCL primitive |
|---|---|
...
| One isolated agent workspace per task | `/ecl-workspace --new --strategy worktree` |
| Agent dispatch and concurrency | `/ecl-manager` (interactive dashboard), `/ecl-autonomous` (unattended) |
| Per-phase plan and discuss steps | `/ecl-discuss-phase` → `/ecl-plan-phase` → `/ecl-execute-phase` |
| Proof-of-work / test evidence | `/ecl-verify-work` (UAT.md persisted across `/clear`) |
| Adversarial review | `/ecl-review` (cross-AI peer review of plans) |
| Human merge gate | `/ecl-ship` (creates PR, optional code review, prepares merge) |
| Follow-up capture | `/ecl-capture`, `/ecl-capture --seed`, `/ecl-new-milestone`, or a manually opened tracker issue |
| Concurrency control | Manager / background-agent semantics (no always-on poller) |

The mapping is one-way: eCL owns the safety gates (verification, human
...
2. **Map to a eCL phase.** If the issue maps onto an existing phase in
   `ROADMAP.md`, select it. If not, run `/ecl-new-milestone` (for a new
   milestone of related issues) or open a phase via `/ecl-phase` /
   `/ecl-phase --insert`. Capture the tracker issue URL in the phase's
   `CONTEXT.md` so traceability survives compaction.
3. **Create an isolated workspace.** Run
   `/ecl-workspace --new --strategy worktree <slug>` to spin up a git
...
4. **Run discuss → plan → execute through eCL.** From inside the
   workspace, run `/ecl-discuss-phase` to clarify ambiguities,
   `/ecl-plan-phase` to produce `PLAN.md`, and either `/ecl-manager`
   (interactive dashboard) or `/ecl-execute-phase` / `/ecl-autonomous`
   (unattended) to implement. Avoid driving raw `claude` invocations
   from outside eCL — that bypasses `STATE.md` updates and the phase
   manifest.
5. **Demand proof-of-work.** Run `/ecl-verify-work` to walk the user
   ...
   `/ecl-plan-phase --gaps` when verification surfaces missed scope.
6. **Pass through the review and ship gates.** Run `/ecl-review` to get
   adversarial peer review of the plan from independent AI CLIs (catches
   blind spots model-by-model), then `/ecl-ship` to open the PR with a
   rich body assembled from the planning artifacts. Both gates require a
   human decision before anything reaches the remote.
7. **Capture follow-up work explicitly.** Use `/ecl-capture` for inline
   notes, `/ecl-capture --seed` for ideas worth a future phase, or
   `/ecl-new-milestone` for a coherent group of follow-ups. Creating a
   tracker issue from a discovered follow-up requires explicit user
   confirmation — eCL does not post to remote trackers automatically.
...
- **Isolated worktrees.** Every issue runs in a `/ecl-workspace --new`
  worktree, so partial work, aborted plans, and exploratory commits
  never touch `main`. `ecl-local-patches/` is the recovery surface if a
  worktree's hand-edits need to come back across an update.
- **Explicit human review.** `/ecl-review` and `/ecl-ship` both stop for
  ...
  something eCL opts into for you.
- **No automatic public posting.** eCL never opens, comments on, or
...
- **Verification before ship.** `/ecl-verify-work`'s UAT.md must record
  evidence before `/ecl-ship` is run. The recommended discipline is to
...
against the worktree, skipping `/ecl-verify-work`, or scripting issue
...
- **No vendoring or copying Symphony code.** eCL reuses its own
  primitives. The mapping above is conceptual; no Symphony-derived
  source ships in this repo.
- **No long-running daemon.** eCL does not poll GitHub or Linear. The
...
- **No mandatory tracker dependency.** The loop works without any
  ...
  the URL goes into `CONTEXT.md`. eCL has no opinion about which
  tracker you use, or whether you use one at all.
- **No bypass of verification, review, or human decision gates.** Even
  when running `/ecl-autonomous`, the verification and review gates
...
- Importing one GitHub or Linear issue into a eCL workspace / phase.
- Exporting `UAT.md` evidence as a comment on the source issue.
- Generating follow-up tracker issues from `/ecl-capture --seed` output.
...
- [docs index](README.md) — full table of contents for eCL Core documentation.
- [docs/USER-GUIDE.md](./USER-GUIDE.md) — task-oriented walkthroughs of individual commands referenced above.
- [docs/COMMANDS.md](COMMANDS.md) — full reference for `/ecl-*` commands.
