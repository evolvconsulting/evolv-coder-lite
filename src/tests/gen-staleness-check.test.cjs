 ...
 * path and passes ECL_REPO_ROOT to the generator subprocess so requireFreshDist()
...
/**
 * Run a gen script via spawnSync with an optional ECL_REPO_ROOT override.
...
function makeTempTree(dist, ts) {
  const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-staleness-'));
...
for (const { script, dist, ts } of GENERATORS) {
  describe(`gen staleness guard — ${script}`, { concurrency: false }, () => {
    ...
    test('exits 1 with "does not exist" when dist file is absent', () => {
      ...
      try {
        const { status, stderr } = runGen(script, { ECL_REPO_ROOT: tmpRoot });
