...
**前提条件:** `.planning/ROADMAP.md` が存在すること。ない場合は先に `/ecl-new-project` を実行してください。

---

## 検討モードを選ぶ

eCL Core は 2 つのモードを提供します。コードベースへの理解度に応じて選択してください。
...
次に `/ecl-plan-phase` を実行すると、プランナーは CONTEXT.md を読み込み、どの決定事項が確定済みかを把握します。ここで既に回答された質問は再確認されません。リサーチャーも最初に CONTEXT.md を読んで調査すべき内容を把握します。

**`/ecl-plan-phase` 実行時に CONTEXT.md がない場合**、コンテキストなしで続行する（計画はあなたの設計方針なしにリサーチと要件のみを使用）か、先に `/ecl-discuss-phase` を実行するかの選択を求められます。
