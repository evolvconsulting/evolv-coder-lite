...
describe('bug-2801: ingest-docs.md workflow calls ecl-tools not ecl-sdk', () => {
  ...
  test('ingest-docs.md init step uses canonical node-path ecl-tools.cjs invocation', () => {
    const content = fs.readFileSync(WORKFLOW_FILE, 'utf-8');
    ...
    // legacy bare `ecl-tools` is the bug being fixed and must not be accepted.
    const initLine = bashLines.find((l) =>
      /\bnode\s+["']?\$HOME\/\.claude\/evolv-coder-lite\/bin\/ecl-tools\.cjs["']?\s+init\s+ingest-docs\b/.test(l)
    );
    assert.ok(initLine, 'workflow must invoke init ingest-docs via canonical node-path ecl-tools.cjs');
  });
