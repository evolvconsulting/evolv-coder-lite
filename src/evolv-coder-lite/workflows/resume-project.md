...
<required_reading>
@~/.claude/evolv-coder-lite/references/continuation-format.md
</required_reading>
...
INIT=$($ECL_SDK query init.resume)
...
**If `planning_exists` is false:** This is a new project - route to /ecl:new-project
</step>
...
- This is the primary resumption source — structured data from `/ecl:pause-work`
...
📋 [N] pending todos — /ecl:capture --list to review
...
1. Resume interrupted agent [if interrupted agent found]
   OR
1. Execute phase (/ecl:execute-phase {phase} ${ECL_WS})
   OR
1. Discuss Phase 3 context (/ecl:discuss-phase 3 ${ECL_WS}) [if CONTEXT.md missing]
   OR
1. Plan Phase 3 (/ecl:plan-phase 3 ${ECL_WS}) [if CONTEXT.md exists or discuss option declined]
...
- **Plan phase** → Show direct next command:
  ...
  **Also available:**
  - `/ecl:discuss-phase [N] ${ECL_WS}` — gather context first
  - `/ecl:plan-phase --research-phase [N] ${ECL_WS}` — investigate unknowns
