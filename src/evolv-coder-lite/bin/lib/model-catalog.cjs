...
//
//   1. Co-located install path — evolv-coder-lite/bin/shared/model-catalog.json
...
//      evolvconsulting/evolv-coder-lite clone (the original path introduced by #3230).
//
//   3. ECL_MODEL_CATALOG env override — allows test harnesses and custom
...
const _catalogCandidates = [
  ...
  process.env.ECL_MODEL_CATALOG ? path.resolve(process.env.ECL_MODEL_CATALOG) : null,
].filter(Boolean);
