...
- **Claude Code / Copilot / OpenCode / Kilo:** `/ecl-command-name [args]` (hyphen form)
- **Gemini CLI:** `/ecl:command-name [args]` (colon form — Gemini namespaces commands under `ecl:`)
...
Heavy workflow skills (`/ecl-plan-phase`, `/ecl-execute-phase`, `/ecl-autonomous`) declare `effort: max`, signalling maximum token budget to the runtime. These skills are spawning orchestrators — they must run at top level so they retain the `Agent` tool needed to spawn subagents. They do **not** carry `context: fork` (see #921).
...
Six namespace routers ship as the first-stage entry points in v1.40. They keep the eager skill-listing token cost low (~120 tokens for 6 routers vs ~2,150 for a flat 86-skill listing) while the full surface remains directly invocable. The model selects a namespace, then routes to the concrete sub-skill. See [#2792](https://github.com/evolvconsulting/evolv-coder-lite/issues/2792).

| Command | Routes to |
|---------|-----------|
| `/ecl-workflow` | Phase pipeline — discuss / plan / execute / verify / phase / progress |
...
| `/ecl-manage` | Management — config, workspace, workstreams, thread, update, ship, inbox |
| `/ecl-ideate` | Exploration & capture — explore, sketch, spike, spec, capture |

The namespace skills are **additive** — every existing concrete command (e.g. `/ecl-plan-phase`, `/ecl-code-review --fix`) is still invocable directly.
...
```bash
/ecl-new-project                    # Interactive mode
/ecl-new-project --auto @prd.md     # Auto-extract from PRD
```
...
Manage eCL workspaces — create, list, or remove isolated workspace environments with repo copies and independent `.planning/` directories.

| Flag | Description |
|------|-------------|
| `--new` | Create a new workspace (use with `--name`, `--repos`, etc.) |
| `--list` | List active eCL workspaces and their status |
...
| `--path /target` | Target directory (default: `~/ecl-workspaces/<name>`) |
...
**Use cases:**
- Multi-repo: work on a subset of repos with isolated eCL state
...
/ecl-workspace --new --name feature-b --repos . --strategy worktree  # Same-repo isolation
/ecl-workspace --list
...
```bash
/ecl-discuss-phase 1                # Interactive discussion for phase 1
/ecl-discuss-phase 1 --all          # Discuss all gray areas without selection step
/ecl-discuss-phase 3 --auto         # Auto-select defaults for phase 3
/ecl-discuss-phase --batch          # Batch mode for current phase
/ecl-discuss-phase 2 --analyze      # Discussion with trade-off analysis
/ecl-discuss-phase 1 --power        # Bulk answers from file
/ecl-discuss-phase 3 --assumptions  # Surface Claude's assumptions before planning
```
...
```bash
/ecl-ui-phase 2                     # Design contract for phase 2
```
...
```bash
/ecl-plan-phase 1                              # Research + plan + verify phase 1
/ecl-plan-phase 3 --skip-research              # Plan without research (familiar domain)
/ecl-plan-phase --auto                         # Non-interactive planning
/ecl-plan-phase 2 --validate                   # Validate state before planning
...
```bash
/ecl-plan-review-convergence 3                    # Default reviewers, 3 cycles
/ecl-plan-review-convergence 3 --codex            # Codex-only review
...
**[BETA]** Offload plan phase to Claude Code's ultraplan cloud; review in browser and import back. The plan drafts remotely so the terminal stays free; review inline comments in a browser, then import the finalized plan back into `.planning/` via `/ecl-import`.

| Flag | Required | Description |
|------|----------|-------------|
| `N` | **Yes** | Phase number to plan remotely |

**Isolation:** Intentionally separate from `/ecl-plan-phase` so upstream ultraplan changes cannot affect the core planning pipeline.

```bash
/ecl-ultraplan-phase 4                  # Offload planning for phase 4
```
...
```bash
/ecl-execute-phase 1                # Execute phase 1
/ecl-execute-phase 1 --wave 2       # Execute only Wave 2
/ecl-execute-phase 1 --validate     # Validate state before execution
/ecl-execute-phase 2 --cross-ai     # Delegate phase 2 to external AI CLI
```
...
**Prerequisites:** Phase verified (`/ecl-verify-work` passed), `gh` CLI installed and authenticated
**Produces:** GitHub PR with rich body from planning artifacts, STATE.md updated

```bash
/ecl-ship 4                         # Ship phase 4
/ecl-ship 4 --draft                 # Ship as draft PR
```
...
```bash
/ecl-milestone-summary                # Summarize current milestone
/ecl-milestone-summary v1.0           # Summarize specific milestone
```
...
```bash
/ecl-new-milestone                  # Interactive
/ecl-new-milestone "v2.0 Mobile"    # Named milestone
/ecl-new-milestone --reset-phase-numbers "v2.0 Mobile"  # Restart milestone numbering at 1
```
...
```bash
/ecl-phase "Add authentication system"          # Append new phase with description
/ecl-phase --insert 3 "Fix auth race condition" # Insert between phase 3 and 4 → creates 3.1
...
```bash
/ecl-validate-phase 2               # Audit test coverage for phase 2
```
...
**Programmatic access:** `node ecl-tools.cjs phase uat-passed <N> [--require-verification] [--raw]` — see [CLI Tools Reference](CLI-TOOLS.md)

```bash
node ecl-tools.cjs phase uat-passed 3                        # Evaluate UAT for phase 3
node ecl-tools.cjs phase uat-passed 3 --require-verification # Also require VERIFICATION.md
node ecl-tools.cjs phase uat-passed 3 --raw                  # Machine-readable JSON output
```
...
| `--next --converge` | When the next action is planning, route it through `/ecl-plan-review-convergence`; requires `workflow.plan_review_convergence=true` |
...
| `--do "task description"` | Analyze freeform intent and dispatch to the most appropriate eCL command |
...
- No project → suggests `/ecl-new-project`
- Phase needs discussion → runs `/ecl-discuss-phase`
- Phase needs planning → runs `/ecl-plan-phase` (or `/ecl-plan-review-convergence` when `--converge` is set)
- Phase needs execution → runs `/ecl-execute-phase`
- Phase needs verification → runs `/ecl-verify-work`
- All phases complete → suggests `/ecl-complete-milestone`

```bash
/ecl-progress                       # "Where am I? What's next?" with auto-routing
/ecl-progress --next                # Advance to next step automatically
/ecl-progress --next --auto         # Chain steps automatically until completion
/ecl-progress --next --auto --converge  # Hands-free run with plan-review convergence
/ecl-progress --do "fix the auth bug"  # Dispatch freeform intent to best eCL command
/ecl-progress --forensic            # Standard report + integrity audit
```
...
```bash
/ecl-resume-work                    # After context reset or new session
```
...
```bash
/ecl-pause-work                     # Creates continue-here.md
/ecl-pause-work --report            # Creates continue-here.md + session report
```
...
```bash
/ecl-manager                        # Open command center dashboard
/ecl-manager --analyze-deps         # Scan ROADMAP phases for dependency relationships before parallel execution
```
...
Show eCL commands at the tier you ask for. Default fits one screen; `--full` is the complete reference; `<topic>` jumps directly to one section.

```bash
/ecl-help                           # One-page tour (default)
/ecl-help --brief                   # ~10-line one-liner refresher of top commands
/ecl-help --full                    # Complete reference (every command, every flag)
/ecl-help <topic>                   # One section only (e.g. /ecl-help debug)
/ecl-help --brief <topic>           # Compact scoped lookup — signature + one-line summary
```

See `evolv-coder-lite/workflows/help/modes/topic.md` for the full alias table. Unknown topics print the recognized list.
...
Socratic ideation session — guide an idea through probing questions, optionally spawn research, then route output to the right eCL artifact (notes, todos, seeds, research questions, requirements, or a new phase).

| Argument | Required | Description |
|----------|----------|-------------|
| `topic` | No | Topic to explore (e.g., `/ecl-explore authentication strategy`) |

```bash
/ecl-explore                        # Open-ended ideation session
/ecl-explore authentication strategy  # Explore a specific topic
```
...
Safe git revert — roll back eCL phase or plan commits using the phase manifest with dependency checks and a confirmation gate.

| Flag | Required | Description |
|------|----------|-------------|
| `--last N` | (one of three required) | Show recent eCL commits for interactive selection |
...
```bash
/ecl-undo --last 5                  # Pick from the 5 most recent eCL commits
/ecl-undo --phase 03                # Revert all commits for phase 3
/ecl-undo --plan 03-02              # Revert commits for plan 02 of phase 3
```
...
Ingest an external plan file into the eCL planning system with conflict detection against `PROJECT.md` decisions before writing anything.

| Flag | Required | Description |
|------|----------|--------------|
| `--from <filepath>` | Yes (or `--from-ecl2`) | Path to the external plan file to import |
| `--from-ecl2` | Yes (or `--from`) | Reverse-migrate a eCL-2 (`.ecl/`) project back to eCL v1 (`.planning/`) format |
| `--path <dir>` | No | With `--from-ecl2`: path to the eCL-2 project directory (defaults to current directory) |

**Process:** Detects conflicts → prompts for resolution → writes as eCL PLAN.md → validates via `ecl-plan-checker`

```bash
/ecl-import --from /tmp/team-plan.md    # Import and validate an external plan
/ecl-import --from-ecl2                # Migrate from eCL-2 back to v1 (current dir)
/ecl-import --from-ecl2 --path ~/old-project  # Migrate from a different path
```
...
Bootstrap or merge a .planning/ setup from existing ADRs, PRDs, SPECs, and docs in a repo. Runs parallel classification (`ecl-doc-classifier`) plus synthesis with precedence rules and cycle detection (`ecl-doc-synthesizer`). Produces a three-bucket conflicts report (`INGEST-CONFLICTS.md`: auto-resolved, competing-variants, unresolved-blockers) and hard-blocks on LOCKED-vs-LOCKED ADR contradictions.
...
**Limits:** v1 caps at 50 docs per invocation. Extracts the shared conflict-detection contract into `references/doc-conflict-engine.md`, which `/ecl-import` also consumes.

```bash
/ecl-ingest-docs                            # Scan repo root, auto-detect mode
/ecl-ingest-docs docs/                      # Only ingest under docs/
/ecl-ingest-docs --manifest ingest.yaml     # Explicit precedence manifest
```
...
Execute ad-hoc task with eCL guarantees.
...
```bash
/ecl-quick                          # Basic quick task
/ecl-quick --discuss --research     # Discussion + research + planning
/ecl-quick --validate               # Plan-checking + verification only
/ecl-quick --full                   # Complete quality pipeline
/ecl-quick list                     # List all quick tasks
/ecl-quick status my-task-slug      # Show status of a quick task
/ecl-quick resume my-task-slug      # Resume a quick task
```
...
| `--converge` | Route each planning step through `/ecl-plan-review-convergence`; requires `workflow.plan_review_convergence=true` |
| `--cross-ai` | Alias for `--converge` |
| Reviewer flags | With `--converge`, pass through `--codex`, `--gemini`, `--claude`, `--opencode`, `--ollama`, `--lm-studio`, `--llama-cpp`, `--all`, and `--max-cycles N` |
...
/ecl-autonomous --only 4 --converge # Run one phase with plan convergence
/ecl-autonomous --converge --all --max-cycles 5
...
**Subcommands:**
- `/ecl-debug list` — List all active debug sessions with status, hypothesis, and next action
- `/ecl-debug status <slug>` — Print full summary of a session (Evidence count, Eliminated count, Resolution, TDD checkpoint) without spawning an agent
- `/ecl-debug continue <slug>` — Resume a specific session by slug (surfaces Current Focus then spawns continuation agent)
- `/ecl-debug [--diagnose] <description>` — Start new debug session (existing behavior; `--diagnose` stops at root cause without applying fix)
...
```bash
/ecl-add-tests 2                    # Generate tests for phase 2
```
...
```bash
/ecl-stats                          # Project metrics dashboard
```
...
```bash
/ecl-profile-user                   # Analyze sessions and build profile
/ecl-profile-user --questionnaire   # Interactive questionnaire fallback
/ecl-profile-user --refresh         # Re-generate from fresh analysis
```
...
```bash
/ecl-health                         # Check integrity
/ecl-health --repair                # Check and fix
/ecl-health --context               # Context-utilization triage
```
...
```bash
/ecl-spike                              # Interactive intake
/ecl-spike "can we stream LLM tokens through SSE"
/ecl-spike --quick websocket-vs-polling
/ecl-spike --wrap-up                    # Package findings into a reusable skill
```
...
```bash
/ecl-sketch                             # Interactive mood intake
/ecl-sketch "dashboard layout"
/ecl-sketch --quick "sidebar navigation"
/ecl-sketch --text "onboarding flow"    # Non-Claude runtime
/ecl-sketch --wrap-up                   # Package winning sketch into a skill
```
...
Post-mortem investigation for failed eCL workflows — diagnoses what went wrong.
...
```bash
/ecl-forensics                              # Interactive — prompted for problem
/ecl-forensics "Phase 3 execution stalled"  # With problem description
```
...
```bash
/ecl-extract-learnings 3                    # Extract learnings from phase 3
/ecl-extract-learnings --all                # Extract from all completed phases
```
...
**Prerequisites:** Active eCL project
**Produces:** Workstream directories under `.planning/`, state tracking per workstream

```bash
/ecl-workstreams                    # List all workstreams
/ecl-workstreams create backend-api # Create new workstream
/ecl-workstreams switch backend-api # Set active workstream
/ecl-workstreams status backend-api # Detailed status
/ecl-workstreams progress           # Cross-workstream progress overview
/ecl-workstreams complete backend-api  # Archive completed workstream
/ecl-workstreams resume backend-api    # Resume work in workstream
```
...
Configure eCL settings interactively — workflow toggles, advanced knobs, integrations, and model profile — with a single consolidated command.
...
/ecl-config --advanced              # Power-user knobs (six-section prompt)
/ecl-config --integrations          # API keys, review CLI routing, agent skills
/ecl-config --profile budget        # Switch to budget profile
/ecl-config --profile quality       # Switch to quality profile
```
...
```bash
/ecl-surface list                   # Show current surface
/ecl-surface profile standard       # Switch to standard profile
/ecl-surface disable utility        # Disable the utility cluster
/ecl-surface reset                  # Restore install-time profile
```
...
```bash
/ecl-map-codebase                   # Full codebase analysis (4 parallel agents)
/ecl-map-codebase auth              # Focus on auth area
/ecl-map-codebase --fast            # Quick tech + arch overview (1 agent)
/ecl-map-codebase --fast --focus quality  # Quality and code health only
/ecl-map-codebase --query authentication  # Search intel for a term
```
...
```bash
/ecl-graphify build                 # Build or rebuild the knowledge graph
/ecl-graphify query authentication  # Search the graph for a term
/ecl-graphify status                # Show freshness and statistics
/ecl-graphify diff                  # Show changes since last build
```

**Programmatic access:** `node ecl-tools.cjs graphify <build|query|status|diff|snapshot>` — see [CLI Tools Reference](CLI-TOOLS.md).

### `/ecl-mempalace-recall`
...
```bash
/ecl-mempalace-recall          # Recall for the current phase
/ecl-mempalace-recall 03-auth  # Recall scoped to a specific phase slug
```

---

### `/ecl-mempalace-capture`
...
```bash
/ecl-mempalace-capture CONTEXT.md   # File CONTEXT.md → decisions room
/ecl-mempalace-capture PLAN.md      # File PLAN.md → planning room
/ecl-mempalace-capture SUMMARY.md   # File SUMMARY.md → milestones room
```
...
```bash
/ecl-ai-integration-phase              # Wizard for the current phase
/ecl-ai-integration-phase 3           # Wizard for a specific phase
```
...
Audit an executed AI phase's evaluation coverage and produce an EVAL-REVIEW.md remediation plan. Checks implementation against the `AI-SPEC.md` evaluation plan produced by `/ecl-ai-integration-phase`. Scores each eval dimension as COVERED/PARTIAL/MISSING.
...
```bash
/ecl-eval-review                       # Audit current phase
/ecl-eval-review 3                     # Audit a specific phase
```
...
Update eCL with changelog preview, and optionally sync skills or reapply local patches.
...
**Spawns:** `ecl-code-reviewer` agent; `ecl-code-fixer` agent (with `--fix`)

**Optional structural pre-pass:** Set `code_quality.fallow.enabled` to `true` to run fallow before the agent review. eCL writes `{phase}/FALLOW.json` and embeds a `Structural Findings (fallow)` section in `REVIEW.md`. Configure scope and profile with `code_quality.fallow.scope` and `code_quality.fallow.profile`.

```bash
/ecl-code-review 3                          # Standard review for phase 3
/ecl-code-review 2 --depth=deep             # Deep cross-file review
/ecl-code-review 4 --files src/auth.ts,src/token.ts  # Explicit file list
/ecl-code-review 3 --fix                    # Review then fix Critical + Warning findings
/ecl-code-review 3 --fix --all             # Review then fix all findings including Info
/ecl-code-review 3 --fix --auto            # Review, fix, and re-review until clean (max 3 iterations)
```
...
```bash
/ecl-audit-fix                              # Run audit-uat, fix medium+ issues (max 5)
/ecl-audit-fix --severity high             # Only fix high-severity issues
/ecl-audit-fix --dry-run                   # Preview classification without fixing
/ecl-audit-fix --max 10 --severity all     # Fix up to 10 issues of any severity
```
...
**Not a replacement for `/ecl-quick`** — use `/ecl-quick` for anything needing research, multi-step planning, or verification.
...
- If `review.default_reviewers` is **unset**, `/ecl-review` runs all detected reviewers (current default behavior).
- If `review.default_reviewers` is **set**, `/ecl-review` runs only that subset (for example `["gemini","codex"]`).
...
**Produces:** `{phase}-REVIEWS.md` — consumable by `/ecl-plan-phase --reviews`

```bash
# set project default reviewers for no-flag /ecl-review runs
ecl config-set review.default_reviewers '["gemini","codex"]'

/ecl-review --phase 2             # runs gemini+codex from config
/ecl-review --phase 3 --all
/ecl-review --phase 2 --gemini
/ecl-review --phase 2 --cursor    # one-off override
```
...
**Purpose:** Reviewers see only code changes, not eCL planning artifacts.
...
**Spawns:** `ecl-security-auditor` agent
...
```bash
/ecl-secure-phase                   # Audit last completed phase
/ecl-secure-phase 5                 # Audit specific phase
```
...
**Spawns:** `ecl-doc-writer` agents (one per doc type), then `ecl-doc-verifier` agents for factual verification
...
```bash
/ecl-docs-update                    # Generate/update docs interactively
/ecl-docs-update --force            # Regenerate all docs
/ecl-docs-update --verify-only      # Verify existing docs only
```
...
**Backlog:** 999.x numbering keeps items outside the active phase sequence; phase directories are created immediately so `/ecl-discuss-phase` and `/ecl-plan-phase` work on them.
**Seeds:** Preserve full WHY, WHEN to surface, and breadcrumbs — consumed by `/ecl-new-milestone`.

**Produces:** `.planning/todos/` (default), note files (--note), ROADMAP.md backlog section (--backlog), `.planning/seeds/SEED-NNN-slug.md` (--seed)

```bash
/ecl-capture "Consider adding dark mode support"   # Add todo
/ecl-capture --note "Caching strategy idea"        # Quick note
/ecl-capture --note list                           # List all notes
/ecl-capture --note promote 3                      # Promote note 3 to todo
/ecl-capture --backlog "GraphQL API layer"         # Add to backlog
/ecl-capture --seed "Add real-time collaboration when WebSocket infra is in place"
/ecl-capture --list                                # Browse and act on todos
```
...
Threads are lightweight cross-session knowledge stores for work that spans multiple sessions but doesn't belong to any specific phase. Lighter weight than `/ecl-pause-work`.

```bash
/ecl-thread                         # List all threads
/ecl-thread list --open             # List only open/in-progress threads
/ecl-thread list --resolved         # List only resolved threads
/ecl-thread status fix-deploy-key   # Show thread status
/ecl-thread close fix-deploy-key    # Mark thread as resolved
/ecl-thread fix-deploy-key-auth     # Resume thread
/ecl-thread "Investigate TCP timeout in pasta service"  # Create new
```
...
```bash
node ecl-tools.cjs state sync             # Reconstruct STATE.md from disk
node ecl-tools.cjs state sync --verify    # Dry-run: show changes without writing
```
...
| Hook | Purpose |
|------|---------|
| `ecl-validate-commit.sh` | Enforce Conventional Commits format on git commit messages |
| `ecl-session-state.sh` | Track session state transitions |
| `ecl-phase-boundary.sh` | Enforce phase boundary checks |
...
To join the eCL Discord community, visit the link in the eCL README or run `/ecl-help` and follow the Discord link shown there.

---

## Contributing: Skill Description Standards

Skill descriptions (the `description:` field in each `commands/ecl/*.md` frontmatter) are
