 ...
 *
 * Ported from evolv-coder-lite/bin/lib/uat.cjs.
...
/** Same string as `buildCheckpoint` in `evolv-coder-lite/bin/lib/uat.cjs`. */
 ...
 *
 * Fixes #2788: when ecl-verifier encodes human items in YAML frontmatter
...
function parseVerificationItems(content: string, status: string, fm?: Record<string, unknown>): Record<string, unknown>[] {
  ...
  if (status === 'human_needed') {
    ...
    // ecl-verifier writes items here; body-section fallback is secondary.
    if (fm) {
