...
**사전 조건:** `/ecl-execute-phase N`을 실행했는데 `VERIFICATION.md` 작성 전에 실행이 중단된 경우, 또는 예상치 못한 출력, 누락된 파일, 멈춘 스피너가 보이는 경우.
...
이것은 정상 동작이며 멈춤이 아닙니다. eCL 서브에이전트는 격리된 컨텍스트 창에서 실행됩니다. 스폰 라인의 활성 상태 메모가 이를 확인해 줍니다. 세션을 중단하지 마세요.
...
eCL는 실행자를 디스패치하기 전에 `SUMMARY.md` 파일이 있는지 확인합니다. 이미 `SUMMARY.md`가 있는 플랜은 자동으로 건너뜁니다.
...
eCL는 다음 실행 시 이를 감지하고 세 가지 옵션이 있는 안전 재개 게이트를 표시합니다:
...
eCL는 증상을 수집하고, 과학적 방법을 사용한 구조화된 조사를 실행하며, 수정안을 제안합니다. 설정에서 `tdd_mode: true`가 지정된 경우 수정을 적용하기 전에 실패하는 테스트를 요구합니다.
...
eCL는 git 히스토리, `.planning/` 아티팩트 완전성, STATE.md 일관성, 커밋되지 않은 작업, 고아 워크트리를 분석합니다. `.planning/forensics/report-<timestamp>.md`에 구조화된 보고서를 작성하고 권장 복구 단계를 제시합니다.

`/ecl-forensics`는 읽기 전용으로 프로젝트 파일을 절대 수정하지 않습니다.
...
eCL는 이미 `SUMMARY.md`가 존재하는 플랜을 건너뛰고 나머지 플랜에 대해서만 실행자를 디스패치합니다.
...
페이즈 `3`의 플랜 `02`에 대한 모든 커밋을 되돌립니다. eCL는 변경 사항을 작성하기 전에 확인 게이트를 표시합니다.
...
페이즈 `3`의 모든 커밋을 되돌립니다. eCL는 이후 페이즈가 이 페이즈에 의존하는지 확인하고 진행 전에 경고합니다.
...
가장 최근 eCL 커밋 다섯 개를 표시하고 어떤 것을 되돌릴지 선택할 수 있게 합니다.
