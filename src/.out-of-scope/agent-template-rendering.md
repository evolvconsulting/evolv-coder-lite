...
**Source:** [#2758](https://github.com/evolvconsulting/evolv-coder-lite/issues/2758)
...
`ecl-sdk agents render` subcommand. Conditional branches resolve at render time
...
- The `use_worktrees` branch in `ecl-executor` is resolved deterministically via
  `ecl-sdk query config-get` in bash — it is not LLM-interpreted.
...
patch-migration surface (`ecl-local-patches/` rebase logic, by the reporter's own
...
`ecl-context-monitor` output despite being asked. Without a measured baseline,
...
The `/ecl-reapply-patches` three-way-merge migration for `ecl-local-patches/`
...
- Provides measured token deltas via `ecl-context-monitor` against a
...
- `evolv-coder-lite/workflows/` — workflow-level config embedding before subagent
