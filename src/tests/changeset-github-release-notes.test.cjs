...
function createTaggedRepo() {
  const repo = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-release-notes-'));
  run('git', ['init', '-q'], repo);
  ...
  writeFragment(repo, 'fix-install-sdk', 'Fixed', 101, '**`ecl-sdk` now installs reliably** — persistent PATH is checked.');
  writeFragment(repo, 'remove-intel-noise', 'Removed', 102, '**`ecl-intel-updater` no longer emits layout detection noise** — ordinary projects stay quiet.');
