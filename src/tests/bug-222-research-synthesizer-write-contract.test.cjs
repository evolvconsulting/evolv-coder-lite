...
const SYNTHESIZER_PATH = path.join(REPO_ROOT, 'agents', 'ecl-research-synthesizer.md');
...
describe('bug #222 recurrence: orchestrator self-heals when synthesizer returns SUMMARY.md inline', () => {
  const WORKFLOWS = [
    path.join(REPO_ROOT, 'evolv-coder-lite', 'workflows', 'new-project.md'),
    path.join(REPO_ROOT, 'evolv-coder-lite', 'workflows', 'new-milestone.md'),
  ];
  ...
  for (const wf of WORKFLOWS) {
    ...
    test(`${name} has the #222 synthesizer SUMMARY.md self-heal guard`, () => {
      ...
      assert.match(text, /ecl:write-continue|verify-summary/i,
      ...
      assert.match(text, /ecl-roadmapper[\s\S]{0,200}?(missing|incomplete|do NOT)/i,
        `${name} must block spawning ecl-roadmapper when SUMMARY.md is missing or incomplete.`);
    ...
    test(`${name} runs the #222 self-heal AFTER the synthesizer and BEFORE ecl-roadmapper`, () => {
      const text = fs.readFileSync(wf, 'utf8');
      const synthIdx = text.indexOf('subagent_type="ecl-research-synthesizer"');
      const healIdx = text.indexOf('Synthesizer output self-heal (#222)');
      const roadIdx = text.indexOf('subagent_type="ecl-roadmapper"');
      assert.ok(synthIdx >= 0, `${name}: synthesizer dispatch not found`);
      assert.ok(healIdx >= 0, `${name}: #222 self-heal block not found`);
      assert.ok(roadIdx >= 0, `${name}: ecl-roadmapper dispatch not found`);
      assert.ok(healIdx > synthIdx, `${name}: self-heal must come AFTER the synthesizer dispatch`);
      assert.ok(roadIdx > healIdx, `${name}: self-heal must come BEFORE the ecl-roadmapper dispatch`);
    });
  }
});
