---
name: ecl:quick
description: Execute a quick task with eCL guarantees (atomic commits, state tracking) but skip optional agents
...
---
<objective>
Execute small, ad-hoc tasks with eCL guarantees (atomic commits, STATE.md tracking).

Quick mode is the same system with a shorter path:
- Spawns ecl-planner (quick mode) + ecl-executor(s)
...
<execution_context>
@~/.claude/evolv-coder-lite/workflows/quick.md
</execution_context>
...
- Check if SUMMARY.md exists; if so, read `status` from its frontmatter via:
  ```bash
  ecl-tools query frontmatter.get .planning/quick/{dir}/SUMMARY.md status
  ```
...
─────────────────────────────────────
Resume with: /ecl:quick resume {slug}
```
...
- Status fields read via `ecl-tools query frontmatter.get` — never eval'd or shell-expanded
</security_notes>
