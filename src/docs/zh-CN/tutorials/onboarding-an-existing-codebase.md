...
在本教程中，您将把 eCL Core 引入一个已有代码的仓库。您将对代码库进行映射，创建一个描述您所*新增*内容的项目，并针对一个小型聚焦变更运行首次讨论与规划循环。完成后，eCL Core 的规划流水线将了解您的技术栈、规范和关注点——并在每次规划时运用这些知识。

---

## 您将构建的内容

我们将向一个现有的 Express 应用程序添加一个 `GET /health` 端点。该变更足够小，不会分散您对真正核心内容的注意力：eCL Core 在规划任何内容之前如何学习您的代码库。
...
在创建项目之前，先让 eCL Core 了解已有的内容。这是使棕地规划准确的关键步骤。

```text
/ecl-map-codebase
```

eCL Core 会派生四个并行映射子代理（您将看到"Spawning 4 parallel codebase mapper agents…"——这需要 1–5 分钟；请勿中断）。每个代理专注于不同的关注点：
...
打开 `.planning/codebase/STACK.md`。您将看到 eCL Core 检测到的语言、运行时、框架版本和关键依赖项——这些内容基于实际读取的文件，而非猜测。

打开 `.planning/codebase/CONVENTIONS.md`。您将看到它从您的源代码中观察到的命名规范、错误处理模式和代码风格规则。eCL Core 为该仓库生成的每个计划都将自动遵循这些规范。
...
现在创建项目。由于 eCL Core 在上一步中发现了现有代码，它已经知道这是一个棕地项目。当您运行 `/ecl-new-project` 时，问题将聚焦于您所*新增*的内容，而非重新描述已有的内容：

```text
/ecl-new-project
```

eCL Core 会询问您想构建什么。请用您正在添加的功能来回答，而不是描述整个代码库：
...
eCL Core 会进一步提出少量澄清问题，然后继续创建需求和路线图。由于它已读取 `ARCHITECTURE.md` 和 `STACK.md`，它会自动将现有能力映射到 `PROJECT.md` 的 **Validated** 部分——您无需描述现有的 API 接口。
...
注意 `.planning/codebase/` 已经从第 3 步存在。eCL Core 在编写 `PROJECT.md` 时读取了这些文件，这就是为什么它无需您描述即可填充已验证的需求。
...
由于 eCL Core 已读取您的 `CONVENTIONS.md` 和 `ARCHITECTURE.md`，其问题基于您的实际代码库——而非通用建议。您可能会看到：
...
打开 `01-01-PLAN.md`。注意 `<files>` 标签引用了 `src/routes/health.js`——正是您在讨论中指定的路径，与 eCL Core 在代码库映射中观察到的路由模式一致。这正是代码库映射发挥作用的体现。
...
对于每个未来的功能，当结构发生重大变化时，再次运行 `/ecl-map-codebase`，以保持代码库映射的时效性。

---

## 您学到了什么

- `/ecl-map-codebase` 如何运行四个并行代理，在 `.planning/codebase/` 中生成 `STACK.md`、`ARCHITECTURE.md`、`CONVENTIONS.md`、`CONCERNS.md`、`STRUCTURE.md`、`TESTING.md` 和 `INTEGRATIONS.md`。
- 在棕地仓库中运行 `/ecl-new-project` 如何将问题聚焦于您所*新增*的内容，并从现有代码中填充已验证的需求。
- 代码库映射如何塑造 `/ecl-discuss-phase` 中的每个问题——文件路径、模式和规范均来自您的实际代码。
...
- [通过命令使用映射代码库](../COMMANDS.md) — 所有 `/ecl-map-codebase` 标志和子命令
- [文档索引](../README.md)
