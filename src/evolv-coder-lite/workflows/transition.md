...
There is no `/ecl-transition` command. This workflow is invoked automatically by
`execute-phase` during auto-advance, or inline by the orchestrator after phase
verification. Users should never be told to run `/ecl-transition`.

**Valid user commands for phase progression:**
- `/ecl:discuss-phase {N}` — discuss a phase before planning
- `/ecl:plan-phase {N}` — plan a phase
- `/ecl:execute-phase {N}` — execute a phase
- `/ecl:progress` — see roadmap progress
...
These will carry forward as debt. Review: `/ecl:audit-uat`
```
...
**Delegate ROADMAP.md and STATE.md updates to `ecl-tools.cjs query phase.complete`:**
...
TRANSITION=$(ecl_run query phase.complete "${current_phase}")
```
...
```text
@~/.claude/evolv-coder-lite/workflows/graduation.md
```
...
**Note:** Basic position updates (Current Phase, Status, Current Plan, Last Activity) were already handled by `ecl-tools.cjs query phase.complete` in the update_roadmap_and_state step.

Verify the updates are correct by reading STATE.md. If the progress bar needs updating, use:

```bash
PROGRESS=$(ecl_run query progress.bar --raw)
```
...
**Use the transition result from `ecl-tools.cjs query phase.complete`:**
...
**Skip this check if NOT in workstream mode** (i.e., `ECL_WORKSTREAM` is not set / flat mode).
In flat mode, go directly to **Route B**.

```bash
# Only check if we're in workstream mode
if [ -n "$ECL_WORKSTREAM" ]; then
  WS_LIST=$(ecl_run query workstream.list --raw)
fi
```
...
Filter out the current workstream (`$ECL_WORKSTREAM`) and any workstreams with
...
Exit skill and invoke SlashCommand("/ecl:plan-phase [X+1] --auto ${ECL_WS}")
...
Exit skill and invoke SlashCommand("/ecl:discuss-phase [X+1] --auto ${ECL_WS}")
...
`/ecl:discuss-phase [X+1] ${ECL_WS}` — gather context and clarify approach

---

**Also available:**
- `/ecl:plan-phase [X+1] ${ECL_WS}` — skip discussion, plan directly
- `/ecl:plan-phase --research-phase [X+1] ${ECL_WS}` — investigate unknowns
...
`/ecl:plan-phase [X+1] ${ECL_WS}`

---

**Also available:**
- `/ecl:discuss-phase [X+1] ${ECL_WS}` — revisit context
- `/ecl:plan-phase --research-phase [X+1] ${ECL_WS}` — investigate unknowns
...
```bash
ecl_run query config-set workflow._auto_chain_active false
```
...
`/ecl:workstreams complete {current_ws_name} ${ECL_WS}`

See overall milestone progress:

`/ecl:workstreams progress ${ECL_WS}`
...
Do NOT suggest `/ecl:complete-milestone` or `/ecl:new-milestone`.
...
Exit skill and invoke SlashCommand("/ecl:complete-milestone {version} ${ECL_WS}")
...
`/ecl:complete-milestone {version} ${ECL_WS}`
