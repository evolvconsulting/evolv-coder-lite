...
```bash
PHASE_STATE=$(ecl-sdk query init.phase-op ${PHASE_NUM})
```
...
```bash
DETAIL=$(ecl-sdk query roadmap.get-phase ${PHASE_NUM})
```
...
```bash
ecl-sdk query commit "docs(${PADDED_PHASE}): smart discuss context" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
