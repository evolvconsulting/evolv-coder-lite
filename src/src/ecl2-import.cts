/**
 * ecl2-import — Reverse migration from eCL-2 (.ecl/) to eCL v1 (.planning/)
 *
 * Reads a eCL-2 project directory structure and produces a complete
 * .planning/ artifact tree in eCL v1 format.
 *
 * eCL-2 hierarchy:  Milestone → Slice → Task
 * eCL v1 hierarchy: Milestone (in ROADMAP.md) → Phase → Plan
 *
 * Mapping rules:
 ...
 *
 * ADR-457 build-at-publish: the hand-written bin/lib/ecl2-import.cjs collapsed
...
/**
 * Find the .ecl/ directory starting from a project root.
 * Returns the absolute path or null if not found.
 */
function findGsd2Root(startPath: string): string | null {
  if (path.basename(startPath) === '.ecl' && fs.existsSync(startPath)) {
    return startPath;
  }
  const candidate = path.join(startPath, '.ecl');
...
/**
 * Parse the ## Slices section from a eCL-2 milestone ROADMAP.md.
...
/**
 * Parse the milestone title from the first heading in a eCL-2 ROADMAP.md.
 * Format: # M001: Title
 */
...
/**
 * Parse a task title from a eCL-2 T##-PLAN.md.
 * Format: # T01: Title
 */
...
/**
 * Parse the ## Description body from a eCL-2 task plan.
 */
...
/**
 * Parse ## Must-Haves items from a eCL-2 task plan.
 */
...
/**
 * Read all task plan files from a eCL-2 tasks/ directory.
 */
...
/**
 * Parse a complete eCL-2 .ecl/ directory into a structured representation.
 */
...
/**
 * Build a eCL v1 PLAN.md from a eCL-2 task.
 */
...
/**
 * Build a eCL v1 SUMMARY.md from a eCL-2 task summary.
 * Strips the eCL-2 frontmatter and preserves the body.
 */
function buildSummaryMd(task: TaskInfo, phasePrefix: string, planPrefix: string): string {
  const raw = task.summary || '';
  // Strip eCL-2 frontmatter block (--- ... ---) if present
  ...
  return [
    ...
    '---',
    '',
    body || 'Task completed (migrated from eCL-2).',
    '',
  ].join('\n');
}

/**
 * Build a eCL v1 XX-CONTEXT.md from a eCL-2 slice.
 */
function buildContextMd(slice: Slice, phasePrefix: string): string {
  const lines = [
    `# Phase ${phasePrefix} Context`,
    '',
    `Migrated from eCL-2 slice ${slice.id}: ${slice.title}`,
  ];
...
/**
 * Build the eCL v1 ROADMAP.md with milestone-sectioned format.
 */
...
/**
 * Build the eCL v1 STATE.md reflecting the current position in the project.
 */
function buildStateMd(phaseMap: PhaseMapEntry[]): string {
  ...
  return [
    ...
    `Status: ${status}`,
    `Last activity: ${today} — Migrated from eCL-2`,
    '',
    `Progress: ${bar} ${pct}%`,
    '',
    '## Accumulated Context',
    ...
    '### Decisions',
    '',
    'Migrated from eCL-2. Review PROJECT.md for key decisions.',
    '',
    '### Blockers/Concerns',
    '',
    'None.',
    ...
    '',
    `Last session: ${today}`,
    'Stopped at: Migration from eCL-2 completed',
    'Resume file: None',
    '',
  ].join('\n');
}
...
/**
 * Convert parsed eCL-2 data into a map of relative path → file content.
...
function buildPreview(gsd2Data: Gsd2Data, artifacts: Map<string, string>, projectDir: string): string {
  ...
  lines.push('  - eCL-2 cost/token ledger (no v1 equivalent)');
  lines.push(`  - eCL-2 database state (rebuilt from files on first ${formatGsdSlash('health', resolveRuntime(projectDir)) as string})`);
...
/**
 * Entry point called from ecl-tools.cjs.
...
function cmdFromGsd2(args: string[], cwd: string, raw: boolean): void {
  ...
  if (!gsdDir) {
    output({ success: false, error: `No .ecl/ directory found in ${projectDir}` }, raw, undefined);
    return;
  }
