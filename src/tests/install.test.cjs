...
describe('getGlobalConfigDir — all runtimes default paths', () => {
  ...
  // no registry entry.
  const { runtimes: _registryRuntimes } = require('../evolv-coder-lite/bin/lib/capability-registry.cjs');
...
describe('getGlobalConfigDir/getConfigDirFromHome — antigravity 2.x layout detection', () => {
  ...
  test('uses ~/.gemini/antigravity-ide when legacy dir is absent and ide dir exists', () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-antigravity-ide-'));
    try {
  ...
  test('uses ~/.gemini/antigravity-cli when only cli dir exists', () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-antigravity-cli-'));
    try {
...
describe('getConfigDirFromHome — spot-checks', () => {
  ...
  test('antigravity returns .agents (local) and legacy fallback global path when no 2.x dirs exist', () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-antigravity-empty-'));
...
describe('install/uninstall — hermes (nested skills/ecl/<router>/skills/<stem>/ layout)', () => {
  let tmpDir;
  let previousCwd;

  beforeEach(() => {
    tmpDir = createTempDir('ecl-hermes-install-');
  ...
  test('installs eCL into ./.hermes and removes it cleanly', () => {
    ...
    // hermes nests: skills/ecl/ecl-<router>/skills/<stem>/SKILL.md (#947 — canonical ecl- prefix)
    const hermesHelpPath = nestedSkillPath(path.join(targetDir, 'skills', 'ecl'), 'ecl-', 'help');
    assert.ok(fs.existsSync(hermesHelpPath),
      `help SKILL.md must exist at nested path: ${path.relative(targetDir, hermesHelpPath)}`);
    assert.ok(fs.existsSync(path.join(targetDir, 'skills', 'ecl', 'DESCRIPTION.md')),
    ...
    assert.ok(
      Object.keys(manifest.files).some(f =>
        f.startsWith('skills/ecl/ecl-' + CHILD_ROUTER['help'] + '/skills/help/')
      ),
    ...
    assert.ok(!fs.existsSync(hermesHelpPath));
    assert.ok(!fs.existsSync(path.join(targetDir, 'skills', 'ecl')));
  ...
  test('installed SKILL.md frontmatter conforms to Hermes spec', () => {
    ...
    const targetDir = path.join(tmpDir, '.hermes');
    const categoryDir = path.join(targetDir, 'skills', 'ecl');
    ...
    assert.strictEqual(descFm.name, 'ecl');
...
describe('install/uninstall — qwen (nested skills/ecl-<router>/skills/<stem>/ layout)', () => {
  let tmpDir;
  let previousCwd;

  beforeEach(() => {
    tmpDir = createTempDir('ecl-qwen-install-');
    previousCwd = process.cwd();
    process.chdir(tmpDir);
  });
  ...
  test('installs eCL into ./.qwen and removes it cleanly', () => {
    ...
    // qwen nests: skills/ecl-<router>/skills/<stem>/SKILL.md
    const qwenHelpPath = nestedSkillPath(path.join(targetDir, 'skills'), 'ecl-', 'help');
    ...
    assert.ok(
      Object.keys(manifest.files).some(f =>
        f.startsWith('skills/ecl-' + CHILD_ROUTER['help'] + '/skills/help/')
      )
    );
...
describe('install/uninstall — trae (nested skills/ecl-<router>/skills/<stem>/ layout)', () => {
  let tmpDir;
  let previousCwd;

  beforeEach(() => {
    tmpDir = createTempDir('ecl-trae-install-');
    previousCwd = process.cwd();
    process.chdir(tmpDir);
  });
  ...
  test('installs eCL into ./.trae and removes it cleanly (typed IR result)', () => {
    ...
    // trae nests: skills/ecl-<router>/skills/<stem>/SKILL.md
    const traeHelpPath = nestedSkillPath(path.join(targetDir, 'skills'), 'ecl-', 'help');
...
describe('uninstall skills cleanup — hermes', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('ecl-hermes-uninstall-');
    previousCwd = process.cwd();
    process.chdir(tmpDir);
  });
  ...
  test('removes skills/ecl/ category dir', () => {
    install(false, 'hermes');
    const targetDir = path.join(tmpDir, '.hermes');
    const categoryDir = path.join(targetDir, 'skills', 'ecl');
  ...
  test('preserves non-eCL skill directories', () => {
    install(false, 'hermes');
    const targetDir = path.join(tmpDir, '.hermes');
...
for (const runtime of ['hermes', 'qwen']) {
  describe(`no Claude references leak into ${runtime} install`, () => {
    ...
    beforeEach(() => {
      tmpDir = createTempDir(`ecl-${runtime}-refs-`);
      previousCwd = process.cwd();
      process.chdir(tmpDir);
...
describe('resolveKiloConfigPath', () => {
  let tmpDir;
  beforeEach(() => { tmpDir = createTempProject('ecl-kilo-'); });
...
describe('configureKiloPermissions', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempProject('ecl-kilo-perms-');
  ...
  test('writes eCL permissions to kilo.json when config is missing', () => {
    configureKiloPermissions(true);
    const configPath = path.join(configDir, 'kilo.json');
    const config = JSON.parse(fs.readFileSync(configPath, 'utf8'));
    const gsdPath = `${configDir.replace(/\\/g, '/')}/evolv-coder-lite/*`;
  ...
  test('writes permissions to an explicit config dir argument', () => {
    ...
    const config = JSON.parse(fs.readFileSync(configPath, 'utf8'));
    const gsdPath = `${explicitDir.replace(/\\/g, '/')}/evolv-coder-lite/*`;
    assert.strictEqual(config.permission.read[gsdPath], 'allow');
    assert.strictEqual(config.permission.external_directory[gsdPath], 'allow');
  });
});
...
describe('Kilo integration — install/uninstall behaviour', () => {
  ...
  const updateWorkflowSrc = fs.readFileSync(
    path.join(__dirname, '..', 'evolv-coder-lite', 'workflows', 'update.md'), 'utf8');
  // #498: update.md's runtime/scope/config-dir resolution moved into the tested
  // projection evolv-coder-lite/bin/lib/update-context.cjs. Custom-config-dir
  // detection (kilo.jsonc, KILO_CONFIG) is now asserted there.
  const updateContextSrc = fs.readFileSync(
    path.join(__dirname, '..', 'evolv-coder-lite', 'bin', 'lib', 'update-context.cjs'), 'utf8');
  ...
  beforeEach(() => {
    tmpDir = createTempDir('ecl-kilo-integration-');
    previousCwd = process.cwd();
    process.chdir(tmpDir);
  ...
  test('install() for kilo writes artifacts to the configDir it returns', () => {
    ...
    //
    // IMPORTANT: ECL_TEST_MODE=1 (set at the top of this file) suppresses the
  ...
  test('uninstall removes eCL permissions from the resolved kilo config path', () => {
    ...
    // strip the eCL permission entries from kilo.json at the resolved path.
    const result = install(false, 'kilo');
    const configDir = result.configDir;
    ...
    const gsdGlob = `${configDir.replace(/\\/g, '/')}/evolv-coder-lite/*`;
    assert.ok(
      beforeConfig.permission.read[gsdGlob] === 'allow',
      'pre-condition: eCL read permission must exist before uninstall',
    );

    uninstall(false, 'kilo');

    // After uninstall the eCL permission keys must be absent. The file may
    // still exist (Kilo preserves user settings) but the evolv-coder-lite/* entries
    // must be gone.
    ...
    assert.ok(
      ...
      `eCL read permission must be removed from ${kiloJsonPath} after uninstall`,
    );
    assert.ok(
      !(afterConfig.permission && afterConfig.permission.external_directory &&
        afterConfig.permission.external_directory[gsdGlob]),
      `eCL external_directory permission must be removed from ${kiloJsonPath} after uninstall`,
    );
  });
...
describe('install — changeset CLI lands at scripts/changeset/cli.cjs (#935)', () => {
  ...
  // by the installer so $ECL_DIR/scripts/changeset/cli.cjs resolves at runtime.
  // Before this fix, scripts/ was never copied and /ecl-update changelog preview
  // silently failed on every real install.
  let tmpDir;
  let previousCwd;

  beforeEach(() => {
    tmpDir = createTempDir('ecl-changeset-install-');
    previousCwd = process.cwd();
    process.chdir(tmpDir);
  });
  ...
  test('installed cli.cjs can run extract subcommand end-to-end (#935)', () => {
    ...
    const claudeDir = path.join(tmpDir, '.claude');
    const cliPath = path.join(claudeDir, 'scripts', 'changeset', 'cli.cjs');
    // Use the CHANGELOG.md that was installed into evolv-coder-lite/ (installed by the installer)
    const changelogPath = path.join(claudeDir, 'evolv-coder-lite', 'CHANGELOG.md');
    assert.ok(fs.existsSync(changelogPath), 'CHANGELOG.md must be installed under evolv-coder-lite/');
    const { spawnSync } = require('node:child_process');
...
describe('install — fix-slash-commands.cjs lands at scripts/fix-slash-commands.cjs (#1223)', () => {
  ...
  // config dir by the installer so evolv-coder-lite/bin/lib/command-roster.cjs can require it
  // via '../../../scripts/fix-slash-commands.cjs'. Before this fix, the file was never
  // installed and every ecl-tools command crashed with MODULE_NOT_FOUND (#1223).
  let tmpDir;
  let previousCwd;

  beforeEach(() => {
    tmpDir = createTempDir('ecl-fix-slash-install-');
    previousCwd = process.cwd();
    process.chdir(tmpDir);
  });
  ...
  test('installed ecl-tools.cjs query loads without MODULE_NOT_FOUND (#1223)', () => {
    // End-to-end smoke: spawning ecl-tools.cjs must not crash with MODULE_NOT_FOUND.
    // This directly exercises the command-roster → fix-slash-commands require chain.
    install(false, 'claude');
    const claudeDir = path.join(tmpDir, '.claude');
    const gsdToolsPath = path.join(claudeDir, 'evolv-coder-lite', 'bin', 'ecl-tools.cjs');
    assert.ok(fs.existsSync(gsdToolsPath), 'pre-condition: ecl-tools.cjs must be installed');
    const { spawnSync } = require('node:child_process');
    const result = spawnSync(
      process.execPath,
    ...
    assert.ok(
      !result.stderr.includes('MODULE_NOT_FOUND'),
      `ecl-tools.cjs must not crash with MODULE_NOT_FOUND; stderr=${result.stderr}`,
    );
    assert.ok(
      !result.stderr.includes('Cannot find module'),
      `ecl-tools.cjs must resolve all modules; stderr=${result.stderr}`,
    );
  });
...
// ─── Section N: readCmdNames() tolerates absent commands/ecl/ dir (#1223) ────────

describe('readCmdNames() — tolerates missing commands/ecl directory (#1223)', () => {
  // Regression guard: on installs where commands/ecl/ does not exist (e.g. skill-based
  ...
  test('readCmdNames() returns [] from a context where commands/ecl/ is absent', () => {
    ...
    // <tmpRoot>/scripts/ — making COMMANDS_DIR = path.join(__dirname,'..','commands','ecl')
    // resolve to <tmpRoot>/commands/ecl, which does NOT exist. Requiring the copy (not
    ...
    // No commands/ecl/ exists anywhere under or adjacent to tmpRoot.
    const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-readcmdnames-absentdir-'));
    try {
      ...
      // → COMMANDS_DIR = path.join(tmpScriptsDir, '..', 'commands', 'ecl') = <tmpRoot>/commands/ecl
      ...
      const spawnResult = spawnSync(process.execPath, ['-e', script], {
        encoding: 'utf8',
        timeout: 10000,
        env: { ...process.env, ECL_TEST_MODE: '1' },
      });
      assert.ok(
        !spawnResult.stderr.includes('THREW:'),
        `readCmdNames() must not throw when commands/ecl/ is absent; stderr=${spawnResult.stderr}`,
      );
      assert.strictEqual(spawnResult.status, 0,
        `readCmdNames() must return [] (exit 0) when commands/ecl/ is absent; ` +
...
describe('antigravity local install writes to .agents/ canonical dir (#791)', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('ecl-antigravity-791-');
    previousCwd = process.cwd();
    process.chdir(tmpDir);
  });
  ...
  test('install writes workspace skills under .agents/skills/', () => {
    ...
    assert.ok(skillEntries.length > 0, 'at least one ecl-* skill must be installed under .agents/skills/');
  ...
  test('installed agent files reference .agents/ not ~/.claude/ or bare .agent/', () => {
    ...
    assert.ok(agentFiles.length > 0, 'pre-condition: at least one ecl-* agent must be installed');
  ...
  test('global antigravity install still writes to ~/.gemini/antigravity (unchanged)', () => {
    const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-ag-global-'));
    const savedHome = process.env.HOME;
    const savedUserProfile = process.env.USERPROFILE;
    const savedAntigravityConfig = process.env.ANTIGRAVITY_CONFIG_DIR;
    delete process.env.ANTIGRAVITY_CONFIG_DIR;
...
describe('windsurf local install writes to .devin/ canonical dir (#1085)', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('ecl-windsurf-1085-');
    previousCwd = process.cwd();
    process.chdir(tmpDir);
  });
  ...
  test('install writes workspace skills under .devin/skills/', () => {
    ...
    const skillEntries = fs.readdirSync(skillsDir, { withFileTypes: true })
      .filter(e => e.isDirectory() && e.name.startsWith('ecl-'));
    assert.ok(skillEntries.length > 0, 'at least one ecl-* skill must be installed under .devin/skills/');
    const firstSkill = path.join(skillsDir, skillEntries[0].name, 'SKILL.md');
    assert.ok(fs.existsSync(firstSkill), `SKILL.md must exist at ${firstSkill}`);
  });
  ...
  test('installed skill content references .devin/ not bare .windsurf/ or ~/.claude/', () => {
    ...
    const skillEntries = fs.readdirSync(skillsDir, { withFileTypes: true })
      .filter(e => e.isDirectory() && e.name.startsWith('ecl-'));
    assert.ok(skillEntries.length > 0, 'pre-condition: at least one ecl-* skill must be installed');
  ...
  test('global windsurf install still writes to ~/.codeium/windsurf/ (unchanged)', () => {
    const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-ws-global-'));
    const savedHome = process.env.HOME;
    const savedUserProfile = process.env.USERPROFILE;
  ...
  test('global windsurf install skill content references codeium path not .devin/', () => {
    const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-ws-global-c-'));
    const savedHome = process.env.HOME;
    const savedUserProfile = process.env.USERPROFILE;
    const savedWindsurfConfig = process.env.WINDSURF_CONFIG_DIR;
    delete process.env.WINDSURF_CONFIG_DIR;
...
//   3. Negative: ecl-nyquist-auditor has NO disallowedTools key (legitimately writes+edits).
...
const GROUP_A_767 = ['ecl-plan-checker', 'ecl-integration-checker', 'ecl-ui-checker'];
const GROUP_B_767 = ['ecl-verifier', 'ecl-doc-verifier', 'ecl-eval-auditor', 'ecl-ui-auditor'];
...
function runGlobalInstall767(runtime, tmpHome) {
  ...
  const isolatedHome = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-767-home-'));

  const prevEnvVar = process.env[envVar];
  const prevCwd = process.cwd();
  const prevHome = process.env.HOME;
  const prevUserProfile = process.env.USERPROFILE;
  const prevSkipStale = process.env.ECL_SKIP_STALE_SDK_CHECK;

  process.env[envVar] = tmpHome;
  process.env.HOME = isolatedHome;
  process.env.USERPROFILE = isolatedHome;
  process.env.ECL_SKIP_STALE_SDK_CHECK = '1';
  ...
  try {
    ...
    if (prevSkipStale === undefined) delete process.env.ECL_SKIP_STALE_SDK_CHECK;
    else process.env.ECL_SKIP_STALE_SDK_CHECK = prevSkipStale;
...
describe('#767 Claude install: Group A agents have disallowedTools = {Write, Edit, MultiEdit}', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('ecl-767-claude-a-');
...
describe('#767 Claude install: Group B agents have disallowedTools = {Edit, MultiEdit}', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('ecl-767-claude-b-');
    claudeHome = path.join(tmpDir, 'claude-home');
    fs.mkdirSync(claudeHome, { recursive: true });
    runGlobalInstall767('claude', claudeHome);
  });
...
describe('#767 Claude install: ecl-nyquist-auditor has no disallowedTools (legitimately writes+edits)', () => {
  let tmpDir;
  let claudeHome;

  beforeEach(() => {
    tmpDir = createTempDir('ecl-767-claude-nyquist-');
    claudeHome = path.join(tmpDir, 'claude-home');
    fs.mkdirSync(claudeHome, { recursive: true });
    runGlobalInstall767('claude', claudeHome);
  });
  ...
  test('ecl-nyquist-auditor.md has NO disallowedTools key', () => {
    const fm = readFrontmatterText(path.join(claudeHome, 'agents', 'ecl-nyquist-auditor.md'));
    const tools = parseDisallowedToolsSet(fm);
    assert.equal(tools, null,
      `ecl-nyquist-auditor must NOT have disallowedTools in Claude frontmatter\nFrontmatter:\n${fm}`);
  });
});
...
describe('#767 Gemini install: read-only agents do NOT contain disallowedTools (cross-runtime leak guard)', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('ecl-767-gemini-');
