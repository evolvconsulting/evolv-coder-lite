 ...
 * and postToolUse. eCL registers two managed command hooks so Cursor users get
 ...
 *   T9  isManagedCursorHookEntry — returns true for eCL-marked entries
 ...
 *   T19 ECL_CURSOR_HOOK_MARKER constant is exported
 *   T20 ECL_CURSOR_SESSION_HOOK_SCRIPT / ECL_CURSOR_POST_TOOL_HOOK_SCRIPT constants
 */
...
const {
  ...
  INSTALL_SURFACES,
} = require('../evolv-coder-lite/bin/lib/runtime-config-adapter-registry.cjs');
...
function managedEntry(command) {
  return { type: 'command', command, [ECL_CURSOR_HOOK_MARKER]: true };
}
...
describe('reconcileCursorHooksJson', () => {
  test('T1: creates new hooks.json with sessionStart and postToolUse', (t) => {
    ...
    const result = reconcileCursorHooksJson(hooksJsonPath, {
      sessionStart: managedEntry('/node /path/ecl-cursor-session-start.js'),
      postToolUse:  managedEntry('/node /path/ecl-cursor-post-tool.js'),
    });
    ...
    assert.equal(hookTable.sessionStart[0][ECL_CURSOR_HOOK_MARKER], true);
    assert.equal(hookTable.postToolUse[0][ECL_CURSOR_HOOK_MARKER], true);
  });
  ...
  test('T2: idempotent — second run produces same content (no write)', (t) => {
    ...
    const hooksJsonPath = path.join(dir, 'hooks.json');
    const entries = {
      sessionStart: managedEntry('/node /ecl-cursor-session-start.js'),
      postToolUse:  managedEntry('/node /ecl-cursor-post-tool.js'),
    };
  ...
  test('T3: preserves user-owned entries in sessionStart', (t) => {
    ...
    // Both user and eCL entries must survive.
    assert.equal(hookTable.sessionStart.length, 2, 'user + eCL entry must coexist');
    const userStays = hookTable.sessionStart.some((e) => e.command === '/my-team/hook.sh');
    const gsdAdded  = hookTable.sessionStart.some((e) => e[ECL_CURSOR_HOOK_MARKER]);
    assert.ok(userStays, 'user-owned entry must be preserved');
    assert.ok(gsdAdded,  'eCL managed entry must be present');
  });
  ...
  // ---------------------------------------------------------------------------
  test('T5: remove-only (null managedEntries) strips eCL entries, preserves user', (t) => {
    const dir = createTempDir();
    t.after(() => cleanup(dir));
    ...
    fs.writeFileSync(hooksJsonPath, makeHooksJson({
      ...
      hooks: {
        sessionStart: [
          ...
          managedEntry('/node /ecl-cursor-session-start.js'),
        ],
        postToolUse: [
          managedEntry('/node /ecl-cursor-post-tool.js'),
        ],
      },
    }));
    ...
    assert.equal(hookTable.postToolUse, undefined, 'postToolUse key must be removed (was eCL-only)');
  });
...
describe('isManagedCursorHookEntry / buildCursorHookEntry', () => {
  test('T9: isManagedCursorHookEntry returns true for eCL-marked entry', () => {
    const entry = { type: 'command', command: '/x', [ECL_CURSOR_HOOK_MARKER]: true };
  ...
  test('T11: buildCursorHookEntry emits correct shape', () => {
    ...
    assert.equal(entry[ECL_CURSOR_HOOK_MARKER], true);
...
describe('removeCursorHooksJson', () => {
  test('T12: removes hooks.json when it becomes empty after eCL removal', (t) => {
    const dir = createTempDir();
    t.after(() => cleanup(dir));
    ...
    assert.equal(result.changed, true);
    // File should be removed (was eCL-only).
...
test('T16: Cursor hook script files exist in hooks/', () => {
  ...
  const sessionStart = path.join(hooksDir, ECL_CURSOR_SESSION_HOOK_SCRIPT);
  const postTool     = path.join(hooksDir, ECL_CURSOR_POST_TOOL_HOOK_SCRIPT);
  assert.ok(fs.existsSync(sessionStart), `${ECL_CURSOR_SESSION_HOOK_SCRIPT} must exist in hooks/`);
  assert.ok(fs.existsSync(postTool),     `${ECL_CURSOR_POST_TOOL_HOOK_SCRIPT} must exist in hooks/`);
});

// ---------------------------------------------------------------------------
// T17: sessionStart script emits JSON with additional_context on stdin close
// ---------------------------------------------------------------------------
test('T17: ecl-cursor-session-start.js emits JSON with additional_context', (t, done) => {
  const hooksDir = path.join(__dirname, '..', 'hooks');
  const scriptPath = path.join(hooksDir, ECL_CURSOR_SESSION_HOOK_SCRIPT);
...
// ---------------------------------------------------------------------------
test('T18: ecl-cursor-post-tool.js emits {} for non-write tool names', (t, done) => {
  const hooksDir = path.join(__dirname, '..', 'hooks');
  const scriptPath = path.join(hooksDir, ECL_CURSOR_POST_TOOL_HOOK_SCRIPT);
  const { execFile } = require('child_process');
...
// ---------------------------------------------------------------------------
// T19: ECL_CURSOR_HOOK_MARKER is exported and is a non-empty string
// ---------------------------------------------------------------------------
test('T19: ECL_CURSOR_HOOK_MARKER is exported and is a non-empty string', () => {
  assert.equal(typeof ECL_CURSOR_HOOK_MARKER, 'string');
  assert.ok(ECL_CURSOR_HOOK_MARKER.length > 0);
});

// ---------------------------------------------------------------------------
// T20: Script name constants are exported and correct
// ---------------------------------------------------------------------------
test('T20: ECL_CURSOR_SESSION_HOOK_SCRIPT and ECL_CURSOR_POST_TOOL_HOOK_SCRIPT are exported', () => {
  assert.equal(ECL_CURSOR_SESSION_HOOK_SCRIPT, 'ecl-cursor-session-start.js');
  assert.equal(ECL_CURSOR_POST_TOOL_HOOK_SCRIPT, 'ecl-cursor-post-tool.js');
});
