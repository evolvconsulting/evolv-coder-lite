...
/**
 * #2529 — /ecl-settings-integrations: configure third-party search and review integrations.
 *
 * Covers:
 ...
 *   - /ecl:settings confirmation output mentions /ecl:settings-integrations
...
const {
  VALID_CONFIG_KEYS,
  isValidConfigKey,
} = require('../evolv-coder-lite/bin/lib/config-schema.cjs');

const REPO_ROOT = path.join(__dirname, '..');
// #2790: settings-integrations.md was consolidated into config.md as the --integrations flag.
const COMMAND_PATH = path.join(REPO_ROOT, 'commands', 'ecl', 'config.md');
const WORKFLOW_PATH = path.join(REPO_ROOT, 'evolv-coder-lite', 'workflows', 'settings-integrations.md');
...
describe('#2529 artifacts', () => {
  ...
  test('config.md frontmatter declares name ecl:config and routes to --integrations', () => {
    const src = fs.readFileSync(COMMAND_PATH, 'utf-8');
    // #2790: consolidated command uses ecl:config name
    assert.match(src, /name:\s*ecl:config/);
    assert.match(src, /description:\s*.+/);
    assert.match(src, /allowed-tools:/);
    assert.match(src, /AskUserQuestion/);
  });

  test('workflow exists at evolv-coder-lite/workflows/settings-integrations.md', () => {
...
describe('#2529 workflow — agent_skills injection', () => {
  ...
  test('agent_skills.<valid-slug> passes validator', () => {
    assert.ok(isValidConfigKey('agent_skills.ecl-executor'));
    assert.ok(isValidConfigKey('agent_skills.ecl-planner'));
...
describe('#2529 config-set round-trip', () => {
  ...
  test('config-set round-trips agent_skills.<agent-type>', (t) => {
    ...
    const r = runGsdTools(
      ['config-set', 'agent_skills.ecl-executor', 'skill-a,skill-b'],
      tmp
    );
    assert.ok(r.success, `agent_skills.ecl-executor set failed: ${r.error}`);
    const cfg = JSON.parse(fs.readFileSync(path.join(tmp, '.planning', 'config.json'), 'utf-8'));
    // Accept either array or string — validator accepts both shapes today.
    const v = cfg.agent_skills?.['ecl-executor'];
    assert.ok(v === 'skill-a,skill-b' || (Array.isArray(v) && v.join(',') === 'skill-a,skill-b'),
      `expected agent_skills.ecl-executor to contain both skills, got ${JSON.stringify(v)}`);
  });
});
...
describe('#2529 config merge safety', () => {
  ...
  test('setting agent_skills.ecl-executor preserves unrelated review.models.codex', (t) => {
    const tmp = createTempProject();
    t.after(() => cleanup(tmp));
    runGsdTools(['config-ensure-section'], tmp);
    runGsdTools(['config-set', 'review.models.codex', 'codex exec'], tmp);

    const r = runGsdTools(['config-set', 'agent_skills.ecl-planner', 'a,b'], tmp);
    assert.ok(r.success, `set failed: ${r.error}`);

    const cfg = JSON.parse(fs.readFileSync(path.join(tmp, '.planning', 'config.json'), 'utf-8'));
    assert.strictEqual(cfg.review?.models?.codex, 'codex exec', 'unrelated review.models.codex must be preserved');
    assert.ok(cfg.agent_skills?.['ecl-planner'], 'agent_skills.ecl-planner must be set');
  });
});

// ─── /ecl-settings mentions /ecl-settings-integrations ──────────────────────

describe('#2529 /ecl-settings mentions new command', () => {
  test('settings workflow mentions canonical /ecl-config --integrations', () => {
    const src = fs.readFileSync(SETTINGS_WORKFLOW_PATH, 'utf-8');
    assert.ok(
      src.includes('/ecl:config --integrations'),
      'settings.md must mention /ecl:config --integrations'
    );
    assert.ok(
      !src.includes('/ecl-settings-integrations'),
      'settings.md must not mention the legacy /ecl-settings-integrations variant'
    );
  });
});
