...
describe('plan-phase workflow: post-planning-gaps gap-analysis uses ecl_run launcher (#621)', () => {
  test('gap-analysis dispatches via ecl_run loop render-hooks plan:post (ADR-857 capability gate)', () => {
    assert.ok(
      workflow.includes('ecl_run loop render-hooks plan:post'),
      'workflow must dispatch gap-analysis via ecl_run loop render-hooks plan:post, not a hardcoded node path or direct ecl_run gap-analysis call'
    );
  });
  ...
  test('post-planning-gaps block still gates on workflow.post_planning_gaps and preserves required args', () => {
    ...
    const hasPhaseDir = workflow.includes('ecl_run check ${hook.check.query} "${PHASE_DIR}" "${PHASE_REQ_IDS}"');
    ...
    assert.ok(
      hasPhaseDir,
      'gap-analysis check dispatch must pass "${PHASE_DIR}" (and "${PHASE_REQ_IDS}") positionally to ecl_run check'
    );
    assert.ok(
      hasPickArg,
