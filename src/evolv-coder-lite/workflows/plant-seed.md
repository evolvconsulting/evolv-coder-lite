...
Seeds auto-surface during /ecl:new-milestone when trigger conditions match the
...
status: dormant
planted: {ISO date}
planted_during: {current milestone/phase from STATE.md, or "unknown" if not in a eCL project}
...
_To be filled in. Run `/ecl:capture --seed --enrich SEED-{PADDED}` to add context._

## When to Surface

**Trigger:** when relevant

This seed will surface during `/ecl:new-milestone` when the milestone scope matches.

## Scope Estimate

**Unknown** — run `/ecl:capture --seed --enrich SEED-{PADDED}` to estimate effort.
...
$ECL_SDK query commit "docs: plant seed — {$IDEA}" --files .planning/seeds/SEED-{PADDED}-{slug}.md
```
</step>
...
Trigger and scope are set to defaults. Run `/ecl:capture --seed --enrich SEED-{PADDED}`
to add trigger conditions, rationale, and scope estimate at your convenience.

This seed will surface automatically when you run /ecl:new-milestone.
```
</step>
...
Commit the update:
```bash
$ECL_SDK query commit "docs: enrich seed ${SEED_ID} — trigger + why + scope" --files "$SEED_FILE"
```
