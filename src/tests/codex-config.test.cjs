...
/**
 * eCL Tools Tests - codex-config.cjs
 *
...
function assertNoCodexBareGsdToolsInvocation(content, label) {
  ...
  for (const pattern of patterns) {
    assert.doesNotMatch(
      content,
      pattern,
      `${label} must not contain a command-position bare ecl-tools invocation`,
    );
  }
}
...
describe('getCodexSkillAdapterHeader', () => {
  test('contains all three sections', () => {
    const result = getCodexSkillAdapterHeader('ecl-execute-phase');
  ...
  test('includes correct invocation syntax', () => {
    const result = getCodexSkillAdapterHeader('ecl-plan-phase');
    assert.ok(result.includes('`$ecl-plan-phase`'), 'has $skillName invocation');
    assert.ok(result.includes('{{ECL_ARGS}}'), 'has ECL_ARGS variable');
  });

  test('section B maps AskUserQuestion parameters', () => {
    const result = getCodexSkillAdapterHeader('ecl-discuss-phase');
  ...
  test('section C maps Task to spawn_agent', () => {
    ...
    assert.match(
      result,
      /Resolved `reasoning_effort="low\|medium\|high\|xhigh"` \(`xhigh` is a eCL\/Codex tier, not a generic runtime enum\) → pass `reasoning_effort`\s+to `spawn_agent` when the runtime\/tool supports it/,
...
You are a eCL plan executor.
    ...
    assert.ok(result.includes('"ecl-executor"'), 'has quoted name');
    assert.ok(result.includes('"Executes eCL plans with atomic commits"'), 'has quoted description');
    ...
    assert.ok(result.includes('role: ecl-executor'), 'role matches agent name');
    assert.ok(result.includes('tools: Read, Write, Edit, Bash, Grep, Glob'), 'tools in role block');
    assert.ok(result.includes('purpose: Executes eCL plans with atomic commits'), 'purpose from description');
...
Run /ecl:execute-phase to proceed.`;

    const result = convertClaudeAgentToCodexAgent(input);
    assert.ok(result.includes('$ecl-execute-phase'), 'converts slash commands');
    assert.ok(!result.includes('/ecl:execute-phase'), 'original slash command removed');
  });
...
INIT=$(node "$HOME/.claude/evolv-coder-lite/bin/ecl-tools.cjs" state load)
node "$HOME/.claude/evolv-coder-lite/bin/ecl-tools.cjs" commit "docs: resolve"`;

    const result = convertClaudeAgentToCodexAgent(input);
    assert.ok(result.includes('$HOME/.codex/evolv-coder-lite/bin/ecl-tools.cjs'), 'replaces $HOME/.claude/ with $HOME/.codex/');
    assert.ok(!result.includes('$HOME/.claude/'), 'no .claude paths remain');
  });

  test('rewrites bare ecl-tools invocations to the Codex shim path', () => {
    const input = `---
name: ecl-planner
...
INIT=$(ecl-tools query init.plan-phase "\${PHASE}")
ecl-tools query state.load 2>/dev/null
if command -v ecl-tools >/dev/null 2>&1; then echo "path fallback"; fi
Use \`ecl-tools query history-digest\` for history.`;

    const result = convertClaudeAgentToCodexAgent(input);
    assert.ok(
      result.includes('INIT=$(node "$HOME/.codex/evolv-coder-lite/bin/ecl-tools.cjs" query init.plan-phase'),
      'rewrites command substitution',
    );
    assert.ok(
      result.includes('node "$HOME/.codex/evolv-coder-lite/bin/ecl-tools.cjs" query state.load'),
      'rewrites line-start command',
    );
    assert.ok(
      result.includes('`node "$HOME/.codex/evolv-coder-lite/bin/ecl-tools.cjs" query history-digest`'),
      'rewrites inline command example',
    );
    assert.ok(result.includes('command -v ecl-tools'), 'keeps PATH resolver probe intact');
...
describe('Codex hyphen-style command prefix conversion', () => {
  test('converts /ecl-command in workflow output to $ecl-command', () => {
    const input = `---
name: ecl-test
description: Test
tools: Read
...
/ecl-discuss-phase 1 — gather context
/ecl-plan-phase 2 — create plan
/ecl-execute-phase 3 — run it`;

    const result = convertClaudeCommandToCodexSkill(input, 'ecl-test');
    assert.ok(result.includes('$ecl-discuss-phase'), 'converts /ecl-discuss-phase');
    assert.ok(result.includes('$ecl-plan-phase'), 'converts /ecl-plan-phase');
    assert.ok(result.includes('$ecl-execute-phase'), 'converts /ecl-execute-phase');
    assert.ok(!result.includes('/ecl-discuss-phase'), 'no /ecl-discuss-phase remains');
  });

  test('converts backtick-wrapped /ecl- commands', () => {
    const input = `---
name: ecl-test
description: Test
tools: Read
...
Run \`/ecl-plan-phase 1\` to plan.`;

    const result = convertClaudeCommandToCodexSkill(input, 'ecl-test');
    assert.ok(result.includes('$ecl-plan-phase'), 'converts backtick-wrapped command');
  });

  test('does not convert /ecl- in file paths', () => {
    const input = `---
name: ecl-test
description: Test
tools: Read
...
node "$HOME/.claude/evolv-coder-lite/bin/ecl-tools.cjs" init`;

    const result = convertClaudeCommandToCodexSkill(input, 'ecl-test');
    assert.ok(result.includes('ecl-tools.cjs'), 'ecl-tools.cjs preserved in path');
    assert.ok(!result.includes('$ecl-tools'), 'no $ecl-tools in file path');
  });

  test('rewrites bare ecl-tools commands in generated Codex skills', () => {
    const input = `---
name: ecl:quick
...
\`\`\`bash
ecl-tools query frontmatter.get .planning/quick/example/SUMMARY.md status
INIT=$(ecl-tools query init.quick)
if command -v ecl-tools >/dev/null 2>&1; then echo ok; fi
\`\`\`

Status fields read via \`ecl-tools query frontmatter.get\`.`;

    const result = convertClaudeCommandToCodexSkill(input, 'ecl-quick');
    assert.ok(
      result.includes('node "$HOME/.codex/evolv-coder-lite/bin/ecl-tools.cjs" query frontmatter.get'),
      'rewrites line-start command in a shell block',
    );
    assert.ok(
      result.includes('INIT=$(node "$HOME/.codex/evolv-coder-lite/bin/ecl-tools.cjs" query init.quick)'),
    ...
    assert.ok(result.includes('command -v ecl-tools'), 'keeps resolver probe intact');
...
\`$ecl-plan-phase 1\``;
    ...
    assert.ok(result.includes('$ecl-plan-phase'), 'command preserved after /clear removal');
  });
...
/ecl-execute-phase 2`;
    ...
    const result = convertClaudeCommandToCodexSkill(input, 'ecl-test');
    assert.ok(!result.includes('/clear'), 'no /clear remains');
    assert.ok(result.includes('$ecl-execute-phase'), 'command converted');
  });
});
...
<role>You check plans.</role>`;
  ...
  test('includes required name and description fields', () => {
    const result = generateCodexAgentToml('ecl-executor', sampleAgent);
    assert.ok(result.includes('name = "ecl-executor"'), 'has name');
  ...
  test('falls back to generated description when frontmatter is missing fields', () => {
    ...
    const result = generateCodexAgentToml('ecl-unknown', minimalAgent);
    assert.ok(result.includes('name = "ecl-unknown"'), 'falls back to agent name');
    assert.ok(result.includes('description = "eCL agent ecl-unknown"'), 'falls back to synthetic description');
  });

  test('defaults unknown agents to read-only', () => {
    const result = generateCodexAgentToml('ecl-unknown', sampleAgent);
  ...
  test('emits model field when modelOverrides contains an entry for the agent (#2256)', () => {
    const overrides = { 'ecl-executor': 'gpt-5.3-codex' };
    const result = generateCodexAgentToml('ecl-executor', sampleAgent, overrides);
  ...
  test('does not emit model field when modelOverrides is null (#2256)', () => {
    const result = generateCodexAgentToml('ecl-executor', sampleAgent, null);
  ...
  test('emits reasoning effort when model override pins Codex model (#838)', () => {
    ...
    assert.ok(
      result.includes('model_reasoning_effort ='),
      'reasoning effort is safe to emit when eCL also pins model'
    );
  });

  test('emits reasoning effort when runtime resolver pins Codex model (#838)', () => {
    const runtimeResolver = { resolve: () => ({ model: 'gpt-5.5' }) };
    const result = generateCodexAgentToml('ecl-executor', sampleAgent, null, runtimeResolver);
  ...
  test('does not emit model field when modelOverrides has no entry for this agent (#2256)', () => {
    const overrides = { 'ecl-planner': 'gpt-5.4' };
    const result = generateCodexAgentToml('ecl-executor', sampleAgent, overrides);
  ...
  test('emits service_tier="flex" and model_verbosity="low" for light-tier agents (#774)', () => {
    // ecl-plan-checker has routingTier:"light" in model-catalog.json
    const lightAgent = `---
name: ecl-plan-checker
...
<role>You check plans.</role>`;
    const result = generateCodexAgentToml('ecl-plan-checker', lightAgent);
  ...
  test('does not emit service_tier or model_verbosity for standard-tier agents (#774)', () => {
    // ecl-executor has routingTier:"standard" in model-catalog.json
    const result = generateCodexAgentToml('ecl-executor', sampleAgent);
  ...
  test('does not emit service_tier or model_verbosity for heavy-tier agents (#774)', () => {
    // ecl-planner has routingTier:"heavy" in model-catalog.json
    const heavyAgent = `---
name: ecl-planner
...
<role>You plan.</role>`;
    const result = generateCodexAgentToml('ecl-planner', heavyAgent);
...
<role>You check plans.</role>`;
  ...
  test('emitted TOML is parseable and contains correct field values for light-tier agents (#774)', () => {
    const lightAgent = `---
name: ecl-codebase-mapper
description: Maps the codebase
---

<role>You map the codebase.</role>`;
    const toml = generateCodexAgentToml('ecl-codebase-mapper', lightAgent);
...
<role>You are an executor.</role>`;
  ...
  test('sandboxTier=none: does NOT emit sandbox_mode', () => {
    const result = generateCodexAgentToml('ecl-executor', sampleAgent, null, null, null, 'none');
  ...
  test('sandboxTier=codex-agent-sandbox: emits sandbox_mode = "workspace-write"', () => {
    const result = generateCodexAgentToml('ecl-executor', sampleAgent, null, null, null, 'codex-agent-sandbox');
...
describe('installCodexConfig sandboxTier threading seam', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-sandboxtier-seam-'));
    agentsSrc = path.join(tmpDir, 'agents');
    targetDir = path.join(tmpDir, 'codex');
    fs.mkdirSync(agentsSrc, { recursive: true });
    fs.mkdirSync(targetDir, { recursive: true });
    // Write a minimal ecl-executor agent fixture
    fs.writeFileSync(path.join(agentsSrc, 'ecl-executor.md'), [
      '---',
      'name: ecl-executor',
      'description: Executes plans',
      'tools: Read, Write, Edit',
  ...
  test('sandboxTier=none: written per-agent .toml does NOT contain sandbox_mode', () => {
    ...
    const tomlPath = path.join(targetDir, 'agents', 'ecl-executor.toml');
...
describe('CODEX_AGENT_SANDBOX', () => {
  ...
  test('workspace-write agents have write tools', () => {
    const writeAgents = [
      'ecl-executor', 'ecl-planner', 'ecl-phase-researcher',
      'ecl-project-researcher', 'ecl-research-synthesizer', 'ecl-verifier',
      'ecl-codebase-mapper', 'ecl-roadmapper', 'ecl-debugger',
    ];
  ...
  test('read-only agents have no write tools', () => {
    const readOnlyAgents = ['ecl-plan-checker', 'ecl-integration-checker'];
...
describe('generateCodexConfigBlock', () => {
  const agents = [
    { name: 'ecl-executor', description: 'Executes plans' },
    { name: 'ecl-planner', description: 'Creates plans' },
  ];

  test('starts with eCL marker', () => {
    const result = generateCodexConfigBlock(agents);
    assert.ok(result.startsWith(ECL_CODEX_MARKER), 'starts with marker');
  });
  ...
  test('#2727: emits [agents.<name>] struct format (Codex 0.120.0+, replaces #2645 [[agents]])', () => {
    ...
    assert.ok(result.includes('[agents.ecl-executor]'), 'executor has struct header');
    assert.ok(result.includes('[agents.ecl-planner]'), 'planner has struct header');
    // Struct format uses the key as the name; no name = field.
    assert.ok(!result.includes('name = "ecl-executor"'), 'no name field in struct format');
    assert.ok(!result.includes('name = "ecl-planner"'), 'no name field in struct format');
  ...
  test('includes per-agent sections with relative paths (no targetDir)', () => {
    const result = generateCodexConfigBlock(agents);
    assert.ok(result.includes('[agents.ecl-executor]'), 'has executor entry');
    assert.ok(result.includes('[agents.ecl-planner]'), 'has planner entry');
    assert.ok(result.includes('config_file = "agents/ecl-executor.toml"'), 'relative config_file without targetDir');
  ...
  test('uses absolute config_file paths when targetDir is provided', () => {
    ...
    assert.ok(result.includes('config_file = "/home/user/.codex/agents/ecl-executor.toml"'), 'absolute executor path');
    assert.ok(result.includes('config_file = "/home/user/.codex/agents/ecl-planner.toml"'), 'absolute planner path');
  ...
  test('#2727: emits [agents.<name>] struct format by default (Codex 0.124.0+)', () => {
    ...
    assert.ok(result.includes('[agents.ecl-executor]'), 'executor uses struct header');
    assert.ok(result.includes('[agents.ecl-planner]'), 'planner uses struct header');
...
describe('stripGsdFromCodexConfig', () => {
  test('returns null for eCL-only config', () => {
    const content = `${ECL_CODEX_MARKER}\n[features]\nmulti_agent = true\n`;
    const result = stripGsdFromCodexConfig(content);
    assert.strictEqual(result, null, 'returns null when eCL-only');
  });
  ...
  test('preserves user content before marker', () => {
    ...
    assert.ok(!result.includes('multi_agent'), 'removes eCL content');
    assert.ok(!result.includes(ECL_CODEX_MARKER), 'removes marker');
  });
  ...
  test('strips injected keys above marker on uninstall', () => {
    ...
    assert.ok(!result.includes(ECL_CODEX_MARKER), 'strips marker');
  });

  test('removes legacy [agents.ecl-*] map sections (self-heal pre-#2645 configs)', () => {
    const content = `[agents.ecl-executor]\ndescription = "test"\nconfig_file = "agents/ecl-executor.toml"\n\n[agents.custom-agent]\ndescription = "user agent"\n`;
    const result = stripGsdFromCodexConfig(content);
    assert.ok(!result.includes('[agents.ecl-executor]'), 'removes legacy eCL agent map section');
    assert.ok(result.includes('[agents.custom-agent]'), 'preserves user agent section');
  });

  test('#2645: removes [[agents]] array-of-tables entries whose name is ecl-*', () => {
    const content = `[[agents]]\nname = "ecl-executor"\ndescription = "test"\nconfig_file = "agents/ecl-executor.toml"\n\n[[agents]]\nname = "custom-agent"\ndescription = "user agent"\n`;
    const result = stripGsdFromCodexConfig(content);
    assert.ok(!/name = "ecl-executor"/.test(result), 'removes managed eCL [[agents]] entry');
    assert.ok(result.includes('name = "custom-agent"'), 'preserves user [[agents]] entry');
  });

  test('#2645: handles mixed legacy + new shapes and multiple user/ecl entries in one file', () => {
    // Multiple eCL entries (both legacy map and new array-of-tables) interleaved
    // with multiple user-authored agents in both shapes — none of the user
    // entries may be removed and all eCL entries must be stripped.
    const content = [
      '[agents.ecl-executor]',
      'description = "legacy ecl"',
      'config_file = "agents/ecl-executor.toml"',
      '',
    ...
    const result = stripGsdFromCodexConfig(content);
    // All eCL entries removed.
    assert.ok(!result.includes('ecl-executor'), 'removes legacy ecl-executor');
    assert.ok(!/name\s*=\s*"ecl-planner"/.test(result), 'removes new ecl-planner');
    assert.ok(!/name\s*=\s*'ecl-debugger'/.test(result), 'removes single-quoted ecl-debugger');
...
describe('migrateCodexHooksMapFormat', () => {
  test('migrates flat [[hooks]] with event key to namespaced [[hooks.<EVENT>]] form', () => {
    ...
    const content = [
      ...
      '[[hooks]]',
      'event = "SessionStart"',
      'command = "node /home/.codex/hooks/ecl-check-update.js"',
      '',
    ].join('\n');
    ...
    assert.strictEqual(parsed.hooks.SessionStart[0].hooks[0].command,
      'node /home/.codex/hooks/ecl-check-update.js');
  ...
  test('converts [hooks.shell] to namespaced AoT [[hooks.shell]] (#2760 CR5 finding 3)', () => {
    ...
    assert.strictEqual(parsed.hooks.shell[0].hooks[0].command, 'node /home/.codex/hooks/ecl-check-update.js');
  ...
  test('end-to-end: install on config with old [hooks] map format produces namespaced AoT (#2637, #2760 CR5)', () => {
    // Simulates the exact old eCL config.toml format that broke on Codex 0.124.0
    const oldContent = [
      '[features]',
      'codex_hooks = true',
      '',
...
describe('Codex hooks emit: migration produces namespaced AoT so managed-emit converges', () => {
  ...
  // install path (writes "# eCL Hooks") detects existing namespaced AoT via
  ...
  let tmpDir;
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-codex-fieldparity-'));
  });
  afterEach(() => {
    cleanup(tmpDir);
  });
...
describe('mergeCodexConfig', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-codex-merge-'));
  });
  ...
  test('case 1: creates new config.toml', () => {
    ...
    assert.ok(content.includes(ECL_CODEX_MARKER), 'has marker');
    assert.ok(content.includes('[agents.ecl-executor]'), 'has agent in struct format');
    assert.ok(!content.includes('[features]'), 'no features section');
    assert.ok(!content.includes('multi_agent'), 'no multi_agent');
  });

  test('case 2: replaces existing eCL block', () => {
    const configPath = path.join(tmpDir, 'config.toml');
    ...
    const newBlock = generateCodexConfigBlock([
      { name: 'ecl-executor', description: 'Updated description' },
      { name: 'ecl-planner', description: 'New agent' },
    ]);
    ...
    assert.ok(content.includes('[agents.ecl-planner]'), 'has new agent in struct format');
    // Verify no duplicate markers
    const markerCount = (content.match(new RegExp(ECL_CODEX_MARKER.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g')) || []).length;
    assert.strictEqual(markerCount, 1, 'exactly one marker');
  });

  test('case 3: appends to config without eCL marker', () => {
    const configPath = path.join(tmpDir, 'config.toml');
    ...
    const content = fs.readFileSync(configPath, 'utf8');
    assert.ok(content.includes('[model]'), 'preserves user content');
    assert.ok(content.includes(ECL_CODEX_MARKER), 'adds marker');
    assert.ok(content.includes('[agents.ecl-executor]'), 'has agent in struct format');
  });

  test('case 3 with existing [features]: preserves user features, does not inject eCL keys', () => {
    const configPath = path.join(tmpDir, 'config.toml');
    ...
    assert.ok(content.includes(ECL_CODEX_MARKER), 'adds marker for agents block');
    assert.ok(content.includes('[agents.ecl-executor]'), 'has agent in struct format');
  });

  test('case 3 strips existing [agents.ecl-*] sections before appending fresh block', () => {
    const configPath = path.join(tmpDir, 'config.toml');
    const existing = [
      '[model]',
      'name = "o3"',
    ...
    const content = fs.readFileSync(configPath, 'utf8');
    // After merge, eCL block is after the marker. Count [agents.ecl-executor] headers:
    // exactly one should exist (the one in the freshly-written eCL block).
    const gsdStructCount = (content.match(/^\[agents\.ecl-executor\]\s*$/gm) || []).length;
    const markerCount = (content.match(new RegExp(ECL_CODEX_MARKER.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g')) || []).length;
    // Struct format does not use name = field
    assert.ok(!content.match(/^name = "ecl-executor"/m), 'no name = field in struct format');

    assert.ok(content.includes('[model]'), 'preserves user content');
    assert.ok(content.includes('[agents.custom-agent]'), 'preserves non-eCL agent section');
    assert.strictEqual(gsdStructCount, 1, 'keeps exactly one [agents.ecl-executor] struct entry');
    assert.strictEqual(markerCount, 1, 'adds exactly one marker block');
    assert.ok(!/\n{3,}# eCL Agent Configuration/.test(content), 'does not leave extra blank lines before marker block');
  });
  ...
  test('case 2 does not inject feature keys', () => {
    ...
    assert.ok(content.includes('[agents.ecl-executor]'), 'has agent from fresh block in struct format');
  });

  test('case 2 strips leaked [agents] and [agents.ecl-*] from before content', () => {
    const configPath = path.join(tmpDir, 'config.toml');
    const brokenContent = [
      '[features]',
    ...
    assert.ok(!beforeMarker.includes('[agents.ecl-'), 'no leaked [agents.ecl-*] above marker');
  });

  test('case 2 strips leaked eCL-managed sections above marker in CRLF files', () => {
    const configPath = path.join(tmpDir, 'config.toml');
    const brokenContent = [
      '[features]',
      'child_agents_md = false',
    ...
    assert.strictEqual(countMatches(beforeMarker, /^\[agents\.ecl-executor\]\s*$/gm), 0, 'removes leaked eCL agent section above marker');
    // New struct format: exactly one [agents.ecl-executor] header in the eCL block (after marker)
    assert.strictEqual(countMatches(content, /^\[agents\.ecl-executor\]\s*$/gm), 1, 'exactly one struct agent header in eCL block');
    assert.strictEqual(countMatches(content, /name = "ecl-executor"/g), 0, 'no name = field in struct format');
    assertUsesOnlyEol(content, '\r\n');
  });

  test('case 2 strips bare [agents] tables (invalid in current Codex schema, #2760) and removes leaked eCL sections in CRLF files', () => {
    const configPath = path.join(tmpDir, 'config.toml');
    const brokenContent = [
      '[features]',
      'child_agents_md = false',
    ...
    // New struct format: exactly one [agents.ecl-executor] in the eCL block (after marker)
    assert.strictEqual(countMatches(content, /^\[agents\.ecl-executor\]\s*$/gm), 1, 'exactly one struct agent header in eCL block');
    assert.strictEqual(countMatches(content, /name = "ecl-executor"/g), 0, 'no name = field in struct format');
    assertUsesOnlyEol(content, '\r\n');
  });
  ...
  test('preserves CRLF when appending eCL block to existing config', () => {
    const configPath = path.join(tmpDir, 'config.toml');
    fs.writeFileSync(configPath, '[model]\r\nname = "o3"\r\n', 'utf8');
    ...
    assert.ok(content.includes(`${ECL_CODEX_MARKER}\r\n`), 'writes marker with CRLF');
    assertUsesOnlyEol(content, '\r\n');
  });

  test('uses the first newline style when appending eCL block to mixed-EOL configs', () => {
    const configPath = path.join(tmpDir, 'config.toml');
...
describe('installCodexConfig (integration)', () => {
  ...
  beforeEach(() => {
    tmpTarget = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-codex-install-'));
  });
  ...
  (hasAgents ? test : test.skip)('generates config.toml and agent .toml files', () => {
    ...
    assert.ok(config.includes(ECL_CODEX_MARKER), 'has eCL marker');
    assert.ok(config.includes('[agents.ecl-executor]'), 'has executor agent in struct format');
    ...
    assert.ok(fs.existsSync(path.join(agentsDir, 'ecl-executor.toml')), 'executor .toml exists');
    assert.ok(fs.existsSync(path.join(agentsDir, 'ecl-plan-checker.toml')), 'plan-checker .toml exists');

    const executorToml = fs.readFileSync(path.join(agentsDir, 'ecl-executor.toml'), 'utf8');
    assert.ok(executorToml.includes('name = "ecl-executor"'), 'executor has name');
    assert.ok(executorToml.includes('description = "Executes eCL plans with atomic commits, deviation handling, checkpoint protocols, and state management. Spawned by execute-phase orchestrator or execute-plan command."'), 'executor has description');
    ...
    const checkerToml = fs.readFileSync(path.join(agentsDir, 'ecl-plan-checker.toml'), 'utf8');
    assert.ok(checkerToml.includes('name = "ecl-plan-checker"'), 'plan-checker has name');
  ...
  (hasAgents ? test : test.skip)('generated Codex agent .toml files do not call bare ecl-tools', () => {
    const { installCodexConfig } = require('../bin/install.js');
    installCodexConfig(tmpTarget, agentsSrc);

    const agentsDir = path.join(tmpTarget, 'agents');
    const tomlFiles = fs.readdirSync(agentsDir)
      .filter((file) => file.startsWith('ecl-') && file.endsWith('.toml'));
...
describe('Codex install hook configuration (e2e)', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-codex-e2e-'));
  ...
  test('Codex install copies hook file that is referenced in hooks.json (#2153)', () => {
    // Regression test: Codex install writes ecl-check-update hook reference into
    ...
    assert.equal(
      hooksJsonCommands.some((cmd) => cmd.includes('ecl-check-update')),
      true,
      'hooks.json references ecl-check-update (.js on POSIX, .cmd on Windows)'
    );
    // The hook file must physically exist at the referenced path
    const hookFile = path.join(codexHome, 'hooks', 'ecl-check-update.js');
    assert.ok(
      fs.existsSync(hookFile),
      `ecl-check-update.js must exist at ${hookFile} — hooks.json references it (directly on POSIX, via .cmd shim on Windows) but file was not installed`
    );
  });
  ...
  test('fresh CODEX_HOME enables codex_hooks without draft root defaults', () => {
    ...
    const hooksJsonCommands = readHooksSessionStartCommands(codexHome);
    const gsdCommands = hooksJsonCommands.filter((cmd) => cmd.includes('ecl-check-update'));
    assert.strictEqual(gsdCommands.length, 1, 'writes one eCL update hook in hooks.json');
    if (process.platform === 'win32') {
      // On Windows, the command is the .cmd shim path (quoted).
      const expectedCmdPath = path.join(codexHome, 'hooks', 'ecl-check-update.cmd').replace(/\\/g, '/');
      ...
      const expectedHookPath = path.join(codexHome, 'hooks', 'ecl-check-update.js').replace(/\\/g, '/');
      const expectedCommand = `"${expectedRunner}" "${expectedHookPath}"`;
      assert.strictEqual(gsdCommands[0], expectedCommand, 'handler command must use absolute node runner pointing at ecl-check-update.js (#3017)');
    }
  ...
  test('existing LF config without [features] gets one features block and preserves user content', () => {
    ...
    const hooksJsonCommands = readHooksSessionStartCommands(codexHome);
    const gsdEntries = hooksJsonCommands.filter((cmd) => cmd.includes('ecl-check-update'));
    assert.strictEqual(gsdEntries.length, 1, 'adds one eCL update hook in hooks.json');
    assertNoDraftRootKeys(content);
  });
  ...
  test('bare top-level keys are NOT trapped under [features] (#1202)', () => {
    ...
    // followed by [projects] section. eCL must not prepend [features] before
  ...
  test('existing empty [features] and codex_hooks = false are normalized and remain idempotent', () => {
    ...
    const hooksJsonCommands = readHooksSessionStartCommands(codexHome);
    const gsdEntries = hooksJsonCommands.filter((cmd) => cmd.includes('ecl-check-update'));
    assert.strictEqual(gsdEntries.length, 1, 'does not duplicate eCL update hook in hooks.json');
    assertNoDraftRootKeys(content);
  });
  ...
  test('quoted [features] headers are recognized as the existing features table', () => {
    ...
    const hooksJsonCommands = readHooksSessionStartCommands(codexHome);
    const gsdEntries = hooksJsonCommands.filter((cmd) => cmd.includes('ecl-check-update'));
    assert.strictEqual(gsdEntries.length, 1, 'keeps one eCL update hook in hooks.json');
    assertNoDraftRootKeys(content);
  });
  ...
  test('quoted table headers containing # are parsed without treating # as a comment start', () => {
    ...
    const hooksJsonCommands = readHooksSessionStartCommands(codexHome);
    const gsdEntries = hooksJsonCommands.filter((cmd) => cmd.includes('ecl-check-update'));
    assert.strictEqual(gsdEntries.length, 1, 'remains idempotent for the eCL hook block in hooks.json');
    assertNoDraftRootKeys(content);
  });
  ...
  test('existing dotted features config stays dotted and does not grow a [features] table', () => {
    ...
    const hooksJsonCommands = readHooksSessionStartCommands(codexHome);
    const gsdEntries = hooksJsonCommands.filter((cmd) => cmd.includes('ecl-check-update'));
    assert.strictEqual(gsdEntries.length, 1, 'adds one eCL update hook for dotted codex_hooks and remains idempotent');
    assertNoDraftRootKeys(content);
  });
  ...
  test('root inline-table features assignments are left untouched without appending invalid dotted keys or hooks', () => {
    ...
    assert.strictEqual(countMatches(content, /ecl-check-update\.js/g), 0, 'does not add the eCL hook block when codex_hooks cannot be enabled safely');
    assert.ok(content.includes('[agents.ecl-executor]'), 'still installs the managed agent block in struct format');
    assertNoDraftRootKeys(content);
  });
  ...
  test('quoted dotted codex_hooks keys stay dotted and are normalized without duplication', () => {
    ...
    const hooksJsonCommands = readHooksSessionStartCommands(codexHome);
    const gsdEntries = hooksJsonCommands.filter((cmd) => cmd.includes('ecl-check-update'));
    assert.strictEqual(gsdEntries.length, 1, 'adds one eCL update hook for quoted dotted codex_hooks and remains idempotent');
    assertNoDraftRootKeys(content);
  });
  ...
  test('existing CRLF codex_hooks = true stays single and preserves non-eCL hooks', () => {
    writeCodexConfig(codexHome, [
      '[features]',
      'codex_hooks = true',
      'other_feature = true',
    ...
    assert.ok(content.includes('other_feature = true'), 'preserves other feature keys');
    assert.strictEqual(countMatches(content, /echo custom-after-command/g), 1, 'preserves non-eCL hook exactly once');
    const hooksJsonCommands = readHooksSessionStartCommands(codexHome);
    const gsdEntries = hooksJsonCommands.filter((cmd) => cmd.includes('ecl-check-update'));
    assert.strictEqual(gsdEntries.length, 1, 'keeps one eCL update hook in hooks.json');
    assertUsesOnlyEol(content, '\r\n');
  ...
  test('codex_hooks = true with an inline comment is treated as enabled for hook installation', () => {
    ...
    const hooksJsonCommands = readHooksSessionStartCommands(codexHome);
    const gsdEntries = hooksJsonCommands.filter((cmd) => cmd.includes('ecl-check-update'));
    assert.strictEqual(gsdEntries.length, 1, 'adds the eCL update hook once in hooks.json');
    assertNoDraftRootKeys(content);
  });
  ...
  test('mixed-EOL configs use the first newline style for inserted Codex content', () => {
    ...
    assert.ok(content.includes(`# eCL Agent Configuration — managed by evolv-coder-lite installer\n`), 'writes the managed agent block using the first newline style');
...
describe('Codex uninstall symmetry for hook-enabled configs', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-codex-uninstall-'));
    codexHome = path.join(tmpDir, 'codex-home');
  });
  ...
  test('fresh install removes the eCL-added codex_hooks feature on uninstall', () => {
    runCodexInstall(codexHome);

    const cleaned = stripGsdFromCodexConfig(readCodexConfig(codexHome));
    assert.strictEqual(cleaned, null, 'fresh eCL-only config strips back to nothing');
  });
  ...
  test('install then uninstall removes [features].codex_hooks while preserving other feature keys, comments, hooks, and CRLF', () => {
    ...
    assert.strictEqual(countMatches(cleaned, /^codex_hooks = true$/gm), 0, 'removes the eCL-added codex_hooks key');
    ...
    assert.strictEqual(countMatches(cleaned, /echo custom-after-command/g), 1, 'preserves non-eCL hooks');
    assert.strictEqual(countMatches(cleaned, /ecl-check-update\.js/g), 0, 'removes only the eCL update hook');
    assert.strictEqual(countMatches(cleaned, /\[agents\.ecl-/g), 0, 'removes managed eCL agent sections');
  ...
  test('install then uninstall removes dotted features.codex_hooks without creating a [features] table', () => {
    ...
    assert.strictEqual(countMatches(cleaned, /^features\.codex_hooks = true$/gm), 0, 'removes the dotted eCL codex_hooks key');
    assert.strictEqual(countMatches(cleaned, /^\[features\]\s*$/gm), 0, 'does not leave behind a [features] table');
    assert.strictEqual(countMatches(cleaned, /echo custom-after-command/g), 1, 'preserves non-eCL hooks');
    assert.strictEqual(countMatches(cleaned, /ecl-check-update\.js/g), 0, 'removes the eCL update hook');
  });
  ...
  test('install then uninstall keeps mixed-EOL user content stable while removing eCL hook state', () => {
...
describe('cleanupCodexSkillMetadataSidecars (#1326)', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-test-sidecar-cleanup-'));
  });
  ...
  test('Codex install does not emit managed agents/openai.yaml sidecars and removes stale ones (#1326)', () => {
    // ecl-foo: managed skill with stale sidecar → sidecar removed, empty agents/ pruned
    const fooAgents = path.join(tmpDir, 'ecl-foo', 'agents');
    fs.mkdirSync(fooAgents, { recursive: true });
    fs.writeFileSync(path.join(tmpDir, 'ecl-foo', 'SKILL.md'), '---\nname: ecl-foo\n---\nBody.\n');
    fs.writeFileSync(path.join(fooAgents, 'openai.yaml'), 'interface:\n  display_name: "foo"\n');

    // ecl-dev-preferences: user-owned → sidecar PRESERVED
    const prefAgents = path.join(tmpDir, 'ecl-dev-preferences', 'agents');
    fs.mkdirSync(prefAgents, { recursive: true });
    fs.writeFileSync(path.join(tmpDir, 'ecl-dev-preferences', 'SKILL.md'), '---\nname: ecl-dev-preferences\n---\nBody.\n');
    ...
    // ecl-bar: managed skill with sidecar + another file in agents/ → sidecar removed, agents/ kept (has other.txt)
    const barAgents = path.join(tmpDir, 'ecl-bar', 'agents');
    fs.mkdirSync(barAgents, { recursive: true });
    fs.writeFileSync(path.join(tmpDir, 'ecl-bar', 'SKILL.md'), '---\nname: ecl-bar\n---\nBody.\n');
    ...
    // helper: non-ecl dir with openai.yaml → UNTOUCHED
    ...
    // ecl-foo: sidecar removed and empty agents/ pruned
    assert.ok(!fs.existsSync(path.join(fooAgents, 'openai.yaml')),
      'ecl-foo/agents/openai.yaml must be removed (managed stale sidecar)');
    assert.ok(!fs.existsSync(fooAgents),
      'ecl-foo/agents/ must be pruned when empty after sidecar removal');

    // ecl-dev-preferences: user-owned, sidecar preserved
    assert.ok(fs.existsSync(path.join(prefAgents, 'openai.yaml')),
      'ecl-dev-preferences/agents/openai.yaml must be preserved (user-owned)');
    assert.strictEqual(fs.readFileSync(path.join(prefAgents, 'openai.yaml'), 'utf8'), userYaml,
      'ecl-dev-preferences/agents/openai.yaml content must be unchanged');

    // ecl-bar: sidecar removed but agents/ kept (still has other.txt)
    assert.ok(!fs.existsSync(path.join(barAgents, 'openai.yaml')),
      'ecl-bar/agents/openai.yaml must be removed');
    assert.ok(fs.existsSync(barAgents),
      'ecl-bar/agents/ must NOT be pruned (still contains other.txt)');
    assert.ok(fs.existsSync(path.join(barAgents, 'other.txt')),
      'ecl-bar/agents/other.txt must be preserved');

    // helper: non-ecl dir untouched
    assert.ok(fs.existsSync(path.join(helperAgents, 'openai.yaml')),
      'helper/agents/openai.yaml must be untouched (non-ecl dir)');
  });
  ...
  test('is a no-op for managed ecl-* dirs with no agents/openai.yaml (#1326)', () => {
    // No sidecar present — should not throw, should not create anything
    const skillDir = path.join(tmpDir, 'ecl-baz');
  ...
  test('does not delete through a symlinked agents/ directory (#1326)', { skip: process.platform === 'win32' }, () => {
    // Setup: a skills dir with ecl-foo/ whose agents/ is a SYMLINK to an external dir.
    // The cleanup must not delete files through the symlink.
    const externalDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-test-symlink-ext-'));
    try {
      ...
      // Create ecl-foo/ in the skills dir and make agents/ a symlink to externalDir.
      const skillDir = path.join(tmpDir, 'ecl-foo');
      fs.mkdirSync(skillDir, { recursive: true });
      ...
      assert.ok(fs.existsSync(agentsLink),
        'ecl-foo/agents symlink must still exist');
    } finally {
      cleanup(externalDir);
    }
  });
  ...
  test('Codex install does not create agents/openai.yaml sidecars for any managed skill (#1326)', () => {
    // Integration test: full Codex install must NOT produce any managed ecl-*/agents/openai.yaml
    ...
    const gsdSkillDirs = fs.readdirSync(skillsDir, { withFileTypes: true })
      .filter(e => e.isDirectory() && e.name.startsWith('ecl-') && e.name !== 'ecl-dev-preferences');
    assert.ok(gsdSkillDirs.length > 0, 'install must create at least one managed ecl-* skill directory');
