...
const REFS_DIR = path.join(__dirname, '..', 'evolv-coder-lite', 'references', 'few-shot-examples');
...
describe('few-shot calibration examples', () => {
  ...
  describe('agent files reference few-shot examples', () => {
    test('ecl-plan-checker.md contains reference to plan-checker few-shot examples', () => {
      const content = readFile(path.join(AGENTS_DIR, 'ecl-plan-checker.md'));
      assert.match(content, /@~\/\.claude\/evolv-coder-lite\/references\/few-shot-examples\/plan-checker\.md/);
    });

    test('ecl-verifier.md contains reference to verifier few-shot examples', () => {
      const content = readFile(path.join(AGENTS_DIR, 'ecl-verifier.md'));
      assert.match(content, /@~\/\.claude\/evolv-coder-lite\/references\/few-shot-examples\/verifier\.md/);
    });
  });
