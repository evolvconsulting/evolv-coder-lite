/**
 * Tests for src/phase-locator.cts (compiled to evolv-coder-lite/bin/lib/phase-locator.cjs).
 *
...
const phaseLocator = require('../evolv-coder-lite/bin/lib/phase-locator.cjs');
...
describe('findPhaseInternal: active phase lookup', () => {
  ...
  test('returns null for falsy phase argument', () => {
    tmpDir = createTempProject('ecl-pl-test-');
  ...
  test('returns null when phases dir does not exist', () => {
    ...
    tmpDir = fs.mkdtempSync(path.join(require('node:os').tmpdir(), 'ecl-pl-test-'));
