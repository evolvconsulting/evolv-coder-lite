/**
 * InitRunner — orchestrates the eCL new-project init workflow.
 *
 ...
 * prompts derived from eCL-1 workflow/agent/template files on disk.
 */
...
import type { GSDTools } from './ecl-tools.js';
...
const ECL_TEMPLATES_DIR = resolveLegacyTemplatesDir();
const ECL_AGENTS_DIR = resolveAgentsDir();
...
export class InitRunner {
  ...
  async run(input: string): Promise<InitResult> {
    ...
    try {
      ...
      const configResult = await this.runStep('config', async () => {
        ...
        // Persist auto_advance via ecl-tools (validates & updates state)
  ...
  private async buildResearchPrompt(
    ...
    const agentDef = await this.readAgentFile('ecl-project-researcher.md');
  ...
  private async buildSynthesisPrompt(): Promise<string> {
    const agentDef = await this.readAgentFile('ecl-research-synthesizer.md');
  ...
  private async buildRoadmapPrompt(): Promise<string> {
    const agentDef = await this.readAgentFile('ecl-roadmapper.md');
  ...
  /**
   * Read a file from the eCL templates directory.
   * Tries sdk/prompts/{relativePath} first (headless versions), then
   * falls back to eCL-1 originals (~/.claude/evolv-coder-lite/).
   */
  private async readGSDFile(relativePath: string): Promise<string> {
    // Try installed eCL first (complete, up-to-date versions)
    const fullPath = join(ECL_TEMPLATES_DIR, '..', relativePath);
    try {
  ...
  private async readAgentFile(filename: string): Promise<string> {
    ...
    const fullPath = join(ECL_AGENTS_DIR, filename);
    try {
      return await readFile(fullPath, 'utf-8');
    } catch {
      // Not installed, fall through to SDK bundled copies
