 ...
 *
 * Multiple eCL agents (ecl-ui-checker, ecl-planner) entered unbounded Read
 ...
 * ecl-pattern-mapper was fixed in #2312; this covers the remaining agents.
 *
...
describe('bug #2346: agent read loop guards', () => {
  ...
  describe('ecl-ui-checker', () => {
    const agentPath = path.join(AGENTS_DIR, 'ecl-ui-checker.md');
    const content = fs.readFileSync(agentPath, 'utf-8');

    test('agent file exists', () => {
      assert.ok(fs.existsSync(agentPath), 'agents/ecl-ui-checker.md must exist');
    });

    test('has <critical_rules> block', () => {
      assert.ok(
        content.includes('<critical_rules>'),
        'ecl-ui-checker.md must have a <critical_rules> block to prevent unbounded read loops (#2346)'
      );
    });
  ...
  describe('ecl-planner', () => {
    const agentPath = path.join(AGENTS_DIR, 'ecl-planner.md');
    const content = fs.readFileSync(agentPath, 'utf-8');

    test('agent file exists', () => {
      assert.ok(fs.existsSync(agentPath), 'agents/ecl-planner.md must exist');
    });

    test('has <critical_rules> block', () => {
      assert.ok(
        content.includes('<critical_rules>'),
        'ecl-planner.md must have a <critical_rules> block to prevent unbounded read loops (#2346)'
      );
    });
