...
function makeTmpDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-hub-logger-test-'));
}
...
describe('Hub + createDefaultLogger — end-to-end', () => {
  ...
  test('audit file written when ECL_AUDIT=1', () => {
    ...
    assert.ok(fs.existsSync(auditPath), 'audit file must exist after dispatch with ECL_AUDIT=1');
