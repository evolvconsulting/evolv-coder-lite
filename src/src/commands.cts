...
/**
 * #488 — Re-sync effort: frontmatter in all installed ecl-*.md agent files to
 ...
 * The runtime resolver (loadConfig) does not merge ~/.ecl/defaults.json when a project
...
function cmdEffortSync(cwd: string, raw: boolean, opts?: { dryRun?: boolean; configDir?: string; runtime?: string }): void {
  ...
  // Use install-time resolvers: they merge ~/.ecl/defaults.json with project config,
  ...
  const files = fs.readdirSync(agentsDir).filter(f => {
    if (!f.startsWith('ecl-') || !f.endsWith('.md')) return false;
...
async function cmdWebsearch(query: string | undefined, options: WebsearchOptions, raw: boolean): Promise<void> {
  ...
  const rawTimeout = parseInt(process.env['ECL_WEBSEARCH_TIMEOUT_MS'] as string, 10);
