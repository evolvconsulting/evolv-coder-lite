...
```
eCL > Discovered {N} docs, which exceeds the v1 cap of 50.
      Use --manifest to narrow the set to ≤ 50 files, or run
      /ecl:ingest-docs again with a narrower <path>.
```
...
For each discovered doc, spawn `ecl-doc-classifier` in parallel. In Claude Code, issue all Task calls in a single message with multiple tool uses so the harness runs them concurrently. For Copilot / sequential runtimes, fall back to sequential dispatch.
...
- `<required_reading>` — `agents/ecl-doc-classifier.md` (the agent definition itself)
...
```
Agent({
  subagent_type: "ecl-doc-synthesizer",
  prompt: "
    ...
    <required_reading>
    - agents/ecl-doc-synthesizer.md
    - evolv-coder-lite/references/doc-conflict-engine.md
    </required_reading>
  "
})
```
...
```
eCL > BLOCKED: {N} blockers must be resolved before ingest can proceed.
```
...
Proceed to routing silently, or optionally display `eCL > No conflicts. Auto-resolved: {N}.`
...
- Next step: `/ecl:plan-phase 1` (new mode) or `/ecl:plan-phase N` (merge, pointing at the first newly-added phase)
