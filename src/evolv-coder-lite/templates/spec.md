...
- `ecl-planner` — reads locked requirements to constrain plan scope
- `ecl-verifier` — uses acceptance criteria as explicit pass/fail checks
...
*Phase: [XX-name]*
*Spec created: [date]*
*Next step: /ecl:discuss-phase [X] — implementation decisions (how to build what's specified above)*
```
...
*Next step: /ecl:discuss-phase 3 — implementation decisions (card layout, loading skeleton, etc.)*
```
...
A `ecl backup` CLI command creates a reproducible database snapshot that can be restored by `ecl restore` (a separate phase).
...
1. **Backup creation**: CLI command executes a full database backup.
   ...
   - Target: `ecl backup` connects to the database (via `DATABASE_URL` env or `--db` flag), runs pg_dump, writes output to `./backups/YYYY-MM-DD_HH-MM-SS.dump`
   - Acceptance: Running `ecl backup` on a test database creates a `.dump` file; running `pg_restore` on that file recreates the database without error
...
- [ ] `ecl backup` creates a `.dump` file in `./backups/YYYY-MM-DD_HH-MM-SS.dump` format
- [ ] `ecl backup` uses `DATABASE_URL` env var or `--db` flag for connection
...
*Spec created: 2025-01-20*
*Next step: /ecl:discuss-phase 2 — implementation decisions (progress reporting, flag design, etc.)*
```
