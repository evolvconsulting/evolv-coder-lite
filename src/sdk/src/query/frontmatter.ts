 ...
 *
 * Ported from evolv-coder-lite/bin/lib/frontmatter.cjs and state.cjs.
 ...
 * // { data: { ecl_state_version: '1.0', milestone: 'v3.0', ... } }
 * ```
 */
...
/**
 * Parse YAML frontmatter body (between `---` fences) using the eCL stack parser.
...
/**
 * First leading frontmatter block only — parity with `evolv-coder-lite/bin/lib/frontmatter.cjs`
 * `extractFrontmatter` (used by `summary-extract` and `history-digest` in ecl-tools.cjs).
 */
 ...
 *
 * Port of `parseMustHavesBlock` from `evolv-coder-lite/bin/lib/frontmatter.cjs` lines 195-301.
