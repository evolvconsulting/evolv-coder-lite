 ...
 *   1. ECL_AGENTS_DIR (explicit override)
 ...
 * repo-local path is never probed when ECL_AGENTS_DIR is unset and the global
 ...
 *   when ECL_AGENTS_DIR is unset AND the global runtime agents dir is absent/empty,
 *   AND a repo-local .claude/agents directory exists.
 *
 * Precedence (post-fix):
 *   ECL_AGENTS_DIR > global runtime dir (non-empty) > <projectDir>/.claude/agents
 */
...
describe('#3751: resolveAgentsDir() repo-local fallback — structural contracts', () => {
  ...
  // ─── Contract 4: ECL_AGENTS_DIR still short-circuits both paths ─────────
  test('resolveAgentsDir still checks ECL_AGENTS_DIR first', () => {
    const fnStart = helpersTs.indexOf('export function resolveAgentsDir');
    const fnEnd = helpersTs.indexOf('\nexport function', fnStart + 1);
    const fnBody = helpersTs.slice(fnStart, fnEnd === -1 ? undefined : fnEnd);
    assert.ok(
      fnBody.includes('ECL_AGENTS_DIR'),
      'resolveAgentsDir must still check ECL_AGENTS_DIR as the first override (#3751)',
    );
    const envIdx = fnBody.indexOf('ECL_AGENTS_DIR');
    const globalIdx = fnBody.indexOf('getRuntimeConfigDir');
    assert.ok(
      envIdx < globalIdx,
      'ECL_AGENTS_DIR check must appear before getRuntimeConfigDir in resolveAgentsDir (#3751)',
    );
  });
...
describe('#3751: resolveAgentsDir() repo-local fallback — runtime behaviour', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-3751-'));
    savedEnv = {
      ECL_AGENTS_DIR: process.env.ECL_AGENTS_DIR,
      CLAUDE_CONFIG_DIR: process.env.CLAUDE_CONFIG_DIR,
      HOME: process.env.HOME,
    };
    // Clear explicit overrides so we exercise the fallback path
    delete process.env.ECL_AGENTS_DIR;
  });

  afterEach(() => {
    fs.rmSync(tmpDir, { recursive: true, force: true });
    // Restore env
    if (savedEnv.ECL_AGENTS_DIR !== undefined) {
      process.env.ECL_AGENTS_DIR = savedEnv.ECL_AGENTS_DIR;
    } else {
      delete process.env.ECL_AGENTS_DIR;
    }
   ...
   */
  test('resolveAgentsDir returns repo-local .claude/agents when global dir is absent and ECL_AGENTS_DIR unset', () => {
    ...
    // Set up repo-local .claude/agents with a eCL agent definition
    const repoRoot = path.join(tmpDir, 'repo');
    const repoLocalAgentsDir = path.join(repoRoot, '.claude', 'agents');
    fs.mkdirSync(repoLocalAgentsDir, { recursive: true });
    fs.writeFileSync(
      path.join(repoLocalAgentsDir, 'ecl-project-researcher.md'),
    ...
    // Since sdk/dist is not pre-built in CI, we use the ecl-tools integration path.
    ...
    const result = runGsdTools(
      ...
      repoRoot,
      {
        ECL_AGENTS_DIR: '',         // explicitly empty — must not win over repo-local
  ...
  /**
   * Counter-test: no .claude/agents anywhere + no ECL_AGENTS_DIR → returns global path,
   * does not throw.
   */
...
describe('#3799: resolveAgentsDir() local-first resolution — structural contracts', () => {
  ...
  // ─── Contract B: ECL_AGENTS_DIR still short-circuits both ───────────────
  test('ECL_AGENTS_DIR still appears before both local and global checks (#3799)', () => {
    const fnStart = helpersTs.indexOf('export function resolveAgentsDir');
    const fnEnd = helpersTs.indexOf('\nexport function', fnStart + 1);
    const fnBody = helpersTs.slice(fnStart, fnEnd === -1 ? undefined : fnEnd);
    ...
    assert.ok(envIdx !== -1, 'ECL_AGENTS_DIR check must still be present (#3799)');
    assert.ok(envIdx < localIdx || localIdx === -1, 'ECL_AGENTS_DIR must appear before local path check (#3799)');
    assert.ok(envIdx < globalIdx, 'ECL_AGENTS_DIR must appear before global path check (#3799)');
  });
});
...
describe('#3799: resolveAgentsDir() local-first resolution — runtime behaviour', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-3799-'));
    savedEnv = {
      ECL_AGENTS_DIR: process.env.ECL_AGENTS_DIR,
      CLAUDE_CONFIG_DIR: process.env.CLAUDE_CONFIG_DIR,
    };
  ...
  afterEach(() => {
    fs.rmSync(tmpDir, { recursive: true, force: true });
    if (savedEnv.ECL_AGENTS_DIR !== undefined) process.env.ECL_AGENTS_DIR = savedEnv.ECL_AGENTS_DIR;
    else delete process.env.ECL_AGENTS_DIR;
  ...
  test('reports agents_installed: true when local .claude/agents has agents and global dir is empty (#3799)', () => {
    ...
    // Set up project-local .claude/agents with a eCL agent definition
    const repoRoot = path.join(tmpDir, 'repo');
    const repoLocalAgentsDir = path.join(repoRoot, '.claude', 'agents');
    fs.mkdirSync(repoLocalAgentsDir, { recursive: true });
    fs.writeFileSync(
    ...
    // ecl-tools query is not available.
  });
  ...
  test('global-only install: resolves to global dir when no project-local .claude/agents (#3799 no-regression)', () => {
    ...
    fs.mkdirSync(fakeGlobalAgents, { recursive: true });
    fs.writeFileSync(
      path.join(fakeGlobalAgents, 'ecl-project-researcher.md'),
      '---\nname: ecl-project-researcher\n---\n',
    );
    process.env.CLAUDE_CONFIG_DIR = fakeGlobalConfig;
    ...
    // Structural contracts provide the authoritative gate when ecl-tools is unavailable.
  });
  ...
  test('local wins over populated global when both .claude/agents dirs exist (#3799)', () => {
    ...
    const fakeGlobalAgents = path.join(fakeGlobalConfig, 'agents');
    fs.mkdirSync(fakeGlobalAgents, { recursive: true });
    fs.writeFileSync(path.join(fakeGlobalAgents, 'ecl-project-researcher.md'), '# global agent\n');
    process.env.CLAUDE_CONFIG_DIR = fakeGlobalConfig;

    const repoRoot = path.join(tmpDir, 'repo-both');
    const repoLocalAgentsDir = path.join(repoRoot, '.claude', 'agents');
    fs.mkdirSync(repoLocalAgentsDir, { recursive: true });
    fs.writeFileSync(path.join(repoLocalAgentsDir, 'ecl-project-researcher.md'), '# local agent\n');
