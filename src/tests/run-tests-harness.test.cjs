...
// runs the harness against it via ECL_TEST_DIR.
...
function runHarness(testDir, args = [], extraEnv = {}) {
  ...
  const env = { ...process.env, ECL_TEST_DIR: testDir, ...extraEnv };
...
describe('run-tests.cjs harness (issue #3597)', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('ecl-3597-harness-');
  });
  ...
  describe('empty-suite behavior', () => {
    ...
    test('ECL_ALLOW_EMPTY_SUITE=1 downgrades empty suite to a warning and exits 0', () => {
      seed(tmpDir, ['a.test.cjs']);
      const r = runHarness(tmpDir, ['--suite', 'security'], { ECL_ALLOW_EMPTY_SUITE: '1' });
      assert.strictEqual(r.status, 0);
...
`;
  ...
  describe('env hermeticity', () => {
    // Regression guard for the two `delete process.env.ECL_PROJECT/ECL_WORKSTREAM`
    ...
    test('harness strips ECL_PROJECT and ECL_WORKSTREAM before running child tests', () => {
      // Write a fixture that asserts both vars are absent in the child process env.
      const FIXTURE = `'use strict';
const { test } = require('node:test');
const assert = require('node:assert/strict');
test('ambient eCL workstream vars are stripped by the runner', () => {
  assert.strictEqual(process.env.ECL_PROJECT, undefined);
...
`;
      ...
      const r = runHarness(tmpDir, [], {
        ECL_PROJECT: 'ambient-proj',
        ECL_WORKSTREAM: 'ambient-ws',
      });
