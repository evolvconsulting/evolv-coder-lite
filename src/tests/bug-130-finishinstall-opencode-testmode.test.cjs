 ...
 * violating the ECL_TEST_MODE side-effect-free contract.
 *
 * configureOpencodePermissions does fs.mkdirSync + fs.writeFileSync, which
 * must NOT run under ECL_TEST_MODE='1'. This test asserts that the opencode
 * config file (opencode.json) is NOT created when ECL_TEST_MODE is set.
 */
...
const FAKE_HOME = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-130-test-'));
...
describe('Bug #130: finishInstall opencode + ECL_TEST_MODE side-effect guard', () => {
  test('configureOpencodePermissions does NOT write opencode.json under ECL_TEST_MODE', () => {
    ...
    assert.equal(
      fs.existsSync(OPENCODE_CONFIG_FILE),
      false,
      `opencode.json must NOT be created under ECL_TEST_MODE; found at ${OPENCODE_CONFIG_FILE}`,
    );
  });
});
