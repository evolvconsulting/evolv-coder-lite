 ...
 * the correct global skills directory for every eCL-supported runtime.
 *
 ...
 *   hermes  — eCL skills nest under skills/ecl/<skillName>/ (not the flat
...
function getGlobalConfigDir(runtime) {
  ...
  switch (runtime) {
    ...
    // ── Hermes Agent ─────────────────────────────────────────────────────────
    // Note: skills use a nested layout (skills/ecl/<skill>/) — see getGlobalSkillDir().
    case 'hermes':
 ...
 * Hermes: <configDir>/skills/ecl  (nested category layout — #2841)
...
function getGlobalSkillsBase(runtime) {
  ...
  if (runtime === 'hermes') return path.join(configDir, 'skills', 'ecl');
 ...
 *
 * @param {string} runtime
 * @param {string} skillName - e.g. 'ecl-executor'
 * @returns {string|null}
 */
