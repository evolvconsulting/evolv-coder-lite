...
**前提条件：** `.planning/ROADMAP.md` 文件已存在。如果没有，请先运行 `/ecl-new-project`。

---

## 选择讨论模式

eCL Core 提供两种模式。根据对代码库的熟悉程度进行选择。
...
当您接下来运行 `/ecl-plan-phase` 时，规划员会读取 CONTEXT.md 以了解哪些决策已锁定。它不会重新询问此处已回答的问题。研究员会首先读取该文件以了解需要调查的内容。

**如果运行 `/ecl-plan-phase` 时 CONTEXT.md 缺失**，系统将提供两种选择：不使用上下文继续（计划仅使用研究和需求，不包含您的设计偏好），或先运行 `/ecl-discuss-phase`。
