 ...
 *
 * Idempotent transform: for every evolv-coder-lite/workflows/*.md (and subdirs),
 * rewrite all bash/sh/shell fenced blocks to:
 *   1. Strip ALL old resolver forms from every bash block (ECL_TOOLS=,
 *      ECL_SDK=, the if/elif/else/fi resolver, _GSD_SHIM_NAME=, and any
 *      previously-inserted ecl_run preamble).
 *   2. Replace $ECL_SDK tokens with ecl_run (idempotent).
 *   3. Insert the canonical preamble at the TOP of ONLY the FIRST bash block
 *      (document order) that contains a ecl_run call. All other bash blocks
 *      keep their ecl_run calls with NO preamble. (Define once per file,
 ...
 * boilerplate (old and new preamble forms) and replace $ECL_SDK tokens.
...
function stripAndReplace(lines, preamble) {
  ...
  // Step 2: Replace $ECL_SDK tokens with ecl_run
 ...
 *      ECL_TOOLS="...ecl-tools.cjs"   (or ...${_GSD_SHIM_NAME})
 *      if [ -f "$ECL_TOOLS" ]; then
 *        ...
 *      fi
 *
 * B) One-liner form:
 *      ECL_TOOLS=...; if [ -f "$ECL_TOOLS" ]; then ECL_SDK=...; elif ...; else ...; fi
 *
 ...
 *
 * F) The ecl_run() function definitions that are part of the preamble
...
function removeResolverLines(lines, preamble) {
  ...
  while (i < lines.length) {
    ...
    // B) One-liner form: ECL_TOOLS=... ; if ... ECL_SDK=... fi
    if (
      /^ECL_TOOLS=.*ecl-tools\.cjs.*;\s*if\s+\[/.test(trimmed) ||
      /^ECL_TOOLS=.*\/\$\{_GSD_SHIM_NAME\}.*;\s*if\s+\[/.test(trimmed)
    ) {
    ...
    // A) Multi-line form: starts with ECL_TOOLS=...ecl-tools.cjs or ECL_TOOLS=...${_GSD_SHIM_NAME}
    if (
      /^ECL_TOOLS=.*ecl-tools\.cjs"$/.test(trimmed) ||
      /^ECL_TOOLS=.*\/\$\{_GSD_SHIM_NAME\}"$/.test(trimmed)
    ) {
      ...
      if (/^if\s+\[\s+-f\s+"\$ECL_TOOLS"\s*\]/.test(ahead)) {
        // Consume until matching `fi`
        i += 2; // skip ECL_TOOLS= and `if [...]`
        let depth = 1;
        ...
        // Just skip the ECL_TOOLS= line
        i++;
        continue;
      }
    }

    // D) Bare ECL_SDK= line (not inside an if block, defensive)
    if (/^ECL_SDK=/.test(trimmed) && !/ecl_run/.test(trimmed)) {
      i++;
      continue;
    }
    ...
    // and ecl_run() function definition lines.
...
function isSdkComment(line) {
  ...
  return /^\s*#\s*SDK resolution/.test(line) || /^\s*#.*prefer local.*ecl-tools/.test(line);
}
...
function isCanonicalPreambleComment(line) {
  return /^\s*#\s*Runtime launcher:.*prefer local ecl-tools\.cjs.*installed ecl-tools on PATH/.test(line);
}

/**
 * Replace all $ECL_SDK tokens with ecl_run in a line.
 * Handles: $ECL_SDK, ${ECL_SDK} forms.
 */
function replaceGsdSdk(line) {
  return line.replace(/\$\{?ECL_SDK\}?(?=\s|$|;|"|'|\))/g, 'ecl_run');
}
 ...
 * 2. Find the FIRST block (document order) that has a ecl_run call.
...
function transformFile(content, preamble) {
  ...
  let changed = false;
  let firstGsdRunBlockIdx = -1; // index into shellBlockRanges of the first ecl_run block (after strip)

  // First: strip + replace in all blocks, find first ecl_run block
  ...
  // Find the first block that has a ecl_run call (after stripping)
  for (let bi = 0; bi < strippedBlocks.length; bi++) {
    if (strippedBlocks[bi].some((l) => /\becl_run\b/.test(l))) {
      firstGsdRunBlockIdx = bi;
      break;
    }
  }

  // Insert preamble into the first ecl_run block only
