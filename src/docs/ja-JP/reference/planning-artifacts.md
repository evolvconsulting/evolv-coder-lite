...
`.planning/` ディレクトリはプロジェクトの eCL Core 共有メモリです。すべてのワークフローはここから読み取り、書き込み、意思決定の監査可能な証跡を残します。このページではすべてのファイル、その目的、どのコマンドが生成・使用するかをマッピングします。[ドキュメントインデックス](../../README.md) も参照してください。
...
| **生成元** | `/ecl-new-project`（初回作成）; 意思決定が検証されると `/ecl-complete-milestone` によって更新されます。 |
| **参照先** | すべてのプランニングワークフロー; `ecl-phase-researcher`、`ecl-planner`（コンテキスト）; `discuss-phase`（過去の意思決定）; `ecl-plan-checker`（プロジェクト制約）。 |
...
| **生成元** | `/ecl-new-project`（初回作成）; `/ecl-phase --insert` および `/ecl-complete-milestone` によって更新されます。 |
| **参照先** | `/ecl-discuss-phase`、`/ecl-plan-phase`、`/ecl-execute-phase`; フェーズ情報を必要とするすべてのオーケストレーションコマンド; `ecl-planner`、`ecl-plan-checker`、`ecl-phase-researcher`。 |
...
| **生成元** | `/ecl-new-project`（初回作成）; `execute-phase` によって要件が完了済みとしてマークされます。 |
| **参照先** | `ecl-planner`（プランはすべてのフェーズ要件 ID に対処しなければならない）; `ecl-plan-checker` ディメンション1（要件カバレッジ）; `discuss-phase`（過去の要件）。 |
...
| **生成元** | `/ecl-new-project`（初回作成）; すべてのフェーズワークフロー、`/ecl-pause-work`、`/ecl-resume-work` によって継続的に更新されます。 |
| **参照先** | すべてのオーケストレーションワークフロー; `/ecl-progress`; `/ecl-quick` 経由のアドホックタスク実行; `ecl-planner` および `ecl-phase-researcher`（プロジェクトの意思決定）。 |
...
| **生成元** | `/ecl-new-project`（初回作成）; `/ecl-settings`（インタラクティブ編集）。 |
| **参照先** | すべてのワークフローとサブエージェント — `ecl-tools query config-get` 経由で初期化時に読み込まれます。 |
...
| **生成元** | `/ecl-complete-milestone`。 |
| **参照先** | `/ecl-audit-milestone`; 人間によるレビュー。 |
...
| **生成元** | `/ecl-discuss-phase`（インタラクティブなディスカッションまたは PRD/ADR エクスプレスパス）。 |
| **参照先** | `ecl-phase-researcher`（調査すべき内容）; `ecl-planner`（ロックされた意思決定）; `ecl-plan-checker` ディメンション7（コンテキスト準拠）。 |
...
| **生成元** | `/ecl-discuss-phase`（`git_commit` ステップ）。 |
| **参照先** | 人間によるレビュー; 振り返り。 |
...
| **生成元** | `/ecl-plan-phase`（`ecl-phase-researcher` エージェント経由）。 |
| **参照先** | `ecl-planner`（プランニングインプット）; `ecl-plan-checker` ディメンション7c（ティア準拠）、ディメンション8（Nyquist）、ディメンション11（リサーチ解決）; `ecl-pattern-mapper`（ファイルリストのソース）。 |
...
| **生成元** | `/ecl-plan-phase`（ステップ5.5、`workflow.nyquist_validation` が有効で RESEARCH.md に Validation Architecture セクションが含まれる場合）。 |
| **参照先** | `ecl-plan-checker` ディメンション8（チェック8e ゲート — Nyquist チェック進行前に存在しなければならない）; `ecl-verifier`。 |

### `<NN>-PATTERNS.md`

| | |
|---|---|
| **用途** | `ecl-pattern-mapper` によって生成されたコードベースアナログマップ。このフェーズで作成または変更される各ファイルに対して、最も近い既存のアナログを特定し、ファイルの役割とデータフローを分類し、具体的なコード抜粋を抽出します。プランナーを一貫したパターンに向けます。 |
| **生成元** | `/ecl-plan-phase`（`ecl-pattern-mapper` エージェント経由、オプション; `workflow.pattern_mapper: false` の場合はスキップ）。 |
| **参照先** | `ecl-planner`（パターンガイダンス）; `ecl-plan-checker` ディメンション12（パターン準拠）。 |
...
| **生成元** | `/ecl-plan-phase`（`ecl-planner` エージェント経由）。プランごとに1ファイル — 例: `03-02-PLAN.md` はフェーズ3、プラン2。 |
| **参照先** | `/ecl-execute-phase`（エグゼキューターエージェントがプランを読んでタスクを実行）; `ecl-plan-checker`（実行前の品質レビュー）; `ecl-verifier`（実行後の検証のために `must_haves` を読む）。 |
...
| **参照先** | `/ecl-progress`（フェーズステータス）; `ecl-planner`（後続のプランが以前のプラン出力への真の依存を持つ場合）; `milestone-summary`。 |
...
| **生成元** | `/ecl-verify-work`（または `/ecl-execute-phase` 内の検証ステップ）。 |
| **参照先** | `plan-phase` クローズドフェーズゲート（`status: passed` の VERIFICATION.md はフェーズを `Complete` としてマークし、`--force` なしの再プランニングをブロックする）; `/ecl-progress`; 人間によるレビュー。 |
...
| **生成元** | `/ecl-audit-uat`（インタラクティブな UAT セッション）。 |
| **参照先** | `/ecl-audit-uat`（以前の UAT セッションの再開）。 |
