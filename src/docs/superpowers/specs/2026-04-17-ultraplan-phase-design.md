# Design: /ecl-ultraplan-phase [BETA]
...
A standalone `/ecl-ultraplan-phase` command that offloads eCL's research+plan phase to Claude Code's ultraplan cloud infrastructure. The plan drafts remotely while the terminal stays free, is reviewed in a rich browser UI with inline comments, then imports back into eCL via the existing `/ecl-import --from` workflow.

This is a **beta of a beta**: ultraplan itself is in research preview, so this command is intentionally isolated from the core `/ecl-plan-phase` pipeline to prevent breakage if ultraplan changes.
...
**In scope:**
- New `commands/ecl/ultraplan-phase.md` command
- New `evolv-coder-lite/workflows/ultraplan-phase.md` workflow
- Runtime gate: Claude Code only (checks `$CLAUDE_CODE_VERSION`)
- Builds structured ultraplan prompt from eCL phase context
- Return path via existing `/ecl-import --from <file>` (no new import logic)

**Out of scope (future):**
- Parallel next-phase planning during `/ecl-execute-phase`
...
```text
/ecl-ultraplan-phase [phase]
        │
        ├─ Runtime gate (CLAUDE_CODE_VERSION check)
        ├─ ecl-sdk query init.plan-phase → phase context
        ...
        └─ /ultraplan <prompt>
                ...
                │
                /ecl-import --from <saved file path>
                        │
                        ├─ Conflict detection
                        ├─ eCL format conversion
                        ├─ ecl-plan-checker validation
                        ├─ ROADMAP.md update
                        └─ Commit
```
...
## Command File (`commands/ecl/ultraplan-phase.md`)

Frontmatter:
- `name: ecl:ultraplan-phase`
- `description:` includes `[BETA]` marker
- `argument-hint: [phase-number]`
- `allowed-tools:` Read, Bash, Glob, Grep
- References: `@~/.claude/evolv-coder-lite/workflows/ultraplan-phase.md`, ui-brand
...
### 1. Banner
Display eCL `► ULTRAPLAN PHASE [BETA]` banner.
...
⚠ /ecl-ultraplan-phase requires Claude Code.
  ...
  Use /ecl-plan-phase for local planning.
```
...
If no `.planning/` exists: error — run `/ecl-new-project` first.
...
- Output format instruction: produce a eCL PLAN.md with standard frontmatter fields
...
When ◆ ultraplan ready:
  ...
  5. Run: /ecl-import --from <the file path Claude prints>
```
...
No new code needed. The user runs `/ecl-import --from <path>` after ultraplan saves the file. That workflow handles everything: conflict detection, eCL format conversion, plan-checker, ROADMAP update, commit.
...
- Workflow has init step (ecl-sdk query)
- Workflow builds ultraplan prompt with phase context
- Workflow triggers `/ultraplan`
- Workflow has return-path instructions (Cancel path, `/ecl-import --from`)
- Workflow does NOT directly implement plan writing (delegates to `/ecl-import`)
