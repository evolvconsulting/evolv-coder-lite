...
beforeEach(async () => {
  tmpDir = await mkdtemp(join(tmpdir(), 'ecl-stubs-'));
...
describe('agentSkills', () => {
  it('returns empty string when agent_skills config is missing', async () => {
    const result = await agentSkills(['ecl-executor'], tmpDir);
...
describe('docsInit', () => {
  it('returns docs context matching ecl-tools docs-init', async () => {
