...
// commands/ecl/*.md files ARE what the runtime loads — testing their
...
const NAMESPACE_SKILLS = [
  { file: 'ns-workflow.md', name: 'ecl-workflow' },
  { file: 'ns-project.md',  name: 'ecl-project' },
  { file: 'ns-review.md',   name: 'ecl-quality' },
  { file: 'ns-context.md',  name: 'ecl-context' },
  { file: 'ns-manage.md',   name: 'ecl-manage' },
  { file: 'ns-ideate.md',   name: 'ecl-ideate' },
];
...
// skills, e.g. `ecl-map-codebase --fast` for the former `ecl-scan`).
const FLAG_FORM_PARENTS = new Set([
  'ecl-code-review',     // --fix absorbs former ecl-code-review-fix
  'ecl-map-codebase',    // --fast absorbs scan, --query absorbs intel
]);
...
// ── Context guard contract on ecl-health ──────────────────────────────
...
describe('ecl-health --context flag is wired into command + workflow', () => {
  const HEALTH_CMD = path.join(COMMANDS_DIR, 'health.md');
  const HEALTH_WORKFLOW = path.join(__dirname, '..', 'evolv-coder-lite', 'workflows', 'health.md');

  test('commands/ecl/health.md argument-hint advertises --context', () => {
  ...
  test('commands/ecl/health.md body documents the three-state utilization table', () => {
    const raw = fs.readFileSync(HEALTH_CMD, 'utf-8');
  ...
  test('evolv-coder-lite/workflows/health.md has a context_check step', () => {
  ...
  test('workflow context_check invokes ecl-sdk query validate.context', () => {
    const raw = fs.readFileSync(HEALTH_WORKFLOW, 'utf-8');
...
describe('namespace nesting completeness (#69)', () => {
  ...
  test('every router requires entry resolves to a real concrete skill file', () => {
    ...
    assert.deepStrictEqual(
      bad,
      [],
      `Router requires entries with no matching commands/ecl/<stem>.md: ${bad.join(', ')}`,
    );
  });
  ...
  test("each router's routing-table rows reference only its own required sub-skills (plus flag variants)", () => {
    ...
    for (const [routerStem, children] of routerRequires) {
      ...
      // Extract ecl-<stem> tokens from table data rows (last cell), strip flags
      for (const line of fm._body.split('\n')) {
        if (!line.startsWith('|') || /^\|[\s\-:|]+\|?\s*$/.test(line)) continue;
        const cells = line.split('|').map((c) => c.trim()).filter(Boolean);
        if (cells.length < 2) continue;
        const lastCell = cells[cells.length - 1];
        for (const match of lastCell.matchAll(/\becl-([a-z][a-z0-9-]*)/g)) {
          const stem = match[1];
          if (!childSet.has(stem)) {
            bad.push(`${routerStem}: body table references ecl-${stem} but it's not in requires`);
          }
        }
      }
    }
...
describe('Namespace router targets resolve to surviving skills', () => {
  ...
  for (const f of fs.readdirSync(COMMANDS_DIR)) {
    ...
    if (base === 'extract_learnings') surviving.add('ecl-extract-learnings');
  }

  for (const { file } of NAMESPACE_SKILLS) {
    test(`${file} — every routing target resolves`, () => {
      const fm = readNamespaceFile(file);
      // Extract every ecl-<name> token that appears in a table-row right column.
      // Strip flag suffixes (`ecl-foo --bar` → `ecl-foo`) before resolving.
      ...
      assert.ok(targets.size > 0, `${file} routing table must reference at least one ecl-* target`);
      ...
      assert.deepStrictEqual(
        unresolved,
        [],
        `${file} routes to skills that don't exist in commands/ecl/: ${unresolved.join(', ')}`,
      );
    });
  }
});
