...
} = require('../evolv-coder-lite/bin/lib/configuration.generated.cjs');
...
describe('auto-update', () => {
  ...
  describe('hook — HEAD-advancing command matchers',
    ...
    for (const cmd of [
      ...
      // #3653 — `ecl-sdk query commit` invokes git via spawnSync('git', [...]),
      ...
      'ecl-sdk query commit "docs: probe" --files .planning/STATE.md',
      'npx ecl-sdk query commit "docs: probe" --files .planning/STATE.md',
    ]) {
    ...
    test('does NOT dispatch on SDK commit-to-subrepo prefix collision', (t) => {
      ...
      const r = runHook(
        tmpDir,
        {
          tool_name: 'Bash',
          tool_input: {
            command: 'ecl-sdk query commit-to-subrepo "msg" --files packages/foo',
          },
        },
        { pathPrepend: mockBin },
      );
    ...
    // `ecl-sdk query` verbs (phase.complete, roadmap.update-plan-progress,
    // state.begin-phase) mutate .md files but do NOT advance HEAD; matching
    // them would cause a spurious rebuild per state mutation.
    for (const cmd of [
      'ecl-sdk query phase.complete 109',
      'ecl-sdk query roadmap.update-plan-progress 109 W001',
      'ecl-sdk query state.begin-phase 110',
    ]) {
