...
/**
 * Bug #3657: verify-reapply-patches false-fails when ecl-pristine/ snapshot is
 * newer than backup-meta baseline.
 *
 * Root cause: the verifier computes user-added lines as
 *   diff(backup, pristine_on_disk)
 * but pristine_on_disk is from a LATER eCL version than the one captured in
...
const SCRIPT = path.join(ROOT, 'evolv-coder-lite', 'bin', 'verify-reapply-patches.cjs');
...
before(() => {
  tmpRoot    = fs.mkdtempSync(path.join(os.tmpdir(), 'ecl-3657-'));
...
describe('Bug #3657: pristine-drift does not produce false FAIL_USER_LINES_MISSING', () => {
  ...
  test('exits 0 with reason=OK_PRISTINE_DRIFT_DETECTED when on-disk pristine hash does not match recorded hash', () => {
    ...
    const FILE = 'agents/ecl-executor.md';
    ...
    // The installer later refreshed ecl-pristine/ to v_new.
  ...
  test('handles mixed drift + real-failure across multiple files independently', () => {
    ...
    const DRIFT_FILE  = 'agents/ecl-executor.md';
  ...
  test('Finding 1: drifted count and drifted_files aggregate correctly across multiple drifted files', () => {
    ...
    const FILE_A = 'agents/ecl-executor.md';
    ...
    const newPristineA = 'refreshed upstream content for file A in the newer eCL snapshot\n';
    const oldPristineB = 'old pristine content for file B that was captured at backup time\n';
    const newPristineB = 'refreshed upstream content for file B in the newer eCL snapshot\n';
  ...
  test('Finding 2: workflow Step 5a source contains drift-check section for DRIFTED_COUNT gate', () => {
    const workflowPath = path.join(ROOT, 'evolv-coder-lite', 'workflows', 'reapply-patches.md');
...
describe('Bug #934: OK_NO_BASELINE when recordedHash present but pristine file absent', () => {
   ...
   * the ecl-pristine/ snapshot is absent from disk (the installer's
  ...
  test('exits 0 with reason=OK_NO_BASELINE when recordedHash present but pristine absent', () => {
    ...
    const FILE = 'evolv-coder-lite/workflows/execute-phase.md';
    ...
    const backupContent =
      ...
      'another upstream line removed upstream between evolv-coder-lite releases here\n' +
    ...
    const installedContent =
      'brand-new upstream line that replaced the old content in evolv-coder-lite 1.4.2\n' +
      'model: sonnet in frontmatter — this is the real user customisation line\n';

    // backup-meta.json records a hash (modern installer) but ecl-pristine/ is absent.
  ...
  test('falls through to over-broad mode when pristine absent AND no recordedHash', () => {
    ...
    const FILE = 'evolv-coder-lite/workflows/plan-phase.md';
