/**
 * Tests for evolv-coder-lite/bin/lib/intel.cjs
 *
 * Covers: query, status, diff, validate, snapshot, patch-meta,
 * extract-exports, enabled/disabled gating, and CLI routing via ecl-tools.
 */
...
const {
  ...
  INTEL_FILES,
} = require('../evolv-coder-lite/bin/lib/intel.cjs');
...
// .ecl-surface.json (no disabled clusters) so intel is surfaced deterministically.
...
function makeSurfacedConfigDir() {
  const dir = createTempDir('ecl-intel-surface-cfg-');
  fs.writeFileSync(
...
describe('intel tri-state gate hermetic regression (isCapabilityActive cutover)', () => {
  ...
  beforeEach(() => {
    tmpConfigDir = createTempDir('ecl-intel-tristate-cfg-');
    tmpProjectDir = createTempProject('ecl-intel-tristate-proj-');
    ...
    // Empty CLAUDE_CONFIG_DIR (no .ecl-surface.json) → defaults to 'full' profile.
...
// ─── CLI routing via ecl-tools ──────────────────────────────────────────────

describe('ecl-tools intel subcommands', () => {
  let tmpDir;
  let surfacedConfigDir;
  let savedEnv;
...
describe('#1000 regression: ecl-intel-updater emits canonical intel filenames', () => {
  // allow-test-rule: source-text-is-the-product — agents/ecl-intel-updater.md IS the
  ...
  const agentPromptPath = path.join(__dirname, '..', 'agents', 'ecl-intel-updater.md');
  ...
  test('references every canonical INTEL_FILES name', () => {
    for (const filename of Object.values(INTEL_FILES)) {
      assert.ok(
        ...
        `ecl-intel-updater.md must instruct writing the canonical intel file "${filename}" (from INTEL_FILES) that the ecl-tools intel CLI reads; it was missing.`,
      );
    }
  });
  ...
  test('does not reference orphaned short filenames the CLI never reads', () => {
    ...
    for (const shortName of forbidden) {
      ...
      assert.strictEqual(
        ...
        0,
        `ecl-intel-updater.md must not reference orphaned short name "${shortName}" the intel CLI never reads. Offending line(s):\n${offendingLines.join('\n')}`,
      );
    }
  });
});
