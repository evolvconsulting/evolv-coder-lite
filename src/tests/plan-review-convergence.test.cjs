/**
 * Tests for ecl:plan-review-convergence command (#2306)
 *
...
const PLAN_PHASE_PATH = path.join(__dirname, '..', 'evolv-coder-lite', 'workflows', 'plan-phase.md');
const PLANNER_REVIEWS_PATH = path.join(__dirname, '..', 'evolv-coder-lite', 'references', 'planner-reviews.md');
const PLAN_CHECKER_PATH = path.join(__dirname, '..', 'agents', 'ecl-plan-checker.md');
...
describe('plan-review-convergence command source (#2306)', () => {
  ...
  test('command name uses ecl: prefix (installer converts to ecl- on install)', () => {
    assert.ok(
      command.includes('name: ecl:plan-review-convergence'),
      'command name must use ecl: prefix so installer converts it to ecl-plan-review-convergence'
    );
  });
  ...
  test('command references the workflow file via execution_context', () => {
    assert.ok(
      command.includes('@$HOME/.claude/evolv-coder-lite/workflows/plan-review-convergence.md'),
  ...
  test('command declares Skill in allowed-tools (required for inline plan-phase invocations)', () => {
    assert.ok(
      ...
      'Skill must be in allowed-tools — command invokes ecl-plan-phase inline via Skill() at depth 0 (#936 fix)'
    );
  });
...
describe('plan-review-convergence workflow: initialization (#2306)', () => {
  ...
  test('workflow calls ecl-tools.cjs init plan-phase for initialization', () => {
    assert.ok(
      workflow.includes('ecl-tools.cjs') && workflow.includes('init') && workflow.includes('plan-phase'),
      'workflow must initialize via ecl-tools.cjs init plan-phase'
    );
  });
...
describe('plan-review-convergence workflow: config gate (#2306-v2)', () => {
  ...
  test('workflow exits with enable instructions when config key is false', () => {
    ...
    assert.ok(
      workflow.includes('ecl config-set workflow.plan_review_convergence true') ||
...
describe('plan-review-convergence workflow: initial planning gate (#2306)', () => {
  ...
  test('workflow runs ecl-plan-phase when no plans exist', () => {
    assert.ok(
      workflow.includes('ecl-plan-phase'),
      'workflow must invoke ecl-plan-phase when no plans exist'
    );
  });
...
describe('plan-review-convergence workflow: convergence loop (#2306)', () => {
  ...
  test('workflow spawns isolated review agent each cycle', () => {
    assert.ok(
      workflow.includes('ecl-review'),
      'workflow must spawn Agent → ecl-review each cycle'
    );
  });
  ...
  test('review agent spawn forwards --ws via ECL_WS (symmetric with replan agent)', () => {
    // Critical correctness bug: if ECL_WS is not forwarded to the review agent,
    // the review reads from the wrong workspace while replanning reads from the correct one.
    const reviewAgentBlock = workflow.match(/ecl-review['"`,\s][\s\S]{0,300}?ECL_WS/);
    assert.ok(
      reviewAgentBlock ||
      (workflow.includes("'ecl-review'") && workflow.includes('{ECL_WS}') &&
       workflow.indexOf('{ECL_WS}') < workflow.indexOf("'ecl-plan-phase'")),
      'review agent spawn must forward {ECL_WS} — workspace flag must reach the reviewer (#2306-v2 --ws fix)'
    );
  });
  ...
  test('workflow updates STATE.md on convergence', () => {
    assert.ok(
      ...
      'workflow must update STATE.md via ecl-tools.cjs when converged'
    );
  });
  ...
  test('workflow invokes inline replan with --reviews flag', () => {
    assert.ok(
      ...
      'inline replan must pass --reviews so ecl-plan-phase incorporates review feedback'
    );
  });
...
describe('plan-review-convergence config schema registration (#2306-v2)', () => {
  ...
  const { VALID_CONFIG_KEYS } = require('../evolv-coder-lite/bin/lib/config-schema.cjs');
  ...
  test('workflow.plan_review_convergence is registered in config-schema.cjs', () => {
    assert.ok(
      ...
      "workflow.plan_review_convergence must be registered in VALID_CONFIG_KEYS in config-schema.cjs so ecl config-set accepts it (#2306-v2)"
    );
  });
});
...
describe('plan-review-convergence reviews-mode incorporation contract (#724)', () => {
  ...
  test('plan-phase planner prompt says REVIEWS.md is feedback input, not the execution contract', () => {
    assert.ok(
      planPhase.includes('<review_incorporation_contract>') &&
        ...
        planPhase.includes('/ecl:execute-phase primarily consumes PLAN.md'),
  ...
  test('planner reviews reference requires actionable findings to appear in PLAN.md or be deferred there', () => {
    assert.ok(
      plannerReviews.includes('/ecl:execute-phase primarily consumes PLAN.md') &&
  ...
  test('ecl-plan-checker has a Review Incorporation dimension for reviews mode', () => {
    assert.ok(
...
describe('plan-review-convergence local model config schema registration (#2306-local)', () => {
  ...
  test('review.ollama_host is registered in config-schema.cjs', () => {
    assert.ok(
      ...
      "review.ollama_host must be in VALID_CONFIG_KEYS so ecl config-set accepts it"
    );
  });
  ...
  test('review.lm_studio_host is registered in config-schema.cjs', () => {
    assert.ok(
      ...
      "review.lm_studio_host must be in VALID_CONFIG_KEYS so ecl config-set accepts it"
    );
  });
  ...
  test('review.llama_cpp_host is registered in config-schema.cjs', () => {
    assert.ok(
      ...
      "review.llama_cpp_host must be in VALID_CONFIG_KEYS so ecl config-set accepts it"
    );
  });
});
...
// On Claude Code a depth-1 Agent has no Agent tool, so ecl-plan-phase wrapped in
// Agent() cannot spawn ecl-planner / ecl-plan-checker → the replan loop breaks.
...
describe('plan-review-convergence workflow: inline plan-phase dispatch (#936)', () => {
  ...
  test('initial planning does NOT wrap ecl-plan-phase inside Agent() (#936 fix)', () => {
    // Pre-fix: Agent( ... Skill('ecl-plan-phase') ... ) in step 4
    // Post-fix: bare Skill(skill="ecl-plan-phase") at orchestrator level
    const blocks = extractAgentBlocks(workflow);
    const wrapping = blocks.filter((b) =>
      /Skill\(\s*skill=['"]ecl-plan-phase['"]/.test(b.blockText)
    );
    assert.deepStrictEqual(
      wrapping.map((b) => b.blockText.slice(0, 80).replace(/\n/g, '\\n')),
      [],
      'Initial planning must NOT wrap ecl-plan-phase inside Agent() — run it inline so ' +
      'it can spawn ecl-planner/ecl-plan-checker at depth 1. See: bug #936'
    );
  });

  test('replan step does NOT wrap ecl-plan-phase inside Agent() (#936 fix)', () => {
    ...
    assert.deepStrictEqual(
      wrapping.map((b) => b.blockText.slice(0, 80).replace(/\n/g, '\\n')),
      [],
      'Replan step must NOT wrap ecl-plan-phase inside Agent() — the replan loop can ' +
      'never produce a plan on Claude Code when plan-phase is at depth 1. See: bug #936'
    );
  });

  test('workflow calls ecl-plan-phase inline (bare Skill outside Agent block) (#936 fix)', () => {
    // After the fix there must be at least one bare Skill(skill="ecl-plan-phase")
    ...
    assert.ok(
      /Skill\(\s*skill=["']ecl-plan-phase["']/.test(masked),
      'plan-review-convergence must contain at least one bare Skill(skill="ecl-plan-phase") ' +
  ...
  test('success_criteria describes inline plan-phase, not Agent → Skill (#936 fix)', () => {
    ...
    assert.ok(
      !successBlock.includes('via Agent → Skill("ecl-plan-phase")'),
