...
process.env.ECL_TEST_MODE = '1'; // must precede require of bin/install.js
...
const LIB = path.join(ROOT, 'evolv-coder-lite', 'bin', 'lib');
...
describe('issue-57 AC2 — config-mutation dispatch is closed over the explicit registry', () => {
  ...
  test('unknown runtime fails loudly through both strict projections (no silent fallthrough)', () => {
    ...
    assert.throws(
      () => resolveRuntimeArtifactLayout(SENTINEL, path.join(os.tmpdir(), 'ecl-57'), 'global'),
      TypeError,
