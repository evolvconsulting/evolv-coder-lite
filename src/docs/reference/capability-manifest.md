...
> **See also:** [How to develop a capability](../how-to/develop-a-capability.md) · [Capability Command Reference](ecl-capability-command.md)

Each capability is a folder `capabilities/<id>/` (or an overlay root `~/.ecl/capabilities/<id>/` / `.ecl/capabilities/<id>/`) containing one `capability.json` declaration.
...
|---|---|---|---|
| `id` | string (kebab-case) | Yes | Unique identifier; **must equal the folder name**. The prefix `ecl-`, `evolv-coder-lite-`, and `anthropic-` are reserved for first-party use. |
...
| `engines` | object | No | Host-compatibility constraint. Sub-field: `ecl` — semver range string (e.g. `">=1.6.0 <3.0.0"`). Acts as a hard gate at install **and** at load; a mismatch blocks installation and causes the overlay to be skipped with a warning at load time. |
| `compatVersions` | object | No | Graceful-downgrade table mapping `"<capVersion>"` to `"<min ecl version>"`. Only meaningful for sources that enumerate versions (git tags, registry, npm); a bare tarball URL carries one version and simply blocks on incompatibility. |
...
|---|---|---|---|
| Query | `{ "query": "<ecl_run query>" }` | Yes | Deterministic first-party code. |
...
- **`engines.ecl` is a hard gate.** A capability whose `engines.ecl` range does not satisfy the installed eCL version is blocked at install and skipped (with a warning) at load time.
...
- **Reserved namespace.** Capability `id` values beginning with `ecl-`, `evolv-coder-lite-`, or `anthropic-` are reserved; third-party capabilities using these prefixes are rejected.
...
```json
{
  ...
  "agents": ["ecl-ui-checker", "ecl-ui-auditor"],
  "hooks": [],
  "config": {
    "workflow.ui_phase": {
      "type": "boolean",
