...
/**
 * Integration test for `evolv-coder-lite/bin/lib/cjs-sdk-bridge.cjs` — locks the
 * load-success invariant that Phase 5/6 silently violated before this PR.
 *
 * Original bug: the bridge used `require('@evolvconsulting/ecl-sdk')` to load the
...
const BRIDGE_PATH = path.join(__dirname, '..', 'evolv-coder-lite', 'bin', 'lib', 'cjs-sdk-bridge.cjs');
